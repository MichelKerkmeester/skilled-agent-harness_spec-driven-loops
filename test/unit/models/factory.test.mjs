...
test("embedding factory resolves catalog entries and rejects unknown models", () => {
  ...
  const expectedModelClasses = new Map([
    ...
    ["local/potion-multilingual-128m", Model2VecEmbeddingModel],
    ...
    ["ollama/nomic-embed-text-v1.5", OllamaEmbeddingModel],
    ["ollama/mxbai-embed-large", OllamaEmbeddingModel],
    ["ollama/bge-m3", OllamaEmbeddingModel],
  ]);
  ...
  for (const entry of catalogEntries) {
    ...
    assert.equal(
      model.info.endpoint,
      entry.backend === "qwen" || entry.backend === "ollama"
        ? options.endpoint
        : undefined,
    );
  }
