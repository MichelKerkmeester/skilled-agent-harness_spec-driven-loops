...
test("CLI completes index, search, explicit refresh, status, and rg workflows", async (t) => {
  ...
  // Detecting staleness costs a full workspace scan, so a Direct query pays for
  // it only under --refresh wait and reports no staleness of its own. The
  // `zg status` call below is what still surfaces the drift.
  assert.doesNotMatch(stale.stderr, /status: possibly_stale/);
  assert.doesNotMatch(stale.stderr, /results: served_from_current_index/);
  await assert.rejects(
  ...
  // Direct mode treats --refresh background as off, so it skips the scan too.
  assert.doesNotMatch(background.stderr, /status: possibly_stale/);
  assert.doesNotMatch(background.stderr, /results: served_from_current_index/);
