...
test("stdio installs can select direct mode and stay managed afterwards", async (t) => {
  const temporaryDirectory = await mkdtemp(
    join(tmpdir(), "zvec-grep-install-direct-mode-"),
  );
  const claudeConfigDirectory = join(temporaryDirectory, ".claude");
  const codexHome = join(temporaryDirectory, ".codex");
  t.after(async () => {
  ...
  const installed = await installTarget(
    "claude",
    { HOME: temporaryDirectory, CLAUDE_CONFIG_DIR: claudeConfigDirectory },
    ["--mode", "direct"],
  );
  assert.match(installed.stdout, /stdio in direct mode; no server is started/);
  ...
  assert.deepEqual(config.mcpServers.zvec_grep, {
    type: "stdio",
    command: "zg",
    args: ["server", "--stdio", "--mode", "direct"],
  });

  // The direct-mode entry stays recognizable as zg-managed, so a later install
  // updates it in place instead of demanding --force.
  await installTarget(
    "claude",
    { HOME: temporaryDirectory, CLAUDE_CONFIG_DIR: claudeConfigDirectory },
    ["--mode", "direct", "--mcp-toolset", "full"],
  );
  const updated = JSON.parse(
    await readFile(join(claudeConfigDirectory, ".claude.json"), "utf8"),
  );
  assert.deepEqual(updated.mcpServers.zvec_grep.args, [
    "server",
    "--stdio",
    "--mcp-toolset",
    "full",
  ...
  await installTarget("codex", { CODEX_HOME: codexHome }, ["--mode", "direct"]);
  assert.match(
    await readFile(join(codexHome, "config.toml"), "utf8"),
    /^args = \["server", "--stdio", "--mode", "direct"\]$/m,
  );

  // The flag only describes an stdio launch; an HTTP entry keeps using the URL.
  const httpConfigDirectory = join(temporaryDirectory, ".claude-http");
  await installTarget(
    "claude",
    { HOME: temporaryDirectory, CLAUDE_CONFIG_DIR: httpConfigDirectory },
    ["--mode", "direct", "--mcp-transport", "http"],
  );
  const http = JSON.parse(
    await readFile(join(httpConfigDirectory, ".claude.json"), "utf8"),
  );
  assert.equal(http.mcpServers.zvec_grep.type, "http");
  assert.equal(http.mcpServers.zvec_grep.args, undefined);
});
