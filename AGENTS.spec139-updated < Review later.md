...
- **Code changes MUST include `workflows-code--opencode` alignment evidence before closure claims.**
...
| **Global quality sweep** | Level 3/3+ only: global tests + bug sweep + compliance audit + standards pathway before closure claim                               |
...
│   3. For code changes, verify `workflows-code--opencode` alignment evidence │
│      exists for changed/added code paths                                     │
│   4. For Level 3/3+, execute Global Quality Sweep and publish evidence       │
│   5. Closure is blocked if unresolved defects are `P0 > 0` or `P1 > 0`       │
│   6. Consistency lock: checklist summary, tasks completion, and              │
│      implementation-summary completion claim MUST agree                       │
│                                                                             │
...
└─────────────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────────────┐
│ GLOBAL QUALITY SWEEP RULE [REQUIRED for Level 3/3+]                        │
│ Trigger: Completion claim for Level 3/3+ implementation work                │
│                                                                             │
│ Required Steps:                                                             │
│   1. One global testing round across all implemented updates/features       │
│   2. One global bug sweep with severity and closure tracking                │
│   3. `sk-code--opencode` compliance audit on changed/added code paths       │
│   4. Conditional standards update pathway:                                  │
│      - Applied with explicit rationale/evidence, OR                         │
│      - `N/A` with explicit rationale/evidence                                │
│                                                                             │
│ Defaults:                                                                   │
│   - Applies to future work (no retroactive rewrite required)                │
│   - P1 deferrals require explicit approval                                  │
│   - Deferrals cannot hide unresolved P0/P1 defects at closure               │
│   - No spec-specific performance thresholds are imposed by this rule        │
└─────────────────────────────────────────────────────────────────────────────┘
                                    ↓
                              ✅ CLAIM COMPLETION
...
│   □ Level 3/3+ completion? Global Quality Sweep done with `P0=0`,`P1=0`?    │
│   □ Consistency lock passed: checklist/tasks/implementation-summary aligned? │
│   □ `workflows-code--opencode` alignment evidence recorded for code changes? │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
□ `workflows-code--opencode` alignment evidence planned for changed code paths?
□ Spec folder exists?
