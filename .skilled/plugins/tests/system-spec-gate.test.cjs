// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: system-spec-gate Regression Tests                                 ║
// ╠══════════════════════════════════════════════════════════════════════════╣
// ║ PURPOSE: Pin the OpenCode adapter's kill-switch ordering contract --      ║
// ║          SYSTEM_SPEC_GATE_DISABLED=1 must be checked BEFORE                  ║
...
function makeProjectDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'system-spec-gate-test-'));
}
...
test('kill switch is a full no-op: chat.system.transform never reads or mutates output before checking SYSTEM_SPEC_GATE_DISABLED', async () => {
...
test('P2 fix: classify and enforce key an absent sessionID under the same fallback -- enforce sees what classify opened', async () => {
  ...
  const projectDir = makeProjectDir();
  try {
    await withEnv('SYSTEM_SPEC_GATE_ENFORCE', '1', async () => {
      const hooks = await MkSpecGatePlugin({ directory: projectDir });
...
// ─────────────────────────────────────────────────────────────────────────────
// P2 highest-blast proof: SYSTEM_SPEC_GATE_ENFORCE unset never denies a Write/Edit
...
test('SYSTEM_SPEC_GATE_ENFORCE unset: no Write/Edit is ever denied through the OpenCode adapter, even after classify opens the gate', async () => {
  const { default: MkSpecGatePlugin } = await loadPlugin();
  const projectDir = makeProjectDir();
  try {
    await withEnv('SYSTEM_SPEC_GATE_ENFORCE', undefined, async () => {
      ...
      for (const tool of ['write', 'edit']) {
        await assert.doesNotReject(
          ...
          `${tool} must never deny while SYSTEM_SPEC_GATE_ENFORCE is unset`,
        );
      }
    });
  } finally {
...
function warningLogPath(projectDir) {
  return path.join(projectDir, '.skilled', 'skills', '.state', 'spec-gate', 'spec-gate-warnings.log');
}
...
test('WS1 telemetry: SYSTEM_SPEC_GATE_DISABLED=1 writes no telemetry line at all', async () => {
  const { default: MkSpecGatePlugin } = await loadPlugin();
  const projectDir = makeProjectDir();
  try {
    await withEnv(DISABLED_ENV, '1', async () => {
