...
Every commit ends with a contiguous trailer paragraph. Packet work carries `Spec: <track>/<packet>[/<phase>...]`, the packet's path below `specs/` with any nested phase included and without the `specs/` segment itself, because the packet query below is anchored on that line and a leading `specs/` would hide every commit for that packet. A commit that touches no packet omits the line. A stamped commit carries `Commit-Id: NNNNNNN`, a seven-digit zero-padded ordinal minted once in commit order and never reissued.
