...
id: "cli-claude-code-EC-012"
version: 1.5.0.5
---

# EC-012 -- cli-claude-code worktree collision

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-claude-code:EC-012`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-claude-code:EC-012` ("runs cli-claude-code from each isolated worktree cwd"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-claude-code.vitest.ts` reports `runs cli-claude-code from each isolated worktree cwd` as passing under the shipped stress-matrix
  cell `cli-claude-code:EC-012`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-012 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `runs cli-claude-code from each isolated worktree cwd` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-012 and report PASS, FAIL, or SKIP with the exact blocker.`
...
The command exits `0`, and the verbose Vitest reporter names `runs cli-claude-code from each isolated worktree cwd` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-claude-code:EC-012` |
...
- Group: Stress Matrix
- Playbook ID: cli-claude-code-EC-012
