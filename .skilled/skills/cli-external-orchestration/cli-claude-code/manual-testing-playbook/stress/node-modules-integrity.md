---
title: "EC-013 -- cli-claude-code node modules integrity"
description: "Hermetic operator check for cli-claude-code:EC-013: preserves independent worktree node_modules boundaries during cli-claude-code dispatch."
matrix_cell: "cli-claude-code:EC-013"
test_file: "tests/stress/cli-adapter/cli-claude-code.vitest.ts"
test_name: "preserves independent worktree node_modules boundaries during cli-claude-code dispatch"
playbook_path: "cli-external-orchestration/cli-claude-code/manual-testing-playbook/stress/node-modules-integrity.md"
id: "cli-claude-code-EC-013"
version: 1.5.0.5
---

# EC-013 -- cli-claude-code node modules integrity

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-claude-code:EC-013`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-claude-code:EC-013` ("preserves independent worktree node_modules boundaries during cli-claude-code dispatch"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-claude-code.vitest.ts` reports `preserves independent worktree node_modules boundaries during cli-claude-code dispatch` as passing under the shipped stress-matrix
  cell `cli-claude-code:EC-013`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-013 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `preserves independent worktree node_modules boundaries during cli-claude-code dispatch` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-013 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .skilled/skills/system-deep-loop/runtime && \
  ...
  -t "preserves independent worktree node_modules boundaries during cli-claude-code dispatch" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `preserves independent worktree node_modules boundaries during cli-claude-code dispatch` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-claude-code:EC-013` |
...
- Group: Stress Matrix
- Playbook ID: cli-claude-code-EC-013
