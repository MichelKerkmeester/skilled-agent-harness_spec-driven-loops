...
- CLI commands shown as `codex <subcommand> [args]` (e.g., `codex exec "..." --model gpt-5.6-luna`)
...
2. Referenced per-feature files under `manual-testing-playbook/NN--category-name/`
...
### CX-001 | Default invocation (gpt-5.6-luna medium fast)

#### Description

Verify the canonical zero-input default dispatch (`gpt-5.6-luna` + `medium` reasoning + `service_tier="fast"`) returns a usable code-generation answer with exit code 0.
...
Expected signals: `codex exec` exits 0. Stdout contains a TypeScript function named `fizzbuzz`. Output references `n`, `Fizz`, `Buzz`, `FizzBuzz` semantics. The dispatched command line includes `--model gpt-5.6-luna`, `-c model_reasoning_effort="high"` and `-c service_tier="fast"`.
...
Verify `gpt-5.6-luna` is the documented default and that explicit `--model gpt-5.6-luna` produces a successful response, then confirm the documented GPT-5.6 roster (`gpt-5.6-luna`, `gpt-5.6-terra`, `gpt-5.6-luna`) is callable via `--model`.

#### Scenario Contract

Prompt: `Confirm the cli-codex gpt-5.6-luna default pin works and the documented gpt-5.6-luna roster is callable.`

Expected signals: The `gpt-5.6-luna` dispatch exits 0 with `--model gpt-5.6-luna` explicitly passed and returns coherent prose. Each GPT-5.6 smoke dispatch exits 0 and returns a reply. The skill reference (`references/cli-reference.md` §5) lists all four models with per-model reasoning-effort ceilings.
...
> **Feature File:** [CX-002](../manual-testing-playbook/cli-invocation/gpt-5-5-model-lock.md)
...
> **Feature File:** [CX-003](../manual-testing-playbook/cli-invocation/codex-exec-review.md)
...
> **Feature File:** [CX-004](../manual-testing-playbook/cli-invocation/explicit-fast-service-tier.md)
...
> **Feature File:** [CX-005](../manual-testing-playbook/sandbox-modes/read-only-sandbox.md)
...
> **Feature File:** [CX-006](../manual-testing-playbook/sandbox-modes/workspace-write-sandbox.md)
...
Prompt: `As a cross-AI orchestrator handling a system-level migration, FIRST capture explicit user approval to use --sandbox danger-full-access, THEN dispatch a benign read-then-write task to /tmp/cli-codex-playbook-cx007/ with --sandbox danger-full-access --ask-for-approval untrusted --model gpt-5.6-luna -c model_reasoning_effort="high" -c service_tier="fast". Verify the approval evidence exists, the dispatch includes both flags, the task completes, and no files outside /tmp/cli-codex-playbook-cx007/ are touched. Return a verdict naming the approval evidence path and confirming danger-full-access was paired with approval-untrusted.`
...
> **Feature File:** [CX-007](../manual-testing-playbook/sandbox-modes/danger-full-access-sandbox.md)
...
> **Feature File:** [CX-008](../manual-testing-playbook/sandbox-modes/approval-policies.md)
...
> **Feature File:** [CX-009](../manual-testing-playbook/reasoning-effort/low-minimal-none-baseline.md)
...
> **Feature File:** [CX-010](../manual-testing-playbook/reasoning-effort/medium-skill-default.md)
...
> **Feature File:** [CX-011](../manual-testing-playbook/reasoning-effort/high-xhigh-override.md)
...
> **Feature File:** [CX-012](../manual-testing-playbook/agent-routing/review-profile.md)
...
> **Feature File:** [CX-013](../manual-testing-playbook/agent-routing/context-profile.md)
...
Prompt: `Spec folder: /tmp/cli-codex-playbook-cx014 (pre-approved, skip Gate 3). As a cross-AI orchestrator handing off a stuck bug after 3+ failed attempts, FIRST create /tmp/cli-codex-playbook-cx014/broken.ts with a deliberate off-by-one bug, THEN dispatch codex exec -p debug "Fix the off-by-one bug in @/tmp/cli-codex-playbook-cx014/broken.ts. Apply the minimal fix and explain root cause." with --model gpt-5.6-luna -c model_reasoning_effort="high" -c service_tier="fast". Verify the dispatch routes via -p debug, the fix is applied to the file, the explanation cites the off-by-one nature, and Codex exits 0. Return a verdict naming the profile, the file path, and the root-cause sentence.`
...
> **Feature File:** [CX-014](../manual-testing-playbook/agent-routing/debug-profile.md)
...
Prompt: `As a cross-AI orchestrator running an architecture review, dispatch codex exec -p ai-council "Plan three caching strategies for a small Express API: in-memory, Redis, and CDN edge. Score each on correctness, maintainability, and performance, then recommend one." with --model gpt-5.6-luna -c model_reasoning_effort="xhigh" --sandbox read-only -c service_tier="fast". Verify the dispatch routes via -p ai-council, exits 0, returns at least three distinct labeled strategies with scores and a final recommendation, makes no file modifications, and the dispatched command line includes -p ai-council. Return a verdict naming the recommended strategy and the score breakdown.`
...
> **Feature File:** [CX-015](../manual-testing-playbook/agent-routing/multi-ai-council-profile.md)
...
> **Feature File:** [CX-016](../manual-testing-playbook/session-continuity/full-auto-hooks.md)
...
> **Feature File:** [CX-017](../manual-testing-playbook/session-continuity/session-resume-fork.md)
...
Verify the documented Codex-generates-Codex-reviews pattern (`integration-patterns.md` §2) executes end-to-end and produces a fix that addresses an inserted defect.

#### Scenario Contract

Prompt: `Spec folder: /tmp/cli-codex-playbook (pre-approved, skip Gate 3). As a cross-AI orchestrator executing the canonical generate-review-fix loop with two Codex calls: STEP 1 dispatch codex exec --model gpt-5.6-luna --sandbox workspace-write -c service_tier="fast" "Create /tmp/cli-codex-playbook-cx018/middleware.ts: an Express rate-limiter middleware with deliberately missing input validation on the limit parameter." STEP 2 dispatch codex exec --model gpt-5.6-luna --sandbox read-only -c model_reasoning_effort="high" -c service_tier="fast" "@/tmp/cli-codex-playbook-cx018/middleware.ts Review for input validation gaps. Return a JSON list of issues." STEP 3 dispatch codex exec --model gpt-5.6-luna --sandbox workspace-write -c service_tier="fast" "@/tmp/cli-codex-playbook-cx018/middleware.ts Fix the issues identified: $(cat /tmp/cx018-review.json)". Verify the final file passes a re-review (no input-validation issues remain). Return a verdict naming each step, the issue count, and the final review verdict.`
...
> **Feature File:** [CX-018](../manual-testing-playbook/integration-patterns/generate-review-fix-cycle.md)
...
> **Feature File:** [CX-019](../manual-testing-playbook/integration-patterns/web-search.md)
...
> **Feature File:** [CX-020](../manual-testing-playbook/integration-patterns/image-input.md)
...
### CX-021 | Use prompt-templates.md inventory

#### Description

Verify the documented prompt templates (`assets/prompt-templates.md` §2-§10) are real, copy-paste ready and produce a successful Codex dispatch when placeholders are filled in.

#### Scenario Contract

Prompt: `Spec folder: /tmp/cli-codex-playbook (pre-approved, skip Gate 3). As a cross-AI orchestrator picking a documented template, copy the "Single-File Application" template from assets/prompt-templates.md §2, fill placeholders for a tiny TypeScript health-check HTTP server, and dispatch the resulting prompt verbatim with --model gpt-5.6-luna --sandbox workspace-write -c model_reasoning_effort="high" -c service_tier="fast" against /tmp/cli-codex-playbook-cx021/. Verify Codex exits 0, the generated file matches the template requirements (single complete file, all imports, error handling, comments, /healthz endpoint), and the operator can identify the template line from prompt-templates.md that was used. Return a verdict naming the template anchor (e.g., "§2 Single-File Application") and confirming the generated file works.`

Expected signals: Generated file exists at `/tmp/cli-codex-playbook-cx021/server.ts`. File contains imports, error handling, comments and a `/healthz` endpoint. Operator records the template's anchor (e.g., `<!-- ANCHOR:code_generation -->` from `prompt-templates.md`). `codex exec` exits 0.
...
> **Feature File:** [CX-021](../manual-testing-playbook/prompt-templates/prompt-templates-inventory.md)

### CX-022 | CLEAR scoring via prompt-quality-card.md

#### Description

Verify the CLEAR 5-check is applied before dispatch (via the canonical card at `sk-prompt/assets/cli-prompt-quality-card.md`, reached through the local `assets/prompt-quality-card.md` delegation) and that an under-scored prompt is escalated to `@prompt-improver` per the documented threshold.

#### Scenario Contract

Prompt: `Spec folder: /tmp/cli-codex-playbook (pre-approved, skip Gate 3). As a cross-AI orchestrator constructing a non-trivial dispatch, FIRST take a deliberately weak prompt ("Fix auth"), score it with the CLEAR 5-check from the canonical card (sk-prompt/assets/cli-prompt-quality-card.md §4: Correctness, Logic, Expression, Arrangement, Reusability), THEN escalate it to a structured prompt by applying the RCAF framework from §3 of the canonical card (CLI Task to Framework Map). Dispatch the improved prompt against /tmp/cli-codex-playbook-cx022/auth.ts with --model gpt-5.6-luna --sandbox workspace-write -c model_reasoning_effort="high" -c service_tier="fast". Verify the operator records the CLEAR scores for both versions, names the framework selected, and Codex produces a meaningfully better implementation from the improved prompt than the weak prompt would have. Return a verdict including both CLEAR score sets and the framework selected.`

Expected signals: Operator records CLEAR scores for the weak prompt (low on Expression and Arrangement). Operator names the chosen framework from the canonical card `sk-prompt/assets/cli-prompt-quality-card.md` §2 (e.g., RCAF). Improved prompt scores higher. Dispatched command line uses the improved prompt and exits 0.
...
> **Feature File:** [CX-022](../manual-testing-playbook/prompt-templates/clear-scoring-quality-card.md)
...
> **Feature File:** [CX-023](../manual-testing-playbook/built-in-tools/review-tui-command.md)
...
> **Feature File:** [CX-024](../manual-testing-playbook/built-in-tools/search-exec-mode.md)
...
> **Feature File:** [CX-025](../manual-testing-playbook/built-in-tools/mcp-server-registration.md)
...
> **Feature File:** [CX-028](../manual-testing-playbook/codex-cloud/codex-cloud-dispatch.md)
...
- CX-001: [Default invocation (gpt-5.6-luna medium fast)](../manual-testing-playbook/cli-invocation/default-invocation.md)
- CX-002: [gpt-5.6-luna default model + GPT-5.6 roster](../manual-testing-playbook/cli-invocation/gpt-5-5-model-lock.md)
...
- CX-005: [read-only sandbox (analysis)](../manual-testing-playbook/sandbox-modes/read-only-sandbox.md)
- CX-006: [workspace-write sandbox (generation)](../manual-testing-playbook/sandbox-modes/workspace-write-sandbox.md)
- CX-007: [danger-full-access sandbox **(DESTRUCTIVE)**](../manual-testing-playbook/sandbox-modes/danger-full-access-sandbox.md)
- CX-008: [--full-auto vs explicit approval policies](../manual-testing-playbook/sandbox-modes/approval-policies.md)

### REASONING EFFORT

- CX-009: [low / minimal / none reasoning baseline](../manual-testing-playbook/reasoning-effort/low-minimal-none-baseline.md)
- CX-010: [medium reasoning (skill default)](../manual-testing-playbook/reasoning-effort/medium-skill-default.md)
- CX-011: [high / xhigh override](../manual-testing-playbook/reasoning-effort/high-xhigh-override.md)

### AGENT ROUTING

- CX-012: [@review profile (read-only)](../manual-testing-playbook/agent-routing/review-profile.md)
- CX-013: [@context profile (architecture mapping)](../manual-testing-playbook/agent-routing/context-profile.md)
- CX-014: [@debug profile (workspace-write fix)](../manual-testing-playbook/agent-routing/debug-profile.md)
- CX-015: [@ai-council profile (multi-strategy planning)](../manual-testing-playbook/agent-routing/multi-ai-council-profile.md)
- CX-026: [@research profile (web-grounded investigation)](../manual-testing-playbook/agent-routing/research-profile.md)

### SESSION CONTINUITY

- CX-016: [--full-auto + native hook integration](../manual-testing-playbook/session-continuity/full-auto-hooks.md)
- CX-017: [session resume / fork](../manual-testing-playbook/session-continuity/session-resume-fork.md)

### INTEGRATION PATTERNS

- CX-018: [Cross-AI generate-review-fix cycle](../manual-testing-playbook/integration-patterns/generate-review-fix-cycle.md)
- CX-019: [Web search via --search](../manual-testing-playbook/integration-patterns/web-search.md)
- CX-020: [Image input via --image](../manual-testing-playbook/integration-patterns/image-input.md)

### PROMPT TEMPLATES

- CX-021: [Use prompt-templates.md inventory](../manual-testing-playbook/prompt-templates/prompt-templates-inventory.md)
- CX-022: [CLEAR scoring via prompt-quality-card.md](../manual-testing-playbook/prompt-templates/clear-scoring-quality-card.md)

### BUILT-IN TOOLS

- CX-023: [/review TUI command](../manual-testing-playbook/built-in-tools/review-tui-command.md)
- CX-024: [--search live browsing in exec](../manual-testing-playbook/built-in-tools/search-exec-mode.md)
- CX-025: [codex mcp server registration](../manual-testing-playbook/built-in-tools/mcp-server-registration.md)
