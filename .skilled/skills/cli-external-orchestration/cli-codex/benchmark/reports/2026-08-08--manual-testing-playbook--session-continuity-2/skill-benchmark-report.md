...
| Executor / model | codex / gpt-5.6-luna (session-continuity) |
| CLI version | — |
| Flag state | `—` |
| Runtime digest | `—` |
| Run revision | — |
...
| -------- | ----- | ----- | ----- | ------------------- |
| CX-017 | manual | session-continuity | — | PASS: resume re-run: step1 wrote user.ts + emitted session id; codex exec resume wrote validate.ts referencing User; fork sub-check SKIP (no TTY) as designed |
