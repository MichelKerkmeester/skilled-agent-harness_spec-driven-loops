...
- Prompt: `Search the repository for every file under .opencode/skills/cli-external-orchestration and summarise each sibling cli-* packet in one paragraph each. Be exhaustive.`
...
perl -e 'alarm 300; exec @ARGV' -- hermes chat -Q --oneshot --ignore-rules --source tool --provider llmgateway --model glm-5.3-flash --reasoning none \
  ...
  -q "Search the repository for every file under .opencode/skills/cli-external-orchestration and summarise each sibling cli-* packet in one paragraph each. Be exhaustive." \
