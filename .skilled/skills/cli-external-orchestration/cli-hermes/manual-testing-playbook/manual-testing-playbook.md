...
4. Dispatches use only roster model ids. The roster is the seven the gateway serves for Hermes, matching the bare literals of Pi's roster; `mimo-v2.6-pro-ultraspeed` answers HTTP 400 on this route and is on neither. An off-roster id fails with exit 1 and a gateway diagnostic on stdout, which `HERMES-003` proves deliberately.
