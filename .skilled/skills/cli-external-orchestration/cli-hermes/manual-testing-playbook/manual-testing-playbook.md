...
4. Dispatches use only roster model ids. The roster is the seven the gateway serves for Hermes, matching Pi's minus `mimo-v2.6-pro-ultraspeed`, which answers HTTP 400 on this route. An off-roster id fails with exit 1 and a gateway diagnostic on stdout, which `HERMES-003` proves deliberately.
