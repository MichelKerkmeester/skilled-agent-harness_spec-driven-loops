...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-pi:EC-001`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-pi:EC-001` ("retains cli-pi authentication diagnostics in bounded lineage output"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-pi.vitest.ts` reports `retains cli-pi authentication diagnostics in bounded lineage output` as passing under the shipped stress-matrix
  cell `cli-pi:EC-001`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-001 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `retains cli-pi authentication diagnostics in bounded lineage output` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-001 and report PASS, FAIL, or SKIP with the exact blocker.`
...
The command exits `0`, and the verbose Vitest reporter names `retains cli-pi authentication diagnostics in bounded lineage output` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-pi:EC-001` |
