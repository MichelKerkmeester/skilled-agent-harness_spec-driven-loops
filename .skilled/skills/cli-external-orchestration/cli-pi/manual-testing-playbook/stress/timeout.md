---
title: "EC-004 -- cli-pi timeout"
description: "Hermetic operator check for cli-pi:EC-004: times out and terminates the captured cli-pi process."
matrix_cell: "cli-pi:EC-004"
test_file: "tests/stress/cli-adapter/cli-pi.vitest.ts"
test_name: "times out and terminates the captured cli-pi process"
playbook_path: "cli-external-orchestration/cli-pi/manual-testing-playbook/stress/timeout.md"
id: "cli-pi-EC-004"
version: 1.5.0.5
---

# EC-004 -- cli-pi timeout

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-pi:EC-004`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-pi:EC-004` ("times out and terminates the captured cli-pi process"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-pi.vitest.ts` reports `times out and terminates the captured cli-pi process` as passing under the shipped stress-matrix
  cell `cli-pi:EC-004`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-004 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `times out and terminates the captured cli-pi process` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-004 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .skilled/skills/system-deep-loop/runtime && \
  ...
  -t "times out and terminates the captured cli-pi process" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `times out and terminates the captured cli-pi process` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-pi:EC-004` |
