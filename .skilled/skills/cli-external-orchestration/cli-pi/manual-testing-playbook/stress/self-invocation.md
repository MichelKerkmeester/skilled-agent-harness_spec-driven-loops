---
title: "EC-014 -- cli-pi self invocation"
description: "Hermetic operator check for cli-pi:EC-014: blocks recursive cli-pi fan-out before process spawn."
matrix_cell: "cli-pi:EC-014"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-pi:EC-014`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-pi:EC-014` ("blocks recursive cli-pi fan-out before process spawn"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-pi.vitest.ts` reports `blocks recursive cli-pi fan-out before process spawn` as passing under the shipped stress-matrix
  cell `cli-pi:EC-014`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-014 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `blocks recursive cli-pi fan-out before process spawn` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-014 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "blocks recursive cli-pi fan-out before process spawn" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `blocks recursive cli-pi fan-out before process spawn` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
- **ledger/artifacts**: The dispatch audit denies repeated cli-pi at the stack layer; fan-out exits non-zero and no provider capture exists.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-pi:EC-014` |
