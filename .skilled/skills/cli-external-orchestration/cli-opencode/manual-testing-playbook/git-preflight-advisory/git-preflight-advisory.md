...
2. Confirm `.skilled/plugins/sk-git-preflight-advisory.js` exists and hooks `tool.execute.before` for `bash`.
...
- **Skip**: only when the plugin file `.skilled/plugins/sk-git-preflight-advisory.js` is missing from
...
|| File | Role |
||---|---|
|| `.skilled/plugins/sk-git-preflight-advisory.js` | The OpenCode plugin: `tool.execute.before` on `bash`, bounded next-turn `experimental.chat.system.transform` delivery, no stdout/stderr |
