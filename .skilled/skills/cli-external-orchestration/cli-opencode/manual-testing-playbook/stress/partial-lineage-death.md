...
id: "cli-opencode-EC-010"
version: 1.4.0.5
---

# EC-010 -- cli-opencode partial lineage death

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-opencode:EC-010`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-opencode:EC-010` ("reports a signal-killed cli-opencode lineage as failed"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-opencode.vitest.ts` reports `reports a signal-killed cli-opencode lineage as failed` as passing under the shipped stress-matrix
  cell `cli-opencode:EC-010`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-opencode:EC-010 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `reports a signal-killed cli-opencode lineage as failed` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-opencode:EC-010 and report PASS, FAIL, or SKIP with the exact blocker.`
...
The command exits `0`, and the verbose Vitest reporter names `reports a signal-killed cli-opencode lineage as failed` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-opencode:EC-010` |
...
- Group: Stress Matrix
- Playbook ID: cli-opencode-EC-010
