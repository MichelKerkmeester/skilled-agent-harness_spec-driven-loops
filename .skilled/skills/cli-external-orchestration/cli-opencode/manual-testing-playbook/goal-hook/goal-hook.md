...
`opencode-goal` (`.skilled/plugins/opencode-goal.js`) is a separate system from the runtime-neutral Pi/Cursor core under `.skilled/hooks/goal/` (see `CE-P03` in the hub playbook). It stores one file per OpenCode session as `<full-sha256-of-session-id>.json`, so filenames are fixed-length and do not expose a reversible session identifier. When a digest-keyed file is absent, a valid earlier hex-keyed file is adopted lazily only after its embedded session id is validated; an occupied digest target remains authoritative. Native `message.updated` events remain the source of `tokens_used`, not a turn-count estimate. Full contract: `.skilled/hooks/goal/goal-plugin.md`.

### Why This Matters

`/goal-opencode` is the only mechanism that gives an OpenCode session a durable, model-visible completion objective across turns. If the command router's action set, the per-session isolation, or the native token accounting silently regress, an operator's `/goal-opencode set` either corrupts a sibling session's goal, reports the wrong usage, or never reaches the model at all -- and nothing in a normal chat transcript would surface that until a goal quietly stalls. This scenario proves the plugin's own production code path (not a reimplementation) produces the documented envelope for the full action set, keeps two sessions' state files independent, and renders a well-formed injection block.
...
- Objective: Confirm the native `opencode-goal` plugin's action set, two-session isolation, fixed opaque state keys, validated lazy migration, long-session persistence, native token accounting, and passive `[active_goal]` injection by driving the shipped hooks directly against scratch state.
- Real user request: `Does /goal-opencode actually work end to end -- can I set a goal, see it reflected correctly, and does it stay scoped to my session without leaking into someone else's chat?`
- Prompt: `As an OpenCode plugin validator, exercise the shipped opencode-goal hooks against scratch state. Verify the full /goal-opencode action set, two-session isolation, fixed 64-character SHA-256 state keys, long-session persistence, validated lazy migration of a legacy hex-keyed file, native message.updated token accounting, one [active_goal] injection, and fail-closed disable behavior. Return the exact envelope lines, state filenames, and a PASS/FAIL verdict.`
- Expected execution process: Import `.skilled/plugins/opencode-goal.js`, instantiate it with a temporary `stateDir`, drive `tool.opencode_goal`, `tool.opencode_goal_status`, `event`, and `experimental.chat.system.transform` across two normal session ids plus one long id, rename one validated digest file into the earlier hex layout and trigger lazy adoption, then run all seven committed unit suites.
- Expected signals: the packet-rendered brief carries the `criteria:` field with one line per completion criterion; `set` returns `STATUS=OK ACTION=set`, `mutation=created`, and a populated RICCE `goal_prompt`; every new basename matches `^[a-f0-9]{64}\.json$`; a 140-character session suffix persists successfully; a valid legacy file moves back to the digest path on read without changing its goal; the two normal sessions remain isolated; native input/output tokens sum exactly to `tokens_used=160` with `usage_source=opencode-native-tokens`; the transform appends one matching `[active_goal:<id>]` block; all actions return their documented success envelopes; the disable switch returns `code=PLUGIN_DISABLED`; and the focused suite passes 137/137.
...
2. Confirm `.skilled/plugins/opencode-goal.js` and the 7 `.skilled/plugins/tests/opencode-goal-*.test.cjs` suites exist, and that the plugin resolves `.skilled/hooks/goal/lib/goal-slice.cjs` (a missing slice module fails the whole plugin load).
...
3. Confirm `node -e "import('./.skilled/plugins/opencode-goal.js')"` succeeds from the repository root; do not add temporary dependency links.
...
6. Rename one digest-keyed record to the earlier hex-keyed name, read it through `opencode_goal_status`, and prove the validated record returns to the digest path while the source disappears.
...
Run `node -e "import('./.skilled/plugins/opencode-goal.js')"`, execute the scratch script below as `node co-039-opencode-goal-live.mjs`, then run `node --test .skilled/plugins/tests/opencode-goal-*.test.cjs`.
...
||---|---|---|---|---|---|---|---|---|
|| CO-039 | Goal hook native opencode-goal validation | Verify actions, opaque persistence, lazy migration, isolation, native accounting, and injection | `As an OpenCode plugin validator, exercise the shipped opencode-goal hooks against scratch state. Verify the full /goal-opencode action set, two-session isolation, fixed 64-character SHA-256 state keys, long-session persistence, validated lazy migration of a legacy hex-keyed file, native message.updated token accounting, one [active_goal] injection, and fail-closed disable behavior. Return the exact envelope lines, state filenames, and a PASS/FAIL verdict.` | 1. `node -e "import('./.skilled/plugins/opencode-goal.js')"` -> 2. `node co-039-opencode-goal-live.mjs` using the scratch script above -> 3. `node --test .skilled/plugins/tests/opencode-goal-*.test.cjs` | Import succeeds; three opaque 64-hex state names are observed; the long id persists; validated legacy state migrates to its digest path; A/B remain isolated; `tokens_used=160`; one matching injection block appears; every disabled action returns `code=PLUGIN_DISABLED`; all 137 tests pass | Scratch-script assertion output, state filename list, exact status envelopes, and TAP summary | PASS only when every storage, migration, isolation, accounting, injection, action, and disable assertion passes with 137/137 tests | Check native session identity and embedded legacy `sessionId` first; an occupied digest target must win without source deletion; token drift means inspect `properties.info.tokens`, never substitute generic usage |

### Optional Supplemental Checks

- **Dependency resolution.** The tracked `.skilled/node_modules/@opencode-ai/plugin` dependency resolves in this checkout. Treat import failure as install-state drift; do not create an untracked cache symlink as part of the playbook.
- **Unit-test suite run.** `node --test .skilled/plugins/tests/opencode-goal-*.test.cjs` passes 137/137 across eight suite files. Re-measure with the command above rather than trusting this number; it has gone stale once already.
- **Live headless `opencode run` attempt -- SKIP, not FAIL.** A live attempt to validate `opencode_goal` through `opencode run` (rather than direct in-process invocation) is documented as not achievable in this environment, for two structural reasons independent of model quality: (1) the `opencode_goal` / `opencode_goal_status` tools are not exposed to the default headless-run agent -- `deepseek-v4-flash` replied verbatim "The `opencode_goal` tool is not available in my tool set. I cannot call tools that don't exist."; (2) `experimental.chat.system.transform` does not fire in `opencode run --session`, even against a pre-seeded ACTIVE goal state matching the shipped schema exactly -- a resumed session asked "what is my active goal?" reported none, with zero `[active_goal]` occurrences in the transcript. Full write-up: `specs/hooks/004-goal-hook-playbooks-and-validation/evidence/opencode-mkgoal-finding.txt`. This is orthogonal to the PASS verdict above: the direct in-process run proves the plugin's own tool/event/transform code is correct; the headless-run surface simply does not expose it to a live model in this environment. All seven plugin suites are green, and a live model-visibility proof still requires an interactive OpenCode TUI or `serve` session.
...
|| File | Role |
||---|---|
|| `.skilled/plugins/opencode-goal.js` | The OpenCode-native goal plugin: `tool.opencode_goal` / `tool.opencode_goal_status`, `event`, `experimental.chat.system.transform` |
|| `.skilled/commands/goal-opencode.md` | The `/goal-opencode` command router whose action set this plugin implements |
|| `.skilled/plugins/tests/opencode-goal-state.test.cjs` | State-store, fixed-key, long-id, and active migration coverage (31 tests) |
|| `.skilled/plugins/tests/opencode-goal-lifecycle.test.cjs` | Lifecycle, archive migration, and status-transition coverage (36 tests) |
|| `.skilled/plugins/tests/opencode-goal-supervisor.test.cjs` | Heuristic / LLM verifier unit coverage (14 tests) |
|| `.skilled/plugins/tests/opencode-goal-continuation.test.cjs` | Guarded auto-continuation unit coverage (24 tests) |
|| `.skilled/plugins/tests/opencode-goal-export-contract.test.cjs` | Plugin export-shape unit coverage (3 tests) |
|| `.skilled/plugins/tests/opencode-goal-capabilities.test.cjs` | Env/capability-flag coverage (8 tests) |
|| `.skilled/plugins/tests/opencode-goal-tool-path.test.cjs` | Tool-context session-id resolution unit coverage (9 tests) |
|| `.skilled/hooks/goal/goal-plugin.md` | Operator contract for the plugin: runtime surfaces, environment variables, output fields, cross-runtime relationship |
