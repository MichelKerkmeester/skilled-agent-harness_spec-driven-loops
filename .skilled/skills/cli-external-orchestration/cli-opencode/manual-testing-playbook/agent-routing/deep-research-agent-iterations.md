...
2. Dispatch `/deep:research:auto` (via `opencode run --command deep/research`, per SKILL.md/`cli-reference.md` §4's `--command <family>/<name>` contract) against that packet with a low iteration cap.
...
| `../../references/agent-delegation.md` (§3 "@deep-research and @deep-review — Loop Executors") | Documents the deep-research LEAF, command-owned contract |
...
| `../../references/agent-delegation.md` | §3 deep-research, LEAF iteration loop executor |
...
| `.skilled/agents/deep-research.md` | Agent definition file |
