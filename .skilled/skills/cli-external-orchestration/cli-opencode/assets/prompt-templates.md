...
Templates are numbered for cross-reference from `references/integration-patterns.md` and `SKILL.md`.
...
```bash
# Dispatch loop — note the </dev/null redirect (see integration-patterns.md §6)
for n in $(seq 1 8); do
  port=$((4100 + n))
  opencode run \
    --share \
...
Append this block to any prompt that produces evidence the calling AI wants to write back through `/speckit:save`.
...
## 16. TEMPLATE 15: MiMo (LLM GATEWAY, COSTAR PLUS LEAN)

**When**: dispatching to MiMo-V2.6-Pro, which this roster reaches through LLM Gateway (DevPass) only: the slug is `llmgateway/mimo-v2.6-pro` (provider `llmgateway`, billed per token), and the gateway serves no ultraspeed tier. The 126/004 benchmark (10/10 real `mimo-v2.6-pro` runs) found **COSTAR wins** (RACE is a statistical-tie fallback). Use **lean-to-medium** pre-planning. MiMo is frontier-correct already, so frame for format and brevity, **not** guardrails. TIDD-EC/dense ranked LAST, and CIDI is unreliable (it intermittently writes to a file instead of returning inline code). **Always pass `--variant high`**. opencode maps low/medium/high to MiMo's reasoning effort (confirmed accepted on opencode 1.15.13), and high is the standing default. **Omit `--agent`** (on opencode 1.15.13 `--agent general` warns and falls back to the default agent). Confirm the live model id with `opencode models llmgateway`.

**Invocation**:

```bash
opencode run \
  --model llmgateway/mimo-v2.6-pro \
  --variant high \
  --format json \
  --dir "$REPO_ROOT" \
  "<prompt>" \
...
- Exact files to read:
  - .skilled/skills/sk-design/sk-design-md-generator/SKILL.md
  - .skilled/skills/sk-design/sk-design-md-generator/references/extraction-workflow.md
  - .skilled/skills/sk-design/sk-design-md-generator/references/design-md-format.md
  - .skilled/skills/sk-design/sk-design-md-generator/references/quality-checklist.md
  - .skilled/skills/sk-design/sk-design-md-generator/references/sk-code-handoff.md
...
- `../references/cli-reference.md` - Full subcommand and flag reference
- `../references/integration-patterns.md` - Three use cases and decision tree
- `../references/agent-delegation.md` - Agent routing matrix
- `../references/opencode-tools.md` - Unique value props
- `./prompt-quality-card.md` - Framework selection and CLEAR check
