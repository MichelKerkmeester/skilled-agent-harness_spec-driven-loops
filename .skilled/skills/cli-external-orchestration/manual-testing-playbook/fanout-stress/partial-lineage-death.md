---
title: "EC-010 -- fanout-run partial lineage death"
description: "Hermetic operator check for fanout-run:EC-010: reports one externally killed lineage without erasing surviving artifacts."
matrix_cell: "fanout-run:EC-010"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-010`.
...
cd .skilled/skills/system-deep-loop/runtime && \
  ...
  -t "reports one externally killed lineage without erasing surviving artifacts" </dev/null
```
...
- **ledger/artifacts**: The killed lineage yields process status 2 and a `partial` envelope; the summary records two successes and one failure, surviving artifacts exist, and the killed PID is dead.
