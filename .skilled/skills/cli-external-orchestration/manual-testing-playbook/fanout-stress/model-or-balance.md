---
title: "EC-002 -- fanout-run model or balance failure"
description: "Hermetic operator check for fanout-run:EC-002: retains fan-out model-or-balance diagnostics in the failed lineage log."
matrix_cell: "fanout-run:EC-002"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-002`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "retains fan-out model-or-balance diagnostics in the failed lineage log" </dev/null
```
...
- **ledger/artifacts**: Exit 3 and the lineage log contains the model-not-found or insufficient-balance diagnostic.
