---
title: "EC-011 -- fanout-run orphan cleanup"
description: "Hermetic operator check for fanout-run:EC-011: reaps the captured fan-out orphan process group without a blanket sweep."
matrix_cell: "fanout-run:EC-011"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-011`.
...
cd .skilled/skills/system-deep-loop/runtime && \
  ...
  -t "reaps the captured fan-out orphan process group without a blanket sweep" </dev/null
```
...
- **ledger/artifacts**: Exit 3; the captured root, child, and grandchild PIDs are dead, while an unrelated provider process remains alive until explicit teardown.
