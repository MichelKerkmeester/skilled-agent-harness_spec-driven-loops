---
title: "EC-012 -- cli-devin worktree collision"
description: "Hermetic operator check for cli-devin:EC-012: runs cli-devin from each isolated worktree cwd."
matrix_cell: "cli-devin:EC-012"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-devin:EC-012`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-devin:EC-012` ("runs cli-devin from each isolated worktree cwd"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-devin.vitest.ts` reports `runs cli-devin from each isolated worktree cwd` as passing under the shipped stress-matrix
  cell `cli-devin:EC-012`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-devin:EC-012 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `runs cli-devin from each isolated worktree cwd` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-devin:EC-012 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .skilled/skills/system-deep-loop/runtime && \
  ...
  -t "runs cli-devin from each isolated worktree cwd" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `runs cli-devin from each isolated worktree cwd` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-devin:EC-012` |
