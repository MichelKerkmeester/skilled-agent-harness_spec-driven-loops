...
Two `UserPromptSubmit` hooks in this repository write `hookSpecificOutput.additionalContext` (the spec-kit prompt adapter and this adapter); the spec-gate classifier writes nothing there and only opens the session gate, putting its question in front of the model later, at the first in-gate tool call. The host concatenates what is written: a live `devin -p` run against CLI 3000.6.14, with a write-intent prompt so the classifier fired, reported the advisor line the spec-kit adapter emits. Ordering is therefore not load-bearing, and an adapter that emits nothing costs the others nothing. Re-check this after a Devin CLI upgrade, since it is host behaviour rather than a documented contract.
