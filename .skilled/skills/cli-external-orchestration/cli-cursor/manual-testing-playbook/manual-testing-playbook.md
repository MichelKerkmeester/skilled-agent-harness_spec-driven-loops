...
6. The skill's reference and asset files exist at `.skilled/skills/cli-external-orchestration/cli-cursor/{references,assets}/` so prompt-quality, hook-contract, and template scenarios resolve.
7. **Enforced model allowlist.** `composer-2.5` (Cursor's own native model, confirmed live at Pro tier) is the documented default; dispatch is otherwise scoped to exactly 21 ids — `composer-2.5`/`composer-2.5-fast`, `cursor-grok-4.5-{low,medium,high}[-fast]` (6 ids), `cursor-grok-4.6-{low,medium,high,xhigh}[-fast]` (8 ids), `gemini-3.8-flash-high` (1 id), `glm-5.2-{high,max}`, and `gpt-5.6-luna-{max,max-fast}` (2 ids) — enforced by `CURSOR_SUPPORTED_MODELS` in `executor-config.ts` and checked in both `fanout-run.cjs` and `dispatch-model.cjs`. `auto` and every hosted GPT/Claude/other-Gemini/Kimi id are OUT OF SCOPE for this skill, not merely undocumented. Effort tiers are reached by exact enumerated id, never a `[effort=...]` bracket (confirmed rejected: `Error: Cannot use this model`). Use the model a scenario names; do not substitute IDs outside this allowlist.
...
| `.skilled/skills/system-spec-kit/runtime/hooks/cursor/{session-start,session-end,shared}.ts` (compiled to `dist/hooks/cursor/*.js`) | Hook adapter contract integration | `CU-013` exercises the confirmed-fires adapters |
| `.skilled/skills/system-spec-kit/runtime/hooks/cursor/{spec-gate-enforce,spec-gate-classify}.mjs` | Runtime hook enforcement/advisory | `CU-013` (enforce, wired to `preToolUse`); `CU-014` (classify, dormant - documented, never wired) |
| `.skilled/hooks/task-dispatch/cursor/task-dispatch-guard.mjs` | `Task`-matcher `preToolUse` dispatch guard (phase 011) | `CU-021` exercises the confirmed live-fire `matcher: "Task"` entry |
| `.skilled/skills/sk-doc/scripts/validate_document.py` | Markdown structure validation for this playbook | This playbook itself (root and every scenario file MUST validate cleanly) |
| `.skilled/skills/system-deep-loop/runtime/tests/stress/cli-adapter/cli-cursor.vitest.ts` | Hermetic fan-out, lineage, timeout, and transport stress cells for the `cli-cursor` adapter | `cli-cursor-EC-001` .. `cli-cursor-EC-014` |
