---
title: "Orca CLI Installation Guide"
description: "Operator-invoked installation and read-only preflight for the Orca CLI workflow packet."
trigger_phrases:
  - "orca install guide"
  - "install orca cli"
  - "orca skill installation"
  - "orca read-only preflight"
...
This guide installs the public `orca-cli` skill package and runs the read-only preflight that the packet requires before any Orca lane. The skill package is not the Orca desktop or runtime installation: do not invent a runtime installer from the skill package command.

---

## 2. PREREQUISITES

- The operator approved the public skill installation.
- Node and `npx` are available on `PATH` for the skills-add flow.
- The Orca desktop or runtime itself is installed separately by the operator. This guide does not install it.
...
## 5. RUNTIME AND ACCOUNT BOUNDARIES
