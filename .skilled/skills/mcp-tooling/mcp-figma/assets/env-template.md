...
Copy the line below into the project `.env`, replace the placeholder with a Figma personal access token (Figma → Settings → Account → Personal Access Tokens), and never commit `.env`. The manual entry that consumes this key lives in [utcp-figma-manual.md](./utcp-figma-manual.md).
...
- [utcp-figma-manual.md](./utcp-figma-manual.md) - The paste-ready `.utcp_config.json` manual entry that consumes this key.
- [mcp-wiring.md](../references/mcp-wiring.md) - The optional Figma MCP wiring reference.
