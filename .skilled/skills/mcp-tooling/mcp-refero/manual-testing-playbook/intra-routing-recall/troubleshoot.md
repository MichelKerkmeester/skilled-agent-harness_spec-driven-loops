...
1. `sed -n '1,15p' .skilled/skills/mcp-tooling/mcp-refero/manual-testing-playbook/intra-routing-recall/troubleshoot.md`
2. `sed -n '/^INTENT_MODEL = {/,/^}/p' .skilled/skills/mcp-tooling/mcp-refero/SKILL.md | sed -n '/"TROUBLESHOOT":/p'`
3. `for p in references/troubleshooting.md references/mcp-wiring.md; do test -e ".skilled/skills/mcp-tooling/mcp-refero/$p" && echo "OK $p" || echo "MISS $p"; done`
