...
1. Code Mode: `clickup_official.clickup_official_clickup_get_custom_fields({list_id: 'LIST_ID'})`  # → find text field ID
2. Code Mode: `clickup_official.clickup_official_clickup_set_custom_field_value({task_id: 'TASK_ID', field_id: 'FIELD_ID', value: 'playbook-test'})`
3. Code Mode: `clickup_official.clickup_official_clickup_get_task({task_id: 'TASK_ID'})`  # → check custom_fields
4. `bash: jq '.custom_fields[] | select(.id=="FIELD_ID") | .value' <<< "$RESULT"`  # → 'playbook-test'

| Feature ID | Feature Name | Scenario Objective | Exact Prompt | Exact Command Sequence | Expected Signals | Evidence | Pass/Fail Criteria | Failure Triage |
|---|---|---|---|---|---|---|---|---|
| MCP-L019 | Set Custom Field Value via MCP | Verify `clickup_set_custom_field_value` sets a field value readable back via get_task | `Set custom field FIELD_ID to value 'playbook-test' on task TASK_ID.` | 1. Code Mode: `clickup_official.clickup_official_clickup_get_custom_fields({list_id: 'LIST_ID'})`  # → find text field ID 2. Code Mode: `clickup_official.clickup_official_clickup_set_custom_field_value({task_id: 'TASK_ID', field_id: 'FIELD_ID', value: 'playbook-test'})` 3. Code Mode: `clickup_official.clickup_official_clickup_get_task({task_id: 'TASK_ID'})`  # → check custom_fields 4. `bash: jq '.custom_fields[] \| select(.id=="FIELD_ID") \| .value' <<< "$RESULT"`  # → 'playbook-test' | Step 2: set exits 0. Step 4: field value is 'playbook-test'. | Code Mode response + terminal output of the verification step(s) above | PASS if step 4 returns 'playbook-test'; FAIL if field value unchanged OR custom field not found in task | See [`../../references/troubleshooting.md`](../../references/troubleshooting.md) |
...
| [`../../feature-catalog/mcp-low-priority/set-custom-field.md`](../../feature-catalog/mcp-low-priority/set-custom-field.md) | Feature catalog source |
...
- Feature file path: `mcp-bulk-and-structure/custom-field.md`
