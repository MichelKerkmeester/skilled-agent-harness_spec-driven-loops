...
> **Capability status: UNSUPPORTED.** A direct `list_tools()` inventory (`references/mcp-tools.md`) found no webhook tools on the registered server. Treat this card as an unsupported historical assumption until a fresh `tool_info()`/`list_tools()` capture confirms an exact callable name and schema.
...
Related references:
- [manage-chat.md](../../feature-catalog/mcp-low-priority/manage-chat.md) — clickup_manage_chat
- [user-groups.md](../../feature-catalog/mcp-low-priority/user-groups.md) — clickup_get_user_groups
