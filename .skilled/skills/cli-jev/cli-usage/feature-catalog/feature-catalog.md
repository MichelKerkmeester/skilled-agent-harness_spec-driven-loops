---
title: "cli-usage: Feature Catalog"
description: "Current-state inventory for the cli-usage transport: its hub registration as packetKind transport, the judgment primitives, the eight dispatch guards, and the surfaces it exposes."
trigger_phrases:
...
This catalog inventories the live `cli-usage` surface. `cli-usage` is the first mode of the
`cli-jev` hub and its only `packetKind: "transport"`: the hub resolves the mode
...
Use this catalog as the canonical inventory for the shipped `cli-usage` surface. The packet owns the
...
- **No executor kind.** `cli-usage` is absent from the deep-loop `EXECUTOR_KINDS` roster, because Jev
