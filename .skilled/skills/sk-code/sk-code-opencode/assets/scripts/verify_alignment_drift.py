...
# A runtime mirror writes this marker into each SKILL.md it copies. The copy holds
# only SKILL.md and names the canonical directory its routes resolve against, so its
# RESOURCE_MAP leaves are absent by design; the canonical SKILL.md is checked instead.
GENERATED_SKILL_COPY_RE = re.compile(r"<!-- generated by [^;>]+; do not edit -->")
    ...
    for root in roots:
        ...
        for current_root, dirs, files in os.walk(abs_root):
            ...
            try:
                ...
                continue
            if GENERATED_SKILL_COPY_RE.search(text):
                continue
