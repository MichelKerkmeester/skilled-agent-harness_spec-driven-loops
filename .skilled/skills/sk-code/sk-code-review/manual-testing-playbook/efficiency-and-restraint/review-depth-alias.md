...
- Expected execution process: Run the deterministic command sequence, capture the transcript, compare the output against review references, and record a PASS, FAIL, or SKIP verdict with rationale; SKIP only when the SK_CODE_REVIEW_DEPTH environment variable cannot be set in the current session.
...
- Feature file path: `efficiency-and-restraint/review-depth-alias.md`
