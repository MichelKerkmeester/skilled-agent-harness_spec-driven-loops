...
1. `sed -n '1,17p' .skilled/skills/sk-code/sk-code-mobile-cli/manual-testing-playbook/holdout/debugging-natural.md`
2. `sed -n '/^INTENT_SIGNALS = {/,/^}/p' .skilled/skills/sk-code/sk-code-mobile-cli/SKILL.md | sed -n '/"DEBUGGING":/p'` — confirm none of its listed keyword substrings appear in the exact prompt.
3. `for p in references/verification/verification.md references/design-system/component-tokens.md references/svelte/svelte.md assets/runes-effect-audit-checklist.md; do test -e ".skilled/skills/sk-code/sk-code-mobile-cli/$p" && echo "OK $p" || echo "MISS $p"; done`
