...
1. `sed -n '1,12p' .skilled/skills/sk-code/sk-code-mobile-cli/manual-testing-playbook/intent-detection/guardrail-routing.md`
2. `sed -n '/^## 2b\. SMART ROUTING/,/^## 3\. SURFACE STANDARDS/p' .skilled/skills/sk-code/sk-code-mobile-cli/SKILL.md | sed -n '/"CODE_QUALITY":/,/\],/p'`
3. `for p in references/conventions/editability-guardrails.md assets/guardrail-audit-checklist.md; do test -e ".skilled/skills/sk-code/sk-code-mobile-cli/$p" && echo "OK $p" || echo "MISS $p"; done`
