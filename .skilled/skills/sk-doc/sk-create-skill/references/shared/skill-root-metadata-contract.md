...
| Class | Roots |
| --- | --- |
| **H** (6) | `cli-external-orchestration`, `mcp-tooling`, `sk-code`, `sk-design`, `sk-doc`, `system-deep-loop` |
| **S** (8) | `cli-orca`, `mcp-code-mode`, `sk-communication`, `sk-git`, `sk-prompt`, `sk-vision`, `system-skill-advisor`, `system-spec-kit` |
...
| `command-metadata.json` | optional, authored | **forbidden** | authored |
...
### Why `command-metadata.json` is hub-optional
...
- `generate-description.ts` and `backfill-graph-metadata.ts` under `system-spec-kit/runtime/cli` are spec-folder-only tooling and have no skill-root analog. `ci-skill-root-metadata.cjs --fix` is the only skill-root regenerator.
