...
3. **Re-mint the activation manifest** to the shadow-child's fresh hash, flipping `servingAuthority` to `"compiled"` (still reversible: flipping it back, or the fleet kill-switch, restores legacy byte-for-byte). Activation and the flip both run the admission check for the hub and refuse anything short of `pass`, and both refuse while the advisor scorer has changed since its last freeze. The freeze is stale today, so no hub can be flipped until it is renewed.
...
- [goal-coverage-buildout.md](../../../../../specs/sk-doc/z_archive/019-skill-routing-refactor/015-router-unification-program/019-routing-coverage-activation-verification/goal-coverage-buildout.md) - the program goal and per-hub coverage recipe (worked example, spec history).
- [handover.md](../../../../../specs/sk-doc/z_archive/019-skill-routing-refactor/015-router-unification-program/019-routing-coverage-activation-verification/013-compiled-coverage-buildout/handover.md) - the verified final state at cutover, when the cohort was seven hubs, and the exact recipe that fixed under-routing (spec history).
