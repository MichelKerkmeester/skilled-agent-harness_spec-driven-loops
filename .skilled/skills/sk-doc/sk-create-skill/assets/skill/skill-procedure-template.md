...
**Standalone skill**: `references/procedures/[procedure-name].md`

**Parent hub, packet-local**: `[packet-name]/procedures/[procedure-name].md`

**Parent hub, cross-packet shared**: `shared/procedures/[procedure-name].md`, only when the procedure genuinely coordinates two or more packets and cannot be owned by one packet without duplicating orchestration.

**Naming Convention**: kebab-case with `.md` extension, same rule as `references/` and `assets/`.

| ✅ Valid Names | ❌ Invalid Names |
|---------------|-----------------|
| `sensitive-field-redaction.md` | `SensitiveFieldRedaction.md` (PascalCase) |
| `digital-signature-verification.md` | `digital_signature_verification.md` (snake_case) |
| `batch-annotation-cleanup.md` | `batchAnnotationCleanup.MD` (wrong case) |
...
**Keep it a plain reference (`skill-reference-template.md`) instead when:**
- There is only one dominant workflow, so no selection step is needed.
...
| Proof gate | The redaction log accounts for every form field and annotation in the source document, and none are silently skipped. |
...
3. Clear sensitive form-field values and strip sensitive metadata keys, and leave non-sensitive content untouched.
...
6. Confirm the log's field count matches the enumeration from step 1, with no silent skips.
```

This example is deliberately self-contained: see [examples-and-maintenance.md](../../references/skill/examples-and-maintenance.md) for how it extends the PDF Editor skill layout into a multi-procedure shape.
...
6. (Parent hubs only) If a request spans multiple packets, the hub still decides the bundle, and cards never change public routing.
...
□ H1 title matches the card title, and the intro is 1-2 sentences with no subsections.
...
`sk-design` (`.skilled/skills/sk-design/`) is an illustrative hub-scale example: its current registry has two workflow modes and one transport packet, with procedure cards owned by the workflow packets plus shared coordination cards. Consult `sk-design/mode-registry.json` before copying this topology. `sk-design/shared/procedure-card-schema.md` is the schema this template generalizes from.
...
- [creation-workflow.md](../../references/skill/creation-workflow.md) - Full skill creation workflow
- [examples-and-maintenance.md](../../references/skill/examples-and-maintenance.md) - Worked example skills, including the procedure-card layout
