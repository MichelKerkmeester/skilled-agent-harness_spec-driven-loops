...
Field guidance (the enums and hard rules are authoritative in MODES.md and profile-validator.cjs):
  ...
  mode                : one of the six A–F values validated against KNOWN_MODES: model-vs-model | framework-bakeoff |
                        ...
                        legacy single-pass profile (validator leaves a mode-less profile alone, and it needs no sweep block).
  ...
  scoring.scorer      : "5dim" (opt-in five-dimension scorer) | "pattern" (default heading/pattern matcher), the ONLY two
                        ...
                        SWEEP sweep-benchmark.cjs ignores this field entirely and always scores code-task fixtures via
                        ...
                        of the validated modes (KNOWN_MODES). It is not validated by profile-validator.cjs, so author it
  ...
  reporting.leaderboard / history : booleans, VALIDATED shape only. CURRENTLY UNSUPPORTED, neither is read at runtime, and
  ...
  benchmark           : the promotion gate, read ONLY by SINGLE-PASS run-benchmark.cjs (`profile.benchmark?.requiredAggregateScore`
                        ...
                        gating against it. requiredAggregateScore and minimumFixtureScore are 0-100 score thresholds, and
