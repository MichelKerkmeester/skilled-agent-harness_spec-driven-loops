...
fixtures use a DIFFERENT shape, described in reviewer-schema.md. Do not fill this template
...
Inline guidance (the fixtures README + scorer are authoritative):
  tests / hidden_tests : each case is { "name": string, "args": [ ... ], "expect": value }.
                         ...
                         function, and a single-argument function still takes a one-element
...
| `task` | string | The full prompt the model receives: what to implement, the rules, the traps, and any output constraint (e.g. "return ONLY the function source, and do not write files"). |
...
| `tests` | array | Visible oracle cases `{name, args[], expect}`. Ship 5+, and these can appear in the prompt. |
| `hidden_tests` | array | Held-out oracle cases in the same shape, biased toward adversarial / invalid-dominant inputs that a lax solution wrongly accepts. |
