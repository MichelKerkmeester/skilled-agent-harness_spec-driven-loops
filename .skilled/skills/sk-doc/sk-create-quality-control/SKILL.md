...
Reach this packet through `sk-doc` hub routing on a doc-quality request. It carries no slash command: `mode-registry.json` declares `"command": null`, no `.skilled/commands/doc/` file exists in any runtime, and `/doc:quality` survives only as historical vocabulary in this packet's trigger list and in the advisor's command-binding allowlist. Whichever way the packet is entered, the run is report-only unless the user explicitly asks for edits.
...
- Computing or interpreting a DQI score from `.skilled/skills/sk-doc/shared/scripts/extract_structure.py`.
...
```bash
python3 .skilled/skills/sk-doc/shared/scripts/check_authored_name_kebab.py <file>
```
...
3. Use `.skilled/skills/sk-doc/shared/scripts/extract_structure.py` as the source of truth for structure, metrics, checklist results and DQI.
4. Use validation output to distinguish blockers, warnings and recommendations.
5. Apply HVR voice review before final output.
6. Validate edited documents with `.skilled/skills/sk-doc/shared/scripts/validate_document.py` when applicable.
...
- `.skilled/skills/sk-doc/shared/scripts/extract_structure.py` - Structure extraction, metrics, checklist data and DQI.
- `.skilled/skills/sk-doc/shared/scripts/validate_document.py` - Pre-delivery document validation gate.
- `.skilled/skills/sk-doc/shared/scripts/quick_validate.py` - Fast validation for folders or skill packets.
- `.skilled/skills/sk-doc/shared/references/filesystem-naming-convention.md` - Structural naming authority and exemption boundary.
- `.skilled/skills/sk-doc/shared/references/validation.md` - DQI scoring, quality bands and gate interpretation.
- `.skilled/skills/sk-doc/sk-create-with-human-voice/references/hvr-rules.md` - Human Voice Rules for natural documentation style.
