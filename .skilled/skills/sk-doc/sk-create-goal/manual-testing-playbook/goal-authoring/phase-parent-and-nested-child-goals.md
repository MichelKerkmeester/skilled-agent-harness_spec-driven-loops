...
- Expected execution process: the workflow compares the fixture phase map with the direct child directories before writing, renders the parent goal at the phase level with one binding row per child and renders each child goal at the child level with no binding anchor. The goal check runs on the parent and on each child.
