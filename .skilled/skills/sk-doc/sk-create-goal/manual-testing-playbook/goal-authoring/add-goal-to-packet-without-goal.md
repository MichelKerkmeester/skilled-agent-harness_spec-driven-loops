...
- Objective: add a goal to an existing single-folder packet that has none and prove the new goal passes every check before handoff
...
- Expected execution process: the workflow confirms the packet exists with its source documents and no goal file, copies the top-level template and fills objective, decisions and criteria from the packet's own sources. The goal check runs before the handoff.
...
Step 5 exits 1 because the fixture packet carries its source documents and no goal file. Step 6 runs the retrofit operation, which the mode treats as its own operation and which copies the top-level template before filling it. Step 7 confirms the file landed at the exact packet path. Step 8 reads the canonical template marker, which is the signal that the template structure was reused rather than typed. Steps 9 and 10 prove the result before handoff: all four named checks pass and the packet report shows a within-budget, non-nested goal.
...
2. If step 8 prints 0 the goal was typed rather than copied from its template. Compare the goal file structure against the top-level template and route the defect to the template step of the workflow.
...
| File | Role |
|---|---|
| [`../../references/parent-and-nested-goals.md`](../../references/parent-and-nested-goals.md) | The retrofit workflow and which template it copies |
| [`../../assets/goal-top-level-template.md`](../../assets/goal-top-level-template.md) | The template the retrofit path copies instead of hand-typing structure |
