...
- Desired user-visible outcome: a rule the user can read once and act on, indistinguishable in shape from the rules already shipped.
...
2. Compare the divider count against the numbered-section count directly. That invariant holds in every shipped rule with no exception, so a mismatch localizes the problem faster than reading the draft.
