---
title: "Output Spec — format × size × detail × audience"
description: "The four dials that shape an imported or exported diagram — format, size, detail level, audience — with size presets, type ramps, detail ceilings, and the fidelity ledger."
trigger_phrases:
  - "output spec dials"
  - "format size detail audience"
  - "size preset viewbox"
  - "type ramp per size"
  - "faithful balanced simplified"
  - "fidelity ledger"
  - "degrade ladder"
...
Four dials decide what an imported diagram becomes. Set them **before** redrawing — they change the deliverable, layout, type ramp, node count, and wording.
...
Always generate the HTML first — `svg` and `png` are produced *from* it via [`export.md`](../import-export/export.md). Never hand-author an SVG file directly; the HTML is the source of truth and the only artifact the taste gate (SKILL.md SUCCESS CRITERIA) is written against.
...
The preset sets the SVG `viewBox`. Every value below is divisible by 4, so the 4px grid rule in SKILL.md HOW IT WORKS still holds.
...
`balanced` and `simplified` sit inside the standard complexity budget (SKILL.md HOW IT WORKS). **`faithful` deliberately exceeds it** — that's the trade, and it comes with conditions:
...
2. **Connector rules don't relax.** SKILL.md RULES connector rules still apply at 24 nodes. If you can't route it without overlaps, you're over the real ceiling — split.
...
1. **Decorative cells** — sticky notes, free-floating text, title blocks, watermarks, the source's own legend. (Notes worth keeping become annotation callouts — max 2, see [primitive-annotation.md](../primitives/primitive-annotation.md).)
...
Run alongside the SKILL.md SUCCESS CRITERIA taste gate.
...
- [ ] Requested non-HTML formats produced via [`export.md`](../import-export/export.md), not hand-authored?
