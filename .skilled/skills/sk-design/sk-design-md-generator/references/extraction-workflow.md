...
**Actions**:
1. Run from the **repo root** (a one-time `cd .skilled/skills/sk-design-md-generator/backend && npm install && npx playwright install chromium` handles setup): `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/extract.ts <url> --fast --output .opencode/specs/<track>/<packet>/output`. `extract.ts` refuses any `--output` that resolves inside the skill, so the relative spec-folder path must resolve from the repo root.
...
**Actions**:
1. `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/validate.ts <DESIGN.md> <--output>/tokens.json` (from the repo root).
...
| `mcp-figma` | Covers Figma files; this skill covers live URLs |
