...
PRE: Wave 1 (SETUP-001) must be PASS. The `backend/node_modules/` directory must exist and the vitest runner must be available. A prior extraction must have produced `<--output>/tokens.json`; if none exists, run `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/extract.ts <url> --fast --output .opencode/specs/<track>/<packet>/output` first.

1. verify tool readiness: `bash: node --version`, glob `backend/node_modules/`  # -> Node 20+, node_modules exists
2. `cd .skilled/skills/sk-design-md-generator/backend && npx vitest run cluster`  # -> exits 0, all cluster tests pass
