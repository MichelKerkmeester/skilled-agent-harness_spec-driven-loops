...
1. Working directory is the **repo root** (`pwd` shows the repository root). The embedded tool lives at `.skilled/skills/sk-design-md-generator/backend/`; run the pipeline scripts (`extract.ts`, `validate.ts`, `report-gen.ts`, `preview-gen.ts`, `proof.ts`) from the repo root with the full script path, so a relative `--output` resolves outside the skill (the output guard refuses skill-internal paths). Run the one-time setup (`npm install`, `npx playwright install chromium`) and the `vitest` suite from the `backend/` directory.
...
> **Do-not-run note for this playbook author and executors:** Examples in this document that show command output are illustrative, so verify exact flags and output with `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/extract.ts --help` and `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/validate.ts --help` (from the repo root) on the live machine. Never fabricate tokens in any scenario; a fabricated token is a HARD FAIL.
...
|---|---|---|
| Extract (Phase 1) | `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/extract.ts <url> [flags]` | `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/extract.ts https://example.com --fast --output .opencode/specs/<track>/<packet>/output` |
| Validate (Phase 3) | `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/validate.ts <DESIGN.md> <tokens.json>` | `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/validate.ts DESIGN.md <--output>/tokens.json` |
...
- Expected execution process: confirm Node 20+; run `cd backend && npm install` and verify zero errors; run `cd backend && npx playwright install chromium` and verify it completes; smoke-test with `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/extract.ts --help` to confirm the CLI parses; attempt `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/extract.ts https://example.com --fast --output .opencode/specs/<track>/<packet>/output` and confirm it runs (or fails with a clear crawl error, not a missing-binary crash)
