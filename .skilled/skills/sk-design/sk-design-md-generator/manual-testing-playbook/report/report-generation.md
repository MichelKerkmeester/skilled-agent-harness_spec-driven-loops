...
4. `cd .skilled/skills/sk-design-md-generator/backend && npx ts-node scripts/report-gen.ts <--output>/tokens.json <--output>/ <--output>/DESIGN.md`  # -> exits 0, writes report.html
5. `bash: ls -la <--output>/report.html`  # -> file exists, > 1 KB
6. `cd .skilled/skills/sk-design-md-generator/backend && npx ts-node scripts/preview-gen.ts <--output>/tokens.json <--output>/`  # -> exits 0, writes preview.html
7. `bash: ls -la <--output>/preview.html`  # -> file exists, > 1 KB
8. `cd .skilled/skills/sk-design-md-generator/backend && npx ts-node scripts/proof.ts https://example.com <--output>/tokens.json <--output>/`  # -> exits 0, writes proof.html + proof-data.json
