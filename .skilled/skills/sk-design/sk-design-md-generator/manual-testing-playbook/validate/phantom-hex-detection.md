...
- Objective: confirm the validator recognizes the v3 schema and correctly distinguishes faithful from fabricated output, including Quick-Start fidelity and primitive-versus-semantic token naming
...
- Expected execution process: run `validate.ts` against a known-good v3 Style Reference + tokens.json pair; confirm PASS with values score 100, claims score >= 80, and zero phantom-hex failures. Then plant a single hex (`#ff0000`) in a copy that does not exist in tokens.json, re-run validation, and confirm a phantom-color failure. Then plant a Quick Start hex not in tokens and confirm a critical `quickstart-phantom-color` failure, plant a `--page-max-width: 100rem` where tokens say `100%` and confirm a `quickstart-maxwidth` warning, and check `references/quality-checklist.md` for the primitive-versus-semantic token naming rule before accepting the Style Reference
- Expected signals: first run exits 0 with `passed: [...]`, `failures: []`, values 100, claims >= 80; planted-hex run exits non-zero with a phantom-color finding for `#ff0000`; planted Quick-Start-hex run reports a critical `quickstart-phantom-color` failure; planted max-width run reports a `quickstart-maxwidth` warning; semantic token names describe UI roles rather than raw scale values
...
1. `cd .skilled/skills/sk-design-md-generator/backend && npx ts-node scripts/validate.ts <path-to-faithful-style-reference.md> <--output>/tokens.json`  # -> exit 0, values 100, claims >= 80, failures array empty
...
4. `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/validate.ts /tmp/planted.md <--output>/tokens.json`  # -> exit non-zero or failures non-empty, at least one phantom-color finding for ff0000
...
6. check token naming against `references/quality-checklist.md`: primitive tokens may use raw values or scale positions such as `blue-600`, while semantic tokens must name roles such as `color-action`, `color-surface`, or `color-danger`
7. agent reports all results: faithful PASS, planted-hex FAIL with phantom-color finding, Quick-Start-planted FAIL with `quickstart-phantom-color` + `quickstart-maxwidth`, and token names passing the primitive-versus-semantic distinction

| Feature ID | Feature Name | Scenario Name / Objective | Exact Prompt | Exact Command Sequence | Expected Signals | Evidence | Pass/Fail Criteria | Failure Triage |
|---|---|---|---|---|---|---|---|---|
| VALIDATE-001 | Fidelity validation | Verify `validate.ts` recognizes the v3 schema, passes on a faithful Style Reference, flags planted Quick Start fabrications, and preserves primitive-versus-semantic token naming | `Validate the Style Reference I just wrote against its tokens.json.` | 1. `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/validate.ts <faithful-style-reference.md> <--output>/tokens.json` -> 2. copy it, append `#ff0000` in a prose line -> 3. `npx ts-node .skilled/skills/sk-design-md-generator/backend/scripts/validate.ts <planted.md> <--output>/tokens.json` -> 4. plant a Quick Start hex not in tokens and a `--page-max-width: 100rem` where tokens say `100%`, re-run -> 5. inspect token naming against `references/quality-checklist.md` SC-09 | Step 1: exit 0, values 100, claims >= 80, zero `phantom-color` failures. Step 2: phantom hex planted. Step 3: exit non-zero (or failures array non-empty), at least one `phantom-color` finding for `#ff0000`. Step 4: critical `quickstart-phantom-color` failure for the planted Quick Start hex AND a `quickstart-maxwidth` warning for the mismatched `--page-max-width`. Step 5: primitive tokens use raw value or scale names while semantic tokens name roles or meaning | Transcript of the validation runs, the planted snippets, the phantom-color / quickstart-phantom-color / quickstart-maxwidth findings, and the token-naming check | PASS if the faithful run passes with no phantom-hex failures and claims score >= 80 AND the planted runs flag `#ff0000`, the Quick Start phantom hex (critical), the `--page-max-width` mismatch (warning), and semantic tokens are role-named rather than raw-value names wearing semantic slots. FAIL if the faithful run reports false positives, any planted fabrication is not flagged, or semantic token names conflate raw scale names with UI roles | 1. Confirm the faithful Style Reference contains only hexes from tokens.json. 2. Confirm the planted hexes are not in tokens.json (check manually). 3. Confirm `checkPhantomColors` and `checkQuickStartFidelity` produced the expected findings, and that `--page-max-width` is compared against `tokens.spacingSystem.maxContentWidth`. 4. Re-read `references/quality-checklist.md` SC-09 and rename semantic tokens by purpose, usage, or state. |
...
| `../../references/design-md-format.md` | v3 Style Reference section specification — defines the named token tables, Components, Quick Start, and Similar Brands sections |
| `../../references/quality-checklist.md` | Pre-validate self-check list |
...
- Feature file path: `validate/phantom-hex-detection.md`
