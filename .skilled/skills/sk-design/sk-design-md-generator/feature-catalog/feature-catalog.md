---
title: "md-generator: Feature Catalog"
description: "Unified capability inventory for the md-generator skill, covering extraction, clustering, v3 Style Reference DESIGN.md writing, validation, reporting, and per-feature detection across a three-phase pipeline."
trigger_phrases:
...
Use this catalog as the inventory for the live `md-generator` surface. The numbered sections below group the skill by capability area so readers can move from a top-level summary into the per-feature detail without losing the pipeline context.
...
- OKLCH color space clustering groups visually similar colors into named roles following the taxonomy in `references/color-role-taxonomy.md`.
...
- The write-phase prompt template (`assets/design-md-prompt-template.md`) and cardinal rules card (`assets/cardinal-rules-card.md`) front-load the fidelity contract.
- The authoring boundary keeps that contract legible by origin: only measured values (present in `tokens.json`) enter the token tables and they enter unlabeled, brief-provided values stay in prose as a stated intent, inferred claims carry `[INFERRED]` and cite a measured token, absent values are stamped or omitted. `references/authoring-boundary.md` draws the line and `assets/source-of-truth-router-card.md` sorts each value before writing. The boundary adds no capability and authoring from a brief alone is forward-authoring, out of scope for this mode.
- The writer loads the v3 section spec from `references/design-md-format.md` and voice rules from `references/writing-style-guide.md` before composing.
