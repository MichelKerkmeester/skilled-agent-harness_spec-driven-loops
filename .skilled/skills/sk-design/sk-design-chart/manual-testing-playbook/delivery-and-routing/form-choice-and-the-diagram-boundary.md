...
2. `agent: Resolve stage two for "make a waterfall chart of the budget movement" from .skilled/skills/sk-design/hub-router.json and mode-registry.json, naming the mode, the matched alias and the resources, and save it as stage-two.txt`
3. `agent: Resolve stage two for "create a diagram of the checkout flow" the same way and save it as neighbour.txt`
