...
```bash
printf '%s' '{"tool_name":"Write","tool_input":{"file_path":"src/login.ts"},"session_id":"hook-demo-a","cwd":"'"$TMPDIR_A"'"}' \
  | env -u AI_SESSION_CHILD SYSTEM_SPEC_GATE_ENFORCE=1 node .skilled/skills/system-spec-kit/runtime/hooks/claude/spec-gate-enforce.mjs
```
...
```bash
printf '%s' '{"tool_name":"Write","tool_input":{"file_path":"src/login.ts"},"session_id":"hook-demo-a","cwd":"'"$TMPDIR_A"'"}' \
  | SYSTEM_SPEC_GATE_ENFORCE=1 AI_SESSION_CHILD=1 node .skilled/skills/system-spec-kit/runtime/hooks/claude/spec-gate-enforce.mjs
```
...
printf '%s' '{"tool_name":"Write","tool_input":{"file_path":".opencode/specs/999-demo/spec.md"},"session_id":"hook-demo-a","cwd":"'"$TMPDIR_A"'"}' \
  | SYSTEM_SPEC_GATE_ENFORCE=1 node .skilled/skills/system-spec-kit/runtime/hooks/claude/spec-gate-enforce.mjs
```
...
```bash
printf '%s' '{"tool_name":"Write","tool_input":{"file_path":"src/login.ts"},"session_id":"hook-demo-a","cwd":"'"$TMPDIR_A"'"}' \
  | SYSTEM_SPEC_GATE_ENFORCE=1 SYSTEM_SPEC_GATE_DISABLED=1 node .skilled/skills/system-spec-kit/runtime/hooks/claude/spec-gate-enforce.mjs
rm -rf "$TMPDIR_A"
```
...
import * as core from '$(pwd)/.skilled/skills/system-spec-kit/runtime/hooks/lib/spec-gate/spec-gate-core.mjs';
...
```bash
LOG=.skilled/skills/.state/spec-gate/spec-gate-warnings.log
wc -l "$LOG"
...
- Deny-predicate documentation: `.skilled/skills/system-spec-kit/runtime/hooks/lib/spec-gate/README.md`
- Live telemetry: `.skilled/skills/.state/spec-gate/spec-gate-warnings.log` (real, project-local, gitignored state)

Provenance: .skilled/plugins/tests/system-spec-gate.test.cjs
