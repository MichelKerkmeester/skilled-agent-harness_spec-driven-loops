...
- Real user request: `` Please validate Phase detection scoring against bash .skilled/skills/system-spec-kit/runtime/cli/spec/recommend-level.sh --recommend-phases --json specs/<target-spec> and tell me whether the expected signals are present: JSON output contains `recommended_phases` (boolean), `phase_score` (number), `suggested_phase_count` (number), and 4 dimension scores: LOC Factor (35%), File Count (20%), Risk Factors (25%), Complexity (20%); simple specs score low. ``
- Prompt: `Validate Phase detection scoring against bash .skilled/skills/system-spec-kit/runtime/cli/spec/recommend-level.sh --recommend-phases --json specs/<target-spec> and report cited pass/fail evidence.`
...
```
Validate Phase detection scoring against bash .skilled/skills/system-spec-kit/runtime/cli/spec/recommend-level.sh --recommend-phases --json specs/<target-spec> and report cited pass/fail evidence.
```

### Commands

1. Create or identify a high-complexity spec folder (>500 LOC, multiple concerns)
2. `bash .skilled/skills/system-spec-kit/runtime/cli/spec/recommend-level.sh --recommend-phases --json specs/<target-spec>`
...
Provenance: manual only - bash .skilled/skills/system-spec-kit/runtime/cli/spec/recommend-level.sh --recommend-phases --json specs/<target-spec>
...
- Feature file path: `tooling-and-scripts/phase-detection-scoring.md`
