...
This scenario validates the end-to-end template compliance enforcement pipeline. It tests Layer 1 (agent contract awareness) and Layer 2 (post-write validation) working together to ensure that spec documents generated by the distributed-governance spec-authoring flow pass `.skilled/skills/system-spec-kit/runtime/cli/spec/validate.sh --strict` on first generation without requiring post-hoc fixes.
...
- Real user request: `Please validate Template Compliance Contract Enforcement against bash .skilled/skills/system-spec-kit/runtime/cli/spec/validate.sh --strict <spec-folder> and tell me whether the expected signals are present: All 5 Level 2 files (spec.md, plan.md, tasks.md, checklist.md, implementation-summary.md) pass validate.sh --strict with exit code 0; no post-hoc fixes needed.`
- Prompt: `Validate Template Compliance Contract Enforcement against bash .skilled/skills/system-spec-kit/runtime/cli/spec/validate.sh --strict <spec-folder> and report cited pass/fail evidence.`
...
```
Validate Template Compliance Contract Enforcement against bash .skilled/skills/system-spec-kit/runtime/cli/spec/validate.sh --strict <spec-folder> and report cited pass/fail evidence.
```

### Commands

1. Create a Level 2 spec folder via distributed-governance spec-authoring flow for a test feature
2. `bash .skilled/skills/system-spec-kit/runtime/cli/spec/validate.sh --strict <spec-folder>`
...
Provenance: manual only - bash .skilled/skills/system-spec-kit/runtime/cli/spec/validate.sh --strict <spec-folder>
...
- Feature file path: `tooling-and-scripts/template-compliance-contract-enforcement-produces-compliant.md`
