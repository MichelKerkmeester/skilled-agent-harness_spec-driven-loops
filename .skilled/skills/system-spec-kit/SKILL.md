...
- `spec/create.sh`
- `spec/archive.sh`
- `spec/refresh-track-roots.mjs`
- `spec/sweep-track-roots.mjs`
...
Run `.skilled/skills/system-spec-kit/runtime/cli/spec/validate.sh <spec-folder> --strict` before completion claims. Validation errors block completion; warnings must be addressed or documented. Startup, resume, hook, goal plugin, code graph, and Code Graph readiness details live in `references/config/hook-system.md`, `.skilled/skills/system-skill-advisor/hooks/skill-advisor-hook.md`, `.skilled/hooks/goal/goal-plugin.md`, `runtime/hooks/README.md` (Claude and OpenCode hook folders; OpenCode uses plugin-backed delivery), and the code graph references.

### OpenCode Goal Plugin

The packet `goal.md` is the source of goal state. A session binds to it (`opencode_goal({ action: "bind", packetPath })` on OpenCode, `bin/goal.cjs bind <packet>` on Pi), the runtime injects the file's durable slice on every turn with the frontmatter stripped, and the per-session record under `.skilled/skills/.state/goal/` keeps only the pointer, liveness and telemetry. When an operator says "set the goal", "bind the goal", "update the goal" or "resend the goal" in conversation, that is this surface: render the parent's chat slice from the file and hand it over to be set, keep reminding while it stays unset and never stop work for it. The chat slice is the durable slice without frontmatter, HTML comments, anchor markers, `---` dividers or heading section numbers, and `node .skilled/hooks/goal/bin/goal.cjs packet <packet> --workspace <repo root>` prints it as `chat_slice`. Never send a parent goal over 4,000 characters: cut the file in the order the goal set-string playbook gives, then send. The OpenCode surface is `.skilled/plugins/opencode-goal.js` plus `.skilled/commands/goal-opencode.md`; it injects with `experimental.chat.system.transform`, observes lifecycle events through the plugin `event` hook, and exposes `opencode_goal` / `opencode_goal_status` plugin tools. Use [`.skilled/hooks/goal/goal-plugin.md`](../../hooks/goal/goal-plugin.md) for the operator contract, restart requirement, environment variables, validation commands, and boundary between raw `objective` and generated `goalPrompt`.
...
17. **Route all code creation/updates through `sk-code`** - Full surface alignment is mandatory before claiming completion
   - **Authoring-time vs review-time load**: `sk-code` is loaded at TWO distinct points in `/speckit:complete`. (a) Authoring-time (Step 10 development): when the implementation target is under `.skilled/skills/`, `.skilled/agents/`, `.skilled/commands/`, or `.opencode/specs/`, load the matching sk-code authoring checklist (`../sk-code/sk-code-opencode/assets/checklists/{surface}-authoring.md`) and, for `.opencode/specs/` targets, the system-spec-kit spec-folder docs (`references/workflows/spec-folder-authoring-checklist.md` + `references/workflows/spec-folder-write-recipe.md`) BEFORE the first write. (b) Review-time (Step 11 review): the existing `sk-code` code-review mode (findings-first baseline + router-selected surface evidence) overlay runs after writes complete. Authoring-time load surfaces invariants the writer needs to honor; review-time load catches drift the writer didn't honor. See the `cross_skill_authoring_load` block in `speckit-complete.yaml` for the YAML contract.
