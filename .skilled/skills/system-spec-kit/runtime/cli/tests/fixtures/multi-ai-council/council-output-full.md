...
Implement `persist-artifacts.cjs` as a parse-and-write helper under `system-spec-kit/runtime/cli/ai-council/`. Add the markdown schema reference and fixture-driven tests. Then update the agent body and runtime mirrors with a caller-owned persistence protocol.
