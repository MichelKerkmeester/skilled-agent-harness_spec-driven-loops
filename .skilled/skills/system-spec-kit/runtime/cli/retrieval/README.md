...
| `lookup-trigger-index.mjs` | Loads the committed index and ranks candidates for one prompt, optionally scoped to a spec folder. The Gate 1 instruction that calls it is authored once, in the root `AGENTS.md` (`CLAUDE.md` is a symlink to it); Pi loads that file from the working directory, while `.codex/AGENTS.md` and `.cursor/rules/skill-routing.md` carry a pointer block generated from the same line by `runtime-mirrors/sync-gate1-pointers.cjs`, so all five runtimes reach it and `--check` reports a stale copy. |
...
| `sweep-memory-residue.mjs` | Streams ripgrep JSON-lines matches for retired memory-MCP terms and classifies each hit against an allowlist. Owned by this retrieval package and rerunnable at any time; it was the acceptance check for the memory decommission and stays the residue guard, with the coverage-parity suite keeping its exclusion delta honest. |
