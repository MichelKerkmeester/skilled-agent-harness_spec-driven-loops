...
describe('smart-router telemetry analyzer', () => {
  ...
  it('emits a no-data report for empty JSONL', () => {
    const root = tempRoot();
    fs.mkdirSync(path.join(root, '.skilled', 'skill', '.state/smart-router-telemetry'), { recursive: true });
    const inputPath = path.join(root, '.skilled', 'skill', '.state/smart-router-telemetry', 'compliance.jsonl');
    fs.writeFileSync(inputPath, '', 'utf8');

    const analysis = analyzeTelemetryFile({
      workspaceRoot: root,
      inputPath: '.skilled/skills/.state/smart-router-telemetry/compliance.jsonl',
      timestamp: '2026-04-19T21:00:00.000Z',
    });
