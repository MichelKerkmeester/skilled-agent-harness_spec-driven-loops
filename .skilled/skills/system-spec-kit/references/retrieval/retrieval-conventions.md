...
| `.skilled/repo-rules` | No | No as a root; reached under `.skilled` | Decided against. The rule documents carry the same frontmatter as spec docs, but they are loaded at Gate 5 through the trigger table in `REPO RULES.md`, not retrieved at Gate 1; indexing them would surface a rule as a context candidate. Their `trigger_phrases` still serve `sk-create-repo-rule`'s own collision check |
