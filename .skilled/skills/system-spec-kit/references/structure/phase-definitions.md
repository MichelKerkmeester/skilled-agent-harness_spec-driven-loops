...
**Detection rule (single source of truth):** A spec folder is a phase parent iff (a) it has at least one direct child matching `^[0-9]{3}-[a-z0-9][a-z0-9-]*$` AND (b) at least one such child has `spec.md` OR `description.json`. The shell helper `is_phase_parent()` (in `.skilled/skills/system-spec-kit/runtime/cli/lib/shell-common.sh`) and the ESM JS helper `isPhaseParent()` (in `.skilled/skills/system-spec-kit/runtime/cli/dist/spec/is-phase-parent.js`) both implement this contract; they MUST agree on every input. The validator's phase-parent branches in `check-files.sh`, `check-level-match.sh`, `check-template-source.sh`, and the native `ANCHORS_VALID` rule in `runtime/lib/validation/orchestrator.ts` skip Level-N expectations when this returns true.
