...
1. **Never create from scratch** - Always scaffold through `create.sh` or render from `.skilled/skills/system-spec-kit/templates/`
...
```bash
bash .skilled/skills/system-spec-kit/runtime/cli/templates/inline-gate-renderer.sh --level <1|2|3|3+|phase> <path-to-template.md.tmpl>
```
...
bash .skilled/skills/system-spec-kit/runtime/cli/templates/inline-gate-renderer.sh \
  --level 3 \
  --out-dir specs/###-name \
  .skilled/skills/system-spec-kit/templates/addons/resource-map.md.tmpl
```
...
```bash
bash .skilled/skills/system-spec-kit/runtime/cli/spec/create.sh --level <N> --path specs/###-name --name feature-name
...
bash .skilled/skills/system-spec-kit/runtime/cli/templates/inline-gate-renderer.sh \
  --level 1 \
  --out-dir specs/###-name \
  .skilled/skills/system-spec-kit/templates/core/tasks.md.tmpl
...
bash .skilled/skills/system-spec-kit/runtime/cli/templates/inline-gate-renderer.sh \
  --level 2 \
  --out-dir specs/###-name \
  .skilled/skills/system-spec-kit/templates/addons/acceptance-criteria.md.tmpl
# Or render with --level 3+ when the packet is Level 3+
```
...
bash .skilled/skills/system-spec-kit/runtime/cli/templates/inline-gate-renderer.sh \
  --level 3 \
  --out-dir specs/###-name \
  .skilled/skills/system-spec-kit/templates/addons/decision-record.md.tmpl
```
...
bash .skilled/skills/system-spec-kit/runtime/cli/templates/inline-gate-renderer.sh \
  --level 3 \
  --out-dir specs/###-name \
  .skilled/skills/system-spec-kit/templates/addons/handover.md.tmpl
```
...
**Creation (JSON mode — preferred):** `node .skilled/skills/system-spec-kit/runtime/cli/dist/continuity/generate-context.js --json '{"specFolder":"###-name","sessionSummary":"..."}' specs/###-name/`
...
**IMPORTANT:**
- **NEVER author continuity surfaces manually** - always use `generate-context.js` via the runtime `node .skilled/skills/system-spec-kit/runtime/cli/dist/continuity/generate-context.js` entrypoint
...
1. **Always scaffold or render from `.skilled/skills/system-spec-kit/templates/`** - Never freehand
...
- [ ] All templates copied from `.skilled/skills/system-spec-kit/templates/` (not created from scratch)
...
**Right:**
```bash
bash .skilled/skills/system-spec-kit/runtime/cli/spec/create.sh --level 1 --path specs/042-feature --name feature-name
```
...
```bash
bash .skilled/skills/system-spec-kit/runtime/cli/spec/validate.sh specs/###-parent-feature/ --recursive
```
