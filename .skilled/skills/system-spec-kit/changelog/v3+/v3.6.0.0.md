...
The skill advisor keeps its working files at the repo root, out of your spec folders. When it ran from inside a packet, its workspace resolver could fall back to that packet and drop a stray `.state/advisor` directory into it. The resolver now refuses to land inside a `specs/` subtree, so the advisor writes its state where it belongs and your spec folders stay clean. A regression test pins the new behavior, and the directories that had already leaked were cleaned up.
