...
| **Doc Corrections** | The Codex MCP config's code-graph database note had its default and legacy paths backwards. Fixed. |
...
The Codex MCP config note claimed the code-graph database default was the legacy shared path and the skill-local path was legacy — it is exactly the other way around, and the note now matches the launcher source.
