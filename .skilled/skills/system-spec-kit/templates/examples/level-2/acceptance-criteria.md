...
| AC-005 | REQ-005 | Given an invalid login, When the error renders, Then it is generic and does not enumerate credentials | `tests/auth/login.test.ts:57` | Met | - |
| AC-006 | REQ-006 | Given a refreshed page, When the stored token is still valid, Then the session is restored without a new login | `tests/auth/session.test.ts:14` | Met | - |
| AC-007 | REQ-007 | Given an email already registered, When registration is retried, Then a clear error is shown and no duplicate row is created | `tests/auth/register.test.ts:63` | Met | - |
