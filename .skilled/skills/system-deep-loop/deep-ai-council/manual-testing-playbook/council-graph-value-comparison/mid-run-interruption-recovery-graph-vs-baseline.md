...
> **Automated test anchor:** `.skilled/skills/system-deep-loop/runtime/tests/integration/council-graph-value-scenarios.vitest.ts` test name `DAC-032 graph beats no-graph baseline`. Measured baseline-vs-graph ratios live in `.skilled/skills/system-deep-loop/runtime/tests/council-graph-value-report.json`.
...
| `../council-graph-integration/council-graph-status-recovery-payload-and-readiness.md` | Functional anchor for `runtime status CLI` recovery payload |
| `../depth-and-failure-handling/resume-after-interrupted-state.md` | Baseline JSONL-resume contract |

### Implementation And Test Anchors

| File | Role |
|---|---|
| `.skilled/skills/system-deep-loop/runtime/scripts/status.cjs` | runtime CLI script with `recovery` field (P2-001 remediation) |
| `.skilled/skills/system-deep-loop/runtime/lib/council/council-graph-db.ts` | Storage layer: namespace-scoped recovery query |
| `.skilled/skills/system-deep-loop/deep-ai-council/references/structure/state-format.md` | Documents append-only `ai-council-state.jsonl` event contract |
| `.skilled/skills/system-deep-loop/deep-ai-council/references/integration/graph-support.md` §5 | Documents the recovery + replay contract |
...
- Feature file path: `council-graph-value-comparison/mid-run-interruption-recovery-graph-vs-baseline.md`
