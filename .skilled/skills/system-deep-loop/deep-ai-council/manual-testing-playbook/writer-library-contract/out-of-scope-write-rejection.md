...
- Expected execution process: Inspect the agent body §16 rule and `lib/persist-artifacts.cjs` path guards, then grep for `OUT_OF_SCOPE_WRITE` and `ai-council`.
...
1. Read `.skilled/agents/ai-council.md` §16.
2. Inspect `.skilled/skills/system-deep-loop/deep-ai-council/scripts/lib/persist-artifacts.cjs` path guard logic.
...
1. `bash: rg -n "OUT_OF_SCOPE_WRITE|ai-council" .skilled/skills/system-deep-loop/deep-ai-council/scripts/lib/persist-artifacts.cjs | head -20`
2. `bash: rg -n "OUT_OF_SCOPE_WRITE" .skilled/agents/ai-council.md`
...
|---|---|---|---|---|---|---|---|---|
| DAC-017 | OUT_OF_SCOPE_WRITE rejection | Verify scoped-write path guard | `As a scoped-write boundary validator, attempt to write outside ai-council/**. Verify the write is rejected with OUT_OF_SCOPE_WRITE before filesystem touch. Return rejection evidence.` | `bash: rg -n "OUT_OF_SCOPE_WRITE\|ai-council" .skilled/skills/system-deep-loop/deep-ai-council/scripts/lib/persist-artifacts.cjs | head -20` -> `bash: rg -n "OUT_OF_SCOPE_WRITE" .skilled/agents/ai-council.md` | Guard and declaration present | Grep output | PASS if lib and agent both include guard evidence | Restore path guard before dynamic testing |
...
| File | Role |
|---|---|
| `.skilled/skills/system-deep-loop/deep-ai-council/scripts/lib/persist-artifacts.cjs` | Path guard implementation |
| `.skilled/agents/ai-council.md` | Scoped-write authority declaration |
...
- Feature file path: `writer-library-contract/out-of-scope-write-rejection.md`
