...
- Expected execution process: Grep SKILL.md and `references/integration/graph-support.md` for graph references and confirm source-of-truth boundaries.
- Expected signals: Graph references state that `ai-council/**` artifacts remain authoritative and graph updates use caller-owned `runtime/` CLI tooling.
...
Inspect Section 1, Section 4, Section 7, and `references/integration/graph-support.md` for unsafe source-of-truth claims.
...
- Feature file path: `scope-boundaries/graph-support-derived-and-scoped.md`
