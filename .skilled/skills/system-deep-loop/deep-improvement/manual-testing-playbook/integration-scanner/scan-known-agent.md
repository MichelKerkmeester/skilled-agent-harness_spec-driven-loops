...
- Prompt: `Validate that scanning a known agent discovers every integration surface and confirms mirror alignment.`
...
|---|---|---|---|---|---|---|---|---|
| IS-001 | Scan Known Agent (Debug) | Validate Scan Known Agent (Debug) | `Validate that scanning a known agent discovers every integration surface and confirms mirror alignment.` | node .skilled/skills/system-deep-loop/deep-improvement/scripts/agent-improvement/scan-integration.cjs --agent=debug<br><br><br>Verification:<br><br><br>node .skilled/skills/system-deep-loop/deep-improvement/scripts/agent-improvement/scan-integration.cjs --agent=debug &#124; python3 -c &quot;import sys,json; d=json.load(sys.stdin); assert d[&#x27;summary&#x27;][&#x27;mirrorSyncStatus&#x27;]==&#x27;all-aligned&#x27;; print(&#x27;PASS&#x27;)&quot; | `status: "complete"` at root level; `surfaces.canonical.exists: true`; `surfaces.mirrors` array with 2 entries, each with `syncStatus: "aligned"`; `summary.totalSurfaces >= 20`; `summary.mirrorSyncStatus: "all-aligned"`; `summary.commandCount >= 1`; `summary.skillCount >= 1`; Exit code is 0 | `terminal transcript, command output, generated files, and PASS/FAIL verdict` | All mirrors report `syncStatus: "aligned"`, `summary.mirrorSyncStatus` equals `"all-aligned"`, `commandCount > 0`, `skillCount > 0`, and exit code is 0. | If `surfaces.canonical.exists` is `false`: check that the debug agent file exists at `.skilled/agents/debug.md`<br>If any mirror has `syncStatus` other than `&quot;aligned&quot;`: compare agent definitions across `.claude/agents/`, `.skilled/agents/`, and `.skilled/agents/` for inconsistencies<br>If `summary.commandCount` is 0: verify debug-related command definitions exist in `.skilled/skills/*/commands/` or `.agents/commands/`<br>If the script errors: verify Node.js version and that `scan-integration.cjs` has no syntax errors |
...
| `integration-scanner/scan-known-agent.md` | Canonical per-feature execution contract |
...
| `../../scripts/agent-improvement/scan-integration.cjs` | Implementation or verification anchor referenced by this scenario |
| `.claude/agents/` | Implementation or verification anchor referenced by this scenario |
| `.skilled/agents/` | Implementation or verification anchor referenced by this scenario |
| `.skilled/agents/` | Implementation or verification anchor referenced by this scenario |
| `.skilled/agents/debug.md` | Implementation or verification anchor referenced by this scenario |
| `.skilled/skills/*/commands/` | Implementation or verification anchor referenced by this scenario |
...
- Feature file path: `integration-scanner/scan-known-agent.md`
