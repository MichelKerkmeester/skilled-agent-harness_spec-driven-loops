...
1. `sed -n '104,126p' .skilled/skills/system-deep-loop/deep-improvement/SKILL.md` - confirm the `FULL_SETUP` keyword list and resource map.
2. `for p in references/shared/quick-reference.md assets/agent-improvement/improvement-charter.md assets/agent-improvement/improvement-strategy.md; do test -e ".skilled/skills/system-deep-loop/deep-improvement/$p" && echo "OK $p" || echo "MISS $p"; done` - confirm every expected resource resolves on disk.
