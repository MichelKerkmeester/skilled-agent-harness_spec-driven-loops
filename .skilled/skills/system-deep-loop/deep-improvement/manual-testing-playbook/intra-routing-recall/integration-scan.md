...
1. `sed -n '104,126p' .skilled/skills/system-deep-loop/deep-improvement/SKILL.md` - confirm the `INTEGRATION_SCAN` keyword list and resource map.
2. `for p in references/shared/quick-reference.md references/agent-improvement/integration-scanning.md references/model-benchmark/evaluator-contract.md references/agent-improvement/profiling-audit-log.md; do test -e ".skilled/skills/system-deep-loop/deep-improvement/$p" && echo "OK $p" || echo "MISS $p"; done` - confirm every expected resource resolves on disk.
...
2. If a different intent wins, compare keyword overlap against the other seven intents (QUICK_REFERENCE, LOOP_EXECUTION, EVALUATION_POLICY, PROMOTION_OPERATIONS, TARGET_ONBOARDING, MODEL_BENCHMARK, FULL_SETUP) before assuming a routing defect.
