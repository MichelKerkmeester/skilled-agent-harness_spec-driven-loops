---
title: "RT-023 -- Audit Journal Lifecycle Event Emission"
...
This document captures the canonical manual-testing contract for `RT-023`.

---

## 1. OVERVIEW

This scenario validates that the improvement journal captures events for each lifecycle boundary: `session_initialized`, `candidate_generated`, `candidate_scored`, `legal_stop_evaluated`, and `session_end`.
...
- Real user request: `` Validate that the improvement journal captures events for each lifecycle boundary: `session_initialized`, `candidate_generated`, `candidate_scored`, `legal_stop_evaluated`, and `session_end`. ``
- Prompt: `Validate that the improvement journal emits every required lifecycle boundary event.`
...
- Expected signals: `improvement-journal.jsonl` file created at the configured journal path; Events appear in chronological order (each has a `timestamp` field); At minimum, the following event types are present after a 1-iteration run:; `session_initialized` (session initialization); `candidate_generated` (candidate proposal); `candidate_scored` (scoring output); `legal_stop_evaluated` (legal stop gate evaluation); `session_ended` or `session_end` (session termination); Each event has `eventType`, `timestamp`, and relevant `details`; No duplicate `session_initialized` events for a single session
...
|---|---|---|---|---|---|---|---|---|
| RT-023 | Audit Journal Lifecycle Event Emission | Validate Audit Journal Lifecycle Event Emission | `Validate that the improvement journal emits every required lifecycle boundary event.` | /deep:agent-improvement &quot;.skilled/agents/debug.md&quot; :confirm --spec-folder={spec} --iterations=1<br><br><br>Verification:<br><br><br>node .skilled/skills/system-deep-loop/deep-improvement/scripts/shared/improvement-journal.cjs --read {spec}/improvement/improvement-journal.jsonl &#124; python3 -c &quot;<br>import sys, json<br>events = json.load(sys.stdin)<br>types = [e[&#x27;eventType&#x27;] for e in events]<br>REQUIRED = [&#x27;session_initialized&#x27;, &#x27;candidate_generated&#x27;, &#x27;candidate_scored&#x27;, &#x27;legal_stop_evaluated&#x27;]<br>END_TYPES = [&#x27;session_ended&#x27;, &#x27;session_end&#x27;]<br>for rt in REQUIRED:<br>    assert rt in types, f&#x27;Missing lifecycle event: {rt}&#x27;<br>assert any(t in types for t in END_TYPES), &#x27;Missing session_ended/session_end event&#x27;<br>print(f&#x27;PASS - {len(events)} events, types: {sorted(set(types))}&#x27;)<br>&quot; | `improvement-journal.jsonl` file created at the configured journal path; Events appear in chronological order (each has a `timestamp` field); At minimum, the following event types are present after a 1-iteration run:; `session_initialized` (session initialization); `candidate_generated` (candidate proposal); `candidate_scored` (scoring output); `legal_stop_evaluated` (legal stop gate evaluation); `session_ended` or `session_end` (session termination); Each event has `eventType`, `timestamp`, and relevant `details`; No duplicate `session_initialized` events for a single session | `terminal transcript, command output, generated files, and PASS/FAIL verdict` | After a single-iteration improvement run, the journal contains at least the five required lifecycle boundary events in chronological order, each with valid structure as enforced by `validateEvent()`. | If journal file is missing: verify `journal.enabled: true` in improvement config and the `journal.path` is correct<br>If a specific event type is absent: trace the orchestrator flow to find where `emitEvent()` should be called for that stage<br>If events are out of order: check timestamp generation (should use `new Date().toISOString()` at emit time)<br>If events have invalid structure: run `validateEvent()` on each event and check the returned errors |
...
| `runtime-truth/audit-journal-emission.md` | Canonical per-feature execution contract |
...
| `.skilled/agents/debug.md` | Implementation or verification anchor referenced by this scenario |
...
- Feature file path: `runtime-truth/audit-journal-emission.md`
