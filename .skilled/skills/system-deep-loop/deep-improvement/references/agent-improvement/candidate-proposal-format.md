...
```bash
node .skilled/skills/system-deep-loop/deep-improvement/scripts/agent-improvement/score-candidate.cjs \
  --candidate={spec_folder}/improvement/candidates/{candidate_id}.md \
  --target={canonical_target_path} \
  --manifest=assets/agent-improvement/target-manifest.jsonc
```
...
| `assets/agent-improvement/improvement-charter.md` | Charter template (read during generation) |
| `assets/agent-improvement/target-manifest.jsonc` | Target boundary definition |
| `feature-catalog/evaluation-loop/candidate-generation.md` | Feature catalog entry |
