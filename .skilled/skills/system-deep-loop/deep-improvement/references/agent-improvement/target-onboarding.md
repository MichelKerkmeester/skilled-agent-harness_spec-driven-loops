...
1. (Optional) Add the manifest entry in `assets/agent-improvement/target-manifest.jsonc` if the target needs explicit classification or guardrail protections
2. Run `generate-profile.cjs` to derive a scoring profile from the agent's own structure and rules:

   ```text
   node scripts/agent-improvement/generate-profile.cjs --agent=.skilled/agents/{name}.md
   ```
...
- `../../assets/agent-improvement/target-manifest.jsonc`
