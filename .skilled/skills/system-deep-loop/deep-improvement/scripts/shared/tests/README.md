...
Run the deep-improvement Vitest battery from `.skilled/skills/system-deep-loop/deep-improvement/scripts/` with `npx --yes vitest@4.1.6 run --config vitest.config.mjs`; the versioned `npx` package supplies the missing Vitest binary without editing `.skilled/package.json` or any lockfile, and running from `scripts/` keeps config resolution on the local `vitest.config.mjs`.
