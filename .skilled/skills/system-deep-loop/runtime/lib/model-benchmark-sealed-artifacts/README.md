---
title: "model-benchmark-sealed-artifacts: Sealed Artifacts"
description: "Model Benchmark sealed-artifacts layer for the durable runtime event spine."
trigger_phrases:
  - "model-benchmark-sealed-artifacts"
  - "model benchmark sealed-artifacts"
---
...
This folder materializes canonical, sealed artifacts from lane evidence for the Model Benchmark runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `model-benchmark-artifact-material.ts` | Canonical materialization of lane artifact content. |
| `model-benchmark-sealed-artifact-types.ts` | Sealed artifact kinds, envelopes and storage contract types. |
| `model-benchmark-sealed-artifacts.ts` | Sealing, parsing, storage and verification helpers. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `model-benchmark-sealed-artifacts.ts` |
...
This folder owns the sealed artifacts step for Model Benchmark. It does not replace the shared substrate or decide consumer-facing workflow policy.
