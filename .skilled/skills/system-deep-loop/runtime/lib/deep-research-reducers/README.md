---
title: "deep-research-reducers: Reducers"
description: "Deep Research reducers layer for the durable runtime event spine."
trigger_phrases:
...
This folder folds verified lane events into deterministic projection state for the Deep Research runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-research-projection-schema.ts` | Projection field ownership, schema versions and guards. |
| `deep-research-projection-types.ts` | Projection state and reducer contract types. |
| `deep-research-reducer.ts` | Deterministic event folding and legacy projection logic. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `deep-research-reducer.ts` |
...
This folder owns the reducers step for Deep Research. It does not replace the shared substrate or decide consumer-facing workflow policy.
