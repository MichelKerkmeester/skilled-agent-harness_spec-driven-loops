---
title: "deep-ai-council-reducers: Reducers"
description: "Deep Ai Council reducers layer for the durable runtime event spine."
trigger_phrases:
  - "deep-ai-council-reducers"
  - "deep-ai-council reducers"
---
...
This folder folds verified lane events into immutable projection state for the Deep Ai Council runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-ai-council-projection-schema.ts` | Projection shape checks and immutable-state guards. |
| `deep-ai-council-projection-types.ts` | Lane-specific types, constants and contract values. |
| `deep-ai-council-reducer.ts` | projection reduction, integrity checks and legacy-view projection |
...
| Primary implementation | `deep-ai-council-reducer.ts` | projection reduction, integrity checks and legacy-view projection. |
...
This folder owns the reducers step for Deep Ai Council. It does not replace the shared substrate or decide consumer-facing workflow policy.
