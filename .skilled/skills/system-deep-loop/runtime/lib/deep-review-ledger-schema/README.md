---
title: "deep-review-ledger-schema: Ledger Schema"
description: "Deep Review ledger-schema layer for the durable runtime event spine."
trigger_phrases:
  - "deep-review-ledger-schema"
  - "deep review ledger-schema"
---
...
This folder defines the lane's durable event and ledger payload contract for the Deep Review runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-review-ledger-schema.ts` | Lane event definitions, payload builders and schema guards. |
| `deep-review-ledger-types.ts` | Lane-specific event, payload and compatibility types. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `deep-review-ledger-schema.ts` |
...
This folder owns the ledger schema step for Deep Review. It does not replace the shared substrate or decide consumer-facing workflow policy.
