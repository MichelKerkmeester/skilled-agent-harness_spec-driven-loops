---
title: "deep-review-reducers: Reducers"
description: "Deep Review reducers layer for the durable runtime event spine."
trigger_phrases:
  - "deep-review-reducers"
  - "deep review reducers"
---
...
This folder folds verified lane events into deterministic projection state for the Deep Review runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-review-projection-schema.ts` | Projection field ownership, schema versions and guards. |
| `deep-review-projection-types.ts` | Projection state and reducer contract types. |
| `deep-review-reducer.ts` | Deterministic event folding and legacy projection logic. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `deep-review-reducer.ts` |
...
This folder owns the reducers step for Deep Review. It does not replace the shared substrate or decide consumer-facing workflow policy.
