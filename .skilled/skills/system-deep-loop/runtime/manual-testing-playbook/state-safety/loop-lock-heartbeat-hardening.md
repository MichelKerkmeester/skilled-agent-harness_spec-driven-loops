...
description: "Manual validation scenario for Loop-lock heartbeat hardening in the runtime/ skill."
version: 1.4.0.15
---
...
- Real user request: `Validate Loop-lock heartbeat hardening and report whether the current source, script surface, and tests agree with the runtime/ contract.`
...
- Feature catalog entry exists at `feature-catalog/state-safety/loop-lock-heartbeat-hardening.md`.
...
- Source excerpts from `lib/deep-loop/loop-lock.ts` showing the anchors named in the commands above, read from the current files rather than recalled.
- Captured stdout and exit status for every command run in this section.
- Output from `tests/unit/loop-lock.vitest.ts` naming the assertions that carry the expected signals.
...
- Group: State safety
- Playbook ID: DLR-036
- Feature catalog entry: `feature-catalog/state-safety/loop-lock-heartbeat-hardening.md`
- Scenario file path: `manual-testing-playbook/state-safety/loop-lock-heartbeat-hardening.md`
