...
description: "Manual validation scenario for Loop lock in the runtime/ skill."
version: 1.4.0.4
---
...
- Real user request: `Validate Loop lock and report whether the current source, script surface, and tests agree with the runtime/ contract.`
...
- Feature catalog entry exists at `feature-catalog/state-safety/loop-lock.md`.
...
1. Run `cd .skilled/skills/system-deep-loop/runtime/ && PATH=/opt/homebrew/bin:$PATH npm test -- tests/unit/loop-lock.vitest.ts` and require EXIT 0.
...
- Group: State safety
- Playbook ID: DLR-008
- Feature catalog entry: `feature-catalog/state-safety/loop-lock.md`
