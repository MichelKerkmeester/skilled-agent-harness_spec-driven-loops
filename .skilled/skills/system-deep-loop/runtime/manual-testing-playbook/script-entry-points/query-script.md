...
- Feature catalog entry exists at `feature-catalog/script-entry-points/query-script.md`.
...
- Source excerpts from `scripts/query.cjs` showing the anchors named in the commands above, read from the current files rather than recalled.
- Captured stdout and exit status for every command run in this section.
- Output from `tests/integration/query-script.vitest.ts` naming the assertions that carry the expected signals.
...
- Playbook ID: DLR-016
- Feature catalog entry: `feature-catalog/script-entry-points/query-script.md`
- Scenario file path: `manual-testing-playbook/script-entry-points/query-script.md`
