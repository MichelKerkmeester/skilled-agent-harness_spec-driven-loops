...
3. Run `bash: cd .skilled/skills/system-deep-loop/runtime/ && PATH=/opt/homebrew/bin:$PATH npm test -- tests/unit/coverage-graph-query.vitest.ts` and require EXIT 0.
...
- Output from `tests/unit/coverage-graph-query.vitest.ts` naming the assertions that carry the expected signals.
