...
3. `bash: cd .skilled/skills/system-spec-kit/runtime && npx vitest run ../../runtime//tests/unit/fanout-salvage.vitest.ts`
...
1. Run `cd .skilled/skills/system-deep-loop/runtime/ && PATH=/opt/homebrew/bin:$PATH npm test -- tests/unit/fanout-run.vitest.ts` and require EXIT 0.
