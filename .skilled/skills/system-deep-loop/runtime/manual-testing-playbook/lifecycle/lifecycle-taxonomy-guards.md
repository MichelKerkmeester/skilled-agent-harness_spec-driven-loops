...
- Feature catalog entry exists at `feature-catalog/lifecycle/lifecycle-taxonomy-guards.md`.
...
- Source excerpts from `lib/deep-loop/lifecycle-taxonomy.cjs` showing the anchors named in the commands above, read from the current files rather than recalled.
- Captured stdout and exit status for every command run in this section.
- Output from `tests/unit/lifecycle-taxonomy-guards.vitest.ts` naming the assertions that carry the expected signals.
...
- Group: Lifecycle
- Playbook ID: DLR-034
- Feature catalog entry: `feature-catalog/lifecycle/lifecycle-taxonomy-guards.md`
- Scenario file path: `manual-testing-playbook/lifecycle/lifecycle-taxonomy-guards.md`
