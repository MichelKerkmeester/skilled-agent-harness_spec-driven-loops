...
- Registry evidence: every deep-loop mode requires a matching `workflowMode` such as `research`, `review`, `ai-council`, `agent-improvement`, or `model-benchmark`; the prompt does not express any of those controls.
