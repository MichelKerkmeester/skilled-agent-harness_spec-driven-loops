...
Run `shouldContinue_review()` (see `../deep-research/references/convergence/convergence.md` Section 10.3):
...
- `cli-claude-code` (spec 019): `claude -p "$(cat prompt)" --model X --permission-mode acceptEdits --output-format text` with optional `--effort Y`. Default permission-mode is `plan` (read-only). We override to `acceptEdits` so iteration writes succeed. The inline workflow branches are not the complete schema: `cli-codex` is also an inline branch, while `cli-cursor`, `cli-devin`, `cli-pi`, and `cli-hermes` use the shared fan-out adapters; the executor-kind authority is `runtime/lib/deep-loop/executor-config.ts` (`EXECUTOR_KINDS`).
...
- **Write containment attributes every new dirty path outside the lineage directory to the lineage, and reports it rather than undoing it.** Attribution on a shared checkout is a guess: a tracked file changed anywhere in the same checkout during a lineage window looks identical whether the lineage, an operator, or another session wrote it. So the default remedy is preserve. The file is left byte-for-byte as it stands, the finding is recorded on the status ledger and the observability stream, and a lane that produced its review artifacts — its iteration records, and `review-report.md` where it reached synthesis — still settles as complete, carrying `completed_with_containment_advisory`. Restoring is opt-in and safe only on a checkout with a single writer; under it a path that was already dirty when the lineage started is put back to its pre-dispatch bytes, never to HEAD, and a path whose baseline could not be captured is preserved regardless. Untracked files are never deleted in either mode. One exemption still applies to attribution: another run's lineage directory under the same top-level packet is treated as that run's own write surface while its loop lock is live, so two lineages can run in sibling phases of one packet at the same time; a directory with no lock, or a stale one, is still attributed here. The runtime records each finding's diff under the lineage directory at `containment/quarantine/<iteration>[-attempt-<n>]/`, in a `containment-out-of-scope/` subdirectory on the default preserve path and `containment-reverted/` only under restore, where bytes really were rolled back; each pass keeps its own directory so a retry never overwrites an earlier pass's evidence. The pre-dispatch baseline bytes go to `<lineage dir>/containment/baseline/`, so an out-of-scope change stays recoverable either way.
...
Each new P0/P1 must carry a typed packet with the following required fields (see `../state/state-format.md` §9 for the full schema and a worked example):
...
Generate the 9 core review-report sections (defined in `review-mode-contract.yaml` under `outputs.reviewReport`) and insert `## Resource Map Coverage Gate` between `Traceability Status` and `Deferred Items` when `config.resource_map_present == true`:
...
Apply the verdict contract from `review-mode-contract.yaml`:
...
| `restart` | fresh | `prior + 1` | prior `review/` tree moved under `review-archive/{timestamp}/` | `restarted` (same field set plus non-null `archivedPath`) | operator wants to clear the workspace and replay with a new angle |
...
In confirm mode (`deep-review-confirm.yaml`), the workflow adds explicit approval gates instead of relying on the pause sentinel:
...
## 8. STATE TRANSITIONS, ERROR HANDLING, AND QUALITY GATES

State-transition rules, error handling, and the review quality gates live in `loop-state-and-gates.md` for navigability.
...
Protocol documents from `deep-research` are cross-referenced, not duplicated:

| Document | Path | Purpose |
|----------|------|---------|
| Loop Protocol (research) | `../deep-research/references/protocol/loop-protocol.md` | Research loop lifecycle (this file adapts Section 9) |
| State Format | `../deep-research/references/state/state-format.md` | JSONL, strategy, and config schemas |
| Convergence | `../deep-research/references/convergence/convergence.md` | `shouldContinue_review()` (Section 10), stuck recovery, quality guards |

### Review-Specific Resources

| Resource | Path | Purpose |
|----------|------|---------|
| Review Contract | `assets/review-mode-contract.yaml` | Single source of truth for dimensions, verdicts, gates, protocols |
| Quick Reference | `references/protocol/quick-reference.md` | One-page review cheat sheet |
| Strategy Template | `assets/deep-review-strategy.md` | Template for review strategy files |
| Config Template | `assets/deep-review-config.json` | Template for review config files |
| Dashboard Template | `assets/deep-review-dashboard.md` | Template for auto-generated dashboard |
...
Executor settings are owned by the YAML workflow and rendered prompt pack. Do not bypass the workflow by hand-dispatching review iterations; each iteration stays LEAF-only and produces the required markdown plus JSONL delta.

### Invariants

Every executor path MUST satisfy:
...
- Missing iteration file: `iteration_file_missing` from `post-dispatch-validate.ts`, emits a `schema_mismatch` conflict event.
...
Non-native executor runs append an `executor` block with model and runtime settings to the iteration's JSONL record via `executor-audit.ts`. Native runs are recoverable from YAML alone.

### Template And Config Surface

The executor-agnostic iteration prompt lives at `assets/prompt-pack-iteration.md.tmpl`, rendered by `prompt-pack.ts` before dispatch and either injected as the agent's context (native) or piped to `opencode run` stdin (cli-opencode). Config is in `assets/deep-review-config.json` under the `executor` key; schema in `runtime//lib/deep-loop/executor-config.ts`. CLI flag precedence: `--executor/--model/--reasoning-effort/--service-tier/--executor-timeout > config file > schema default`.
...
- Convergence detection, lifecycle events (new/resume/restart), the review dimensions pass, and stuck_recovery stay YAML-driven.
