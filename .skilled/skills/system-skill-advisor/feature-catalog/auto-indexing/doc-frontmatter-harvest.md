...
| File | Layer | Role |
|---|---|---|
| `.skilled/skills/system-skill-advisor/runtime/lib/skill-graph/doc-frontmatter.ts` | Library | Flag gate, tier weights, list-aware parser, capped walker |
| `.skilled/skills/system-skill-advisor/runtime/lib/skill-graph/skill-graph-db.ts` | Library | `skill_docs` DDL + `harvestSkillDocs()` + scan-result counters |
| `.skilled/skills/system-skill-advisor/runtime/lib/scorer/projection.ts` | Library | Flag-gated, missing-table-tolerant doc-trigger projection load |
| `.skilled/skills/system-skill-advisor/runtime/lib/scorer/lanes/derived.ts` | Library | Top-3 tier-weighted doc scoring, 0.45 cap, docs-first evidence |
| `.skilled/skills/system-skill-advisor/runtime/handlers/advisor-recommend.ts` | Handler | Sanitized `matchedDocs` extraction, `doc_reference_signal` evidence type |
| `.skilled/skills/system-skill-advisor/runtime/lib/daemon/watcher.ts` | Daemon | `doc-frontmatter` watch targets when the flag is on |
| `.skilled/bin/system-skill-advisor-launcher.cjs` | Launcher | `SPECKIT_ADVISOR_DOC_TRIGGERS` in `CHILD_ENV_ALLOWLIST` |
| `.skilled/skills/system-skill-advisor/runtime/scripts/skill_advisor.py` | Compat | Gate-2 parity harvest merged into both graph-loader paths |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `.skilled/skills/system-skill-advisor/runtime/tests/skill-doc-harvest.vitest.ts` | Automated test | Parser, walker, flag-gated lifecycle, capping, invariance, sanitization (11 cases) |
...
- [derived-extraction.md](../../feature-catalog/auto-indexing/derived-extraction.md).
- [anti-stuffing.md](../../feature-catalog/auto-indexing/anti-stuffing.md).
