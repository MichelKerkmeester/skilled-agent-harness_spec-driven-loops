...
4. **CLI front door.** The hook calls `buildSkillAdvisorBriefFromCli`, which runs `skill-advisor.cjs advisor_recommend --json ... --no-warm-only` within the `SPECKIT_CLAUDE_HOOK_TIMEOUT_MS` budget (2,500 ms by default). The Claude shim sets the child budget to 2,200 ms when the operator has not set the variable. An operator value above about 2,200 ms lets the shim kill the hook at 2,500 ms first, so the turn gets `{}`. The CLI owns the warm-daemon probe, starts the daemon when the socket is cold and falls back to the local Python scorer with a `degraded` result. The request carries `includeCompiledRoute: false`, so the daemon skips compiled-route enrichment and starts no `compiled-route.cjs` child. A daemon still running an older build rejects that option with JSON-RPC `-32602` and the CLI retries once without it.
5. **Render.** `renderAdvisorBrief` produces the model-visible `Advisor:` text. When there is no brief, the handler emits `renderAdvisorFallbackDirective`: one status line, then the directives block. The line reads `Advisor: outage (<status>)` with the command to route by hand when the advisor failed or timed out, `Advisor: no skill matched.` when it answered with nothing above threshold, and `Advisor: prompt skipped.` when the prompt gate declined. Raw prompt text is never persisted in diagnostics, cache metadata, status, or attribution.
6. **Directive-lifecycle dedup.** `decideDirectiveLifecycleDelivery` suppresses a re-delivery of the same directive to the same session within its cadence, so a repeated prompt does not re-inject an identical brief. A repeat in a known session keeps the first line, a route or a fallback status, and drops the directives block.
...
| **Pi** | `system-skill-advisor/hooks/pi/prompt-advisor.ts` (real file; `.pi/extensions/` symlinks here) | `user_input` / prompt event via `.pi/extensions/` | Imports the advisor module **in-process** (not a subprocess). Pi awaits input handlers before agent processing, so the old blocking-spawn bridge stalled every send; in-process calls remove the stall and let the module-level prompt cache work. Captures raw user text (bounded 32 KiB, 64-session LRU). The in-process call passes `runtime: 'pi'` and races a timer of the advisor budget plus 300 ms, delivering the fallback directives if the call hangs | Model-visible message; delivery-state machine suppresses same-session re-delivery |
...
| `system-skill-advisor/hooks/pi/prompt-advisor.ts` | Native Pi extension. Imports the advisor module in-process, captures raw user text (bounded LRU), runs a delivery-state machine that suppresses same-session re-delivery, and sends the brief as a model-visible message. Races the advisor call against a deadline of the advisor budget plus 300 ms and delivers the fallback directives if it hangs. |
...
| Dedup | Directive-lifecycle dedup suppresses re-delivery of the same directive to the same session within its cadence. OpenCode adds transform-dedup across co-resident transforms. The plugin decides same-message transform dedup on the full advisor block before lifecycle reduction, so a repeated transform for the same message is suppressed. |
| Diagnostics | Each hook record carries `runtime` (the Codex, Cursor and Devin adapters set `SPECKIT_RUNTIME` and Pi passes `pi`) plus `emittedBytes` and `directivesSuppressed`. Records are written only with `SKILL_ADVISOR_DEBUG` set, and the subprocess hook waits up to 100 ms for pending writes before exit. |
