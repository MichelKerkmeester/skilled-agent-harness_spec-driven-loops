...
run_from_repo python3 .skilled/skills/system-skill-advisor/runtime/scripts/skill_graph_compiler.py --validate-only
...
if run_from_repo node .skilled/bin/skill-advisor.cjs skill_graph_scan --trusted --warm-only --format json --timeout-ms 60000 >/dev/null 2>&1; then
    ...
    run_from_repo node --input-type=module -e "
        const { indexSkillMetadata } = await import('${REPO_ROOT}/.skilled/skills/system-skill-advisor/runtime/dist/runtime/lib/skill-graph/skill-graph-db.js');
        const result = indexSkillMetadata('.skilled/skills');
...
run_from_repo python3 .skilled/skills/system-skill-advisor/runtime/scripts/skill_graph_compiler.py --export-json

log_step "Running advisor health check"
run_from_repo python3 .skilled/skills/system-skill-advisor/runtime/scripts/skill_advisor.py --health
