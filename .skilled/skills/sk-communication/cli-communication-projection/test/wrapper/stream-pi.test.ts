// ───────────────────────────────────────────────────────────────────
// MODULE: Pi Print-Mode Wrapper Tests
// ───────────────────────────────────────────────────────────────────
...
describe('pi stream capture', () => {
  it('captures print-mode output, projects, and re-renders', async () => {
    process.env.COMMUNICATION_PROJECTION_ENABLED = '1';
    const transport = vi.fn(makeProjectingTransport());

    const result = await projectRuntimeStream('pi', {
      capturedText: SOURCE,
