...
`.opencode/scripts/git-hooks/` holds the hook scripts this repo installs into `.git/hooks/`. Hooks here are advisory-first: each one's primary check has its own bypass env var, though `pre-commit` layers a few genuinely blocking sub-gates on top of its advisory headline check.
...
| Boundary | Rule |
|---|---|
| Blocking vs advisory | Only `pre-commit`'s four named sub-gates may fail a commit. Every other check in this folder is advisory or best-effort (`\|\| true` on the marker call). |
