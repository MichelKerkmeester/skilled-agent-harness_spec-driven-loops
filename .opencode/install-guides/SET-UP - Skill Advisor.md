...
> **Ownership:** As of `013/009/008`, `skill_graph_*` tools are served by the `system_skill_advisor` MCP server, not `system-spec-memory`. Operations remain identical; public tool ids are unchanged.
...
5. On test failure: run the per-run rollback script that Phase 3 generated under <packet_scratch>/rollback-<timestamp>.sh
   ...
   The script also runs npm --prefix .opencode/skills/system-skill-advisor/mcp-server run build at the end.
```
...
| MCP server is built | `ls .opencode/skills/system-skill-advisor/mcp-server/dist/` shows JS |
| `skill_graph_scan` available | Tool appears in your AI client's tool list |
| Baseline tests pass | `npm --prefix .opencode/skills/system-skill-advisor/mcp-server test` is green |
...
2. **Re-index the SQLite graph** (REQUIRED — without this, signal edits have ZERO effect on routing):
    ...
    - Or via Python compatibility: `python3 .opencode/skills/system-skill-advisor/mcp-server/scripts/skill_graph_compiler.py --export-json --pretty` then re-index through MCP or the trusted CLI form above
...
> **Critical**: The advisor reads scoring inputs from `.opencode/skills/system-skill-advisor/mcp-server/database/skill-graph.sqlite`, NOT from `graph-metadata.json` directly. Editing JSON without running `skill_graph_scan` will produce identical pre-edit scores.
...
```bash
# Tests still green
npm --prefix .opencode/skills/system-skill-advisor/mcp-server test
...
| Cannot parse `explicit.ts` | `git restore --source=HEAD -- .opencode/skills/system-skill-advisor/mcp-server/lib/scorer/lanes/explicit.ts` (restores from HEAD without affecting unrelated WIP) |
| MCP server missing | `npm --prefix .opencode/skills/system-spec-kit/mcp-server install && npm --prefix .opencode/skills/system-skill-advisor/mcp-server run build` |
...
- **Operator setup + native MCP install (merged 2026-05-15):** `.opencode/skills/system-skill-advisor/INSTALL-GUIDE.md`
