// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: system-completion-sentinel Regression Tests                       ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
// ║          ing system-spec-gate.js's own session.created sweep contract. Also  ║
...
const KILL_SWITCH_ENV = 'SYSTEM_COMPLETION_SENTINEL_DISABLED';
const pluginUrl = pathToFileURL(path.join(__dirname, '..', 'system-completion-sentinel.js')).href;
...
function makeProjectDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'system-completion-sentinel-test-'));
}
...
test('kill switch (SYSTEM_COMPLETION_SENTINEL_DISABLED=1) makes session.created a full no-op, sweep included', async () => {
  const { default: MkCompletionSentinelPlugin } = await loadPlugin();
  const projectDir = makeProjectDir();
  try {
