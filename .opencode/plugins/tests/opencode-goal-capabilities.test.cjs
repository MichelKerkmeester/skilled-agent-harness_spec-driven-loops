// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: opencode-goal Capability Tests                                    ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
async function withState(fn) {
  ...
  const stateDir = await mkdtemp(join(tmpdir(), 'opencode-goal-capabilities-'));
  const originalAutonomy = process.env.OPENCODE_GOAL_AUTONOMY;
  const originalMaxAutoTurns = process.env.OPENCODE_GOAL_MAX_AUTO_TURNS;
  const originalMaxWallMs = process.env.OPENCODE_GOAL_MAX_WALL_MS;

  try {
    return await fn({ helpers, pluginModule, stateDir });
  } finally {
    restoreEnv('OPENCODE_GOAL_AUTONOMY', originalAutonomy);
    restoreEnv('OPENCODE_GOAL_MAX_AUTO_TURNS', originalMaxAutoTurns);
    restoreEnv('OPENCODE_GOAL_MAX_WALL_MS', originalMaxWallMs);
...
test('history lists archived goals and leaves active state untouched', async () => withState(async ({ helpers, pluginModule, stateDir }) => {
  ...
  const emptyHistory = await plugin.tool.opencode_goal.execute({ action: 'history' }, {});
  ...
  const history = await plugin.tool.opencode_goal.execute({ action: 'history' }, {});
...
test('env caps configure new goals and status exposes remaining budgets', async () => withState(async ({ helpers, stateDir }) => {
  process.env.OPENCODE_GOAL_MAX_AUTO_TURNS = '3';
  process.env.OPENCODE_GOAL_MAX_WALL_MS = '4000';
...
test('goal command documentation routes new verbs and budget validation', async () => {
  ...
  assert.match(commandDoc, /opencode_goal\(\{ action: "history" \}\)/);
  assert.match(commandDoc, /opencode_goal\(\{ action: "doctor" \}\)/);
  assert.match(commandDoc, /opencode_goal\(\{ action: "health" \}\)/);
  assert.match(commandDoc, /opencode_goal\(\{ action: "resume" \}\)/);
