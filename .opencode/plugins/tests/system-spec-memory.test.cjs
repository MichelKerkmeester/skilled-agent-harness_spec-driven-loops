// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: system-spec-memory Regression Tests                              ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
async function statusForHome(home, configSetup) {
  ...
  try {
    ...
    return hooks.tool.system_spec_memory_status.execute();
  } finally {
...
function configPath(home) {
  return path.join(home, '.config', 'opencode', 'plugin', 'system-spec-memory.json');
}
...
function runUserPromptFixture(targetSource, input, timeout = 5000) {
  const root = makeTempDir('system-spec-memory-prompt');
...
test('reports missing, malformed, unreadable, and valid configuration safely', async () => {
  const roots = Array.from({ length: 4 }, (_, index) => makeTempDir(`system-spec-memory-config-${index}`));
  try {
...
test('uses stable bounded markers and keeps ordinary message events inside the TTL', async () => {
  const root = makeTempDir('system-spec-memory-cache');
...
`);
  try {
    ...
    assert.match(output.system[0], /\[system-spec-memory:continuity:[a-f0-9]{16}\]$/);
...
test('does not let an invalidated in-flight result replace a newer lookup', async () => {
  const root = makeTempDir('system-spec-memory-race');
  const countPath = path.join(root, 'count');
...
test('accepts bridge stdout at the ceiling and rejects the first byte over it', async () => {
  const root = makeTempDir('system-spec-memory-output');
...
`);
  try {
    ...
    const status = await rejectedHooks.tool.system_spec_memory_status.execute();
...
test('fails open when the bridge exits before consuming stdin', async () => {
  const root = makeTempDir('system-spec-memory-epipe');
...
test('status names the runtime continuity capability boundaries', async () => {
  const root = makeTempDir('system-spec-memory-capability');
  const binary = writeExecutable(root, 'bridge.js', `
process.stdin.resume();
...
`);
  try {
    const hooks = await createPlugin(binary, { directory: root });
    const status = await hooks.tool.system_spec_memory_status.execute();
...
test('same-message continuity contributions are suppressed only after the first delivery', async () => {
  const root = makeTempDir('system-spec-memory-dedup');
  const binary = writeExecutable(root, 'bridge.js', `
process.stdin.resume();
...
`);
  try {
    ...
    assert.deepEqual(receipt.transforms.map((entry) => ({
      ...
      { transform: 'system-spec-memory', outcome: 'delivered' },
      { transform: 'system-spec-memory', outcome: 'suppressed_duplicate' },
    ]);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
...
test('distinct continuity messages with identical text both receive full delivery', async () => {
  const root = makeTempDir('system-spec-memory-distinct');
  const binary = writeExecutable(root, 'bridge.js', `
process.stdin.resume();
...
test('flag-off continuity delivery preserves repeated output byte-for-byte', async () => {
  const root = makeTempDir('system-spec-memory-flag-off');
  const binary = writeExecutable(root, 'bridge.js', `
process.stdin.resume();
...
test('unresolvable continuity identity fails open with full delivery', async () => {
  const root = makeTempDir('system-spec-memory-unresolved');
  const binary = writeExecutable(root, 'bridge.js', `
process.stdin.resume();
...
test('a throwing system push releases the transform reservation', async () => {
  const root = makeTempDir('system-spec-memory-throwing-push');
  const binary = writeExecutable(root, 'bridge.js', `
process.stdin.resume();
