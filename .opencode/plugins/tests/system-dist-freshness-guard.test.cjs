// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: system-dist-freshness-guard Regression Tests                     ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
async function loadPlugin() {
  const pluginUrl = pathToFileURL(path.join(__dirname, '..', 'system-dist-freshness-guard.js')).href;
...
test('injects stale and check-error diagnostics without terminal output', async (t) => {
  const tmpDir = temporaryDirectory(t, 'system-dist-freshness-guard-diagnostics-');
...
test('invalidates a warm cache for watched source mutations', async (t) => {
  for (const tool of ['write', 'edit', 'apply_patch']) {
    const tmpDir = temporaryDirectory(t, `system-dist-freshness-guard-${tool}-`);
...
test('bounds session state and rotates the audit log', async (t) => {
  const tmpDir = temporaryDirectory(t, 'system-dist-freshness-guard-retention-');
  writeCodeModeFixture(tmpDir, true);
...
test('evicts the oldest session ID when the deduplication cap is reached', async (t) => {
  const tmpDir = temporaryDirectory(t, 'system-dist-freshness-guard-session-cap-');
  writeAllPackageFixtures(tmpDir);
