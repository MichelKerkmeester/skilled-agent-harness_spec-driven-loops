// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: system-deep-loop-guard Regression Tests                           ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
async function loadPlugin() {
  const pluginUrl = pathToFileURL(path.join(__dirname, '..', 'system-deep-loop-guard.js')).href;
...
async function main() {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'system-deep-loop-guard-'));
  ...
  assert.match(readGuardLog(tmpDir), /system-deep-loop-guard.*mode mismatch/i);
  ...
  process.env['SYSTEM_DEEP_LOOP_GUARD_ACTIVE_RETENTION_DAYS'] = '1';
  ...
  process.env['SYSTEM_DEEP_LOOP_GUARD_ARCHIVE_RETENTION_DAYS'] = '1';
  ...
  delete process.env['SYSTEM_DEEP_LOOP_GUARD_ACTIVE_RETENTION_DAYS'];
  delete process.env['SYSTEM_DEEP_LOOP_GUARD_ARCHIVE_RETENTION_DAYS'];
  delete process.env[WARN_LOG_MAX_BYTES_ENV];

  fs.rmSync(tmpDir, { recursive: true, force: true });
  console.log('system-deep-loop-guard.test.cjs: all assertions passed');
}
