...
| File | Responsibility |
|---|---|
| `cli-dispatch-audit.js` | Records redacted dispatch telemetry after completed CLI calls. |
| `codex-hooks-watchdog.js` | Watches Codex hook installation and reports drift. |
| `sk-communication-projection.js` | Projects assistant text through the `chat.message` hook, gated by enablement and a kill-switch, with byte-exact restore. |
| `system-completion-sentinel.js` | Checks completion evidence at session lifecycle points. |
| `system-deep-loop-guard.js` | Checks deep-loop Task dispatches and repeat handoffs. |
| `system-dist-freshness-guard.js` | Reports stale compiled outputs and invalidates diagnostics. |
| `sk-git-preflight-advisory.js` | Advises on Git command scope before execution. |
| `opencode-goal.js` | Stores and injects session goals and lifecycle state. |
| `mcp-route-guard.js` | Advises when native MCP calls should use Code Mode. |
| `sk-code-post-edit-quality.js` | Runs bounded post-edit quality checks. |
| `system-skill-advisor.js` | Injects skill-routing guidance and exposes advisor status. |
| `system-spec-gate.js` | Classifies and evaluates mutation-gate state. |
| `system-spec-memory.js` | Injects Spec Kit continuity and exposes memory status. |
| `system-speckit-completion.js` | Exposes read-only completion evidence. |
