// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: Spec Memory OpenCode Plugin (system-spec-memory)                  ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
const PLUGIN_ID = 'system-spec-memory';
...
const BRIEF_MARKER_PREFIX = '[system-spec-memory:continuity:';
const BRIEF_MARKER_DIGEST_CHARS = 16;
const MIN_MAX_BRIEF_CHARS = BRIEF_MARKER_PREFIX.length + BRIEF_MARKER_DIGEST_CHARS + 1;
const DISABLED_ENV = 'SYSTEM_SPEC_MEMORY_PLUGIN_DISABLED';
const LEGACY_DISABLED_ENV = 'SPECKIT_SPEC_MEMORY_PLUGIN_DISABLED';
const BRIDGE_PATH = fileURLToPath(new URL('../skills/system-spec-kit/mcp-server/plugin-bridges/system-spec-memory-bridge.mjs', import.meta.url));
const SOURCE_PATHS = [
  BRIDGE_PATH,
...
async function loadConfig() {
  const path = join(homedir(), '.config', 'opencode', 'plugin', 'system-spec-memory.json');
  let raw;
  try {
...
function normalizeOptions(rawOptions) {
  ...
  const envCacheTtlMs = Number(process.env.SYSTEM_SPEC_MEMORY_CACHE_TTL_MS);
  const envBridgeTimeoutMs = Number(process.env.SYSTEM_SPEC_MEMORY_BRIDGE_TIMEOUT_MS);
  const envCliTimeoutMs = Number(process.env.SYSTEM_SPEC_MEMORY_CLI_TIMEOUT_MS);
  const envMaxBriefChars = Number(process.env.SYSTEM_SPEC_MEMORY_MAX_BRIEF_CHARS);
  const envMaxCacheEntries = Number(process.env.SYSTEM_SPEC_MEMORY_MAX_CACHE_ENTRIES);
  ...
  return {
    ...
    nodeBinary: typeof options.nodeBinaryOverride === 'string' && options.nodeBinaryOverride.trim()
      ...
      : (process.env.SYSTEM_SPEC_MEMORY_NODE_BINARY || process.env.SPEC_KIT_PLUGIN_NODE_BINARY || DEFAULT_NODE_BINARY),
    specFolder: typeof options.specFolder === 'string' && options.specFolder.trim()
      ? options.specFolder.trim()
      : (process.env.SYSTEM_SPEC_MEMORY_SPEC_FOLDER || undefined),
...
export default async function MkSpecMemoryPlugin(ctx, rawOptions) {
  ...
  return {
    ...
    tool: {
      system_spec_memory_status: tool({
