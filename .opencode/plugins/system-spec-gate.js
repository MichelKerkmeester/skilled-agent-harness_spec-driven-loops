// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: system-spec-gate OpenCode Plugin (adapter)                        ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
import * as guardCore from '../skills/system-spec-kit/runtime/hooks/lib/spec-gate/spec-gate-core.mjs';
 ...
 * field is ever present). Mirrors system-skill-advisor.js's own ctx.client.session
 ...
 * - SYSTEM_SPEC_GATE_DISABLED=1 makes the whole plugin a full no-op.
 *
  ...
  return {
    ...
    async 'experimental.chat.system.transform'(input, output) {
      try {
        ...
        // all -- so SYSTEM_SPEC_GATE_DISABLED=1 is a genuine full no-op, not just
    ...
    async 'tool.execute.before'(input, output) {
      try {
        ...
        if (result.decision === 'deny') {
          throw new Error(`system-spec-gate: ${result.detail}`);
        }
      } catch (err) {
        if (err instanceof Error && err.message.startsWith('system-spec-gate:')) throw err;
