// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: Skill Advisor OpenCode Plugin (system-skill-advisor)              ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
async function loadConfig() {
  const path = join(homedir(), '.config', 'opencode', 'plugin', 'system-skill-advisor.json');
  try {
...
function normalizeOptions(rawOptions) {
  ...
  const envCacheTTLMs = Number(process.env.SYSTEM_SKILL_ADVISOR_CACHE_TTL_MS);
  const envThreshold = Number(process.env.SYSTEM_SKILL_ADVISOR_THRESHOLD_CONFIDENCE);
  const envMaxTokens = Number(process.env.SYSTEM_SKILL_ADVISOR_MAX_TOKENS);
  const envBridgeTimeoutMs = Number(process.env.SYSTEM_SKILL_ADVISOR_BRIDGE_TIMEOUT_MS);
  const envMaxPromptBytes = Number(process.env.SYSTEM_SKILL_ADVISOR_MAX_PROMPT_BYTES);
  const envMaxBriefChars = Number(process.env.SYSTEM_SKILL_ADVISOR_MAX_BRIEF_CHARS);
  const envMaxCacheEntries = Number(process.env.SYSTEM_SKILL_ADVISOR_MAX_CACHE_ENTRIES);
  ...
  return {
    ...
    nodeBinary: typeof options.nodeBinaryOverride === 'string' && options.nodeBinaryOverride.trim()
      ...
      : (process.env.SYSTEM_SKILL_ADVISOR_NODE_BINARY || process.env.SPEC_KIT_PLUGIN_NODE_BINARY || DEFAULT_NODE_BINARY),
