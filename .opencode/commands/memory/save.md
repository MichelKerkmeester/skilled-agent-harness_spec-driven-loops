...
| `handover_state` | `handover.md::session-log` | Stop-state, blockers, recent action, next safe action. Use `.opencode/skills/system-spec-kit/templates/handover.md` for initial creation when `handover.md` does not exist. |
...
| **JSON File** (explicit apply)    | `node .opencode/skills/system-spec-kit/scripts/dist/memory/generate-context.js ${TMPDIR:-/tmp}/save-context-data.json <spec-folder>` | Rich context with decisions, files, triggers after apply or full-auto selection |
...
node .opencode/skills/system-spec-kit/scripts/dist/memory/generate-context.js "$TEMP_FILE" specs/###-folder/
