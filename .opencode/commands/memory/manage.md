...
- `/memory:manage` = MANAGEMENT LIFECYCLE (modify, delete, maintain, checkpoint, ingest)
...
node .opencode/skills/system-spec-kit/scripts/dist/graph/backfill-graph-metadata.js --dry-run --active-only
node .opencode/skills/system-spec-kit/scripts/dist/graph/backfill-graph-metadata.js --active-only
```
...
Script path: `.opencode/skills/system-spec-kit/scripts/memory/cleanup-index-scope-violations.ts`
...
Runs `memory_retention_sweep` to audit or delete governed spec-doc records whose `delete_after` timestamp has expired. The tool descriptor is canonical at `.opencode/skills/system-spec-kit/mcp_server/tool-schemas.ts:330`.
