...
Related commands: `/memory:save` (write conversation context into packet continuity surfaces); `/speckit:resume` (session recovery and continuation, which owns the continuity ladder). (`/memory:manage` and `/memory:learn` are retiring — the indexed-continuity database they administered is being removed.)
