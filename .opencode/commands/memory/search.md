...
Related commands: `/memory:save` (save conversation context); `/memory:manage` (database management, checkpoints, ingest, retention, and health); `/speckit:resume` (session recovery and continuation). (`/memory:learn` is deprecated — the constitutional-memory layer was retired.)
