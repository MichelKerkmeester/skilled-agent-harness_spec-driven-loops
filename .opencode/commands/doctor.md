...
Per ADR-005 in `001-router-phase/decision-record.md`: parsing flags BEFORE target resolution is FORBIDDEN — cross-target flag schemas collide (e.g. `--scope` means different things in `code-graph` vs `deep-loop`). Target-first is the only safe order.
