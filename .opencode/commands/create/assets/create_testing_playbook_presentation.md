# /create:testing-playbook Presentation Contract

This file is the single source of truth for user-facing presentation in `/create:testing-playbook`: startup questions, setup/status dashboards, operation display, and result templates. The command router owns asset routing only. The workflow YAML owns execution behavior.
...
Restart with: @markdown /create:testing-playbook <skill-name> [create|update]
...
For `:auto`, resolve setup using the system auto-mode contract, then load `create_testing_playbook_auto.yaml` only when every required field is available.
...
PRE-BOUND SETUP ANSWERS:
  ...
  playbook_root: .opencode/skills/system-spec-kit/manual_testing_playbook
...
| `playbook_root` | Yes | marker or derived from skill path/name | derived | No |
...
A) Load handover plus implementation-summary continuity
B) Load canonical spec docs up to a recent cap
C) Skip
...
| `playbook_root` | Yes | derived from skill name and path | `[value]` |
...
STATUS=[OK|CANCELLED|FAIL] PATH=[playbook_root]
```
