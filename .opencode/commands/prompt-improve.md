...
Create or improve AI prompts by invoking the **sk-prompt** skill. Transforms vague requests into structured, scored prompts using 7 proven frameworks (RCAF, COSTAR, RACE, CIDI, TIDD-EC, CRISPE, CRAFT), DEPTH thinking methodology, and CLEAR quality scoring.
...
Follow the inline `sk-prompt` pipeline for the resolved mode:
...
| `/create:readme`   | Create supporting documentation artifact (if prompt results should be packaged)      |
...
- Failed to load sk-prompt SKILL.md before enhancement
