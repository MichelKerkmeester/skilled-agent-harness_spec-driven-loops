...
Manage the passive session goal through the `opencode-goal` plugin. `/goal-opencode` is a state-free router: it resolves the requested action from `$ARGUMENTS` and dispatches to the `opencode_goal` / `opencode_goal_status` plugin tools, which own all goal state and session resolution.
...
- `history`, `doctor`, and `health` route to read-only `opencode_goal` actions. `history` returns `archive_count` and `archive_N_*` fields. `doctor`/`health` return `active_state_file_count`, `archive_file_count`, log byte sizes, `last_sweep_at`, and `orphan_candidate_count`.
- `clear`, `complete`, `resume`, and `pause [reason]` route to `opencode_goal`.
- Any other non-empty `QUERY` is treated as bare goal text and routes to `opencode_goal` with `action: "set"` and `objective: QUERY`.
...
Your FIRST and ONLY action is the single tool call selected below. Do NOT read files, glob, grep, or explore the repository — the `opencode_goal` / `opencode_goal_status` tools own all goal state and session resolution. Make the call immediately, then print its result verbatim.
...
4. If `FIRST` is `history`, call `opencode_goal({ action: "history" })`.
5. If `FIRST` is `doctor`, call `opencode_goal({ action: "doctor" })`.
6. If `FIRST` is `health`, call `opencode_goal({ action: "health" })`.
7. If `FIRST` is `clear`, call `opencode_goal({ action: "clear" })`.
8. If `FIRST` is `complete`, call `opencode_goal({ action: "complete" })`.
9. If `FIRST` is `pause`, call `opencode_goal({ action: "pause", reason: REST })`.
10. If `FIRST` is `resume`, call `opencode_goal({ action: "resume" })`.
11. For any other non-empty `QUERY`, call `opencode_goal({ action: "set", objective: QUERY })`.
...
- Reads go through `opencode_goal_status`; mutations go through `opencode_goal`.
