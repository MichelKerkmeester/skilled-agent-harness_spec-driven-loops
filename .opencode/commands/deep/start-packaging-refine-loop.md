...
# /deep:start-packaging-refine-loop
...
>
> This command is **general-agent based** — it orchestrates the deep-improvement skill in packaging-benchmark-refine mode (Lane D). Gate 1 (@general verification) and Gate 2 (the BLOCKED Setup phase) are HARD BLOCKS; neither may be skipped.
...
│   └─ general_agent_verified = TRUE → Read `.opencode/skills/deep-improvement/SKILL.md` and `references/packaging-benchmark-refine/operator_guide.md`, then continue to the Setup phase (also a HARD BLOCK)
│
└─ IF NO or UNCERTAIN:
    │
    ...
    │   │   /deep:start-packaging-refine-loop [arguments]            │
    │   └────────────────────────────────────────────────────────────┘
    │
...
The adapter (`scripts/packaging-benchmark-refine/run-packaging-refine.cjs`) validates the contract and spawns the packaging-owned loop host; all guarded-loop logic (frozen scoring surface, kill-switches, worktree promote-N, resume) lives with the packaging.
