...
- **trace mode** — `router` (Mode A, deterministic, default/CI) or `live` (Mode B, BUILT — playbook corpus dispatched through `cli-opencode`, or `codex` via `--executor=codex`; browser scenarios route to `bdg`).
