...
4. If `lane=model-benchmark` is resolved by explicit lane, marker lane, `--profile`, benchmark-profile input, or Q(lane), hand off to `/deep:start-model-benchmark-loop` and load `.opencode/commands/deep/assets/deep_start-model-benchmark-loop_{auto,confirm}.yaml`; do not ask the Lane A Q0/Q1/Q3 questions.
...
8. For `:confirm`, use the presentation contract's consolidated setup prompt to bind missing setup values, then load `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_confirm.yaml`.
...
| Mode | Workflow |
|------|----------|
| `:auto` | `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_auto.yaml` |
| `:confirm` or interactive choice | `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_confirm.yaml` |
...
This command is the Lane A agent-improvement entry point. If setup resolves `lane=model-benchmark`, route to `/deep:start-model-benchmark-loop`; related follow-on commands include `/speckit:complete`, `/prompt`, and `/deep:start-review-loop`.
