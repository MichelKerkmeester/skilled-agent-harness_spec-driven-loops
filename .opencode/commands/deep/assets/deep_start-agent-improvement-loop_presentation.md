# Deep Start Agent Improvement Loop Presentation Contract

Presentation source of truth for `/deep:start-agent-improvement-loop`. The command router owns only mode selection, setup binding, safety gates, Lane B handoff, and asset loading. Workflow YAML owns execution. This file owns visible prompts, dashboard/checkpoint layout, result displays, examples, and next-step suggestions.
...
For `:confirm` or no suffix, the consolidated setup prompt MUST be your FIRST response. No implementation or file-modifying tool calls before asking. Lightweight read-only discovery to suggest a spec folder or load prior context is allowed, then ask ALL questions immediately and wait.
...
- **Benchmark assets** (post-060/005): static at `.opencode/skills/deep-improvement/assets/model_benchmark/benchmark-profiles/default.json` + `assets/model_benchmark/benchmark-fixtures/*.json`. Materializer at `.opencode/skills/deep-improvement/scripts/shared/materialize-benchmark-fixtures.cjs` writes fixture markdown to `{spec_folder}/improvement/benchmark-outputs/` before `run-benchmark.cjs` consumes them. `benchmark_completed` event is gated on `report.json` existing.
...
- **Dispatcher**: `scripts/model-benchmark/dispatch-model.cjs` is the model-agnostic dispatcher (executor routing across cli-opencode, cli-claude-code, cli-codex). It loads only on the model-benchmark path, never in agent-improvement mode.
