# Deep Ask AI Council Presentation Contract

Presentation source of truth for `/deep:ask-ai-council`. The command router owns only mode selection, safety gates, and asset loading. Workflow YAML owns execution. This file owns visible prompts, dashboard/checkpoint layout, result displays, examples, and next-step suggestions.
...
- Ready for: implementation planning or next packet phase
- For single-round planning, use the regular `ai-council` agent behavior.
- Route the completed session through `/memory:save` or the YAML-owned `generate-context.js` call for the resolved spec folder.
