...
No standalone presentation-text asset exists for this command (unlike `/deep:review`'s `deep_review_presentation.txt`) — every dashboard, prompt, and result-template string this mode needs is inline inside the two workflow YAMLs above, since this router runs in `fallback` injection mode (`.opencode/skills/system-deep-loop/shared/rollout/command-injection-rollout.json` has no `deep/alignment` entry, so `resolve-injection-mode.cjs`'s `DEFAULT_MODE` of `"fallback"` applies) and does not require a compiled contract to be freshly generated. A future phase MAY add a presentation.txt plus a `compile-command-contracts.cjs` registration to move this command to `"fix"` mode, mirroring `/deep:review`'s and `/deep:research`'s current rollout state; that upgrade is not performed here.
