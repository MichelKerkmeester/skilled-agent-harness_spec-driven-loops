...
The YAML workflow verifies the orchestrating agent, resolves setup inputs, loads the `sk-prompt/sk-prompt-improve` packet, then runs the DEPTH pipeline for the resolved enhancement mode (inline, or dispatched to `@prompt-improver` when `--agent` or high complexity is resolved), scores the result with CLEAR, delivers the enhanced prompt with a transparency report, and optionally saves it into a spec folder's `prompts/` directory. The confirm workflow pauses at the Discover, Prototype, and Test phases; the auto workflow runs end to end and stops only if CLEAR stays below threshold after three iterations.
