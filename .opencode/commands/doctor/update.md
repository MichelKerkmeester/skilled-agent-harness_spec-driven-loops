...
- **NO NEW MCP TOOLS**: use existing `code_graph_*`, `memory_*`, `skill_graph_*`, `advisor_*`, `ccc_*`, `eval_run_ablation`, and `session_health` surfaces; deep-loop graph operations run through `.opencode/skills/deep-loop-runtime/scripts/*.cjs`.
...
| `.opencode/skills/deep-loop-runtime/storage/deep-loop-graph.sqlite` and `.opencode/skills/deep-loop-runtime/storage/deep-loop-graph.sqlite.pre-doctor-update.*.bak`                                                               | deep research/review graph DB          |
