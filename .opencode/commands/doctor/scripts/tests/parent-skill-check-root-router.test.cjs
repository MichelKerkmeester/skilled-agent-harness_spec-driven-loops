...
function activeRouter({ defaults = '[]', pathForBeta = 'pkg-two/references/world.md', sharedControls = null } = {}) {
  return [
    ...
    '# demo-hub Surface Router',
    '',
    // RRC-009: an active router owns orienting prose, so the clean fixture
    // carries the two sections the contract requires of a real one.
    '## 1. OVERVIEW',
    '',
    'Second-layer surface router for the demo hub. Stage one picks the workflow',
    'mode; this document maps request intent to that mode packet-local leaves.',
    '',
    '## 2. INTENT MODEL',
    '',
    '- **alpha leaves** — the resources an ALPHA request loads.',
    '- **beta leaves** — the resources a BETA request loads.',
    '',
