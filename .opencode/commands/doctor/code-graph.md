...
Doctor reads `code_graph_status({}).activeScope` before missed-file analysis. The 011 broader default excludes skip 5 default-excluded `.opencode` folders unless the active scope opts them in: `.opencode/skills/**`, `.opencode/agents/**`, `.opencode/commands/**`, `.opencode/specs/**`, and `.opencode/plugins/**`.
...
- `.opencode/commands/doctor/skill-advisor.md` — sibling doctor command (5-phase mutation pattern; pattern source for Phase B)
...
| `.opencode/skills/system-spec-kit/mcp_server/dist/` | medium | TypeScript compiled output |
| `.opencode/skills/system-spec-kit/mcp_server/node_modules/` | high | nested vendor |
