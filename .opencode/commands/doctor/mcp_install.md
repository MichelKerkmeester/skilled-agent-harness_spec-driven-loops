...
python3 --version # Must be 3.11+ (for the spec-kit CocoIndex fork at .opencode/skills/mcp-coco-index/; upstream cocoindex-code may have different Python bounds)
npm --version
npx --version
```
...
**Code Mode:**
```bash
cd .opencode/skills/mcp-code-mode/mcp_server
npm install
npm run build
```
...
```bash
bash .opencode/commands/doctor/scripts/mcp-doctor.sh --server <name>
```
