...
3. Completely clean the memory database, then run a full rescan/rebuild of memory indexes before normal usage.
4. For frontmatter operations, keep migration order `normalize -> verify -> rebuild`; dry-run first, then apply, then reindex.
5. For phased specs, use `--recommend-phases`, `--phase`, and `validate.sh --recursive` with parent/child linkage checks.
6. For review workflows, pull latest to use the `sk-code--review` baseline + overlay contract and findings-first output structure.
7. For AGENTS.md consumers, no behavior migration is required; structure is condensed and references are centralized.
8. No database schema migration is required; deferred follow-up items remain documented in component changelogs.
