...
**Codex Platform Integration** — Added Codex CLI profile system and sub-agent dispatch mechanism to `.codex/config.toml`. Four profiles (`fast`, `balanced`, `powerful`, `readonly`) using gpt-5.3 family with reasoning effort differentiation. Converted 8 `.codex/agents/` files to Codex-native frontmatter. Downgraded handover agent to Haiku across all platforms.

> Spec folder: `anobel.com/.opencode/specs/04--agent-orchestration/z_archive/016-handover-model-codex-compat/` (Level 3)
