...
**Cognitive Memory System with FSRS** — Major upgrade implementing spaced repetition with 100M+ validated algorithm, 30 bug fixes from 10-agent audit, and Mermaid diagrams across all 20 agent/command files.
...
- **Restart MCP server** — Schema v4 migration runs automatically on first start
- **No breaking changes** — Existing memories receive default values (stability=1.0, difficulty=5.0)
- **FSRS active** — Memory decay now uses validated power-law formula
