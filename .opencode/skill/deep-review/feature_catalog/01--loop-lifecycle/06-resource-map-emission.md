...
| `.opencode/skill/deep-review/scripts/reduce-state.cjs` | Reducer | Adds the `--emit-resource-map` path and honors `config.resource_map.emit`. |
...
| `.opencode/skill/deep-review/references/convergence.md` | Reference | Documents the convergence-to-synthesis emission step and operator opt-out. |
...
| `.opencode/skill/deep-review/manual_testing_playbook/06--synthesis-save-and-guardrails/029-resource-map-emission.md` | Manual playbook | Verifies synthesis emits the review resource map and that opt-out skips cleanly. |
...
- Primary sources: `.opencode/skill/system-spec-kit/scripts/resource-map/extract-from-evidence.cjs`, `.opencode/skill/deep-review/scripts/reduce-state.cjs`, `.opencode/command/spec_kit/assets/spec_kit_deep-review_auto.yaml`, `.opencode/command/spec_kit/assets/spec_kit_deep-review_confirm.yaml`
