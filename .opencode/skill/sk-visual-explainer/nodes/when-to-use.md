...
**Data gathering:** Calls `sk-git` to collect diff output, changed file list, commit messages, and PR description.
...
**Data gathering:** Loads `system-spec-kit` memory entries from the time window. May call `sk-git` for commit history.
