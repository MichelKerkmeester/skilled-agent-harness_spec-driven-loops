...
- RCAF Prompt: `As an external-AI conductor preparing a spec folder for a small feature, dispatch claude -p --agent speckit and ask for a Level recommendation plus a file list. Verify the response names a documentation level (1, 2, or 3), lists the required canonical files for that level (spec.md, plan.md, tasks.md, implementation-summary.md at minimum), and explains the level choice in one sentence. Return a verdict naming the level, the files, and the rationale.`
