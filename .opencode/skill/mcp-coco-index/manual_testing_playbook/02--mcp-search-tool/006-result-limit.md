...
- Prompt: `Compare MCP CocoIndex limit 1 vs limit 10 and verify result counts obey both limits.`
...
|---|---|---|---|---|---|---|---|---|
| MCP-006 | Result limit | Verify `limit` controls output count | `Compare MCP CocoIndex limit 1 vs limit 10 and verify result counts obey both limits.` | 1. `mcp__cocoindex_code__search({ "query": "error handling", "limit": 1 })` -> 2. Count results (expect exactly 1) -> 3. `mcp__cocoindex_code__search({ "query": "error handling", "limit": 10 })` -> 4. Count results (expect up to 10) | Step 2: exactly 1 result; Step 4: result count between 1 and 10 (inclusive); Step 4 count >= Step 2 count | Side-by-side result counts from both calls | PASS if step 2 returns exactly 1 AND step 4 returns more than 1 (up to 10); FAIL if `limit` is ignored | Verify `limit` parameter type is integer; check default (5) if parameter omitted |
