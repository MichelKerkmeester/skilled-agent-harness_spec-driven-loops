...
- RCAF Prompt: `As a manual-testing orchestrator, call session_bootstrap and session_resume, then inspect the CocoIndex status fields against the current Spec Kit Memory recovery surfaces in this repository. Verify session_bootstrap.resume.cocoIndex.available and session_resume.cocoIndex.available match binary presence on disk, both binaryPath values are non-empty strings, session_bootstrap includes structuralContext, and both calls complete without error regardless of binary availability. Return a concise user-visible pass/fail verdict with the main reason.`
