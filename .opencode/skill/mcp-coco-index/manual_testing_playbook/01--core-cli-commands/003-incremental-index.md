...
- Real user request: `Please verify incremental indexing picks up new content and drops deleted content.`
- RCAF Prompt: `As a manual-testing orchestrator, create a temp file, reindex, search for it, delete it, reindex again against the current CocoIndex CLI, daemon, and MCP surfaces in this repository. Verify Step 3: search returns at least 1 result referencing ccc_test_incremental.py; Step 6: search returns 0 results for that file. Return a concise user-visible pass/fail verdict with the main reason.`
- Expected execution process: Run the TEST EXECUTION command sequence for `CCC-003`, capture the listed evidence, compare observed output with the expected signals, and return the verdict to the user.
