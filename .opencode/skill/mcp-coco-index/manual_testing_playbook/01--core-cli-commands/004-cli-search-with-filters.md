...
|---|---|---|---|---|---|---|---|---|
| CCC-004 | CLI search with filters | Verify `--lang` (repeatable) and `--limit` filters work | `Search for "function" filtered to Python and TypeScript with limit 3. Capture the evidence needed to prove Step 1: returns results; Step 2: all file extensions are .py, .ts, or .tsx; Step 3: result count <= 3. Return a concise user-facing pass/fail verdict with the main reason.` | 1. `bash: ccc search "function" --lang python --lang typescript --limit 3` -> 2. Verify all returned file paths end in `.py` or `.ts`/`.tsx` -> 3. Verify result count is at most 3 | Step 1: returns results; Step 2: all file extensions are `.py`, `.ts`, or `.tsx`; Step 3: result count <= 3 | Search output with file paths and count | PASS if all results match language filter AND count <= 3; PARTIAL if count correct but one result has wrong extension; FAIL if filter is ignored | Check `--lang` values against supported language code values in tool_reference.md; verify index contains Python/TypeScript files |
