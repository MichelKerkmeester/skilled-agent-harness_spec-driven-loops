...
- Prompt: `Verify ccc reset --all -f clears the index, then rebuilds to non-zero counts; return pass/fail with reason.`
...
|---|---|---|---|---|---|---|---|---|
| CCC-005 | Index reset **(DESTRUCTIVE)** | Verify `ccc reset --all` clears index; status shows empty; rebuild succeeds | `Verify ccc reset --all -f clears the index, then rebuilds to non-zero counts; return pass/fail with reason.` | 1. `bash: ccc reset --all -f` -> 2. `bash: ccc status` (expect zero/empty counts) -> 3. `bash: ccc init` -> 4. `bash: ccc index` -> 5. `bash: ccc status` (expect non-zero counts) | Step 1: exits without error; Step 2: shows zero files or "not initialized"; Step 4: rebuilds with non-zero counts; Step 5: shows non-zero file and chunk counts | Full transcript of all 5 steps; before/after status output | PASS if reset clears index AND rebuild restores non-zero counts; FAIL if reset errors or rebuild produces zero counts | Check `ccc reset` flags; verify `-f` bypasses confirmation; check disk space for rebuild |
