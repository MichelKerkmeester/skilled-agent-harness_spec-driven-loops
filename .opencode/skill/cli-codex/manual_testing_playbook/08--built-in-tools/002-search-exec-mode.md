...
- RCAF Prompt: `As a cross-AI orchestrator confirming --search works in exec mode (not just TUI), dispatch codex exec --search --model gpt-5.5 --sandbox read-only -c model_reasoning_effort="high" -c service_tier="fast" "What was the headline release of any major JavaScript runtime announced in March 2026? Cite one source." Verify Codex exits 0, the response indicates web access was used (URL citation, "according to" phrasing, or a date-stamped fact later than the model's training cutoff), and the dispatched command line includes --search. Return a verdict naming the cited release and the source URL.`
