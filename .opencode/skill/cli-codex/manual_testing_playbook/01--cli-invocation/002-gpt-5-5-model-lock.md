...
- RCAF Prompt: `As a cross-AI orchestrator validating the cli-codex model contract, dispatch a small documentation-style request explicitly pinned to --model gpt-5.5 with -c service_tier="fast". Verify the response is coherent, exit code is 0, and confirm via the skill docs that no other model ID is in scope. Return a one-line PASS/FAIL with the observed exit code and the model named in the dispatch.`
