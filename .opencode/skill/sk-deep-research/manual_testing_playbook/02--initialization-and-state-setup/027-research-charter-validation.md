...
- Orchestrator prompt: Validate the research charter validation contract for sk-deep-research. Confirm that initialization Step 5a verifies the presence of "Non-Goals" and "Stop Conditions" sections in strategy.md, appends empty placeholders if missing, and presents the charter for user review in confirm mode. Return a concise operator-facing verdict.
