...
- Orchestrator prompt: Validate the state-reconstruction contract for sk-deep-research. Confirm that the workflow can scan iteration-NNN.md files, reconstruct JSONL iteration records, and log a state_reconstructed event, then return a concise operator-facing verdict.
