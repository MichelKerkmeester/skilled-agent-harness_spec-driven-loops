...
- Orchestrator prompt: Validate the pause-sentinel contract for sk-deep-research. Confirm that `scratch/.deep-research-pause` is checked between research iterations, `{spec_folder}/review/.deep-research-pause` is checked between review iterations, both emit a paused event, and both halt the loop without entering synthesis, then return a concise operator-facing verdict.
