...
- Orchestrator prompt: Validate the pause-resume contract for sk-deep-research. Confirm that removing scratch/.deep-research-pause lets the loop log a resumed event and continue from the read-state step instead of starting from scratch, then return a concise verdict.
...
|---|---|---|---|---|---|---|---|---|
| DR-016 | Resume after pause sentinel removal | Verify that removing the pause sentinel lets the loop resume from read-state rather than re-initializing. | Validate the pause-resume contract for sk-deep-research. Confirm that removing `scratch/.deep-research-pause` lets the loop log a resumed event and continue from the read-state step instead of starting from scratch, then return a concise verdict. | 1. `bash: rg -n 'On resume|resumed|Continue from step_read_state' .opencode/skill/sk-deep-research/references/loop_protocol.md .opencode/skill/sk-deep-research/references/state_format.md` -> 2. `bash: rg -n 'resumed|on_resume|skip_to: phase_loop|skip_to: gate_init_approval' .opencode/command/spec_kit/assets/spec_kit_deep-research_auto.yaml .opencode/command/spec_kit/assets/spec_kit_deep-research_confirm.yaml` -> 3. `bash: rg -n 'Delete the file to resume|Auto-resume' .opencode/skill/sk-deep-research/README.md` | The loop logs `resumed`, continues from state read, and does not recreate config/strategy files during a valid resume. | Capture the resumed-event wording, the resume branch behavior, and the user-facing pause/resume explanation. | PASS if resume continues from persisted state after logging a resumed event; FAIL if resume implies a fresh initialization path. | Check both the pause subsection and the broader auto-resume classification rules because they work together. |
