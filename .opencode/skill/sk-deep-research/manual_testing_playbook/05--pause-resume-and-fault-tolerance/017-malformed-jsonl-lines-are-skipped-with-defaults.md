...
- Orchestrator prompt: Validate the JSONL fault-tolerance contract for sk-deep-research. Confirm that malformed lines are skipped, missing fields are defaulted, and a warning is emitted instead of crashing the loop, then return a concise operator-facing verdict.
