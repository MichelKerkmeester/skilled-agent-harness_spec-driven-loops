...
- Orchestrator prompt: Validate the source diversity quality guard for sk-deep-research. Confirm that when composite convergence votes STOP, the guard checks each answered question for >= 2 independent sources, and that a violation emits a guard_violation event and overrides the decision to CONTINUE.
