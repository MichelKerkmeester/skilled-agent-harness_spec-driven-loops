...
- Orchestrator prompt: Validate the focus alignment quality guard for sk-deep-research. Confirm that when composite convergence votes STOP, the guard compares answered questions against the original key questions from initialization, and that a mismatch emits a guard_violation event with guard="focus_alignment" and overrides the decision to CONTINUE.
