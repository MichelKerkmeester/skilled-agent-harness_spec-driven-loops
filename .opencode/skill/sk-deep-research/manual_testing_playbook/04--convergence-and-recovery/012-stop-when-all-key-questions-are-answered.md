...
- Orchestrator prompt: Validate the all-questions-answered stop contract for sk-deep-research. Confirm that the loop stops when the tracked key questions are answered and that this condition is checked before softer convergence logic, then return a concise verdict.
