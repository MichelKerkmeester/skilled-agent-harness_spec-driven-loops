...
| `.opencode/skill/sk-deep-research/scripts/reduce-state.cjs` | Reducer script; `renderDashboard` generates the dashboard content; `reduceResearchState` writes it to disk |
| `.opencode/command/spec_kit/assets/spec_kit_deep-research_auto.yaml` | Workflow algorithm; inspect `step_reduce_state` and `step_generate_dashboard` |
| `.opencode/command/spec_kit/assets/spec_kit_deep-research_confirm.yaml` | Workflow algorithm; inspect `step_reduce_state` and `step_generate_dashboard` |
| `.opencode/skill/system-spec-kit/scripts/tests/deep-research-reducer.vitest.ts` | Automated verification; proves reducer idempotency and dashboard content correctness |
