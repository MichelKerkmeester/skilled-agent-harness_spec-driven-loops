...
- Manage iteration state outside `{spec_folder}/research/` or `{spec_tree_root}/research/{phaseId}-pt-{NN}/`
...
The research state packet always lives in the spec tree root's `research/` folder. Root-spec targets use `{spec_folder}/research/`. Child-phase targets resolve `{spec_tree_root}/research/{phaseId}-pt-{NN}/`, where `{phaseId}` is the first 3 characters of the immediate child-phase segment (e.g. "019") and `{NN}` is a zero-padded sequential counter scoped to that phase within the artifact root.

Example: `.../026-graph.../019-system-hardening/001-initial-research/004-desc-regen/` → `research/019-pt-03/`

```text
research/
  [{phaseId}-pt-{NN}/]              # Present only when the target spec is a nested child phase
