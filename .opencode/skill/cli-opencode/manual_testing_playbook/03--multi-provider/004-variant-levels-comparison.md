...
This scenario validates Variant levels for `CO-012`. It focuses on confirming the `--variant` flag accepts the documented per-provider range and that the response depth materially differs between extremes when the same prompt is dispatched. Underlying-model conventions still apply: a Claude model under `github-copilot/` accepts `minimal`, `low`, `medium`, `high`, `max`; a GPT model under `github-copilot/` accepts `minimal`, `low`, `medium`, `high`, `xhigh`; `opencode-go` and `deepseek` (reasoner) accept reasoning-effort variants on their reasoning models.

### Why This Matters

The cli-opencode skill defaults to `--variant high` because routine dispatches benefit from elevated reasoning effort (per SKILL.md §3 and `references/cli_reference.md` §5). If the variant flag is silently ignored or the levels do not actually shift reasoning depth, the skill default loses its value and operators have no real way to dial cost vs depth. This test proves the variant flag is functional by comparing `minimal` vs the upper bound (`max` for claude-sonnet-4.6) for the same prompt and confirming the response byte count and depth differ materially.
...
| `opencode-go/deepseek-v4-pro` | `--variant` accepted; effect depends on opencode-go routing |
