...
- RCAF Prompt: `As an external-AI conductor preparing to dispatch into a remote OpenCode server, verify the cli-opencode documentation contract for cross-server dispatch is intact. Grep the cli_reference.md flag table for --attach, grep the subcommand map for opencode attach, and run opencode run --help to confirm the flag is in the live binary's help output. Return a concise pass/fail verdict naming each evidence source and confirming the flag is documented and reachable.`
