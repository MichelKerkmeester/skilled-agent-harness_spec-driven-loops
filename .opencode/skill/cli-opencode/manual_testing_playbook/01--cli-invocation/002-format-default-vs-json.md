...
- RCAF Prompt: `As an external-AI conductor verifying both opencode run output formats are intact, dispatch the same short prompt twice with --format default and --format json, then compare. Verify the default output is human-readable and the JSON output parses with jq and contains the documented event shape (type, timestamp, session_id, payload). Return a concise pass/fail verdict naming the event types found in the JSON variant.`
