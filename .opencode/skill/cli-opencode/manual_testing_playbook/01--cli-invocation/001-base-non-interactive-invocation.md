...
This scenario validates Base non-interactive invocation for `CO-001`. It focuses on confirming the canonical `opencode run --model github-copilot/gpt-5.5 --agent general --variant high --format json --dir <repo-root> "<prompt>"` dispatch returns a coherent JSON event stream and exits cleanly from a non-OpenCode runtime.
...
- Objective: Confirm that the canonical default invocation `opencode run --model github-copilot/gpt-5.5 --agent general --variant high --format json --dir "$(pwd)" "<prompt>" 2>&1` returns a parseable JSON event stream and exits 0.
