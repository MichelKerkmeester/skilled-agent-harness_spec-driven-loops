...
- Pass/fail: PASS if recovery uses worktree-created branch policy per `SKILL.md §3` and shared recovery patterns in `references/shared_patterns.md §6`. FAIL if the AI runs destructive reset, drops the commit, or creates a branch directly with `git checkout -b`.
...
| `../05--recovery-and-edge-cases/002-accidental-commit-wrong-branch.md` | Canonical per-feature execution contract |
...
- Feature file path: `05--recovery-and-edge-cases/002-accidental-commit-wrong-branch.md`
