...
Validates the complete `/improve:agent-improver` loop targeting a non-standard agent (debug.md) to confirm the pipeline is not hardcoded to specific agents.

## Prompt / Command

```text
/improve:agent-improver ".opencode/agent/debug.md" :confirm --spec-folder=specs/03--skilled-agent-orchestration/041-sk-agent-improver-loop/008-sk-agent-improver-holistic-evaluation --iterations=1
```
...
- Dashboard reflects debug-specific scoring, not recycled handover data
...
- If scores look identical to handover: check that the candidate file path is correctly resolved to `debug.md` and passed through each stage
