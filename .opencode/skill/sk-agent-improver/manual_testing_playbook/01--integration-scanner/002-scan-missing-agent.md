...
```bash
node .opencode/skill/sk-agent-improver/scripts/scan-integration.cjs --agent=nonexistent-agent-xyz
```

### Verification (copy-paste)

```bash
node .opencode/skill/sk-agent-improver/scripts/scan-integration.cjs --agent=nonexistent-agent-xyz | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['status']=='complete'; assert d['surfaces']['canonical']['exists']==False; assert d['summary']['missingCount']>0; print('PASS')"
```
