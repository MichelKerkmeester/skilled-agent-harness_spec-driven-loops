...
```bash
node .opencode/skill/sk-agent-improver/scripts/scan-integration.cjs --agent=debug
```

### Verification (copy-paste)

```bash
node .opencode/skill/sk-agent-improver/scripts/scan-integration.cjs --agent=debug | python3 -c "import sys,json; d=json.load(sys.stdin); s=d['summary']; assert s['totalSurfaces']>=20; assert s['commandCount']>=1; assert s['skillCount']>=5; print(f'PASS: {s[\"totalSurfaces\"]} surfaces, {s[\"commandCount\"]} commands, {s[\"skillCount\"]} skills')"
```
