...
- Prompt: `As a routing accuracy operator, validate semantic search routing against .opencode/skill/skill-advisor/scripts/skill_advisor.py "find code that handles auth". Verify mcp-coco-index is top-1 with confidence >= 0.80. Return a concise pass/fail verdict with the main reason and cited evidence.`
...
`As a routing accuracy operator, validate semantic search routing against .opencode/skill/skill-advisor/scripts/skill_advisor.py "find code that handles auth". Verify mcp-coco-index is top-1 with confidence >= 0.80. Return a concise pass/fail verdict with the main reason and cited evidence.`
