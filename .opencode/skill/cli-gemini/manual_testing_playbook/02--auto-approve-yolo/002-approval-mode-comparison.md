...
- RCAF Prompt: `As a cross-AI orchestrator selecting an approval posture, invoke Gemini CLI twice against the cli-gemini skill in this repository: once with default approval and once with --approval-mode plan, both restricted to read-only tools. Verify both runs return a non-empty answer with no operator prompt blocking stdin and that neither modifies the working tree. Return a concise pass/fail verdict with the main reason and a one-line note about whether default vs plan produced a meaningful behavioural difference for this read-only prompt.`
