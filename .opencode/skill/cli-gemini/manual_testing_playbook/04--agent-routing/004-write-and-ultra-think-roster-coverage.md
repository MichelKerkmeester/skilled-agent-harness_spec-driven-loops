...
- RCAF Prompt: `As a cross-AI orchestrator covering the Gemini agent roster, dispatch two calls against the cli-gemini skill in this repository: first @write agent for a small README skeleton, then @ultra-think agent for two competing API caching strategies. Verify the @write output contains at least two markdown headings and the @ultra-think output names at least two distinct caching approaches. Constrain both to read-only operation. Return a concise pass/fail verdict with the main reason and a one-line summary per agent.`
