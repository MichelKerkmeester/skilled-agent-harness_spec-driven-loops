...
- Prompt: `As a cross-AI orchestrator handing a single file to Gemini for analysis, invoke Gemini CLI against the cli-gemini skill in this repository using the @ file-reference syntax to pass SKILL.md. Verify the answer includes a verbatim quote that proves Gemini actually read the file (the only-supported model name). Return a concise pass/fail verdict with the main reason and the quoted string.`
