...
- Orchestrator prompt: Validate the resume classification contract for sk-deep-review. Confirm that the step_classify_session logic detects existing config, JSONL, and strategy files in review/ and classifies the session as "resume", skipping to phase_loop, then return a concise user-facing pass/fail verdict.
