...
This scenario validates malformed JSONL lines are skipped with defaults for `DRV-023`. The objective is to verify that when the review state log (`deep-research-state.jsonl`) contains malformed or unparseable lines, they are skipped gracefully and sensible defaults are applied so the loop can continue.
...
| File | Role |
|---|---|
| `.opencode/skill/sk-deep-research/references/state_format.md` | JSONL schema and parsing rules; use `ANCHOR:state-log` |
| `.opencode/skill/sk-deep-research/references/convergence.md` | Default values on parse failure; graceful degradation rules |
| `.opencode/skill/sk-deep-research/references/loop_protocol.md` | Error handling in iteration loop; use `ANCHOR:phase-iteration-loop` |
