...
- Orchestrator prompt: Validate the confirm entrypoint for sk-deep-review. Confirm that /spec_kit:deep-review:confirm is documented consistently across the README, quick reference, command entrypoint, and confirm YAML workflow, and that approval gates are present at each phase transition, then return a concise user-facing pass/fail verdict.
