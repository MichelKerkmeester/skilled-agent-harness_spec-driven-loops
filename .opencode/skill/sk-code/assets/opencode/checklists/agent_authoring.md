...
- Use this when updating mirrored agent files across OpenCode, Claude, Codex, and Gemini.

## 3. PRE-CHECKS

- [ ] Read canonical agent examples at `.opencode/agent/code.md`, `.opencode/agent/review.md`, and `.opencode/agent/orchestrate.md`.
- [ ] Read `sk-doc` source rules at `.opencode/skill/sk-doc/references/specific/agent_creation.md`.
...
- [ ] Identify the 4-runtime mirror set: `.opencode/agent/`, `.claude/agents/`, `.codex/agents/`, and `.gemini/agents/`.
...
- [ ] Run `bash .opencode/skill/system-spec-kit/scripts/spec/validate.sh <path> --strict` when the agent change is part of a spec folder.
- [ ] grep verification: `rg -n "name:|description:|allowed-tools:|permission:|LEAF|orchestr" .opencode/agent/<name>.md`.
- [ ] Cross-runtime mirror parity check: compare `.opencode/agent/<name>.md`, `.claude/agents/<name>.md`, `.codex/agents/<name>.md`, and `.gemini/agents/<name>.md` where the agent is runtime-wide.
...
- Prior examples: `.opencode/agent/code.md`, `.opencode/agent/review.md`, `.opencode/agent/orchestrate.md`
- Verification recipes: `.opencode/skill/sk-code/assets/opencode/checklists/universal_checklist.md`
