...
- Use this when authoring a new MCP server under `.opencode/skill/<skill-name>/mcp_server/`.
...
- [ ] Run `bash .opencode/skill/system-spec-kit/scripts/spec/validate.sh <path> --strict` when the MCP server change is part of a spec folder.
- [ ] grep verification: `rg -n "schema|register|tool|error|envelope|success" .opencode/skill/<skill-name>/mcp_server`.
