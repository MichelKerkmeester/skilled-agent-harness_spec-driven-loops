...
> **Cross-stack motion.dev reference**: For motion.dev API surface, decision matrix, and integration patterns that apply across stacks (not just Webflow), see [`../../references/motion_dev/quick-start.md`](../../references/motion_dev/quick-start.md) and the documents in [`references/motion_dev/`](../../references/motion_dev/). The MOTION constraint above remains a Webflow-CDN-specific platform marker.
