...
Prefer CSS first for simple hover/focus states, one-off transitions, and static keyframes that do not need JS state or sequencing. Existing Webflow guidance already encodes this as "CSS first; Motion.dev when you need programmatic control" (Repo: `.opencode/skill/sk-code/references/webflow/implementation/animation_workflows.md`).
