...
1. `.opencode/skill/sk-code/SKILL.md` is at HEAD-of-main and the "Unsupported / Unknown" row in the Multi-Repository Architecture table is intact.
...
1. **Advisor probe**:
   ```
   bash: python3 .opencode/skill/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py "Add a request-ID middleware to my Go HTTP server in cmd/api/main.go and return it in the X-Request-ID response header." --threshold 0.8 > /tmp/skc-SD003-advisor.txt
   ```
...
- `.opencode/skill/sk-code/SKILL.md` — Multi-Repository Architecture table with "Unsupported / Unknown" row.
- `.opencode/skill/sk-code/references/router/code_surface_detection.md` — Marker definitions (none should match Go).
