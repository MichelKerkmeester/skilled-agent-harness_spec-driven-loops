...
**Exact prompt**:
```
Refactor the parseExecutorConfig function in .opencode/skill/system-spec-kit/mcp_server/lib/deep-loop/executor-config.ts to throw on missing model when type is cli-codex.
```
...
1. Target file exists: `bash: test -f .opencode/skill/system-spec-kit/mcp_server/lib/deep-loop/executor-config.ts`
2. TypeScript sub-language reference set intact: `bash: ls .opencode/skill/sk-code/references/opencode/typescript/ | sort` returns exactly `quality_standards.md style_guide.md quick_reference.md` (in some sort order).
...
- `.opencode/skill/sk-code/SKILL.md` (lines 78-90 — sub-detection table).
- `.opencode/skill/sk-code/references/router/code_surface_detection.md` (lines 50-62).
- `.opencode/skill/sk-code/references/opencode/typescript/{style_guide,quality_standards,quick_reference}.md`.
