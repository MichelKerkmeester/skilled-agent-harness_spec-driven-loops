...
description: Fast-path framework selection and CLEAR checks for Copilot CLI prompt construction.
---
...
Fast-path prompt-quality guidance for Copilot CLI dispatches. Use this asset before building a routine CLI prompt so the router stays lightweight while still applying framework selection and a quick CLEAR pass.

## 1. OVERVIEW

### Purpose

Provide a small, always-load asset for Copilot CLI prompt construction that improves quality without pulling in the full prompt-engineering skill on routine dispatches.
...
- Logic: Does it explain how Copilot should reason or decide?
...
Copilot-specific example: if the task needs a large `copilot -p` prompt plus `--allow-all-tools` or a model-selection note, get the structured `ENHANCED_PROMPT` from `@improve-prompt` first and then hand that to Copilot CLI.
