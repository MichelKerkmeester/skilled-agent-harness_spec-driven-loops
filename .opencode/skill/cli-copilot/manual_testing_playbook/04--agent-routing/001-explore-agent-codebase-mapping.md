...
- Prompt: `As a cross-AI orchestrator using Copilot agent routing, dispatch the @Explore agent against the cli-copilot skill in this repository to produce a structured map of the references/ folder. Verify the answer names at least three real reference files with a one-line purpose for each, and that the working tree is unchanged after the call. Return a concise pass/fail verdict with the main reason and the names of the files Copilot cited.`
