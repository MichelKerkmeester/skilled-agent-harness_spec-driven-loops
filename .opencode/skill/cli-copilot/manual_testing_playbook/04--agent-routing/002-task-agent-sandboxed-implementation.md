...
- RCAF Prompt: `As a cross-AI orchestrator delegating implementation work, dispatch the @Task agent against the cli-copilot skill in this repository to implement a small documented function in /tmp/cp-011-sandbox/calc.py. Verify the resulting file exists, contains the requested function with a docstring, and the working tree is unchanged. Return a concise pass/fail verdict with the main reason and the function signature line.`
