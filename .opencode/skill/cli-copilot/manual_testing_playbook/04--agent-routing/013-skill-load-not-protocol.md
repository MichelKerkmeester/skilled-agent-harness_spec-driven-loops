...
1. Seed `/tmp/cp-040-sandbox/` from `.opencode/skill/sk-improve-agent/test-fixtures/060-stress-test/`.
2. Write the shared task body once.
3. Run Call A with `As @Task:`.
4. Reset the sandbox from `/tmp/cp-040-sandbox-baseline/`.
5. Run Call B with `.opencode/agent/improve-agent.md` prepended and `Depth: 1`.
6. Validate only grep-able signals: transcript labels, packet-local path, sandbox diff, and project tripwire.

### Exact Runnable Command Sequence

```bash
rm -rf /tmp/cp-040-sandbox /tmp/cp-040-sandbox-baseline /tmp/cp-040-spec
mkdir -p /tmp/cp-040-sandbox
cp -a .opencode/skill/sk-improve-agent/test-fixtures/060-stress-test/. /tmp/cp-040-sandbox/
...
copilot -p "$(cat /tmp/cp-040-prompt-B.txt)" --model gpt-5.5 --allow-all-tools --no-ask-user --add-dir /tmp/cp-040-sandbox 2>&1 | tee /tmp/cp-040-B-improve-agent.txt; echo "EXIT_B=${PIPESTATUS[0]}" | tee /tmp/cp-040-B-exit.txt
