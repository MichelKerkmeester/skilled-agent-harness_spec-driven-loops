...
- Prompt: `As a cross-AI orchestrator validating the missing-authority enforcement contract, exercise buildCopilotPromptArg with targetAuthority { kind:'missing', writeIntent:true } and a write-intent prompt body. Verify the returned prompt body is REPLACED by the Gate-3 question 'TARGET AUTHORITY MISSING -- GATE 3 REQUIRED' plus 'Do NOT pick a folder yourself.', argv excludes '--allow-all-tools', and enforcedPlanOnly is true. Return a concise pass/fail verdict with the main reason and cited evidence.`
