...
This scenario validates that the current release uses standalone `new`-mode sessions, so continuation happens by archiving the previous run and starting a fresh `/improve:deep-agent-improvement` session.
...
- Real user request: `` Validate that the current release uses standalone `new`-mode sessions, so continuation happens by archiving the previous run and starting a fresh `/improve:deep-agent-improvement` session. ``
