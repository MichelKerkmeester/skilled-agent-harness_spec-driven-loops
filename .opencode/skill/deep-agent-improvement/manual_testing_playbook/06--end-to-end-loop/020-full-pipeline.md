...
- RCAF Prompt: `` As a manual-testing orchestrator, validate the complete /improve:deep-agent-improvement loop end-to-end using the debug agent as the target against the current deep-agent-improvement command, runtime artifacts, and validation scripts. Verify Init phase creates `improvement/` directory with config, charter, strategy, and manifest. Return a concise operator-facing PASS/FAIL verdict with the decisive evidence. ``
