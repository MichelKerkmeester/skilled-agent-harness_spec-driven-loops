...
### Phase 3 — Style
- Use README Ledger tokens (`--bg`, `--surface`, `--text`, `--accent`, `--muted`, `--border`) as default.
- Keep `--ve-*` aliases only as compatibility shims for legacy snippets.
- Typography default for README Ledger is `Inter` + `JetBrains Mono`.
- Outside README Ledger profile, do not use Inter/Roboto/Arial as primary display/body fonts.
...
- Layout shell: desktop `260px` sidebar + content; hide sidebar on mobile (`<1024px`).
...
- README Ledger token contract (`--bg`, `--surface`, `--text`, `--accent`, `--muted`, `--border`) or `--ve-*` compatibility aliases.
...
- Background polling loops (`fetch`, storage watchers, hidden timers) in static outputs.
- Any `setInterval` usage except a visible `#clock` UTC updater at 1-second cadence.
