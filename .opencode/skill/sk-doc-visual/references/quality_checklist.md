...
Detailed pass/fail criteria for 9 core quality checks plus hardening signals. Run every check before delivering any sk-doc-visual output.

---

## 1. OVERVIEW

Use this checklist as the final verification gate before delivering any doc-visual HTML file.
...
- Forgetting to define `--accent-dim` (or `--ve-accent-dim` alias) in the alternate color scheme
...
**Fix:**
- Ensure `@media (prefers-color-scheme: dark|light)` defines all core ledger tokens (`--bg`, `--surface`, `--text`, `--accent`, `--muted`, `--border`) and any required aliases
...
- Check that `--border`/`--border-bright` (or `--ve-border`/`--ve-border-bright`) are defined in both themes
...
**Fail:**
- `--muted`/`--ve-text-dim` on `--surface2`/`--ve-surface2` falls below 4.5:1
...
For contrast issues, darken `--muted` (or `--ve-text-dim`) or lighten `--surface2` (or `--ve-surface2`) until the ratio passes. Test both light and dark modes independently.
...
## Check 9A: README Ledger Shell Signals

**Purpose:** Verify default README Ledger profile behavior and primitives are present.

**How to run:**
- Inspect markup for `.terminal-header`, `#clock`, `.toc-link`, `.glass-card`, and `.ledger-line`.
- Scroll through section anchors and confirm `.toc-link.active::before` (active-dot state) updates.
- Emulate `prefers-reduced-motion: reduce` and verify `.reveal` content and `.viz-fill` bars remain readable.

**Pass:**
- `.terminal-header` exists and `#clock` renders visible UTC text.
- TOC active state is visible without relying on color alone (dot + text emphasis).
- Reduced-motion mode shows full content immediately with no hidden reveal/viz elements.

**Fail:**
- Missing terminal header/clock shell, no active-dot indicator, or reduced-motion mode hides content.

**Fix:**
- Add default primitives from `css_patterns.md` README Ledger section.
- Ensure JS bootstrap sets active links and reveal states on `DOMContentLoaded`.
- Provide reduced-motion overrides for `.reveal`, `.scanline`, and `.viz-fill`.
...
README Ledger default profile also requires Check 9A shell verification (`.terminal-header`, `#clock`, active-dot TOC, reduced-motion-safe reveal/viz behavior).
