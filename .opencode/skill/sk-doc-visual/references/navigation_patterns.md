---
description: "In-page navigation patterns for README Ledger layouts with sticky sidebar TOC, mobile sidebar-hide behavior, and scroll-spy via IntersectionObserver"
---
...
Patterns for in-page navigation: README Ledger default uses a sticky desktop sidebar TOC and hides the sidebar on mobile, with scroll spy via IntersectionObserver.
...
Token note: examples use `--ve-*` aliases for compatibility. Map them from README Ledger tokens via `references/css_patterns.md`.
...
The sidebar TOC is 260px wide in the default README Ledger shell. Content takes the remaining space. The sidebar sticks to the viewport top as the user scrolls.
...
## Mobile Layout — Hide Sidebar, Keep Content Full Width

On screens narrower than 1024px, hide the sidebar TOC and keep a clean single-column content flow.
...
Prevent section headings from being hidden behind the sticky terminal header when navigating via anchor links or scroll spy.
...
.page-content section {
  padding-top: 40px;
  margin-top: -40px;  /* compensate for anchor offset rhythm */
}
