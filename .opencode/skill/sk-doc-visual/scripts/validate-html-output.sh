#!/usr/bin/env bash
# ───────────────────────────────────────────────────────────────
# COMPONENT: DOC VISUAL HTML OUTPUT VALIDATOR
# ───────────────────────────────────────────────────────────────
# Static analysis checks for sk-doc-visual HTML output.
#
...
echo ""
echo -e "${BOLD}Doc Visual — HTML Output Validator${RESET}"
...
if echo "$HEAD_CONTENT" | grep -qi '<!DOCTYPE html'; then
  ...
  info "Every sk-doc-visual page must begin with <!DOCTYPE html>"
fi
...
if grep -qi 'fonts\.googleapis\.com\|fonts\.gstatic\.com\|fontshare\.com\|bunny\.net/fonts' "$HTML_FILE"; then
  ...
  info "Doc visual pages should use a Google Font pairing (see quick_reference.md)"
...
if grep -qi 'prefers-color-scheme' "$HTML_FILE"; then
  ...
  info "All sk-doc-visual pages must support both light and dark themes"
  info "Add: @media (prefers-color-scheme: dark) { :root { --bg: ...; } }"
fi
...
# ── CHECK 11: Token system coverage (legacy + README Ledger) ──────────────────
...
README_LEDGER_TOKENS=("--bg" "--surface" "--text" "--accent" "--muted" "--border")
LEDGER_TOKEN_COUNT=0
for token in "${README_LEDGER_TOKENS[@]}"; do
  if grep -qiE -- "${token}[[:space:]]*:" "$HTML_FILE"; then
    LEDGER_TOKEN_COUNT=$((LEDGER_TOKEN_COUNT + 1))
  fi
done

HAS_VE_TOKEN_SYSTEM=false
HAS_LEDGER_TOKEN_SYSTEM=false
if [[ $VE_TOKEN_COUNT -ge 6 ]]; then
  HAS_VE_TOKEN_SYSTEM=true
fi
if [[ $LEDGER_TOKEN_COUNT -eq 6 ]]; then
  HAS_LEDGER_TOKEN_SYSTEM=true
fi

if $HAS_VE_TOKEN_SYSTEM && $HAS_LEDGER_TOKEN_SYSTEM; then
  pass "Dual token systems detected (--ve-* and README Ledger token set)"
elif $HAS_VE_TOKEN_SYSTEM; then
  pass "--ve-* token system detected (${VE_TOKEN_COUNT} unique tokens)"
elif $HAS_LEDGER_TOKEN_SYSTEM; then
  pass "README Ledger token system detected (${LEDGER_TOKEN_COUNT}/6 required tokens)"
elif [[ $VE_TOKEN_COUNT -ge 1 || $LEDGER_TOKEN_COUNT -ge 3 ]]; then
  warn "Limited token coverage (--ve: ${VE_TOKEN_COUNT}, ledger: ${LEDGER_TOKEN_COUNT}/6)"
  info "Use full README Ledger token set or a broader --ve-* variable system"
else
  fail "No valid design token system found"
  info "Define README Ledger tokens (--bg/--surface/--text/--accent/--muted/--border) or legacy --ve-* tokens"
fi

# ── CHECK 12: Typography guardrails ────────────────────────────────────────────
section "12" "Typography Guardrails"

PRIMARY_FONT_LINES=$(grep -oiE -- '--(ve-)?font-(display|body)[[:space:]]*:[^;]+' "$HTML_FILE" 2>/dev/null || true)
MONO_FONT_LINES=$(grep -oiE -- '--(ve-)?font-mono[[:space:]]*:[^;]+' "$HTML_FILE" 2>/dev/null || true)

PRIMARY_HAS_INTER=false
PRIMARY_HAS_ROBOTO_OR_ARIAL=false
MONO_HAS_JETBRAINS=false
GOOGLE_HAS_JETBRAINS=false

if echo "$PRIMARY_FONT_LINES" | grep -qi 'Inter'; then
  PRIMARY_HAS_INTER=true
fi
if echo "$PRIMARY_FONT_LINES" | grep -qiE 'Roboto|Arial'; then
  PRIMARY_HAS_ROBOTO_OR_ARIAL=true
fi
if echo "$MONO_FONT_LINES" | grep -qi 'JetBrains Mono'; then
  MONO_HAS_JETBRAINS=true
fi
if grep -qiE 'family=JetBrains\+Mono([:&]|$)' "$HTML_FILE"; then
  GOOGLE_HAS_JETBRAINS=true
fi

if $PRIMARY_HAS_ROBOTO_OR_ARIAL; then
  fail "Primary font variables use Roboto/Arial (disallowed)"
  info "Use README Ledger defaults (Inter + JetBrains Mono) or non-generic alternatives"
elif $PRIMARY_HAS_INTER; then
  if $HAS_LEDGER_TOKEN_SYSTEM; then
    if $MONO_HAS_JETBRAINS || $GOOGLE_HAS_JETBRAINS; then
      pass "README Ledger typography detected (Inter primary + JetBrains Mono)"
    else
      fail "Inter primary detected without JetBrains Mono pairing for README Ledger profile"
      info "Use --font-mono (or --ve-font-mono) with JetBrains Mono when Inter is primary"
    fi
  else
    fail "Inter primary is only allowed when README Ledger token system is present"
    info "Add full ledger tokens or switch to a non-Inter primary pairing"
  fi
else
  pass "Primary typography guardrails passed (no Roboto/Arial primary variables)"
fi
