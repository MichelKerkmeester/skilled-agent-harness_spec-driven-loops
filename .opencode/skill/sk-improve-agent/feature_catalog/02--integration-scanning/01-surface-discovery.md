...
One part of the discovery map is stale in the current runtime. The script hardcodes the skill-advisor path as `.opencode/skill/scripts/skill_advisor.py`, but the repo's actual advisor script lives under `.opencode/skill/skill-advisor/scripts/skill_advisor.py`, so that discovery branch only succeeds if the older path also exists.
