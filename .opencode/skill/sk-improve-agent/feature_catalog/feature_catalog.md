...
`scan-integration.cjs` scans the canonical agent file, three runtime mirrors, improve command markdown, YAML workflow assets, skill references, global docs, and a skill-advisor path constant. That last path currently points at `.opencode/skill/scripts/skill_advisor.py`, so the skill-advisor surface only registers if that stale location exists.
