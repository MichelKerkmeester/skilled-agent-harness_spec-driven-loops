...
`handlers/advisor-recommend.ts` implements the tool. Input is validated by `schemas/advisor-tool-schemas.ts` (Zod strict). Output carries `recommendations[]`, optional `laneBreakdown[]` when `includeAttribution: true`, `lifecycle` redirect metadata, `cache` state, `trustState`, `warnings`, and `abstainReasons`. The handler enforces sanitization at the envelope boundary (see `lib/derived/sanitizer.ts`) and never echoes prompt text into response metadata. Cache behavior is tied to generation (see [`01--daemon-and-freshness/07-cache-invalidation.md`](../01--daemon-and-freshness/07-cache-invalidation.md)).
