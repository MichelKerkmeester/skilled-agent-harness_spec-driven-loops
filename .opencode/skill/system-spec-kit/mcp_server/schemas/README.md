...
`schemas/` holds a single file that defines every Zod validation schema used by MCP tool handlers. All 27 tool schemas are declared here, exported through a `TOOL_SCHEMAS` registry and consumed by tool modules via the `validateToolArgs()` function.
...
| File | Purpose |
|---|---|
| `tool-input-schemas.ts` | Defines Zod schemas for all 27 MCP tools, exports `TOOL_SCHEMAS` registry, `validateToolArgs()`, `getToolSchema()`, `formatZodError()`, `getSchema()` and the `ToolSchemaValidationError` class |
