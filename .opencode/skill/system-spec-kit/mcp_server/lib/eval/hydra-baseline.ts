...
interface CaptureHydraBaselineOptions {
  specFolder?: string | null;
  evalRunId?: number;
  persist?: boolean;
  metadata?: Record<string, unknown>;
...
function persistHydraBaselineSnapshot(snapshot: HydraBaselineSnapshot, evalDb: Database.Database): number {
  ...
  for (const [metricName, metricValue] of Object.entries(snapshot.metrics)) {
    insert.run(snapshot.evalRunId, metricName, metricValue, 'hydra-baseline', null, metadata);
    persisted += 1;
  }
 ...
 */
function captureHydraBaselineSnapshot(options: CaptureHydraBaselineOptions = {}): HydraBaselineSnapshot {
  ...
  try {
    ...
    if (options.persist) {
      snapshot.persistedRows = persistHydraBaselineSnapshot(snapshot, evalDb);
    }
...
export {
  captureHydraBaselineSnapshot,
  persistHydraBaselineSnapshot,
};

export type {
  CaptureHydraBaselineOptions,
  HydraBaselineSnapshot,
};
