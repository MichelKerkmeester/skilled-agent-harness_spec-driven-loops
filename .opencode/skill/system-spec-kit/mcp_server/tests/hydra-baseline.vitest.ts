...
import { captureHydraBaselineSnapshot } from '../lib/eval/hydra-baseline';
...
describe('Hydra baseline snapshots', () => {
  let tempDir = '';

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hydra-baseline-'));
    closeEvalDb();
  });
  ...
  it('captures and persists baseline metrics beside the target context db', () => {
    ...
    const snapshot = captureHydraBaselineSnapshot({
      contextDbPath,
      evalRunId: 42,
      persist: true,
    ...
    try {
      const row = evalDb.prepare(`
        ...
        WHERE eval_run_id = 42 AND channel = 'hydra-baseline'
      `).get() as { total: number };
