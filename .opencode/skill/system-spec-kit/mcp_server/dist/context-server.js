...
import { ListToolsRequestSchema, CallToolRequestSchema, ListPromptsRequestSchema, ListResourcesRequestSchema, ListResourceTemplatesRequestSchema, } from '@modelcontextprotocol/sdk/types.js';
// Core modules
import { DEFAULT_BASE_PATH, ALLOWED_BASE_PATHS, DATABASE_PATH, checkDatabaseUpdated, setEmbeddingModelReady, waitForEmbeddingModel, init as initDbState } from './core/index.js';
...
import { indexSingleFile, handleMemoryStats, } from './handlers/index.js';
...
// Hooks
import { MEMORY_AWARE_TOOLS, extractContextHint, autoSurfaceMemories, autoSurfaceAtToolDispatch, autoSurfaceAtCompaction, appendAutoSurfaceHints, syncEnvelopeTokenCount, serializeEnvelopeWithTokenCount, recordToolCall, } from './hooks/index.js';
...
import { getStartupEmbeddingDimension, resolveStartupEmbeddingConfig, validateConfiguredEmbeddingsProvider, } from '@spec-kit/shared/embeddings/factory';
...
import { detectRuntime } from './lib/code-graph/runtime-detection.js';
...
import { isDegreeBoostEnabled, isDynamicInitEnabled, isFileWatcherEnabled, } from './lib/search/search-flags.js';
...
function isStartupScanDisabled() {
    const env = process.env.SPECKIT_DISABLE_STARTUP_SCAN?.trim().toLowerCase();
...
const GRAPH_CONTEXT_EXCLUDED_TOOLS = new Set([
    ...MEMORY_AWARE_TOOLS,
    'code_graph_query',
    'code_graph_context',
    'code_graph_scan',
...
function isMutationStatus(status) {
...
export function getDetectedRuntime() {
    return detectedRuntime;
}
function resolveToolCallId(request) {
...
function resolveSessionTrackingId(args, extra) {
    const transportSessionId = typeof extra?.sessionId === 'string'
...
function runAfterToolCallbacks(tool, callId, result) {
    if (afterToolCallbacks.length === 0) {
        return;
    }
    queueMicrotask(() => {
        for (const callback of afterToolCallbacks) {
            void callback(tool, callId, result).catch((error) => {
...
function looksLikeGraphableFilePath(value) {
...
function normalizeGraphFilePath(value) {
...
function extractFilePathsFromToolArgs(args) {
...
function buildDispatchGraphContext(filePaths, deadlineMs) {
    const files = [];
    for (const filePath of filePaths) {
        ...
        for (const node of outline) {
            ...
            for (const entry of graphDb.queryEdgesFrom(node.symbolId)) {
                ...
                if (!bucket) {
                    bucket = { relationTypes: new Set(), symbols: [] };
...
async function resolveDispatchGraphContext(toolName, args) {
    ...
    const buildPromise = new Promise((resolve) => {
        queueMicrotask(() => {
            try {
                const context = buildDispatchGraphContext(filePaths, startedAt + GRAPH_ENRICHMENT_TIMEOUT_MS);
...
function injectSessionPrimeHints(envelope, meta, sessionPrimeContext) {
    ...
    hints.push(`Session priming: loaded ${constitutionalCount} constitutional memories and ${codeGraphState}`);
...
export function registerAfterToolCallback(fn) {
    afterToolCallbacks.push(fn);
}
async function invalidateReinitializedDbCaches() {
...
──────────────────────────────────────────────────────────────── */
const server = new Server({ name: 'context-server', version: '1.7.2' }, {
    capabilities: {
        tools: {},
...
const serverWithInstructions = server;
/* ───────────────────────────────────────────────────────────────
...
server.setRequestHandler(CallToolRequestSchema, async (request, _extra) => {
    ...
    const callId = resolveToolCallId(request);
    ...
    try {
        ...
        if (typeof args.specFolder === 'string' && args.specFolder) {
            recordMetricEvent({ kind: 'spec_folder_change', specFolder: args.specFolder });
        }
        const dbReinitialized = await checkDatabaseUpdated();
        if (dbReinitialized) {
            await invalidateReinitializedDbCaches();
        }
        let sessionPrimeContext = null;
        try {
            sessionPrimeContext = await primeSessionIfNeeded(name, args, sessionTrackingId);
        }
        catch (primeErr) {
        ...
        let autoSurfacedContext = null;
        const isCompactionLifecycleCall = name === 'memory_context' && args.mode === 'resume';
            ...
            if (queryStr && codeSearchPattern.test(queryStr)) {
                try {
                    ...
                    if (envelope && typeof envelope === 'object' && !Array.isArray(envelope)) {
                        const existingHints = Array.isArray(envelope.hints) ? envelope.hints : [];
        ...
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return wrapForMCP(errorResponse, true);
        }
        catch (wrapError) {
...
function getStartupWorkspaceRoots(basePath) {
    ...
    return Array.from(new Set([basePath, derivedMemoryRoot, ...ALLOWED_BASE_PATHS]
        .filter((root) => typeof root === 'string' && root.trim().length > 0)
        .map((root) => path.resolve(root))));
}
function getPendingRecoveryLocations(basePath) {
    const scanLocations = [];
 ...
 */
async function recoverPendingFiles(basePath) {
    ...
    try {
        ...
        const isCommittedInDb = (originalPath) => {
            const row = database.prepare('SELECT 1 FROM memory_index WHERE file_path = ?').get(originalPath);
            return !!row;
        };
        const rawResults = existingScanLocations.flatMap((location) => transactionManager.recoverAllPendingFiles(location, isCommittedInDb));
...
async function startupScan(basePath) {
    ...
    try {
        ...
        const scanRoots = Array.from(new Set([basePath, ...ALLOWED_BASE_PATHS]
            .filter((root) => typeof root === 'string' && root.trim().length > 0)
            .map((root) => path.resolve(root))));
        const allFiles = [];
        const seen = new Set();
...
async function fatalShutdown(reason, exitCode) {
    if (shuttingDown)
        return;
    shuttingDown = true;
...
async function removeIndexedMemoriesForFile(filePath) {
    const database = vectorIndex.getDb();
    if (!database) {
        return;
    }
...
process.on('unhandledRejection', (reason, promise) => {
    void fatalShutdown(`Unhandled rejection at: ${String(promise)} reason: ${reason instanceof Error ? reason.stack ?? reason.message : String(reason)}`, 1);
});
/* ───────────────────────────────────────────────────────────────
   8. MAIN
──────────────────────────────────────────────────────────────── */
async function main() {
    ...
    console.error(`[context-server] Detected runtime: ${detectedRuntime.runtime} (hookPolicy=${detectedRuntime.hookPolicy})`);
    ...
    let startupEmbeddingConfig = null;
    if (process.env.SPECKIT_SKIP_API_VALIDATION !== 'true') {
        ...
        try {
            ...
            const validation = startupEmbeddingConfig.validation;
            if (!validation.valid) {
    ...
    if (!process.env.EMBEDDING_DIM) {
        process.env.EMBEDDING_DIM = String(startupEmbeddingConfig?.dimension ?? getStartupEmbeddingDimension());
    }
    ...
    try {
        ...
        if (report.orphanedVectors > 0)
            console.error(`[context-server] WARNING: ${report.orphanedVectors} orphaned entries detected`);
        ...
        const walRow = database.prepare('PRAGMA journal_mode').get();
        ...
        const memoryCountRow = database.prepare('SELECT COUNT(*) as cnt FROM memory_index').get();
        const memoryCount = Number(memoryCountRow?.cnt ?? 0);
        let projectionCount = 0;
        let projectionTableExists = true;
        try {
            const projectionCountRow = database.prepare('SELECT COUNT(*) as cnt FROM active_memory_projection').get();
    ...
    if (isStartupScanDisabled()) {
        console.error('[context-server] Startup scan disabled (SPECKIT_DISABLE_STARTUP_SCAN=true)');
    }
    else {
...
//# sourceMappingURL=context-server.js.map