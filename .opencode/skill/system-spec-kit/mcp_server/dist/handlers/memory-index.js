...
import { processBatches, requireDb, toErrorMessage } from '../utils/index.js';
...
import { EMPTY_ALIAS_CONFLICT_SUMMARY, createDefaultDivergenceReconcileSummary, detectAliasConflictsFromIndex, summarizeAliasConflicts, runDivergenceReconcileHooks, } from './memory-index-alias.js';
...
async function indexSingleFile(filePath, force = false, options) {
...
async function handleMemoryIndexScan(args) {
    ...
    const { specFolder: spec_folder = null, force = false, includeConstitutional: include_constitutional = true, includeSpecDocs: include_spec_docs = true, incremental = true } = args;
    ...
    const specFiles = memoryParser.findMemoryFiles(workspacePath, { specFolder: spec_folder });
    const constitutionalFiles = include_constitutional ? findConstitutionalFiles(workspacePath) : [];
    const specDocFiles = include_spec_docs ? findSpecDocuments(workspacePath, { specFolder: spec_folder }) : [];
    const canonicalKeyCache = new Map();
    const getCachedKey = (filePath) => {
    ...
    const deleteStaleIndexedRecords = (paths) => {
        if (paths.length === 0) {
        ...
        for (const staleRecordId of staleRecordIds) {
            try {
                const staleSnapshot = vectorIndex.getDb()?.prepare('SELECT spec_folder, file_path FROM memory_index WHERE id = ?').get(staleRecordId);
                if (vectorIndex.deleteMemory(staleRecordId)) {
                    deleted++;
                    // Record DELETE history only after confirmed deletion.
                    try {
                        recordHistory(staleRecordId, 'DELETE', staleSnapshot?.file_path ?? null, null, 'mcp:memory_index_scan', staleSnapshot?.spec_folder ?? null);
                    }
                    catch (_histErr) {
    ...
    const runScanInvalidationHooks = (context) => {
        try {
    ...
    if (files.length === 0) {
        ...
        if (incremental && !force) {
            const categorized = incrementalIndex.categorizeFilesForIndexing([]);
    ...
    if (incremental && !force) {
        ...
        const categorized = incrementalIndex.categorizeFilesForIndexing(files);
    ...
    const successfullyIndexedFiles = [];
    if (filesToIndex.length > 0) {
        const batchResults = await processBatches(filesToIndex, async (filePath) => {
        ...
        for (let i = 0; i < batchResults.length; i++) {
            ...
            if (result.error) {
                ...
                const isSuccessfulStatus = result.status === 'success' ||
    ...
    if (include_spec_docs) {
        try {
            ...
            const affectedSpecFolders = new Set();
        ...
        `);
                ...
                for (const folder of affectedSpecFolders) {
                    const rows = selectDocIds.all(folder);
                    const docIds = {};
