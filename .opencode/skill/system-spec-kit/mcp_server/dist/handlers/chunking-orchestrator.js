...
function computeNormalizedChunkCacheKey(content) {
...
function applyPostInsertMetadataFallback(db, memoryId, fields) {
    const entries = Object.entries(fields).filter(([col, value]) => ALLOWED_METADATA_COLUMNS.has(col) && value !== undefined);
    if (entries.length === 0) {
        return;
    }
    // Use COALESCE for encoding_intent to preserve existing value when new value is null
    const setClause = entries.map(([column]) => column === 'encoding_intent' ? `${column} = COALESCE(?, ${column})` : `${column} = ?`).join(', ');
    const values = entries.map(([, value]) => value);
    db.prepare(`UPDATE memory_index SET ${setClause} WHERE id = ?`).run(...values, memoryId);
}
function deleteMemoriesBulk(memoryIds) {
    const deletedIds = [];
    const failedIds = [];
 ...
 */
async function indexChunkedMemoryFile(filePath, parsed, { force = false, applyPostInsertMetadata } = {}) {
    ...
    const setupParent = database.transaction(() => {
      ...
      ORDER BY id DESC
      LIMIT 1
    `).get(parsed.specFolder, canonicalFilePath, filePath);
        let pid;
        if (existing && !force) {
            pid = existing.id;
    ...
    `).all(parentId);
        ...
        const finalizeSwapTx = database.transaction((newChildIds) => {
            const importanceWeight = calculateDocumentWeight(filePath, parsed.documentType);
            const specLevel = isSpecDocumentType(parsed.documentType)
                ? detectSpecLevelFromParsed(filePath)
                : null;
      ...
      `).run(parentId, ...newChildIds);
        ...
        SET content_hash = ?,
            ...
            quality_flags = ?
        WHERE id = ?
      `).run(parsed.contentHash, parsed.contextType, parsed.importanceTier, importanceWeight, parentEncodingIntent, chunkResult.parentSummary, fileMtimeMs, parsed.documentType || 'memory', specLevel, parsed.qualityScore ?? 0, JSON.stringify(parsed.qualityFlags ?? []), parentId);
...
//# sourceMappingURL=chunking-orchestrator.js.map