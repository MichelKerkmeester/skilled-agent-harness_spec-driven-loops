...
import { findSimilarMemories, reinforceExistingMemory, updateExistingMemory, logPeDecision, } from '../pe-gating.js';
export function evaluateAndApplyPeDecision(database, parsed, embedding, force, validationWarnings, embeddingStatus, filePath, scope) {
    let peDecision = { action: 'CREATE', similarity: 0 };
    let candidates = [];
    ...
    if (!force && candidates.length > 0) {
        peDecision = predictionErrorGate.evaluateMemory(parsed.contentHash, parsed.content, candidates, { specFolder: parsed.specFolder });
        ...
        if ((peDecision.action === predictionErrorGate.ACTION.REINFORCE ||
        ...
        switch (peDecision.action) {
            case predictionErrorGate.ACTION.REINFORCE: {
                const existingId = peDecision.existingMemoryId;
            ...
            case predictionErrorGate.ACTION.UPDATE: {
                ...
                if (!embedding) {
                    console.warn('[Memory Save] embedding unexpectedly null in UPDATE path, falling through to CREATE');
                    break;
                }
...
//# sourceMappingURL=pe-orchestration.js.map