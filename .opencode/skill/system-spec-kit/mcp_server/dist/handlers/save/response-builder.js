...
export function buildIndexResult({ database, existing, embeddingStatus, id, parsed, validation, reconWarnings, peDecision, embeddingFailureReason, asyncEmbedding, causalLinksResult, enrichmentStatus, filePath, }) {
    let resultStatus;
    if (existing) {
    ...
    if (causalLinksResult) {
        ...
        if (causalLinksResult.errors.length > 0) {
            result.causalLinks.errors = causalLinksResult.errors;
        }
    }
...
export function buildSaveResponse({ result, filePath, asyncEmbedding, requestId }) {
    ...
    if (result.warnings && result.warnings.length > 0) {
        ...
        const typedWarnings = result.warnings.map((w) => {
        ...
        response.typedWarnings = typedWarnings;
        ...
        if (anchorWarnings.length > 0)
            parts.push(`${anchorWarnings.length} anchor issue(s)`);
        if (persistenceWarnings.length > 0)
            parts.push(`${persistenceWarnings.length} file-persistence warning(s)`);
    ...
    if (result.causalLinks) {
        ...
        if (result.causalLinks.inserted > 0) {
            hints.push(`Created ${result.causalLinks.inserted} causal graph edge(s)`);
        }
        if (result.causalLinks.unresolved_count > 0) {
            hints.push(`${result.causalLinks.unresolved_count} causal link reference(s) could not be resolved`);
        }
    }
    if (postMutationFeedback) {
    ...
    if (result.embeddingStatus === 'success') {
        retryManager.processRetryQueue(2).catch((err) => {
    ...
    try {
        ...
        if (consolidation) {
            ...
            hints.push(`N3-lite consolidation: +${consolidation.hebbian.strengthened} strengthened, ` +
