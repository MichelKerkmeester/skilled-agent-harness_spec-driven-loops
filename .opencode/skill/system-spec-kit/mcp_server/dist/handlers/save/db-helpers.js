 ...
 */
export function hasReconsolidationCheckpoint(database, specFolder) {
    try {
        const tableExists = database.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='checkpoints'").get();
        if (!tableExists) {
            return false;
        }
