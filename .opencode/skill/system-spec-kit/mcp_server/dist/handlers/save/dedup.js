...
const DEDUP_ELIGIBLE_EMBEDDING_STATUSES = ['success', 'partial'];
...
function buildScopedWhereClauses(scope) {
...
function selectLatestExistingRow(database, parsed, pathColumn, pathValue, scopeClauses, scopeParams) {
    const whereClauses = [
        'spec_folder = ?',
        'parent_id IS NULL',
        `${pathColumn} = ?`,
    ...
    ORDER BY id DESC
    LIMIT 1
  `).get(parsed.specFolder, pathValue, ...scopeParams);
}
...
function areEquivalentStringArrays(left, right) {
...
function isSamePathMetadataEquivalent(existing, parsed) {
...
export function checkExistingRow(database, parsed, canonicalFilePath, filePath, force, warnings, scope = {}) {
    const { clauses: scopeClauses, params: scopeParams } = buildScopedWhereClauses(scope);
    const candidates = [
        selectLatestExistingRow(database, parsed, 'canonical_file_path', canonicalFilePath, scopeClauses, scopeParams),
    ];
    if (filePath !== canonicalFilePath) {
        candidates.push(selectLatestExistingRow(database, parsed, 'file_path', filePath, scopeClauses, scopeParams));
    }
...
export function checkContentHashDedup(database, parsed, force, warnings, samePathExclusion, scope = {}) {
    if (!force) {
        const { clauses: scopeClauses, params: scopeParams } = buildScopedWhereClauses(scope);
        const whereClauses = [
            'spec_folder = ?',
    ...
    `).get(...duplicateParams);
        if (duplicateByHash) {
            const verifiedMatch = verifyStoredContentMatch(duplicateByHash.content_text, duplicateByHash.file_path, parsed.content);
