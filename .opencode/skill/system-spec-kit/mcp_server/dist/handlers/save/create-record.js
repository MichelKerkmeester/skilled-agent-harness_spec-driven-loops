...
export function resolveCreateRecordLineage(peDecision, samePathExistingId) {
...
function recordCrossPathSupersedesEdge(database, memoryId, supersededMemoryId, reason) {
...
export function findSamePathExistingMemory(database, specFolder, canonicalFilePath, filePath, scope = {}) {
    ...
    ORDER BY id DESC
    LIMIT 1
  `).get(specFolder, canonicalFilePath, filePath, tenantId, tenantId, userId, userId, agentId, agentId, sessionId, sessionId, sharedSpaceId, sharedSpaceId);
}
/**
 ...
 */
export function createMemoryRecord(database, parsed, filePath, embedding, embeddingFailureReason, peDecision, scope = {}) {
    if (!embedding) {
    ...
    const indexWithMetadata = database.transaction(() => {
      ...
      `).run(new Date().toISOString(), predecessorMemoryId);
        ...
        recordCrossPathSupersedesEdge(database, memory_id, lineageRouting.causalSupersedesMemoryId, peDecision.reason);
        ...
        try {
            ...
            if (predecessorMemoryId != null) {
                recordHistory(predecessorMemoryId, 'UPDATE', samePathExisting?.title ?? null, parsed.title ?? filePath, 'mcp:memory_save');
            }
        }
        catch (_histErr) {
