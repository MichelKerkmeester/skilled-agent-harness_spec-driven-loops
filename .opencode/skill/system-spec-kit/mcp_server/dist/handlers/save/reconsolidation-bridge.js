...
import { isEncodingIntentEnabled, isReconsolidationEnabled as isReconsolidationFlagEnabled, isAssistiveReconsolidationEnabled as _isAssistiveReconsolidationEnabled, } from '../../lib/search/search-flags.js';
 ...
 */
export function isAssistiveReconsolidationEnabled() {
 ...
 */
export function classifyAssistiveSimilarity(similarity) {
    if (similarity >= ASSISTIVE_AUTO_MERGE_THRESHOLD)
        return 'auto_merge';
    if (similarity >= ASSISTIVE_REVIEW_THRESHOLD)
        return 'review';
 ...
 */
export function classifySupersededOrComplement(olderContent, newerContent) {
 ...
 */
export function logAssistiveRecommendation(recommendation) {
    console.warn(`[reconsolidation-bridge] assistive recommendation: ` +
 ...
 */
export async function runReconsolidationIfEnabled(database, parsed, filePath, force, embedding, scope) {
            ...
            if (!hasCheckpoint) {
                ...
                const reconResult = await reconsolidate({
                    ...
                    findSimilar: (emb, opts) => {
                        const searchEmb = emb instanceof Float32Array ? emb : new Float32Array(emb);
                        ...
                        const scopeFiltered = results.filter((r) => {
                            if (scope?.tenantId && r.tenant_id && r.tenant_id !== scope.tenantId)
                                return false;
                            if (scope?.userId && r.user_id && r.user_id !== scope.userId)
                                return false;
                            if (scope?.agentId && r.agent_id && r.agent_id !== scope.agentId)
                                return false;
                            if (scope?.sharedSpaceId && r.shared_space_id && r.shared_space_id !== scope.sharedSpaceId)
                                return false;
                            return true;
    ...
    // ─────────────────────────────────────────────────────────────
    let assistiveRecommendation = null;
    if (!force && isAssistiveReconsolidationEnabled() && embedding) {
        try {
            // Find the top similar memory using the existing vector search
            const searchEmb = embedding instanceof Float32Array ? embedding : new Float32Array(embedding);
            ...
            `).run(topId);
                        ...
                        console.warn(`[reconsolidation-bridge] assistive auto-merge: archived older=${topId} ` +
                    ...
                    assistiveRecommendation = {
                        ...
                        description: `Review borderline similarity ${similarity.toFixed(3)} between the pending save ` +
            ...
            console.warn(`[reconsolidation-bridge] assistive reconsolidation error (proceeding with normal save): ${message}`);
...
//# sourceMappingURL=reconsolidation-bridge.js.map