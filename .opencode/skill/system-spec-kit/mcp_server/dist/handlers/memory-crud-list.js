...
async function handleMemoryList(args) {
    ...
    const { limit: rawLimit = 20, offset: rawOffset = 0, specFolder, sortBy = 'created_at', includeChunks = false, } = args;
    ...
    try {
        ...
        const countResult = database.prepare(countSql).get(...baseParams);
    ...
    const memories = rows.map((row) => ({
        ...
        triggerCount: safeJsonParseTyped(row.trigger_phrases, 'array', []).length,
...
//# sourceMappingURL=memory-crud-list.js.map