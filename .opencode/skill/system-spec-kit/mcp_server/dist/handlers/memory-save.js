...
import { evaluateMemorySufficiency, MEMORY_SUFFICIENCY_REJECTION_CODE, } from '@spec-kit/shared/parsing/memory-sufficiency';
...
import { createMemoryRecord, findSamePathExistingMemory } from './save/create-record.js';
import { buildGovernancePostInsertFields, ensureGovernanceRuntime, recordGovernanceAudit, validateGovernedIngest, } from '../lib/governance/scope-governance.js';
import { assertSharedSpaceAccess, recordSharedConflict, } from '../lib/collab/shared-spaces.js';
...
import { validateMemoryQualityContent, determineValidationDisposition, } from './v-rule-bridge.js';
...
import { applyInsufficiencyMetadata, buildInsufficiencyRejectionResult, buildTemplateContractRejectionResult, buildDryRunSummary, } from './save/validation-responses.js';
...
const MANUAL_FALLBACK_SOURCE_CLASSIFICATION = 'manual-fallback';
...
function isVRuleUnavailableResult(value) {
...
function classifyMemorySaveSource(content) {
...
function shouldBypassTemplateContract(sourceClassification, sufficiencyResult, templateContract) {
...
function buildQualityLoopMetadata(parsed, database) {
    return {
        title: parsed.title ?? '',
        ...
        resolveReference: (reference) => resolveMemoryReference(database, reference) !== null,
    };
}
function prepareParsedMemoryForIndexing(parsed, database, options = {}) {
    ...
    if (validation.warnings && validation.warnings.length > 0) {
        ...
        validation.warnings.forEach((warning) => console.warn(`[memory]   - ${warning}`));
    }
    ...
    if (vRuleResult && !isVRuleUnavailableResult(vRuleResult) && !vRuleResult.valid) {
        const vRuleDisposition = determineValidationDisposition(vRuleResult.failedRules, parsed.memoryTypeSource || null);
        if (vRuleDisposition && vRuleDisposition.disposition === 'abort_write') {
            ...
            return {
                ...
                templateContract: { valid: false, violations: [], missingAnchors: [], unexpectedTemplateArtifacts: [] },
                specDocHealth: null,
        ...
        if (vRuleDisposition && vRuleDisposition.disposition === 'write_skip_index') {
            ...
            const parsedWithIndexHints = parsed;
    ...
    if (parsed.specFolder && parsed.filePath) {
        try {
            ...
            console.warn(`[memory-save] spec-doc-health annotation skipped for ${path.basename(parsed.filePath)}: ${message}`);
        }
    }
    return {
        parsed,
...
async function finalizeMemoryFileContent(filePath, content) {
    ...
    try {
        ...
        if (backupCreated) {
            try {
                ...
                console.warn('[memory-save] Auto-fix file restore failed after finalize error:', restoreErr instanceof Error ? restoreErr.message : String(restoreErr));
            }
        }
        throw writeErr;
    }
...
function recordCrossPathPeSupersedes(database, memoryId, supersededMemoryId, samePathMemoryId, reason) {
    ...
    causalEdges.insertEdge(String(memoryId), String(supersededMemoryId), causalEdges.RELATION_TYPES.SUPERSEDES, 1.0, evidence, true, 'auto');
}
function createChunkedInsertTracker() {
    return {
        parentId: null,
        childIds: new Set(),
    };
}
function trackChunkedInsert(tracker, memoryId, fields) {
...
function rollbackCreatedChunkTree(database, tracker) {
    if (tracker.parentId == null) {
        return { attempted: false, cleaned: false };
    }
    const childIds = new Set(tracker.childIds);
    try {
        const persistedChildRows = database.prepare(`
      SELECT id
      FROM memory_index
      WHERE parent_id = ?
    `).all(tracker.parentId);
...
function captureAtomicSaveOriginalState(filePath) {
...
function getAtomicSaveErrorMessage(error) {
    return error instanceof Error ? error.message : String(error);
}
function deleteAtomicSaveFile(filePath) {
...
function restoreAtomicSaveOriginalState(filePath, originalState) {
    try {
...
function cleanupAtomicSavePendingFile(pendingPath) {
    try {
...
async function processPreparedMemory(prepared, filePath, options = {}) {
    const { force = false, asyncEmbedding = false, persistQualityLoopContent = true, refreshFromDiskAfterLock = false, specFolderLockAlreadyHeld = false, scope = {}, qualityGateMode = 'enforce', } = options;
    const evaluatePreparedMemory = (currentPrepared) => {
        const { parsed, validation, qualityLoopResult, sufficiencyResult, templateContract, sourceClassification, } = currentPrepared;
        ...
        if (!templateContract.valid) {
            if (templateContractBypassed) {
                console.warn(`[memory-save] Template contract bypassed in ${MANUAL_FALLBACK_SOURCE_CLASSIFICATION} mode for ${path.basename(filePath)}: ${templateContract.violations.map((v) => v.message || v.rule).join('; ')}`);
            }
            else if (qualityGateMode === 'warn-only') {
                console.warn(`[memory-save] Template contract warn-only (spec doc) for ${path.basename(filePath)}: ${templateContract.violations.map((v) => v.message || v.rule).join('; ')}`);
            }
            else {
    ...
    const runWithinSpecFolderLock = async () => {
        ...
        const { parsed, validation, finalizedFileContent, } = activePrepared;
        const canonicalFilePath = getCanonicalPathKey(filePath);
        const samePathExisting = findSamePathExistingMemory(database, parsed.specFolder, canonicalFilePath, filePath, scope);
        ...
        const existingResult = checkExistingRow(database, parsed, canonicalFilePath, filePath, force, validation.warnings, scope);
        if (existingResult)
        ...
        const { embedding, status: embeddingStatus, failureReason: embeddingFailureReason, pendingCacheWrite, } = embeddingResult;
            ...
            try {
                const qualityGateResult = runQualityGate({
                    ...
                    findSimilar: embedding ? (emb, gateOptions) => {
                        return findSimilarMemories(emb, {
        ...
        // PE GATING
        const peResult = evaluateAndApplyPeDecision(database, parsed, embedding, force, validation.warnings, embeddingStatus, filePath, scope);
        if (peResult.earlyReturn)
            return peResult.earlyReturn;
        let reconResult = {
            earlyReturn: null,
            warnings: [],
        };
        ...
        reconResult = await runReconsolidationIfEnabled(database, parsed, filePath, force, embedding, scope);
        ...
        if (shouldChunkContent) {
            ...
            const chunkedResult = await indexChunkedMemoryFile(filePath, parsed, {
                ...
                applyPostInsertMetadata: (db, memoryId, fields) => {
                    applyPostInsertMetadata(db, memoryId, fields);
                    trackChunkedInsert(chunkedInsertTracker, memoryId, fields);
                },
            });
                ...
                try {
                    const finalizeChunkedPeTx = database.transaction(() => {
                        if (chunkedResult.id > 0) {
                            recordCrossPathPeSupersedes(database, chunkedResult.id, peResult.supersededId, samePathExisting?.id ?? null, peResult.decision.reason);
                        }
        ...
        const writeTransaction = database.transaction(() => {
            // CREATE NEW MEMORY
            existing = findSamePathExistingMemory(database, parsed.specFolder, canonicalFilePath, filePath, scope);
            const memoryId = existing && existing.content_hash !== parsed.contentHash
                ...
                : createMemoryRecord(database, parsed, filePath, embedding, embeddingFailureReason, peResult.decision, scope);
        ...
        const id = writeTransaction.immediate();
...
async function indexMemoryFile(filePath, { force = false, parsedOverride = null, asyncEmbedding = false, scope = {}, qualityGateMode = 'enforce', } = {}) {
...
async function handleMemorySave(args) {
    ...
    const { filePath: file_path, force = false, dryRun = false, skipPreflight = false, asyncEmbedding = false, tenantId, userId, agentId, sessionId, sharedSpaceId, provenanceSource, provenanceActor, governedAt, retentionPolicy, deleteAfter, } = args;
    // Validate inputs before any I/O (checkDatabaseUpdated is deferred until after validation)
    if (!file_path || typeof file_path !== 'string') {
        throw new Error('filePath is required and must be a string');
    }
    await checkDatabaseUpdated();
    const validatedPath = validateFilePathLocal(file_path);
    const database = requireDb();
    ...
    if (dryRun && skipPreflight) {
        ...
        const templateContractPass = preparedDryRun.templateContract.valid
            || shouldBypassTemplateContract(preparedDryRun.sourceClassification, preparedDryRun.sufficiencyResult, preparedDryRun.templateContract);
        const { createMCPSuccessResponse } = await import('../lib/response/envelope.js');
        const dryRunSummary = shouldBypassTemplateContract(preparedDryRun.sourceClassification, preparedDryRun.sufficiencyResult, preparedDryRun.templateContract)
            ? 'Dry-run would pass in manual-fallback mode with deferred indexing.'
            : buildDryRunSummary(preparedDryRun.sufficiencyResult, preparedDryRun.qualityLoopResult, preparedDryRun.templateContract);
    ...
    if (!skipPreflight) {
        ...
        const preflightResult = preflight.runPreflight({
            ...
            database: database,
            find_similar: findSimilarMemories,
        ...
        if (!preflightResult.pass) {
            const errorMessages = preflightResult.errors.map((e) => typeof e === 'string' ? e : e.message).join('; ');
            ...
            throw new preflight.PreflightError(errorCode, `Pre-flight validation failed: ${errorMessages}`, {
                errors: preflightResult.errors,
                warnings: preflightResult.warnings,
                recoverable: true,
    ...
    if (typeof result.id === 'number' && result.id > 0 && result.status !== 'unchanged' && result.status !== 'duplicate') {
        ...
        try {
            ...
            try {
                delete_memory_from_database(database, result.id);
            }
 ...
 */
async function atomicSaveMemory(params, options = {}) {
    ...
    const mergeErrorMetadata = (entry) => {
        if (!entry) {
            return;
        }
        errorMetadata = {
    ...
    for (let attempt = 1; attempt <= maxIndexAttempts; attempt++) {
        ...
        try {
            const prepared = prepareParsedMemoryForIndexing(memoryParser.parseMemoryContent(file_path, content), database);
    ...
    const message = indexResult.message ?? (indexResult.status === 'duplicate'
...
function getAtomicityMetrics() {
    return transactionManager.getMetrics();
}
/* --- 12. EXPORTS --- */
export { 
// Primary exports (defined in this module)
indexMemoryFile, indexChunkedMemoryFile, handleMemorySave, atomicSaveMemory, getAtomicityMetrics, };
