...
async function handleMemoryStats(args) {
    ...
    const { folderRanking = 'count', excludePatterns = [], includeScores = false, includeArchived = false, limit: rawLimit = 10, } = args || {};
    ...
    let total = 0;
    let statusCounts = { success: 0, pending: 0, failed: 0, retry: 0, partial: 0 };
    let dates = { oldest: null, newest: null };
    let triggerCount = 0;
    let topFolders;
    ...
    try {
        const totalResult = database.prepare('SELECT COUNT(*) as count FROM memory_index').get();
        total = (totalResult && typeof totalResult.count === 'number') ? totalResult.count : 0;
        statusCounts = vectorIndex.getStatusCounts();
        dates = (database.prepare('SELECT MIN(created_at) as oldest, MAX(created_at) as newest FROM memory_index').get() || { oldest: null, newest: null });
        const triggerResult = database.prepare("SELECT SUM(json_array_length(trigger_phrases)) as count FROM memory_index WHERE trigger_phrases IS NOT NULL AND trigger_phrases != '[]'").get();
        triggerCount = (triggerResult && typeof triggerResult.count === 'number') ? triggerResult.count : 0;
        const tierRows = database.prepare('SELECT importance_tier, COUNT(*) as count FROM memory_index GROUP BY importance_tier').all();
        for (const row of tierRows) {
            tierBreakdown[row.importance_tier || 'normal'] = row.count;
        }
        const lastIndexedRow = database.prepare('SELECT MAX(updated_at) as last_indexed FROM memory_index').get();
    ...
    try {
        if (folderRanking === 'count') {
            const folderRows = database.prepare('SELECT spec_folder, COUNT(*) as count FROM memory_index GROUP BY spec_folder ORDER BY count DESC').all();
      ...
      `).all();
            ...
            try {
                ...
                if (folderRanking === 'recency') {
                    scoredFolders.sort((a, b) => (b.recencyScore ?? 0) - (a.recencyScore ?? 0));
                }
                else if (folderRanking === 'importance') {
                    scoredFolders.sort((a, b) => (b.importanceScore ?? 0) - (a.importanceScore ?? 0));
                }
                ...
                for (const memory of allMemories) {
                    const folder = memory.spec_folder || 'unknown';
            ...
            if (includeScores || folderRanking === 'composite') {
                topFolders = scoredFolders.map((folder) => ({
...
//# sourceMappingURL=memory-crud-stats.js.map