...
import { buildOpenCodeTransportPlan, } from '../lib/context/opencode-transport.js';
...
export async function handleSessionResume(args) {
    ...
    const graphOps = buildCodeGraphOpsContract({
        graphFreshness: codeGraph.status,
        sourceSurface: 'session_resume',
    });
    const result = {
        memory: memoryResult,
    ...
    if (!args.minimal) {
        recordBootstrapEvent('tool', Date.now() - startMs, 'full');
    }
    return {
        content: [{
                type: 'text',
