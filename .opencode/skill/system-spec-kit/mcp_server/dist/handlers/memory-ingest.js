...
import { createIngestJob, enqueueIngestJob, getIngestJob, cancelIngestJob, getIngestProgressPercent, getIngestForecast, } from '../lib/ops/job-queue.js';
 ...
 */
async function handleMemoryIngestStart(args) {
    await checkDatabaseUpdated();
    let paths = Array.isArray(args.paths)
        ? args.paths.filter((entry) => typeof entry === 'string' && entry.trim().length > 0)
        : [];
    if (paths.length === 0) {
    ...
    if (withinLength.length !== paths.length) {
        console.warn(`[memory-ingest] Dropped ${paths.length - withinLength.length} path(s) longer than ${MAX_PATH_LENGTH} characters`);
    }
    paths = withinLength;
    if (paths.length === 0) {
        throw new Error('paths must be a non-empty array of file paths');
    ...
    const allowedBasePaths = Array.from(new Set([derivedAllowedBasePath, ...ALLOWED_BASE_PATHS].map((basePath) => path.resolve(basePath))));
    ...
    for (const inputPath of paths) {
        ...
        const isWithinAllowedBase = allowedBasePaths.some((basePath) => (realPath === basePath || realPath.startsWith(`${basePath}${path.sep}`)));
...
async function handleMemoryIngestStatus(args) {
    await checkDatabaseUpdated();
...
async function handleMemoryIngestCancel(args) {
    await checkDatabaseUpdated();
    if (!args.jobId || typeof args.jobId !== 'string') {
        throw new Error('jobId is required and must be a string');
    }
