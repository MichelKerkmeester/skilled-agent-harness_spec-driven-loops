...
function sanitizeErrorForHint(msg) {
    return msg
        .replace(/(^|[\s(])\/(?:[^/\n]+\/)*[^:\n)"'\]]+/g, (_match, prefix) => `${prefix}[path]`)
        .replace(/(^|[\s(])[A-Za-z]:\\(?:[^\\\n]+\\)*[^:\n)"'\]]+/g, (_match, prefix) => `${prefix}[path]`)
...
function redactPath(absolutePath) {
    ...
    if (opencodeIdx !== -1)
        return normalizedPath.slice(opencodeIdx + 1);
    if (specsIdx !== -1)
        return normalizedPath.slice(specsIdx + 1);
...
function isSpecsAliasPath(filePath) {
    const normalizedPath = toNormalizedPath(filePath);
    return (normalizedPath.includes(DOT_OPENCODE_SPECS_SEGMENT) ||
...
function getDivergentAliasGroups(rows, limit) {
    if (!rows.length) {
        return [];
    }
    ...
    for (const [normalizedPath, bucket] of buckets.entries()) {
        ...
        const variants = Array.from(bucket.variants.entries())
...
async function handleMemoryHealth(args) {
    ...
    const { reportMode = 'full', limit: rawLimit = DEFAULT_DIVERGENT_ALIAS_LIMIT, specFolder, autoRepair = false, confirmed = false, } = args ?? {};
    ...
    let memoryCount = 0;
    let aliasConflicts = summarizeAliasConflicts([]);
    let aliasRows = [];
    let divergentAliasGroups = [];
    ...
    try {
      ...
      `;
            aliasRows = database.prepare(aliasSql).all(...params)
    ...
    let providerMetadata = embeddings.getProviderMetadata();
    let profile = embeddings.getEmbeddingProfile();
    ...
    const trackRepairOutcome = (succeeded) => {
    ...
    if (!profile) {
        try {
            ...
            profile = await embeddings.getEmbeddingProfileAsync();
            providerMetadata = embeddings.getProviderMetadata();
        }
        catch (profileError) {
    ...
    if (database) {
        try {
            const memoryCountRow = database.prepare('SELECT COUNT(*) as count FROM memory_index').get();
            const ftsCountRow = database.prepare('SELECT COUNT(*) as count FROM memory_fts').get();
            if (memoryCountRow.count !== ftsCountRow.count) {
                hints.push(`FTS5 index out of sync: memory_index has ${memoryCountRow.count} rows, memory_fts has ${ftsCountRow.count} rows. ` +
                ...
                if (autoRepair) {
                    try {
                        ...
                        const repairedFtsCountRow = database.prepare('SELECT COUNT(*) as count FROM memory_fts').get();
    ...
    if (autoRepair && database) {
        try {
            ...
            if (postRepairReport.orphanedVectors > 0 ||
                postRepairReport.missingVectors > 0 ||
                postRepairReport.orphanedFiles.length > 0 ||
                postRepairReport.orphanedChunks > 0) {
                repair.warnings.push(`Post-repair integrity still degraded: orphanedVectors=${postRepairReport.orphanedVectors}, ` +
...
//# sourceMappingURL=memory-crud-health.js.map