...
import { assertSharedSpaceAccess, createSharedSpaceIfAbsent, enableSharedMemory, ensureSharedCollabRuntime, isSharedMemoryEnabled, upsertSharedMembership, upsertSharedSpace, } from '../lib/collab/shared-spaces.js';
...
function throwSharedMemoryAuthError(message, response) {
 ...
 */
export function resolveAdminActor(tool) {
...
function validateSharedCallerIdentity(args) {
    ...
    if (hasBlankUser || hasBlankAgent) {
        throwSharedMemoryAuthError('Actor identities must be non-empty, non-whitespace strings.', createCallerAuthErrorResponse({
            tool: args.tool,
    ...
    if (hasUser && hasAgent) {
        throwSharedMemoryAuthError('Provide only one actor identity.', createCallerAuthErrorResponse({
            tool: args.tool,
    ...
    if (!hasUser && !hasAgent) {
        throwSharedMemoryAuthError('Caller authentication is required for shared-memory operations.', createCallerAuthErrorResponse({
            tool: args.tool,
...
function isTrustedCallerIdentityBindingEnabled() {
...
export function validateCallerAuth(args, tenantId) {
    const normalizedTenantId = typeof tenantId === 'string' ? tenantId.trim() : '';
    if (args.tool !== 'shared_memory_enable' && normalizedTenantId.length === 0) {
        throwSharedMemoryAuthError('Tenant scope is required for shared-memory admin mutations.', createCallerAuthErrorResponse({
            tool: args.tool,
    ...
    if (!isTrustedCallerIdentityBindingEnabled()) {
        throwSharedMemoryAuthError('Shared-memory admin mutations require trusted identity binding.', createCallerAuthErrorResponse({
            tool: args.tool,
...
function buildActorScope(actor, tenantId, sharedSpaceId) {
    return {
        tenantId,
        sharedSpaceId,
...
function recordSharedSpaceAdminAudit(database, args) {
...
function getSharedSpaceAccessErrorMessage(tool, spaceId, reason) {
    switch (reason) {
...
function normalizeOwnerAdminDenyReason(reason) {
    return reason === 'shared_space_membership_required'
        ? 'shared_space_owner_required'
        : reason;
}
function createSharedSpaceAuthError(tool, reason, error) {
    return createMCPErrorResponse({
        tool,
        error,
...
function createSharedMemoryEnableAuthError(reason, error) {
    return createMCPErrorResponse({
...
function createSharedMemoryInternalError(tool, prefix, error, hint) {
...
function warnTrustedAdminIdentityAssumption() {
    if (!isTrustedCallerIdentityBindingEnabled()) {
        return;
    }
...
function getAllowedSharedSpaceIdsForCaller(database, actor, tenantId) {
  ...
  `).all(actor.subjectType, actor.subjectId, normalizedTenantId, normalizedTenantId);
 ...
 */
export async function handleSharedSpaceUpsert(args) {
    try {
      ...
      `).get(args.spaceId);
            if (existingSpace) {
                if (!isAdmin) {
                    const access = assertSharedSpaceAccess(db, buildActorScope(actor, args.tenantId, args.spaceId), args.spaceId, 'owner');
                    if (!access.allowed) {
      ...
      `).get(args.spaceId);
        ...
        return createSharedMemoryInternalError('shared_space_upsert', 'Shared space upsert failed', error, 'Retry the operation. If the error persists, check database connectivity.');
    }
}
/**
 ...
 */
export async function handleSharedSpaceMembershipSet(args) {
    try {
        // WARNING: Admin mutations trust caller-supplied actor identity until transport-auth binding is added.
        warnTrustedAdminIdentityAssumption();
        const db = requireDb();
        ...
        if (!isAdmin) {
            ...
            if (!access.allowed) {
                ...
                return createSharedSpaceAuthError('shared_space_membership_set', reason, getSharedSpaceAccessErrorMessage('shared_space_membership_set', args.spaceId, reason));
            }
        }
        db.transaction(() => {
            upsertSharedMembership(db, {
        ...
        return createSharedMemoryInternalError('shared_space_membership_set', 'Shared space membership update failed', error, 'Retry the operation. If the error persists, check database connectivity.');
    }
}
/**
 ...
 */
export async function handleSharedMemoryStatus(args) {
    try {
        const db = requireDb();
        ...
        return createSharedMemoryInternalError('shared_memory_status', 'Shared memory status failed', error, 'Retry the operation. If the error persists, check database connectivity.');
    }
}
/**
...
export async function handleSharedMemoryEnable(args) {
    try {
        ...
        if (!isAdmin) {
            return createSharedMemoryEnableAuthError('shared_memory_enable_admin_required', 'Only the configured shared-memory admin can enable shared memory.');
        }
        const db = requireDb();
        ...
        return createSharedMemoryInternalError('shared_memory_enable', 'Shared memory enable failed', error, 'Retry the operation. If the error persists, check database connectivity.');
    }
}
/**
 ...
 */
async function createSharedSpacesReadme() {
