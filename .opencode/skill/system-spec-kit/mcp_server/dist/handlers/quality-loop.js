...
function triggerPhrasesChanged(original, updated) {
 ...
 */
function scoreTriggerPhrases(metadata) {
 ...
 */
function scoreAnchorFormat(content) {
    const issues = [];
 ...
 */
function scoreTokenBudget(content, charBudget = DEFAULT_CHAR_BUDGET) {
    const issues = [];
 ...
 */
function scoreCoherence(content, metadata = {}) {
    const issues = [];
    let score = 0;
    ...
    const resolveReference = typeof metadata.resolveReference === 'function'
        ...
        : null;
    const normalizedSelfTargets = new Set([metadata.title, metadata.filePath]
 ...
 */
function computeMemoryQualityScore(content, metadata) {
    ...
    const total = triggerResult.score * QUALITY_WEIGHTS.triggers +
 ...
 */
function attemptAutoFix(content, metadata, issues) {
    ...
    if (hasTriggerIssue) {
        const existingTriggers = Array.isArray(fixedMetadata.triggerPhrases)
            ? fixedMetadata.triggerPhrases
            : [];
        const extracted = extractTriggersFromContent(fixedContent, fixedMetadata.title);
...
function extractTriggersFromContent(content, title) {
    let triggers = [];
    ...
    triggers = triggers.filter(t => !CONTAM_HEADING_PATTERNS.some(p => p.test(t)));
 ...
 */
function normalizeAnchors(content) {
    const openPattern = /<!--\s*ANCHOR:\s*([\w-]+)\s*-->/g;
    const closePattern = /<!--\s*\/ANCHOR:\s*([\w-]+)\s*-->/g;
    const openAnchors = [];
    const closeAnchors = [];
 ...
 */
function runQualityLoop(content, metadata, options) {
        ...
        if (score.total >= threshold) {
            ...
            return {
                ...
                fixedTriggerPhrases: triggerPhrasesChanged(metadata.triggerPhrases, currentMetadata.triggerPhrases)
                    ? currentMetadata.triggerPhrases
                    : undefined,
            };
        }
 ...
 */
function logQualityMetrics(score, attempts, passed, rejected, emitEvalMetrics = true) {
    try {
        if (!emitEvalMetrics)
            return;
        // Use eval logger's feature flag check
        if (process.env.SPECKIT_EVAL_LOGGING?.toLowerCase() !== 'true')
            return;
        const db = initEvalDb();
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(0, // No eval run for quality metrics
        'memory_quality_score', score.total, 'quality_loop', attempts, metadata);
    }
    catch (err) {
...
export { computeMemoryQualityScore, scoreTriggerPhrases, scoreAnchorFormat, scoreTokenBudget, scoreCoherence, attemptAutoFix, extractTriggersFromContent, normalizeAnchors, runQualityLoop, logQualityMetrics, 
// Re-exported from search-flags for backward compatibility
isQualityLoopEnabled, QUALITY_WEIGHTS, DEFAULT_TOKEN_BUDGET, DEFAULT_CHAR_BUDGET, };
//# sourceMappingURL=quality-loop.js.map