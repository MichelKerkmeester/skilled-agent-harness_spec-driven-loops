...
// Formatters
import { calculateTokenMetrics } from '../formatters/index.js';
// Lib modules
...
function fetchMemoryRecords(memoryIds) {
    ...
    if (!db)
        return records;
    try {
        const stmt = db.prepare('SELECT * FROM memory_index WHERE id = ?');
...
async function getTieredContent(memoryInfo, tier) {
    if (tier === 'COLD' || tier === 'DORMANT' || tier === 'ARCHIVED')
        return '';
    try {
...
async function handleMemoryMatchTriggers(args) {
    const { prompt, limit: rawLimit = 3, session_id: rawSessionId, turnNumber: rawTurnNumber = 1, include_cognitive: includeCognitive = true } = args;
    ...
    catch (_error) { /* eval logging must never break triggers handler */ }
    const logFinalTriggerEval = (memoryIds, latencyMs) => {
        try {
    ...
    if (useCognitive) {
        try {
            decayStats = { decayedCount: workingMemory.batchUpdateScores(sessionId) };
        }
        catch (err) {
            const message = toErrorMessage(err);
    ...
    let results = triggerMatchResult.matches;
    ...
    if (specFolder || tenantId || userId || agentId || sharedSpaceId) {
        ...
        `).all(...memoryIds);
                ...
                results = results.filter(match => {
                    ...
                    if (specFolder && row.spec_folder !== specFolder)
                        return false;
                    ...
                    if (agentId && row.agent_id !== agentId)
                        return false;
                    if (sharedSpaceId && row.shared_space_id !== sharedSpaceId)
                        return false;
                    return true;
                });
            }
    ...
    if (useCognitive) {
        ...
        for (const match of results) {
            try {
                ...
                workingMemory.setAttentionScore(sessionId, match.memoryId, 1.0);
        ...
        if (coActivation.isEnabled()) {
            for (const memoryId of activatedMemories) {
                try {
                    const boosted = coActivation.spreadActivation([memoryId])
                        .map(r => ({ memoryId: r.id }));
        ...
        formattedResults = await Promise.all(tieredResults.map(async (r) => {
            const content = await getTieredContent({
                filePath: r.filePath,
                title: r.title,
    ...
    catch (_error) { /* instrumentation must never cause triggers handler to fail */ }
...
//# sourceMappingURL=memory-triggers.js.map