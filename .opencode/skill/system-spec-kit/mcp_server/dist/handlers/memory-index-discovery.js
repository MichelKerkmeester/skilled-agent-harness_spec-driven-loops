...
import { matchesSpecDocumentPath, SPEC_DOCUMENT_FILENAMES, } from '../lib/config/spec-doc-paths.js';
 ...
 */
export function findSpecDocuments(workspacePath, options = {}) {
    ...
    function walkSpecsDir(specsRoot, dir) {
        try {
            ...
            for (const entry of entries) {
                if (entry.isDirectory()) {
                    ...
                    if (options.specFolder) {
                        ...
                        if (relativePath !== normalizedSpecFolder &&
 ...
 */
export function detectSpecLevel(specPath) {
    try {
...
export function findConstitutionalFiles(workspacePath) {
    const results = [];
            ...
            try {
                ...
                for (const file of files) {
                    if (file.isFile() && file.name.endsWith('.md')) {
                        if (EXCLUDED_CONSTITUTIONAL_BASENAMES.has(file.name.toLowerCase()))
                            continue;
...
//# sourceMappingURL=memory-index-discovery.js.map