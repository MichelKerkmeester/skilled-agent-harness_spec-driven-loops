...
function createDatabaseUnavailableDeleteResponse() {
...
async function handleMemoryDelete(args) {
    ...
    let mutationLedgerWarning = null;
    ...
    if (numericId !== null) {
        ...
        database.transaction(() => {
            ...
            if (deletedCount > 0) {
                ...
                try {
                    recordHistory(numericId, 'DELETE', singleSnapshot?.file_path ?? null, null, 'mcp:memory_delete', singleSnapshot?.spec_folder ?? null);
                }
                catch (_histErr) {
            ...
            for (const memory of memories) {
                if (vectorIndex.deleteMemory(memory.id)) {
                    ...
                    try {
                        ...
                        recordHistory(memory.id, 'DELETE', snapshot?.file_path ?? null, null, 'mcp:memory_delete', snapshot?.spec_folder ?? null);
                    }
                    catch (_histErr) {
    ...
    let postMutationFeedback = null;
    if (deletedCount > 0) {
        let postMutationHooks;
        try {
    ...
    const data = { deleted: deletedCount };
    if (checkpointName) {
...
//# sourceMappingURL=memory-crud-delete.js.map