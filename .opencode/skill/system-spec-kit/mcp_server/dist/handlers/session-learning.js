...
const schemaInitializedDbs = new WeakSet();
...
function hasLegacyUniqueConstraint(database) {
...
function migrateLegacySchema(database) {
    ...
    `);
        ...
        throw new MemoryError(ErrorCodes.DATABASE_ERROR, 'Failed to migrate session_learning schema', { originalError: message });
    }
}
/** Initialize the session_learning table schema if not already created */
function ensureSchema(database) {
    if (schemaInitializedDbs.has(database))
        return;
    try {
        database.exec(SCHEMA_SQL);
        ...
        throw new MemoryError(ErrorCodes.DATABASE_ERROR, 'Failed to initialize session_learning schema', { originalError: message });
    }
}
/**
 ...
 */
function validateScores(knowledgeScore, uncertaintyScore, contextScore) {
    const scores = [
...
async function handleTaskPreflight(args) {
    const { specFolder: spec_folder, taskId, knowledgeScore: knowledge_score, uncertaintyScore: uncertainty_score, contextScore: context_score, knowledgeGaps = [], sessionId: session_id = null } = args;
    ...
    const existing = database.prepare(`SELECT id, phase
     ...
     LIMIT 1`).get(spec_folder, taskId, normalizedSessionId, normalizedSessionId);
    if (existing) {
    ...
    `);
        try {
            updateStmt.run(normalizedSessionId, knowledge_score, uncertainty_score, context_score, gapsJson, now, existing.id);
  ...
  `);
    try {
        const result = stmt.run(spec_folder, taskId, normalizedSessionId, knowledge_score, uncertainty_score, context_score, gapsJson, now, now);
...
async function handleTaskPostflight(args) {
    const { specFolder: spec_folder, taskId, knowledgeScore: knowledge_score, uncertaintyScore: uncertainty_score, contextScore: context_score, gapsClosed = [], newGapsDiscovered = [], sessionId: session_id = null, } = args;
    // Validate inputs before any I/O (checkDatabaseUpdated is deferred until after validation)
    if (!spec_folder) {
        throw new MemoryError(ErrorCodes.MISSING_REQUIRED_PARAM, 'specFolder is required', { param: 'specFolder' });
    }
  ...
  `;
    const params = [spec_folder, taskId];
    let query = baseQuery;
    ...
    ORDER BY CASE phase WHEN 'preflight' THEN 0 ELSE 1 END,
             ...
             id DESC
  `;
    const candidates = database.prepare(query).all(...params);
    ...
    if (normalizedSessionId === null && preflightCandidates.length > 1) {
        throw new MemoryError(ErrorCodes.INVALID_PARAMETER, `Multiple open preflight records found for spec_folder="${spec_folder}" and task_id="${taskId}". Provide sessionId to disambiguate postflight.`, { specFolder: spec_folder, taskId: taskId });
    }
    if (normalizedSessionId === null && preflightCandidates.length === 0 && completeCandidates.length > 1) {
        throw new MemoryError(ErrorCodes.INVALID_PARAMETER, `Multiple completed learning cycles found for spec_folder="${spec_folder}" and task_id="${taskId}". Provide sessionId to update the intended record.`, { specFolder: spec_folder, taskId: taskId });
    }
    const preflightRecord = (preflightCandidates[0] ?? completeCandidates[0]);
    if (!preflightRecord) {
        throw new MemoryError(ErrorCodes.FILE_NOT_FOUND, normalizedSessionId === null
            ? `No preflight record found for spec_folder="${spec_folder}" and task_id="${taskId}". Call task_preflight first.`
            : `No preflight record found for spec_folder="${spec_folder}", task_id="${taskId}", session_id="${normalizedSessionId}". Call task_preflight first.`, { specFolder: spec_folder, taskId: taskId, sessionId: normalizedSessionId });
    }
    ...
    try {
      ...
      UPDATE session_learning SET
        ...
        updated_at = ?
      WHERE id = ?
    `).run(knowledge_score, uncertainty_score, context_score, deltaKnowledge, deltaUncertainty, deltaContext, learningIndexRounded, gapsClosedJson, newGapsJson, now, now, preflightRecord.id);
        ...
        return createMCPSuccessResponse({
            ...
            data: {
                ...
                record: {
                    ...
                    specFolder: spec_folder,
                    taskId: taskId,
                    sessionId: preflightRecord.session_id ?? normalizedSessionId,
                    baseline: {
...
async function handleGetLearningHistory(args) {
    const { specFolder: spec_folder, sessionId: session_id, limit = 10, onlyComplete: only_complete = false, includeSummary: include_summary = true } = args;
    // Validate inputs before any I/O (checkDatabaseUpdated is deferred until after validation)
    if (!spec_folder) {
        throw new MemoryError(ErrorCodes.MISSING_REQUIRED_PARAM, 'specFolder is required', { param: 'specFolder' });
    }
    ...
    `;
        ...
        const learningHistory = rows.map((row) => {
            let knowledgeGaps = [];
            let gapsClosed = [];
            let newGapsDiscovered = [];
            try {
                knowledgeGaps = row.knowledge_gaps ? JSON.parse(row.knowledge_gaps) : [];
            }
            catch (_error) { /* ignore */ }
            try {
                gapsClosed = row.gaps_closed ? JSON.parse(row.gaps_closed) : [];
            }
            catch (_error) { /* ignore */ }
            try {
                newGapsDiscovered = row.new_gaps_discovered ? JSON.parse(row.new_gaps_discovered) : [];
            }
            catch (_error) { /* ignore */ }
            const baseResult = {
                taskId: row.task_id,
                ...
                phase: row.phase,
                preflight: {
                    knowledge: row.pre_knowledge_score ?? null,
                    uncertainty: row.pre_uncertainty_score ?? null,
                    context: row.pre_context_score ?? null,
            ...
            if (row.phase === 'complete') {
                return {
                    ...
                    phase: 'complete',
                    postflight: {
                        knowledge: row.post_knowledge_score ?? null,
                        uncertainty: row.post_uncertainty_score ?? null,
                        context: row.post_context_score ?? null,
                        timestamp: row.completed_at ?? null,
                    },
                    deltas: {
                        knowledge: row.delta_knowledge ?? null,
                        uncertainty: row.delta_uncertainty ?? null,
                        context: row.delta_context ?? null,
                    },
                    learningIndex: row.learning_index ?? null,
        ...
        WHERE spec_folder = ?
      `;
            const summaryParams = [spec_folder];
            if (normalizedSessionId) {
            ...
            const stats = database.prepare(summarySql).get(...summaryParams);
            const summary = {
            ...
            if (stats.avg_learning_index !== null) {
                const avgLI = stats.avg_learning_index;
                if (avgLI > 15) {
        ...
        if (responseSummary?.interpretation) {
            hints.push(responseSummary.interpretation);
        }
        return createMCPSuccessResponse({
...
//# sourceMappingURL=session-learning.js.map