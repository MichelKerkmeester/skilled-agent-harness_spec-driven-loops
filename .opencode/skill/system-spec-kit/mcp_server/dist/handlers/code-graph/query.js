...
function resolveSubject(subject) {
    ...
    const byId = d.prepare('SELECT symbol_id FROM code_nodes WHERE symbol_id = ?').get(subject);
    if (byId)
        return byId.symbol_id;
    // Try as fqName
    const byFq = d.prepare('SELECT symbol_id FROM code_nodes WHERE fq_name = ? LIMIT 1').get(subject);
    if (byFq)
        return byFq.symbol_id;
    // Try as name
    const byName = d.prepare('SELECT symbol_id FROM code_nodes WHERE name = ? LIMIT 1').get(subject);
    if (byName)
        return byName.symbol_id;
    return null;
}
/** BFS transitive traversal from a symbolId via the given edge type */
function transitiveTraversal(startId, edgeType, direction, maxDepth, limit) {
