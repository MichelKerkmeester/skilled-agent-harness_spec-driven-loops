...
async function handleMemoryUpdate(args) {
    ...
    if (triggerPhrases !== undefined)
        updateParams.triggerPhrases = triggerPhrases;
    if (importanceWeight !== undefined)
        updateParams.importanceWeight = importanceWeight;
    if (importanceTier !== undefined)
        updateParams.importanceTier = importanceTier;
    ...
    if (title !== undefined && title !== existing.title) {
        ...
        if (newEmbedding) {
            ...
            if (allowPartialUpdate) {
                ...
                throw new MemoryError(ErrorCodes.EMBEDDING_FAILED, 'Failed to regenerate embedding (null result), update rolled back. No changes were made.', { memoryId: id });
            }
        }
    }
        ...
        runInTransaction(database, () => {
            ...
            if ((updateParams.title !== undefined || updateParams.triggerPhrases !== undefined) && bm25Index.isBm25Enabled()) {
                try {
                    ...
                    const row = database.prepare('SELECT title, content_text, trigger_phrases, file_path FROM memory_index WHERE id = ?').get(id);
                    if (row) {
                    ...
                    const isBm25InfraFailure = (msg) => msg.includes('not a function') ||
            ...
            try {
                recordHistory(id, 'UPDATE', existing.title ?? null, updateParams.title ?? existing.title ?? null, 'mcp:memory_update');
            }
            catch (_histErr) {
...
//# sourceMappingURL=memory-crud-update.js.map