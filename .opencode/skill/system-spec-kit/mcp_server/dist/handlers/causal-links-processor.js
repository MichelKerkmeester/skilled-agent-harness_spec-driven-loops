...
function resolveMemoryReference(database, reference) {
    const resolved = resolveMemoryReferencesBatch(database, [reference]).get(reference);
    return typeof resolved === 'number' ? resolved : null;
}
function buildPreparedReference(reference) {
    ...
    return {
        ...
        isPathLike: normalizedLower.includes('specs/')
            || normalizedLower.includes('memory/')
            || normalizedReference.includes('/'),
    };
}
function buildPathExactCandidates(normalizedReference) {
...
function getMemoryIndexColumns(database) {
    const rows = database.prepare('PRAGMA table_info(memory_index)').all();
...
function resolveMemoryReferencesBatch(database, references) {
    ...
    const setResolved = (reference, id) => {
        if (typeof id === 'number' && Number.isFinite(id)) {
            resolved.set(reference.original, id);
        }
    };
    // 1) Batch numeric IDs
    const numericIds = Array.from(new Set(preparedRefs.map((ref) => ref.numericId).filter((id) => typeof id === 'number')));
    ...
    const pathCandidateRefs = new Map();
    for (const ref of preparedRefs) {
    ...
    if (pathCandidates.size > 0) {
      ...
      `).all(...values);
        ...
        for (const row of pathRows) {
            const rowKeys = [row.canonical_file_path, row.file_path].filter((value) => typeof value === 'string');
    ...
    const titleCandidates = Array.from(new Set(preparedRefs
    ...
    for (const ref of preparedRefs) {
        ...
        if (ref.isSessionLike || ref.isPathLike) {
            const byPath = hasCanonicalFilePath
                ? byPathFallbackStmt.get(`%${ref.escapedLike}%`, `%${ref.escapedLike}%`)
                : byPathFallbackStmt.get(`%${ref.escapedLike}%`);
            if (byPath) {
                setResolved(ref, byPath.id);
                continue;
            }
        }
        const byTitlePartial = byTitlePartialStmt.get(`%${escapeLikePattern(ref.trimmed)}%`);
        if (byTitlePartial) {
...
function processCausalLinks(database, memoryId, causalLinks) {
    const result = {
        processed: 0,
        inserted: 0,
        resolved: 0,
...
//# sourceMappingURL=causal-links-processor.js.map