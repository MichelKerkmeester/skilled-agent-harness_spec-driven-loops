...
/* ───────────────────────────────────────────────────────────────
   2. HANDLER
──────────────────────────────────────────────────────────────── */
async function handleMemoryBulkDelete(args) {
    ...
    const countResult = database.prepare(countSql).get(...countParams);
    ...
    const memoriesToDelete = database.prepare(selectSql).all(...selectParams);
    ...
    const bulkDeleteTx = database.transaction(() => {
        for (const memory of memoriesToDelete) {
            if (vectorIndex.deleteMemory(memory.id)) {
                ...
                try {
                    recordHistory(memory.id, 'DELETE', memory.file_path ?? null, null, 'mcp:memory_bulk_delete', memory.spec_folder ?? null);
                }
                catch (_histErr) {
