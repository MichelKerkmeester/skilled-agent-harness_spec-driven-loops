...
import { init as initHybridSearch, hybridSearchEnhanced, bm25Search, ftsSearch, } from '../lib/search/hybrid-search.js';
...
import { ALL_CHANNELS, isAblationEnabled, runAblation, assertGroundTruthAlignment, storeAblationResults, formatAblationReport, toHybridSearchFlags, } from '../lib/eval/ablation-framework.js';
import { generateDashboardReport, formatReportJSON, formatReportText, } from '../lib/eval/reporting-dashboard.js';
import { analyzeKValueSensitivityBatch, formatKValueReport, } from '../lib/eval/k-value-analysis.js';
import { createUnifiedGraphSearchFn, computeDegreeScores, DEGREE_CHANNEL_WEIGHT, } from '../lib/search/graph-search-fn.js';
import { isDegreeBoostEnabled } from '../lib/search/search-flags.js';
function normalizeChannels(input) {
    if (!Array.isArray(input) || input.length === 0)
        return ALL_CHANNELS;
    const valid = input.filter((value) => ALL_CHANNELS.includes(value));
    return valid.length > 0 ? valid : ALL_CHANNELS;
}
function initializeEvalHybridSearch(database) {
...
async function withAblationDb(run) {
...
function buildRawFusionLists(database, graphSearchFn, query, embedding, limit) {
    const lists = [];
    if (embedding) {
        ...
        if (vectorResults.length > 0) {
            lists.push({
                source: 'vector',
                results: vectorResults.map((row) => ({ id: row.id })),
            });
        }
    }
    ...
    if (graphResults.length > 0) {
        lists.push({
            source: 'graph',
            results: graphResults.map((row) => ({ id: row.id })),
        });
    }
 ...
 */
async function handleEvalRunAblation(args) {
    ...
    if (!isAblationEnabled()) {
        throw new MemoryError(ErrorCodes.INVALID_PARAMETER, 'Ablation is disabled. Set SPECKIT_ABLATION=true to run ablation studies.', { flag: 'SPECKIT_ABLATION' });
    }
    ...
    const report = await withAblationDb(async (db, dbPath) => {
        try {
            ...
            throw new MemoryError(ErrorCodes.INVALID_PARAMETER, error instanceof Error ? error.message : String(error), { dbPath });
        }
        ...
        const searchFn = async (query, disabledChannels) => {
            ...
            return results.map((row, index) => ({
                memoryId: Number(row.parentMemoryId ?? row.id),
                score: row.score,
                rank: index + 1,
            }));
        };
    ...
    if (!report) {
        throw new MemoryError(ErrorCodes.DATABASE_ERROR, 'Ablation run returned no report. Check feature flag and ground truth availability.', {});
    }
 ...
 */
async function handleEvalKSensitivity(args) {
    await checkDatabaseUpdated();
...
async function handleEvalReportingDashboard(args) {
    await checkDatabaseUpdated();
...
//# sourceMappingURL=eval-reporting.js.map