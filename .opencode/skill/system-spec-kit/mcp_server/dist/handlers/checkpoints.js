...
function resolveValidationQueryText(database, queryId) {
  ...
  `).get();
    ...
    WHERE id = ?
      ...
      AND TRIM(query_text) != ''
    LIMIT 1
  `).get(Number.parseInt(consumptionMatch[1], 10));
    return typeof row?.query_text === 'string' && row.query_text.trim().length > 0
        ? row.query_text
        : null;
}
function parseMemoryId(rawId) {
...
function parseCheckpointMetadata(rawMetadata) {
    if (!rawMetadata) {
        return {};
    }
...
function validateCheckpointScope(args) {
    const validateValue = (value, field, options = {}) => {
        if (value === undefined) {
            return undefined;
        }
    ...
    const hasActorOrSharedSpaceScope = userId !== undefined || agentId !== undefined || sharedSpaceId !== undefined;
    if (hasActorOrSharedSpaceScope
        && tenantId !== undefined
...
function hasCheckpointScope(scope) {
    return (scope.tenantId !== undefined
        || scope.userId !== undefined
        || scope.agentId !== undefined
        || scope.sharedSpaceId !== undefined);
}
function requiresCheckpointTenant(scope) {
    return scope.tenantId === undefined && (scope.userId !== undefined
        || scope.agentId !== undefined
        || scope.sharedSpaceId !== undefined);
}
function mergeCheckpointScopeMetadata(metadata, scope) {
    return {
        ...(metadata ?? {}),
...
function checkpointMatchesScope(rawMetadata, scope) {
    const metadata = parseCheckpointMetadata(rawMetadata);
    const matchesScopeField = (field) => {
...
function createCheckpointScopeValidationError(tool) {
...
async function handleCheckpointDelete(args) {
    ...
    const success = checkpoints.deleteCheckpoint(name, scope);
...
async function handleMemoryValidate(args) {
    const startTime = Date.now();
    await checkDatabaseUpdated();
    const { id, wasUseful, queryId, queryTerms, resultRank, totalResultsShown, searchMode, intent, sessionId, notes, } = args;
    ...
    const result = confidenceTracker.recordValidation(database, memoryId, wasUseful);
    try {
    ...
    let learnedFeedback = null;
    let groundTruthSelectionId = null;
