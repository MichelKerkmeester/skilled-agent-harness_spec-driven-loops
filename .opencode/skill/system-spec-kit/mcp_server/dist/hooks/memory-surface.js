 ...
 */
function isSessionPrimed(sessionId) {
...
──────────────────────────────────────────────────────────────── */
function extractContextHint(args) {
    ...
    if (args.concepts && Array.isArray(args.concepts) && args.concepts.length > 0) {
        const strings = args.concepts.filter((c) => typeof c === 'string');
        if (strings.length > 0)
            return strings.join(' ');
    }
    return null;
...
──────────────────────────────────────────────────────────────── */
async function getConstitutionalMemories() {
    const now = Date.now();
    ...
    try {
      ...
      LIMIT 10
    `).all();
        constitutionalCache = rows.map((r) => ({
            id: r.id,
...
function clearConstitutionalCache() {
    constitutionalCache = null;
    constitutionalCacheTime = 0;
}
function getCodeGraphStatusSnapshot() {
    try {
...
──────────────────────────────────────────────────────────────── */
function enforceAutoSurfaceTokenBudget(result, tokenBudget, hookName) {
    if (!result) {
        return null;
    }
    const measureTokens = (candidate) => estimateTokenCount(JSON.stringify(candidate));
    ...
    if (tokenCount > budgetLimit) {
        console.warn(`[SK-004] Auto-surface output exceeded ${hookName} token budget ` +
            `(${tokenCount} > ${budgetLimit}); dropping payload`);
        return null;
    }
    console.warn(`[SK-004] Auto-surface output truncated to fit ${hookName} token budget ` +
 ...
 */
function getAttentionWeightedMemoryIds(limit = 5) {
    try {
        const db = vectorIndex.getDb();
        if (!db)
            return new Set();
      ...
      LIMIT ?
    `).all(workingMemory.DECAY_FLOOR, oneHourAgo, limit);
...
function applyAttentionBoost(triggered, attentionIds) {
...
async function autoSurfaceMemories(contextHint, tokenBudget = TOOL_DISPATCH_TOKEN_BUDGET, hookName = 'memory-aware') {
...
function buildPrimePackage(toolArgs, graphSnapshot) {
    ...
    const taskFields = ['input', 'query', 'prompt'];
    let currentTask = null;
    for (const f of taskFields) {
        if (typeof toolArgs[f] === 'string' && toolArgs[f].length >= 3) {
            currentTask = toolArgs[f].slice(0, 200);
            break;
        }
    }
    // Code graph freshness
    let codeGraphStatus = 'empty';
...
async function primeSessionIfNeeded(toolName, toolArgs, sessionId) {
    ...
    try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        console.warn(`[SK-004] Session priming failed on first tool call '${toolName}'` +
 ...
 */
function resetSessionPrimed(sessionId) {
    if (sessionId) {
 ...
 */
async function autoSurfaceAtToolDispatch(toolName, toolArgs, options) {
 ...
 * @returns AutoSurfaceResult or null if nothing to surface / hook disabled
 */
async function autoSurfaceAtCompaction(sessionContext, options) {
    // Allow integration layer to disable this hook via config flag
...
export { 
// Constants
MEMORY_AWARE_TOOLS, CONSTITUTIONAL_CACHE_TTL, TOOL_DISPATCH_TOKEN_BUDGET, COMPACTION_TOKEN_BUDGET, 
// Functions
extractContextHint, getConstitutionalMemories, clearConstitutionalCache, autoSurfaceMemories, primeSessionIfNeeded, resetSessionPrimed, autoSurfaceAtToolDispatch, autoSurfaceAtCompaction, 
// T018: Session tracking for session_health tool
recordToolCall, getSessionTimestamps, getLastActiveSessionId, isSessionPrimed, markSessionPrimed, getCodeGraphStatusSnapshot, };
//# sourceMappingURL=memory-surface.js.map