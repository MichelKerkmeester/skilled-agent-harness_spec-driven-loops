...
import { parseHookStdin, hookLog, truncateToTokenBudget, withTimeout, HOOK_TIMEOUT_MS, COMPACTION_TOKEN_BUDGET, } from './shared.js';
...
import { createPreMergeSelectionMetadata, } from '../../lib/context/shared-payload.js';
...
function extractAttentionSignals(lines) {
    const freq = new Map();
...
function renderTriggeredMemories(autoSurfaced) {
 ...
 */
async function buildMergedContext(transcriptLines) {
    ...
    hookLog('info', 'compact-inject', `Compaction auto-surface returned ${autoSurfaced.constitutional.length} constitutional and ${autoSurfaced.triggered.length} triggered memories (${autoSurfaced.latencyMs}ms)`);
...
async function buildMergedPayloadContract(transcriptLines) {
    const sanitizedLines = stripRecoveredCompactLines(transcriptLines);
    const filePaths = extractFilePaths(sanitizedLines);
    const topics = extractTopics(sanitizedLines);
    const attentionSignals = extractAttentionSignals(sanitizedLines);
...
//# sourceMappingURL=compact-inject.js.map