...
import { parseHookStdin, hookLog, withTimeout, HOOK_TIMEOUT_MS, } from './shared.js';
...
const SPEC_FOLDER_PREFIXES = ['.opencode/specs/', 'specs/'];
...
function resolveGenerateContextScriptPath() {
...
function runContextAutosave(sessionId) {
    ...
    const result = spawnSync(process.execPath, [scriptPath, '--json', JSON.stringify(payload)], {
        cwd: process.cwd(),
        encoding: 'utf-8',
...
function selectDetectedSpecFolder(normalizedMatches, currentSpecFolder) {
...
function storeTokenSnapshot(sessionId, usage, cost) {
    const state = loadState(sessionId);
    if (!state)
        return;
...
async function main() {
    ...
    if (input.transcript_path) {
        ...
        try {
            const { usage, newOffset } = await parseTranscript(input.transcript_path, startOffset);
            if (usage.messageCount > 0) {
                ...
                hookLog('info', 'session-stop', `Parsed ${usage.messageCount} messages: ${usage.promptTokens} prompt + ${usage.completionTokens} completion = ${usage.totalTokens} total tokens`);
            }
        }
        catch (err) {
    ...
    if (input.transcript_path) {
        const detectedSpec = detectSpecFolder(input.transcript_path, stateBeforeStop?.lastSpecFolder ?? null);
        if (!stateBeforeStop?.lastSpecFolder && detectedSpec) {
            ...
            updateState(sessionId, { lastSpecFolder: detectedSpec });
            hookLog('info', 'session-stop', `Retargeted autosave spec folder from ${stateBeforeStop.lastSpecFolder} to ${detectedSpec}`);
        }
...
export function detectSpecFolder(transcriptPath, currentSpecFolder = null) {
    let fileDescriptor;
    try {
...
function normalizeSpecFolderPath(rawPath) {
...
//# sourceMappingURL=session-stop.js.map