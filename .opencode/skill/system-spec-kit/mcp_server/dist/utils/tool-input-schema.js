...
function validateProperty(field, value, schema) {
...
function matchesSchemaConstraint(args, constraint) {
 ...
 */
export function validateToolInputSchema(toolName, args, toolDefinitions) {
    ...
    if (Array.isArray(schema['x-requiredAnyOf']) && schema['x-requiredAnyOf'].length > 0) {
        const branchMatched = schema['x-requiredAnyOf'].some((fields) => Array.isArray(fields) && fields.every((field) => hasValue(args[field])));
        if (!branchMatched) {
...
//# sourceMappingURL=tool-input-schema.js.map