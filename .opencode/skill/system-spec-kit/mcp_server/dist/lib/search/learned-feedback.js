...
import { parseLearnedTriggers, serializeLearnedTriggers, } from '../storage/learned-triggers-schema.js';
// ───────────────────────────────────────────────────────────────
// 3. CONSTANTS
// ───────────────────────────────────────────────────────────────
 ...
 * @param db - SQLite database connection
 */
function ensureLearnedTriggersIndex(db) {
    if (_learnedTriggersIndexEnsured)
        return;
    try {
 ...
 */
export function isLearnedFeedbackEnabled() {
 ...
 */
export function isMemoryEligible(memoryAgeMs) {
 ...
 */
export function extractLearnableTerms(queryTerms, existingTriggers, denylist = DENYLIST) {
 ...
 */
export function recordSelection(queryId, memoryId, queryTerms, resultRank, db) {
    try {
        ...
        const memory = db.prepare('SELECT created_at, trigger_phrases, learned_triggers FROM memory_index WHERE id = ?').get(memoryId);
        if (!memory) {
        ...
        if (memory.trigger_phrases) {
            if (Array.isArray(memory.trigger_phrases)) {
                ...
                try {
                    ...
                    existingTriggers = memory.trigger_phrases.split(',').map((t) => t.trim());
                }
            }
        }
        ...
        db.prepare('INSERT INTO learned_feedback_audit (memory_id, action, terms, source, timestamp, shadow_mode) VALUES (?, ?, ?, ?, ?, ?)').run(memoryId, 'add', JSON.stringify(terms), queryId, now, shadowMode ? 1 : 0);
 ...
 */
export function applyLearnedTriggers(memoryId, terms, db, source = 'unknown') {
    try {
        const row = db.prepare('SELECT learned_triggers FROM memory_index WHERE id = ?').get(memoryId);
        if (!row) {
        ...
        for (const term of terms) {
            ...
            if (existingTerms.has(normalizedTerm))
                continue;
            if (existing.length + newEntries.length >= MAX_TERMS_PER_MEMORY)
                break;
            newEntries.push({
                term: normalizedTerm,
                addedAt: nowSeconds,
        ...
        db.prepare('UPDATE memory_index SET learned_triggers = ? WHERE id = ?').run(serialized, memoryId);
        return true;
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
 ...
 */
function isInShadowPeriod(db) {
    try {
        ensureAuditTable(db);
        const row = db.prepare('SELECT MIN(timestamp) AS earliest FROM learned_feedback_audit').get();
        if (!row?.earliest) {
 ...
 */
export function queryLearnedTriggers(query, db) {
    try {
        if (!isLearnedFeedbackEnabled()) {
            return [];
        }
        ...
        const rows = db.prepare(`SELECT id, learned_triggers FROM memory_index WHERE learned_triggers IS NOT NULL AND learned_triggers != '[]'`).all();
        const matches = [];
        const nowSeconds = Math.floor(Date.now() / 1000);
 ...
 */
export function expireLearnedTerms(db) {
    try {
        ensureLearnedTriggersIndex(db);
        const rows = db.prepare(`SELECT id, learned_triggers FROM memory_index WHERE learned_triggers IS NOT NULL AND learned_triggers != '[]'`).all();
        const nowSeconds = Math.floor(Date.now() / 1000);
        ...
        for (const row of rows) {
            ...
            if (expired.length > 0) {
                ...
                db.prepare('INSERT INTO learned_feedback_audit (memory_id, action, terms, source, timestamp, shadow_mode) VALUES (?, ?, ?, ?, ?, ?)').run(row.id, 'expire', JSON.stringify(expired.map((e) => e.term)), 'ttl_expiry', Date.now(), 0);
                // Update the column with only valid entries
                db.prepare('UPDATE memory_index SET learned_triggers = ? WHERE id = ?').run(serializeLearnedTriggers(valid), row.id);
                affectedCount++;
            }
        }
 ...
 */
export function clearAllLearnedTriggers(db) {
    try {
        ensureAuditTable(db);
        ensureLearnedTriggersIndex(db);
        ...
        for (const row of rows) {
            const entries = parseLearnedTriggers(row.learned_triggers);
            if (entries.length > 0) {
                db.prepare('INSERT INTO learned_feedback_audit (memory_id, action, terms, source, timestamp, shadow_mode) VALUES (?, ?, ?, ?, ?, ?)').run(row.id, 'clear', JSON.stringify(entries.map((e) => e.term)), 'rollback', Date.now(), 0);
            }
        }
        // Reset all learned_triggers to empty
        const result = db.prepare(`UPDATE memory_index SET learned_triggers = '[]' WHERE learned_triggers IS NOT NULL AND learned_triggers != '[]'`).run();
 ...
 */
export function getAuditLog(db, memoryId, limit = 50) {
    try {
        ensureAuditTable(db);
        let sql;
        let params;
...
//# sourceMappingURL=learned-feedback.js.map