 ...
 */
function classifyReason(ctx, status) {
 ...
 */
function generateSuggestedQueries(ctx) {
    const suggestions = [];
    ...
    const trimmedWords = [...words].reverse().reduce((acc, word) => {
        if (acc.length === 0 && stopWords.has(word.toLowerCase()))
            return acc;
        return [word, ...acc];
    }, []);
 ...
 */
function recommendAction(status, reason) {
    if (status === 'no_results') {
        if (reason === 'spec_filter_too_narrow')
            return 'retry_broader';
        if (reason === 'low_signal_query')
            return 'switch_mode';
 ...
 */
export function buildGraphExpandedFallback(ctx, db) {
    if (!isGraphFallbackEnabled())
        return [];
    const status = classifyStatus(ctx);
    if (status !== 'no_results' && status !== 'low_confidence')
        return [];
    try {
 ...
 */
export function buildRecoveryPayload(ctx) {
    const status = classifyStatus(ctx);
 ...
 * @param ctx - Retrieval outcome context
 */
export function shouldTriggerRecovery(ctx) {
    if (ctx.resultCount === 0)
        return true;
    if (ctx.evidenceGap)
        return true;
...
//# sourceMappingURL=recovery-payload.js.map