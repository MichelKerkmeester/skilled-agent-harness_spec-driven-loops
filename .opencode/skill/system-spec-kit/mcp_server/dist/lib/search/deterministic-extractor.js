 ...
 */
export function extractCodeFenceTechnologies(content) {
    const seen = new Set();
    const re = /```(\w[\w.-]*)/g;
    let match;
 ...
 */
export function extractRelationPhrases(content) {
    const relations = [];
    const RELATION_VERBS = /\b(depends on|implements|extends|inherits from|uses|requires|calls|overrides)\b/gi;
    // Simple sentence-level capture: SUBJECT <VERB> OBJECT
    const re = /\b([\w][\w\s-]{0,40}?)\s+(?:depends on|implements|extends|inherits from|uses|requires|calls|overrides)\s+([\w][\w\s-]{0,40})\b/gi;
 ...
 */
export function createTypedEdges(db, memoryId, edges, onWriteFn) {
    if (edges.length === 0)
        return 0;
    try {
    ...
    `);
        ...
        const runTransaction = db.transaction(() => {
            for (const edge of edges) {
                const result = insertStmt.run(edge.sourceId, edge.targetId, edge.relation, edge.strength, edge.evidence);
...
//# sourceMappingURL=deterministic-extractor.js.map