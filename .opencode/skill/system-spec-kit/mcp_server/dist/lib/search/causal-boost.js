 ...
 */
const RELATION_WEIGHT_MULTIPLIERS = {
    supersedes: 1.5,
    contradicts: 0.8,
    caused: 1.0,
    enabled: 1.0,
 ...
 */
function isTypedTraversalEnabled() {
 ...
 */
function isSparseMode(graphDensity) {
    if (!isTypedTraversalEnabled())
        return false;
    if (typeof graphDensity !== 'number' || !Number.isFinite(graphDensity))
        return false;
 ...
 */
function resolveSparseFirstTraversalPolicy(graphDensity) {
 ...
 */
function resolveTraversalDepth(depth = DEFAULT_TYPED_TRAVERSAL_DEPTH) {
    if (!isTypedTraversalEnabled()) {
        return MAX_HOPS;
    }
 ...
 *            PREFERENCE → supports
 */
const EDGE_LABEL_ALIASES = {
    CORRECTION: ['supersedes', 'contradicts'],
    DEPENDS_ON: ['caused', 'enabled', 'derived_from'],
    EXTENDS: ['enabled', 'derived_from'],
    PREFERENCE: ['supports'],
};
function resolveEdgePrior(relation, intent) {
    if (!isTypedTraversalEnabled())
        return 1.0;
 ...
 */
function computeHopDecay(hopDistance) {
 ...
 */
function computeIntentTraversalScore(seedScore, relation, hopDistance, freshness, intent) {
...
function getIntentEdgePriorities(intent) {
    return (INTENT_EDGE_PRIORITY[intent] ?? []).slice(0, 2);
}
function computeIntentEdgePrior(intent, edgeType) {
    ...
    const directMatchIndex = priorities.findIndex((priority) => priority.toUpperCase() === normalizedEdgeType);
...
function computeTraversalHopDecay(hopDistance) {
    return computeHopDecay(hopDistance);
}
function computeTraversalFreshnessFactor(updatedAt, now = new Date()) {
...
function buildPipelineRow(row, baseScore, causalBoost) {
 ...
 */
function normalizeIds(inputIds) {
    const ids = new Set();
 ...
 */
function computeBoostByHop(hopDistance) {
    if (!Number.isFinite(hopDistance) || hopDistance <= 0)
        return 0;
 ...
 */
function getNeighborBoosts(memoryIds, maxHops = MAX_HOPS) {
  ...
  `;
    try {
        const rows = db.prepare(query).all(...originIds, ...originIds, maxHops, ...originIds);
...
function fetchNeighborRows(memoryIds) {
 ...
 */
function applyCausalBoost(results, options = {}) {
    ...
    if (isTypedTraversalEnabled()) {
        ...
        for (const [neighborId, neighborBoost] of neighborBoosts) {
            ...
            WHERE source_id IN (${seedIds.map(() => '?').join(',')})
              AND target_id = ?
            LIMIT 1
          `).get(...seedIds.map(String), String(neighborId));
                    if (edgeRow?.relation)
                        dominantRelation = edgeRow.relation;
                }
                catch (_err) {
                    // Fall back to default relation on query failure
                }
            }
            const intentScore = computeIntentTraversalScore(avgSeedScore, dominantRelation, neighborBoost.hopCount, clampedFreshness, intent);
            intentBoosts.set(neighborId, intentScore);
        }
    }
    // Select the active boost map: intent-aware when D3-002 flag is on, classic otherwise
    const classicBoosts = new Map();
    for (const [neighborId, neighborBoost] of neighborBoosts) {
 ...
 */
function injectGraphContext(query, database) {
    const empty = {
        activated: false,
        matchedConcepts: [],
        relatedMemoryIds: [],
    ...
    if (!isGraphContextInjectionEnabled())
        return empty;
    try {
...
export { MAX_HOPS, MAX_BOOST_PER_HOP, RELATION_WEIGHT_MULTIPLIERS, 
// D3 Phase A exports
SPARSE_DENSITY_THRESHOLD, SPARSE_MAX_HOPS, DEFAULT_TYPED_TRAVERSAL_DEPTH, INTENT_EDGE_PRIORITY, DEFAULT_EDGE_PRIORITY, DEFAULT_EDGE_PRIOR, EDGE_PRIOR_TIERS, EDGE_LABEL_ALIASES, init, isEnabled, isTypedTraversalEnabled, isSparseMode, resolveSparseFirstTraversalPolicy, resolveTraversalDepth, resolveEdgePrior, computeHopDecay, computeIntentTraversalScore, getIntentEdgePriorities, computeIntentEdgePrior, computeTraversalHopDecay, computeTraversalFreshnessFactor, computeIntentAwareTraversalScore, getNeighborBoosts, applyCausalBoost, 
// Phase B T020
injectGraphContext, };
//# sourceMappingURL=causal-boost.js.map