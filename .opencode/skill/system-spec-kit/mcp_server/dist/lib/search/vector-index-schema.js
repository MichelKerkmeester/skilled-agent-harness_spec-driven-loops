...
import { get_error_message, } from './vector-index-types.js';
...
const REQUIRED_TABLES = ['memory_index', 'schema_version'];
const REQUIRED_INDEXES_BY_TABLE = {
    memory_index: [
        'idx_stability',
        'idx_last_review',
        'idx_fsrs_retrieval',
...
const REQUIRED_MEMORY_INDEX_COLUMNS = [
    'id',
    'spec_folder',
    'file_path',
    'importance_tier',
...
const REQUIRED_MEMORY_CONFLICT_COLUMNS = [
    'id',
    'timestamp',
    'action',
    'new_memory_hash',
...
const REQUIRED_LINEAGE_TABLES = [
    MEMORY_LINEAGE_TABLE,
    ACTIVE_MEMORY_PROJECTION_TABLE,
];
const REQUIRED_LINEAGE_COLUMNS = {
    [MEMORY_LINEAGE_TABLE]: [
        'memory_id',
        'logical_key',
        'version_number',
...
function hasTable(database, tableName) {
    const row = database.prepare("SELECT 1 AS present FROM sqlite_master WHERE type='table' AND name = ?").get(tableName);
    return row?.present === 1;
}
function hasIndex(database, indexName) {
    const row = database.prepare("SELECT 1 AS present FROM sqlite_master WHERE type='index' AND name = ?").get(indexName);
    return row?.present === 1;
}
...
function getTableSql(database, tableName) {
...
function hasEmbeddingCacheDimensionsPrimaryKey(database) {
...
function ensureEmbeddingCacheSchema(database) {
...
function createMemoryConflictsTable(database) {
    database.exec(`
...
function createMemoryConflictIndexes(database, context) {
    createRequiredIndex(database, 'idx_conflicts_memory', 'CREATE INDEX IF NOT EXISTS idx_conflicts_memory ON memory_conflicts(existing_memory_id)', context);
    createRequiredIndex(database, 'idx_conflicts_timestamp', 'CREATE INDEX IF NOT EXISTS idx_conflicts_timestamp ON memory_conflicts(timestamp DESC)', context);
}
function getFirstAvailableColumnExpression(availableColumns, candidates, fallback) {
...
function hasUnifiedMemoryConflictsTable(database) {
...
function migrateMemoryConflictsTable(database) {
    if (!hasTable(database, 'memory_conflicts')) {
...
function summarizeCompatibilityFailures(report) {
    const details = [];
...
function logDuplicateColumnMigrationSkip(columnName, error) {
    logger.warn(`Migration skipped existing ${columnName} column`, {
        error: error instanceof Error ? error.message : String(error),
    });
}
function getMigrationAllowedBasePaths() {
 ...
 * @returns Nothing.
 */
export function run_migrations(database, from_version, to_version) {
    const migrations = {
        1: () => {
        ...
        4: () => {
            ...
            createRequiredIndex(database, 'idx_stability', 'CREATE INDEX IF NOT EXISTS idx_stability ON memory_index(stability DESC)', 'Migration v4');
            createRequiredIndex(database, 'idx_last_review', 'CREATE INDEX IF NOT EXISTS idx_last_review ON memory_index(last_review)', 'Migration v4');
            createRequiredIndex(database, 'idx_fsrs_retrieval', 'CREATE INDEX IF NOT EXISTS idx_fsrs_retrieval ON memory_index(stability, difficulty, last_review)', 'Migration v4');
        ...
        13: () => {
            ...
            createRequiredIndex(database, 'idx_document_type', 'CREATE INDEX IF NOT EXISTS idx_document_type ON memory_index(document_type)', 'Migration v13');
            createRequiredIndex(database, 'idx_doc_type_folder', 'CREATE INDEX IF NOT EXISTS idx_doc_type_folder ON memory_index(document_type, spec_folder)', 'Migration v13');
        ...
        `);
            ...
            try {
                const rows = database.prepare('SELECT id, file_path FROM memory_index WHERE content_text IS NULL').all();
                let backfilled = 0;
                const allowedBasePaths = getMigrationAllowedBasePaths();
                const updateStmt = database.prepare('UPDATE memory_index SET content_text = ? WHERE id = ?');
        ...
        15: () => {
            ...
            createRequiredIndex(database, 'idx_quality_score', 'CREATE INDEX IF NOT EXISTS idx_quality_score ON memory_index(quality_score)', 'Migration v15');
    ...
    `).all();
        ...
        if (updates.length > 0) {
            const updateStmt = database.prepare('UPDATE memory_index SET spec_folder = ? WHERE id = ? AND spec_folder = ?');
    ...
    migrations[24] = () => {
        createRequiredIndex(database, 'idx_trigger_cache_source', `CREATE INDEX IF NOT EXISTS idx_trigger_cache_source
       ON memory_index(embedding_status, id)
       WHERE embedding_status = 'success'
         AND trigger_phrases IS NOT NULL
         AND trigger_phrases != '[]'
         AND trigger_phrases != ''`, 'Migration v24');
        createRequiredIndex(database, 'idx_spec_folder_created_at', 'CREATE INDEX IF NOT EXISTS idx_spec_folder_created_at ON memory_index(spec_folder, created_at DESC)', 'Migration v24');
    ...
    `).run();
        ...
        const hasContextTypeColumn = database.prepare(`SELECT COUNT(*) as cnt FROM pragma_table_info('memory_index') WHERE name = 'context_type'`).get();
        if (hasContextTypeColumn.cnt > 0) {
            // Get current table SQL to check if CHECK constraint needs updating
            const tableInfo = database.prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name='memory_index'`).get();
            if (tableInfo?.sql && !tableInfo.sql.includes("context_type IN ('research', 'implementation', 'planning', 'general')")) {
                ...
                const indexes = database.prepare(`SELECT sql FROM sqlite_master WHERE type='index' AND tbl_name='memory_index' AND sql IS NOT NULL`).all();
                ...
                createSql = createSql.replace(/CHECK\s*\(\s*context_type\s+IN\s*\([^)]+\)\s*\)/i, "CHECK(context_type IN ('research', 'implementation', 'planning', 'general'))");
                // If no CHECK existed (column added via ALTER TABLE), add one
                if (!createSql.includes('CHECK(context_type')) {
                    createSql = createSql.replace(/context_type\s+TEXT\s+DEFAULT\s+'general'/i, "context_type TEXT DEFAULT 'general' CHECK(context_type IN ('research', 'implementation', 'planning', 'general'))");
                }
 ...
 */
export function ensure_schema_version(database) {
    database.exec(`
  ...
  `);
    ...
    if (!compatibility.compatible) {
        logger.warn('Backward-compatibility validation detected schema gaps', compatibility);
    }
    initHistory(database);
    return current_version;
}
export function ensureLineageTables(database) {
    database.exec(`
  ...
  `);
    const lineageTableInfo = database.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='memory_lineage'").get();
  ...
  `);
    const projectionTableInfo = database.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='active_memory_projection'").get();
...
function ensureMemoryIndexGovernanceColumns(database) {
...
export function ensureGovernanceTables(database) {
...
export function ensureSharedSpaceTables(database) {
 ...
 */
export function validate_backward_compatibility(database) {
    try {
...
export function validateLineageSchemaSupport(database) {
    try {
...
function safe_get_schema_version(database) {
    try {
 ...
 * @returns Nothing.
 */
export function migrate_confidence_columns(database) {
    const columns = database.prepare(`PRAGMA table_info(memory_index)`).all();
 ...
 * @returns Nothing.
 */
export function ensure_canonical_file_path_support(database) {
    const columns = database.prepare('PRAGMA table_info(memory_index)').all();
    ...
    try {
        ...
        createRequiredIndex(database, 'idx_save_parent_canonical_path', SAVE_PARENT_CANONICAL_PATH_INDEX_SQL, 'ensure_canonical_file_path_support');
    }
    catch (e) {
  ...
  `);
    let updated = 0;
    const backfillTx = database.transaction((rows) => {
        for (const row of rows) {
            if (!row.file_path) {
                continue;
            }
 ...
 * @returns Nothing.
 */
export function migrate_constitutional_tier(database) {
    const tableSql = getTableSql(database, 'memory_index');
    if (tableSql) {
    ...
    `).get().count;
        throw new Error(constitutionalCount > 0
 ...
 * @returns Nothing.
 */
export function create_common_indexes(database) {
    try {
    ...
    try {
        database.exec('CREATE INDEX IF NOT EXISTS idx_canonical_file_path ON memory_index(canonical_file_path)');
        database.exec('CREATE INDEX IF NOT EXISTS idx_spec_canonical_path ON memory_index(spec_folder, canonical_file_path)');
        createRequiredIndex(database, 'idx_save_parent_canonical_path', SAVE_PARENT_CANONICAL_PATH_INDEX_SQL, 'create_common_indexes');
    }
    catch (err) {
    ...
    try {
        ...
        createRequiredIndex(database, 'idx_save_parent_content_hash_scope', SAVE_PARENT_CONTENT_HASH_SCOPE_INDEX_SQL, 'create_common_indexes');
 ...
 */
export function ensureCompanionTables(database) {
    database.exec(`
 ...
 */
function migrateSessionStateSpecFolders(database, updates) {
    if (!hasTable(database, 'session_state'))
        return;
    if (updates.length === 0)
        return;
    ...
    for (const u of updates) {
        if (!mapping.has(u.oldSpecFolder))
            mapping.set(u.oldSpecFolder, new Set());
        mapping.get(u.oldSpecFolder).add(u.canonical);
    }
    const updateStmt = database.prepare('UPDATE session_state SET spec_folder = ? WHERE session_id = ? AND spec_folder = ?');
    const sessionRows = database.prepare(`SELECT session_id, spec_folder FROM session_state WHERE spec_folder IS NOT NULL`).all();
    let updated = 0;
...
function migrateHistorySpecFolders(database, updates) {
    if (!hasTable(database, 'memory_history'))
        return;
    if (!getTableColumns(database, 'memory_history').includes('spec_folder'))
        return;
    if (updates.length === 0)
        return;
    const mapping = new Map();
    ...
    const updateStmt = database.prepare('UPDATE memory_history SET spec_folder = ? WHERE rowid = ? AND spec_folder = ?');
 ...
 * @returns Nothing.
 */
export function create_schema(database, options) {
  ...
  `).get();
    if (table_exists) {
        ...
        if (!compatibility.compatible) {
            logger.warn('Existing schema is not fully backward-compatible after bootstrap', compatibility);
        }
...
//# sourceMappingURL=vector-index-schema.js.map