 ...
 */
export function cheapSeedRetrieve(query, options) {
    const limit = options?.limit ?? SEED_LIMIT;
    try {
        const db = requireDb();
        const rawResults = fts5Bm25Search(db, query, { limit });
        return rawResults.slice(0, limit).map((row) => ({
 ...
 */
function buildReformulationPrompt(query, seeds) {
 ...
 */
async function callLlmForReformulation(prompt) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), REFORMULATION_TIMEOUT_MS);
    try {
        ...
        if (!response.ok) {
            console.warn(`[llm-reformulation] LLM endpoint returned HTTP ${response.status}`);
            return null;
        }
 ...
 */
function parseReformulationResponse(rawText) {
    try {
        ...
        const variants = rawVariants
            .filter((v) => typeof v === 'string' && v.trim().length >= MIN_OUTPUT_LENGTH)
            .map((v) => v.trim())
...
export const llm = {
    ...
    async rewrite(params) {
        ...
        const fallback = { abstract: q, variants: [] };
        ...
        try {
            ...
            const msg = err instanceof Error ? err.message : String(err);
            console.warn(`[llm-reformulation] llm.rewrite failed, returning fallback: ${msg}`);
        }
        return fallback;
    },
};
 ...
 */
export function fanout(queries) {
    const seen = new Set();
    const result = [];
...
//# sourceMappingURL=llm-reformulation.js.map