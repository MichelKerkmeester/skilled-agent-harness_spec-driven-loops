...
function extractSignals(row) {
    ...
    if (graphContrib) {
        ...
        if (causalDelta > 0)
            signals.push('causal_boosted');
        if (coActDelta > 0)
            signals.push('graph_boosted');
        if (commDelta > 0)
            signals.push('community_boosted');
        if (sigDelta > 0 && !signals.includes('graph_boosted'))
            signals.push('graph_boosted');
    }
    // Session boost
    ...
    for (const anchor of anchorMeta) {
        if (anchor && typeof anchor === 'object') {
            ...
            if (typeof label === 'string' && label.trim()) {
                const anchorSignal = `anchor:${label.toLowerCase().replace(/\s+/g, '-')}`;
 ...
 */
function selectTopSignals(signals) {
 ...
 */
function composeSummary(topSignals, rank) {
    ...
    const descriptions = topSignals.map((signal) => {
        if (signal === 'semantic_match')
            return 'semantic similarity';
        if (signal === 'lexical_match')
            return 'keyword match';
        if (signal === 'graph_boosted')
            return 'graph co-activation';
        if (signal === 'session_boosted')
            return 'session attention';
        if (signal === 'causal_boosted')
            return 'causal graph';
        if (signal === 'community_boosted')
            return 'community detection';
        if (signal === 'reranker_support')
            return 'reranker confirmation';
        if (signal === 'feedback_boosted')
            return 'learned trigger';
        if (signal === 'validation_quality')
            return 'high spec quality';
 ...
 */
function extractChannelContribution(row) {
    ...
    const graphTotal = typeof graphContrib?.totalDelta === 'number'
        ? Math.abs(graphContrib.totalDelta)
        : 0;
 ...
 */
export function attachExplainabilityToResults(results, options = {}, forceEnabled) {
    const enabled = forceEnabled !== undefined ? forceEnabled : isResultExplainEnabled();
    if (!enabled)
        return results;
    return results.map((row, index) => attachResultExplainability(row, index, options));
}
// ── Test surface ───────────────────────────────────────────────
...
//# sourceMappingURL=result-explainability.js.map