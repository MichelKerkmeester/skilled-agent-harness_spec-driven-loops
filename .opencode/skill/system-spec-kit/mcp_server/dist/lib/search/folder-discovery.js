...
function cachedFoldersMatchDiscoveredState(cache, discoveredSpecFolders) {
...
function normalizeBasePaths(basePaths) {
    const normalized = [];
    const seen = new Set();
...
function discoverSpecFolders(basePath) {
    const normalizedBasePath = resolveRealPathSafe(basePath) ?? path.resolve(basePath);
    const discovered = [];
    const visited = new Set([normalizedBasePath]);
    const walk = (currentPath, depth) => {
        if (depth > MAX_SPEC_DISCOVERY_DEPTH)
            return;
        let entries;
        try {
        ...
        for (const entry of entries) {
            ...
            if (visited.has(canonicalFolderPath))
                continue;
...
function collectDiscoveredSpecState(basePaths) {
 ...
 */
export function extractDescription(specContent) {
 ...
 */
export function findRelevantFolders(query, cache, limit = 3) {
 ...
 */
export function generateFolderDescriptions(specsBasePaths) {
    const now = new Date().toISOString();
    const normalizedBasePaths = normalizeBasePaths(specsBasePaths);
    const byCanonicalFolderPath = new Map();
    for (const basePath of normalizedBasePaths) {
        ...
        for (const discoveredFolder of discoveredFolders) {
            ...
            const folderEntry = _processSpecFolder(discoveredFolder.basePath, discoveredFolder.folderPath, discoveredFolder.specMdPath, now);
            if (folderEntry) {
    ...
    const folders = Array.from(byCanonicalFolderPath.values()).sort((a, b) => a.specFolder.localeCompare(b.specFolder));
    return {
        version: 1,
        generated: now,
        folders,
 ...
 * @param specsBasePaths - Array of absolute directory paths to scan.
 */
export function repairStaleDescriptions(specsBasePaths) {
    const normalizedBasePaths = normalizeBasePaths(specsBasePaths);
    for (const basePath of normalizedBasePaths) {
        const discoveredFolders = discoverSpecFolders(basePath);
        for (const discoveredFolder of discoveredFolders) {
            if (!isPerFolderDescriptionStale(discoveredFolder.folderPath))
                continue;
            try {
                const repaired = generatePerFolderDescription(discoveredFolder.folderPath, discoveredFolder.basePath);
                if (repaired)
 ...
 */
function _processSpecFolder(basePath, folderPath, specMdPath, timestamp) {
    let content;
    try {
    ...
    const description = rawDescription || slugifyFolderName(path.basename(folderPath)).replace(/-/g, ' ') || path.basename(folderPath);
 ...
 */
export function slugifyFolderName(folderName) {
    return folderName
        .replace(/^\d+-?/, '')
        .replace(/[^a-z0-9-]/gi, '-')
        .toLowerCase()
 ...
 */
export function generatePerFolderDescription(folderPath, basePath) {
 ...
 */
export function loadPerFolderDescription(folderPath) {
    ...
    try {
        ...
        if (!parsed ||
            ...
            !parsed.keywords.every((e) => typeof e === 'string') ||
            (parsed.specId !== undefined && typeof parsed.specId !== 'string') ||
            (parsed.folderSlug !== undefined && typeof parsed.folderSlug !== 'string') ||
            (parsed.parentChain !== undefined && (!Array.isArray(parsed.parentChain) || !parsed.parentChain.every((e) => typeof e === 'string'))) ||
            (parsed.memorySequence !== undefined && typeof parsed.memorySequence !== 'number') ||
            (parsed.memoryNameHistory !== undefined && (!Array.isArray(parsed.memoryNameHistory) || !parsed.memoryNameHistory.every((e) => typeof e === 'string')))) {
        ...
        if (!parsed.parentChain)
            parsed.parentChain = [];
        if (parsed.memorySequence === undefined)
            parsed.memorySequence = 0;
        if (!parsed.memoryNameHistory)
            parsed.memoryNameHistory = [];
        return parsed;
    }
    catch (_error) {
        return null;
 ...
 * @param folderPath - Absolute path to the spec folder.
 */
export function savePerFolderDescription(desc, folderPath) {
    ...
    try {
        ...
        catch (_e) { /* already renamed or missing */ }
    }
}
/**
 ...
 */
export function isPerFolderDescriptionStale(folderPath) {
    const descPath = path.join(folderPath, 'description.json');
 ...
 */
export function loadDescriptionCache(cachePath) {
 ...
 */
export function saveDescriptionCache(cache, cachePath) {
 ...
 */
export function getSpecsBasePaths(workspacePath) {
 ...
 */
export function isCacheStale(cache, basePaths) {
    if (!cache || !cache.generated)
        return true;
    let cacheTime;
    try {
 ...
 */
export function ensureDescriptionCache(basePaths) {
 ...
 */
export function discoverSpecFolder(query, basePaths, threshold = 0.3) {
    try {
...
//# sourceMappingURL=folder-discovery.js.map