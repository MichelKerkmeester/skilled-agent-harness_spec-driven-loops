 ...
 */
export function lowConfidence(baseline) {
 ...
 */
function resolveBaselineScore(result) {
 ...
 */
function buildHyDEPrompt(query, format, maxTokens) {
 ...
 */
async function callLlmForHyDE(prompt, maxTokens) {
 ...
 */
export async function generateHyDE(query, format = 'markdown-memory', options) {
    if (!isHyDEEnabled()) {
        return null;
    }
    ...
    try {
        ...
        const embedding = embeddingRaw instanceof Float32Array
            ? embeddingRaw
            : Float32Array.from(embeddingRaw);
        const result = { pseudoDocument, embedding };
 ...
 */
export function vectorOnly(embedding, limit, specFolder) {
 ...
 */
export async function runHyDE(query, baseline, limit, specFolder) {
    if (!isHyDEEnabled()) {
        return [];
    }
    ...
    try {
        ...
        if (process.env.SPECKIT_HYDE_LOG?.toLowerCase().trim() === 'true') {
            console.warn(`[hyde] shadow results: ${candidates.length} candidates for query "${query}" ` +
