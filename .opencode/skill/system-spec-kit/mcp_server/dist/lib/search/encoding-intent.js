 ...
 */
export function classifyEncodingIntent(content) {
    ...
    if (codeScore > 0.4 && codeScore > structuredScore)
        return 'code';
    if (structuredScore > 0.4 && structuredScore >= codeScore)
        return 'structured_data';
    return 'document';
}
function computeCodeScore(content) {
    ...
    const importLines = lines.filter((l) => /^\s*(import|export|require|const|let|var|function|class|interface|type)\b/.test(l)).length;
...
function computeStructuredScore(content) {
    const lines = content.split('\n');
    const totalLines = lines.length || 1;
    ...
    if (jsonBlocks > 2)
        structuredIndicators += 0.2;
    return Math.min(1.0, structuredIndicators);
}
//# sourceMappingURL=encoding-intent.js.map