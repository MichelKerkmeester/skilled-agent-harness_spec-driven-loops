 ...
 * ```
 */
function classifyByConfidence(results) {
    let high = 0;
    let medium = 0;
    let low = 0;
...
function pruneExpiredCursorEntries(ttlMs = DEFAULT_CURSOR_TTL_MS, now = Date.now()) {
 ...
 * ```
 */
function buildDigest(classification) {
    const parts = [];
    if (classification.high > 0)
        parts.push(`${classification.high} strong`);
    if (classification.medium > 0)
        parts.push(`${classification.medium} moderate`);
    if (classification.low > 0)
        parts.push(`${classification.low} weak`);
 ...
 * ```
 */
function isProgressiveDisclosureEnabled() {
 ...
 * ```
 */
function generateSummaryLayer(results) {
 ...
 * ```
 */
function extractSnippets(results) {
 ...
 * ```
 */
function createCursor(resultSet, pageSize = DEFAULT_PAGE_SIZE, query = '', options) {
 ...
 * ```
 */
function resolveCursor(cursor, pageSize = DEFAULT_PAGE_SIZE, options) {
    const ttlMs = options?.ttlMs ?? DEFAULT_CURSOR_TTL_MS;
    const payload = decodeCursor(cursor);
    if (!payload)
        return null;
    const storeKey = payload.cursorKey ?? payload.queryHash;
    if (typeof payload.scopeKey === 'string'
 ...
 * ```
 */
function buildProgressiveResponse(results, pageSize = DEFAULT_PAGE_SIZE, query = '', options) {
    if (!Array.isArray(results) || results.length === 0) {
        return {
 ...
 * ```
 */
function clearCursorStore() {
    cursorStore.clear();
}
// -- Exports --
export { 
// Constants
DEFAULT_PAGE_SIZE, DEFAULT_CURSOR_TTL_MS, SNIPPET_MAX_LENGTH, 
// Feature flag
isProgressiveDisclosureEnabled, 
// Public API
generateSummaryLayer, extractSnippets, createCursor, resolveCursor, buildProgressiveResponse, 
// Testing utilities
clearCursorStore, 
// Internal helpers (exported for testing)
hashQuery, encodeCursor, decodeCursor, classifyByConfidence, buildDigest, };
