 ...
 */
export function analyzeChannelRepresentation(topK, allChannelResults) {
    ...
    const representedChannels = new Set();
    ...
    const underRepresentedChannels = [];
    for (const [channelName, channelResults] of allChannelResults) {
        if (channelResults.length === 0)
            continue; // channel returned nothing — not penalised
    ...
    for (const channelName of underRepresentedChannels) {
        ...
        if (best === null)
            continue; // no qualifying result — skip channel
        const promotedItem = {
            ...best,
            source: channelName,
...
//# sourceMappingURL=channel-representation.js.map