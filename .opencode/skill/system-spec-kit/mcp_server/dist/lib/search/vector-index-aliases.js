...
import { parse_trigger_phrases, get_error_message, } from './vector-index-types.js';
import { initialize_db, search_weights, } from './vector-index-store.js';
import { vector_search, vector_search_enriched, apply_smart_ranking, apply_diversity, generate_query_embedding, } from './vector-index-queries.js';
...
export class LRUCache {
    ...
    constructor(max_size, ttl_ms) {
        ...
        this.ttl_ms = ttl_ms;
        this.cache = new Map();
        this.head = { key: '__head__', value: null, timestamp: 0, prev: null, next: null };
        this.tail = { key: '__tail__', value: null, timestamp: 0, prev: this.head, next: null };
 ...
 */
export function get_cache_key(query, limit, options) {
 ...
 */
export async function cached_search(query, limit = 20, options = {}) {
 ...
 */
export function clear_search_cache(spec_folder = null) {
    if (!query_cache) {
        return 0;
    }
    if (spec_folder) {
 ...
 */
export function learn_from_selection(search_query, selected_memory_id) {
    if (!search_query || !selected_memory_id)
        return false;
    const database = initialize_db();
    let memory;
    try {
        memory = database.prepare('SELECT trigger_phrases FROM memory_index WHERE id = ?').get(selected_memory_id);
    }
    catch (e) {
    ...
    const new_terms = search_query
        ...
        if (term.length < 4)
            return false;
        if (stop_words.includes(term))
            return false;
        if (existing.some((e) => e.toLowerCase() === term))
            return false;
        if (/^\d+$/.test(term))
            return false;
        return true;
    ...
    try {
        database.prepare('UPDATE memory_index SET trigger_phrases = ? WHERE id = ?').run(JSON.stringify(updated), selected_memory_id);
        return true;
    }
    catch (e) {
 ...
 */
export async function link_related_on_save(new_memory_id, content) {
 ...
 */
export function record_access(memory_id) {
    try {
        const database = initialize_db();
        const now = Date.now();
 ...
 */
export async function enhanced_search(query, limit = 20, options = {}, database = initialize_db()) {
...
//# sourceMappingURL=vector-index-aliases.js.map