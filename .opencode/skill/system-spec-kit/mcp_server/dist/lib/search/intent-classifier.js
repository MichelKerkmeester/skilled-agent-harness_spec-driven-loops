 ...
 */
const INTENT_NEGATIVE_PATTERNS = {
    understand: [
...
const INTENT_CENTROIDS = buildIntentCentroids();
/* --- 2. SCORING FUNCTIONS --- */
/**
 ...
 */
function calculateKeywordScore(query, intent) {
 ...
 */
function calculatePatternScore(query, intent) {
 ...
 */
function isExplicitSpecLookup(query) {
    return (/(?:find|show|get)\s+(?:me\s+)?(?:the\s+)?(?:spec|specification|requirements|scope|plan|checklist)/i.test(query) ||
 ...
 */
function toDeterministicEmbedding(text) {
 ...
 */
function calculateCentroidScore(query, intent) {
...
function buildRankedIntents(scores) {
 ...
 */
function applyIntentWeights(results, intent) {
    ...
    const timestamps = results.map(r => {
        const dateStr = r.created_at || r.last_accessed || r.last_review;
        if (!dateStr)
            return 0;
    ...
    return results.map((r, i) => {
        ...
        return {
            ...r,
            intentAdjustedScore: ((r.similarity || 0) / 100) * weights.similarity +
                (r.importance_weight || 0.5) * weights.importance +
                recencyRaw * weights.recency,
        };
    }).sort((a, b) => (b.intentAdjustedScore || 0) - (a.intentAdjustedScore || 0));
}
/**
 ...
 */
function getProfileForIntent(intent) {
...
export { INTENT_TYPES, INTENT_KEYWORDS, INTENT_PATTERNS, INTENT_WEIGHT_ADJUSTMENTS, INTENT_LAMBDA_MAP, INTENT_TO_PROFILE, 
// Scoring
calculateKeywordScore, calculatePatternScore, calculateCentroidScore, dotProduct, toDeterministicEmbedding, INTENT_CENTROIDS, 
// Classification
classifyIntent, detectIntent, getIntentWeights, applyIntentWeights, getQueryWeights, isValidIntent, getValidIntents, getIntentDescription, getProfileForIntent, };
