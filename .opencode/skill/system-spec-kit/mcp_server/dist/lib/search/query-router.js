...
import { classifyQueryComplexity, isComplexityRouterEnabled, } from './query-classifier.js';
import { getStrategyForQuery } from './artifact-routing.js';
import { classifyIntent } from './intent-classifier.js';
/** All available channels in execution order. */
const ALL_CHANNELS = ['vector', 'fts', 'bm25', 'graph', 'degree'];
...
const FALLBACK_CHANNELS = ['vector', 'fts'];
 ...
 */
function enforceMinimumChannels(channels) {
    const result = [...new Set(channels)];
    if (result.length >= MIN_CHANNELS)
        return result;
 ...
 */
function getChannelSubset(tier, config) {
...
function shouldPreserveBm25(query) {
 ...
 */
function routeQuery(query, triggerPhrases) {
...
export { 
// Constants
DEFAULT_ROUTING_CONFIG, ALL_CHANNELS, MIN_CHANNELS, FALLBACK_CHANNELS, 
// Functions
getChannelSubset, routeQuery, 
