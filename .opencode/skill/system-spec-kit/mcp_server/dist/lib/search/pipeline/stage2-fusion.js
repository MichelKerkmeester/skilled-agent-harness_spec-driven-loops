...
import { isEnabled as isCoActivationEnabled, spreadActivation, getRelatedMemoryCounts, resolveCoActivationBoostFactor, } from '../../cognitive/co-activation.js';
...
import { isNegativeFeedbackEnabled, isCommunityDetectionEnabled, isGraphCalibrationProfileEnabled, isGraphSignalsEnabled, isUsageRankingEnabled, resolveGraphWalkRolloutState, isLearnedStage2CombinerEnabled, isResultProvenanceEnabled, } from '../search-flags.js';
...
let cachedLearnedStage2Model = null;
let learnedStage2ModelLoadPromise = null;
let learnedStage2ModelLoadPromisePath = null;
function clampMultiplier(value) {
    if (!Number.isFinite(value))
        return 1;
    if (value < MIN_VALIDATION_MULTIPLIER)
        return MIN_VALIDATION_MULTIPLIER;
    if (value > MAX_VALIDATION_MULTIPLIER)
        return MAX_VALIDATION_MULTIPLIER;
    return value;
}
function isShadowLearningModelLoadEnabled() {
    return process.env.SPECKIT_SHADOW_LEARNING?.toLowerCase().trim() === 'true';
}
function resolveLearnedStage2ModelPath() {
...
async function loadPersistedLearnedStage2Model() {
    ...
    learnedStage2ModelLoadPromise = (async () => {
        try {
 ...
 */
function applyValidationSignalScoring(results) {
    if (!Array.isArray(results) || results.length === 0)
        return results;
    const adjusted = results.map((row) => {
        const metadata = row.validationMetadata;
    ...
    return sortDeterministicRows(adjusted);
}
// -- Internal helpers --
/**
...
function withSyncedScoreAliases(row, score) {
...
function syncScoreAliasesInPlace(rows) {
...
function withGraphContribution(row, key, delta, source, injected = false) {
    ...
    const sources = Array.isArray(current.sources)
        ? current.sources.filter((value) => typeof value === 'string')
        : [];
...
function countGraphContribution(rows, key) {
    return rows.filter((row) => {
        const graphContribution = row.graphContribution;
        return typeof graphContribution?.[key] === 'number' && Math.abs(graphContribution[key]) > 0;
    }).length;
}
function countGraphInjected(rows) {
    return rows.filter((row) => {
        const graphContribution = row.graphContribution;
        return graphContribution?.injected === true;
    }).length;
}
function resolveGraphContributionValue(graphContribution, key) {
    const value = graphContribution?.[key];
    return typeof value === 'number' && Number.isFinite(value) ? value : 0;
}
function applyGraphCalibrationProfileToResults(results) {
 ...
 */
function populateGraphEvidence(results) {
    if (!isResultProvenanceEnabled())
        return results;
    if (!Array.isArray(results) || results.length === 0)
        return results;
    let db = null;
    ...
    const graphBoostedIds = [];
    for (const row of results) {
        const gc = row.graphContribution;
        if (!gc)
            continue;
    ...
    `).all(...graphBoostedIds.map(String), ...graphBoostedIds.map(String));
    ...
    `).all(...graphBoostedIds.map(String));
    ...
    return results.map((row) => {
        ...
        if (causalDelta !== 0)
            boostFactors.push({ type: 'causal', delta: causalDelta });
        if (coActivationDelta !== 0)
            boostFactors.push({ type: 'co-activation', delta: coActivationDelta });
        if (communityDelta !== 0)
            boostFactors.push({ type: 'community', delta: communityDelta });
        if (graphSignalDelta !== 0)
            boostFactors.push({ type: 'graph-signals', delta: graphSignalDelta });
        return {
            ...row,
            graphEvidence: {
                edges,
 ...
 */
function strengthenOnAccess(db, memoryId, retrievability) {
    ...
    const clampedR = (typeof retrievability === 'number' && retrievability >= 0 && retrievability <= 1)
        ...
        : 0.9;
    try {
        const memory = db.prepare('SELECT stability, difficulty, review_count FROM memory_index WHERE id = ?').get(memoryId);
        if (!memory)
            return null;
        const grade = fsrsScheduler.GRADE_GOOD;
        const difficultyBonus = Math.max(0, (0.9 - clampedR) * 0.5);
        const newStability = fsrsScheduler.updateStability(memory.stability || fsrsScheduler.DEFAULT_INITIAL_STABILITY, memory.difficulty || fsrsScheduler.DEFAULT_INITIAL_DIFFICULTY, grade, clampedR) * (1 + difficultyBonus);
        db.prepare(`
      UPDATE memory_index
      SET stability = ?,
    ...
    `).run(newStability, Date.now(), memoryId);
        return { stability: newStability, difficulty: memory.difficulty || fsrsScheduler.DEFAULT_INITIAL_DIFFICULTY };
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
 ...
 */
function applyIntentWeightsToResults(results, weights) {
    if (!Array.isArray(results) || results.length === 0)
        return results;
    if (!weights)
        return results;
    const scored = results.map((row) => {
        ...
        const recencyTimestamp = row.created_at ?? '';
        const importanceTier = row.importance_tier ?? 'normal';
        const recency = computeRecencyScore(recencyTimestamp, importanceTier);
        const intentScore = similarity * weights.similarity +
...
function applyArtifactRouting(results, routingResult) {
    if (!Array.isArray(results) || results.length === 0)
        return results;
    if (!routingResult || routingResult.confidence <= 0)
        return results;
    // Obtain boostFactor from the strategy object (passed through config)
    const strategy = routingResult.strategy;
    ...
    if (boostFactor === 1.0) {
        ...
        return sortDeterministicRows(results);
    }
    ...
    return sortDeterministicRows(boosted);
}
/**
 ...
 */
function applyFeedbackSignals(results, query) {
    if (!Array.isArray(results) || results.length === 0)
        return results;
    let db = null;
    try {
    ...
    let learnedBoostMap = new Map();
    try {
        const learnedMatches = queryLearnedTriggers(query, db);
    ...
    let negativeFeedbackStats = new Map();
    if (isNegativeFeedbackEnabled()) {
        try {
            const memoryIds = results.map((r) => r.id);
            negativeFeedbackStats = getNegativeFeedbackStats(db, memoryIds);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
    ...
    return results.map((row) => {
        ...
        if (isNegativeFeedbackEnabled()) {
            ...
            if (negStats && negStats.negativeCount > 0) {
                currentScore = applyNegativeFeedback(currentScore, negStats.negativeCount, negStats.lastNegativeAt);
            }
        }
        if (currentScore === resolveBaseScore(row))
            return row;
        return withSyncedScoreAliases(row, currentScore);
    });
}
function applyUsageRankingBoost(db, results) {
    ...
    `).all(...resultIds);
        ...
        const maxAccess = usageRows.reduce((currentMax, row) => Math.max(currentMax, Number.isFinite(row.access_count) ? row.access_count : 0), 0);
        if (maxAccess <= 0) {
            return results;
        }
 ...
 */
function applyTestingEffect(db, results) {
    if (!db || !Array.isArray(results) || results.length === 0)
        return;
    for (const row of results) {
        try {
            const lastReview = row.last_review || row.created_at;
            if (!lastReview)
                continue;
            ...
            const currentR = fsrsScheduler.calculateRetrievability(stability, Math.max(0, elapsedDays));
...
function recordAdaptiveAccessSignals(db, results, query) {
    if (!db || !Array.isArray(results) || results.length === 0)
        return;
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `);
    const writeAdaptiveSignals = db.transaction((accessedResults) => {
        for (const result of accessedResults) {
            insertAdaptiveSignal.run(result.id, 'access', 1.0, query || '', '', '{}');
        }
    });
    try {
        ...
        console.warn('[stage2-fusion] adaptive access signal write failed:', err?.message ?? err);
    }
}
 ...
 */
export async function executeStage2(input) {
    ...
    let results = candidates.map(row => ({ ...row }));
    ...
    if (isHybrid && config.enableSessionBoost && config.sessionId) {
        try {
            const { results: boosted, metadata: sbMeta } = sessionBoost.applySessionBoost(results, config.sessionId);
            results = boosted;
    ...
    try {
        ...
        if (recencyBoostedCount > 0) {
            syncScoreAliasesInPlace(results);
            metadata.recencyFusionApplied = true;
            metadata.recencyFusionBoosted = recencyBoostedCount;
        }
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        ...
        try {
            ...
            if (topIds.length > 0) {
                ...
                if (spreadResults.length > 0) {
                    ...
                    results = sortDeterministicRows(results);
                    metadata.coActivationApplied = true;
                }
            }
        }
        catch (err) {
    ...
    if (isCommunityDetectionEnabled() && isGraphUnifiedEnabled()) {
        ...
        try {
            ...
            if (boosted.length > results.length) {
                results = boosted.map((row) => beforeCommunityIds.has(row.id)
                    ? row
                    : withGraphContribution(row, 'communityDelta', 0, 'community', true));
                metadata.communityBoostApplied = true;
            }
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
        ...
        if (injectedIds.length > 0) {
            try {
                ...
                const stateRows = db.prepare(`SELECT id, memory_state FROM memory_index WHERE id IN (${placeholders})`).all(...injectedIds);
    ...
    if (isGraphSignalsEnabled() && isGraphUnifiedEnabled()) {
        try {
            ...
            metadata.graphSignalsApplied = true;
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
    ...
    if (!isHybrid && config.intentWeights) {
        try {
            ...
            results = weighted.map((result) => typeof result.intentAdjustedScore === 'number'
        ...
        try {
            ...
            for (const row of results) {
                const features = extractFeatureVector({
                    ...
                    validationScore: (() => {
                        const vm = row.validationMetadata;
                ...
                if (shadow) {
                    console.warn(`[stage2-fusion] shadow-learned id=${row.id} manual=${shadow.manualScore.toFixed(4)} learned=${shadow.learnedScore.toFixed(4)} delta=${shadow.delta.toFixed(4)}`);
                }
            }
        }
        catch (err) {
    ...
    if (metadata.graphContribution) {
        ...
        if (!metadata.graphContribution.killSwitchActive &&
            ...
            results.length > 0) {
            console.warn('[stage2-fusion] Graph channel active (bounded_runtime) but zero contribution — causal_edges table may be sparse or candidates lack graph connectivity');
        }
    }
    // -- Trace --
    if (config.trace) {
        addTraceEntry(config.trace, 'fusion', candidates.length, results.length, Date.now() - start, {
...
//# sourceMappingURL=stage2-fusion.js.map