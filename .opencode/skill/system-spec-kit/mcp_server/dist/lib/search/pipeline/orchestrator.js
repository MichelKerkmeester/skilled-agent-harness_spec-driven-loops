 ...
 */
export async function executePipeline(config) {
    const timing = {};
    ...
    try {
        const t0 = Date.now();
        stage1Result = await withTimeout(executeStage1({ config }), STAGE_TIMEOUT_MS, 'Stage 1: Candidate Generation');
        timing.stage1 = Date.now() - t0;
    }
    catch (err) {
        throw new MemoryError('PIPELINE_STAGE1_FAILED', `Candidate generation failed: ${err instanceof Error ? err.message : String(err)}`, { cause: err instanceof Error ? err.message : String(err) });
    }
    // -- Stage 2: Fusion + Signal Integration (falls back to unsorted candidates) --
    let stage2Result;
    try {
        const t0 = Date.now();
        stage2Result = await withTimeout(executeStage2({
            candidates: stage1Result.candidates,
            config,
            stage1Metadata: stage1Result.metadata,
        }), STAGE_TIMEOUT_MS, 'Stage 2: Fusion');
        ...
        stage2Result = {
            ...
            metadata: {
                sessionBoostApplied: 'failed',
                causalBoostApplied: 'failed',
                intentWeightsApplied: 'failed',
                artifactRoutingApplied: 'failed',
                feedbackSignalsApplied: 'failed',
                qualityFiltered: 0,
                durationMs: 0,
            },
        };
    ...
    try {
        const t0 = Date.now();
        stage3Result = await withTimeout(executeStage3({
            scored: stage2Result.scored,
            config,
        }), STAGE_TIMEOUT_MS, 'Stage 3: Rerank');
    ...
    try {
        const t0 = Date.now();
        stage4Result = await withTimeout(executeStage4({
            results: stage3Result.reranked,
            config,
            stage1Metadata: { constitutionalInjected: stage1Result.metadata.constitutionalInjected },
        }), STAGE_TIMEOUT_MS, 'Stage 4: Filter');
