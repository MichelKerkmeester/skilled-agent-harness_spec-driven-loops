...
import { isMultiFacet, decompose, mergeByFacetCoverage as mergeFacetCoveragePools, MAX_FACETS, } from '../query-decomposer.js';
...
function applyArchiveFilter(results, includeArchived) {
    if (includeArchived)
        return results;
    return results.filter((row) => {
...
function applyFolderFilter(results, specFolder) {
    if (!specFolder)
        return results;
    return results.filter((row) => {
...
function applyTierFilter(results, tier) {
    if (!tier)
        return results;
...
function readFiniteScoreMap(value) {
...
function getCandidateSources(row) {
...
function getCandidateSourceScores(row) {
...
function annotateBranchScore(row, branchLabel) {
...
function mergeCandidateRows(existing, incoming, branchLabel) {
    ...
    const mergedChannelAttribution = mergeStringLists(existing.channelAttribution, incoming.channelAttribution, mergedSources);
    const mergedSourceScores = mergeScoreMaps(getCandidateSourceScores(existing), getCandidateSourceScores(incoming));
    const mergedBranchScores = mergeScoreMaps(readFiniteScoreMap(existing.stage1BranchScores), incomingBranchScores);
    return {
        ...secondary,
        ...primary,
...
function mergeCandidateBatches(batches, options = {}) {
 ...
 */
async function buildDeepQueryVariants(query) {
    try {
        const expanded = expandQuery(query);
        const variants = new Set(expanded);
        ...
        console.warn(`[stage1-candidate-gen] buildDeepQueryVariants failed, using original query: ${msg}`);
        return [query];
    }
}
function normalizeFacetText(fragment) {
    return fragment
        .replace(/^[\s,:;]+|[\s,.:;?!]+$/g, '')
        .replace(/\s+/g, ' ')
        .trim();
}
function decomposeQueryFacets(query) {
...
function buildQueryDecompositionPool(query, mode) {
...
function mergeQueryFacetCoverage(resultSets) {
 ...
 */
export async function executeStage1(input) {
    ...
    const { query, queryEmbedding, concepts, searchType, mode, limit, specFolder, tenantId, userId, agentId, sharedSpaceId, tier, contextType, includeArchived, includeConstitutional, qualityThreshold, trace, } = config;
    let candidates = [];
    let channelCount = 0;
            ...
            if (routing.graphActivated && routing.concepts.length > 0) {
                ...
                if (isQueryConceptExpansionEnabled()) {
                    try {
                        ...
                        const expansionTerms = getConceptExpansionTerms(routing.concepts, originalTokens, 5);
    ...
    if (searchType === 'multi-concept' && Array.isArray(concepts) && concepts.length >= 2) {
        ...
        for (const concept of concepts) {
            ...
            if (!emb) {
                throw new Error(`[stage1-candidate-gen] Failed to generate embedding for concept: "${concept}"`);
            }
    ...
    //
        ...
        const effectiveEmbedding = queryEmbedding ?? (await embeddings.generateQueryEmbedding(query));
        ...
        if (mode === 'deep' && isMultiQueryEnabled()) {
            ...
            if (isQueryDecompositionEnabled() && isMultiFacet(query)) {
                try {
                    ...
                    try {
                        ...
                        console.warn(`[stage1-candidate-gen] D2 faceted query decomposition failed, falling back to basic decomposition: ${facetMsg}`);
                    }
                    if (facets.length === 0) {
                    ...
                    if (facets.length > 0) {
                        ...
                        const facetSettledResults = await withTimeout(Promise.allSettled(allQueries.map(async (q) => {
                            ...
                            return hybridSearch.collectRawCandidates(q, facetEmbedding, { limit, specFolder, includeArchived });
                        })), DECOMPOSITION_TIMEOUT_MS, 'D2 facet decomposition');
                    ...
                    console.warn(`[stage1-candidate-gen] D2 query decomposition failed, falling through to expansion: ${decompMsg}`);
            ...
            if (candidates.length === 0) {
                ...
                if (queryVariants.length > 1) {
                    try {
                        ...
                        const variantSettledResults = await withTimeout(Promise.allSettled(queryVariants.map(async (variant) => {
                            ...
                            const variantResults = await hybridSearch.collectRawCandidates(variant, variantEmbedding, { limit, specFolder, includeArchived });
                            return variantResults;
                        })), DEEP_EXPANSION_TIMEOUT_MS, 'Deep-mode query expansion');
                        const variantResultSets = variantSettledResults.map((result, idx) => {
                            if (result.status === 'fulfilled')
                                return result.value;
                        ...
                        candidates = mergeCandidateBatches(variantResultSets.map((rows, index) => ({
                            label: queryVariants[index] ?? `deep-variant-${index}`,
                            rows,
                        })));
                    }
                    catch (expandErr) {
                        const expandMsg = expandErr instanceof Error ? expandErr.message : String(expandErr);
                        console.warn(`[stage1-candidate-gen] Deep query expansion failed, falling back to single hybrid: ${expandMsg}`);
                        // Fall through to single hybrid search below
                        channelCount = 1;
                        candidates = (await hybridSearch.collectRawCandidates(query, effectiveEmbedding, { limit, specFolder, includeArchived }));
                    }
                }
                else {
            ...
            if (isEmbeddingExpansionEnabled() && isExpansionActive(query)) {
                try {
                    ...
                    const expansionEmbedding = effectiveEmbedding instanceof Float32Array
                        ? effectiveEmbedding
                        : Float32Array.from(effectiveEmbedding);
                    ...
                    if (expanded.expanded.length > 0 && expanded.combinedQuery !== query) {
                        ...
                        const [baselineResults, expansionResults] = await Promise.all([
                            hybridSearch.collectRawCandidates(query, effectiveEmbedding, { limit, specFolder, includeArchived }).catch((err) => {
                                console.warn('[stage1-candidate-gen] Baseline candidate collection failed:', err instanceof Error ? err.message : String(err));
                                return [];
                            }),
                            embeddings.generateQueryEmbedding(expanded.combinedQuery).then(async (expandedEmb) => {
                                if (!expandedEmb) {
                                    console.warn('[stage1-candidate-gen] Expanded query embedding generation returned null');
                                    return [];
                                }
                                return hybridSearch.collectRawCandidates(expanded.combinedQuery, expandedEmb, { limit, specFolder, includeArchived });
                            }).catch((err) => {
                                console.warn('[stage1-candidate-gen] Expansion candidate collection failed:', err instanceof Error ? err.message : String(err));
                                return [];
                            }),
                        ]);
                        channelCount = 2;
                        ...
                        candidates = mergeCandidateBatches([
                            { label: query, rows: baselineResults },
                            { label: expanded.combinedQuery, rows: expansionResults },
                        ]);
                        if (trace) {
                            addTraceEntry(trace, 'candidate', channelCount, candidates.length, 0, {
                    ...
                    console.warn(`[stage1-candidate-gen] R12 embedding expansion failed, using standard hybrid: ${r12Msg}`);
                }
            }
            ...
            if (!r12ExpansionApplied) {
                try {
                    channelCount = 1;
                    const hybridResults = (await hybridSearch.collectRawCandidates(effectiveQuery, effectiveEmbedding, { limit, specFolder, includeArchived }));
                    candidates = hybridResults;
                }
                catch (hybridErr) {
                    const hybridMsg = hybridErr instanceof Error ? hybridErr.message : String(hybridErr);
                    console.warn(`[stage1-candidate-gen] Hybrid search failed, falling back to vector: ${hybridMsg}`);
                    ...
                    if (isTemporalContiguityEnabled()) {
                        vectorResults = vectorSearchWithContiguity(vectorResults, 3600) ?? vectorResults;
                    }
    ...
    // -- Unknown search type -----------------------------------------------------
        ...
        throw new Error(`[stage1-candidate-gen] Unknown searchType: "${searchType}". Expected 'multi-concept', 'hybrid', or 'vector'.`);
    }
    ...
    if (contextType) {
        candidates = candidates.filter((r) => resolveRowContextType(r) === contextType);
    }
    ...
    const hasGovernanceScope = Boolean(tenantId
        || userId
        || agentId
        || sharedSpaceId);
    ...
    if (includeConstitutional && !tier) {
        const existingConstitutional = candidates.filter((r) => r.importance_tier === 'constitutional');
        if (existingConstitutional.length === 0) {
            // Fix #16 — Reuse cached embedding instead of generating a new one
            const constitutionalEmbedding = cachedEmbedding ?? queryEmbedding ?? (await embeddings.generateQueryEmbedding(query));
            if (constitutionalEmbedding) {
                const constitutionalResults = vectorIndex.vectorSearch(constitutionalEmbedding, {
                ...
                const filteredConstitutional = shouldApplyScopeFiltering
                    ? filterRowsByScope(contextFilteredConstitutional, scopeFilter, allowedSharedSpaceIds)
        ...
        candidates = candidates.filter((r) => r.importance_tier !== 'constitutional');
    }
    ...
    if (mode === 'deep' && isLlmReformulationEnabled() && searchType === 'hybrid') {
        try {
            ...
            if (allQueries.length > 1) {
                const reformEmbedding = cachedEmbedding ?? queryEmbedding ?? (await embeddings.generateQueryEmbedding(query));
                if (reformEmbedding) {
                    // FIX #7: Use Promise.allSettled so one failing reformulation
                    // branch does not discard results from all other branches.
                    const reformSettledResults = await Promise.allSettled(allQueries.map(async (q, idx) => {
                        ...
                        return hybridSearch.collectRawCandidates(q, emb, { limit, specFolder, includeArchived });
                    }));
    ...
    if (isMemorySummariesEnabled()) {
        try {
            ...
            if (checkScaleGate(db)) {
                const summaryEmbedding = queryEmbedding ?? (await embeddings.generateQueryEmbedding(query));
                if (summaryEmbedding) {
                    ...
                    if (summaryResults.length > 0) {
                        ...
                        if (newSummaryIds.length > 0) {
                            ...
                            const memRows = db.prepare(`SELECT id, title, spec_folder, file_path, importance_tier, importance_weight, quality_score, created_at, is_archived, context_type, tenant_id, user_id, agent_id, session_id, shared_space_id FROM memory_index WHERE id IN (${placeholders})`).all(...newSummaryIds);
            ...
            if (candidateIds.length > 0) {
                ...
                if (surrogateMap.size > 0) {
                    ...
                    candidates = candidates.map((row) => {
                        const surrogates = surrogateMap.get(row.id);
                        if (!surrogates)
                            return row;
    ...
    if (trace) {
        addTraceEntry(trace, 'candidate', channelCount, candidates.length, durationMs, {
            searchType,
            mode: mode ?? null,
            channelCount,
...
//# sourceMappingURL=stage1-candidate-gen.js.map