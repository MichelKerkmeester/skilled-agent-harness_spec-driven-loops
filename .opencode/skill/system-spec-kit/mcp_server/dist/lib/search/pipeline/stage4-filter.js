...
function resolveStateForFiltering(row) {
 ...
 */
export function filterByMemoryState(results, minState, applyStateLimits) {
 ...
 */
export function extractScoringValue(row) {
 ...
 */
export async function executeStage4(input) {
    ...
    const filterResult = filterByMemoryState(results, config.minState, config.applyStateLimits);
    let workingResults = filterResult.filtered;
    ...
    // -- Trace entry --
    if (config.trace) {
        addTraceEntry(config.trace, 'filter', results.length, workingResults.length, durationMs, {
            stateFiltered,
            evidenceGapDetected,
            trmEnabled: isTRMEnabled(),
...
//# sourceMappingURL=stage4-filter.js.map