...
const TEXT_FIELD_PRIORITY = ['content', 'file_path'];
/**
...
function resolveRerankOutputScore(raw, fallback) {
 ...
 */
export async function executeStage3(input) {
    const stageStart = Date.now();
    const { scored, config } = input;
    let results = scored;
    let rerankApplied = false;
    let rerankProvider = 'none';
    ...
    if (config.trace) {
        addTraceEntry(config.trace, 'rerank', beforeRerank, results.length, Date.now() - rerankStart, { rerankApplied, provider: rerankProvider });
    }
    ...
    if (isMMREnabled() && results.length >= MMR_MIN_CANDIDATES) {
        try {
            ...
            if (numericIds.length >= MMR_MIN_CANDIDATES) {
                ...
                const embRows = db.prepare(`SELECT rowid, embedding FROM vec_memories WHERE rowid IN (${placeholders})`).all(...numericIds);
    ...
    const { results: aggregated, stats: chunkStats } = await collapseAndReassembleChunkResults(results);
    results = aggregated;
    if (config.trace) {
        addTraceEntry(config.trace, 'final-rank', beforeMpab, results.length, Date.now() - mpabStart, {
 ...
 */
async function applyCrossEncoderReranking(query, results, options) {
    // Feature-flag guard
    ...
    if (isLocalRerankerEnabled()) {
        try {
            ...
            const localRows = localReranked.map((row) => {
                const original = rowMap.get(row.id);
                const rerankScoreRaw = row.rerankerScore ?? row.score;
                const rerankScore = resolveRerankOutputScore(rerankScoreRaw, original ? effectiveScore(original) : 0);
                return {
                    ...(original ?? row),
                    ...row,
            ...
            console.warn(`[stage3-rerank] Local reranking failed: ${toErrorMessage(err)} — returning original results`);
            return { rows: results, applied: false, provider: 'local-gguf' };
        }
    }
    ...
    try {
        ...
        const reranked = await crossEncoder.rerankResults(query, documents, {
            limit: options.limit,
            useCache: true,
            applyLengthPenalty: options.applyLengthPenalty,
        });
        const rerankProvider = reranked.some((result) => result.scoringMethod === 'fallback')
            ? 'fallback-sort'
            : 'cross-encoder';
        ...
        for (const rerankResult of reranked) {
            ...
            const rerankScore = resolveRerankOutputScore(rerankResult.rerankerScore ?? rerankResult.score, effectiveScore(original));
            rerankedRows.push({
                ...original,
        ...
        console.warn(`[stage3-rerank] Cross-encoder reranking failed: ${toErrorMessage(err)} — returning original results`);
 ...
 */
function resolveDisplayText(row) {
 ...
 */
async function collapseAndReassembleChunkResults(results) {
    const stats = {
    ...
    for (const [parentId, chunks] of chunksByParent) {
        ...
        chunks.sort((a, b) => {
            const aIdx = (a.chunk_index ?? a.chunkIndex) ?? 0;
            const bIdx = (b.chunk_index ?? b.chunkIndex) ?? 0;
            return aIdx - bIdx;
        });
    ...
    const reassembledRows = await Promise.all(chunkGroups.map((group) => reassembleParentRow(group, stats)));
    ...
    const merged = sortDeterministicRows(Array.from(mergedMap.values()));
 ...
 */
function electBestChunk(chunks) {
    return chunks.reduce((best, current) => (compareDeterministicRows({ ...current, score: floorScore(effectiveScore(current)) }, { ...best, score: floorScore(effectiveScore(best)) }) < 0 ? current : best));
}
/**
 ...
 */
async function reassembleParentRow(group, stats) {
    const { parentId, bestChunk, parentScore } = group;
    try {
        const db = requireDb();
        // Query the parent memory row for content and metadata
        const parentRow = db
            .prepare(`SELECT id, file_path, content_text, importance_tier, importance_weight,
        ...
        const reassembledContent = parentRow.content_text ?? bestChunk.content;
        // Merge parent metadata onto the best-chunk base row
        const reassembled = {
            ...bestChunk,
            // Override identity fields with parent values
            id: parentId,
            file_path: parentRow.file_path ?? bestChunk.file_path,
            importance_tier: parentRow.importance_tier ?? bestChunk.importance_tier,
            importance_weight: parentRow.importance_weight ?? bestChunk.importance_weight,
            quality_score: parentRow.quality_score ?? bestChunk.quality_score,
            created_at: parentRow.created_at ?? bestChunk.created_at,
            context_type: parentRow.context_type ?? bestChunk.context_type,
        ...
        console.warn(`[stage3-rerank] MPAB DB reassembly failed for parent ${parentId}: ${toErrorMessage(err)} — using chunk fallback`);
        stats.fallback++;
        return markFallback(bestChunk, parentScore);
    }
}
 ...
 */
function markFallback(chunk, parentScore) {
    const parentId = chunk.parent_id ??
        chunk.parentId ??
        chunk.parentMemoryId ??
        chunk.id;
    return {
        ...chunk,
        id: parentId,
...
//# sourceMappingURL=stage3-rerank.js.map