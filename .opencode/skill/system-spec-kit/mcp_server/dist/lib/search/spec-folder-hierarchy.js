 ...
 */
export function invalidateHierarchyCache(database) {
 ...
 */
export function buildHierarchyTree(database) {
  ...
  `).all();
    ...
    const tree = { roots, nodeMap };
    // Store in cache with TTL
    hierarchyCache.set(database, { tree, expiresAt: Date.now() + HIERARCHY_CACHE_TTL_MS });
    return tree;
}
function ensureNodeExists(nodeMap, folderPath, memoryCount) {
 ...
 */
export function getAncestorPaths(folderPath) {
    const ancestors = [];
 ...
 */
export function getSiblingPaths(folderPath, tree) {
    const parentPath = getParentPath(folderPath);
    if (!parentPath)
        return [];
 ...
 */
export function getDescendantPaths(folderPath, tree) {
 ...
 */
export function getRelatedFolders(folderPath, tree) {
    const related = [folderPath];
 ...
 */
export function queryHierarchyMemories(database, specFolder, limit = 50) {
    ...
    LIMIT ?
  `).all(...relatedFolders, limit);
    return rows.map(row => ({
        ...row,
        relevance: relevanceMap.get(row.spec_folder) ?? 0.3,
    })).sort((a, b) => b.relevance - a.relevance);
}
//# sourceMappingURL=spec-folder-hierarchy.js.map