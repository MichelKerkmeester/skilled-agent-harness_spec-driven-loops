...
import { clear_search_cache, } from './vector-index-aliases.js';
import { get_error_message, to_embedding_buffer, VectorIndexError, VectorIndexErrorCode, } from './vector-index-types.js';
import { clear_constitutional_cache, get_embedding_dim, initialize_db, init_prepared_statements, refresh_interference_scores_for_folder, sqlite_vec_available as get_sqlite_vec_available, } from './vector-index-store.js';
const logger = createLogger('VectorIndex');
function isExpectedMissingVecMemoriesTable(error) {
...
function deleteAncillaryMemoryRows(database, id) {
...
function upsert_active_projection(database, specFolder, canonicalFilePath, anchorId, memoryId, updatedAt) {
    ...
    database.prepare('DELETE FROM active_memory_projection WHERE active_memory_id = ? AND logical_key != ?').run(memoryId, logicalKey);
    database.prepare(`
 ...
 * ```
 */
export function index_memory(params, database = initialize_db()) {
    const { specFolder, filePath, parentId = null, anchorId = null, title = null, triggerPhrases = [], importanceWeight = 0.5, embedding, encodingIntent, documentType = 'memory', specLevel = null, contentText = null, qualityScore = 0, qualityFlags = [], appendOnly = false, } = params;
    if (!embedding) {
    ...
    if (embedding.length !== expected_dim) {
        ...
        throw new VectorIndexError(`Embedding must be ${expected_dim} dimensions, got ${embedding.length}`, VectorIndexErrorCode.EMBEDDING_VALIDATION);
    }
    ...
    `).run(specFolder, filePath, canonicalFilePath, anchorId, title, triggers_json, importanceWeight, now, now, embeddingsProvider.getModelName(), now, embedding_status, encodingIntent ?? 'document', documentType, specLevel, contentText, qualityScore, JSON.stringify(qualityFlags), parentId);
 ...
 * @returns The indexed memory identifier.
 */
export function index_memory_deferred(params, database = initialize_db()) {
    const { specFolder, filePath, parentId = null, anchorId = null, title = null, triggerPhrases = [], importanceWeight = 0.5, failureReason = null, encodingIntent, documentType = 'memory', specLevel = null, contentText = null, qualityScore = 0, qualityFlags = [], appendOnly = false, } = params;
    const now = new Date().toISOString();
    const triggers_json = JSON.stringify(triggerPhrases);
    const canonicalFilePath = getCanonicalPathKey(filePath);
    const stmts = init_prepared_statements(database);
    ...
    `).run(specFolder, filePath, canonicalFilePath, anchorId, title, triggers_json, importanceWeight, now, now, failureReason, encodingIntent ?? 'document', documentType, specLevel, contentText, qualityScore, JSON.stringify(qualityFlags), parentId);
        const row_id = BigInt(result.lastInsertRowid);
 ...
 * ```
 */
export function update_memory(params, database = initialize_db()) {
    const { id, title, triggerPhrases, importanceWeight, importanceTier, embedding, canonicalFilePath, encodingIntent, documentType, specLevel, contentText, qualityScore, qualityFlags, } = params;
    const now = new Date().toISOString();
 ...
 * ```
 */
export function delete_memory(id, database = initialize_db()) {
 ...
 * ```
 */
export function delete_memory_from_database(database, id) {
    const sqlite_vec = get_sqlite_vec_available();
        ...
        if (sqlite_vec) {
            try {
                ...
                if (!isExpectedMissingVecMemoriesTable(e)) {
                    throw new VectorIndexError(`Vector deletion failed for memory ${id}: ${get_error_message(e)}`, VectorIndexErrorCode.MUTATION_FAILED);
                }
            }
        }
 ...
 * ```
 */
export function delete_memory_by_path(spec_folder, file_path, anchor_id = null, database = initialize_db()) {
    ...
    ORDER BY id DESC
    LIMIT 1
  `).get(spec_folder, canonicalPath, file_path, anchor_id, anchor_id);
    if (row) {
 ...
 * ```
 */
export function delete_memories(memory_ids, database = initialize_db()) {
    ...
    let failed = 0;
    const deletedIds = [];
    const specFolderById = new Map();
    for (const id of memory_ids) {
        if (!Number.isInteger(id) || id <= 0) {
            continue;
        }
        try {
            const row = database.prepare('SELECT spec_folder FROM memory_index WHERE id = ?').get(id);
    ...
    const delete_transaction = database.transaction(() => {
        ...
        if (failed_ids.length > 0) {
            throw new VectorIndexError(`Failed to delete memories: ${failed_ids.join(', ')}. Transaction rolled back.`, VectorIndexErrorCode.MUTATION_FAILED);
        }
        return {
 ...
 */
export function update_embedding_status(id, status, database = initialize_db()) {
 ...
 */
export function update_confidence(memory_id, confidence, database = initialize_db()) {
...
//# sourceMappingURL=vector-index-mutations.js.map