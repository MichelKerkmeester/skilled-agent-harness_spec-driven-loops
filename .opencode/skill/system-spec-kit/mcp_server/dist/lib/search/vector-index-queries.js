...
import { get_error_message, parse_trigger_phrases, to_embedding_buffer, VectorIndexError, VectorIndexErrorCode, } from './vector-index-types.js';
import { initialize_db, get_embedding_dim, get_constitutional_memories, init_prepared_statements, validate_file_path_local, safe_read_file_async, safe_parse_json, search_weights, sqlite_vec_available as get_sqlite_vec_available, } from './vector-index-store.js';
 ...
 * ```
 */
export function get_memory(id, database = initialize_db()) {
 ...
 */
export function get_memories_by_folder(spec_folder, database = initialize_db()) {
 ...
 */
export function get_memory_count(database = initialize_db()) {
    const stmts = init_prepared_statements(database);
 ...
 */
export function get_status_counts(database = initialize_db()) {
    const rows = database.prepare(`
 ...
 */
export function get_stats(database = initialize_db()) {
 ...
 * ```
 */
export function vector_search(query_embedding, options = {}, database = initialize_db()) {
    const sqlite_vec = get_sqlite_vec_available();
    if (!sqlite_vec) {
        console.warn('[vector-index] Vector search unavailable - sqlite-vec not loaded');
        return [];
    }
    const { limit = 10, specFolder = null, minSimilarity = 0, useDecay = true, tier = null, contextType = null, includeConstitutional = true, includeArchived = false } = options;
    ...
    if (!query_embedding || query_embedding.length !== expected_dim) {
        throw new VectorIndexError(`Invalid embedding dimension: expected ${expected_dim}, got ${query_embedding?.length}`, VectorIndexErrorCode.EMBEDDING_VALIDATION);
    }
 ...
 */
export function get_constitutional_memories_public(options = {}, database = initialize_db()) {
 ...
 * ```
 */
export function multi_concept_search(concept_embeddings, options = {}) {
    const sqlite_vec = get_sqlite_vec_available();
    if (!sqlite_vec) {
    ...
    for (const emb of concepts) {
        if (!emb || emb.length !== expected_dim) {
            throw new VectorIndexError(`Invalid embedding dimension: expected ${expected_dim}, got ${emb?.length}`, VectorIndexErrorCode.QUERY_FAILED);
        }
    }
    ...
    const max_distance = 2 * (1 - minSimilarity / 100);
    const distance_expressions = concept_buffers.map((_, i) => `vec_distance_cosine(v.embedding, ?) as dist_${i}`).join(', ');
    const distance_filters = concept_buffers.map((_, _i) => `vec_distance_cosine(v.embedding, ?) <= ?`).join(' AND ');
    ...
    const similarity_select = concept_buffers.map((_, i) => `ROUND((1 - sub.dist_${i} / 2) * 100, 2) as similarity_${i}`).join(', ');
 ...
 */
export function extract_title(content, filename) {
 ...
 */
export function extract_snippet(content, max_length = 200) {
    if (!content || typeof content !== 'string') {
        return '';
    }
 ...
 */
export function extract_tags(content) {
    if (!content || typeof content !== 'string') {
 ...
 */
export function extract_date(content, file_path) {
    if (content && typeof content === 'string') {
 ...
 */
export async function generate_query_embedding(query) {
 ...
 * @returns The matching memory rows.
 */
export function keyword_search(query, options = {}, database = initialize_db()) {
  ...
  `;
    ...
    const filtered = scored
        ...
        .sort((a, b) => Number(b.keyword_score ?? 0) - Number(a.keyword_score ?? 0))
        .slice(0, limit);
 ...
 * ```
 */
export async function vector_search_enriched(query, limit = 20, options = {}, database = initialize_db()) {
    ...
    const file_contents = await Promise.all(raw_results.map((row) => safe_read_file_async(row.file_path)));
 ...
 * ```
 */
export async function multi_concept_search_enriched(concepts, limit = 20, options = {}) {
    const start_time = Date.now();
    if (!Array.isArray(concepts) || concepts.length < 2 || concepts.length > 5) {
        throw new VectorIndexError('Multi-concept search requires 2-5 concepts', VectorIndexErrorCode.QUERY_FAILED);
    }
 ...
 * @returns A promise that resolves to enriched search results.
 */
export async function multi_concept_keyword_search(concepts, limit = 20, options = {}) {
    const { specFolder = null } = options;
    if (!concepts.length) {
        console.warn('[vector-index] multi_concept_keyword_search: empty concepts array');
        return [];
    }
    const concept_results = concepts.map((concept) => keyword_search(concept, { limit: 100, specFolder }));
    ...
    const rows = limited_ids.map(id => id_to_row.get(id)).filter((row) => Boolean(row));
    const file_contents = await Promise.all(rows.map(row => safe_read_file_async(row.file_path)));
 ...
 */
export function parse_quoted_terms(query) {
    if (!query || typeof query !== 'string') {
 ...
 */
export function apply_smart_ranking(results) {
 ...
 */
export function apply_diversity(results, diversity_factor = 0.3) {
 ...
 */
export function get_related_memories(memory_id) {
    try {
        const database = initialize_db();
    ...
    `).get(memory_id);
        ...
        return related.map((rel) => {
            ...
            return null;
        }).filter((relatedMemory) => Boolean(relatedMemory));
    }
    catch (error) {
 ...
 */
export function get_usage_stats(options = {}) {
    const { sortBy = 'access_count', order = 'DESC', limit = 20 } = options;
 ...
 */
export function find_cleanup_candidates(options = {}) {
    const database = initialize_db();
    const { maxAgeDays = 90, maxAccessCount = 2, maxConfidence = 0.4, limit = 50 } = options;
  ...
  `;
    let rows;
    try {
        rows = database.prepare(sql).all(cutoff_iso, maxAccessCount, maxConfidence, cutoff_iso, limit);
    }
    catch (e) {
    ...
    return rows.map((row) => {
        ...
        const last_access_string = format_age_string(typeof row.last_accessed === 'number' ? new Date(row.last_accessed).toISOString() : null);
        const reasons = [];
 ...
 */
export function get_memory_preview(memory_id, max_lines = 50) {
    const database = initialize_db();
    let memory;
    try {
    ...
    return {
        ...
        lastAccessString: format_age_string(typeof memory.last_accessed === 'number' ? new Date(memory.last_accessed).toISOString() : null),
        content
    };
}
/* ───────────────────────────────────────────────────────────────
 ...
 */
export function verify_integrity(options = {}, database = initialize_db()) {
    ...
    const total_vectors = sqlite_vec
        ? database.prepare('SELECT COUNT(*) as count FROM vec_memories').get().count
        : 0;
...
//# sourceMappingURL=vector-index-queries.js.map