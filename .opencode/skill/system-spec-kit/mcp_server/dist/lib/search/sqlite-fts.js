 ...
 * ```
 */
function fts5Bm25Search(db, query, options = {}) {
    ...
    const archivalFilter = !includeArchived
        ...
        : '';
    const deprecatedTierFilter = "AND (m.importance_tier IS NULL OR m.importance_tier != 'deprecated')";
...
function isFts5Available(db) {
    try {
        const result = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='memory_fts'`).get();
        return !!result;
    }
    catch (error) {
...
//# sourceMappingURL=sqlite-fts.js.map