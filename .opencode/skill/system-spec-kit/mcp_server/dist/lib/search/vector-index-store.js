...
import { get_error_code, get_error_message, parse_trigger_phrases, VectorIndexError, VectorIndexErrorCode, } from './vector-index-types.js';
import { create_schema, ensure_schema_version, } from './vector-index-schema.js';
function loadSearchWeights() {
...
let _cached_queries = null;
let _cached_mutations = null;
let _cached_aliases = null;
...
function get_existing_embedding_dimension(database) {
...
function validate_embedding_dimension_for_connection(database, _sqlite_vec_available) {
    if (!database) {
 ...
 */
export function validate_file_path_local(file_path) {
 ...
 */
export async function safe_read_file_async(file_path) {
 ...
 */
export function safe_parse_json(json_string, default_value = []) {
...
const database_connection_listeners = new Set();
function set_active_database_connection(connection, target_path, vec_available) {
    const previousDb = db;
...
export function sqlite_vec_available() {
    return sqlite_vec_available_flag;
}
export function on_database_connection_change(listener) {
    database_connection_listeners.add(listener);
    return () => {
        database_connection_listeners.delete(listener);
    };
}
const constitutional_cache = new Map();
...
const constitutional_cache_loading = new Map();
let last_db_mod_time = 0;
let last_constitutional_cache_db_path = null;
function get_constitutional_cache_db_scope() {
    if (db_path === ':memory:') {
        return db_path;
    }
    return path.resolve(db_path);
}
function build_constitutional_cache_key(spec_folder, includeArchived) {
    const db_scope = get_constitutional_cache_db_scope();
    return `${db_scope}::${spec_folder || 'global'}:${includeArchived ? 'arch' : 'noarch'}`;
}
function refresh_constitutional_cache_db_state() {
...
function is_constitutional_cache_valid() {
    if (constitutional_cache.size === 0)
        return false;
    try {
        const current_db_path = get_constitutional_cache_db_scope();
        if (last_constitutional_cache_db_path &&
            last_constitutional_cache_db_path !== current_db_path) {
            return false;
        }
        if (current_db_path !== ':memory:' && fs.existsSync(current_db_path)) {
...
const prepared_statements_cache = new WeakMap();
/**
 ...
 */
export function init_prepared_statements(database) {
 ...
 * @returns Nothing.
 */
export function clear_prepared_statements(database) {
    if (database) {
 ...
 */
export function get_constitutional_memories(database, spec_folder = null, includeArchived = false) {
 ...
 * @returns Nothing.
 */
export function clear_constitutional_cache(spec_folder = null) {
    if (spec_folder) {
 ...
 * @returns Nothing.
 */
export function refresh_interference_scores_for_folder(database, specFolder) {
    if (!specFolder)
        return;
    try {
        const rows = database.prepare(`SELECT m.id
       FROM memory_index m
       JOIN active_memory_projection p ON p.active_memory_id = m.id
       WHERE m.spec_folder = ?
         AND m.parent_id IS NULL
         AND COALESCE(m.is_archived, 0) = 0
         AND COALESCE(m.importance_tier, 'normal') != 'deprecated'`).all(specFolder);
        if (rows.length === 0)
            return;
 ...
 * ```
 */
export function initialize_db(custom_path = null) {
    if (db && !custom_path) {
        return db;
    }
...
export class SQLiteVectorStore extends IVectorStore {
    ...
    async search(embedding, topK, options = {}) {
        ...
        if (!embedding || embedding.length !== expected_dim) {
            throw new VectorIndexError(`Invalid embedding dimension: expected ${expected_dim}, got ${embedding?.length}`, VectorIndexErrorCode.EMBEDDING_VALIDATION);
        }
     ...
     * ```
     */
    async upsert(_id, embedding, metadata) {
        this._ensureInitialized();
        const database = this._getDatabase();
        const expected_dim = get_embedding_dim();
        if (!embedding || embedding.length !== expected_dim) {
            throw new VectorIndexError(`Embedding dimension mismatch: expected ${expected_dim}, got ${embedding?.length}`, VectorIndexErrorCode.EMBEDDING_VALIDATION);
        }
        ...
        if (!params.specFolder || !params.filePath) {
            throw new VectorIndexError('metadata must include spec_folder and file_path', VectorIndexErrorCode.STORE_ERROR);
        }
    ...
    async getConstitutionalMemories(options = {}) {
        this._ensureInitialized();
        const database = this._getDatabase();
...
//# sourceMappingURL=vector-index-store.js.map