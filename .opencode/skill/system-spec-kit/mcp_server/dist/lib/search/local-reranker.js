...
async function ensureModelLoaded(modelPath) {
    ...
    modelLoadPromise = (async () => {
        ...
        const loadModel = cachedLlama.loadModel;
...
async function scorePrompt(context, prompt) {
    if (!context || typeof context !== 'object') {
        throw new Error('Invalid llama context');
    }
    const directFns = ['score', 'getScore', 'evaluate', 'eval'];
    for (const fnName of directFns) {
        const fn = context[fnName];
        if (typeof fn === 'function') {
            const raw = await fn.call(context, prompt);
    ...
    if (typeof getSequence === 'function') {
        ...
        if (sequence && typeof sequence === 'object') {
            const seqEval = sequence.evaluate;
 ...
 */
export async function canUseLocalReranker() {
 ...
 */
export async function rerankLocal(query, candidates, topK) {
    ...
    try {
        ...
        const reranked = await Promise.race([
            (async () => {
                ...
                console.error(`[local-reranker] reranked=${rerankCount} total=${candidates.length} durationMs=${elapsed} model=${path.basename(modelPath)}`);
...
export async function disposeLocalReranker() {
...
//# sourceMappingURL=local-reranker.js.map