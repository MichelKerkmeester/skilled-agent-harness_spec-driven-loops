 ...
 */
function extractTermsFromContents(contents, queryTokens, maxTerms) {
    const freq = new Map();
    for (const text of contents) {
        ...
        for (const token of tokens) {
            if (token.length < MIN_TERM_LENGTH)
                continue;
            if (EXPANSION_STOP_WORDS.has(token))
                continue;
    ...
    return Array.from(freq.entries())
        .sort(([aKey, aCount], [bKey, bCount]) => bCount !== aCount ? bCount - aCount : aKey.localeCompare(bKey))
        .slice(0, maxTerms)
        .map(([term]) => term);
}
// ───────────────────────────────────────────────────────────────
 ...
 */
export async function expandQueryWithEmbeddings(query, embedding, options) {
    ...
    try {
        ...
        for (const mem of similarMemories) {
            ...
            const contentField = (mem.content_text ?? mem.content);
        ...
        const queryTokens = new Set((query.toLowerCase().match(/\b[a-z][a-z0-9_-]*\b/g) ?? []).filter(t => t.length >= MIN_TERM_LENGTH && !EXPANSION_STOP_WORDS.has(t)));
 ...
 */
export function isExpansionActive(query) {
    if (!isEmbeddingExpansionEnabled())
        return false;
    const result = classifyQueryComplexity(query);
    return result.tier !== 'simple';
}
//# sourceMappingURL=embedding-expansion.js.map