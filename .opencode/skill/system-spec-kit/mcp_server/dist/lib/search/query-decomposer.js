...
function questionWordGlobalRegex() {
 ...
 */
export function isMultiFacet(query) {
    if (typeof query !== 'string')
        return false;
    const trimmed = query.trim();
    if (trimmed.length < MIN_QUERY_LENGTH_FOR_DECOMPOSITION)
        return false;
 ...
 */
function splitOnConjunctions(query) {
    return query
 ...
 */
function splitOnSentences(query) {
    return query
        .split(/[.?;]\s+/)
        .map(normalizeFragment)
        .filter((f) => f.split(/\s+/).length >= MIN_FRAGMENT_TOKEN_COUNT);
 ...
 */
export function mergeByFacetCoverage(pools) {
    if (pools.length === 0)
        return [];
