 ...
 */
export function generateSurrogateQuestions(title, headings, content) {
    ...
    for (const heading of headings) {
        ...
        if (questions.size >= MAX_SURROGATE_QUESTIONS)
            break;
    }
 ...
 */
export function generateSurrogates(content, title) {
    if (!isQuerySurrogatesEnabled())
        return null;
 ...
 */
function keywordOverlap(queryTokens, target) {
 ...
 */
export function matchSurrogates(query, surrogates) {
    ...
    const score = Math.min(1.0, aliasScore * 0.3 +
        questionScore * 0.4 +
        summaryScore * 0.2 +
        headingScore * 0.1);
    return {
...
//# sourceMappingURL=query-surrogates.js.map