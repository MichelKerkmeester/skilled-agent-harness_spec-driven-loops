 ...
 */
const CONDITION_KEYWORDS = ['when', 'if', 'for', 'during', 'before', 'after', 'on'];
 ...
 */
function extractCandidateLines(content) {
    ...
    for (const raw of lines) {
        ...
        const hasImperative = IMPERATIVE_KEYWORDS.some((kw) => lower.startsWith(kw + ' ') || lower.includes(' ' + kw + ' '));
        if (hasImperative) {
 ...
 */
export function extractRetrievalDirective(content, title) {
    if (!content && !title)
        return null;
 ...
 */
export function formatDirectiveMetadata(directive) {
 ...
 */
export function enrichWithRetrievalDirectives(results) {
...
//# sourceMappingURL=retrieval-directives.js.map