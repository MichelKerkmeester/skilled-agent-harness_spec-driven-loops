...
function deduplicateResults(results, sessionId) {
    const defaultMeta = { deduplicated: false, seenCount: 0, deprioritizedCount: 0 };
 ...
 */
function computeGoalAlignment(goal, content) {
    if (!goal || !content)
        return 0;
    const goalWords = new Set(goal
        .toLowerCase()
        .split(/\s+/)
        .filter((w) => w.length > 2));
    if (goalWords.size === 0)
 ...
 */
function refineForGoal(results, sessionId) {
    const defaultMeta = { refined: false, activeGoal: null, boostedCount: 0 };
    if (!Array.isArray(results) || results.length === 0) {
        return { results, metadata: defaultMeta };
    }
    if (!isSessionRetrievalStateEnabled()) {
...
export { 
// Constants
SESSION_TTL_MS, MAX_SESSIONS, SEEN_DEDUP_FACTOR, GOAL_BOOST_MAX, 
// Feature flag
...
deduplicateResults, 
// Goal-aware refinement
refineForGoal, computeGoalAlignment, };
