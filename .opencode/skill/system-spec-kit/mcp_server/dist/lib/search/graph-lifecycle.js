...
import { extractHeadings, extractAliases, extractRelationPhrases, extractCodeFenceTechnologies, createTypedEdges as _createTypedEdgesWithCallback, EXPLICIT_ONLY_EVIDENCE } from './deterministic-extractor.js';
 ...
 */
export function resolveGraphRefreshMode() {
...
export function isGraphRefreshEnabled() {
...
import { isGraphRefreshDisabled, isLlmGraphBackfillEnabled, } from './search-flags.js';
 ...
 */
export function markDirty(nodeIds) {
    if (!isGraphRefreshEnabled())
        return 0;
...
export function clearDirtyNodes() {
    _dirtyNodes = { nodeIds: new Set(), markedAt: 0 };
}
// ───────────────────────────────────────────────────────────────
 ...
 */
function resolveLocalThreshold() {
    const raw = process.env.SPECKIT_GRAPH_LOCAL_THRESHOLD;
    if (!raw)
        return DEFAULT_LOCAL_RECOMPUTE_THRESHOLD;
 ...
 */
export function estimateComponentSize(db, nodeIds) {
    if (nodeIds.length === 0)
        return 0;
    ...
    `);
        while (queue.length > 0 && edgesScanned < LOCAL_RECOMPUTE_EDGE_LIMIT) {
            ...
            const rows = stmt.all(current, current, LOCAL_RECOMPUTE_EDGE_LIMIT - edgesScanned);
 ...
 */
export function recomputeLocal(db, nodeIds) {
    if (nodeIds.length === 0)
        return 0;
    try {
 ...
 */
let _scheduledRefreshTimer = null;
 ...
 */
export function scheduleGlobalRefresh(delayMs = DEFAULT_SCHEDULE_DELAY_MS) {
 ...
 */
export function registerGlobalRefreshFn(fn) {
    _globalRefreshFn = fn;
}
// ───────────────────────────────────────────────────────────────
 ...
 */
export function onWrite(db, payload) {
 ...
 */
export function onIndex(db, memoryId, content, options) {
    const skippedResult = {
        edgesCreated: 0,
 ...
 */
function _scheduleLlmBackfill(memoryId) {
    if (!_llmBackfillFn)
        return false;
...
//# sourceMappingURL=graph-lifecycle.js.map