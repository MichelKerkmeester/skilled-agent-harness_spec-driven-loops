...
import { isMMREnabled, isCrossEncoderEnabled, isLocalRerankerEnabled, isSearchFallbackEnabled, isDocscoreAggregationEnabled, isDegreeBoostEnabled, isContextHeadersEnabled, } from './search-flags.js';
...
import { truncateByConfidence, isConfidenceTruncationEnabled, DEFAULT_MIN_RESULTS, GAP_THRESHOLD_MULTIPLIER, } from './confidence-truncation.js';
import { getDynamicTokenBudget, isDynamicTokenBudgetEnabled, DEFAULT_TOKEN_BUDGET_CONFIG, } from './dynamic-token-budget.js';
import { ensureDescriptionCache, getSpecsBasePaths } from './folder-discovery.js';
import { isFolderScoringEnabled, lookupFolders, computeFolderRelevanceScores, enrichResultsWithFolderScores, twoPhaseRetrieval, } from './folder-relevance.js';
...
function toHybridResult(result) {
    const sourceCandidate = result.source;
    const primarySource = result.sources[0] ?? 'hybrid';
    const scoreCandidate = result.score;
    return {
        ...result,
        id: result.id,
...
const CONTEXT_HEADER_TOKEN_OVERHEAD = Math.ceil((CONTEXT_HEADER_MAX_CHARS + CONTEXT_HEADER_SEPARATOR_CHARS) / 4);
let db = null;
let vectorSearchFn = null;
let graphSearchFn = null;
const graphMetrics = {
    totalQueries: 0,
    graphHits: 0,
    graphOnlyResults: 0,
 ...
 */
function init(database, vectorFn = null, graphFn = null) {
    db = database;
 ...
 */
function bm25Search(query, options = {}) {
    if (!isBm25Enabled()) {
    ...
    try {
        ...
        if (specFolder) {
            ...
            try {
                ...
                const rows = db.prepare(`SELECT id, spec_folder FROM memory_index WHERE id IN (${placeholders})`).all(...ids);
 ...
 */
function combinedLexicalSearch(query, options = {}) {
...
function resolveRawCandidateScore(result) {
...
function getCandidateSources(result) {
    const sourceList = result.sources;
    if (Array.isArray(sourceList)) {
        return sourceList.filter((value) => typeof value === 'string');
    }
    return typeof result.source === 'string' && result.source.length > 0
        ? [result.source]
        : [];
}
function getCandidateSourceScores(result) {
...
function mergeRawCandidate(existing, incoming) {
    if (!existing) {
...
function collectCandidatesFromLists(lists, limit) {
    ...
    return applyResultLimit(Array.from(deduped.values()).sort((a, b) => {
...
function mergeRawCandidateSets(existing, incoming, limit) {
    const merged = new Map();
    ...
    return applyResultLimit(Array.from(merged.values()).sort((a, b) => {
        const aScore = typeof a.score === 'number' && Number.isFinite(a.score) ? a.score : 0;
        const bScore = typeof b.score === 'number' && Number.isFinite(b.score) ? b.score : 0;
        return bScore - aScore;
    }), limit);
}
function getAllowedChannels(options) {
    const allowed = new Set(['vector', 'fts', 'graph', 'degree']);
    if (isBm25Enabled()) {
        allowed.add('bm25');
    }
    if (options.useVector === false)
        allowed.delete('vector');
    if (options.useBm25 === false)
        allowed.delete('bm25');
    if (options.useFts === false)
        allowed.delete('fts');
...
function applyAllowedChannelOverrides(options, allowedChannels, overrides = {}) {
    return {
        ...options,
        ...overrides,
...
function markFallbackRetry(results) {
    for (const result of results) {
        result.fallbackRetry = true;
    }
    return results;
}
function toEmbeddingBufferView(value) {
...
async function executeFallbackPlan(query, embedding, options = {}, planKind, overrides = {}) {
 ...
 */
async function hybridSearch(query, embedding, options = {}) {
    const { limit = DEFAULT_LIMIT, specFolder, minSimilarity = 0, useBm25 = isBm25Enabled(), useFts = true, useVector = true, useGraph = true, includeArchived = false, } = options;
    const results = [];
    // Vector search
 ...
 */
async function hybridSearchEnhanced(query, embedding, options = {}) {
...
async function collectAndFuseHybridResults(query, embedding, options = {}) {
    try {
        ...
        const allPossibleChannels = ['vector', 'fts', 'bm25', 'graph', 'degree'];
        const activeChannels = options.forceAllChannels
            ? new Set(allPossibleChannels)
            : new Set(routeResult.channels);
        ...
        if (skippedChannels.length > 0) {
            s3meta.routing = {
                ...
                features: routeResult.classification.features,
            };
        }
        ...
        if (budgetResult.applied && !evaluationMode) {
            s3meta.tokenBudget = {
                ...
                configValues: DEFAULT_TOKEN_BUDGET_CONFIG,
        ...
        let semanticResults = [];
        let ftsChannelResults = [];
        let bm25ChannelResults = [];
        const vectorEmbeddingCache = new Map();
            ...
            try {
                ...
                for (const result of semanticResults) {
                    if (typeof result.id !== 'number')
                        continue;
                    const embeddingCandidate = toEmbeddingBufferView(result.embedding
                        ?? result.embeddingBuffer);
        ...
        if (options.skipFusion) {
            return {
                ...
                fusedResults: collectCandidatesFromLists(lists.filter((list) => list.source !== 'degree'), options.limit ?? DEFAULT_LIMIT),
            };
        }
        if (lists.length === 0) {
            return null;
        ...
        for (const [, sources] of sourceMap) {
            if (sources.size > 1)
                graphMetrics.multiSourceResults++;
            if (sources.size === 1 && sources.has('graph'))
                graphMetrics.graphOnlyResults++;
        }
        ...
        const fusedResults = fused.map(toHybridResult).map((row) => {
            const rowRecord = row;
            if (rowRecord.parentMemoryId !== undefined)
                return row;
            const normalizedParentMemoryId = rowRecord.parent_id ?? rowRecord.parentId;
            if (normalizedParentMemoryId === undefined)
                return row;
            return {
                ...row,
...
async function enrichFusedResults(query, execution, options = {}, initialResults = execution.fusedResults) {
    const { evaluationMode, intent, lists, routeResult, budgetResult, s3meta, vectorEmbeddingCache, } = execution;
    ...
    if (isDocscoreAggregationEnabled()) {
        try {
            const chunkResults = fusedHybridResults.filter(r => r.parentMemoryId != null && r.chunkIndex != null);
            if (chunkResults.length > 0) {
                const nonChunkResults = fusedHybridResults.filter(r => r.parentMemoryId == null || r.chunkIndex == null);
                const collapsed = collapseAndReassembleChunkResults(chunkResults.map(r => ({
                    id: r.id,
    ...
    try {
        const channelResultSets = new Map();
        for (const list of lists) {
        ...
        const enforcementResult = enforceChannelRepresentation(fusedHybridResults.map(r => ({ ...r, source: r.source || 'hybrid' })), channelResultSets, limit);
        if (enforcementResult.enforcement.applied) {
            fusedHybridResults = enforcementResult.results;
            s3meta.enforcement = {
                applied: true,
    ...
    const shadowMeta = initialResults;
    ...
    let reranked = fusedHybridResults.slice(0, limit);
    ...
    if (db && isMMREnabled()) {
        ...
        if (numericIds.length >= MMR_MIN_CANDIDATES) {
            try {
                const embeddingMap = new Map(vectorEmbeddingCache);
                const missingIds = numericIds.filter((id) => !embeddingMap.has(id));
                if (missingIds.length > 0) {
                    const placeholders = missingIds.map(() => '?').join(', ');
                    const embRows = db.prepare(`SELECT rowid, embedding FROM vec_memories WHERE rowid IN (${placeholders})`).all(...missingIds);
                    for (const row of embRows) {
                        if (Buffer.isBuffer(row.embedding)) {
                            embeddingMap.set(row.rowid, new Float32Array(row.embedding.buffer, row.embedding.byteOffset, row.embedding.byteLength / 4));
                        }
                    }
                }
                const mmrCandidates = [];
                ...
                if (mmrCandidates.length >= MMR_MIN_CANDIDATES) {
                    ...
                    const nonEmbeddedRows = reranked.filter(r => !embeddedIdSet.has(r.id));
                    const rerankedById = new Map(reranked.map((result) => [canonicalResultId(result.id), result]));
                    const diversifiedRows = diversified.map((candidate) => {
    ...
    if (topIds.length > 0) {
        try {
            const spreadResults = spreadActivation(topIds);
            ...
            if (spreadResults.length > 0) {
                ...
                for (const result of reranked) {
                    const boost = spreadMap.get(result.id);
                    if (boost !== undefined) {
                        // M10 FIX: Update all score aliases so downstream consumers see the boost
                        const boostedScore = (result.score ?? 0) + boost * CO_ACTIVATION_CONFIG.boostFactor;
                        result.score = boostedScore;
                        if ('rrfScore' in result)
                            result.rrfScore = boostedScore;
                        if ('intentAdjustedScore' in result)
                            result.intentAdjustedScore = boostedScore;
                    }
                }
            }
        ...
        try {
            ...
            if (numericIds.length > 0) {
                ...
                if (folderMap.size > 0) {
                    ...
                    reranked = enrichResultsWithFolderScores(postFolderResults, folderScores, folderMap);
                }
            }
        }
        catch (_folderErr) {
    ...
    if (!evaluationMode) {
        try {
            const truncationResult = truncateByConfidence(reranked.map(r => ({ ...r, id: r.id, score: r.score })));
            if (truncationResult.truncated) {
                reranked = truncationResult.results.map(r => r);
                s3meta.truncation = {
                    truncated: true,
    ...
    if (reranked.length > 0) {
        reranked = reranked.map((row) => {
            const existingTraceMetadata = typeof row.traceMetadata === 'object' && row.traceMetadata !== null && !Array.isArray(row.traceMetadata)
                ? row.traceMetadata
                : {};
            return {
                ...row,
 ...
 */
async function collectRawCandidates(query, embedding, options = {}) {
    const { allowedChannels, stages } = await executeFallbackPlan(query, embedding, options, isSearchFallbackEnabled() ? 'tiered' : 'adaptive', { stopAfterFusion: true });
    ...
    if (isSearchFallbackEnabled()) {
        ...
        if (mergedResults.length > 0)
            return applyResultLimit(mergedResults, options.limit);
    }
    else {
        const stagedResults = retryResults.length > 0 ? retryResults : primaryResults;
        if (stagedResults.length > 0)
            return applyResultLimit(stagedResults, options.limit);
    }
    if (allowedChannels.has('fts')) {
        const ftsFallback = collectCandidatesFromLists([{ source: 'fts', results: ftsSearch(query, options) }], options.limit);
        if (ftsFallback.length > 0)
            return ftsFallback;
    }
    if (allowedChannels.has('bm25')) {
        const bm25Fallback = collectCandidatesFromLists([{ source: 'bm25', results: bm25Search(query, options) }], options.limit);
        if (bm25Fallback.length > 0)
            return bm25Fallback;
    }
 ...
 */
async function searchWithFallback(query, embedding, options = {}) {
    ...
    const { allowedChannels, stages } = await executeFallbackPlan(query, embedding, options, 'adaptive');
 ...
 */
function structuralSearch(options = {}) {
    if (!db) {
...
function extractSpecSegments(filePath) {
...
let descMapCache = null;
let descMapRefreshing = false;
const DESC_MAP_TTL_MS = 60_000;
function rebuildDescriptionTailMap() {
    const descByTail = new Map();
...
function buildDescriptionTailMap() {
...
function injectContextualTree(row, descCache) {
    const rowData = row;
 ...
 */
function calibrateTier3Scores(existing, tier3) {
...
function checkDegradation(results) {
    ...
    if (!lowQuality && !insufficientResults)
        return null;
    return {
 ...
 */
async function searchWithFallbackTiered(query, embedding, options = {}) {
...
function estimateStructuredValueChars(value, seen) {
    if (value == null)
        return 4;
 ...
 */
function estimateResultTokens(result) {
    const record = result;
 ...
 */
function createSummaryFallback(result, budget) {
 ...
 */
function truncateToBudget(results, budget, options) {
    ...
    if (sorted.length === 1) {
        ...
        console.warn(`[hybrid-search] Token budget overflow (single-result fallback): ` +
    ...
    if (accepted.length === 0 && sorted.length > 0) {
        ...
        console.warn(`[hybrid-search] Token budget overflow (top-result fallback): ` +
            `${totalTokens} tokens > ${effectiveBudget} budget`);
        return { results: [outputResult], truncated: true, overflow };
    }
    const overflow = {
    ...
    console.warn(`[hybrid-search] Token budget overflow: ${totalTokens} tokens > ${effectiveBudget} budget, ` +
...
export { init, bm25Search, isBm25Available, combinedLexicalSearch, collectRawCandidates, isFtsAvailable, ftsSearch, hybridSearch, hybridSearchEnhanced, searchWithFallback, getGraphMetrics, resetGraphMetrics, 
// Token budget validation
estimateTokenCount, estimateResultTokens, truncateToBudget, getTokenBudget, DEFAULT_TOKEN_BUDGET, SUMMARY_MAX_CHARS, 
