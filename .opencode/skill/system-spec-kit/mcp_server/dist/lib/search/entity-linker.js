 ...
 * ```
 */
export function nounPhrases(query) {
    ...
    for (const token of raw) {
        if (token.length < MIN_NOUN_PHRASE_TOKEN_LENGTH)
            continue;
 ...
 * ```
 */
export function matchAliases(tokens, extraAliases) {
    const combined = extraAliases
        ? { ...BUILTIN_CONCEPT_ALIASES, ...extraAliases }
        : BUILTIN_CONCEPT_ALIASES;
    const matched = new Set();
    for (const token of tokens) {
        if (matched.size >= MAX_CONCEPTS_PER_QUERY)
            break;
 ...
 * ```
 */
export function loadConceptAliasTable(db) {
    try {
        // entity_catalog may not have an alias column — check schema first
        const tableInfo = db.prepare(`PRAGMA table_info(entity_catalog)`).all();
        ...
        if (!hasAlias || !hasCanonical) {
            ...
            if (!altHasEntityText)
                return {};
            const rows = db.prepare(`SELECT entity_text FROM entity_catalog LIMIT 500`).all();
            const result = {};
        ...
        const rows = db.prepare(`SELECT alias_text, canonical_name FROM entity_catalog LIMIT 500`).all();
        const result = {};
        for (const row of rows) {
 ...
 * ```
 */
export function routeQueryConcepts(query, db) {
    const empty = { concepts: [], graphActivated: false };
    try {
 ...
 */
export function getConceptExpansionTerms(concepts, originalTokens, maxTerms = 5) {
 ...
 * ```
 */
export function buildEntityCatalog(db) {
    ...
    `).all();
        for (const row of rows) {
            ...
            if (!entry || !sets) {
                ...
                sets = { memoryIdSet: new Set(), specFolderSet: new Set() };
 ...
 * ```
 */
export function findCrossDocumentMatches(db) {
    const catalog = buildEntityCatalog(db);
    const matches = [];
 ...
 */
function getEdgeCount(db, nodeId) {
    try {
        const row = db.prepare(`SELECT COUNT(*) AS cnt FROM causal_edges WHERE source_id = ? OR target_id = ?`).get(nodeId, nodeId);
        return row?.cnt ?? 0;
    }
    catch (error) {
 ...
 */
function getSpecFolder(db, memoryId) {
    try {
        const row = db.prepare(`SELECT spec_folder FROM memory_index WHERE id = ?`).get(memoryId);
 ...
 */
function sanitizeDensityThreshold(value) {
 ...
 */
function getEntityLinkingDensityThreshold() {
 ...
 * ```
 */
export function computeEdgeDensity(db) {
 ...
 */
function getGlobalEdgeDensityStats(db) {
    try {
        const edgeRow = db.prepare(`SELECT COUNT(*) AS cnt FROM causal_edges`).get();
        const totalEdges = edgeRow?.cnt ?? 0;
        const memoryRow = db.prepare(`SELECT COUNT(*) AS cnt FROM memory_index`).get();
 ...
 */
function batchGetEdgeCounts(db, nodeIds) {
  ...
  `;
    try {
        const rows = db.prepare(sql).all(...nodeIds, ...nodeIds);
        for (const row of rows) {
 ...
 * ```
 */
export function createEntityLinks(db, matches, options) {
    let linksCreated = 0;
    ...
    for (const match of matches) {
        ...
        for (let i = 0; i < memoryIds.length; i++) {
            for (let j = i + 1; j < memoryIds.length; j++) {
                ...
                if (sourceCount >= MAX_EDGES_PER_NODE)
                    continue;
                if (targetCount >= MAX_EDGES_PER_NODE)
                    continue;
 ...
 * ```
 */
export function getEntityLinkStats(db) {
    try {
        // Count total edges created by entity_linker
        const totalRow = db.prepare(`SELECT COUNT(*) AS cnt FROM causal_edges WHERE created_by = 'entity_linker'`).get();
        const totalEntityLinks = totalRow?.cnt ?? 0;
        // Count distinct entity names from evidence field (parse "Cross-doc entity: <name>")
        const evidenceRows = db.prepare(`SELECT DISTINCT evidence FROM causal_edges WHERE created_by = 'entity_linker'`).all();
    ...
    `).get();
        ...
        const totalRow2 = db.prepare(`SELECT COUNT(*) AS cnt FROM memory_index`).get();
 ...
 * ```
 */
export function hasEntityInfrastructure(db) {
    try {
        const row = db.prepare(`SELECT COUNT(*) AS cnt FROM entity_catalog`).get();
        return (row?.cnt ?? 0) > 0;
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
 ...
 * ```
 */
export function runEntityLinking(db) {
    const emptyResult = { linksCreated: 0, entitiesProcessed: 0, crossDocMatches: 0 };
