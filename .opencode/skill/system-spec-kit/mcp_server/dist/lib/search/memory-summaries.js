 ...
 */
export async function generateAndStoreSummary(db, memoryId, content, embeddingFn) {
    ...
    `);
        stmt.run(memoryId, summary, embeddingBlob, keySentencesJson, memoryId, now, now);
        return { stored: true, summary };
    }
    catch (error) {
        console.warn(`[memory-summaries] Failed to generate/store summary for memory ${memoryId}:`, error instanceof Error ? error.message : error);
        return { stored: false, summary: '' };
    }
}
/**
 ...
 */
export function querySummaryEmbeddings(db, queryEmbedding, limit) {
    try {
    ...
    `).all(fetchCap);
        ...
        console.warn('[memory-summaries] Failed to query summary embeddings:', error instanceof Error ? error.message : error);
        return [];
    }
}
/**
 ...
 */
export function checkScaleGate(db) {
    try {
    ...
    `).get();
        ...
        console.warn('[memory-summaries] Failed to check scale gate:', error instanceof Error ? error.message : error);
        return false;
    }
}
// ───────────────────────────────────────────────────────────────
...
//# sourceMappingURL=memory-summaries.js.map