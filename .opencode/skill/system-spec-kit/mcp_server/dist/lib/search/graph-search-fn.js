...
// ───────────────────────────────────────────────────────────────
let ftsTableAvailabilityPerDb = new WeakMap();
/**
 ...
 */
function isFtsTableAvailable(database) {
    const cached = ftsTableAvailabilityPerDb.get(database);
    if (typeof cached === 'boolean') {
        return cached;
    }
    try {
        const result = database.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='memory_fts'`).get();
 ...
 */
function queryCausalEdges(database, query, limit, specFolder) {
    const graphResults = [];
    try {
 ...
 */
function queryCausalEdgesFTS5(database, query, limit) {
    ...
    LIMIT ?
  `).all(sanitized, oversampleLimit, oversampleLimit);
 ...
 */
function queryCausalEdgesLikeFallback(database, query, limit) {
    ...
    LIMIT ?
  `).all(likeParam, likeParam, likeParam, likeParam, oversampleLimit);
    // Return one candidate entry per memory node (source_id and target_id) with
 ...
 */
let degreeCachePerDb = new WeakMap();
let dbCacheRebindRegistered = false;
function getDegreeCacheState(database) {
    let state = degreeCachePerDb.get(database);
    if (!state) {
        state = {
            perNodeBoost: new Map(),
            maxTypedDegree: null,
        };
 ...
 */
function computeTypedDegree(database, memoryId) {
 ...
 */
function normalizeDegreeToBoostedScore(rawDegree, maxDegree) {
    if (rawDegree <= 0 || maxDegree <= 0)
        return 0;
 ...
 */
function computeMaxTypedDegree(database) {
    ...
    catch (_err) { // Subgraph computation failure — return default weights
        cacheState.maxTypedDegree = DEFAULT_MAX_TYPED_DEGREE;
        return DEFAULT_MAX_TYPED_DEGREE;
    }
}
 ...
 */
function computeTypedDegreesBatch(database, candidateIds) {
 ...
 */
function computeDegreeScores(database, memoryIds) {
    const results = new Map();
    if (memoryIds.length === 0)
        return results;
    // Identify constitutional memories (excluded from degree boosting)
    const constitutionalIds = new Set();
    try {
        const placeholders = memoryIds.map(() => '?').join(',');
        const constitutionalRows = database.prepare(`SELECT id FROM memory_index WHERE id IN (${placeholders}) AND importance_tier = 'constitutional'`).all(...memoryIds.map(String));
...
function clearDegreeCache() {
    degreeCachePerDb = new WeakMap();
    ftsTableAvailabilityPerDb = new WeakMap();
}
/** Clear degree cache for a specific database instance. */
function clearDegreeCacheForDb(database) {
 ...
 */
function createUnifiedGraphSearchFn(database, _legacyArg) {
    return function unifiedGraphSearch(query, options) {
...
// 6. EXPORTS
// ───────────────────────────────────────────────────────────────
export { createUnifiedGraphSearchFn, 
// Typed-degree computation (R4 5th RRF channel)
EDGE_TYPE_WEIGHTS, DEFAULT_MAX_TYPED_DEGREE, MAX_TOTAL_DEGREE, DEGREE_BOOST_CAP, DEGREE_CHANNEL_WEIGHT, computeTypedDegree, normalizeDegreeToBoostedScore, computeMaxTypedDegree, computeDegreeScores, clearDegreeCache, clearDegreeCacheForDb, };
//# sourceMappingURL=graph-search-fn.js.map