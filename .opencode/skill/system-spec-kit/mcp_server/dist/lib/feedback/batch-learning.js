...
import { initFeedbackLedger, getFeedbackEvents, } from './feedback-ledger.js';
/* ───────────────────────────────────────────────────────────────
   1. CONSTANTS
----------------------------------------------------------------*/
...
const initializedBatchDbs = new WeakSet();
/**
 ...
 */
export function initBatchLearning(db) {
    if (initializedBatchDbs.has(db))
        return;
 ...
 */
export function aggregateEvents(db, since, until) {
    try {
        ...
        for (const ev of events) {
            ...
            if (!entry) {
                entry = { sessions: new Set(), strong: 0, medium: 0, weak: 0 };
        ...
        for (const [memoryId, data] of byMemory) {
            const weightedScore = data.strong * CONFIDENCE_WEIGHTS.strong +
 ...
 */
export function applyMinSupportFilter(signals, minSupport = MIN_SUPPORT_SESSIONS) {
    const eligible = [];
    const skipped = [];
 ...
 */
export function enforceBoostCap(proposedBoost, maxBoostDelta = MAX_BOOST_DELTA) {
 ...
 */
export function computeShadowRankDelta(db, memoryId, boost) {
    try {
        const row = db.prepare('SELECT importance_weight FROM memory_index WHERE id = ? LIMIT 1').get(memoryId);
        if (!row)
            return null;
 ...
 */
export function shadowApply(db, signal, runAt) {
    try {
        initBatchLearning(db);
      ...
      VALUES (?, ?, ?, ?, ?, ?, 1)
    `).run(signal.memoryId, runAt, signal.sessionCount, signal.weightedScore, boost, shadowDelta);
 ...
 */
export function runBatchLearning(db, opts = {}) {
    ...
    try {
        ...
        const totalEventsProcessed = allSignals.reduce((sum, s) => sum + s.strongCount + s.mediumCount + s.weakCount, 0);
        ...
        for (const signal of eligible) {
            ...
            const cappedSignal = { ...signal, computedBoost: cappedBoost };
 ...
 */
export function getBatchLearningHistory(db, memoryId) {
    try {
        initBatchLearning(db);
        return db.prepare(`
 ...
 */
export function getBatchLearningCount(db, memoryId) {
    try {
        initBatchLearning(db);
        if (memoryId) {
            const row = db.prepare('SELECT COUNT(*) as count FROM batch_learning_log WHERE memory_id = ?').get(memoryId);
            return row.count;
        }
        const row = db.prepare('SELECT COUNT(*) as count FROM batch_learning_log').get();
        return row.count;
    }
    catch {
        return 0;
...
//# sourceMappingURL=batch-learning.js.map