import { logFeedbackEvent, isImplicitFeedbackLogEnabled, } from './feedback-ledger.js';
/* ───────────────────────────────────────────────────────────────
   1. CONFIGURATION
──────────────────────────────────────────────────────────────── */
 ...
 */
function detectFlow(sessionId, tokens, timestamp) {
    const entries = sessionQueries.get(sessionId);
 ...
 */
export function trackQueryAndDetect(db, sessionId, query, queryId, shownMemoryIds) {
 ...
 */
export function logResultCited(db, sessionId, queryId, memoryIds) {
    if (!isImplicitFeedbackLogEnabled())
        return;
 ...
 */
export function logFollowOnToolUse(db, sessionId) {
    if (!isImplicitFeedbackLogEnabled())
        return;
    if (!sessionId)
        return;
    ...
    if (elapsed > FOLLOW_ON_WINDOW_MS)
        return;
    if (latest.shownMemoryIds.length === 0)
        return;
    const timestamp = Date.now();
 ...
 */
export function getSessionQueryCount(sessionId) {
    return sessionQueries.get(sessionId)?.length ?? 0;
}
//# sourceMappingURL=query-flow-tracker.js.map