 ...
 */
export function initShadowScoringLog(db) {
 ...
 */
export function selectHoldoutQueries(queryIds, opts = {}) {
 ...
 */
function stratifiedSample(queryIds, intentClasses, targetCount, seed) {
    const rng = seededRandom(seed);
 ...
 */
export function logRankDelta(db, comparison, cycleId, evaluatedAt) {
    ...
    `);
        ...
        for (const d of comparison.deltas) {
            stmt.run(comparison.queryId, d.resultId, d.liveRank, d.shadowRank, d.delta, d.direction, evaluatedAt, cycleId);
            inserted++;
        }
        return inserted;
    }
 ...
 */
export function getShadowScoringHistory(db, opts = {}) {
    try {
        initShadowScoringLog(db);
        const conditions = [];
        const params = [];
 ...
 */
export function recordCycleResult(db, result) {
    if (!isShadowFeedbackEnabled())
        return null;
    try {
        initShadowScoringLog(db);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(result.cycleId, result.evaluatedAt, result.queryCount, result.meanNdcgDelta, result.meanMrrDelta, result.meanKendallTau, result.totalImproved, result.totalDegraded, result.totalUnchanged, result.isImprovement ? 1 : 0);
 ...
 */
export function getConsecutiveImprovements(db) {
    try {
        initShadowScoringLog(db);
 ...
 */
export function isPromotionReady(db) {
 ...
 */
export function getCycleResults(db) {
    try {
        initShadowScoringLog(db);
        const rows = db.prepare(`
 ...
 */
export function evaluatePromotionGate(db) {
    try {
        initShadowScoringLog(db);
 ...
 */
export function runShadowEvaluation(db, allQueryIds, computeLiveRanks, computeShadowRanks, opts = {}) {
    if (!isShadowFeedbackEnabled())
        return null;
