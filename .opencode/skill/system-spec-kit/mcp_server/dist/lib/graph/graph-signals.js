...
// ───────────────────────────────────────────────────────────────
// 1. IMPORTS
import { STAGE2_GRAPH_BONUS_CAP, clampStage2GraphBonus, } from '../search/pipeline/ranking-contract.js';
// ───────────────────────────────────────────────────────────────
// 2. SESSION CACHE
// ───────────────────────────────────────────────────────────────
 ...
 */
export function clearGraphSignalsCache() {
    momentumCache.clear();
    depthCache.clear();
}
 ...
 */
export function snapshotDegrees(db) {
    try {
 ...
 */
function getCurrentDegree(db, memoryId) {
    try {
 ...
 */
function getPastDegree(db, memoryId) {
    try {
        const row = db.prepare(`
      SELECT degree_count
 ...
 */
export function computeMomentum(db, memoryId) {
    try {
 ...
 */
export function computeMomentumScores(db, memoryIds) {
...
function computeGraphWalkMetrics(db, memoryIds) {
    const uniqueIds = Array.from(new Set(memoryIds.filter((memoryId) => Number.isInteger(memoryId))));
    const scores = new Map(uniqueIds.map((memoryId) => [memoryId, { raw: 0, normalized: 0 }]));
    ...
    for (const memoryId of uniqueIds) {
        const directNeighbors = graph.get(memoryId) ?? new Set();
        let directHits = 0;
        const secondHopHits = new Set();
...
function computeGraphWalkScores(db, memoryIds) {
 ...
 */
export function computeCausalDepthScores(db, memoryIds) {
    const results = new Map();
 ...
 */
export function applyGraphSignals(rows, db, options = {}) {
