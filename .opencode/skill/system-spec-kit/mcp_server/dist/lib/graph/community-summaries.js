...
function ensureCommunitySummariesTable(db) {
    db.exec(`
...
function selectMemberRows(db, memberIds) {
...
function buildSummary(communityId, memberIds, rows) {
    const topTitles = rows
        .map((row) => row.title?.trim())
        .filter((title) => Boolean(title))
        .slice(0, 3);
    const folders = Array.from(new Set(rows
        .map((row) => row.spec_folder?.trim())
        .filter((folder) => Boolean(folder)))).slice(0, 3);
    const sentences = [];
    const folderPhrase = folders.length > 0
        ? ` across ${folders.length} spec folder${folders.length === 1 ? '' : 's'}`
        : '';
    sentences.push(`Community ${communityId} groups ${memberIds.length} causally linked memories${folderPhrase}.`);
...
export function generateCommunitySummaries(db, communities) {
    ...
    try {
      ...
      `);
            ...
            for (const community of communities) {
                ...
                const summary = buildSummary(community.communityId, community.memberIds, memberRows);
                insert.run(community.communityId, summary, JSON.stringify(community.memberIds), community.memberIds.length, timestamp, timestamp);
                summaries.push({
