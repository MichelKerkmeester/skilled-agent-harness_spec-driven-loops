 ...
 *   Grep search active
 */
export function normalizeMarkdownTables(content) {
 ...
 */
export function normalizeMarkdownLists(content) {
    return content
 ...
 */
export function normalizeHeadings(content) {
    return content.replace(/^#{1,6}\s+(?:\d+\.\s+)?(.+)$/gm, (_match, heading) => heading.trim());
}
// ───────────────────────────────────────────────────────────────
 ...
 */
export function normalizeContentForEmbedding(content) {
 ...
 *   bm25-index.ts ~line 245 where `content_text` is used for token building.
 */
export function normalizeContentForBM25(content) {
