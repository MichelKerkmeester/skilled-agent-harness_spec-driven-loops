...
import { canClassifyAsSpecDocument, extractSpecFolderFromSpecDocumentPath, isWorkingArtifactPath, matchesSpecDocumentPath, SPEC_DOCUMENT_FILENAMES, } from '../config/spec-doc-paths.js';
 ...
 */
export const MEMORY_FILE_PATTERN = /specs\/([^/]+)(?:\/[^/]+)*\/memory\/[^/]+\.(?:md|txt)$/i;
/**
 * Defines the MAX_CONTENT_LENGTH constant.
 */
const _parsedMaxLen = parseInt(process.env.MCP_MAX_CONTENT_LENGTH || '250000', 10);
export const MAX_CONTENT_LENGTH = Number.isFinite(_parsedMaxLen) && _parsedMaxLen > 0 ? _parsedMaxLen : 250000;
/**
...
function detectBomlessUtf16Encoding(buffer) {
 ...
 */
export function parseMemoryContent(filePath, content, options = {}) {
    ...
    const typeInference = inferMemoryType({
        filePath,
        content: content,
 ...
 */
export function extractDocumentType(filePath) {
    const normalizedPath = filePath.replace(/\\/g, '/');
    const basename = path.basename(filePath).toLowerCase();
    // Spec folder document types
    const FILENAME_TO_DOC_TYPE = {
        'spec.md': 'spec',
        'plan.md': 'plan',
        'tasks.md': 'tasks',
    ...
    if (canClassifyAsSpecDocument(filePath)) {
        ...
        if (docType && matchesSpecDocumentPath(filePath, basename))
            return docType;
    }
...
export function extractSpecFolder(filePath) {
...
function truncateTitle(title, maxLength = MAX_MEMORY_TITLE_LENGTH) {
...
function normalizeExtractedTitle(raw) {
...
function getFirstMeaningfulLine(section) {
...
function extractFrontmatterBlock(content) {
    ...
    const frontmatterMatch = content.match(/^(?:\uFEFF)?(?:\s*<!--[\s\S]*?-->\s*)*---\s*\r?\n([\s\S]*?)\r?\n---(?:\s*\r?\n|$)/);
...
function stripLeadingFrontmatter(content) {
    ...
    return content.replace(/^(?:\uFEFF)?(?:\s*<!--[\s\S]*?-->\s*)*---\s*\r?\n[\s\S]*?\r?\n---(?:\s*\r?\n|$)?/, '');
}
...
export function extractTitle(content) {
    ...
    if (frontmatter) {
        ...
        if ((rawFrontmatterTitle.startsWith('"') && rawFrontmatterTitle.endsWith('"')) ||
...
export function extractTriggerPhrases(content) {
    const triggers = [];
    const frontmatter = extractFrontmatterBlock(content) ?? '';
...
export function extractContextType(content) {
...
export function extractImportanceTier(content, options = {}) {
 ...
 */
export function extractCausalLinks(content) {
    const causalLinks = {
        caused_by: [],
        supersedes: [],
        derived_from: [],
 ...
 */
export function hasCausalLinks(causalLinks) {
    if (!causalLinks)
        return false;
    return Object.values(causalLinks).some((arr) => Array.isArray(arr) && arr.length > 0);
}
// ───────────────────────────────────────────────────────────────
...
function isMarkdownOrTextFile(filePath) {
...
export function isMemoryFile(filePath) {
    ...
    const isConstitutional = (normalizedPath.endsWith('.md') &&
...
export function validateAnchors(content) {
    const warnings = [];
...
export function validateParsedMemory(parsed) {
    const errors = [];
    const warnings = [];
...
function normalizeSpecFolderFilter(value) {
    return value.replace(/\\/g, '/').replace(/^\/+|\/+$/g, '');
}
function matchesSpecFolderFilter(extractedFolder, specFolderFilter) {
...
export function findMemoryFiles(workspacePath, options = {}) {
