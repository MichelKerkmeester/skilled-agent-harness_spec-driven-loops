import { ensureSharedSpaceTables, } from '../search/vector-index-schema.js';
import { isGovernanceGuardrailsEnabled, normalizeScopeContext, recordGovernanceAudit, } from '../governance/scope-governance.js';
...
function isDefaultOffFlagEnabled(...flagNames) {
...
function normalizeConflictKind(metadata) {
...
function normalizeExplicitStrategy(metadata) {
...
function resolveSharedConflictStrategy(database, args) {
    ...
    WHERE space_id = ?
      AND logical_key = ?
  `).get(args.spaceId, args.logicalKey);
 ...
 */
export function isSharedMemoryEnabled(database) {
 ...
 */
export function enableSharedMemory(database) {
 ...
 */
export function ensureSharedCollabRuntime(database) {
 ...
 */
export function getSharedRolloutMetrics(database, tenantId) {
  ...
  `).get(normalizedTenantId, normalizedTenantId);
 ...
 */
export function getSharedRolloutCohortSummary(database, tenantId) {
    ensureSharedCollabRuntime(database);
    const normalizedTenantId = normalizeScopeContext({ tenantId }).tenantId ?? null;
  ...
  `).all(normalizedTenantId, normalizedTenantId);
    ...
    GROUP BY s.space_id
  `).all(normalizedTenantId, normalizedTenantId);
    const membershipCounts = new Map(membershipRows.map((row) => [row.space_id, row.membership_count]));
    const summaryByCohort = new Map();
 ...
 */
export function getSharedConflictStrategySummary(database, spaceId) {
    ensureSharedCollabRuntime(database);
  ...
  `).all(spaceId ?? null, spaceId ?? null).map((row) => ({
        ...
        distinctLogicalKeys: row.distinct_logical_keys,
 ...
 */
function assertValidSharedSpaceDefinition(definition) {
...
function getSharedSpaceWriteValues(definition) {
    return {
...
export function createSharedSpaceIfAbsent(database, definition) {
    assertValidSharedSpaceDefinition(definition);
    ensureSharedCollabRuntime(database);
    const { rolloutEnabledValue, rolloutCohortValue, killSwitchValue, metadataValue, } = getSharedSpaceWriteValues(definition);
    ...
    RETURNING space_id
  `).get(definition.spaceId, definition.tenantId, definition.name, rolloutEnabledValue, rolloutCohortValue, killSwitchValue, metadataValue);
    return { created: row !== undefined };
}
export function upsertSharedSpace(database, definition) {
    assertValidSharedSpaceDefinition(definition);
    ensureSharedCollabRuntime(database);
    const { rolloutEnabledValue, rolloutCohortValue, killSwitchValue, metadataValue, } = getSharedSpaceWriteValues(definition);
    database.prepare(`
  ...
  `).run(definition.spaceId, definition.tenantId, definition.name, rolloutEnabledValue, rolloutCohortValue, killSwitchValue, metadataValue, rolloutEnabledValue, killSwitchValue);
}
/**
 ...
 */
export function upsertSharedMembership(database, membership) {
 ...
 */
export function getAllowedSharedSpaceIds(database, scope) {
    ...
    `).all(subjectType, subjectId, normalizedScope.tenantId);
 ...
 */
export function assertSharedSpaceAccess(database, scope, spaceId, requiredRole = 'viewer') {
    const normalizedScope = normalizeScopeContext(scope);
    const auditAccessDecision = (decision, reason, metadata) => {
        try {
    ...
    const deny = (reason, metadata) => {
        auditAccessDecision('deny', reason, metadata);
        return { allowed: false, reason };
    };
    const allow = (metadata) => {
  ...
  `).get(spaceId);
    ...
    END ASC
    LIMIT 1
  `).get(spaceId, normalizedScope.userId ?? '', normalizedScope.agentId ?? '');
 ...
 */
export function recordSharedConflict(database, args) {
    ensureSharedCollabRuntime(database);
    ...
    `).run(args.spaceId, args.logicalKey, args.existingMemoryId, args.incomingMemoryId, resolved.strategy, args.actor, resolved.metadata ? JSON.stringify(resolved.metadata) : null);
        recordGovernanceAudit(database, {
...
//# sourceMappingURL=shared-spaces.js.map