...
function getRankAtIndex(_result, index) {
    return index + 1;
}
/* ───────────────────────────────────────────────────────────────
   2. INTERNAL HELPERS
 ...
 */
function buildRelevanceMap(groundTruth) {
    const map = new Map();
 ...
 */
function buildGroundTruthMap(groundTruth) {
    const map = new Map();
    for (const entry of groundTruth) {
 ...
 */
export function computeMRR(results, groundTruth, k = 5) {
    if (results.length === 0 || groundTruth.length === 0)
        return 0;
    if (groundTruth.every(entry => entry.relevance <= 0))
        return 0;
 ...
 */
export function computeNDCG(results, groundTruth, k = 10) {
    if (results.length === 0 || groundTruth.length === 0)
        return 0;
    if (groundTruth.every(entry => entry.relevance <= 0))
        return 0;
 ...
 */
export function computeRecall(results, groundTruth, k = 20) {
    if (results.length === 0 || groundTruth.length === 0)
        return 0;
    const relevantIds = new Set(groundTruth.filter(e => e.relevance > 0).map(e => e.memoryId));
 ...
 */
export function computePrecision(results, groundTruth, k = 20) {
    if (k <= 0 || results.length === 0 || groundTruth.length === 0)
        return 0;
    const relevantIds = new Set(groundTruth.filter(e => e.relevance > 0).map(e => e.memoryId));
    const topResults = topK(results, k);
 ...
 */
export function computeF1(results, groundTruth, k = 20) {
    if (k <= 0)
        return 0;
 ...
 * @returns Value in [0, 1]. Returns 0 for empty inputs.
 */
export function computeMAP(results, groundTruth, k = 20) {
    if (results.length === 0 || groundTruth.length === 0)
        return 0;
    const relevantIds = new Set(groundTruth.filter(e => e.relevance > 0).map(e => e.memoryId));
    if (relevantIds.size === 0)
 ...
 */
export function computeHitRate(results, groundTruth, k = 1) {
    if (results.length === 0 || groundTruth.length === 0)
        return 0;
    const relevanceMap = buildRelevanceMap(groundTruth);
    const topResults = topK(results, k);
    for (const r of topResults) {
        if ((relevanceMap.get(r.memoryId) ?? 0) > 0)
            return 1;
    }
    return 0;
}
 ...
 */
export function computeInversionRate(results, groundTruth) {
    if (results.length < 2 || groundTruth.length === 0)
        return 0;
    const relevanceMap = buildRelevanceMap(groundTruth);
 ...
 */
export function computeConstitutionalSurfacingRate(results, constitutionalIds, k = 5) {
    if (results.length === 0 || constitutionalIds.length === 0)
        return 0;
    const constitutionalSet = new Set(constitutionalIds);
    const topResults = topK(results, k);
    for (const r of topResults) {
        if (constitutionalSet.has(r.memoryId))
            return 1;
    }
    return 0;
}
 ...
 * @returns Value in [0, 1]. Returns 0 when no relevant items exist.
 */
export function computeImportanceWeightedRecall(results, groundTruth, tierWeights = {}, k = 20) {
    if (results.length === 0 || groundTruth.length === 0)
        return 0;
    if (groundTruth.every(entry => entry.relevance <= 0))
        return 0;
 ...
 */
export function computeColdStartDetectionRate(results, groundTruth, memoryTimestamps, cutoffHours = 48, k = 10, evaluatedAt) {
    if (results.length === 0 || groundTruth.length === 0)
        return 0;
    ...
    const recentRelevantIds = new Set();
    for (const entry of groundTruth) {
    ...
    if (recentRelevantIds.size === 0)
        return 0;
    // Check if any recent relevant memory appears in top-K results
    const topResults = topK(results, k);
    for (const r of topResults) {
        if (recentRelevantIds.has(r.memoryId))
            return 1;
    }
    return 0;
}
 ...
 * @returns Value in [0, 1]. Returns 0 for empty inputs.
 */
export function computeIntentWeightedNDCG(results, groundTruth, intentType, k = 10) {
    if (results.length === 0 || groundTruth.length === 0)
        return 0;
    ...
    const weightedGT = groundTruth.map(e => ({
        ...e,
...
export function computeAllMetrics(params) {
    const { results, groundTruth, constitutionalIds = [], memoryTimestamps = {}, intentType = 'understand', evaluatedAt, } = params;
    return {
