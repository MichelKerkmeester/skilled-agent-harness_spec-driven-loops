...
import { computeRecall, computeMRR, computeNDCG, computePrecision, computeMAP, computeHitRate, } from './eval-metrics.js';
 ...
 */
export function isAblationEnabled() {
 ...
 */
export function inspectGroundTruthAlignment(database) {
    const uniqueMemoryIds = Array.from(new Set(GROUND_TRUTH_RELEVANCES.map((row) => row.memoryId))).sort((left, right) => left - right);
    ...
    const rows = database.prepare(`SELECT id, parent_id FROM memory_index WHERE id IN (${placeholders})`).all(...uniqueMemoryIds);
    ...
    const parentMemoryIds = new Set();
    const chunkMemoryIds = new Set();
 ...
 */
export function assertGroundTruthAlignment(database, options = {}) {
    ...
    if (summary.chunkRelevanceCount > 0) {
        const sampleText = summary.chunkExamples
            ...
            .join(', ');
        details.push(`chunk-backed relevances=${summary.chunkRelevanceCount} across ${summary.chunkMemoryCount} IDs`
            + (sampleText ? ` (examples: ${sampleText})` : ''));
    }
    if (summary.missingRelevanceCount > 0) {
        const sampleText = summary.missingExamples.join(', ');
        details.push(`missing relevances=${summary.missingRelevanceCount} across ${summary.missingMemoryCount} IDs`
            + (sampleText ? ` (examples: ${sampleText})` : ''));
    }
    throw new Error(`Ground truth is not aligned to parent memories${contextSuffix}: ${details.join('; ')}. `
 ...
 */
function getGroundTruthForQuery(queryId) {
 ...
 */
function getQueriesToEvaluate(config) {
 ...
 */
function signTestPValue(nPositive, nNegative) {
    const n = nPositive + nNegative;
    if (n < 5)
        return null; // Insufficient data for meaningful test
    ...
    function logBinomial(nVal, kVal) {
        if (kVal < 0 || kVal > nVal)
            return -Infinity;
        if (kVal === 0 || kVal === nVal)
            return 0;
 ...
 */
function computeQueryMetrics(results, gt) {
    return {
 ...
 */
function buildAggregatedMetrics(baselinePerQuery, ablatedPerQuery) {
    const bMetrics = [...baselinePerQuery.values()];
    const aMetrics = [...ablatedPerQuery.values()];
    function avg(vals) {
        return vals.length === 0 ? 0 : vals.reduce((a, b) => a + b, 0) / vals.length;
    }
    function entry(bVals, aVals) {
        const b = avg(bVals);
        const a = avg(aVals);
 ...
 */
export async function runAblation(searchFn, config = { channels: ALL_CHANNELS }) {
    ...
    if (querySelection.missingQueryIds && querySelection.missingQueryIds.length > 0) {
        console.warn(`[ablation] Requested groundTruthQueryIds not found in the static dataset: ${querySelection.missingQueryIds.join(', ')}`);
    }
    try {
        ...
        const baselineRecalls = new Map();
        const baselineMetricsPerQuery = new Map();
        let evaluatedCount = 0;
        const noDisabled = new Set();
        ...
        for (const channel of config.channels) {
            const disabledSet = new Set([channel]);
            const ablatedRecalls = new Map();
            const ablatedMetricsPerQuery = new Map();
            ...
            try {
                ...
                console.warn(`[ablation] Channel "${channel}" failed${querySuffix}; continuing with remaining channels:`, msg);
            }
        }
        const report = {
 ...
 */
export function storeAblationResults(report) {
    if (!isAblationEnabled())
        return false;
    try {
        const db = getDb();
    ...
    `);
        ...
        const baselineQueryCount = report.evaluatedQueryCount
            ?? report.queryCount
            ?? report.results[0]?.queryCount
            ?? 0;
        const writeAll = db.transaction(() => {
            // Store baseline recall
            insertSnapshot.run(evalRunId, `ablation_baseline_recall@${recallK}`, report.overallBaselineRecall, 'all', baselineQueryCount, JSON.stringify({
                runId: report.runId,
            ...
            for (const result of report.results) {
                insertSnapshot.run(evalRunId, `ablation_recall@${recallK}_delta`, result.delta, result.channel, result.queryCount, JSON.stringify({
                    runId: report.runId,
                ...
                if (result.metrics) {
                    for (const [metricName, entry] of Object.entries(result.metrics)) {
                        ...
                        const isSyntheticTokenUsage = metricName === 'token_usage'
                        ...
                        insertSnapshot.run(evalRunId, `ablation_${metricName}_delta`, typedEntry.delta, result.channel, result.queryCount, JSON.stringify({
                            runId: report.runId,
...
export function formatAblationReport(report) {
    ...
    const sorted = [...report.results].sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta));
...
function getVerdict(result) {
    ...
    if (result.delta < 0) {
        ...
        if (isSignificant && absDelta >= 0.05)
            return 'CRITICAL';
        if (isSignificant)
            return 'important';
 ...
 */
export function toHybridSearchFlags(disabledChannels) {
    return {
...
//# sourceMappingURL=ablation-framework.js.map