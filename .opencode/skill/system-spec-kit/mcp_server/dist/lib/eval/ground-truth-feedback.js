 ...
 */
export function resetFeedbackSchemaFlag() {
 ...
 */
export function recordUserSelection(queryId, memoryId, selectionContext = {}) {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(queryId, memoryId, selectionContext.searchMode ?? null, selectionContext.intent ?? null, selectionContext.selectedRank ?? null, selectionContext.totalResultsShown ?? null, selectionContext.sessionId ?? null, selectionContext.notes ?? null);
 ...
 */
export function getSelectionHistory(queryId, limit = 100) {
    try {
        ensureFeedbackSchema();
        const db = getDb();
 ...
 */
export function generateLlmJudgeLabels(querySelectionPairs) {
 ...
 */
export function saveLlmJudgeLabels(labels) {
    try {
        ensureFeedbackSchema();
        const db = getDb();
    ...
    `);
        ...
        const insertAll = db.transaction(() => {
            for (const label of labels) {
                const result = insert.run(label.queryId, label.memoryId, label.relevance, label.confidence, label.reasoning ?? null);
                if (result.changes > 0)
                    count++;
            }
        });
 ...
 */
export function computeJudgeAgreement(llmLabels, manualLabels, targetRate = 0.80) {
 ...
 */
export function getGroundTruthCorpusSize() {
    try {
        ensureFeedbackSchema();
        const db = getDb();
        // Manual/synthetic ground truth
        const manualRow = db.prepare('SELECT COUNT(*) as cnt FROM eval_ground_truth').get();
        const manualPairs = manualRow?.cnt ?? 0;
        // User selection pairs (distinct query_id + memory_id combinations)
        const selectionRow = db.prepare('SELECT COUNT(DISTINCT query_id || \':\' || memory_id) as cnt FROM eval_user_selections').get();
        const selectionPairs = selectionRow?.cnt ?? 0;
        // LLM-judge pairs
        const llmRow = db.prepare('SELECT COUNT(*) as cnt FROM eval_llm_judge_labels').get();
...
//# sourceMappingURL=ground-truth-feedback.js.map