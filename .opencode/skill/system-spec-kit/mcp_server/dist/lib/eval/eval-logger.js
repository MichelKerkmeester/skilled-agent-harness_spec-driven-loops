...
function generateEvalRunId() {
    ...
    if (!_evalRunCounterInitialized) {
        try {
            const db = getDb();
            if (db) {
                const channelMax = db.prepare('SELECT MAX(eval_run_id) as maxId FROM eval_channel_results').get();
                const finalMax = db.prepare('SELECT MAX(eval_run_id) as maxId FROM eval_final_results').get();
...
function logSearchQuery(params) {
    const noop = { queryId: 0, evalRunId: 0 };
    if (!isEvalLoggingEnabled())
        return noop;
    try {
        const db = getDb();
      ...
      VALUES (?, ?, ?)
    `).run(params.query ?? '', params.intent ?? null, params.specFolder ?? null);
 ...
 */
function logChannelResult(params) {
    if (!isEvalLoggingEnabled())
        return;
    if (!params.evalRunId || !params.queryId)
        return;
    try {
        const db = getDb();
        db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(params.evalRunId, params.queryId, params.channel ?? 'unknown', params.resultMemoryIds ? JSON.stringify(params.resultMemoryIds) : null, params.scores ? JSON.stringify(params.scores) : null, params.latencyMs ?? null, params.hitCount ?? 0);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
 ...
 * Fail-safe: never throws.
 */
function logFinalResult(params) {
    if (!isEvalLoggingEnabled())
        return;
    if (!params.evalRunId || !params.queryId)
        return;
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(params.evalRunId, params.queryId, params.resultMemoryIds ? JSON.stringify(params.resultMemoryIds) : null, params.scores ? JSON.stringify(params.scores) : null, params.fusionMethod ?? 'rrf', params.latencyMs ?? null);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
...
//# sourceMappingURL=eval-logger.js.map