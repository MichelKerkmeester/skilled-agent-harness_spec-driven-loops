 ...
 */
export function measureEdgeDensity(database) {
    try {
        // Total edges
 ...
 */
export function formatDensityReport(result) {
    const lines = [
        '=== Edge Density Report ===',
 ...
 */
function buildR10Recommendation(density, edgeCount, nodeCount, totalMemories) {
    const targetDensity = MODERATE_THRESHOLD;
    const denominator = totalMemories > 0 ? totalMemories : nodeCount;
    const edgesNeeded = denominator > 0 ? Math.ceil(targetDensity * denominator) - edgeCount : 0;
    return [
...
//# sourceMappingURL=edge-density.js.map