...
function safeScalarMetric(database, sql, params = [], field = 'value') {
    try {
...
function resolveContextDbPath(explicitPath) {
...
function buildMetrics(evalDb, contextDb) {
    const metrics = {
        'retrieval.eval_queries_total': safeScalarMetric(evalDb, 'SELECT COUNT(*) AS value FROM eval_queries'),
        'retrieval.eval_channel_rows_total': safeScalarMetric(evalDb, 'SELECT COUNT(*) AS value FROM eval_channel_results'),
        'retrieval.avg_channel_hit_count': safeScalarMetric(evalDb, 'SELECT COALESCE(AVG(hit_count), 0) AS value FROM eval_channel_results'),
        'retrieval.avg_channel_latency_ms': safeScalarMetric(evalDb, 'SELECT COALESCE(AVG(latency_ms), 0) AS value FROM eval_channel_results'),
        'retrieval.eval_final_rows_total': safeScalarMetric(evalDb, 'SELECT COUNT(*) AS value FROM eval_final_results'),
        'retrieval.avg_final_latency_ms': safeScalarMetric(evalDb, 'SELECT COALESCE(AVG(latency_ms), 0) AS value FROM eval_final_results'),
    };
    if (!contextDb) {
    ...
    metrics['isolation.memory_rows_total'] = safeScalarMetric(contextDb, 'SELECT COUNT(*) AS value FROM memory_index');
    metrics['isolation.distinct_spec_folders'] = safeScalarMetric(contextDb, "SELECT COUNT(DISTINCT spec_folder) AS value FROM memory_index WHERE spec_folder IS NOT NULL AND TRIM(spec_folder) != ''");
    metrics['isolation.unscoped_rows'] = safeScalarMetric(contextDb, "SELECT COUNT(*) AS value FROM memory_index WHERE spec_folder IS NULL OR TRIM(spec_folder) = ''");
    metrics['isolation.missing_session_scope_rows'] = safeScalarMetric(contextDb, "SELECT COUNT(*) AS value FROM memory_index WHERE session_id IS NULL OR TRIM(session_id) = ''");
    metrics['isolation.unknown_context_type_rows'] = safeScalarMetric(contextDb, "SELECT COUNT(*) AS value FROM memory_index WHERE context_type IS NULL OR TRIM(context_type) = '' OR context_type NOT IN ('research','implementation','planning','general','decision','discovery')");
    metrics['schema.version'] = safeScalarMetric(contextDb, 'SELECT version AS value FROM schema_version WHERE id = 1');
    return metrics;
}
/**
 ...
 */
function persistMemoryStateBaselineSnapshot(snapshot, evalDb) {
 ...
 */
function captureMemoryStateBaselineSnapshot(options = {}) {
...
//# sourceMappingURL=memory-state-baseline.js.map