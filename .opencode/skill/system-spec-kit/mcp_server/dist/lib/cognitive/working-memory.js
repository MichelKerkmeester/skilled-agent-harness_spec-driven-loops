...
const sessionModeRegistry = new Map();
/* --- 5. INITIALIZATION --- */
...
function ensureSchema() {
    ...
    try {
        db.exec(SCHEMA_SQL);
        const wmColumns = db.prepare('PRAGMA table_info(working_memory)').all()
...
function migrateLegacyWorkingMemorySchema() {
    if (!db)
        return;
    const tableInfo = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='working_memory'").get();
...
function cleanupOldSessions() {
    ...
    try {
        ...
        const staleSessionIds = db.prepare("SELECT DISTINCT session_id FROM working_memory WHERE datetime(last_focused) < datetime(?, '-' || ? || ' seconds')").all(nowIso, timeoutSeconds);
        const result = db.prepare("DELETE FROM working_memory WHERE datetime(last_focused) < datetime(?, '-' || ? || ' seconds')").run(nowIso, timeoutSeconds);
...
function getSessionEventCounter(sessionId) {
    const current = getLatestSessionEventCounter(sessionId);
    return current === null ? 0 : current;
}
function getSessionPromptContext(sessionId, floor = DECAY_FLOOR, limit = 5) {
    if (!db)
        return [];
    ensureSchema();
    ...
    try {
      ...
      LIMIT ?
    `).all(sessionId, safeFloor, safeLimit);
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
...
function getSessionInferredMode(sessionId) {
    const mode = sessionModeRegistry.get(sessionId);
    return typeof mode === 'string' && mode.length > 0 ? mode : null;
}
function setSessionInferredMode(sessionId, mode) {
 ...
 */
function calculateTier(attentionScore) {
 ...
 */
function setAttentionScore(sessionId, memoryId, score) {
    if (!db)
        return false;
    ensureSchema();
    ...
    try {
        ...
        const existing = db.prepare('SELECT id FROM working_memory WHERE session_id = ? AND memory_id = ?').get(sessionId, memoryId);
        if (existing) {
            db.prepare(`
      ...
      `).run(clampedScore, currentEventCounter, sessionId, memoryId);
        }
        else {
            // Check capacity
        ...
        VALUES (?, ?, ?, ?, 0)
      `).run(sessionId, memoryId, clampedScore, currentEventCounter);
        }
        return true;
    }
    catch (error) {
...
function upsertExtractedEntry(input) {
    if (!db)
        return false;
    ensureSchema();
    const { sessionId, memoryId, attentionScore, sourceTool, sourceCallId, extractionRuleId, redactionApplied, } = input;
    ...
    `).run(sessionId, memoryId, clampedScore, currentEventCounter, sourceTool, sourceCallId, extractionRuleId, redactionApplied ? 1 : 0);
        ...
        const currentRow = db.prepare('SELECT focus_count FROM working_memory WHERE session_id = ? AND memory_id = ?').get(sessionId, memoryId);
...
function nextEventCounter(sessionId) {
    if (!db)
        return 0;
    const current = getLatestSessionEventCounter(sessionId);
    if (current === null) {
 ...
 */
function enforceMemoryLimit(sessionId) {
    if (!db)
        return 0;
    try {
        const count = db.prepare('SELECT COUNT(*) as count FROM working_memory WHERE session_id = ? AND memory_id IS NOT NULL').get(sessionId);
        if (count.count < WORKING_MEMORY_CONFIG.maxCapacity)
            return 0;
 ...
 */
function batchUpdateScores(sessionId) {
    if (!db)
        return 0;
    ensureSchema();
        ...
        db.transaction(() => {
            for (const entry of entries) {
                ...
                if (rawScore < DELETE_THRESHOLD) {
                    const deleteResult = deleteStmt.run(entry.id);
                ...
                const updateResult = updateStmt.run(nextScore, entry.id);
...
function getCurrentEventCounter(sessionId) {
    const current = getLatestSessionEventCounter(sessionId);
    if (current === null) {
        return 0;
    }
...
function getLatestSessionEventCounter(sessionId) {
    if (!db)
        return null;
    try {
...
function calculateEventDistance(currentCounter, entryCounter) {
...
/* --- 9. EXPORTS --- */
export { WORKING_MEMORY_CONFIG, SCHEMA_SQL, INDEX_SQL, 
// Initialization
init, ensureSchema, 
// Session management
getOrCreateSession, clearSession, cleanupOldSessions, 
// Working memory operations
getWorkingMemory, getSessionMemories, sessionExists, getSessionEventCounter, getSessionPromptContext, getSessionInferredMode, setSessionInferredMode, calculateTier, setAttentionScore, upsertExtractedEntry, enforceMemoryLimit, batchUpdateScores, DECAY_FLOOR, DELETE_THRESHOLD, EVENT_DECAY_FACTOR, MENTION_BOOST_FACTOR, EVENT_COUNTER_MODULUS, MAX_MENTION_COUNT, 
// Utilities
isEnabled, getConfig, getSessionStats, };
