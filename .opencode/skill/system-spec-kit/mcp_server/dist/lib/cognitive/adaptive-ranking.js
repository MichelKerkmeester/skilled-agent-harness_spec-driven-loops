...
const adaptiveThresholdCache = new WeakMap();
function getCachedThresholdForDb(database) {
    if (!database)
        return undefined;
    return adaptiveThresholdCache.get(database);
}
function setCachedThresholdForDb(database, config, updatedAt) {
    adaptiveThresholdCache.set(database, { config, updatedAt });
}
function parseAdaptiveTimestamp(value) {
    if (!value)
        return null;
...
function isCachedThresholdFresh(cachedUpdatedAt, persistedUpdatedAt) {
...
function mapAdaptiveThresholdRowToSnapshot(row) {
...
function mergeAdaptiveThresholds(overrides = {}) {
    return {
        maxAdaptiveDelta: clampAdaptiveDelta(overrides.maxAdaptiveDelta ?? DEFAULT_ADAPTIVE_THRESHOLDS.maxAdaptiveDelta),
        minSignalsForPromotion: clampSignalThreshold(overrides.minSignalsForPromotion ?? DEFAULT_ADAPTIVE_THRESHOLDS.minSignalsForPromotion),
        signalWeights: {
            access: roundAdaptiveNumber(overrides.signalWeights?.access ?? DEFAULT_ADAPTIVE_THRESHOLDS.signalWeights.access),
            outcome: roundAdaptiveNumber(overrides.signalWeights?.outcome ?? DEFAULT_ADAPTIVE_THRESHOLDS.signalWeights.outcome),
            correction: roundAdaptiveNumber(overrides.signalWeights?.correction ?? DEFAULT_ADAPTIVE_THRESHOLDS.signalWeights.correction),
        },
    };
}
function isAdaptiveFlagEnabled(...flagNames) {
...
function roundAdaptiveNumber(value, digits = 3) {
    const factor = 10 ** digits;
    return Math.round(value * factor) / factor;
}
function clampAdaptiveDelta(value) {
    return roundAdaptiveNumber(Math.max(MIN_ALLOWED_ADAPTIVE_DELTA, Math.min(MAX_ALLOWED_ADAPTIVE_DELTA, value)));
}
function clampSignalThreshold(value) {
    return Math.max(MIN_ALLOWED_SIGNALS_FOR_PROMOTION, Math.min(MAX_ALLOWED_SIGNALS_FOR_PROMOTION, Math.trunc(value)));
}
function getAdaptiveThresholdMetadata(database) {
...
function persistAdaptiveThresholdConfig(database, config, options = {}) {
  ...
  `).run(config.maxAdaptiveDelta, config.signalWeights.outcome, config.signalWeights.correction, config.signalWeights.access, config.minSignalsForPromotion, updatedAt, lastTuneWatermark);
    setCachedThresholdForDb(database, config, updatedAt);
    return config;
}
function getAdaptiveThresholdConfig(database) {
    if (!database) {
        return mergeAdaptiveThresholds();
    }
    ensureAdaptiveTables(database);
...
function isAdaptiveEnabled() {
    return isAdaptiveFlagEnabled('SPECKIT_MEMORY_ADAPTIVE_RANKING', 'SPECKIT_HYDRA_ADAPTIVE_RANKING');
}
/**
 ...
 */
export function ensureAdaptiveTables(database) {
    database.exec(`
  ...
  `);
    const thresholdColumns = database.prepare(`PRAGMA table_info(adaptive_thresholds)`).all();
  ...
  `);
    database.exec('CREATE INDEX IF NOT EXISTS idx_adaptive_signal_events_memory_type ON adaptive_signal_events(memory_id, signal_type)');
}
/**
 ...
 */
export function recordAdaptiveSignal(database, event) {
    if (getAdaptiveMode() === 'disabled')
        return;
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(event.memoryId, event.signalType, typeof event.signalValue === 'number' ? event.signalValue : 1, event.query ?? null, event.actor ?? null, event.metadata ? JSON.stringify(event.metadata) : null);
}
/**
 ...
 */
export function resetAdaptiveState(database) {
    ensureAdaptiveTables(database);
 ...
 */
export function setAdaptiveThresholdOverrides(overrides = {}, database) {
    ...
    const nextOverrides = {
        maxAdaptiveDelta: typeof overrides.maxAdaptiveDelta === 'number' && Number.isFinite(overrides.maxAdaptiveDelta)
            ...
            : undefined,
        minSignalsForPromotion: typeof overrides.minSignalsForPromotion === 'number' && Number.isFinite(overrides.minSignalsForPromotion)
            ? overrides.minSignalsForPromotion
            : undefined,
        signalWeights: overrides.signalWeights
            ? {
                access: typeof overrides.signalWeights.access === 'number' && Number.isFinite(overrides.signalWeights.access)
                    ? overrides.signalWeights.access
                    : undefined,
                outcome: typeof overrides.signalWeights.outcome === 'number' && Number.isFinite(overrides.signalWeights.outcome)
                    ? overrides.signalWeights.outcome
                    : undefined,
                correction: typeof overrides.signalWeights.correction === 'number' && Number.isFinite(overrides.signalWeights.correction)
 ...
 */
export function resetAdaptiveThresholdOverrides(database) {
    if (database) {
        ensureAdaptiveTables(database);
 ...
 */
export function getAdaptiveThresholdSnapshot(database) {
    return getAdaptiveThresholdConfig(database);
}
/**
 ...
 */
export function summarizeAdaptiveSignalQuality(database) {
    ensureAdaptiveTables(database);
  ...
  `).get(thresholds.minSignalsForPromotion);
 ...
 */
export function tuneAdaptiveThresholdsAfterEvaluation(database, gateResult) {
    ensureAdaptiveTables(database);
  ...
  `).get();
    ...
    const clampWeight = (key, value) => roundAdaptiveNumber(Math.max(ADAPTIVE_SIGNAL_WEIGHTS[key] - WEIGHT_CLAMP_RANGE, Math.min(ADAPTIVE_SIGNAL_WEIGHTS[key] + WEIGHT_CLAMP_RANGE, value)));
    const hasStrongSignals = (summary.totalSignals >= previous.minSignalsForPromotion * 2
    ...
    if (shouldRelaxThresholds) {
        ...
        nextSignalWeights.outcome = clampWeight('outcome', previous.signalWeights.outcome + (0.005 * relaxationMultiplier));
    }
    ...
    const next = persistAdaptiveThresholdConfig(database, mergeAdaptiveThresholds({
...
function getSignalDeltas(database, memoryIds) {
    ensureAdaptiveTables(database);
  ...
  `).all(...memoryIds);
    ...
    for (const memoryId of memoryIds) {
        ...
        if (!totals)
            continue;
        const rawDelta = (totals.access * thresholds.signalWeights.access) +
 ...
 */
export function buildAdaptiveShadowProposal(database, query, results) {
    const mode = getAdaptiveMode();
    if (mode === 'disabled' || results.length === 0)
        return null;
    const thresholds = getAdaptiveThresholdConfig(database);
    ...
    }).sort((a, b) => compareAdaptiveRows({ ...a, score: a.shadowScore }, { ...b, score: b.shadowScore }));
  ...
  `).all(...results.map((row) => row.id));
    ...
    const rows = shadow.map((row, index) => ({
        memoryId: row.id,
    ...
    const promotedIds = rows
        .filter((row) => row.shadowRank < row.productionRank
...
//# sourceMappingURL=adaptive-ranking.js.map