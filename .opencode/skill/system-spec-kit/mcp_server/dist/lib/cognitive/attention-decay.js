...
import { calculateFiveFactorScore, calculateTemporalScore, calculateUsageScore, calculateImportanceScore, calculatePatternScore, calculateCitationScore, FIVE_FACTOR_WEIGHTS, } from '../scoring/composite-scoring.js';
...
function getDecayRate(importanceTier) {
 ...
 */
function calculateRetrievabilityDecay(stability, elapsedDays) {
 ...
 */
function applyFsrsDecay(memory, baseScore = 1.0) {
 ...
 */
function activateMemoryWithFsrs(memoryId, grade = 3) {
    if (!db)
        return false;
    try {
        const memory = db.prepare('SELECT stability, difficulty, last_review, review_count FROM memory_index WHERE id = ?').get(memoryId);
        if (!memory)
            return false;
        const params = {
            stability: memory.stability || 1.0,
            difficulty: memory.difficulty || 5.0,
            lastReview: memory.last_review || null,
            reviewCount: memory.review_count || 0,
        };
        let result;
      ...
      SET stability = ?,
          ...
          last_accessed = ?
      WHERE id = ?
    `).run(result.stability, result.difficulty, result.lastReview, result.reviewCount, Date.now(), memoryId);
        return true;
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
 ...
 */
function calculateCompositeAttention(memory, options = {}) {
    return calculateFiveFactorScore(memory, options);
}
/**
 * Get detailed attention breakdown for a memory.
 */
function getAttentionBreakdown(memory) {
    const temporal = calculateTemporalScore(memory);
    const usage = calculateUsageScore(memory.access_count || 0);
    const importance = calculateImportanceScore(String(memory.importance_tier || memory.importanceTier || 'normal'), memory.importance_weight);
    const pattern = calculatePatternScore(memory);
    const citation = calculateCitationScore(memory);
    ...
    const composite = (temporal * (weights.temporal || 0.25) +
 ...
 */
function applyCompositeDecay(memories) {
    return memories
        .map(m => ({
        ...m,
 ...
 */
function getActiveMemories(limit = 20) {
    if (!db) {
        console.warn('[attention-decay] Database not initialized. Server may still be starting up.');
        return [];
    }
...
getDecayRate, calculateRetrievabilityDecay, applyFsrsDecay, 
// Activation
activateMemory, activateMemoryWithFsrs, 
// Composite attention
calculateCompositeAttention, getAttentionBreakdown, applyCompositeDecay, getActiveMemories, 
// Session
clearSession, };
//# sourceMappingURL=attention-decay.js.map