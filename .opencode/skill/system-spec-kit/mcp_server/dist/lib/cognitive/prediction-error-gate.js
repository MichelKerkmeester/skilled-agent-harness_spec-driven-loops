 ...
 */
function evaluateMemory(newContentHash, newContent, candidates, options = {}) {
    if (!candidates || candidates.length === 0) {
        // T-09: Log no-candidate CREATE decisions for full audit trail
        if (db) {
            const record = formatConflictRecord(ACTION.CREATE, newContentHash, null, 0, 'No existing candidates found', { detected: false, type: null, description: null, confidence: 0 }, truncateContent(newContent), '', options.specFolder || null);
            logConflict(record);
        }
        return {
    ...
    if (relevant.length === 0) {
        ...
        if (db) {
            const record = formatConflictRecord(ACTION.CREATE, newContentHash, null, 0, 'No relevant candidates after filtering', { detected: false, type: null, description: null, confidence: 0 }, truncateContent(newContent), '', options.specFolder || null);
            logConflict(record);
        }
        return {
            action: ACTION.CREATE,
    ...
    const existingContent = (topCandidate.content || '');
    ...
    if (db) {
        const record = formatConflictRecord(action, newContentHash, topCandidate.id, similarity, reason, contradiction, truncateContent(newContent), truncateContent(existingContent), options.specFolder || null);
        logConflict(record);
    }
    return {
        action,
 ...
 */
function filterRelevantCandidates(candidates) {
    return candidates
 ...
 */
function formatConflictRecord(action, newMemoryHash, existingMemoryId, similarity, reason, contradiction, newContentPreview, existingContentPreview, specFolder) {
    return {
        action,
...
function logConflict(record) {
    ...
    `).run(record.action, record.new_memory_hash, record.existing_memory_id, record.similarity, record.reason, record.contradiction_detected, record.contradiction_type, record.new_content_preview, record.existing_content_preview, record.spec_folder);
    }
    catch (error) {
 ...
 */
function getConflictStats(specFolder = null) {
    if (!db) {
    ...
    try {
        ...
        const total = db.prepare(`SELECT COUNT(*) as count FROM memory_conflicts ${folderFilter}`).get(...params);
        const byAction = db.prepare(`SELECT action, COUNT(*) as count FROM memory_conflicts ${folderFilter} GROUP BY action`).all(...params);
        const contradictions = db.prepare(`SELECT COUNT(*) as count FROM memory_conflicts WHERE contradiction_detected = 1 ${specFolder ? 'AND spec_folder = ?' : ''}`).get(...params);
        const avgSim = db.prepare(`SELECT AVG(similarity) as avg_sim FROM memory_conflicts ${folderFilter}`).get(...params);
        const actionMap = {};
 ...
 */
function batchEvaluate(items, options = {}) {
    const results = [];
 ...
 */
function calculateSimilarityStats(candidates) {
    if (!candidates || candidates.length === 0) {
...
// Core functions
init, evaluateMemory, detectContradiction, formatConflictRecord, logConflict, getConflictStats, getRecentConflicts, batchEvaluate, calculateSimilarityStats, filterRelevantCandidates, getActionPriority, truncateContent, };
//# sourceMappingURL=prediction-error-gate.js.map