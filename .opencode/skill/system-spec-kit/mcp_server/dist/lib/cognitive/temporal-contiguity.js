...
function formatBoundaryTimestamp(timestampMs, anchorTimestamp) {
 ...
 */
export function vectorSearchWithContiguity(results, windowSeconds) {
    if (results === null)
        return null;
    if (results.length <= 1)
 ...
 */
export function getTemporalNeighbors(memoryId, windowSeconds) {
    if (!db) {
    ...
    const clampedWindow = Math.max(1, Math.min(MAX_WINDOW, windowSeconds));
    try {
        const anchor = db.prepare('SELECT created_at, spec_folder FROM memory_index WHERE id = ?').get(memoryId);
        if (!anchor)
            return [];
        ...
        const lowerBound = formatBoundaryTimestamp(anchorTimestampMs - clampedWindow * 1000, anchor.created_at);
        const upperBound = formatBoundaryTimestamp(anchorTimestampMs + clampedWindow * 1000, anchor.created_at);
    ...
    `).all(memoryId, anchor.spec_folder, lowerBound, upperBound);
        const neighbors = [];
 ...
 */
export function buildTimeline(specFolder, limit) {
    if (!db) {
        console.warn('[temporal-contiguity] Database not initialized.');
        return [];
    }
...
//# sourceMappingURL=temporal-contiguity.js.map