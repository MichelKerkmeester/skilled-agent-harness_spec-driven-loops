...
import { createScopeFilterPredicate, hasScopeConstraints, normalizeScopeContext, } from '../governance/scope-governance.js';
...
function batchedInQuery(db, sql, ids, batchSize = 500) {
    const results = [];
    for (let i = 0; i < ids.length; i += batchSize) {
        const batch = ids.slice(i, i + batchSize);
        const placeholders = batch.map(() => '?').join(', ');
        const batchSql = sql.replace('__PLACEHOLDERS__', placeholders);
        results.push(...db.prepare(batchSql).all(...batch));
    }
    return results;
}
...
function getDatabase() {
    if (!db)
        throw new Error('Database not initialized. The checkpoints module requires the MCP server to be running. Restart the MCP server and retry.');
    return db;
}
function isRestoreInProgress() {
    return restoreInProgress;
}
function getRestoreBarrierStatus() {
...
function setRestoreBarrierHooks(hooks) {
    restoreBarrierHooks = hooks ?? {};
}
function acquireRestoreBarrier() {
    restoreInProgress = true;
    restoreBarrierHooks.afterAcquire?.();
}
function releaseRestoreBarrier() {
    try {
...
function tableExists(database, tableName) {
    try {
        const row = database.prepare("SELECT name FROM sqlite_master WHERE type IN ('table','view') AND name = ?").get(tableName);
        return !!row?.name;
    }
    catch (_error) {
        return false;
...
function toBuffer(value) {
    ...
    if (value && typeof value === 'object') {
        const maybeSerialized = value;
        if (maybeSerialized.type === 'Buffer' &&
...
function deserializeSnapshotValue(value) {
    if (value === undefined) {
        return null;
    }
    return toBuffer(value) ?? value;
}
function getMemoryIds(memories) {
    const ids = new Set();
...
function getSharedSpaceIdsFromMemories(memories) {
    const ids = new Set();
    for (const memory of memories) {
...
function checkpointMetadataMatchesScope(rawMetadata, scope) {
    ...
    return ((normalizedScope.tenantId === undefined || metadata.tenantId === normalizedScope.tenantId)
...
function hasDirectScopeColumns(columns) {
...
function getScopeFilterContext(database, scope = {}) {
    const normalizedScope = normalizeScopeContext(scope);
    if (!hasScopeConstraints(normalizedScope)) {
        return {
            normalizedScope,
            allowedSharedSpaceIds: new Set(),
            predicate: null,
        };
    }
    ...
    return {
        ...
        predicate: createScopeFilterPredicate(normalizedScope, allowedSharedSpaceIds),
    };
}
function getScopedMemories(database, specFolder, scope = {}) {
    const { normalizedScope, allowedSharedSpaceIds, predicate } = getScopeFilterContext(database, scope);
    const baseMemories = specFolder
        ? database.prepare('SELECT * FROM memory_index WHERE spec_folder = ?').all(specFolder)
        : database.prepare('SELECT * FROM memory_index').all();
...
function getCurrentScopedMemoryIds(database, specFolder, scope = {}) {
    return getScopedMemories(database, specFolder, scope).memoryIds;
}
function buildRestoreScopeDeleteWhere(columns, checkpointSpecFolder, scope = {}) {
    const normalizedScope = normalizeScopeContext(scope);
    const clauses = [];
    const params = [];
...
function deleteRowsByIds(database, tableName, columnName, ids) {
    if (ids.length === 0) {
        return;
    }
...
function deleteRowsByStringIds(database, tableName, columnName, ids) {
    if (ids.length === 0) {
        return;
    }
    const sql = `DELETE FROM ${tableName} WHERE ${columnName} IN (__PLACEHOLDERS__)`;
...
function snapshotCausalEdgesForMemoryIds(database, memoryIds) {
...
function deleteCausalEdgesForMemoryIds(database, memoryIds) {
    if (memoryIds.length === 0 || !tableExists(database, 'causal_edges')) {
        return;
    }
...
function getTableSnapshotColumns(database, tableName) {
    if (tableName === 'vec_memories') {
        return ['rowid', 'embedding'];
    }
    return getTableColumns(database, tableName);
}
function selectTableRows(database, tableName, options) {
    ...
    const scopePredicate = hasScope
        ? createScopeFilterPredicate(normalizedScope, allowedSharedSpaceIds)
        : null;
    ...
    if (tableName === 'vec_memories') {
        if (memoryIds.length > 0) {
            return batchedInQuery(database, 'SELECT rowid AS rowid, embedding FROM vec_memories WHERE rowid IN (__PLACEHOLDERS__)', memoryIds);
        }
        if (specFolder) {
    ...
    `).all(specFolder);
        ...
        return hasScope ? [] : database.prepare('SELECT rowid AS rowid, embedding FROM vec_memories').all();
    }
    if (tableName === 'causal_edges') {
        if (memoryIds.length > 0) {
            return snapshotCausalEdgesForMemoryIds(database, memoryIds);
        }
        return hasScope ? [] : database.prepare('SELECT * FROM causal_edges').all();
    }
    const columns = new Set(getTableColumns(database, tableName));
    if (tableName === 'shared_spaces') {
        if (sharedSpaceIds.length > 0) {
            return batchedInQuery(database, 'SELECT * FROM shared_spaces WHERE space_id IN (__PLACEHOLDERS__)', sharedSpaceIds);
        }
        if (normalizedScope.tenantId) {
            return database.prepare('SELECT * FROM shared_spaces WHERE tenant_id = ?').all(normalizedScope.tenantId);
        }
        return hasScope ? [] : database.prepare('SELECT * FROM shared_spaces').all();
    }
    if (tableName === 'shared_space_members') {
        if (sharedSpaceIds.length > 0) {
            return batchedInQuery(database, 'SELECT * FROM shared_space_members WHERE space_id IN (__PLACEHOLDERS__)', sharedSpaceIds);
        }
        return hasScope ? [] : database.prepare('SELECT * FROM shared_space_members').all();
    }
    if (tableName === 'shared_space_conflicts') {
        ...
        if (sharedSpaceIds.length > 0) {
            return batchedInQuery(database, 'SELECT * FROM shared_space_conflicts WHERE space_id IN (__PLACEHOLDERS__)', sharedSpaceIds);
        }
        return hasScope ? [] : database.prepare('SELECT * FROM shared_space_conflicts').all();
    }
    if (specFolder && columns.has('spec_folder')) {
        return database.prepare(`SELECT * FROM ${tableName} WHERE spec_folder = ?`).all(specFolder);
    }
    if (memoryIds.length > 0 && columns.has('memory_id')) {
        return batchedInQuery(database, `SELECT * FROM ${tableName} WHERE memory_id IN (__PLACEHOLDERS__)`, memoryIds);
    }
    ...
    const allRows = database.prepare(`SELECT * FROM ${tableName}`).all();
    if (!scopePredicate) {
        return allRows;
    }
...
function createTableSnapshot(database, tableName, options) {
...
function buildLegacyTableSnapshots(snapshot) {
...
function clearTableForRestoreScope(database, tableName, options) {
    const { checkpointSpecFolder, memoryIds, sharedSpaceIds, edgeIds, scope = {}, allowFullTableFallback = true, } = options;
    const normalizedScope = normalizeScopeContext(scope);
    const hasScope = hasScopeConstraints(normalizedScope);
    if (!tableExists(database, tableName)) {
        return;
    ...
    const columns = new Set(getTableColumns(database, tableName));
    const { clauses: scopedClauses, params: scopedParams } = buildRestoreScopeDeleteWhere(columns, checkpointSpecFolder, normalizedScope);
    if (tableName === 'memory_index') {
...
function clearDerivedTablesForRestore(database, options) {
...
function restoreMergeTableAtomically(database, tableName, tableSnapshot, options) {
...
function ensureWorkingMemorySchema(database) {
    try {
        database.exec(`
    ...
    `);
        const wmColumns = database.prepare('PRAGMA table_info(working_memory)').all()
            .map((column) => column.name);
...
function restoreGenericTable(database, tableName, tableSnapshot) {
    if (!tableExists(database, tableName)) {
        return 0;
    }
...
function runPostRestoreRebuilds(database, checkpointSpecFolder) {
    ...
    for (const { name, deps, run } of rebuildSteps) {
        ...
        if (missingDeps.length > 0) {
            skipped.add(name);
            console.warn(`[checkpoints] Skipping post-restore rebuild "${name}" because dependencies did not complete: ${missingDeps.join(', ')}`);
            continue;
        }
        try {
            run();
    ...
    console.warn(`[checkpoints] Post-restore rebuild summary: completed=${[...completed].join(', ') || 'none'}; skipped=${[...skipped].join(', ') || 'none'}`);
}
function normalizeMemoryColumnValue(column, value) {
    if (value === undefined) {
...
function getMemoryRestoreColumns(database, memories) {
 ...
 */
function validateMemoryRow(row, index) {
...
function createCheckpoint(options = {}) {
    ...
    const { name = `checkpoint-${Date.now()}`, specFolder = null, includeEmbeddings: _includeEmbeddings = true, metadata = {}, scope = {}, } = options;
    try {
        // Wrap snapshot SELECTs + INSERT + overflow DELETE in a transaction for atomicity
        const checkpointInfo = database.transaction(() => {
            const { memories, memoryIds: scopedMemoryIds, allowedSharedSpaceIds, normalizedScope, } = getScopedMemories(database, specFolder, scope);
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(name, now, specFolder, gitBranch, compressed, JSON.stringify({
                ...metadata,
            ...
            const checkpointCount = database.prepare('SELECT COUNT(*) as count FROM checkpoints').get();
...
function listCheckpoints(specFolder = null, limit = 50, scope = {}) {
    const database = getDatabase();
    try {
...
function getCheckpoint(nameOrId, scope = {}) {
    const database = getDatabase();
    try {
...
function restoreCheckpoint(nameOrId, clearExisting = false, scope = {}) {
    const database = getDatabase();
    const result = {
        restored: 0,
        skipped: 0,
    ...
    try {
        ...
        const scopedMemoryIdsToReplace = Array.from(new Set([...currentScopedMemoryIds, ...snapshotMemoryIds]));
        const currentScopedSharedSpaceIds = checkpointSpecFolder
            ? []
            : getSharedSpaceIdsFromMemories(getScopedMemories(database, null, normalizedScope).memories);
        const sharedSpaceIds = Array.from(new Set([
        ...
        const restoreTx = database.transaction(() => {
            ...
            const txErrors = [];
            const rolledBackTables = new Set();
            const restoredMemoryIds = new Set();
            for (const memory of memoryRows) {
                ...
                WHERE spec_folder = ?
                  ...
                  AND id != ?
              `).get(memory.spec_folder, memory.file_path, memory.anchor_id ?? null, memory.anchor_id ?? null, memory.id);
                        if (existingByPath) {
                            console.error(`[checkpoints] Skipping restore of memory ${memory.id}: identity "${memory.spec_folder}:${memory.file_path}:${String(memory.anchor_id ?? '')}" already exists as memory ${existingByPath.id}`);
                            result.skipped++;
                            continue;
                        }
              ...
              `).get(memory.id);
                        if (existingById) {
                            ...
                            const sameIdentity = (existingById.spec_folder === memory.spec_folder
                                && existingById.file_path === memory.file_path
                                && existingAnchor === incomingAnchor);
                            if (!sameIdentity) {
                                console.error(`[checkpoints] Skipping restore of memory ${memory.id}: id collision with existing identity "${existingById.spec_folder}:${existingById.file_path}:${String(existingById.anchor_id ?? '')}"`);
                                result.skipped++;
                                continue;
                            }
                        }
                    ...
                    const values = memoryRestoreColumns.map((column) => normalizeMemoryColumnValue(column, memory[column]));
                    const insertResult = memoryInsertStmt.run(...values);
                    ...
                    if (!clearExisting && insertResult.changes === 0 && memoryUpdateStmt) {
                        const updateValues = nonIdColumns.map((column) => normalizeMemoryColumnValue(column, memory[column]));
                        updateValues.push(memory.id); // WHERE id = ?
                        memoryUpdateStmt.run(...updateValues);
                    }
                    restoredMemoryIds.add(memory.id);
                    result.restored++;
                }
                catch (e) {
            ...
            if (clearExisting && txErrors.length > 0) {
                ...
                throw new Error(`Restore aborted: ${txErrors.length} error(s) during restore with clearExisting=true. ` +
...
function deleteCheckpoint(nameOrId, scope = {}) {
    const database = getDatabase();
    try {
        const checkpoint = getCheckpoint(nameOrId, scope);
        if (!checkpoint) {
        ...
        const result = typeof nameOrId === 'number'
            ? database.prepare('DELETE FROM checkpoints WHERE id = ?').run(nameOrId)
            : database.prepare('DELETE FROM checkpoints WHERE name = ?').run(nameOrId);
