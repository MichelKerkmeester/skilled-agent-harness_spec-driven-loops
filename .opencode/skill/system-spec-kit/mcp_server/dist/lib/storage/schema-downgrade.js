...
const REMOVED_COLUMNS = ['parent_id', 'chunk_index', 'chunk_label'];
const V15_COLUMNS = [
    'id',
    'spec_folder',
    'file_path',
...
function ensureColumnsExist(database, requiredColumns) {
...
function createMemoryIndexV15(database) {
    database.exec(`
...
function createV15Indexes(database) {
    database.exec(`
...
function createFtsArtifacts(database) {
    database.exec(`
...
/* ───────────────────────────────────────────────────────────────
   4. MAIN API
──────────────────────────────────────────────────────────────── */
function downgradeSchemaV16ToV15(database, options = {}) {
    ...
    const row = database.prepare('SELECT COUNT(*) as count FROM memory_index').get();
    return {
        fromVersion: TARGET_FROM_VERSION,
        toVersion: TARGET_TO_VERSION,
...
//# sourceMappingURL=schema-downgrade.js.map