...
function appendEntry(db, entry) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
    const result = stmt.run(entry.mutation_type, entry.reason, entry.prior_hash, entry.new_hash, linkedJson, metaJson, entry.actor, entry.session_id ?? null);
    const id = result.lastInsertRowid;
    const row = db.prepare('SELECT * FROM mutation_ledger WHERE id = ?').get(id);
    return row;
}
/* ───────────────────────────────────────────────────────────────
   6. QUERY ENTRIES
----------------------------------------------------------------*/
function getEntries(db, opts = {}) {
    const conditions = [];
    const params = [];
...
function getEntryCount(db) {
    const row = db.prepare('SELECT COUNT(*) as count FROM mutation_ledger').get();
    return row.count;
}
/* ───────────────────────────────────────────────────────────────
   7. LINKED ENTRIES LOOKUP
----------------------------------------------------------------*/
function getLinkedEntries(db, memoryId) {
    ...
    ORDER BY ml.id ASC
  `;
    return db.prepare(sql).all(memoryId);
}
/* ───────────────────────────────────────────────────────────────
 ...
 */
function verifyAppendOnly(db) {
    const triggers = db.prepare("SELECT name FROM sqlite_master WHERE type = 'trigger' AND tbl_name = 'mutation_ledger'").all();
...
function getDivergenceReconcileAttemptCount(db, normalizedPath) {
  ...
  `).get(DIVERGENCE_RECONCILE_REASON, targetPath);
    return row?.cnt ?? 0;
}
function hasDivergenceEscalationEntry(db, normalizedPath) {
    const targetPath = normalizePath(normalizedPath);
    if (targetPath.length === 0) {
        return false;
    }
    ...
    WHERE mutation_type = 'reindex'
      AND reason = ?
      AND json_extract(decision_meta, '$.normalizedPath') = ?
  `).get(DIVERGENCE_RECONCILE_ESCALATION_REASON, targetPath);
    return (row?.cnt ?? 0) > 0;
}
function buildDivergenceReconcilePolicy(normalizedPath, attemptsSoFar, maxRetries = DEFAULT_DIVERGENCE_RECONCILE_MAX_RETRIES) {
...
function buildDivergenceEscalationPayload(policy, variants) {
    return {
...
function recordDivergenceReconcileHook(db, input) {
    initLedger(db);
