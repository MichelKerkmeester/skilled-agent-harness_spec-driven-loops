...
import { updateEdge, getStaleEdges, countEdgesForNode, MAX_EDGES_PER_NODE, MAX_AUTO_STRENGTH, MAX_STRENGTH_INCREASE_PER_CYCLE, STALENESS_THRESHOLD_DAYS, DECAY_STRENGTH_AMOUNT, DECAY_PERIOD_DAYS, } from './causal-edges.js';
/* ───────────────────────────────────────────────────────────────
   2. CONSTANTS
----------------------------------------------------------------*/
 ...
 */
export function scanContradictions(database) {
    const pairs = [];
    try {
...
function checkSqliteVecAvailable(database) {
    try {
        const result = database.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='vec_memories'").get();
        return !!result;
    }
    catch (_error) {
        return false;
 ...
 */
function scanContradictionsVector(database) {
    const pairs = [];
  ...
  `).all();
    ...
    for (let i = 0; i < memories.length; i++) {
        for (let j = i + 1; j < memories.length; j++) {
            ...
            const similarity = computeCosineSimilarity(new Float32Array(memories[i].embedding.buffer, memories[i].embedding.byteOffset, memories[i].embedding.byteLength / 4), new Float32Array(memories[j].embedding.buffer, memories[j].embedding.byteOffset, memories[j].embedding.byteLength / 4));
 ...
 */
function scanContradictionsHeuristic(database) {
    const pairs = [];
    const memories = database.prepare(`
 ...
 */
export function hasNegationConflict(textA, textB) {
 ...
 */
export function buildContradictionClusters(database, pairs) {
    const clusters = [];
    for (const pair of pairs) {
        const memberIds = new Set([pair.memoryA.id, pair.memoryB.id]);
        ...
        `).all(String(seedId), String(seedId));
 ...
 */
export function runHebbianCycle(database) {
    let strengthened = 0;
    let decayed = 0;
    try {
      ...
      `).all(DECAY_PERIOD_DAYS, DECAY_PERIOD_DAYS);
 ...
 */
export function detectStaleEdges(_database) {
 ...
 */
export function checkEdgeBounds(nodeId) {
 ...
 */
export function runConsolidationCycle(database) {
 ...
 */
export function runConsolidationCycleIfEnabled(database) {
    ...
    `).run();
        const row = database.prepare('SELECT last_run_at FROM consolidation_state WHERE id = 1').get();
        if (row?.last_run_at) {
        ...
        SELECT CASE
          ...
          ELSE 0
        END AS is_due
      `).get(row.last_run_at, CONSOLIDATION_INTERVAL_DAYS);
            if (due.is_due !== 1) {
                if (beganImmediate) {
