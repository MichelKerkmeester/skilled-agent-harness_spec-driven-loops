...
import { getHistoryEventsForLineage, init as initHistory, recordHistory } from './history.js';
...
function getMemoryRow(database, memoryId) {
    const row = database.prepare('SELECT * FROM memory_index WHERE id = ?').get(memoryId);
    if (!row) {
...
function getSafeHistoryEvents(database, memoryId) {
    try {
...
function hasLogicalKeySeparatorCollision(...components) {
...
function buildLogicalKey(row) {
    ...
    if (hasLogicalKeySeparatorCollision(row.spec_folder, canonicalPath, anchorId)) {
        logger.warn(`[lineage-state] Logical key component contains '::'; using hashed structured key for spec_folder=${row.spec_folder}, path=${canonicalPath}, anchor=${anchorId}`);
...
function getLineageRow(database, memoryId) {
...
function getActiveProjection(database, logicalKey) {
    const row = database.prepare(`
    SELECT *
...
function getLatestLineageRowForLogicalKey(database, logicalKey) {
    const row = database.prepare(`
    SELECT *
    FROM memory_lineage
    WHERE logical_key = ?
...
function loadLineageRowsForMemory(database, memoryId) {
...
function isLogicalVersionConflict(error) {
    ...
    const code = typeof error === 'object' && error !== null && typeof Reflect.get(error, 'code') === 'string'
        ...
        : '';
    return ((code.includes('SQLITE_CONSTRAINT') || message.includes('SQLITE_CONSTRAINT') || message.includes('UNIQUE constraint failed'))
        && message.includes('memory_lineage.logical_key')
        && message.includes('memory_lineage.version_number'));
}
function buildMetadata(row, actor, historyEvents = []) {
...
function resolveSnapshotFromLineageRow(database, row) {
...
function upsertActiveProjection(database, logicalKey, rootMemoryId, activeMemoryId, updatedAt) {
    ...
    database.prepare('DELETE FROM active_memory_projection WHERE active_memory_id = ? AND logical_key != ?').run(activeMemoryId, logicalKey);
    database.prepare(`
...
function bindHistory(database) {
    if (typeof database.exec === 'function') {
        initHistory(database);
    }
}
function markHistoricalPredecessor(database, memoryId, updatedAt) {
    database.prepare(`
    UPDATE memory_index
...
function insertAppendOnlyMemoryIndexRow(params) {
  ...
  `).run(parsed.specFolder, filePath, canonicalFilePath, parsed.title, JSON.stringify(parsed.triggerPhrases), importanceWeight, now, now, embeddingsProvider.getModelName(), embedding ? now : null, embeddingStatus, embeddingFailureReason, encodingIntent ?? 'document', parsed.documentType || 'memory', specLevel, parsed.content, parsed.qualityScore ?? 0, JSON.stringify(parsed.qualityFlags ?? []));
...
function getExistingLineageTransition(database, memoryId) {
 ...
 */
function validateTransitionInput(transitionEvent, predecessorMemoryId, validFrom, predecessor) {
    ...
    if (predecessor && new Date(validFrom).getTime() < new Date(predecessor.valid_from).getTime()) {
        throw new Error(`E_LINEAGE: valid_from (${validFrom}) is earlier than predecessor valid_from (${predecessor.valid_from})`);
    }
}
/**
 ...
 */
export function seedLineageFromCurrentState(database, memoryId, options = {}) {
    ...
    `).run(memoryId, logicalKey, memoryId, validFrom, options.transitionEvent ?? 'BACKFILL', actor, buildMetadata(row, actor, historyEvents));
 ...
 */
export function recordLineageTransition(database, memoryId, options = {}) {
    bindHistory(database);
    ensureLineageTables(database);
    const cached = getExistingLineageTransition(database, memoryId);
    if (cached)
    ...
    for (let attempt = 0; attempt <= MAX_LINEAGE_VERSION_RETRIES; attempt += 1) {
        ...
        const recordTransitionTx = database.transaction(() => {
            ...
            if (predecessorMemoryId != null) {
                ...
                if (predecessor) {
                    if (predecessor.logical_key !== rowLogicalKey) {
                        throw new Error(`E_LINEAGE: predecessor ${predecessorMemoryId} logical identity ${predecessor.logical_key} ` +
                    ...
                    if (seeded.logicalKey !== rowLogicalKey) {
                        throw new Error(`E_LINEAGE: predecessor ${predecessorMemoryId} logical identity ${seeded.logicalKey} ` +
                            `does not match memory ${memoryId} logical identity ${rowLogicalKey}`);
                    }
            ...
            if (predecessorMemoryId != null) {
                if (predecessor && predecessor.valid_to) {
                    logger.warn(`Predecessor ${predecessorMemoryId} already superseded (valid_to: ${predecessor.valid_to}). ` +
      ...
      `).run(memoryId, logicalKey, versionNumber, rootMemoryId, predecessorMemoryId, validFrom, transitionEvent, actor, buildMetadata(row, actor, historyEvents));
        ...
        try {
            ...
            logger.warn(`Retrying lineage insert for memory ${memoryId} after logical_key/version_number conflict ` +
 ...
 */
export function createAppendOnlyMemoryRecord(params) {
...
function resolveLogicalKey(database, memoryId) {
 ...
 */
export function inspectLineageChain(database, memoryId) {
    bindHistory(database);
    ensureLineageTables(database);
 ...
 */
export function summarizeLineageInspection(database, memoryId) {
    bindHistory(database);
    ensureLineageTables(database);
    const lineage = loadLineageRowsForMemory(database, memoryId);
    if (!lineage) {
 ...
 */
export function resolveActiveLineageSnapshot(database, memoryId) {
    ensureLineageTables(database);
    const lineage = loadLineageRowsForMemory(database, memoryId);
    if (!lineage) {
        return null;
 ...
 */
export function resolveLineageAsOf(database, memoryId, asOf) {
    ensureLineageTables(database);
    const lineage = loadLineageRowsForMemory(database, memoryId);
    if (!lineage) {
        return null;
    ...
    const row = [...lineage.rows].reverse().find((candidate) => (candidate.valid_from <= asOfTimestamp
        && (candidate.valid_to == null || candidate.valid_to > asOfTimestamp))) ?? null;
    return row ? resolveSnapshotFromLineageRow(database, row) : null;
}
/**
 ...
 */
export function validateLineageIntegrity(database) {
    ensureLineageTables(database);
    const issues = [];
    const missingPredecessors = [];
    const duplicateActiveLogicalKeys = [];
  ...
  `).all();
    for (const issue of orphanPredecessors) {
        ...
        issues.push(`Lineage ${issue.logical_key} version ${issue.memory_id} references missing predecessor ${issue.predecessor_memory_id}`);
    }
  ...
  `).all();
    ...
    const activeProjectionCount = database.prepare('SELECT COUNT(*) AS total FROM active_memory_projection').get().total;
    const lineageRowCount = database.prepare('SELECT COUNT(*) AS total FROM memory_lineage').get().total;
    return {
 ...
 */
export function backfillLineageState(database, options = {}) {
    bindHistory(database);
    ensureLineageTables(database);
  ...
  `).all();
    ...
    const execute = database.transaction(() => {
        ...
        `).run(row.id, logicalKey, index + 1, group[0].id, predecessor?.id ?? null, successor?.id ?? null, validFrom, validTo, actor, buildMetadata(row, actor, historyEvents));
                if (predecessor) {
                    markHistoricalPredecessor(database, predecessor.id, validFrom);
                }
            }
            const latest = group[group.length - 1];
            upsertActiveProjection(database, logicalKey, group[0].id, latest.id, normalizeTimestamp(latest.updated_at ?? latest.created_at));
        }
    });
    execute();
    return {
 ...
 */
export function getActiveProjectionRow(database, memoryId) {
    const lineage = loadLineageRowsForMemory(database, memoryId);
    if (!lineage) {
        return null;
    }
 ...
 */
export function getLatestLineageForMemory(database, memoryId) {
    const lineage = loadLineageRowsForMemory(database, memoryId);
 ...
 */
export function recordLineageVersion(database, params) {
    if (typeof database.exec !== 'function') {
 ...
 * @returns Active lineage snapshot when one exists.
 */
export function getActiveMemoryProjection(database, target) {
 ...
 * @returns Snapshot active at the requested time when one exists.
 */
export function resolveMemoryAsOf(database, target) {
 ...
 * @returns Backfill summary including scanned and seeded rows.
 */
export function runLineageBackfill(database, options = {}) {
 ...
 */
export function benchmarkLineageWritePath(database, options) {
    bindHistory(database);
    ensureLineageTables(database);
...
//# sourceMappingURL=lineage-state.js.map