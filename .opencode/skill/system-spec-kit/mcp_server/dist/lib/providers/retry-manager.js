...
function getRetryStatus(memory) {
...
function getRetryCountValue(memory) {
    return Number(memory.retry_count ?? memory.retryCount ?? 0) || 0;
}
function getLastRetryAtValue(memory) {
...
function detectEmbeddingProviderName(source) {
    ...
    if (lower.includes('azure'))
        return 'azure-openai';
    if (lower.includes('openai') || lower.includes('text-embedding'))
        return 'openai';
    if (lower.includes('system'))
        return 'system';
    return null;
}
function shouldSanitizeEmbeddingFailure(message) {
    const lower = message.toLowerCase();
    return (/^embedding_[a-z_]+/.test(lower) ||
...
function inferEmbeddingFailure(message) {
    ...
    if (lower.includes('embedding_rate_limit') ||
        lower.includes('rate limit') ||
        lower.includes('too many requests') ||
        lower.includes('429')) {
        return {
...
function sanitizeEmbeddingFailure(raw, options = {}) {
...
export function sanitizeEmbeddingFailureMessage(raw, options = {}) {
    const failure = sanitizeEmbeddingFailure(raw, options);
    return failure.publicMessage.length > 0 ? failure.publicMessage : null;
}
export function sanitizeAndLogEmbeddingFailure(context, raw, options = {}) {
    const failure = sanitizeEmbeddingFailure(raw, options);
...
function claimRetryCandidate(memory) {
...
let providerCircuitOpenedAt = null;
function isProviderCircuitOpen() {
    if (providerCircuitOpenedAt === null)
        return false;
...
function applyRetryHealthTransition(previousStatus, nextStatus) {
...
function refreshRetryHealthSnapshot(stats) {
...
function isEligibleForRetry(row, now) {
    ...
    if (row.embedding_status === 'retry' && row.last_retry_at) {
        ...
        const retryCount = row.retry_count ?? row.retryCount ?? 0;
 ...
 */
function getEmbeddingRetryStats() {
    return {
...
/* ───────────────────────────────────────────────────────────────
   4. RETRY OPERATIONS
──────────────────────────────────────────────────────────────── */
async function retryEmbedding(id, content, claimedPreviousStatus = null) {
    const db = vectorIndex.getDb();
    if (!db)
    ...
    try {
        ...
        const previousStatus = claimedPreviousStatus ?? (typeof memory.embedding_status === 'string'
            ? memory.embedding_status
            : typeof memory.embeddingStatus === 'string'
                ? memory.embeddingStatus ?? null
                : null);
        ...
        let embedding = null;
        if (cachedEmbedding) {
            embedding = new Float32Array(cachedEmbedding.buffer, cachedEmbedding.byteOffset, Math.floor(cachedEmbedding.byteLength / Float32Array.BYTES_PER_ELEMENT));
        }
        else {
            ...
            try {
                ...
                const sanitizedError = sanitizeAndLogEmbeddingFailure(`[retry-manager] Embedding retry failed for #${id}`, providerError, { provider: modelId, force: true }) ?? 'EMBEDDING_PROVIDER_ERROR (provider=unknown, type=provider_error)';
            ...
            if (embedding) {
                ...
                storeEmbedding(db, contentHash, modelId, Buffer.from(embedding.buffer, embedding.byteOffset, embedding.byteLength), embedding.length);
            }
            else {
                recordProviderFailure();
            }
      ...
      `).run(now, now, id);
        ...
        recordProviderFailure();
        // Security: raw provider errors sanitized before persistence/response
        const sanitizedMessage = sanitizeAndLogEmbeddingFailure(`[retry-manager] Unexpected retry error for #${id}`, error, { provider: null, force: true }) ?? 'EMBEDDING_PROVIDER_ERROR (provider=unknown, type=provider_error)';
...
function markAsFailed(id, reason) {
    ...
    const previousStatus = memory && typeof memory.embedding_status === 'string'
        ? memory.embedding_status
        : memory && typeof memory.embeddingStatus === 'string'
            ? memory.embeddingStatus ?? null
            : null;
    db.prepare(`
    UPDATE memory_index
...
async function processRetryQueue(limit = 3, contentLoader = null) {
    ...
    for (const memory of queue) {
        ...
        details.push({ id: memory.id, ...result });
    }
    return results;
}
/* ───────────────────────────────────────────────────────────────
...
──────────────────────────────────────────────────────────────── */
function startBackgroundJob(options = {}) {
    ...
    runBackgroundJob(config.batchSize).catch((err) => {
...
async function runBackgroundJob(batchSize = BACKGROUND_JOB_CONFIG.batchSize) {
...
async function claimAndRetryEmbedding(id, content, expectedStatus = 'pending') {
    const memory = vectorIndex.getMemory(id);
    if (!memory) {
        return { success: false, error: 'Memory not found' };
    }
