 ...
 */
export function normalizeScopeContext(input) {
    return {
 ...
 */
export function isScopeEnforcementEnabled() {
 ...
 */
export function isGovernanceGuardrailsEnabled() {
 ...
 */
export function requiresGovernedIngest(input) {
 ...
 */
export function validateGovernedIngest(input) {
    const scope = normalizeScopeContext(input);
    const issues = [];
    ...
    const retentionPolicy = input.retentionPolicy === 'ephemeral' || input.retentionPolicy === 'shared'
    ...
    if (!scope.tenantId)
        issues.push('tenantId is required for governed ingest');
    if (!scope.sessionId)
        issues.push('sessionId is required for governed ingest');
    if (!scope.userId && !scope.agentId)
        issues.push('userId or agentId is required for governed ingest');
    if (!provenanceSource)
        issues.push('provenanceSource is required for governed ingest');
    if (!provenanceActor)
        issues.push('provenanceActor is required for governed ingest');
 ...
 */
export function buildGovernancePostInsertFields(decision) {
    return {
 ...
 */
export function ensureGovernanceRuntime(database) {
 ...
 */
export function recordGovernanceAudit(database, entry) {
  ...
  `).run(entry.action, entry.decision, entry.memoryId ?? null, entry.logicalKey ?? null, scope.tenantId ?? null, scope.userId ?? null, scope.agentId ?? null, scope.sessionId ?? null, scope.sharedSpaceId ?? null, entry.reason ?? null, entry.metadata ? JSON.stringify(entry.metadata) : null);
}
function matchesExactScope(rowValue, requestedValue) {
    if (!requestedValue)
        return true;
 ...
 */
export function hasScopeConstraints(scope) {
    return Boolean(scope.sharedSpaceId
        || scope.tenantId
        || scope.userId
        || scope.agentId
        || scope.sessionId);
...
function buildGovernanceAuditWhereClause(filters) {
...
function hasGovernanceAuditFilters(filters) {
    const normalized = normalizeScopeContext(filters);
 ...
 */
export function createScopeFilterPredicate(scope, allowedSharedSpaceIds) {
 ...
 */
export function reviewGovernanceAudit(database, filters = {}) {
    ensureGovernanceRuntime(database);
  ...
  `).all(...params, ...params);
    ...
    return {
        ...
        summary: {
            ...
            byDecision: Object.fromEntries(byDecisionRows.map((row) => [row.aggregate_key, row.aggregate_count])),
 ...
 */
export function benchmarkScopeFilter(rows, scope, options = {}) {
 ...
 */
export function filterRowsByScope(rows, scope, allowedSharedSpaceIds) {
    return rows.filter(createScopeFilterPredicate(scope, allowedSharedSpaceIds));
}
//# sourceMappingURL=scope-governance.js.map