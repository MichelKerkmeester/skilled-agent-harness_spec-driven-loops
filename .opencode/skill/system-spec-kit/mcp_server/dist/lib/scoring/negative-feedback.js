 ...
 */
export function computeConfidenceMultiplier(negativeCount, lastNegativeAt) {
 ...
 */
export function applyNegativeFeedback(score, negativeCount, lastNegativeAt) {
...
function ensureNegativeFeedbackTable(db) {
    db.exec(NEGATIVE_FEEDBACK_TABLE_SQL);
    db.exec(NEGATIVE_FEEDBACK_INDEX_SQL);
}
/** Record one negative validation event for a memory. */
export function recordNegativeFeedbackEvent(db, memoryId, atMs = Date.now()) {
    ensureNegativeFeedbackTable(db);
    db.prepare('INSERT INTO negative_feedback_events (memory_id, created_at_ms) VALUES (?, ?)').run(memoryId, atMs);
}
/**
 * Batch-load negative feedback stats for a set of memory IDs.
 * Returns an empty map when no IDs are provided.
 */
export function getNegativeFeedbackStats(db, memoryIds) {
    const stats = new Map();
    ...
    const rows = db.prepare(`SELECT memory_id, COUNT(*) AS negative_count, MAX(created_at_ms) AS last_negative_at
     FROM negative_feedback_events
     WHERE memory_id IN (${placeholders})
     GROUP BY memory_id`).all(...uniqueIds);
...
//# sourceMappingURL=negative-feedback.js.map