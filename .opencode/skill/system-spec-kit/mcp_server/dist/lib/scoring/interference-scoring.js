...
function getMemoryIndexScopeInfo(database) {
    const columns = database.prepare('PRAGMA table_info(memory_index)').all();
    const columnSet = new Set(columns
        .map((column) => (typeof column.name === 'string' ? column.name : ''))
        .filter(Boolean));
    return {
        hasActiveProjection: Boolean(database.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'active_memory_projection'").get()),
        hasArchivedColumn: columnSet.has('is_archived'),
        hasImportanceTierColumn: columnSet.has('importance_tier'),
    };
}
function buildRetrievableMemoryPredicates(alias, scopeInfo) {
 ...
 */
export function computeInterferenceScore(database, memoryId, specFolder, threshold = INTERFERENCE_SIMILARITY_THRESHOLD) {
    if (!specFolder)
        return 0;
    ...
    const targetRow = database.prepare(`SELECT m.title, m.trigger_phrases
     FROM memory_index m
    ...
    const siblings = database.prepare(`SELECT m.id, m.title, m.trigger_phrases
     FROM memory_index m
     ${scopeInfo.hasActiveProjection ? 'JOIN active_memory_projection p ON p.active_memory_id = m.id' : ''}
     WHERE m.spec_folder = ?
       AND m.id != ?
       AND ${siblingPredicates.join('\n       AND ')}`).all(specFolder, memoryId);
 ...
 */
export function computeInterferenceScoresBatch(database, memoryIds, 
// Threshold was hardcoded to
// INTERFERENCE_SIMILARITY_THRESHOLD. Now configurable for testing/tuning.
threshold = INTERFERENCE_SIMILARITY_THRESHOLD) {
    ...
    const rows = database.prepare(`SELECT m.id, m.spec_folder, m.title, m.trigger_phrases
     FROM memory_index m
     ${scopeInfo.hasActiveProjection ? 'JOIN active_memory_projection p ON p.active_memory_id = m.id' : ''}
    ...
    for (const [folder, folderRows] of byFolder) {
        ...
        const allInFolder = database.prepare(`SELECT m.id, m.title, m.trigger_phrases
       FROM memory_index m
       ${scopeInfo.hasActiveProjection ? 'JOIN active_memory_projection p ON p.active_memory_id = m.id' : ''}
       WHERE m.spec_folder = ?
 ...
 */
export function applyInterferencePenalty(score, interferenceScore) {
    // Graduated: default-ON. Set SPECKIT_INTERFERENCE_SCORE=false to disable.
    if (process.env.SPECKIT_INTERFERENCE_SCORE?.toLowerCase() === 'false')
        return score;
    if (!interferenceScore || interferenceScore <= 0)
        return score;
    const penalty = INTERFERENCE_PENALTY_COEFFICIENT * interferenceScore;
    return Math.min(1, Math.max(0, score + penalty));
}
//# sourceMappingURL=interference-scoring.js.map