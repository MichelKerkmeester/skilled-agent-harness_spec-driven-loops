...
let fsrsSchedulerPromise = null;
let fsrsSchedulerLoadError = null;
async function loadFsrsScheduler() {
    ...
    const loadPromise = (async () => {
        try {
            fsrsScheduler = await import('../cognitive/fsrs-scheduler.js');
...
export const RECENCY_SCALE_DAYS = 1 / DECAY_RATE;
...
const RETRIEVABILITY_TIER_MULTIPLIER = {
    constitutional: 0.1,
    critical: 0.3,
    important: 0.5,
    normal: 1.0,
...
const CLASSIFICATION_CONTEXT_STABILITY_MULTIPLIER = {
    decision: Infinity,
    research: 2.0,
    implementation: 1.0,
    discovery: 1.0,
...
const CLASSIFICATION_TIER_STABILITY_MULTIPLIER = {
...
function applyClassificationDecayFallback(stability, contextType, importanceTier) {
...
export const CITATION_DECAY_RATE = 0.1;
export const CITATION_MAX_DAYS = 90;
...
export const DOCUMENT_TYPE_MULTIPLIERS = {
    spec: 1.4,
    decision_record: 1.4,
    plan: 1.3,
    tasks: 1.1,
...
export const PATTERN_ALIGNMENT_BONUSES = {
    exact_match: 0.3,
    partial_match: 0.15,
 ...
 */
function parseLastAccessed(value) {
    if (value == null)
        return null;
    ...
    if (!isNaN(asNum) && asNum > 1e12)
        return asNum; // Reasonable ms epoch (after ~2001)
    return null;
}
/**
 ...
 */
export function calculateRetrievabilityScore(row) {
    const scheduler = getFsrsScheduler();
    let stability = row.stability || 1.0;
    if (!isFinite(stability))
        stability = 1.0;
    const lastReview = row.lastReview || row.last_review || row.updated_at || row.created_at;
 ...
 */
export function calculateUsageScore(accessCount) {
 ...
 */
export function calculateImportanceScore(tier, baseWeight) {
 ...
 */
export function calculateCitationScore(row) {
    ...
    if (isNaN(timestamp))
        return 0; // No valid citation date → score 0
    const elapsedMs = Date.now() - timestamp;
    const elapsedDays = Math.max(0, elapsedMs / (1000 * 60 * 60 * 24));
 ...
 */
export function calculatePatternScore(row, options = {}) {
    let score = 0;
    ...
    if (row.anchors && options.anchors) {
        const rowAnchors = Array.isArray(row.anchors)
            ? row.anchors.filter((a) => typeof a === 'string')
            : typeof row.anchors === 'string' ? [row.anchors] : [];
        const queryAnchors = Array.isArray(options.anchors) ? options.anchors : [options.anchors];
        const anchorMatches = queryAnchors.filter(qa => rowAnchors.some(ra => ra && qa && ra.toLowerCase().includes(qa.toLowerCase())));
 ...
 */
export function calculateRecencyScore(timestamp, tier = 'normal') {
 ...
 */
export function getTierBoost(tier) {
 ...
 */
function applyPostProcessingAndObserve(composite, row, queryIdPrefix) {
    // Apply the document type multiplier.
    const docType = row.document_type || 'memory';
    ...
    const interferenceScore = row.interference_score || 0;
    ...
    catch (_err) { /* Telemetry must never affect scoring — fail-safe swallow */ }
    return finalScore;
}
// ───────────────────────────────────────────────────────────────
 ...
 */
export function calculateFiveFactorScore(row, options = {}) {
    const rawWeights = { ...FIVE_FACTOR_WEIGHTS, ...options.weights };
    ...
    const weights = Math.abs(wSum - 1.0) > 0.001 && wSum > 0
        ? {
    ...
    const composite = (temporalScore * weights.temporal +
 ...
 * @returns Composite score 0-1
 */
export function calculateCompositeScore(row, options = {}) {
    ...
    const composite = (similarity * weights.similarity +
 ...
 */
export function applyFiveFactorScoring(results, options = {}) {
 ...
 * @returns Scored and sorted results with composite_score and _scoring breakdown
 */
export function applyCompositeScoring(results, options = {}) {
    if (options.use_five_factor_model) {
 ...
 */
export function getFiveFactorBreakdown(row, options = {}) {
    const rawWeights = { ...FIVE_FACTOR_WEIGHTS, ...options.weights };
    const wSum = rawWeights.temporal + rawWeights.usage + rawWeights.importance + rawWeights.pattern + rawWeights.citation;
    const weights = Math.abs(wSum - 1.0) > 0.001 && wSum > 0
        ? {
 ...
 */
export function getScoreBreakdown(row, options = {}) {
    if (options.use_five_factor_model) {
 ...
 */
export function isCompositeNormalizationEnabled() {
 ...
 */
export function normalizeCompositeScores(scores) {
    if (scores.length === 0)
        return [];
    if (!isCompositeNormalizationEnabled())
        return scores;
...
//# sourceMappingURL=composite-scoring.js.map