...
/* ───────────────────────────────────────────────────────────────
   3. CONSTANTS
──────────────────────────────────────────────────────────────── */
const ACTIVE_STATES = new Set(['queued', 'parsing', 'embedding', 'indexing']);
...
/* ───────────────────────────────────────────────────────────────
   4. HELPERS
──────────────────────────────────────────────────────────────── */
function toPublicPathLabel(filePath) {
...
function withBusyRetrySync(operation) {
    for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt += 1) {
        try {
            return operation();
        }
...
function ensureIngestJobsTable() {
    const db = requireDb();
    db.exec(`
...
function resetIncompleteJobsToQueued() {
    const db = requireDb();
...
async function createIngestJob(args) {
    ...
    await withBusyRetry(() => db.prepare(`
      INSERT INTO ingest_jobs (
        id, state, spec_folder, paths_json, files_total, files_processed, errors_json, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(args.id, 'queued', args.specFolder ?? null, JSON.stringify(normalizedPaths), normalizedPaths.length, 0, JSON.stringify([]), timestamp, timestamp));
    return {
        id: args.id,
        state: 'queued',
...
async function setIngestJobState(jobId, nextState) {
    const db = requireDb();
...
async function incrementProcessed(jobId) {
    const db = requireDb();
    const updatedAt = nowIso();
    await withBusyRetry(() => db.prepare(`
      UPDATE ingest_jobs
...
async function appendIngestError(jobId, filePath, error) {
    const db = requireDb();
    const current = getIngestJob(jobId);
    if (!current) {
        throw new Error(`Ingest job not found: ${jobId}`);
 ...
 */
export function getIngestProgressPercent(job) {
    if (job.filesTotal <= 0)
        return 0;
...
export function getIngestForecast(job) {
...
async function processQueuedJob(jobId) {
    ...
    if (!latest1 || TERMINAL_STATES.has(latest1.state))
        return;
    ...
    if (!latest2 || TERMINAL_STATES.has(latest2.state))
        return;
...
function enqueueIngestJob(jobId) {
...
/* ───────────────────────────────────────────────────────────────
   7. INITIALIZATION
──────────────────────────────────────────────────────────────── */
function initIngestJobQueue(config) {
