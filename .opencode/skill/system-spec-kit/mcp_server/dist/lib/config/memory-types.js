import { canClassifyAsSpecDocument, matchesSpecDocumentPath, } from './spec-doc-paths.js';
 ...
 */
export const HALF_LIVES_DAYS = Object.fromEntries(Object.entries(MEMORY_TYPES).map(([type, config]) => [type, config.halfLifeDays]));
 ...
 */
export const PATH_TYPE_PATTERNS = [
 ...
 */
export function getHalfLifeForType(typeName) {
    const config = MEMORY_TYPES[typeName.toLowerCase()];
 ...
 */
export function resolveSpecDocumentType(filePath) {
 ...
 */
export function getHalfLife(type) {
    if (!type || typeof type !== 'string') {
        return 60;
    }
...
export function isDecayEnabled(type) {
...
// 5. RESET-TO-DEFAULTS
// ───────────────────────────────────────────────────────────────
export function getDefaultHalfLives() {
    return {
        working: 1,
        episodic: 7,
        prospective: 14,
...
export const SPEC_DOCUMENT_CONFIGS = [
 ...
 */
export function inferDocumentTypeFromPath(filePath) {
    const normalizedPath = filePath.replace(/\\/g, '/');
    const BASENAME_BY_DOCUMENT_TYPE = {
        spec: 'spec.md',
        plan: 'plan.md',
        tasks: 'tasks.md',
 ...
 */
export function getSpecDocumentConfig(documentType) {
    return SPEC_DOCUMENT_CONFIGS.find(c => c.documentType === documentType) ?? null;
}
// ───────────────────────────────────────────────────────────────
// 6. HALF-LIFE VALIDATION
// ───────────────────────────────────────────────────────────────
export function validateHalfLifeConfig(config) {
    const errors = [];
