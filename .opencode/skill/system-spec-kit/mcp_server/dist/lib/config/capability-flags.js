...
const SUPPORTED_PHASES_ARRAY = ['baseline', 'lineage', 'graph', 'adaptive', 'scope-governance', 'shared-rollout'];
 ...
 */
const SPECKIT_PARSER_ENV = 'SPECKIT_PARSER';
...
const SUPPORTED_PHASES = new Set(SUPPORTED_PHASES_ARRAY);
function hasExplicitDisableFlag(flagNames) {
...
function normalizeIdentity(flagName, identity) {
...
function isMemoryRoadmapCapabilityEnabled(flagNames, identity, defaultValue = true) {
    const candidates = Array.isArray(flagNames) ? flagNames : [flagNames];
...
function getMemoryRoadmapPhase() {
    ...
    if (canonicalPhase && SUPPORTED_PHASES.has(canonicalPhase)) {
        return canonicalPhase;
    }
    const legacyPhase = process.env[LEGACY_PHASE_ENV]?.trim().toLowerCase();
    if (legacyPhase && SUPPORTED_PHASES.has(legacyPhase)) {
        return legacyPhase;
    }
    return 'shared-rollout';
}
/** Returns the full capability snapshot for memory roadmap controls. */
function getMemoryRoadmapCapabilityFlags(identity) {
    return {
        lineageState: isMemoryRoadmapCapabilityEnabled([CAPABILITY_ENV.lineageState, LEGACY_CAPABILITY_ENV.lineageState], identity),
        graphUnified: isMemoryRoadmapCapabilityEnabled([CAPABILITY_ENV.graphUnified, LEGACY_CAPABILITY_ENV.graphUnified], identity),
        adaptiveRanking: isMemoryRoadmapCapabilityEnabled([CAPABILITY_ENV.adaptiveRanking, LEGACY_CAPABILITY_ENV.adaptiveRanking], identity, false),
        scopeEnforcement: isMemoryRoadmapCapabilityEnabled([CAPABILITY_ENV.scopeEnforcement, LEGACY_CAPABILITY_ENV.scopeEnforcement], identity),
        governanceGuardrails: isMemoryRoadmapCapabilityEnabled([CAPABILITY_ENV.governanceGuardrails, LEGACY_CAPABILITY_ENV.governanceGuardrails], identity),
        ...
        sharedMemory: isMemoryRoadmapCapabilityEnabled([CAPABILITY_ENV.sharedMemory, LEGACY_CAPABILITY_ENV.sharedMemory], identity, false),
    };
}
/** Returns defaults consumed by telemetry/checkpoint paths for phase tracking. */
function getMemoryRoadmapDefaults(identity) {
    return {
...
SPECKIT_PARSER_ENV, };
//# sourceMappingURL=capability-flags.js.map