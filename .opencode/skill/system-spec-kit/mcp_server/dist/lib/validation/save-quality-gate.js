...
let configTableEnsuredForDb = null;
/**
 ...
 */
function loadActivationTimestampFromDb() {
    try {
 ...
 */
function persistActivationTimestampToDb(timestamp) {
    try {
        const db = vectorIndex.getDb();
        if (!db)
            return;
 ...
 */
function clearActivationTimestampFromDb() {
    try {
        const db = vectorIndex.getDb();
        if (!db)
            return;
 ...
 */
let qualityGateActivatedAt = null;
/**
 ...
 */
export function isWarnOnlyMode() {
 ...
 */
export function setActivationTimestamp(timestamp) {
 ...
 */
export function resetActivationTimestamp() {
 ...
 */
function ensureActivationTimestampInitialized() {
    if (qualityGateActivatedAt !== null)
        return;
 ...
 */
export function isSaveQualityGateExceptionsEnabled() {
...
export function countStructuralSignals(params) {
    ...
    if (params.specFolder && params.specFolder.trim().length > 0)
        count++;
    if (params.anchor && params.anchor.trim().length > 0)
        count++;
    return count;
}
/**
...
export function isShortCriticalException(params) {
    if (!isSaveQualityGateExceptionsEnabled())
        return false;
    // P1-3: Use shared resolver instead of hardcoded check
    if (!params.contextType || resolveCanonicalContextType(params.contextType) !== 'planning')
        return false;
...
export function isQualityGateExceptionEnabled() {
...
export function validateStructural(params) {
    ...
    if (!params.content || params.content.trim().length === 0) {
        ...
        if (exceptionApplies) {
            ...
            console.warn(`${SHORT_DECISION_EXCEPTION_LOG_PREFIX} | context_type=${params.contextType ?? 'unknown'} | content_length=${params.content.trim().length} | bypassing min-length check`);
        }
        else {
            reasons.push(`Content too short: ${params.content.trim().length} chars (min: ${MIN_CONTENT_LENGTH})`);
        }
    }
    if (!params.specFolder || params.specFolder.trim().length === 0) {
        ...
        reasons.push(`Invalid spec folder format: "${params.specFolder}" — must match pattern ${SPEC_FOLDER_PATTERN.source}`);
    }
    return {
 ...
 */
export function scoreTriggerQuality(triggerPhrases) {
 ...
 */
export function scoreLengthQuality(content) {
 ...
 */
export function scoreAnchorQuality(anchors) {
 ...
 */
export function scoreMetadataQuality(content) {
...
export function scoreContentQuality(params) {
    ...
    if (roundedDensity < SIGNAL_DENSITY_THRESHOLD) {
        reasons.push(`Signal density ${roundedDensity.toFixed(2)} below threshold ${SIGNAL_DENSITY_THRESHOLD}`);
 ...
 */
export function checkSemanticDedup(embedding, specFolder, findSimilar) {
    try {
...
export function runQualityGate(params) {
    ...
    let semanticDedup = null;
    if (params.embedding && params.findSimilar) {
        semanticDedup = checkSemanticDedup(params.embedding, params.specFolder, params.findSimilar);
    ...
    if (warnOnly && wouldReject) {
        console.warn(`[QUALITY-GATE] warn-only | score: ${contentQuality.signalDensity.toFixed(2)} | would-reject: true | reasons: [${allReasons.join(', ')}]`);
    }
...
//# sourceMappingURL=save-quality-gate.js.map