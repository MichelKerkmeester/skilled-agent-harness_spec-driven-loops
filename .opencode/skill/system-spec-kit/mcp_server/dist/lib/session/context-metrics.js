...
export function recordBootstrapEvent(source, durationMs, completeness) {
...
function computeGraphFreshness() {
    try {
        const stats = getGraphStats();
        if (stats.totalFiles === 0)
            return 0.0;
        if (!stats.lastScanTimestamp)
            return 0.5;
...
function computeContinuity() {
    if (specFolderTransitions <= 1)
        return 1.0;
    if (specFolderTransitions <= 3)
        return 0.5;
    return 0.0;
}
/**
 ...
 */
export function getLastToolCallAt() {
...
export function computeQualityScore() {
    ...
    const score = (factors.recency * 0.35 +
