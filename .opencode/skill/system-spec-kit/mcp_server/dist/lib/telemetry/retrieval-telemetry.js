...
import { sanitizeRetrievalTracePayload, } from './trace-schema.js';
 ...
 */
function isExtendedTelemetryEnabled() {
...
──────────────────────────────────────────────────────────────── */
function recordLatency(t, stage, ms) {
    if (!t.enabled)
        return;
...
function recordMode(t, mode, override, pressure, tokenUsageRatio) {
    if (!t.enabled)
        return;
...
function recordFallback(t, reason) {
    if (!t.enabled)
        return;
...
function recordQualityProxy(t, results, boostDelta, extractionCount) {
    if (!t.enabled)
        return;
...
function recordTracePayload(t, payload) {
    if (!t.enabled)
        return false;
...
function recordTransitionDiagnostics(t, transition) {
    if (!t.enabled)
        return;
    if (!transition) {
    ...
    t.transitionDiagnostics = {
        ...
        signalSources: Array.isArray(transition.signalSources)
            ? transition.signalSources.filter((signal) => typeof signal === 'string')
            : [],
        reason: typeof transition.reason === 'string' && transition.reason.length > 0 ? transition.reason : null,
    };
}
function recordGraphWalkDiagnostics(t, update) {
    if (!t.enabled)
        return;
    const current = t.graphWalkDiagnostics ?? {
        rolloutState: 'off',
...
function recordLifecycleForecastDiagnostics(t, forecast, context = {}) {
    if (!t.enabled)
        return;
    ...
    const riskSignals = Array.isArray(candidate.riskSignals)
        ? candidate.riskSignals.filter((signal) => typeof signal === 'string')
        : [];
    t.lifecycleForecastDiagnostics = {
        state: typeof context.state === 'string' && context.state.length > 0 ? context.state : null,
        progress: typeof context.progress === 'number' && Number.isFinite(context.progress)
            ? Math.max(0, Math.min(100, context.progress))
            : 0,
        filesProcessed: typeof context.filesProcessed === 'number' && Number.isFinite(context.filesProcessed)
            ? Math.max(0, Math.trunc(context.filesProcessed))
            : 0,
        filesTotal: typeof context.filesTotal === 'number' && Number.isFinite(context.filesTotal)
            ? Math.max(0, Math.trunc(context.filesTotal))
            : 0,
        etaSeconds: typeof candidate.etaSeconds === 'number' && Number.isFinite(candidate.etaSeconds)
            ? Math.max(0, candidate.etaSeconds)
            : null,
        etaConfidence: typeof candidate.etaConfidence === 'number' && Number.isFinite(candidate.etaConfidence)
            ? Math.max(0, Math.min(1, candidate.etaConfidence))
            : null,
        failureRisk: typeof candidate.failureRisk === 'number' && Number.isFinite(candidate.failureRisk)
...
function recordArchitecturePhase(t, update) {
    if (!t.enabled)
        return;
    if (update.phase) {
...
function recordGraphHealth(t, update) {
    if (!t.enabled)
        return;
    t.graphHealth = {
        ...t.graphHealth,
...
function recordAdaptiveEvaluation(t, update) {
    if (!t.enabled)
        return;
    t.adaptive = {
        ...t.adaptive,
 ...
 */
function computeQualityProxy(t) {
...
function normalizeGraphHealthMetrics(value) {
...
function summarizeGraphHealthDashboard(payloads) {
    ...
    for (const payload of payloads) {
        const graphHealth = normalizeGraphHealthMetrics(isObjectRecord(payload) ? payload.graphHealth : undefined);
        if (!graphHealth) {
            continue;
        }
...
function sampleTracePayloads(payloads, options = {}) {
    ...
    for (const payload of payloads) {
        const tracePayload = sanitizeRetrievalTracePayload(isObjectRecord(payload) ? payload.tracePayload : undefined);
        const graphHealth = normalizeGraphHealthMetrics(isObjectRecord(payload) ? payload.graphHealth : undefined);
...
function toJSON(t) {
    ...
    const tracePayload = sanitizeRetrievalTracePayload(t.tracePayload);
    const modePayload = {
...
//# sourceMappingURL=retrieval-telemetry.js.map