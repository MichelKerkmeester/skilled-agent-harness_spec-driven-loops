 ...
 */
function collectEvalChannelsFromRow(row) {
 ...
 */
function buildEvalChannelPayloads(rows) {
    ...
    for (const row of rows) {
        ...
        for (const channel of channels) {
            const bucket = byChannel.get(channel) ?? new Map();
    ...
    return Array.from(byChannel.entries()).map(([channel, idToScore]) => {
 ...
 */
function summarizeGraphWalkDiagnostics(rows) {
    let rolloutState = 'off';
...
//# sourceMappingURL=eval-channel-tracking.js.map