...
export function filterEntities(entities) {
    return entities.filter((entity) => {
        ...
        if (words.length > 0 && words.every((w) => isEntityDenied(w)))
            return false;
        return true;
    });
}
 ...
 */
export function storeEntities(db, memoryId, entities) {
    try {
...
export function refreshAutoEntitiesForMemory(db, memoryId, entities) {
    try {
 ...
 */
export function updateEntityCatalog(db, entities) {
    try {
        let upserted = 0;
        const selectStmt = db.prepare('SELECT id, aliases, memory_count FROM entity_catalog WHERE canonical_name = ?');
        const insertStmt = db.prepare(`
    ...
    `);
        const runInTransaction = db.transaction(() => {
            for (const entity of entities) {
                ...
                if (!canonical)
                    continue;
                const existing = selectStmt.get(canonical);
                if (existing) {
 ...
 */
export function rebuildEntityCatalog(db) {
    try {
    ...
    `);
        const runInTransaction = db.transaction(() => {
            ...
            for (const canonical of Array.from(aggregates.keys()).sort()) {
                ...
                insertStmt.run(canonical, JSON.stringify(Array.from(aggregate.aliases).sort()), aggregate.entityType, aggregate.memoryIds.size);
            }
        });
        runInTransaction();
 ...
 */
export function rebuildAutoEntities(db, options = {}) {
 ...
 */
function deduplicateEntities(raw) {
    const map = new Map();
