 ...
 */
export function loadOntologySchema(schemaPath) {
    ...
    try {
        ...
        const parsed = JSON.parse(envSchema);
        return normalizeOntologySchema(parsed);
    }
    catch (error) {
...
function normalizeOntologySchema(parsed) {
 ...
 */
export function validateExtraction(entity, relation, schema) {
    if (!isOntologyHooksEnabled()) {
        return true;
    }
    const entityLower = entity.toLowerCase();
    const relationLower = relation.toLowerCase();
    const entityAllowed = schema.entityTypes.some((t) => t.toLowerCase() === entityLower);
    const relationAllowed = schema.relationTypes.some((t) => t.toLowerCase() === relationLower);
    ...
    return schema.extractionRules.some((rule) => rule.entityType === entityLower && rule.relationType === relationLower);
}
//# sourceMappingURL=ontology-hooks.js.map