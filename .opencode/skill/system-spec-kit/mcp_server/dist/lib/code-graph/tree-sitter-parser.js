...
import { extractEdges, capturesToNodes } from './structural-indexer.js';
...
const SUPPORTED_LANGUAGES = ['javascript', 'typescript', 'python', 'bash'];
...
async function getLanguageGrammar(language) {
...
function extractName(node, language) {
    ...
    if (node.type === 'export_statement' || node.type === 'export_declaration') {
        ...
        if (decl)
            return extractName(decl, language);
        return null;
    }
...
function extractExtendsName(node) {
...
function extractImplementsNames(node) {
    for (const child of node.namedChildren) {
        if (child.type === 'class_heritage') {
            for (const clause of child.namedChildren) {
    ...
    if (parent?.type === 'decorated_definition') {
        for (const child of parent.namedChildren) {
            if (child.type === 'decorator') {
                ...
                if (nameNode)
                    decorators.push(nameNode.text.replace(/^@/, ''));
            }
        }
    }
    for (const child of node.namedChildren) {
...
function resolveKind(node, language) {
...
function emitImportCaptures(node, _language, lines, captures) {
    const sig = extractSignature(node, lines);
    const startLine = node.startPosition.row + 1;
    const endLine = node.endPosition.row + 1;
    function pushImport(name) {
        captures.push({
            name,
            kind: 'import',
            startLine, endLine,
    ...
    if (!emitted) {
        for (const child of node.namedChildren) {
            if (child.type === 'dotted_name' || child.type === 'identifier') {
                pushImport(child.text);
                emitted = true;
            }
        }
    }
}
function emitExportCaptures(node, language, lines, captures, parentClassName) {
    const sig = extractSignature(node, lines);
    const startLine = node.startPosition.row + 1;
    const endLine = node.endPosition.row + 1;
...
function walkAST(rootNode, language, lines) {
    const captures = [];
    const kindMap = getKindMap(language);
        ...
        if (nodeType === 'decorated_definition') {
            ...
            for (const child of node.namedChildren) {
                if (child.type === 'decorator') {
                    const nameNode = child.childForFieldName('name') ?? child.namedChildren[0];
                    if (nameNode)
                        decoratorNames.push(nameNode.text.replace(/^@/, ''));
                }
...
export class TreeSitterParser {
    parse(content, language) {
        ...
        try {
            ...
            const captures = walkAST(tree.rootNode, language, lines);
