...
function findBraceBlockEndLine(lines, startIndex) {
...
function findPythonBlockEndLine(lines, startIndex) {
...
function getCaptureFqName(capture) {
...
function extractDecoratorNames(line) {
    const matches = line.matchAll(/@([A-Za-z_][A-Za-z0-9_]*)/g);
    const names = [...matches].map(match => match[1]);
    return [...new Set(names)];
}
function consumeDecorators(pendingDecorators) {
...
function extractParameterCaptures(paramsText, parentName, lineNum, lineLength) {
    if (!paramsText.trim())
        return [];
...
function parseJsTs(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (funcMatch) {
            ...
            captures.push(...extractParameterCaptures(funcMatch[2] ?? '', getCaptureFqName(functionCapture), lineNum, line.length));
        }
        ...
        if (arrowMatch && !funcMatch) {
            ...
            captures.push(functionCapture);
            captures.push(...extractParameterCaptures(arrowMatch[2] ?? '', getCaptureFqName(functionCapture), lineNum, line.length));
        }
        ...
        const methodMatchWithModifier = currentClass
            ? line.match(/^\s*(?:(?:public|private|protected|static|readonly|abstract|async|override|get|set)\s+)+(?:\*\s*)?(#?\w+)\s*(?:<[^>]*>)?\s*\(([^)]*)\)\s*(?::\s*([^{]+))?\s*\{?/)
            : null;
        ...
        if (methodMatch && currentClass) {
            ...
            if (methodName !== 'constructor') {
                ...
                captures.push(...extractParameterCaptures(methodMatch[2] ?? '', getCaptureFqName(methodCapture), lineNum, line.length));
            }
        }
        // Interfaces
...
function parsePython(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (funcMatch) {
            ...
            const isClassMemberLevel = currentClass && indent > classIndent && (classMethodIndent === -1
            ...
            if (isClassMemberLevel && currentClass) {
                ...
                captures.push(methodCapture);
                captures.push(...extractParameterCaptures(funcMatch[3] ?? '', getCaptureFqName(methodCapture), lineNum, line.length));
            }
            else {
                currentClass = null;
                const functionCapture = {
                ...
                captures.push(functionCapture);
                captures.push(...extractParameterCaptures(funcMatch[3] ?? '', getCaptureFqName(functionCapture), lineNum, line.length));
            }
        }
...
function attachFilePath(result, filePath) {
...
export function capturesToNodes(captures, filePath, language, content) {
    const lines = content.split('\n');
    const moduleNode = {
...
export function extractEdges(nodes, contentLines, captures) {
    const edges = [];
    const nodesByName = new Map();
    const nodesByFqName = new Map();
    ...
    const captureByFqName = new Map();
    if (captures) {
        for (const capture of captures) {
            captureByFqName.set(getCaptureFqName(capture), capture);
        }
    }
    const preferredKinds = (name, kinds, excludeId) => {
    ...
    if (contentLines) {
        const callableNodes = nodes.filter(n => n.kind === 'function' || n.kind === 'method');
        ...
        for (const caller of callableNodes) {
            ...
            while ((match = callPattern.exec(bodyText)) !== null) {
                ...
                if (calledName === caller.name || seen.has(calledName))
                    continue;
                // Skip common JS/TS keywords that precede parens
                if (['if', 'for', 'while', 'switch', 'catch', 'return', 'throw', 'new', 'typeof', 'await', 'async', 'function', 'class', 'const', 'let', 'var'].includes(calledName))
                    continue;
                seen.add(calledName);
    ...
    if (captures) {
        for (const capture of captures) {
            if (!capture.decoratorNames?.length)
                continue;
...
export async function parseFile(filePath, content, language) {
    const startTime = Date.now();
    const contentHash = generateContentHash(content);
    try {
...
function normalizeGlobPath(pathValue) {
...
function shouldExcludePath(rootDir, fullPath, excludePatterns, isDirectory) {
...
function findFiles(rootDir, pattern, excludeGlobs, maxSize) {
    ...
    function walk(currentDir) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (targetExt && !entry.name.endsWith(targetExt))
                    continue;
                try {
...
export async function indexFiles(config) {
    const results = [];
    ...
    for (const result of results) {
        if (!testPattern.test(result.filePath))
            continue;
...
//# sourceMappingURL=structural-indexer.js.map