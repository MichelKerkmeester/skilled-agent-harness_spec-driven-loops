...
`;
function getCurrentFileMtimeMs(filePath) {
    try {
...
export function initDb(dbDir) {
    ...
    try {
        ...
        if (db) {
            ...
            catch { /* best effort cleanup for failed init */ }
        }
        db = null;
        dbPath = null;
        throw err;
...
function getMetadata(key) {
    const d = getDb();
    const row = d.prepare('SELECT value FROM code_graph_metadata WHERE key = ?').get(key);
...
export function getLastGitHead() {
    return getMetadata('last_git_head');
}
export function setLastGitHead(head) {
    setMetadata('last_git_head', head);
}
/** Insert or update a file record, returning the file ID */
export function upsertFile(filePath, language, contentHash, nodeCount, edgeCount, parseHealth, parseDurationMs) {
    const d = getDb();
    const now = new Date().toISOString();
    const fileMtimeMs = getCurrentFileMtimeMs(filePath) ?? 0;
    const existing = d.prepare('SELECT id FROM code_files WHERE file_path = ?').get(filePath);
    if (existing) {
        d.prepare(`
...
export function replaceNodes(fileId, nodes) {
    const d = getDb();
  ...
  `);
    const tx = d.transaction(() => {
        const oldSymbolRows = d.prepare('SELECT symbol_id FROM code_nodes WHERE file_id = ?').all(fileId);
      ...
      `).run(...oldSymbolIds, ...oldSymbolIds);
        ...
        for (const n of nodes) {
            insert.run(n.symbolId, fileId, n.filePath, n.fqName, n.kind, n.name, n.startLine, n.endLine, n.startColumn, n.endColumn, n.language, n.signature ?? null, n.docstring ?? null, n.contentHash);
        }
    });
    tx();
}
/** Batch insert edges (deletes edges from the source nodes first) */
export function replaceEdges(sourceIds, edges) {
    const d = getDb();
    const insert = d.prepare(`
...
export function isFileStale(filePath) {
    const d = getDb();
    const row = d.prepare('SELECT file_mtime_ms FROM code_files WHERE file_path = ?').get(filePath);
    if (!row)
        return true;
...
export function ensureFreshFiles(filePaths) {
...
export function getTrackedFiles() {
    const d = getDb();
    const rows = d.prepare('SELECT file_path FROM code_files').all();
    return rows.map((row) => row.file_path);
}
/** Remove a file and its nodes/edges from the graph */
export function removeFile(filePath) {
    const d = getDb();
    const file = d.prepare('SELECT id FROM code_files WHERE file_path = ?').get(filePath);
    if (!file)
        return;
...
export function queryOutline(filePath) {
    const d = getDb();
    const rows = d.prepare(`
...
export function queryStartupHighlights(limit = 5) {
    const d = getDb();
...
export function queryEdgesFrom(symbolId, edgeType) {
    const d = getDb();
    ...
    const edges = d.prepare(sql).all(...params);
    return edges.map(e => {
        const edge = rowToEdge(e);
        const targetRow = d.prepare('SELECT * FROM code_nodes WHERE symbol_id = ?').get(edge.targetId);
        return { edge, targetNode: targetRow ? rowToNode(targetRow) : null };
    });
}
/** Query: get edges to a symbol */
export function queryEdgesTo(symbolId, edgeType) {
    const d = getDb();
    ...
    return edges.map(e => {
        const edge = rowToEdge(e);
        const sourceRow = d.prepare('SELECT * FROM code_nodes WHERE symbol_id = ?').get(edge.sourceId);
...
export function getStats() {
    const d = getDb();
    const totalFiles = d.prepare('SELECT COUNT(*) as c FROM code_files').get().c;
    const totalNodes = d.prepare('SELECT COUNT(*) as c FROM code_nodes').get().c;
    const totalEdges = d.prepare('SELECT COUNT(*) as c FROM code_edges').get().c;
    const nodesByKind = {};
    const kindRows = d.prepare('SELECT kind, COUNT(*) as c FROM code_nodes GROUP BY kind').all();
    for (const r of kindRows)
        nodesByKind[r.kind] = r.c;
    const edgesByType = {};
    const typeRows = d.prepare('SELECT edge_type, COUNT(*) as c FROM code_edges GROUP BY edge_type').all();
    for (const r of typeRows)
        edgesByType[r.edge_type] = r.c;
    const parseHealthSummary = {};
    const healthRows = d.prepare('SELECT parse_health, COUNT(*) as c FROM code_files GROUP BY parse_health').all();
    for (const r of healthRows)
        parseHealthSummary[r.parse_health] = r.c;
    // Last scan timestamp
    const lastScan = d.prepare('SELECT MAX(indexed_at) as last FROM code_files').get();
...
export function getTokenUsageRatio(sessionMetrics) {
    if (!sessionMetrics)
        return 0;
...
//# sourceMappingURL=code-graph-db.js.map