...
// Internal helpers
// ───────────────────────────────────────────────────────────────
function getCurrentGitHead(rootDir) {
    try {
...
function partitionTrackedFiles(filePaths) {
    const existingFiles = [];
    const deletedFiles = [];
...
function cleanupDeletedTrackedFiles(filePaths) {
    for (const filePath of filePaths) {
        graphDb.removeFile(filePath);
    }
    return filePaths.length;
}
function appendCleanupReason(reason, removedDeletedCount) {
...
function detectState(rootDir) {
    const d = getDb();
    // Condition (a): Graph is empty
    const nodeCount = d.prepare('SELECT COUNT(*) as c FROM code_nodes').get().c;
    if (nodeCount === 0) {
...
async function indexWithTimeout(config, timeoutMs) {
    ...
    try {
        const results = await Promise.race([
            ...
            new Promise((_, reject) => {
                controller.signal.addEventListener('abort', () => reject(new Error(`Auto-indexing timed out after ${timeoutMs}ms`)));
            }),
        ]);
        // Persist results to DB
        for (const result of results) {
            try {
                const fileId = graphDb.upsertFile(result.filePath, result.language, result.contentHash, result.nodes.length, result.edges.length, result.parseHealth, result.parseDurationMs);
...
export async function ensureCodeGraphReady(rootDir, options = {}) {
 ...
 */
export function getGraphFreshness(rootDir) {
    try {
        const state = detectState(rootDir);
