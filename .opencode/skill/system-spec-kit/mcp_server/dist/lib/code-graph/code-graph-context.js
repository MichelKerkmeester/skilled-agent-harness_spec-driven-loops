...
import { resolveSeeds } from './seed-resolver.js';
...
export function buildContext(args) {
    ...
    if (resolvedAnchors.length === 0 && args.subject) {
        ...
        if (subjectRef)
            resolvedAnchors.push(subjectRef);
    }
...
function buildEmptyFallback(queryMode, budgetTokens) {
    return {
        queryMode,
        resolvedAnchors: [],
        graphContext: [],
...
function buildCombinedSummary(anchors, sections) {
    if (anchors.length === 0)
        return 'No anchors resolved.';
...
function suggestNextActions(anchors, sections, mode) {
    const actions = [];
...
function computeFreshness() {
    try {
        ...
        const row = d.prepare('SELECT MAX(indexed_at) as last FROM code_files').get();
...
function expandAnchor(anchor, mode, remainingMs) {
    ...
    switch (mode) {
        case 'neighborhood': {
            ...
            for (const edgeType of ['CALLS', 'IMPORTS', 'CONTAINS']) {
        ...
        case 'impact': {
            ...
            for (const edgeType of ['CALLS', 'IMPORTS']) {
...
function resolveSubjectToRef(subject) {
    try {
        const d = graphDb.getDb();
 ...
 */
function formatTextBrief(sections, budgetTokens, _anchors) {
...
//# sourceMappingURL=code-graph-context.js.map