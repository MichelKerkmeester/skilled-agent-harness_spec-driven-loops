...
export const NODE_MODULES = path.join(SERVER_DIR, 'node_modules');
export const LIB_DIR = path.join(import.meta.dirname, '..', 'lib');
export const SHARED_DIR = path.join(SERVER_DIR, '..', 'shared');
export function resolveDatabasePaths() {
    ...
    try {
        ...
        try {
            allowedPrefixes.push(fs.realpathSync(tmpDir));
        }
        catch { /* ignore */ }
    }
    ...
    if (!allowedPrefixes.some((prefix) => {
        ...
        throw new Error(`Database directory "${resolved}" is outside the project root and home directory. ` +
...
export const DATABASE_DIR = resolvedDatabasePaths.databaseDir;
export const DATABASE_PATH = resolvedDatabasePaths.databasePath;
export const DB_UPDATED_FILE = resolvedDatabasePaths.dbUpdatedFile;
// ────────────────────────────────────────────────────────────────
// 3. BATCH PROCESSING CONFIGURATION 
// ────────────────────────────────────────────────────────────────
const parsedBatchSize = parseInt(process.env.SPEC_KIT_BATCH_SIZE || '5', 10);
export const BATCH_SIZE = Number.isFinite(parsedBatchSize) && parsedBatchSize > 0 ? parsedBatchSize : 5;
const parsedBatchDelayMs = parseInt(process.env.SPEC_KIT_BATCH_DELAY_MS || '100', 10);
export const BATCH_DELAY_MS = Number.isFinite(parsedBatchDelayMs) && parsedBatchDelayMs > 0 ? parsedBatchDelayMs : 100;
// ────────────────────────────────────────────────────────────────
// 4. RATE LIMITING CONFIGURATION 
// ────────────────────────────────────────────────────────────────
export const INDEX_SCAN_COOLDOWN = 60000;
// ────────────────────────────────────────────────────────────────
...
// 6. PATH VALIDATION 
// ────────────────────────────────────────────────────────────────
export const DEFAULT_BASE_PATH = process.env.MEMORY_BASE_PATH || process.cwd();
...
// ────────────────────────────────────────────────────────────────
export const CONSTITUTIONAL_CACHE_TTL = 60000;
// ────────────────────────────────────────────────────────────────
// 8. COGNITIVE CONFIGURATION 
// ────────────────────────────────────────────────────────────────
function parseCognitiveConfig() {
    return loadCognitiveConfigFromEnv(process.env);
}
let _cognitiveConfig = null;
...
export const COGNITIVE_CONFIG = COGNITIVE_CONFIG_LAZY;
/* ───────────────────────────────────────────────────────────────
