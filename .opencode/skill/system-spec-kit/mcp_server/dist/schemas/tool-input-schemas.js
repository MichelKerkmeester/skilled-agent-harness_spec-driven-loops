...
const positiveIntMax = (max) => safeNumericPreprocess.pipe(z.number().int().min(1).max(max));
const boundedNumber = (min, max) => safeNumericPreprocess.pipe(z.number().min(min).max(max));
const optionalStringArray = z.array(z.string()).optional();
const PATH_TRAVERSAL_MESSAGE = 'Path must not contain traversal sequences';
const isSafePath = (value) => !value.includes('..') && !value.includes('\0');
...
export const TOOL_SCHEMAS = {
    ...
    memory_quick_search: memoryQuickSearchSchema,
    memory_match_triggers: memoryMatchTriggersSchema,
    memory_save: memorySaveSchema,
    ...
    memory_validate: memoryValidateSchema,
    memory_bulk_delete: memoryBulkDeleteSchema,
    checkpoint_create: checkpointCreateSchema,
    checkpoint_list: checkpointListSchema,
    checkpoint_restore: checkpointRestoreSchema,
    checkpoint_delete: checkpointDeleteSchema,
    task_preflight: taskPreflightSchema,
    task_postflight: taskPostflightSchema,
    memory_drift_why: memoryDriftWhySchema,
    memory_causal_link: memoryCausalLinkSchema,
    memory_causal_stats: memoryCausalStatsSchema,
    memory_causal_unlink: memoryCausalUnlinkSchema,
    eval_run_ablation: evalRunAblationSchema,
    eval_reporting_dashboard: evalReportingDashboardSchema,
    memory_index_scan: memoryIndexScanSchema,
    memory_get_learning_history: memoryGetLearningHistorySchema,
    memory_ingest_start: memoryIngestStartSchema,
    memory_ingest_status: memoryIngestStatusSchema,
    memory_ingest_cancel: memoryIngestCancelSchema,
...
export function formatZodError(toolName, error) {
    ...
    return new ToolSchemaValidationError(toolName, `Invalid arguments for "${toolName}". ${lines.join(' ')}`, {
        tool: toolName,
...
export function validateToolArgs(toolName, rawInput) {
...
//# sourceMappingURL=tool-input-schemas.js.map