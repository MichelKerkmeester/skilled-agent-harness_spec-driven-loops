...
let _vector_index = null;
let _checkpoints = null;
let _access_tracker = null;
let _mutation_ledger = null;
let _trigger_matcher = null;
let _core_index = null;
let _startup_checks = null;
let _causal_edges = null;
let _history = null;
let _schema_downgrade = null;
...
async function runStats() {
    ...
    const db = vectorIndex.getDb();
    // Total count
    const totalRow = db.prepare('SELECT COUNT(*) as count FROM memory_index').get();
    console.log(`\nMemory Database Statistics`);
    console.log(`${'-'.repeat(50)}`);
    console.log(`  Database:  ${databasePath}`);
    console.log(`  Total:     ${totalRow.count} memories`);
    // Tier distribution
    const tiers = db.prepare('SELECT importance_tier, COUNT(*) as count FROM memory_index GROUP BY importance_tier ORDER BY count DESC').all();
    console.log(`\n  Tier Distribution:`);
    ...
    // Top folders
    const folders = db.prepare('SELECT spec_folder, COUNT(*) as count FROM memory_index WHERE spec_folder IS NOT NULL GROUP BY spec_folder ORDER BY count DESC LIMIT 10').all();
    if (folders.length > 0) {
        console.log(`\n  Top Folders:`);
    ...
    try {
        ...
        try {
            const legacyVersion = db.prepare("SELECT value FROM config WHERE key = 'schema_version'").get();
            if (legacyVersion) {
    ...
    try {
        const chunkedParents = db.prepare("SELECT COUNT(*) as count FROM memory_index WHERE embedding_status = 'partial'").get();
        const childChunks = db.prepare('SELECT COUNT(*) as count FROM memory_index WHERE parent_id IS NOT NULL').get();
...
async function runBulkDelete() {
    ...
    if (skipCheckpoint)
        console.log(`  Checkpoint:  skipped (--skip-checkpoint)`);
    ...
    if (dryRun) {
        ...
        const sampleParams = [tier];
        if (specFolder) {
            sampleSql += ' AND spec_folder = ?';
            sampleParams.push(specFolder);
        }
        if (olderThanDays) {
            sampleSql += ` AND created_at < datetime('now', '-' || ? || ' days')`;
            sampleParams.push(parseInt(olderThanDays, 10));
        }
        sampleSql += ' ORDER BY created_at ASC LIMIT 10';
        const samples = db.prepare(sampleSql).all(...sampleParams);
        if (samples.length > 0) {
    ...
    if (olderThanDays) {
        ...
        selectParams.push(parseInt(olderThanDays, 10));
    }
    const toDelete = db.prepare(selectSql).all(...selectParams);
...
──────────────────────────────────────────────────────────────── */
async function runSchemaDowngrade() {
