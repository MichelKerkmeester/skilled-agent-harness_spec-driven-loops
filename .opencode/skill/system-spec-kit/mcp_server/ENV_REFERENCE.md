...
| `SPECKIT_CODE_GRAPH_INDEX_SKILLS` | `false` | boolean or csv | Maintainer opt-in for structural code graph scans to include `.opencode/skill/**`. Set `true` for all skills, or a comma-separated list such as `sk-code-review,sk-doc` for selected skills. Overridden by per-call `includeSkills` when provided. | `code_graph/lib/index-scope-policy.ts`, `code_graph/handlers/scan.ts` |
| `SPECKIT_CODE_GRAPH_INDEX_AGENTS` | `false` | boolean | Maintainer opt-in for structural code graph scans to include `.opencode/agent/**`. Overridden by per-call `includeAgents` when provided. | `code_graph/lib/index-scope-policy.ts`, `code_graph/handlers/scan.ts` |
| `SPECKIT_CODE_GRAPH_INDEX_COMMANDS` | `false` | boolean | Maintainer opt-in for structural code graph scans to include `.opencode/command/**`. Overridden by per-call `includeCommands` when provided. | `code_graph/lib/index-scope-policy.ts`, `code_graph/handlers/scan.ts` |
