# SPECKIT\_\* Environment Variable Reference

> Comprehensive reference for all `SPECKIT_*` environment variables used by the Spec Kit Memory MCP server.

All variables are optional. The server runs with sensible defaults when no variables are set. Variables use **graduated semantics** unless noted: they default to ON and are disabled by setting `=false`.
...
2. [Search Pipeline -- Core](#2-search-pipeline----core)
3. [Search Pipeline -- Fusion & Scoring](#3-search-pipeline----fusion--scoring)
4. [Search Pipeline -- Query Intelligence](#4-search-pipeline----query-intelligence)
5. [Graph](#5-graph)
6. [Graph -- Calibration](#6-graph----calibration)
7. [Cognitive](#7-cognitive)
8. [Feedback & Learning](#8-feedback--learning)
9. [Governance & Scope](#9-governance--scope)
10. [UX & Response Formatting](#10-ux--response-formatting)
11. [Evaluation & Telemetry](#11-evaluation--telemetry)
12. [Indexing](#12-indexing)
13. [Reranker](#13-reranker)
14. [Embedding](#14-embedding)
15. [Roadmap Phase Control (Legacy: HYDRA\_\*)](#15-roadmap-phase-control)
...
| `SPECKIT_VRULE_OPTIONAL` | `false` | boolean | When `true`, V-rule validation is bypassed if the module fails to load. Opt-in. | `handlers/v-rule-bridge.ts` |

## 2. Search Pipeline -- Core
...
## 3. Search Pipeline -- Fusion & Scoring
...
| `SPECKIT_RECENCY_FUSION_WEIGHT` | `0.07` | number | **New (Phase A-D).** Weight of recency signal in Stage 2 fusion scoring. | `lib/search/pipeline/stage2-fusion.ts` |
| `SPECKIT_RECENCY_FUSION_CAP` | `0.10` | number | **New (Phase A-D).** Maximum recency contribution cap in Stage 2 fusion. | `lib/search/pipeline/stage2-fusion.ts` |
| `SPECKIT_DOC_TYPE_WEIGHT_FACTOR` | `1.2` | number | **New (Phase A-D).** Proportional weight shift factor per document type in adaptive fusion (20% shift at 1.2). | `shared/algorithms/adaptive-fusion.ts` |
...
## 4. Search Pipeline -- Query Intelligence
...
| `SPECKIT_INTENT_AUTO_PROFILE` | `true` | boolean | **New (Phase A-D).** Intent-to-profile auto-routing: auto-selects response profile from classifyIntent() results. Graduated ON. | `lib/search/search-flags.ts` |
...
| `SPECKIT_GRAPH_WEIGHT_CAP` | `0.15` | number | **New (Phase A-D).** Maximum graph contribution in Stage 2 scoring. | `lib/search/graph-calibration.ts` |
...
| `SPECKIT_PRESSURE_POLICY` | `true` | boolean | Token-pressure policy for memory\_context responses. Graduated ON. | `lib/search/search-flags.ts` |
| `SPECKIT_AUTO_RESUME` | `true` | boolean | Automatic session resume context injection for memory\_context. Graduated ON. | `lib/search/search-flags.ts` |
| `SPECKIT_MEMORY_ADAPTIVE_MODE` | `shadow` | string | Adaptive ranking mode: `shadow` (compute but don't apply) or `promoted` (apply to ranking). | `lib/cognitive/adaptive-ranking.ts` |
...
These variables control memory roadmap capabilities. Each has a **legacy `SPECKIT_HYDRA_*` alias** that is checked as a fallback. Prefer the `SPECKIT_MEMORY_*` canonical names.
...
These variables are no longer actively used but may still be referenced in compatibility code.

| Variable | Status | Replacement | Notes |
|----------|--------|-------------|-------|
| `SPECKIT_EAGER_WARMUP` | **Deprecated** | (removed) | Embedding model now uses lazy loading exclusively. Compatibility flag only. |
| `SPECKIT_LAZY_LOADING` | **Deprecated** | (removed) | Lazy loading is now always enabled. Compatibility flag only. |
| `SPECKIT_SHADOW_SCORING` | **Deprecated** | `SPECKIT_SHADOW_FEEDBACK` | Shadow scoring flag has been removed; shadow evaluation uses SHADOW_FEEDBACK. |
| `SPECKIT_RSF_FUSION` | **Deprecated** | `SPECKIT_RRF` | Referenced in tests only; appears to be a legacy alias. |
