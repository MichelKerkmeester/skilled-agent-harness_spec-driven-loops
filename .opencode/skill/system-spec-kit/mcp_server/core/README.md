...
```
core/
├── index.ts       # Module aggregator (re-exports config + db-state) [source]
├── config.ts      # Configuration constants and path management [source]
├── db-state.ts    # Database state management and lifecycle [source]
└── README.md      # This file
```
...
└── db-state.js    # Compiled database state manager
```
