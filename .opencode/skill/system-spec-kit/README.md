...
| **Preflight**       | `detect_changes` (012/002, registered handler — tool-schema deferred per ADR-012-003) | Read-only diff -> affected-symbol mapping with hard refuse on stale graph |
...
_Documentation version: 3.1 | Last updated: 2026-04-25 | Skill version: 3.3.1.0 | Phase 012 adds the read-only `detect_changes` handler (registered, tool-schema deferred), the typed phase-DAG runner, edge `reason`/`step` plus `blast_radius` enrichment, Skill Advisor affordance evidence, and Memory causal trust display badges._
