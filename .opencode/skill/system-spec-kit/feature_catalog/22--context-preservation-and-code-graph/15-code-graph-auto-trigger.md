...
The exact recovery-routing payload is **handler-local** (shared vocabulary, per-handler shape — see `24-code-graph-readiness-contract.md`):

- `code_graph_query` blocked-read responses on `full_scan` readiness emit `fallbackDecision: { nextTool, reason, retryAfter? }`. Empty/stale full-scan states return `nextTool:"code_graph_scan"`, `reason:"full_scan_required"`, `retryAfter:"scan_complete"`. Selective-reindex and fresh paths emit no `fallbackDecision`. Readiness-crash (`freshness === 'error'`) states return `nextTool:"rg"`, `reason:"scan_failed"`.
- `code_graph_context` blocked-read responses on `full_scan` readiness use the same `fallbackDecision` contract. On a readiness-crash (`freshness === 'error'`), packet 016 (`016-degraded-readiness-envelope-parity`) makes context preserve the structured degraded envelope (`readiness`, `canonicalReadiness`, `trustState`, `graphAnswersOmitted: true`, plus an `rg` recovery signal) before any `buildContext()` attempt, so callers can distinguish "graph unavailable" from "internal handler bug." The exact field path for the `rg` recovery signal — whether it is exposed as `fallbackDecision: { nextTool: "rg", ... }` or under a different field — is documented in packet 016's `implementation-summary.md`. Catalog readers MUST consult that source before inferring the shape from this page.[^c016]
- `code_graph_status` (packet 014) does not return blocked-read payloads but exposes the same readiness vocabulary via `readiness`, `canonicalReadiness`, and `trustState`; packet 016 also pins that the readiness snapshot is preserved when `graphDb.getStats()` fails so the action-level surface survives DB-unavailable states.

[^c016]: See `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/015-mcp-runtime-stress-remediation/016-degraded-readiness-envelope-parity/implementation-summary.md` for the canonical field name. Until 016 lands, the contract defined in packet 011's `review-report.md` §3 / §7 Packet A is the binding expectation.
