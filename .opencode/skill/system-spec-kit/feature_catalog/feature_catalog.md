...
| `SPECKIT_TIER3_ROUTING` | `false` | boolean | `handlers/memory-save.ts`, `lib/routing/content-router.ts` | **Default OFF (opt-in).** Enables the live Tier 3 LLM classifier for ambiguous canonical save routing. When `true` and `LLM_REFORMULATION_ENDPOINT` is configured, `memory_save` wires Tier 3 into the 8-category content router; when disabled or unavailable, routing stays on Tier 1/Tier 2 with Tier 2 penalty fallback or refusal. Does not disable explicit `routeAs` overrides. |
