...
| File | Layer | Role |
|------|-------|------|
| `.opencode/skill/system-spec-kit/mcp_server/handlers/memory-search.ts` | Handler | Search handler entry point: parameter resolution, cache, session dedup, deep-mode expansion, result formatting |
| `.opencode/skill/system-spec-kit/mcp_server/lib/cache/tool-cache.ts` | Lib | Per-parameter-combination result caching via `toolCache.withCache()` |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/pipeline/index.ts` | Lib | Pipeline barrel export (`executePipeline`) |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/pipeline/orchestrator.ts` | Lib | 4-stage pipeline orchestration (sole runtime path) |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/artifact-routing.ts` | Lib | Artifact-class routing (spec/plan/tasks/checklist/memory) |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/chunk-reassembly.ts` | Lib | Chunk-to-parent reassembly post-pipeline |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/search-utils.ts` | Lib | Quality filtering, cache arg building, artifact routing helpers |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/intent-classifier.ts` | Lib | Intent detection for non-hybrid intent weights |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/session-boost.ts` | Lib | Session attention boost flag check |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/causal-boost.ts` | Lib | Causal neighbor boost flag check |
| `.opencode/skill/system-spec-kit/mcp_server/lib/search/search-flags.ts` | Lib | Feature flag registry |
| `.opencode/skill/system-spec-kit/mcp_server/lib/session/session-manager.ts` | Lib | Session lifecycle for cross-turn dedup |
| `.opencode/skill/system-spec-kit/mcp_server/lib/response/envelope.ts` | Lib | Response envelope formatting |
| `.opencode/skill/system-spec-kit/mcp_server/lib/feedback/feedback-ledger.ts` | Lib | Implicit feedback logging |
| `.opencode/skill/system-spec-kit/mcp_server/lib/eval/eval-logger.ts` | Lib | Evaluation event logger |
| `.opencode/skill/system-spec-kit/mcp_server/lib/telemetry/consumption-logger.ts` | Lib | Agent consumption logging |
| `.opencode/skill/system-spec-kit/mcp_server/lib/telemetry/retrieval-telemetry.ts` | Lib | Retrieval telemetry |
