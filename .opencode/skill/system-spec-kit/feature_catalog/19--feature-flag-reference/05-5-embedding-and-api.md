...
| `VOYAGE_API_KEY` | _(none)_ | string | `shared/embeddings/factory.ts` | API key for the Voyage AI embeddings and reranker provider. In `'auto'` mode, Voyage is preferred over OpenAI when this key is present, using `voyage-4` embeddings plus the Voyage `rerank-2.5` path. |
