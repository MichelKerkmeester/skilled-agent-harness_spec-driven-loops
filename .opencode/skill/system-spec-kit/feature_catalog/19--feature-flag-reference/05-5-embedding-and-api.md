...
|---|---|---|---|---|
| `COHERE_API_KEY` | _(none)_ | string | `lib/search/cross-encoder.ts` | API key for the Cohere reranker provider. Provider resolution checks `VOYAGE_API_KEY` first, then `COHERE_API_KEY`, then the local reranker when `RERANKER_LOCAL='true'`. |
...
These runtime overrides are implemented in retrieval and ranking code paths but were previously missing from this reference page.
