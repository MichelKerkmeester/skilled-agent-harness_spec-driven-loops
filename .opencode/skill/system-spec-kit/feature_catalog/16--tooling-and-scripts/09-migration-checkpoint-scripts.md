...
This document captures the implemented behavior, source references, and validation scope for Migration checkpoint scripts.
...
`create-checkpoint.ts` copies a target SQLite file into a timestamped checkpoint file, writes a JSON sidecar with schema version, size, note, and Hydra rollout metadata, and supports `--json` output for automation. `restore-checkpoint.ts` restores a checkpoint into a target database path, creates a timestamped pre-restore backup when replacing an existing file, and verifies that the restored file opens as SQLite before reporting success.
...
| `mcp_server/lib/config/capability-flags.ts` | Lib | Hydra roadmap metadata attached to checkpoint sidecars |
