...
exports.deriveParentPath = deriveParentPath;
exports.applyTreeThinning = applyTreeThinning;
/** Defines default thinning config. */
exports.DEFAULT_THINNING_CONFIG = {
    mergeThreshold: 200,
...
const token_estimate_1 = require("@spec-kit/shared/utils/token-estimate");
Object.defineProperty(exports, "estimateTokenCount", { enumerable: true, get: function () { return token_estimate_1.estimateTokenCount; } });
/**
 ...
 */
function buildMergedSummary(children) {
    return children
        .map((child) => {
 ...
 */
function resolveAction(tokenCount, memory, config) {
    if (memory) {
 ...
 */
function applyTreeThinning(files, config) {
    const cfg = { ...exports.DEFAULT_THINNING_CONFIG, ...config };
    const thinned = [];
    const toMerge = [];
    // Pass 1: classify each file
    for (const file of files) {
        const tokenCount = (0, token_estimate_1.estimateTokenCount)(file.content);
