...
import { persistCanonicalContinuityShadowArtifacts } from './canonical-continuity-shadow';
...
const WORKFLOW_CANONICAL_ROUTING_CATEGORIES = new Set<RoutingCategory>([
  'narrative_progress',
  'narrative_delivery',
  'decision',
  'handover_state',
...
const WORKFLOW_CANONICAL_MERGE_MODES = new Set<WorkflowCanonicalMergeMode>([
  'append-as-paragraph',
  'insert-new-adr',
  'append-table-row',
  'update-in-place',
...
const WORKFLOW_COMPACT_URL_RE = /https?:\/\/\S+|www\.\S+/gi;
const WORKFLOW_COMPACT_MARKDOWN_RE = /[`*_>#]/g;
const WORKFLOW_COMPACT_LIST_PREFIX_RE = /^\s*(?:[-*+]|\d+\.)\s+/u;
const WORKFLOW_COMPACT_DISCOURSE_RE = /\b(?:because|so that|which means|the reason|details|summary)\b/gi;
const WORKFLOW_CANONICAL_NEXT_ACTION_VERBS = new Set([
  'add',
  'align',
  'audit',
  'build',
...
const WORKFLOW_CANONICAL_SENTINELS = new Set(['', 'none', 'n/a', 'na']);
const WORKFLOW_PREFER_SOURCE_RUNTIME =
...
function normalizeWorkflowCanonicalMode(value: unknown): {
  mode: WorkflowCanonicalContinuityMode | null;
  downgraded: boolean;
} {
  if (typeof value !== 'string') {
    return { mode: null, downgraded: false };
  }

  switch (value.trim().toLowerCase()) {
    case 'shadow':
    case 'shadow_only':
      return { mode: 'shadow', downgraded: false };
    case 'off':
    case 'disabled':
    case 'false':
      return { mode: 'off', downgraded: false };
    case 'canonical':
    case 'live':
    case 'cutover':
    case 'on':
      return { mode: 'shadow', downgraded: true };
    default:
      return { mode: null, downgraded: false };
  }
}

function normalizeWorkflowCanonicalRouteAs(value: unknown): RoutingCategory | undefined {
  return typeof value === 'string' && WORKFLOW_CANONICAL_ROUTING_CATEGORIES.has(value as RoutingCategory)
    ? value as RoutingCategory
    : undefined;
}

function normalizeWorkflowCanonicalMergeMode(value: unknown): WorkflowCanonicalMergeMode | undefined {
  return typeof value === 'string' && WORKFLOW_CANONICAL_MERGE_MODES.has(value as WorkflowCanonicalMergeMode)
    ? value as WorkflowCanonicalMergeMode
    : undefined;
}

function coerceWorkflowCanonicalPacketLevel(specFolder: string): WorkflowCanonicalPacketLevel {
  const specMdPath = path.join(specFolder, 'spec.md');
  try {
    if (fsSync.existsSync(specMdPath)) {
      const header = fsSync.readFileSync(specMdPath, 'utf8').slice(0, 2048);
    ...
    // Fall through to sibling inference below.
  }

  try {
    const siblings = new Set(fsSync.readdirSync(specFolder).map((entry) => entry.toLowerCase()));
    if (siblings.has('decision-record.md')) {
      return 'L3';
    }
    if (siblings.has('checklist.md')) {
      return 'L2';
    }
  } catch {
    // Ignore and use the safest fallback.
  }

  return 'L1';
}

function compactWorkflowCanonicalText(value: unknown, fallback: string, enforceVerb = false): string {
  const raw = typeof value === 'string' ? value : '';
  const normalized = raw
    .replace(/\r?\n+/g, ' ')
    .replace(WORKFLOW_COMPACT_URL_RE, ' ')
    .replace(WORKFLOW_COMPACT_MARKDOWN_RE, ' ')
    .replace(WORKFLOW_COMPACT_DISCOURSE_RE, ' ')
    .replace(WORKFLOW_COMPACT_LIST_PREFIX_RE, '')
    .replace(/\s+/g, ' ')
    .trim()
    .replace(/[.!?]+$/g, '');

  const source = normalized.length > 0 ? normalized : fallback;
  const tokens = source.split(/\s+/).slice(0, 16);
  let compact = tokens.join(' ').slice(0, 96).trim();
  if (compact.length === 0) {
    compact = fallback;
  }

  if (enforceVerb) {
    const firstToken = compact.split(/\s+/)[0]?.toLowerCase() ?? '';
    if (!WORKFLOW_CANONICAL_NEXT_ACTION_VERBS.has(firstToken)) {
      compact = fallback;
    }
  }
...
function normalizeWorkflowCanonicalBlockers(value: unknown): string[] {
  if (typeof value !== 'string') {
    return [];
  }
  ...
  return value
    .split(/[;,]/)
    .map((entry) => compactWorkflowCanonicalText(entry, '').toLowerCase() === 'none'
      ? ''
      : compactWorkflowCanonicalText(entry, ''))
    .filter((entry) => !WORKFLOW_CANONICAL_SENTINELS.has(entry.toLowerCase()))
    .slice(0, 5);
}

function normalizeWorkflowCanonicalQuestionIds(value: unknown): string[] {
  ...
  for (const entry of value) {
    ...
    if (!/^Q[1-9][0-9]*$/.test(candidate) || seen.has(candidate)) {
      continue;
    }
    seen.add(candidate);
...
function normalizeWorkflowCanonicalCompletionPercent(value: unknown): number {
...
function buildWorkflowCanonicalChunkText(input: WorkflowCanonicalContinuityInput): string {
  const segments: string[] = [];
  if (input.summary.trim().length > 0) {
    segments.push(input.summary.trim());
  }
  if (typeof input.sessionData.LAST_ACTION === 'string' && input.sessionData.LAST_ACTION.trim().length > 0) {
    segments.push(`Recent action: ${input.sessionData.LAST_ACTION.trim()}`);
  }
  if (typeof input.sessionData.NEXT_ACTION === 'string' && input.sessionData.NEXT_ACTION.trim().length > 0) {
    segments.push(`Next safe action: ${input.sessionData.NEXT_ACTION.trim()}`);
  }
  for (const decision of input.decisionSummaries.slice(0, 3)) {
    if (decision.title) {
      segments.push(`Decision: ${decision.title}`);
    }
  ...
  for (const file of input.files.slice(0, 5)) {
    segments.push(file.description ? `${file.path}: ${file.description}` : file.path);
  }
  for (const observation of input.collectedData.observations ?? []) {
    if (typeof observation?.title === 'string' && observation.title.trim().length > 0) {
      segments.push(observation.title.trim());
    }
    if (typeof observation?.narrative === 'string' && observation.narrative.trim().length > 0) {
      segments.push(observation.narrative.trim());
    }
    for (const fact of observation?.facts ?? []) {
      if (typeof fact === 'string' && fact.trim().length > 0) {
        segments.push(fact.trim());
      }
    }
  }
...
function deriveWorkflowCanonicalPacketPointer(specFolder: string, specFolderName: string): string {
  const normalizedRelativeToRepo = path.relative(CONFIG.PROJECT_ROOT, specFolder).replace(/\\/g, '/');
  if (
    normalizedRelativeToRepo.length > 0
    && normalizedRelativeToRepo !== '.'
    && !normalizedRelativeToRepo.startsWith('../')
    && normalizedRelativeToRepo.includes('/')
  ) {
    return normalizedRelativeToRepo;
  }

  return specFolderName.includes('/') ? specFolderName : `specs/${specFolderName}`;
}

function buildWorkflowThinContinuityRecord(input: WorkflowCanonicalContinuityInput): ThinContinuityRecord {
  const record: ThinContinuityRecord = {
    packet_pointer: deriveWorkflowCanonicalPacketPointer(input.specFolder, input.specFolderName),
    last_updated_at: new Date().toISOString(),
    last_updated_by: 'generate-context',
    recent_action: compactWorkflowCanonicalText(input.sessionData.LAST_ACTION, 'Saved context wrapper'),
    next_safe_action: compactWorkflowCanonicalText(input.sessionData.NEXT_ACTION, 'Inspect routed save shadow', true),
    blockers: normalizeWorkflowCanonicalBlockers(input.sessionData.BLOCKERS),
    key_files: input.files
      .map((file) => file.path.trim())
      .filter((filePath) => filePath.length > 0 && !path.isAbsolute(filePath) && !filePath.includes('\\'))
      .slice(0, 5),
    completion_pct: normalizeWorkflowCanonicalCompletionPercent(
      (input.collectedData as Record<string, unknown>).completionPercent
        ?? (input.collectedData as Record<string, unknown>).completion_percent
        ?? (input.collectedData as Record<string, unknown>).completionPercentage
        ?? (input.collectedData as Record<string, unknown>).percentComplete
    ),
    open_questions: normalizeWorkflowCanonicalQuestionIds(
      (input.collectedData as Record<string, unknown>).openQuestions
        ?? (input.collectedData as Record<string, unknown>).open_questions
    ),
    answered_questions: normalizeWorkflowCanonicalQuestionIds(
      (input.collectedData as Record<string, unknown>).answeredQuestions
        ?? (input.collectedData as Record<string, unknown>).answered_questions
    ),
  };

  if (input.sessionData.SESSION_ID) {
    record.session_dedup = {
      fingerprint: `sha256:${createHash('sha256').update(`${input.specFolderName}:${input.sessionData.SESSION_ID}:${input.memoryTitle}`).digest('hex')}`,
      session_id: input.sessionData.SESSION_ID,
...
export function resolveWorkflowCanonicalContinuityOptions(
  collectedData: Record<string, unknown> | null | undefined,
  requested: WorkflowCanonicalContinuityOptions | null | undefined = undefined,
  cliTargetProvided = false,
): WorkflowCanonicalContinuityResolvedOptions | null {
  const warnings: string[] = [];
  const payloadObject = isPlainObject(collectedData?.canonicalContinuity)
    ? collectedData?.canonicalContinuity as Record<string, unknown>
    : isPlainObject(collectedData?.canonical_continuity)
      ? collectedData?.canonical_continuity as Record<string, unknown>
      : null;

  const explicitMode = normalizeWorkflowCanonicalMode(requested?.mode);
  const payloadMode = normalizeWorkflowCanonicalMode(payloadObject?.mode);
  if (explicitMode.downgraded || payloadMode.downgraded) {
    warnings.push('Canonical continuity requests above shadow mode were downgraded to shadow-only preview.');
  }

  const routeAs = normalizeWorkflowCanonicalRouteAs(
    requested?.routeAs
      ?? payloadObject?.routeAs
      ?? payloadObject?.route_as
      ?? collectedData?.routeAs
      ?? collectedData?.route_as
  );
  const mergeModeHint = normalizeWorkflowCanonicalMergeMode(
    requested?.mergeModeHint
      ?? payloadObject?.mergeModeHint
      ?? payloadObject?.merge_mode_hint
      ?? collectedData?.mergeModeHint
      ?? collectedData?.merge_mode_hint
  );

  const mode = explicitMode.mode
    ?? payloadMode.mode
    ?? (routeAs || mergeModeHint ? 'shadow' : 'off');
  ...
  return {
    ...
    routeAs,
    mergeModeHint,
    buildShadowPlan: requested?.buildShadowPlan,
    targetAuthority: cliTargetProvided ? 'cli' : 'payload',
    requestSource: requested
      ? 'workflow-options'
      : payloadObject
        ? 'payload-object'
        : 'payload-hints',
...
async function loadCanonicalContinuityRouterFactory(): Promise<{
  createContentRouter: () => {
    classifyContent: (
      chunk: { id: string; text: string; sourceField?: string; routeAs?: RoutingCategory },
      context: {
        specFolder: string;
        packetLevel: WorkflowCanonicalPacketLevel;
        sessionMeta: {
          spec_folder: string;
          packet_level: WorkflowCanonicalPacketLevel;
          packet_kind: string;
          save_mode: string;
    ...
    ) => Promise<WorkflowRoutingDecision>;
  };
  createRoutingAuditEntry: (
    decision: {
      ...
      tierUsed: 1 | 2 | 3;
      alternatives: Array<{ category: RoutingCategory; confidence: number }>;
      cacheKey: string;
    },
  ...
  const scriptsRoot = path.resolve(WORKFLOW_MODULE_DIR, '..');
  const candidates = WORKFLOW_PREFER_SOURCE_RUNTIME
    ? [
        path.resolve(scriptsRoot, '../mcp_server/lib/routing/content-router.ts'),
        path.resolve(scriptsRoot, '../mcp_server/dist/lib/routing/content-router.js'),
      ]
    : [
        path.resolve(scriptsRoot, '../mcp_server/dist/lib/routing/content-router.js'),
        path.resolve(scriptsRoot, '../mcp_server/lib/routing/content-router.ts'),
  ...
  throw new Error('content-router module was not found in mcp_server/dist or source');
}

async function loadThinContinuityValidator(): Promise<{
  ...
  const candidates = WORKFLOW_PREFER_SOURCE_RUNTIME
    ? [
        path.resolve(scriptsRoot, '../mcp_server/lib/continuity/thin-continuity-record.ts'),
        path.resolve(scriptsRoot, '../mcp_server/dist/lib/continuity/thin-continuity-record.js'),
      ]
    : [
        path.resolve(scriptsRoot, '../mcp_server/dist/lib/continuity/thin-continuity-record.js'),
        path.resolve(scriptsRoot, '../mcp_server/lib/continuity/thin-continuity-record.ts'),
  ...
  throw new Error('thin-continuity-record module was not found in mcp_server/dist or source');
}

async function loadCanonicalContinuityRolloutModule(): Promise<{
  isCanonicalContinuityShadowDualWriteEnabled: () => boolean;
}> {
  const scriptsRoot = path.resolve(WORKFLOW_MODULE_DIR, '..');
  const candidates = WORKFLOW_PREFER_SOURCE_RUNTIME
    ? [
        path.resolve(scriptsRoot, '../mcp_server/lib/continuity/canonical-continuity-rollout.ts'),
        path.resolve(scriptsRoot, '../mcp_server/dist/lib/continuity/canonical-continuity-rollout.js'),
      ]
    : [
        path.resolve(scriptsRoot, '../mcp_server/dist/lib/continuity/canonical-continuity-rollout.js'),
        path.resolve(scriptsRoot, '../mcp_server/lib/continuity/canonical-continuity-rollout.ts'),
  ...
  throw new Error('canonical-continuity-rollout module was not found in mcp_server/dist or source');
}

export async function buildDefaultWorkflowCanonicalContinuityShadowPlan(
  input: WorkflowCanonicalContinuityInput,
): Promise<WorkflowCanonicalContinuityShadowPlan> {
  const packetLevel = coerceWorkflowCanonicalPacketLevel(input.specFolder);
  const routingModule = await loadCanonicalContinuityRouterFactory();
  const continuityModule = await loadThinContinuityValidator();
  const router = routingModule.createContentRouter();
  const chunkText = buildWorkflowCanonicalChunkText(input);
  const continuityValidation = continuityModule.validateThinContinuityRecord(
    buildWorkflowThinContinuityRecord(input),
    {
      fallbackActor: 'generate-context',
      fallbackPacketPointer: input.specFolderName,
      fallbackTimestamp: new Date().toISOString(),
      fallbackSessionId: input.sessionData.SESSION_ID ?? undefined,
    },
  );
  const routingChunk = {
    id: input.sessionData.SESSION_ID || input.memoryTitle,
    text: chunkText,
    sourceField: input.decisionSummaries.length > 0
      ? 'decisions'
      : (input.collectedData.observations?.length ?? 0) > 0
        ? 'observations'
        : 'unknown',
    routeAs: input.request.routeAs,
  };
  const routingContext = {
    specFolder: input.specFolderName,
    packetLevel,
    sessionMeta: {
      spec_folder: input.specFolderName,
      packet_level: packetLevel,
      packet_kind: input.specFolderName.includes('/') ? 'phase' : 'feature',
      save_mode: 'auto',
      recent_docs_touched: [],
      recent_anchors_touched: [],
      likely_phase_anchor: null,
      session_id: input.sessionData.SESSION_ID ?? undefined,
    },
  };
  ...
  const routingAudit = routingModule.createRoutingAuditEntry(
    decision,
    routingChunk,
    routingContext,
    Math.max(0, Date.now() - decisionStartMs),
  );

  const warnings = [...input.request.warnings];
  if (
    input.request.mergeModeHint
    && input.request.mergeModeHint !== decision.target.mergeMode
  ) {
    warnings.push(
      `Merge-mode hint "${input.request.mergeModeHint}" did not match routed mode "${decision.target.mergeMode}".`
    );
  }

  return {
    mode: 'shadow',
    status: decision.refusal ? 'refused' : 'planned',
    requestSource: input.request.requestSource,
    targetAuthority: input.request.targetAuthority,
    ...
    route: {
      ...
      target: {
        docPath: decision.target.docPath,
        anchorId: decision.target.anchorId,
        mergeMode: decision.target.mergeMode,
      },
    ...
    continuity: {
      ok: continuityValidation.ok,
      bytes: continuityValidation.bytes ?? null,
      errors: continuityValidation.errors.map((error: { code: string; field: string; message: string }) => `${error.code}:${error.field}:${error.message}`),
      yaml: continuityValidation.yaml ?? null,
      record: continuityValidation.record ?? null,
    },
    routingAudit,
  };
}
...
export interface WorkflowOptions {
  ...
  sessionId?: string;
  /** Optional shadow-only routed canonical continuity preview configuration. */
  canonicalContinuity?: WorkflowCanonicalContinuityOptions;
}
...
export interface WorkflowResult {
  ...
  warnings: string[];
  /** Optional routed canonical continuity shadow preview captured during the run. */
  routedCanonicalContinuity: WorkflowCanonicalContinuityShadowPlan | null;
...
export type WorkflowCanonicalContinuityMode = 'off' | 'shadow';
...
export type WorkflowCanonicalMergeMode =
  | 'append-as-paragraph'
  | 'insert-new-adr'
  | 'append-table-row'
  | 'update-in-place'
...
type WorkflowCanonicalTargetAuthority = 'cli' | 'payload';
type WorkflowCanonicalRequestSource = 'workflow-options' | 'payload-object' | 'payload-hints';
type WorkflowCanonicalPacketLevel = 'L1' | 'L2' | 'L3' | 'L3+';
...
export interface WorkflowCanonicalContinuityOptions {
  mode?: WorkflowCanonicalContinuityMode | 'canonical' | 'live' | string;
  routeAs?: RoutingCategory;
  mergeModeHint?: WorkflowCanonicalMergeMode;
  buildShadowPlan?: (
    input: WorkflowCanonicalContinuityInput,
  ) => Promise<WorkflowCanonicalContinuityShadowPlan> | WorkflowCanonicalContinuityShadowPlan;
}

export interface WorkflowCanonicalContinuityResolvedOptions extends WorkflowCanonicalContinuityOptions {
  mode: 'shadow';
  targetAuthority: WorkflowCanonicalTargetAuthority;
  requestSource: WorkflowCanonicalRequestSource;
  warnings: string[];
}

export interface WorkflowCanonicalDecisionSummary {
  title?: string;
  rationale?: string;
}

export interface WorkflowCanonicalFileSummary {
  path: string;
  description?: string;
}

export interface WorkflowCanonicalContinuityInput {
  request: WorkflowCanonicalContinuityResolvedOptions;
  specFolder: string;
  specFolderName: string;
  collectedData: CollectedDataFull;
  ...
  memoryTitle: string;
  summary: string;
  decisionSummaries: WorkflowCanonicalDecisionSummary[];
  files: WorkflowCanonicalFileSummary[];
}

export interface WorkflowCanonicalContinuityShadowPlan {
  mode: 'shadow';
  ...
  continuity: {
    ...
    errors: string[];
    yaml: string | null;
    record: ThinContinuityRecord | null;
  };
  routingAudit: {
    ts: string;
    component: 'content-router';
...
async function runWorkflow(options: WorkflowOptions = {}): Promise<WorkflowResult> {
  ...
  let flagDrivenCanonicalContinuity: WorkflowCanonicalContinuityOptions | undefined;
  if (!options.canonicalContinuity) {
    try {
      const rolloutModule = await loadCanonicalContinuityRolloutModule();
      if (rolloutModule.isCanonicalContinuityShadowDualWriteEnabled()) {
        flagDrivenCanonicalContinuity = { mode: 'shadow' };
      }
    } catch (error: unknown) {
      const warning = `Canonical continuity rollout flag unavailable: ${error instanceof Error ? error.message : String(error)}`;
      workflowWarnings.push(warning);
      warn(`   Warning: ${warning}`);
    }
  }
  const canonicalContinuityRequest = resolveWorkflowCanonicalContinuityOptions(
    collectedData as Record<string, unknown>,
    options.canonicalContinuity ?? flagDrivenCanonicalContinuity,
    Boolean(activeSpecFolderArg),
  );
  let routedCanonicalContinuity: WorkflowCanonicalContinuityShadowPlan | null = null;
  if (canonicalContinuityRequest) {
    log('Step 7.7: Building canonical continuity shadow plan...');
    for (const requestWarning of canonicalContinuityRequest.warnings) {
      workflowWarnings.push(requestWarning);
      warn(`   Warning: ${requestWarning}`);
    }
    try {
      const buildShadowPlan = canonicalContinuityRequest.buildShadowPlan ?? buildDefaultWorkflowCanonicalContinuityShadowPlan;
      routedCanonicalContinuity = await buildShadowPlan({
        request: canonicalContinuityRequest,
        specFolder,
        specFolderName,
        collectedData,
        sessionData,
        memoryTitle,
        summary: sessionData.SUMMARY || memoryDescription,
        decisionSummaries: decisions.DECISIONS.map((decision) => ({
      ...
      log(`   Canonical continuity shadow status: ${routedCanonicalContinuity.status}`);
      if (routedCanonicalContinuity.route) {
        log(
          `   Shadow target: ${routedCanonicalContinuity.route.target.docPath}::${routedCanonicalContinuity.route.target.anchorId} ` +
          `(${routedCanonicalContinuity.route.target.mergeMode}, confidence ${routedCanonicalContinuity.route.confidence.toFixed(2)})`
        );
      }
      for (const continuityWarning of routedCanonicalContinuity.warnings) {
        if (!workflowWarnings.includes(continuityWarning)) {
          workflowWarnings.push(continuityWarning);
        }
      }
    } catch (error: unknown) {
      const errMsg = error instanceof Error ? error.message : String(error);
      const warning = `Canonical continuity shadow preview failed: ${errMsg}`;
      workflowWarnings.push(warning);
      warn(`   Warning: ${warning}`);
      routedCanonicalContinuity = {
        mode: 'shadow',
        status: 'unavailable',
        requestSource: canonicalContinuityRequest.requestSource,
        targetAuthority: canonicalContinuityRequest.targetAuthority,
        legacyMemoryWritePreserved: true,
        warnings: [warning],
        route: null,
        continuity: {
  ...
  const files: Record<string, string> = {
    ...
    'metadata.json': JSON.stringify({
      ...
      canonicalContinuityShadow: routedCanonicalContinuity,
      embedding: {
        status: 'pending',
        model: MODEL_NAME,
  ...
  if (routedCanonicalContinuity) {
    try {
      const legacyMemoryPath = path.posix.join(
        'memory',
        (duplicateExistingFilename && duplicateExistingFilename.length > 0) ? duplicateExistingFilename : ctxFilename,
      );
      const shadowArtifacts = persistCanonicalContinuityShadowArtifacts({
        specFolderAbsolute: specFolder,
        specFolderRelative: specFolderName,
        sessionId: sessionData.SESSION_ID ?? null,
        legacyMemoryPath,
        plan: routedCanonicalContinuity,
      });
      log(`   Canonical continuity routing audit -> ${path.relative(specFolder, shadowArtifacts.routingAuditPath ?? shadowArtifacts.shadowComparePath)}`);
      log(`   Canonical continuity shadow compare -> ${path.relative(specFolder, shadowArtifacts.shadowComparePath)} (${shadowArtifacts.shadowCompareEntry.shadow_result})`);
    } catch (error: unknown) {
      const warning = `Canonical continuity shadow logging failed: ${error instanceof Error ? error.message : String(error)}`;
      workflowWarnings.push(warning);
      warn(`   Warning: ${warning}`);
    }
  }
