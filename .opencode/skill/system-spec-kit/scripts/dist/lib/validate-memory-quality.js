...
exports.VALIDATION_RULE_METADATA = exports.HARD_BLOCK_RULES = void 0;
exports.determineValidationDisposition = determineValidationDisposition;
exports.getRuleMetadata = getRuleMetadata;
exports.shouldBlockIndex = shouldBlockIndex;
exports.shouldBlockWrite = shouldBlockWrite;
exports.validateMemoryQualityContent = validateMemoryQualityContent;
exports.validateMemoryQualityFile = validateMemoryQualityFile;
...
const VALIDATION_RULE_METADATA = {
    V1: {
        ruleId: 'V1',
...
exports.VALIDATION_RULE_METADATA = VALIDATION_RULE_METADATA;
const HARD_BLOCK_RULES = Object.values(VALIDATION_RULE_METADATA)
    .filter((metadata) => metadata.blockOnWrite)
    .map((metadata) => metadata.ruleId);
exports.HARD_BLOCK_RULES = HARD_BLOCK_RULES;
...
function describeYamlParseError(error) {
...
function validateFrontMatterSyntax(raw) {
    try {
        const parsed = loadYamlValue(raw);
 ...
 */
function extractAndValidateFrontMatter(content) {
...
function extractYamlValueFromContent(content, key) {
...
function parseYamlListFromContent(content, key) {
    const frontMatter = extractFrontMatter(content);
...
function parseToolCount(content) {
...
function parseYamlIntegerFromContent(content, key) {
...
function hasSignificantFileCountDivergence(filesystemFileCount, capturedFileCount) {
...
function countDistinctSpecIds(content) {
...
function countSpecIdsInValues(values) {
    return countDistinctSpecIds(values.join('\n'));
}
function extractCurrentSpecId(specFolder) {
...
function resolveSpecFolderPath(specFolder) {
    ...
    let currentDir = cwd;
    while (true) {
        candidatePaths.add(path_1.default.resolve(currentDir, trimmedSpecFolder));
        candidatePaths.add(path_1.default.resolve(currentDir, 'specs', trimmedSpecFolder));
        candidatePaths.add(path_1.default.resolve(currentDir, '.opencode', 'specs', trimmedSpecFolder));
    ...
    for (const candidatePath of candidatePaths) {
        try {
            if (fs_1.default.statSync(candidatePath).isDirectory()) {
 ...
 */
function extractAllowedSpecIds(specFolder) {
    const matches = specFolder.match(SPEC_ID_REGEX) ?? [];
    ...
    try {
        const childEntries = fs_1.default.readdirSync(resolvedSpecFolder, { withFileTypes: true });
        for (const entry of childEntries) {
            if (!entry.isDirectory() || !/^\d{3}-/.test(entry.name)) {
                continue;
            }
            const childSpecPath = path_1.default.resolve(resolvedSpecFolder, entry.name, 'spec.md');
            if (fs_1.default.existsSync(childSpecPath)) {
        ...
        const parentDir = path_1.default.dirname(resolvedSpecFolder);
        if (parentDir !== resolvedSpecFolder) {
            try {
                const siblingEntries = fs_1.default.readdirSync(parentDir, { withFileTypes: true });
        ...
        (0, logger_1.structuredLog)('warn', 'Failed to scan child phase folders during V8 allowlist extraction', {
            specFolder,
            resolvedSpecFolder,
            error: message,
        });
...
function ruleAppliesToSource(metadata, source) {
    if (metadata.appliesToSources === 'all') {
        return true;
    }
    const capabilities = (0, source_capabilities_1.getSourceCapabilities)(source);
...
function hasExecutionSignals(content) {
    return EXECUTION_SIGNAL_PATTERNS.some((pattern) => pattern.test(content));
}
function validateMemoryQualityContent(content, options) {
    ...
    const malformedSpecFolder = /\*\*|\*|\[/.test(specFolder) ||
    ...
    const resolvedSpecFolder = resolveSpecFolderPath(specFolder);
    if (resolvedSpecFolder) {
        const specMdPath = path_1.default.resolve(resolvedSpecFolder, 'spec.md');
        if (fs_1.default.existsSync(specMdPath)) {
            try {
                const specContent = fs_1.default.readFileSync(specMdPath, 'utf-8');
                ...
                const message = error instanceof Error ? error.message : String(error);
                (0, logger_1.structuredLog)('warn', 'Failed to read related_specs during V8 allowlist extraction', {
                    specFolder,
                    specMdPath,
                    error: message,
                });
    ...
    const scatteredForeignMentions = [];
    ...
    if (specFolder && !isMemoryDirFile && !isSpecDoc) {
        ...
        let specTriggerPhrases = [];
        if (resolvedSpecFolder) {
            const specMdCandidates = [
                path_1.default.resolve(resolvedSpecFolder, 'spec.md'),
                // specFolder might be just a relative path or short name
            ];
            for (const candidate of specMdCandidates) {
                if (fs_1.default.existsSync(candidate)) {
                    try {
                        const specContent = fs_1.default.readFileSync(candidate, 'utf-8');
                        specTriggerPhrases = parseYamlListFromContent(specContent, 'trigger_phrases');
                    }
                    catch (error) {
                        const message = error instanceof Error ? error.message : String(error);
                        (0, logger_1.structuredLog)('warn', 'Failed to read spec.md during V12 topical coherence validation', {
                            candidate,
                            specFolder,
                            error: message,
                        });
    ...
    (0, logger_1.structuredLog)('info', 'contamination_audit', contaminationAudit);
    return {
...
function validateMemoryQualityFile(filePath) {
    let content;
    try {
