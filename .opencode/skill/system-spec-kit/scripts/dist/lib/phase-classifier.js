...
exports.classifyConversationExchanges = classifyConversationExchanges;
...
// 3. VECTOR OPERATIONS
// ───────────────────────────────────────────────────────────────
function createEmptyPhaseScores() {
    return {
        Research: 0,
        Planning: 0,
        Implementation: 0,
...
function buildCombinedText(exchange) {
    return [
        exchange.prompt,
        ...exchange.narratives,
        ...exchange.factTexts,
...
function buildExchangeSignals(exchange) {
    const combinedText = buildCombinedText(exchange);
    const extracted = semantic_signal_extractor_1.SemanticSignalExtractor.extract({
        text: combinedText,
        mode: 'all',
...
// ───────────────────────────────────────────────────────────────
function keywordScore(vector, keywords) {
    let score = 0;
...
function scoreCluster(cluster, combinedText, toolNames, observationTypes) {
...
function selectPrimaryPhase(scores) {
    ...
    const bestPhase = PHASE_PRIORITY.reduce((winner, candidate) => (scores[candidate] > scores[winner] ? candidate : winner), PHASE_PRIORITY[0]);
    return {
        phase: bestPhase,
        confidence: scores[bestPhase] / total,
    };
}
function buildTopicLabel(dominantTerms, fallback) {
    const labelTerms = dominantTerms.slice(0, 2);
    return labelTerms.length > 0 ? labelTerms.join(' / ') : fallback;
}
function classifyCluster(cluster, exchanges) {
...
// ───────────────────────────────────────────────────────────────
function deriveFlowPattern(clusters) {
...
function buildTimelinePhases(clusters) {
...
function toClassifiedExchange(exchange) {
    return {
        ...exchange,
...
function classifyConversationExchanges(exchanges) {
    ...
    return {
        topicClusters: classifiedClusters.map(({ vector: _vector, semanticVector: _semanticVector, startTimestamp: _startTimestamp, endTimestamp: _endTimestamp, ...cluster }) => cluster),
        phases,
...
function classifyConversationPhase(toolCalls, messageContent) {
    ...
    return result.phases[0]?.PHASE_NAME ?? 'Discussion';
}
//# sourceMappingURL=phase-classifier.js.map