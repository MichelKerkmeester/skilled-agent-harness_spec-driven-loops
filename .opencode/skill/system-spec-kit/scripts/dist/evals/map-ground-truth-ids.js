...
function loadGroundTruthQueries() {
...
// ───────────────────────────────────────────────────────────────
function mapQueryToMemories(db, query) {
    ...
    ranked = ranked.map(c => {
        let boost = 1.0;
        if (c.importanceTier === 'constitutional')
            boost = 3.0;
        if (c.importanceTier === 'critical')
            boost = 2.0;
        if (c.importanceTier === 'important')
            boost = 1.5;
        if (c.documentType === 'spec')
            boost *= 1.3;
        if (c.documentType === 'decision_record')
            boost *= 1.3;
...
function main() {
    ...
    const totalMemories = db.prepare('SELECT COUNT(*) as cnt FROM memory_index WHERE parent_id IS NULL').get()?.cnt ?? 0;
    ...
    if (WRITE) {
        const currentGroundTruth = JSON.parse(fs.readFileSync(GT_PATH, 'utf-8'));
...
main();
//# sourceMappingURL=map-ground-truth-ids.js.map