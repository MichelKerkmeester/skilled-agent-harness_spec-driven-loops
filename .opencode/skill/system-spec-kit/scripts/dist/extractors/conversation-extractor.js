 ...
 */
function extractFromJsonPayload(collectedData) {
    const messages = [];
    const timestamp = (0, message_utils_1.formatTimestamp)(undefined, 'readable');
    ...
    if (nextSteps.length > 0) {
        const stepsText = nextSteps
            .map((step) => typeof step === 'string' ? step : step?.description || JSON.stringify(step))
            .filter(Boolean)
            .join('; ');
...
async function extractConversations(collectedData) {
    ...
    const consumedObservationIndexes = new Set();
    for (let i = 0; i < userPrompts.length; i++) {
        ...
        const userMessage = {
            TIMESTAMP: (0, message_utils_1.formatTimestamp)(rawTimestamp, 'readable'),
            ROLE: 'User',
        ...
        const relatedObs = observations
            ...
            return timeDiff < config_1.CONFIG.MESSAGE_TIME_WINDOW;
        })
            .sort((a, b) => {
        ...
        const toolCalls = relatedObs.flatMap(({ obs }) => {
            ...
            return factTexts.map((fact) => {
                const detection = (0, tool_detection_1.detectToolCall)(fact);
                if (!detection)
                    return null;
                const toolIndex = fact.search(new RegExp(`\\b${detection.tool}\\b`, 'i'));
                if (toolIndex >= 0 && (0, tool_detection_1.isProseContext)(fact, toolIndex)) {
                    return null;
                }
                ...
                return {
                    ...
                    RESULT_PREVIEW: resultMatch?.[1] ? (0, message_utils_1.truncateToolOutput)(resultMatch[1], config_1.CONFIG.TOOL_PREVIEW_LINES) : '',
                    HAS_MORE: (resultMatch?.[1]?.split('\n').length ?? 0) > config_1.CONFIG.TOOL_PREVIEW_LINES,
                };
            }).filter(Boolean);
        });
        const narratives = relatedObs.map(({ obs }) => obs.narrative).filter(Boolean);
        const factTexts = relatedObs.flatMap(({ obs }) => (0, fact_coercion_1.coerceFactsToText)(obs.facts, {
            component: 'conversation-extractor',
            fieldPath: 'observations[].facts',
        }));
        const observationTypes = relatedObs.map(({ obs }) => (0, file_extractor_1.detectObservationType)(obs));
        ...
        if (relatedObs.length > 0) {
            ...
            const summary = (0, message_utils_1.summarizeExchange)(userMessage.CONTENT, narratives.join(' '), toolCalls);
            tempMessages.push({
                tempId: assistantTempId,
                rawTimestamp: assistantTimestamp,
                message: {
                    TIMESTAMP: (0, message_utils_1.formatTimestamp)(assistantTimestamp, 'readable'),
                    ROLE: 'Assistant',
    ...
    const messageIndexByTempId = new Map();
    ...
    if (!jsonModeHandled && userPrompts.length <= 1 && collectedData.sessionSummary) {
        ...
        if (Array.isArray(keyDecisions) && keyDecisions.length > 0) {
            MESSAGES.push({
                TIMESTAMP: timestamp,
                ROLE: 'Assistant',
                CONTENT: `Key decisions: ${keyDecisions.map((d) => typeof d === 'string' ? d : d?.title || JSON.stringify(d)).join('; ')}`,
                TOOL_CALLS: [],
            });
        }
        const nextSteps = collectedData.nextSteps;
        if (Array.isArray(nextSteps) && nextSteps.length > 0) {
            MESSAGES.push({
                TIMESTAMP: timestamp,
                ROLE: 'Assistant',
                CONTENT: `Next steps: ${nextSteps.map((s) => String(s)).join('; ')}`,
                TOOL_CALLS: [],
            });
        }
    }
    ...
    const classification = (0, phase_classifier_1.classifyConversationExchanges)(exchangeInputs.map(({ tempMessageIds: _tempMessageIds, ...exchange }) => exchange));
    ...
    const autoGeneratedFlow = flowchartGen.generateConversationFlowchart(phases, userPrompts[0]?.prompt);
    return {
        MESSAGES,
