...
exports.SEVERITY_RANK = exports.DEFAULT_DENYLIST = void 0;
exports.filterContamination = filterContamination;
exports.getContaminationPatternLabels = getContaminationPatternLabels;
...
exports.DEFAULT_DENYLIST = DEFAULT_DENYLIST;
/* ───────────────────────────────────────────────────────────────
   3. FILTER LOGIC
------------------------------------------------------------------*/
...
function getDenylistLabel(entry) {
    return entry instanceof RegExp ? entry.toString() : entry.label;
}
function getDenylistPattern(entry) {
    return entry instanceof RegExp ? entry : entry.pattern;
}
function getDenylistSeverity(entry, options) {
    if (entry instanceof RegExp) {
        return 'high';
    }
    const sourceCapabilities = options?.sourceCapabilities ?? (0, source_capabilities_1.getSourceCapabilities)(options?.captureSource);
...
function maxSeverityOf(a, b) {
    if (a === null)
        return b;
    return SEVERITY_RANK[a] >= SEVERITY_RANK[b] ? a : b;
}
function getContaminationPatternLabels(denylist = DEFAULT_DENYLIST) {
...
function filterContamination(input, denylist = DEFAULT_DENYLIST, options) {
...
//# sourceMappingURL=contamination-filter.js.map