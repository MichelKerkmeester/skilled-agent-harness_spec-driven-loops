...
exports.toCanonicalRelativePath = toCanonicalRelativePath;
exports.getDescriptionTierRank = getDescriptionTierRank;
...
function normalizeDescriptionForValidation(description) {
    if (!description) {
        return '';
    }
    return description
...
function normalizeRelativePath(filePath, projectRoot) {
    if (!filePath)
        return '';
    let cleaned = node_path_1.posix.normalize(filePath.replace(/\\/g, '/'));
    if (projectRoot) {
        // F-02: Segment-boundary containment check prevents partial-prefix matches
        // (e.g., root="/foo/bar" must not match "/foo/barbaz/file")
        const normalizedRoot = node_path_1.posix.normalize(projectRoot.replace(/\\/g, '/'))
            .replace(/\/+$/, '');
        ...
        else if (node_path_1.posix.isAbsolute(cleaned)) {
...
function toCanonicalRelativePath(filePath, projectRoot) {
...
// ───────────────────────────────────────────────────────────────
function getDescriptionTierRank(tier) {
