...
exports.normalizeMemoryNameCandidate = normalizeMemoryNameCandidate;
exports.slugify = slugify;
exports.isContaminatedMemoryName = isContaminatedMemoryName;
exports.isGenericContentTask = isGenericContentTask;
exports.pickBestContentName = pickBestContentName;
exports.ensureUniqueMemoryFilename = ensureUniqueMemoryFilename;
exports.generateContentSlug = generateContentSlug;
// ───────────────────────────────────────────────────────────────
// 1. SLUG UTILS
// ───────────────────────────────────────────────────────────────
...
function hashFallbackSlug(seed) {
 ...
 */
function isContaminatedMemoryName(candidate) {
 ...
 */
function isGenericContentTask(task) {
 ...
 */
function pickBestContentName(candidates) {
    const seen = new Set();
 ...
 */
function truncateSlugAtWordBoundary(slug, max = DEFAULT_SLUG_MAX_LENGTH) {
    if (slug.length <= max)
        return slug;
...
function ensureUniqueMemoryFilename(contextDir, filename) {
    ...
    for (let attempt = 0; attempt < 100; attempt++) {
        ...
        try {
            ...
            const randomCode = randomErr.code;
            if (randomCode !== 'EEXIST')
                throw randomErr;
        }
    }
 ...
 */
function generateContentSlug(task, fallback, alternatives = []) {
