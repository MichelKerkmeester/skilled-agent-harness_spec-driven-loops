...
exports.buildSpecAffinityTargets = buildSpecAffinityTargets;
exports.matchesSpecAffinityFilePath = matchesSpecAffinityFilePath;
exports.evaluateSpecAffinityText = evaluateSpecAffinityText;
exports.matchesSpecAffinityText = matchesSpecAffinityText;
exports.evaluateCollectedDataSpecAffinity = evaluateCollectedDataSpecAffinity;
exports.extractSpecIds = extractSpecIds;
...
function containsWordBoundary(normalizedText, phrase) {
...
function extractSpecIds(value) {
    return Array.from(new Set((value.match(SPEC_ID_REGEX) || []).map((specId) => specId.toLowerCase())));
}
function resolveSpecFolderPathCandidates(specFolderHint) {
...
function parseFrontmatterValue(content, keyName) {
    if (!content) {
        return [];
    }
...
function extractFilesToChange(content) {
    if (!content) {
        return [];
    }
...
function readSpecMetadata(specFolderHint) {
    ...
    let descriptionTriggerPhrases = [];
    if (descriptionDoc) {
        try {
            const parsed = JSON.parse(descriptionDoc);
            ...
            if (Array.isArray(parsed.triggerPhrases)) {
                descriptionTriggerPhrases = parsed.triggerPhrases.filter((value) => typeof value === 'string');
            }
        }
        catch {
...
function buildSlugCandidates(specFolderHint) {
...
function buildStrongKeywordTokens(values) {
...
function buildSpecAffinityTargets(specFolderHint) {
...
function matchesSpecAffinityFilePath(filePath, targets) {
    const normalizedPath = normalizePathLike(filePath);
    if (!normalizedPath) {
        return false;
    }
    return targets.fileTargets.some((target) => (normalizedPath === target
        || normalizedPath.endsWith(`/${target}`)
        || normalizedPath.includes(`/${target}/`)));
}
function countKeywordMatches(normalizedText, strongKeywordTokens) {
    return strongKeywordTokens.filter((token) => containsWordBoundary(normalizedText, token));
}
function evaluateSpecAffinityText(text, targets) {
    const rawText = typeof text === 'string' ? text : '';
    const normalizedText = normalizeText(rawText);
    const normalizedPathText = normalizePathLike(rawText);
    const matchedFileTargets = targets.fileTargets.filter((target) => (normalizedPathText.includes(target)
...
function gatherCollectedDataPaths(data) {
    const paths = [];
    for (const observation of data.observations || []) {
...
function evaluateCollectedDataSpecAffinity(data, targetsOrHint) {
    const targets = typeof targetsOrHint === 'string'
        ? buildSpecAffinityTargets(targetsOrHint)
        : targetsOrHint;
    const matchedFileTargets = new Set();
    const matchedPhrases = new Set();
    const matchedKeywordTokens = new Set();
    const foreignSpecIds = new Set();
    ...
    const hasAnchor = (matchedFileTargets.size > 0
...
//# sourceMappingURL=spec-affinity.js.map