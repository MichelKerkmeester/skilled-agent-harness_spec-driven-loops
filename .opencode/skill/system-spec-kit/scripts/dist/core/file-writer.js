...
function canonicalizeForDuplicateComparison(content) {
    return content
...
function verifyResolvedWriteTarget(resolvedContextDir, filePath, filename) {
...
async function backupExistingFileAtomically(filePath, backupPath) {
    let existingFile;
    try {
...
function validateContentSubstance(content, filename) {
    ...
    if (stripped.length < MIN_SUBSTANCE_CHARS) {
        throw new Error(`Empty/template-only content in ${filename}: ` +
...
async function writeFilesAtomically(contextDir, files) {
    ...
    for (const [filename, content] of Object.entries(files)) {
        (0, validation_utils_1.validateNoLeakedPlaceholders)(content, filename);
        ...
        const warnings = (0, validation_utils_1.validateAnchors)(content);
        if (warnings.length)
            console.warn(`   Warning: ${filename}: ${warnings.join(', ')}`);
        ...
        try {
            ...
            const tempFd = await fs.open(tempPath, fsSync.constants.O_CREAT | fsSync.constants.O_EXCL | fsSync.constants.O_WRONLY, 0o600);
            try {
            ...
            if (stat.size !== Buffer.byteLength(content, 'utf-8'))
            ...
            catch { /* temp file cleanup — failure is non-critical */ }
            ...
            const rollbackErrors = [...currentFileErrors];
