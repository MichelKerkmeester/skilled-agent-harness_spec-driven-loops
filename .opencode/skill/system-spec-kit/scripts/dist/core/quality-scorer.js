...
const DESCRIPTION_TIER_SCORES = {
    placeholder: 0,
    'activity-only': 0.35,
    semantic: 0.75,
    'high-confidence': 1,
};
function getDescriptionTrustMultiplier(file) {
    if (file._synthetic) {
        return 0.5;
    }
...
function getDescriptionQualityScore(file) {
    const tier = (0, file_helpers_1.validateDescription)(file.DESCRIPTION || '').tier;
...
function hasSpecificPrimaryTitle(content) {
    const titleCandidate = (0, slug_utils_1.pickBestContentName)([
        extractFrontmatterTitle(content),
        extractHeadingTitle(content),
    ]);
    return titleCandidate.length >= 8;
}
function hasMeaningfulObservationTitle(title) {
    if (!title) {
        return false;
    }
    const normalized = (0, slug_utils_1.normalizeMemoryNameCandidate)(title);
    if (normalized.length < 10) {
        return false;
    }
    return !(0, slug_utils_1.isGenericContentTask)(normalized) && !(0, slug_utils_1.isContaminatedMemoryName)(normalized);
}
/**
...
function scoreMemoryQuality(content, triggerPhrases, keyTopics, files, observations, sufficiencyResult, hadContamination = false, contaminationSeverity = null) {
    const warnings = [];
    ...
    const filesWithDesc = files.filter((file) => (0, file_helpers_1.getDescriptionTierRank)((0, file_helpers_1.validateDescription)(file.DESCRIPTION || '').tier) >= (0, file_helpers_1.getDescriptionTierRank)('semantic'));
    if (files.length === 0) {
    ...
    if (observations.length === 0) {
        ...
        const titles = observations
            .map((observation) => (0, slug_utils_1.normalizeMemoryNameCandidate)(observation.TITLE || ''))
    ...
    let scoreCap = null;
    const effectiveSeverity = contaminationSeverity || 'medium';
    ...
    if (sufficiencyResult && !sufficiencyResult.pass) {
        ...
        warnings.push(`Insufficient context for a durable memory: ${sufficiencyResult.reasons.join(' ')}`);
    }
    if (scoreCap !== null) {
