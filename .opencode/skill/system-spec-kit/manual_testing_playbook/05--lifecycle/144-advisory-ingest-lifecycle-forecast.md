---
title: "NEW-144 -- Advisory ingest lifecycle forecast"
description: "This scenario validates Advisory ingest lifecycle forecast for `NEW-144`. It focuses on Verify `memory_ingest_status` exposes advisory forecast fields and degrades safely on sparse progress."
---
...
This scenario validates Advisory ingest lifecycle forecast for `NEW-144`. It focuses on Verify `memory_ingest_status` exposes advisory forecast fields and degrades safely on sparse progress.

---

## 2. CURRENT REALITY

Operators run the exact prompt and command sequence for `NEW-144` and confirm the expected signals without contradicting evidence.
