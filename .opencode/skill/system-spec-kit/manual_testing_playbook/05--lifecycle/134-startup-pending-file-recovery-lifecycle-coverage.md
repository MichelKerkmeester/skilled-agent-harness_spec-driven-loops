...
- Prompt: `As a lifecycle validation operator, validate Startup pending-file recovery lifecycle coverage against recoverAllPendingFiles. Verify startup recovery scans allowed roots and preserves stale pending files for manual review. Return a concise pass/fail verdict with the main reason and cited evidence.`
