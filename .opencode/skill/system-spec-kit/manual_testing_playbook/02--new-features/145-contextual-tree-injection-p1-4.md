---
title: "NEW-145 -- Contextual tree injection (P1-4)"
description: "This scenario validates Contextual tree injection (P1-4) for `NEW-145`. It focuses on Verify hierarchical spec-folder headers are injected into search results when `SPECKIT_CONTEXT_HEADERS=true` and suppressed when disabled."
---
...
This scenario validates Contextual tree injection (P1-4) for `NEW-145`. It focuses on Verify hierarchical spec-folder headers are injected into search results when `SPECKIT_CONTEXT_HEADERS=true` and suppressed when disabled.

---

## 2. CURRENT REALITY

Operators run the exact prompt and command sequence for `NEW-145` and confirm the expected signals without contradicting evidence.
...
- Feature file path: `02--new-features/145-contextual-tree-injection-p1-4.md`
