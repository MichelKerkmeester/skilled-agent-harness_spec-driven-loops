---
title: "NEW-116 -- Chunking safe swap atomicity (P0-6)"
description: "This scenario validates Chunking safe swap atomicity (P0-6) for `NEW-116`. It focuses on Verify re-chunking indexes new chunks before deleting old ones, and old chunks survive if new indexing fails."
---
...
This scenario validates Chunking safe swap atomicity (P0-6) for `NEW-116`. It focuses on Verify re-chunking indexes new chunks before deleting old ones, and old chunks survive if new indexing fails.

---

## 2. CURRENT REALITY

Operators run the exact prompt and command sequence for `NEW-116` and confirm the expected signals without contradicting evidence.
...
- Feature file path: `02--new-features/116-chunking-safe-swap-atomicity-p0-6.md`
