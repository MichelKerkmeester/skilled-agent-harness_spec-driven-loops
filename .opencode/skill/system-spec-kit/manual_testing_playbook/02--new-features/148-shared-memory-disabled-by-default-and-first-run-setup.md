---
title: "NEW-148 -- Shared-memory disabled-by-default and first-run setup"
description: "This scenario validates Shared-memory disabled-by-default and first-run setup for `NEW-148`. It focuses on Confirm shared memory is off by default, enable flow works, persistence survives restart, env var overrides DB, and enable is idempotent."
---
...
This scenario validates Shared-memory disabled-by-default and first-run setup for `NEW-148`. It focuses on Confirm shared memory is off by default, enable flow works, persistence survives restart, env var overrides DB, and enable is idempotent.

---

## 2. CURRENT REALITY

Operators run the exact prompt and command sequence for `NEW-148` and confirm the expected signals without contradicting evidence.
...
- Feature file path: `02--new-features/148-shared-memory-disabled-by-default-and-first-run-setup.md`
