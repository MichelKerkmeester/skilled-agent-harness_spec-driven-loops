---
title: "NEW-098 -- Local GGUF reranker via node-llama-cpp (P1-5)"
description: "This scenario validates Local GGUF reranker via node-llama-cpp (P1-5) for `NEW-098`. It focuses on Confirm reranker gating and graceful fallback."
---
...
This scenario validates Local GGUF reranker via node-llama-cpp (P1-5) for `NEW-098`. It focuses on Confirm reranker gating and graceful fallback.

---

## 2. CURRENT REALITY

Operators run the exact prompt and command sequence for `NEW-098` and confirm the expected signals without contradicting evidence.
...
- Feature file path: `02--new-features/098-local-gguf-reranker-via-node-llama-cpp-p1-5.md`
