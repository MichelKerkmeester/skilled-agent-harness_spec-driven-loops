---
title: "NEW-100 -- Async shutdown with deadline (server lifecycle)"
description: "This scenario validates Async shutdown with deadline (server lifecycle) for `NEW-100`. It focuses on Confirm graceful shutdown completes async cleanup."
---
...
This scenario validates Async shutdown with deadline (server lifecycle) for `NEW-100`. It focuses on Confirm graceful shutdown completes async cleanup.

---

## 2. CURRENT REALITY

Operators run the exact prompt and command sequence for `NEW-100` and confirm the expected signals without contradicting evidence.
...
- Feature file path: `02--new-features/100-async-shutdown-with-deadline-server-lifecycle.md`
