...
- Real user request: `Please validate 2. Session and Cache against memory_search({ query:"DISABLE_SESSION_DEDUP session cache policy settings", limit:20 }) and tell me whether the expected signals are present: Session/cache controls found.`
- RCAF Prompt: `As a feature-flag validation operator, validate 2. Session and Cache against memory_search({ query:"DISABLE_SESSION_DEDUP session cache policy settings", limit:20 }). Verify session/cache controls found. Return a concise pass/fail verdict with the main reason and cited evidence.`
