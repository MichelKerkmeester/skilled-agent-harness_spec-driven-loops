...
| 261d | MCP auto-priming | Session-scoped priming isolation | `Validate 261d session-scoped priming isolation` | Call `memory_stats({ sessionId: "prime-session-a" })`, then call `memory_stats({ sessionId: "prime-session-b" })` in a fresh second session | Each session receives its own first-call PrimePackage because priming is tracked per session in `primedSessionIds` | Two MCP response envelopes showing independent primePackage delivery for session A and session B | PASS if the second session receives its own PrimePackage independently of the first session | Check `hooks/memory-surface.ts` session-scoped `primedSessionIds` handling and session reset paths |
