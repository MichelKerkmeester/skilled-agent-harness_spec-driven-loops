...
- Prompt: `Run the canonical M-007 session-capturing closure verification scenario. Capture the evidence needed to prove Coverage is sourced from the M-007 closure suite, including JSON authority, shipped structured-summary fields (`toolCalls`, `exchanges`), file-backed JSON authority, stateless enrichment, runtime fallback ordering, Phase 017 stateless quality-gate behavior, Phase 017 recovery-flag enforcement, Phase 018 output-quality hardening, insufficiency rejection, and indexing readiness. Return a concise user-facing pass/fail verdict with the main reason.`
- Expected signals: Coverage is sourced from the M-007 closure suite, including JSON authority, shipped structured-summary fields (`toolCalls`, `exchanges`), file-backed JSON authority, stateless enrichment, runtime fallback ordering, Phase 017 stateless quality-gate behavior, Phase 017 recovery-flag enforcement, Phase 018 output-quality hardening, insufficiency rejection, and indexing readiness.
