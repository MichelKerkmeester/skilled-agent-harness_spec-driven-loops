...
description: "This scenario validates /memory:manage command routing for `186`. It focuses on Verify the command's default stats mode and key subcommand routing: scan, cleanup, bulk-delete, tier, triggers, validate, delete, health, checkpoint, and ingest."
---
...
- Objective: Verify `/memory:manage` default stats dashboard and subcommand routing for all 11 modes (stats, scan, cleanup, bulk-delete, tier, triggers, validate, delete, health, checkpoint, ingest)
- Prompt: `Validate /memory:manage command routing across all management modes. Capture the evidence needed to prove No-args shows stats dashboard; each subcommand routes to the correct MCP tool; unrecognized mode returns error. Return a concise user-facing pass/fail verdict with the main reason.`
