---
title: "NEW-135 -- Grep traceability for feature catalog code references"
description: "This scenario validates Grep traceability for feature catalog code references for `NEW-135`. It focuses on Verify `grep -r \"// Feature catalog: <feature>\" mcp_server/` returns handler + lib hits."
---
...
This scenario validates Grep traceability for feature catalog code references for `NEW-135`. It focuses on Verify `grep -r "// Feature catalog: <feature>" mcp_server/` returns handler + lib hits.

---

## 2. CURRENT REALITY

Operators run the exact prompt and command sequence for `NEW-135` and confirm the expected signals without contradicting evidence.
