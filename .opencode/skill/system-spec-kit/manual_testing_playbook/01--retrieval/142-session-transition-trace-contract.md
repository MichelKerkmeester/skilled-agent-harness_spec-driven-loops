---
title: "NEW-142 -- Session transition trace contract"
description: "This scenario validates Session transition trace contract for `NEW-142`. It focuses on Verify `memory_context` emits trace-only session transitions with no non-trace leakage."
---
...
This scenario validates Session transition trace contract for `NEW-142`. It focuses on Verify `memory_context` emits trace-only session transitions with no non-trace leakage.

---

## 2. CURRENT REALITY

Operators run the exact prompt and command sequence for `NEW-142` and confirm the expected signals without contradicting evidence.
