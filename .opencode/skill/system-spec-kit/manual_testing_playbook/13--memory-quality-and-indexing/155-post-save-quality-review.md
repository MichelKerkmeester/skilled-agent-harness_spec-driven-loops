...
| 155 | Post-save quality review | Recovery/stateless mode skips review | `Run generate-context.js with --recovery flag. Confirm the POST-SAVE QUALITY REVIEW block reports SKIPPED (not PASSED or FAILED) because payload field values are unavailable in stateless mode.` | 1) `node .opencode/skill/system-spec-kit/scripts/dist/memory/generate-context.js --recovery <spec-folder>` 2) Capture stdout 3) Locate `POST-SAVE QUALITY REVIEW` block 4) Assert status = SKIPPED | `POST-SAVE QUALITY REVIEW -- SKIPPED` in stdout | CLI stdout REVIEW block | PASS if SKIPPED is reported; FAIL if review attempts field comparison without payload data | Inspect mode gating in `scripts/core/post-save-review.ts` and `scripts/core/workflow.ts` |
