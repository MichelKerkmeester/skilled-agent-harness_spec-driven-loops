...
argument-hint: "enable | create <spaceId> <tenantId> <name> | member <spaceId> <subjectType> <subjectId> <role> | status [--tenant <id>] [--user <id>] [--agent <id>]"
...
> **Deny-by-default model:** Shared spaces require explicit membership before access. Creating a space does not grant access: memberships must be added separately. No implicit access is granted at any point in the lifecycle. This is a rollout-dependent feature; the shared-memory subsystem is opt-in.
...
| `/memory:shared create team-alpha tenant-1 "Team Alpha"` | Create shared space |
| `/memory:shared member team-alpha user user-42 editor` | Grant editor access |
...
$ARGUMENTS
    ...
    ├─ "create <spaceId> <tenantId> <name>"                  → CREATE/UPDATE SPACE (Section 6)
    ├─ "member <spaceId> <subjectType> <subjectId> <role>"   → SET MEMBERSHIP (Section 7)
...
→ Available Subcommands ──────────────────────────
  ...
  Spaces use a deny-by-default model requiring explicit membership.
...
**Trigger:** `/memory:shared create <spaceId> <tenantId> <name>`

Creates a new shared-memory space or updates an existing one.

### Workflow

1. Parse `spaceId`, `tenantId`, and `name` from arguments
2. Apply optional rollout parameters (`rolloutEnabled`, `rolloutCohort`, `killSwitch`)
3. Call `shared_space_upsert()` with all parameters
4. Display confirmation with space details
...
**Trigger:** `/memory:shared member <spaceId> <subjectType> <subjectId> <role>`

Sets membership for a user or agent in a shared-memory space.

### Workflow

1. Parse `spaceId`, `subjectType`, `subjectId`, and `role` from arguments
2. Validate `subjectType` is `user` or `agent`
3. Validate `role` is `owner`, `editor`, or `viewer`
4. Call `shared_space_membership_set()` with all parameters
...
spec_kit_memory_shared_space_upsert({ spaceId, tenantId, name, rolloutEnabled, rolloutCohort, killSwitch })
spec_kit_memory_shared_space_membership_set({ spaceId, subjectType, subjectId, role })
...
| `shared_space_upsert` | L5 | `create` | Create or update a shared-memory space |
| `shared_space_membership_set` | L5 | `member` | Set deny-by-default membership |
