...
2. CHECK $ARGUMENTS for research topic:
   |-- Has content (ignoring :auto/:confirm and flags) -> research_topic = $ARGUMENTS, omit Q0
   +-- Empty -> include Q0
...
Conduct autonomous iterative deep research with convergence detection. Each iteration dispatches a fresh `@deep-research` agent that reads externalized state, performs focused research, and writes findings to files.
...
Run an iterative research loop: initialize state, dispatch `@deep-research` agent per iteration, evaluate convergence, and synthesize findings into research.md. Use when deep investigation requiring multiple rounds of discovery.

---

## 2. CONTRACT

**Inputs:** `$ARGUMENTS` -- Research topic with optional flags
**Outputs:** Spec folder with research.md + state files + `STATUS=<OK|FAIL|CANCELLED>`
