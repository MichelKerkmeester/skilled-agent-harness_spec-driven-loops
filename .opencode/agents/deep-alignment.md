...
> **SPEC FOLDER PERMISSION:** @deep-alignment writes only the resolved local-owner `alignment/` packet for the target spec, and only through Bash-mediated file operations. Two boundaries protect this, and they are NOT equal in strength — be honest about which is which:
