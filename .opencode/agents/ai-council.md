...
The canonical schema for §8 lives at `.opencode/skills/sk-ai-council/references/output_schema.md` — both this section and the `persist-artifacts.cjs` helper cite it. Schema changes require lockstep update of all three.
...
Reference: `.opencode/skills/sk-ai-council/references/folder_layout.md`.
...
Reference: `.opencode/skills/sk-ai-council/references/state_format.md`.
...
Evolution is additive-only: v1 callers must keep working, and old rows are not rewritten. Full state-format rules live in `.opencode/skills/sk-ai-council/references/state_format.md`.
...
4. **Helper fallback**: non-council callers may still invoke `.opencode/skills/sk-ai-council/scripts/persist-artifacts.cjs`. The helper is now a thin CLI wrapper around the same library exports.
...
The payload routes through existing decision-record, implementation-summary, and handover categories. No new ANCHOR family is introduced. See `.opencode/skills/sk-ai-council/references/command_wiring.md`.
