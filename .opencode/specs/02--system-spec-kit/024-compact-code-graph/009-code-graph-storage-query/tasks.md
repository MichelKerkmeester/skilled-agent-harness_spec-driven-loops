...
- [x] Implement `code_graph_scan` handler — `handlers/code-graph/scan.ts`; accepts workspaceRoot, pathGlobs, languages, incremental, force params
- [x] Implement incremental mode: skip unchanged files via content-hash match — scan.ts
...
- [x] Subject resolution fallback chain: symbolId -> fqName -> filePath — query.ts
...
- [x] Implement schema versioning (SCHEMA_VERSION=1) for future migrations — code-graph-db.ts
