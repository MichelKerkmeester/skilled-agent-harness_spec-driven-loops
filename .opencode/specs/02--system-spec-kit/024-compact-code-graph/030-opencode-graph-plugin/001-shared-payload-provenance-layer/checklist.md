---
title: "Verification Checklist: Shared Payload and Provenance Layer [02--system-spec-kit/024-compact-code-graph/030-opencode-graph-plugin/001-shared-payload-provenance-layer]"
...
- [x] CHK-023 [P1] Phase verification command is still recorded [EVIDENCE: `npm run typecheck`; `npx vitest run tests/session-resume.vitest.ts tests/session-bootstrap.vitest.ts tests/startup-brief.vitest.ts tests/structural-contract.vitest.ts tests/compact-merger.vitest.ts tests/hook-session-start.vitest.ts`]
<!-- /ANCHOR:testing -->
