...
| `006-extra-features` | `007-combined-bug-fixes` | Yes - predecessor exists. |
| `006-extra-features` | `008-architecture-audit` | Yes - successor exists. |
| `008-architecture-audit` | `006-extra-features` | Yes - predecessor exists. |

## Contradictions Found
- No direct feature-behavior contradiction was found between `002-indexing-normalization`, `006-extra-features`, and `008-architecture-audit`; their scopes are mostly distinct.
...
- `006-extra-features` says its predecessor includes "folded historical continuity from retired `008-architecture-audit`", but `008-architecture-audit` is present as an active `Complete` spec and names `006-extra-features` as its predecessor. That creates a contradictory lifecycle state for spec 009.
- `008-architecture-audit` also states that work previously tracked in `008-architecture-audit` was merged into the current `008-architecture-audit` spec, which is self-referential and likely meant to refer to another retired spec ID.
...
- **ISS-D02-003** - Spec 009 lifecycle state is contradictory: `006-extra-features` treats `008-architecture-audit` as retired historical continuity, while `008-architecture-audit` is present and marked `Complete`.
- **ISS-D02-004** - `008-architecture-audit` contains self-referential merge language ("former spec `008-architecture-audit` is consolidated into this spec folder"), which makes lineage unclear.
