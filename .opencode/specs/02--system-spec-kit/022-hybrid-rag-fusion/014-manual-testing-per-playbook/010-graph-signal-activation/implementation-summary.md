...
| Decision | Why |
|----------|-----|
| 091 upgraded to PASS | ANCHOR-as-graph-node feature (catalog `09-anchor-tags-as-graph-nodes.md`) is PLANNED/DEFERRED — a future roadmap item explicitly excluded from current test coverage. The test in `anchor-metadata.vitest.ts` actively guards against edge creation, confirming no accidental implementation. Core N2 (momentum, depth, community) is fully implemented and all scenario pass criteria are satisfied. |
...
| Check | Result |
|-------|--------|
| 15/15 scenarios have verdicts (15 PASS) | PASS |
| 091 ANCHOR-as-node explicitly DEFERRED/SKIPPED — not a test failure | NOTED |
