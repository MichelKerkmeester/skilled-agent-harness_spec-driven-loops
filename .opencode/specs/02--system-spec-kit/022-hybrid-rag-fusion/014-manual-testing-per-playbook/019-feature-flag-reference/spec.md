...
| REQ-008 | Document NEW-125 Hydra roadmap capability flag isolation with its exact prompt, two-snapshot command sequence, evidence target, and feature link. | PASS if runtime SPECKIT_GRAPH_UNIFIED flag does not flip roadmap metadata and prefixed SPECKIT_HYDRA_* env vars can explicitly opt out of the roadmap snapshot |
...
- **SC-002**: `plan.md` defines how evidence, verdicts, and coverage for EX-028, EX-029, EX-030, EX-031, EX-032, EX-033, EX-034, and NEW-125 will be collected.
...
| Dependency | MCP runtime and `dist` build of `capability-flags.js` | Required to execute `memory_search`, `memory_context`, and NEW-125 node-based snapshot scenarios | Verify the dist build is current before executing NEW-125 and confirm `memory_search` is available |
| Risk | NEW-125 requires a freshly compiled dist build; stale builds produce incorrect snapshot results | High | Run `npm run build` inside the mcp_server directory before executing NEW-125 and inspect `capability-flags.ts` if snapshots diverge from expectations |
...
- For NEW-125, should both dist build runs use the same `mcp_server` working directory, or is there a preferred test environment that isolates env var bleed between the two snapshot commands?
<!-- /ANCHOR:questions -->
