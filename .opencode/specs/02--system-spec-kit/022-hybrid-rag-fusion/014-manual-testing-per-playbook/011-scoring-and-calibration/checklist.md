...
- [x] CHK-040 [P0] 159 (Learned Stage 2 Combiner): flag ON pass -- shadow scoring output emitted alongside live combiner, live ranking unaffected; flag OFF pass -- no shadow output; evidence captured -- `learned-combiner.js:386-400`: `shadowScore()` returns null when flagEnabled=false (line 388); returns `{learnedScore, manualScore, delta}` when ON; `predict()` at line 193-199 clamps [0,1]; `isLearnedStage2CombinerEnabled()` at `search-flags.ts:383`
...
- [x] CHK-060 [P0] All 22 scenarios have a verdict (PASS, PARTIAL, or FAIL) with explicit rationale referencing review protocol acceptance rules -- All 22 verdicts: PASS; each with file:line evidence citations
