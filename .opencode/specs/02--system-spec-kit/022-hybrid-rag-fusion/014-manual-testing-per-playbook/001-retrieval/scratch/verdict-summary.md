...
| 6 | 086 | BM25 trigger re-index gate | **PASS** | Trigger edit caused immediate cache invalidation + FTS re-index; new phrase searchable as #1 result; checkpoint restore clean |
| 7 | 109 | 3-tier search fallback | **PARTIAL** | Structural fallback fires for nonsense queries (1 channel vs normal 2-3); _degradation property not surfaced; SPECKIT_SEARCH_FALLBACK=false comparison not executable |
| 8 | 142 | Session trace contract | **PARTIAL** | Trace/non-trace gating works at pipeline level; sessionTransition fields absent (expected: fresh session, no prior state); full contract requires established session |
| 9 | 143 | Bounded graph-walk rollout | **PARTIAL** | bounded_runtime state verified with correct trace data; 1/3 rollout states tested; trace_only/off require env var changes at MCP server level |

---

## 5-Point Review Protocol

| Check | EX-001 | EX-002 | EX-003 | EX-004 | EX-005 | 086 | 109 | 142 | 143 |
|-------|--------|--------|--------|--------|--------|---------|---------|---------|---------|
...
*109/143: Partial command sequence — env var steps not executable from tool context.
...
- 109: Cannot disable SPECKIT_SEARCH_FALLBACK
- 142: Cannot establish prior session state for transition testing
- 143: Cannot switch between rollout states (trace_only, bounded_runtime, off)
