...
description: "Phase 013 defines the execution plan for 25 memory-quality-and-indexing manual tests in the Spec Kit Memory system. It sequences preconditions, non-destructive checks, sandboxed destructive scenarios, evidence capture, and review-protocol verdicting for save, indexing, and memory-maintenance behavior."
trigger_phrases:
...
- [x] PASS, PARTIAL, FAIL, and coverage rules were loaded from [`../../manual_testing_playbook/review_protocol.md`](../../manual_testing_playbook/review_protocol.md).
- [x] All 25 test IDs were mapped to feature-catalog context, including source-backed judgment calls for M-001 through M-004.
...
### Definition of Done
- [ ] All 25 scenarios have prompt, command, evidence, and verdict records tied to the exact playbook wording.
- [ ] Every destructive scenario was executed in a disposable sandbox or restored from checkpoint before phase closeout.
- [ ] Coverage is reported as 25/25 scenarios for Phase 013 with no skipped test IDs.
...
- **Verdict layer**: review-protocol checks that classify each scenario as PASS, PARTIAL, or FAIL and enforce full phase coverage.
...
- [ ] Verify the playbook, review protocol, and all linked feature files are open and match the Phase 013 test inventory.
...
- [ ] Run inspection and dry-run-first scenarios before mutation-heavy cases: NEW-039, NEW-040, NEW-041, NEW-045, NEW-046, NEW-047, NEW-048, NEW-069, NEW-073, NEW-131, NEW-133, M-001, M-002, and M-004.
...
- [ ] Restrict NEW-042, NEW-043, NEW-044, NEW-111, NEW-119, NEW-132, M-003, M-005, M-006, M-007, and M-008 to disposable sandboxes, checkpoints, or isolated runtime sessions.
...
- [ ] For each scenario, capture prompt, exact command sequence, raw output, expected signals, evidence artifact paths, and reviewer rationale.
...
- [ ] Assign PASS, PARTIAL, or FAIL per scenario and summarize phase coverage as 25/25 scenarios with linked evidence references.
<!-- /ANCHOR:phases -->
...
| NEW-133 | Confirm dry-run previews preflight plus semantic insufficiency without indexing side effects | `Validate memory_save dryRun preview behavior, including insufficiency detection.` | MCP |
...
| [`../../feature_catalog/13--memory-quality-and-indexing/`](../../feature_catalog/13--memory-quality-and-indexing/) plus [`../../feature_catalog/02--mutation/10-per-memory-history-log.md`](../../feature_catalog/02--mutation/10-per-memory-history-log.md) | Internal | Green | Test-to-feature context and cross-category mapping rationale lose their canonical references |
