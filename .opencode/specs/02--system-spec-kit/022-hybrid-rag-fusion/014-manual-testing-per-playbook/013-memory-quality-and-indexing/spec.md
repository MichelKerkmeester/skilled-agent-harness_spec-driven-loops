...
description: "Phase 013 documents the memory-quality-and-indexing manual test packet for the Spec Kit Memory system. It now tracks 49 exact scenario IDs by expanding the dedicated memory-section sub-scenarios alongside the original top-level inventory plus Wave 3-5 additions."
trigger_phrases:
...
### Problem Statement
Manual test scenarios for memory-quality-and-indexing currently live in the central playbook and need structured per-phase documentation that preserves exact prompts, command sequences, evidence expectations, and review-protocol verdict rules. The earlier Phase 013 packet only modeled `26` top-level scenarios and collapsed the dedicated memory-section sub-scenarios, which meant active exact IDs such as `M-005a`, `M-006b`, and `M-007g` were not represented literally in the phase documentation. Wave 3-5 additions introduce batch learned feedback aggregation, assistive reconsolidation threshold tiers, implicit feedback logging, hybrid decay policy, and save quality gate exceptions.

### Purpose
Provide a single memory-quality-and-indexing specification that maps all `49` exact Phase 013 scenario IDs to canonical feature context and playbook-derived acceptance criteria so execution and review remain consistent with the current manual testing playbook.
<!-- /ANCHOR:problem -->
...
| M-006 | Stateless Enrichment and Alignment Guardrails | [`../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/18-session-enrichment-and-alignment-guards.md`](../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/18-session-enrichment-and-alignment-guards.md) | Direct category match for the umbrella stateless enrichment scenario. |
| M-006a | Unborn-HEAD and dirty snapshot fallback | [`../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/18-session-enrichment-and-alignment-guards.md`](../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/18-session-enrichment-and-alignment-guards.md) | Explicit child scenario for pre-commit repo snapshot behavior. |
| M-006b | Detached-HEAD snapshot preservation | [`../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/18-session-enrichment-and-alignment-guards.md`](../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/18-session-enrichment-and-alignment-guards.md) | Explicit child scenario for detached-HEAD commit identity retention. |
| M-006c | Similar-folder boundary protection | [`../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/18-session-enrichment-and-alignment-guards.md`](../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/18-session-enrichment-and-alignment-guards.md) | Explicit child scenario for similarly named foreign-folder exclusion. |
...
| 176 | Implicit Feedback Log (SPECKIT_IMPLICIT_FEEDBACK_LOG) | [`../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/22-implicit-feedback-log.md`](../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/22-implicit-feedback-log.md) | Validates shadow-only implicit feedback event ledger records 5 event types with correct confidence tiers. |
| 177 | Hybrid Decay Policy (SPECKIT_HYBRID_DECAY_POLICY) | [`../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/23-hybrid-decay-policy.md`](../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/23-hybrid-decay-policy.md) | Validates type-aware no-decay FSRS policy assigns Infinity stability to decision/constitutional/critical types. |
| 178 | Save Quality Gate Exceptions (SPECKIT_SAVE_QUALITY_GATE_EXCEPTIONS) | [`../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/24-save-quality-gate-exceptions.md`](../../../../../skill/system-spec-kit/feature_catalog/13--memory-quality-and-indexing/24-save-quality-gate-exceptions.md) | Validates short-critical quality gate exception allows decision documents with >=2 structural signals to bypass 50-char minimum. |

### Out of Scope
- Executing the 49 exact scenario IDs and assigning final run verdicts.
...
| REQ-047 | Document 176 (Implicit Feedback Log) with its exact playbook prompt, command sequence, evidence target, and mapped feature link. Feature flag: `SPECKIT_IMPLICIT_FEEDBACK_LOG` (default: OFF). | PASS when flag ON: the shadow-only implicit feedback ledger records all 5 event types, each event carries the expected confidence tier, and no live ranking changes are driven directly by the shadow log; PASS when flag OFF: no implicit feedback events are recorded. FAIL when any event type is missing, confidence tiers drift from the documented mapping, or disabling the flag still records events. |
| REQ-048 | Document 177 (Hybrid Decay Policy) with its exact playbook prompt, command sequence, evidence target, and mapped feature link. Feature flag: `SPECKIT_HYBRID_DECAY_POLICY` (default: OFF). | PASS when flag ON: decision, constitutional, and critical memory types receive Infinity stability with no decay, while other memory types continue to use the configured FSRS decay policy; PASS when flag OFF: all memory types follow the default decay behavior. FAIL when protected types decay despite the flag being enabled, when unprotected types incorrectly receive Infinity stability, or when disabling the flag leaves the no-decay behavior active. |
| REQ-049 | Document 178 (Save Quality Gate Exceptions) with its exact playbook prompt, command sequence, evidence target, and mapped feature link. Feature flag: `SPECKIT_SAVE_QUALITY_GATE_EXCEPTIONS` (default: OFF). | PASS when flag ON: short critical decision documents that provide at least 2 structural signals bypass the 50-character minimum, the exception is limited to the documented qualifying cases, and non-qualifying short saves still fail the quality gate; PASS when flag OFF: all short saves remain subject to the standard minimum-length gate. FAIL when non-qualifying documents bypass the gate, when qualifying decision documents are still rejected, or when disabling the flag still allows the exception path. |

No P1 items are defined for this phase; all 49 exact scenario IDs are required for Phase 013 coverage.
<!-- /ANCHOR:requirements -->
...
- **SC-001**: All 49 exact Phase 013 scenario IDs are documented with exact prompts, command sequences, evidence expectations, and review-protocol verdict language.
- **SC-002**: Every exact scenario ID in this phase links to a feature-catalog file, including explicit source-backed mappings for `M-005a..c`, `M-006a..c`, `M-007a..j`, `155`/`155-F`, `164`, `165`, `176`, `177`, and `178`.
...
- **SC-004**: Reviewers can assess the phase as 49/49 documented exact IDs with no omitted dedicated-memory sub-scenarios and no shorthand-only `M-007` coverage.
<!-- /ANCHOR:success-criteria -->
