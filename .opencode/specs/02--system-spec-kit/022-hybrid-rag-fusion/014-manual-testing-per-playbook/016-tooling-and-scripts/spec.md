...
| `REQ-154` | Document `154` JSON-primary deprecation posture validation covering routine saves requiring `--json`/`--stdin`, direct positional capture requiring `--recovery`, and operator guidance reflecting the two-mode contract. | PASS if routine saves require JSON mode, `--recovery` flag is enforced for positional capture, and operator-facing docs reflect the two-mode contract. |
