...
trigger_phrases:
  ...
  - "NEW-061 NEW-113 PHASE-005 verification"
...
- [x] CHK-010 [P0] NEW-061 documents the tree-thinning validation prompt with expected signals: small files merged, positive token savings, and preserved content integrity [EVIDENCE: spec.md REQ-NEW-061 and plan.md testing strategy row]
- [x] CHK-011 [P0] NEW-062 documents the progressive-validation prompt with expected signals: levels 1-4 produce progressively stricter validation and exit codes match severity [EVIDENCE: spec.md REQ-NEW-062 and plan.md testing strategy row]
- [x] CHK-012 [P0] NEW-070 documents the dead-code-removal audit prompt with expected signals: removed symbols have zero references and representative flows execute cleanly [EVIDENCE: spec.md REQ-NEW-070 and plan.md testing strategy row]
- [x] CHK-013 [P0] NEW-089 documents the code-standards-alignment prompt with expected signals: zero naming, commenting, and import-order mismatches [EVIDENCE: spec.md REQ-NEW-089 and plan.md testing strategy row]
- [x] CHK-014 [P0] NEW-099 documents the SPECKIT_FILE_WATCHER prompt with expected signals: debounce works, hash dedup prevents redundant reindex, and ENOENT is handled silently [EVIDENCE: spec.md REQ-NEW-099 and plan.md testing strategy row]
- [x] CHK-015 [P0] NEW-113 documents the standalone admin CLI prompt covering all 4 commands (stats, bulk-delete, reindex, downgrade) with expected signals: dry-run, safety prompt, and best-effort checkpoint [EVIDENCE: spec.md REQ-NEW-113 and plan.md testing strategy row]
- [x] CHK-016 [P0] NEW-127 documents the migration checkpoint script verification prompt with expected signal: `migration-checkpoint-scripts.vitest.ts` completes with all tests passing [EVIDENCE: spec.md REQ-NEW-127 and plan.md testing strategy row]
- [x] CHK-017 [P0] NEW-128 documents the schema compatibility validation prompt with expected signal: `vector-index-schema-compatibility.vitest.ts` completes with all tests passing [EVIDENCE: spec.md REQ-NEW-128 and plan.md testing strategy row]
- [x] CHK-018 [P0] NEW-135, NEW-136, and NEW-137 document their feature-catalog grep traceability and annotation prompts with expected signals: multi-layer hits, zero mismatches, and no orphaned references [EVIDENCE: spec.md REQ-NEW-135 through REQ-NEW-137 and plan.md rows]
- [x] CHK-019 [P0] NEW-138 documents the MODULE header compliance prompt with expected signal: zero `TS-MODULE-HEADER` findings [EVIDENCE: spec.md REQ-NEW-138 and plan.md testing strategy row]
- [x] CHK-020 [P0] NEW-139 documents the full M-007 closure verification prompt covering JSON authority, stateless enrichment, all native fallback chain paths (OpenCode, Claude, Codex, Copilot, Gemini), numeric quality calibration, and indexing readiness [EVIDENCE: spec.md REQ-NEW-139 and plan.md testing strategy row with full M-007 prompt]
- [x] CHK-021 [P0] NEW-147 documents the `/memory:learn` constitutional manager prompt with expected signals: all command flows match the constitutional contract and no active docs advertise legacy behavior [EVIDENCE: spec.md REQ-NEW-147 and plan.md testing strategy row]
- [x] CHK-022 [P0] NEW-149 documents the rendered-memory template contract prompt with expected signals: malformed files rejected before write/index, apply-mode reports are validator-clean, and active audit shows no structural violations [EVIDENCE: spec.md REQ-NEW-149 and plan.md testing strategy row]
...
- [ ] CHK-024 [P1] Destructive test boundary is clearly stated for NEW-099, NEW-113, NEW-139, NEW-149, PHASE-002, PHASE-003, PHASE-004, and PHASE-005 — sandbox targets or disposable folders only [EVIDENCE: plan.md Phase 3 instructions reference disposable scope and cleanup rules]
<!-- /ANCHOR:code-quality -->
...
- [ ] CHK-030 [P0] All 12 non-destructive scenarios (NEW-061, NEW-062, NEW-070, NEW-089, NEW-127, NEW-128, NEW-135, NEW-136, NEW-137, NEW-138, NEW-147, PHASE-001) have been executed and evidence is captured [EVIDENCE: named evidence artifacts per scenario present]
- [ ] CHK-031 [P0] All 8 destructive/sandbox scenarios (NEW-099, NEW-113, NEW-139, NEW-149, PHASE-002, PHASE-003, PHASE-004, PHASE-005) have been executed in disposable sandbox targets with evidence captured [EVIDENCE: named evidence artifacts and sandbox cleanup confirmation present]
...
- [x] CHK-041 [P0] NEW-113 bulk-delete coverage uses `specs/test-sandbox` or equivalent disposable scope only; no production memory folders are targeted [EVIDENCE: plan.md Phase 3 NEW-113 step specifies disposable scope]
- [x] CHK-042 [P1] NEW-139 and NEW-149 malformed/thin memory files are kept outside active production memory folders except when the test explicitly audits the active corpus [EVIDENCE: plan.md Phase 3 NEW-139 and NEW-149 steps include scope boundary]
- [ ] CHK-043 [P2] Evidence logs from watcher tests (NEW-099) do not include raw absolute paths from non-sandbox directories [EVIDENCE: evidence review confirms path sanitization]
<!-- /ANCHOR:security -->
