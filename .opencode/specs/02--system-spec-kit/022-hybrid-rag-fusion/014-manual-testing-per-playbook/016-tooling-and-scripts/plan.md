...
- [ ] Run documentation, grep, audit, and targeted-suite scenarios first: `NEW-061`, `NEW-062`, `NEW-070`, `NEW-089`, `NEW-127`, `NEW-128`, `NEW-135`, `NEW-136`, `NEW-137`, `NEW-138`, `NEW-147`, and `PHASE-001`.
...
- [ ] Constrain watcher, admin CLI, session-capture, malformed-memory, and phase-folder workflows to sandbox targets: `NEW-099`, `NEW-113`, `NEW-139`, `NEW-149`, `PHASE-002`, `PHASE-003`, `PHASE-004`, and `PHASE-005`.
- [ ] For `NEW-113`, use `specs/test-sandbox` or equivalent disposable scope and preserve any checkpoint-warning evidence.
- [ ] For `NEW-139` and `NEW-149`, keep malformed or thin-memory files outside active production memory folders except when the test explicitly audits the active corpus.
...
|---|---|---|---|
| `NEW-061` | Tree thinning for spec folder consolidation | `Validate tree thinning behavior (PI-B1).` | manual |
| `NEW-062` | Progressive validation for spec documents | `Run progressive validation (PI-B2).` | manual |
| `NEW-070` | Dead code removal | `Audit dead code removal outcomes.` | manual |
| `NEW-089` | Code standards alignment | `Validate code standards alignment outcomes.` | manual |
| `NEW-099` | Real-time filesystem watching (P1-7) | `Validate SPECKIT_FILE_WATCHER behavior.` | manual |
| `NEW-113` | Standalone admin CLI | `Validate standalone admin CLI commands.` | manual |
| `NEW-127` | Migration checkpoint scripts | `Run the migration checkpoint script verification suite.` | manual |
| `NEW-128` | Schema compatibility validation | `Run the schema compatibility validation suite.` | manual |
| `NEW-135` | Grep traceability for feature catalog code references | `Validate feature catalog grep traceability.` | manual |
| `NEW-136` | Feature catalog annotation name validity | `Validate all Feature catalog annotation names against catalog.` | manual |
| `NEW-137` | Multi-feature annotation coverage | `Validate multi-feature files carry all applicable annotations.` | manual |
| `NEW-138` | MODULE: header compliance via `verify_alignment_drift.py` | `Validate MODULE: header compliance across all non-test .ts files.` | manual |
| `NEW-139` | Session capturing pipeline quality | `Run full closure verification for spec 010-perfect-session-capturing, including JSON authority, stateless enrichment, the full native fallback chain (OpenCode, Claude, Codex, Copilot, Gemini), numeric quality calibration, and indexing readiness.` | manual |
| `NEW-147` | Constitutional memory manager command | `Validate /memory:learn constitutional manager flow and documentation consistency.` | manual |
| `NEW-149` | Rendered memory template contract | `Validate the rendered-memory template contract for memory_save, generate-context, and historical remediation.` | MCP |
