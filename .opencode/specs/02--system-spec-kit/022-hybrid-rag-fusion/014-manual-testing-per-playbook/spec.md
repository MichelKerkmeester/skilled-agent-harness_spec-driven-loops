...
description: "Umbrella parent packet for manual testing phases. This root spec tracks 19 feature-catalog categories against the current 229KB playbook inventory, including 195 top-level IDs and 213 exact scenario IDs once dedicated memory sub-scenarios are included."
trigger_phrases:
  - "manual testing"
  - "testing playbook"
  - "phase parent"
...
### Problem Statement
The manual testing playbook is no longer accurately represented by the original parent packet. The playbook still contains `195` top-level IDs (`EX-001..035`, `001..149`, `PHASE-001..005`, `M-001..008`), but the dedicated memory section now expands that inventory to `213` exact scenario IDs once suffixed sub-scenarios such as `M-005a..c`, `M-006a..c`, and `M-007a..j` are included. The current parent packet still treats the top-level cross-reference index as the authoritative coverage unit, which allows the packet to look complete while exact playbook scenarios remain undocumented.
...
| `implementation-summary.md` | Modify | Parent summary updated to 213 exact IDs and the March 17 validator state |
...
| 013 | `013-memory-quality-and-indexing/` | Memory quality tests | 42 exact IDs (039..048, 069, 073, 092, 111, 119, 131..133, M-001..008, M-005a..c, M-006a..c, M-007a..j) | Draft |
...
| REQ-003 | Every active exact playbook scenario ID appears in exactly one phase spec.md | Exact-ID audit finds `0` missing IDs and `0` duplicate owners across `213` exact scenario IDs |
...
- **SC-001**: All `213` exact scenario IDs from the current playbook are documented across the 19 phase folders with exact-one ownership.
