...
description: "Umbrella parent packet for manual testing phases. This root spec tracks 19 feature-catalog categories against the current 229KB playbook inventory, including 195 top-level IDs and 227 exact scenario IDs once dedicated memory sub-scenarios are included."
trigger_phrases:
  - "manual testing"
  - "testing playbook"
  - "phase parent"
...
| 010 | `010-graph-signal-activation/` | Graph signal tests | 10 exact IDs (016..022, 081, 091, 120) | Draft |
| 011 | `011-scoring-and-calibration/` | Scoring tests | 17 exact IDs (023..032, 066, 074, 079, 098, 102, 118, 121) | Draft |
| 012 | `012-query-intelligence/` | Query intelligence tests | 6 exact IDs (033..038) | Draft |
| 013 | `013-memory-quality-and-indexing/` | Memory quality tests | 44 exact IDs (039..048, 069, 073, 092, 111, 119, 131..133, M-001..008, M-005a..c, M-006a..c, M-007a..j, 155, 155-F) | Draft |
...
- **Normalization carried forward** -> the packet now uses bare numeric `NNN` IDs instead of provisional `NEW-NNN` markers, and Phase `016` includes scenarios `153` through `153-L` and `154`, while Phase `013` includes `155` and `155-F`, bringing the exact-ID total to `227`.
...
| REQ-003 | Every active exact playbook scenario ID appears in exactly one phase spec.md | Exact-ID audit finds `0` missing IDs and `0` duplicate owners across `227` exact scenario IDs |
...
- **SC-001**: All `227` exact scenario IDs from the current playbook are documented across the 19 phase folders with exact-one ownership.
