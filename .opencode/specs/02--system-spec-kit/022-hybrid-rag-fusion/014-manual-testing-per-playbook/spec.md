...
description: "Umbrella parent packet for manual testing phases. This root spec tracks 19 feature-catalog categories against the current playbook inventory, including 195 top-level IDs and 241 exact scenario IDs once dedicated memory sub-scenarios and Wave 2-4 additions (156-169) are included."
trigger_phrases:
  - "manual testing"
  - "testing playbook"
  - "phase parent"
...
### Problem Statement
The manual testing playbook is no longer accurately represented by the original parent packet. The playbook still contains `195` top-level IDs (`EX-001..035`, `001..155`, `PHASE-001..005`, `M-001..008`), but the dedicated memory section now expands that inventory to `241` exact scenario IDs once suffixed sub-scenarios such as `M-005a..c`, `M-006a..c`, `M-007a..j`, `153-A..L`, `155-F`, and Wave 2-4 additions `156..169` are included. The current parent packet still treats the top-level cross-reference index as the authoritative coverage unit, which allows the packet to look complete while exact playbook scenarios remain undocumented.

### Purpose
Provide a canonical parent packet with phase linkage across 19 child folders, using the current exact-ID playbook inventory (241 exact IDs) as the authoritative coverage unit and recording the real validator state for the packet.
<!-- /ANCHOR:problem -->
...
- Exact-ID coverage: every active scenario ID from the current playbook, including the dedicated memory-section suffixed IDs and Wave 2-4 additions (156-169), mapped to exactly one phase.
...
| `implementation-summary.md` | Modify | Parent summary updated to 227 exact IDs and the March 17 validator state |
...
| 010 | `010-graph-signal-activation/` | Graph signal tests | 13 exact IDs (016..022, 081, 091, 120, 156, 157, 158) | Draft |
| 011 | `011-scoring-and-calibration/` | Scoring tests | 19 exact IDs (023..032, 066, 074, 079, 098, 102, 118, 121, 159, 160) | Draft |
| 012 | `012-query-intelligence/` | Query intelligence tests | 9 exact IDs (033..038, 161, 162, 163) | Draft |
| 013 | `013-memory-quality-and-indexing/` | Memory quality tests | 46 exact IDs (039..048, 069, 073, 092, 111, 119, 131..133, M-001..008, M-005a..c, M-006a..c, M-007a..j, 155, 155-F, 164, 165) | Draft |
...
| 018 | `018-ux-hooks/` | UX hook tests | 9 exact IDs (103..107, 166, 167, 168, 169) | Draft |
...
| REQ-003 | Every active exact playbook scenario ID appears in exactly one phase spec.md | Exact-ID audit finds `0` missing IDs and `0` duplicate owners across `241` exact scenario IDs |
...
- **SC-001**: All `241` exact scenario IDs from the current playbook are documented across the 19 phase folders with exact-one ownership.
