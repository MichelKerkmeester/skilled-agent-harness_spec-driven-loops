MCP issues detected. Run /mcp list for status.## GEMINI-DELTA: Spec 030 Design Review

### Dimension Scores
| Dimension | Score (1-10) | Assessment |
|-----------|-------------|------------|
| 1. Problem Framing | 6 | Identifies valid boundary gaps but entirely misses the CRITICAL evasion vectors identified in predecessor Spec 029 (e.g., `core/*` not blocked, dynamic imports undetected). |
...
#### CRITICAL
- **Ignored Spec 029 Evasion Vectors**: Spec 030 completely ignores the critical enforcement flaws found in the Spec 029 audit (e.g., `check-no-mcp-lib-imports.ts` does not block `core/*` imports, ignores dynamic imports, and fails on relative paths). Remediation of the enforcement *scripts themselves* must be included.
...
1. **Add Enforcement Script Fixes**: Add a Phase to update `check-no-mcp-lib-imports.ts` and `check-api-boundary.sh` to fix the evasion vectors identified in Spec 029 (block `core/*`, detect relative paths, and catch dynamic imports).
