...
- [ ] **T31** Correct the root 022 status contract for phase 015.
...
- [ ] **T32** Expand epic certification to the live 11-child subtree.
...
- [ ] **T33** Repair sprint-tail navigation from sprint 010 to 011.
...
- [ ] **T34** Truth-sync the retrieval audit coverage claim from 10 to the live 11-feature inventory.
...
- [ ] **T35** Reconcile `021-remediation-revalidation` with the still-open 022 release packet.
...
- [ ] **T36** Replace unbacked Hydra safety-rail verification with real drill evidence or honest pending state.
...
- [ ] **T37** Truth-sync root 022 direct/recursive directory totals.
  - **Files:** `.opencode/specs/02--system-spec-kit/022-hybrid-rag-fusion/spec.md`
...
- [ ] **T38** Correct `006-feature-catalog` snippet totals.
...
- [ ] **T39** Correct `006-feature-catalog` category totals.
  - **Files:** `.opencode/specs/02--system-spec-kit/022-hybrid-rag-fusion/006-feature-catalog/spec.md`
...
- [ ] **T40** Add live child `022` to the 007 umbrella inventory.
...
- [ ] **T41** Fix stale evaluation inventory counts in 007/009.
...
- [ ] **T42** Fix stale scoring inventory counts in 007/011.
...
- [ ] **T43** Rebuild the 015 umbrella playbook totals from the live tree.
...
- [ ] **T44** Fix `014-agents-md-alignment` command inventory drift.
...
- [ ] **T45** Fix `018-rewrite-system-speckit-readme` command inventory drift.
...
- [ ] **T46** Fix `016-rewrite-memory-mcp-readme` tool inventory drift.
...
- [ ] **T47** Truth-sync root README agent/MCP totals and missing `@deep-review`.
...
- [ ] **T48** Refresh the root 022 checklist against the current validator.
...
- [ ] **T49** Make the epic phase map mirror child status labels verbatim.
...
- [ ] **T50** Resolve contradictory shipped-model/final-phase language in `010-template-compliance-enforcement`.
...
- [ ] **T51** Fix impossible upstream blocker totals in the Hydra umbrella checklist.
...
- [ ] **T52** Remove premature `Complete` status from Hydra children awaiting sign-off.
...
- [ ] **T53** Align Hydra child summaries with the umbrella’s actual activation caveats.
...
- [ ] **T54** Reconcile sequencing/dependency drift between session phases 007 and 008.
...
- [ ] **T55** Reopen or truthfully scope `016-json-mode-hybrid-enrichment`.
...
- [ ] **T56** Align `017-json-primary-deprecation` docs with the shipped runtime.
...
- [ ] **T57** Reconcile the T04 contradiction inside this packet’s story of prior remediation.
...
- [ ] **T58** Resolve `012-command-alignment` done/not-done contradictions.
...
- [ ] **T59** Remove over-claiming from `013-agents-alignment`.
...
- [ ] **T60** Fix the 015 umbrella’s `Complete` claim while child packets remain open.
...
- [ ] **T61** Remove the false verified-P1 claim from `013-memory-quality-and-indexing`.
...
- [ ] **T62** Update the executed second-half 015 packets that still say `Not Started`.
...
- [ ] **T63** Sync rewrite packet status with actual task completion.
...
- [ ] **T64** Add a root navigation/traceability contract to `005-architecture-audit`.
...
- [ ] **T65** Repair broken evidence links in `005-architecture-audit` and `010-template-compliance-enforcement`.
...
- [ ] **T66** Add a traceability contract for completed 007 second-half phases.
...
- [ ] **T67** Complete the Level 3+ companion docs for `016-json-mode-hybrid-enrichment`.
...
- [ ] **T68** Remove orphaned references to deleted `013-memory-generation-quality`.
  - **Files:** `.opencode/specs/02--system-spec-kit/022-hybrid-rag-fusion/**/*.md` (all docs referencing `013-memory-generation-quality`)
  ...
  - **Verification:** `rg -n "013-memory-generation-quality" .opencode/specs/02--system-spec-kit/022-hybrid-rag-fusion`

- [ ] **T69** Fix the wrong parent pointer under `011-skill-alignment/001-post-session-capturing-alignment`.
...
- [ ] **T70** Repair nonexistent playbook-path references in 015 child packets 003/004/007.
...
- [ ] **T71** Replace placeholder testing packets 020-022 with real packets or honest draft status.
...
- [ ] **T72** Make the BM25 spec-folder scope filter fail closed.
...
- [ ] **T73** Bind working-memory access to trusted server-side session scope.
...
- [ ] **T74** Scope the governance audit away from full-table enumeration by default.
...
- [ ] **T75** Sanitize persisted/surfaced embedding-provider failure messages.
...
- [ ] **T76** Add an atomic claim step before retry work is processed.
...
- [ ] **T77** Remove stale auto-entity rows during in-place memory updates.
...
- [ ] **T78** Make SIGINT/SIGTERM cleanup clear workflow locks before reporting success.
...
- [ ] **T79** Stop structured JSON saves from reporting complete when `nextSteps` remain pending.
...
- [ ] **T80** Return bounded validation errors for empty `--json` input.
...
- [ ] **T81** Align startup embedding-dimension validation with runtime fallback rules.
...
- [ ] **T82** Reject invalid `EMBEDDINGS_PROVIDER` values at startup.
  - **Files:** `.opencode/skill/system-spec-kit/shared/embeddings.ts`, `.opencode/skill/system-spec-kit/shared/embeddings/factory.ts`, `.opencode/skill/system-spec-kit/mcp_server/context-server.ts`
...
- [ ] **T83** Respect configured `VOYAGE_BASE_URL` during startup validation.
...
- [ ] **T84** Remove dead MCP-server code that still appears in release surfaces.
...
- [ ] **T85** Keep `npm run check` green after the v3 code/doc sweep.
...
- [ ] **T86** Remove the production TODO marker from vector-index mutations.
...
- [ ] **T87** Eliminate or regenerate orphaned dist artifacts.
...
- [ ] **T88** Remove stale catalog references to deleted code/tests.
...
- [ ] **T89** Add catalog coverage for the three uncataloged audit categories.
...
- [ ] **T90** Replace brittle number-based catalog/audit matching with slug-based matching.
...
- [ ] **T91** Migrate Python CLI scripts from `sys.argv` to `argparse`.
...
- [ ] **T92** Move shell strict mode to the top of the affected scripts.
...
- [ ] **T93** Improve weak `description.json` metadata for umbrella packets.
...
- [ ] **T94** Raise playbook coverage above the current 75% and clean orphan scenarios.
...
- [ ] **T95** Resolve dormant code modules instead of leaving them half-supported.
...
- [ ] **T96** Fix stale sprint metadata in sprints 5, 6, and 11.
...
- [ ] **T97** Close the architecture components gap between docs and code.
...
- [ ] Historical Phases 1-4 remain preserved as completed reference and are not used as the active release gate.
- [ ] All active v3 tasks `T31-T97` are completed and evidence-backed.
- [ ] `.opencode/skill/system-spec-kit/scripts/spec/validate.sh` passes on the target tree with exit code `0` or `1`, and no unresolved errors remain in the v3 remediation surface.
- [ ] `npm run check` passes after all doc/code truth-sync changes.
- [ ] `npm run test` passes after all doc/code truth-sync changes.
- [ ] A fresh review-report re-verification/deep review confirms the 022 tree reaches a `100/100` release-readiness score with no open P0/P1 contradictions.
<!-- /ANCHOR:completion -->
