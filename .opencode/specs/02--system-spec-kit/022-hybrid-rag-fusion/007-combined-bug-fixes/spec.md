...
1. `node generate-context.js "007-combined-bug-fixes"` succeeds (bare name)
...
| Field | Value |
|-------|-------|
| **Canonical Spec Folder** | `007-combined-bug-fixes` |
| **Parent Spec** | `.opencode/specs/02--system-spec-kit/022-hybrid-rag-fusion/` |
...
This unified spec makes `007-combined-bug-fixes` the only canonical identity and records all three workstreams in one standard Level 2 packet.
<!-- /ANCHOR:problem-013 -->
...
`007-combined-bug-fixes` is the canonical destination for merged remediation and audit follow-up.

It supersedes active planning ownership from:
- the historical remediation-epic lineage now folded into `007-combined-bug-fixes` (including consolidated 002/022/023/024/025/026 roots)
- `008-architecture-audit` (cross-AI audit provenance and session handover state)

The remediation-epic lineage now lives inside `007-combined-bug-fixes`, while `008-architecture-audit` is retained as historical lineage; `007-combined-bug-fixes` is the live canonical folder.
...
- Runtime verification tasks (005-core-rag-sprints-0-to-9/019-extra-features T012-T098) -- requires live MCP server
...
1. **Given** the canonical folder is validated, **when** the validator runs for `007-combined-bug-fixes`, **then** `TEMPLATE_SOURCE`, `FRONTMATTER_VALID`, and `LEVEL_MATCH` pass without missing-marker findings.
...
| P0-003 | Verify   | 41 runtime verification tasks never executed against live MCP server             | 005-core-rag-sprints-0-to-9/019-extra-features        |
| P0-004 | Verify   | eval_run_ablation never run -- spec's own P0 hard blocker for release            | 005-core-rag-sprints-0-to-9/019-extra-features        |
