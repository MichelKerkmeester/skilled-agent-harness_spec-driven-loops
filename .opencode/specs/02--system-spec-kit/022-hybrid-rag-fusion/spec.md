...
description: "Root Level 3+ coordination packet for the Hybrid RAG Fusion program, tracking the live 18-phase tree and the current 107 numbered spec directories on disk."
trigger_phrases:
...
This root packet is the coordination layer for the Hybrid RAG Fusion program. It records the live 18 direct phases under `022`, the current 107 numbered spec directories on disk, and the remaining subtree normalization work without overstating completion.
...
Provide a concise root packet that records the current 022 tree truth, anchors the 18 direct phases, and makes remaining subtree normalization explicit.
<!-- /ANCHOR:problem -->
...
| `.opencode/specs/02--system-spec-kit/022-hybrid-rag-fusion/002-*/spec.md` through `018-*/spec.md` | Modify | Keep direct-child navigation aligned with the live root tree |
<!-- /ANCHOR:scope -->
...
| 009 | `009-perfect-session-capturing/` | Session-capturing packet family | In Progress |
...
| 014 | `014-manual-testing-per-playbook/` | Manual testing playbook umbrella | Draft |
| 015 | `015-rewrite-memory-mcp-readme/` | README rewrite | In Progress |
| 016 | `016-update-install-guide/` | Install guide update | In Progress |
| 017 | `017-rewrite-system-speckit-readme/` | System Spec Kit README rewrite | In Progress |
| 018 | `018-rewrite-repo-readme/` | Repository README rewrite | In Progress |
...
| REQ-002 | Root status claims use verified current tree counts | Root docs state `107` numbered spec directories and direct child counts `001=10`, `007=21`, `008=6`, `009=20`, `010=1`, `014=19` |
| REQ-003 | Root packet preserves the current truth of phases `009` and `014` | Root docs state that phase `009` has 20 children and phase `014` remains draft or documentation-only |
...
| REQ-005 | Root packet includes a live direct-phase map | `PHASE DOCUMENTATION MAP` lists all 18 direct phases with truthful status values |
...
- **SC-002**: Root docs consistently use `107` numbered spec directories and the verified direct-child counts.
- **SC-003**: Direct child packets `002-018` keep explicit root-facing phase navigation entries.
...
- Phase `014` remains draft and documentation-only even though its child inventory is large.
...
|-----------|-------|----------|
| Scope | 21/25 | Root packet rewrite plus 17 direct child navigation edits |
...
**Acceptance Criteria**:
1. Given any direct child packet from `002` to `018`, when I open it, then it contains consistent parent and neighboring-phase rows.
...
1. **Given** the root packet is opened, **When** the metadata table is reviewed, **Then** it reports `107` numbered spec directories and the verified direct-child counts.
2. **Given** the direct phase map is opened, **When** the user scans phases `001` through `018`, **Then** every live direct child appears exactly once with a truthful status.
...
4. **Given** a reader checks phase `014`, **When** they read the root packet, **Then** they see that the umbrella remains draft or documentation-only.
