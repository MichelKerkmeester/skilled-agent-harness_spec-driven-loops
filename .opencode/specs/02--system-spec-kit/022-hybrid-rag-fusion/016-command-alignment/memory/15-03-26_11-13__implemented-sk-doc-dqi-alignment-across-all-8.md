...
### Decision 1: Decision: Tag selection for code blocks uses text for output/dashboards, yaml fo

**Context**: Decision: Tag selection for code blocks uses text for output/dashboards, yaml for config, javascript for tool calls, bash for CLI, markdown for templates. Rationale: matches actual content semantics per sk-doc requirements.

**Timestamp**: 2026-03-15T10:13:40.760Z
...
```
╭────────────────────────────────────────────────╮
│  DECISION: Decision: Tag selection for code b  │
│  Context: Decision: Tag selection for code...  │
│  Confidence: 65% | 2026-03-15 @ 10:13:40       │
╰────────────────────────────────────────────────╯
                      │
                      ▼
              ╱────────────────╲
...
└──────────────────┘
             ...
             │  │                                        │
             │  │  Rationale:                            │
             │  │  matches actual content semantics per  │
             │  │  sk-doc requirements                   │
             │  └────────────────────────────────────────┘
             │           │
             └─────┬─────┘
...
**Rationale**: matches actual content semantics per sk-doc requirements
...
### Decision 2: Decision: Em dashes in display/output template blocks, frontmatter descriptions,

**Context**: Decision: Em dashes in display/output template blocks, frontmatter descriptions, and table cell placeholders are exempt from replacement. Rationale: these are structural/visual elements, not prose punctuation.

**Timestamp**: 2026-03-15T10:13:40.761Z
...
```
╭────────────────────────────────────────────────╮
│  DECISION: Decision: Em dashes in display/out  │
│  Context: Decision: Em dashes in display/o...  │
│  Confidence: 65% | 2026-03-15 @ 10:13:40       │
╰────────────────────────────────────────────────╯
                      │
                      ▼
...
└──────────────────┘
             ...
             │  │                                        │
             │  │  Rationale:                            │
             │  │  these are structural/visual           │
             │  │  elements, not prose punctuation       │
             │  └────────────────────────────────────────┘
             │           │
             └─────┬─────┘
                   │
...
**Rationale**: these are structural/visual elements, not prose punctuation
...
### Decision 3: Decision: Move sqlite-vec-darwin-arm64 from dependencies to optionalDependencies

**Context**: Decision: Move sqlite-vec-darwin-arm64 from dependencies to optionalDependencies in scripts/package.json. Rationale: platform-specific native module fails npm ci on Linux CI runners; optionalDependencies lets npm skip incompatible platforms.
...
```
╭────────────────────────────────────────────────╮
│  DECISION: Decision: Move sqlite-vec-darwin-a  │
│  Context: Decision: Move sqlite-vec-darwin...  │
│  Confidence: 65% | 2026-03-15 @ 10:13:40       │
╰────────────────────────────────────────────────╯
                      │
                      ▼
...
└──────────────────┘
             ...
             │  │  fails npm ci on Linux CI runners;     │
             │  │  optionalDependencies lets npm skip    │
             │  └────────────────────────────────────────┘
             │           │
             └─────┬─────┘
                   │
...
1. **Option 1**
   Decision: Move sqlite-vec-darwin-arm64 from dependencies to optionalDependencies

#### Chosen Approach

**Selected**: Option 1

**Rationale**: platform-specific native module fails npm ci on Linux CI runners; optionalDependencies lets npm skip incompatible platforms
...
### Decision 4: Decision: Spec folder reflects actual implementation (7 commands) not original p

**Context**: Decision: Spec folder reflects actual implementation (7 commands) not original plan (8 commands). Rationale: ingest was folded into manage, learning history moved to analyze — documenting actual state, not aspirational plan.
...
```
╭────────────────────────────────────────────────╮
│  DECISION: Decision: Spec folder reflects act  │
│  Context: Decision: Spec folder reflects a...  │
│  Confidence: 65% | 2026-03-15 @ 10:13:40       │
╰────────────────────────────────────────────────╯
                      │
                      ▼
...
└──────────────────┘
             ...
             │  │                                        │
             │  │  Rationale:                            │
             │  │  ingest was folded into manage,        │
             │  │  learning history moved to analyze —   │
             │  │  documenting actual state, not asp     │
             │  └────────────────────────────────────────┘
             │           │
             └─────┬─────┘
                   │
...
1. **Option 1**
   Decision: Spec folder reflects actual implementation (7 commands) not original p

#### Chosen Approach

**Selected**: Option 1

**Rationale**: ingest was folded into manage, learning history moved to analyze — documenting actual state, not aspirational plan
...
### Decision 5: Decision: Added import-policy allowlist entry for rebuild-auto-entities.

**Context**: Decision: Added import-policy allowlist entry for rebuild-auto-entities.ts. Rationale: CLI script needs direct access to entity extraction internals; rebuildAutoEntities not yet exposed via api/ boundary.
...
```
╭────────────────────────────────────────────────╮
│  DECISION: Decision: Added import-policy allo  │
│  Context: Decision: Added import-policy al...  │
│  Confidence: 65% | 2026-03-15 @ 10:13:40       │
╰────────────────────────────────────────────────╯
                      │
                      ▼
...
└──────────────────┘
             ...
             │  │                                        │
             │  │  Rationale:                            │
             │  │  CLI script needs direct access to     │
             │  │  entity extraction internals;          │
             │  │  rebuildAutoEntities not yet exposed   │
             │  │  v                                     │
             │  └────────────────────────────────────────┘
             │           │
             └─────┬─────┘
...
1. **Option 1**
   Decision: Added import-policy allowlist entry for rebuild-auto-entities.

#### Chosen Approach

**Selected**: Option 1

**Rationale**: CLI script needs direct access to entity extraction internals; rebuildAutoEntities not yet exposed via api/ boundary
