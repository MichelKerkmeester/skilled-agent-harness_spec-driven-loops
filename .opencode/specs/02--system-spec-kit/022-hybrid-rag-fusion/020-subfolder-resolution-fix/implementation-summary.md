...
| Field | Value |
|-------|-------|
| **Spec Folder** | 020-subfolder-resolution-fix |
...
Bare-name and relative-path inputs to `generate-context.js` now resolve correctly for spec folders nested 3+ levels deep (e.g., `specs/02--system-spec-kit/022-hybrid-rag-fusion/020-subfolder-resolution-fix/`). Previously only the full `.opencode/specs/...` path worked.
...
| Bare name `019-skill-command-alignment` | PASS (resolves via recursive search) |
