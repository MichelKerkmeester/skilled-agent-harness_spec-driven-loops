...
- [x] CHK-012 [P1] Coverage-drift alignment for legacy compatibility coverage is documented with repository evidence [EVIDENCE: `.opencode/skill/system-spec-kit/mcp_server/tests/README.md` + `feature_catalog` wording now distinguish primary behavioral coverage from supplemental compatibility coverage.]
...
- [x] CHK-021 [P0] Recursive spec validation completed for parent + child phases [EVIDENCE: On 2026-03-15, `bash .opencode/skill/system-spec-kit/scripts/spec/validate.sh --recursive .opencode/specs/02--system-spec-kit/022-hybrid-rag-fusion/007-code-audit-per-feature-catalog` returned PASS with `All 21 phases passed` and `Summary: Errors: 0  Warnings: 0`.]
- [x] CHK-022 [P1] Placeholder scan completed for phase 021 artifacts [EVIDENCE: `rg -n "\\[NAME\\]|\\[YYYY-MM-DD\\]|\\[###-feature-name\\]|\\[path\\]" .opencode/specs/02--system-spec-kit/022-hybrid-rag-fusion/007-code-audit-per-feature-catalog/021-remediation-revalidation/*.md` returned no matches (rg exit 1).]
...
- [x] CHK-050 [P1] Modified files are contained within `007-code-audit-per-feature-catalog/` markdown scope [EVIDENCE: in-scope paths only.]
