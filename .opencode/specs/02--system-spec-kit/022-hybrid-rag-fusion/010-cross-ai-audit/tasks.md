...
**Files:** 8 total production files (7 in `mcp_server/lib` + 1 in `shared/`) | **Effort:** 1.0h opt / 2-3h real | **Risk:** Medium
...
- [x] Fix spec status wording ("In Progress" vs handover "complete") — set to actual state
- [x] Fix checklist test-pass evidence wording drift — normalize to implemented state
- [x] Fix agent count: `handover.md:15` (15 agents) vs `implementation-summary.md:33` (17 agents)
- [x] Fix standards claim contradiction in implementation-summary ("all aligned" vs "19/20 fail")
...
| P1-6 | Math.max/min spread (8 total production files: 7 in `mcp_server/lib` + 1 in `shared/`; stack overflow >100K) | P1 | G2 + G4 + G5 (25 matches) | 3/3 |
...
| CR-P1-7 | Cross-doc contradictions (status, counts, claims) | P1 | Codex | `spec.md`, `checklist.md`, `implementation-summary.md` |
