...
| **Created** | 2026-03-02 |
| **Spec Folder** | `022-hybrid-rag-fusion/010-cross-ai-audit` |
...
2. **Latent code risks** — Math.max/min spread patterns in 8 total production files (7 in `mcp_server/lib` + 1 in `shared/`) that crash Node.js at >100K array elements; session-manager transaction gap allowing concurrent race conditions
...
**P1-6: Math.max/min spread patterns** — 8 total production files (7 in `mcp_server/lib` + 1 in `shared/`) use `Math.max(...array)` which causes stack overflow at >100K elements. G4 found 25 active matches across the codebase. Safe reduce patterns already exist in `intent-classifier.ts` and `rrf-fusion.ts` but weren't applied universally. Confirmed by G2 + G4 + multi-agent deep review (3/3 models).
...
**R2a: Math.max/min spread patterns (8 total production files: 7 in `mcp_server/lib` + 1 in `shared/`; 25 total matches including tests)**
...
- [ ] All P1 code safety issues (Math.max across 8 total production files: 7 in `mcp_server/lib` + 1 in `shared/`, plus session tx gap) fixed and verified
