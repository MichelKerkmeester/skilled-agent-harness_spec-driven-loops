...
|---|---|---|---|
| `.opencode/skill/system-spec-kit/scripts/utils/slug-utils.ts` | Yes | `generateContentSlug()`, `pickBestContentName()`, `ensureUniqueMemoryFilename()` | No `ensureUniqueSlug()` export. Current helper already resolves collisions by scanning `*.md`, appending `-1..-100`, then hash fallback. |
...
- **What exists now:** `ensureUniqueMemoryFilename(contextDir, filename)` already implements collision resolution with sequential suffixes and a hash fallback.
...
- **Target:** `scripts/utils/slug-utils.ts`
- **Status:** EXISTS
- **What exists now:** Retry loop is capped at 100 attempts, then falls back to a 6-character SHA1 suffix.
...
- **Status:** EXISTS
- **What exists now:** `generateFolderDescriptions()` already prefers per-folder `description.json` when present and fresh, then falls back to `spec.md` extraction.
...
- **Target:** `mcp_server/lib/search/folder-discovery.ts`
- **Status:** EXISTS
- **What exists now:** When no usable per-folder file is found, aggregation falls back to the legacy `spec.md` extraction path.
...
- **Status:** EXISTS
- **What exists now:** Integration tests cover both stale per-folder fallback and a mixed-mode case where one folder uses `description.json` and another falls back to `spec.md`.
