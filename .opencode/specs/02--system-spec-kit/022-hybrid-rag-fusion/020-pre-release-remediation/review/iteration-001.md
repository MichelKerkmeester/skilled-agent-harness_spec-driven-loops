...
- **Evidence:** The summary claims epic `research.md` was restored and that only one intentional strict-validation error remains, but a fresh local `validate.sh` run still fails on incomplete AI protocol state, a missing `research.md` reference, and template-header drift.
