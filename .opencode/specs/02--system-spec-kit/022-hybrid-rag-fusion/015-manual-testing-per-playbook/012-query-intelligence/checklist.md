...
- [x] CHK-011 [P0] 034 (RSF shadow mode R14/N1) executed: PASS — `rsf-fusion.ts:4-23` `@deprecated` module-level comment confirms RSF dormant; `hybrid-search.ts` imports `fuseResultsMulti` (RRF) for live ranking; `Sprint3PipelineMeta.rsfShadow` is evaluation-only (`hybrid-search.ts:150-151`); no live RSF branch wired
