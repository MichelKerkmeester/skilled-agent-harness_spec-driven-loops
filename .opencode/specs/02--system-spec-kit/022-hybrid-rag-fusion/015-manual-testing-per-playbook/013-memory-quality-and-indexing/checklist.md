...
- [x] CHK-050 [P0] 164 executed and verdicted -- PASS. batch-learning.ts constants correct (MIN_SUPPORT=3, MAX_BOOST=0.10, WINDOW=7d, CONFIDENCE_WEIGHTS). runBatchLearning(database) wired from context-server.ts startup (after retry manager); try/catch non-fatal; feature flag gate inside function.
...
- [x] CHK-072 [P1] Former PARTIAL verdicts reviewed: all 3 remediated to PASS:
