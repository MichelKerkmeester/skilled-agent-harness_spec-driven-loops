...
description: "Manual test scenarios for tooling-and-scripts are distributed across the hybrid-rag-fusion playbook and related feature sources. This phase packet consolidates the 20 assigned scenarios into one structured spec with exact coverage, links, and acceptance criteria."
trigger_phrases:
  - "manual testing"
  - "tooling and scripts"
  - "phase 016"
...
### Problem Statement
Manual test scenarios for tooling-and-scripts need structured per-phase documentation instead of remaining spread across the monolithic playbook and mixed feature references. Operators need one canonical packet that preserves exact prompts, command sequences, evidence expectations, and verdict criteria for all 20 phase-016 scenarios.
...
### In Scope
- Consolidate all 20 phase-016 scenarios: `NEW-061`, `NEW-062`, `NEW-070`, `NEW-089`, `NEW-099`, `NEW-113`, `NEW-127`, `NEW-128`, `NEW-135`, `NEW-136`, `NEW-137`, `NEW-138`, `NEW-139`, `NEW-147`, `NEW-149`, `PHASE-001`, `PHASE-002`, `PHASE-003`, `PHASE-004`, and `PHASE-005`.
...
|-----------|-------------|-------------|
| `016-tooling-and-scripts/spec.md` | Create | Phase 016 testing specification with scenario coverage and acceptance criteria |
| `016-tooling-and-scripts/plan.md` | Create | Phase 016 execution plan with prompts, phases, dependencies, and rollback guidance |
<!-- /ANCHOR:scope -->
...
| `REQ-NEW-139` | Document `NEW-139` session-capturing-pipeline-quality coverage using the canonical `M-007` playbook section because the detailed `NEW-139` scenario row is not present in the main scenario table. | PASS if all automated commands pass, package-clean MCP verification passes alongside the scripts-side closure suite, `M-007a` validates and indexes successfully, `M-007b` proves thin aligned JSON fails `INSUFFICIENT_CONTEXT_ABORT`, `M-007c` hard-fails `ALIGNMENT_BLOCK`, `M-007d` proves enrichment plus rendered-memory contract compliance, `M-007e` proves OpenCode precedence does not override alignment blocking, `M-007f` through `M-007i` prove per-backend fallback behavior, and `M-007j` proves final `NO_DATA_AVAILABLE` behavior. |
...
- **SC-001**: All 20 assigned scenarios are documented with source-linked scope entries and per-test acceptance criteria.
...
- **SC-004**: Cross-cutting exceptions are transparent, including the `NEW-139` playbook-row gap and the phase-system catalog anchors used for `PHASE-001` through `PHASE-005`.
<!-- /ANCHOR:success-criteria -->
...
| Risk | `NEW-139` exists in the cross-reference index but lacks a dedicated main scenario-table row | Documentation could drift if a prompt or acceptance rule is invented. | Use the detailed `M-007 Session Capturing Pipeline Quality` section as the canonical source and state that choice explicitly. |
...
- Should `NEW-139` be promoted from the cross-reference index into the main scenario table so future phase packets do not need to source it from `M-007`?
