...
| NEW-107 | Checkpoint confirmName and schema enforcement | [`../../feature_catalog/18--ux-hooks/03-checkpoint-delete-confirmname-safety.md`](../../feature_catalog/18--ux-hooks/03-checkpoint-delete-confirmname-safety.md) | `Validate checkpoint delete confirmName enforcement across handler and schema layers.` | `1) npx vitest run tests/handler-checkpoints.vitest.ts tests/tool-input-schema.vitest.ts tests/mcp-input-validation.vitest.ts 2) inspect rejection assertions for missing confirmName 3) inspect success assertions for safetyConfirmationUsed=true` |
...
- Documenting non-UX-hooks phases from other phase folders within `019-manual-testing-per-playbook/`.
...
|----|-------------|---------------------|
| REQ-001 | Document NEW-103 UX hook module coverage with its exact playbook prompt, vitest command, evidence target, and feature link. | PASS if `tests/hooks-ux-feedback.vitest.ts` passes with no failing assertions covering latency/cache-clear booleans and finalized hint payload |
| REQ-002 | Document NEW-104 mutation save-path UX parity and no-op hardening with its exact prompt, dual-assertion command sequence, evidence target, and feature link. | PASS if duplicate-save no-ops suppress false hook metadata and atomic-save responses match the primary save contract |
| REQ-003 | Document NEW-105 context-server success-envelope finalization with its exact prompt, four-step vitest sequence, evidence target, and feature link. | PASS if `tests/context-server.vitest.ts` passes and assertions cover the final success-envelope path end to end |
| REQ-004 | Document NEW-106 hooks barrel and README synchronization with its exact prompt, ripgrep command sequence, evidence target, and feature link. | PASS if both files reference the new modules and contract fields |
| REQ-005 | Document NEW-107 checkpoint confirmName and schema enforcement with its exact prompt, three-suite vitest sequence, evidence target, and feature link. | PASS if the three suites pass and prove required `confirmName` enforcement end to end |
...
- **SC-002**: `plan.md` defines how evidence, verdicts, and coverage for NEW-103, NEW-104, NEW-105, NEW-106, and NEW-107 will be collected.
- **SC-003**: Reviewers can audit every Phase 018 scenario using this folder plus the linked playbook (`../../manual_testing_playbook/manual_testing_playbook.md`) and review protocol (`../../manual_testing_playbook/review_protocol.md`).
...
| Dependency | MCP runtime and vitest test suite for hook and save-path tests | Required to execute NEW-103, NEW-104, NEW-105, and NEW-107 scenarios | Confirm vitest suite is compiled and test files exist before execution |
| Dependency | `ripgrep` (`rg`) CLI tool available in runtime environment | Required to execute NEW-106 barrel and README checks | Verify `rg` is installed; fall back to `grep -r` if absent |
| Risk | NEW-107 involves validating rejection of missing `confirmName` which could mask false positives if schema enforcement is incomplete | High | Run all three suites (handler, tool-input-schema, mcp-input-validation) together; do not accept partial suite pass |
<!-- /ANCHOR:risks -->
...
- Should NEW-106 `rg` commands target the repository root or a specific `mcp_server/hooks/` path for consistent cross-machine results?
- Is there a vitest alias or npm script that wraps the three-suite NEW-107 run, or must all three files be listed explicitly each time?
<!-- /ANCHOR:questions -->
