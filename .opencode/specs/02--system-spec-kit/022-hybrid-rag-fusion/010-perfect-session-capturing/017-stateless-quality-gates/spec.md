...
| **Status** | Complete |
| **Created** | 2026-03-18 |
| **Branch** | `017-stateless-quality-gates` |
| **Parent Spec** | ../spec.md |
| **Parent Plan** | ../plan.md |
| **Phase** | 17 of 20 |
| **Predecessor** | 016-multi-cli-parity |
| **Successor** | 018-runtime-contract-and-indexability |
...
Before this phase shipped, `node generate-context.js "016-multi-cli-parity"` entered stateless mode and still applied file-mode assumptions to raw CLI captures. Three compounding issues produced false-positive aborts:
...
- **SC-001**: **Given** a legitimate Claude Code session on spec folder `017-stateless-quality-gates`, **When** `node generate-context.js "017-stateless-quality-gates"` is run, **Then** a memory file is created with no QUALITY_GATE_ABORT.
...
**As a** developer using Claude Code, **I want** `node generate-context.js "017-stateless-quality-gates"` to succeed without writing to `/tmp`, **so that** the explicit spec folder argument works as documented without manual JSON scaffolding.

**Acceptance Criteria**:
1. Given a Claude Code session on a legitimate spec folder, When I run `node generate-context.js "017-stateless-quality-gates"`, Then a memory file is created with quality >= 60 and no QUALITY_GATE_ABORT.
