...
- Stale hard-coded path: `scratch/launch-qa-validation.sh:18` points `SPEC_DIR` at `012-perfect-session-capturing`, which does not match the actual spec root `010-perfect-session-capturing`. This makes the launcher look stale and potentially unsafe to re-run as-is.
...
- First heading: `# Research: 010-perfect-session-capturing`
...
2. Fix `scratch/launch-qa-validation.sh` so `SPEC_DIR` points at `010-perfect-session-capturing` (or derive it dynamically from `SCRIPT_DIR`) before anyone reuses the launcher.
