...
- [x] CHK-023 [P1] Existing test baselines pass with the derived field (no behavioral regression). Evidence: `node test-scripts-modules.js` passed with `384` passed / `0` failed / `5` skipped on 2026-03-17, so the stale baseline failure note is no longer true.
