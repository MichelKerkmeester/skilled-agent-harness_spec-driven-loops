...
**Date:** 2026-03-05
**Specs Reviewed:** 012-architecture-audit, 030-architecture-boundary-remediation (now archived at `012-architecture-audit/scratch/merged-030-architecture-boundary-remediation`)
...
**Spec 013 Gap:** Phase 3 says "pre-commit or CI" but lacks concrete target file, trigger strategy, or fail policy.

**Recommendation:** Mandate CI-level enforcement in 013's spec.md as a hard requirement, not an option. Define: which CI system, which pipeline stage, what happens on failure.
...
1. Mandate CI enforcement (not pre-commit-only) in 013 spec
