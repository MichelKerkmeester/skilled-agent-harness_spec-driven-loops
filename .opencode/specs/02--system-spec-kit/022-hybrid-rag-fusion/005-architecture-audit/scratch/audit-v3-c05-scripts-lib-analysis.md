...
Description: One of the calibration input commands still targets `.opencode/specs/02--system-spec-kit/020-mcp-working-memory-hybrid-rag/scratch`, which no longer exists in this workspace. The script then repeats the entire 10-command block five times.
