...
|---|-------------|-----------|-----------|
| 1 | `008-combined-bug-fixes` | 2026-02-22 | Session selection finalization, detector regression tests (32 passed) |
...
Bare-name and relative-path inputs to `generate-context.js` now resolve correctly for spec folders nested 3+ levels deep (e.g., `specs/02--system-spec-kit/022-hybrid-rag-fusion/008-combined-bug-fixes/`). Previously only the full `.opencode/specs/...` path worked.
...
| Bare name `008-combined-bug-fixes` | PASS (resolves via recursive search) |
