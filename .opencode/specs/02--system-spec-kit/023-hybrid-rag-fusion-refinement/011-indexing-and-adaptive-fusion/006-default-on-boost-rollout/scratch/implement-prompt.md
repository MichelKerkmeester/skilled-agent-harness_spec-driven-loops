...
copilot -p "$(cat .opencode/specs/02--system-spec-kit/023-hybrid-rag-fusion-refinement/011-indexing-and-adaptive-fusion/006-default-on-boost-rollout/scratch/implement-prompt.md)" \
  --model gpt-5.4 \
  --allow-all-tools \
  2>&1
```
...
```
Spec folder: .opencode/specs/02--system-spec-kit/023-hybrid-rag-fusion-refinement/011-indexing-and-adaptive-fusion/006-default-on-boost-rollout/ (pre-approved, skip Gate 3)
```
