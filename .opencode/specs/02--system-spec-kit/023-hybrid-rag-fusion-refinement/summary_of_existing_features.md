...
Single deletes run inside a database transaction (`memory-crud-delete.ts:70-85`): the memory record is removed from `memory_index`, the vector embedding is deleted via `vectorIndex.deleteMemory(id)`, all associated causal graph edges are removed via `causalEdges.deleteEdgesForMemory(id)`, and a mutation ledger entry is recorded. If any step fails, the entire transaction rolls back — no partial deletes. This atomicity guarantee was added in Phase 018 (CR-P1-1) to prevent orphaned data from inconsistent cleanup.
