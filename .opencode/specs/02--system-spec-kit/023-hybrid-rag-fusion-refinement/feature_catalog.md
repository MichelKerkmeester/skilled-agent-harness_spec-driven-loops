...
A 6-agent cross-AI research effort (3 Codex gpt-5.3-codex, 3 Gemini 3.1-pro-preview) analyzed external systems (Cognee, QMD, ArtemXTech) and produced 16 recommendations. After triple-verification against the feature catalog, 4 were fully implemented, 7 partially implemented, and 5 genuinely new. This sprint completes the bounded gaps and builds net-new operational tooling. Spec folder: `019-sprint-9-extra-features` (Level 3+). 12 features total: 3 P0 (foundation), 4 P1 (quality and operations), 5 P2 (deferred innovation).

### Strict Zod schema validation (P0-1)

**PLANNED (Sprint 019).** All 24 MCP tool inputs across layers L1-L7 are migrated from plain object descriptions to Zod schemas with runtime validation. Currently, `mcp_server/tool-schemas.ts` (lines 1-396) defines tool parameters as plain objects with no runtime enforcement, leaving the system vulnerable to hallucinated parameters generated by calling LLMs.

Enforcing strict validation ensures that any extra or hallucinated fields are explicitly rejected at the boundary, returning a clear Zod error to the calling agent. This behavior is controlled by the `SPECKIT_STRICT_SCHEMAS` flag, which defaults to `true`. When disabled, the schemas fall back to `.passthrough()` mode for backward compatibility during a transition period. The feature adds the `zod` dependency to the project.

### Provenance-rich response envelopes (P0-2)

**PLANNED (Sprint 019).** *(Overlap note: extends existing `memory_search` result formatting, where internal PipelineRow scoring exists but is dropped at Stage 4 exit around lines 934-945 of hybrid-search.ts.)* The `FormattedSearchResult` type at `formatters/search-results.ts:57-78` currently exposes only basic fields (id, specFolder, filePath, title, similarity). Internal `PipelineRow` has 10+ score fields (`rrfScore`, `intentAdjustedScore`, `stage2Score`, `attentionScore`, `quality_score`) that are dropped before reaching the caller.

This feature extends the result format into a comprehensive `MemoryResultEnvelope` type with three new objects, opt-in via `includeTrace: true` (default `false` for backward compatibility). The `scores` object exposes: `semantic`, `lexical`, `fusion`, `intentAdjusted`, `composite`, `rerank`, `attention`. The `source` object exposes: `file`, `anchorIds`, `anchorTypes`, `lastModified`, `memoryState`. The `trace` object exposes: `channelsUsed`, `pipelineStages`, `fallbackTier`, `queryComplexity`, `expansionTerms`, `budgetTruncated`, `scoreResolution`. When `includeTrace` is `false`, these objects are omitted entirely.

### Async ingestion job lifecycle (P0-3)

**PLANNED (Sprint 019).** *(Overlap note: supersedes the current lightweight `asyncEmbedding` path in memory-save.ts around lines 1106-1115, which uses `setImmediate()` + `retryManager.retryEmbedding()` for single-file async saves.)* Bulk indexing of large spec directories can hit MCP timeout limits. This feature introduces a full job lifecycle with SQLite persistence for crash recovery.

A dedicated background job queue in `lib/ops/job-queue.ts` (~200 LOC) manages ingestion tasks through a state machine: `queued → parsing → embedding → indexing → complete/failed/cancelled`. Three new MCP tools provide direct control: `memory_ingest_start` (returns job ID in <100ms), `memory_ingest_status` (returns progress, files processed, errors), and `memory_ingest_cancel` (stops in-progress jobs). The queue caps at 10 concurrent jobs, with overflow queued in `queued` state. On server restart, incomplete jobs are reset to `queued` for automatic retry.

### Contextual tree injection (P1-4)

**PLANNED (Sprint 019).** *(Overlap note: builds on PI-B3 spec folder description discovery, which provides cached one-sentence descriptions used for routing in `memory_context`. The gap is that these descriptions are not string-prepended into returned chunks.)* Retrieved memory chunks gain hierarchical context headers so the calling agent immediately understands the structural origin of each result without additional round-trips to `memory_list`.

Headers follow the format `[parent > child — description]`, capped at 100 characters to conserve context windows. The injection uses the existing PI-B3 cached descriptions and is inserted in Stage 4 output after token-budget truncation, before the final return. When no spec folder association exists, injection is gracefully skipped. This behavior is governed by the `SPECKIT_CONTEXT_HEADERS` flag (default `true`).

### Local GGUF reranker via node-llama-cpp (P1-5)

**PLANNED (Sprint 019).** *(Overlap note: implements the previously unimplemented `RERANKER_LOCAL` flag while `SPECKIT_CROSS_ENCODER` remains the existing default-on rerank control.)* Stage 3 reranking currently supports Cohere and Voyage remote APIs. This feature adds a Node-native GGUF execution path via `node-llama-cpp` in a new `lib/search/local-reranker.ts` (~200 LOC), integrating into the existing Stage 3 reranking slot at approximately lines 850-900 of `hybrid-search.ts`.

The system uses a quantized `bge-reranker-v2-m3.Q4_K_M.gguf` model (~350MB). Before loading, `canUseLocalReranker()` checks that `RERANKER_LOCAL=true`, the model file exists, and at least 4GB free memory is available. If any check fails, the pipeline silently falls back to the existing algorithmic RRF scoring. The performance target is <500ms for top-20 candidates. The loaded model is cached at module level to avoid reload per query and disposed on server shutdown.

### Dynamic server instructions at MCP initialization (P1-6)

**PLANNED (Sprint 019).** Currently, `context-server.ts` initialization (lines 511-550) performs SQLite version check, embedding readiness and session manager init, but injects no dynamic information about the memory system's state. Calling LLMs start each session without awareness of what knowledge is available.

This feature adds a `buildServerInstructions()` function that generates a concise overview at startup: total memory count, active spec folder count, available search channels, and stale memory count (with a warning when stale count exceeds 10). The instruction string is injected via `server.setInstructions()` and reuses existing `memory_stats` handler data. The feature is gated by `SPECKIT_DYNAMIC_INIT` (default `true`).

### Real-time filesystem watching with chokidar (P1-7)

**PLANNED (Sprint 019).** The memory index currently relies on pull-based `memory_index_scan` for synchronization with the filesystem. This feature adds push-based file watching via `chokidar` in a new `lib/ops/file-watcher.ts` (~150 LOC) to reduce the delay between a file change and its availability in search results.

To ensure stability, the watcher implements a 2-second debounce and uses the existing TM-02 SHA-256 content hashing to deduplicate identical writes. SQLite WAL mode is enforced, and an exponential backoff retry mechanism (up to 3 attempts with delays of 1s, 2s, 4s) handles `SQLITE_BUSY` locks during concurrent write operations. The watcher is registered for cleanup in the server shutdown handler to prevent process leaks. This subsystem is controlled by the `SPECKIT_FILE_WATCHER` flag (default `false`) and watches only configured spec directories (no arbitrary path traversal).
...
| `SPECKIT_CONTEXT_HEADERS` | `true` | boolean | `lib/search/hybrid-search.ts` | **PLANNED (Sprint 019).** P1-4: Contextual tree injection into returned chunks. When enabled, string-prepends hierarchical context headers (`[parent > child — description]`, max 100 chars) using PI-B3 cached spec folder descriptions. Injected in Stage 4 after token-budget truncation. |
...
| `SPECKIT_DYNAMIC_INIT` | `true` | boolean | `context-server.ts` | **PLANNED (Sprint 019).** P1-6: Dynamic server instructions at MCP initialization. Generates a memory-system overview (total memories, spec folder count, channels, stale count) and injects via `server.setInstructions()`. Reuses existing `memory_stats` handler data. |
...
| `SPECKIT_FILE_WATCHER` | `false` | boolean | `lib/ops/file-watcher.ts` | **PLANNED (Sprint 019).** P1-7: Real-time filesystem watching with chokidar. Adds push-based indexing of spec directories with 2s debounce, TM-02 SHA-256 content-hash deduplication, WAL mode enforcement, and exponential backoff for `SQLITE_BUSY`. |
...
| `SPECKIT_RESPONSE_TRACE` | `false` | boolean | `handlers/memory-search.ts` | **PLANNED (Sprint 019).** P0-2: Include provenance data (scores, source, trace) in `memory_search` response envelopes. Opt-in via `includeTrace: true` parameter. When disabled, response format is unchanged (backward compatible). |
...
| `SPECKIT_STRICT_SCHEMAS` | `true` | boolean | `tool-schemas.ts` | **PLANNED (Sprint 019).** P0-1: Controls Zod schema enforcement mode for all 24 MCP tool inputs. When `true`, `.strict()` rejects unexpected parameters. When `false`, `.passthrough()` allows undocumented parameters for backward compatibility. |
