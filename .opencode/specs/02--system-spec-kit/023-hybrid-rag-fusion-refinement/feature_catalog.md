...
A 6-agent cross-AI research effort (3 Codex gpt-5.3-codex, 3 Gemini 3.1-pro-preview) analyzed external systems (Cognee, QMD, ArtemXTech) and produced 16 recommendations. These were distilled into Sprint 019 scope: 12 features total (3 P0 foundation, 4 P1 quality and operations, 5 P2 deferred innovation). Implementation work is in progress and verification remains open in `019-sprint-9-extra-features/checklist.md` (current summary: P0 4/28, P1 0/49, P2 0/11, total 4/88). The entries below capture implementation candidates and rollout details pending checklist closure.

### Strict Zod schema validation (P0-1)

**IMPLEMENTATION CANDIDATE (Sprint 019; verification pending).** All 28 MCP tool inputs across layers L1-L7 are migrated from plain object descriptions to Zod schemas with runtime validation. `mcp_server/tool-schemas.ts` defines tool parameters with full Zod enforcement including `.strict()` mode, rejecting hallucinated parameters at the boundary with clear Zod errors.
...
**IMPLEMENTATION CANDIDATE (Sprint 019; verification pending).** *(Overlap note: builds on PI-B3 spec folder description discovery, which provides cached one-sentence descriptions used for routing in `memory_context`.)* Retrieved memory chunks gain hierarchical context headers so the calling agent immediately understands the structural origin of each result without additional round-trips to `memory_list`.
...
**IMPLEMENTATION CANDIDATE (Sprint 019; verification pending).** At startup, `context-server.ts` now injects dynamic information about the memory system's state so calling LLMs start each session with awareness of available knowledge.
...
**IMPLEMENTATION CANDIDATE (Sprint 019; verification pending).** The memory index currently relies on pull-based `memory_index_scan` for synchronization with the filesystem. This feature adds push-based file watching via `chokidar` in `lib/ops/file-watcher.ts` to reduce the delay between a file change and its availability in search results.
...
| `SPECKIT_DYNAMIC_INIT` | `true` | boolean | `context-server.ts` | **IMPLEMENTATION CANDIDATE (Sprint 019; verification pending).** P1-6: Dynamic server instructions at MCP initialization. `buildServerInstructions()` generates a memory-system overview (total memories, spec folder count, channels, stale count) and injects via `server.setInstructions()`. Reuses existing `memory_stats` handler data. |
...
| `SPECKIT_RESPONSE_TRACE` | `false` | boolean | `handlers/memory-search.ts` | **IMPLEMENTATION CANDIDATE (Sprint 019; verification pending).** P0-2: Include provenance data (scores, source, trace) in `memory_search` response envelopes. Opt-in via `includeTrace: true` parameter. When disabled, response format is unchanged (backward compatible). |
