...
- [ ] Test: Runs with real spec folder (simulation mode)
  ```bash
  node .opencode/skill/system-spec-kit/scripts/generate-context.js .opencode/specs/02--system-spec-kit/z_archive/001-fix-command-dispatch/z_archive/051-lib-consolidation
...
- [ ] Test: JSON input mode works
  ```bash
  echo '{"specFolder":".opencode/specs/02--system-spec-kit/z_archive/001-fix-command-dispatch/z_archive/051-lib-consolidation","sessionSummary":"Test summary"}' > /tmp/test-context.json
...
- [ ] Test: `memory_save` workflow (via generate-context.js)
  ...
  echo '{"specFolder":".opencode/specs/02--system-spec-kit/z_archive/001-fix-command-dispatch/z_archive/051-lib-consolidation","sessionSummary":"Verification test run","triggerPhrases":["lib consolidation","verification test"]}' > /tmp/verify-save.json
  ...
  ls -la .opencode/specs/02--system-spec-kit/z_archive/001-fix-command-dispatch/z_archive/051-lib-consolidation/memory/ 2>/dev/null || echo "No memory folder yet"
  ```
