...
| **Status** | Complete |
| **Created** | 2026-03-31 |
| **Branch** | `007-upgrade-level-script` |
<!-- /ANCHOR:metadata -->
...
### Problem Statement
This archived folder captures work that added or refined a spec folder level upgrade script. The older archive shape no longer matched the active template contract, which caused strict validation errors.

### Purpose
Keep a concise archival summary of the level upgrade script work in a format that passes the current validator.
<!-- /ANCHOR:problem -->
...
### In Scope
- Preserve the archived topic and folder identity for the level upgrade script work.
...
- Performing new level upgrade script changes outside this archived folder.
...
|----|-------------|---------------------|
| REQ-001 | The archive must clearly state that the folder documents a spec folder level upgrade script. | A maintainer can identify the archived topic from spec.md alone. |
...
- **Given** a maintainer opens this archived folder, **when** they read the core documents, **then** they can understand that the archived work improved level upgrade automation for spec folders.
