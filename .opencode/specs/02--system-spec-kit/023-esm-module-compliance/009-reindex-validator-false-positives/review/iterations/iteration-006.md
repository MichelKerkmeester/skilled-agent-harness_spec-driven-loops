...
- Evidence:
  ...
  - The spec/checklist explicitly records the downstream effect of this churn: force reindex accumulated 13,211 duplicate rows that later had to be deleted. [SOURCE: `.opencode/specs/02--system-spec-kit/023-esm-module-compliance/009-reindex-validator-false-positives/spec.md:104-106`; `.opencode/specs/02--system-spec-kit/023-esm-module-compliance/009-reindex-validator-false-positives/checklist.md:39-40`]
- Impact:
...
- I did not find the exact `DELETE FROM memory_index WHERE id NOT IN (SELECT MAX(id) FROM memory_index GROUP BY file_path)` statement in the reviewed source files; only the spec/checklist mention the cleanup. [SOURCE: `.opencode/specs/02--system-spec-kit/023-esm-module-compliance/009-reindex-validator-false-positives/spec.md:105-106`; `.opencode/specs/02--system-spec-kit/023-esm-module-compliance/009-reindex-validator-false-positives/checklist.md:39-40`]
