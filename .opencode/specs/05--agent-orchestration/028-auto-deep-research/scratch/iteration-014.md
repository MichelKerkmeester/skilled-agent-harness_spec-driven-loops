...
**Refinement**: Copy the fault-tolerance pattern (silent skip + defaults) directly. Our iteration files contain `## Assessment` sections with all needed state data. Recovery function: scan `scratch/iteration-*.md`, parse Assessment sections, reconstruct JSONL. Effort confirmed as Small.
...
**Refinement**: Our `scratch/research-ideas.md` should be checked at TWO points: (1) during strategy initialization (already proposed), (2) during stuck recovery (already proposed), AND (3) in the auto-resume message (new -- if ideas exist, mention them to the next agent).
...
**New proposal**: Add `scratch/.deep-research-pause` sentinel check to the orchestrator dispatch. If file exists, pause the loop with a message. Delete file to resume. Trivial to implement (1 line: `fs.existsSync()`), high user-control value.
