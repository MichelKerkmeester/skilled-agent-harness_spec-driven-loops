...
- `[SOURCE: .opencode/skills/sk-deep-research/assets/deep_research_config.json:3-13]` The current config exposes max iterations, convergence, stuck threshold, duration, and per-iteration tool/time budgets.
- `[SOURCE: .opencode/skills/sk-deep-research/references/loop_protocol.md:179-189]` The loop protocol treats time and tool-call limits as soft caps, but it does not specify cooldown windows, retry backoff, or model fallback behavior.
