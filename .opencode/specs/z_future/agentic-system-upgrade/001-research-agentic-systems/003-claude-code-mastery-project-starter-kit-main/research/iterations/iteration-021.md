...
## Research question
Is `system-spec-kit` asking operators to remember too many lifecycle entry points (`/speckit:plan`, `/speckit:implement`, `/speckit:complete`, `/speckit:resume`) compared with the external repo's more centralized workflow surface?
...
## Evidence
- The local command README and Spec Kit README present a broad command matrix, then ask the operator to choose among separate planning, implementation, completion, resume, handover, debug, deep-review, and deep-research entry points. [SOURCE: .opencode/commands/README.txt:84-100] [SOURCE: .opencode/commands/speckit/README.txt:54-76] [SOURCE: .opencode/commands/speckit/README.txt:121-159]
- Local lifecycle commands are not just labels; they each carry their own autonomous workflow logic and branching prompts, especially in the completion and resume assets. [SOURCE: .opencode/commands/speckit/implement.md:29-120] [SOURCE: .opencode/commands/speckit/assets/speckit_complete_auto.yaml:30-108] [SOURCE: .opencode/commands/speckit/assets/speckit_resume_auto.yaml:32-92]
...
- **Target file or module:** `.opencode/commands/speckit/README.txt`, command routing surfaces, workflow asset entrypoints
...
I looked for evidence that the local split materially reduces ambiguity for operators, but the docs mostly shift the burden onto the user to pick the correct command before the automation begins. [SOURCE: .opencode/commands/speckit/README.txt:83-111]
