...
- `system-spec-kit` exposes 8 primary lifecycle commands in `spec_kit` and 4 more in `memory`, with additional mode suffixes and subcommands inside those groups. [SOURCE: .opencode/commands/spec_kit/README.txt:43-111] [SOURCE: .opencode/commands/memory/README.txt:38-131]
- The local command docs are well structured, but they still require the operator to choose the right command family before any YAML workflow helps them. [SOURCE: .opencode/commands/spec_kit/README.txt:43-76] [SOURCE: .opencode/commands/memory/README.txt:38-66]
