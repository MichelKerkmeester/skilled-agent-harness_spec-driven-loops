...
- Local `@orchestrate` already forbids direct implementation and codebase exploration, and it enforces single-hop delegation, but it still permits reading agent definitions and command specs rather than banning operational reads categorically. [SOURCE: .opencode/agents/orchestrate.md:36-46]
- Local `@deep-research` is explicitly LEAF-only and prohibits nested dispatch, showing that the repo already uses stronger role-specific boundaries when a workflow needs them. [SOURCE: .opencode/agents/deep-research.md:28-42]
