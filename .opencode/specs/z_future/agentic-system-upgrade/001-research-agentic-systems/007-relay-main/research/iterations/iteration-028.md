...
- Constitutional gate notes still point back to the full gate definitions in `AGENTS.md`, creating a multi-document contract stack. [SOURCE: .opencode/skills/system-spec-kit/constitutional/gate-enforcement.md:51-68]
- The hook system adds `PreCompact`, `SessionStart`, and `Stop` lifecycle hooks plus a separate tool-based fallback path via `session_bootstrap()` and `session_resume()`. [SOURCE: .opencode/skills/system-spec-kit/references/config/hook_system.md:3-16] [SOURCE: .opencode/skills/system-spec-kit/references/config/hook_system.md:48-54]
