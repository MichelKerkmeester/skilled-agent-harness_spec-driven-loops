...
- `system-spec-kit` currently treats repo-local references, templates, and scripts as the operational sources of truth. [SOURCE: .opencode/skills/system-spec-kit/SKILL.md:90-118]
- Runtime routing in this repo is based on checked-in runtime directories such as `.opencode/agents/`, `.claude/agents/`, `.codex/agents/`, and `.gemini/agents/`, not on marketplace-installed bundles. [SOURCE: AGENTS.md:277-288]

## Analysis
Babysitter's marketplace solves a real problem: distributing one orchestration system across many harnesses and optional extensions. `system-spec-kit` does not have the same packaging problem today. Its workflows, templates, and commands already live in one repo and are versioned together. [SOURCE: external/README.md:168-178] [SOURCE: .opencode/skills/system-spec-kit/SKILL.md:90-118]
...
- **Target file or module:** `.opencode/skills/system-spec-kit/` distribution model and runtime-resolution concept
...
I looked for a present-day `system-spec-kit` need to distribute optional workflow bundles through a separate marketplace and found a single-repo, checked-in source-of-truth model instead. [SOURCE: .opencode/skills/system-spec-kit/SKILL.md:90-118] [SOURCE: AGENTS.md:277-288]
