...
## Research question
Is the `/memory:*` command surface well integrated with `/speckit:*`, or has it become an awkward parallel control plane for ordinary work?
...
## Evidence
- `/memory:save` is a substantial workflow of folder detection, duplicate checks, token-budget checks, anchor validation, naming conflict handling, alignment checks, and manual AI-composed context extraction. [SOURCE: .opencode/commands/memory/save.md:7-47] [SOURCE: .opencode/commands/memory/save.md:57-76] [SOURCE: .opencode/commands/memory/save.md:89-173] [SOURCE: .opencode/commands/memory/save.md:182-191]
- `/memory:search` is a second large public surface that mixes ordinary context lookup with preflight/postflight, causal graph, ablation, and dashboards. [SOURCE: .opencode/commands/memory/search.md:2-4] [SOURCE: .opencode/commands/memory/search.md:53-92] [SOURCE: .opencode/commands/memory/search.md:113-139]
- `/speckit:resume` already overlaps that terrain by auto-detecting sessions, choosing resume context, and loading just enough state to continue. [SOURCE: .opencode/commands/speckit/resume.md:29-45] [SOURCE: .opencode/commands/speckit/resume.md:85-141] [SOURCE: .opencode/commands/speckit/resume.md:200-202]
- The resume YAML pushes even harder toward automatic continuity: it prioritizes handover, `session_bootstrap()`, `memory_context()`, and sufficiency-based gap filling, with the explicit rule to stop loading once the next safe action is known. [SOURCE: .opencode/commands/speckit/assets/speckit_resume_auto.yaml:69-93] [SOURCE: .opencode/commands/speckit/assets/speckit_resume_auto.yaml:127-155] [SOURCE: .opencode/commands/speckit/assets/speckit_resume_auto.yaml:195-210]
...
- **Current system-spec-kit surface:** Operators can use `/memory:save`, `/memory:search`, `/memory:manage`, and `/memory:learn` in parallel with `/speckit:*`, while `/speckit:resume` separately implements continuity recovery. [SOURCE: .opencode/commands/memory/save.md:51-85] [SOURCE: .opencode/commands/memory/search.md:53-92] [SOURCE: .opencode/commands/speckit/resume.md:200-202]
...
- **What system-spec-kit should ADD for better UX:** Add one advanced memory/admin surface for search, repair, analytics, and learning, while ordinary `/speckit:*` commands own routine save/load behavior automatically.
...
- **Current system-spec-kit approach:** Expose memory as a separate public command family while also embedding continuity behavior into `/speckit:resume` and completion flows. [SOURCE: .opencode/commands/memory/save.md:51-76] [SOURCE: .opencode/commands/memory/search.md:53-92] [SOURCE: .opencode/commands/speckit/assets/speckit_resume_auto.yaml:69-93]
...
- **Verdict:** MERGE
- **If REFACTOR/PIVOT/SIMPLIFY — concrete proposal:** Keep `/memory:search` and `/memory:manage` for advanced use, but make save/load decisions default lifecycle behavior inside `/speckit:*` and hide `/memory:save` from standard workflow docs.
...
- **Target file or module:** `.opencode/commands/memory/save.md`, `.opencode/commands/memory/search.md`, `.opencode/commands/speckit/resume.md`, `.opencode/commands/speckit/assets/speckit_resume_auto.yaml`
...
I looked for evidence that routine manual memory operations are central to the happy path, but the strongest current Spec Kit UX already points the other way: "load just enough state" and "stop once the next safe action is known." [SOURCE: .opencode/commands/speckit/assets/speckit_resume_auto.yaml:73-93]
