...
- [SOURCE: .opencode/commands/spec_kit/assets/spec_kit_deep-research_auto.yaml:328-343] `system-spec-kit` already uses a reducer model in deep research where iteration files feed synchronized state artifacts, showing the repo is comfortable with compressed, workflow-owned state rather than raw chat accumulation.
