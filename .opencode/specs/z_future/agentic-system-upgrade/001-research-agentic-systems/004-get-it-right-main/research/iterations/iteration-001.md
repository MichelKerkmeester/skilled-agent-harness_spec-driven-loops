...
- [SOURCE: .opencode/commands/speckit/assets/speckit_implement_auto.yaml:392-433] `system-spec-kit`'s implementation workflow is rich in task execution and checkpoints, but it is still a forward-moving development phase rather than an attempt-scoped retry controller.
- [SOURCE: .opencode/commands/speckit/assets/speckit_implement_auto.yaml:445-472] Completion validation happens near the end of the implementation phase, after work is already done, not as a loop control surface between bounded attempts.
...
I looked for an existing attempt-level retry controller in `speckit_implement_auto.yaml` and found sequential development, checkpointing, and completion validation, but no comparable retry loop or conditional refactor path.

## Follow-up questions for next iteration
- Should the retry loop live inside `/speckit:implement`, or should it be a separate opt-in command?
