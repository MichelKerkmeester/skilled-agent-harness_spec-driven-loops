...
- [SOURCE: .opencode/commands/spec_kit/assets/spec_kit_implement_auto.yaml:392-433] `system-spec-kit`'s implementation workflow is rich in task execution and checkpoints, but it is still a forward-moving development phase rather than an attempt-scoped retry controller.
- [SOURCE: .opencode/commands/spec_kit/assets/spec_kit_implement_auto.yaml:445-472] Completion validation happens near the end of the implementation phase, after work is already done, not as a loop control surface between bounded attempts.
