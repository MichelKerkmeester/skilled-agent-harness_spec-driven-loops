...
## Method
Compared Ralph's Create PRD -> Convert -> Run flow with the setup prompts and step counts in `/speckit:plan`, `/speckit:implement`, and `/speckit:complete`, focusing on how many branching decisions the operator must make.
...
- `/speckit:plan` opens with a large consolidated setup prompt, multiple workflow modes, and explicit handoff to `/speckit:implement`. [SOURCE: .opencode/commands/speckit/plan.md:31-145] [SOURCE: .opencode/commands/speckit/plan.md:151-153]
- `/speckit:implement` and `/speckit:complete` each repeat setup, mode, and validation logic instead of feeling like one continuous guided flow. [SOURCE: .opencode/commands/speckit/implement.md:35-120] [SOURCE: .opencode/commands/speckit/implement.md:171-205] [SOURCE: .opencode/commands/speckit/complete.md:1-4]
...
I looked for evidence that the current split already feels like one guided flow through wrappers or quick-reference defaults, but the docs still present these as separate primary choices. [SOURCE: .opencode/skills/system-spec-kit/references/workflows/quick_reference.md:116-130] [SOURCE: .opencode/commands/speckit/plan.md:31-145]
