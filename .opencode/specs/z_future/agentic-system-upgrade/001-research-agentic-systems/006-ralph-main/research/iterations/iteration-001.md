...
## Method
Read `external/ralph.sh`, `external/prompt.md`, and `external/README.md` to trace Ralph's control loop, then compared that loop to `.opencode/agents/deep-research.md` and `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml`.
...
- `system-spec-kit` already uses the same fresh-context idea for deep research: the agent is defined as a single-iteration leaf, and the YAML loop dispatches it once per iteration with externalized state files. [SOURCE: .opencode/agents/deep-research.md:24-29] [SOURCE: .opencode/agents/deep-research.md:50-60] [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:20-24] [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:294-320]
