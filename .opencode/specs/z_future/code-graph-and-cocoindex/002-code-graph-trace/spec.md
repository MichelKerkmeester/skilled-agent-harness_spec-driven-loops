...
_memory:
  continuity:
    ...
    answered_questions: []
---
# Feature Specification: 002 — Code Graph Trace
...
| **Packet ID** | `system-spec-kit/028-code-graph-and-cocoindex/002-code-graph-trace` |
| **Depends on** | `system-spec-kit/028-code-graph-and-cocoindex/001-code-graph-hld-lld` |
<!-- /ANCHOR:metadata -->
...
- Preservation of the pt-04 dependency audit outcome: Phase 001 is the HLD/LLD classifier dependency for trace; Phase 005 is not the classifier source.
...
| `mcp_server/code_graph/lib/code-graph-trace.ts` | Create | `002-lib-impl` | FilePath-grounded trace resolver using Phase 001 classifier. |
...
- `001-contract` can start after `system-spec-kit/028-code-graph-and-cocoindex/001-code-graph-hld-lld/001-contract` publishes.
- `002-lib-impl` starts after local `001-contract` and `system-spec-kit/028-code-graph-and-cocoindex/001-code-graph-hld-lld/002-lib-impl` merge.
...
| `002-lib-impl` | integrated trace tool | Resolver consumes Phase 001 classifier and returns filePath-grounded trace output. | Product tests in `004-test`. |
<!-- /ANCHOR:phase-map -->
