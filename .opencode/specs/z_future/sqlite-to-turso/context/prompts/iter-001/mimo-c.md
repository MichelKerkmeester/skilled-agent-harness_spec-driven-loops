...
## Objective
Your perspective: MIGRATION RISK. Find every better-sqlite3-specific API coupling point in the slice: .pragma() calls, loadExtension()/sqlite-vec loading, prepared-statement idioms (.raw(), .pluck(), .iterate()), synchronous Database constructor options, backup()/serialize(), user-defined functions, and any construct a Turso/libSQL driver swap would break. Tag each with which Turso gap it exposes. Return structured findings.
...
---
Pre-plan (lean):
1. Read each slice file; locate driver-API coupling points -> candidate list with exact line numbers.
2. For each candidate: classify kind, capture signature + evidence path:line, map to a Turso gap, score relevance -> final findings array.
