...
def build_refs(section: SectionSpec) -> List[str]:
    refs = [
...
def build_generate_prompt(
    ...
    return textwrap.dedent(
        ...
        COACH FEEDBACK FROM PREVIOUS ITERATION
        {coach_feedback if coach_feedback else "- none"}
        """
    ).strip()


def build_review_prompt(section: SectionSpec, variant_number: int, direction_name: str) -> str:
    return textwrap.dedent(
        f"""
        As @review agent, critique the attached generated HTML for section {section.key} variant v{variant_number} ({direction_name}).

        Review against these criteria:
        - style-token adherence and non-flashy visual language
        - structural uniqueness for the requested direction
        - readability, hierarchy, and practical implementation quality
        - responsive and accessibility signals (reduced motion, color-scheme)
        ...
        {{
          ...
          "distinctiveness": 0,
          "issues": [
            {{"severity": "critical|major|minor", "message": "...", "fix": "..."}}
          ]
        }}
        """
    ).strip()
...
def parse_review_json(text: str) -> Dict[str, object]:
...
def run_gemini(prompt: str, model: str, timeout_seconds: int, refs: List[str]) -> subprocess.CompletedProcess:
    cmd = ["gemini", prompt, "-m", model, "-o", "text", *refs]
    try:
...
def format_review_feedback(review_data: Dict[str, object]) -> str:
    issues = review_data.get("issues", [])
    if not isinstance(issues, list) or not issues:
        return "Reviewer found quality issues. Improve clarity, token adherence, and structural distinctiveness."
    lines: List[str] = []
    for issue in issues[:4]:
        if not isinstance(issue, dict):
            continue
        severity = str(issue.get("severity", "major"))
        message = str(issue.get("message", "Issue detected"))
        fix = str(issue.get("fix", "Apply a concrete fix"))
        lines.append(f"- [{severity}] {message} | Fix: {fix}")
    return "\n".join(lines) if lines else "Reviewer requested another revision pass."
...
def run_job(
    ...
    candidate_file = section_out_dir / f"{section.key}-v{variant_number}.candidate.html"
    generate_prompt_file = PROMPT_DIR / f"{section.key}-v{variant_number}.generate.prompt.txt"
    review_prompt_file = PROMPT_DIR / f"{section.key}-v{variant_number}.review.prompt.txt"
    generate_log_file = LOG_DIR / f"{section.key}-v{variant_number}.generate.log"
    review_log_file = LOG_DIR / f"{section.key}-v{variant_number}.review.log"
    ...
    refs = build_refs(section)
    coach_feedback = ""
    last_missing: List[str] = []
    last_review_score = None

    for attempt in range(1, max_attempts + 1):
        generate_prompt = build_generate_prompt(section, variant_number, direction_name, direction_desc, coach_feedback)
        generate_prompt_file.write_text(generate_prompt, encoding="utf-8")

        gen_proc = run_gemini(generate_prompt, model, timeout_seconds, refs)
        gen_log_body = textwrap.dedent(
            f"""
            attempt: {attempt}
            section: {section.key}
            variant: v{variant_number}
            direction: {direction_name}
            return_code: {gen_proc.returncode}

            ---- STDERR ----
            {gen_proc.stderr}

            ---- STDOUT PREVIEW ----
            {gen_proc.stdout[:3000]}
            """
        ).strip()
        with generate_log_file.open("a", encoding="utf-8") as f:
            f.write(gen_log_body + "\n\n")

        if gen_proc.returncode != 0:
            coach_feedback = "Generator returned non-zero exit code. Rebuild full HTML and satisfy all constraints."
            if attempt < max_attempts:
                time.sleep(2 + random.random() * 4)
            continue

        html = strip_fences(gen_proc.stdout)
        missing = validate_html(html)
        if missing:
            last_missing = missing
            coach_feedback = "Local validation failed: " + ", ".join(missing)
            if attempt < max_attempts:
        ...
        review_prompt = build_review_prompt(section, variant_number, direction_name)
        review_prompt_file.write_text(review_prompt, encoding="utf-8")
        review_refs = [f"@{rel(candidate_file)}", *refs]
        review_proc = run_gemini(review_prompt, model, timeout_seconds, review_refs)

        review_log_body = textwrap.dedent(
            f"""
            attempt: {attempt}
            section: {section.key}
            variant: v{variant_number}
            ...
            ---- STDOUT ----
            {review_proc.stdout[:6000]}
            """
        ).strip()
        with review_log_file.open("a", encoding="utf-8") as f:
            f.write(review_log_body + "\n\n")

        if review_proc.returncode != 0:
            coach_feedback = "Review agent failed; regenerate with stricter hierarchy and stronger token adherence."
            if attempt < max_attempts:
                time.sleep(2 + random.random() * 3)
            continue

        review_data = parse_review_json(review_proc.stdout)
        if not review_data:
            coach_feedback = "Reviewer output was not valid JSON; regenerate and keep structure clearer and more disciplined."
            if attempt < max_attempts:
                time.sleep(1 + random.random() * 2)
            continue

        review_pass = bool(review_data.get("pass", False))
        score_raw = review_data.get("score", 0)
        if isinstance(score_raw, (int, float, str)):
            try:
                review_score = int(float(score_raw))
            except ValueError:
                review_score = 0
        else:
            review_score = 0
        last_review_score = review_score
        ...
        if review_pass and review_score >= min_review_score and not has_critical:
        ...
        coach_feedback = format_review_feedback(review_data)
        if attempt < max_attempts:
            time.sleep(1 + random.random() * 2)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate section HTML variants via actively engaged Gemini generation+review loops.")
    parser.add_argument("--start-variant", type=int, default=4, help="First variant number to generate.")
    parser.add_argument("--end-variant", type=int, default=7, help="Last variant number to generate.")
    parser.add_argument("--concurrency", type=int, default=2, help="Maximum concurrent job workers.")
    parser.add_argument("--max-attempts", type=int, default=3, help="Max generate/review cycles per job.")
    parser.add_argument("--timeout-seconds", type=int, default=420, help="Timeout per Gemini invocation.")
    ...
    parser.add_argument("--min-review-score", type=int, default=84, help="Minimum review score required to accept output.")
...
def main() -> None:
    ...
    for wave in waves:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                print(
                    f"[{status}] {result['section']} v{result['variant']} ({result['direction']}) attempts={result['attempts']} review={result.get('review_score')}",
                    flush=True,
                )
    ...
    manifest_path = QA_DIR / f"generation-manifest-engaged-v{args.start_variant}-v{args.end_variant}.json"
