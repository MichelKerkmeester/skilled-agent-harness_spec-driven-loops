# Historical ChatGPT vs Removed Copilot Agent Diff

## Method

- Compared files one-to-one by basename between the historical ChatGPT agent suite and the removed Copilot agent suite.
...
- Included only these 8 agent files: context agent, debug agent, handover agent, orchestrate agent, research agent, review agent, speckit agent, write agent.
