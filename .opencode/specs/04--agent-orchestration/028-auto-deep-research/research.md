...
- **Feature/Spec**: `specs/04--agent-orchestration/028-auto-deep-research`
- **Status**: Complete
...
Our system has `/spec_kit:research` which runs a 9-step single-pass research workflow. It supports parallel agent dispatch for steps 3-5 and produces a `research.md` with 17 sections. However, it does NOT support iterative deepening, progressive refinement, or autonomous loop execution. Research is one-shot: the agent investigates once and documents findings.
...
1. **`@deep-research` agent** (`.claude/agents/autoresearch.md`) -- LEAF agent with loop-aware instructions, full tool access (Read, Write, Edit, Bash, Grep, Glob, WebFetch, spec_kit_memory)
2. **`/spec_kit:deep-research` command** (`.opencode/command/autoresearch/`) -- YAML workflow defining the loop protocol, dispatch modes, and convergence criteria
...
| `research.md` | Markdown | Progressive synthesis (updated each iteration) | Updated each iteration |
...
[Evaluate] Orchestrator reads updated state, checks convergence
       ...
       |
       └── Converged --> [Synthesize] Final research.md + memory save
```
...
output:
  progressive_synthesis: true  # update research.md each iteration
...
User: /spec_kit:deep-research "How do WebSocket reconnection strategies work across browsers?"
  ...
  --> Produces research.md + memory save
...
```
.claude/agents/autoresearch.md          # Agent definition
.opencode/command/autoresearch/          # Command definition
  autoresearch.md                        # Command spec
  assets/
    autoresearch_auto.yaml              # Autonomous mode workflow
    autoresearch_confirm.yaml           # Interactive mode workflow
.opencode/skill/sk-deep-research/        # Skill reference
  SKILL.md                             # Protocol documentation
  references/
    loop-protocol.md                   # Detailed loop specification
    state-format.md                    # JSONL/strategy file formats
    convergence.md                     # Convergence detection algorithms
  templates/
    deep-research-config.json           # Config template
    deep-research-strategy.md           # Strategy file template
```

### Agent Definition (autoresearch.md)
...
4. **Write everything down**: Findings go to `scratch/iteration-NNN.md`, not held in context
...
mkdir -p specs/04--agent-orchestration/028-auto-deep-research/scratch

# Initialize config
cat > scratch/deep-research-config.json << 'EOF'
{
...
echo '{"type":"config","topic":"Autonomous deep research loop integration","maxIterations":10}' > scratch/deep-research-state.jsonl

# Initialize strategy
cat > scratch/deep-research-strategy.md << 'EOF'
# Research Strategy
## Topic
...
FOR each iteration (1..maxIterations):
  1. READ scratch/deep-research-state.jsonl
  2. READ scratch/deep-research-strategy.md
...
5. WRITE scratch/iteration-003.md with findings
...
**Diagnostic Commands**:
- `cat scratch/deep-research-state.jsonl | jq .` -- validate JSONL
- `wc -l scratch/deep-research-state.jsonl` -- count iterations
- `grep "newInfoRatio" scratch/deep-research-state.jsonl | jq .newInfoRatio` -- track convergence
...
Instead of synthesizing research.md only at the end, update it after each iteration. This means the final synthesis step is incremental (add new findings) rather than comprehensive (process all findings from scratch).
...
3. If corrupted, restore from git: `git checkout -- scratch/deep-research-state.jsonl`
...
6. Ideas backlog file (scratch/research-ideas.md) -- from autoresearch-opencode
...
4. Progressive synthesis (research.md grows across iterations)
