...
- **Parent packet:** `.opencode/specs/deep-loops/030-agent-loops-improved/` (the `002-implementation` parent was ungrouped earlier this arc; its children are now 156 root phases `002`–`008`; the whole 156 tree validates 63/63).
- **This phase parent:** `123-agent-loops-improved/008-loop-systems-remediation/` with 6 children (below).
...
- `123-agent-loops-improved/{spec.md, description.json, graph-metadata.json}` — modified (phase-map gained the `009` row; metadata re-regenerated). **Uncommitted.**
