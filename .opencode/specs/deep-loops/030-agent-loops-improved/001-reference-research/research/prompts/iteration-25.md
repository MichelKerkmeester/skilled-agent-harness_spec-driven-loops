...
Segment: 4 (map -> workflows + speckit) | Iteration: 25 of 58
Questions: 0/49 checked | Last focus: S3-10
Last 2 ratios: 0.58 -> 0.62 | Stuck count: 0
...
Iteration: 25 of 58
Dimension this iteration: D2 (target-mapping)
Focus Area: [S4-01] Should we add a dedicated injection inbox (`research/inbox.jsonl` or a new `<!-- ANCHOR:injected-questions -->` block) given `reduce-state.cjs:800` rewrites the key-questions anchor every iteration — and how do injected items get promoted without losing provenance? -> deep_research_strategy.md + reduce-state.cjs
...
Last 3 Iterations Summary: run 22: S3-06 (ratio 0.57); run 23: S3-09 (ratio 0.58); run 24: S3-10 (ratio 0.62)
...
- Write iteration narrative to: .opencode/specs/deep-loops/030-agent-loops-improved/001-reference-research/research/iterations/iteration-025.md
- Write per-iteration delta file to: .opencode/specs/deep-loops/030-agent-loops-improved/001-reference-research/research/deltas/iter-025.jsonl
...
1. Iteration narrative markdown at `.opencode/specs/deep-loops/030-agent-loops-improved/001-reference-research/research/iterations/iteration-025.md` with headings: Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus. In Findings give: reference mechanism + file:line; exact OUR target file; port-difficulty (easy/med/hard); quick-win vs deep-rewrite tag.

2. Canonical JSONL record APPENDED to the State Log, EXACT type "iteration":
`{"type":"iteration","iteration":25,"newInfoRatio":<0..1>,"status":"<complete|insight|stuck>","focus":"S4-01","graphEvents":[...]}`
Append single-line with trailing newline: `echo '<json>' >> .opencode/specs/deep-loops/030-agent-loops-improved/001-reference-research/research/deep-research-state.jsonl`. Do NOT pretty-print; it MUST land in the file. Do NOT write an `executor` block.

3. Per-iteration delta file at `.opencode/specs/deep-loops/030-agent-loops-improved/001-reference-research/research/deltas/iter-025.jsonl`: the same iteration record plus one record per finding/observation/graphEvent, each on its own line.
