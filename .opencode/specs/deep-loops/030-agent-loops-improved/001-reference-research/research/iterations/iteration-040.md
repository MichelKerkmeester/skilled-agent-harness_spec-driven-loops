...
1. Ranked backlog item: add a nullable wait checkpoint object to the research session schema before adding behavior.
   - Reference mechanism: loop-cli carries `nextRunAt` and `remainingDelayMs` in controller state and public loop metadata [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/core/loop-controller.ts:22`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/types.ts:81`]. The constructor restores both from persisted state [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/core/loop-controller.ts:77`].
...
2. Ranked backlog item: create an explicit pre-dispatch wait persist point; the current auto loop has no durable wait boundary to resume.
   - Reference mechanism: loop-cli sets `remainingDelayMs`, announces `waiting` with `nextRunAt`, sleeps in chunks, and refreshes remaining time after each chunk [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/core/loop-controller.ts:228`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/core/loop-controller.ts:266`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/core/loop-controller.ts:273`]. The daemon persists on the `waiting` event and skips writes when serialized state has not changed [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/manager.ts:257`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/manager.ts:279`].
...
4. Ranked backlog item: make migration nullable and conservative; old packets must remain ordinary resumes.
   - Reference mechanism: loop-cli's controller restore accepts optional state fields and defaults absent `nextRunAt`/`remainingDelayMs` to null [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/core/loop-controller.ts:13`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/core/loop-controller.ts:77`]. It also migrates older storage formats without requiring every loop record to carry new fields first [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/state.ts:45`].
