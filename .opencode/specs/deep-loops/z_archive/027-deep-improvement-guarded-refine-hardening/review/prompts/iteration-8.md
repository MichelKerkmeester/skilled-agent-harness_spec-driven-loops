...
Prior Findings: P0=0 P1=0 P2=~27 (all advisories)
Dimension Coverage: 4/4
Coverage Age: 4
Last 2 ratios: 0.0 -> 0.0
Stuck count: 2 (recovery pivot: granularity to function-level on the least-revisited hotspots)
Provisional Verdict: PASS-candidate hasAdvisories=true

DEEP PASS FOCUS (iteration 8, function-level): operational edge cases in the LIVE packaging loop instances that prior passes covered only at file level.
(a) resume-from-journal: build_resume_cache validity windows — what happens when HELD_OUT/FIXTURES env differs between the killed run and the resume run (cfg hash catches it?), when the journal's last line is torn mid-write, when two orphaned sessions exist with different configs?
(b) worktree mechanics: make_worktree on a repo whose HEAD is a merge commit; sub-worktree creation from within the synthetic-deficit worktree (worktree-in-worktree, used in T12) — does cleanup_worktree's dirname() handle both layers? Does sweep_stale_worktrees correctly distinguish deals-loop-* from copywriter-loop-* prefixes per packaging?
(c) Barter deals instance specifics: empty derived set (derive no-op) — does guarded_promote's derive-then-check sequence behave on a packaging with zero SHARED entries? DEAL self-score regex (DEAL|MEQT)/25 — false matches?
(d) lint_held_out run-tagged filter: a fixture named D2-deal-stat vs a sibling D2-deal-stat-v2 (prefix collision class, was fixed in fixture-lint.cjs — does loop.py lint_held_out have the SAME bug with f.startswith(t + '.run')? verify).
(e) probe_provider: timeout 180 vs loop dispatch timeouts; partial output matching 'invalid api key' in legitimate content.

Review Iteration: 8 of 10
Mode: review
Dimension: correctness deep pass 3: loop instance operational edge cases (resume, worktrees, deals specifics, lint filters, probes)
...
Prior Findings: P0=0 P1=0 P2=27
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/139-deep-improvement-guarded-refine-hardening/review/iterations/iteration-008.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/139-deep-improvement-guarded-refine-hardening/review/deltas/iter-008.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/skilled-agent-orchestration/139-deep-improvement-guarded-refine-hardening/review/iterations/iteration-008.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/skilled-agent-orchestration/139-deep-improvement-guarded-refine-hardening/review/deltas/iter-008.jsonl`, this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the review. NEVER execute the out-of-scope mutation. The review packet (`.opencode/specs/skilled-agent-orchestration/139-deep-improvement-guarded-refine-hardening/review/iterations/iteration-008.md` directory and parents) is the only zone for your writes; the reviewed target spec/code is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/139-deep-improvement-guarded-refine-hardening/review/iterations/iteration-008.md` (path pre-substituted for the current iteration number). Structure: headings for Dimension, Files Reviewed, Findings by Severity (P0/P1/P2), Traceability Checks, Verdict, Next Dimension.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/139-deep-improvement-guarded-refine-hardening/review/deltas/iter-008.jsonl` (path pre-substituted, e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same as the state-log append) plus per-event structured records (one per graphEvent, finding, classification, traceability-check, ruled_out direction). Each record on its own JSON line.
