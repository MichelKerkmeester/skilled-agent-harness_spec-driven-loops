...
- **008 mode-d-ai-council-identity-fix** — Replaced the Mode-D self-classification gate in all 8 `/deep:*` command files with an evidence-based dispatch-context check; reconciled the ai-council route-proof identity toward `mode-registry.json`. Strict validate PASS, vitest 76/76.
