'use strict';
// Lineage synthesis driver: regenerates reducer-owned files + writes research.md,
// resource-map.md, convergence report, and the synthesis_complete JSONL event.
// DIR = lineage root (parent of prompts/).
...
// --- parse iterations from state log ---
const records = fs.readFileSync(STATE_LOG, 'utf8').split('\n').filter(Boolean).map(JSON.parse);
const iters = records.filter(r => r.type === 'iteration').sort((a,b)=>a.run-b.run);
const totalIter = iters.length;
const answeredKQs = ['KQ1','KQ2','KQ3','KQ4','KQ5','KQ6','KQ7','KQ8','KQ9'];
const avgRatio = (iters.reduce((s,i)=>s+i.newInfoRatio,0)/totalIter).toFixed(3);
const trendStr = iters.map(i=>i.newInfoRatio.toFixed(2)).join(' -> ');

// ============ 1. FINDINGS REGISTRY ============
const registry = {
  version: '1.0',
  specFolder: '.opencode/specs/deep-loops/031-deep-loop-issues-with-gpt-opencode/007-gpt-behavioral-hardening-research',
  lineage: 'glm-max',
  sessionId: 'fanout-glm-max-1782886969751-3481xi',
  openQuestions: [],
  resolvedQuestions: answeredKQs.map(k=>{
    const map = {
      KQ1:'Decisive external smoke (8-run, route-proof-augmented, baseline-controlled) — iter 5',
      KQ2:'Symptoms = MIX: mis-routing (A/B/C, orchestrate rows incomplete) + new mode D (GPT-vs-state-machine literalism) + latency (role-resolution overhead) — iter 6,7,8',
      KQ3:'Keep ai-council mode:all; do NOT convert; add route-proof header — iter 9,10,11',
      KQ4:'Orchestrate delegates deep-dispatch to deep.md via single deterministic rule (dispatch @deep + STOP) — iter 12,13',
      KQ5:'Feasible hook-based enforcement plugin in system-skill-advisor; detection-layer, not hard identity — iter 14,15',
      KQ6:'8-run, 2-metric benchmark (latency + route-proof) reusable as baseline now + regression gate after fixes — iter 16,17',
      KQ7:'Literal-safe pattern = table + bounded clarification + structured header + hard boundaries; apply to ROUTING surfaces only — iter 18,19',
      KQ8:'6 in-scope paths (orchestrate, 4 command seams, CLI seam, advisor plugin) + .claude mirrors; Codex + improvement-family deferred — iter 20,21,22',
      KQ9:'WAIT on KQ1 external smoke + cheaper layers; unpark FIX-5 only on measured negative-gate (GPT<4/4 vs Claude=4/4 after KQ4+KQ5) — iter 23,24',
    };
    return { id:k, text: map[k], resolvedAtRun: {KQ1:5,KQ2:8,KQ3:11,KQ4:13,KQ5:15,KQ6:17,KQ7:19,KQ8:22,KQ9:24}[k] };
  }),
  keyFindings: [
    'Root cause reconfirmed: subagent_type normalized to "general"; soft identity boundary (../001 §1).',
    'deep.md literal-safe = deterministic table + single bounded clarification + structured header + 5 hard boundaries (deep.md:34-98).',
    'orchestrate.md deep-path is judgment-dependent: missing deep-context/deep-review rows + fill-in Deep Route field (orchestrate.md:95-105,207).',
    'Phase 005 inconclusive cause = cli-opencode 3-layer self-invocation guard tripped on OPENCODE_PID (cli-opencode/SKILL.md:66,87,319; 005:120).',
    'NEW mode D: GPT reads soft advisory command-prose as hard gates and halts (separable from A/B/C routing + latency) — iter 7.',
    'KQ5 plugin catches route/identity mismatch but NOT semantic wrong-mode (the §5 false-negative) — iter 25.',
    'FIX-5 wait is risk-asymmetric (reversible) vs unpark-now (architectural, irreversible) — iter 24.',
  ],
  ruledOutDirections: [
    { approach:'Re-running the smoke from inside an OpenCode session', reason:'OPENCODE_PID guard structurally trips', evidence:'005:120; cli-opencode/SKILL.md:319', iteration:5 },
    { approach:'Treating all operator symptoms as one mis-routing class', reason:'conflates latency, routing, and state-machine literalism', evidence:'iter 6', iteration:6 },
    { approach:'Converting ai-council to subagent-only now', reason:'removes non-broken depth-0 parallel feature; symptoms do not involve council', evidence:'ai-council.md:55-60; research-prompt.md:21', iteration:11 },
    { approach:'Applying the literal-safe table pattern to leaf/execution surfaces', reason:'harms Claude evidence-response (mode D) and removes controlled flex-escape', evidence:'deep.md:51-59,66', iteration:19 },
    { approach:'Codex parity in this work', reason:'TOML-location contradiction unresolved', evidence:'../001 §8', iteration:20 },
    { approach:'Unparking FIX-5 immediately on operator evidence alone', reason:'architectural blast radius; cheaper layers unproven-insufficient; wait is reversible', evidence:'../001 §8b; iter 23', iteration:24 },
  ],
  metrics: {
    iterationsCompleted: totalIter,
    answeredCount: answeredKQs.length,
    totalQuestions: 9,
    averageNewInfoRatio: Number(avgRatio),
  },
};
fs.writeFileSync(path.join(DIR,'findings-registry.json'), JSON.stringify(registry, null, 2)+'\n');

// ============ 2. STRATEGY (reducer-refreshed machine sections) ============
const strat = `---
...
> Reducer-refreshed after ${totalIter} iterations. Status: complete (synthesized).
...
## 5. STOP CONDITIONS
Met: ${totalIter} iterations reached (stop policy max-iterations); all 9 KQs answered with citations.

<!-- ANCHOR:answered-questions -->
## 6. ANSWERED QUESTIONS
${registry.resolvedQuestions.map(r=>`- **${r.id}** (iter ${r.resolvedAtRun}): ${r.text}`).join('\n')}
<!-- /ANCHOR:answered-questions -->
...
<!-- /ANCHED:exhausted-approaches -->

<!-- ANCHOR:ruled-out-directions -->
## 10. RULED OUT DIRECTIONS
${registry.ruledOutDirections.map(r=>`- **${r.approach}** (iter ${r.iteration}): ${r.reason} [SOURCE: ${r.evidence}]`).join('\n')}
<!-- /ANCHOR:ruled-out-directions -->
...
- Average newInfoRatio: ${avgRatio} | Generation: 1
`;
fs.writeFileSync(path.join(DIR,'deep-research-strategy.md'), strat);

// ============ 3. DASHBOARD ============
const iterTable = iters.map(i=>`| ${i.run} | ${i.focusTrack} | ${i.newInfoRatio.toFixed(2)} | ${i.findingsCount} | ${i.status} |`).join('\n');
const dash = `# Deep Research Dashboard — glm-max lineage
...
## Question Status
${answeredKQs.length}/${answeredKQs.length} answered (KQ1-KQ9 all resolved)

## Convergence Trend
newInfoRatio: ${trendStr}
Average: ${avgRatio} | Direction: descending (coverage -> deepening -> adversarial -> synthesis), as expected for a max-iterations run.

## Dead Ends / Ruled Out
${registry.ruledOutDirections.map(r=>`- ${r.approach} (iter ${r.iteration})`).join('\n')}

## Blocked Stops
None (stop policy = max-iterations; convergence treated as telemetry only).

## Next Focus
SYNTHESIS COMPLETE.
`;
fs.writeFileSync(path.join(DIR,'deep-research-dashboard.md'), dash);

// ============ 4. RESEARCH.md (synthesis) ============
const research = `# Research: GPT Behavioral Hardening Follow-Up (Packet 031) — glm-max lineage
...
- **newInfoRatio trend:** ${trendStr}
- **Average newInfoRatio:** ${avgRatio} (descending arc: coverage 1-5 → mechanism/council/orchestrate/plugin 6-15 → benchmark/pattern/propagation/FIX5 16-24 → adversarial re-checks 25-26 → analytical synthesis 27-30).
...
// ============ 5. RESOURCE-MAP.md ============
const resmap = `# Resource Map — glm-max lineage (evidence-derived)

> Emitted from converged research delta evidence. resource-map.md was NOT present at the spec-folder root at init (coverage gate skipped); this lineage-derived map inventoried the files cited as evidence across iterations 1-30.

## Documents
- \`../001-deep-agent-router-and-orchestration/research/research.md\` — predecessor research (root cause, FIX-5 false-negative, phase breakdown)
- \`../005-gpt-verification-smoke/verification-smoke.md\` — inconclusive smoke procedure + results
- \`../006-host-hard-identity-fix5/decision-record.md\` — FIX-5 parked trigger
- \`research-prompt.md\` — research charter (KQ1-KQ9, boundaries, stop conditions)
- \`../goal-prompt.md\` — packet objective + observed symptoms

## Agents
- \`.opencode/agents/deep.md\` — primary deep-loop router (literal-safe reference)
- \`.opencode/agents/orchestrate.md\` — senior orchestrator (judgment-dependent deep path)
- \`.opencode/agents/ai-council.md\` — multi-strategy council (mode: all)
- \`.claude/agents/{orchestrate,deep,ai-council}.md\` — Claude runtime mirrors (must receive same edits)

## Commands
- \`.opencode/commands/deep/{research,review,context,ai-council}.md\` — deep-loop command entry points (Resolved-route header seams)

## Skills
- \`.opencode/skills/deep-loop-workflows/mode-registry.json\` — single source of truth for deep-mode routing
- \`.opencode/skills/cli-opencode/SKILL.md\` — self-invocation guard (3-layer OPENCODE_PID detector)
- \`.opencode/skills/system-skill-advisor\` — candidate home for the KQ5 enforcement plugin
- \`.opencode/skills/deep-loop-workflows/deep-research/{SKILL.md,references/**}\` — loop protocol/state/convergence contract

## Config
- \`spec.md\`, \`plan.md\`, \`tasks.md\`, \`implementation-summary.md\` — this phase's spec docs
`;
fs.writeFileSync(path.join(DIR,'resource-map.md'), resmap);

// ============ 6. JSONL: synthesis_complete + maxIterations event ============
const fd = fs.openSync(STATE_LOG, 'a');
fs.writeSync(fd, JSON.stringify({
  type:'event', event:'synthesis_complete', mode:'research', totalIterations: totalIter,
  answeredCount: answeredKQs.length, totalQuestions: 9,
  stopReason:'maxIterationsReached', averageNewInfoRatio: Number(avgRatio),
  sessionId:'fanout-glm-max-1782886969751-3481xi', generation:1,
  timestamp:'2026-07-01T10:30:00Z',
})+'\n');
fs.writeSync(fd, JSON.stringify({
  type:'event', event:'fanout_lineage_complete', mode:'research', label:'glm-max',
  sessionId:'fanout-glm-max-1782886969751-3481xi', lineageMode:'new', generation:1,
  totalIterations: totalIter, stopReason:'maxIterationsReached',
  timestamp:'2026-07-01T10:31:00Z',
})+'\n');
fs.closeSync(fd);

console.log(`Synthesis complete. ${totalIter} iterations, 9/9 KQs answered, avg newInfoRatio ${avgRatio}.`);
console.log('Wrote: findings-registry.json, deep-research-strategy.md, deep-research-dashboard.md, research.md, resource-map.md + 2 JSONL events.');
