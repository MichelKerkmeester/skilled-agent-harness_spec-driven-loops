...
1. Deliverable A: replace Phase 0 self-assessment gates in all 8 `/deep:*` command docs with deterministic invocation-context checks. The current check asks the model to decide if it is `@general`; that is the Mode-D trap. [SOURCE: .opencode/commands/deep/research.md:39-72]
