...
| Propagation scope | `orchestrate.md` (+ `.claude` mirror), the 4 deep command YAML/prompt seams, `orchestrate-topic.cjs` (ai-council identity fix), all 8 command files' Phase-0 blocks (Mode D fix), the new plugin surface. | High (5 lineages) |
...
1. **008 — Mode-D + route-identity fixes (low-risk, land first).** Replace the Phase-0 self-classification gate in all 8 `/deep:*` command files with a deterministic dispatch-context check. Reconcile the ai-council route-proof identity (`orchestrate-topic.cjs` + `deep_ai-council_auto.yaml`) toward the registry, both files together. Bundle `.claude` mirrors.
