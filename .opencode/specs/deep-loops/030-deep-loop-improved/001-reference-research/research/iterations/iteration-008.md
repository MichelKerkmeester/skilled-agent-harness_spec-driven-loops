...
1. Reference mechanism: the serialize-diff gate is not actually in `src/daemon/state.ts`; it lives one layer up in `LoopManager`. The manager keeps `lastSerialized` per loop [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/manager.ts:23`], fans multiple lifecycle events into one `persist` callback [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/manager.ts:263`], builds a `LoopMeta`, serializes it, returns early when unchanged, and only then calls `saveLoop` [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/manager.ts:279`].
...
2. Reference mechanism: `state.ts` is the persistence sink, not the diff owner. It reads the whole `loops.json` array with corrupt-read fallback to `[]` [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/state.ts:29`], serializes arrays with pretty JSON before atomic write [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/state.ts:40`], and `saveLoop` performs whole-array upsert before writing [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/daemon/state.ts:87`].
...
4. Reference mechanism: `loop-cli-main` combines diff suppression with a minimal atomic rename helper that writes a temp sibling and renames it [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/shared/fs-utils.ts:9`]. Our writer is already stronger: it writes a temp file, fsyncs it, renames, then attempts to fsync the parent directory [TARGET: `.opencode/skills/deep-loop-runtime/lib/deep-loop/atomic-state.ts:58`].
