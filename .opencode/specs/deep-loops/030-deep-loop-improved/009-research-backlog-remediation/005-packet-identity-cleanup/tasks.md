...
_memory:
  continuity:
    packet_pointer: "deep-loops/030-deep-loop-improved/009-research-backlog-remediation/005-packet-identity-cleanup"
...
- [x] T001 `grep -rln "123-agent-loops-improved\|156-agent-loops-improved"` under `030-deep-loop-improved/` — real count was much larger than the original 16-file estimate once logs/iterations/changelogs were included (~166 raw hits); scoped the audit to the files that could plausibly carry a live navigational reference (all phase-parent `spec.md` files)
