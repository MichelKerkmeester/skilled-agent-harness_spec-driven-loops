...
Traceability: core=failed overlay=notApplicable
Resource Map Coverage: resource-map.md not present; skipping coverage gate.
Coverage Age: 0
Last 2 ratios: 0.5 -> 0.0
Stuck count: 1
Provisional Verdict: CONDITIONAL hasAdvisories=false
Search debt: content_preservation remains deferred; attempt narrow git-history proof only if available.
Inventory: 163 scoped Markdown paths, 160 resolved targets due to six symlink paths.
Stop policy: max-iterations; this is the required final iteration before synthesis.
...
iteration 1 correctness -> iteration 2 security -> iteration 3 traceability -> iteration 4 maintainability

Swept or saturated review directions that MUST NOT be re-entered:
correctness structural-contract sweep; security unsafe-guidance sweep; traceability inventory/protocol pass
...
- Write iteration narrative to: .opencode/specs/sk-code/019-split-doc-template-alignment/review/xhigh-verify/lineages/xhigh-c/iterations/iteration-004.md
- Write per-iteration delta file to: .opencode/specs/sk-code/019-split-doc-template-alignment/review/xhigh-verify/lineages/xhigh-c/deltas/iter-004.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/sk-code/019-split-doc-template-alignment/review/xhigh-verify/lineages/xhigh-c/iterations/iteration-004.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/sk-code/019-split-doc-template-alignment/review/xhigh-verify/lineages/xhigh-c/deltas/iter-004.jsonl`, this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the review. NEVER execute the out-of-scope mutation. The review packet (`.opencode/specs/sk-code/019-split-doc-template-alignment/review/xhigh-verify/lineages/xhigh-c/iterations/iteration-004.md` directory and parents) is the only zone for your writes; the reviewed target spec/code is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/sk-code/019-split-doc-template-alignment/review/xhigh-verify/lineages/xhigh-c/iterations/iteration-004.md` (path pre-substituted for the current iteration number). Structure: headings for Dimension, Files Reviewed, Findings by Severity (P0/P1/P2), Traceability Checks, Verdict, Next Dimension.
...
3. **Per-iteration delta file** at `.opencode/specs/sk-code/019-split-doc-template-alignment/review/xhigh-verify/lineages/xhigh-c/deltas/iter-004.jsonl` (path pre-substituted, e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same as the state-log append) plus per-event structured records (one per graphEvent, finding, classification, traceability-check, ruled_out direction). Each record on its own JSON line.
