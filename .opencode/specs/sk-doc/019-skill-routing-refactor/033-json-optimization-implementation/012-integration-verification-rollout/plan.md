...
### Tested rollback records for the program's high-blast changes

- **Derived regenerator + freshness gate (003).** Data surface: `git revert` of its commit restores prior `graph-metadata.json` state and removes the gate from CI in one step; the regenerator never rewrites authored fields, so no derived state needs reconstruction. Tested equivalence: `git checkout HEAD -- <graph-metadata>` round-trips were exercised repeatedly during that phase's corpus isolation with byte-identical restores.
- **manual→edges migration (008).** Single-commit data-only change; `git revert` restores the prior `manual`/`edges` blocks exactly. Post-revert the daemon needs one `advisor_rebuild --trusted` so the live `skill_edges` drops the migrated rows — the same one-step reload this phase proved. Rollback rehearsed during the phase via `git checkout HEAD --` restores with corpus re-verification at each step.
- **Command-bridge cutover (011).** The strongest record of all: the live cutover was *actually rolled back in production flow* — three corpus-gate catches, then `git checkout HEAD --` restored the hand-authored bridges with the corpus byte-identical in all three regimes afterward. The shadow machinery (generator, pinned drift-guard, e2e) survives rollback untouched by design; the revert path is proven, not theoretical.
<!-- /ANCHOR:rollback -->
