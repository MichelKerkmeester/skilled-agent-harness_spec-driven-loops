...
Every item carries evidence specific to itself — a distinct command, artifact section, or file reference. Four items that certified a regression as absent, or a completion as clean, were re-opened by the evidence-integrity repair because phase 013 disproved them, and restated against the measured figures rather than reverted to a pass.

**Amendment (2026-07-30):** those four rows were closed once the certified properties became independently true — the regression was fixed and every pin restored, the packet gate went genuinely green, and the once-unbuilt ingestion phase was delivered and CI-verified. Each row keeps its RE-OPENED history inline so the false certification remains on the record; the closure marks the property's current truth, not a retraction of that history.
...
<!-- ANCHOR:testing -->
## Testing
- [x] CHK-006 [P0] Pinned corpus rerun (full/holdout/ambiguity) under the same reproducible env as the `002` baseline; top-1 delta reported [evidence: RE-OPENED history — false as certified: results/final-corpus-capture.md reports Python advisor accuracy and top-3 only, with no holdout top-1 row at all, so the metric that moved was never displayed. Phase 013 measured the shipped HEAD holdout top-1 at 51/72 versus the 002 pin 53/72 — a −2 that this item passed over. Fixed in 013, restored to 53/72. CLOSED 2026-07-30: the certified property is now true — the capture reports the full metric set with holdout top-1 restored to 53/72 after the diagnosis phase fixed the stale delegation-scorer path, re-verified post-cutover locally by exit code and live in CI (corpus-gate step of run 30548794458).]
- [x] CHK-007 [P0] Top-3 metric added and reported for all three slices [evidence: RE-OPENED history — the top-3 number (holdout 53/72) was reported, but the regression was laundered: results/final-corpus-capture.md line 14 dismisses the true 55/72 pin as a stale pre-program note. Phase 013 confirmed the 002 pin is 55/72 and the shipped HEAD was 53/72 (−2); the "zero-delta" top-3 claim was measured against a redefined baseline. Fixed in 013, restored to 55/72. CLOSED 2026-07-30: top-3 is measured and at pin for both pinned slices — full corpus 176/195 and holdout 55/72 — with the 55/72 pin standing un-redefined; the ambiguity slice carries a top-1 pin only (no top-3 pin exists), recorded as a coverage note rather than silently claimed.]
...
## Fix Completeness
- [x] CHK-012 [P0] No unexplained top-1/top-3 regression vs the `002` baseline; any regression carries an explicit operator-approved reason [evidence: RE-OPENED history — false: phase 013 found an unexplained, un-approved −2 on holdout top-1 (53→51), holdout top-3 (55→53) and the delegation bucket (10→8) in the shipped HEAD. Root cause: a mode-packet rename orphaned the delegation scorer's model-profiles path, emptying the model-alias table. Diagnosed and fixed in 013, all three restored to pin. CLOSED 2026-07-30: the regression was diagnosed to a single root cause (mode-packet rename orphaning the model-profiles path), fixed in one line, and every affected metric restored exactly to pin with no other metric moving — zero delta vs the 002 baselines, held through the command-bridge cutover and the live CI run.]
...
- [x] CHK-019 [P1] Parent packet (`033-json-optimization-implementation`) completion metadata reconciled — `spec.md`/`implementation-summary.md` do not claim Complete before this checklist is green [evidence: RE-OPENED history — false: the parent spec.md claimed Status Complete while this checklist certified a regression as absent and `validate --recursive --strict` failed packet-wide. Phase 015 (REQ-005) withdraws the parent Complete claim pending a green gate; phase 016 re-establishes it after the metadata fingerprints regenerate. CLOSED 2026-07-30: the premature claim was withdrawn, fingerprints regenerated, and the parent re-claims Complete only now — with validate --recursive --strict at Errors:0 packet-wide and every phase delivered, including the ingestion phase that was still unbuilt when the claim was first made.]
<!-- /ANCHOR:docs -->
...
| Corpus rerun + top-1/top-3 delta | 2 | 2/2 — re-opened after the false certification, closed after the fix restored every pin (history inline) |
| Daemon reindex proof | 2 | 2/2 |
| Cache invalidation | 2 | 2/2 |
| Rollback plans (003/008/011) | 3 | 3/3 |
| Completion-honesty (parent Complete claim) | 1 | 1/1 — closed once the packet became genuinely complete with the gate green |

**Verification Date**: 2026-07-30 (re-opened by the evidence-integrity repair; closed after remediation restored the certified properties)
<!-- /ANCHOR:summary -->
