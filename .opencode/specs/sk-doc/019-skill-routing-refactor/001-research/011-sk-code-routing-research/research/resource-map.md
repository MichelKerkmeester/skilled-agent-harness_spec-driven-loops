---
title: "Resource Map — Diagnose sk-code skill-routing faults and optimize them, applying the sk-doc typed-pair routing standard. Carry in verified evidence that sk-code scores about 65/100 with 18/18 surface routing but about 50% leaf-file recall; packet 015 produced leaf-manifest.json and manifest-gated typed-gold derivation with mean typedPairRecall 0.729 over 14 scenarios; investigate the untyped DEFAULT_RESOURCE preamble routing_contract_error, benchmark scoring, shared manifest qualification, and concrete routing template, logic, and JSON configuration optimizations."
description: "Auto-generated research resource map from convergence evidence."
---
# Resource Map

<!-- SPECKIT_TEMPLATE_SOURCE: resource-map | v1.1 -->

---

## Summary

- **Total references**: 26
- **By category**: READMEs=0, Documents=4, Commands=0, Agents=0, Skills=20, Specs=1, Scripts=0, Tests=0, Config=1, Meta=0
- **Missing on disk**: 6
- **Scope**: research convergence output for 014-sk-code-router-alignment
- **Generated**: 2026-07-17T04:32:24.045Z

> **Action vocabulary**: `Created` · `Updated` · `Analyzed` · `Removed` · `Cited` · `Validated` · `Moved` · `Renamed`.
> **Status vocabulary**: `OK` · `MISSING` · `PLANNED`.

## 2. Documents

> Long-form markdown artifacts that are not READMEs: guides, specs, references, install docs, catalogs, playbooks.

| Path | Action | Status | Note |
|------|--------|--------|------|
| research/deep-research-state.jsonl | Cited | MISSING | Citations=1; Iterations=1 |
| research/deep-research-strategy.md | Cited | MISSING | Citations=1; Iterations=1 |
| research/iterations/iteration-005.md | Cited | MISSING | Citations=1; Iterations=1 |
| research/iterations/iteration-006.md | Cited | MISSING | Citations=1; Iterations=1 |

---

## 5. Skills

> `.opencode/skills/**` including `SKILL.md`, `references/`, `assets/`, `feature_catalog/`, `manual_testing_playbook/`, `scripts/`, `shared/`, `mcp_server/`.

| Path | Action | Status | Note |
|------|--------|--------|------|
| .opencode/skills/sk-code/benchmark/reports/baseline/skill-benchmark-report.json | Cited | OK | Citations=2; Iterations=2 |
| .opencode/skills/sk-code/benchmark/fixtures/sk_code/sk-code-loadspeed-001.private.json | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-code/benchmark/live_final/skill-benchmark-report.json | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-code/code-opencode/SKILL.md | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-code/code-webflow/SKILL.md | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-code/manual_testing_playbook/cross_stack_routing/webflow_plus_motion_dev.md | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-code/manual_testing_playbook/surface_detection/webflow_detection.md | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-code/mode-registry.json | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-code/shared/references/smart_routing.md | Cited | OK | Citations=2; Iterations=2 |
| .opencode/skills/sk-code/SKILL.md | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-doc/create-skill/references/parent_skill/parent_hub_router_schema.md | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-doc/create-skill/scripts/generate-leaf-manifest.cjs | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/sk-doc/create-skill/scripts/lib/leaf-resource-contract.cjs | Cited | OK | Citations=2; Iterations=2 |
| .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/codex-executor.cjs | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/live-executor.cjs | Cited | OK | Citations=3; Iterations=3 |
| .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/load-playbook-scenarios.cjs | Cited | OK | Citations=2; Iterations=2 |
| .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/router-replay.cjs | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/run-skill-benchmark.cjs | Cited | OK | Citations=1; Iterations=1 |
| .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/score-skill-benchmark.cjs | Cited | OK | Citations=2; Iterations=2 |
| .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/score-skill-benchmark.cjs:179-291,1146-1257,1450-1725 | Cited | MISSING | Citations=1; Iterations=1 |

---

## 6. Specs

> `.opencode/specs/**` and `specs/**`. Takes precedence over `Config` for spec-folder JSON metadata.

| Path | Action | Status | Note |
|------|--------|--------|------|
| .opencode/specs/sk-doc/019-skill-routing-refactor/014-sk-code-router-alignment/research/findings-registry.json | Cited | OK | Citations=1; Iterations=1 |

---

## 9. Config

> Machine-readable configuration: `.json`, `.jsonc`, `.yaml`, `.yml`, `.toml`, `.env.example`.

| Path | Action | Status | Note |
|------|--------|--------|------|
| research/findings-registry.json | Cited | MISSING | Citations=1; Iterations=1 |

---
