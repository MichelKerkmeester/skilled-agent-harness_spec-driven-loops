...
> **Serving-authority note (current state):** this before/after describes the *authored* routing config — the JSON files above are the authoring source of truth. For hubs that have cut over, that authored config is now compiled and *served* by the compiled-routing runtime (Group E, `015-router-unification-program`, promoted to `.opencode/bin/lib/compiled-routing/`), which is the current serving authority: the JSONs still author it, the runtime resolves it. Legacy authored-config serving stays available and reversible until each hub's cutover gate passes.
