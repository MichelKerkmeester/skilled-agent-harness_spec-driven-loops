...
- [x] T001 Re-verify every load-bearing receipt named in `spec.md` against source, recording confirmed versus inferred and re-anchoring on symbols (treat `file:line` as +/-10). (REQ-001; evidence audit)
- [x] T002 Capture baseline SHA-256 values for `router-replay.cjs`, `score-skill-benchmark.cjs`, and `load-playbook-scenarios.cjs`; make digest equality a gate for every later step. (REQ-008; frozen-scorer integrity)
- [x] T003 Inventory every `SPECKIT_COMPILED_ROUTING` read, every eligibility consumer, every engine-entrypoint call site, the seven activation manifests, and the seven per-hub bundles. (REQ-001, REQ-002; runtime inventory) {deps: T001}
- [x] T004 Choose the stable runtime directory for the promoted closure and record the decision. (REQ-001; chose `.opencode/bin/lib/compiled-routing/`) {deps: T003}
...
- [x] T012 Add the `SPECKIT_COMPILED_ROUTING` entry to `ENV-REFERENCE.md` (default-off, tri-state, `=0` kill-switch, eligibility gating). (REQ-006; environment reference) {deps: T003}
- [x] T013 Tri-state the flag in `resolve.cjs` (near `:22-23`) with an empty per-hub default-on cohort; unset resolves to legacy for all hubs. (REQ-004; resolver flag site) {deps: T005}
- [x] T014 Tri-state the flag in `advisor-recommend.ts` (near `:362`) with identical semantics; keep the additive attach byte-identical. (REQ-004; advisor flag site) {deps: T013}
- [x] T015 Add stderr breadcrumbs to the three catches (`bin/compiled-route.cjs` near `:36-38`, `resolve.cjs` near `:45-47`, `advisor-recommend.ts` near `:352`) without changing the fallback outcome; stop discarding the child stderr the advisor breadcrumb needs. (REQ-005; three catches) {deps: T005, T014}
- [x] T016 Add the durable no-spec-import lint/CI rule scanning runtime dirs for `require`/import under `.opencode/specs`, exiting non-zero with the offending file named. (REQ-007; new lint/CI check) {deps: T005}
- [x] T017 Add positive (seeded spec-import) and negative (clean runtime) fixtures for the durable rule and wire the check into CI. (REQ-007; `.github/workflows/runtime-no-spec-import.yml`) {deps: T016}
