...
| PS-15 | P2 | sk-design / deep-loop | Undeclared per-mode and identity fields with no extension declaring them (`proceduresPath`, `loopHostMode`, one-off `mutating`, `externalLoopOwner`, spec-folder `packet_id` inside skill graph-metadata) |
...
- `loopHostMode` on the four improvement modes (`deep-loop-workflows/mode-registry.json:115,142,165,188`) — implied by `backendKind: improvement-host` but declared by nothing.
...
- `externalLoopOwner` one-off on ai-system-improvement (`:192`) — descriptive, but same undeclared status.
...
**Recommendation:** Add an "optional per-mode fields" table to the doctrine (command/agent/artifactRoot/loopHostMode/proceduresPath — owner, meaning, which hubs use them); delete the orphan `mutating` (or set it on all four lanes with a defined meaning distinct from `mutatesWorkspace`); strip `packet_id` from deep-loop's skill graph-metadata; either declare `proceduresPath` via a small `procedures` extension or fold it into a documented optional field.
...
| One mode ↔ one packet (doctrine's 1:1 reading) | ⚠️ 2→1 create-skill (PS-17) | ✅ | ✅ | ⚠️ 4→1 deep-improvement (PS-17) |
...
| Undeclared per-mode fields | ✅ (`command` only) | ✅ | ⚠️ `proceduresPath` (PS-15) | ⚠️ `loopHostMode`, `mutating`, `externalLoopOwner` (PS-15) |
