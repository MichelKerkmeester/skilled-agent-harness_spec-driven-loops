...
- [x] T011 Run `validate.sh --recursive --strict` on 999; confirm 0 content/structure errors for this child (the not-yet-generated `description.json`/`graph-metadata.json` are produced by the operator afterward). [EVIDENCE: `validate.sh --recursive --strict` on 999 → Errors 0 for every folder including this child (009 → 0/0); the only tree warning is a pre-existing continuity-freshness lag in sibling 001-research, which is untouched and out of scope.]
<!-- /ANCHOR:phase-3 -->
...
- [x] Recursive strict validation clean (content/structure) on the 999 parent
<!-- /ANCHOR:completion -->
