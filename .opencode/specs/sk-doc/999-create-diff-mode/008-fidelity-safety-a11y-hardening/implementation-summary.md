...
description: "Landed the reconciled GPT-5.6 review fixes in create_diff.py and validate_report.py behind an 18-test regression suite: strict UTF-8 decode, a correct empty/EOF-newline line model, an HTMLParser safety gate, legend contrast, and a keyboard-operable scoped side-by-side scroll."
trigger_phrases:
...
| `scripts/validate_report.py` | Rewrite | Real safety gate: parses with `html.parser` (robust to the malformed-attribute bypass), asserts the exact CSP directive set, inspects every URL-bearing attribute, and rejects `@import`/non-`data:` `url()`. |
| `scripts/test_create_diff.py` | Create | 18-test stdlib regression suite locking REQ-001..005 plus zero-JS/CSP/escaping/byte-reproducibility/pairing/collapse. |
...
Each finding was treated as a hypothesis and re-verified against the source before any edit — three of the reviewer's specific characterizations were corrected in the process (the empty-file case is a pure addition plus a phantom line, not "one changed line"; the `@@` header is a documented human scan anchor, not a patch header measured against `difflib`). Fixes were made surgically inside existing functions, prefer-refuse-over-fabricate for the engine and parse-don't-regex for the validator. The four demo reports were regenerated and re-validated after every behavior change, and the whole set stays byte-reproducible under a fixed `SOURCE_DATE_EPOCH`.
<!-- /ANCHOR:how-delivered -->
...
| Rewrite the validator on `html.parser` instead of patching the regex | The regex tag boundary broke on a malformed `alt=">"`, hiding a live `onerror`. A parser is the only robust fix; it also enables exact-CSP and URL-attribute checks. |
...
| Check | Result |
|-------|--------|
| Regression suite | PASS: `python3 scripts/test_create_diff.py` — Ran 18 tests, OK. |
| Strict decode (REQ-001) | PASS: `\xff` vs `\xfe` refused with exit 3; valid UTF-8 still diffs. |
| Legend contrast (REQ-002) | PASS: `--text` on `--add-inline`/`--del-inline` ≥ 4.5:1 measured in both themes. |
| Safety gate (REQ-003) | PASS: hostile payload (handler + remote `<video>` + `@import` + `javascript:` + `default-src *`) FAILS; the four real reports PASS. |
...
| code-opencode alignment | PASS: `# COMPONENT:` header + numbered section dividers added to all three scripts; `py_compile` and the 18-test suite still green after. |
| Canon gates | PASS: `package_skill.py --check` PASS (8 intentional 017 hyphen-naming advisories); `parent-skill-check.cjs` all invariants, 0 warnings; `check-frontmatter-versions.sh` clean. |
