...
- [x] CHK-022 [P0] REQ-003 validator FAILS every hostile hazard class and PASSES the four canonical reports. [EVIDENCE: `SafetyValidatorAllowlist` one-hazard-per-case (disallowed element/attribute, live handler, external/`data:`/`javascript:` href, `@import`, `url()`, refresh/unknown `http-equiv`, duplicate attribute, non-ASCII/mis-placed/duplicate/weakened/missing CSP); `ReportConformance.test_four_canonical_reports_pass_allowlist`.]
...
- [x] CHK-025 [P0] REQ-006 regression suite is checked in and green. [EVIDENCE: `test_create_diff.py` — Ran 39 tests, OK.]
...
- [x] CHK-033 [P0] Algorithm invariants hold: a reported "identical" implies byte-equal-after-normalization (no `�` collapse); a validator PASS implies conformance to the emitter's exact dialect (no live hazard). [EVIDENCE: `test_create_diff.py` StrictDecode + SafetyValidatorAllowlist + ReportConformance classes — 39/39 pass.]
<!-- /ANCHOR:fix-completeness -->
...
- [x] CHK-040 [P0] The validator is now a real gate: an `html.parser` allowlist of the emitter's exact dialect, robust to malformed attributes that defeated the old regex denylist. [EVIDENCE: `<img alt=">" onerror=…>` handler caught; any tag/attribute/URL/CSP outside the emitter's dialect fails; previously PASSed.]
...
| Gate | State | Evidence |
|------|-------|----------|
| Regression suite | PASS | `test_create_diff.py` — 39 tests, OK |
...
| Safety gate (REQ-003) | PASS | allowlist rejects every hazard class one-per-test; 4 canonical reports PASS |
...
| code-opencode alignment | PASS | COMPONENT header + section dividers on all 3 scripts; `py_compile` + 39 tests still green |
