...
_memory:
  continuity:
    ...
    open_questions: []
    answered_questions:
      - "The root playbook has five underscore-bearing category directories and 21 underscore-bearing Markdown files, representing 20 scenarios plus the index."
...
| Field | Value |
|-------|-------|
| **Packet** | sk-doc/017-hyphen-naming-convention/008-component-migration/007-system-deep-loop/008-manual-testing-playbook |
| **Level** | 2 |
| **Priority** | P1 |
| **Status** | Planned |
| **Created** | 2026-07-14 |
...
- The 21 underscore-bearing Markdown files in that tree, including `manual_testing_playbook.md`, `command_bridge_guard.md`, `ai_system_improvement.md`, `mode_hint_override.md`, `runtime_loop_council.md`, and `externalized_state.md`.
...
|-----------|-------------|-------------|
| `.opencode/skills/system-deep-loop/manual_testing_playbook/` | Rename/reference update | Rename the root, five categories, and 20 scenario files plus the index. |
...
|----|-------------|---------------------|
| REQ-001 | Every root playbook name is classified once | The map covers the root, five categories, index, and 20 scenario files with no unknown or duplicate target. |
| REQ-002 | Scenario coverage and identity are preserved | All 20 scenario files remain discoverable under the same category and scenario IDs after renaming. |
...
- **SC-002**: The benchmark sees the same 20 scenario IDs and category structure through the new kebab-case paths.
