---
title: "Feature Specification: verify code agent naming (017 phase 002)"
description: "The agents surface needs an explicit candidate audit for code across the three runtime agent directories. This verify-only phase records the actual definition paths and proves whether any in-scope snake_case filesystem name requires a rename."
trigger_phrases:
  - "code agent naming"
  - "agents phase 002"
  - "code filename verification"
importance_tier: "important"
contextType: "planning"
parent: "sk-doc/017-hyphen-naming-convention/008-component-migration/014-agents"
_memory:
  continuity:
    packet_pointer: "sk-doc/017-hyphen-naming-convention/008-component-migration/014-agents/002-code-verify"
...
# Feature Specification: Code Agent Naming Verification

> Phase adjacency under the 017 agents component parent (grouping order, not a runtime dependency): predecessor 001-ai-council-verify; successor 003-context-verify.
...
| Field | Value |
|-------|-------|
| **Packet** | sk-doc/017-hyphen-naming-convention/008-component-migration/014-agents/002-code-verify |
| **Level** | 2 |
| **Priority** | P1 |
| **Status** | Planned |
| **Created** | 2026-07-14 |
| **Owner skill** | sk-doc |
| **Origin** | Phase 002 of the 017 agents component migration |
<!-- /ANCHOR:metadata -->

<!-- ANCHOR:problem -->
## 2. PROBLEM & PURPOSE

The agents surface must be checked before any migration batch treats this definition as a rename candidate. The current inventory contains exactly three runtime definitions for code: .opencode/agents/code.md, .claude/agents/code.md, and .codex/agents/code.toml. Each basename is already kebab-case, so this phase must record an explicit zero-candidate result rather than silently disappearing from the migration evidence.

The purpose is to produce a reproducible, read-only candidate-set record for code, grounded in the filesystem naming policy and suitable for the agents rollup gate.
...
### In Scope
- Inventory the three code definition paths across the OpenCode, Claude, and Codex runtime agent directories.
...
|-----------|-------------|-------------|
| .opencode/agents/code.md | Inspect | Definition filename is already kebab-case; no rename expected |
| .claude/agents/code.md | Inspect | Definition filename is already kebab-case; no rename expected |
| .codex/agents/code.toml | Inspect | Codex definition filename is already kebab-case; no rename expected |
<!-- /ANCHOR:scope -->
...
- **SC-001**: All three code definition paths are recorded against the pinned baseline.
