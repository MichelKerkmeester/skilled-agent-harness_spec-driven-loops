---
title: "Feature Specification: verify design agent naming (017 phase 009)"
description: "The agents surface needs an explicit candidate audit for design across the three runtime agent directories. This verify-only phase records the actual definition paths and proves whether any in-scope snake_case filesystem name requires a rename."
trigger_phrases:
  - "design agent naming"
  - "agents phase 009"
  - "design filename verification"
importance_tier: "important"
contextType: "planning"
parent: "sk-doc/017-hyphen-naming-convention/008-component-migration/014-agents"
_memory:
  continuity:
    packet_pointer: "sk-doc/017-hyphen-naming-convention/008-component-migration/014-agents/009-design-verify"
...
# Feature Specification: Design Agent Naming Verification

> Phase adjacency under the 017 agents component parent (grouping order, not a runtime dependency): predecessor 008-deep-review-verify; successor 010-markdown-verify.
...
| Field | Value |
|-------|-------|
| **Packet** | sk-doc/017-hyphen-naming-convention/008-component-migration/014-agents/009-design-verify |
| **Level** | 2 |
| **Priority** | P1 |
| **Status** | Planned |
| **Created** | 2026-07-14 |
| **Owner skill** | sk-doc |
| **Origin** | Phase 009 of the 017 agents component migration |
<!-- /ANCHOR:metadata -->

<!-- ANCHOR:problem -->
## 2. PROBLEM & PURPOSE

The agents surface must be checked before any migration batch treats this definition as a rename candidate. The current inventory contains exactly three runtime definitions for design: .opencode/agents/design.md, .claude/agents/design.md, and .codex/agents/design.toml. Each basename is already kebab-case, so this phase must record an explicit zero-candidate result rather than silently disappearing from the migration evidence.

The purpose is to produce a reproducible, read-only candidate-set record for design, grounded in the filesystem naming policy and suitable for the agents rollup gate.
...
### In Scope
- Inventory the three design definition paths across the OpenCode, Claude, and Codex runtime agent directories.
...
|-----------|-------------|-------------|
| .opencode/agents/design.md | Inspect | Definition filename is already kebab-case; no rename expected |
| .claude/agents/design.md | Inspect | Definition filename is already kebab-case; no rename expected |
| .codex/agents/design.toml | Inspect | Codex definition filename is already kebab-case; no rename expected |
<!-- /ANCHOR:scope -->
...
- **SC-001**: All three design definition paths are recorded against the pinned baseline.
