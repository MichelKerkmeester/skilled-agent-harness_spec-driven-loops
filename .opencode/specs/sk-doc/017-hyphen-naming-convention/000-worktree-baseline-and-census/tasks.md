...
- [ ] T003 Create the isolated migration worktree off BASE by allocating an owner-first branch via `.opencode/skills/sk-git/scripts/worktree-naming.sh create sk-doc 017-hyphen-naming` (never hand-number the counter) — branch `sk-doc/{NNNN}-017-hyphen-naming`, dir `.worktrees/{NNNN}-sk-doc-017-hyphen-naming`, with isolated `SPEC_KIT_DB_DIR` / `SPECKIT_CODE_GRAPH_DB_DIR` / `SPECKIT_IPC_SOCKET_DIR`
