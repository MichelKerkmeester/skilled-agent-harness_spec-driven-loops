...
must reference the current `sk-doc/015-sk-doc-parent` packet path so the tooling never accidentally rewrites its
...
- **Deny-list**: `z_archive/`, files named `CHANGELOG*`/`changelog*`, spec-folder history/`implementation-summary.md`
  narrative, this packet's own `research.md`/`decision-record.md`, and the current `sk-doc/015-sk-doc-parent`
...
- **REQ-002:** Enumeration is **scoped to exactly the 111 in-scope files** across the 9 named packet paths — not a
  ...
  deny-list references the **current `sk-doc/015-sk-doc-parent`** packet path, not the stale, nonexistent
...
1. A dry-run against the current tree reports exactly **111 renames** across the 9 named packets, **88 `stage:`
   ...
   the corrected `015-sk-doc-parent` path).
...
- *Risk:* repeating 108's stale self-exclusion bug (a deny-list entry pointing at a path — `999-sk-doc-parent` —
  that no longer exists, since this hub was renumbered to `015-sk-doc-parent`) → mitigated by R2 hardcoding the
