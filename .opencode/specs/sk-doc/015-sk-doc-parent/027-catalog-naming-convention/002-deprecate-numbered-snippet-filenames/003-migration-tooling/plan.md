...
<!-- ANCHOR:summary -->
## 1. SUMMARY
Build one deterministic rename engine, adapted from `013-catalog-playbook-snippet-denumbering`'s proven
...
reference the current `sk-doc/015-sk-doc-parent` packet. No tree mutation here — Phase 004 runs it with `--apply`.
<!-- /ANCHOR:summary -->
...
references `sk-doc/015-sk-doc-parent` (not the stale `999-sk-doc-parent` path 108 carried); `stage:` injection is
...
adapted from the archived `013-catalog-playbook-snippet-denumbering/002-migration-tooling-and-dry-run/tooling/denumber-snippets.cjs`.
...
2. **Deny-list filter** — a predicate that excludes any candidate under `z_archive/`, named `CHANGELOG*` /
   ...
   `decision-record.md`, and (correcting 108's bug) the **current** `sk-doc/015-sk-doc-parent` packet path — not
...
### Phase 2: Implement the collision check and the corrected deny-list predicate (`015-sk-doc-parent`, not the stale
...
108 bug); feed the real `sk-doc/015-sk-doc-parent` path → must match. Inspect the `stage:` injection preview for
