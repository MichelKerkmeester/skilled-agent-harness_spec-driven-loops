...
<!-- ANCHOR:code-quality -->
## Code Quality
- [x] CHK-002 [P0] Enumeration is scoped to exactly the 9 named packet paths (not a generic tree walk); deny-list references the current `sk-doc/015-sk-doc-parent` path, not the stale 108 `999-sk-doc-parent` path; script comments keep durable WHY only (no spec/packet ids).
<!-- /ANCHOR:code-quality -->
...
- [x] CHK-005 [P0] Deny-list regression test passes: stale `999-sk-doc-parent` path excluded/not-matched, current `sk-doc/015-sk-doc-parent` path correctly matched.
...
<!-- ANCHOR:sign-off -->
## Sign-off
Migration tooling deterministic, dry-run-clean, deny-list-honoring (`015-sk-doc-parent` corrected), and
