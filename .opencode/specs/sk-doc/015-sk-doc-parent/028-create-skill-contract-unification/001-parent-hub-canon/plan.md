---
title: "Implementation Plan: Parent Hub Canon Conformance"
description: "Per-file dispatch harness: a fresh LUNA MAX update then a fresh Sonnet-5 xhigh verify then the validator gate, run >=5 in parallel across the 7 files in this batch."
trigger_phrases:
  - "001-parent-hub-canon plan"
...
# Implementation Plan: Parent Hub Canon Conformance
...
| **Validator** | `node .opencode/commands/doctor/scripts/parent-skill-check.cjs <hub-dir>  # 0 warnings` |
| **Workspace** | isolated git worktree at the origin tip; one SKILL.md (+ its resources) per work-item |

### Overview
Each of the 7 files is one independent work-item: LUNA MAX updates it to the parent-hub canon enforced by `parent-skill-check.cjs`, a fresh Sonnet-5
...
- [x] File inventory frozen (7 files) [EVIDENCE: spec.md SCOPE table]
...
3. **Verify (Sonnet-5 xhigh)** — a fresh Agent reads the diff, confirms conformance to the parent-hub canon enforced by `parent-skill-check.cjs`, and rejects any behavioral/scope change.
...
- [ ] Dispatch LUNA MAX updates in waves of >=5 across the 7 files
- [ ] As each update returns, launch its fresh Sonnet-5 xhigh verifier

### Phase 3: Verification
- [ ] Validator green on all 7 files
- [ ] Owning-hub regression check green
...
|-----------|-------|------|
| Per-file contract | each SKILL.md | `node .opencode/commands/doctor/scripts/parent-skill-check.cjs <hub-dir>` |
...
| Core (7 files, waves of >=5) | Medium | ~2 update+verify waves |
