...
- `.opencode/commands/doctor/scripts/parent-skill-check.cjs`: add a guard immediately after existing resource-existence checks that validates both declared path roles and rejects packet-prefixed `leafResourceId` values or unqualified `hubLoadAddress` values. Existing checks prove existence, not coordinate semantics. [SOURCE: .opencode/specs/sk-doc/019-sk-doc-router-alignment/010-sk-doc-routing-research/research/iterations/iteration-005.md:32]
...
- `.opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/d5-connectivity.cjs`: consume the same manifest for hub D5 instead of treating a root-only `references/`/`assets/` scan as nested coverage. [SOURCE: .opencode/specs/sk-doc/019-sk-doc-router-alignment/010-sk-doc-routing-research/research/iterations/iteration-005.md:40]
...
Gold cleanup must follow the namespace/manifest decision; doing it first would merely encode another guess. At the score join, persist exact normalized expected and observed arrays plus hashes of the fixture source, `hub-router.json`, `mode-registry.json`, `leaf-manifest.json`, and second-layer router. Report emission must refuse any scored row missing this block. The current runner aggregates and writes after scoring, while retained evidence can be only a truncated response head. [SOURCE: .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/run-skill-benchmark.cjs:210] [SOURCE: .opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/run-skill-benchmark.cjs:215] [SOURCE: .opencode/specs/sk-doc/019-sk-doc-router-alignment/010-sk-doc-routing-research/research/iterations/iteration-003.md:100]
