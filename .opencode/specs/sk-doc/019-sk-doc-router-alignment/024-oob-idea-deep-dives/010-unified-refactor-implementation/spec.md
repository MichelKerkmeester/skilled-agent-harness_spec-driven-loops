...
Implement the unified router refactor designed by the Opus-4.8 council in the sibling research packet (`../009-unified-refactor-research/unified-refactor-synthesis.md`). The refactor is **not a router rewrite** — it is a shadow compiler plus additive semantic gates, activated one skill at a time behind a fenced activation selector, with legacy serving-authoritative until each gate passes. Route-gold stays green at every step; the shared benchmark scorer is never edited.
...
## RELATED DOCUMENTS
- **Source design**: `../009-unified-refactor-research/unified-refactor-synthesis.md` + `../009-unified-refactor-research/ai-council/`
