...
Every GLM design claim is **single-lineage inference from one model, with no live router execution.** The two sourced facts (uniform weights; the pre-resolution precedent) are the durable, model-independent takeaways. Treat (T, R, P), the typed-handoff contract details, and the "defaultMode is a bug" reframe as **proposals to validate**, not settled conclusions. Full GLM narrative: `../023-oob-glm-parallel/research/lineages/glm-oob/research.md`.
