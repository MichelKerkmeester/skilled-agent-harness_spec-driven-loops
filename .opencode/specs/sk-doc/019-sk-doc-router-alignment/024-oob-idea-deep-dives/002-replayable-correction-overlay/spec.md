...
Seed evidence (do NOT re-derive): `../../021-default-mode-policy-research/research/lineages/sol-oob/iterations/iteration-002.md` and lineage `research.md` §7 (two-plane learning), §11(6). The key constraint: correction telemetry is deliberately prompt-free, so it can support aggregate calibration but not per-prompt gold reconstruction.
