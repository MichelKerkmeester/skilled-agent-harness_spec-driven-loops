...
function authoredHashes() {
  ...
  return Object.fromEntries(Object.entries(sources.bytes).map(([name, bytes]) => [name, sha256(bytes)]));
}
...
function scorerScenario(entry) {
  return {
    classKind: 'routing',
    expectedIntent: entry.gold.expectedIntents[0] || 'defer',
    ...
    scenarioId: entry.id,
    source: { featureFile: 'real-sk-design-canary', shape: 'sk-design' },
  };
}

function compileFromSources(sources, activationGeneration) {
...
function compileWithRouter(snapshot, mutate) {
  const sources = sourceBytes();
  const router = JSON.parse(sources.bytes['hub-router.json'].toString('utf8'));
  mutate(router);
  sources.bytes['hub-router.json'] = Buffer.from(`${JSON.stringify(router, null, 2)}\n`, 'utf8');
  return compileFromSources(sources, snapshot.policy.activationGeneration);
}
...
function assertCompiled(snapshot) {
  ...
  assert.strictEqual(fs.readFileSync(path.join(CHILD_ROOT, 'compiled', 'PolicyCardV1.md'), 'utf8'), card.markdown);
...
function runRouteCases(snapshot, fixture) {
  ...
  for (const entry of fixture.cases) {
    ...
    const pairs = leafPairsForDecision(snapshot, decision, entry);
...
function runDocumentParity(snapshot, fixture) {
  ...
  const cases = [
    ...fixture.cases,
    { id: 'qualified-explicit', prompt: 'unrelated', explicitMode: 'sk-design/audit' },
  ];
  ...
  return { cases: rows.length, terminal: 'DOCUMENT_ONLY_UNATTESTED' };
}
...
function runAuthoredFalsifiers(snapshot) {
  assert.throws(() => compileWithRouter(snapshot, (router) => {
    router.routerPolicy.defaultMode = 'interface';
  }), (error) => assertCode(error, 'DEFAULT_MODE_INVALID'));
  assert.throws(() => compileWithRouter(snapshot, (router) => {
  ...
  parsedRouter.routerPolicy.defaultMode = 'interface';
  assert.throws(() => compileRegistry({
    activationGeneration: snapshot.policy.activationGeneration,
    hubRouter: parsedRouter,
    packetRouterMarkdown: Object.fromEntries(Object.entries(sources.routerSourceIds).map(([mode, id]) => (
      [mode, sources.bytes[id].toString('utf8')]
    ))),
    registry: JSON.parse(sources.bytes['mode-registry.json'].toString('utf8')),
    routerSourceIds: sources.routerSourceIds,
    skillMarkdown: sources.bytes['SKILL.md'].toString('utf8'),
    sourceBytes: sources.bytes,
  ...
  packetRouterMarkdown[routerMode] = `${originalRouter}\n`;
  assert.throws(() => compileRegistry({
    activationGeneration: snapshot.policy.activationGeneration,
    hubRouter: JSON.parse(sources.bytes['hub-router.json'].toString('utf8')),
    packetRouterMarkdown,
    registry: JSON.parse(sources.bytes['mode-registry.json'].toString('utf8')),
    routerSourceIds: sources.routerSourceIds,
    skillMarkdown: sources.bytes['SKILL.md'].toString('utf8'),
    sourceBytes: sources.bytes,
...
function executionContext(snapshot, evaluated) {
  const registryHash = snapshot.sourceHashes.find((entry) => entry.sourceId === 'mode-registry.json').hash;
  return {
    ...
    preconditions: ['mutating-actor-only'],
...
function runAuthority(snapshot, fixture) {
  const mdCase = fixture.cases.find((entry) => entry.id === 'single-md-generator');
  const evaluated = evaluateCanary(snapshot, mdCase);
  const context = executionContext(snapshot, evaluated);
  ...
  assert.strictEqual(leg.target.mutatesWorkspace, true);
  ...
  for (const caseId of ['single-interface', 'single-open-design-transport']) {
    ...
    const localPlane = new DestinationExecutionPlane({ planningEpoch: localContext.epoch });
    const localLeg = localPlane.prepare(result.decision, localContext).preparedLegs[0];
    assertThrowsCode(
      () => commitActor(localPlane, localLeg, {}, adapter, options),
      ExecutionProtocolError,
      'ROLE_CANNOT_COMMIT',
    );
  }
  ...
  return {
    ...
    mutatingActorCommitAfterVerify: true,
    readOnlyActorCommitRejected: true,
    shadowOnly: true,
    transportCommitRejected: true,
  };
}
...
function validateCanary() {
  ...
  const falsifiers = runAuthoredFalsifiers(snapshot);
  ...
  return {
    ...
    archetype: {
      ...
      compiledTwoAxisPairs: snapshot.policy.compositionRules.length,
      defaultMode: snapshot.routingModel.defaultMode,
      modes: snapshot.routingModel.tieBreak,
      namedBundleRules: snapshot.routingModel.bundleRules.map((rule) => ({
        name: rule.name,
        targets: rule.targetWorkflowModes,
      })),
