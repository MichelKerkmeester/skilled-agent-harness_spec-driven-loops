...
description: "Risk-ordered remediation of the nine deep-review findings; all four tranches (tranche 1's two quick wins through tranche 4's trust boundary + cache lifecycle) are now built and verified. Round 2: a follow-up re-review's iteration 1 surfaced three further issues (RR-P1-A, RR-P1-B, RR-P2-C), closed by a lock/journal redesign."
trigger_phrases:
...
5. **Round 2 — re-review remediation (built).** A follow-up `/deep:review`'s iteration 1 surfaced three further issues in the tranches above, now closed: RR-P1-A (atomic `mkdir` lock winner election), RR-P1-B (write-ahead journal fence recovery), RR-P2-C (honest lock-comment residual). The loop then halted on an unrelated reducer/strategy-anchor workflow-infra bug — an external blocker, not a packet finding.

All four tranches now carry cited verification (`node --check`, the 9/9 harness, and targeted teeth tests) — see `## 5. TESTING STRATEGY`. Round 2 raises the harness to 10/10.
...
**Round 2 (re-review remediation).** A fresh 10-iteration `/deep:review` of the four tranches above completed iteration 1, then halted on an unrelated reducer/strategy-anchor workflow-infra bug — an external blocker in the review harness itself, not a finding against this packet. Iteration 1 surfaced three real issues, closed by a redesign:

- **RR-P1-A (lock winner election).** `shared/hub-lock.cjs`'s stale-lock reclaim was a write-then-read, not an atomic election — two stale-lock reclaimers could both enter the critical section. Fix: rebuilt on an atomic `mkdir` lock directory (an atomic exclusive create — a real winner election); owner identity (pid + nonce + lease) lives in `owner.json` inside the lock dir; stale reclaim re-checks staleness immediately before clearing the dead dir and re-races the `mkdir`.
- **RR-P1-B (write-ahead journal).** `flip-serving.cjs`'s `reconcileTuple` rebuilt the record but did not advance the fence, so it could certify a crash-interrupted flip at a stale epoch. Fix: replaced with a write-ahead journal — `.flip-journal.json` (intent + selectedPolicy + before/after fence) is written before the tuple mutates and cleared only after every write; `recoverFromJournal()` completes an interrupted flip deterministically to the journal's intended (advanced) fence.
- **RR-P2-C (honest lock documentation).** `hub-lock.cjs`'s comment overclaimed a PID-reuse/start-stamp check the code did not implement. Fix: corrected to describe the real mechanism (mkdir lock + lease + `kill(pid,0)`) and to honestly document the residual — perfectly race-free stale reclaim needs OS advisory locking (flock/fcntl), unavailable in zero-dep Node; PID reuse within an unexpired lease is bounded by the 10-minute lease, not detected.
