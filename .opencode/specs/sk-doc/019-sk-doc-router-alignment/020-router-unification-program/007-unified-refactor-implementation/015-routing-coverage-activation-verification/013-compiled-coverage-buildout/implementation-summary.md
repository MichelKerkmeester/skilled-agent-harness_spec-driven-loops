...
1. **The advisor-side compiled-routing enrichment cohort is still empty.** `.opencode/skills/system-skill-advisor/mcp-server/lib/compiled-routing-flag.ts` defines its own separate `DEFAULT_ON_HUBS: ReadonlySet<string> = new Set<string>()` (confirmed empty live during this reconciliation pass) — a third copy of the concept, used only by the advisor recommend path's enrichment, not the main routing resolver. The main runtime resolver's cohort (the one that actually serves routing decisions) IS fully populated; this is a follow-up for advisor-side enrichment parity, not a routing-correctness gap.
2. **No dedicated vitest lock-in test yet for the SD-015 non-route/surface-layer exemption clause.** The clause is live in production and covered by the 47/47 existing scenario-level assertions plus the full suite passing, but (confirmed by direct grep during this reconciliation pass) `compiled-routing-parity.vitest.ts` has no test whose name or content specifically targets this clause — a regression here would currently be caught only indirectly.
...
- [ ] Wire the advisor-side `compiled-routing-flag.ts` `DEFAULT_ON_HUBS` cohort to mirror the main runtime resolver's, so advisor recommend enrichment also reflects the flip.
- [ ] Add a dedicated vitest lock-in test for the SD-015 non-route/surface-layer gold-exemption clause in `compiled-routing-parity.vitest.ts`.
