...
> **The fallback-resource rule — what `null` should point at.** When a hub can't pick a mode, the fallback must load the **routing helper**: the hub's `shared/references/smart_routing.md` (the intent→mode map) plus `mode-registry.json` (the mode menu). That points the request at *how to choose a mode* — never at a guessed child, never at neutral filler. The parent SKILL.md always stays in control (it *is* the router); the only real choice is which resource the fallback loads, and it should be the thing that helps route. So every flip is a two-field change: `defaultMode: null` **and** `defaultResource: ["shared/references/smart_routing.md", "mode-registry.json"]`.
>
> This corrects three current wrongs at once: the biased child pointers (sk-prompt, cli-external, mcp-tooling all load a *child* SKILL.md on fallback today), the vestigial filler (system-deep-loop → hub `README.md`, sk-design → four `shared/` vocab docs), and the two existing `null` hubs that also miss it (sk-doc → `quick_reference.md`, sk-code → `shared/README.md`). None of them load the mode-map, even though every hub ships one. Even `sk-prompt` (which keeps its default lean) should load the mode-map alongside it.
...
1. **Canonize a third archetype: "defer-routed keyword hub"** (`defaultMode: null`, `hub-identity` as discovery-only, and `defaultResource` pointing at the **routing helper** — `shared/references/smart_routing.md` + `mode-registry.json`, the mode-map — so the fallback helps pick the mode rather than suggesting a child). Today the canon blesses only *keyword-routed* (non-null default) and *detection-routed* (sk-code). The four flips create keyword hubs with null defaults, which the schema doesn't yet describe — without this third archetype the freshly hardened template will quietly pressure future authors back toward named defaults.
