...
- [ ] Implement the corrected deny-list predicate (`z_archive/`, `CHANGELOG*`/`changelog*`, history, this packet's evidence, and the current `sk-doc/014-sk-doc-parent` path — fixing the 108 stale `999-sk-doc-parent` self-exclusion bug).
...
- [ ] Deny-list regression test: stale `999-sk-doc-parent` path does not match, current `sk-doc/014-sk-doc-parent` path does; excluded-surface summary empty; re-run byte-identical (idempotent); `validate.sh --strict` Errors 0.
<!-- /ANCHOR:phase-3 -->
