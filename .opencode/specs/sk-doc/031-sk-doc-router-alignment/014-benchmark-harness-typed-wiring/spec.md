...
description: "Make the typed routing contract flow end-to-end and be measurable against blind scenarios. Fix the two-classifier decoupling that makes the dispatcher emit zero leaves, wire the skill-benchmark loader/runner/live/scorer to typed gold, correct the false selected-map join, apply SD-015 option C, relocate the contract library out of the sk-doc packet, then build a sealed independently-authored holdout corpus behind offline and live gates before any propagation to other skills."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Authored 014 scaffold from the verified SOL-redirect ledger"
    next_safe_action: "Start Phase 1 (couple leaf selection to the capped hub modes in router-replay/executor-dispatch)"
    blockers: []
    key_files:
      - "spec.md"
      - "plan.md"
    ...
    open_questions: []
    answered_questions:
      - "Selection-fix mechanism ratified: stage it. Couple leaf selection to the capped hub modes now (Option 1); design toward a single unified routing contract (Option 3) as the durable follow-up once the holdout corpus can prove it"
...
**Key Decisions**: The selection fix is staged — couple leaf selection to the capped hub modes now, design toward a single unified routing contract later. SD-015 adopts option C. The contract library moves out of the sk-doc packet. A sealed, independently-authored holdout corpus is the real evidence, not the fitted fixtures.

**Critical Dependencies**: The selection-architecture fix (Phase 1) unblocks every measurement — until the dispatcher stops emitting zero, no downstream metric is meaningful.
...
1. **Two decoupled classifiers (root cause).** Router-replay selects hub modes from `hub-router.json` AND independently re-scores the prompt through `smart_routing.md` for leaf pairs. The surface leaf classifier is not constrained by the hub-selected modes. The hub over-selects generic authoring modes, the `MAX_WORKFLOW_MODES=2` cap keeps those, and `buildTypedResourceContract` filters the correct leaf pairs to **zero** (confirmed on SD-003/015/016; core out-of-fixture prompts route to 0 leaves).
...
- **Selection-architecture fix (staged).** Couple the surface leaf classifier's output to the hub-selected capped modes so the dispatcher stops emitting zero, implemented in `router-replay.cjs` and `executor-dispatch.cjs`. The change is explicitly designed as step one toward a single unified routing contract (see decision-record ADR-001), not as the terminal design.
...
- **The unified single-contract rewrite (Option 3) itself.** This packet builds *toward* it (ADR-001) but does not land it; that is a scoped follow-up once the holdout corpus can prove it.
...
|-----------|-------------|-------------|
| `.opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/router-replay.cjs` | Modify | Couple leaf selection to the capped hub modes; update relocated contract-lib import |
| `.opencode/skills/system-deep-loop/deep-improvement/scripts/skill-benchmark/executor-dispatch.cjs` | Modify | Constrain typed-pair contract to hub-selected modes; keep the 2-mode cap meaningful |
...
|----|-------------|---------------------|
| REQ-001 | Couple leaf selection to the capped hub modes so the dispatcher stops emitting zero on core scenarios | For SD-003/015/016 and a set of core out-of-fixture prompts, `buildTypedResourceContract` returns a non-empty, mode-consistent pair set; a regression test pins the previously-zero cases to non-zero |
...
|------|------------|--------|------------|
| Coupling leaf selection to a wrong hub-mode pick makes leaves inherit the hub's error (single point of failure) | Medium | Medium | Staged design: the coupling is a measurable interim step; the holdout corpus surfaces hub mis-selection, and ADR-001 commits to the unified-contract follow-up that removes the two-classifier disagreement class |
...
- **NFR-R02**: Leaf selection is deterministic given a prompt and a frozen router: the coupling to hub modes introduces no run-to-run variance in the offline path.
...
- A prompt that legitimately spans two modes: the hub may select two modes and the leaf set is the union across both, still bounded by the cap.

### Error Scenarios
- The hub selects the wrong modes: the coupling makes the leaves follow, producing a wrong-but-mode-consistent set. This is a measurable failure the holdout surfaces, not a silent zero.
...
|-----------|-------|----------|
| Scope | 20/25 | Files: the six-script benchmark harness, the topology validator, the relocated contract library, the SD-015 fixture, and a new sealed corpus with its tooling. Systems: hub router, leaf classifier, benchmark scorer, create-skill contract |
| Risk | 16/25 | No auth or public API surface, but the selection coupling changes runtime dispatch and the relocation moves guard-loaded code; a bug breaks every benchmark scenario at once |
| Research | 6/20 | Root cause and fix direction are settled by the verified ledger; remaining work is implementation plus corpus authoring, not investigation |
| Multi-Agent | 6/15 | Single implementer per phase, but the holdout corpus needs an independent author and two reviewers |
| Coordination | 12/15 | Seven dependency-ordered phases, an independent-authoring gate, and a concurrently-active shared harness |
...
|---------|-------------|--------|------------|------------|
| R-001 | The leaf→hub coupling makes leaves inherit a wrong hub-mode pick | M | M | Staged design; the holdout corpus surfaces hub mis-selection; ADR-001 commits to the unified-contract successor |
...
**Acceptance Criteria**:
1. Given SD-003, SD-015 or SD-016, or a core out-of-fixture prompt, When `buildTypedResourceContract` runs, Then it returns a non-empty pair set whose every `workflowMode` is in the hub selection.
...
- The exact shared location for the relocated contract library (system-level lib vs shared skill-support tree) is an implementation choice deferred to Phase 4 after the consumer inventory.
- Who authors the independent holdout corpus (a fresh agent blind to the router vs an operator-designated author) is an operator-policy decision that gates Phase 5.
