---
title: "Feature Specification: verify debug agent naming (017 phase 004)"
...
_memory:
  continuity:
    packet_pointer: "sk-doc/019-hyphen-naming-convention/008-component-migration/014-agents/004-debug-verify"
    last_updated_at: "2026-07-14T00:00:00Z"
    last_updated_by: "codex"
    recent_action: "Authored debug phase docs"
...
# Feature Specification: Debug Agent Naming Verification

> Phase adjacency under the 017 agents component parent (grouping order, not a runtime dependency): predecessor 003-context-verify; successor 005-deep-alignment-verify.
...
| Field | Value |
|-------|-------|
| **Packet** | sk-doc/019-hyphen-naming-convention/008-component-migration/014-agents/004-debug-verify |
| **Level** | 2 |
| **Priority** | P1 |
| **Status** | Planned |
| **Created** | 2026-07-14 |
| **Owner skill** | sk-doc |
| **Origin** | Phase 004 of the 017 agents component migration |
<!-- /ANCHOR:metadata -->

<!-- ANCHOR:problem -->
## 2. PROBLEM & PURPOSE

The agents surface must be checked before any migration batch treats this definition as a rename candidate. The current inventory contains exactly three runtime definitions for debug: .opencode/agents/debug.md, .claude/agents/debug.md, and .codex/agents/debug.toml. Each basename is already kebab-case, so this phase must record an explicit zero-candidate result rather than silently disappearing from the migration evidence.

The purpose is to produce a reproducible, read-only candidate-set record for debug, grounded in the filesystem naming policy and suitable for the agents rollup gate.
...
### In Scope
- Inventory the three debug definition paths across the OpenCode, Claude, and Codex runtime agent directories.
...
|-----------|-------------|-------------|
| .opencode/agents/debug.md | Inspect | Definition filename is already kebab-case; no rename expected |
| .claude/agents/debug.md | Inspect | Definition filename is already kebab-case; no rename expected |
| .codex/agents/debug.toml | Inspect | Codex definition filename is already kebab-case; no rename expected |
<!-- /ANCHOR:scope -->
...
- **SC-001**: All three debug definition paths are recorded against the pinned baseline.
