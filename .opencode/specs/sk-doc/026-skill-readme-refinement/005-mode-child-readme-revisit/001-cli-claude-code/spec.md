...
# Feature Specification: Phase 1 cli-claude-code README rewrite
...
| **Successor** | `002-cli-codex` |
| **Handoff Criteria** | The cli-claude-code README is purpose-first on the refined template, HVR clean, versioned with a matching changelog entry, validated with zero issues, and this phase folder validates with zero errors |
<!-- /ANCHOR:metadata -->
...
### Problem Statement
The cli-claude-code README at `.opencode/skills/cli-external-orchestration/cli-claude-code/README.md` still carries the older tabular reference-card style. Its body is a nine-section card deck with an AT A GLANCE table, a HOW IT WORKS section, a TROUBLESHOOTING table and an FAQ. It predates the mcp-obsidian pilot, which set the standard for mode skill READMEs: narrative, purpose-first documents in the Human Voice Rules, validated by the sk-doc README validator. The frontmatter version field (1.1.0.20) also lags the changelog top (v1.4.0.0), so the rewrite is the moment to re-align the release record.

### Purpose
Rewrite the cli-claude-code README against the refined README template from phase 001, using the mcp-obsidian README as the pilot exemplar. The rewrite opens with a one-line pitch and a problem-first OVERVIEW, keeps every factual surface of the current document (dispatch lifecycle, self-invocation guard, agent delegation, auth pre-flight, memory handback, troubleshooting), bumps the version field and adds a changelog entry, then validates the result.

**End goal:** a purpose-first narrative README that passes `validate_document.py --type readme` with zero issues, passes the HVR grep, and reads as the stable reference for external runtimes that dispatch to the Claude Code CLI.
<!-- /ANCHOR:problem -->
...
|-----------|-------------|-------------|
| `.opencode/skills/cli-external-orchestration/cli-claude-code/README.md` | Rewrite | Purpose-first narrative README per the refined template, with a one-line pitch, a problem-first OVERVIEW and a bumped version field |
| `.opencode/skills/cli-external-orchestration/cli-claude-code/changelog/v<version>.md` | Add | Changelog entry for the README rewrite release, named per the skill changelog convention |
| `.opencode/specs/sk-doc/026-skill-readme-refinement/005-mode-child-readme-revisit/001-cli-claude-code/spec.md` | Create | Phase specification (this file) |
| `.opencode/specs/sk-doc/026-skill-readme-refinement/005-mode-child-readme-revisit/001-cli-claude-code/plan.md` | Create | Phase implementation plan |
| `.opencode/specs/sk-doc/026-skill-readme-refinement/005-mode-child-readme-revisit/001-cli-claude-code/tasks.md` | Create | Phase task list |
| `.opencode/specs/sk-doc/026-skill-readme-refinement/005-mode-child-readme-revisit/001-cli-claude-code/checklist.md` | Create | Phase verification checklist |
...
| REQ-002 | Baseline inventory | The current README is inventoried before the rewrite. The baseline record holds the version field value, the validator output and the link state of `.opencode/skills/cli-external-orchestration/cli-claude-code/README.md` |
...
| ID | Requirement | Acceptance Criteria |
|----|-------------|---------------------|
| REQ-007 | Facts preserved | A section-by-section diff against the old README shows no factual content lost. The dispatch lifecycle, self-invocation guard, agent roster, auth pre-flight and memory handback all survive |
...
- **SC-001**: The cli-claude-code README is a purpose-first narrative document with a one-line pitch and a problem-first OVERVIEW.
...
| Risk | Factual loss in a large rewrite | Dispatch facts disappear | REQ-007 gates a section-by-section diff |
