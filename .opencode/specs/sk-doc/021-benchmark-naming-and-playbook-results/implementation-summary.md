...
| **Commits** | `c7e89ec88a`, `cf027a535c`, `e16382d845`, `ff34e4c284` |
<!-- /ANCHOR:metadata -->
...
`<skill>/benchmark/reports/<dated-run>/` and writes six files: the machine record, the rendered report,
...
**An index that cannot drift.** The row is appended by the same code path that writes the report.
Re-running into the same folder refreshes the row rather than duplicating it.
...
| Lane C suite | 259 passed, 11 failed, the same 11 as the stashed baseline |
| New storage suite | 10 passed |
...
| End-to-end run | Six files in the dated path plus an appended index row |
