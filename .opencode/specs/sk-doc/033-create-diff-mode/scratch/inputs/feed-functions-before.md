...
1. **Rulesets** → Filters the candidate pool
2. **Weighted Scoring** → Ranks whatever survives the filter
...
A deal's final score is the sum of its weighted signals: multiply each signal's value by its weight, then add them together. The highest score ranks first.
...
*   Full for 7 days, then decays to 0 at 60 days
...
*   Inverted: full at 0 applications, 0 at 10
