...
3. **Compiled validation not run in the worktree.** `parent-skill-check.cjs` and `validate.sh --recursive` run on main post-merge; this phase's verification is deterministic grep + on-disk existence + JSON validity.
