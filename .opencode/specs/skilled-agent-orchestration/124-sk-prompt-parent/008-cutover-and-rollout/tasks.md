...
- [x] T001 Confirm prior construction phases have landed before cutover execution — Evidence: `bash .../validate.sh .../124-sk-prompt-parent --recursive --strict` PASSED 0/0 for phases 001-007 before this phase's execution began.
...
- [x] T005 Run `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh .opencode/specs/skilled-agent-orchestration/124-sk-prompt-parent --recursive --strict` — Evidence: all 9 folders (parent + 8 phases) report `Errors: 0  Warnings: 0`, `RESULT: PASSED`.
...
- [x] T008 Roll up `.opencode/specs/skilled-agent-orchestration/124-sk-prompt-parent/description.json` after all gates pass — Evidence: `generate-description.js` re-run against the updated parent `spec.md` (Status: Complete, phase table all Complete).
- [x] T009 Roll up `.opencode/specs/skilled-agent-orchestration/124-sk-prompt-parent/graph-metadata.json` with complete status and `last_active_child_id` set to `008` after all gates pass — Evidence: `derived.status: "complete"`, `derived.last_active_child_id: "skilled-agent-orchestration/124-sk-prompt-parent/008-cutover-and-rollout"` (phase parents have no implementation-summary.md, so `backfill-graph-metadata.js` preserves rather than derives these two fields — set directly per the doctrine's phase-parent lean-trio rollup path).
<!-- /ANCHOR:phase-2 -->
...
- [x] T011 Re-run recursive strict spec validation after parent metadata rollup — Evidence: `bash .../validate.sh .../124-sk-prompt-parent --recursive --strict` — `Errors: 0  Warnings: 0` across all 9 folders after the T008/T009 rollup.
