...
_memory:
  continuity:
    ...
    recent_action: "ADR-001 accepted; no 002 source build"
    next_safe_action: "Carry NFR-S01 into 003's hub union-grant documentation and remaining deep-loop validation"
    blockers: []
    key_files:
    ...
    completion_pct: 70
    open_questions:
      - "NFR-S01 is unresolved in 002 and carried to 003's union-grant decision."
    answered_questions:
      - "ADR-001 Accepted Option E: Skill(parent) loads the hub, and the hub routes to the nested mode packet."
      - "No runtime enhancement or 002 source build is required; options A/B remain fallback complementary surfaces."
---
...
**Critical Dependencies**: The accepted mechanism depends on the top-level parent hub remaining invocable and on the hub router loading nested packets from `mode-registry.json`. NFR-S01 is not proven in this packet; phase 003 documents the hub union-grant pattern.
...
|-------|-------|
| **Parent Spec** | `../spec.md` |
| **Predecessor** | `../001-native-invocability-planning/spec.md` |
| **Successor** | `../003-deep-loop-alignment/spec.md` |
| **Level** | 3 |
| **Priority** | P1 |
| **Status** | Decision complete; NFR-S01 carried to 003 |
| **Created** | 2026-06-26 |
...
- Carrying NFR-S01 forward to phase 003 because this packet does not prove per-mode tool-permission narrowing.

### Out of Scope
- Any implementation of skills, agents, commands, runtime code, or configuration in this packet. This packet is decision-only; no source build occurs in 002.
...
|-----------|-------------|-------------|
| `.opencode/specs/skilled-agent-orchestration/119-parent-skill-native-invocability/002-invocability-mechanism/spec.md` | Update | This specification |
| `.opencode/specs/skilled-agent-orchestration/119-parent-skill-native-invocability/002-invocability-mechanism/plan.md` | Update | Decision-aligned plan/status |
| `.opencode/specs/skilled-agent-orchestration/119-parent-skill-native-invocability/002-invocability-mechanism/tasks.md` | Update | Decision and carry-forward task breakdown |
| `.opencode/specs/skilled-agent-orchestration/119-parent-skill-native-invocability/002-invocability-mechanism/decision-record.md` | Update | ADR-001 Accepted mechanism decision |
<!-- /ANCHOR:scope -->
...
| REQ-003 | Phase 002 is scoped as decision-only, with no source build in this packet | plan.md and implementation-summary.md state that the accepted mechanism is documented here and source alignment belongs to downstream packets |
...
| REQ-006 | NFR-S01 is not falsely marked satisfied in 002 | spec.md, decision-record.md, checklist.md, and implementation-summary.md carry NFR-S01 to 003's hub union-grant decision |
<!-- /ANCHOR:requirements -->
...
### Security
- **NFR-S01**: UNRESOLVED in 002 and carried to 002. Option E preserves the one-hub identity, but the hub's `allowed-tools` are the union the modes need; this packet does not prove runtime narrowing to each nested packet's own declaration.
...
|-----------|-------|----------|
| Scope | 12/25 | Decision-only in 002; downstream alignment touches hub, command, and agent surfaces |
...
- NFR-S01 is unresolved in 002: Option E uses the parent hub's union `allowed-tools`. Phase 003 owns the explicit union-grant decision and any follow-up validation.
<!-- /ANCHOR:questions -->
