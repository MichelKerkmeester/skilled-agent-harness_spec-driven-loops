...
description: "Records the accepted parent-skill native invocability mechanism. ADR-001 chooses Option E, invokable-hub routing, after comparing options A through E. This packet has no source build; NFR-S01 is resolved in phase 003 as union-grant accepted."
trigger_phrases:
...
_memory:
  continuity:
    ...
    next_safe_action: "Use 003's accepted hub union-grant decision for NFR-S01"
    blockers: []
    key_files:
    ...
    answered_questions:
      ...
      - "NFR-S01 is resolved in 003: union-grant accepted."
---
...
- Legacy public surfaces can differ from packet folder/name identity; phase 003 resolves the deep-loop packet folder while preserving the command and agent surfaces.
<!-- /ANCHOR:adr-001-context -->
...
| NFR-S01 permission language overclaims per-mode narrowing | M | Resolved in 003: union-grant accepted and documented as the hub permission contract |
<!-- /ANCHOR:adr-001-consequences -->
...
| 3 | **Sufficient?** | PASS | The decision record is sufficient for phase 002; downstream packets own implementation and validation |
...
**How to roll back**: Revert the phase 002 authored markdown docs. No source, runtime, or configuration changed in 002, so nothing else reverts.
<!-- /ANCHOR:adr-001-impl -->
<!-- /ANCHOR:adr-001 -->
