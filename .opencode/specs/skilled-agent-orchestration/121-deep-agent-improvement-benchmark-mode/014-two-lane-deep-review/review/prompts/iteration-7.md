DEEP-REVIEW iteration 7 of 10. Dimension focus this iteration: traceability.
...
READ each of these files (you have read-only repo access from the repo root) and review ONLY for traceability issues in THIS iteration:
...
Prior findings already reported across earlier iterations (do NOT repeat these; find NEW traceability issues or confirm none):
...
OUTPUT CONTRACT: Output ONLY a single JSON array, no prose and no markdown fences. Each element: {"id":"traceability-7-N","severity":"P0|P1|P2","dimension":"traceability","title":"short","file":"path","line":123,"evidence":"what you actually saw in the file","fix":"concrete fix"}. If you find no NEW traceability issues this iteration, output exactly [].
