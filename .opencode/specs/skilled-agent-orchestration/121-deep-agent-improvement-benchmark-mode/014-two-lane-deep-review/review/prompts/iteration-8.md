DEEP-REVIEW iteration 8 of 10. Dimension focus this iteration: maintainability.
...
READ each of these files (you have read-only repo access from the repo root) and review ONLY for maintainability issues in THIS iteration:
...
Prior findings already reported across earlier iterations (do NOT repeat these; find NEW maintainability issues or confirm none):
...
OUTPUT CONTRACT: Output ONLY a single JSON array, no prose and no markdown fences. Each element: {"id":"maintainability-8-N","severity":"P0|P1|P2","dimension":"maintainability","title":"short","file":"path","line":123,"evidence":"what you actually saw in the file","fix":"concrete fix"}. If you find no NEW maintainability issues this iteration, output exactly [].
