...
**Packet**: `.opencode/specs/skilled-agent-orchestration/071-sk-doc-router-stress-test/`
...
### For sk-doc router (no changes needed in this packet)
- The router LOGIC (deterministic Python pseudocode) appears sound — codex's 67% intent accuracy + 66.7% resource accuracy suggests router output is interpretable when the model is gpt-5.5/high.
- copilot's 40% intent accuracy is the model interpreting things differently, NOT the router being wrong. No router fix needed.

### For cli-* skill maintenance
- **cli-codex SHOULD be preferred default executor for sk-doc dispatches** (when available). Document this in cli-codex's README + sk-doc's invocation hints.
- **cli-copilot's verbosity** when describing routing should be flagged; the `cli_prompt_quality_card.md` could note that copilot tends to over-list resources, so reflective-framing prompts should narrow further (e.g., "ONLY list the 3-5 resources that WOULD load; do NOT list resources from other intents").
- **cli-opencode timeout default of 120s** is too tight for some scenarios with deepseek-v4-pro variant=high. Consider raising to 180s in run-matrix.sh.

### For future router-stress packets
- Always start with reflective-framing scenarios (use 071's playbook as template).
- Refine resource detection: parse only POSITIVE mentions ("loaded", "selected", "fetched"), exclude NEGATIVE ("not loaded", "excluded").
- Cap dispatch timeout at 180s (not 120s) for opencode.
- Capture per-cell tokens in delta JSONL with a Python helper (not bash regex).

### Follow-up packet 072 candidate (if accepted)
- **Scope**: refine measurement script (P1-001 + P2-001/002 fixes), re-run matrix with cleaner extraction, publish updated matrix.csv. ~2 hr packet.
- **Trigger**: only if user wants tighter accuracy numbers; current data is directionally sound.
...
- **Synthesis script**: `003-synthesize/scripts/extract_metrics.py`
- **Matrix data**: `003-synthesize/matrix.csv` (45 rows × 19 columns)
- **This report**: `003-synthesize/review-report.md`

---

## 8. Verdict (final)

**REMEDIATE_AND_SHIP** — ship 071 as-is. The methodology bug (P0-001) is already fixed inline. P1 findings are measurement refinements, not router defects. No follow-up REQUIRED, but follow-up RECOMMENDED if user wants tighter copilot accuracy numbers.

The headline takeaway: **cli-codex (gpt-5.5/high/fast) is the clear best executor for sk-doc routing tasks** — 67% intent accuracy, 66.7% resource accuracy, 28s avg, 38k tokens, 100% reliability. Use it as the default whenever available; fall back to cli-opencode for cost-tolerant batch work; cli-copilot's verbosity makes it a poor fit for narrow routing-trace tasks even though it's the fastest.
