...
### Purpose
Harden the framework to publication-grade reliability per `../001-design-research/research/research.md` (§5 stats, §6 dispatch, §4 fixtures) + `../001-design-research/research/deltas/deltas.jsonl` P1 deltas — without breaking Lane B or the MVP.
<!-- /ANCHOR:problem -->
...
- Does the OpenCode `--format json` event stream actually expose token usage on the live binary? **RESOLVED: yes — a real-dispatch smoke captured 39,395 in / 63 out, cost 0; the research "usage unverified" caveat is closed.**
- Is the `repeatabilityTolerance` of 0.03 wide enough to absorb single-sample bootstrap jitter without masking real regressions? **RESOLVED: calibrated 0->0.03 with a calibration note; revisit if multi-sample runs show drift.**
- Can the capability table discriminate M3 vs MiMo on a real partial-credit run? **DEFERRED to the 004-capability-discrimination successor phase (hard fixtures + cwd-isolation + a real capability run).**
<!-- /ANCHOR:questions -->
