...
| **Framework** | Reuses 114/002 rig + 114/003 loop |
| **Storage** | `116/state/`, `116/iterations/`, `116/synthesis-v2.md` |
...
### Definition of Ready
- [ ] 114/002 rig present and dry-run green
- [ ] 114/003 loop runs in mock mode
...
### Pattern
Sidecar augmentation. Extraction script lives in 116 packet; score-variant.cjs (in 114/002) gains an env-gated extraction call. The re-run reuses 114/003 loop unchanged.

### Key Components
- `116/scripts/extract-files-from-markdown.cjs` — parse + write
- `114/002/scripts/score-variant.cjs` — env-gated extraction call
- `116/scripts/loop-v2.cjs` — wrapper that sets env + invokes 114/003 loop with output redirected to 116/state
...
|---------|--------------|--------|--------------|
| 114/002 score-variant.cjs | Scoring orchestrator | Add env-gated extraction call before det checks | Diff review + canned-output test |
| 114/002 fixtures/*/seed/ | Pristine seed scenarios | Read-only (extraction snapshots + restores) | Pre/post seed hash compare |
| 116/scripts/ | New | Create extract + loop-v2 | ls + node --check |
<!-- /ANCHOR:affected-surfaces -->
...
### Phase 1: Setup
- [ ] Verify 114/002 rig green
- [ ] Verify claude-sonnet auth
- [ ] Create 116 directory tree
...
- [ ] Modify 114/002 score-variant.cjs: env-gated extraction call + seed snapshot/restore
- [ ] Build 116/scripts/loop-v2.cjs wrapper
...
|------------|------|--------|-------------------|
| 114/002 rig | Internal | Green | Hard blocker |
| 114/003 loop runner | Internal | Green | Hard blocker |
...
- **Procedure**: `rm -rf 116/state 116/iterations 116/synthesis-v2.md`; restore fixture seeds via 114/003/scripts/seed-fixtures.cjs --force; revert score-variant.cjs changes via git
<!-- /ANCHOR:rollback -->
...
- [ ] 116/state clean (no prior run artifacts)
- [ ] Operator confirms grader cost ceiling (default $10)

### Rollback Procedure
1. Stop running loop (Ctrl+C or pause sentinel)
2. `rm -rf 116/state 116/iterations 116/synthesis-v2.md`
3. Restore seeds: `node 114/003/scripts/seed-fixtures.cjs --force`
4. Revert score-variant.cjs: `git checkout HEAD -- 114/002/scripts/score-variant.cjs`
...
|-----------|------------|----------|--------|
| Setup | 114/002 + 114/003 + auth | 116 dir tree | Build |
...
### ADR-001: Sidecar augmentation (NOT rewrite of 114/002)

**Status**: Proposed

**Context**: The extraction layer needs to run BEFORE deterministic checks in score-variant.cjs. Two options: (a) add the call in 114/002 (modifies that packet's surface), or (b) build a wrapper in 116 that reimplements scoring with extraction included.

**Decision**: Option (a) — minimal modification to 114/002 behind env flag. Preserves 114/003's mock-mode behavior (env unset) and enables extraction for the 116 re-run (env set).

**Consequences**:
- Improves: single source of truth for scoring; extraction available for future packets opting in
- Costs: 114/002 changes are now shared; tested via env-off path to confirm 003 behavior unchanged

**Alternatives Rejected**:
- Wrapper in 116 that reimplements score-variant: duplication; risk of drift from 002 logic
