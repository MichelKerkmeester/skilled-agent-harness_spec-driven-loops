 ...
 * scripts/loop-v2.cjs
 *
 * Thin wrapper that invokes the 114/003 loop runner with EVAL_LOOP_EXTRACT=true
 * and the live claude-sonnet grader. State + iteration files write to 116/state
 * and 116/iterations (NOT 114/003's state dir) so the v1 baseline is preserved.
 *
 * Strategy: change cwd to 116, run loop, then loop writes to its PACKET_ROOT/state.
 * loop.cjs derives PACKET_ROOT from __dirname so we cannot change it directly;
 * the right move is to copy the loop runner to 116 as a one-line forwarder...
 *
 * Simpler: copy the iteration outputs from 114/003/state to 116/state after each
 * iter completes. But that's racy.
 *
 * Cleanest: run the loop from 114/003 (state goes to 114/003/state), then
 * post-process: move the new artifacts to 116/state.
 *
 * Implementation: invoke 114/003 loop with env vars set; tail state for new rows;
 * mirror them to 116/state after loop completes. This keeps 003 loop unchanged.
 */
...
const LOOP_PACKET_ROOT = path.resolve(PACKET_ROOT, '..', '114-cli-devin-swe16-prompt-optimization', '003-eval-loop');
...
function archive003State(snapshot) {
  // After the run, copy 003 state files into 116/state (replacing any prior 116 state)
...
function clear003StateForFreshRun() {
  ...
  // (003's prior run artifacts already committed in 116-eval-loop)
  if (fs.existsSync(STATE_DIR_003)) {
    for (const f of fs.readdirSync(STATE_DIR_003)) {
      const abs = path.join(STATE_DIR_003, f);
...
function main() {
  ...
  process.stdout.write('loop-v2: archiving 003 state to 116/state (with -v2 suffix)\n');
  archive003State();
