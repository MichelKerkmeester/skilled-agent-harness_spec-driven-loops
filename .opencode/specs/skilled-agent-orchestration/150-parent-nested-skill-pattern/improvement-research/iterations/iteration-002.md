...
## Finding 3 — Mode discoverability is asymmetric: 4 of 8 modes have no advisor map entry; nothing asserts "unscored ⇒ has a command" · **P2/P3**

Routing classes (from `mode-registry.json`): `lexical` = research/review/ai-council (3, Python-scored + both maps); `alias-fold` = agent-improvement (1, TS map only, not lexically scored); `metadata` = context (1, no map); `command-bridge` = model/skill/ai-system (3, no map). So **5 of 8 are not lexically advisor-scored, and 4 of 8 have no advisor map entry at all.**
...
## Finding 4 — `deep-improvement` hosting 4 modes: considered a split, recommend **KEEP** (cohesive via real shared seams) · **P3**

I examined whether the 4-lane packet should split. It should not — the cohesion is structural, not cosmetic:
- All four share three pluggable seams (candidate-source, dispatcher, scorer) and a single entrypoint `loop-host.cjs --mode` (`deep-improvement/SKILL.md:300-302`); the agent-improvement path stays byte-identical with no flag (`:41`).
...
The one cohesion outlier is Lane D `ai-system-improvement` (the only `external-adapter`, `mode-registry.json:133`): its loop runs *outside* the packet (`<packaging-root>/benchmark/_loop/loop.py`), so it shares the philosophy/guardrails but not the runtime. Even so it reuses loop-host's mode dispatch. **No split.** Optional P3: keep an eye on the 564-line SKILL.md density (the lane-scoped smart-router already mitigates it). Near-optimal.
...
`backendKind == runtime-loop-type` ⟺ `runtimeLoopType != null` (perfect correlation across all 8 modes), so `backendKind` looks partly derivable. But the two fields serve **two different consumers**: `runtimeLoopType` is the literal `--loop-type` CLI arg validated independently by `convergence.cjs:310-311` (`research|review|council|context`), while `backendKind` drives the hub's dispatch switch and — critically — splits the `null`-loop case into `improvement-host` (×3) vs `external-adapter` (×1), which `runtimeLoopType` alone can't. Collapsing them would couple the runtime contract to the router. **Keep as-is — near-optimal.** (The `explicit null is load-bearing, never inferred` rule, SKILL.md:84, is the right call and worth preserving.)
...
[
  ...
  },
  {
    ...
    "rationale": "5 of 8 modes are not lexically advisor-scored and 4 of 8 have no advisor map entry; this is mostly correct by design (context stays metadata-routed, improvement lanes are command-gated) but nothing prevents a future mode from being both unscored AND command-less = undiscoverable",
  ...
  {
    "title": "Keep deep-improvement as one 4-lane packet (do NOT split); optionally sharpen Lane D external-adapter boundary docs",
    "rationale": "the four lanes share candidate/dispatcher/scorer seams plus loop-host/reduce-state/promote-candidate with mode-tagged records, so a split would fork shared scripts and raise coupling; Lane D is the only external-adapter outlier but still reuses loop-host dispatch",
