...
- Evidence:
  ...
  - `.opencode/specs/skilled-agent-orchestration/070-sk-deep-rename/002-skill-folder-rename/spec.md:74` says to rename `.opencode/skill/deep-review/` to `.opencode/skill/deep-review/`.
  - `.opencode/specs/skilled-agent-orchestration/070-sk-deep-rename/002-skill-folder-rename/plan.md:98` lists `.opencode/skill/deep-review/` as the old review skill root and then `git mv` to the same path.
  - `.opencode/specs/skilled-agent-orchestration/070-sk-deep-rename/002-skill-folder-rename/tasks.md:67` marks a task complete for renaming `.opencode/skill/deep-review/` to `.opencode/skill/deep-review/`.
...
- Concrete fix: restore source-side names in Phase 002 narrative and metadata where the text documents the rename:
  ...
  Then re-run a focused grep for `deep-review to deep-review`, `deep-research to deep-research`, and same-path `.opencode/skill/deep-*` rename rows under `002-skill-folder-rename/`.
...
- The requested description files are correct:
  ...
  - `.opencode/specs/skilled-agent-orchestration/070-sk-deep-rename/002-skill-folder-rename/description.json:3` says `git mv .opencode/skill/sk-deep-review -> deep-review and sk-deep-research -> deep-research`.
...
- Historical telemetry retains old entries. `.opencode/skill/.smart-router-telemetry/compliance.jsonl:6` and `.opencode/skill/.smart-router-telemetry/compliance.jsonl:7` still record `sk-deep-research` and `sk-deep-review`; `shadow-deltas.jsonl` files also retain historical prompt payloads with old names.
- Renamed skill changelog directories exist at `.opencode/skill/deep-review/changelog/` and `.opencode/skill/deep-research/changelog/`. The sampled historical changelogs contain current `deep-*` names and older deep-loop migration references; no illegitimate active old-name issue was found there in this pass.
...
- Listed and sampled renamed skill changelog directories under `.opencode/skill/deep-review/changelog/` and `.opencode/skill/deep-research/changelog/`.
...
- Include a final adversarial grep for identity-rename wording under the full Packet 070 tree:
  ...
  - `.opencode/skill/deep-review/` renamed to `.opencode/skill/deep-review/`
  - `.opencode/skill/deep-research/` renamed to `.opencode/skill/deep-research/`
