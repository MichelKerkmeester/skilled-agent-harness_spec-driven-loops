...
- Read local skill contracts and references for `sk-interface-design` and `mcp-magicpath`.
...
| `sk-interface-design/SKILL.md` and references | Current design-judgment skill surface |
...
## 6. sk-interface-design Current State

`sk-interface-design` is strong where Claude Design needs design judgment: it grounds the subject, creates a compact token system, critiques against AI-default looks, builds from the revised plan, and self-critiques against a quality floor. [SOURCE: file:.opencode/skills/sk-interface-design/SKILL.md:90]

Its key gaps are persistent context and observable iteration. It does not require a design-system snapshot before choosing tokens, does not persist a user-visible design revision ledger, and does not emit a standard handoff manifest. Its CSV inventory must remain critique-against data, not a generator. [SOURCE: file:.opencode/skills/sk-interface-design/references/design_inventory.md:67]
...
| Dimension | Claude Design Target | `sk-interface-design` | `mcp-magicpath` |
|---|---|---|---|
...
`sk-interface-design` needs more context and handoff structure. `mcp-magicpath` needs more design-process and feedback structure. The right answer is a shared protocol that lets each skill remain itself.

## 10. ADOPT / ADAPT / SKIP Recommendations

| Skill | Rank | Recommendation | Reason |
|---|---|---|---|
| `sk-interface-design` | ADOPT | Add a design-context intake snapshot | Mirrors Claude Design attachments while staying local and prompt/file based. |
| `sk-interface-design` | ADOPT | Emit a design handoff manifest | Makes tokens, critique, quality checks, and implementation notes durable. |
| `sk-interface-design` | ADAPT | Persist a lightweight revision ledger for visual feedback | Adds iteration visibility without becoming a canvas product. |
| `sk-interface-design` | SKIP | Auto-generate from design CSVs | Violates critique-against catalog rules. |
...
5. Update `mcp-magicpath` AUTHOR and REPO_IMPORT paths to explicitly call the `sk-interface-design` process before canvas code work, then store the design manifest next to canvas session notes.
...
| Convert `sk-interface-design` CSVs into a design generator | The catalog is explicitly for naming and deviating from defaults, not choosing final designs. | `design_inventory.md` | 2 |
...
- Should `mcp-magicpath` always invoke `sk-interface-design` in AUTHOR and REPO_IMPORT, or only when the brief has free visual axes?
- How much of the handoff manifest should be machine-readable JSON versus markdown?

## 13. Risks and Constraints

- Overfitting to Claude Design could dilute `sk-interface-design`'s anti-default stance.
- Overloading `mcp-magicpath` with design judgment would duplicate `sk-interface-design` instead of composing it.
...
2. Update `sk-interface-design` to produce and consume the context snapshot and handoff manifest.
...
- Whether they rank MagicPath theme preflight above `sk-interface-design` design-context intake.
- Whether they propose any export/handoff formats beyond manifest plus code/canvas link.
- Whether they preserve the no-generator rule for `sk-interface-design` data files.
...
- `file:.opencode/skills/sk-interface-design/SKILL.md`
- `file:.opencode/skills/sk-interface-design/references/design_principles.md`
- `file:.opencode/skills/sk-interface-design/references/ux_quality_reference.md`
- `file:.opencode/skills/sk-interface-design/references/design_inventory.md`
...
- Quality guard result: pass. Evidence includes three external Claude Design docs, five `sk-interface-design` files, five `mcp-magicpath` files, and parent spec requirements.
