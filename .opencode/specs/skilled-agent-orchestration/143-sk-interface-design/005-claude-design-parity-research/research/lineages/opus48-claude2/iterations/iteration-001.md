...
## Focus
Map what `sk-interface-design` and `mcp-magicpath` do today, lock the five Claude Design parity dimensions as the assessment axes (R1), and assign a first-cut current-state verdict per dimension per skill. This baseline frames iterations 2–4 (per-dimension deep gaps) and iteration 5 (scorecard + recommendation).

## Findings

### F1 — The parity target is a fixed, seeded capability set (not to be re-derived)
Claude Design's capabilities are seeded in the spec and are out-of-scope to re-derive: conversational generation of designs/prototypes/presentations; **org design-system inheritance**; **canvas + inline-comment iteration**; **context attachments**; **multi-format export**; **Claude Code handoff**; **named quality levers**. [SOURCE: file:.opencode/specs/skilled-agent-orchestration/143-sk-interface-design/005-claude-design-parity-research/spec.md:141] These collapse cleanly into the five R1 dimensions: (1) design-system inheritance, (2) iteration/visual feedback, (3) context grounding, (4) quality levers, (5) output/handoff. [SOURCE: file:.opencode/specs/skilled-agent-orchestration/143-sk-interface-design/005-claude-design-parity-research/spec.md:90]

### F2 — sk-interface-design is a *judgment* skill, not a *generation* skill
It produces a design **direction** (palette, type, layout, signature, copy) via a two-pass process: ground the subject → brainstorm a compact token system → critique against AI-default looks → build → self-critique. [SOURCE: file:.opencode/skills/sk-interface-design/SKILL.md:92] Its differentiator is the **anti-default philosophy**: reject the three clustered AI looks (cream+serif+terracotta; near-black+one acid accent; broadsheet hairlines) on any free axis. [SOURCE: file:.opencode/skills/sk-interface-design/references/design_principles.md:59] It owns the *look* and explicitly hands implementation to `sk-code`; it does not render or build. [SOURCE: file:.opencode/skills/sk-interface-design/SKILL.md:184]

### F3 — sk-interface-design already has token-system *thinking* but no token *artifact*
Step 2 brainstorms a token system as "4–6 named hex values" + display/body/utility faces + layout + signature — but as in-thinking planning material, not an emitted, machine-readable token file. [SOURCE: file:.opencode/skills/sk-interface-design/references/design_principles.md:61] There is no mechanism to ingest an existing design system, nor to emit one. This is the seed for the dimension-1 (inheritance) gap.

### F4 — sk-interface-design's "quality floor" is adjacent to "quality levers" but not user-selectable
It enforces an objective quality floor (responsive, visible keyboard focus, reduced motion) via `ux_quality_reference.md`, applied as a pass/fail gate after direction is set. [SOURCE: file:.opencode/skills/sk-interface-design/SKILL.md:115] This is a *floor* (binary, always-on), whereas Claude Design's "named quality levers" are *dials* the user turns. Adjacent capability, different shape — seeds dimension-4.

### F5 — sk-interface-design's iteration is self-critique-in-thinking, with an optional screenshot
The loop is "screenshot if possible, remove one accessory, confirm the quality floor" — a single self-review, not an external visual-feedback loop driven by rendered output and comments. [SOURCE: file:.opencode/skills/sk-interface-design/references/design_principles.md:73] Screenshotting is routed through `mcp-chrome-devtools`, and only post-build. [SOURCE: file:.opencode/skills/sk-interface-design/SKILL.md:188] Seeds dimension-2.
...
mcp-magicpath auto-applies sk-interface-design whenever it produces or shapes UI (canvas authoring, repo recreation, post-`add` adapt). [SOURCE: file:.opencode/skills/mcp-magicpath/SKILL.md:250] So parity improvements can be split cleanly: *judgment-side* gaps belong to sk-interface-design; *canvas/CLI-side* gaps belong to mcp-magicpath. This composition is the single most important structural fact for the recommendation.
...
| 4. Quality levers | Binary quality *floor*, not dials (F4) | None of its own; inherits floor via sk-interface-design (F9) |
| 5. Output / handoff | None (hands to sk-code) (F2) | React/TS install = code handoff; no multi-format export (F8) |

## Sources Consulted
- spec.md (topic, seeded Claude Design capabilities, R1 dimensions, known magicpath gaps) [SOURCE: file:.opencode/specs/skilled-agent-orchestration/143-sk-interface-design/005-claude-design-parity-research/spec.md:56]
- sk-interface-design/SKILL.md (process, quality floor, integration boundary) [SOURCE: file:.opencode/skills/sk-interface-design/SKILL.md:1]
- sk-interface-design/references/design_principles.md (anti-default calibration, token brainstorm, self-critique) [SOURCE: file:.opencode/skills/sk-interface-design/references/design_principles.md:1]
