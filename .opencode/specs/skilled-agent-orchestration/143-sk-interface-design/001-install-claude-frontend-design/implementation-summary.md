...
description: "Vendored Anthropic's frontend-design skill into the framework as sk-interface-design, with a lean house SKILL.md, a reference-template principles doc, README, schema-2 graph metadata, catalog + root README registration, and advisor routing."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Shipped sk-interface-design skill install and registration"
    ...
    open_questions: []
    answered_questions:
      - "Skill name: sk-interface-design"
...
| Field | Value |
|-------|-------|
| **Spec Folder** | 143-sk-interface-design/001-install-claude-frontend-design |
...
| `.opencode/skills/sk-interface-design/references/design_principles.md` | Created | Full upstream guidance, reference template, verbatim |
...
| `.opencode/skills/sk-interface-design/graph-metadata.json` | Created | Schema-2 advisor node (sk-code family) |
| `.opencode/skills/sk-code/graph-metadata.json` | Modified | Reciprocal sibling edge to sk-interface-design |
| `.opencode/skills/mcp-magicpath/SKILL.md` | Modified | Auto-apply sk-interface-design when authoring or shaping UI (RULES rule 6, Author-direction note, loading-levels row, router note, integration) |
| `.opencode/skills/mcp-magicpath/graph-metadata.json` | Modified | `depends_on` sk-interface-design (0.7) + related_to; reciprocal `prerequisite_for` added on sk-interface-design |
...
| Decision | Why |
|----------|-----|
| Named it `sk-interface-design` | The operator chose this over `frontend-design` or `sk-frontend-design`; it fits the sk-code family and pairs with sk-code |
...
| Wired mcp-magicpath to auto-apply sk-interface-design | The operator wanted the design skill used automatically whenever mcp-magicpath designs. The runtime trigger is in mcp-magicpath's SKILL.md (RULES rule 6 + Author-direction instruction: load and apply sk-interface-design before building UI). The advisor co-surfacing is a `depends_on` -> `prerequisite_for` edge pair (weight 0.7). Verified in the live graph: `depends_on(mcp-magicpath)` and `dependents(sk-interface-design)` both resolve the edge. |
<!-- /ANCHOR:decisions -->
