---
title: "Changelog: Claude Design parity keystone build [143-sk-interface-design/007-claude-design-parity-build]"
...
> Spec folder: `.opencode/specs/skilled-agent-orchestration/143-sk-interface-design/007-claude-design-parity-build` (Level 2)
> Parent packet: `.opencode/specs/skilled-agent-orchestration/143-sk-interface-design`

### Summary

The parity build landed the keystone without bloating either skill. One shared protocol wires `sk-interface-design` and `mcp-magicpath` into the loop identified by the research. It deliberately avoids a style chooser and keeps `design_principles.md` untouched.
...
- Authored `sk-interface-design/references/claude_design_parity.md` as the shared protocol.
- Hooked `sk-interface-design/SKILL.md` through Core References and the resource-loading row.
- Hooked `sk-interface-design/graph-metadata.json` through key files.
...
| Check | Result |
|-------|--------|
| `package_skill.py sk-interface-design` | PASS: valid across 41 files. |
| `package_skill.py mcp-magicpath` | PASS: valid across 14 files. |
| `SKILL.md` size | PASS: `sk-interface-design` 1564 words and `mcp-magicpath` 2282 words, both under cap. |
...
| `sk-interface-design/SKILL.md, graph-metadata.json` | Modified | References, resource-loading and key-file pointers. |
