...
`tool-schemas.ts` defines `ccc_status`, `ccc_reindex`, and `ccc_feedback` at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:735`, `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:741`, and `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:753`, then includes them in `TOOL_DEFINITIONS` at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:975`-`.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:978`. The memory reference also documents those tools as L7 maintenance calls at `.opencode/skill/system-spec-kit/references/memory/memory_system.md:139`-`.opencode/skill/system-spec-kit/references/memory/memory_system.md:141`.

The dispatch side does not include a CocoIndex dispatcher. `ALL_DISPATCHERS` only includes context, memory, causal, checkpoint, lifecycle, code graph, skill graph, advisor, and coverage graph modules at `.opencode/skill/system-spec-kit/mcp_server/tools/index.ts:91`-`.opencode/skill/system-spec-kit/mcp_server/tools/index.ts:100`; `lifecycle-tools.ts` handles L7-ish tools but its `TOOL_NAMES` stops at `session_bootstrap` and does not include any `ccc_*` names at `.opencode/skill/system-spec-kit/mcp_server/tools/lifecycle-tools.ts:39`-`.opencode/skill/system-spec-kit/mcp_server/tools/lifecycle-tools.ts:52`. Because `dispatchTool()` returns `null` for unrecognized names at `.opencode/skill/system-spec-kit/mcp_server/tools/index.ts:109`-`.opencode/skill/system-spec-kit/mcp_server/tools/index.ts:120`, `context-server.ts` then throws `Unknown tool: ${name}` at `.opencode/skill/system-spec-kit/mcp_server/context-server.ts:1022`-`.opencode/skill/system-spec-kit/mcp_server/context-server.ts:1028`.
...
The public `memory_save` descriptor says routed saves write continuity into canonical spec documents at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:222`-`.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:224`, and its schema exposes `routeAs` / `mergeModeHint` as canonical-routing hints. Runtime default is planner-first: `SPECKIT_SAVE_PLANNER_MODE` defaults to `plan-only` at `.opencode/skill/system-spec-kit/mcp_server/ENV_REFERENCE.md:179`, and `handleMemorySave()` treats any routed save as a plan when `plannerMode !== 'full-auto'` at `.opencode/skill/system-spec-kit/mcp_server/handlers/memory-save.ts:2703`-`.opencode/skill/system-spec-kit/mcp_server/handlers/memory-save.ts:2724`. That branch returns a `plannerMode: 'plan-only'` response rather than applying the canonical write at `.opencode/skill/system-spec-kit/mcp_server/handlers/memory-save.ts:3028`-`.opencode/skill/system-spec-kit/mcp_server/handlers/memory-save.ts:3057`.

The handler's own follow-up tells callers to retry with `plannerMode: 'full-auto'` at `.opencode/skill/system-spec-kit/mcp_server/handlers/memory-save.ts:2980`-`.opencode/skill/system-spec-kit/mcp_server/handlers/memory-save.ts:2987`, but `plannerMode` is not accepted by the MCP input schema. The Zod schema only allows `filePath`, `force`, `dryRun`, `skipPreflight`, `asyncEmbedding`, `routeAs`, `mergeModeHint`, and governance fields at `.opencode/skill/system-spec-kit/mcp_server/schemas/tool-input-schemas.ts:245`-`.opencode/skill/system-spec-kit/mcp_server/schemas/tool-input-schemas.ts:255` and `.opencode/skill/system-spec-kit/mcp_server/schemas/tool-input-schemas.ts:720`-`.opencode/skill/system-spec-kit/mcp_server/schemas/tool-input-schemas.ts:724`.
...
`references/memory/memory_system.md` says the public surface is "54 `spec_kit_memory` tools" and "50 local descriptors from `mcp_server/tool-schemas.ts` plus 4 Skill Advisor descriptors" at `.opencode/skill/system-spec-kit/references/memory/memory_system.md:96`-`.opencode/skill/system-spec-kit/references/memory/memory_system.md:99`. The table that follows ends after `ccc_feedback()` at `.opencode/skill/system-spec-kit/references/memory/memory_system.md:101`-`.opencode/skill/system-spec-kit/references/memory/memory_system.md:141`.

The registry now includes local descriptors that the table does not list, including `memory_retention_sweep` at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:329`-`.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:332`, `code_graph_verify` at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:657`-`.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:677`, `skill_graph_scan/query/status/validate` at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:692`-`.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:731`, and `deep_loop_graph_*` tools at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:818`-`.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:897`. `TOOL_DEFINITIONS` also imports the 4 advisor descriptors and then appends the CocoIndex and coverage graph tools at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:971`-`.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:984`.
