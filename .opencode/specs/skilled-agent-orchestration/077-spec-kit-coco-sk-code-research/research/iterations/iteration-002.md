...
`validate.sh` delegates to the Node validation orchestrator whenever `.opencode/skill/system-spec-kit/mcp_server/dist/lib/validation/orchestrator.js` or the TS fallback exists, then exits with that result at `.opencode/skill/system-spec-kit/scripts/spec/validate.sh:839`-`.opencode/skill/system-spec-kit/scripts/spec/validate.sh:858` and `.opencode/skill/system-spec-kit/scripts/spec/validate.sh:1019`. The Node orchestrator's graph check is only `fs.existsSync(path.join(folder, 'graph-metadata.json')) ? 'pass' : 'warn'` at `.opencode/skill/system-spec-kit/mcp_server/lib/validation/orchestrator.ts:355`-`.opencode/skill/system-spec-kit/mcp_server/lib/validation/orchestrator.ts:366`.

The stronger shell rule parses JSON and requires top-level `schema_version`, `packet_id`, `spec_folder`, `parent_id`, `children_ids`, `manual`, and `derived` fields at `.opencode/skill/system-spec-kit/scripts/rules/check-graph-metadata.sh:27`-`.opencode/skill/system-spec-kit/scripts/rules/check-graph-metadata.sh:42`, but that rule is bypassed on the default path. A synthetic fixture with `{bad json` in `graph-metadata.json` still produced a default `GRAPH_METADATA_PRESENT` pass, so this is not only theoretical drift.

### F-002-002 — Default Node validator omits registry-owned phase rules [P1]
The shell registry declares `PHASE_LINKS`, `PHASE_PARENT_CONTENT`, and `GRAPH_METADATA_PRESENT` as runnable rules at `.opencode/skill/system-spec-kit/scripts/lib/validator-registry.json:107`-`.opencode/skill/system-spec-kit/scripts/lib/validator-registry.json:169`, and `validation_rules.md` states that `validate.sh` delegates to the Node orchestrator by default at `.opencode/skill/system-spec-kit/references/validation/validation_rules.md:982`. The Node orchestrator, however, emits a fixed subset of entries at `.opencode/skill/system-spec-kit/mcp_server/lib/validation/orchestrator.ts:355`-`.opencode/skill/system-spec-kit/mcp_server/lib/validation/orchestrator.ts:366`; it does not run `PHASE_LINKS` or `PHASE_PARENT_CONTENT`.

This creates visible test/docs drift: `test-phase-validation.js` expects `PHASE_LINKS` to appear as a warning for broken phase links at `.opencode/skill/system-spec-kit/scripts/tests/test-phase-validation.js:334`-`.opencode/skill/system-spec-kit/scripts/tests/test-phase-validation.js:339`, while the default orchestrator cannot produce that rule. The fix target is to either port these registry rules into the Node orchestrator or make `validate.sh` run both paths without double-counting.

### F-002-003 — Mandatory metadata is enforced only for phase parents [P2]
Repo instructions state that every Level 1+ spec folder must contain both `description.json` and `graph-metadata.json` at `AGENTS.md:282`. The default Node validator only adds `description.json` and `graph-metadata.json` to `FILE_EXISTS` when the detected level is `phase` at `.opencode/skill/system-spec-kit/mcp_server/lib/validation/orchestrator.ts:168`-`.opencode/skill/system-spec-kit/mcp_server/lib/validation/orchestrator.ts:177`. The shell `check-files.sh` has the same phase-parent-only metadata branch at `.opencode/skill/system-spec-kit/scripts/rules/check-files.sh:44`-`.opencode/skill/system-spec-kit/scripts/rules/check-files.sh:52`.
...
The resume docs say a phase parent should redirect only when `derived.last_active_child_id` is non-null and `derived.last_active_at` parses as ISO-8601 within 24 hours; malformed, missing, or stale pointers should fall back to child listing at `.opencode/skill/system-spec-kit/references/workflows/quick_reference.md:468`-`.opencode/skill/system-spec-kit/references/workflows/quick_reference.md:474`. The structure docs also treat `children_ids`, `derived.last_active_child_id`, and `derived.last_active_at` as the phase-parent graph rollup at `.opencode/skill/system-spec-kit/references/structure/phase_definitions.md:79`-`.opencode/skill/system-spec-kit/references/structure/phase_definitions.md:99`.

The shell graph rule only type-checks optional `last_active_child_id` / `last_active_at` fields at `.opencode/skill/system-spec-kit/scripts/rules/check-graph-metadata.sh:38`-`.opencode/skill/system-spec-kit/scripts/rules/check-graph-metadata.sh:40`, and the default Node path does not parse them at all. Existing tests cover pointer writes and concurrent child saves at `.opencode/skill/system-spec-kit/scripts/tests/phase-parent-pointer.vitest.ts:61`-`.opencode/skill/system-spec-kit/scripts/tests/phase-parent-pointer.vitest.ts:124`, but not validator rejection or warning for a stale pointer, a malformed timestamp, or a pointer that is absent from `children_ids` / direct child folders.
