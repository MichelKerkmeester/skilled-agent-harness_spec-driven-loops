...
The P1 list should start with system-spec-kit workflow loading, not validator cleanup. `system-spec-kit/SKILL.md` requires all code creation/updates to route through `sk-code` at `.opencode/skill/system-spec-kit/SKILL.md:401` and repeats that code updates route through `sk-code` at `.opencode/skill/system-spec-kit/SKILL.md:440`. The `/spec_kit:implement` auto workflow names `sk-code` only in the review agent overlay at `.opencode/command/spec_kit/assets/spec_kit_implement_auto.yaml:209`-`.opencode/command/spec_kit/assets/spec_kit_implement_auto.yaml:216`, while the development step at `.opencode/command/spec_kit/assets/spec_kit_implement_auto.yaml:402`-`.opencode/command/spec_kit/assets/spec_kit_implement_auto.yaml:423` proceeds through task parsing, setup, TDD, core development, integration, and checklist updates without a concrete `sk-code` resource-load gate. `/spec_kit:complete` has the same shape: pre-work checks a generic skills folder at `.opencode/command/spec_kit/assets/spec_kit_complete_auto.yaml:538`-`.opencode/command/spec_kit/assets/spec_kit_complete_auto.yaml:549`, planning explorers use CocoIndex at `.opencode/command/spec_kit/assets/spec_kit_complete_auto.yaml:667`-`.opencode/command/spec_kit/assets/spec_kit_complete_auto.yaml:706`, and development says "Check skills folder for coding standards" at `.opencode/command/spec_kit/assets/spec_kit_complete_auto.yaml:859`-`.opencode/command/spec_kit/assets/spec_kit_complete_auto.yaml:889` but does not name the selected `sk-code` OpenCode resources.
...
The dedicated `mcp-coco-index` skill is clear that the MCP server exposes only `search`: `.opencode/skill/mcp-coco-index/SKILL.md:211`-`.opencode/skill/mcp-coco-index/SKILL.md:220` says CLI handles `ccc status`, `ccc index`, `ccc search`, and `ccc reset`, while MCP exposes `search` only and advises `refresh_index=true` on the first query. Its tool reference repeats that `status`, `index`, and `reset` are CLI-only at `.opencode/skill/mcp-coco-index/references/tool_reference.md:18`-`.opencode/skill/mcp-coco-index/references/tool_reference.md:23`.

That conflicts with the system-spec-kit surface from iteration 3: `ccc_status`, `ccc_reindex`, and `ccc_feedback` are documented/registered there but not dispatched, producing an `Unknown tool` path (`iteration-003.md:17`-`iteration-003.md:22`). The mcp-coco-index fork also has refresh/wait behavior at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/daemon.py:454`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/daemon.py:520`, while CLI output includes fork telemetry such as `dedupedAliases`, `uniqueResultCount`, `raw_score`, `path_class`, and `rankingSignals` at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/cli.py:136`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/cli.py:161`.
...
- Q4: Partially resolved. Local ingestion of `.opencode/skill/sk-code/` resources was shown in iteration 9, but rank smoke tests should wait until canonical OpenCode authoring/spec resources exist.
