...
`sk-code` declares `JAVASCRIPT` as an OpenCode language route with `.js`, `.mjs`, `.cjs`, CommonJS, Node, and MCP signals at `.opencode/skill/sk-code/SKILL.md:100`-`.opencode/skill/sk-code/SKILL.md:102`. The manual testing playbook coverage note lists OPENCODE language sub-detection for TypeScript, Python, Shell, and JSON/JSONC only at `.opencode/skill/sk-code/manual_testing_playbook/manual_testing_playbook.md:52`-`.opencode/skill/sk-code/manual_testing_playbook/manual_testing_playbook.md:57`, and the language scenario table is explicitly `LS-001..LS-004` for TypeScript, Python, Shell, and Config at `.opencode/skill/sk-code/manual_testing_playbook/manual_testing_playbook.md:209`-`.opencode/skill/sk-code/manual_testing_playbook/manual_testing_playbook.md:216`.
...
`sk-code` says OPENCODE covers `.opencode/` skills, agents, commands, MCP servers, hooks, scripts, tests, config, and language code at `.opencode/skill/sk-code/SKILL.md:20`-`.opencode/skill/sk-code/SKILL.md:23`. The canonical OpenCode references include a skill directory sketch at `.opencode/skill/sk-code/references/opencode/shared/code_organization.md:394`-`.opencode/skill/sk-code/references/opencode/shared/code_organization.md:418` and a detailed MCP server structure immediately after, but there are no equivalent first-class reference files or checklists for authoring `.opencode/agent/*` or `.opencode/command/*` artifacts. The asset checklist set is language-only: JavaScript, TypeScript, Python, Shell, Config, and Universal under `.opencode/skill/sk-code/assets/opencode/checklists/`.
...
The current router says OpenCode resources live under `references/opencode/` and `assets/opencode/` at `.opencode/skill/sk-code/SKILL.md:110`-`.opencode/skill/sk-code/SKILL.md:117`. The shared OpenCode directory convention still documents an OpenCode skill structure with `references/shared/`, `references/javascript/`, `references/typescript/`, and `assets/checklists/` directly under the skill root at `.opencode/skill/sk-code/references/opencode/shared/code_organization.md:397`-`.opencode/skill/sk-code/references/opencode/shared/code_organization.md:416`. It also omits the Config resource folder despite `CONFIG` being a declared OpenCode language route at `.opencode/skill/sk-code/SKILL.md:100`-`.opencode/skill/sk-code/SKILL.md:106`.
...
`sk-code` lists `system-spec-kit` as the integration owner for spec folder gates, validation, memory, and context preservation at `.opencode/skill/sk-code/SKILL.md:200`-`.opencode/skill/sk-code/SKILL.md:203`, and the OPENCODE verification row mentions "spec validation for changed spec folders" at `.opencode/skill/sk-code/SKILL.md:127`-`.opencode/skill/sk-code/SKILL.md:132`. The loaded OpenCode references and checklist only cover spec-folder path invariants: allowed roots, naming pattern, containment checks, and restore safety at `.opencode/skill/sk-code/references/opencode/shared/universal_patterns.md:418`-`.opencode/skill/sk-code/references/opencode/shared/universal_patterns.md:426` and `.opencode/skill/sk-code/assets/opencode/checklists/universal_checklist.md:89`-`.opencode/skill/sk-code/assets/opencode/checklists/universal_checklist.md:96`.
