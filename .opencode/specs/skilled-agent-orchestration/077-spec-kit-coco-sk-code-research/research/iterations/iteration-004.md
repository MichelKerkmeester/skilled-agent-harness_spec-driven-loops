...
`mcp-coco-index` documents the intended split as "MCP server exposes `search` only" and says "Index/status/reset operations remain CLI-only unless routed through the Spec Kit Memory `ccc_*` tools" at `.opencode/skill/mcp-coco-index/SKILL.md:220`. The dedicated tool reference repeats that `status`, `index`, and `reset` are CLI-only at `.opencode/skill/mcp-coco-index/references/tool_reference.md:22`-`.opencode/skill/mcp-coco-index/references/tool_reference.md:23` and `.opencode/skill/mcp-coco-index/references/tool_reference.md:230`-`.opencode/skill/mcp-coco-index/references/tool_reference.md:233`.

The fallback owner named by that sentence is not callable today. `system-spec-kit` defines `ccc_status`, `ccc_reindex`, and `ccc_feedback` at `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:735`-`.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts:755` and documents them at `.opencode/skill/system-spec-kit/references/memory/memory_system.md:139`-`.opencode/skill/system-spec-kit/references/memory/memory_system.md:141`, but `ALL_DISPATCHERS` has no CocoIndex dispatcher at `.opencode/skill/system-spec-kit/mcp_server/tools/index.ts:91`-`.opencode/skill/system-spec-kit/mcp_server/tools/index.ts:101`; unrecognized names return `null` at `.opencode/skill/system-spec-kit/mcp_server/tools/index.ts:103`-`.opencode/skill/system-spec-kit/mcp_server/tools/index.ts:120`.
...
`mcp-coco-index/SKILL.md` says result rows carry seven extra fields, including `source_realpath`, `content_hash`, `dedupedAliases`, and `uniqueResultCount`, at `.opencode/skill/mcp-coco-index/SKILL.md:272`. `references/tool_reference.md` repeats this for CLI rows at `.opencode/skill/mcp-coco-index/references/tool_reference.md:75`, for MCP rows at `.opencode/skill/mcp-coco-index/references/tool_reference.md:273`, and the example shows row-level `source_realpath`, `content_hash`, `dedupedAliases`, and object-shaped `rankingSignals` at `.opencode/skill/mcp-coco-index/references/tool_reference.md:409`-`.opencode/skill/mcp-coco-index/references/tool_reference.md:430`.

The actual runtime shape is narrower. The daemon protocol `SearchResult` includes `file_path`, `language`, `content`, `start_line`, `end_line`, `score`, `raw_score`, `path_class`, and `rankingSignals: list[str]` only at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/protocol.py:95`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/protocol.py:114`. The MCP `CodeChunkResult` mirrors that same row shape at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/server.py:42`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/server.py:68`. `source_realpath` and `content_hash` are selected for dedup inside `query.py` at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/query.py:74`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/query.py:86`, but `_ranked_result()` discards them before returning `QueryResult` at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/query.py:176`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/query.py:217`.
...
`ccc mcp` starts a background index task as soon as the MCP server starts at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/cli.py:473`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/cli.py:499`. The MCP `search` tool also defaults `refresh_index` to `true` at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/server.py:116`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/server.py:123` and runs `client.index(project_root)` before search when that flag is true at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/server.py:137`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/server.py:151`.

The skill already warns that simultaneous `refresh_index=true` requests can produce `ComponentContext` errors at `.opencode/skill/mcp-coco-index/SKILL.md:313`-`.opencode/skill/mcp-coco-index/SKILL.md:318`. This code path means a normal first MCP query can overlap with the startup index, and both routes share the same daemon-backed client object from `ccc mcp`.
...
The CLI docs present `--path` as optional with default `.` at `.opencode/skill/mcp-coco-index/references/tool_reference.md:36`-`.opencode/skill/mcp-coco-index/references/tool_reference.md:43`, and the CLI/MCP mapping only says CLI path is a single string while MCP `paths` is a list at `.opencode/skill/mcp-coco-index/references/tool_reference.md:279`-`.opencode/skill/mcp-coco-index/references/tool_reference.md:287`.

The actual CLI has a hidden current-directory default. If `--path` is omitted and the user runs `ccc search` from below the project root, `resolve_default_path()` returns `"{relative-cwd}/*"` at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/cli.py:92`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/cli.py:101`, and `search()` applies that default at `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/cli.py:367`-`.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/cli.py:374`. MCP searches have no current working directory equivalent and default to the whole project unless `paths` is supplied.

Concrete fix target: document the CLI's cwd-scoped default in the parameter table and recommend explicit `--path` / `paths` for parity. `references/search_patterns.md:145`-`.opencode/skill/mcp-coco-index/references/search_patterns.md:150` already hints at this, but the primary tool reference still hides the behavior.

### sk-code

No new direct sk-code finding this iteration. The next pass should test whether CocoIndex ingests `.opencode/skill/sk-code/` resources and whether semantic queries surface those references with usable rank.
