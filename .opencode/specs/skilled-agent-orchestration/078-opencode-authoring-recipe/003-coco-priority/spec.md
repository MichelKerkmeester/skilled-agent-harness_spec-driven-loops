...
_memory:
  continuity:
    ...
    key_files:
      ...
      - .opencode/skill/mcp-coco-index/tests/test_settings.py
...
### In Scope
- Add `CANONICAL_RESOURCE_PATHS: list[str]` constant in `settings.py` (default contains `.opencode/skill/*/assets/opencode/**`, `.opencode/skill/*/assets/motion_dev/**`, and similar known-canonical sk-code patterns)
...
|----|-------------|---------------------|-------------|
| REQ-001 | CANONICAL_RESOURCE_PATHS constant exists with default sk-code asset patterns | `grep "CANONICAL_RESOURCE_PATHS" settings.py` returns ≥1 hit; default includes `.opencode/skill/*/assets/opencode/**` | F-001-002, F-005-001 |
...
| REQ-004 | canonical paths bypass exclusion under defaults | tests/test_settings.py canonical-bypass test PASS for `.opencode/skill/sk-code/assets/opencode/skill_authoring.md` | F-001-002, F-009-004 |
...
| REQ-008 | changelog/v1.3.0.0.md created (compact format) | File present at `.opencode/skill/mcp-coco-index/changelog/v1.3.0.0.md` | — |
...
|----|-------------|---------------------|
| REQ-013 | is_canonical_path() helper handles glob patterns (e.g., `**/assets/opencode/**`) | Test asserts `.opencode/skill/sk-code/assets/opencode/checklists/skill_authoring.md` matches pattern `.opencode/skill/*/assets/opencode/**` |
...
- **SC-001**: A fresh-clone smoke test asserts `.opencode/skill/sk-code/assets/opencode/` is ingested under defaults (closes the portability gap surfaced by F-001-002 + F-005-001).
...
**Given** CANONICAL_RESOURCE_PATHS is shipped with default sk-code patterns, **When** a fresh ProjectSettings instance is created, **Then** canonical_resource_paths includes `.opencode/skill/*/assets/opencode/**`.

**Given** canonical_resource_paths is set, **When** the path filter checks `.opencode/skill/sk-code/assets/opencode/checklists/skill_authoring.md`, **Then** is_canonical_path() returns True and exclusion is bypassed.
...
| Risk | Default CANONICAL_RESOURCE_PATHS is too aggressive (over-includes) | Low | Default scoped to `.opencode/skill/*/assets/opencode/**` and motion_dev, NOT all hidden dirs; users can opt-out via empty list |
