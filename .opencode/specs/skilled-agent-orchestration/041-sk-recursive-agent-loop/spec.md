---
title: "Feature Specification: 041 Recursive Agent Loop [skilled-agent-orchestration/041-sk-recursive-agent-loop/spec]"
description: "Parent packet for the sk-improve-agent program. Phases 001-007 build the evaluator-first MVP through doc alignment, promotion verification, runtime alignment, command rename, and wording cleanup. Phase 008 adds holistic 5-dimension evaluation with integration scanning, dynamic profiling, and any-agent support. Renamed from sk-recursive-agent to sk-improve-agent."
trigger_phrases:
  - "041 spec"
...
| 9 | [`009-sk-improve-agent-self-test/`](./009-sk-improve-agent-self-test/) | Complete | Self-referential test: ran the full `/improve:agent` loop targeting `agent-improver.md` itself. 3 iterations, baseline 99→100 (systemFitness fix), plateau at 100 across all 5 dimensions. [Implementation summary](./009-sk-improve-agent-self-test/implementation-summary.md). |
...
### Problem Statement
The original `041-sk-improve-agent-loop` packet grew into a longer agent-improver program with seven explicit implementation phases. Keeping any of those phases out of the root packet would leave the program history incomplete and make the completion state harder to trust.

### Purpose
Keep `041-sk-improve-agent-loop` as the parent packet for the `sk-improve-agent` program so that completed work lives under explicit child phases and future work continues as `009-*` and later phases inside this same folder.
<!-- /ANCHOR:problem -->
...
### In Scope
- Parent packet documentation for `041-sk-improve-agent-loop`
...
- Changing the shipped `sk-improve-agent` behavior beyond path and packet-structure alignment
...
| REQ-002 | The old root implementation docs move into phase `001` | MVP docs and runtime evidence now live under `001-sk-improve-agent-mvp/` |
| REQ-003 | The former full-skill follow-up becomes phase `002` | Full-skill docs and evidence now live under `002-sk-improve-agent-full-skill/` |
...
| REQ-013 | The holistic evaluation + rename is represented as phase `008` | Root docs, phase lineage, and examples point to the completed `008` phase. Skill renamed from sk-recursive-agent to sk-improve-agent. |
<!-- /ANCHOR:requirements -->
...
- **SC-002**: No stale references to `042-sk-improve-agent-full-skill` or the old flat `041/.../improvement/` layout remain, and operator examples point at the current child-phase path.
...
1. **Given** a maintainer opens `041-sk-improve-agent-loop/`, **when** they read the root spec, **then** they can see exactly what phases `001` through `008` delivered.
...
- [`001-sk-improve-agent-mvp/`](./001-sk-improve-agent-mvp/) — MVP implementation packet
- [`002-sk-improve-agent-full-skill/`](./002-sk-improve-agent-full-skill/) — Full-skill expansion packet
- [`003-sk-improve-agent-doc-alignment/`](./003-sk-improve-agent-doc-alignment/) — sk-doc alignment and packet closeout packet
- [`004-sk-improve-agent-promotion-verification/`](./004-sk-improve-agent-promotion-verification/) — promotion-path and repeatability verification packet
- [`005-sk-improve-agent-package-runtime-alignment/`](./005-sk-improve-agent-package-runtime-alignment/) — stricter template-fidelity and runtime-parity correction packet
- [`006-sk-improve-agent-command-rename/`](./006-sk-improve-agent-command-rename/) — command-entrypoint rename and wrapper/YAML sync packet
- [`007-sk-improve-agent-wording-alignment/`](./007-sk-improve-agent-wording-alignment/) — wording-only cleanup across current agent-improver surfaces
- [`008-sk-improve-agent-holistic-evaluation/`](./008-sk-improve-agent-holistic-evaluation/) — 5-dimension evaluation, integration scanner, dynamic profiling, any-agent support, rename to sk-improve-agent
<!-- /ANCHOR:success-criteria -->
