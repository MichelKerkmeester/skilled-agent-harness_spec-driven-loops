...
`@orchestrate` is consumed via the Task tool / orchestrator dispatch. Improvements ship as canonical `.opencode/agent/orchestrate.md` updates plus 3 mirror updates. Score profile is dynamic (generated per-target via `generate-profile.cjs`).
<!-- /ANCHOR:architecture -->
...
| **P2** Dispatch | `/improve:agent .opencode/agent/orchestrate.md` | `<packet>/candidate.md` |
...
`rollback-candidate.cjs --target=.opencode/agent/orchestrate.md --backup=<pre-promote-backup>` reverts canonical. Mirror revert via git revert of the sync commit. 
<!-- /ANCHOR:rollback -->
