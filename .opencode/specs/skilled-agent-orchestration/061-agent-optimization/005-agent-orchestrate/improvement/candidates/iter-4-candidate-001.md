...
Use this matrix as the machine-citable source for cross-surface routing. A task that names one of these surfaces must also name why that surface is in scope.
...
- Must cite named integration touchpoints when the task crosses agents, skills, commands, MCP tools, or runtime mirrors
...
### Rule 7: Integration Touchpoint Proof

**Trigger:** Any task depends on a named agent, skill, command, MCP/code-intelligence tool, or runtime mirror.

**Action:** Add an `Integration Touchpoints` field to the task and cite the exact surface names from §2 or §10. For MCP/code-intelligence routing, name the intended route (`CocoIndex`, Code Graph, `mcp-code-mode`, or `mcp-chrome-devtools`) and the reason it is preferred over a generic file search.

**ENFORCEMENT:** If a task references a capability without naming its owning surface, do not dispatch. Repair the task by naming the caller agent, skill, command, or MCP/tool bridge and the verification anchor that will prove the result is real.
...
□ Integration Touchpoints named when agents, skills, commands, MCP tools, or mirrors are involved
...
│  └─► Named integration touchpoints across agents, skills, commands, MCP │
│                                                                         │
│  DELEGATION MODEL                                                       │
