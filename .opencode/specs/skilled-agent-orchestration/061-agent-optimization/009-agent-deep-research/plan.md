...
`@deep-research` is consumed via a specific spec_kit command flow. Improvements ship as canonical `.opencode/agent/deep-research.md` updates plus 3 mirror updates. Score profile is dynamic (generated per-target via `generate-profile.cjs`).
<!-- /ANCHOR:architecture -->
...
| **P2** Dispatch | `/improve:agent .opencode/agent/deep-research.md` | `<packet>/candidate.md` |
...
`rollback-candidate.cjs --target=.opencode/agent/deep-research.md --backup=<pre-promote-backup>` reverts canonical. Mirror revert via git revert of the sync commit. 
<!-- /ANCHOR:rollback -->
