...
`@improve-prompt` is consumed via the Task tool / orchestrator dispatch. Improvements ship as canonical `.opencode/agent/improve-prompt.md` updates plus 3 mirror updates. Score profile is dynamic (generated per-target via `generate-profile.cjs`).
<!-- /ANCHOR:architecture -->
...
| **P2** Dispatch | `/improve:agent .opencode/agent/improve-prompt.md` | `<packet>/candidate.md` |
...
`rollback-candidate.cjs --target=.opencode/agent/improve-prompt.md --backup=<pre-promote-backup>` reverts canonical. Mirror revert via git revert of the sync commit. 
<!-- /ANCHOR:rollback -->
