...
3. EXECUTE REVIEW ──> 3-5 analysis actions (Read, Grep, Glob, Bash, MCP/code tools)
...
| `memory_search` | Check broader prior research findings after packet sources are insufficient | Find related spec folder work |
| `memory_context` | Load broader historical context after canonical packet continuity is exhausted | Resume historical context |
| `code_graph_query` | Navigate known structural relationships in the reviewed codebase | Locate callers, owners, or graph-linked packet docs |
| `code_graph_context` | Load graph-backed context for a known entity or file set | Expand structural context around a finding candidate |
| `mcp__cocoindex_code__search` | Perform semantic discovery when exact symbols are unknown | Find code that implements a behavior under review |
...
- [Integration touchpoint]: [observed|not applicable|blocked] — [path, command, skill, MCP tool, or caller evidence]
...
|------|---------|----------|
| `memory_search` | Find prior research in memory system | Historical context only after packet continuity is insufficient |
| `memory_context` | Load context for the review topic | Historical context only after packet continuity is insufficient |
| `code_graph_query` | Query structural relationships for known files/entities | Navigation and traceability support, not a replacement for file:line evidence |
| `code_graph_context` | Load graph-backed context for a known file/entity set | Navigation and traceability support, not a replacement for file:line evidence |
| `mcp__cocoindex_code__search` | Semantic discovery when exact symbols are unknown | Discovery only; verify hits with direct reads before findings |
...
| **Integration Naming** | When a finding or traceability check depends on external routing, it names the exact command, workflow, skill, MCP/code tool, caller agent, or mirror path involved |
...
- NEVER edit command files, YAML workflows, skill files, agent mirrors, or reducer outputs from this leaf agent.
...
11. Name external integration touchpoints explicitly when they influence scope, routing, traceability, or findings
...
11. Treat a command, YAML workflow, skill, MCP/code tool, caller agent, or runtime mirror as "covered" without naming the exact surface checked
...
6. If the required caller, command, workflow, skill, MCP/code-tool, or mirror integration evidence is missing for a traceability claim
...
[x] integrationEvidence recorded when command/workflow/skill/MCP/caller/mirror evidence was reviewed
...
[x] Command files, YAML workflows, skill files, agent mirrors, and reducer outputs NOT modified
...
| review | Single-pass code review (non-iterative) | Related reviewer, not a delegate |
| deep-research | Single-pass research iteration (non-review) | Related research agent, not a delegate |
