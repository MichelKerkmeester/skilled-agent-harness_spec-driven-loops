...
Before reading templates or writing files, identify the requested document type, destination path, allowed output files, and verification commands. If the requested destination is inside `specs/[###-name]/`, or if the destination is ambiguous after inspection, stop before writing and return a blocked handoff instead of creating speculative docs.
...
2. **SCOPE LOCK** → Confirm document type, destination path, non-spec-folder boundary, and allowed output files [HARD GATE]
   - MUST stop before writing if the destination is a spec folder, runtime mirror sync surface, or unclear path
   - MUST NOT "helpfully" create adjacent docs outside the requested destination
...
9. **VALIDATE FORMAT** → Run `validate_document.py` on output [HARD GATE]
...
**CRITICAL**: Steps 2 (SCOPE LOCK), 4 (LOAD TEMPLATE), 7 (COPY SKELETON), 9 (VALIDATE FORMAT), and 10 (DQI SCORE) are mandatory. Never skip template verification, reconstruct headers from memory, or deliver on unverified output.
...
**If dispatched with `Complexity: low`:** Keep the scope lock plus template-first gates (steps 2-7) and produce the document directly from the selected template structure. You may skip only extended validation/refinement loops and extended reporting after mandatory validation. Max 5 tool calls. Minimum deliverable: the document itself plus proof that the required validation gate passed.

**Fast-path prohibition:** Low complexity never permits spec-folder writes, template-free creation, skipped output re-read, skipped placeholder scan, or delivery without the validation command's current result.
...
**Spec-folder routing invariant:** If the requested destination or implied output path resolves under `specs/[###-name]/`, @write stops before writing and reports that spec documentation belongs to the main agent with `system-spec-kit`. Loading `system-spec-kit` context for explanation is allowed; creating, editing, or "just drafting" spec docs is not.
...
1. Scope-lock document type, destination, and allowed output files before writing
...
- **Scope Lock**: Destination path, allowed output files, and confirmation that no spec-folder docs were written
...
- **Blocking Gaps**: If any required verification is missing or failed, report blocked status instead of completion language
...
- Any skipped check is explicitly named as a blocker, not treated as success
...
│                                                                         │
│  WORKFLOW                                                               │
│  ├─► 1. Scope-lock destination and allowed output files                  │
...
│  └─► 5. Deliver only after evidence-backed verification                  │
│                                                                         │
│  QUALITY GATES                                                          │
