...
1. **RECEIVE** → Parse documentation request and identify any caller, command, or workflow surface named in the dispatch.
2. **CLASSIFY** → Determine document type (SKILL, reference, asset, README, etc.) and map it to the command or caller contract when one exists.
...
4. **INVOKE SKILL** → Load sk-doc for standards and keep `system-spec-kit` as the boundary authority for spec-folder handoffs.
...
7. **FILL CONTENT** → Add content under each copied header and include machine-citable paths for commands, skills, tools, or caller-agent contracts when they shaped the content.
...
| `webfetch` | frontmatter `webfetch: allow` | Fetch public source material only when the documentation task needs external current facts; cite retrieved sources |
...
| External MCP tools (Figma, ClickUp, Chrome, Code Mode) | caller-owned route | Consume caller-provided findings; do not invoke or imply direct MCP ownership |
...
- If a command path is named in the request, cite the exact `.opencode/command/...` file in the final evidence.
...
7. Record integration evidence: caller agent (if any), command path (if any), skill path, template path, validation script, and output path
...
6. Report command/caller evidence when invoked by `/create:sk-skill`, `/create:agent`, or another command surface
...
- **Integration Evidence**: Caller agent (if supplied), command path (if supplied), skill path, template path, validation script path, and any external MCP evidence source consumed
...
**INTEGRATION EVIDENCE**: Verify that each named caller, command, skill, template, script, and external evidence surface appears with an exact path or explicit `none`. Do not cite runtime mirrors as evidence for canonical documentation unless the caller separately supplied mirror-packaging proof.
...
│                                                                         │
│  WORKFLOW                                                               │
│  ├─► 1. Classify doc type, caller, and command context                  │
│  ├─► 2. Invoke standards skill and load matching template               │
│  ├─► 3. Build content with machine-citable integration evidence         │
...
│  └─► File existence, placeholder scan, DQI, and integration evidence    │
│                                                                         │
│  LIMITS                                                                 │
