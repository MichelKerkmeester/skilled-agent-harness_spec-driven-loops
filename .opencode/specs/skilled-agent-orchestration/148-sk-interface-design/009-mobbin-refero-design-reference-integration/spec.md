...
_memory:
  continuity:
    ...
    recent_action: "Wired Mobbin/Refero into Code Mode and integrated them into sk-interface-design"
...
This is **Phase 9** of the 148-sk-interface-design packet. The earlier phases built the skill, de-vendored it from `ui-ux-pro-max`, and added the Claude Design parity loop and variation-diversity. This phase adds a real-world shipped-UI reference path so the skill can name the real-world default for a category and deviate from it deliberately.
...
### Out of Scope
- Installing Refero's own `refero_skill` - the user asked for sk-interface-design integration, not a separate skill.
...
- **SC-002**: sk-interface-design passes `package_skill --check` and routes a design task to the new reference as a critique-against source, never a chooser.
