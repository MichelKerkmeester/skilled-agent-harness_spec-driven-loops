...
description: "Plan to split the shared parity doc and strip Open Design naming from sk-interface-design so it stands alone, with one-way coupling preserved from mcp-open-design."
trigger_phrases:
...
### Key Components
- **`real_ui_loop.md`** (sk-interface-design): the transport-agnostic loop, vendor-neutral, §6/§7/§8 preserved
...
### Data Flow
A standalone reader of sk-interface-design sees only generic grounding (a real design system you own) + Mobbin/Refero. A framework user doing Open Design work routes through mcp-open-design, which mandates this skill and owns the transport doc that plugs into real_ui_loop.md.
<!-- /ANCHOR:architecture -->
...
- [x] Strip Open Design naming from sk-interface-design content (primary surfaces + feature_catalog + playbook)
...
|-----------|-------|-------|
| Standalone | No Open Design naming in sk-interface-design live content | `rg -i` over the skill (excl. changelog) |
| Reverse coupling | mcp-open-design still mandates sk-interface-design | `rg 'sk-interface-design' mcp-open-design/SKILL.md` |
