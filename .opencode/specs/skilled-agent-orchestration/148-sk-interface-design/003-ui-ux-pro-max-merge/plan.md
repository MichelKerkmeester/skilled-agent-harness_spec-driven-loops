...
description: "Three-phase plan to merge ui-ux-pro-max's data + search layers into sk-interface-design: Phase 1 quality-floor data + licensing foundation, Phase 2 zero-dep search script, Phase 3 aesthetic critique-against inventory + advisor routing."
trigger_phrases:
  - "ui-ux-pro-max merge plan"
  - "sk-interface-design data merge plan"
...
### Overview
Adopt the data + search layers of `ui-ux-pro-max` into `sk-interface-design` in three independently shippable phases. Quality-floor data lands first (lowest risk, also stands up licensing); the search script second; the aesthetic critique-against inventory + advisor routing last.
<!-- /ANCHOR:summary -->
...
|---------|--------------|--------|--------------|
| `sk-interface-design/SKILL.md` | Skill entry doc | update (frontmatter license + short routing section) | size check + advisor discovery |
| `sk-interface-design/references/design_principles.md` | Primary authority | unchanged | no diff |
| `sk-interface-design/graph-metadata.json` | Advisor routing | update (trigger phrases / intents / key_files); keep existing edges | `skill_graph_validate` |
| `sk-interface-design/assets/data/`, `scripts/`, new references | New surfaces | create | files present + license-mapped |
