...
description: "Forward-looking summary for the planned three-phase merge of ui-ux-pro-max data + search into sk-interface-design. Status planned; this document is completed as each phase ships."
trigger_phrases:
  - "ui-ux-pro-max merge summary"
  - "sk-interface-design data merge summary"
...
| Field | Value |
|-------|-------|
| **Spec Folder** | skilled-agent-orchestration/148-sk-interface-design/003-ui-ux-pro-max-merge |
...
`sk-interface-design` now ships real design data for the first time, across all three phases, while staying lean (SKILL.md 1437 words) and anti-default. The skill passes `package_skill.py`; `design_principles.md` is byte-for-byte unchanged.
...
| File | Action | Purpose |
|------|--------|---------|
| `sk-interface-design/assets/data/*.csv` (8) | Created | Adopted MIT design data (quality floor + aesthetic inventory) |
...
| `sk-interface-design/scripts/design_search{,_core}.py` | Created | Optional zero-dep query-only search |
| `sk-interface-design/LICENSE-ui-ux-pro-max.txt`, `THIRD-PARTY-NOTICES.md` | Created | Licensing artifacts |
| `sk-interface-design/SKILL.md`, `graph-metadata.json` | Modified | Routing + license declaration |
