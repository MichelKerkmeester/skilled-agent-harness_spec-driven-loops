...
description: Session tracking for the ui-ux-pro-max → sk-interface-design merge research (claude-opus-4-8 lineage).
trigger_phrases:
  - "ui-ux-pro-max merge strategy"
  - "sk-interface-design merge research strategy"
...
Research-only investigation: can the vendored MIT `ui-ux-pro-max-skill-main` repo (data, scripts, patterns) merge into and improve the Apache-2.0 house `sk-interface-design` skill — what to ADOPT/ADAPT/SKIP, how to integrate into house structure, and the licensing path. No edits to `sk-interface-design` in this packet.

---

## 2. TOPIC

ui-ux-pro-max merge into sk-interface-design — per-asset-class adopt/adapt/skip verdict, integration design, license/attribution path, and cross-lineage cross-check.
...
## 4. NON-GOALS
- Implementing the merge / editing `sk-interface-design` (separate follow-up packet).
...
- Anchoring licensing on the existing `sk-interface-design` LICENSE precedent (iter 4): avoided inventing a new mechanism.
...
**External repo** `.opencode/specs/.../148-sk-interface-design/external/ui-ux-pro-max-skill-main/` (MIT, NextLevelBuilder, v2.5.0):
...
**Target skill** `.opencode/skills/sk-interface-design/` (Apache-2.0, vendored Anthropic frontend-design, v1.0.0):
