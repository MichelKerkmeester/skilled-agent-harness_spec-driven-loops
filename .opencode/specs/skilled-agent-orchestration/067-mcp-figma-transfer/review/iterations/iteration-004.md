...
2. **Install-guide skill counts are stale and disagree with both the skill index and actual disk state.** `rg -n "mcp-figma" .opencode/install_guides/README.md ".opencode/install_guides/SET-UP - AGENTS.md"` returns no hits, so the deleted skill name is gone. The remaining counts are inconsistent:
   - `.opencode/install_guides/README.md:1453` says `.opencode/skill/` has 18 skills.
...
| E. Skill index consistency | FAIL | Exact requested count `ls -d .opencode/skill/*/ | wc -l` returns 17. `.opencode/skill/README.md:44` says 21 and `:54` says 16. MCP integration skills are correct at 3 on `:58`, and `rg "mcp-figma" .opencode/skill/README.md` returns no hits. |
