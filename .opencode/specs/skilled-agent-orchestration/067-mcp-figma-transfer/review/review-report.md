...
| **P1-6** | Skill index `.opencode/skill/README.md` has CONTRADICTORY counts: line 44 says 21 skill folders; line 54 says 16. Actual `ls -d .opencode/skill/*/` returns 17. | iter 004 | `.opencode/skill/README.md:44, 54` | Reconcile both counts to 17 (current actual) |
...
| **P2-3** | Telemetry retention preserves 4 mcp-figma rows in `compliance.jsonl` (gitignored). Per D2 spirit currently preserved as immutable history. | iter 005 | `.opencode/skill/.smart-router-telemetry/compliance.jsonl` | Optional purge for fresh telemetry baseline; otherwise leave as-is |
...
9. **R-009:** Reconcile skill index counts (`.opencode/skill/README.md`).
