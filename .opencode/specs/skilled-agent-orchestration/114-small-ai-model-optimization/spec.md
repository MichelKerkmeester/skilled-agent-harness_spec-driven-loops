...
_memory:
  continuity:
    ...
    key_files:
      - "spec.md"
      - "007-rename-sk-prompt-small-model/implementation-summary.md"
    session_dedup:
    ...
    answered_questions:
      ...
      - "RQ5 outcome (post-synthesis): new sk-small-model skill (renamed sk-prompt-small-model via Phase 7 on 2026-05-21) with enhances edges to cli-devin + cli-opencode — shipped via 002-foundation-routing"
---
...
| 007 | `007-rename-sk-prompt-small-model/` | Post-completion rename: `sk-small-model` skill → `sk-prompt-small-model` (filesystem + frontmatter + sibling enhances edges + manual playbooks + root behavioral docs + auto-memory + advisor reindex). Identity-only refactor; behavior unchanged. P1. ~35 min. | **In progress** (started 2026-05-21) |
...
**Phase F (007-hardening-ci) was deleted 2026-05-18** per user direction — CI staleness checks and over-broad-glob lints will be handled manually or deferred to a future packet if/when needed. The 007 slot was reused 2026-05-21 by the `sk-prompt-small-model` rename packet (Phase 7 row above) — a different scope from the original hardening-CI plan.
...
| 006-cross-skill-propagation | 007-rename-sk-prompt-small-model | [Criteria TBD] | [Verification TBD] |
| 007-sk-prompt-small-model-rename | 008-sk-prompt-small-model-rename | [Criteria TBD] | [Verification TBD] |
<!-- /ANCHOR:phase-map -->
