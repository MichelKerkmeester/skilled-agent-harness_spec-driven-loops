---
title: "Feature Specification: Rename sk-small-model → sk-ai-small-model and propagate references"
...
_memory:
  continuity:
    ...
    answered_questions:
      - "Rename target name: sk-ai-small-model (aligns with sk-code/sk-doc/sk-prompt naming family; explicit ai prefix matches phase parent slug small-ai-model-optimization)"
...
# Feature Specification: Phase 7 — rename sk-small-model → sk-ai-small-model
...
**Deliverables**:
- Renamed directory `.opencode/skills/sk-ai-small-model/` (old `sk-small-model/` directory physically gone).
- Updated frontmatter `name:` field in SKILL.md.
- Updated `graph-metadata.json` for sk-ai-small-model + cli-devin + cli-opencode (the two sibling skills whose `enhances` edges target it).
...
**Changelog**:
- When this phase closes, write `../changelog/changelog-114-007-rename-sk-ai-small-model.md` via `nested-changelog.js`.
<!-- /ANCHOR:phase-context -->
...
- **Skill body** (rename directory + edit frontmatter/body): `.opencode/skills/sk-small-model/` → `.opencode/skills/sk-ai-small-model/`. Files affected: `SKILL.md`, `README.md`, `description.json`, `graph-metadata.json`, `references/pattern-index.md`, `changelog/v0.1.0.0.md`, `changelog/v0.2.0.0.md`. Add `changelog/v0.3.0.0.md` documenting the rename.
...
- **Manual testing playbooks** (rename + edit content):
  - `.opencode/skills/cli-devin/manual_testing_playbook/03--model-presets/005-swe16-via-sk-small-model-and-sk-prompt.md` → `005-swe16-via-sk-ai-small-model-and-sk-prompt.md`
  - `.opencode/skills/cli-devin/manual_testing_playbook/03--model-presets/006-deepseek-v4-via-sk-small-model-and-sk-prompt.md` → `006-deepseek-v4-via-sk-ai-small-model-and-sk-prompt.md`
  - `.opencode/skills/cli-devin/manual_testing_playbook/manual_testing_playbook.md` (index references)
  - `.opencode/skills/cli-opencode/manual_testing_playbook/07--prompt-templates/004-deepseek-v4-via-opencode-go-with-sk-small-model.md` → `004-…-with-sk-ai-small-model.md`
  - `.opencode/skills/cli-opencode/manual_testing_playbook/07--prompt-templates/005-kimi-k2-6-via-opencode-go-with-sk-small-model.md` → `005-…-with-sk-ai-small-model.md`
...
- `sk-ai-small-model/changelog/v0.1.0.0.md` + `v0.2.0.0.md` (after rename) — these document past version state under the old name; only add a NEW `v0.3.0.0.md` for the rename rather than rewriting v0.1/v0.2.
...
|-----------|-------------|-------------|
| `.opencode/skills/sk-small-model/` | Rename (dir) | → `.opencode/skills/sk-ai-small-model/` via `git mv` |
| `.opencode/skills/sk-ai-small-model/SKILL.md` | Modify | `name: sk-ai-small-model` in frontmatter; update H1 + in-body references |
| `.opencode/skills/sk-ai-small-model/README.md` | Modify | Rename in title + body references |
| `.opencode/skills/sk-ai-small-model/description.json` | Modify | Update `skill_id` and any `path`/`name` fields |
| `.opencode/skills/sk-ai-small-model/graph-metadata.json` | Modify | `skill_id: sk-ai-small-model`; entity name + path; derived.key_files paths |
| `.opencode/skills/sk-ai-small-model/references/pattern-index.md` | Modify | Header + any self-references |
| `.opencode/skills/sk-ai-small-model/changelog/v0.3.0.0.md` | Create | Document the rename |
| `.opencode/skills/cli-devin/graph-metadata.json` | Modify | `enhances[].target: sk-ai-small-model` + `related_to` |
...
| `.opencode/skills/cli-opencode/graph-metadata.json` | Modify | `enhances[].target: sk-ai-small-model` + `related_to` |
...
|----|-------------|---------------------|
| REQ-001 | Skill directory renamed via `git mv`, old path physically gone | `ls .opencode/skills/sk-small-model 2>&1` returns no-such-file; `ls .opencode/skills/sk-ai-small-model/SKILL.md` exists; `git log --follow` traces rename history |
| REQ-002 | SKILL.md frontmatter `name:` is `sk-ai-small-model`; H1 and any in-body skill self-references updated | `rg "^name: sk-ai-small-model" .opencode/skills/sk-ai-small-model/SKILL.md` matches; `rg "sk-small-model" .opencode/skills/sk-ai-small-model/` returns no live hits except inside `changelog/v0.1.0.0.md` and `v0.2.0.0.md` (historical version files) |
| REQ-003 | Sibling skill `enhances` edges updated in cli-devin + cli-opencode graph-metadata.json | `jq '.edges.enhances[].target' .opencode/skills/cli-devin/graph-metadata.json` includes `sk-ai-small-model` and excludes `sk-small-model`; same for cli-opencode |
...
| REQ-005 | Compiled `skill-graph.json` regenerated; advisor surfaces `sk-ai-small-model` on small-model dispatch prompts | `jq '.skills | keys[]' .opencode/skills/system-skill-advisor/mcp_server/scripts/skill-graph.json` contains `sk-ai-small-model` and does NOT contain `sk-small-model`; `advisor_recommend` MCP call on a representative small-model prompt returns `sk-ai-small-model` in top-3 |
| REQ-006 | `parent.graph-metadata.json` refreshed via `generate-context.js` so 007 is a registered child and `derived.last_active_child_id` points to it | `jq '.children_ids' …/114/graph-metadata.json` includes `…/007-rename-sk-ai-small-model`; `jq '.derived.last_active_child_id' …` matches `…/007-…` |
...
| REQ-010 | Phase-parent forward note added to `114/spec.md` PHASE DOCUMENTATION MAP without rewriting prior phase rows | `rg "Phase 7 \(007-rename-sk-ai-small-model\)" …/114/spec.md` matches; phases 001-006 row text unchanged from before the rename |
...
- **SC-001**: After rename + reindex, a fresh advisor query for a representative small-model dispatch prompt (`"dispatch swe-1.6 via cli-devin"`) returns `sk-ai-small-model` in top-3 with confidence ≥ 0.7.
...
- **SC-004**: `git log --follow .opencode/skills/sk-ai-small-model/SKILL.md` traces back to the original `sk-small-model/SKILL.md` (rename history preserved).
...
| NFR-004 | Advisor freshness | After `skill_graph_compiler.py --export-json --pretty`, the compiled graph timestamps (`generated_at`) move forward and the new name appears in `skills` keys. | `jq '.generated_at, (.skills \| has("sk-ai-small-model"))'` shows a fresh timestamp + `true` |
<!-- /ANCHOR:nfr -->
...
- Should sk-ai-small-model `changelog/v0.1.0.0.md` and `v0.2.0.0.md` be edited to use the new name? **Decision**: NO — those are historical version notes; add a NEW `v0.3.0.0.md` documenting the rename.
