...
STATE SUMMARY (auto):
Iteration 36 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.66
Active angle: [D4-A5/D4] feeds_design_decision is caller-supplied + defaults False — invert to deny-by-default with a checkable positive exemption token (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D4-A5 / D4] (corpus: local-repo):
feeds_design_decision is caller-supplied + defaults False — invert to deny-by-default with a checkable positive exemption token
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D4-A5.
Last 3 Iterations Summary: run 33: [D4-A2/D4] od CLI Bash bypass: PreToolUse matcher  (0.68); run 34: [D4-A3/D4] the exemption is self-judged + post-hoc (0.63); run 35: [D4-A4/D4] the gate never executes: design_gate/re (0.66)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-036.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-036.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-036.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-036.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
