...
STATE SUMMARY (auto):
Iteration 24 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.62
Active angle: [D3-A7/D3] utilization invisible in live dispatch (014 intentRecall=0) — a mandatory machine-readable mode-declaration token (ROUTED: workflowMode=...) (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D3-A7 / D3] (corpus: local-repo):
utilization invisible in live dispatch (014 intentRecall=0) — a mandatory machine-readable mode-declaration token (ROUTED: workflowMode=...)
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D3-A7.
Last 3 Iterations Summary: run 21: [D3-A4/D3] ambiguous/multi-axis prompt resolution  (0.69); run 22: [D3-A5/D3] negative routing: design taste prompts  (0.64); run 23: [D3-A6/D3] registry completeness + alias-collision (0.62)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-024.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-024.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-024.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-024.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
