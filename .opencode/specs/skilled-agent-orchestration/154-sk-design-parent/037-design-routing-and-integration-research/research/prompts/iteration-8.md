...
STATE SUMMARY (auto):
Iteration 8 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.63
Active angle: [D2-4/D2] /design:* deliverable/output shape unspecified (only STATUS=OK/FAIL) — name the artifact (DESIGN.md, scored report, preflight card, build manifest) (corpus: impeccable-main)
...
THIS ITERATION'S ANGLE [D2-4 / D2] (corpus: impeccable-main):
/design:* deliverable/output shape unspecified (only STATUS=OK/FAIL) — name the artifact (DESIGN.md, scored report, preflight card, build manifest)
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D2-4.
Last 3 Iterations Summary: run 5: [D2-1/D2] /design:* generic arg-hint -> real per-m (0.72); run 6: [D2-2/D2] /design:* commands carry no concrete inv (0.66); run 7: [D2-3/D2] /design:* no when-to-use-vs-sibling disc (0.63)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-008.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-008.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-008.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-008.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
