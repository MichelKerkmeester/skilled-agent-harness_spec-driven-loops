...
STATE SUMMARY (auto):
Iteration 50 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.69
Active angle: [D5-A5/D5] smart-router blindness — add a DESIGN intent to each cli-* router (or ALWAYS baseline) so the design contract loads for a routine design prompt (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D5-A5 / D5] (corpus: local-repo):
smart-router blindness — add a DESIGN intent to each cli-* router (or ALWAYS baseline) so the design contract loads for a routine design prompt
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D5-A5.
Last 3 Iterations Summary: run 47: [D5-A2/D5] model the design manifest on the Gate-3 (0.64); run 48: [D5-A3/D5] the mcp-open-design pairing is parent-r (0.66); run 49: [MON-B9/D6] designer-skills-main residual craft co (0.69)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-050.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-050.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-050.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-050.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
