...
STATE SUMMARY (auto):
Iteration 16 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.62
Active angle: [D2-12/D2] command description as auto-trigger keyword surface — does the agent ever pick a specific /design:* command, or does it collapse to the hub (dead weight)? (corpus: impeccable-main)
...
THIS ITERATION'S ANGLE [D2-12 / D2] (corpus: impeccable-main):
command description as auto-trigger keyword surface — does the agent ever pick a specific /design:* command, or does it collapse to the hub (dead weight)?
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D2-12.
Last 3 Iterations Summary: run 13: [D2-9/D2] no chaining/handoff between /design:* co (0.66); run 14: [D2-10/D2] register (brand vs product) not pinnabl (0.68); run 15: [D2-11/D2] command metadata drift - derive /design (0.62)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-016.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-016.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-016.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-016.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
