...
STATE SUMMARY (auto):
Iteration 27 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.64
Active angle: [D3-A10/D3] a standing routing self-test gold corpus (prompt->expected workflowMode), hint-free + adversarial minimal pairs — the measurement engine (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D3-A10 / D3] (corpus: local-repo):
a standing routing self-test gold corpus (prompt->expected workflowMode), hint-free + adversarial minimal pairs — the measurement engine
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D3-A10.
Last 3 Iterations Summary: run 24: [D3-A7/D3] utilization invisible in live dispatch: (0.61); run 25: [D3-A8/D3] tool-permission correctness across the  (0.6); run 26: [D3-A9/D3] sub-agent/small-model dispatch utilizat (0.64)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-027.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-027.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-027.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-027.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
