...
STATE SUMMARY (auto):
Iteration 42 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.64
Active angle: [D4-A11/D4] temporal decay/freshness binding — proof token TTL + invalidate when brief/subject changes (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D4-A11 / D4] (corpus: local-repo):
temporal decay/freshness binding — proof token TTL + invalidate when brief/subject changes
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D4-A11.
Last 3 Iterations Summary: run 39: [D4-A8/D4] inner-generator context loss: start_run (0.64); run 40: [D4-A9/D4] sub-agent/Task delegation laundering: p (0.66); run 41: [MON-B9/D6] designer-skills-main residual build/vi (0.64)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-042.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-042.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-042.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-042.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
