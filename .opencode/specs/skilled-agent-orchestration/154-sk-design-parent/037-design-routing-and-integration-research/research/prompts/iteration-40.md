...
STATE SUMMARY (auto):
Iteration 40 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.64
Active angle: [D4-A9/D4] sub-agent/Task delegation laundering — the proof token must travel in the dispatch payload + re-validate at the child's tool boundary (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D4-A9 / D4] (corpus: local-repo):
sub-agent/Task delegation laundering — the proof token must travel in the dispatch payload + re-validate at the child's tool boundary
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D4-A9.
Last 3 Iterations Summary: run 37: [MON-B3/D6] designer-skills-main 9-plugin command  (0.71); run 38: [MON-B3/D6] designer-skills-main command grammar l (0.62); run 39: [D4-A8/D4] inner-generator context loss: start_run (0.64)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-040.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-040.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-040.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-040.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
