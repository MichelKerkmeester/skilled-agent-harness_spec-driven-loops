...
STATE SUMMARY (auto):
Iteration 44 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.67
Active angle: [D4-A13/D4] HTTP/fourth-surface escape — only a daemon-side precondition on start_run covers all of CLI/MCP/HTTP/Skills (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D4-A13 / D4] (corpus: local-repo):
HTTP/fourth-surface escape — only a daemon-side precondition on start_run covers all of CLI/MCP/HTTP/Skills
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D4-A13.
Last 3 Iterations Summary: run 41: [MON-B9/D6] designer-skills-main residual build/vi (0.64); run 42: [D4-A11/D4] temporal decay/freshness binding - pro (0.62); run 43: [D4-A12/D4] headless automation time-bomb - od aut (0.67)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-044.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-044.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/iterations/iteration-044.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/154-sk-design-parent/037-design-routing-and-integration-research/research/deltas/iter-044.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
