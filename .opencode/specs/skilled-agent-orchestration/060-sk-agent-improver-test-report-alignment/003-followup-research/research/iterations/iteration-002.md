...
- **Ruled out: reusing the 059 Coder Acceptance Rubric unchanged.** Its dimensions grade code implementation output; 063 needs to grade command entrypoint fidelity, artifact/journal truth, producer/consumer compatibility, governance stops, and sandbox containment [SOURCE: `.opencode/agents/code.md:128-140`, `.opencode/commands/improve/agent.md:266-280`].
- **Ruled out: event-name-only PASS.** A journal can contain `legal_stop_evaluated` while reducer-visible gate results remain empty if the event details are not nested under `gateResults` [SOURCE: `.opencode/commands/improve/assets/improve_improve-agent_auto.yaml:192-198`, `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:213-218`].
