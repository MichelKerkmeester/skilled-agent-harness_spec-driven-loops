...
_memory:
  continuity:
    ...
    completion_pct: 5
    open_questions:
      - "Concrete diff sketches to land in 064 (the actionable code-edit packet)"
      - "Whether 063 should reuse CP-040..CP-045 IDs or create CP-046..CP-051 variants"
...
Phase 003 takes those two artifacts (R1 scores + meta-finding) as the input to 10 cli-copilot deep-research iterations and produces concrete recommendations for: (a) how to actually improve sk-improve-agent further given what R1 showed, (b) how packet 063 should structure its command-flow stress tests, (c) whether other meta-agents in the system need the same treatment, (d) how to generalize the test-layer-selection lesson into a reusable testing template.
...
- What 063 should look like operationally (not just "it should test command flow" but "here's the exact dispatch shape and grep contract")
...
- Synthesize 10 iterations into `research/research.md` with: gap analysis (per-RQ), 063 packet sketch, generalization recommendations, reusable rubric
- Hand-off notes to packets 063 (command-flow stress tests) and 064 (further sk-improve-agent edits if the research supports them)
...
- Designing 063's playbook entries (that's 063's job; 003 sketches the approach)
...
2. `research/research.md` synthesis with: §1 Executive Summary, §2 Methodology, §3 Per-RQ Findings, §4 063 Packet Sketch, §5 Other Meta-Agent Audit, §6 Reusable Rubric Template, §7 Hand-off Notes
