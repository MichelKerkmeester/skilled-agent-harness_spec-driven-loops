...
However, the auto YAML emits `gate_evaluation` with `gateName: "stop_check"`, `gateResult`, and `stopReason`, not a `legal_stop_evaluated` row with the five named gate bundles (`.opencode/commands/improve/assets/improve_improve-agent_auto.yaml:189-191`). The confirm YAML similarly emits `gate_evaluation` for `gate_after_score`, and only confirm mode has a promotion command path (`.opencode/commands/improve/assets/improve_improve-agent_confirm.yaml:226-232`). Therefore a grep-only stress test should not just grep for `gate_evaluation`; it should require explicit `legal_stop_evaluated` or `blocked_stop` rows containing all five gate keys. As currently wired, the five gates are policy text and reducer-compatible if present, but not reliably grep-checkable from every normal journal run.
...
- **"Skill load fires scripts"** is ruled out. The command first only reads `SKILL.md` (`.opencode/commands/improve/agent.md:238-242`); script execution is later and explicit (`agent.md:244-255`).
...
For `.opencode/commands/improve/agent.md` under `## 4. WORKFLOW STEPS`, make the stress-test discipline explicit.
...
For `.opencode/commands/improve/assets/improve_improve-agent_auto.yaml`, make legal-stop output grep-checkable.
