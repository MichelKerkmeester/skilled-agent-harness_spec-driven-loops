...
I grepped the triad plus references/assets for `Critic`, `critic`, `challenge`, `red-team`, and `red team`; there were no matches. The current active safeguards are proposal-only constraints, deterministic scoring, and violation recovery (`.opencode/commands/improve/agent.md:420-455`), but none forces a candidate-time adversarial challenge like "argue this proposal is overfitting the scorer" before returning.
