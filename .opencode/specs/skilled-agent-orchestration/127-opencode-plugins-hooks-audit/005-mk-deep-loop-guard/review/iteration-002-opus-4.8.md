...
|----------|---------|--------------|----------|-------------------|
| F1 | P1 | **confirmed** | P1 | Confirmed at mk-deep-loop-guard.js:81 (map.set(mode.agent, mode)). mode-registry.json maps agent 'deep-improvement' from FOUR modes (agent-improvement:103, model-benchmark:129, skill-benchmark:152, ai-system-improvement:175); last-writer-wins leaves registry.get('deep-improvement').workflowMode === 'ai-system-improvement'. Check 1 (line 391) then false-mismatches any dispatch that declares mode=agent-improvement\|model-benchmark\|skill-benchmark against target deep-improvement — a false warn always, and a false BLOCK under MK_DEEP_LOOP_GUARD_REJECT=1. Defeats the mismatch guard for 3 of 4 improvement modes. Fix (agent→Set of modes) is correct. |
