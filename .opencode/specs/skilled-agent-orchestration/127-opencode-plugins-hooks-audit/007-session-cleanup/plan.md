---
title: "Implementation Plan: session-cleanup audit"
description: "The read-only audit approach for session-cleanup and its Claude hook version."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Executed read-only audit of session-cleanup"
    ...
    answered_questions: []
---
# Implementation Plan: session-cleanup audit
...
### Overview
Dispatch one read-only GPT-5.6-Sol-Fast auditor over `.opencode/plugins/session-cleanup.js` plus its Claude hook version, capturing structured findings on correctness, parity, and refinements.
<!-- /ANCHOR:summary -->
...
|---------|--------------|--------|--------------|
| `.opencode/plugins/session-cleanup.js` | Session-end MCP cleanup plugin | audited (no change) | `review/review-report.md` (10 findings) |
<!-- /ANCHOR:affected-surfaces -->
