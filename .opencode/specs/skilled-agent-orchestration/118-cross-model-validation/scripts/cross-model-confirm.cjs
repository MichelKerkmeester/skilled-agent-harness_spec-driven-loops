 ...
 * from 114/003 × 7 fixtures from 114/002 × N models (default: 2). Scores
 * each dispatch via 114/003/score-variant.cjs (which auto-extracts files
 * via 116/extract-files-from-markdown.cjs when EVAL_LOOP_EXTRACT=true).
 *
...
// Reference scores from prior runs on SWE-1.6 (114/003 + 116/v2)
...
function buildAnalysis(rows, models, variants, fixtures) {
  ...
  lines.push('- Grader: claude-sonnet-4-5 (matches 114/003 baseline). Grader cache shared with prior runs via sha256(swe16_output_text) key, so re-dispatches of identical outputs incur no grader cost.');
  lines.push('- Per-fixture seed snapshot/restore is active via 114/003/score-variant.cjs `snapshotDir`+`restoreFromSnapshot` helpers when `EVAL_LOOP_EXTRACT=true`.');
