...
description: "Plans the packet-local confirm harness for validating held 114/003 findings on deepseek-v4-pro and kimi-k2.6. The implementation has not started yet."
trigger_phrases:
...
### Overview
Packet 118 will add a confirm-only harness that dispatches 114/003 variants against 114/002 fixtures on two model routes. The harness will reuse 116 extraction and 114/003 scoring so the final analysis can decide whether held findings from packet 117 should propagate cross-CLI.
<!-- /ANCHOR:summary -->
...
- [ ] Packet 118 implementation summary is updated from planning state
<!-- /ANCHOR:quality-gates -->
...
### Key Components
- **Fixture source**: `.opencode/specs/skilled-agent-orchestration/114-cli-devin-swe16-prompt-optimization/002-eval-rig/fixtures`
- **Variant source**: `.opencode/specs/skilled-agent-orchestration/114-cli-devin-swe16-prompt-optimization/003-eval-loop/variants`
- **Dispatch surface**: cli-opencode for both `deepseek/deepseek-v4-pro --variant high` and `opencode-go/kimi-k2.6 --variant high`
- **Extraction**: `extract()` from `.opencode/specs/skilled-agent-orchestration/116-cli-devin-extraction-rerun/scripts/extract-files-from-markdown.cjs`
- **Scoring**: `scoreVariantFixture()` from `.opencode/specs/skilled-agent-orchestration/114-cli-devin-swe16-prompt-optimization/003-eval-loop/scripts/score-variant.cjs`
...
|---------|--------------|--------|--------------|
| Packet 118 scripts | No harness exists yet | Create `cross-model-confirm.cjs` | Node syntax check and dry-run |
...
| Packet 116 extraction | Converts markdown output to fixture files | Reuse `extract()` | Extraction status in result rows |
...
| claude-sonnet-4-5 grader | External grader | Planned | Scores lose parity with 114/003 |
| 114/002 fixtures | Internal evidence | Available | Matrix cannot match SWE 1.6 baseline |
| 114/003 variants and scoring | Internal evidence | Available | Variant comparisons cannot match prior run |
| 116 extraction | Internal evidence | Available | D1 acceptance extraction is weaker or duplicated |
<!-- /ANCHOR:dependencies -->
...
- **Procedure**: Revert packet-local harness, state, and analysis files; leave packet 117 unchanged and keep held findings unpropagated.
<!-- /ANCHOR:rollback -->
...
- [ ] Harness writes only under packet 118
...
4. Keep packet 117 held findings unpropagated.
...
114/002 fixtures ─────┐
114/003 variants ─────┤
114/003 scoring ──────┼──> cross-model-confirm.cjs ──> result matrix ──> decision analysis
116 extraction ───────┤
cli-opencode routes ──┘
```
...
| Extraction and scoring | 116 and 114/003 modules | D1 and weighted scores | Decision gates |
