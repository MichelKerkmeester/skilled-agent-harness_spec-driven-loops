...
const dispatchModule = require('./dispatch-minimax.cjs');
...
async function scoreVariantOnFixtures(variantPath, variantId, variantSig, opts) {
  ...
  let parallelWave = config.target_model.max_concurrent || 3;
...
async function main() {
  ...
  for (let run = (initialState.filter((r) => r.type === 'iteration').length) + 1; run <= maxIters; run++) {
    ...
    const proposal = mutateModule.proposeNextVariant({
      bestVariant: bestVariant || { meta: { framework: 'RCAF', preplanning_density: 'medium', thinking_threshold: '5' } },
      noImprovementCount,
    });
    if (!proposal) {
