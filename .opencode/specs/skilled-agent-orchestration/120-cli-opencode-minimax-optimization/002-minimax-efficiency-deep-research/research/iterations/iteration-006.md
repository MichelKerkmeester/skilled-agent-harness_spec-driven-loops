...
- Read `sk-prompt-small-model/SKILL.md` and `graph-metadata.json`; trigger phrases already include MiniMax, but the dispatch matrix and `cli-opencode` ownership text still omit MiniMax. [SOURCE: .opencode/skills/sk-prompt-small-model/SKILL.md] [SOURCE: .opencode/skills/sk-prompt-small-model/graph-metadata.json]
...
### F4 - sk-prompt-small-model needs matrix/ownership reconciliation, not new trigger phrases
...
- Leave runtime logic out of `sk-prompt-small-model`; the skill remains a sentinel/index.
...
6. `sk-prompt-small-model/SKILL.md`: add MiniMax to the dispatch matrix and ownership text.
7. `sk-prompt-small-model/references/pattern-index.md`: only add a link-only MiniMax policy pointer if the cli-opencode prompt-quality note is implemented.

No change is needed to `sk-prompt-small-model/graph-metadata.json` in this follow-on packet.
