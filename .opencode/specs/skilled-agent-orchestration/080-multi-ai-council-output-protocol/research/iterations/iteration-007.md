...
- Inspected the current multi-ai-council agent state contract in `.opencode/agent/multi-ai-council.md`.
- Compared the reference state schema in `.opencode/skill/system-spec-kit/references/multi-ai-council/state-format.md` with the real packet 080 smoke-test state log.
...
The current council state schema does not use a `type` discriminator. The agent body documents five event shapes at `.opencode/agent/multi-ai-council.md:621` through `.opencode/agent/multi-ai-council.md:626`, and all examples at `.opencode/agent/multi-ai-council.md:629` through `.opencode/agent/multi-ai-council.md:634` are bare `{"event":...}` JSONL records. The reference state-format file repeats the same shape at `.opencode/skill/system-spec-kit/references/multi-ai-council/state-format.md:7` through `.opencode/skill/system-spec-kit/references/multi-ai-council/state-format.md:43`.
...
The formal v1 schema is intentionally small. The agent body lists `tokens?`, `convergence_score?`, `new_findings_count?`, and `convergence?` as optional fields at `.opencode/agent/multi-ai-council.md:623` through `.opencode/agent/multi-ai-council.md:626`. The reference file has the same narrow contract at `.opencode/skill/system-spec-kit/references/multi-ai-council/state-format.md:15` through `.opencode/skill/system-spec-kit/references/multi-ai-council/state-format.md:43`.
...
This mirrors the deep-research state-format approach. Its convergence fields are optional and explicitly backward compatible: old records without the field parse normally at `.opencode/skill/deep-research/references/state_format.md:152` through `.opencode/skill/deep-research/references/state_format.md:167`.
...
Existing v1 rows omit all of these fields. New v1.1 emitters may add `schema_version:"1.1"` and `protocol:"multi-ai-council"` to each new line, but they must not rewrite old lines. This keeps the append-only invariant from `.opencode/agent/multi-ai-council.md:637` intact.
...
Deep-research parses JSONL line by line, collects valid records, and records corruption warnings for malformed lines rather than assuming every line has the latest shape. The parser behavior is visible in `.opencode/skill/deep-research/scripts/reduce-state.cjs:86` through `.opencode/skill/deep-research/scripts/reduce-state.cjs:109`. Deep-review exposes the same backward-compatible parser contract at `.opencode/skill/deep-review/scripts/reduce-state.cjs:107` through `.opencode/skill/deep-review/scripts/reduce-state.cjs:125`.
