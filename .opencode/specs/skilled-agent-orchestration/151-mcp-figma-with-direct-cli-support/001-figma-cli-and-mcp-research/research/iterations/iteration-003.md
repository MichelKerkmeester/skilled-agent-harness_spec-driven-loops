...
7. **INTEGRATION POINTS** — Bash owns figma-cli; Code Mode owns optional MCP; sk-interface-design owns design judgment; sk-code owns adapting exports into code; chrome-devtools for browser preview only.
...
## RULES (full)
**ALWAYS:** (1) verify binary on PATH (figma-cli → figma-ds-cli → `--version`/`--help`); (2) require Figma Desktop running; (3) confirm daemon status before daemon-backed commands; (4) prefer `connect --safe` for first setup; (5) gate every destructive verb (confirm + explicit target + command preview + rollback); (6) treat eval/raw/run as arbitrary mutation; (7) apply sk-interface-design when a read/export feeds a design decision; (8) keep official MCP optional/opt-in (no silent CLI→MCP switch).
...
- **depends_on:** sk-interface-design (0.7, design judgment when reads/exports feed a decision); mcp-code-mode (0.45, optional MCP via Code Mode).
