...
- True coverage gaps include:
  - `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/settings.py:68` hardcodes `assets/motion_dev/**`.
  - `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/lib/scorer/lanes/explicit.ts:70` and `skill_advisor.py:1358` still route on `webflow`.
  - `.opencode/skill/system-spec-kit/scripts/.folder-list.txt:17` and `.scan-lines.txt:17` contain `00--anobel.com` paths.
  - `.opencode/skill/mcp-code-mode/references/workflows.md:106` contains `A. Nobel & Zn`, which the original regex missed.
...
- `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/lib/scorer/lanes/explicit.ts:69`
- `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py:1337`
...
- `.opencode/skill/cli-opencode/README.md:128`
- `.opencode/skill/cli-opencode/references/opencode_tools.md:52`
...
### `.opencode/skill/mcp-code-mode/references/naming_convention.md`
Verdict: **PASS**.
...
### `.opencode/skill/mcp-code-mode/manual_testing_playbook/06--third-party-via-cm/002-myservice-list-sites.md`
Verdict: **FAIL**.
...
### `.opencode/skill/mcp-coco-index/SKILL.md` and `mcp_server/cocoindex_code/settings.py`
Verdict: **FAIL**.

The docs now use `.opencode/skill/*/assets/<library>/**`, which conveys the generic library-specific subfolder pattern. The executable default still hardcodes `.opencode/skill/*/assets/motion_dev/**` in `settings.py:68`, so the implementation did not follow the doc cleanup.

### `.opencode/skill/sk-code-review/README.md` and `SKILL.md`
Verdict: **PASS**.
...
- `.opencode/skill/mcp-code-mode/manual_testing_playbook/07--recovery-and-config/002-disabled-server-omitted.md`: PASS.
- `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/graph-metadata.json`: PASS.
- `.opencode/skill/system-spec-kit/references/structure/folder_structure.md`: PASS.
- `.opencode/skill/mcp-chrome-devtools/examples/README.md`: FAIL, dead links to old `sk-code/references/debugging` and `sk-code/references/implementation` paths.
- `.opencode/skill/cli-opencode/SKILL.md`: PASS in sampled region.
...
`git show --stat 61004e357 -- .opencode/skill/sk-code` returned no changes.
...
- `specs/skilled-agent-orchestration/071-stack-agnostic-cleanup`: PASS.
...
|---|---|---:|---|---|---|
| V-001 | P0 | 1 | `.opencode/skill/mcp-coco-index/mcp_server/cocoindex_code/settings.py:68` | Non-`sk-code` executable default hardcodes `assets/motion_dev/**` and comments `motion.dev`. | Replace with a generic canonical assets pattern or configurable library-asset default, then update tests/docs together. |
| V-002 | P0 | 1 | `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/lib/scorer/lanes/explicit.ts:70` | system-spec-kit advisor source still contains `webflow` route keywords and WEBFLOW/OPENCODE surface taxonomy comments outside `sk-code`. | Move stack-specific routing cues into `sk-code` metadata or rename them to generic code-surface examples owned by the advisor. |
| V-003 | P0 | 1 | `.opencode/skill/system-spec-kit/scripts/.folder-list.txt:17` | Tracked scan/cache artifacts and tests still leak `00--anobel.com` paths. | Remove generated scan artifacts from tracked content or regenerate with neutral fixtures; update tests using generic fixture paths. |
| V-004 | P0 | 2 | `.opencode/skill/mcp-code-mode/references/workflows.md:106` | MyService workflow expected logs still mention `A. Nobel & Zn`, a real codebase/client name missed by the regex. | Replace with a neutral example name such as `Example Corp`; broaden future searches to `nobel|a[ ._-]*nobel`. |
| V-005 | P0 | 1 | `.opencode/skill/cli-opencode/README.md:128` | Changed cli-opencode docs still hardcode `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public`. | Replace local paths with `$REPO_ROOT`, `/path/to/repo`, or a temp sandbox path in all changed cli-opencode examples. |
| V-006 | P1 | 2 | `.opencode/skill/mcp-code-mode/manual_testing_playbook/06--third-party-via-cm/002-myservice-list-sites.md:44` | MyService manual test uses `myservice.myservice_list_sites()` while the naming docs teach `myservice.myservice_sites_list()`. | Align the scenario with the documented placeholder API name and re-check all MyService examples for consistency. |
| V-007 | P2 | 2 | `.opencode/skill/mcp-chrome-devtools/examples/README.md:123` | Example README links to old `sk-code/references/debugging` and `references/implementation` paths that no longer exist. | Point links at existing `sk-code/references/webflow/...` paths or remove surface-specific cross-links from this agnostic skill. |
