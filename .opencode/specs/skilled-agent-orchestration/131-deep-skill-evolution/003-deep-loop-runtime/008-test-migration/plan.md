...
- **Shared invocation helper**: small TS helper (e.g., `tests/helpers/spawn-cjs.ts`) that wraps `child_process.spawn` and returns `{ exit, stdout, stderr, parsedJson }`. All 4 `*-script.vitest.ts` files use it.
...
- [ ] Author `tests/helpers/spawn-cjs.ts` (shared invocation helper)
