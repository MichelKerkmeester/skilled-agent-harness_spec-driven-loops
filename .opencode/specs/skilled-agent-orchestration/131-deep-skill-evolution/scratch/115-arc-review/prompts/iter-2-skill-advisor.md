...
7. `.opencode/skills/sk-ai-council/graph-metadata.json` + `sk-ai-small-model/graph-metadata.json`
8. 4 sibling skill graph-metadata.json (deep-research, deep-agent-improvement, system-spec-kit, system-skill-advisor)

### Checks
1. Compiled skill-graph.json: `families.sk-util` includes `sk-ai-council` + `sk-ai-small-model`. `families.deep-loop` does NOT include either.
2. `adjacency.sk-ai-council` + `adjacency.sk-ai-small-model` both present with correct edges.
