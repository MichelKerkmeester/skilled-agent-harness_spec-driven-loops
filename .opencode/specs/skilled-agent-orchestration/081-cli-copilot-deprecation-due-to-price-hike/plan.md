...
The cli-copilot skill is a self-contained directory under `.opencode/skill/cli-copilot/` with ~50+ playbook scenarios, references, scripts, and assets. It is wired into the repository through six surfaces:
...
5. **Routing docs** — `CLAUDE.md`, `AGENTS.md`, root `README.md`, `DEPLOYMENT.md`, `.opencode/skill/README.md`, `install_guides/`.
...
- `rm -rf .opencode/skill/cli-copilot/`
- `rm -rf .opencode/changelog/cli-copilot/`
- `rm -f .github/hooks/spec-kit-copilot-hook.sh`
- `rm -rf .opencode/skill/system-spec-kit/mcp_server/hooks/copilot/`
- `rm -f .opencode/skill/system-spec-kit/mcp_server/matrix_runners/adapter-cli-copilot.ts`
- `rm -f .opencode/skill/system-spec-kit/mcp_server/tests/matrix-adapter-copilot.vitest.ts`
- `rm -f .opencode/skill/system-spec-kit/feature_catalog/16--tooling-and-scripts/36-copilot-target-authority-helper.md`
- `rm -f .opencode/skill/system-spec-kit/feature_catalog/18--ux-hooks/21-shared-provenance-and-copilot-compact-cache-parity.md`
- `rm -f .opencode/skill/system-spec-kit/manual_testing_playbook/18--ux-hooks/274-shared-provenance-and-copilot-compact-cache-parity.md`
- `rm -f .opencode/skill/deep-research/manual_testing_playbook/03--iteration-execution-and-state-discipline/030-cli-copilot-target-authority-dispatch.md`
- `rm -f .opencode/skill/deep-review/manual_testing_playbook/03--iteration-execution-and-state-discipline/016-cli-copilot-target-authority-dispatch.md`

**Batch B2 — Skill advisor scoring (single cli-codex agent, file-locked)**

Edit (precision required):
- `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py` (~12 lines: TOKEN_BOOSTS["copilot"], PHRASE_BOOSTS keys ending in "copilot"/"cli-copilot", routing-table entries listing cli-copilot among delegate/opinion/validate options, command-bridge tiebreaker prose, AMBIGUITY_DELTA fixtures)
- `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/scripts/skill-graph.json` (cli-copilot node at line 9 + every related_to edge listing cli-copilot at lines 41/49/66/74/157/171/206/378 + the cli-copilot block at line 54)
- `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/graph-metadata.json` (one edge entry on line 13)
- `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/scripts/check-prompt-quality-card-sync.sh` (drop the cli-copilot path from the SYNC_PATHS array)
- `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/scripts/fixtures/skill_advisor_regression_cases.jsonl` (delete the P1-CLI-004 line)
...
- Agent B4a: `.opencode/agent/multi-ai-council.md`
...
- Agent B4e: `.opencode/command/spec_kit/{deep-research.md,deep-review.md,assets/spec_kit_deep-research_auto.yaml}` (single agent, three files in same skill area)
...
- `CLAUDE.md`, `AGENTS.md`, `README.md`, `DEPLOYMENT.md`, `.opencode/skill/README.md`, `.opencode/install_guides/README.md`, `.opencode/install_guides/SET-UP - AGENTS.md`.
...
- Agent B6a: `.opencode/skill/system-spec-kit/feature_catalog/16--tooling-and-scripts/37-cli-matrix-adapter-runners.md` + `.opencode/skill/system-spec-kit/manual_testing_playbook/16--tooling-and-scripts/280-cli-matrix-adapter-runner-smoke.md` + `.opencode/skill/system-spec-kit/mcp_server/matrix_runners/{run-matrix.ts,matrix-manifest.json,README.md}` (matrix-runner cluster).
...
- Run `python3 .opencode/skill/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py --rebuild` (or equivalent MCP tool).
...
|------------|-------|---------|
| `cli-codex` skill | `.opencode/skill/cli-codex/` | Dispatcher for B2..B6 |
| `validate.sh` | `.opencode/skill/system-spec-kit/scripts/spec/validate.sh` | Strict spec validation |
| Skill advisor | `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/` | Re-index target |
...
| `generate-context.js` | `.opencode/skill/system-spec-kit/scripts/dist/memory/` | Canonical save |
...
3. The cli-copilot skill content remains accessible in git history forever; re-creation is `git checkout HEAD~ -- .opencode/skill/cli-copilot/`.
...
4. **Catastrophic — full revert wanted**: `git revert <packet-commit-range>` creates new commits undoing the deletion. Re-creation of cli-copilot skill content from `git checkout HEAD~ -- .opencode/skill/cli-copilot/`.
