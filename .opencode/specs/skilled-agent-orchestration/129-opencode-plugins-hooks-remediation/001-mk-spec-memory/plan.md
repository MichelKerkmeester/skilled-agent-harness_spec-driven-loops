---
title: "Implementation Plan: mk-spec-memory remediation"
description: "The fix approach for the 19 mk-spec-memory findings, grouped by severity."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Drafted fix approach for mk-spec-memory"
    ...
    answered_questions: []
---
# Implementation Plan: mk-spec-memory remediation
...
### Overview
Apply 19 fixes to `mk-spec-memory` and its Claude hook version, ordered P1 -> P2 -> refinement, each verified against its finding. Capture a green test baseline first.
<!-- /ANCHOR:summary -->
...
### Key Components
- **Plugin entrypoint**: `.opencode/plugins/mk-spec-memory.js`
- **Affected surfaces**: 7 file(s) across plugin, bridge, and Claude hook
...
| `.opencode/skills/system-spec-kit/mcp_server/hooks/claude/session-stop.ts` | audited surface | apply fixes | finding no longer reproduces; tests green |
| `.opencode/skills/system-spec-kit/mcp_server/hooks/claude/compact-inject.ts` | audited surface | apply fixes | finding no longer reproduces; tests green |
| `.opencode/skills/system-spec-kit/mcp_server/hooks/claude/user-prompt-submit.ts` | audited surface | apply fixes | finding no longer reproduces; tests green |
| `.opencode/plugins/mk-spec-memory.js` | audited surface | apply fixes | finding no longer reproduces; tests green |
| `.claude/settings.json` | audited surface | apply fixes | finding no longer reproduces; tests green |
| `.opencode/skills/system-spec-kit/mcp_server/plugin_bridges/spec-kit-opencode-message-schema.mjs` | audited surface | apply fixes | finding no longer reproduces; tests green |
<!-- /ANCHOR:affected-surfaces -->
...
- [ ] Apply 9 P1 fixes (see tasks.md Phase 2)
- [ ] Apply 6 P2 fixes
- [ ] Apply 4 refinements or log as follow-ups
