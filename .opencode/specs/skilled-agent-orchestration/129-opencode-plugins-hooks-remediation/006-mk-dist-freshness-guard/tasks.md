...
<!-- ANCHOR:phase-1 -->
## Phase 1: Setup
- [x] T001 Capture a green baseline of the mk-dist-freshness-guard test suite before any change
    - Evidence: baseline captured; final plugin suite runs 188/189 (the single fail is the pre-existing `mk-goal-tool-path` deep-loops path artifact, unrelated to this packet).
- [x] T002 Confirm each targeted finding reproduces against current code
    - Evidence: findings F1-F7 + O1-O5 confirmed against real code during the two-model fix-design pass (`fix-design/fix-design.md`).
<!-- /ANCHOR:phase-1 -->
...
- [x] T003 [P1 (GPT P1 / Opus P2)] A source-hash mismatch can be silently blessed as fresh (`.opencode/skills/system-spec-kit/scripts/lib/dist-freshness.cjs:430`)
    - IMPLEMENTED (F1): build-attestation activated — `scripts/finalize-dist.mjs` now records `origin:'build'` via `recordPackageBuild`, wired through `package.json` `prepare-build`. Verified `origin:build` in the dist freshness cache; build-origin hash mismatch => stale, checker/absent origin keeps the load-bearing mtime fallback.
...
- [x] T004 [P1 (GPT P1 / Opus P2)] Checker errors are reported as fresh and suppressed by both consumers (`.opencode/skills/system-spec-kit/scripts/lib/dist-freshness.cjs:354`)
    - IMPLEMENTED (F2): `error` is now a first-class status distinct from fresh; `check-dist-staleness.sh` gained a `status=='error'` branch that surfaces a bounded, non-blocking CHECK ERROR line (still exit 0) alongside the lib-level aggregation.
...
- [x] T005 [P1 (GPT P1 / Opus P2)] OpenCode can retain a fresh cache for two minutes after a source edit (`.opencode/plugins/mk-dist-freshness-guard.js:109`)
    - IMPLEMENTED (F3, already-shipped): `mk-dist-freshness-guard.js` invalidates the stale cache in `tool.execute.before` for editor tools targeting a watched source, so the next system-transform recomputes from post-write disk state (OpenCode<->Claude parity).
...
- [x] T006 [P1 (GPT P1 / Opus P2)] Sequential subprocess timeouts exceed the enclosing Claude hook timeout (`.opencode/skills/sk-code/code-quality/scripts/hooks/claude-posttooluse.sh:68`)
    - IMPLEMENTED (F4, already-shipped): `claude-posttooluse.sh` enforces a single shared monotonic deadline under the 10s hook budget, passing only the remaining budget to each subprocess so total wall time stays < 10s.
...
- [x] T007 [P1 (GPT P1 / Opus P2)] Valid but malformed Claude hook JSON can crash the fail-safe hook (`.opencode/skills/sk-code/code-quality/scripts/hooks/claude-posttooluse.sh:47`)
    - IMPLEMENTED (F5, already-shipped): `claude-posttooluse.sh` type-validates the root object, `tool_input`, `file_path`, and `cwd` (defaulting `cwd` to `os.getcwd()`) and wraps `main()` in a top-level fail-open boundary so any malformed shape still exits 0.
...
- [x] T008 [P2] Standalone checker fallback resolves one directory too shallow (`.opencode/skills/sk-code/code-quality/scripts/check-dist-staleness.sh:17`)
    - IMPLEMENTED (F6): fixed the fallback parent traversal from 4 to 5 levels in `check-dist-staleness.sh`, so the appended path reaches the workspace root and no longer produces `.opencode/.opencode/...`.
...
- [x] T009 [P2 · Opus-new] Coverage divergence: Claude warns only for the edited file's package; OpenCode warns for ALL stale packages (`.opencode/skills/system-spec-kit/scripts/lib/dist-freshness.cjs:524`)
    - IMPLEMENTED (O1): added a `--all` mode to `check-dist-staleness.sh` plus a Claude SessionStart registration for cross-package coverage, and a coverage-contract docstring documenting the PostToolUse edited-file-scope as intentional (mirrors OpenCode's two-trigger model).
...
- [x] T010 [refinement] Session deduplication and the audit log have no retention bounds (`.opencode/plugins/mk-dist-freshness-guard.js:88`)
    - IMPLEMENTED (F7, already-shipped): `mk-dist-freshness-guard.js` handles `session.deleted`/disposal events to evict/clear the session Set and adds size-bounded log rotation to `appendGuardLog`, all fail-open.
...
- [ ] T011 [RECLASSIFIED: by-design] Risky-bash trigger refreshes the cache but surfaces the warning one turn late (`.opencode/plugins/mk-dist-freshness-guard.js:117`)
    - DISPOSITION: by-design, no code change (confirmed this session). `tool.execute.before` exposes only `{args}`; there is no warning channel and stdout is forbidden by the TUI-safety invariant. The next-turn, non-blocking refresh is the intended contract. Left unchecked because no fix task remains.
...
- [x] T012 [refinement · Opus-new] Freshness cache is never anchored to dist/build identity, so dist reverts/tampering with unchanged source read as fresh (`.opencode/skills/system-spec-kit/scripts/lib/dist-freshness.cjs:430`)
    - IMPLEMENTED (O3, already-shipped): the cache in `dist-freshness.cjs` is anchored to dist identity (distMtime/distSize alongside sourceHash, origin field); the fast path requires both source and dist identity to match, else falls through to the load-bearing mtime path.
    - Source: Opus iteration-2 (new) · fix-design: both models agree
...
- [x] T013 [refinement · Opus-new] Orphan .tmp cache files accumulate on crash between write and rename (`.opencode/skills/system-spec-kit/scripts/lib/dist-freshness.cjs:267`)
    - IMPLEMENTED (O4, already-shipped): `writeStoredSourceHash` in `dist-freshness.cjs` best-effort sweeps stale `.dist-freshness-*.*.tmp` siblings older than a short age threshold, keeping the same-dir atomic write+rename (no EXDEV regression).
    - Source: Opus iteration-2 (new) · fix-design: both models agree
...
- [ ] T014 [refinement · Opus-new] '.json' in SOURCE_EXTENSIONS makes data/config JSON count as watched sources, risking false stale warnings (`.opencode/skills/system-spec-kit/scripts/lib/dist-freshness.cjs:15`)
    - DISPOSITION: by-design, no code change (confirmed this session). Dropping `.json` from SOURCE_EXTENSIONS was rejected: compiled code genuinely consumes build-input JSON (e.g. advisor prompt-policy data, routing-prototypes/ground-truth JSON copied by finalize-dist), so removing it would create false-fresh regressions. `.json` stays watched. Left unchecked because no fix task remains.
    - Source: Opus iteration-2 (new) · fix-design: both models agree
...
- [x] T015 Re-run the mk-dist-freshness-guard test suite; confirm green
...
- [x] T016 Verify each fixed finding no longer reproduces
    - Evidence: F1 verified via `origin:build` present in the dist freshness cache; F2/F6/O1 verified via `check-dist-staleness.sh` behavior; already-shipped F3/F4/F5/F7/O3/O4 carry their own in-suite coverage.
- [x] T017 Verify OpenCode<->Claude parity for this plugin
    - Evidence: F2/O1 deliberately touch both surfaces; F3 closes the OpenCode cache-invalidation gap against Claude's per-edit check; F4/F5/F6 harden the Claude hook to plugin parity.
<!-- /ANCHOR:phase-3 -->
...
<!-- ANCHOR:completion -->
## Completion Criteria
- [x] All P1 tasks `[x]` (T003-T007: F1-F5 all implemented)
- [x] P2 + refinements applied or deferred with rationale (T008/T009/T012/T013 implemented; T011/T014 by-design, no code change)
- [x] Plugin tests green; no `[B]` blocked tasks (188/189; the 1 fail is a pre-existing unrelated path artifact)
<!-- /ANCHOR:completion -->
