...
description: "Phase parent for the spec-kit `:auto` setup-resolution contract: 001 introduced the three-tier flow for /spec_kit:deep-review; 002 generalizes it to all 12 user-invokable command surfaces."
trigger_phrases:
...
_memory:
  continuity:
    packet_pointer: "skilled-agent-orchestration/103-spec-kit-auto-mode-noninteractive-contract"
...
Phase 1 (001, ex-028) introduced and verified the contract for `/spec_kit:deep-review`. Phase 2 (002) lifts the contract into a shared reference doc and migrates the remaining 11 `:auto` commands across `/spec_kit/`, `/create/`, and `/improve/` to cite it, with full live `:auto` dispatch verification per command.

## 3. Scope

In scope:
- Author the three-tier `:auto` contract for `/spec_kit:deep-review` (Phase 1, completed pre-rename as packet 028).
...
- Origin: `.opencode/specs/skilled-agent-orchestration/102-sk-doc-skill-readme-and-structure/004-sk-doc-playbook-markdown-agent-coverage/implementation-summary.md` §Known Limitations (F-Stage-E-001)
