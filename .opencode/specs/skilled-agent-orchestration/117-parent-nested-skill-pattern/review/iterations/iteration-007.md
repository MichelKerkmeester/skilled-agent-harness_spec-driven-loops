...
[
  ...
  },
  {
    ...
    "issue": "Hard-encodes 1:1 mode->packet ('no two modes resolve to the same packet folder') and folder==packetSkillName==deep-<mode> (:179,:273, validated :288), but the cited canonical deep-loop-workflows maps 3 modes to one deep-improvement packet and names a packet 'ai-council' (not deep-ai-council). The doctor check enforces neither, so step_5:288 would FAIL a doctor-valid canonical-shaped layout and the update branch mis-handles packet reuse.",
