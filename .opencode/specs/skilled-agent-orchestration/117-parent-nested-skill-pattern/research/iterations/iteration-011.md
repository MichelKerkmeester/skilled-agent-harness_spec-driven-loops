...
| `command-bridge` | agent-improvement, model-benchmark, skill-benchmark | routed by `/deep:*` command, not lexical scoring; `legacyAliases` still feed canonicalization |
...
1. **Author `advisorRouting` on all 7 modes** (no code change). Populate `legacyAliases` as the **union** of current Python `SKILL_ALIAS_GROUPS` + TS `RAW_ALIAS_GROUPS`. Add a test asserting union ⊇ both hardcoded sets. Behavior unchanged → fixtures green.
...
```json
{
  ...
  "decisions": [
    ...
    },
    {
      ...
      "resolution": "routingClass enum lexical|metadata|command-bridge; only lexical modes seed DEEP_ROUTING_SKILLS/MODE_BY_KEY; context=metadata (no lexical entry); 3 improvement lanes=command-bridge",
      "evidence": "skill_advisor.py:2307,2316-2318 / aliases.ts:86-89"
    },
    {
      "question": "T3 deep-improvement one packet three modes",
