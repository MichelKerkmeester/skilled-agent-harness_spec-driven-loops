...
- A pure-field registry loader **regresses routing** as-is. `DEEP_ROUTING_MODE_BY_KEY`'s key for council is `"deep-ai-council"`, but the registry's council `packet` is `"ai-council"` (`mode-registry.json:45`) with `deep-ai-council` buried in `aliases` (`:49`). And `deep-improvement` is **one packet → three `workflowMode`s** (`:51-96`), so array-order can't pick the advisor default. So **both** the runtime-derivation path and a guard test need the registry enriched with an explicit `advisorRouting` block (`routingClass`, `legacyCanonicalId`, `advisorDefaultMode`) — confirming tensions #1, #2, #3. Neither approach is free of that.
...
```json
{
  ...
  "decisions": [
    {
      ...
      "resolution": "Add an explicit per-mode advisorRouting block (routingClass, legacyCanonicalId, advisorDefaultMode); pure-field derivation regresses because council packet='ai-council' != key 'deep-ai-council' and deep-improvement is 1 packet -> 3 modes",
  ...
  "risks": [
    ...
    "context + 3 improvement lanes remain fixture-untested, so any registry refactor can regress them invisibly"
  ]
}
```
