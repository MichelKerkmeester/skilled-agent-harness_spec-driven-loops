---
title: "Implementation Plan: system-skill-advisor README"
description: "Gather context with a two-iteration deep-context sweep, dual-draft with DeepSeek and MiMo, then merge the narrative README, correcting three stale facts."
trigger_phrases:
...
# Implementation Plan: system-skill-advisor README
...
Run the full per-skill recipe: a two-iteration deep-context sweep with DeepSeek v4 Pro and MiMo v2.5 Pro as read-only seats, host verification of the tools, lanes and trust model against the live source, a synthesized context report, then a dual-model draft the host merges and validates. The facts were mostly accurate, so the merge is voice-focused with three small corrections.
<!-- /ANCHOR:summary -->
...
- [x] Nine tools, five lanes and the trust model verified against source

### Definition of Done
- [x] README rewritten to the narrative skeleton
- [x] `validate_document.py --type readme` passes (0 issues)
- [x] Prose HVR-clean, three stale facts corrected
<!-- /ANCHOR:quality-gates -->
...
Seats return findings, host synthesizes the report and resolves one model disagreement against source, models draft, host corrects three stale facts and merges into the README.
<!-- /ANCHOR:architecture -->
...
- [x] Synthesize context-report.md (one disagreement host-resolved: seven embedder manifests, not one)
...
| Accuracy | Tools, lanes, trust model, paths | Host SKILL.md, lane-registry.ts, tool descriptors and references |
<!-- /ANCHOR:testing -->
