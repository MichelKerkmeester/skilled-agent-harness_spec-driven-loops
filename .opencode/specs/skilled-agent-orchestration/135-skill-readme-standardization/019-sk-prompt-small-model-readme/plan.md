...
Run the full per-skill recipe: a two-iteration deep-context sweep with DeepSeek v4 Pro and MiMo v2.5 Pro as read-only seats, host correction of the executor count and the minimax-2.7 evidence, a synthesized context report, then a dual-model draft the host merges and validates.
<!-- /ANCHOR:summary -->
...
- [x] Framework map, ownership split and executor count verified

### Definition of Done
- [x] README rewritten to the narrative skeleton
- [x] `validate_document.py --type readme` passes (0 issues)
- [x] Prose HVR-clean, executor count corrected, version dropped
<!-- /ANCHOR:quality-gates -->
...
Gather then draft then merge. Read-only seats gather, the host corrects the facts and writes the final file.

### Key Components
- **context/context-report.md**: the synthesized factual map with the framework map and ownership split.
- **drafts/**: the two model drafts the host merges.

### Data Flow

Seats return findings, host synthesizes the report and pins the corrected facts, models draft, host scans and merges into the README.
<!-- /ANCHOR:architecture -->
...
- [x] Synthesize context-report.md with the framework map and ownership split
...
| Accuracy | Framework map, ownership, paths, benchmark dirs | Host SKILL.md, model-profiles.json and tree |
<!-- /ANCHOR:testing -->
