---
title: "Implementation Plan: sk-doc README"
description: "Gather context with a two-iteration deep-context sweep, dual-draft with DeepSeek and MiMo, drop the drifted counts, then merge the narrative README."
trigger_phrases:
  - "sk-doc readme plan"
...
# Implementation Plan: sk-doc README
...
Run the full per-skill recipe: a two-iteration deep-context sweep with DeepSeek v4 Pro and MiMo v2.5 Pro as read-only seats, host enumeration of the drifted counts, a synthesized context report, then a dual-model draft the host merges and validates against sk-doc's own validator.
<!-- /ANCHOR:summary -->
...
Gather then draft then merge. Read-only seats gather, the host drops the counts and writes the final file.

### Key Components
- **context/context-report.md**: the synthesized factual map with the count drift listed.
- **drafts/**: the two model drafts the host merges.

### Data Flow

Seats return findings, host synthesizes the report and lists every drifted count to drop, models draft, host scans and merges into the README.
<!-- /ANCHOR:architecture -->
...
- [x] Synthesize context-report.md with the count drift enumerated
...
| Accuracy | Pipeline, DQI, commands, no counts | Host SKILL.md and tree |
<!-- /ANCHOR:testing -->
