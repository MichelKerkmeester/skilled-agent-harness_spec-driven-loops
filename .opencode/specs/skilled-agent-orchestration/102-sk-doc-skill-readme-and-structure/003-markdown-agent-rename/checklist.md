...
- [x] CHK-011 [P0] No stale references remain - Evidence: targeted `rg "@create|create\\.md|Create-Doc Agent|name: create"` checks return no matches in runtime agent and create-command scopes.
...
- [x] CHK-020 [P0] All acceptance criteria met - Evidence: `markdown.md` or `markdown.toml` exists where `create` agent files existed and command-family `/create:*` references remain.
...
- [x] CHK-022 [P1] Edge cases tested - Evidence: `.codex` used TOML instead of Markdown and was renamed to `markdown.toml`; `/create:*` command invocations were preserved.
...
- [x] CHK-FIX-003 [P0] Consumer inventory completed for changed paths and docs. - Evidence: grep inventory covered runtime agents, create commands, YAML assets, `AGENTS.md` and sk-doc agent template references.
