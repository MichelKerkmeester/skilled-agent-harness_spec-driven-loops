...
- [x] T001 Inventory every Gemini runtime + model reference outside `specs/**`. [EVIDENCE: global `rg "gemini"` mapped references across system-spec-kit, system-code-graph, system-skill-advisor, deep-loop-runtime, deep-improvement, sk-prompt-small-model, sk-doc, cli-*, plugins, scripts, docs, and changelogs]
...
- [x] T007 Wave 1 model refs: remove `gemini-flash` from deep-loop-runtime fallback-router + test (8/8 GREEN); sk-prompt-small-model `model-profiles.json` / `per-model-budgets.json` / refs; cli-devin `references/quota-fallback.md`. [EVIDENCE: fallback-router 8/8; `rg "gemini-flash"` clean in those surfaces]
...
- [x] T015 Wave 4 changelogs: edit 43 changelog files (agent-orchestration ×9, system-spec-kit ×8, deep-ai-council ×4, deep-improvement ×5, deep-research ×6, deep-review ×4, cli-* ×5, sk-doc ×1, sk-prompt-small-model ×1) + top-level `PUBLIC_RELEASE.md`; remove Gemini, reconcile runtime/mirror counts (e.g. 5 to 4, 4 to 3), remove Gemini-only sections/rows. [EVIDENCE: operator-approved; counts reconciled]
<!-- /ANCHOR:phase-2 -->
