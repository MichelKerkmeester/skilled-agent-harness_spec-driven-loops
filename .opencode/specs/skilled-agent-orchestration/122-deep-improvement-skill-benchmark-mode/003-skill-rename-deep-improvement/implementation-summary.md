...
- **Not committed yet:** the rename spans shared infra across ~180 files; committing scoped to the surface (excluding daemon `graph-metadata.json` churn + parallel-session artifacts) is the recommended next action to protect it from the active parallel-session-revert hazard before Phase 004 builds on it.
