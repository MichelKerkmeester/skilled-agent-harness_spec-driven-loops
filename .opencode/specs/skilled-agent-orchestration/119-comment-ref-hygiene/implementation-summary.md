...
Running the spec-kit suite to confirm the comment edits were inert surfaced several failures. Five were safe to fix here (three infra + two stale tests after intent was confirmed); the rest are other agents' active-lane contradictions (flagged, not touched).
