...
Running the spec-kit suite to confirm the comment edits were inert surfaced several failures. Three were safe to fix here; the rest are other agents' active-lane contradictions (flagged, not touched).
...
**Flagged, intentionally NOT fixed** (test asserts old behavior after *other agents'* deliberate source changes, in actively-churning lanes — fixing blind risks masking a regression or colliding with in-flight work): `default-model-selection` (hf-local default BGE→nomic in the shared embeddings registry), `health-reporting` (HfLocalProvider metadata shape), `gate-3-classifier` (`:auto` now requires the `spec_kit` token — tied to the live `spec_kit`→`speckit` rename), `spec-doc-structure` (sufficiency/verification enforcement), `template-structure` (phase-child addendum merge), `retry-manager` T49 (DB-fixture-dependent). Each should be reconciled by whoever owns that change. None are caused by the comment work.
<!-- /ANCHOR:what-built -->
