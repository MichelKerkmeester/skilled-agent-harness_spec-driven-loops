...
- **template-structure** — `c6805196a7`. Packet 117 stripped the `<!-- ANCHOR:phase-map -->` wrapper from the `valid-phase` fixture, but `inferPhaseSpecAddenda` reads it via `fileContainsAnchor` to detect child phases → returned `[]`. Restored the fixture anchor. 8/8.
