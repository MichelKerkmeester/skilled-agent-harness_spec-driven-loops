...
That makes **B) DISCOVERABLE-NESTED** the wrong default. Adding nested `graph-metadata.json` files would make each packet separately advisor-discoverable, and `ai-council` would have a forced identity choice because the folder is `ai-council` while the packet `SKILL.md` name is `deep-ai-council` (`.opencode/specs/skilled-agent-orchestration/155-parent-nested-skill-pattern/001-rename-fix-and-shared-decision/decision-record.md:88-98`). The accepted decision says that mismatch is cosmetic under one identity because discovery keys on graph metadata, not packet folder/name (`.opencode/specs/skilled-agent-orchestration/155-parent-nested-skill-pattern/001-rename-fix-and-shared-decision/decision-record.md:92-106`).
