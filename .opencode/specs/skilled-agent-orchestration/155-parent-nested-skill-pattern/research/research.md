...
2. **Enrich `mode-registry.json` with an explicit per-mode `advisorRouting` block** — `{ routingClass: lexical|metadata|command-bridge, advisorDefaultMode?: bool, legacyAliases: string[], packetSkillName: string }`. This makes the registry the authoritative declarative description of *how each mode routes*, including the system-ID aliases the advisor actually keys on (which the existing `aliases[]` phrases do not contain).
