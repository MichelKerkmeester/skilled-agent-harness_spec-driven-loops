...
- **`/doctor:parent-skill` validator** — a read-only route in `commands/doctor/_routes.yaml` + `doctor_parent-skill.yaml` workflow asset + `scripts/parent-skill-check.cjs` (12 invariant checks: one identity, no nested metadata, registry/packet consistency, the 3-tier discriminator + advisorRouting per mode, drift-guard presence + the optional projection cross-check) + a `speckit.md` router row.
- **Routing/discovery benchmark (dogfood)** — a `deep-loop-workflows/` skill-benchmark fixtures corpus (5 mode scenarios, public/private pairs, gold = `deep-loop-workflows` + mode) + a routing-precision scorecard, reusing the skill's own skill-benchmark harness rather than building new benchmarking.
...
- `/doctor:parent-skill` run on the `deep-loop-workflows` reference: **all 12 invariants PASS, exit 0**; negative-path proof: broken fixture → exit 1, missing dir → exit 2; `check-comment-hygiene.sh` on the script → exit 0; `route-validate.sh` → 8 routes validated.
- Benchmark fixtures: 10 files, 0 invalid, gold = `deep-loop-workflows` + correct mode; routing-precision scorecard **3/3** lexical modes.
