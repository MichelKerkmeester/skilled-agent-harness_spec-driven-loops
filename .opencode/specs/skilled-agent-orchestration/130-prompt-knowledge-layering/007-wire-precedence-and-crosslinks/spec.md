...
trigger_phrases:
  ...
  - "sk-prompt-small-model hub reference"
  - "cli-* precedence block"
importance_tier: "important"
contextType: "implementation"
_memory:
  continuity:
    packet_pointer: "skilled-agent-orchestration/130-prompt-knowledge-layering/007-wire-precedence-and-crosslinks"
...
This is **Phase 7** of the Prompt-knowledge layering across CLI skills, sk-prompt frameworks, and the sk-prompt-small-model model-craft hub specification.

**Scope Boundary**: SKILL.md files for all 5 cli-* skills, cli-devin's prompt_templates.md, the sk-prompt-small-model canonical card, and pattern-index.md. No executor mechanics, no model profile JSON, no tests.

**Dependencies**:
- Phase 006 thinned and standardized cli-* cards — this phase builds on that baseline.
- sk-prompt-small-model hub profiles (`references/models/`) must exist for the pattern-index repoint to be valid.
...
- Duplication-guard description replacing the stale Mirror Sync section in sk-prompt-small-model SKILL.md
...
### Problem Statement
Each cli-* skill defined prompt-composition guidance independently, so a caller could not predict which rule prevailed when two constraints matched the same dispatch. cli-devin's "MUST compose through sk-prompt" mandate conflicted with the hub-profile lookup that sk-prompt-small-model introduced, and the canonical card's Mirror Sync section described a dead process. The pattern-index rows for MiniMax and MiMo pointed to fragmented framework notes rather than the hub profiles that are now the authoritative source.
...
- Replace the stale Mirror Sync section in sk-prompt-small-model SKILL.md with a duplication-guard description
- Repoint pattern-index.md MiniMax and MiMo rows to hub profiles; update cli-opencode ownership cell to craft-vs-mechanics

### Out of Scope
- Model profile JSON edits — owned by sk-prompt-small-model hub maintenance
...
| `.opencode/skills/sk-prompt-small-model/SKILL.md` | Modify | Replace stale Mirror Sync section with duplication-guard description |
| `.opencode/skills/sk-prompt-small-model/references/pattern-index.md` | Modify | Repoint MiniMax and MiMo rows; update cli-opencode ownership cell |
<!-- /ANCHOR:scope -->
...
| REQ-003 | sk-prompt-small-model canonical card no longer contains Mirror Sync section | Grep for "Mirror Sync" returns no hit in the file |
...
- **SC-001**: Every cli-* SKILL.md contains a 3-tier precedence block referencing sk-prompt-small-model as tier 2
...
|------|------|--------|------------|
| Dependency | sk-prompt-small-model hub profiles (`references/models/`) must exist | Repoint would point at missing files | Confirm files exist before editing pattern-index.md |
