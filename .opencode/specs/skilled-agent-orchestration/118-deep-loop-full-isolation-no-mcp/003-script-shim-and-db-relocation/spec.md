...
- Author a small shared helper (`scripts/_lib/db-open.cjs` or inline in each script) for `better-sqlite3` open + close discipline
...
| `.opencode/skills/deep-loop-runtime/scripts/_lib/db-open.cjs` | Create | Shared open + close + error helper (optional inline) |
...
- Should the shared open/close helper live in `scripts/_lib/` or be inlined per-script for clarity? **Decision: shared helper `scripts/_lib/db-open.cjs` to keep error-path discipline consistent (try/finally plus close); each script imports it.**
