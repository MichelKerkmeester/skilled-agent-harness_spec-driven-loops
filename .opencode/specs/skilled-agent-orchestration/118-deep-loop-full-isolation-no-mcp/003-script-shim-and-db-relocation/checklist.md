...
- [ ] CHK-010 [P0] Each `.cjs` file passes `node -c <file>` syntax check
  - **Evidence**: _to be filled — output of `for f in scripts/*.cjs scripts/_lib/*.cjs; do node -c "$f"; done`_
...
- [ ] CHK-014 [P1] Shared `_lib/db-open.cjs` exports `openDatabase` and `withDatabase`
  - **Evidence**: _to be filled — `grep -E 'module.exports' scripts/_lib/db-open.cjs`_
