...
- **Line**: 80
- **Evidence**: "A shared helper `.opencode/skills/deep-loop-runtime/scripts/_lib/db-open.cjs` exposes `openDatabase(path)` and `withDatabase(path, fn)`. `withDatabase` wraps a `try/finally` so `db.close()` runs on every exit path"
