...
- **Status**: CONFIRMED
- **Evidence**: ADR-001 line 80 documents: "A shared helper `.opencode/skills/deep-loop-runtime/scripts/_lib/db-open.cjs` exposes `openDatabase(path)` and `withDatabase(path, fn)`." This file does not exist. Scripts use direct pattern: `try { ... } finally { db.closeDb(); }` (convergence.cjs:340-342).
- **Action**: Keep as P1
