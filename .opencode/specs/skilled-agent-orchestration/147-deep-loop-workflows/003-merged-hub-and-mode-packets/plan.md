...
- [ ] T11 Strict validate + frozen-boundary check (`.opencode/specs/skilled-agent-orchestration/147-deep-loop-workflows/003-merged-hub-and-mode-packets`) — _verify:_ validate exit 0; git diff empty on deep-loop-runtime (MCP-free preserved) and on all 5 old deep-<name>/ dirs (still live)
