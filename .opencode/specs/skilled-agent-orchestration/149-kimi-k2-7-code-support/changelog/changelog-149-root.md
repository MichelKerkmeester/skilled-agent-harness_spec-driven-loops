...
| [`005-filename-underscore-alignment`](./changelog-149-005-filename-underscore-alignment.md) | Complete | Renamed seven `sk-prompt-small-model` documentation and asset files to the underscore convention while preserving dashed model profiles by contract. |
...
- `kimi-k2.7-code` in `sk-prompt-small-model/assets/model-profiles.json`, later renamed to `model_profiles.json`.
- `sk-prompt-small-model/references/models/kimi-k2.7-code.md`, a seven-section prompt-craft profile.
...
- `sk-prompt-small-model/SKILL.md` gained Kimi K2.7 frontmatter, keywords, activation triggers, keyword triggers, `MODEL_ALIASES` and a dispatch-matrix row.
...
- Seven dash-named `sk-prompt-small-model` documentation and asset files moved to underscore names, while the four model-profile files stayed dashed because the guard derives their paths from model ids.
...
|------|--------|--------------|
| `sk-prompt-small-model/assets/model-profiles.json` then `model_profiles.json` | Updated | Added Kimi K2.7 Code, retired Kimi K2.6, promoted run 007 guidance and added the timeout weakness. |
| `sk-prompt-small-model/references/models/kimi-k2.7-code.md` | Created and updated | Added the profile, recorded run 006, replaced it with run 007 guidance and added the broad-scope timeout caveat. |
| `sk-prompt-small-model/references/models/kimi-k2.6.md` | Updated | Added the historical banner. |
| `sk-prompt-small-model/references/models/_index.md` | Updated | Moved Kimi K2.7 Code active, moved Kimi K2.6 historical and mirrored the final empirical status. |
| `sk-prompt-small-model/SKILL.md` | Updated | Added Kimi K2.7 routing, aliases and dispatch matrix content, then repointed renamed links. |
| `cli-opencode/SKILL.md` | Updated | Added provider login, model selection, renamed references and the operational caveat. |
| `sk-prompt-small-model/graph-metadata.json` and `cli-opencode/graph-metadata.json` | Updated | Added routing metadata for Kimi K2.7 Code and Kimi For Coding. |
...
| `sk-prompt-small-model` references and assets | Renamed | Seven dash-named files moved to underscore names and inbound links were repaired. |
