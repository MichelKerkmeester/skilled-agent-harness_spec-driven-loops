...
**Key result:** Beyond the 005 Claude Design baseline, the field adds **2 strong net-new ideas** — T2 *constrain generation to a registered design system* (keystone; `mcp-magicpath` ADOPT "reuse-before-generate", `sk-design-interface` ADAPT critique check) and T3 *build-error self-healing loop* (`mcp-magicpath` ADOPT) — plus T1/T4/T5 smaller ones, and confirms the anti-default guardrail by counter-example (multi-variant "pick-a-vibe" menus → SKIP).
