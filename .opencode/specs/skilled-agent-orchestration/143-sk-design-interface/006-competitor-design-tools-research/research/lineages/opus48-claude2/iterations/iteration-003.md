...
- **Relevance:** **The keystone net-new theme of this packet.** 005's "inherit-if-present" means *ground in* a system (soft). Subframe's model is *hard*: a **registry of allowed components/tokens the generator must compose from**. This is anti-default-aligned (it *prevents* generic output by construction) AND lean-CLI-hostable:
  ...
  - For `sk-design-interface`: ADAPT (it is judgment, not a generator) — when a component registry/token system is present, the critique step gains a hard check: "is this composed from the existing system, or am I reinventing a component the system already has?"
...
- Q3 (partial): F15 split into ADOPT (mcp-magicpath reuse-before-generate) / ADAPT (sk-design-interface critique check).
