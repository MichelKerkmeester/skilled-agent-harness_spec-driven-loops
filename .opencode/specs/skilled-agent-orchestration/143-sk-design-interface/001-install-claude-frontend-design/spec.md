...
description: "Vendor Anthropic's frontend-design skill into the framework as sk-design-interface (sk-code family) with a lean house-template SKILL.md, a reference-template principles doc, README, schema-2 graph metadata, catalog registration, and advisor-graph indexing."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Authored spec for sk-design-interface framework install"
    ...
    open_questions: []
    answered_questions:
      - "Skill name: sk-design-interface (sk-code family)"
...
### Purpose
Vendor that skill into `.opencode/skills/sk-design-interface/` so the advisor surfaces it whenever UI work is detected, giving every future design task a deliberate, non-templated point of view.
<!-- /ANCHOR:problem -->
...
|-----------|-------------|-------------|
| `.opencode/skills/sk-design-interface/SKILL.md` | Create | Lean house-template runtime instructions (WHEN TO USE / SMART ROUTING / HOW IT WORKS / RULES / REFERENCES) |
| `.opencode/skills/sk-design-interface/references/design_principles.md` | Create | Full upstream guidance, conformed to the reference template (verbatim prose) |
| `.opencode/skills/sk-design-interface/LICENSE.txt` | Create | Anthropic Apache-2.0 license, preserved verbatim |
| `.opencode/skills/sk-design-interface/README.md` | Create | House-voice 9-section README |
| `.opencode/skills/sk-design-interface/graph-metadata.json` | Create | Schema-2 advisor graph node |
| `.opencode/skills/sk-code/graph-metadata.json` | Modify | Add reciprocal sibling edge to sk-design-interface |
...
|----|-------------|---------------------|
| REQ-001 | Conformant skill folder exists | **Given** the framework, **When** listing `.opencode/skills/sk-design-interface/`, **Then** SKILL.md, README.md, graph-metadata.json, references/design_principles.md and LICENSE.txt are present |
...
| REQ-003 | Advisor routes to it | **Given** a scanned graph, **When** running `advisor_recommend` on a design prompt, **Then** `sk-design-interface` is the top match and `skill_graph_validate` reports errorCount 0 |
...
| REQ-005 | Catalog + root README updated | **Given** both READMEs, **When** reading them, **Then** sk-design-interface appears and the skill count is a consistent 23 |
...
- **SC-001**: The advisor routes a UI/design prompt to `sk-design-interface` with confidence >= 0.8 and uncertainty <= 0.35 (Gate 2).
...
- None. Skill name (`sk-design-interface`) and framework install target were resolved with the operator.
<!-- /ANCHOR:questions -->
