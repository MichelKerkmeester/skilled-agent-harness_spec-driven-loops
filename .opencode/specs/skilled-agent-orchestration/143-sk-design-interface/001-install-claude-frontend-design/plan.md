...
description: "Vendor Anthropic's frontend-design skill as sk-design-interface: relocate guidance to a reference, author a lean conformant SKILL.md + README + schema-2 graph metadata, register in catalog and advisor, validate."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Authored plan for sk-design-interface install"
...
- [x] Name and family decided with operator (sk-design-interface, sk-code family)
...
### Data Flow
Design prompt -> advisor scores graph -> routes to sk-design-interface -> agent reads SKILL.md -> loads design_principles.md -> hands implementation to sk-code.
<!-- /ANCHOR:architecture -->
...
- **Procedure**: `rm -rf .opencode/skills/sk-design-interface`, revert the catalog/root-README edits and the sk-code sibling edge, then re-run `skill_graph_scan`.
<!-- /ANCHOR:rollback -->
...
### Rollback Procedure
1. Remove `.opencode/skills/sk-design-interface/`.
