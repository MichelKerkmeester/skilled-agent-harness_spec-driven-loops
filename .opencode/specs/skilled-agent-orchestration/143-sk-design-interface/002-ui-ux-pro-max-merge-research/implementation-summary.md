...
description: "A 10-iteration parallel-by-model deep-research loop produced a cross-checked recommendation for merging the vendored MIT ui-ux-pro-max repo into sk-design-interface: adopt the data and search layers (quality-floor directly, aesthetics as a critique-against inventory), skip the packaging, mixed-license with attribution."
trigger_phrases:
  - "ui-ux-pro-max merge summary"
  - "sk-design-interface research outcome"
...
| Field | Value |
|-------|-------|
| **Spec Folder** | skilled-agent-orchestration/143-sk-design-interface/002-ui-ux-pro-max-merge-research |
...
This packet is research, not code. It produced a model-cross-checked answer to one question: can the vendored `ui-ux-pro-max` repo make `sk-design-interface` materially better, and if so, how. The deliverable is `research/research.md` with a concrete Merge Recommendation. Nothing in `.opencode/skills/sk-design-interface/` was touched.
...
1. **Recommendation only.** No change is made to `sk-design-interface`; adopting the recommendation requires a follow-up merge packet.
