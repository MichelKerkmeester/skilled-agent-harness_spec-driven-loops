---
title: "Research Synthesis — ui-ux-pro-max → sk-design-interface merge (opus48 lineage)"
description: "Merge recommendation for the vendored MIT ui-ux-pro-max-skill-main repo into the Apache-2.0 house sk-design-interface skill: per-asset ADOPT/ADAPT/SKIP, integration design, licensing path, and negative knowledge. claude-opus-4-8 fan-out lineage."
trigger_phrases:
...
# Research Synthesis — ui-ux-pro-max → sk-design-interface merge (opus48 lineage)
...
**Scope:** Research-only. Produces a recommendation; no edits to `sk-design-interface`.
...
The vendored `ui-ux-pro-max-skill-main` (MIT, NextLevelBuilder, v2.5.0) is worth merging into the house `sk-design-interface` skill (Apache-2.0, vendored Anthropic frontend-design) **asymmetrically and with careful framing**. Its value is concentrated in two layers that integrate very differently:
...
**Inputs:** `external/ui-ux-pro-max-skill-main/` (data, scripts, templates, CLI, SKILL.md, skill.json, LICENSE, CLAUDE.md); `.opencode/skills/sk-design-interface/` (SKILL.md, design_principles.md, graph-metadata.json, LICENSE.txt); house conventions (`sk-code` references, skill-dir layout, SKILL.md sizes).

---

## 3. The two skills in one paragraph each

**Target — `sk-design-interface`** (Apache-2.0, ~186 L SKILL.md + 87 L `design_principles.md` + README + LICENSE.txt). Pure prose. Philosophy: distinctive, anti-default visual design via a brainstorm → critique-against-AI-defaults → build → self-critique process; one justified aesthetic risk; subject-grounded; restraint. No data, no scripts. graph-metadata: family `sk-code`; `enhances sk-code`; `prerequisite_for mcp-magicpath`. [SOURCE: file:.opencode/skills/sk-design-interface/SKILL.md] [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:59]
...
`sk-design-interface`: "make deliberate, opinionated choices … a design that reads as a templated default has failed" and §4 names the three current AI-default clusters to avoid. `ui-ux-pro-max`: per-product-type best-match recommendations — `ui-reasoning.csv` literally emits `{"if_checkout":"emphasize-trust","if_hero_needed":"use-3d-hyperrealism"}`. A naive merge would bolt a default-generator onto an anti-default skill. [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:59] [SOURCE: file:external/.../data/ui-reasoning.csv:5]
...
- **Advisor routing:** extend `graph-metadata.json` `intent_signals`/`derived.trigger_phrases` with "color palette", "font pairing", "chart type", "ux checklist", "design tokens"; add new `key_files`; **avoid** stack/implementation signals owned by `sk-code`. Keep the `enhances sk-code` / `prerequisite_for mcp-magicpath` edges. [SOURCE: file:.opencode/skills/sk-design-interface/graph-metadata.json:36-47]

---

## 8. Licensing & attribution path (Q3)

- **Compatible, no copyleft.** MIT material may be incorporated into the Apache-2.0 skill; the result is a **mixed-license skill** (MIT data+scripts, Apache-2.0 principles). No relicensing needed. [SOURCE: file:external/.../LICENSE:5-13] [SOURCE: file:.opencode/skills/sk-design-interface/LICENSE.txt:2]
...
- **Extend the existing precedent.** `sk-design-interface` is the only house skill already shipping `LICENSE.txt` + a `license:` frontmatter field. Update that frontmatter to declare the mix (`Apache-2.0 (principles) + MIT (data & search)`) and credit the upstream repo in `metadata.source`. No new framework mechanism. [SOURCE: file:.opencode/skills/sk-design-interface/SKILL.md:6-9]
...
- External repo: `.opencode/specs/skilled-agent-orchestration/143-sk-design-interface/external/ui-ux-pro-max-skill-main/` (MIT) — `LICENSE`, `skill.json`, `CLAUDE.md`, `.claude/skills/ui-ux-pro-max/SKILL.md`, `src/ui-ux-pro-max/data/*.csv`, `src/ui-ux-pro-max/scripts/{core,search,design_system}.py`.
- Target skill: `.opencode/skills/sk-design-interface/` — `SKILL.md`, `references/design_principles.md`, `graph-metadata.json`, `LICENSE.txt`.
