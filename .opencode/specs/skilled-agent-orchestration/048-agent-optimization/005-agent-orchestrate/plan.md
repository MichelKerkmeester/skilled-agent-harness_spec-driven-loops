---
title: "Plan: @orchestrate Agent Optimization"
...
_memory:
  continuity:
    ...
    answered_questions: []
---
# Plan: @orchestrate Agent Optimization
...
Single linear iteration applying the validated sk-improve-agent v2 substrate (proven 060 → PASS 6/0/0) against `@orchestrate`. Standard 9-step pipeline. Mode: `:confirm`.
<!-- /ANCHOR:summary -->
...
`@orchestrate` is consumed via the Task tool / orchestrator dispatch. Improvements ship as canonical `.opencode/agents/orchestrate.md` updates plus 3 mirror updates. Score profile is dynamic (generated per-target via `generate-profile.cjs`).
<!-- /ANCHOR:architecture -->
...
`rollback-candidate.cjs --target=.opencode/agents/orchestrate.md --backup=<pre-promote-backup>` reverts canonical. Mirror revert via git revert of the sync commit. 
<!-- /ANCHOR:rollback -->
