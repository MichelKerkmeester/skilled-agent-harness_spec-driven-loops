...
## Findings
- `sk-deep-research`'s active reducer does not implement the shared-graph bridge: it only imports `node:fs` and `node:path`, and the only reducer references matching `graph|convergence|upsert|deep_loop` are pass-through reads of precomputed `convergenceSignals`, not graph parsing or MCP calls (.opencode/skills/sk-deep-research/scripts/reduce-state.cjs:11-12; .opencode/skills/sk-deep-research/scripts/reduce-state.cjs:246-247; .opencode/skills/sk-deep-research/scripts/reduce-state.cjs:271; .opencode/skills/sk-deep-research/scripts/reduce-state.cjs:442).
- Both deep-loop convergence contracts still document a reducer-owned in-memory graph bridge that is not present in the live reducer: deep research says "the reducer builds an in-memory coverage graph" when `graphEvents` exist, and deep review says the same for review iterations (.opencode/skills/sk-deep-research/references/convergence.md:1181-1205; .opencode/skills/sk-deep-review/references/convergence.md:658-683).
- The shared MCP surface is ready for the bridge, but it is only exposed as tool definitions in the current runtime evidence I inspected: `deep_loop_graph_upsert` is described as reducer-owned iteration persistence and `deep_loop_graph_convergence` as the graph-backed stop assessor, yet this pass found no active command/skill/agent callsite invoking either tool outside specs and handler definitions (.opencode/skills/system-spec-kit/mcp_server/tool-schemas.ts:779-823; .opencode/skills/system-spec-kit/mcp_server/tool-schemas.ts:856-869).
- `graphEvents` are not fully write-only overall, but the concrete downstream consumer I found is test/replay code rather than the normal loop runtime: `coverage-graph-db.vitest.ts` manually simulates replay by iterating over `record.graphEvents` arrays and rebuilding node/edge collections from JSONL-shaped records (.opencode/skills/system-spec-kit/mcp_server/tests/coverage-graph-db.vitest.ts:135-185).
...
- `sk-improve-agent` follows a different path entirely: its resume contract replays a local "coverage graph + registry", and `mutation-coverage.cjs` persists an `improvement` JSON graph with `fs`/`path` helpers and `loopType: 'improvement'`, while the agent only consumes an orchestrator-provided summary as advisory context. That means it does not supply the missing shared `deep_loop_graph_*` bridge for research/review convergence (.opencode/skills/sk-improve-agent/SKILL.md:294-300; .opencode/skills/sk-improve-agent/scripts/mutation-coverage.cjs:9-10; .opencode/skills/sk-improve-agent/scripts/mutation-coverage.cjs:21; .opencode/skills/sk-improve-agent/scripts/mutation-coverage.cjs:50-67; .opencode/agents/agent-improver.md:175-177).
...
- .opencode/skills/sk-deep-research/scripts/reduce-state.cjs:11-12
- .opencode/skills/sk-deep-research/references/convergence.md:1181-1205
- .opencode/skills/sk-deep-review/references/convergence.md:658-683
- .opencode/skills/system-spec-kit/mcp_server/tool-schemas.ts:779-869
- .opencode/skills/system-spec-kit/mcp_server/tests/coverage-graph-db.vitest.ts:135-185
...
- .opencode/skills/sk-improve-agent/scripts/mutation-coverage.cjs:16-67
- .opencode/agents/agent-improver.md:175-177
