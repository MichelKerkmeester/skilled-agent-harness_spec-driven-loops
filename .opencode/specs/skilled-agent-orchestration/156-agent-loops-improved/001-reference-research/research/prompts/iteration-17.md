...
Segment: 3 (map -> deep-loop-runtime) | Iteration: 17 of 58
Questions: 0/36 checked | Last focus: S2-08
Last 2 ratios: 0.78 -> 0.77 | Stuck count: 0
...
Focus Area: [S3-01] Our `loop-lock.ts` carries `lastHeartbeatIso`/`ttlMs` but who calls `refreshLoopLock` during a long iteration — should we adopt loop-cli's pause-aware heartbeat cadence so a live-but-slow loop is never judged stale (`isStaleLoopLock` 2×TTL)? (build on S1-04) -> loop-lock.ts
...
Last 3 Iterations Summary: run 14: S2-06 (ratio 0.76); run 15: S2-07 (ratio 0.78); run 16: S2-08 (ratio 0.77)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/001-reference-research/research/iterations/iteration-017.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/001-reference-research/research/deltas/iter-017.jsonl
...
1. Iteration narrative markdown at `.opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/001-reference-research/research/iterations/iteration-017.md` with headings: Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus. In Findings give: reference mechanism + file:line; exact OUR target file; port-difficulty (easy/med/hard); quick-win vs deep-rewrite tag.

2. Canonical JSONL record APPENDED to the State Log, EXACT type "iteration":
`{"type":"iteration","iteration":17,"newInfoRatio":<0..1>,"status":"<complete|insight|stuck>","focus":"S3-01","graphEvents":[...]}`
...
3. Per-iteration delta file at `.opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/001-reference-research/research/deltas/iter-017.jsonl`: the same iteration record plus one record per finding/observation/graphEvent, each on its own line.
