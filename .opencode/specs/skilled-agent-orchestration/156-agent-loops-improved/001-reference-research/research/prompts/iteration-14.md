...
Segment: 2 (mine kasper) | Iteration: 14 of 58
Questions: 0/24 checked | Last focus: S2-05
...
Focus Area: [S2-06] How does kasper do incremental fuzzy finding-merge — cheap similarity (`weaknessSimilarity`/`weaknessCache`) first, then LLM consolidation only on leftover duplicates? -> deep-loop-runtime/lib/coverage-graph/coverage-graph-query.ts
...
Last 3 Iterations Summary: run 11: S2-03 (ratio 0.84); run 12: S2-04 (ratio 0.81); run 13: S2-05 (ratio 0.79)
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/001-reference-research/research/iterations/iteration-014.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/001-reference-research/research/deltas/iter-014.jsonl
...
1. Iteration narrative markdown at `.opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/001-reference-research/research/iterations/iteration-014.md` with headings: Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus. In Findings give: reference mechanism + file:line; exact OUR target file; port-difficulty (easy/med/hard); quick-win vs deep-rewrite tag.

2. Canonical JSONL record APPENDED to the State Log, EXACT type "iteration":
`{"type":"iteration","iteration":14,"newInfoRatio":<0..1>,"status":"<complete|insight|stuck>","focus":"S2-06","graphEvents":[...]}`
...
3. Per-iteration delta file at `.opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/001-reference-research/research/deltas/iter-014.jsonl`: the same iteration record plus one record per finding/observation/graphEvent, each on its own line.
