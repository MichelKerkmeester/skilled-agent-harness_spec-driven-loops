...
- **Parent packet:** `.opencode/specs/skilled-agent-orchestration/156-agent-loops-improved/` (the `002-implementation` parent was ungrouped earlier this arc; its children are now 156 root phases `002`–`008`; the whole 156 tree validates 63/63).
- **This phase parent:** `156-agent-loops-improved/009-loop-systems-remediation/` with 6 children (below).
...
- `009-loop-systems-remediation/` — the 6 child folders are scaffolded; specs partially authored (states below). **Untracked.**
- `156-agent-loops-improved/{spec.md, description.json, graph-metadata.json}` — modified (phase-map gained the `009` row; metadata re-regenerated). **Uncommitted.**
...
3. `validate.sh 009-loop-systems-remediation --recursive` → clean; also re-validate `156 --recursive`.
