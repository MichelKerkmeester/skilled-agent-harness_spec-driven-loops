...
The remaining high-value proposals are contract clarifications that reduce agent friction without changing mode ownership: add a shared handoff schema, normalize stale nested-packet path vocabulary, state parent-vs-packet permission precedence, and preserve checklist overlap as write-time prevention plus author-side enforcement. [SOURCE: research/iterations/iteration-005.md]

## Ranked Proposals After Iteration 5

1. Repair or retire/re-scope `code-verify/assets/scripts/verify_stack_folders.py` for the current `mode-registry.json` + sibling surface packet architecture.
2. Add a shared cross-mode handoff schema/matrix for implement -> quality -> debug -> verify and verify handbacks.
3. Normalize stale path vocabulary in shared lifecycle docs and mode contracts to current nested packet paths.
4. Add parent-vs-packet tool-surface precedence wording after registry resolution.
5. Document intentional checklist overlap as “implement consumes write-time checklists; quality enforces author-side gates.”

## Iteration 7 Synthesis

`code-implement` and `code-quality` ancillary docs corroborate the existing ranking rather than adding a new top proposal. Their READMEs extend the stale-path normalization scope: visible labels still name local or monolithic paths (`references/webflow/`, `references/opencode/`, `assets/opencode-checklists/`) while link targets use current sibling packets. [SOURCE: research/iterations/iteration-007.md]

`code-quality/assets/code_quality_checklist.md` is correctly Webflow/frontend-oriented and explicitly treats formal findings-first review as `code-review`'s baseline, supporting the recommendation to document boundaries rather than merge quality/review/verify packets. `check-dist-staleness.sh` adds one implementation detail for the handoff schema: generated-dist warnings are advisory and always exit 0, so they should be surfaced as risk/evidence, not treated as a hard verifier pass. [SOURCE: research/iterations/iteration-007.md]

## Ranked Proposals After Iteration 7

1. Repair or retire/re-scope `code-verify/assets/scripts/verify_stack_folders.py` for the current `mode-registry.json` + sibling surface packet architecture; update `code-verify/README.md` and script README semantics with it.
2. Add a shared cross-mode handoff schema/matrix for implement -> quality -> debug -> verify and verify handbacks, including advisory generated-dist warning fields.
3. Normalize stale path vocabulary across `SKILL.md`, READMEs, checklist assets, and shared lifecycle docs to current nested packet paths.
4. Add parent-vs-packet tool-surface precedence wording after registry resolution.
5. Document intentional checklist/review overlap as “implement consumes write-time checklists; quality enforces author-side gates; review produces findings-first reports.”

## Iteration 8 Final Synthesis

The research loop has converged on a stable proposal ranking. Iterations 5-7 repeatedly tested the ranked matrix against core contracts, READMEs, checklists, scripts, and helper assets; subsequent evidence refined scope but did not change the order. The remaining uncertainty is implementation detail, not research direction. [SOURCE: research/iterations/iteration-008.md]

## Final Ranked Upgrade Proposals

1. **P0/P1 — Repair or retire/re-scope `code-verify/assets/scripts/verify_stack_folders.py`.** Align it with the current `mode-registry.json` plus sibling surface-packet architecture, or remove it from advertised verification paths if it is obsolete. Update `code-verify/README.md` and the script README semantics at the same time. [SOURCE: research/iterations/iteration-005.md] [SOURCE: research/iterations/iteration-006.md]
2. **P1 — Add a shared cross-mode handoff schema/matrix.** Standardize fields for implement -> quality -> debug -> verify and verify handbacks: `changedScope`, `surface`, `baseline`, `qualityEvidence`, `distWarning`, `symptom`, `reproductionCommand`, `beforeAfter`, `acceptedRisks`, and `targetVerificationCommands`. [SOURCE: research/iterations/iteration-004.md] [SOURCE: research/iterations/iteration-007.md]
3. **P1 — Normalize stale path vocabulary across workflow docs/assets.** Update `SKILL.md`, READMEs, checklist assets, and shared lifecycle docs so visible labels match current sibling packet ownership (`code-webflow`, `code-opencode`, `code-animation`, `shared`). [SOURCE: research/iterations/iteration-006.md] [SOURCE: research/iterations/iteration-007.md]
4. **P1/P2 — Add parent-vs-packet tool-surface precedence wording.** Keep the parent hub routing-only, but state that after mode resolution the selected packet's registry/tool-surface contract governs execution. [SOURCE: research/iterations/iteration-004.md] [SOURCE: research/iterations/iteration-005.md]
5. **P2 — Document intentional overlap boundaries.** State explicitly: `code-implement` consumes write-time resources, `code-quality` enforces author-side gates, `code-debug` repairs evidenced failures, `code-verify` collects non-mutating final evidence, and `code-review` produces findings-first reports. [SOURCE: research/iterations/iteration-001.md] [SOURCE: research/iterations/iteration-007.md]

## Validation Hooks For Follow-up Implementation

- Run/replace the stack-folder integrity check after proposal 1 is implemented; expected behavior must match the current registry/surface-packet architecture. [SOURCE: research/iterations/iteration-005.md]
- Validate doc links and visible path labels across the four workflow packet READMEs/SKILL.md files plus relevant checklist assets after proposal 3. [SOURCE: research/iterations/iteration-006.md] [SOURCE: research/iterations/iteration-007.md]
- Verify `mode-registry.json` tool-surface contracts remain unchanged unless an implementation plan explicitly amends them. [SOURCE: research/iterations/iteration-004.md]

## Convergence Note

Convergence reached at iteration 8 with `newInfoRatio = 0.00`: no new proposal class, no ranking change, and no unresolved source contradiction requiring more research before implementation planning. [SOURCE: research/iterations/iteration-008.md]

## Iteration 6 Synthesis

Ancillary README and checklist evidence does not overturn the iteration 5 ranking. It strengthens the top proposal because `code-verify/README.md` advertises `verify_stack_folders.py` as a user-facing integrity check, while the script still targets the obsolete `STACK_FOLDERS` architecture. [SOURCE: research/iterations/iteration-006.md]

The stale-path proposal is broader than previously stated: `code-verify` README/checklist material and `code-debug` README material both mix current sibling-packet links with old monolithic labels such as `references/webflow-verification/*`, `references/webflow/verification/*`, and `references/webflow-debugging/*`. The upgrade should normalize visible labels, checklist prose, and contract text together. [SOURCE: research/iterations/iteration-006.md]

## Ranked Proposals After Iteration 6
