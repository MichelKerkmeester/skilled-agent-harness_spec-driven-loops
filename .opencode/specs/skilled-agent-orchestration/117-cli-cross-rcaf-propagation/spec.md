...
trigger_phrases:
  - "117 cli cross rcaf propagation"
...
Packet 117 records completed cross-CLI propagation of the 114/003 eval-loop finding that medium-density pre-planning beats dense pre-planning on SWE 1.6. The work updates the sk-prompt master CLI prompt quality card, mirrors the same guidance into four sibling CLI prompt quality cards, bumps sibling skill frontmatter versions, and adds sibling changelog entries.

**Key Decisions**: propagate only the model-agnostic medium pre-plan finding; keep bundle-gate-aversion and framework-dominates-anti-hallucination out of cross-CLI guidance until packet 118 validates them on frontier models

**Critical Dependencies**: packet 118 for cross-model validation of the held findings
...
### Purpose
Keep all CLI prompt quality cards aligned on medium pre-planning density while reserving unvalidated model-specific findings for packet 118.
<!-- /ANCHOR:problem -->
...
### Out of Scope
- Bundle-gate-aversion propagation, because it was measured only on SWE 1.6 and needs packet 118 cross-model confirmation.
...
| ID | Requirement | Acceptance Criteria |
|----|-------------|---------------------|
| REQ-004 | Document why the held findings are excluded | Spec and ADR point to packet 118 for validation |
...
- **SC-002**: RCAF remains documented as already-present shared default guidance, not as a new packet 117 change.
- **SC-003**: Bundle-gate-aversion and anti-hallucination-deprioritization remain deferred to packet 118.
<!-- /ANCHOR:success-criteria -->
...
|------|------|--------|------------|
| Dependency | Packet 118 cross-model validation | Held findings cannot safely propagate without broader evidence | Keep them out of skill bodies until 118 completes |
...
- Premature propagation: Use packet 118 decision gates before adding bundle-gate or anti-hallucination findings cross-CLI.
...
| Research | 14/20 | Depends on 114/003 eval-loop synthesis and cross-model deferral judgment |
...
|---------|-------------|--------|------------|------------|
| R-001 | Held findings propagate before frontier validation | H | M | Document 118 as the required validation packet |
...
**Acceptance Criteria**:
1. Given packet 117 docs, When a maintainer looks for bundle-gate-aversion propagation, Then the docs point to packet 118 instead of claiming completion.

---

## 12. OPEN QUESTIONS

- None for packet 117. The implementation scope is complete.
<!-- /ANCHOR:questions -->
