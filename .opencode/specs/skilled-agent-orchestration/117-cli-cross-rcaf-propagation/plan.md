...
description: "Describes the completed plan for adding medium pre-planning density guidance to the master CLI prompt quality card and four sibling mirrors. The plan keeps RCAF as already-present context and excludes findings reserved for packet 118."
trigger_phrases:
...
### Definition of Ready
- [x] 114/003 eval-loop synthesis identified medium pre-planning as the portable finding
- [x] RCAF default confirmed as already present across all five cards
- [x] Bundle-gate and anti-hallucination findings assigned to packet 118 validation
...
- [x] Packet docs state the held findings are excluded from 117
<!-- /ANCHOR:quality-gates -->
...
### Data Flow
The 114/003 eval-loop synthesis feeds the sk-prompt master card, then the same guidance is mirrored into sibling CLI cards. Packet 118 receives the unpropagated findings for validation before any future cross-CLI guidance update.
<!-- /ANCHOR:architecture -->
...
- [x] Identify the medium pre-planning density finding from 114/003.
- [x] Confirm RCAF was already present across all five cards.
- [x] Assign bundle-gate-aversion and anti-hallucination findings to packet 118.
...
- [x] Mark packet 117 complete.
- [x] Run final strict validation on packet 117 after documentation fill.
<!-- /ANCHOR:phases -->
...
| Spec validation | Packet 117 documentation structure | `validate.sh --strict` |
<!-- /ANCHOR:testing -->
...
|------------|------|--------|-------------------|
| 114/003 eval-loop synthesis | Internal evidence | Green | Medium pre-plan propagation lacks empirical basis |
| Packet 118 cross-model validation | Follow-on packet | Planned | Held findings remain unpropagated |
...
- **Procedure**: Revert the packet 117 card guidance, sibling version bumps, and sibling changelog entries from the follow-on commit.
<!-- /ANCHOR:rollback -->
...
|-------|------------|--------|
| Evidence boundary | 114/003 synthesis | Card and release updates |
...
### Rollback Procedure
1. Revert prompt-card guidance changes from the packet 117 follow-on commit.
...
|-----------|------------|----------|--------|
| Evidence boundary | 114/003 synthesis | Propagation decision | Card updates |
...
**Context**: 114/003 produced one broadly portable prompt-composition finding and two findings that may depend on model capability.

**Decision**: Share medium pre-planning density across CLI prompt quality cards now; defer bundle-gate-aversion and framework-dominates-anti-hallucination to packet 118.
