...
| **Invoke with** | `/deep:start-research-loop:auto "topic"` (autonomous) or `:confirm` (approval-gated). Keyword triggers include "autoresearch", "deep research" and "research loop" |
...
```bash
node .opencode/skills/deep-research/scripts/reduce-state.cjs <spec-folder>
```
...
Because state is on disk, a crashed run resumes from the packet files. Use `/deep:start-research-loop:auto` again and the workflow picks up the active lineage.
...
| Strategy or dashboard drift from iteration files | The reducer did not run after the last iteration write | Run `node .opencode/skills/deep-research/scripts/reduce-state.cjs <spec-folder>` to regenerate derived files |
...
```bash
/deep:start-research-loop:auto "GPU memory allocation strategies" --executor=cli-opencode --model=deepseek-v4-pro
```
...
```bash
python3 .opencode/skills/sk-doc/scripts/validate_document.py .opencode/skills/deep-research/README.md --type readme
```
