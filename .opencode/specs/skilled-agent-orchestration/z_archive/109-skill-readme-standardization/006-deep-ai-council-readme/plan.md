---
title: "Implementation Plan: deep-ai-council README"
description: "Gather context with a two-iteration deep-context sweep, dual-draft with DeepSeek and MiMo, verify reference paths, then merge the narrative README."
trigger_phrases:
...
_memory:
  continuity:
    packet_pointer: "skilled-agent-orchestration/z_archive/109-skill-readme-standardization/006-deep-ai-council-readme"
...
# Implementation Plan: deep-ai-council README
...
Run the full per-skill recipe: a two-iteration deep-context sweep with DeepSeek v4 Pro and MiMo v2.5 Pro as read-only seats, host verification of the reference tree, a synthesized context report, then a dual-model draft the host merges and validates.
<!-- /ANCHOR:summary -->
...
Gather then draft then merge. Read-only model seats gather context, the host verifies paths and writes the final file.
...
Seats return findings, host synthesizes the report, models draft from the report, host verifies paths and merges into the README.
<!-- /ANCHOR:architecture -->
...
- [x] Dual-draft, verify reference paths, merge into the README
...
| Accuracy | Reference and asset paths | `find` / `ls` against the real tree |
<!-- /ANCHOR:testing -->
