---
title: "Implementation Plan: system-code-graph README"
description: "Gather context with a two-iteration deep-context sweep, dual-draft with DeepSeek and MiMo, then merge the narrative README (voice-only, facts already accurate)."
trigger_phrases:
...
_memory:
  continuity:
    packet_pointer: "skilled-agent-orchestration/z_archive/109-skill-readme-standardization/021-system-code-graph-readme"
...
# Implementation Plan: system-code-graph README
...
Run the full per-skill recipe: a two-iteration deep-context sweep with DeepSeek v4 Pro and MiMo v2.5 Pro as read-only seats, host verification of the tools and the readiness model against the live schemas, a synthesized context report, then a dual-model draft the host merges and validates. The facts were already accurate, so the merge is voice-focused.
<!-- /ANCHOR:summary -->
...
- [x] Eight tools and the readiness model verified against the schemas

### Definition of Done
- [x] README rewritten to the narrative skeleton
- [x] `validate_document.py --type readme` passes (0 issues)
- [x] Prose HVR-clean, facts preserved
<!-- /ANCHOR:quality-gates -->
...
Seats return findings, host synthesizes the report, models draft, host fixes one semicolon and merges into the README.
<!-- /ANCHOR:architecture -->
...
- [x] Synthesize context-report.md (verification found no stale facts)
- [x] Dual-draft and merge into the README

### Phase 3: Verification
- [x] `validate_document.py --type readme` passes
- [x] HVR prose scan clean (one semicolon fixed)
...
| Accuracy | Tools, readiness model, paths | Host SKILL.md, tool-schemas.ts and references |
<!-- /ANCHOR:testing -->
