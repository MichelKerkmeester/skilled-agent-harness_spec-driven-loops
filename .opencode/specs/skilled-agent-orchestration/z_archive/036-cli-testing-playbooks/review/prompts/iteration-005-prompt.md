You are running iteration 5 (FINAL) of a 5-iteration deep review on spec folder `.opencode/specs/skilled-agent-orchestration/z_archive/036-cli-testing-playbooks/`.
...
2. **Surface missed findings** — what did iters 1-4 NOT catch that they should have? Specifically scan:
   ...
   - Any spec files I may have not touched: `.opencode/specs/skilled-agent-orchestration/z_archive/036-cli-testing-playbooks/scratch/` (if it exists), `description.json`, `graph-metadata.json`
...
Write findings to `.opencode/specs/skilled-agent-orchestration/z_archive/036-cli-testing-playbooks/review/iterations/iteration-005.md` with this expanded structure:
