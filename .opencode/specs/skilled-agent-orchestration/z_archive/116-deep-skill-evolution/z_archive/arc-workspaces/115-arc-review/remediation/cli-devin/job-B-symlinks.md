...
**Reference** (correctly-handled sister case from 007): `.opencode/changelog/sk-prompt-models` → `../skills/sk-prompt-models/changelog` — this symlink IS correct. Use it as the template for the sk-ai-council symlink creation.

## ACTION (pre-planning — execute in order)

### Step 1: Inspect current state
- **Run** `ls -la .opencode/changelog/ | grep -E "(deep-ai-council|sk-ai-council|sk-prompt-models)"`.
- **Verify**: `deep-ai-council` symlink exists and points at `../skills/deep-ai-council/changelog`; `sk-prompt-models` symlink exists and points at `../skills/sk-prompt-models/changelog`; `sk-ai-council` symlink does NOT exist.
