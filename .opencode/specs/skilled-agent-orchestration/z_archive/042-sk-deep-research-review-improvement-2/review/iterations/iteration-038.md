...
● Read coverage-graph-contradictions.vitest.ts
  │ .opencode/skills/system-spec-kit/scripts/tests/coverage-graph-contradictions.vitest.ts
  └ L1:260 (166 lines read)

● List directory .opencode/skills/system-spec-kit/mcp_server/tests/archive
  └ 2 files found
...
● Search (grep)
  ...
  │ **/*.{ts,js,cjs,mjs,md,json,yml,yaml} (.opencode/skills/system-spec-kit)
  └ 5 lines found
...
● Search (glob)
  │ "**/vitest.config.{ts,js,mjs,cjs}" in .opencode/skills/system-spec-kit
  └ 2 files found
...
● Search (glob)
  │ "**/archive/**/coverage-graph*.vitest.ts" in .opencode/skills/system-spec-kit
  └ 2 files found

● Search (glob)
  │ "**/coverage-graph*.vitest.ts" in .opencode/skills/system-spec-kit/scripts/tests
  └ 7 files found
...
● Search (grep)
  │ "vitest|test|archive" (.opencode/skills/system-spec-kit/package.json)
  └ 6 lines found

● Search (grep)
  │ "coverage-graph cross-layer integration|coverage-graph-integration|cross-layer" in
  │ **/coverage-graph*.vitest.ts (.opencode/skills/system-spec-kit/scripts/tests)
  └ 5 lines found
...
● Check whether archive tests are listed by vitest configs (shell)
  │ cd /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-spec
...
● Search (grep)
  │ "vitest\.config\.ts" in **/*.{md,json,ts,js,yml,yaml} (.opencode/skills/system-spec-kit)
  └ 5 lines found
...
- **Severity:** P2
- **Dimension:** maintainability
- **File:** `.opencode/skills/system-spec-kit/scripts/tests/coverage-graph-integration.vitest.ts`
- **Line:** `134-476`
...
- **Evidence:** `coverage-graph-integration.vitest.ts` covers relation alignment, clamping, self-loop rejection, namespace isolation, convergence, and contradiction contracts at `.opencode/skills/system-spec-kit/scripts/tests/coverage-graph-integration.vitest.ts:136-203`, `208-305`, `316-367`, `372-476`. The older suite already covers the same core cross-layer contract space at `.opencode/skills/system-spec-kit/scripts/tests/coverage-graph-cross-layer.vitest.ts:147-180`, `216-235`, `238-350`. Both files also repeat the same workspace-root + `createRequire` bootstrap pattern at `.opencode/skills/system-spec-kit/scripts/tests/coverage-graph-integration.vitest.ts:34-80` and `.opencode/skills/system-spec-kit/scripts/tests/coverage-graph-cross-layer.vitest.ts:34-85`.
...
- **Severity:** P2
- **Dimension:** maintainability
- **File:** `.opencode/skills/system-spec-kit/vitest.config.ts`
- **Line:** `5-8`
...
- **Evidence:** The files are archived at `.opencode/skills/system-spec-kit/mcp_server/tests/archive/coverage-graph-db.vitest.ts` and `.opencode/skills/system-spec-kit/mcp_server/tests/archive/coverage-graph-tools.vitest.ts`, and they still read like executable test suites rather than tombstones (`.../coverage-graph-db.vitest.ts:1-17`, `.../coverage-graph-tools.vitest.ts:1-17`). `mcp_server/vitest.config.ts` explicitly excludes `mcp_server/tests/archive/**` at `.opencode/skills/system-spec-kit/mcp_server/vitest.config.ts:17-24`, but the root config includes `mcp_server/tests/**/*.vitest.ts` with no matching exclusion at `.opencode/skills/system-spec-kit/vitest.config.ts:5-8`.
...
`coverage-graph-integration.vitest.ts` **does** use real TS imports from `coverage-graph-db.js` (`.opencode/skills/system-spec-kit/scripts/tests/coverage-graph-integration.vitest.ts:21-33`), `session-isolation.vitest.ts` **does** include colliding-ID higher-level fixtures that reach query/status/convergence handlers (`.opencode/skills/system-spec-kit/scripts/tests/session-isolation.vitest.ts:371-471`), the stale DB/tool suites **are** archived, and the new contradictions suite **exists** at `.opencode/skills/system-spec-kit/scripts/tests/coverage-graph-contradictions.vitest.ts:74-166`. The remaining issues are both **P2 maintainability drift** items around test-suite ownership and archive hygiene.
...
- **Reviewed directly:**  
  ...
  - `.opencode/skills/system-spec-kit/mcp_server/tests/archive/coverage-graph-db.vitest.ts`  
  - `.opencode/skills/system-spec-kit/mcp_server/tests/archive/coverage-graph-tools.vitest.ts`  
  - `.opencode/skills/system-spec-kit/{vitest.config.ts,mcp_server/vitest.config.ts}`  
