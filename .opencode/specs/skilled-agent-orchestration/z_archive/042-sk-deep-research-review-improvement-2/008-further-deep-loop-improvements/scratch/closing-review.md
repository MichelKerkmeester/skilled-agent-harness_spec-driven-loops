...
- Evidence:
  - `.opencode/skills/system-spec-kit/mcp_server/handlers/coverage-graph/convergence.ts:188-205`
  - `.opencode/skills/sk-deep-research/scripts/reduce-state.cjs:217-237`
  - `.opencode/skills/sk-deep-review/scripts/reduce-state.cjs:392-413`
  - `.opencode/skills/system-spec-kit/scripts/tests/graph-aware-stop.vitest.ts:199-225`
...
- Evidence:
  ...
  - `.opencode/skills/sk-deep-review/scripts/reduce-state.cjs:737-753`
- Description:
...
- Evidence:
  - `.opencode/skills/sk-deep-review/scripts/reduce-state.cjs:186-222`
  - `.opencode/skills/sk-deep-review/scripts/reduce-state.cjs:604-623`
...
- Evidence:
  ...
  - `.opencode/skills/system-spec-kit/mcp_server/handlers/coverage-graph/convergence.ts:196-199`
  - `.opencode/skills/system-spec-kit/mcp_server/handlers/coverage-graph/query.ts:67-73`
  - `.opencode/skills/system-spec-kit/mcp_server/lib/coverage-graph/coverage-graph-query.ts:226-273`
  - `.opencode/skills/system-spec-kit/scripts/tests/session-isolation.vitest.ts:162-194`
- Description:
...
| REQ-004 | Pass | Improve-agent auto/confirm wire journal events at start, iteration boundaries, and end: `.opencode/commands/deep/assets/improve_agent-improver_auto.yaml:132-183`, `.opencode/commands/deep/assets/improve_agent-improver_confirm.yaml` mirrors. |
| REQ-005 | Pass | CLI example corrected in `.opencode/commands/deep/start-agent-improvement-loop.md` and implementation summary documents the fix: `.opencode/specs/skilled-agent-orchestration/042-sk-deep-research-review-improvement-2/008-further-deep-loop-improvements/implementation-summary.md:48-50`. |
...
| REQ-018 | Pass | `persistentSameSeverity` and `severityChanged` arrays shipped: `.opencode/skills/sk-deep-review/scripts/reduce-state.cjs:469-509,774-776`. |
| REQ-019 | Pass | Improve-agent dashboard includes distinct Sample Quality section: `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:747-763,781-818`. |
| REQ-020 | Pass | Interrupted-session fixture exists: `.opencode/skills/sk-deep-research/scripts/tests/fixtures/interrupted-session/README.md:1-1` and spec summary `.opencode/specs/.../implementation-summary.md:77-79`. |
| REQ-021 | Pass | Review blocked-stop fixture exists and exercises reducer surfacing: `.opencode/skills/sk-deep-review/scripts/tests/fixtures/blocked-stop-session/README.md:1-24`. |
...
| REQ-023 | Pass with test blind spot | `graph-aware-stop.vitest.ts` exists, but it seeds synthetic `signals.blendedScore` instead of the real handler shape: `.opencode/skills/system-spec-kit/scripts/tests/graph-aware-stop.vitest.ts:195-244`. |
| REQ-024 | Pass with test blind spot | `session-isolation.vitest.ts` exists, but it explicitly blesses the `all_sessions_default` fallback when `sessionId` is omitted: `.opencode/skills/system-spec-kit/scripts/tests/session-isolation.vitest.ts:162-194`. |
...
- Research reducer expects `signals.blendedScore` or `signals.score` and otherwise records `0`: `.opencode/skills/sk-deep-research/scripts/reduce-state.cjs:225-229`.
- Review reducer averages raw numeric signals when no score field exists: `.opencode/skills/sk-deep-review/scripts/reduce-state.cjs:397-413`.
- The canonical handler returns raw `signals` plus `decision`/`blockers`, not a numeric score field: `.opencode/skills/system-spec-kit/mcp_server/handlers/coverage-graph/convergence.ts:188-205`.
