...
## Findings
- `trade-off-detector.cjs` treats two trajectory points as sufficient evidence: `length < 2` is the only guard, and the detector immediately diffs each consecutive pair without consulting the shipped `trajectory.minDataPoints: 3` setting. That means one sharp delta between iteration 1 and 2 can already emit a trade-off even though the README and playbook describe a minimum 3-data-point requirement. (`.opencode/skills/sk-improve-agent/scripts/trade-off-detector.cjs:64-67`, `.opencode/skills/sk-improve-agent/scripts/trade-off-detector.cjs:85-127`, `.opencode/skills/sk-improve-agent/assets/improvement_config.json:96-99`, `.opencode/skills/sk-improve-agent/README.md:67`, `.opencode/skills/sk-improve-agent/manual_testing_playbook/06--end-to-end-loop/023-trade-off-detection.md:19-24`, `.opencode/skills/sk-improve-agent/manual_testing_playbook/06--end-to-end-loop/023-trade-off-detection.md:32-35`)
- `benchmark-stability.cjs` does not compensate for that low-sample risk because it enforces no minimum replay count at all. Any non-empty result array is eligible, `stddev()` returns `0` for a single sample, `stabilityCoefficient()` converts that to `1.0`, and `measureStability()` therefore marks one-sample dimensions as fully stable; the configured `benchmarkStability.replayCount: 3` is descriptive only in the inspected helper path. (`.opencode/skills/sk-improve-agent/scripts/benchmark-stability.cjs:20-27`, `.opencode/skills/sk-improve-agent/scripts/benchmark-stability.cjs:69-92`, `.opencode/skills/sk-improve-agent/scripts/benchmark-stability.cjs:103-171`, `.opencode/skills/sk-improve-agent/assets/improvement_config.json:120-123`)
- The shipped tests lock in that under-constrained behavior instead of guarding against it. Trade-off tests only rule out `0` or `1` trajectory point and then accept a 2-point trade-off as valid, while benchmark-stability tests explicitly accept a 2-replay sample as stable and only check instability once there are 3 replay results with visible variance. (`.opencode/skills/sk-improve-agent/scripts/tests/trade-off-detector.vitest.ts:58-79`, `.opencode/skills/sk-improve-agent/scripts/tests/benchmark-stability.vitest.ts:80-121`, `.opencode/skills/sk-improve-agent/scripts/tests/benchmark-stability.vitest.ts:124-139`)
- Reducer stop-gating collapses every non-`benchmark-pass` benchmark record into `benchmarkFailCount` and `weakBenchmarkRuns`, then stops after the configured maximum of 2 weak runs, but it never reads replay counts, per-dimension coefficients, or `stabilityWarning` payloads. In practice the operator-facing stop reason cannot distinguish “unstable because sample size is too small” from “genuine benchmark regression.” (`.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:173-186`, `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:259-289`, `.opencode/skills/sk-improve-agent/assets/improvement_config.json:72-79`, `.opencode/skills/sk-improve-agent/assets/improvement_config_reference.md:57-60`)
- The published dashboard contract promises reducer-populated trade-off visibility, but the shipped renderer does not surface it. `improvement_strategy.md` reserves a reducer-owned “Trade-Off Detection” table, yet `reduce-state.cjs` only renders counts, repeated failure modes, dimensional progress, and a generic benchmark-failure recommendation, so neither real Pareto trade-offs nor insufficient-sample instability are clearly labeled in the visible dashboard path. (`.opencode/skills/sk-improve-agent/assets/improvement_strategy.md:123-136`, `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:371-438`)
...
- `.opencode/skills/sk-improve-agent/scripts/trade-off-detector.cjs:64-127`
- `.opencode/skills/sk-improve-agent/scripts/benchmark-stability.cjs:20-191`
- `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:173-186`
- `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:259-289`
- `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:371-438`
- `.opencode/skills/sk-improve-agent/scripts/tests/trade-off-detector.vitest.ts:58-79`
- `.opencode/skills/sk-improve-agent/scripts/tests/benchmark-stability.vitest.ts:80-139`
- `.opencode/skills/sk-improve-agent/assets/improvement_config.json:72-123`
- `.opencode/skills/sk-improve-agent/assets/improvement_strategy.md:123-136`
- `.opencode/skills/sk-improve-agent/README.md:67`
- `.opencode/skills/sk-improve-agent/manual_testing_playbook/06--end-to-end-loop/023-trade-off-detection.md:19-35`
- `.opencode/skills/sk-improve-agent/manual_testing_playbook/07--runtime-truth/029-benchmark-stability.md:64-80`
