---
title: "Implementation Summary: cli-opencode Skill Creation [skilled-agent-orchestration/z_archive/035-cli-opencode-creation/implementation-summary]"
...
All under `.opencode/skills/cli-opencode/`:
...
- `.opencode/skills/cli-claude-code/graph-metadata.json` — added cli-opencode sibling at weight 0.5
- `.opencode/skills/cli-codex/graph-metadata.json` — added cli-opencode sibling at weight 0.5
- `.opencode/skills/cli-copilot/graph-metadata.json` — added cli-opencode sibling at weight 0.5
- `.opencode/skills/cli-gemini/graph-metadata.json` — added cli-opencode sibling at weight 0.5
- `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill-graph.json` — regenerated by `init-skill-graph.sh`; cli-opencode now in adjacency, signals, hub_skills
- `.opencode/skills/system-spec-kit/mcp_server/database/skill-graph.sqlite` — cli-opencode node + 8 symmetric sibling edges inserted via the same `indexSkillMetadata` shape (workaround for the pre-existing `skill_advisor` folder-name mismatch that blocks the MCP `skill_graph_scan` path)
...
- `.opencode/skills/README.md` — 8 line-anchored patches: lines 44/54/57 (counts), 134 ("five CLI skills"), insert after 151 (table row), insert after 201 (tree entry), insert after 257 (signals row), insert after 509 (related-docs link)
...
**Pinned in**: `.opencode/skills/cli-opencode/SKILL.md` §2 pseudocode (lines ~91-117), the cli-opencode integration_patterns file §5 (decision tree), the cli-opencode prompt_templates asset template 12 (refusal message).
