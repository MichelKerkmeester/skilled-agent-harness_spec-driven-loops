...
description: "Create a /create:prompt command wrapping sk-improve-prompt skill"
trigger_phrases:
  - "feature"
  - "specification"
  - "create"
...
The sk-improve-prompt skill provides powerful prompt engineering capabilities (7 frameworks, DEPTH processing, CLEAR scoring), but users must manually load the SKILL.md and follow its multi-step pipeline. There is no dedicated command to invoke this skill directly, creating friction and inconsistency in prompt creation workflows.

### Purpose

Create a `/create:prompt` command in the `create/` namespace that provides a streamlined, mode-aware entry point for prompt creation and improvement using the sk-improve-prompt skill, with argument routing, framework auto-selection, and structured CLEAR-scored output delivery.
...
- Modifications to sk-improve-prompt SKILL.md or its references
...
- New scoring systems or frameworks beyond what sk-improve-prompt provides
...
| REQ-003 | Argument routing dispatches sk-improve-prompt modes | Mode prefixes ($text, $improve, $refine, $short, $json, $yaml, $raw) correctly detected and routed |
| REQ-004 | Skill invocation instructions are complete | Command loads sk-improve-prompt SKILL.md and follows the enhancement pipeline (framework selection → DEPTH → CLEAR scoring) |
...
- **SC-001**: `/create:prompt "my vague prompt"` invokes sk-improve-prompt and delivers an enhanced prompt with CLEAR score ≥ 40/50
- **SC-002**: Command file passes command_template.md validation checklist (§15) — all required items checked
- **SC-003**: Mode prefix detection correctly routes `$text`, `$improve`, `$refine`, `$short`, `$json`, `$yaml`, `$raw` to appropriate sk-improve-prompt modes

---

### Acceptance Scenarios

- **Given** a user invokes `/create:prompt` with a vague prompt, **when** the command routes into `sk-improve-prompt`, **then** the workflow returns an enhanced prompt rather than a manual skill-loading detour.
...
|------|------|--------|------------|
| Dependency | sk-improve-prompt skill must exist and be functional | Command cannot work without the skill | Skill already exists and is complete (verified in 001-initial-creation spec) |
...
- None — all requirements are clear based on sk-improve-prompt SKILL.md and command_template.md patterns.
