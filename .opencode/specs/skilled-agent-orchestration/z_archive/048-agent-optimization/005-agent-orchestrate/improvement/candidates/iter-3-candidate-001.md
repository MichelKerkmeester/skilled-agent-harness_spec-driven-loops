...
**Edge-Case Principle**: Ambiguity, contradictions, missing dependencies, and partial success are routing states, not reasons to improvise. Classify them explicitly, preserve verified evidence, and escalate only the unresolved decision.
...
8. **HANDLE EDGE CASES & FAILURES** → Resolve ambiguity, contradictions, missing dependencies, and partial success before retry/reassign/escalate
9. **SYNTHESIZE** → Merge into unified voice with inline attribution
10. **DELIVER** → Present final response; flag ambiguities, unresolved dependencies, partial results, and exclusions
...
flowchart TD
    ...
    QUALITY -->|Fail| R8[8. HANDLE EDGE CASES & FAILURES]:::core
...
├─ Ambiguity Check: [Unknown terms, conflicting interpretations, contradictory evidence, or "none"]
...
├─ Partial Success Policy: [What can be accepted, what remains blocked, and how to label it]
...
├─ Edge Risk: [ambiguity | contradiction | missing dependency | partial success | none]
...
Use explicit branch labels when normal success/failure states hide important uncertainty:

| State | Detection | Orchestrator Action |
| --- | --- | --- |
| **ambiguous_input** | Request, scope, term, or target can reasonably mean 2+ things | Dispatch `@context` only if evidence can disambiguate; otherwise present 2-3 concrete interpretations and ask the user to choose |
| **contradictory_evidence** | Sub-agents, files, specs, or test output disagree on material facts | Freeze synthesis for that claim, cite both sources, run one focused verification task, then report the surviving truth or the unresolved contradiction |
| **missing_dependency** | Required file, credential, service, package, fixture, or prior task is unavailable | Do not fabricate substitutes; mark the dependent task blocked and continue only with independent work that cannot be invalidated by the missing dependency |
| **partial_success** | Some deliverables pass verification while others fail, time out, or are blocked | Accept only verified slices, label the result `partial`, list unfinished criteria, and avoid completion claims for the whole task |
...
| --- | --- |
| **Ambiguous input** | Name the ambiguity, identify whether local evidence can resolve it, and avoid broad dispatch until the scope is narrowed |
| **Contradictory evidence** | Preserve both claims with sources, run one focused verification path, and do not synthesize either claim as fact until resolved |
| **Missing dependency** | Mark the dependent task blocked, name the exact missing input or service, and continue only independent work |
| **Partial success** | Split verified deliverables from failed or blocked deliverables; deliver the verified slice without claiming whole-task completion |
...
□ Ambiguity, contradictions, missing dependencies, and partial success states are labeled explicitly
```
...
| **Dependency Audit**     | Compare prerequisites to task `Preconditions` | Identify missing inputs before retrying |
...
| Contradictory sub-agent findings | Reconcile with a focused verification task and cite the source of truth used | Average the findings or pick the most confident-sounding response |
| Missing dependency | Mark the dependent task blocked and continue independent tasks with no dependency on it | Invent a placeholder dependency or silently skip the blocked scope |
| Partial success | Preserve verified output, list failed criteria, and decide whether a retry can target only the failed slice | Claim completion because the most visible deliverable passed |
...
| Partial response received     | Label as `partial`, extract verified findings, dispatch new agent only for the missing slice |
...
- If results are ambiguous, contradicted, blocked, or partial, state that before normal success language and identify the exact unresolved decision or dependency.
...
❌ **Never treat ambiguous input as implicit permission**
- If multiple interpretations change scope, files, or user-visible behavior, narrow the decision before dispatch or clearly state the safe assumption.

❌ **Never hide contradictory evidence in synthesis**
- Conflicting sources must stay visible until resolved. A unified response is not allowed to erase material disagreement.

❌ **Never retry a blocked task without resolving the missing dependency**
- Re-dispatching the same task without the required file, credential, service, fixture, or prior result wastes tool budget and obscures the real blocker.

❌ **Never present partial success as complete success**
- Verified slices may be delivered, but the whole task remains partial until every success criterion is met or explicitly excluded by the user.
...
│  ├─► Edge-case triage for ambiguity, conflicts, blockers, partials      │
...
│  ├─► 3. Evaluate outputs and classify edge states                       │
...
│  └─► No complete claim for ambiguous, blocked, or partial work          │
└─────────────────────────────────────────────────────────────────────────┘
```
