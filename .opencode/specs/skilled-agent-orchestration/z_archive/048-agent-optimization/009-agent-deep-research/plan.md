---
title: "Plan: @deep-research Agent Optimization"
...
_memory:
  continuity:
    ...
    answered_questions: []
---
# Plan: @deep-research Agent Optimization
...
Single linear iteration applying the validated sk-improve-agent v2 substrate (proven 060 → PASS 6/0/0) against `@deep-research`. Standard 9-step pipeline. Mode: `:auto`.
<!-- /ANCHOR:summary -->
...
`@deep-research` is consumed via a specific spec_kit command flow. Improvements ship as canonical `.opencode/agents/deep-research.md` updates plus 3 mirror updates. Score profile is dynamic (generated per-target via `generate-profile.cjs`).
<!-- /ANCHOR:architecture -->
...
| **P2** Dispatch | `/deep:start-agent-improvement-loop .opencode/agents/deep-research.md` | `<packet>/candidate.md` |
...
`rollback-candidate.cjs --target=.opencode/agents/deep-research.md --backup=<pre-promote-backup>` reverts canonical. Mirror revert via git revert of the sync commit. 
<!-- /ANCHOR:rollback -->
