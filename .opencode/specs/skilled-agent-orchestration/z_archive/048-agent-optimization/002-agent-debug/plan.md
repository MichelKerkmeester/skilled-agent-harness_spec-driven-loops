---
title: "Plan: @debug Agent Optimization"
...
_memory:
  continuity:
    packet_pointer: "skilled-agent-orchestration/z_archive/048-agent-optimization/002-agent-debug"
...
Single linear iteration applying the validated sk-improve-agent v2 substrate (proven 060 → PASS 6/0/0) against `@debug`. Standard 9-step pipeline. Mode: `:confirm`.
<!-- /ANCHOR:summary -->
...
`@debug` is consumed via the Task tool / orchestrator dispatch. Improvements ship as canonical `.opencode/agents/debug.md` updates plus 3 mirror updates. Score profile is dynamic (generated per-target via `generate-profile.cjs`).
<!-- /ANCHOR:architecture -->
...
| **P2** Dispatch | `/deep:start-agent-improvement-loop .opencode/agents/debug.md` | `<packet>/candidate.md` |
...
`rollback-candidate.cjs --target=.opencode/agents/debug.md --backup=<pre-promote-backup>` reverts canonical. Mirror revert via git revert of the sync commit. 
<!-- /ANCHOR:rollback -->
