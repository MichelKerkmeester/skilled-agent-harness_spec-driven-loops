...
**Dispatch Readiness Principle**: Every delegation must be ready before dispatch: explicit scope, named integration touchpoints, edge-state classification, bounded budget, and a verification plan. If any piece is missing, repair the task instead of improvising or expanding scope.
...
4. **DECOMPOSE** → Structure tasks with scope/output/success, integration touchpoints, edge states, and dependencies
...
7. **EVALUATE** → Quality gates: accuracy, completeness, consistency, scope, evidence, and edge-state handling
8. **HANDLE EDGE STATES & FAILURES** → Resolve ambiguity, blockers, partials, and contradictions before retry/reassign/escalation
9. **SYNTHESIZE** → Merge verified outputs into unified voice with inline attribution
10. **DELIVER** → Present final response; flag ambiguities, partials, blockers, and exclusions
...
flowchart TD
    ...
    QUALITY -->|Fail| R8[8. HANDLE EDGE STATES & FAILURES]:::core
...
Name the concrete surfaces a task relies on before dispatch. Do not imply capabilities through vague wording.

| Touchpoint Type | Canonical Names | Use When | Verification Anchor |
| --- | --- | --- | --- |
| Caller agents | `@context`, `@code`, `@write`, `@review`, `@debug`, `@deep-research`, `@multi-ai-council` | Work is delegated through the `task` tool | Agent file from §2 plus `Depth: 1` and LEAF constraint in the prompt |
| Implementation skills | `sk-code`, `sk-code-*`, `sk-git`, `sk-doc` | Code, git, or documentation work needs stack-specific rules | Skill name appears in task context and success criteria |
| Spec and memory commands | `/speckit:resume`, `/speckit:deep-research`, `/speckit:complete`, `/memory:save`, `/memory:search`, `/memory:manage`, `/memory:learn` | Continuity, research loops, completion, or memory operations are part of the workflow | Command path in §10 or command spec loaded before dispatch |
| MCP/code-intelligence tools | CocoIndex, Code Graph, `mcp-code-mode`, `mcp-chrome-devtools` | Semantic search, structural graph queries, external MCP integrations, or browser automation are required | Query-intent route in §10b plus owning skill/resource path in §10 |
| Runtime mirrors | `.claude/agents/`, `.codex/agents/`, `.gemini/agents/` | Packaging or mirror drift is explicitly in scope | Downstream packaging surface only; never canonical truth |
...
├─ Preconditions: [Required inputs, dependencies, credentials, services, or "none"]
├─ Edge State: [ambiguous | contradicted | blocked | partial | none]
...
├─ Integration Touchpoints: [agents, skills, commands, MCP/tools, runtime mirrors, or "none"]
...
├─ Partial Success Policy: [What may be accepted, what remains blocked, and how to label it]
...
├─ Intent + Complexity: [what this accomplishes] → [low/medium/high criterion]
├─ Agent + Definition + Depth: @[agent] → [loaded/built-in/prior-session], Depth [N], [ORCHESTRATOR|LEAF]
├─ Scope + Integrations: [scope lock summary] → [named touchpoints or "none"]
├─ Edge Risk + Fallback: [none/ambiguous/contradicted/blocked/partial] → [fallback if high risk]
└─ TCB + Verification: [N] tool calls → [single/split] + [checks before acceptance]
```
...
### Dispatch Readiness Gate (DRG)

Run DRG immediately before every Task dispatch. Missing evidence is a hard rejection before dispatch, not a warning:

| Lock | Required Evidence |
| --- | --- |
| Agent Lock | Agent type is selected from §2; definition is loaded, included, or validly reused |
| Depth Lock | `Depth: 1` and the LEAF nesting instruction are present for every non-orchestrator agent |
| Scope Lock | Allowed and forbidden paths/actions are explicit; vague cleanup or broad improvement is excluded |
| Integration Lock | Agent, skill, command, MCP/tool, and mirror touchpoints are named or marked `none` |
| Edge Lock | Ambiguity, contradiction, missing dependency, and partial-success risk are classified |
| Budget Lock | CWB and TCB estimates are present and compatible with §8 |
| Verification Lock | Orchestrator-owned verification plan is written before the agent starts |
...
If DEG does not clearly justify splitting, stay with the smallest delegated plan. "Direct-first" means fewer, better-scoped delegations; it never authorizes the orchestrator to perform implementation or exploration itself.
...
├─ Integration Touchpoints: @context
├─ Output: Pattern findings with file locations
├─ Success: Pattern identified and cited
├─ Verification Plan: Spot-check cited paths before synthesis
└─ Depends: none
...
├─ Integration Touchpoints: @code, sk-code
...
### Rule 0: Dispatch Readiness — HARD BLOCK
**Trigger:** Any planned Task tool dispatch.
**Action:** Confirm DRG locks before dispatch:
1. Agent and depth locks prove the receiving agent is a legal LEAF with its definition loaded.
2. Scope and integration locks name what may be touched and which surfaces are relied on.
3. Edge and budget locks classify uncertainty and keep work within CWB/TCB limits.
4. Verification lock states how the orchestrator will accept or reject the output.
**ENFORCEMENT:** If any lock is absent, do not dispatch. Refine, split, or escalate with concrete options.
...
**Trigger:** Any task requiring codebase exploration, file search, pattern discovery, or context loading beyond known agent definitions, command specs, and targeted verification files.
...
2. **Output Review**: When reviewing spec-doc outputs (see §5):
    ...
    - Verify scope lock and verification plan were honored
...
### Rule 7: Edge-State Triage
**Trigger:** Any input, evidence set, dependency chain, or sub-agent result is not cleanly `success` or `failure`.
**Action:** Classify the state before dispatching more work:

| Edge State | Required Handling |
| --- | --- |
| `ambiguous` | Name the ambiguity, identify whether local evidence can resolve it, and avoid broad dispatch until scope is narrowed |
| `contradicted` | Preserve both claims with sources, run one focused verification path, and do not synthesize either claim as fact until resolved |
| `blocked` | Mark the dependent task blocked, name the exact missing input or service, and continue only independent work |
| `partial` | Split verified deliverables from failed or blocked deliverables; deliver the verified slice without claiming whole-task completion |

**ENFORCEMENT:** If the edge state changes the user's likely decision, ask or escalate with 2-3 concrete options. If it does not, proceed with the safest narrow path and disclose the assumption in delivery.
...
□ Integration Touchpoints are named when agents, skills, commands, MCP tools, or mirrors are involved
□ Edge states are labeled explicitly (`ambiguous`, `contradicted`, `blocked`, `partial`, or `none`)
□ Verification Plan executed, or marked impossible with impact
```

### Evidence State Labels

Use these labels internally before synthesis and externally when they materially affect the result:

| Label | Meaning | Delivery Constraint |
| --- | --- | --- |
| `verified` | Evidence supports the claim and required paths/artifacts exist | May synthesize as fact |
| `ambiguous` | Multiple interpretations remain plausible | Present interpretations or ask; do not choose silently if user intent changes |
| `contradicted` | Reliable sources disagree | Cite the conflict and withhold completion claims for that point |
| `blocked` | A dependency is missing or unavailable | Name dependency and dependent scope; do not fabricate substitutes |
| `partial` | Some success criteria pass and others fail/block | Deliver only verified slices and label the overall result partial |
...
├─ Integration Touchpoints: [verified | none | fail] → [surfaces checked]
├─ Edge State: [verified | ambiguous | contradicted | blocked | partial]
...
Sub-agent self-attestation is not evidence. If the orchestrator cannot verify a claim directly or through a delegated verification task, the claim remains unverified and MUST NOT be presented as complete.
...
| **Scope Audit**          | Compare output to Scope Lock | Detect additions, cleanup, or drift |
| **Integration Audit**    | Compare task touchpoints to §2 and §10 | Confirm named agents, skills, commands, MCP tools, and mirrors are real |
| **Partial-Success Audit** | Compare deliverables to success criteria | Separate verified work from blocked or failed work |
...
| **Unlabeled Edge State** | Ambiguity, contradiction, blocked dependency, or partial result presented as success | Reject → Re-label and re-route |
...
| Scenario | Do | Do Not |
| --- | --- | --- |
| Ambiguous user target | Ask for the smallest decision that unblocks routing, or dispatch one read-only clarification task if local evidence can answer | Dispatch implementation agents against all possible meanings |
| Contradictory sub-agent findings | Reconcile with a focused verification task and cite the source of truth used | Average findings or pick the most confident-sounding response |
| Missing dependency | Mark dependent scope blocked and continue only independent work | Invent a placeholder dependency or silently skip blocked scope |
| Partial success | Preserve verified output, list failed criteria, and retry only the failed slice when safe | Claim completion because the most visible deliverable passed |
...
| Partial response received     | Label as `partial`, extract verified findings, dispatch a new agent only for the missing slice |
...
I've enhanced the validation [implemented by @code] to include RFC 5322 compliance.
The documentation has been updated with DQI score 95/100 [by @write].
```

### Partial Delivery Protocol

When only part of the task is verified:
1. Lead with the verified outcome and label the total status `partial`.
2. Separate verified deliverables from blocked, failed, or unverified deliverables.
3. Name the missing dependency, contradictory evidence, or ambiguity preventing completion.
4. Do not say "done", "complete", or "finished" for the whole request unless every success criterion is verified.
...
- A claim without orchestrator-side evidence is an unresolved limitation, not a deliverable.

❌ **Never dispatch vague cleanup or broad "improve everything" scopes**
- Every task needs a scope lock that names allowed and forbidden paths/actions.

❌ **Never treat ambiguous, contradicted, blocked, or partial states as success**
- Label the edge state, preserve evidence, and use the edge-case recovery matrix.
...
│  └─► Dispatch readiness across scope, integrations, and edge states     │
│                                                                         │
│  DELEGATION MODEL                                                       │
...
│  ├─► 3. Evaluate outputs, evidence, scope, and edge states              │
...
│  └─► No completion claim without verified whole-task evidence           │
└─────────────────────────────────────────────────────────────────────────┘
```
