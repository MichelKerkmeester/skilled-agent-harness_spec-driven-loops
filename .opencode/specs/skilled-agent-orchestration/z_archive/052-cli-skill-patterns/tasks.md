...
- [x] T001 Snapshot per-skill grep inventory: for each unique term in spec.md §3 do-not-collapse list, run `grep -c '<term>' .opencode/skills/<skill>/SKILL.md` and capture in `specs/052-cli-skill-patterns/scratch/baseline-grep.txt`.
...
- [x] T002 Read `.opencode/skills/cli-copilot/SKILL.md` end-to-end; identify §3 location to insert Error Handling table.
- [x] T003 Add Error Handling table to `.opencode/skills/cli-copilot/SKILL.md` §3 (mirror cli-opencode's table shape: `| Issue | Solution |` columns with copilot-specific rows: command not found, autopilot misuse, max-3-concurrent breach, GH token expiry, model switch failure).
...
- [x] T006 Read `.opencode/skills/cli-opencode/SKILL.md`; locate the 3 UNKNOWN_FALLBACK_CHECKLIST occurrences.
...
- [x] T009 Read `.opencode/skills/cli-claude-code/SKILL.md`; identify §3 location to insert Default Invocation block.
...
- [x] T013 Read `.opencode/skills/cli-codex/SKILL.md`; identify INTENT_SIGNALS dict (currently 8 intents).
...
- [x] T017 Read `.opencode/skills/cli-gemini/SKILL.md`; section-order audit only — confirm 8-section header order matches canonical.
...
- [x] T023 Run subsection presence check: `grep -lE '^### (Default Invocation|Error Handling|Provider Auth Pre-Flight)' .opencode/skills/cli-*/SKILL.md` returns all 5 paths for each pattern.
- [x] T024 Run `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh specs/052-cli-skill-patterns --strict`; exit 0 required.
- [x] T025 Run `doctor:skill-advisor :auto` to re-index; diff each skill's `graph-metadata.json` for regression.
- [x] T026 Run anchor cross-link audit: `grep -rn '<!-- ANCHOR:' .opencode/skills/cli-*/SKILL.md` and verify all anchor IDs resolve in the same file.
