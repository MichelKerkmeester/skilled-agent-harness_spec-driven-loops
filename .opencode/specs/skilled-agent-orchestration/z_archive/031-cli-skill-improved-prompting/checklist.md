...
_memory:
  continuity:
    packet_pointer: "skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting"
...
---
title: "Verification Checklist: CLI Skill [skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting/checklist]"
...
_memory:
  continuity:
    ...
    blockers: []
    key_files:
      - ".opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting/checklist.md"
      - ".opencode/skills/skill-advisor/scripts/check-prompt-quality-card-sync.sh"
    session_dedup:
...
- [x] CHK-001 [P0] Requirements are documented in `spec.md` [Evidence: `.opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting/spec.md`]
- [x] CHK-002 [P0] Technical approach is defined in `plan.md` [Evidence: `.opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting/plan.md`]
- [x] CHK-003 [P1] Current guard constraints and active runtime directories were inspected from real repo files [Evidence: `rg -n "prompt_quality_card\\.md|LOADING_LEVELS\\[\\\"ALWAYS\\\"\\]" .opencode/skills/cli-*/SKILL.md`; `ls .opencode/agents/improve-prompt.md .claude/agents/improve-prompt.md .codex/agents/improve-prompt.toml .gemini/agents/improve-prompt.md`]
- [x] CHK-004 [P1] Optional drift-fixture decision is recorded before packet closeout [Evidence: `.opencode/skills/skill-advisor/scripts/check-prompt-quality-card-sync.sh`; `bash .opencode/skills/skill-advisor/scripts/check-prompt-quality-card-sync.sh -> SYNC OK`]
<!-- /ANCHOR:pre-impl -->
...
- [x] CHK-010 [P0] The canonical prompt-quality card exists under `sk-improve-prompt/assets/` [Evidence: `ls .opencode/skills/sk-improve-prompt/assets/cli_prompt_quality_card.md`]
- [x] CHK-011 [P0] Four local mirror cards exist under the CLI skill `assets/` folders [Evidence: `ls .opencode/skills/cli-claude-code/assets/prompt_quality_card.md .opencode/skills/cli-codex/assets/prompt_quality_card.md .opencode/skills/cli-copilot/assets/prompt_quality_card.md .opencode/skills/cli-gemini/assets/prompt_quality_card.md`]
- [x] CHK-012 [P0] Every CLI skill adds the local prompt-quality card to the `ALWAYS` loading block [Evidence: `.opencode/skills/cli-claude-code/SKILL.md:112,146`; `.opencode/skills/cli-codex/SKILL.md:108,142`; `.opencode/skills/cli-copilot/SKILL.md:106,140`; `.opencode/skills/cli-gemini/SKILL.md:105,139`]
- [x] CHK-013 [P0] No CLI skill uses a `..` routable path for prompt-quality resources [Evidence: `if grep -H 'prompt_quality_card' .opencode/skills/cli-*/SKILL.md | grep '\\.\\.'; then echo FOUND; else echo NO_DOTDOT_MATCHES; fi` -> `NO_DOTDOT_MATCHES`]
- [x] CHK-014 [P1] Each CLI skill documents the CLEAR pre-dispatch check and escalation trigger rule [Evidence: `.opencode/skills/cli-claude-code/assets/prompt_quality_card.md:29,39`; `.opencode/skills/cli-codex/assets/prompt_quality_card.md:29,39`; `.opencode/skills/cli-copilot/assets/prompt_quality_card.md:29,39`; `.opencode/skills/cli-gemini/assets/prompt_quality_card.md:29,39`; skill rule lines at `429`, `446`, `299`, `373`]
- [x] CHK-015 [P1] The `sk-improve-prompt` skill definition documents the agent invocation contract and fast-path asset [Evidence: `.opencode/skills/sk-improve-prompt/SKILL.md:343,424-426`; `rg -n "FRAMEWORK:|CLEAR_SCORE:|ENHANCED_PROMPT:|ESCALATION_NOTES:" .opencode/skills/sk-improve-prompt/SKILL.md`]
<!-- /ANCHOR:code-quality -->
...
- [x] CHK-024 [P1] Strict packet validation passes for `.opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting/` [Evidence: `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh --strict .opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting` -> `RESULT: PASSED`]
<!-- /ANCHOR:testing -->
...
- [x] CHK-030 [P0] Compliance/security-sensitive prompts are documented as escalation triggers, not fast-path-only cases [Evidence: `.opencode/skills/cli-claude-code/assets/prompt_quality_card.md:39`; same wording present in the three other mirror cards]
- [x] CHK-031 [P1] The `@improve-prompt` agent remains read-only and leaf-only across all runtime mirrors [Evidence: `.opencode/agents/improve-prompt.md:4,20`; `.claude/agents/improve-prompt.md:3,21`; `.codex/agents/improve-prompt.toml:4,6`; `.gemini/agents/improve-prompt.md:3,23`]
- [x] CHK-032 [P1] No change weakens the existing runtime sandbox or routing constraints described in the current agent formats [Evidence: `.codex/agents/improve-prompt.toml:6` retains `sandbox_mode = "read-only"`; CLI skills continue to use local card paths under `.opencode/skills/cli-*/assets/` only]
<!-- /ANCHOR:security -->
...
- [x] CHK-041 [P1] The implementation summary is updated from placeholder to delivery summary once implementation completes [Evidence: `.opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting/implementation-summary.md`]
...
- [x] CHK-050 [P1] New assets stay inside the correct skill trees and do not introduce shared cross-skill routing paths [Evidence: all prompt-quality cards live under `.opencode/skills/.../assets/`; `NO_DOTDOT_MATCHES` guard check passed]
- [x] CHK-051 [P1] Runtime mirrors exist only in the active runtime directories already present in this repo [Evidence: `ls .opencode/agents/improve-prompt.md .claude/agents/improve-prompt.md .codex/agents/improve-prompt.toml .gemini/agents/improve-prompt.md`]
...
- [x] CHK-100 [P0] Architecture decisions are captured in `decision-record.md` [Evidence: `.opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting/decision-record.md`]
...
- [x] CHK-132 [P2] Optional drift-check automation is reviewed for maintenance cost before landing [Evidence: implementation summary decision table plus landed `.opencode/skills/skill-advisor/scripts/check-prompt-quality-card-sync.sh`]
<!-- /ANCHOR:compliance-verify -->
...
- [x] CHK-140 [P1] All planning docs exist in the packet root [Evidence: `rg --files .opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting`]
...
- [x] CHK-142 [P2] The implementation summary is converted from placeholder to final summary at closeout [Evidence: `.opencode/specs/skilled-agent-orchestration/z_archive/031-cli-skill-improved-prompting/implementation-summary.md`]
