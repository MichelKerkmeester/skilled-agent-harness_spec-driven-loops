...
_memory:
  continuity:
    packet_pointer: "skilled-agent-orchestration/z_archive/038-agent-debug-integration"
...
- [x] T007 Run baseline `validate.sh --strict` on `.opencode/specs/skilled-agent-orchestration/z_archive/038-agent-debug-integration/` to confirm spec-doc compliance before edits begin
<!-- /ANCHOR:phase-1 -->
...
- [x] T010 Rewrite `.opencode/agents/debug.md` line 3 description (user-invoked / prompted-offer language)
- [x] T011 Rewrite `.opencode/agents/debug.md` line 24 lead paragraph to match new description
- [x] T012 Update `.opencode/agents/debug.md` line ~461 Related Resources table row for `orchestrate` ("Prompts user to dispatch debug after 3 failures")
- [x] T013 Rewrite `.opencode/agents/orchestrate.md` line 99 priority routing table cell
...
- [x] T040 Static grep: `grep -rn "auto-dispatch\|Task tool dispatches a focused\|automatically routes" .opencode/agent .claude/agents .codex/agents .gemini/agents .opencode/commands/spec_kit` → expect zero matches
...
- [x] T046 Run `validate.sh --strict` on `.opencode/specs/skilled-agent-orchestration/z_archive/038-agent-debug-integration/` → expect exit code 0 or 1 (warnings ok), never 2
- [x] T047 Static schema-parity check: scaffold output sections match Debug Context Handoff schema lines 60-89 of `.opencode/agents/debug.md`
- [x] T048 Write a manual-testing playbook entry under `.opencode/skills/cli-*/manual_testing_playbook/` documenting the 3-branch rehearsal
