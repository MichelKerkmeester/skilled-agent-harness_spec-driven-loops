...
The policy layer defines five legal-stop bundles and says a session may not claim convergence unless all pass: `contractGate`, `behaviorGate`, `integrationGate`, `evidenceGate`, and `improvementGate` (`.opencode/skills/sk-improve-agent/SKILL.md:268-280`). The journal helper accepts event types including `gate_evaluation`, `legal_stop_evaluated`, and `blocked_stop` (`.opencode/skills/sk-improve-agent/scripts/improvement-journal.cjs:49-69`), and the reducer consumes `legal_stop_evaluated` by reading `details.gateResults` (`.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:213-217`) before rendering the gate names in the dashboard (`reduce-state.cjs:880-897`).

The missing piece is producer-side specificity. `improvement-journal.cjs` validates required `stopReason` and `sessionOutcome` only for `session_ended/session_end` events; it does not validate that `legal_stop_evaluated` contains all five gate keys or that `blocked_stop` contains failed gate details (`improvement-journal.cjs:80-106`). Auto mode emits `gate_evaluation` with `gateName: "stop_check"`, `gateResult`, and `stopReason`, not `legal_stop_evaluated` with a full `gateResults` object (`.opencode/commands/deep/assets/deep_start-agent-improvement-loop_auto.yaml:186-204`). Confirm mode similarly emits `gate_evaluation` after the operator gate and then `session_end` (`.opencode/commands/deep/assets/deep_start-agent-improvement-loop_confirm.yaml:217-245`).
...
| B | Disciplined `/deep:start-agent-improvement-loop <agent> :auto --spec-folder=<sandbox-spec> --iterations=1` | `integration-report.json`, `dynamic-profile.json`, packet-local candidate path, `candidate_generated`, `candidate_scored`, no canonical diff before promotion |

The exact PASS signals can be grep-only: `test -f improvement/integration-report.json`, `test -f improvement/dynamic-profile.json`, `grep '"eventType":"candidate_generated"' improvement/improvement-journal.jsonl`, `grep '"eventType":"candidate_scored"' improvement/improvement-journal.jsonl`, and `git diff --quiet -- .opencode/agents/<target>.md` before promotion.
...
- I ruled out "mirror patching is expected during proposal generation": the agent must stay proposal-only and never edit canonical targets or runtime mirrors (`.opencode/agents/improve-agent.md:24-40`), while mirror drift is explicitly downstream packaging work (`mirror_drift_policy.md:37-68`).
- I ruled out "legal-stop evidence is LLM-judge-based": the intended evidence path is JSONL event fields, but the current producer path is under-specified (`improvement-journal.cjs:49-69`, `reduce-state.cjs:213-217`, `deep_start-agent-improvement-loop_auto.yaml:189-204`).

## Sketched Diff (if any)

For `.opencode/skills/sk-improve-agent/scripts/scan-integration.cjs` at `MIRROR_TEMPLATES`, current text:
...
For `.opencode/skills/sk-improve-agent/scripts/improvement-journal.cjs`, add event validation for legal-stop details:
...
| Field | Scenario |
| --- | --- |
| Purpose | Prove `/deep:start-agent-improvement-loop` cannot claim `converged` without JSONL evidence for all five legal-stop gates. |
...
| Call B | Disciplined `/deep:start-agent-improvement-loop <target> :auto --spec-folder=/tmp/cp-061-spec --iterations=1`. |
