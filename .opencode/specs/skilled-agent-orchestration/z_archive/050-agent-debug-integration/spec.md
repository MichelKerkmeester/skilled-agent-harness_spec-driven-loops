...
| `.opencode/skills/system-spec-kit/templates/debug-delegation.md` | Verify only | Confirm template field schema matches the Debug Context Handoff format at `.opencode/agents/debug.md:60-89`. Modify only if drift is found. |
...
| REQ-008 | A short manual-testing playbook entry under `.opencode/skills/cli-*/manual_testing_playbook/` documents the failure-threshold prompt rehearsal so the path stays exercised. | New `.md` entry exists with steps to reproduce the synthetic 3-failure flow and verify the prompt + scaffold generation. |
<!-- /ANCHOR:requirements -->
...
- **SC-001**: After this packet ships, `grep -ri "auto-dispatch\|automatically routes.*debug\|Task tool dispatches" .opencode/agent .claude/agents .codex/agents .gemini/agents .opencode/commands/spec_kit` returns zero matches.
...
| Dependency | `.opencode/agents/debug.md:60-89` Debug Context Handoff schema. | The pre-fill scaffold mirrors this schema exactly. | If the schema changes, update the scaffold generator. Add a post-write validator check that asserts schema parity (lint rule). |
| Dependency | `validate.sh --strict` for spec folder template compliance. | Required for REQ-007. | Already exists at `.opencode/skills/system-spec-kit/scripts/spec/validate.sh`; no new tooling needed. |
<!-- /ANCHOR:risks -->
...
### Maintainability
- **NFR-M01**: The pre-fill scaffold generator should live in a single shared utility (likely under `.opencode/skills/system-spec-kit/scripts/`), not duplicated between `implement_auto.yaml` and `complete_auto.yaml`.
