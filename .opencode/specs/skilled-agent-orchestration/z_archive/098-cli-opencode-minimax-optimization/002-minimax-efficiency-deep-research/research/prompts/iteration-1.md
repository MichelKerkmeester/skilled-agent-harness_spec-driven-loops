...
Next focus: Q1 - Establish MiniMax 2.7 API characteristics (context window, reasoning/--variant controls, pricing, latency).
...
Iteration: 1 of 10
Focus Area: Q1 - Establish MiniMax 2.7's API characteristics (context window, reasoning/--variant controls, tool-calling, pricing, latency) from MiniMax.io documentation and the just-added cli-opencode minimax provider config, since every downstream budget/verification/routing decision depends on these facts.
Remaining Key Questions: Q1 MiniMax 2.7 API characteristics (context/reasoning/pricing/latency); Q2 context-budget tuple + output-verification recipe (reuse 114); Q3 prompt-quality/RCAF + --variant mapping; Q4 quota-pool/fallback (minimax-api) + permissions-matrix; Q5 routing heuristics vs deepseek/qwen/glm + concrete file deltas.
...
- Write iteration narrative to: .opencode/specs/skilled-agent-orchestration/z_archive/098-cli-opencode-minimax-optimization/002-minimax-efficiency-deep-research/research/iterations/iteration-001.md
- Write per-iteration delta file to: .opencode/specs/skilled-agent-orchestration/z_archive/098-cli-opencode-minimax-optimization/002-minimax-efficiency-deep-research/research/deltas/iter-001.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/skilled-agent-orchestration/z_archive/098-cli-opencode-minimax-optimization/002-minimax-efficiency-deep-research/research/iterations/iteration-001.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/skilled-agent-orchestration/z_archive/098-cli-opencode-minimax-optimization/002-minimax-efficiency-deep-research/research/deltas/iter-001.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
