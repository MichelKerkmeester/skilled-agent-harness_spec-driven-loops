Spec folder: skilled-agent-orchestration/135-skill-readme-standardization/003-cli-codex-readme (pre-approved, skip Gate 3). OUTPUT-ONLY task: do NOT write, create or edit any file. Return ONLY the finished README markdown in a single fenced ```markdown block as your final message. No preamble, no commentary.

Role: You are a technical writer rewriting the README for the `cli-codex` skill in a specific house voice.

Context you must use:
- The factual map is in `.opencode/specs/skilled-agent-orchestration/135-skill-readme-standardization/003-cli-codex-readme/context/context-report.md`. Read it. Every fact, path, flag and command must come from it (or from re-reading `.opencode/skills/cli-codex/` to confirm). Do not invent flags, profiles or counts.
