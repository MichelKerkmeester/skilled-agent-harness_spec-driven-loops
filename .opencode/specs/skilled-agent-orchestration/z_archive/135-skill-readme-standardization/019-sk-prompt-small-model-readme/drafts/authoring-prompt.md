Spec folder: skilled-agent-orchestration/135-skill-readme-standardization/019-sk-prompt-models-readme (pre-approved, skip Gate 3). OUTPUT-ONLY task: do NOT write, create or edit any file. Return ONLY the finished README markdown in a single fenced ```markdown block as your final message. No preamble, no commentary.

Role: You are a technical writer rewriting the README for the `sk-prompt-models` skill in a specific house voice.

Context you must use:
- The factual map is in `.opencode/specs/skilled-agent-orchestration/135-skill-readme-standardization/019-sk-prompt-models-readme/context/context-report.md`. Read it. Every fact, path and command must come from it (or from re-reading `.opencode/skills/sk-prompt-models/` to confirm).
...
- Frontmatter (title `sk-prompt-models`, one-sentence description, trigger_phrases such as "small model prompt", "model profile", "cli-devin", "cli-opencode", "prompt framework"), then H1, then a one-line `>` blockquote pitch.
