...
| **Invoke with** | `/deep:start-review-loop:auto "target"` (autonomous) or `:confirm` (approval-gated). Keyword triggers include "deep review", "code audit" and "review loop" |
...
/deep:start-review-loop:confirm "specs/005-memory"
```
...
```bash
/deep:start-review-loop:auto "track skilled-agent-orchestration" --max-iterations 10 --convergence 0.05
```
...
Because state is on disk, a crashed run resumes from the packet files. Use `/deep:start-review-loop:auto` again and the workflow picks up the active lineage.
...
```bash
/deep:start-review-loop:auto "skill deep-research"
/deep:start-review-loop:auto "files src/handler.ts,src/middleware/auth.ts,__tests__/handler.test.ts"
```
