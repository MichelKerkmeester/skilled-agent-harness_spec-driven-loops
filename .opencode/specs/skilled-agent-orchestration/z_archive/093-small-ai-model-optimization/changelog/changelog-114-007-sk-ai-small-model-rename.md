---
title: "Changelog: Phase 7 — rename sk-small-model → sk-prompt-small-model [114-small-ai-model-optimization/007-sk-prompt-small-model-rename]"
description: "Chronological changelog for the Phase 7 — rename sk-small-model → sk-prompt-small-model phase."
trigger_phrases:
  - "phase changelog"
  - "nested changelog"
  - "phase completion"
...
> Spec folder: `.opencode/specs/skilled-agent-orchestration/z_archive/093-small-ai-model-optimization/007-sk-prompt-small-model-rename` (Level 2)
> Parent packet: `.opencode/specs/skilled-agent-orchestration/z_archive/093-small-ai-model-optimization`
...
- CHK-010 git mv .opencode/skills/sk-small-model .opencode/skills/sk-prompt-small-model succeeded; old dir absent; new dir present
- CHK-016 New sk-prompt-small-model/changelog/v0.3.0.0.md exists and documents the rename
- CHK-030 No hardcoded secrets introduced
- CHK-031 No new input handling, no new validation surfaces
- CHK-040 spec.md + plan.md + tasks.md + checklist.md + implementation-summary.md synchronized (all reference sk-prompt-small-model as canonical)
...
- CHK-011 sk-prompt-small-model/SKILL.md frontmatter name: reads sk-prompt-small-model
- CHK-012 sk-prompt-small-model/graph-metadata.json skill_id reads sk-prompt-small-model
...
- Live-surface underscore variant - rg "sk_small_model" .opencode/skills/sk-prompt-small-model/
- Historical-surface preservation - git diff --stat on 114/{001-006,review}/** + 026/.../iteration-009.md + sk-prompt-small-model/changelog/v0.1+v0.2
...
|---|---|---|
| `.opencode/skills/sk-small-model/` → `.opencode/skills/sk-prompt-small-model/` | Renamed dir | `git mv` whole directory; history preserved |
| `sk-prompt-small-model/SKILL.md` | Updated | frontmatter `name`, H1, in-body refs (incl. underscored Python identifier `sk_small_model` → `sk_ai_small_model`) |
| `sk-prompt-small-model/README.md` | Updated | title, H1, all body refs |
| `sk-prompt-small-model/description.json` | Updated | `name`, keywords[0], `pattern_index` path |
| `sk-prompt-small-model/graph-metadata.json` | Updated | `skill_id`, entities[].name/path, derived.key_files |
| `sk-prompt-small-model/references/pattern-index.md` | Updated | header + table self-refs |
| `sk-prompt-small-model/changelog/v0.3.0.0.md` | Created | Rename changelog with verification table |
| `cli-devin/graph-metadata.json` + `cli-opencode/graph-metadata.json` | Updated | `edges.enhances[].target` → `sk-prompt-small-model` |
...
| `~/.claude/projects/.../memory/feedback_skill_graph_compiler_rebuild.md` | Updated (surgical) | Tagged `(renamed sk-prompt-small-model 2026-05-21)` on 2026-05-18 narrative |
