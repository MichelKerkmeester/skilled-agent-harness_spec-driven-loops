...
| @multi-ai-council | `.claude/agents/multi-ai-council.md` | Planning-only multi-strategy architect (max 3 strategies)                              |
...
- Every custom agent has a definition file in `.claude/agents/`. These files contain specialized templates, enforcement rules, and quality standards. Dispatching a generic agent with "you are @debug" in the prompt loses the specialized debugging workflow. ALWAYS read and include the actual agent definition file. See §2.
