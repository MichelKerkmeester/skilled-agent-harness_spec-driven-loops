...
description: "Plan for an evidence-only 10-iteration deep-research run that investigates why deep-review can miss bugs found by focused bug-hunt research loops. The run uses cli-codex with gpt-5.5, high reasoning, and fast service tier."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Defined auto-mode research execution plan."
    next_safe_action: "Initialize deep-research state and run iteration 001."
    blockers: []
    key_files:
...
### Definition of Done
- [ ] 10 deep-research iterations complete or a workflow-owned halt reason is recorded.
- [ ] Research artifacts include config, state log, per-iteration markdown, deltas, dashboard, registry, and synthesis.
- [ ] Final synthesis ranks findings and recommendations.
- [ ] Packet metadata validates and indexes.
<!-- /ANCHOR:quality-gates -->
...
- **CLI Codex executor**: Runs each iteration with `gpt-5.5`, `high`, and `fast` settings.
...
- [ ] Generate `description.json` and refresh `graph-metadata.json`.
- [ ] Validate packet docs.

### Phase 2: Implementation
- [ ] Initialize `research/` state with executor settings.
- [ ] Run iterations 001-010 through the command-owned workflow semantics.
- [ ] Validate narrative, state-log, and delta artifacts for each iteration.

### Phase 3: Verification
- [ ] Generate or update `research/research.md` from iteration evidence.
- [ ] Index packet artifacts with Spec Kit Memory.
...
| Deep-Research Execution | High | 10 iteration runtimes |
