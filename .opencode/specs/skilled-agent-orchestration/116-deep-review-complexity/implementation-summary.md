...
_memory:
  continuity:
    ...
    recent_action: "Completed 10 auto deep-research iterations and synthesized findings."
...
| Field | Value |
|-------|-------|
| **Spec Folder** | `skilled-agent-orchestration/116-deep-review-complexity` |
...
This packet completed an evidence-only 10-iteration research run investigating why deep-review can miss bugs that focused deep-research finds. It produced packet-local research state, per-iteration narratives, per-iteration deltas, and a final synthesis at `research/research.md`. It does not change deep-review behavior yet.

### Research Packet Definition

The spec, plan, tasks, checklist, and decision record define an evidence-only workflow. The packet ran 10 deep-research iterations through `cli-codex` using `gpt-5.5`, `high` reasoning, and `fast` service tier, followed by a synthesis that ranks findings and recommendations.

### Research Outcome

The synthesis finds that deep-review's strongest rigor starts after a candidate finding exists. Focused deep-research performs better at bug discovery because it preserves hypotheses, unanswered questions, observations, ruled-out directions, and next-focus state as first-class research state. The highest-leverage recommendation is a versioned `searchLedger` enforced by deep-review validation and surfaced by reducer/dashboard/report outputs.
<!-- /ANCHOR:what-built -->
...
The packet was repaired from scaffold placeholders into populated Level 3 documentation, then executed to the requested 10-iteration research depth. Final strict validation and memory indexing are the remaining operational closeout steps.
<!-- /ANCHOR:how-delivered -->
...
| Deep-research iterations | PASS: 10 iteration narratives and 10 delta files exist. |
