...
description: "Synthesis of 10 deep-research iterations investigating why focused deep-research bug-finding can surface more bugs than deep-review."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Synthesized 10 deep-research iterations"
...
The 10-iteration research loop supports a stable diagnosis: `deep-review` is not shallow because it lacks review rigor. It is shallow at times because its strongest rigor starts after a candidate finding exists. Focused `deep-research` finds more bugs because it preserves hypothesis search, unanswered questions, observations, edges, and ruled-out directions as first-class state.

The highest-leverage fix is a versioned `searchLedger` for `deep-review`: each non-trivial review iteration should emit target-selection and candidate-search rows before severity adjudication. Those rows should be validator-enforced, reducer-preserved, dashboard-visible, report-visible, and eventually graph/convergence-aware.

Primary evidence sources are `research/iterations/iteration-001.md` through `research/iterations/iteration-010.md`, `research/deltas/iter-001.jsonl` through `research/deltas/iter-010.jsonl`, and `research/deep-research-dashboard.md`.
<!-- /ANCHOR:summary -->
...
Add `reviewDepthSchemaVersion`, `targetSelection`, `searchLedger`, `searchCoverage`, and `linkedCandidateIds` to the review iteration schema. `searchLedger` should be the canonical name; use `candidateMatrix` only as a temporary alias if needed.
...
Update `post-dispatch-validate.ts` so non-trivial review iterations fail when `findingsNew` has the wrong shape, `findingDetails` lacks rich fields for active findings, or `searchLedger` rows are missing, uncited, or have unsupported dispositions.

### R3: Persist null-search evidence through reducer, dashboard, and report

Priority: P0.

Reducer output should track `candidateCoverage`, `searchDebt`, `ruledOutCandidates`, and `searchCoverage`. The dashboard and final review report should show search debt next to active severity debt. The review report should include a first-class Search Ledger section before the appendix.
...
### R7: Treat higher iteration defaults as a supporting change only
...
- `post-dispatch-validate.ts`: reject missing ledger rows, bad `findingsNew` shape, uncited candidate rows, invalid dispositions, and shallow `findingDetails`.
...
- Deep-review reducer fixtures: add a `candidate-ledger-session` fixture and assert candidate coverage, search debt, ruled-out candidates, and dashboard Search Ledger output.
- Coverage graph tests: add candidate node vocabulary and STOP_BLOCKED cases for missing candidate-class or invariant/test coverage.
...
- Graphless runs still produce a text/JSON fallback ledger.
...
1. Add schema/docs/prompt support for `reviewDepthSchemaVersion`, `targetSelection`, and `searchLedger`.
2. Enforce `findingsNew`, rich `findingDetails`, and `searchLedger` shape in post-dispatch validation.
3. Extend reducer, registry, dashboard, and synthesis/report output to preserve search coverage and search debt.
4. Add candidate-saturation stop blockers using the ledger.
5. Add graph vocabulary for candidate coverage once ledger semantics are stable.
6. Add seeded behavior tests and manual playbook scenarios.
7. Revisit iteration defaults and convergence thresholds after the schema proves useful.
<!-- /ANCHOR:implementation-order -->
