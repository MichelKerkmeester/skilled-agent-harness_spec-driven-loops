...
- [x] CHK-001 [P0] Research complete: Unified Graph Architecture research verified (research.md exists in 138 root and covers all 4 integration patterns)
  - **Evidence**: Research complete in 138-hybrid-rag-fusion root (12 research files covering all 4 patterns)
- [x] CHK-002 [P0] Both source workstreams analyzed: 001-system-speckit-hybrid-rag-fusion (RAG Fusion) and 002-skill-graph-integration (Skill Graphs) specs reviewed for integration surface
...
- [x] CHK-010 [P0] `graphSearchFn` wired at `context-server.ts:566` — `hybridSearch.init()` receives exactly 3 arguments (semanticSearchFn, keywordSearchFn, graphSearchFn)
...
- [x] CHK-011 [P0] `graphSearchFn` wired at `db-state.ts:140` — re-initialization path also passes all 3 arguments (no regression from init-only wiring)
  - **Evidence**: db-state.ts reinitializeDatabase() passes graphSearchFnRef as 3rd arg to hybridSearch.init()
- [x] CHK-012 [P0] `createUnifiedGraphSearchFn()` queries BOTH causal edges (memory MCP SQLite) AND SGQS skill graph (filesystem-derived) and merges results
...
- [x] CHK-013 [P0] `SkillGraphCacheManager` is instantiated once at module load, not per query — prevents per-query filesystem rebuild (TTL ≥ 5 minutes)
  - **Evidence**: skillGraphCache singleton exported at module level in skill-graph-cache.ts with DEFAULT_TTL_MS = 300_000 (5 min)
- [x] CHK-014 [P0] `graphWeight` field added to `FusionWeights` interface in `adaptive-fusion.ts` — no existing fields renamed or removed
...
- [x] CHK-015 [P0] All 6 intent profiles updated with explicit `graphWeight` numeric values (research, debugging, implementation, exploration, synthesis, general)
...
- [x] CHK-016 [P0] Co-activation spreading return value is captured and incorporated into graph results — not discarded at the call site (previously lines 406-416)
...
- [x] CHK-017 [P0] Feature flag `SPECKIT_GRAPH_UNIFIED` gates all new code paths — setting to `false` at runtime bypasses the entire graph channel without restart
...
- [x] CHK-018 [P0] Zero schema migrations: v15 SQLite schema unchanged, no new tables or columns added for this integration
...
- [x] CHK-019 [P0] Zero external dependencies: no Neo4j client, no new npm packages in package.json
  - **Evidence**: No new dependencies in package.json — uses existing better-sqlite3, existing SGQS graph-builder
- [x] CHK-030 [P1] Graph search results use namespace-prefixed IDs exclusively: `mem:{id}` for memory nodes and `skill:{relative-path}` for skill graph nodes
  - **Evidence**: graph-search-fn.ts uses `mem:${row.id}` for causal edges (line 82) and `skill:${node.path}` for SGQS results (line 122)
- [x] CHK-031 [P1] Score normalization applied: all graph channel scores normalized to [0, 1] range before entering RRF re-ranking to ensure compatibility
...
- [x] CHK-020 [P0] Unit tests pass for graph-search-fn.ts — both the causal edge path (SQLite query) and SGQS path (cache hit and miss) covered
...
- [x] CHK-035 [P1] No hardcoded file paths in SkillGraphCacheManager — skill directory derived from runtime config or environment variable
...
- [x] CHK-036 [P1] Graph query paths do not accept user-supplied strings directly — all node IDs are internally generated, not passed from external input
...
- [x] CHK-037 [P2] OWASP review: no injection vector exists via graph query construction (SQLite queries use parameterized statements only)
...
- [x] CHK-038 [P1] spec.md, plan.md, tasks.md, and checklist.md in this folder (003-unified-graph-intelligence) are synchronized with implemented scope
...
- [x] CHK-039 [P1] TypeScript interfaces `UnifiedNodeId`, `GraphSearchResult`, `UnifiedEdge` documented in spec.md with field-level descriptions
...
- [ ] CHK-041 [P2] Integration architecture diagram (component + data-flow) present in plan.md showing how W:001, W:002, and W:003 connect
  - **Evidence**: Deferred (P2). plan.md contains text-based architecture description covering all 3 workstreams. ASCII/visual diagram not created.
...
- [x] CHK-043 [P1] All temporary files (benchmark scripts, debug output, prototype code) live exclusively in scratch/ — none in spec folder root or project root
...
- [x] CHK-044 [P1] scratch/ cleaned before completion claim — only reference artifacts retained (benchmark data may be promoted to docs)
...
- [x] CHK-045 [P2] Session context saved to memory/ using generate-context.js (not manual Write tool) before closing implementation sessions
...
- [x] CHK-046 [P1] New source files follow project directory conventions — no files created at unexpected paths
...
- [ ] CHK-103 [P2] Migration path documented in decision-record.md: how to graduate from virtual adapter to a real graph DB in a future phase if needed
  - **Evidence**: Deferred (P2). ADR-001 documents the Virtual Graph Adapter choice and alternatives considered. Migration path to real graph DB not formally documented.
...
- [x] CHK-110 [P0] Graph channel adds ≤ 15ms to pipeline latency (within the 45ms total scatter-gather budget allocated across 3 channels)
...
- [x] CHK-111 [P1] SkillGraphCacheManager hit rate > 95% in steady-state — cache misses occur only on TTL expiry or first cold start
...
- [x] CHK-112 [P1] Graph channel returns results for ≥ 15% of queries in the test set (channel not dead — confirms graph connectivity is meaningful)
...
- [ ] CHK-113 [P2] Full 7-pattern pipeline (all optional patterns P2 enabled) stays within 120ms p95 budget — documented as aspirational benchmark
  - **Evidence**: Deferred (P2). Individual pattern benchmarks pass (T011). Full 7-pattern combined measurement requires live DB with all flags enabled.
...
- [x] CHK-130 [P1] No new npm dependencies added — `package.json` diff confirms no additions to dependencies or devDependencies
...
- [x] CHK-131 [P1] No `neo4j` references exist anywhere in the codebase — confirmed by project-wide grep
  - **Evidence**: Zero neo4j references in codebase. Grep for 'neo4j' returns no matches.
- [x] CHK-132 [P2] OWASP review complete: no injection vulnerabilities identified in graph query paths (all SQLite statements parameterized, no eval or dynamic query construction)
...
- [x] CHK-140 [P1] spec.md, plan.md, tasks.md, checklist.md in 003-unified-graph-intelligence are synchronized — all reflect final implemented scope with no stale placeholder sections
...
- [x] CHK-141 [P1] TypeScript interfaces documented in spec.md with full field-level descriptions and usage examples for `UnifiedNodeId`, `GraphSearchResult`, `UnifiedEdge`
...
- [ ] CHK-142 [P2] Integration architecture diagram present in plan.md showing the data-flow from query intake through the 3-channel scatter-gather to RRF re-ranking with graph weight applied
  - **Evidence**: Deferred (P2). Same as CHK-041.
