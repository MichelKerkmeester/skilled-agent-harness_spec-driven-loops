...
| 9. `research.md` synthesis | Produces canonical research output in research format | Produces findings-first review report (`review-report.md`) with severity, evidence, gaps, and recommendations | REPLACE | This is the one component not preserved semantically. The current synthesis contract is explicitly for `research.md` and a 17-section research output, which does not fit a review report. [loop_protocol.md L412] [state_format.md L389] [spec_kit_deep-research_auto.yaml L332] |
...
| 13. Pause sentinel | Graceful between-iteration pause via `scratch/.deep-research-pause` | Same | REUSE-AS-IS | The behavior is content-agnostic. Only the user-facing copy/file name is arguably cosmetic. [loop_protocol.md L109] [spec_kit_deep-research_auto.yaml L209] |
...
1. **Reusing `research.md` with a cosmetic rename only.** Rejected because the current synthesis contract is research-shaped, not findings-first review-shaped. [loop_protocol.md L412]
