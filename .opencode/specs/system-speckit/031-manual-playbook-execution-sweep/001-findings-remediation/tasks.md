...
- [ ] T-0048 Fix needs-rebuild sentinel not clearing on successful boot rebuild (0048)
- [ ] T-0056 Add `scope` to `memory_causal_stats` accepted parameters, or correct scenario expectation (0056)
- [ ] T-0059 Fix `memory_get_learning_history` over-filtering (0059)
- [ ] T-0062 [B] Re-run scenario with stricter evidence requirement before proposing a fix (0062)
- [ ] T-0064 Fix eval reporting dashboard 0-eval-runs issue (0064)
- [ ] T-0074 Replace remaining unsafe `Math.max/min` spread call sites (0074)
- [ ] T-0077 Fix sha-256 dedup lookup predicate (canonical-path resolution before dedup check) (0077)
- [ ] T-0079 Fix inert/active logger-gate toggle default (0079)
- [ ] T-0085 Wire eval DB write path for `memory_search`/`memory_context` events (0085)
- [ ] T-0087 Re-run int8 quantization benchmark, update no-go decision record (0087)
- [ ] T-0110 Fix `SPECKIT_GRAPH_UNIFIED` Stage-2 flag propagation/cache invalidation (0110)
- [ ] T-0113 Fix adaptive-ranking `adaptiveShadow` proposal emission gate (0113)
- [ ] T-0126 Correct scenario doc's stale Vitest baseline count (350 → current), confirm no real regression first (0126)
- [ ] T-0129 Fix Stage-2 score sync for non-hybrid path / confirm routing (0129)
- [ ] T-0133 Apply `QUALITY_FLOOR` as a hard filter before min-representation selection (0133)
- [ ] T-0134 Wire `confidenceTruncation` trace metadata into real query execution path (0134)
- [ ] T-0156 Gate `encoding_intent` writes behind first-time-index check in `indexMemoryDeferred` (0156)
- [ ] T-0174 Fix `E_SESSION_SCOPE` on `memory_context` folder-routing check (0174)
- [ ] T-0175 [P0] Fix `vec_memories`/`vec_768` dual-write desync and blob-encoding mismatch (0175) — highest severity, real data-integrity gap
- [ ] T-0176 Fix quality-check retry loop not honoring `maxRetries` override (0176)
- [ ] T-0183 Fix DB hot-rebinding: invalid tier "scratch" + missing `.db-updated` marker + degraded health (0183) — confirm scenario-doc vs code bug first
- [ ] T-0185 [P0] Audit/fix DB-path construction so all consumers use the shared resolver (0185) — real correctness risk, processes could read/write different DB files
- [ ] T-0188 Confirm embedding model catalog change is intentional; fix scenario doc or revert (0188)
- [ ] T-0190 Confirm shadow-period gate is working-as-designed or misconfigured for learned-trigger persistence (0190)
- [ ] T-0192 Remove remaining V1 pipeline code path and `isPipelineV2Enabled()` flag check (0192)
