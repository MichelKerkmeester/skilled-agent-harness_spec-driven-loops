...
3. Re-checked the internal comparison surfaces: `AGENTS.md`, `.codex/AGENTS.md`, `.opencode/commands/speckit/README.txt`, `.opencode/commands/speckit/{plan,implement}.md`, `.opencode/skills/system-spec-kit/templates/`, `.opencode/skills/system-spec-kit/scripts/templates/compose.sh`, `.opencode/skills/system-spec-kit/scripts/spec/validate.sh`, `.opencode/skills/system-spec-kit/references/hooks/skill-advisor-hook.md`, `.opencode/skills/mcp-code-mode/SKILL.md`, and `.opencode/agents/*.md`.
...
| 8 | Personas/tone | Use SPAR personas as evaluation fixtures and audience maps, not runtime identities. | `adapt` | `065-persona-evaluation-fixtures` | SPAR has six personas in `external/Research/Personas/Personas.md`; internal `.codex/AGENTS.md` and `.opencode/agents/*.md` are voice and role-contract based. |
