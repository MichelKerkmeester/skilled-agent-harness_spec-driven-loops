...
> **Sort key:** git commit order on `system-speckit/004-memory-search-intelligence`, oldest to newest.
...
[`001-speckit-memory/022-kept-off-flag-resolution/`](./002-speckit-memory/030-kept-off-flag-resolution/).
...
[`001-speckit-memory/023-new-feature-research-build/`](./002-speckit-memory/032-new-feature-research-build/) and the held flags are recorded in
...
**CORRECTION (2026-07-01, drift audit remediation):** the "None flips" verdict above was superseded later in the 004-dark-flag-graduation track. `004-dark-flag-graduation/001-multihop-tail-appends` rewired the tail-additive path per the truncation-exemption probe named below, and its implementation-summary.md records a GRADUATE verdict with completeRecall@20 rising from 0.5625 to 0.9375 - the earlier REFINE/held call did not stand as the final answer. See `../feature-flags.md` and `004-dark-flag-graduation/001-multihop-tail-appends/` for the current state.
...
[`002-spec-data-quality/`](./003-spec-data-quality/), each carrying the full doc set of spec, plan, tasks,
...
[`changelog/002-spec-data-quality/`](./changelog/003-spec-data-quality/) with the
...
[`002-spec-data-quality/review/review-report-round2.md`](./003-spec-data-quality/review/review-report-round2.md).
...
`system-code-graph`, `002-skill-advisor`, `system-deep-loop` and `002-spec-data-quality`, each telling only its own
...
to 0, the envelope-fidelity enforcement and the four-flag scoring hardening. Under `002-spec-data-quality` the
