---
title: "Feature Specification: Phase 13: absorb-028-004-review-remediation-closeout"
description: "Program bookkeeping closeout: point the absorbed trackers (028/006/002, 028/006/004, ex-031 Group-A) at their owning phases as verify-first-then-close, reconstruct and complete the 91-item P2 mapping, run a finding-level completeness sweep of the findings ledger for silent drops, record three scaffolding tooling bugs, and run the final recursive validation."
trigger_phrases:
...
_memory:
  continuity:
    packet_pointer: "system-speckit/004-memory-search-intelligence/001-speckit-memory/031-fix-deep-dive-p0-p2-findings-for-mk-spec-memory/013-absorb-028-004-review-remediation-closeout"
...
# Feature Specification: Phase 13: absorb-028-004-review-remediation-closeout
...
**Scope Boundary**: Spec-document and metadata edits only. This phase writes tracker docs, mapping tables, parent rollups, and generator-owned JSON metadata. It ships zero code changes — every code fix belongs to phases 001-012, and the three tooling bugs it records are routed, not fixed here.
...
**Deliverables**:
- Absorbed trackers updated in place with pointers: `../../../004-review-remediation/002-memory-schema-and-concurrency/`, `../../../004-review-remediation/004-p2-triage/`, `../../../000-release-cleanup/015-manual-playbook-execution-sweep/001-findings-remediation/tasks.md`.
...
### In Scope
- Absorption pointer edits as verify-first-then-close (plan-review SYSTEMIC #1 confirmed P1-2/P1-4/P1-5 are ALREADY FIXED in live code): 028/006/002 item P1-2 (run the derived_id migration + add a twin-identity test) and item P1-4 (verify the scan already runs outside the lock + add a concurrency/interleaving test) → phase 008, item P1-5 (verify the fresh in-tx revalidation + add a regression test) → phase 009 — the pointers must not instruct 008/009 to re-fix correct code; ex-031 Group-A rows (T-0211/T-0212/REQ-214) → phase 007.
- Reconstructing and completing 028/006/004's 91-item P2 mapping table (the frozen per-item review-report source is unrecoverable — see REQ-002): reconstruct the per-item list from the deep-dive findings-ledger P2 entries cross-referenced with 004-p2-triage's own G1-G15 lens grouping, then map each item → covered-by-phase-NNN of this program, or accept-as-is with a reason.
...
- Finding-level completeness sweep over `../research/findings-ledger.md` (one row per finding, not one row per section — the no-silent-drops guarantee rests on the finding-level table in this phase's `tasks.md`, per plan-review SYSTEMIC #4 which caught 11 findings + a security item the section-level sweep missed).
- Recording three new tooling findings (create.sh child-level, upgrade-level.sh addendum paths, generate-description.js ignored `--level`) with reproducible evidence and routing them to a follow-on owner.
- Final program validation: `validate.sh --strict` across parent + 13 children, scoped `memory_index_scan`, `/memory:save` closeout.

### Out of Scope
- Any mcp_server or script code change — code fixes live in phases 001-012; the three tooling bugs are recorded and routed here, not fixed.
...
|-----------|-------------|-------------|
| `../../../004-review-remediation/002-memory-schema-and-concurrency/tasks.md` | Modify | Mark T005/T006 absorbed → phase 008, T007 absorbed → phase 009 as verify-first-then-close (already fixed in code per SYSTEMIC #1), with pointer paths |
| `../../../004-review-remediation/002-memory-schema-and-concurrency/spec.md` | Modify | Status note: scope absorbed into program 016 phases 008/009 |
| `../../../004-review-remediation/002-memory-schema-and-concurrency/implementation-summary.md` | Modify | Record absorption disposition instead of pending contract |
| `../../../004-review-remediation/004-p2-triage/spec.md` | Modify | Complete the reconstructed 91-item per-item mapping; repath this tracker's own inherited dead `../../archive/review-report.md` pointer (spec.md:65,86) to the reconstructed list |
| `../../../004-review-remediation/004-p2-triage/tasks.md` | Modify | Close T004-T011 against the completed mapping with evidence |
| `../../../004-review-remediation/004-p2-triage/implementation-summary.md` | Modify | Record the finalized triage state and routing |
| `../../../004-review-remediation/spec.md` | Modify | Phase-map rows for 002/004 updated from pending to absorbed/closed with pointers |
| `../../../004-review-remediation/graph-metadata.json` | Regenerate | Derived status refresh after the parent spec update |
...
|----|-------------|---------------------|
| REQ-001 | 028/006/002 tracker points at its absorbing phases as verify-first-then-close — plan-review SYSTEMIC #1 confirmed P1-2/P1-4/P1-5 are ALREADY FIXED in live code, so the pointers schedule verification + tests, not re-fixes: P1-2 → phase 008 (run the derived_id backfill migration + add a twin-identity test), P1-4 → phase 008 (verify the semantic-edge scan already runs outside `BEGIN IMMEDIATE` + add a concurrency/interleaving test), P1-5 → phase 009 (verify the fresh in-tx revalidation before DELETE + add a regression test) | Grep of `002-memory-schema-and-concurrency/tasks.md` shows T005/T006/T007 each carrying an absorbed-pointer line naming the owning phase folder AND framing it as verify-first-then-close (no row instructs 008/009 to re-implement already-correct code); no row still reads as an unowned pending contract |
| REQ-002 | 028/006/004's 91-item P2 mapping is reconstructed and complete — the frozen per-item source (`028/006/archive/review-report.md` and `../../archive/review-report.md`) does not exist (verified 2026-07-03: neither path resolves; no per-item enumeration survives anywhere in the 028 packet), so the list is reconstructed from the deep-dive findings-ledger P2 entries cross-referenced with 004-p2-triage's own G1-G15 lens grouping (`../../../004-review-remediation/004-p2-triage/spec.md` §"P2 Lens Triage"), then each item → covered by phase NNN of this program, or accept-as-is with a reason | The reconstructed table lists one row per item; zero rows without a disposition; every phase pointer names an existing child folder of this program; the reconstructed count is reconciled against the frozen "91" headline with any delta and its cause recorded (the G1-G15 counts are the review's own "approximate" figures, summing to ~79-86) rather than asserting a false 91 |
...
| REQ-004 | Finding-level completeness sweep: every finding in `../research/findings-ledger.md` is mapped to a phase task or explicitly accepted-as-is with a reason — no silent drops. The no-silent-drops guarantee rests on the finding-level table (one row per finding), not the section-level index: plan-review SYSTEMIC #4 traced 11 findings + a security item that the section-level sweep let slip past, pre-enumerated now with their owning phases | The finding-level table in this phase's `tasks.md` carries one row per previously-silent finding with its owning phase (the 13 pre-enumerated from SYSTEMIC #4), and the section-level index (L1-L9, DUP MECHANISM, agents E/F/I/A/C/G/D/H, absent Agent B via report §3/§4) is retained only as a coarse cross-check; zero unresolved rows in either |
...
| REQ-008 | The three scaffolding tooling findings are recorded as tracked items with repro and routed to a follow-on owner | All three findings (TOOL-1 create.sh child-level, TOOL-2 upgrade-level.sh addendum paths, TOOL-3 generate-description.js ignored `--level`) appear in this phase's `tasks.md` with script-line anchors and a reproducing command; the routing decision (owner or new packet) is recorded |

### Acceptance Scenarios

1. **Given** the absorbed 028/006/002 tracker, **When** a reader opens its `tasks.md` after this phase, **Then** rows T005/T006/T007 each name the program phase that owns the follow-up as verify-first-then-close (verification + tests, not a re-fix of already-correct code) and the doc makes no unowned pending claim.
2. **Given** the 91-item P2 set — whose frozen per-item source `028/006/archive/review-report.md` no longer exists and is reconstructed from the deep-dive ledger + 004-p2-triage's G1-G15 grouping, **When** the reconstructed mapping table is complete, **Then** every item resolves to exactly one disposition, every covered-by pointer names an existing phase folder, and the reconstructed count is reconciled against the "91" headline with any delta explained.
...
4. **Given** packet 028's parent rollup, **When** the status reconciliation lands, **Then** the phase-map row for 004-review-remediation no longer reports a stale blocker state and the regenerated graph metadata parses clean.
5. **Given** the findings ledger, **When** the finding-level completeness sweep finishes, **Then** the tasks.md finding-level table shows zero findings without either a phase-task owner or an accept-as-is reason, including the 13 previously-silent findings pre-enumerated from plan-review SYSTEMIC #4.
...
- **SC-002**: The reconstructed P2 list is fully mapped (count reconciled against the "91" headline with any delta explained; frozen per-item source unrecoverable); the finding-level ledger completeness table is complete with no silent drops (every previously-silent finding from SYSTEMIC #4 carries an owning phase).
...
- **SC-005**: All three tooling findings are reproducible from their recorded evidence by a reader with no session context.
<!-- /ANCHOR:success-criteria -->
...
- Who owns the three spec-kit tooling fixes (create.sh child-level, upgrade-level.sh addendum paths, generate-description.js ignored `--level`): a new speckit packet or an existing tooling tracker? Routed during execution, fix out of this program's scope.
<!-- /ANCHOR:questions -->
...
### Data Boundaries
- Frozen 91-item P2 source is unrecoverable (verified absent): reconstruct the per-item list from the deep-dive ledger + 004-p2-triage's G1-G15 grouping and record the count-reconciliation delta against the "91" headline — do NOT fabricate items to force the total to 91.
...
| Research | 12/20 | Full finding-level ledger sweep (~150 findings, 13 pre-enumerated silent drops), reconstructed 91-item P2 mapping, 14-row open-item sweep |
| **Total** | **30/70** | **Level 2** |
<!-- /ANCHOR:complexity -->
