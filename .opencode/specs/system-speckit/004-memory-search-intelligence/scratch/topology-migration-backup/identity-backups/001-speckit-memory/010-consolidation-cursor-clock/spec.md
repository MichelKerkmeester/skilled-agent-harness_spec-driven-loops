...
_memory:
  continuity:
    packet_pointer: "system-speckit/004-memory-search-intelligence/001-speckit-memory/010-consolidation-cursor-clock"
...
| ID | Requirement | Acceptance Criteria |
|----|-------------|---------------------|
| REQ-005 | C-G1: clock-driver around the existing cursor | A registerInterval-hosted driver advances the cursor on a clock (not just on save). Missed-tick behavior is Skip. The tick logs-and-continues, never fatal. It sits on C4-A so clock-replays do not double-apply |
...
| REQ-008 | LT-turn-cadence-trigger: turn-count cadence gate | A persistent `turns_counter` gates background consolidation work (`turns_counter % frequency == 0`, default 5). It is distinct from the existing token-ratio pressure gate and cron retention sweep |
