...
- [x] CHK-012 [P1] A4 reads the 015-calibrated scale, with graceful fallback.
  - **Evidence**: `memory-search.ts` imports `resolveAbsoluteRelevance`, and `tests/memory-search-quality-filter.vitest.ts` covers cosine-scale rows, lexical-only fallback, `null` similarity and `averageSimilarity`.
- [x] CHK-013 [P1] A7 keeps the on-disk value byte-identical and the lease policy untouched.
  - **Evidence**: `maintenance-marker.ts` derives `MAINTENANCE_MARKER_TTL_MS = 60_000 × 3 = 180_000`, and `git status --short .opencode/bin` showed no launcher edits.
<!-- /ANCHOR:code-quality -->
...
- [x] CHK-FIX-003 [P0] A7 preserves the live daemon-liveness contract.
  - **Evidence**: `derived TTL = ownerLease.ttlMs × K (K=3)` = 180000 stays > the 2× reclaim window (120s), and focused vitest checks the relationship against the launcher lease literal.
...
- [x] CHK-FIX-005 [P1] Evidence is pinned to seams and research citations.
  - **Evidence**: `tasks.md` records the file:line seams, and `spec.md` cites the 008 deltas (A4 → iter-002, A7 → iter-007) and the 015 fix reference.
<!-- /ANCHOR:fix-completeness -->
...
- [x] CHK-041 [P1] A7's two invariants are documented in-module.
  - **Evidence**: `maintenance-marker.ts` comments state the stale-reclaim relationship and the refresh-before-half-window rule using durable WHY only, and comment hygiene passed on touched code/test files.
...
- [x] CHK-051 [P1] No temp files left outside `scratch/`.
  - **Evidence**: No scratch/baseline files were created, and `git status --short` shows only scoped code/test/docs changes.
<!-- /ANCHOR:file-org -->
