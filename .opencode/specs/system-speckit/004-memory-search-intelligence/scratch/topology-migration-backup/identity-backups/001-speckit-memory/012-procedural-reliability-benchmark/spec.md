...
_memory:
  continuity:
    packet_pointer: "system-speckit/004-memory-search-intelligence/001-speckit-memory/012-procedural-reliability-benchmark"
...
**Critical dependencies**: three pieces gate the build. First, the shared bounded-Beta posterior primitive (`01-go-candidates.md` Shared-infrastructure, one f64 `(α₀+s)/(α₀+β₀+s+f)` + per-consumer adapters, because the live integer scorer throws on fractional inputs). Second, a net-new `'outcome'` emission source (`feedback-ledger.ts`). Third, for bad-pattern, a table-rebuild schema migration (the 6-value `RELATION_TYPES` / `CHECK` constraint has no `HAS_FAILURE`).
...
| **Level** | 3 |
| **Priority** | P2 |
| **Status** | Complete (no-go: flags and code removed in 007 flag-resolution, all 4 candidates benchmark-gated and unpromoted) |
| **Created** | 2026-06-19 |
...
| **Parent Packet** | system-speckit/004-memory-search-intelligence/001-speckit-memory |
<!-- /ANCHOR:metadata -->
...
| REQ-002 | Benefit micro-benchmark gates the reliability fold | A single benchmark shows reliability-weighting out-earns the existing `access`/confirmation signals on accrued/synthetic outcomes, and a non-result keeps M-procedural-reliability-recall PENDING. No candidate has a measured before/after number today [`03-corrections-caveats-and-residuals.md:33`] |
| REQ-003 | Shared bounded-Beta primitive is f64 + adapter, not the integer scorer | `(α₀+s)/(α₀+β₀+s+f)`, cold-start `0.5`, count-floor, built once as an f64 export with a procedural adapter, because the live integer `computeScore`/`shouldDemote` throws on the fractional inputs reliability needs [`01-go-candidates.md:65`, `bayesian-scorer.ts:182-191`] |
...
| Dependency | Shared bounded-Beta f64 primitive | Integer scorer throws on fractional inputs | f64 export + procedural adapter (REQ-003), shared with Advisor C4 (build once and wire twice) |
...
- **Single failed procedure (`0/1`)**: posterior `1/3`, penalized but not zeroed, so the procedure can still surface for an unrelated problem.
...
- **Benchmark-first, one candidate at a time.** Build the shared prerequisites (outcome emitter → f64 Beta primitive + adapter) BEFORE any reliability fold, then run the benefit micro-benchmark BEFORE promoting M-procedural-reliability-recall.
...
> All four candidates are **PENDING**. None appears in the shipped Wave-0 record (Wave-0 record, candidates 1-13, the procedural cluster is absent and the closest, C4-A idempotency, is unrelated). Gate keys: `needs-benchmark` (a benefit micro-benchmark must pass first), `schema-migration` (a table-rebuild is required), `shared-infra-dep` (depends on the outcome emitter + f64 Beta primitive), `already-exists-residual` (mechanism ships, only a thin residual is net-new).

| # | Candidate | Status | Gate | 030 Commit | Research evidence |
|---|-----------|--------|------|-----------|-------------------|
| 1 | M-procedural-reliability-recall | **PENDING** (default-off plumbing built) | needs-benchmark | none (not in 030 §14) | Flags registered: `SPECKIT_PROCEDURAL_OUTCOME_EMITTER`, `SPECKIT_PROCEDURAL_RELIABILITY_RECALL`, deterministic tests pass. Promotion still needs a measured benchmark delta [`iter-018.jsonl:4`, `iter-015.jsonl:2`] |
| 2 | M-bad-pattern-negative-memory | **PENDING** | schema-migration (+ filter-site audit) | none (not in 030 §14) | REFUTED-as-reuse, `HAS_FAILURE` rejected at 2 layers → table-rebuild, or `'deprecated'`/`contradicts` precedent + audit [`iter-021.jsonl:2`, `iter-018.jsonl:5`] |
| 3 | M-skill-induction-repetition | **PENDING** | needs-benchmark (heaviest, write-side risk) | none (not in 030 §14) | REFUTED-as-reuse, closed `ReconsolidationAction` union + no repetition counter → new action + counter + precision gate [`iter-021.jsonl:3`, `iter-018.jsonl:6`] |
| 4 | M-procedural-version-reset | **PENDING** | already-exists-residual | none (not in 030 §14) | REFUTED-as-net-new, append-only deprecate-never-delete already covers it, only the explicit reliability-reset-to-zero residual rides REQ-001 [`iter-021.jsonl:4`] |
