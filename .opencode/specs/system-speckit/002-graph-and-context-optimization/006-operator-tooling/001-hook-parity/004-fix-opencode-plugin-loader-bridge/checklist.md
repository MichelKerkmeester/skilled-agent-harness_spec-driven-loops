---
title: "...-spec-kit/026-graph-and-context-optimization/007-hook-parity/005-fix-opencode-plugin-loader-bridge/checklist]"
...
_memory:
  continuity:
    packet_pointer: "system-speckit/002-graph-and-context-optimization/006-operator-tooling/001-hook-parity/004-fix-opencode-plugin-loader-bridge"
...
- [x] **P0-05** Helper files relocated atomically. [EVIDENCE: 3 helpers moved from `.opencode/plugins/` to `.opencode/skills/system-spec-kit/mcp_server/plugin_bridges/`; imports updated. `git mv` could not take `.git/index.lock` in the sandbox, so the worktree records delete/add pairs until staged by the user.]
...
- [ ] **P1-04** No vitest regression elsewhere. [EVIDENCE: deferred — see implementation-summary.md Known Limitations item 1; out-of-scope Copilot hook wiring blocker tracked in parent 007-hook-parity docs as priority follow-up.]
...
- [x] **P1-S01** Helper relocation does not expose new attack surface. [EVIDENCE: helper files moved to `.opencode/skills/system-spec-kit/mcp_server/plugin_bridges/` and remain invoked by `node` subprocess spawn from plugin code; no new OpenCode plugin entrypoint was added.]
...
- [x] **P1-F02** Helper files live in their new home (e.g., `.opencode/skills/system-spec-kit/mcp_server/plugin_bridges/`) and are reachable from the plugin imports. [EVIDENCE: `ls .opencode/skills/system-spec-kit/mcp_server/plugin_bridges/` shows the 3 helper files; focused bridge tests and direct smokes pass.]
...
- 2026-04-22T15:13Z PASS: Outcome A implemented; three helpers moved to `.opencode/skills/system-spec-kit/mcp_server/plugin_bridges/`; two plugin files updated.
