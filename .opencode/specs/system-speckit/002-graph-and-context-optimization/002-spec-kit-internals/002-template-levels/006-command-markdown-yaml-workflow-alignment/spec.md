...
| `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/008-template-levels/graph-metadata.json` | Modify | Add this child packet and set it as the last active child. |
<!-- /ANCHOR:scope -->
...
**Acceptance Criteria**:
1. **Given** final docs are complete, **When** parent `graph-metadata.json` is read, **Then** `children_ids` includes `006-command-markdown-yaml-workflow-alignment`.
2. **Given** final docs are complete, **When** parent `graph-metadata.json` is read, **Then** `derived.last_active_child_id` is `006-command-markdown-yaml-workflow-alignment`.
