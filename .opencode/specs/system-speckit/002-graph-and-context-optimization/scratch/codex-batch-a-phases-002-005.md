# Codex CLI Prompt — Batch A: deep-review phases 002-005 (4 phases × 5 iterations)

You are running as codex CLI (`gpt-5.4`, `reasoning_effort=high`, `service_tier=fast`, `sandbox=workspace-write`). Self-contained prompt. This is **Batch A of 3** in a split batch deep-review of 026 phases 002-014. You review 4 phases here; Batches B and C review the remaining 9. All 3 batches share a single state file and whichever batch finishes last writes the consolidated report.

## SCOPE — BATCH A

Run a 5-iteration deep-review on each of these 4 phase folders:

| # | Phase slug |
|---|---|
| 1 | `002-implement-cache-warning-hooks` |
| 2 | `003-memory-quality-issues` (parent level only; sub-phases 003/001–003/009 are internal organization, review as one unit) |
...
**After this batch completes**: update `026/review/batch-phase-review-state.json` with the 4 phases you reviewed. If (and only if) the state file shows all 13 phases across Batches A+B+C are complete, also write the consolidated report at `026/review/batch-phase-review-consolidated.md`.
...
- You may WRITE only to:
  - `026/review/<phase-slug>/*` (new per-phase review packets for the 4 phases in this batch)
...
6. **If `026/review/batch-phase-review-state.json` exists**, read it first. It means Batch B or C ran before this one. Do NOT re-review phases already listed in `phaseResults`. Start from the first Batch A phase that is NOT yet in `phaseResults`.
...
1. Skip any Batch A phase that already appears in `phaseResults`
2. Start from the first unreviewed Batch A phase
3. Preserve the existing `phasesCompleted`, `phaseResults`, `aggregateFindings`, `startedAt`

## PER-PHASE EXECUTION PROTOCOL (repeat for each Batch A phase)
...
Continue to the next Batch A phase in the list. Do NOT stop between phases unless you hit the batch-level budget.

## BATCH-LEVEL EARLY-STOP

Stop the batch run if:
- **Budget exhausted**: you've used more than ~80% of your internal tool-call budget. Write partial state with `status: "paused"` and leave remaining Batch A phases in `phasesRemaining`. The operator can dispatch a resume prompt.
- **3+ consecutive phase errors**: stop and report.
- **All 4 Batch A phases done**: proceed to Step 7.
...
- **If YES**: write `026/review/batch-phase-review-consolidated.md` with this structure:
  ...
  description: "Consolidated verdict across all 13 child phases of 002-graph-and-context-optimization. Aggregate findings: <N> P0 / <N> P1 / <N> P2. Overall program verdict: <verdict>."
  importance_tier: "important"
  contextType: "review-report"
  ---
...
- **If NO** (other batches still pending): do NOT write the consolidated report. Just print in your output that phases X/13 are complete and Batches B and/or C still need to run.

## CONSTRAINTS (absolute)

- **READ ONLY for the review target**: do NOT modify any source file, test file, packet doc, or memory file
- **WRITE ONLY to allowed paths**: `026/review/<phase-slug>/*` for the 4 Batch A phases, `batch-phase-review-state.json`, and (conditionally) `batch-phase-review-consolidated.md`
...
```
=== BATCH_A_DEEP_REVIEW_RESULT ===
BATCH_STATUS: complete | paused | failed
PHASES_IN_THIS_BATCH: 002-implement-cache-warning-hooks, 003-memory-quality-issues, 004-agent-execution-guardrails, 005-provisional-measurement-contract

PER_PHASE_SUMMARY:
  002-implement-cache-warning-hooks:  iters=<n> verdict=<PASS|CONDITIONAL|FAIL> P0=<n> P1=<n> P2=<n> stopReason=<reason>
...
CONSOLIDATED_REPORT_WRITTEN: yes | no (no if phasesCompleted < 13; run Batches B and C first)
...
FILES_CREATED:
  4 x 026/review/<phase-slug>/ directories
  4 x review-report.md
  20 x iteration-NNN.md max (fewer if early-stopped)
  4 x (deep-review-config.json + state.jsonl + findings-registry.json + strategy.md + dashboard.md)
...
NEXT_STEP_RECOMMENDATION:
  - If phasesCompleted < 13: dispatch Batch B (phases 006-010) next
  - If Batch A paused mid-phase: dispatch resume prompt to finish remaining Batch A phases first
  - If consolidated report written: review it and decide next actions
=== END_BATCH_A_DEEP_REVIEW_RESULT ===
```
