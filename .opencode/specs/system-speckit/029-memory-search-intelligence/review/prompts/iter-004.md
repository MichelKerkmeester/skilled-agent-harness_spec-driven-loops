...
Retry; registry counts in prompt.

Review Iteration: 4 of 10
Mode: review
Dimension: maintainability
Review Target: .opencode/specs/system-speckit/029-memory-search-intelligence — SESSION SCOPE: Documentation-alignment review of the 028 Phase R remediation + 017-023 reconciliation merge (origin tip 8d52813080): spec-folder docs for children 006-016 (template conformance, evidence quality, status coherence per sk-doc standards); mcp_server READMEs (handlers/, lib/search/, lib/storage/) vs merged code; ENV_REFERENCE.md truthfulness; system-spec-kit SKILL.md/reference alignment for touched surfaces; comment hygiene; doc drift introduced by the merge. Alignment authorities: sk-doc create-quality-control standards, sk-code surface doctrine.
Review Scope Files: .opencode/specs/system-speckit/029-memory-search-intelligence/013-drift-marker-pipeline-resilience/*.md, .opencode/specs/system-speckit/029-memory-search-intelligence/014-self-healing-internals-hardening/*.md, .opencode/specs/system-speckit/029-memory-search-intelligence/015-validation-hardening-fixes/*.md, .opencode/specs/system-speckit/029-memory-search-intelligence/016-cross-package-flag-governance/*.md — slice 4 RETRY: template conformance, evidence quality, status coherence
...
- Config: .opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-config.json
- State Log: .opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-state.jsonl
- Findings Registry: .opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-findings-registry.json
- Strategy: .opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-strategy.md
- Write iteration narrative to: .opencode/specs/system-speckit/029-memory-search-intelligence/review/iterations/iteration-004.md
- Write per-iteration delta file to: .opencode/specs/system-speckit/029-memory-search-intelligence/review/deltas/iter-004.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/system-speckit/029-memory-search-intelligence/review/iterations/iteration-004.md`, this iteration's narrative markdown
  - `.opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-state.jsonl`, append-only JSONL state log
  - `.opencode/specs/system-speckit/029-memory-search-intelligence/review/deltas/iter-004.jsonl`, this iteration's delta JSONL
  - `.opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-strategy.md`, strategy.md (in-place updates only)
  - `.opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-findings-registry.json`, findings registry (in-place updates only)
- **BANNED OPERATIONS (NEVER execute against any path)**: `rm`, `rm -rf`, `git rm`, `mv`, `sed -i` (including `sed -i ''`), `rmdir`, `find ... -delete`, shell output-redirect truncate `>` against any file not in the allowed-write list, and any tool call whose effect is to delete, rename, or replace a file outside the allowed-write list. Reading is unrestricted; **writing, renaming, and deleting are scoped**.
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the review. NEVER execute the out-of-scope mutation. The review packet (`.opencode/specs/system-speckit/029-memory-search-intelligence/review/iterations/iteration-004.md` directory and parents) is the only zone for your writes; the reviewed target spec/code is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/system-speckit/029-memory-search-intelligence/review/iterations/iteration-004.md` (path pre-substituted for the current iteration number). Structure: headings for Dimension, Files Reviewed, Findings by Severity (P0/P1/P2), Traceability Checks, Verdict, Next Dimension.

2. **Canonical JSONL iteration record** APPENDED to `.opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-state.jsonl`. The record MUST use `"type":"iteration"` EXACTLY, NOT `"iteration_delta"` or any other variant. The reducer counts records where `type === "iteration"` only; other types are silently ignored. Required schema:
...
Append via single-line JSON with newline terminator, e.g. `echo '<single-line-json>' >> .opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-state.jsonl`. Do NOT pretty-print. Do NOT print to stdout only; it MUST land in the state log file.

3. **Per-iteration delta file** at `.opencode/specs/system-speckit/029-memory-search-intelligence/review/deltas/iter-004.jsonl` (path pre-substituted, e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same as the state-log append) plus per-event structured records (one per graphEvent, finding, classification, traceability-check, ruled_out direction). Each record on its own JSON line.
...
## SESSION BINDINGS (substitute into your JSONL records)
run: run-004 | sessionId: deep-review-20260710T153457Z | generation: 1 | lineageMode: new
Use REAL current ISO timestamps and real durationMs. APPEND DISCIPLINE (a prior iteration failed on this): append to the state log ONLY via a plain shell append — echo '<single-line-json>' >> <state log path>. NEVER use apply_patch/edit tools on deep-review-state.jsonl or delta files; the log is append-only and sibling records may differ from what you expect. Registry and strategy updates: read-then-rewrite is fine (you are the only writer right now).
Fabricated or synthetic timestamps are detected by an anomaly checker and fail the iteration.
Read .opencode/specs/system-speckit/029-memory-search-intelligence/review/deep-review-strategy.md FIRST for the slice plan and seed hypotheses; update its iteration log section when you finish.
GATE 3 / spec-folder: pre-answered (this review packet is the bound write authority). Non-interactive one-shot: never ask questions; proceed to completion.
