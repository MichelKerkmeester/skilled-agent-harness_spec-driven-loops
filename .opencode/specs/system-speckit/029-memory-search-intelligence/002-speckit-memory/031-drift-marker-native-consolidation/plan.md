---
title: "Implementation Plan: Drift-Marker Native Consolidation"
...
_memory:
  continuity:
    packet_pointer: "system-speckit/029-memory-search-intelligence/002-speckit-memory/031-drift-marker-native-consolidation"
    last_updated_at: "2026-07-11T17:47:04Z"
    last_updated_by: "openai-gpt-5.6-sol"
    recent_action: "Reconciled completed implementation and verification evidence into the plan phases"
    next_safe_action: "No implementation work remains; preserve the open commit-evidence pin in checklist.md"
    blockers: []
    key_files:
...
### Phase 1: Setup
- [x] Re-confirm all cited line ranges against the live tree in case a concurrent session has
  touched `memory-drift-marker.sh` or any of the four TS modules since this plan was written
- [x] Confirm the exact export list needed on `mcp_server/api/index.ts` (see Required
  inventories above) and stage the barrel addition
- [x] Add the `staleMs` parameter to `spec-folder-mutex.ts`'s `isReclaimableLock`, default
...
- [x] Create `scripts/git-hooks/drift-marker-write.ts` implementing the eight-step flow in Key
  Components above, importing all reused logic from `@spec-kit/mcp-server/api`
- [x] Port the diff-tree text-parsing step (the one piece with no TS equivalent) verbatim from
  `memory-drift-marker.sh:38-48`
- [x] Wire the `owner.json`-writing lock-acquire so dead-owner reclaim works immediately, not
  only after the 45s mtime fallback (REQ-004a)

### Phase 3: Hook Shrink
- [x] Replace `memory-drift-marker.sh:28-142`'s heredoc with one delegated call to the compiled
...
- [x] Confirm `post-commit`, `post-merge`, and `post-rewrite` need zero edits (REQ-007)

### Phase 4: Verification
- [x] New vitest coverage: dedup-key collapsing, atomic-write crash-mid-write recovery,
...
- [x] `spec-folder-mutex-liveness.vitest.ts` re-run to confirm the mutex's own 5-minute default
  and existing callers are unaffected (REQ-004c, NFR-C02)
- [x] `npx tsx evals/check-no-mcp-lib-imports.ts` and `bash check-api-boundary.sh` against the
  new file (REQ-005)
- [x] `npx tsx evals/check-source-dist-alignment.ts` to confirm the compiled `dist/` output the
  git hook actually calls matches the checked-in `.ts` source
- [x] Manual smoke test: a real commit touching `.opencode/specs`, run through the actual
  `post-commit` hook, confirms the marker is written exactly as before
- [x] `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh <this-spec-folder> --strict`
...
- [x] REQ-003 (byte-identical no-override output) verified before merge
- [x] `spec-folder-mutex-liveness.vitest.ts` confirmed green with the new `staleMs` parameter in
  place before merge
- [x] A real `post-commit` smoke run confirmed against a live commit before merge
