...
_memory:
  continuity:
    packet_pointer: "system-speckit/029-memory-search-intelligence/002-speckit-memory/016-orphan-sweep-scoped-scan-safety"
...
- [x] CHK-064 [P0] A synthetic long sweep, run through the REAL runGlobalOrphanSweep() function under an ENQUEUE-heavy synthetic orphan backlog (not a scan-only mirror harness), demonstrates at least one additional ctx.onPhase/marker-refresh call beyond the single phase-entry call (REQ-002) -- tests/orphan-sweep-time-budget-and-refresh.vitest.ts "re-fires ctx.onPhase during an enqueue-page-heavy sweep..." test (lines 254-290), PASS: 2500 real orphan rows queued into the drift-suspect queue (real `appendMemoryDriftSuspects` path) across multiple 200-row `ORPHAN_SWEEP_LIMIT` enqueue pages of the exported runIndexScan, onPhase('orphan-sweep') observed >1 time. This test exercises enqueue only, on a fresh suspect queue -- it never calls `deleteIndexedRecordIds`. Delete-cascade confirmation (`deleteIndexedRecordIds` -> `vectorIndex.deleteMemory` + `causalEdges.deleteEdgesForMemory` + `recordHistory`) is a separate code path (`runSuspectConfirmation`, handlers/memory-index.ts:993-1081) covered by the same file's "queues a discovered orphan during one scan and tombstones it only on the next scan" test (lines 177-223), not by this refresh-cadence measurement.
- [x] CHK-065 [P1] No gap between refresh calls during the synthetic long sweep exceeds the chosen cadence interval, and the interval stays under MAINTENANCE_MARKER_REFRESH_BEFORE_MS = 90,000ms (REQ-002, NFR-P02) -- enforced structurally by the rate-gate condition (`now - lastRefreshAt >= refreshCadenceMs`) checked after each 200-row `ORPHAN_SWEEP_LIMIT` enqueue page inside `runGlobalOrphanSweep`'s `refreshMarkerIfDue` (handlers/memory-index.ts:928-933, :958-962); env values are clamped (budget <= 90,000ms, cadence strictly below budget) via `resolveOrphanSweepRuntimeConfig` (handlers/memory-index.ts:338-360); the test suite captures real timestamps and asserts consecutive refresh-gap bounds plus page-bounded callback counts (tests/orphan-sweep-time-budget-and-refresh.vitest.ts:254-314) -- MEASURED, not inferred. Correction (2026-07-10 reconciliation): a chunked delete-cascade path (`ORPHAN_SWEEP_DELETE_CHUNK_SIZE = 25` with interleaved deadline/refresh checks inside `deleteIndexedRecordIds`) was added by the Phase R remediation commit `f9afa7a76c` but was removed by the subsequent packets-017-023 consolidation merge `ae4799bcfe` (confirmed absent at HEAD via `grep -n "ORPHAN_SWEEP_DELETE_CHUNK_SIZE\|chunkSize" handlers/memory-index.ts` returning no matches). The shipped refresh guarantee covers only the enqueue-page loop above; there is currently no delete-chunk loop to measure.
