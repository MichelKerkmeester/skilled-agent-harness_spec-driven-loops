...
_memory:
  continuity:
    packet_pointer: "system-speckit/029-memory-search-intelligence/001-speckit-memory/020-eval-calibration-ab"
...
**Dependencies**:
- **The 019 eval-harness spine (BLOCKING).** The ECE metric lane (`C9-3` over `lib/eval/eval-metrics.ts`) and the class-parameterized promotion gate (`A8` un-welding `lib/feedback/shadow-scoring.ts:43,:68,:93` from its hardcoded `meanNdcgDelta`) are the evidence machinery both candidates promote against (sibling phase `019-eval-harness-extension`). They are not yet a built sibling under `001-speckit-memory` (only the gate-zero precondition `001-corpus-reindex-gate-zero` exists). A2's ECE validation lane and A3's metric reporting both block on it.
- **Gate-zero corpus reindex** (`001-corpus-reindex-gate-zero`), no recall/calibration number is trustworthy against a partly-cold index. Reindex is gate-zero for every candidate here.
...
### Purpose
Make the dormant isotonic calibration promotable on real evidence, harvest the label pairs, binarize the graded golden set, build the missing ECE lane (as the harness consumer) and graduate the flag only when a real fit beats identity on held-out ECE. Then measure each of the three default-on levers with an on/off A/B on the golden set (fixing the S5 eval-mode blind spot first), reporting whether each earns its default-on status. Nothing graduates without the 019 harness evidence.
<!-- /ANCHOR:problem -->
...
### Out of Scope
- **Building the 019 eval-harness spine itself**, the `C9-4` coverage guard, `C9-1` single-pass emit, `C9-2` `enrichGroundTruth` tagging, the `C9-3` three corpus metrics as new plumbing and the `A8` per-class promotion gate are a separate sibling phase. This phase is a *consumer* (A2/A3 are the harness's downstream intelligence-class candidates), and it adds only the calibration-specific label-harvest + ECE-validation wiring and the A/B searchFn.
- The corpus reindex (`001-corpus-reindex-gate-zero`), a prior gate-zero phase, not re-run here.
...
| `.opencode/specs/system-speckit/029-memory-search-intelligence/001-speckit-memory/020-eval-calibration-ab/*` | Create | This phase's documentation. |
<!-- /ANCHOR:scope -->
...
| REQ-010 | Verification avoids live shards and host daemons. | Tests use temp/in-memory fixtures and the golden set, with no access to `mcp_server/database/**`. |
<!-- /ANCHOR:requirements -->
...
| Risk | 16/25 | Changes reported confidence magnitudes (on promotion) and measures default-on levers. Phantom-proxy, non-binary-label, eval-mode-blindness and S5-demotion traps, plus unmeasured leverage. |
