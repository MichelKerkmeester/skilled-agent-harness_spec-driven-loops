...
_memory:
  continuity:
    packet_pointer: "system-speckit/029-memory-search-intelligence/001-speckit-memory/008-edge-presence-currentness"
...
- [ ] T006 [P] Candidate C3-C: add the `TemporalMode` enum (Current/AsOf/AsKnownAt/History) + a `current-support` provider, keep Current byte-identical via `active_memory_projection`, gate AsKnownAt on C3-B (`contradiction-detection.ts` new enum/provider) [Pending].
