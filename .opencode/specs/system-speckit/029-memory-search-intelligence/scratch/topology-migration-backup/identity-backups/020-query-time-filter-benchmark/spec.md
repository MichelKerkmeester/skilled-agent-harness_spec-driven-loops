...
2. **The filter's concurrency-safety behavior is proven only under a simulated single-process lock, never
   ...
   found and fixed under `027-graph-and-context-optimization/007-mcp-daemon-reliability/{032,033,034}`
...
|-----------|-------------|--------------|
| `.opencode/specs/system-speckit/029-memory-search-intelligence/020-query-time-filter-benchmark/scripts/query-time-filter-latency-benchmark.mjs` | Create | REQ-001's self-contained latency harness, modeled on `004-dark-flag-graduation`'s per-child benchmark pattern |
| `.opencode/specs/system-speckit/029-memory-search-intelligence/020-query-time-filter-benchmark/results/` | Create | Raw benchmark output (JSON) backing the numbers reported in `implementation-summary.md` |
...
| `.opencode/specs/system-speckit/029-memory-search-intelligence/011-automatic-drift-self-healing/checklist.md` | Modify | Close `CHK-064` with a pointer to this phase's benchmark evidence once REQ-001 lands |
<!-- /ANCHOR:scope -->
