---
title: "Implementation Plan: Query-Time Existence Filter Benchmark & Hardening [template:level_2/plan.md]"
...
_memory:
  continuity:
    ...
    recent_action: "Authored plan.md alongside spec.md, status PLANNED"
    next_safe_action: "Await operator approval, then begin Phase 1"
    blockers:
      - "011's Layer 1 code is the object under test; re-confirm cited lines before Phase 2 starts"
    key_files:
      - ".opencode/skills/system-spec-kit/mcp_server/handlers/memory-search.ts"
      - ".opencode/skills/system-spec-kit/mcp_server/stress_test/durability/index-scan-coalescing-stress.vitest.ts"
      - ".opencode/specs/system-speckit/029-memory-search-intelligence/004-dark-flag-graduation/002-retrieval-class-weights/scripts/retrieval-class-routing-benchmark.mjs"
    session_dedup:
...
### Definition of Ready
- [ ] Requirements documented in spec.md with concrete acceptance criteria
- [ ] The four cited gaps (REQ-008 unexecuted, single-process-only concurrency proof, split unit/confirmation
...
- [ ] `004-dark-flag-graduation`'s benchmark safety pattern and `stress_test/durability/`'s isolation
      pattern reviewed as the two structural precedents this phase reuses

### Definition of Done
- [ ] All four REQ-001..REQ-004 acceptance criteria met with evidence in `implementation-summary.md`
- [ ] New tests passing: the stress file under `npm run stress:durability`, the e2e file under the normal
      vitest run
- [ ] `011/checklist.md` CHK-064 closed with a pointer to this phase's evidence (REQ-005)
- [ ] Docs updated (spec/plan/tasks/checklist/implementation-summary, plus `ENV_REFERENCE.md`)
<!-- /ANCHOR:quality-gates -->
...
### Phase 1: Setup
- [ ] Re-confirm all file:line references cited in spec.md and this plan against the live tree (011/014 may
      still be uncommitted at implementation time)
- [ ] Confirm the `004-dark-flag-graduation` benchmark harness pattern and the `stress_test/durability/`
      isolation pattern are still current by reading one harness of each kind end to end
- [ ] Identify or adapt a representative query set for REQ-001 from an existing sibling benchmark's fixtures

### Phase 2: Core Implementation
- [ ] Build the REQ-001 latency benchmark harness and run it, capturing real p50/p95/mean numbers
- [ ] Build the REQ-002 concurrency stress test under `stress_test/durability/`
- [ ] Build the REQ-003 end-to-end handler-level test
- [ ] Implement the REQ-004 aggregate counter (Option A or B, decision recorded) plus its read-accessible
      getter and `ENV_REFERENCE.md` update

### Phase 3: Verification
- [ ] REQ-001 numbers written into `implementation-summary.md` with the reproduce command
- [ ] REQ-002 stress test passes under `npm run stress:durability`
- [ ] REQ-003 e2e test passes under the normal vitest run
- [ ] REQ-004's aggregate-counter test passes and `ENV_REFERENCE.md` is updated
- [ ] `011/checklist.md` CHK-064 closed with a pointer to this phase's evidence (REQ-005)
- [ ] Code-review confirms no Layer 1/2/3 filtering/hook/sweep logic changed beyond the REQ-004 counter
      (REQ-006)
...
- [ ] Benchmark harness confirmed to open only a read-only backup, never the live DB, before its first run
- [ ] Stress test confirmed to use only a throwaway in-memory DB before its first run
- [ ] REQ-004's counter confirmed flag-gated (only accumulates when `SPECKIT_QUERY_TIME_EXISTENCE_FILTER`
