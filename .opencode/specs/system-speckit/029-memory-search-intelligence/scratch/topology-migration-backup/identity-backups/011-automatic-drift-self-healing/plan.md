...
_memory:
  continuity:
    packet_pointer: "system-speckit/029-memory-search-intelligence/011-automatic-drift-self-healing"
...
|------------|------|--------|-------------------|
| `007-search-index-integrity-sweep` | Internal | Planned, not yet shipped | Hard blocker — implementation cannot start until this lands and the tree is at a clean baseline |
| `008-metadata-rename-reconciliation` (specifically its `parentChain` resolver-routing fix) | Internal | Planned, not yet shipped | Hard blocker for Layer 2's post-move JSON regeneration step specifically; without it, regeneration would reintroduce the same drift bug it's meant to heal |
