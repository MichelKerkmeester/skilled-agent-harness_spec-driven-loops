...
| **Parent Spec** | ../spec.md |
| **Successor** | ../002-novel-embedding-drift-monitor/spec.md |
<!-- /ANCHOR:metadata -->
...
- The B1 sweep entrypoint and CI fan-out plumbing - owned by 011-scheduled-dq-sweep, this detector mounts on it.
...
| Dependency | 011-scheduled-dq-sweep | The detector has no fan-out caller of its own | Mount on the B1 sweep, it is one more detector in the report-mode set |
...
- **Depends on 011-scheduled-dq-sweep**: the detector is one entry in the B1 report-mode fan-out, it has no scheduler, CI workflow or runner of its own. It ships after the sweep exists.
- **Not gated on 015-prodmode-recall-gate**: this detector emits findings, never a vector row, so it is floor-bypassing by construction and the C2 prod-mode completeRecall@3 gate does not apply. Its value proof is a found contradiction on the live corpus, not a recall rise.
