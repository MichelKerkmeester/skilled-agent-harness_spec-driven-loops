...
_memory:
  continuity:
    packet_pointer: "029-memory-search-intelligence/003-spec-data-quality/002-retroactive-automation/013-retrieval-feedback-edge"
...
### Out of Scope
- Acting on edge-b below-floor rows. They are C2-gated and require the prod-mode completeRecall@3 proof from `015-prodmode-recall-gate`, which does not exist yet.
...
|------|------|--------|------------|
| Dependency | `015-prodmode-recall-gate` | Edge-b below-floor actions cannot be acted on until the prod-mode completeRecall@3 gate exists. | Keep edge-b advisory only. The gate is the explicit unblock condition named in the row. |
