...
### tri-176 — CLOSED
`manual_testing_playbook/08--scorer-fusion/projection.md:57` now points unbounded-projection triage at "traversal bounds in `mcp_server/lib/scorer/lanes/graph-causal.ts`" and notes `projection.ts` only clamps stored edge weights. Code: `lanes/graph-causal.ts` exists with `maxDepth` option (:9), default 2 (:27), depth gate (:61).
