...
`feature_catalog/11--scoring-and-calibration/shadow-feedback-holdout-evaluation.md:18-24` still describes live weekly holdout replay over 20% of queries, "Enabled by default", results logged to `shadow_scoring_log`. `manual_testing_playbook/11--scoring-and-calibration/shadow-feedback-speckit-shadow-feedback.md:18-24` makes empty `shadow_scoring_log` a FAIL. Current code makes the empty pool the *correct* outcome (`shadow-evaluation-runtime.ts:204-211`, `:423-427`) — a correct install fails the manual test.
