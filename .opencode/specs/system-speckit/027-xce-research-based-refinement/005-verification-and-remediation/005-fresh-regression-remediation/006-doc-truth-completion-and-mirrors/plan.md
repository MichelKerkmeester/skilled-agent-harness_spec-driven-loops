---
title: "Implementation Plan: Doc-Truth, Completion-Claim & Runtime-Mirror Reconciliation"
description: "Approach for remediating this sub-phase's 42 deep-review findings: confirm-then-fix each cited location, validate.sh --strict; cross-runtime body-diff + grep checks. Scaffold only."
...
Remediate the 42 findings in tasks.md. For each: open the cited `file:line`, confirm the defect, apply the registry recommendation (or record refuted-with-reason), and verify.
<!-- /ANCHOR:summary -->
...
- **Verify**: validate.sh --strict; cross-runtime body-diff + grep checks; update the registry finding status.
<!-- /ANCHOR:phases -->
