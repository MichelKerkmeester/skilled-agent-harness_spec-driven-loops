...
- [x] T003 [P] Document OpenCode-first wording requirements and shared recovery vocabulary, marking `context-server.ts` and runtime responses as the source of truth while `AGENTS.md` and `.opencode/agents/context.md` mirror that contract (`AGENTS.md`, `.opencode/agents/context.md`)
...
- [x] T009 Update first-turn/server guidance so OpenCode knows when structural context is already injected and when to recover (`mcp_server/context-server.ts`, `AGENTS.md`, `.opencode/agents/context.md`)
<!-- /ANCHOR:phase -->
