...
_memory:
  continuity:
    packet_pointer: "system-speckit/z_archive/024-compact-code-graph/024-hookless-priming-optimization"
...
| # | Item | What | Files |
|---|------|------|-------|
| 1 | Reframe @context-prime as best-effort | Remove "mandatory" language from orchestrate.md; add "skip if urgent" logic | .opencode/agents/context.md, orchestrate.md |
| 2 | Slim agent to 2 calls | Change from 4 calls (memory_context + code_graph_status + ccc_status + session_health) to 2 (session_resume + session_health) | .opencode/agents/context.md + all runtime copies |
| 3 | Urgency-aware bootstrap | If first message looks urgent ("fix", "error", "bug"), skip blocking prime and let MCP auto-prime handle it | .opencode/agents/context.md |
