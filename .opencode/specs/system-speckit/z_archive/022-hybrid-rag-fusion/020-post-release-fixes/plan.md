---
title: "Implementation Plan: 020 Pre-Release [system-spec-kit/022-hybrid-rag-fusion/020-post-release-fixes/plan]"
...
_memory:
  continuity:
    packet_pointer: "system-speckit/z_archive/022-hybrid-rag-fusion/020-post-release-fixes"
...
- [x] Refresh the modularization guard in `.opencode/skills/system-spec-kit/mcp_server/tests/modularization.vitest.ts` so `formatters/search-results.js` uses extended limit `536` and adjacent actual-count note `536`; verify with `npx vitest run tests/modularization.vitest.ts`, `timeout 180 npm run test:core`, and `python3 .opencode/skills/sk-code-opencode/scripts/verify_alignment_drift.py --root .opencode/skills/system-spec-kit`
...
| Validator tooling under `.opencode/skills/system-spec-kit/scripts/spec/` | Internal | Green | The packet cannot prove closure honestly |
<!-- /ANCHOR:dependencies -->
