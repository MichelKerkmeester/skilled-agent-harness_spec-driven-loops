...
● Read flowchart-generator.ts
  │ .opencode/skills/system-spec-kit/scripts/lib/flowchart-generator.ts
  └ 1 line read
...
● Read simulation-factory.ts
  │ .opencode/skills/system-spec-kit/scripts/lib/simulation-factory.ts
  └ 1 line read

● Search (glob)
  │ "**/shared/**/*{chunk*,embed*,trigger*}*" in .opencode/skills/system-spec-kit
  └ 4 files found
...
● Read semantic-signal-extractor.ts
  │ .opencode/skills/system-spec-kit/scripts/lib/semantic-signal-extractor.ts
  └ 422 lines read

● Read structure-aware-chunker.ts
  │ .opencode/skills/system-spec-kit/scripts/lib/structure-aware-chunker.ts
  └ 222 lines read
...
● Read topic-keywords.ts
  │ .opencode/skills/system-spec-kit/scripts/lib/topic-keywords.ts
  └ 45 lines read

● Read anchor-generator.ts
  │ .opencode/skills/system-spec-kit/scripts/lib/anchor-generator.ts
  └ 303 lines read

● Read decision-tree-generator.ts
  │ .opencode/skills/system-spec-kit/scripts/lib/decision-tree-generator.ts
  └ 213 lines read

● Read ascii-boxes.ts
  │ .opencode/skills/system-spec-kit/scripts/lib/ascii-boxes.ts
  └ 236 lines read
...
● Count lines in target and shared files (shell)
  │ wc -l '.opencode/skills/system-spec-kit/scripts/lib/'*.ts
  │ '.opencode/skills/system-spec-kit/shared/'*.ts '.opencode/skills/system-spec-kit/shared/lib/'*.ts
  │ | sed 's#^#LINES #'
  └ 24 lines...
...
● Search (grep)
  ...
  │ Timestamp)\b" in **/*.{ts,js} (.opencode/skills/system-spec-kit)
  └ 5 lines found
...
● Search (grep)
  ...
  │ **/*.{ts,js} (.opencode/skills/system-spec-kit)
  └ 94 lines found
...
● Search (grep)
  ...
  │ d)\b" in **/*.{ts,js} (.opencode/skills/system-spec-kit)
  └ 5 lines found
...
● Search (grep)
  ...
  │ wrapSectionsWithAnchors)\b" in **/*.{ts,js} (.opencode/skills/system-spec-kit)
  └ 74 lines found

● Diff shared and script chunkers (shell)
  │ diff -u '.opencode/skills/system-spec-kit/shared/lib/structure-aware-chunker.ts'
  │ '.opencode/skills/system-spec-kit/scripts/lib/structure-aware-chunker.ts' | sed -n '1,200p'
  └ 66 lines...
...
● Search (grep)
  ...
  │ mulationWarning|markAsSimulated)\b" in **/*.{ts,js} (.opencode/skills/system-spec-kit)
  └ 5 lines found
...
● Search (grep)
  │ "\b(tokenizeTopicWords)\b" in **/*.{ts,js} (.opencode/skills/system-spec-kit)
  └ 1 line found
...
● Search (grep)
  ...
  │ FileChanges|extractDecisions)\b" in **/*.{ts,js} (.opencode/skills/system-spec-kit)
  └ 88 lines found
...
- **Valid frontmatter can be treated as “missing,” which can prepend a second frontmatter block**
  - **File:** `.opencode/skills/system-spec-kit/scripts/lib/frontmatter-migration.ts`
...
- **Flowchart generation misclassifies long sequential workflows as parallel and then drops phases**
  - **File:** `.opencode/skills/system-spec-kit/scripts/lib/flowchart-generator.ts`
...
- **Content deduplication is O(n²) and uses a weak similarity heuristic**
  - **File:** `.opencode/skills/system-spec-kit/scripts/lib/content-filter.ts`
...
- **Topic/phrase dedupe uses O(n²) `indexOf` filters**
  - **File:** `.opencode/skills/system-spec-kit/scripts/lib/semantic-signal-extractor.ts`
...
- **Message classification work is repeated multiple times**
  - **File:** `.opencode/skills/system-spec-kit/scripts/lib/semantic-summarizer.ts`
...
- **Write-only / unused code**
  - **Files:**  
    - `.opencode/skills/system-spec-kit/scripts/lib/decision-tree-generator.ts` — `asciiBoxesAvailable` only assigned (`34`, `46`)  
    - `.opencode/skills/system-spec-kit/scripts/lib/topic-keywords.ts` — `tokenizeTopicWords()` has no repo consumers (`26`)  
    - `.opencode/skills/system-spec-kit/scripts/lib/anchor-generator.ts` — `category` is computed but unused in `wrapSectionsWithAnchors()` (`241`)
...
- **Magic widths and duplicated formatting policy**
  - **Files:** `.opencode/skills/system-spec-kit/scripts/lib/ascii-boxes.ts`, `.opencode/skills/system-spec-kit/scripts/lib/decision-tree-generator.ts`
...
- **Simulation helpers intentionally fork common utilities**
  - **File:** `.opencode/skills/system-spec-kit/scripts/lib/simulation-factory.ts`
...
- **Embeddings:** `.opencode/skills/system-spec-kit/scripts/lib/embeddings.ts:8-9` is a clean re-export of `.opencode/skills/system-spec-kit/shared/embeddings.ts`. No issue.
