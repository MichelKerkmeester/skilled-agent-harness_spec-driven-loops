...
| 185 | /memory:search command routing | **PASS** | `.opencode/commands/memory/search.md` documents the no-args prompt, analysis subcommands, first-token dispatch, and retrieval/analysis tool surface for the renamed command |
| 187 | Quick search (memory_quick_search) | **PASS** | `.opencode/skills/system-spec-kit/mcp_server/tools/memory-tools.ts:47-65` (delegation layer), `schemas/tool-input-schemas.ts:389-485` (schema + governed params), `tests/memory-tools.vitest.ts:41-64` (scope forwarding) |
...
| `.opencode/commands/memory/search.md` | 185, 187 — `/memory:search` routing gates, tool ownership, and subcommand contract |
| `.opencode/skills/system-spec-kit/mcp_server/tools/memory-tools.ts` | 187 — `memory_quick_search` delegation into `handleMemorySearch()` |
| `.opencode/skills/system-spec-kit/mcp_server/schemas/tool-input-schemas.ts` | 187 — `memory_quick_search` schema export and governed parameter allowlist |
| `.opencode/skills/system-spec-kit/mcp_server/tests/memory-tools.vitest.ts` | 187 — governed retrieval field forwarding coverage |
