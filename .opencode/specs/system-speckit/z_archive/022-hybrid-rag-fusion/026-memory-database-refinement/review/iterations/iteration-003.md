...
**Evidence**
At `.opencode/skills/system-spec-kit/mcp_server/lib/storage/causal-edges.ts:418-430`, traversal initializes one `visited` set for the whole walk and skips any edge whose next node ID was seen earlier:
`const visited = new Set<string>();`
`if (visited.has(nextId)) continue;`

That means edge-level uniqueness is never tracked, only node-level uniqueness. The existing "complex diamond cycle" test at `.opencode/skills/system-spec-kit/mcp_server/tests/causal-edges.vitest.ts:719-729` only asserts the final node set, so it would not catch the missing `C -> D` edge.
...
**Evidence**
At `.opencode/skills/system-spec-kit/mcp_server/handlers/causal-graph.ts:304-319`, `direction` is mapped to forward/backward/both and forward traversal is used for `outgoing`.
At `.opencode/skills/system-spec-kit/mcp_server/handlers/causal-graph.ts:395-412`, the response always serializes:
`causedBy: chain.by_cause, enabledBy: chain.by_enabled, ...`

For `both`, the handler explicitly merges forward and backward traversals at `.opencode/skills/system-spec-kit/mcp_server/handlers/causal-graph.ts:307-316`, but still exposes only relation-based buckets, not direction-aware buckets.
...
**Evidence**
At `.opencode/skills/system-spec-kit/mcp_server/lib/storage/causal-edges.ts:434-443`, the child strength is computed only from the current edge:
`const weightedStrength = Math.min(1, edge.strength * weight);`

The parent node's `strength` is ignored entirely.

The intended cumulative behavior is also implied by the weight test at `.opencode/skills/system-spec-kit/mcp_server/tests/causal-edges.vitest.ts:811-816`, which compares multi-hop scores using repeated multiplication:
...
**Evidence**
At `.opencode/skills/system-spec-kit/mcp_server/handlers/causal-graph.ts:307-316`, forward and backward traversals are executed separately.
At `.opencode/skills/system-spec-kit/mcp_server/handlers/causal-graph.ts:328-357`, additional `memory_index` lookups happen afterward in a separate phase.

Within storage, `getCausalChain()` itself issues per-node queries recursively (`getEdgesFrom()` / `getEdgesTo()`) at `.opencode/skills/system-spec-kit/mcp_server/lib/storage/causal-edges.ts:421-448`, so even one traversal is assembled from many discrete reads rather than a single consistent snapshot.
...
**Evidence**
`.opencode/skills/system-spec-kit/mcp_server/lib/storage/causal-edges.ts:39-40` defines `MAX_EDGES_LIMIT = 100`.
`.opencode/skills/system-spec-kit/mcp_server/lib/storage/causal-edges.ts:337-369` applies that limit in `getEdgesFrom()` / `getEdgesTo()`.
`.opencode/skills/system-spec-kit/mcp_server/lib/storage/causal-edges.ts:424-426` calls those helpers with no explicit override from traversal.
