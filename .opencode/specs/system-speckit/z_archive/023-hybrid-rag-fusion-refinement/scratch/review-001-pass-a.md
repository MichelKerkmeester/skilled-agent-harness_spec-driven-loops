...
| Add `verbatimModuleSyntax: true` to `shared/tsconfig.json` | No | This is a shipped phase task (`001-shared-esm-migration/spec.md:75,90`; `tasks.md:45`) and is present in the file (`.opencode/skills/system-spec-kit/shared/tsconfig.json:7-9`), but the changelog never mentions it. |
...
| Replace the no-op shared build with real `tsc --build` compilation | Yes | Covered in the fourth Architecture item (`.../changelog-001-shared-esm-migration.md:31-36`) and matches the package script (`.opencode/skills/system-spec-kit/shared/package.json:13-15`). |
