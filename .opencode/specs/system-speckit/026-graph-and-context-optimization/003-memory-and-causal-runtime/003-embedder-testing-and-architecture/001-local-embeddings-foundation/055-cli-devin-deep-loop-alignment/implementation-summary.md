...
_memory:
  continuity:
    packet_pointer: "system-speckit/026-graph-and-context-optimization/003-memory-and-causal-runtime/003-embedder-testing-and-architecture/001-local-embeddings-foundation/055-cli-devin-deep-loop-alignment"
...
- `.opencode/commands/deep/start-research-loop.md`: executor enum strings updated to list all 6 validator-accepted kinds (`native | cli-codex | cli-gemini | cli-claude-code | cli-opencode | cli-devin`) at line 79, line 124, and the executor option list.
- `.opencode/commands/deep/start-review-loop.md`: same enum-string updates at parallel locations.
- `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml`: `if_cli_devin:` branch ported in from `deep_start-review-loop_auto.yaml:806-829` (25 lines at the executor switch); follow-on edit wired `--agent-config` into the dispatch (inline `sed` substitutes `<repo-root>` and writes to a temp JSON in the prompt dir).
- `.opencode/commands/deep/assets/deep_start-review-loop_auto.yaml`: `if_cli_devin:` branch (already existed before 059) wired `--agent-config` into the dispatch using the same `sed` pattern, pointing at the review-iter recipe.
...
2. **Command updates** — sonnet @markdown applied 3 edits across 3 files (deep-research.md, deep-review.md, deep_start-research-loop_auto.yaml). All 3 surfaces pass sk-doc validate / yaml safe_load.
