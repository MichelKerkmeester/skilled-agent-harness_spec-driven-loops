...
- T001: Author `011-research-post-stress-finding-followups/{spec.md, plan.md, tasks.md, checklist.md, description.json, graph-metadata.json, implementation-summary.md}`
- T002: Update parent `003-fix-mcp-runtime-stress-findings/{spec.md, description.json, graph-metadata.json, resource-map.md}`
...
>
> **Source of evidence**: read .../003-fix-mcp-runtime-stress-findings/010-stress-test-close-loop-measurement-rerun/findings.md Recommendations §1-5; per-cell scores at `010-stress-test-close-loop-measurement-rerun/runs/*/score.md`; preflight log at `runs/preflight.log`.
>
> **(P0) cli-copilot `/memory:save` Gate 3 bypass.** I1/cli-copilot scored 2/8 (was 1/8 in v1.0.1). It still auto-selects a spec folder from session-bootstrap context (e.g., `004-legacy-phase-parent-migration`) and writes `last_save_at` + runs `memory_index_scan` without asking the operator. Codex and opencode honor planner-first. Investigate: where in `mcp_server/hooks/copilot/` or the `/memory:save` planner-first contract (`004-memory-save-rewrite/`) does the bypass originate? Recommend the minimal sufficient fix — tighter planner default, explicit Gate 3 prompt at CLI entry, model-side preamble, or copilot-specific intent classifier hook.
>
...
- 010-stress-test-close-loop-measurement-rerun/findings.md exists on disk (it does — committed in 3568eb1a5)
- `003-009` remediation packets exist on disk (they do — committed earlier)

**Read targets (per-iteration):**
- 010-stress-test-close-loop-measurement-rerun/findings.md — Recommendations + Cross-Reference table
- `010-stress-test-close-loop-measurement-rerun/runs/{cell}/{cli}-N/score.md` — per-cell evidence
- 004-memory-save-rewrite/spec.md — planner-first contract
- 005-code-graph-fail-fast-routing/spec.md — fallbackDecision matrix
...
2. Discard the new packet by `git rm -r 011-research-post-stress-finding-followups/` and reverting the parent-metadata edits.
