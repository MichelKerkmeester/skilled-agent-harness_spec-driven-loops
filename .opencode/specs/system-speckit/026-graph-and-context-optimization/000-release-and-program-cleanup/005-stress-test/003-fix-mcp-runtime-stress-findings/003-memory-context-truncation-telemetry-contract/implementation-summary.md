---
title: "Implementation Summary: memory_context truncation contract + token telemetry [system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/003-fix-mcp-runtime-stress-findings/003-memory-context-truncation-telemetry-contract/implementation-summary]"
...
| Field | Value |
|-------|-------|
| **Spec Folder** | 003-memory-context-truncation-telemetry-contract |
...
The MCP daemon restart procedure is owned by packet 008-mcp-daemon-rebuild-protocol. The source is patched and local compiled `dist` output contains the marker fields, but live runtime probes will not reflect the change until the MCP-owning client restarts its daemon child process:
