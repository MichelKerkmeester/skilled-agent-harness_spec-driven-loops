...
3. v1.0.2 baseline: `83.8%`, not exact same-cell comparable because v1.0.2 used 30 CLI-model cells (`010-stress-test-close-loop-measurement-rerun/findings-rubric.json:65`).
...
- **Corpus**: `SEARCH_QUALITY_EXTENDED_CORPUS`, version `004-search-rag-measurement-implementation-v1`, 12 cases (`mcp_server/stress_test/search-quality/corpus.ts:192`).
- **Rubric**: correctness / robustness / telemetry / regression-safety, scale `0-2`, equal weights. This follows the generalized stress-cycle dimensions (`feature_catalog/stress-testing/01-stress-test-cycle.md:40`).
...
- **Comparison method**: same 12-case layout as v1.0.3 for telemetry comparability. v1.0.2 remains a directional baseline only because its sidecar is a 30-cell CLI-model rubric (`010-stress-test-close-loop-measurement-rerun/findings.md:28`, `010-stress-test-close-loop-measurement-rerun/findings-rubric.json:65`).
...
Latency deltas are directional: v1.0.4 includes live handler response formatting and audit reads, while v1.0.3's harness metrics were fixture-only (`021-stress-test-enterprise-wiring-expansion/findings-v1-0-3.md:20`).
...
|---------------|---------------|----------|
| Live handler probe blocked by 30s readiness timeout | RESOLVED | v1.0.3 recorded the timeout caveat at `021-stress-test-enterprise-wiring-expansion/findings-v1-0-3.md:14`; v1.0.4 captured 16 live handler envelopes and 16 audit rows (`measurements/v1-0-4-summary.json:583`). |
| Harness fixture-only telemetry | RESOLVED | v1.0.3 recommended first-class export mode at `021-stress-test-enterprise-wiring-expansion/findings-v1-0-3.md:107`; v1.0.4 uses `telemetryExportPath` and records the export base (`measurements/v1-0-4-summary.json:7`). |
