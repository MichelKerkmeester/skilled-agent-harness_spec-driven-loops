...
_memory:
  continuity:
    packet_pointer: "system-speckit/028-xce-research-based-refinement/002-memory-store-and-search/002-memory-index-causal-lifecycle/004-write-path-reconciliation"
...
The idempotency receipt (retry-safe `memory_save`/`memory_update`) is the write primitive this reconciliation depends on, and the caura-memclaw study validates the async post-insert-enrichment model already required by REQ-010. Treat the receipt as a precondition for safe replay of the statediff action plan. Source: research/008-caura-memclaw-fleet-memory-teachings/sub-packet-proposals.md; planned in 010/002.
