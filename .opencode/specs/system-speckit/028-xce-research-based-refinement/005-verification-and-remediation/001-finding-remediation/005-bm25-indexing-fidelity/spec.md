---
title: "Feature Specification: Finding Remediation Lane: Bm25 Indexing Fidelity"
description: "Packed BM25 production-path fidelity: incremental indexing collapsing saves to body-only text (losing title/trigger weighting), RSS gate covering the sync path only, comparison-helper weighting blind spot."
trigger_phrases:
  - "bm25-indexing-fidelity remediation"
  - "027 finding lane 005"
  - "epic sweep lane 005"
...
_memory:
  continuity:
    packet_pointer: "system-speckit/028-xce-research-based-refinement/005-verification-and-remediation/001-finding-remediation/005-bm25-indexing-fidelity"
    ...
    session_dedup:
      ...
      session_id: "2026-06-11-005-bm25-indexing-fidelity-scaffold"
...
# Feature Specification: Finding Remediation Lane: Bm25 Indexing Fidelity
...
### Problem Statement
The epic deep-review sweep left this lane with 4 unverified P1 claims and 1 P2 clusters. Unverified single-seat claims can be neither fixed nor dismissed without evidence.

### Purpose
Packed BM25 production-path fidelity: incremental indexing collapsing saves to body-only text (losing title/trigger weighting), RSS gate covering the sync path only, comparison-helper weighting blind spot.
<!-- /ANCHOR:problem -->
...
### In Scope
- The lane's backlog entries in `../backlog/p1-backlog.json` and `../backlog/p2-backlog.json` (lane = `005-bm25-indexing-fidelity`).
