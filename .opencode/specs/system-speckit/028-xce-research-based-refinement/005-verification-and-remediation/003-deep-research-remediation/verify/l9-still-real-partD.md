...
`manual_testing_playbook/scorer-fusion/projection.md:57` still instructs "Inspect projection caps in `projection.ts`". `scorer/projection.ts` contains only the edge-weight clamp `Math.max(0, Math.min(1, row.weight))` (~:306); traversal bounding lives in `scorer/lanes/graph-causal.ts` (`maxDepth` option :9, default 2 at :27, depth gate :61). The advisor doc batch did not touch this playbook section.
