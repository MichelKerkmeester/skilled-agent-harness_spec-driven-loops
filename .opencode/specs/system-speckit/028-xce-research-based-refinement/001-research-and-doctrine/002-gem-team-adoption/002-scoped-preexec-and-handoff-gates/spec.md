...
_memory:
  continuity:
    packet_pointer: "system-speckit/028-xce-research-based-refinement/001-research-and-doctrine/002-gem-team-adoption/002-scoped-preexec-and-handoff-gates"
...
| **Parent Packet** | `028-xce-research-based-refinement/001-research-and-doctrine/002-gem-team-adoption` |
...
| `.opencode/skills/sk-code/SKILL.md` | Not modified (by design) | Gate B is owned centrally in `@orchestrate` + `agent-io-contract.md`; per REQ-001/NFR-Maintainability the predicate is deliberately NOT copied into this leaf surface (tracked deferral) |
...
| REQ-005 | The boundary contract-first gate (Gate B) triggers for API/schema/integration only, and is owned centrally (not duplicated into the leaf code path). | `@orchestrate` defines the Gate B predicate (`change_class ∈ {api,schema,integration}` ⇒ require a contract / boundary test / executable acceptance check before production edits) at `orchestrate.md:262`, codified by the shared contract `agent-io-contract.md:130`; it explicitly does not trigger on ordinary edits. Per REQ-001/NFR-Maintainability and `agent-io-contract.md:115` (leaf agents must not carry predicate copies), the gate is intentionally NOT added to `sk-code/SKILL.md` — that named edit is a tracked, by-design deferral so the predicate cannot drift across agents. |
