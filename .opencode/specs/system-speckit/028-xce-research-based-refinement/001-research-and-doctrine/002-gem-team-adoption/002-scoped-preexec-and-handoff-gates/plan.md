---
title: "Implementation Plan: 027/006/002 Scoped Pre-Execution & Handoff Gates"
...
_memory:
  continuity:
    ...
    answered_questions: []
---
# Implementation Plan: 027/006/002 Scoped Pre-Execution & Handoff Gates
...
Phase 002 wires three scoped gates onto existing agent contracts. The predicates live in `@orchestrate`; `@debug` emits a typed debug→implement handoff; `@code` receiver-validates it; `sk-code` adds a boundary contract-first check for API/schema/integration intent only; and `@orchestrate` adds a pre-mortem field for medium/high work. The work is additive docs/contract edits plus scaffold flags — no runtime rewrite, no governance or validator change, and 001's typed envelope must land first.
<!-- /ANCHOR:summary -->
...
- **Typed debug-handoff (Gate A)**: `root_cause`/`target_files`/`fix_recommendations`/`confidence`, emitted by `@debug`, preserved by `@orchestrate`, validated by `@code`. Reuses 001's envelope `confidence`/`failure_type`.
...
|-------|------------|--------|
| Phase 1: Setup | `001` envelope shipped | Gate A reuses 001's `confidence`/`failure_type`; insertion points must be read before editing. |
...
|------------|------|--------|-------------------|
| `001-typed-agent-io-adapter` typed envelope | Upstream (hard) | Must ship first (Wave 1) | Gate A's `confidence`/`failure_type` fields have no canonical home until 001 lands |
