...
## YOUR ANGLE (#7)
Cross-model adherence baseline: from the captured gpt-5.5-medium transcripts (/tmp/cmdtest/*.json if readable, else reason from the contracts), characterize which contract features survived dispatch and which were paraphrased away, and rank command families by adherence risk.

## OUTPUT
ONLY a fenced ```json block:
{"angle": 7, "findings": [{"class": "DOC-DRIFT|REFINEMENT|NEW-FEATURE|BUG", "severity": "P1|P2|P3", "title": "<short>", "evidence": "<file:line or concrete comparison>", "detail": "<2-3 sentences>", "fix_sketch": "<one sentence>"}], "summary": "<2 sentences>"}