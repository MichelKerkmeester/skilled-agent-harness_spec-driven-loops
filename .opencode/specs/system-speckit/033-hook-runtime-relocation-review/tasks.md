...
- [x] T042 Probe gate: a load-time probe extension proved Pi's loader accepts symlinked `*.ts` in `.pi/extensions/` and resolves relative imports against the SYMLINK path, not the realpath (`import.meta.url` reported the `.pi/extensions/` URL; the `.pi`-base import resolved OK, the realpath-base import failed). [evidence: Pi's own `dist/core/extensions/loader.js` also explicitly discovers `isSymbolicLink()` entries]
- [x] T043 `git mv` all 13 Pi extensions + `lib/claude-hook-adapter.ts` to their owners (portable trio of concerns -> `.opencode/hooks/{dispatch,mcp-route-guard,post-edit-quality}/pi/`; spec-gate + session bridges + adapter lib -> `system-spec-kit/mcp-server/hooks/pi/`; `prompt-advisor.ts` -> `system-skill-advisor/hooks/pi/`; `git-preflight-advisory.ts` -> `sk-git/scripts/hooks/pi/`), with relative symlinks back and imports byte-identical per the probe finding. [evidence: 12 top-level + 1 lib symlink, 0 broken]
- [x] T044 Live behavioral proof through the symlinks: `pi -p` session injected the full Gate-3 A-E question via the relocated `spec-gate-classify.ts`. [evidence: model reproduced the injected question verbatim on request]
- [x] T045 [P] Behavioral "WHAT IT DOES AND INJECTS" sections added to the 4 injecting concern READMEs with verbatim strings from the cores (block/warn envelopes, 5 hard-rule messages, audit JSONL shape, hygiene/dist banners, deep-loop warning templates, Fable deny reason); `shared/` clarified as injecting nothing.
- [x] T046 [P] `.pi/extensions/README.md` + `lib/README.md` rewritten as the symlink mirror index; 3 new behavioral READMEs for the skill-side `pi/` folders; root tree README's Pi paragraph corrected (Pi CAN live here via symlinks) and KEY FILES table updated.
- [x] T047 Verification: live Pi run clean with all 13 symlinked extensions, 11 touched docs validate 0 issues, core suites re-run 0 fail. [evidence: `pi --offline --approve -p` exit 0 with the injected Gate-3 question reproduced; `validate_document.py` runs; combined `node --test` output, this session]
