...
- [ ] T023 [P1] The recovery sweep deletes each stale processing file before the merged canonical marker is durably written, and the failure log claims files were "left unconsumed" after they are already gone (`mcp_server/lib/storage/memory-drift-processing-sweep.ts:103` vs `:126,:131`). It also treats every `.processing-*` file as stale with no PID/lease check and merges without the producer's lock or temp+rename (`:65,:111-126`). Publish the merged marker atomically under the shared writer lock first, delete sources only after the rename commits, and skip claims owned by live PIDs. Evidence: forced write-failure test asserting sources survive; concurrent-boot claim test.
- [ ] T024 [P1] Default-on feedback logging performs one synchronous insert per result on the search response path, plus `trackQueryAndDetect`/`logResultCited`, each exposed to the shared 10s busy_timeout (`mcp_server/handlers/memory-search.ts:2029`). Queue/batch feedback events off the response path in one transaction with bounded retries and backpressure.
- [ ] T025 [P2] The drift-suspect write is bounded-synchronous and lossy on timeout — the catch only logs and the batch is dropped (`handlers/memory-search.ts:421`). Defer a copied ID batch until after response construction with bounded retry, or explicitly document "bounded synchronous, lossy best effort" and rename the test/acceptance claims ("response not delayed" is currently overstated — `tests/memory-search-drift-suspect-write-timeout.vitest.ts:107` measures the write inside the elapsed window).
- [ ] T026 [P2] The `busy_timeout` restore in `finally` runs outside a nested catch; a restore failure escapes the stated non-fatal boundary (`handlers/memory-search.ts:433`). Guard restoration; on failure log and recycle the connection.
- [ ] T027 [P2] The malformed-marker test asserts the second, no-op invocation — the first call's `unrecoverable: 1` result is discarded (`mcp_server/tests/startup-checks.vitest.ts:364`). Invoke once, assert the first result and the preserve/delete policy; add a forced read-error case.
- [ ] T028 [P2] `ENV_REFERENCE.md:430` documents `SPECKIT_CONSUMPTION_LOG` default `true` while the implementation enables only on explicit `true`/`1` — runtime default OFF (`consumption-logger.ts:97-100`). Fix the doc or deliberately graduate with hot-path-safe logging.
- [ ] T029 [P2] `decision-audit.ts:36` resolves its default output path against `process.cwd()`, which has already created two spurious nested `.opencode/skills/system-spec-kit/...` trees (skill root and `scripts/`), fragmenting audit history. Anchor the default to the workspace/package root or DB directory; make the append async/buffered; add default-path tests from repo root, skill root, and scripts root; remove the two stray trees. Root-cause the sibling symptom: the untracked `vectors/` directory at the skill root comes from the active DB directory resolving there during some run (`mcp_server/lib/search/vector-index-store.ts:425`) — add a guard/assertion on the resolved base dir.
