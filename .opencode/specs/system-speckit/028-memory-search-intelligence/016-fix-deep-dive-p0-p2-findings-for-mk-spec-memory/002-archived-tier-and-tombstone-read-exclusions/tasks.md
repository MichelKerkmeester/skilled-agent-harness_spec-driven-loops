...
_memory:
  continuity:
    ...
    recent_action: "Authored task breakdown from phase decomposition and deep-dive findings"
    next_safe_action: "Start T-001 (confirm-before-fix group) - no code changes before T-006 baseline"
    blockers: []
    key_files:
...
- [ ] T-004 [P] Confirm channel filter gaps: graph injection has no tier/tombstone filter (lib/search/causal-boost.ts:457,687), summary-lane archive filter is a no-op stub (lib/search/pipeline/stage1-candidate-gen.ts:167), rescue backfill bypasses gates (lib/search/rerank/retrieval-rescue.ts:388), community injection has no existence/tier check (lib/graph/community-detection.ts) — refs: Chain A step 4 🟢, #7 🟡, ledger DUP MECHANISM, Agent C P2, Agent D P2
  <!-- agent: direct | deps: [] | touched-files: [] -->
- [ ] T-005 [P] Confirm archived-tier phantom state: 'archived' referenced in exclusion predicates but absent from VALID_TIERS (lib/scoring/importance-tiers.ts); `includeArchived` hardcoded false; `is_archived=0` on all 33,101 rows — refs: ledger L5 🟢, Agent H refinement, Agent E contract
  <!-- agent: direct | deps: [] | touched-files: [] -->
...
- [ ] T-007 Create shared active-row predicate module: `ACTIVE_ROW_SQL(alias)` + `isActiveRow(row, opts)` with `{ includeArchived, lane }` options; unit tests for all option/row-state combos (lib/search/active-row-predicate.ts) — refs: phase-decomposition §002 predicate line, ADR-001/ADR-002
  <!-- agent: direct | deps: [T-006] | touched-files: [mcp_server/lib/search/active-row-predicate.ts, mcp_server/tests/unit/active-row-predicate.vitest.ts] -->
...
- [ ] T-017 Archived tier end-to-end: add 'archived' to VALID_TIERS (lib/scoring/importance-tiers.ts); settable via memory_update; unhardcode `includeArchived` param through the search surface (lib/search/search-utils.ts, lib/storage/ports/vector-store.ts) — refs: ledger L5 🟢, Agent H refinement, Agent E contract
...
- [ ] T-020 Parser substring tier fix + retro-fix migration: tier from frontmatter only (lib/parsing/memory-parser.ts:892); migration recomputes tier for the 948-row substring-inflated population (272 z_archive overlap resolved by T-018 first) with audit (lib/search/vector-index-schema.ts) — refs: ledger Agent H P2 🟢(data), ADR-004
...
- **Specification**: See `spec.md` (REQ-001..REQ-010 map to T-007..T-020)
