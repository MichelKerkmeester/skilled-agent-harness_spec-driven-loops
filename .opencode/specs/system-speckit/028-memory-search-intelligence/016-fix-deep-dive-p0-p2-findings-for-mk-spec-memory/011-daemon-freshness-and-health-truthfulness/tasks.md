...
- [ ] T004 Deadlock fix: export cache-write helpers from the checker that reuse its own source *enumeration* `collectSourceFiles` PLUS `hashSourceFiles`/`cachePathFor` (keep enumeration + cache-path logic single-sourced) and have `finalize-dist.mjs` write per-entry source-hash caches after a successful build for mcp_server entries (default, spec-memory-cli, validation-orchestrator). ENUMERATION INVARIANT: the finalizer MUST hash the checker's own enumerated set (`collectSourceFiles`, walked at dist-freshness.cjs:333) — reusing only the hash fn is insufficient; if the finalizer's file set diverges from the checker's, the stored hash never matches the recompute at :349 and the deadlock persists. Verify the checker returns fresh via the hash short-circuit (:349-364), not the mtime path, immediately after finalize — breaks the bootstrap deadlock where the cache is only written after an mtime pass at dist-freshness.cjs:394 [Chain C 🟢] (.opencode/skills/system-spec-kit/mcp_server/scripts/finalize-dist.mjs, .opencode/skills/system-spec-kit/scripts/lib/dist-freshness.cjs)
...
- [ ] T006 Exit taxonomy: run the consumer inventory FIRST (`rg -n "75|exitCode|staleDistWarning|fail_open" .opencode/bin .opencode/skills/system-spec-kit/mcp_server/hooks .claude --glob '*.{ts,js,cjs,md}'`). CONSERVATIVE DEFAULT: document stale-dist as a non-retryable sub-case *within* exit 75 (do NOT move it off 75) — CLAUDE.md's "75 = retryable" is a live consumer contract (hooks, doctor routes, README) and stale/missing already exits 75 at spec-memory.cjs:55-56. Introduce a distinct non-retryable stale-dist exit code ONLY if the inventory proves a caller actually mis-retries stale-dist; otherwise keep it inside 75. Record the decision + rationale inline here; unify the missing-dist/stale-dist recovery texts (dist-freshness.cjs:314,390); commit the reconciled shim state from T002 [#28 🟢, Agent I P1] (.opencode/bin/spec-memory.cjs, .opencode/skills/system-spec-kit/scripts/lib/dist-freshness.cjs)
