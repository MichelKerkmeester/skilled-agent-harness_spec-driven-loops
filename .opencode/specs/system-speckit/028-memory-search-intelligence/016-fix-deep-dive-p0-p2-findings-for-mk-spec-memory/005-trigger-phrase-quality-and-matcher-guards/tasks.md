...
- [ ] T007 specFolder prefix matching in match_triggers scope filter, consistent with every other read surface; keep fail-closed behavior for scoped requests (.opencode/skills/system-spec-kit/mcp_server/handlers/memory-triggers.ts:449,269)
...
- [ ] T013 (path, mtime)-keyed phrase extraction cache: cache loader reuses extraction results for unchanged (path, mtime) pairs; TTL expiry stops triggering full-corpus disk re-reads (.opencode/skills/system-spec-kit/mcp_server/lib/parsing/trigger-matcher.ts:333)
