...
_memory:
  continuity:
    ...
    recent_action: "F1+F2 implemented, tested (real DB), validate.sh --strict PASSED"
...
- [x] T007 [P] Unit test: synthetic long sweep, run through the REAL runGlobalOrphanSweep() function under a DELETE-heavy synthetic orphan backlog (not a scan-only mirror harness -- a follow-up adversarial review found the pure SCAN path already safe, so this must exercise the DELETE cascade at deleteIndexedRecordIds, memory-index.ts:650-724, to verify anything new), spy on ctx.onPhase shows more than one refresh call, with no gap between calls exceeding the chosen cadence interval (REQ-002) -- tests/orphan-sweep-time-budget-and-refresh.vitest.ts, 2500-row real DELETE-cascade backlog via exported runIndexScan, PASS
