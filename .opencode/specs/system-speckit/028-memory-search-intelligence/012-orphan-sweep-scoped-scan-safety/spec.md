...
- **SC-001**: A large orphan backlog (larger than the chosen time-budget's worth of pages) can no longer
  ...
  **MET**: `tests/orphan-sweep-time-budget-and-refresh.vitest.ts` -- a 2500-row real DELETE-heavy backlog
  through the real `runIndexScan`/`runGlobalOrphanSweep` produces more than one `ctx.onPhase('orphan-sweep')`
