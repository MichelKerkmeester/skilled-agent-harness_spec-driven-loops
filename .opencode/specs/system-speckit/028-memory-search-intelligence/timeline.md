...
**CORRECTION (2026-07-01, drift audit remediation):** the "None flips" verdict above was superseded later in the 007-dark-flag-graduation track. `007-dark-flag-graduation/001-multihop-tail-appends` rewired the tail-additive path per the truncation-exemption probe named below, and its implementation-summary.md records a GRADUATE verdict with completeRecall@20 rising from 0.5625 to 0.9375 - the earlier REFINE/held call did not stand as the final answer. See `../feature-flags.md` and `007-dark-flag-graduation/001-multihop-tail-appends/` for the current state.
...
On 2026-07-05 the `008-speckit-surface-alignment` remediation packet was documented as its own closure milestone. The packet began from the surface-alignment audit and the Fable review that trimmed the record to citation-verified drift. Audit phases `011-code-graph-doc-audit` and `012-stress-and-skillmd-audit` produced read-only reports, then `011-fix-code-graph-docs` and `012-fix-stress-docs` corrected the confirmed documentation surfaces. Phase `013-deep-research-loop-instrumentation` shipped the reducer-side `novelty_signal_inert` warning for flat-high `newInfoRatio` with focused RED/GREEN coverage, and phase `014-recorded-failure-closure` reconciled the recorded deep-research cap plus added the constitutional recorded-failure route and surfacer. The new 008 changelog entry is [`changelog/changelog-008-speckit-surface-alignment.md`](./changelog/changelog-008-speckit-surface-alignment.md).
