...
description: "Phase parent for the seven release, memory-engine, skill-advisor, data-quality, remediation, dark-flag and surface-alignment tracks."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Renumbered 028 top-level to a contiguous 000-006 after the subsystem extraction"
    next_safe_action: "Commit the renumber and push to origin/028"
    blockers: []
    key_files:
      - "spec.md"
      - "context-index.md"
      ...
      - "002-skill-advisor/spec.md"
      - "000-release-cleanup/spec.md"
      - "003-spec-data-quality/SUMMARY.md"
...
### Problem Statement
Packet 028 owns the planning record that turns external memory-system research into shipped retrieval intelligence, release cleanup, review remediation, data-quality generation, dark-flag graduation and surface-alignment work. After the 2026-07-06 subsystem extraction its top-level children are a contiguous 000 through 006: release cleanup, the memory-search engine, the held skill-advisor follow-up, the spec-data-quality lineage, the review-remediation track, the dark-flag graduation program and the surface-alignment remediation — so research inputs, candidate plans, cleanup scopes, child validation state and benchmark verdicts are easy to navigate. The kept-off flag-resolution, new-feature-research-build and reranker-research phases are nested under `001-speckit-memory` as children 022 through 024, followed by the off-corpus eval gate, lexical-grounding floor, envelope-fidelity enforcement, scoring-hardening and substrate sandbox cleanup builds as children 025 through 029, and a 30th child, `030-opencode-temp-worker-reaping`, a planned scaffold not yet started.

### Purpose
Provide the root purpose, child map and cross-packet boundary for packet 028. After the 2026-07-06 subsystem extraction, 028 is scoped to the **memory-search engine** (`001-speckit-memory`) and **spec data-quality** (`003-spec-data-quality`), plus release-cleanup, the held skill-advisor follow-up, review-remediation and the dark-flag/surface-alignment programs. The code-graph, skill-advisor-runtime and deep-loop subsystems were extracted to their own sibling packets — see `context-index.md` for the full migration record and a consolidated summary of the memory-search and data-quality work.
...
- Phase-documentation map for children 000 through 006.
...
- Cross-packet boundary to the extracted code-graph, skill-advisor-runtime and deep-loop siblings.
...
| `002-skill-advisor/spec.md` | Modify | 002 | Held skill-advisor follow-up phase-parent map |
...
| 002 | `002-skill-advisor/` | Holds only the in-progress `001-hard-rule-and-dispatch-preflight-hardening`. The advisor runtime work (formerly phases 001-008 of this track) was extracted to `system-skill-advisor/002-skill-advisor-runtime/` on 2026-07-06 | Phase parent |
| 003 | `003-spec-data-quality/` | Spec-kit data-quality research that then shipped: child phases spanning the go/no-go research scaffold, benchmark and generated-JSON bridge, generated-metadata build, full-repo JSON migration, flag-graduation benchmark and search-quality/evidence-gap wave, plus the drift-audit, metadata-status-integrity and create.sh-corruption phases (045-053) re-nested here from top-level on 2026-07-04 | Complete |
| 004 | `004-review-remediation/` | Six-child review-remediation track (`001-eval-benchmark-fidelity` through `006-review-record-packet-type`); the `002-memory-schema-and-concurrency` and `004-p2-triage` closures were absorbed into the 016 deep-dive program on 2026-07-04 | Phase parent |
| 005 | `005-dark-flag-graduation/` | Eight-child dark-flag graduation suite that benchmarked built-but-default-off capabilities, returned graduate, refine or cut verdicts and closed follow-up validation and a deep review | Phase parent |
| 006 | `006-speckit-surface-alignment/` | Five-child surface-alignment remediation: false-now doc corrections, stress-doc fixes, the stress-and-skill.md audit, recorded-failure closure and manual-test verification | Phase parent |
...
- Child 002 holds only the in-progress skill-advisor follow-up `001-hard-rule-and-dispatch-preflight-hardening`; the advisor runtime phases were extracted to `system-skill-advisor`.
...
- Child 004 is the review-remediation track, with six children; the `002` and `004` closures were absorbed into the 016 deep-dive program and closed 2026-07-04.
- Child 005 is the dark-flag graduation suite, with eight benchmark, cleanup, validation, follow-up and review children.
- Child 006 is the surface-alignment remediation, with five doc-correction, fix, audit, recorded-failure-closure and manual-test-verification children.
- **Top-level ceiling (governance):** All new phase work nests as a child of an existing top-level folder by subject; never create a new top-level `0NN-` folder. On 2026-07-06 the three subsystem phase-parents that had accreted here were extracted to their own sibling packets — the code-graph track to `system-code-graph/`, the deep-loop track to `system-deep-loop/`, and the skill-advisor runtime (its former phases 001-008) to `system-skill-advisor/`, leaving `002-skill-advisor/` holding only the in-progress hard-rule follow-up. 028 is now scoped to the memory-search engine (`001-speckit-memory`) and data-quality (`003-spec-data-quality`) plus release-cleanup, review-remediation and the surface-alignment/dark-flag programs. Full migration record: `context-index.md`.
...
- **Child parents**: `000-release-cleanup/`, `001-speckit-memory/`, `002-skill-advisor/`, `003-spec-data-quality/`, `004-review-remediation/`, `005-dark-flag-graduation/`, `006-speckit-surface-alignment/`
