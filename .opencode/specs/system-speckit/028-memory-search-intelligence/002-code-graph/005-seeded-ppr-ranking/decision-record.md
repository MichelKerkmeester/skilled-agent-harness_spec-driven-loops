...
- The 027 substrate lives in a different subsystem (Memory MCP) than the consumer (Code Graph), so cross-subsystem reuse may need a thin adapter.
<!-- /ANCHOR:adr-001-context -->
...
| Someone forks a second walker under time pressure | M | This ADR + REQ-001 record the reuse directive, plus a review gate |
<!-- /ANCHOR:adr-001-consequences -->
...
**We chose**: build a SingleHop/MultiHop/Entity query-class taxonomy and GATE PPR ON for impact/multi-hop modes only (OFF for the single-hop default, fail-safe toward OFF on an ambiguous class), then land the PPR MECHANISM with safe defaults and treat the tuned damping factor, power-method cap and decay magnitudes as a separate benchmark follow-up.
...
| Build the vector-seed union | Lexical keeps expansion alive when the embedder is down | Re-introduces the semantic/vector backend the module deliberately removed, a scope violation | 1/10 |
...
| Risk | Impact | Mitigation |
|------|--------|------------|
| Someone reads CUT as deferred and tries to wire a vector backend | M | This ADR records it as a SCOPE VIOLATION, not deferred, plus a review gate |
<!-- /ANCHOR:adr-003-consequences -->
