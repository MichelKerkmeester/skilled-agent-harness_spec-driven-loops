...
Mode: review
Dimension: DETERMINISM — TARGET 1 ONLY. Building on iteration 1's call-graph, determine whether the corruption is deterministic: does it happen on EVERY '--phase --phase-parent' invocation, or only under specific conditions (e.g. a positional feature_description argument vs a flag-based one, a stale/missing parent description.json at scaffold time, argument-parsing/shift bugs in create.sh that misalign $1/$2 across the parent vs child code paths, a shared/reused variable in generate-description.ts that isn't reset between parent-metadata-preserve and child-metadata-create calls, etc.). If you reproduce, do it ONLY in an isolated /tmp copy per the constraint in iteration 1 (never against a real packet in this repo), and clean up afterward. State your confidence and what conditions trigger vs do not trigger the bug, with file:line evidence.
...
- Write iteration narrative to: .opencode/specs/system-speckit/028-memory-search-intelligence/003-spec-data-quality/047-generated-metadata-status-integrity/review/iterations/iteration-002.md
- Write per-iteration delta file to: .opencode/specs/system-speckit/028-memory-search-intelligence/003-spec-data-quality/047-generated-metadata-status-integrity/review/deltas/iter-002.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/system-speckit/028-memory-search-intelligence/003-spec-data-quality/047-generated-metadata-status-integrity/review/iterations/iteration-002.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/system-speckit/028-memory-search-intelligence/003-spec-data-quality/047-generated-metadata-status-integrity/review/deltas/iter-002.jsonl`, this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the review. NEVER execute the out-of-scope mutation. The review packet (`.opencode/specs/system-speckit/028-memory-search-intelligence/003-spec-data-quality/047-generated-metadata-status-integrity/review/iterations/iteration-002.md` directory and parents) is the only zone for your writes; the reviewed target spec/code is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/system-speckit/028-memory-search-intelligence/003-spec-data-quality/047-generated-metadata-status-integrity/review/iterations/iteration-002.md` (path pre-substituted for the current iteration number). Structure: headings for Dimension, Files Reviewed, Findings by Severity (P0/P1/P2), Traceability Checks, Verdict, Next Dimension.
...
3. **Per-iteration delta file** at `.opencode/specs/system-speckit/028-memory-search-intelligence/003-spec-data-quality/047-generated-metadata-status-integrity/review/deltas/iter-002.jsonl` (path pre-substituted, e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same as the state-log append) plus per-event structured records (one per graphEvent, finding, classification, traceability-check, ruled_out direction). Each record on its own JSON line.
