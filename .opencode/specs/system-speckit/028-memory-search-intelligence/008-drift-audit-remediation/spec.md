...
description: "Fix the 75 documentation/metadata drift and bug findings surfaced by a GPT-5.5-fast (high) cli-opencode audit of the 028-memory-search-intelligence packet: 24 confirmed high/critical (re-verified against real files) plus 51 unverified medium/low findings."
trigger_phrases:
...
_memory:
  continuity:
    packet_pointer: "system-speckit/028-memory-search-intelligence/008-drift-audit-remediation"
...
> **PASS-2 FOLLOW-UP (2026-07-01):** a second pass, `009-drift-audit-deep-history-correction` (sibling folder), supplements the 4 code-gap findings' corrections here with real git history -- all 4 were built, shadow-shipped, benchmarked, and deliberately deleted for cause, not simply absent. This pass's GENUINELY_ABSENT verdicts were correct about the current tree but incomplete on that fuller history. See `../009-drift-audit-deep-history-correction/` for details.
...
|-----------|-------------|-------------|
| 42 directories across `.opencode/specs/system-speckit/028-memory-search-intelligence/**` and 2 skill source files | Modify | Corrected per-finding, see `tasks.md` Phase 2 for the full per-directory task list |
<!-- /ANCHOR:scope -->
...
- **Source audit**: 10-dispatch GPT-5.5-fast (high) sweep across `../000-release-cleanup/` through `../007-dark-flag-graduation/` (findings enumerated in `tasks.md`)
- **Plan**: See `plan.md`
