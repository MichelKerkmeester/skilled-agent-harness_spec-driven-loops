...
- Any change to `feature-flags.md`'s narrative sections describing other flag families
  (`028-memory-search-intelligence/feature-flags.md`), beyond whatever this packet's own findings
...
| `.opencode/specs/system-speckit/028-memory-search-intelligence/021-graph-preservation-quality-benchmark/benchmark-results.md` | Create | Findings record: per-flag, per-slice Recall@K/nDCG/MRR deltas on the reindexed snapshot |
