...
- The Copilot branch always uses `--allow-all-tools`, which is the broadest permission surface in the matrix, and there is no parallel config knob to narrow it. The command surface is therefore more permissive than the shared executor schema suggests. [Evidence: .opencode/commands/spec_kit/assets/spec_kit_deep-research_auto.yaml:528-551; .opencode/commands/spec_kit/deep-research.md:114-119]
- Cross-CLI delegation is explicitly documented as possible inside each executor sandbox, and runtime enforcement is explicitly absent, so a mis-scoped executor can invoke additional CLIs without a built-in recursion or privilege boundary check. [Evidence: .opencode/skills/sk-deep-research/SKILL.md:73-88; .opencode/specs/system-spec-kit/026-graph-and-context-optimization/008-runtime-executor-hardening/002-sk-deep-cli-runtime-execution/002-runtime-matrix/decision-record.md:119-160]
