...
1. Skip any Batch C phase that already appears in `phaseResults`
2. Start from the first unreviewed Batch C phase
3. Preserve the existing `phasesCompleted`, `phaseResults`, `aggregateFindings`, `startedAt`

## PER-PHASE EXECUTION PROTOCOL (repeat for each Batch C phase)
...
Continue to the next Batch C phase in the list. Do NOT stop between phases unless you hit the batch-level budget.

## BATCH-LEVEL EARLY-STOP

Stop the batch run if:
- **Budget exhausted**: you've used more than ~80% of your internal tool-call budget. Write partial state with `status: "paused"` and leave remaining Batch C phases in `phasesRemaining`. The operator can dispatch a resume prompt.
- **3+ consecutive phase errors**: stop and report.
- **All 4 Batch C phases done**: proceed to Step 7.
...
- **If YES**: write `026/review/batch-phase-review-consolidated.md` with this structure:
  ...
  description: "Consolidated verdict across all 13 child phases of 027-graph-and-context-optimization. Aggregate findings: <N> P0 / <N> P1 / <N> P2. Overall program verdict: <verdict>."
  importance_tier: "important"
  contextType: "review-report"
  ---
...
- **WRITE ONLY to allowed paths**: `026/review/<phase-slug>/*` for the 4 Batch C phases, `batch-phase-review-state.json`, and (conditionally) `batch-phase-review-consolidated.md`
...
CONSOLIDATED_REPORT_WRITTEN: yes | no (no if phasesCompleted < 13; Batches A and/or B still need to run)
...
NEXT_STEP_RECOMMENDATION:
  - If phasesCompleted < 13: dispatch the remaining batch (A or B) next
  - If Batch C paused mid-phase: dispatch resume prompt to finish remaining Batch C phases first
  - If consolidated report written: review it and decide next actions
=== END_BATCH_C_DEEP_REVIEW_RESULT ===
```
