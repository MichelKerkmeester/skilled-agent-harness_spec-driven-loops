...
2. **Secondary ownership adoption unproven — split-brain mechanism identified.** After release a secondary can bridge to the surviving daemon's socket, but full re-election of recycle ownership is not yet validated. Three-way cross-validation (claude2 + cli-opencode + cli-codex) pinned the exact blocker: the released daemon lease still records the EXITING launcher's pid (`buildSharedLeaseObject` writes `pid: process.pid`), so an adopter that liveness-checks `lease.pid` sees a dead pid, treats the lease as stale, and respawns a SECOND daemon (split-brain). Before the flag can be enabled, the validation pass must make the release rewrite the lease into an explicit released/ownerless state AND teach the adopter to adopt-not-respawn on it. This is precisely why the flag ships default-off.
