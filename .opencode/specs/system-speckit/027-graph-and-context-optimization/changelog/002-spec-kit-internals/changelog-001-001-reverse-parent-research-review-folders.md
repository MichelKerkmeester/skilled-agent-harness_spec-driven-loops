...
A heuristic migration script discovered and moved 86 misplaced child-owned packets, rewrote 131 live canonical references with 0 conflicts. A follow-up manual owner-map pass moved 49 additional residual root packets, rewrote 24 more references with 0 conflicts, then cleared the remaining root `research/` or `review/` packet directories under `027-graph-and-context-optimization`. Across both passes, 135 child-owned packets were restored to owner-local folders with 155 live canonical references rewritten.
