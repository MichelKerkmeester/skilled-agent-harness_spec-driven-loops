...
- [ ] T-430a DECISION — `capability-flags.ts` source-path: ADD allowlist entry to `workflow-invariance.vitest.ts` allowing the literal symbol/path `capability-flags` in source-code identifiers (TS file names + import statements only). Rationale: renaming the source file is a separate refactor with broader callers (per ADR-005 maintainer-facing exemption). Doc references in `feature_catalog/feature-flag-reference/01-1-search-pipeline-features-speckit.md` (line 80-83/137) still get prose cleanup per T-431.
- [ ] T-431 MODIFY `system-spec-kit/feature_catalog/` per §2.5g — **22 MODIFY files**:
  - HEAVY: `feature_catalog.md` master (10 line ranges), `tooling-and-scripts/30-template-composition-system.md` (full rewrite for greenfield), `22--context-preservation-and-code-graph/25-resource-map-template.md` (repoint root-template ref)
  - MODERATE: `feature-flag-reference/01-1-search-pipeline-features-speckit.md` + `11-memory-roadmap-capability-flags.md` (banned-vocab retitle), `pipeline-architecture/22-mcp-server-public-api-barrel.md`, `tooling-and-scripts/{37-cli-matrix-adapter-runners, 36-copilot-target-authority-helper}.md`, `lifecycle/01-checkpoint-creation-checkpointcreate.md`
  - MECHANICAL (banned-word only): 13 files in retrieval, mutation, 09, 11, 14, 16, 17, 18, 21, 22 subdirs
- [ ] T-432 MODIFY `system-spec-kit/manual_testing_playbook/` per §2.5g — **8 MODIFY files**:
  - HEAVY: `manual_testing_playbook.md` master (3 line refs + 1 stale link), `tooling-and-scripts/244-template-composition-system.md` (full rewrite — replaces `compose.sh`/composer-drift validation with Level generation/invariance checks against new flow), `22--context-preservation-and-code-graph/270-resource-map-template.md`, `feature-flag-reference/125-memory-roadmap-capability-flags.md`
  - MODERATE: `stress-testing/01-run-stress-cycle.md` (line 129 "phase manifest" → "phase map"), `tooling-and-scripts/280-cli-matrix-adapter-runner-smoke.md` (lines 96/105), `22--context-preservation-and-code-graph/250-session-start-startup.md` (lines 20/23/63/72 `kind=startup` wording), `mutation/192-correction-tracking-with-undo.md` (line 44)
...
- **Source design**: `../002-manifest-driven-template-design/research/research.md`
- **ADRs (inherited)**: `../002-manifest-driven-template-design/decision-record.md`
<!-- /ANCHOR:cross-refs -->
