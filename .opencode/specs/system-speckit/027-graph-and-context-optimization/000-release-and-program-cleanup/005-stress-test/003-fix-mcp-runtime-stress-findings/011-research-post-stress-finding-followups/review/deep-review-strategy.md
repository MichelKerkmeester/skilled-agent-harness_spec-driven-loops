...
**The 4 packets**:
- **012-copilot-target-authority-gate-helper**: `buildCopilotPromptArg` + `validateSpecFolder` + Gate-3 enforcement at deep-loop dispatch
- **013-code-graph-degraded-stress-cell**: test-only integration sweep with isolated `SPEC_KIT_DB_DIR`
- **014-code-graph-status-readiness-snapshot**: read-only `getGraphReadinessSnapshot` + `code_graph_status` surfacing `readiness.action`
- **015-cocoindex-seed-telemetry-passthrough**: per-seed `rawScore`/`pathClass`/`rankingSignals` through `code_graph_context` anchors

**Source of evidence**:
- research.md at `../../../011-post-stress-finding-remediation-research/research/research.md`
