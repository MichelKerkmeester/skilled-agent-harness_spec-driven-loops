...
| 3 | `027-graph-and-context-optimization/` (4 phase-parents) + z_archive/z_future entries (~7) | 12 |
<!-- /ANCHOR:scope -->
...
| REQ-005 | 026 regression preserved. | `validate.sh --strict --json` against `027-graph-and-context-optimization/` after migration shows the same 3 parent-level error rules as the pre-migration baseline (`FRONTMATTER_MEMORY_BLOCK`, `SPEC_DOC_INTEGRITY`, `TEMPLATE_SOURCE`); zero new error classes. |
