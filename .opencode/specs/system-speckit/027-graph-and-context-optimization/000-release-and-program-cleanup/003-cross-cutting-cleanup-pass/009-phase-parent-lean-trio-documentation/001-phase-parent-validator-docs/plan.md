...
- [ ] `validate.sh --strict` still passes on `027-graph-and-context-optimization/` with no new errors (regression baseline preserved)
...
- [ ] Validator regression test on `027-graph-and-context-optimization/` — must continue to pass strict validation.
...
| Regression | `validate.sh --strict` on `027-graph-and-context-optimization/` (legacy heavy parent) | scripts/tests run after Phase A |
...
**Given** the existing `027-graph-and-context-optimization/` parent (legacy heavy docs), **when** I run `validate.sh --strict 026/`, **then** exit code is 0 and no NEW errors are introduced by this packet.
...
- [ ] `validate.sh --strict` regression baseline captured for `027-graph-and-context-optimization/` and at least three other phase-bearing folders before patches land
