...
The caura-memclaw study CONFIRMS this child's direction: MemClaw uses a single implicit global vector dimension with no per-row embedder or version tag, so the planned explicit chunk-fingerprint + embedder-tuple + derived-state design is the stronger, drift-safe choice. Keep the `last_dedup_checked_at` incremental marker. Source: research/008-caura-memclaw-fleet-memory-teachings/sub-packet-proposals.md; planned in 010/002.
