...
`manual_testing_playbook/governance/feature-flag-governance.md:36-40` still lists the manual pseudo-steps ("enumerate flags", "verify each flag row", "record any flag"); failure triage (:57 region) still says to diff by hand. The only test reading `ENV_REFERENCE.md` is `scripts/tests/warm-start-bundle-benchmark.vitest.ts.test.ts`, which checks a single flag's default — not a code-declared-vs-documented diff. No executable governance guard exists.
