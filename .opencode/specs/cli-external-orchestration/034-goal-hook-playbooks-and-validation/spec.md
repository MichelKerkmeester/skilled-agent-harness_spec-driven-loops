...
description: "Packet 032 shipped the cross-runtime goal hook but left no manual-testing-playbook coverage and no live proof the injection reaches the model turn in each runtime. This packet authors the playbook scenarios (by reference) and captures live validation evidence per runtime."
trigger_phrases:
  - "goal hook playbook"
...
| **Status** | Complete |
| **Created** | 2026-07-29 |
| **Branch** | `034-goal-hook-playbooks-and-validation` |
<!-- /ANCHOR:metadata -->
...
### Out of Scope
- Modifying goal-hook implementation code — packet 032 already shipped and closed it; this packet is documentation plus validation only
...
| `.opencode/specs/cli-external-orchestration/034-goal-hook-playbooks-and-validation/evidence/*.txt` | Create | Live validation capture files (already staged, this packet) |
<!-- /ANCHOR:scope -->
...
|------|------|--------|------------|
| Dependency | Packet `032-goal-hooks-cross-runtime` (goal-hook implementation) | Validation is meaningless if the hook code is not shipped | Confirmed shipped at `completion_pct: 100` before this packet started |
