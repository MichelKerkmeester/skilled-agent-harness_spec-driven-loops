...
- [ ] CHK-003 [P1] Baseline validator run captured before any edit
- [ ] CHK-004 [P1] `create-skill` packet-level templates read fresh
<!-- /ANCHOR:pre-impl -->
...
- [ ] CHK-010 [P0] `SKILL.md` frontmatter `name` equals the folder name AND the registry `packetSkillName` (`cli-cursor`)
- [ ] CHK-011 [P0] `SKILL.md` `version` is four-part (`"1.0.0.0"`); `description` ≤130 chars, no angle brackets
- [ ] CHK-012 [P1] `hard_rules` frontmatter block mirrors the `cursor-availability-required`/`self-invocation-prohibited`/`deep-loop-runtime-required` triad
- [ ] CHK-013 [P1] Self-invocation guard function present in Section 2, built from confirmed signals only (per ADR-002); `command -v cursor-agent` probe present (canonical binary, not the `agent` alias)
<!-- /ANCHOR:code-quality -->
...
- [ ] CHK-022 [P1] `mode-registry.json`'s `cli-cursor` entry matches the exact schema given in `spec.md` REQ-004; aliases don't collide case-folded with siblings
...
- [ ] CHK-FIX-003 [P0] Consumer inventory completed for the 3 existing sibling mode entries — confirm none of their fields change
...
- [ ] CHK-FIX-007 [P1] Evidence pinned to the phase's commit SHA once landed, not a moving branch range
<!-- /ANCHOR:fix-completeness -->
...
- [ ] CHK-030 [P0] No hardcoded secrets or credentials in any authored file; no Cursor auth token embedded in `SKILL.md`/`README.md`/`references/`/`assets/`
- [ ] CHK-031 [P0] N/A - no user-input validation surface (static packet authoring)
- [ ] CHK-032 [P1] `shared-editor-config.md` warns that a dispatched `cursor-agent` inherits the operator's `~/.cursor/` config; no secret is copied into the doc
<!-- /ANCHOR:security -->
...
- [ ] CHK-041 [P1] Every authored reference/asset file cites phase 001's grounded facts, not invented specifics; auth-gated facts (model roster) marked TBD
- [ ] CHK-042 [P2] `prompt-quality-card.md` confirmed a thin delegator with no fabricated Cursor per-model defaults
<!-- /ANCHOR:docs -->
...
- [ ] CHK-103 [P2] N/A - no migration path applicable; this is new packet construction
<!-- /ANCHOR:arch-verify -->
...
- [ ] CHK-120 [P0] Rollback procedure documented in `plan.md` §7 (remove `cli-cursor/`, revert 5 hub-root file edits, regenerate `leaf-manifest.json`, re-validate)
...
- [ ] CHK-142 [P2] `cli-cursor/README.md` reviewed as the user-facing documentation for this packet
