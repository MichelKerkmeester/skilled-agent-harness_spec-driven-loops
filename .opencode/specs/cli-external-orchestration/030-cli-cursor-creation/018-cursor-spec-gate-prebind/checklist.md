...
- [x] CHK-020 [P0] Valid `MK_SPEC_FOLDER` produces satisfied state consumed as allow. [EVIDENCE: named integration rows pass in the 9/9 prebind suite.]
...
- [x] CHK-022 [P0] Disabled and child sessions produce no state and are complete no-ops. [EVIDENCE: prebind 9/9, core 67/67, plugin 11/11, and the live Claude child probe pass.]
...
- [x] CHK-024 [P1] Invalid declarations, padded session ids, and repeated terminal states behave deterministically. [EVIDENCE: the 9/9 prebind suite includes a padded-id row that requires a real deny after verbatim open-state lookup.]
<!-- /ANCHOR:testing -->
...
- [x] CHK-FIX-001 [P0] Findings classified as cross-consumer and matrix/evidence issues. [EVIDENCE: `spec-gate-prebind.test.mjs` invokes both startup producer and enforce consumer across 9 named subtests.]
...
- [x] CHK-032 [P1] Only opted-in, top-level sessions can enter enforceable open state. [EVIDENCE: inert, child, disabled, and enforced rows pass in the 9/9 process suite.]
<!-- /ANCHOR:security -->
