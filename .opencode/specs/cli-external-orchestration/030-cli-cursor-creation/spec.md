...
| `sk-prompt/prompt-models/{assets/model-profiles.json,references/models/composer.md,references/models/_index.md}`, `system-skill-advisor/mcp-server/scripts/check-prompt-quality-card-sync.sh` | Modify/Created | 005 | Add a Composer profile + `cli-cursor` executor rows and CI gate coverage. |
