...
_memory:
  continuity:
    ...
    answered_questions:
      ...
      - "Reuses 113/002 eval rig + 113/003 loop infrastructure; only adds extraction layer + re-runs"
...
**Critical Dependencies**: 113/002 eval rig (unmodified — extraction is a new script alongside, not a rewrite); 113/003 loop runner (unmodified); cli-devin SWE 1.6 (unchanged); claude-sonnet via cli-claude-code (new dependency for this run vs 003's mock).
...
- Modification to 113/002 `scripts/score-variant.cjs` — call extraction before deterministic checks when `EVAL_LOOP_EXTRACT=true`
...
|-----------|-------------|-------------|
| `113-cli-devin-prompt-quality/005-extraction-rerun/scripts/extract-files-from-markdown.cjs` | Create | Parse markdown → write files to fixture CWD |
| `113-cli-devin-prompt-quality/002-eval-rig/scripts/score-variant.cjs` | Modify | Call extraction when env flag set; restore fixture CWD between variants |
| `113-cli-devin-prompt-quality/005-extraction-rerun/state/eval-loop-state-v2.jsonl` | Create | New run's state (separate from 003's state.jsonl) |
| `113-cli-devin-prompt-quality/005-extraction-rerun/iterations/iteration-NNN.md` | Create (5) | One per variant run |
| `113-cli-devin-prompt-quality/005-extraction-rerun/synthesis-v2.md` | Create | New ranking + comparison vs 003 |
<!-- /ANCHOR:scope -->
...
| Coordination | 8/15 | Reuses 113/002+003 infrastructure |
...
**Acceptance Criteria**:
1. Given the extraction layer is built, When operator runs `node 113-cli-devin-prompt-quality/005-extraction-rerun/scripts/loop-v2.cjs --real`, Then the loop dispatches 5 variants × 7 fixtures with extraction + live grader and exits with synthesis-v2.md written.
...
- **Upstream contract**: `../001-council-design/ai-council/council-report.md`
- **Upstream rig**: `../002-eval-rig/`
- **Upstream loop**: `../003-eval-loop/`
- **Upstream synthesis (v1)**: `../003-eval-loop/synthesis.md`
