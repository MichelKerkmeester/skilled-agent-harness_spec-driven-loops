...
Questions: 0/6 answered | Last focus: Iteration 4 — native Claude import coverage matrix
Last 2 ratios: 0.82 -> 0.86
Resource map: resource-map.md not present; skipping coverage gate.
Memory context refresh: phase 001 implementation-summary.md is on disk in spec folder
Next focus: Iteration 5 — full per-hook per-plugin port verdict table. Consolidate every Claude hook (8 event keys, 19 handlers from iter 1) and every OpenCode plugin (15 entrypoints from iter 1) plus the 7 guard cores (iter 3) into one ADR-001-ready v
...
Iteration: 5 of 10
Focus Area: Iteration 5 — full per-hook per-plugin port verdict table. Consolidate every Claude hook (8 event keys, 19 handlers from iter 1) and every OpenCode plugin (15 entrypoints from iter 1) plus the 7 guard cores (iter 3) into one ADR-001-ready verdict table with one row per item. Each row MUST carry: source id (event/matcher/plugin path), Devin verdict (portable 1:1 / needs adaptation / cannot port), Devin target (PreToolUse matcher/PostToolUse/UserPromptSubmit/SessionStart/SessionEnd), rationale (2-3 sentences), and evidence file:line citations. Also produce the companion native-import column from iter 4 to make the trade-off explicit per row.
...
<!-- ANCHOR:next-focus -->
Last 3 Iterations Summary: run 2: Iteration 2 — Devin subagent dispatch tool and Task-guard po (0.78) | run 3: Iteration 3 — 7 guard cores reusability audit (0.82) | run 4: Iteration 4 — native Claude import coverage matrix (0.86)
...
- Write iteration narrative to: .opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-005.md
- Write per-iteration delta file to: .opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-005.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-005.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-005.jsonl`, this iteration's delta JSONL
- **BANNED OPERATIONS (NEVER execute against any path)**: `rm`, `rm -rf`, `git rm`, `mv`, `sed -i` (including `sed -i ''`), `rmdir`, `find ... -delete`, shell output-redirect truncate `>` against any file not in the allowed-write list, and any tool call whose effect is to delete, rename, or replace a file outside the allowed-write list. Reading is unrestricted; **writing, renaming, and deleting are scoped**.
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the research. NEVER execute the out-of-scope mutation. The research packet (`.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-005.md` directory and parents) is the only zone for your writes; the researched target/topic surface is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-005.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
