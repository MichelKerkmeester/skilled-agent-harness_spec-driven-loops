...
Questions: 0/6 answered | Last focus: Iteration 8 — OpenCode plugins that cannot port + alternatives
Last 2 ratios: 0.9 -> 0.92
Resource map: resource-map.md not present; skipping coverage gate.
Memory context refresh: phase 001 implementation-summary.md is on disk in spec folder
Next focus: Iteration 9 — ADR-001 evidence package assembly. Consolidate every prior iteration into a single ADR-001-ready evidence pack at research/evidence/ including: (a) the full verdict table from iter 5, (b) the 7 guard cores adapter table from i
...
Iteration: 9 of 10
Focus Area: Iteration 9 — ADR-001 evidence package assembly. Consolidate every prior iteration into a single ADR-001-ready evidence pack at research/evidence/ including: (a) the full verdict table from iter 5, (b) the 7 guard cores adapter table from iter 3, (c) the native-import coverage matrix from iter 4, (d) the bash-only cwd/env contract from iter 6, (e) the PreCompact/PostCompaction analysis from iter 7, (f) the OpenCode cannot-port alternatives from iter 8. Add the proposed .devin/hooks.v1.json skeleton (single concrete shape, 8 events, only the necessary matchers, only the portable 1:1 + needs-adaptation rows; deliberately omitting cannot-port rows). Cross-check every claim against findings-registry.json — flag any unmatched registry keys.
...
<!-- ANCHOR:next-focus -->
Last 3 Iterations Summary: run 6: Iteration 6 — bash-only hooks cwd/env and exit/stdout contra (0.84) | run 7: Iteration 7 — PreCompact → PostCompaction substitution (0.9) | run 8: Iteration 8 — OpenCode plugins that cannot port + alternativ (0.92)
...
- Write iteration narrative to: .opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-009.md
- Write per-iteration delta file to: .opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-009.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-009.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-009.jsonl`, this iteration's delta JSONL
- **BANNED OPERATIONS (NEVER execute against any path)**: `rm`, `rm -rf`, `git rm`, `mv`, `sed -i` (including `sed -i ''`), `rmdir`, `find ... -delete`, shell output-redirect truncate `>` against any file not in the allowed-write list, and any tool call whose effect is to delete, rename, or replace a file outside the allowed-write list. Reading is unrestricted; **writing, renaming, and deleting are scoped**.
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the research. NEVER execute the out-of-scope mutation. The research packet (`.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-009.md` directory and parents) is the only zone for your writes; the researched target/topic surface is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-009.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-009.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
