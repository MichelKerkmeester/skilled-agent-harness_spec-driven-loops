...
Questions: 0/6 answered | Last focus: Iteration 7 — PreCompact → PostCompaction substitution
Last 2 ratios: 0.84 -> 0.9
Resource map: resource-map.md not present; skipping coverage gate.
Memory context refresh: phase 001 implementation-summary.md is on disk in spec folder
Next focus: Iteration 8 — OpenCode plugins that cannot port + alternatives. From iter 1, several OpenCode plugins are "cannot port 1:1" because they register plugin tools, message transforms, system transforms, or disposal events that Devin does not ex
...
Iteration: 8 of 10
Focus Area: Iteration 8 — OpenCode plugins that cannot port + alternatives. From iter 1, several OpenCode plugins are "cannot port 1:1" because they register plugin tools, message transforms, system transforms, or disposal events that Devin does not expose (mk-code-graph.js, mk-goal.js, mk-skill-advisor.js, mk-spec-memory.js, mk-speckit-completion.js). For each: describe what the plugin does, what Devin CANNOT replicate via hooks, and the recommended non-hook alternative (Devin skills/rules, devin CLI subcommands, devin mcpServers, or scripts/skills invoked via UserPromptSubmit). Produce a per-plugin "alternative path" table.
...
<!-- ANCHOR:next-focus -->
Last 3 Iterations Summary: run 5: Iteration 5 — full per-hook per-plugin port verdict table (0.88) | run 6: Iteration 6 — bash-only hooks cwd/env and exit/stdout contra (0.84) | run 7: Iteration 7 — PreCompact → PostCompaction substitution (0.9)
...
- Write iteration narrative to: .opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-008.md
- Write per-iteration delta file to: .opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-008.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-008.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-008.jsonl`, this iteration's delta JSONL
- **BANNED OPERATIONS (NEVER execute against any path)**: `rm`, `rm -rf`, `git rm`, `mv`, `sed -i` (including `sed -i ''`), `rmdir`, `find ... -delete`, shell output-redirect truncate `>` against any file not in the allowed-write list, and any tool call whose effect is to delete, rename, or replace a file outside the allowed-write list. Reading is unrestricted; **writing, renaming, and deleting are scoped**.
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the research. NEVER execute the out-of-scope mutation. The research packet (`.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-008.md` directory and parents) is the only zone for your writes; the researched target/topic surface is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-008.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-008.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
