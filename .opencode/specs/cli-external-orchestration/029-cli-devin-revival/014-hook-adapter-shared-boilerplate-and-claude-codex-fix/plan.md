...
- [x] Migrated adapters' existing test suites pass unchanged. [EVIDENCE: spec-gate core 67/67, devin spec-gate 15/15, cursor prebind 16/16 all unchanged.]
- [x] Claude/Codex discriminating masking-regression test added and green. [EVIDENCE: new `spec-gate-claude.test.mjs` 13/13, `spec-gate-codex.test.mjs` 14/14.]
<!-- /ANCHOR:quality-gates -->
...
- [x] Create `hook-adapter-shared.mjs` exporting `readStdin()`/`parseJsonFailOpen()`.
- [x] Create `hook-adapter-shared.cjs` as its CommonJS twin.
- [x] Confirm both are byte-behavior-identical to the boilerplate they replace via a shared micro-test. [EVIDENCE: every migrated adapter's pre-existing suite passes unchanged post-migration.]

### Phase 2: Claude/Codex alias fix
- [x] Apply `firstNonBlankString()` to Claude's `spec-gate-enforce.mjs` `filePathFrom()`.
- [x] Apply `firstNonBlankString()` to Codex's `spec-gate-enforce.mjs` `filePathFrom()`, diff-reviewing to confirm `apply_patch` parsing is untouched. [EVIDENCE: `git diff` shows `pathsFromPatch()` unchanged.]
- [x] Add a discriminating masking-regression test row to each runtime's existing spec-gate test suite. [EVIDENCE: new Claude/Codex suites include the row; Devin/Cursor already had it from phase 012.]

### Phase 3: Boilerplate migration and closeout
- [x] Migrate all Q6-sampled adapters (spec-gate-enforce ×4, task-dispatch-guard ×2, mcp-route-guard ×3) to import the shared helper.
- [x] Run every migrated adapter's existing test suite; confirm unchanged pass counts.
- [x] Grep-confirm no remaining inline boilerplate duplication in migrated files.
- [x] Run phase 014 strict and recursive parent strict validation. [EVIDENCE: see checklist.md CHK-050.]
<!-- /ANCHOR:phases -->
