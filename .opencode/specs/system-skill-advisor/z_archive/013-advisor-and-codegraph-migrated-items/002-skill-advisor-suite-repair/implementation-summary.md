...
| Field | Value |
|-------|-------|
| **Spec Folder** | 002-skill-advisor-suite-repair |
...
The legacy `deep-research`, `deep-review`, and `deep-ai-council` skill directories no longer exist on disk; only `deep-loop-workflows` does. Routing already lands on the merged node, so the repair was to make the tests and data agree with reality. The native-scorer council fixture and the intent-prompt corpus now expect `deep-loop-workflows`; the CLI parity row points at the merged skill that actually has a `SKILL.md`; and the metadata-category allowlist in `skill_graph_compiler.py` now recognizes the `workflow` and `design` categories that `deep-loop-workflows` and `sk-design-interface` carry.
