---
title: "Feature Specification: system-spec-kit/026-graph-and-context-optimization/006-skill-advisor/005-skill-advisor-documentation/004-documentation-quality-refactor (phase parent)"
...
_memory:
  continuity:
    ...
    next_safe_action: "Run 20-iter deep-research in 001-documentation-quality-audit-research"
    blockers: []
    key_files:
...
# Feature Specification: system-spec-kit/026-graph-and-context-optimization/006-skill-advisor/005-skill-advisor-documentation/004-documentation-quality-refactor (phase parent)
...
| 006 | `006-clean-deferred-documentation/` | Tier A (F30 cross-links + F33 SOURCE FILES) + Tier B (F23/F24/F44 re-verify) + Tier C (bulk Oxford comma sweep 943→0). Closes the deferred backlog that ships without architectural decisions. | Complete |
| 007 | `007-finalize-documentation-quality-refactor/` | Final-pass closure: semicolon HVR sweep (137→30 prose), F34 playbook deviation notes (20 files), 4 new ref docs (query-cookbook, validation-baselines, daemon-lease-contract, skill-graph-drift), deferred-decisions.md for Tier D. cli-opencode dispatch attempted but failed on InstanceRef (v1.15.1 vs v1.3.17 baseline drift); executed in-session instead. | Complete |
| 008 | `008-tier-d-documentation-execution/` | Tier D execution: F4 `.devin/hooks.v1.json` migration (UserPromptSubmit→system-skill-advisor, SessionStart→system-code-graph with conceptual-ownership pivot), F6 deprecation banners on 4 OLD hook READMEs with 90-day window, F37 catalog/playbook §17.5 cross-reference table. F35/F36 stay status-quo per deferred-decisions.md recommendations. | Complete |

### Phase Transition Rules

- Each phase MUST pass `validate.sh --strict` independently before the next phase begins
- Parent spec tracks aggregate progress via this map
- Use `/spec_kit:resume .opencode/specs/system-spec-kit/026-graph-and-context-optimization/006-skill-advisor/005-skill-advisor-documentation/004-documentation-quality-refactor/[NNN-phase]/` to resume a specific phase
...
| 002 | 003 | All P0 bugs fixed; grep for stale paths returns 0 hits | `rg "006-system-skill-advisor-package-extraction" .opencode/skills/system-skill-advisor` → 0 |
