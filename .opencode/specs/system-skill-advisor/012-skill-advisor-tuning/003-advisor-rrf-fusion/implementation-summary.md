...
_memory:
  continuity:
    packet_pointer: "system-skill-advisor/012-skill-advisor-tuning/003-advisor-rrf-fusion"
...
- **The per-seam verdicts are tracked as an owned graduation decision.** The GRADUATE (RRF core + conflict-rerank seam) and CUT (self-recommendation guard) verdicts are recorded with owner and decision-state in the Graduation Decision (tracked) table in `benchmark-results.md`; both are DEFERRED and neither has executed — `SPECKIT_ADVISOR_RRF_FUSION` is still default-off pending the coordinated reindex window, and the CUT is adopted but the guard code still lives in `fusion.ts` pending deletion in the gated code window.
<!-- /ANCHOR:decisions -->
