...
## Findings
1. `deriveEntities()` seeds entities from `key_files` using the full `filePath` as the map key, but stores only the basename in `.name`. That design allows duplicates whenever two different paths share the same basename. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:418-446]
