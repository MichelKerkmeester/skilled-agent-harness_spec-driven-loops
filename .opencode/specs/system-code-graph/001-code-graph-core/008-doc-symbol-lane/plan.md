...
- [x] No data migration (doc nodes reuse the existing node/edge schema) - confirmed, doc nodes shipped on the existing node/edge write path
- [x] Baseline code-lane node/edge result set captured, idempotence verified - rescan yields byte-identical doc nodes/edges
- [x] Markdown glob decision recorded - json/yaml/toml-first, `**/*.md` stays opt-in in the default globs
- [x] Q7 sink confirmed absent → `emitLeaseMetric()` defaults no-op (no dashboard wired)
