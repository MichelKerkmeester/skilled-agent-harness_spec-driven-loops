...
- `.opencode/skills/system-spec-kit/mcp_server/code-graph/handlers/index.ts`
...
- Rewriting CALLS extraction to use AST `call_expression` nodes would be good hardening for member-call fidelity, but it is not required to fix this specific zero-edge incident because the live graph already stores CALLS edges for the real implementation symbol [iterations/iteration-02.md#evidence,.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/structural-indexer.ts:942-973].
- This does not look like a one-off stale row in `code-graph.sqlite`; the same wrapper-vs-function shape repeats across multiple handler families and even the code-graph handler index itself [iterations/iteration-03.md#evidence,.opencode/skills/system-spec-kit/mcp_server/handlers/index.ts:212-320,.opencode/skills/system-spec-kit/mcp_server/code-graph/handlers/index.ts:5].
