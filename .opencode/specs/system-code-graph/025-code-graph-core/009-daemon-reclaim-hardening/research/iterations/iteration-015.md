...
--------
user
You are GPT-5.5 at xhigh reasoning doing an ADVERSARIAL verification pass over a hardening design produced by a different model (GLM-5.2). Be skeptical: your job is to CONFIRM what is right, REFUTE what is wrong, and find GAPS the prior research missed — not to agree. Read the synthesized design at .opencode/specs/system-code-graph/025-code-graph-core/009-daemon-reclaim-hardening/research/research.md and the actual source .opencode/bin/mk-code-index-launcher.cjs and .opencode/bin/launcher-ipc-bridge.cjs (and the child initDb under system-code-graph/mcp_server if useful). Ground every claim in the REAL code (cite function/line). Incident recap: the code-index daemon wedged (PID alive, IPC socket never created, lease file gone, 17MB orphaned WAL) and /mcp reconnect returned -32000.
...
exec
/bin/zsh -lc "nl -ba .opencode/specs/system-code-graph/025-code-graph-core/009-daemon-reclaim-hardening/research/research.md | sed -n '1,260p'" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
exec
...
.opencode/skills/system-code-graph/manual_testing_playbook/manual_testing_playbook.md:70:| 002 | query self-heal stale file | [query-self-heal-stale-file.md](./read-path-freshness/query-self-heal-stale-file.md) |
.opencode/skills/system-code-graph/manual_testing_playbook/manual_testing_playbook.md:80:| 005 | code_graph_verify blocked on stale | [code-graph-verify-blocked-on-stale.md](./manual-scan-verify-status/code-graph-verify-blocked-on-stale.md) |
...
.opencode/skills/system-code-graph/manual_testing_playbook/detect-changes/detect-changes-no-inline-index.md:3:description: "Verify detect_changes is read-only and blocks stale readiness instead of performing inline indexing."
.opencode/skills/system-code-graph/manual_testing_playbook/detect-changes/detect-changes-no-inline-index.md:15:Verify detect_changes is read-only and blocks stale readiness instead of performing inline indexing.
.opencode/skills/system-code-graph/manual_testing_playbook/detect-changes/detect-changes-no-inline-index.md:21:- Objective: Verify detect_changes is read-only and blocks stale readiness instead of performing inline indexing.
.opencode/skills/system-code-graph/manual_testing_playbook/detect-changes/detect-changes-no-inline-index.md:22:- Real user request: `Validate that detect_changes refuses stale graph state and asks for code_graph_scan instead of repairing inline.`
.opencode/skills/system-code-graph/manual_testing_playbook/detect-changes/detect-changes-no-inline-index.md:23:- Operator prompt: `Validate detect_changes against stale graph state in a disposable workspace. Show that it blocks with scan guidance instead of inline indexing, then return PASS/FAIL with diff and JSON evidence.`
.opencode/skills/system-code-graph/manual_testing_playbook/detect-changes/detect-changes-no-inline-index.md:25:- Expected signals: The stale call returns `status:"blocked"` and says to run `code_graph_scan`. It must not silently repair via inline indexing.
.opencode/skills/system-code-graph/manual_testing_playbook/detect-changes/detect-changes-no-inline-index.md:27:- Pass/fail: PASS if stale readiness blocks with scan guidance and no inline indexing occurs. FAIL if detect_changes repairs inline, omits the required action or fails to return a structured blocked payload.
.opencode/skills/system-code-graph/manual_testing_playbook/detect-changes/detect-changes-no-inline-index.md:42:The stale call returns `status:"blocked"` and says to run `code_graph_scan`. It must not silently repair via inline indexing.
...
.opencode/skills/system-code-graph/feature_catalog/detect-changes/detect-changes-preflight.md:3:description: "Read-only diff preflight that maps unified-diff hunks to indexed symbols and refuses stale or unverifiable graph state."
.opencode/skills/system-code-graph/feature_catalog/detect-changes/detect-changes-preflight.md:18:`detect_changes` maps unified diff hunks to structural symbols through line-range overlap. Its safety property is refusal: stale, empty, error or failed-verification graphs produce `status:"blocked"` instead of false-safe empty impact.
.opencode/skills/system-code-graph/feature_catalog/detect-changes/detect-changes-preflight.md:40:| `.opencode/skills/system-code-graph/mcp_server/handlers/detect-changes.ts:1-12` | Handler | states the stale-graph hard-refuse invariant |
.opencode/skills/system-code-graph/feature_catalog/detect-changes/detect-changes-preflight.md:41:| `.opencode/skills/system-code-graph/mcp_server/handlers/detect-changes.ts:241-260` | Handler | checks readiness before diff parsing and blocks stale state |
...
.opencode/skills/system-code-graph/feature_catalog/mcp-tool-surface/code-index-cli.md:36:`.opencode/bin/code-index.cjs` defaults the socket dir to `/tmp/mk-code-index`, refuses missing or stale dist with exit 69 (`SPECKIT_CODE_INDEX_CLI_DEV_ALLOW_STALE=1` is the development override), and maps spawn failure to 75. The entrypoint shares the program taxonomy: 0 success, 1 runtime, 64 usage/validation (including JSON-RPC `-32602`), 69 protocol, 75 retryable. `--warm-only` (default via `SPECKIT_CODE_INDEX_CLI_WARM_ONLY`) probes the socket and exits 75 instead of auto-spawning.
.opencode/skills/system-code-graph/feature_catalog/mcp-tool-surface/code-index-cli.md:44:| `.opencode/bin/code-index.cjs` | Script | Stable shim: socket-dir defaulting, dist-freshness refusal, spawn-failure mapping |
.opencode/skills/system-code-graph/feature_catalog/mcp-tool-surface/code-index-cli.md:54:| `mcp_server/tests/code-index-cli-blocked-read.vitest.ts` | Automated test | Nine stale-readiness cases asserting `status:blocked` plus `requiredAction` |
.opencode/skills/system-code-graph/feature_catalog/mcp-tool-surface/code-index-cli.md:56:| `mcp_server/tests/code-index-cli-owner-respawn.vitest.ts` | Automated test | Real owner-lease read and fresh-launcher takeover |
.opencode/skills/system-code-graph/feature_catalog/mcp-tool-surface/code-index-cli.md:58:| `mcp_server/tests/code-index-cli-harness.ts` | Test harness | Sandboxed socket/DB environment shared by the suites |
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:2:title: "005 code_graph_verify blocked on stale"
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:3:description: "Prove code_graph_verify refuses stale graph state before executing the gold battery."
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:6:  - "code graph verify blocked on stale"
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:11:# 005 code_graph_verify blocked on stale
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:15:Prove code_graph_verify refuses stale graph state before executing the gold battery.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:21:- Objective: Prove code_graph_verify refuses stale graph state before executing the gold battery.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:22:- Real user request: `Confirm code_graph_verify blocks on stale graph state, then passes through to verification after an explicit rescan.`
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:23:- Operator prompt: `Validate code_graph_verify around stale graph state. Show the blocked payload before rescan and the verification payload after explicit scan, then return PASS/FAIL.`
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:26:- Desired user-visible outcome: A concise verdict explaining whether verify protects against stale input and resumes after scan.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:27:- Pass/fail: PASS if stale verify blocks and post-scan verify returns ok with pass-rate data. FAIL if verify runs on stale state, remains blocked after rescan or omits readiness/pass-rate fields.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md:50:Run with `allowInlineIndex:true` and a single stale file to test the selective repair option.
...
.opencode/skills/system-code-graph/feature_catalog/manual-scan-verify-status/code-graph-scan.md:19:The reindex writer is bitemporal. When the default-off `SPECKIT_CODE_GRAPH_EDGE_BITEMPORAL_READS` flag is on, a reindex closes superseded edges by stamping `invalid_at` at the next graph generation instead of deleting them, so prior generations stay readable through the as-of readers. With the flag off the writer keeps its delete-and-replace behavior. The default-off `SPECKIT_CODE_GRAPH_REVERSE_DEP_FORCE_PARSE` flag enables the staleness-repair reverse-dependency expansion: when a stale file's reindex changes its symbol identity, every importer of that file is pulled back into the parse batch so cross-file edges rebind to the new symbol ids. That expansion is bounded by the `SPECKIT_CODE_GRAPH_REVERSE_DEP_DEGREE_CAP` degree cap, default 15: a renamed hot dependency whose importer fan-in degree exceeds the cap drops out of the force-parse set and its importers rebind lazily on their next edit. A cap of zero leaves every refactored dependency in, byte-identical to the uncapped path, and the cap reads only inside the force-parse branch, so it has no effect while the force-parse flag is off.
.opencode/skills/system-code-graph/feature_catalog/manual-scan-verify-status/code-graph-scan.md:46:| `.opencode/skills/system-code-graph/mcp_server/lib/code-graph-db.ts` | Library | bitemporal close-and-insert reindex writer that stamps `invalid_at` on superseded edges under the bitemporal-reads flag, plus the governance-vocabulary edge-type `CHECK` migration gated by `SPECKIT_CODE_GRAPH_EDGE_GOVERNANCE_VOCAB` |
.opencode/skills/system-code-graph/feature_catalog/read-path-freshness/ensure-code-graph-ready.md:3:description: "Shared readiness helper that detects empty, stale, full-scan and selective-reindex states and can run bounded selective repair on read paths."
.opencode/skills/system-code-graph/feature_catalog/read-path-freshness/ensure-code-graph-ready.md:39:| `.opencode/skills/system-code-graph/mcp_server/lib/ensure-ready.ts:141-225` | Library | detects empty, stale, full-scan and selective-reindex states |
.opencode/skills/system-code-graph/feature_catalog/read-path-freshness/ensure-code-graph-ready.md:41:| `.opencode/skills/system-code-graph/mcp_server/lib/ensure-ready.ts:398-421` | Library | runs selective reindex for stale tracked files |
.opencode/skills/system-code-graph/feature_catalog/read-path-freshness/ensure-code-graph-ready.md:42:| `.opencode/skills/system-code-graph/mcp_server/lib/ensure-ready.ts:272-287` | Library | stages persistence so failed writes leave files stale for retry |
...
.opencode/skills/system-code-graph/feature_catalog/read-path-freshness/query-self-heal.md:32:When stale files exceed the selective threshold or Git HEAD changed, query blocks and tells the operator to run `code_graph_scan`. Use `rg` when readiness crashes or the graph is unavailable.
.opencode/skills/system-code-graph/feature_catalog/read-path-freshness/query-self-heal.md:69:- [../../manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md](../../manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md)
...
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-scan-full.md:24:- Expected execution process: Call `code_graph_scan({"rootDir":"$WORK","incremental":false,"persistBaseline":true})` in a disposable copy and inspect the returned payload.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-scan-full.md:35:Call `code_graph_scan({"rootDir":"$WORK","incremental":false,"persistBaseline":true})` in a disposable copy.
...
.opencode/skills/system-code-graph/feature_catalog/manual-scan-verify-status/code-graph-status.md:17:`code_graph_status` reports graph health and readiness. It uses a read-only readiness snapshot so status calls do not repair stale state.
.opencode/skills/system-code-graph/feature_catalog/manual-scan-verify-status/code-graph-status.md:35:Status can recommend `rg` or `code_graph_scan`. It does not perform either. Treat stale status as an operator prompt, not repair proof.
...
.opencode/skills/system-code-graph/feature_catalog/manual-scan-verify-status/code-graph-verify.md:3:description: "Manual gold-query verification tool that blocks on stale readiness and executes the persisted code-graph battery only when fresh."
.opencode/skills/system-code-graph/feature_catalog/manual-scan-verify-status/code-graph-verify.md:31:The handler refuses stale graphs. Run `code_graph_scan({ incremental:false })` before verification when blocked.
.opencode/skills/system-code-graph/feature_catalog/manual-scan-verify-status/code-graph-verify.md:60:- [../../manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md](../../manual_testing_playbook/manual-scan-verify-status/code-graph-verify-blocked-on-stale.md)
...
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:3:description: "Verify status reads readiness and graph quality without mutating or repairing stale state."
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:15:Verify status reads readiness and graph quality without mutating or repairing stale state.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:21:- Objective: Verify status reads readiness and graph quality without mutating or repairing stale state.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:22:- Real user request: `Inspect code_graph_status around a stale-file fixture and confirm it reports diagnostics without repairing the graph.`
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:23:- Operator prompt: `Inspect code_graph_status around a stale-file fixture. Show readiness and graph-quality diagnostics without repair or scan mutation, then return PASS/FAIL with before-and-after evidence.`
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:24:- Expected execution process: Capture `code_graph_status({})` twice around a stale-file fixture, compare `lastPersistedAt`, readiness and scan counts and confirm no `code_graph_scan` was invoked.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:25:- Expected signals: Status returns diagnostic fields including `freshness`, `readiness`, `canonicalReadiness`, `trustState`, `graphQualitySummary` and, when tombstone auditing is enabled, tombstone retained counts by kind/reason plus recent retained tombstones. It does not repair stale state.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:27:- Pass/fail: PASS if status reports diagnostics without changing persistence or scan evidence and exposes tombstone audit status under the explicit flag. FAIL if it repairs stale state, invokes scan, omits readiness or graph-quality diagnostics, or leaks tombstone state into live graph query results.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:35:1. Capture `code_graph_status({})` twice around a stale-file fixture.
.opencode/skills/system-code-graph/manual_testing_playbook/manual-scan-verify-status/code-graph-status-readonly.md:42:Status returns diagnostic fields including `freshness`, `readiness`, `canonicalReadiness`, `trustState` and `graphQualitySummary`, but does not repair stale state. With tombstone auditing enabled, the status payload also includes retained deletion-lineage counts and recent retained tombstones; with the flag unset, the default path remains compact and audit-free.
...
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/ensure-ready-selective-reindex.md:3:description: "Verify that one stale tracked file can be selectively reindexed by the readiness helper without broad full-scan behavior."
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/ensure-ready-selective-reindex.md:15:Verify that one stale tracked file can be selectively reindexed by the readiness helper without broad full-scan behavior.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/ensure-ready-selective-reindex.md:21:- Objective: Verify that one stale tracked file can be selectively reindexed by the readiness helper without broad full-scan behavior.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/ensure-ready-selective-reindex.md:22:- Real user request: `Validate that code_graph can repair a single stale tracked file without triggering an unrequested full scan.`
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/ensure-ready-selective-reindex.md:23:- Operator prompt: `Validate selective code-graph readiness in a disposable workspace. Show that one stale tracked file repairs without an unrequested full scan and return PASS/FAIL with command evidence and JSON excerpts.`
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/ensure-ready-selective-reindex.md:26:- Desired user-visible outcome: A concise verdict explaining whether selective reindex repaired the stale file without broad scan behavior.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/ensure-ready-selective-reindex.md:27:- Pass/fail: PASS if the stale file is repaired and no hidden full scan appears. FAIL if the call blocks unexpectedly, performs an unrequested full scan or omits readiness evidence.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/ensure-ready-selective-reindex.md:35:1. Create a disposable copy and run `code_graph_scan({"rootDir":"$WORK","incremental":false})`.
...
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:2:title: "002 query self-heal stale file"
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:6:  - "query self heal stale file"
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:11:# 002 query self-heal stale file
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:22:- Real user request: `Check that code_graph_query self-heals a single stale file but refuses broad stale state until an explicit scan runs.`
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:23:- Operator prompt: `Validate code_graph_query against single-file and broad stale states in a disposable workspace. Show the self-heal and blocked-scan paths, then return PASS/FAIL with JSON excerpts.`
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:24:- Expected execution process: Run a full scan in a disposable copy, modify one tracked file and call `code_graph_query` for outline, then modify more than 50 tracked files and call `code_graph_query` again.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:25:- Expected signals: Single-file stale state is repaired or answered with readiness metadata. Broad stale state returns a blocked payload with `requiredAction:"code_graph_scan"` or equivalent fallback decision.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:26:- Desired user-visible outcome: A concise verdict distinguishing the allowed self-heal path from the blocked broad-stale path.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:27:- Pass/fail: PASS if single-file stale state self-heals and broad stale state blocks with scan guidance. FAIL if broad state is silently repaired, single-file readiness metadata is missing or the handler crashes.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:35:1. Run a full scan in a disposable copy.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:41:Single-file stale state is repaired or answered with readiness metadata. Broad stale state returns a blocked payload with `requiredAction:"code_graph_scan"` or equivalent fallback decision.
.opencode/skills/system-code-graph/manual_testing_playbook/read-path-freshness/query-self-heal-stale-file.md:49:Run the same broad stale fixture through `code_graph_context` to compare blocked envelope parity.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/mcp-json-server-key-rename.md:25:- Expected signals: mcpServers contains mk_code_index key. mk_code_index command is `node` and args includes `.opencode/bin/mk-code-index-launcher.cjs`. Key system_code_graph does NOT exist.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/mcp-json-server-key-rename.md:38:4. Verify `mk_code_index.command` is `node` and args includes `mk-code-index-launcher.cjs`.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/mcp-json-server-key-rename.md:42:mk_code_index key present, system_code_graph absent, command points to node, args points to mk-code-index-launcher.cjs.
.opencode/skills/system-code-graph/manual_testing_playbook/mcp-tool-surface/code-graph-query-blast-radius.md:79:6. **Readiness refusal:** if you can artificially mark the graph stale (e.g., touch a file mtime), repeat step 1. Expected: `status:"blocked"`, `requiredAction:"code_graph_scan"`, NO affectedFiles returned (hard refuse, not soft degrade per the readiness invariant in architecture.md §6).
.opencode/skills/system-code-graph/manual_testing_playbook/context-retrieval/code-graph-context-readiness-block.md:22:- Real user request: `Check that code_graph_context blocks broad stale state and returns readiness metadata instead of omitting graph-safety details.`
.opencode/skills/system-code-graph/manual_testing_playbook/context-retrieval/code-graph-context-readiness-block.md:23:- Operator prompt: `Validate code_graph_context against broad stale state in a disposable workspace. Show the blocked payload, graph omission and readiness metadata, then return PASS/FAIL.`
.opencode/skills/system-code-graph/manual_testing_playbook/context-retrieval/code-graph-context-readiness-block.md:27:- Pass/fail: PASS if broad stale state blocks with graph omission and readiness metadata. FAIL if context answers stale graph data, omits `requiredAction` or drops readiness/trust fields.
.opencode/skills/system-code-graph/manual_testing_playbook/context-retrieval/code-graph-context-readiness-block.md:49:Call with a single stale file and verify selective self-heal mirrors query behavior.
...
.opencode/skills/system-code-graph/manual_testing_playbook/mcp-tool-surface/code-index-cli-fallback-surface.md:11:This scenario validates the 028 CLI fallback for the mk-code-index server. `node .opencode/bin/code-index.cjs` exposes the same 8 tools as the `mcp__mk_code_index__*` registration via a manifest generated from `CODE_GRAPH_TOOL_SCHEMAS`, with the shared exit taxonomy 0/1/64/69/75. The check runs entirely against a sandboxed socket directory plus the shipped sandboxed suites, so host daemons are never contacted.
.opencode/skills/system-code-graph/manual_testing_playbook/mcp-tool-surface/code-index-cli-fallback-surface.md:52:- `warm-only exit=75` with no daemon spawned and no socket created in the sandbox.
.opencode/skills/system-code-graph/manual_testing_playbook/mcp-tool-surface/code-index-cli-fallback-surface.md:54:- The blocked-read suite passes its nine stale-readiness cases (`status:blocked` plus `requiredAction`, exit 0).
.opencode/skills/system-code-graph/manual_testing_playbook/mcp-tool-surface/code-index-cli-fallback-surface.md:82:| `.opencode/bin/code-index.cjs` | Stable shim with dist-freshness and socket guards |
.opencode/skills/system-code-graph/manual_testing_playbook/mcp-tool-surface/tool-call-shape-validation.md:24:- Expected execution process: Call `code_graph_query` with missing `subject`, call `code_graph_query` with missing `operation`, and call `detect_changes` with missing `diff`. (Notes: `code_graph_verify` has no required `rating` field in the current schema, and `code_graph_apply` deliberately declares no required fields — a bare apply routes to staleness-based re-scan by design — so neither is a valid missing-field target.)
...
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/unicode-normalization-fix-from-009.md:26:- Desired user-visible outcome: A concise verdict confirming stale sibling dist output is gone.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:3:description: "Verify the mk-code-index launcher starts cleanly with the [mk-code-index-launcher] prefix on stderr."
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:15:Verify the mk-code-index launcher starts cleanly with the [mk-code-index-launcher] prefix on stderr, reaches stdio readiness, and documents the shared idle-timeout guardrail.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:21:- Objective: Verify the mk-code-index launcher starts cleanly with the [mk-code-index-launcher] prefix on stderr, reaches stdio readiness, and exposes the shared idle-timeout documentation.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:22:- Real user request: `Check that the mk-code-index-launcher.cjs starts without error, prints the [mk-code-index-launcher] prefix on stderr, and documents the idle-timeout guardrail.`
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:24:- Expected execution process: Run `timeout 8 node .opencode/bin/mk-code-index-launcher.cjs </dev/null` and capture stderr. Then grep Code Graph docs for `SPECKIT_LAUNCHER_IDLE_TIMEOUT_MIN`.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:25:- Expected signals: Stderr contains `[mk-code-index-launcher]` prefix. No legacy `[system_code_graph]` or `[system-code-graph]` prefixes. No unhandled exception. Code Graph README or IPC README documents `SPECKIT_LAUNCHER_IDLE_TIMEOUT_MIN`.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:27:- Pass/fail: PASS if launcher starts with [mk-code-index-launcher] prefix, no errors, and idle-timeout docs are discoverable. FAIL if launcher crashes, shows legacy name, fails to print the expected prefix, or omits the idle-timeout guardrail from docs.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:35:1. Run `timeout 8 node .opencode/bin/mk-code-index-launcher.cjs </dev/null 2>&1 | head -10`.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:36:2. Check stderr for `[mk-code-index-launcher]` prefix.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:38:4. Run `rg -n "SPECKIT_LAUNCHER_IDLE_TIMEOUT_MIN" .opencode/skills/system-code-graph/README.md .opencode/skills/system-code-graph/mcp_server/lib/ipc/README.md`.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:42:Stderr shows `[mk-code-index-launcher] loaded N env(s) from .env.local` or similar. No legacy name. The grep command finds the shared idle-timeout setting in Code Graph docs.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:50:Check launcher file path in .claude/mcp.json points to mk-code-index-launcher.cjs.
.opencode/skills/system-code-graph/manual_testing_playbook/post-rename-infrastructure/launcher-startup-prefix.md:60:| `../../mcp_server/lib/ipc/launcher-idle-timeout.ts` | Idle-timeout guardrail implementation |
...
.opencode/skills/system-code-graph/manual_testing_playbook/doctor-code-graph/code-graph-apply-sub-operations.md:18:- Code graph index in any state (apply paths can recover from stale/error states).
.opencode/skills/system-code-graph/manual_testing_playbook/doctor-code-graph/code-graph-apply-sub-operations.md:58:   Expected: returns refusal requiring `confirm:true`. Hard-stale recovery is destructive and must be acknowledged.
...
.opencode/skills/system-code-graph/manual_testing_playbook/doctor-code-graph/doctor-apply-mode-policy.md:46:Remove scratch/apply artifacts from disposable copy.
