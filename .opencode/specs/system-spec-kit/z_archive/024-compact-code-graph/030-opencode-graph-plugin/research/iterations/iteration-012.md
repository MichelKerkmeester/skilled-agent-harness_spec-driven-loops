...
[SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/code-graph/ensure-ready.ts:109-165`]

But `session_health`, startup priming, and structural bootstrap mostly reduce staleness to "older than 24h". [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/session/session-snapshot.ts:52-59`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/session/session-snapshot.ts:214-257`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/session-health.ts:73-85`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/claude/session-prime.ts:138-149`]
...
`code_graph_query` and `code_graph_context` call readiness checks with inline indexing disabled, but they discard the returned `action/files/reason` payload. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/code-graph/context.ts:89-94`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/code-graph/query.ts:118-123`]
...
In `memory-surface`, `staleFiles` is actually counting parse-health problems while freshness is determined separately from `lastScanAt`. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:215-236`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:437-447`]
...
During reindex, per-file persistence failures are skipped best-effort, and debounce state is process-global rather than keyed by workspace/worktree. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/code-graph/ensure-ready.ts:183-197`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/code-graph/ensure-ready.ts:215-223`]
