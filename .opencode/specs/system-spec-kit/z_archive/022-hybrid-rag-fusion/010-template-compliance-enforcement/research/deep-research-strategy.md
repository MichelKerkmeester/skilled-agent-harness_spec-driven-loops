...
- Finding the uninstalled pre-commit hook: High-value discovery showing a designed-but-undeployed solution exists at `.opencode/skills/system-spec-kit/scripts/spec/pre-commit-spec-validate.sh` with matching `.speckit-enforce.yaml` config (iteration 4)
