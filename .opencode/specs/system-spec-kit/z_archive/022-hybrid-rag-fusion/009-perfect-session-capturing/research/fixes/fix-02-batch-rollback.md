...
● Edit .opencode/skills/system-spec-kit/scripts/core/file-writer.ts
...
● Read .opencode/skills/system-spec-kit/scripts/core/file-writer.ts lines 1-140
  └ 130 lines read
...
Verified the change with `npm run -s lint` in `.opencode/skills/system-spec-kit/scripts` (passed) and confirmed the new `backupPath`/`existedBefore` logic is present with `rg`.
