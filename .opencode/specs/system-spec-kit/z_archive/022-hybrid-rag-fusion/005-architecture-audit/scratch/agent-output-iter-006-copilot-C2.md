...
● Explore Audit MCP surface
  ...
  │   │ Env/Public/.opencode/skills/system-spec-kit/mcp_server/handlers/checkpoints.ts"
  │   └ 80 lines...
  └ Create audit output file (shell)
      ...
      │
      │ ## Files Audited
      │ 1. /.opencode/skills/system-spec-kit/mcp_server/api/*.ts (eval, index, indexing, providers, …
...
● Search (grep)
  │ "process\.env\.[A-Z0-9_]+" in **/*.{ts,tsx} (.opencode/skills/system-spec-kit/mcp_server)
  └ 5 lines found

● Search (grep)
  │ "MemoryIndexScanArgs|ScanArgs|parseValidatedArgs\(|parseArgs<|validateToolArgs\(" in
  │ **/*.{ts,tsx} (.opencode/skills/system-spec-kit/mcp_server)
  └ 78 lines found

● Search (grep)
  │ "export \* as" in **/*.{ts,tsx} (.opencode/skills/system-spec-kit/mcp_server)
  └ 2 lines found

● Search env usage in audited files (shell)
  │ cd '/Users/michelkerkmeester/MEGA/Development/Opencode Env/Public' && rg -n
  │ "process\.env\.[A-Z0-9_]+" .opencode/skills/system-spec-kit/mcp_server/api
  │ .opencode/skills/system-spec-kit/mcp_server/tools .opencode/skills/system-spec-kit/mcp_server/core
  └ 10 lines...

● Search (grep)
  │ "parseValidatedArgs\(" in **/*.{ts,tsx} (.opencode/skills/system-spec-kit/mcp_server)
  └ No matches found
...
- **Clean `api/` vs `tools/` boundary.** `api/` is a curated public surface with explicit `ARCH-1` comments, while `tools/` owns protocol dispatch and hands validated args to handlers. The only slight seam is `api/indexing.ts` importing a handler directly for orchestration. `.opencode/skills/system-spec-kit/mcp_server/api/index.ts:4-6`, `.opencode/skills/system-spec-kit/mcp_server/api/indexing.ts:4-15`, `.opencode/skills/system-spec-kit/mcp_server/tools/index.ts:15-35`

- **Tool schema pattern is consistent, but not fully DRY.** Dispatchers uniformly do `validateToolArgs(...)` then `parseArgs<T>(...)`, which is good. But `tools/types.ts` already defines `parseValidatedArgs()` and none of these dispatchers use it; `shared_memory_enable` also falls back to `Record<string, unknown>` instead of a named args type. `.opencode/skills/system-spec-kit/mcp_server/tools/types.ts:17-34`, `.opencode/skills/system-spec-kit/mcp_server/tools/memory-tools.ts:42-55`, `.opencode/skills/system-spec-kit/mcp_server/tools/lifecycle-tools.ts:48-64`
...
- **Error propagation is mostly correct, with two deliberate swallow/fallback paths.** `initializeIndexingRuntime()` and `checkDatabaseUpdated()` fail loudly on hard errors, but `getLastScanTime()` and `setLastScanTime()` only log and return fallback behavior, which can mask config-table persistence problems. `.opencode/skills/system-spec-kit/mcp_server/api/indexing.ts:37-40`, `.opencode/skills/system-spec-kit/mcp_server/core/db-state.ts:117-142`, `.opencode/skills/system-spec-kit/mcp_server/core/db-state.ts:228-263`, `.opencode/skills/sk-code-opencode/SKILL.md:367-370`

- **Configuration is centralized, but not perfectly.** `core/config.ts` is clearly the main config source, but `core/db-state.ts` still reads `process.env.SPECKIT_GRAPH_UNIFIED` directly, so env access is slightly scattered. `.opencode/skills/system-spec-kit/mcp_server/core/config.ts:42-56`, `.opencode/skills/system-spec-kit/mcp_server/core/config.ts:63-149`, `.opencode/skills/system-spec-kit/mcp_server/core/db-state.ts:182-186`

- **Barrel exports are clean overall, with one surface-broadening choice.** `api/index.ts` and `core/index.ts` use explicit grouped exports, but `api/providers.ts` and `api/search.ts` expose whole namespaces via `export * as`, which is less selective than the rest of the public surface. `.opencode/skills/system-spec-kit/mcp_server/api/index.ts:8-55`, `.opencode/skills/system-spec-kit/mcp_server/core/index.ts:5-67`, `.opencode/skills/system-spec-kit/mcp_server/api/providers.ts:7-13`, `.opencode/skills/system-spec-kit/mcp_server/api/search.ts:8-20`
