...
- **Description**: The `.opencode/skills/` directory contains 18 actual skills (excluding `scripts/` utility folder and `README.md`). The root README claims 16 skills in multiple places. Two skills are missing from the Skills Library listing: `sk-deep-research` and `mcp-coco-index`.
- **Evidence**: `ls .opencode/skills/ | grep -v README | grep -v scripts` lists 18 skill directories, each with a `SKILL.md`. The root README Skills Library (Section 8) lists only 16 skills. `sk-deep-research` is referenced elsewhere in the README (the `@deep-research` agent uses it) but is not listed in the skills table. `mcp-coco-index` is referenced in CLAUDE.md as "CocoIndex Code MCP" but absent from the skills table.
...
- **Evidence**: `ls .opencode/commands/spec_kit/*.md` shows 8 files including `deep-research.md`. `ls .opencode/commands/create/*.md` shows 7 files including `feature-catalog.md` and `testing-playbook.md`. The root README spec_kit table (line 344-354) lists 7 commands, missing `/spec_kit:deep-research`. The create table (line 368-374) lists 5 commands, missing `/create:feature-catalog` and `/create:testing-playbook`.
...
- **Severity**: HIGH
- **Category**: drift
- **Location**: `/Users/michelkerkmeester/MEGA/Development/Opencode Env/Public/.opencode/skills/system-spec-kit/README.md` (line 52)
...
- **Evidence**: `ls .opencode/commands/memory/*.md` returns 6 files. The Spec Kit README heading says "(7)" but body lists 6 bullet rows plus 1 sub-feature annotation.
...
- **Severity**: MEDIUM
- **Category**: completeness
- **Location**: `/Users/michelkerkmeester/MEGA/Development/Opencode Env/Public/.opencode/skills/system-spec-kit/README.md` (lines 292-302)
...
- **Severity**: MEDIUM
- **Category**: drift
- **Location**: `/Users/michelkerkmeester/MEGA/Development/Opencode Env/Public/.opencode/skills/system-spec-kit/mcp_server/package.json` (line 4)
...
- **Severity**: MEDIUM
- **Category**: completeness
- **Location**: `.opencode/specs/system-spec-kit/022-hybrid-rag-fusion/016-rewrite-memory-mcp-readme/` through `019-rewrite-repo-readme/`
...
- **Severity**: MEDIUM
- **Category**: drift
- **Location**: `/Users/michelkerkmeester/MEGA/Development/Opencode Env/Public/.opencode/skills/system-spec-kit/mcp_server/INSTALL_GUIDE.md` (lines 295, 390)
...
- **Description**: The root README Configuration section (line 554) says the default database path is `.opencode/skills/system-spec-kit/shared/mcp_server/database/context-index.sqlite`. The Install Guide (line 81, 282) says the canonical path is `.opencode/skills/system-spec-kit/mcp_server/dist/database/context-index.sqlite`. These are different paths -- the root README has `shared/mcp_server/database/` while the Install Guide has `mcp_server/dist/database/`.
- **Evidence**: Root README line 554: `Default database path: .opencode/skills/system-spec-kit/shared/mcp_server/database/context-index.sqlite`. Install Guide lines 81, 109: `mcp_server/dist/database/context-index.sqlite`.
...
- **Severity**: MEDIUM
- **Category**: quality
- **Location**: `.opencode/skills/system-spec-kit/manual_testing_playbook/` vs `.opencode/specs/.../015-manual-testing-per-playbook/`
...
- **Severity**: MEDIUM
- **Category**: quality
- **Location**: `/Users/michelkerkmeester/MEGA/Development/Opencode Env/Public/.opencode/skills/system-spec-kit/mcp_server/README.md` (lines 235, 293)
...
- **Description**: The spec_kit commands table in Section 7 lists 7 commands but `.opencode/commands/spec_kit/` contains 8 files including `deep-research.md`. The `/spec_kit:deep-research` command is mentioned in Section 4 (Quick Reference table in CLAUDE.md) and the `@deep-research` agent is documented, but the command itself is not in the root README's command table.
- **Evidence**: `ls .opencode/commands/spec_kit/*.md` returns 8 files. The table header does not state a count but lists 7 rows.
...
- **Description**: The create commands table lists 5 commands but `.opencode/commands/create/` contains 7 `.md` files. Missing: `/create:feature-catalog` and `/create:testing-playbook`.
- **Evidence**: `ls .opencode/commands/create/*.md` returns 7 files including `feature-catalog.md` and `testing-playbook.md`.
