---
title: "Feature Specification: Full Spec Folder Document [system-spec-kit/z_archive/009-full-spec-doc-indexing/spec]"
...
| **Status** | Complete |
| **Created** | 2026-03-31 |
| **Branch** | `009-full-spec-doc-indexing` |
<!-- /ANCHOR:metadata -->
...
### Problem Statement
This archived folder captures work that indexed full spec folder documents. The older archive shape no longer matched the active template contract, which caused strict validation errors.

### Purpose
Keep a concise archival summary of the full spec document indexing work in a format that passes the current validator.
<!-- /ANCHOR:problem -->
...
### In Scope
- Preserve the archived topic and folder identity for full spec folder document indexing.
...
- Performing new indexing work outside this archived folder.
...
|----|-------------|---------------------|
| REQ-001 | The archive must clearly state that the folder documents full spec folder document indexing. | A maintainer can identify the archived topic from spec.md alone. |
...
- **Given** a maintainer opens this archived folder, **when** they read the core documents, **then** they can understand that the archived work indexed full spec folder documents.
...
| Risk | Historical detail is condensed | Some original indexing detail is no longer in top-level docs. | Preserve the feature summary here and rely on git history for deeper reconstruction. |
<!-- /ANCHOR:risks -->
