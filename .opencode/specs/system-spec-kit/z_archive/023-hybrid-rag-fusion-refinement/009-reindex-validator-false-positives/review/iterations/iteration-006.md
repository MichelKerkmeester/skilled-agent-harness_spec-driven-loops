...
- Evidence:
  - The scan handler disables the incremental fast path whenever `force` is set, then still sends every file through `indexSingleFile(...)`; `warn-only` only changes `qualityGateMode`, not whether the file is skipped. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-index.ts:367-406`]
  - Both same-path unchanged detection and content-hash duplicate detection explicitly short-circuit only when `!force`, so forced scans bypass the cheap exits that normally avoid rework. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/save/dedup.ts:182-239`; `.opencode/skills/system-spec-kit/mcp_server/handlers/save/dedup.ts:242-307`]
  - The save pipeline performs embedding-cache lookup / generation before the write transaction, and because the duplicate guards above are disabled under `force`, unchanged files still reach that stage. On a cache miss they will regenerate embeddings even though content is unchanged. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:728-805`]
  - The write path then enters an immediate transaction and, for same-path existing files, routes through append-only lineage logic. That ultimately forces `vector-index` into the `INSERT` path instead of the in-place `UPDATE` path when `appendOnly` is true. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:927-981`; `.opencode/skills/system-spec-kit/mcp_server/handlers/save/create-record.ts:166-177`; `.opencode/skills/system-spec-kit/mcp_server/handlers/save/create-record.ts:180-250`; `.opencode/skills/system-spec-kit/mcp_server/lib/search/vector-index-mutations.ts:194-253`]
...
- Evidence:
  - The new fallback regex for missing `spec_folder` is a single non-nested match against `filePath`; by itself it is cheap. [SOURCE: `.opencode/skills/system-spec-kit/scripts/lib/validate-memory-quality.ts:624-637`]
  - The heavier part is the follow-on work done on every validation call: resolving candidate spec-folder paths by walking parent directories and calling `fs.statSync`, scanning child phase folders with `fs.readdirSync`, and re-reading `spec.md` once for `related_specs` and again for `trigger_phrases`. [SOURCE: `.opencode/skills/system-spec-kit/scripts/lib/validate-memory-quality.ts:482-515`; `.opencode/skills/system-spec-kit/scripts/lib/validate-memory-quality.ts:524-555`; `.opencode/skills/system-spec-kit/scripts/lib/validate-memory-quality.ts:706-733`; `.opencode/skills/system-spec-kit/scripts/lib/validate-memory-quality.ts:829-846`]
- Impact:
...
- Performance-wise, the table does have an index on `file_path`, so a one-off 14k-row dedup grouped by that column is not alarming on its own. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/search/vector-index-schema.ts:1929-1930`; `.opencode/skills/system-spec-kit/mcp_server/lib/search/vector-index-schema.ts:2375`; `.opencode/skills/system-spec-kit/mcp_server/lib/storage/schema-downgrade.ts:178`]
...
- In the reviewed code, adding `'planning'` appears in table-creation / rebuild DDL, not as a standalone `ALTER TABLE ... ADD CONSTRAINT` on an existing SQLite table. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/search/vector-index-schema.ts:2254`; `.opencode/skills/system-spec-kit/mcp_server/lib/storage/schema-downgrade.ts:137`]
- The normal upgrade path shown in this file adds the `context_type` column without rebuilding the table, so this change does not introduce a new per-row validation sweep on already-upgraded databases. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/search/vector-index-schema.ts:1656-1659`]

### 3. Template backfill over 2,383 files with `--include-archive`

- No severity finding for the reviewed scale.
- The CLI is a synchronous maintenance script that discovers targets, deduplicates them, then processes files serially with one read/build/write pass per file. [SOURCE: `.opencode/skills/system-spec-kit/scripts/memory/backfill-frontmatter.ts:236-303`; `.opencode/skills/system-spec-kit/scripts/memory/backfill-frontmatter.ts:343-395`; `.opencode/skills/system-spec-kit/scripts/memory/backfill-frontmatter.ts:487-571`]
