...
1. `.opencode/skills/system-spec-kit/mcp_server/lib/search/vector-index-store.ts` — find per-call `await import(...)` in hot retrieval/read methods (around lines 20, 942). Replace with module-level cached lazy init:
```typescript
...
2. `.opencode/skills/system-spec-kit/mcp_server/cli.ts` — move heavy top-level imports (vector-index, checkpoint, access-tracker, mutation-ledger, trigger-matcher, startup-checks — around lines 15-24) into per-command handler functions. Keep type imports and argument parsing at the top level. Make sure `--help` and `--version` don't trigger heavy imports.
