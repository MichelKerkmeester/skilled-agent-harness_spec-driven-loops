...
Research Topic: system-code-graph uplift: skill usefulness, marketing README rewrite (problem-first arc with HVR clean prose), and sk-doc 1:1 alignment (validate_document.py --type <type> exit-0 across every authored doc). Inputs: Public root README + system-spec-kit README as structural exemplars for the marketing voice; sk-doc readme_template.md + hvr_rules.md + template_rules.json as the validation contract; system-code-graph current authored docs as the targets; INSTALL_GUIDE.md lines 49/56/195 as confirmed drift. Three downstream implementation children scaffolded as stubs at 001-skill-md-and-references-polish/, 002-readme-marketing-rewrite/, 003-sk-doc-1to1-alignment/.
...
- Write iteration narrative to: `.opencode/specs/system-spec-kit/029-system-code-graph-uplift/research/iterations/iteration-001.md`
- Write per-iteration delta file to: `.opencode/specs/system-spec-kit/029-system-code-graph-uplift/research/deltas/iter-001.jsonl`
...
1. **Iteration narrative markdown** at `.opencode/specs/system-spec-kit/029-system-code-graph-uplift/research/iterations/iteration-001.md`. Structure: H2 headings for Focus, Actions Taken, Findings (per-file subsections with file:line citations), Questions Answered, Questions Remaining, Next Focus.

2. **Canonical JSONL iteration record** APPENDED (echo with `>>`) to `.opencode/specs/system-spec-kit/029-system-code-graph-uplift/research/deep-research-state.jsonl`. Required schema:
...
Optional: write the per-iteration delta file to `.opencode/specs/system-spec-kit/029-system-code-graph-uplift/research/deltas/iter-001.jsonl` with the same iteration record plus one record per finding. If the recipe's write-permission denies the deltas path, the workflow will synthesize the delta file from your state-log append.
