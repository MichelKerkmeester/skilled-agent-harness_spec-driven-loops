...
- **Dispositioned-deferred (REQ-004, needs operator sign-off):** `doctor_memory.yaml` and `doctor_code-graph.yaml` carry no CLI probes — a genuine diagnostic gap (both routes depend solely on MCP tools, the surface unreachable in a transport flap). Recommended additive fix (verified against shipped CLIs): `node .opencode/bin/spec-memory.cjs memory_health --format json --timeout-ms 3000 --warm-only` and `node .opencode/bin/code-index.cjs code_graph_status --format json --timeout-ms 3000 --warm-only`, plus matching `_routes.yaml` rows. Left unedited per the spec's no-edit-without-sign-off rule.
