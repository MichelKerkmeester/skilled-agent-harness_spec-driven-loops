...
_memory:
  continuity:
    packet_pointer: ".opencode/specs/system-spec-kit/030-token-aware-chunking"
    ...
    session_dedup:
      ...
      session_id: "2026-05-14-030-token-aware-chunking"
...
| `.opencode/specs/system-spec-kit/030-token-aware-chunking/` | Create | Track scope, plan, tasks, verification, and metadata for this packet. |
<!-- /ANCHOR:scope -->
...
| REQ-007 | Packet validation and TypeScript checks must pass | `npx tsc --noEmit`, targeted Vitest, and `validate.sh --strict` all exit 0 for packet 030. |
<!-- /ANCHOR:requirements -->
...
| Risk | Dirty parallel-track worktree | Accidental staging can pollute the commit. | Stage only the 030 packet, llama-cpp provider, and new 030 test file. |
<!-- /ANCHOR:risks -->
