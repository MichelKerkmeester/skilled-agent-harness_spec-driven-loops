...
description: "Task list for adding tokenizer-budget truncation to the llama-cpp embedding provider and validating the 030 packet."
trigger_phrases:
  - "030 token-aware chunking tasks"
...
- [x] T003 Confirm the scoped fix is limited to `llama-cpp.ts`, a new test file, and the 030 packet docs.
<!-- /ANCHOR:phase-1 -->
...
- [x] T013 Run `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh .opencode/specs/system-spec-kit/030-token-aware-chunking --strict`.
- [B] T014 Stage only the 030 packet, provider file, and new 030 test file. Blocked by `.git/index.lock` EPERM.
- [B] T015 Commit `fix(030): token-aware llama-cpp truncation (bug-B)` on `main`. Blocked by `.git/index.lock` EPERM.
<!-- /ANCHOR:phase-3 -->
