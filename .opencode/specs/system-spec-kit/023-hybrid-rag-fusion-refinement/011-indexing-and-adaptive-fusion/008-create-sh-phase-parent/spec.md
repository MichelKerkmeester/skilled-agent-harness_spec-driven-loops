...
- Define validation rules for nested spec trees, including track folders like `system-spec-kit`
...
| REQ-003 | Validate nested parent paths under both `specs/` and `.opencode/specs/` | Parent paths like `system-spec-kit/023-hybrid-rag-fusion-refinement/011-indexing-and-adaptive-fusion` are accepted when they resolve under an approved root |
...
### Path Boundaries
- Nested parent under `.opencode/specs/` with a track folder prefix such as `system-spec-kit`
