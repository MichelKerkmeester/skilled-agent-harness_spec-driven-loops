...
<!-- ANCHOR:adr-001 -->
## ADR-001: Consume cached summaries only as gated hints
...
The research demoted warm-start work because cached summaries can be warm but wrong. Public now has the producer and reader substrate needed to try the consumer safely, but only if it remains subordinate to bootstrap and live recovery.
...
**We chose**: Open the cached-summary consumer packet in 024 and make it additive, gated, and subordinate to bootstrap and resume authority.
...
|--------|------|------|-------|
| **Gated cached-summary consumer** | Uses the existing continuity train and preserves authority boundaries | Requires evaluation on a frozen resume corpus | 8/10 |
| **Ungated startup fast path** | Looks faster and simpler | Contradicts the research and risks correctness regressions | 2/10 |
