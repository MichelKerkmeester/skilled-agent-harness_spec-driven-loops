...
- [ ] CHK-P1-026 [Finding: OPUS-A3-001] Verify all 20 description.json files under 009-perfect-session-capturing have an explicit `status` field -- `find .../009-perfect-session-capturing -name "description.json" -exec grep -L '"status"' {} \;` returns empty (all have status)
- [ ] CHK-P1-027 [Finding: OPUS-A2-006] Verify parent checklist summary counts are correct -- re-count all `[P0]`, `[P1]`, `[P2]` markers in the parent `009-perfect-session-capturing/checklist.md` and confirm the summary table matches
...
- [ ] CHK-P2-003 [Finding: OPUS-A2-009] Verify parent spec.md Phase Documentation Map table includes a row for Phase 019 -- inspect `009-perfect-session-capturing/spec.md` Section 4 table for a 019 entry
