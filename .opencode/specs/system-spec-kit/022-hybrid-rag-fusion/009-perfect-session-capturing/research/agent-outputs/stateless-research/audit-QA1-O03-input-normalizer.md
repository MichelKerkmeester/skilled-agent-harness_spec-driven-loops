...
```
The `> 2` threshold drops legitimate 2-character folder segments such as "ui", "db", "ci", "qa", "mx". A spec folder like `system-spec-kit/022-hybrid-rag-fusion/015-ui` would have its most specific segment ("ui") excluded from relevance matching, causing the filter to rely only on ancestor segments — which are overly broad and match unrelated content.
