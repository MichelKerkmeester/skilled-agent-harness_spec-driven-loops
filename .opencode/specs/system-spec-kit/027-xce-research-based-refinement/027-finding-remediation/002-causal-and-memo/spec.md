---
title: "Feature Specification: Finding Remediation Lane: Causal And Memo"
description: "Causal graph and memo correctness: governed drift scope, correction undo edge state, corrections bypassing edge validation, memo DAG multi-writer cycle checks, causal-boost undirected scoring and unbounded neighbor fetch."
trigger_phrases:
  - "causal-and-memo remediation"
  - "027 finding lane 002"
  - "epic sweep lane 002"
...
_memory:
  continuity:
    ...
    session_dedup:
      ...
      session_id: "2026-06-11-002-causal-and-memo-scaffold"
...
# Feature Specification: Finding Remediation Lane: Causal And Memo
...
### Problem Statement
The epic deep-review sweep left this lane with 14 unverified P1 claims and 5 P2 clusters. Unverified single-seat claims can be neither fixed nor dismissed without evidence.

### Purpose
Causal graph and memo correctness: governed drift scope, correction undo edge state, corrections bypassing edge validation, memo DAG multi-writer cycle checks, causal-boost undirected scoring and unbounded neighbor fetch.
<!-- /ANCHOR:problem -->
...
### In Scope
- The lane's backlog entries in `../backlog/p1-backlog.json` and `../backlog/p2-backlog.json` (lane = `002-causal-and-memo`).
