---
title: "Tasks: Phase 19: sk-prompt-small-model Frontmatter Alignment"
description: "Task Format: T### [P?] Description (file path)"
trigger_phrases:
  - "sk-prompt-small-model frontmatter tasks"
...
# Tasks: Phase 19: sk-prompt-small-model Frontmatter Alignment
...
- [x] T001 Capture coverage-mode baseline: 14/14 docs fail; non-profile docs miss the three detailed fields, the 7 model profiles also miss `description` and carry non-contract registry keys (`check-skill-doc-frontmatter.sh --skill sk-prompt-small-model --coverage`)
<!-- /ANCHOR:phase-1 -->
...
- [x] T002 Author full contract on the 7 model profiles with model-named trigger phrases and new one-line descriptions; drop `model_id`/`profile_of`/`status`/`last_benchmarked` (`.opencode/skills/sk-prompt-small-model/references/models/{deepseek-v4-pro,glm-5.1,kimi-k2.6,mimo-v2.5-pro,minimax-2.7,minimax-m3,qwen3.6}.md`)
...
- [x] T005 Coverage check green: `PASS mode=coverage scope=sk-prompt-small-model docs=14 carrying-detailed-block=14 violations=0`
- [x] T006 Python local-mode smoke with `SPECKIT_ADVISOR_DOC_TRIGGERS=true`: "minimax m3 prompt framework" ranks sk-prompt-small-model first (0.95) with `!minimax m3 prompt framework(signal)`; live-daemon `matchedDocs` smoke rides packet 145 T025
