...
_memory:
  continuity:
    ...
    recent_action: "Scaffolded 027/002 phase from sub-packet-proposals.md Proposal 2"
    next_safe_action: "Implement code-graph-trace.ts (depends on Phase 001 HLD/LLD)"
    blockers: []
...
- Downward impact tracing (that's Phase 003).
...
| REQ-004 | `architectural_role` uses Phase 001's `classifyFileRole(filePath, db)` AND **MUST equal `generateHLD(filePath, db).file_role`** for the same indexed state | Shared-contract test imports Phase 001's exported signature; equality enforced. (Amended per B-iter006-002.) |
...
- **SC-004**: Phase 003 (impact analysis) can reuse trace chains for downstream-dependency narrative.
<!-- /ANCHOR:success-criteria -->
...
| Risk | **Parallel work before Phase 001 ships can hide classifier-contract drift** | Medium | Use typed test doubles ONLY before Phase 001 ships; later add shared-contract test against real `classifyFileRole()` (REQ-004 amended). |
| Risk | Deeply nested traces (10+ levels) hit budget | Low | Cap at maxDepth (default 5); truncate with marker |
| Dependency | Phase 027/001 (HLD/LLD) must ship first | Internal | Hard sequence — Phase 002 cannot start until 001 lands; REQ-015 from 001 pins `classifyFileRole(filePath, db)` signature |
...
- Symbol at file-level (no class container): `class: null`, `file: <path>`, role from Phase 001.
...
| Risk (depth-cap + Phase 001 dep) | 6/25 |
