---
title: "Implementation Plan: 027/003 Code Graph Trace"
description: "Plan for filePath-based trace resolution with optional containment display, Phase 002 role reuse, and deferred package metadata."
trigger_phrases:
  - "027 003 trace plan"
...
_memory:
  continuity:
    ...
    next_safe_action: "Wait for Phase 002 classifyFileRole export, then implement trace core"
    blockers:
      - "Phase 002 classifyFileRole export."
    key_files:
      - "spec.md"
      - "tasks.md"
      - "checklist.md"
...
---
# Implementation Plan: 027/003 Code Graph Trace
...
### Overview
Phase 003 adds a `code_graph_trace` tool that resolves architectural trace output from reliable graph facts. pt-02 corrected the original plan: file and module ownership come from `CodeNode.filePath` and an explicit file-path policy, while CONTAINS/fqName metadata is optional display context.
<!-- /ANCHOR:summary -->
...
### Definition of Ready
- [ ] Phase 002 `classifyFileRole(filePath, db)` signature exists.
...
- **Phase 002 `classifyFileRole`**: source of architectural role value.

### Data Flow
The handler receives a symbol id, the library loads the subject `CodeNode`, resolves file ownership from `filePath`, derives module from path policy, decorates the chain with available class/method metadata, and aliases architectural role to Phase 002's file role.
<!-- /ANCHOR:architecture -->
...
| Phase 002 `classifyFileRole` | Role source | Consume | Equality contract test |
...
| Integration | MCP handler and Phase 002 role equality | Vitest |
...
|------------|------|--------|-------------------|
| Phase 002 `classifyFileRole` | Internal | Pending | Cannot emit authoritative `architectural_role` |
...
|-------|------------|--------|
| Setup | Phase 002 export | Core implementation |
| Core implementation | Setup | Verification |
| Verification | Core implementation | Phase 006 evaluation |
<!-- /ANCHOR:phase-deps -->
...
- [ ] Confirm Phase 002 contract tests pass.
