...
Nothing implemented yet. This sub-phase was scaffolded from `../../review/fresh-regression-75/deep-review-findings-registry.json`; its 8 findings are enumerated as tasks in `tasks.md` and indexed in `../fix-coverage.json`.
<!-- /ANCHOR:what-built -->
...
Review-side verification recorded in the registry + `round2/code-verdicts.json`. Implementation verification (vitest per fix against a fixture graph DB) is pending; confirm via `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh <this-folder> --strict`.
<!-- /ANCHOR:verification -->
