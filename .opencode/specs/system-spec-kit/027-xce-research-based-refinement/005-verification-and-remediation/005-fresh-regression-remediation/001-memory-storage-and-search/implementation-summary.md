...
description: "Planning-only status for this remediation sub-phase: 34 findings carried as tasks; no fixes applied yet."
...
Nothing implemented yet. This sub-phase was scaffolded from `../../review/fresh-regression-75/deep-review-findings-registry.json`; its 34 findings are enumerated as tasks in `tasks.md` and indexed in `../fix-coverage.json`.
<!-- /ANCHOR:what-built -->
...
Review-side verification recorded in the registry + `round2/code-verdicts.json`. Implementation verification (vitest regression test per fix; capture baseline→after delta) is pending; confirm via `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh <this-folder> --strict`.
<!-- /ANCHOR:verification -->
