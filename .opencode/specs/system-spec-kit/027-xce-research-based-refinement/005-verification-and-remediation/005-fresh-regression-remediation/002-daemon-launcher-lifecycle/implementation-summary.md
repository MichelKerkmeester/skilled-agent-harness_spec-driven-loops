...
Nothing implemented yet. This sub-phase was scaffolded from `../../review/fresh-regression-75/deep-review-findings-registry.json`; its 15 findings are enumerated as tasks in `tasks.md` and indexed in `../fix-coverage.json`.
<!-- /ANCHOR:what-built -->
...
Review-side verification recorded in the registry + `round2/code-verdicts.json`. Implementation verification (vitest in the isolated fake-root harness; no live recycles) is pending; confirm via `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh <this-folder> --strict`.
<!-- /ANCHOR:verification -->
