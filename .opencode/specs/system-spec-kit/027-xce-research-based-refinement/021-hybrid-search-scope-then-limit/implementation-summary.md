...
3. **Pre-existing follow-up (out of this phase's scope).** The raw spec-folder `LIKE` prefix pattern still exists in more than one retrieval lane: the FTS5 predicate uses `LIKE ? || '/%'`, while vector and hybrid/keyword call sites also pass raw `${specFolder}/%` values without an `ESCAPE` clause. Current spec-folder identifiers are restricted to digits, lowercase, and hyphens, so there is no real exposure today; escaping the prefix across all affected lanes is tracked as a separate hardening item.
<!-- /ANCHOR:limitations -->
