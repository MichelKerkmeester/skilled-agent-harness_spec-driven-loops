...
`feature_catalog/03--lifecycle-routing/schema-migration.md:21`: "The migration runs internally during daemon bring-up." `lib/lifecycle/schema-migration.ts`'s only importers are `tests/lifecycle-derived-metadata.vitest.ts:35` and `stress_test/skill-advisor/lifecycle-routing-stress.vitest.ts:18` — no daemon or production caller. `README.md:108` and `references/runtime/freshness_contract.md:132-135` both state the daemon only watches/invalidates and that SQLite mutation happens only via `advisor_rebuild`/`skill_graph_scan`. Not touched by today's advisor doc batch.

### tri-087 — Manual baseline says 52 cases, fixture has 50 — STILL-REAL (doc-only)
`manual_testing_playbook/10--python-compat/regression-suite.md:55` still treats "Fewer than 52 cases" as partial load; `references/hooks/skill_advisor_hook.md:227` still says "52/52 Python regression suite passed"; `mcp_server/scripts/fixtures/skill_advisor_regression_cases.jsonl` has exactly 50 rows (wc -l re-run today).
