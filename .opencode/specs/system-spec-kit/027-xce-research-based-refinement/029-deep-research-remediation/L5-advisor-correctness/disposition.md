...
## Code queue (open)
Code-small: tri-168 (recommend shadow-sink writes on read path) — opt-in gate implemented (path env or `SPECKIT_ADVISOR_SHADOW_DELTA_ENABLED`, launcher-allowlisted, default-off test added), awaiting next verification batch; the gate is sink-mechanics-neutral so the L7 shadow-telemetry cluster can build on it.
