---
title: "Decision Record — Phase 011 Coco-Memory Context Extras"
description: "ADRs for Phase 011: L3 designation, capture path resolution, separate response group rationale, post-retrieval curator placement, budget split, JSON schema validation gate, default-off + Phase-006 lift requirement."
trigger_phrases:
  - "027 phase 011 ADRs"
...
# Architectural Decision Records: Phase 011 Coco-Memory Context Extras
...
**Decision:** Designate Phase 011 as Level 3.
...
- **Both require Phase-006 promotion gate** — active mode requires eval lift over deterministic baseline.
...
- **Largest LOC budget after Phase 009** — ~500-800 prod + ~340-570 tests; bundles two features for governance efficiency.
...
**Rationale:**
- **Clear semantics** — A3 (Phase 009) changes weights; A4 surfaces examples. Mixing the two would blur the mechanisms and make failures harder to debug.
...
- **Bounded overfetch** — start with 100-300, NOT 2K; can raise if Phase-006 eval shows benefit.
...
- 2K candidate budget v1 — rejected (too large; explore via Phase-006 eval first).
...
## ADR-007: Default-Off + Phase-006 Lift Requirement
...
**Decision:** Both features default-off. Active mode requires Phase-006 eval lift over deterministic baseline.
...
- **Phase-006 eval is the trust boundary** — without it, "looks better" isn't proof.
...
- Phase-006 eval harness must include task sets for both features.
...
Designate Phase 011 as **Level 3**. Apply full L3 file contract: spec.md, plan.md, tasks.md, checklist.md, decision-record.md, implementation-summary.md, description.json, graph-metadata.json, plus per-child resource-map.md per user directive.
<!-- /ANCHOR:adr-001-decision -->
