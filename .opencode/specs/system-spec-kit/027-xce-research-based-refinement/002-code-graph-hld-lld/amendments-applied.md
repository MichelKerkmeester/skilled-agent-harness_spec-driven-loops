...
| **B-iter006-001** | `file_role` is open string in Phase 002 but consumed as cross-phase contract by Phase 003; closed enum would already exclude `empty` | REQ-007 amended (open-string + reserved edge labels) | iteration-006.md |
| **B-iter006-002** | `classifyFileRole()` referenced by Phase 003 but not pinned as exported Phase 002 signature | NEW REQ-015 (public classifier contract) | iteration-006.md |
...
| **REQ-015** | **NEW P0** | Public classifier contract: export `classifyFileRole(filePath, db)`. Phase 003 imports this. Closes B-iter006-002. |
...
- None for Phase 002 — all proposed amendments applied.
...
- ⚠️ Conditional: REQ-016 omni wire-contract integration is large; if scope-cuts needed, the alternative (REMOVE `omni` from Phase 002 scope) is explicit and pre-approved.
