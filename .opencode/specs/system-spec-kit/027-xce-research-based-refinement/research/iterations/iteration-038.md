...
- `002-feedback-p0-correctness/spec.md` — P0 priority, LOC ~50-80 prod + 60-100 tests, no blockers (`002-feedback-p0-correctness/spec.md:59,65`)
- `003-memoization-dependency-dag-foundation/spec.md` — Level 3, DAG-aware scan planner, no hard blockers beyond schema extensions (`003-memoization-dependency-dag-foundation/spec.md:43-50,80-130`)
- `004-causal-graph-lifecycle-tombstones/spec.md` — Level 2, depends on causal-edges schema in place; 005 explicitly blocks on 004 (`004-causal-graph-lifecycle-tombstones/spec.md:43-47`; `005-frontmatter-causal-edge-promoter/spec.md:blockers:- "004"`)
- `005-frontmatter-causal-edge-promoter/spec.md` — blocks on 004; incremental blast radius because it only writes `auto`-provenance edges and never overwrites manual ones (`005-frontmatter-causal-edge-promoter/spec.md:REQ-007,REQ-008`)
- `006-statediff-reconciliation-layer/spec.md` — blocks on 003 for DAG action-plan; succeeds 005 in dependency chain (`006-statediff-reconciliation-layer/spec.md:blockers:- "003"`)
- `007-memory-semantic-triggers/spec.md` — no hard blockers; LOC ~280-430 prod + 180-280 tests; false-positive risk is the primary concern (`007-memory-semantic-triggers/spec.md:description,LOC budget`)
- `008-feedback-reducers/spec.md` — phase parent with 5 children; child 001-aggregator depends on 002; total family LOC ~70+370+265+385+100 = ~1190 (`008-feedback-reducers/spec.md:57-63`)
...
**Verdict**: ADOPT  
**Cite**: `002-feedback-p0-correctness/spec.md:49` — "Ordering decision: '009 P0 fixes before reducers' and before all code_graph phases in the refreshed 027 sequence."  
**Cite**: `002-feedback-p0-correctness/spec.md:59,65` — Priority P0, LOC ~50-80 prod + 60-100 tests, no blockers.  
**Cite**: `008-feedback-reducers/spec.md:59` — 008/001-aggregator "Depends On: 027-xce-research-based-refinement/002-feedback-p0-correctness".
...
**Cite**: `004-causal-graph-lifecycle-tombstones/spec.md:43` — "Successor: 005-frontmatter-causal-edge-promoter"; "005 depends on this cleanup foundation before increasing generated edge volume."  
**Cite**: `005-frontmatter-causal-edge-promoter/spec.md:blockers:- "004"` — explicit hard blocker.  
**Cite**: `003-memoization-dependency-dag-foundation/spec.md:Predecessor:001-rename, Successor:004-causal-graph-lifecycle-tombstones` — 003 feeds 004 in spec chain.
...
**Verdict**: ADOPT  
**Cite**: `005-frontmatter-causal-edge-promoter/spec.md:REQ-004` — "Promoted rows include `created_by='auto'`, `extraction_method='frontmatter'`, `confidence=1.0`."  
**Cite**: `005-frontmatter-causal-edge-promoter/spec.md:REQ-007` — "Auto edges participate in lifecycle cleanup" via 004 tombstone path.  
**Cite**: `005-frontmatter-causal-edge-promoter/spec.md:REQ-008` — "If a manual edge already exists for the same identity, promoter does not reduce strength, evidence, or creator without explicit policy."
...
**Verdict**: ADAPT  
**Cite**: `006-statediff-reconciliation-layer/spec.md:blockers:- "003"` — hard dependency on 003 DAG planner.  
**Cite**: `006-statediff-reconciliation-layer/spec.md:Predecessor:005-frontmatter-causal-edge-promoter` — chains after 005.  
**Cite**: `006-statediff-reconciliation-layer/spec.md:REQ-004` — "Stale deletes remain guarded by replacement-index success. Existing behavior that defers stale cleanup after failed replacement indexing is preserved in statediff planning."
...
**Verdict**: ADOPT  
**Cite**: `007-memory-semantic-triggers/spec.md:description` — "Lexical remains primary precision path; semantic adds paraphrase recall as feature-flagged UNION."  
**Cite**: `007-memory-semantic-triggers/spec.md:blockers:[]` — no declared blockers.  
**Cite**: `007-memory-semantic-triggers/spec.md:LOC budget` — ~280-430 production + ~180-280 tests.  
...
**Verdict**: ADAPT  
**Cite**: `008-feedback-reducers/spec.md:57-63` — child table: 001-aggregator→{002-coco-rerank-consumer, 003-causal-reducer, 004-retention-reducer}→005-env-tests-integration.  
**Cite**: `008-feedback-reducers/spec.md:39` — "P0 correctness fixes are owned by 012." [Note: renumbered to 002 in current scaffold.]  
...
|------|-------|-------|--------------------------|-------|------|-----------|
| 1 | 002 | Feedback P0 Correctness | ~110-180 | HIGH (P0 safety) | LOW (3 surgical fixes, no schema change) | Blocks entire 008 family; must ship first per pt-04 ordering decision (`002-feedback-p0-correctness/spec.md:49`) |
| 2 | 003+004 | Memoization DAG + Tombstone Lifecycle (merge-sprint) | ~240-360 | HIGH (foundation for 005,006) | MEDIUM (schema migration + DAG planner) | Co-located schema migration reduces deployment windows; 004 is explicit prereq of 005 (`005-frontmatter-causal-edge-promoter/spec.md:blockers`) |
| 3 | 005 | Frontmatter Causal-Edge Promoter | ~200-340 | HIGH (automatic causal graph population) | LOW (additive-only, idempotent, never overwrites manual edges) | Small blast radius; acceptance criteria fully specified; ships immediately after 003+004 |
| 4 | 007 | Memory Semantic Triggers | ~460-710 | HIGH (recall improvement) | MEDIUM (false-positive risk, mitigated by default-off + guards) | No blockers from 003-006; can develop in parallel with 006; ships when shadow-eval period passes |
| 5 | 006 | Statediff Reconciliation Layer | ~340-540 | HIGH (write-path correctness) | HIGH (touches all durable projections) | Ships after 005 stable; REQ-004 safety guard must be preserved; highest architectural risk of all non-008 phases |
...
|-------|--------|-----------|
| 003+004 | **MERGE-SPRINT** (keep separate specs, ship same branch) | Co-located schema migration; 004 is required prerequisite of 005; saves one deployment window (`004-causal-graph-lifecycle-tombstones/spec.md:43`; `005-frontmatter-causal-edge-promoter/spec.md:blockers`) |
...
| 007 | **KEEP** but can parallelize with 006 | No schema blockers; development can overlap with 006 provided 007 stays default-off until shadow-eval passes |
...
| None | **SPLIT** | No phase requires splitting; 002 was already the split of the old 009 Sub-Phase 1 P0 bundle (`002-feedback-p0-correctness/spec.md:31`) |
