...
iteration: 036
rq: RQ-N7
phase_target: 008-feedback-reducers
newInfoRatio: 0.72
verdict: ADAPT
---

# Iteration 036 — RQ-N7: 008-feedback-reducers Sub-Phase Decomposition

## Research Question

What sub-phases should `008-feedback-reducers` spawn? Specifically: (A) isolate P0 correctness fixes into the earliest sub-phase, (B) gate learning reducer consumers on P0 correctness + eval evidence, (C) reuse `relation-coverage.ts` and `stage4-filter.ts` infrastructure.

---

## Context Recovery

The pt-04 audit (`research/027-xce-research-pt-04/research.md:L28-L29`) gave a REVISE_SCOPE verdict for what was then numbered "Phase 009 — Feedback Reducers" (now `008-feedback-reducers`). The specific finding:
...
However, an important discrepancy emerged during this research: the current `008-feedback-reducers/spec.md` (`008-feedback-reducers/spec.md:L37-L38`) already maps its hard dependency to `002-feedback-p0-correctness`, NOT to a child sub-phase named "P0 correctness fixes." The spec's metadata reads:

> `Hard Dependency: system-spec-kit/027-xce-research-based-refinement/002-feedback-p0-correctness`
> — `008-feedback-reducers/spec.md:L37`

This means the P0 correctness split prescribed by pt-04 has already been resolved architecturally: the correctness fixes live in sibling packet `002-feedback-p0-correctness`, not as a child of 008.
...
The current `008-feedback-reducers/spec.md:L55-L63` phase map shows five children:

| Child | Title | Depends-on | LOC est |
|-------|-------|-----------|---------|
| `001-aggregator` | Shared feedback aggregation | `002-feedback-p0-correctness` (hard) | ~70 |
...
Source: `008-feedback-reducers/spec.md:L57-L63`

The `001-aggregator` child reads `feedback_events` from the ledger (`008-feedback-reducers/001-aggregator/spec.md:L50`) and depends directly on `002-feedback-p0-correctness` being landed first. This satisfies pt-04's requirement that P0 correctness fixes not wait on eval — they are owned by a separate sibling packet that is a hard dependency for the aggregator.
...
**Implication:** The 008-feedback-reducers decomposition should be anchored to `feedback-ledger.ts` (the current source of `feedback_events`) rather than to any JSONL file path. The `001-aggregator/spec.md:L50` already correctly references `feedback_events` as the source table, not a JSONL file.

### F-036-005 — eval gating is correctly handled by soft dependency on 006

The `008-feedback-reducers/spec.md:L38` lists `028/004-code-graph-adoption-eval, 028/006-coco-intent-steering` as soft dependencies. The pt-04 audit noted that the soft dependency on 006 (eval) is reasonable, but P0 fixes must not wait on it. Since P0 fixes are now in sibling packet `002-feedback-p0-correctness` (hard dep), and the learning reducers softly depend on eval, the gating structure is correct.

Source: `008-feedback-reducers/spec.md:L38`
...
**No new sub-phase is needed.** The pt-04 split is already implemented via the sibling `002-feedback-p0-correctness` dependency. The five existing children are the correct decomposition.

| Sub-phase | Title | Depends-on | LOC estimate | P-priority |
|-----------|-------|-----------|-------------|-----------|
| `001-aggregator` | Shared feedback-aggregation.ts | `002-feedback-p0-correctness` (hard) | ~70 | P0 |
...
Source evidence for table: `008-feedback-reducers/spec.md:L55-L76`, `relation-coverage.ts:L36-L45`, `stage4-filter.ts:L64-L80`, `batch-learning.ts:L34-L51`

---

## Summary

The `008-feedback-reducers` phase parent already implements the pt-04 REVISE_SCOPE recommendation correctly: P0 correctness fixes are in sibling packet `002-feedback-p0-correctness` (hard dependency), and all five learning reducer children are gated on that landing. The `ccc-feedback.ts` JSONL handler referenced in pt-04 has been superseded by the SQLite-backed `feedback-ledger.ts`. Two targeted amendments to `003-causal-reducer` and `004-retention-reducer` specs are recommended to make `relation-coverage.ts` and `stage4-filter.ts` reuse explicit.
