...
iteration: 035
rq: RQ-N6
phase_target: 007-memory-semantic-triggers
newInfoRatio: 0.62
verdict: ADAPT
---
...
- `007-memory-semantic-triggers/spec.md:43` (executive summary): "semantic-only hits activate at reduced attention (`min(0.85, semanticScore)`) so they cannot masquerade as exact triggers."
...
- `007-memory-semantic-triggers/spec.md:42`: "Default-off behind `SPECKIT_SEMANTIC_TRIGGERS=false` + sub-flag `SPECKIT_SEMANTIC_TRIGGERS_MODE=shadow|union` (shadow first)."
...
1. **Parent scope memories** — memories in ancestor folders (e.g., if `M` is in `system-spec-kit/027-xce-research-based-refinement/007-memory-semantic-triggers`, then `system-spec-kit/027-xce-research-based-refinement` is a relevant parent scope).
2. **Sibling scope memories** — memories in sibling phase folders (e.g., `006-statediff-reconciliation-layer`) that the model should be aware of when working in this packet.
