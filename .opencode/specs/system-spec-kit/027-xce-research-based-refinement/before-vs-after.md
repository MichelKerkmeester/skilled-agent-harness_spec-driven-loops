...
The idempotency receipt path requires the embedding coverage to be reliable for near-duplicate checks to be useful. The feature flag holds it back until embedding recall is confirmed healthy across the index. Near-duplicate detection is advisory rather than blocking because false positives on genuinely distinct records would suppress valid saves. The flag-on correctness work in `002-memory-store-and-search/014` was a prerequisite to any enablement, not the enablement itself: the flag stays off pending a dist rebuild and a deliberate enablement decision that still has to settle force-retry-conflict handling and receipt TTL.
...
The vector read path now detects a shard that fails its integrity and dimension checks, quarantines it rather than serving it, seeds degraded-vector state for observability, and schedules an auto-rebuild with hardened dimension discovery (`002-memory-store-and-search/008`). That repair is durable across a restart (`002-memory-store-and-search/011`): a repair-pending sentinel is persisted at quarantine, and boot uses a completeness check, the vector rowcount against the index success count rather than a file-exists probe, to decide between resuming a real repair and clearing a stale sentinel on an already-rebuilt shard. The quarantine rename is itself a durable marker, so repair resumes even when no sentinel write could land, and an in-flight guard stops a double boot-attach from scheduling the same rebuild twice.

The lexical fallback was rebuilt as a packed in-memory BM25 engine with typed-array postings and restored BM25F field weighting (`002-memory-store-and-search/009`), so title and trigger matches outrank body noise, with packed ranking meeting or beating the legacy engine on the baseline metrics (the restored weighting intentionally re-orders results where the legacy engine had lost the title signal). The realistic-corpus re-validation exposed a ~687MB warmup RSS spike (686.8MB committed gate) against a 150MB budget, which `002-memory-store-and-search/010` cut to a 136.5MB peak-sampled spike, under budget, through no-copy chunked packed postings and Uint8 to Uint16 to Uint32 width promotion, with the packed engine's ranking unchanged by the optimization (warmed-versus-direct identical) and the hard RSS gate re-enabled. The scope-then-limit bug was fixed in `002-memory-store-and-search/012`: the lane now over-fetches the candidate set when a scope or database filter is present, resolves the per-candidate metadata in batches that stay under the SQLite parameter limit, applies the filters, and only then truncates to the caller's limit.
...
### 13. RETRIEVAL CALIBRATION & OUTPUT PARITY — THE GATE THAT READ THE WRONG SCALE

§12 closes on a confident note: the resilience work is live, the smarter-ranking work is staged. Then a real user ran two ordinary searches — "Semantic Search" and "agent improvement" — and both came back `weak` with `do_not_cite` and a single visible result, even though the specs they were looking for plainly exist. Chasing that surfaced a calibration bug sitting underneath everything else, and the fix became its own arc: phase `015` (the gate), phase `016` (a ten-iteration deep-research pass), and phase `017` (seven implementation phases). This sub-section is that arc.

**Before**

The request-quality verdict and the per-result confidence read the wrong number. Search fuses several channels with reciprocal-rank fusion, whose scores land around 0.03 for a strong match. The gate compared those fused scores against cosine-scale thresholds — 0.7 for high, 0.4 for low — so `good` was structurally unreachable and almost every genuine hit was labeled `weak`, `do_not_cite`. On top of that miscalibration, the corpus the user wanted was unreachable for two separate reasons. Cold and deprecated rows, about a quarter of the index, were hard-excluded from every channel, and the vector lane joined a projection that held one active row per logical key, so a cold orphan with no active winner was invisible to semantic search. The render path made every search look empty anyway: a token-budget trim broke on the first oversized result and collapsed a five-result set to one. And the `/memory:search` command contract had its own failures — weak models dropped the query and asked the startup question instead of searching, and different models rendered the same result with different field names and scales (`confidence 0.36` on one, `similarity 0.68` on another), so the output was not comparable across surfaces.

**After**

The gate now reads absolute relevance. A new resolver prefers the cosine similarity over the fused score for the confidence and request-quality calibration, with an effective-score fallback, while leaving ranking order untouched — fusion still ranks, the calibration only changes how the gate reads relevance. On the live daemon this was confirmed end to end: a 0.89 cosine match reads `requestQuality good`, `cite_results`, confidence 0.81, where before it would have read weak. Cold and deprecated tiers are admitted into the lexical and trigger channels, and the FSRS scheduler is left to rank them by temperature; a projection backfill admits a cold orphan into the vector lane only when its logical key has no active winner, so archived content becomes reachable without breaking the one-active-per-key invariant. All three behaviors ship default-on. That is phase `015`.

Phase `016` then ran a ten-iteration deep-research pass (Opus 4.8 through a second account, five iterations on retrieval and five on output parity) to find what `015` did not cover, grounding every finding in the live daemon and the search code. Phase `017` implemented the result across seven phases: token-budget truncation now skips rather than breaks, floors a minimum of three results, and pages the remainder so a found set is never hidden; a strong top hit earns `good` on its own through a top-dominant and margin-aware verdict; short generic queries escalate from the smallest path for better recall; per-result confidence lets relevance dominate, with an opt-in isotonic calibration shipped as machinery plus an unvalidated proxy seed only; the rendered head is reordered by absolute cosine relevance behind a default-on, fully reversible flag; the command contract resolves the query deterministically in a shell header so a populated query never reaches the ask path; and the output contract mandates one score, one scale and one name — `similarity`, 0 to 1, two decimals — on every surface, with `confidence` and percentages banned from rendered output. The fifth research problem, tuning how FSRS ranks cold tiers, is a deliberate no-change: `015` already admits those rows and FSRS already ranks them, so tuning would only fight the scheduler. A cross-encoder reranker was excluded outright at the operator's direction.

**Impact**

A genuinely relevant search now reads as citable instead of being told not to trust it. The specs that prompted this — the archived hybrid-RAG work and the rest — are reachable in the ranked path rather than hidden behind a tier filter. A found set renders in full rather than collapsing to one row. Weak models execute the search instead of asking what you wanted, and the same result reads the same way whether it came from a command, a direct prompt or a conversation, so two surfaces produce comparable, diffable output.

**Why mostly always-on, some staged, full payoff pending adoption**

The calibration fix, the cold-tier inclusion, the vector backfill, the truncation safety, the verdict aggregation, the generic-query routing, the relevance rebalance, the cosine reorder and the two command-contract fixes are all corrections to behavior that was measurably wrong, so they ship on by the same logic as the resilience half of §12. The one genuinely new results-affecting lever, isotonic confidence calibration, stays opt-in and default-off until a real labeled set from live traffic replaces the proxy seed, because a model fit on proxy labels has no business reordering production results. Two caveats keep this honest. The code is committed and the dist is built, but the default-on flags take effect only on the next daemon recycle, so the win is staged until that transparent recycle runs. And the generic-query recall lift cannot be measured until the deferred corpus reindex restores the un-embedded rows, which waits until the machine is free. The unit and contract layers are proven; the live end-to-end win on the two original queries is the claim still to be confirmed.
...
The advisor-reconnect phase (`003-advisor-and-codegraph/003`) brought the advisor launcher up to parity. A second session now bridges the live daemon through the session proxy, and a dead-socket respawn decision is acted on: the launcher reaps the dead owner and respawns a replacement under a bootstrap lock, rather than leaving the session stranded or starting a second writer. All three daemon launchers now share the same resilience bar.
...
**Causal traversal BFS (`003-advisor-and-codegraph/001`).** Two recursive-CTE graph traversals whose join shape defeated index use were replaced by a single shared app-level BFS helper. Same traversal results, including multi-root fan-in, fewer full scans.
...
**Storage adapter ports (`004-shared-infrastructure/003`).** A five-port adapter seam (vector, lexical, traversal, maintenance, contention) now sits over the better-sqlite3 layer. A coupling-grep inventory trends residual direct access down toward the ports, with the hybrid lexical path, BM25 side-index maintenance, and vector-shard lifecycle pragmas kept as documented exceptions rather than forced through the ports. Testability today, room to move the persistence layer later, no caller-visible behavior change.

### 5. COMMAND PRESENTATION & WORKFLOW SEPARATION

**Command presentation and workflow separation (`004-shared-infrastructure/002`).** The memory, speckit, create and doctor command families had their workflow routing split from their Markdown presentation contracts. A structural refactor with behavior and rendered output unchanged.

Separately, the daemon-CLI front doors gained a UX hardening pass (`004-shared-infrastructure/004`): a freshness-gate fix and offline smoke, per-command help with aliases and clear errors, a unified daemon CLI reference, a hardened fallback envelope and bridge allowlist, and compact output with shell completion. The CLI surface stays at full parity with the daemons (spec-memory 37, code-index 8, skill-advisor 9).

### 6. AUTONOMOUS DEPENDENCY PATCHING

**Autonomous dependency patching (`004-shared-infrastructure/005`).** A packet-local shell entrypoint scans the OpenCode skill package roots, runs `npm audit` on each, applies supported override remediation, regenerates lockfiles without executing install scripts, and re-audits. The shipped run found all five roots clean; CI integration remains optional.

### 7. IPC CLIENT CAP HARDENING

**IPC client cap hardening (`004-shared-infrastructure/007`).** The shared daemon IPC socket server capped concurrent clients at 8 and refused extras by accepting then instantly closing them — indistinguishable from a dead daemon to probes. Session fan-outs past 8 saturated the cap, so every new session's bridge probe failed with exit 75 and the plugin printed a skip banner that the TUI rendered into the input field. The cap is now 64 in both module copies and pinned in all nine daemon env blocks, and the plugin's skip diagnostic is silent by default (debug-gated, still inspectable via its status tool).

### 8. CODE MODE ORPHAN LIFECYCLE

**Code Mode orphan lifecycle (`004-shared-infrastructure/006`).** The mcp-code-mode stdio MCP server previously had no session-lifetime handling, so a hard-killed session left it alive forever at PPID 1 — sixteen such orphans had accumulated and degraded shared daemon infrastructure. The server now exits on stdin EOF, on transport close, or when a 15-second watchdog observes reparenting to PID 1, and the accumulated orphans were reaped to a zero census.

### 9. MCP CONFIG ALIGNMENT & DAEMON RE-ELECTION DEFAULT-ON

**MCP config alignment and daemon re-election default-on (`004-shared-infrastructure/008`).** Daemon re-election was on only because the runtime configs set `SPECKIT_DAEMON_REELECTION`. The launcher's own default was off, so an unconfigured launcher fell back to kill-on-disposal and every doc claimed the configs were the on-switch. The launcher default is now genuinely on, off only on an explicit `0` or `off`, the reelection unit suite asserts the inverted contract, and the living docs were corrected while the historical changelog stayed intact. In the same pass the four MCP runtime configs were sorted, cleaned and aligned 1:1: an invalid-JSON missing comma that made two of them unparseable was fixed, the drift-prone `_NOTE_*` trivia (per-server token-budget estimates and full tool-list dumps) were trimmed while the operational notes were kept, the redundant explicit reelection entries were dropped, the legacy `SPECKIT_SKILL_ADVISOR_HOOK_DISABLED` name was renamed to `MK_SKILL_ADVISOR_HOOK_DISABLED`, and every server's env block was alphabetised byte-identical across files. The note keys were later de-numbered and re-sorted into a logical per-server reading order, with the four configs kept aligned 1:1. Embeddings were left as-is because Ollama is already preferred through `EMBEDDINGS_PROVIDER=auto` and `HF_EMBED_SERVER_URL` is only the shared hf-local fallback socket, not a HuggingFace forcing switch.

### 10. CODE GRAPH INDEXES CODE ONLY

**Code graph indexes code only (`004-shared-infrastructure/009`).** The code graph's default include globs pulled in markdown, so prose docs (the framework's own `SKILL.md`, agent, command and spec markdown, and external users' READMEs) were walked and stored as `language='doc'` nodes that no consumer queries. `**/*.md` was removed from the default include globs, so the graph now indexes code and structured config (json, yaml and toml, which still register as `language='doc'`) and skips prose. Maintainer-only indexing of the framework's own skills and plugins became selectable: a pure resolver maps `SPECKIT_CODE_GRAPH_MAINTAINER_MODE` to all five `.opencode` categories, a comma subset such as `skills,plugins`, or none, and the gitignored `.env.local` carries the maintainer's `skills,plugins` choice so external users index only their own code. A combined five-iteration deep review of this change and the config-alignment phase (`openai/gpt-5.5-fast --variant xhigh`) returned a CONDITIONAL verdict, and all eight findings it surfaced were verified real and remediated, the most important being that the incremental reindex path now enforces the same file-type allowlist as the full walk so a changed markdown file cannot re-enter the graph through a stale-file scan.
...
The residual-design-units phase (`005-verification-and-remediation/004`) resolved the residual design units that the deep-research-remediation phase deferred because each touched a high-blast-radius subsystem. Three items shipped as code. The `memory_health` report was exceeding its 1000-token advisory budget: re-tiered to 1500 tokens per tool and the near-static `exclusionAudit.entries` block gated behind the existing `includeFullReport` flag, with `data.routing` byte-for-byte unchanged. The ingest job-queue lifecycle was generalized into a kind-agnostic `maintenance_jobs` store so `memory_index_scan` can run as an opt-in background task via a new `background` flag, with two new tools (`memory_index_scan_status` and `memory_index_scan_cancel`) expanding the spec-memory surface from 37 to 39. The shadow-evaluation cycle was skipping every run because the clean consumption log keeps no raw query text. It now builds a privacy-preserving synthetic replay corpus from non-reversible signals (intent enum, coarse result-count bucket, counted-never-decoded query hash) where every synthetic query is drawn verbatim from a static seed vocabulary, guarded by a fail-closed privacy check and a SENTINEL test. Three items were disposed without code. The launcher parity port for `mk-code-index` was documented as a staged follow-on rather than mutated: true adoption requires a daemon backend-only mode plus idle-monitor grace and owner-proxy wiring, a multi-stage launcher-critical change that is inert until a fresh session. A proposed spec-to-file crosswalk (tri-163) was refuted because the named `COVERED_BY` edge is a deep-loop context-sweep relation between investigation slices rather than a spec-to-file edge, making a literal crosswalk a category error. And a write-path stress harness (tri-129) was deferred because six existing stress and durability suites already cover the constituent risks in aggregate.

The fresh-regression-remediation phase (`005-verification-and-remediation/005`) closed the loop by reviewing the whole epic again and then fixing everything the review raised. A 75-seat read-only deep-review ran across three models (Opus 4.8 via the claude2 binary, GPT-5.5-fast and Kimi K2.7 via cli-opencode) over 15 fresh angles so no single loop could converge early. Deduplication produced 113 unique findings with zero P0. Round-2 adversarial verification re-checked every code-defect P1 candidate with the opposite model and reduced 16 candidates to 5 confirmed, with the rest of the drift confirmed as documentation and control-metadata. Per directive, all 113 findings were then carried into a seven-sub-phase remediation rather than triaged out: the refuted ones as hardening and the asserted ones fixed as stated. The five confirmed code defects were fixed and mutation-verified: the causal-edges generation now bumps before cache invalidation on a memory delete so search stops serving stale causal-boosted results, the save mutex reaps only a provably dead owner instead of a live one, the legacy history rebuild runs inside a transaction, version-append paths carry a human row's manual provenance onto its successor instead of relabelling it agent, and the bootstrap lock reclaims the instant its owner dies instead of waiting out a five-minute TTL that had blocked dead-socket respawn. The launcher, code-graph, CLI front-door and consolidation lanes fixed their own confirmed and P2 items, each behind regression tests in an isolated harness with no live daemon recycled. The remaining drift was reconciled as documentation and metadata: omitted children restored to description.json, stale phase pointers advanced, the tool count corrected from 37 to 39, dead asset links repaired and runtime mirror paths aligned. The pattern the review named is the same one the prior epic-sweep flagged, shipped behavior largely holds while documentation and control-metadata drift ahead of code.

The deep-review-017-021-remediation phase (`005-verification-and-remediation/006`) applied the same loop to the search-and-output-intelligence work that shipped after the prior sweep — phases 017 through 021, covering search/output intelligence, reindex cancellation, maintenance-grace re-election, background embedding and the cooperative heavy phases. A 50-pass review across four models (deepseek-v4-pro, mimo-v2.5-pro, kimi-k2.7 and opus-4.8) returned five per-folder syntheses, four PASS and one CONDITIONAL (017), with zero confirmed P0, one confirmed P1, and the rest P2, doc-drift or test-debt. The one P1 was an unquoted `-- $ARGUMENTS` in the `/memory:search` argument-resolution header; the renderer was confirmed by binary inspection and a live probe to splice the string raw rather than shell-quote it, so the substitution is now single-quoted and a query carrying shell metacharacters resolves verbatim. A renderer-level argv fix for an embedded single quote remains the documented residual, bounded to self-injection. The five confirmed code defects were fixed and unit-gated: the 017 calibration cluster gained a weight-sum assertion, PAV equal-mean pooling with a drift-guard against the offline fit, and calibration-cache mtime invalidation; the 021 empty-files scan branch now wraps its tail phases in `timedPhase` so instrumentation and marker refresh stay symmetric across both paths, with the near-duplicate count captured; the 018 `cancelledJobIds` Set is cleared on every terminal state and a cancelled file is counted distinctly rather than as a failure; and the 021 cancelled-run pending counts are recomputed before the cancel returns. The 017/019/020 doc drift was reconciled to shipped reality, including the `.opencode/bin` path corrections, the `jobId` to `labels[]` marker schema, the 60s to 180s TTL, and 017's seven children advancing from scaffold to a completed state. The remediation shipped in two commits (`4faff94927` code and tests, `4c14606d66` packet and doc reconciliation): full typecheck clean, the unit suites moved from a 30/30 baseline to 58/58 (+28 tests, zero regressions), five stress suites of 28 tests passed, and `validate.sh --strict` was green on the packet and on the reconciled 017, 019 and 020 folders. A follow-on 30-pass gpt-5.5-fast broaden review then swept search/retrieval, store/index and server infrastructure, and eight refute-first verifiers triaged its 52 findings to nine fixes (`55b977951d`). The headline was a default-on cross-tenant scope leak: `retrieval-rescue`'s Stage-2 lexical-backfill and sibling injection re-queried the index without re-applying the governance and `spec_folder` scope filter the constitutional and community paths already run, so a scoped search could pull rows from outside its scope — now re-filtered on every injected row and pinned by a P0 scope-isolation regression test. The rest hardened the same surface: community-fallback member scoping, summary-lane scope-in-SQL with an active/expiry gate, a `folderBoost` cache-key fix and clamp-ceiling correction, `retrievalLevel` and `includeArchived` schema honesty, and store/infra hygiene across `memory_save` allowed-root probing, `hf-model-server` socket ownership, a single-transaction index stale-cleanup and the `ENV_REFERENCE` unix-only transport claim. Typecheck stayed clean with 448 affected tests plus the new scope-isolation test and zero new failures against baseline. This session then rebuilt the dist and recycled the live daemon onto it, so the 006 fixes now serve rather than sit inert, and baked a durable wedged-daemon CLI-fallback rule across the eight daemon-using deep-loop and review agents (`61b5aab102`) so an autonomous agent never stalls on a flapping MCP transport.
...
The everyday call shape of memory retrieval, storage and session startup is unchanged for existing users, and enabling any of the flags is an explicit opt-in with documented environment variables in `ENV_REFERENCE.md`. The read path did get more resilient by default: a malformed vector shard is detected and quarantined rather than served, the lexical fallback ranks with restored field weights under its memory budget, and a scoped search resolves its filters before truncating. None of that needs a flag and none of it changes the response contract for the common path. The vector repair durability and the idempotency flag-on correctness are inert until a dist rebuild adopts them. The fresh-regression code fixes across the write path, launchers, code graph, CLI front-doors and consolidation cycle are likewise validated in tests and inert until a dist rebuild and daemon recycle adopt them. As of 2026-06-18 a dist rebuild and live daemon recycle ran, so the committed code fixes compiled into that build — including the 006 broaden-round search-scope fixes — are now serving on the live daemon; flag-gated behaviors remain opt-in regardless.

On the shared-daemon side, daemon re-election is now on by default in the launcher code rather than only because the runtime configs set it, the four MCP runtime configs are aligned 1:1 and trimmed of drift-prone note trivia, and the code graph indexes code only with markdown dropped from its default scan and maintainer-only skill and plugin indexing selectable through one env knob.

The changelog index at `changelog/README.md` and the chronological view at `timeline.md` now cover every shipped phase across the six themed tracks: release-cleanup, research-and-doctrine, memory-store-and-search, advisor-and-codegraph, shared-infrastructure and verification-and-remediation. The manual testing playbook covers the CLI stress scenarios and the new diagnostic surfaces.
