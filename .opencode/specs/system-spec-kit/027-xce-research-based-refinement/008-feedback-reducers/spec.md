---
title: "Phase 008 — Shared Feedback Reducers (3 P0 Fixes + 1 Aggregation + 3 Consumers)"
...
_memory:
  continuity:
    ...
    recent_action: "Scaffolded 027/008 from pt-03 RQ-A3+B3+B4"
...
Phase 008 implements all three as a single L3 packet with one shared aggregation layer feeding three independent decision consumers, plus 3 P0 precondition fixes that MUST land in Sub-Phase 1 BEFORE any reducer ships.
...
**Universal pattern (per pt-03):** all 3 reducers default-off, shadow-first, fail-closed; live mutation requires Phase-005 eval lift over rule-based baseline.
...
| **Depends on** | none hard; soft on `005-code-graph-adoption-eval` (Phase-005 eval gate for live mutation) |
...
- Re-embedding triggered by feedback (preserves Phase 007's no-re-embed contract).
- Active TTL mutation without shadow eval window (ADR-006 binding).
- Cross-system shared cache for feedback events (Phase 009 territory).
- Coco exemplar surfacing from positive feedback (Phase 010 RQ-A4 territory).
<!-- /ANCHOR:scope -->
...
- Phase 005 eval (when shipped) measures retention quality (prevented-then-cited deletes; extended-then-unused records).
<!-- /ANCHOR:success -->
