---
title: "Feature Specification: Memory Commands Presentation Workflow Separation (Phase Parent)"
description: "Family phase parent for extracting presentation contracts from memory commands command routers into dedicated Markdown files."
trigger_phrases:
  - "memory command presentation"
  - "memory search startup questions"
...
_memory:
  continuity:
    packet_pointer: "system-spec-kit/027-xce-research-based-refinement/004-shared-infrastructure/002-command-presentation-workflow-separation/001-memory-commands"
...
# Feature Specification: Memory Commands Presentation Workflow Separation (Phase Parent)
...
### Problem Statement
The memory commands command family owns presentation instructions inline with routing instructions. That makes the display contract harder for assistants to find and follow, especially when command files are long or route through workflow assets.

### Purpose
Coordinate extraction for memory commands commands so each command.md can become a thin router to its workflow asset plus a Markdown presentation contract for startup questions, dashboards, and results display.
...
### In Scope
- Owned command scope: `.opencode/commands/memory/*.md` and the workflow asset YAML files those commands route to.
...
- This family carries the motivating UX work: make `/memory:search` smarter and more open-ended at startup, reduce menu-heavy prompts, and render cleaner results so GPT-via-opencode stops returning bad or cluttered information.
...
|-----------|-------------|-------|-------------|
| `.opencode/commands/memory/*.md` | Future Review/Modify | 001-003 | Inventory inline presentation and then thin-router rewiring |
| `.opencode/commands/memory/assets/**/*.yaml` | Future Review/Modify | 001-003 | Inventory inline presentation and then thin-router rewiring |
| `.opencode/commands/memory/assets/**/*.yml` | Future Review/Modify | 001-003 | Inventory inline presentation and then thin-router rewiring |
