...
## YOUR ANGLE (#5)
Startup routing friction: evaluate each command's ROUTING ASSETS / STARTUP ROUTING sections for whether a dispatched model is explicitly told to READ the presentation asset before responding, and propose a uniform mandatory-read convention.

## OUTPUT
ONLY a fenced ```json block:
{"angle": 5, "findings": [{"class": "DOC-DRIFT|REFINEMENT|NEW-FEATURE|BUG", "severity": "P1|P2|P3", "title": "<short>", "evidence": "<file:line or concrete comparison>", "detail": "<2-3 sentences>", "fix_sketch": "<one sentence>"}], "summary": "<2 sentences>"}