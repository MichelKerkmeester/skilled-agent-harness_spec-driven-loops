...
## YOUR ANGLE (#10)
Unified adherence patch set: synthesize angles 1-9 into a concrete minimal diff plan per command family (which files change, what moves inline, what phrasing changes), ordered by expected adherence gain per token added.

## OUTPUT
ONLY a fenced ```json block:
{"angle": 10, "findings": [{"class": "DOC-DRIFT|REFINEMENT|NEW-FEATURE|BUG", "severity": "P1|P2|P3", "title": "<short>", "evidence": "<file:line or concrete comparison>", "detail": "<2-3 sentences>", "fix_sketch": "<one sentence>"}], "summary": "<2 sentences>"}