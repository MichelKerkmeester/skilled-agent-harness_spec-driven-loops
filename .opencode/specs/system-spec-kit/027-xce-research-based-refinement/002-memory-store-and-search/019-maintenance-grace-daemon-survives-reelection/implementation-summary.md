...
- The marker is scoped to the scan job, so it protects the scan but not the POST-scan work. Because the scan defers embeddings (`asyncEmbedding`), the actual vector writes run in the background-embedding queue after the scan completes, and that queue is busy-but-unprotected. A live run confirmed this: the scan completed and the daemon survived, then a separate re-election fired during the post-scan embedding burst and recycled the daemon (the index stayed functional and the deferred-embedding gap drains on the next queue pass, but the reindex's data goal is not yet re-election-proof end to end). Extending the marker to cover the background-embedding queue, or making that queue cooperative, is the follow-on that closes the loop.
<!-- /ANCHOR:limitations -->
