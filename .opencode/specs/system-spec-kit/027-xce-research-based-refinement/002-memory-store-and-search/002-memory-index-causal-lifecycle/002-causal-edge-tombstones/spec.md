...
_memory:
  continuity:
    packet_pointer: "027-xce-research-based-refinement/002-memory-store-and-search/002-memory-index-causal-lifecycle/002-causal-edge-tombstones"
...
Make soft-delete FIRST-TIMESTAMP-IDEMPOTENT: COALESCE-preserve the first tombstone timestamp so repeat deletes do not extend the retention window. Split active vs purgeable rows into partial indexes. Keep entity/co-occurrence signals as recall evidence only, never as causal truth. Source: research/008-caura-memclaw-fleet-memory-teachings/sub-packet-proposals.md; planned in 010/004.
