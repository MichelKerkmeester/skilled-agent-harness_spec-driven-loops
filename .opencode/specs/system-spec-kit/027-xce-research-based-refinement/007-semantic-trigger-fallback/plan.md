...
│      └─ if miss: skip semantic stage; never embed in hot path  │
│                                                                │
...
- `memory_index_scan` triggers resumable per-memory backfill of `memory_trigger_embeddings`.
...
- Interrupted backfill resumes without duplicate rows and without treating partial rows as ready.
...
- Promotion checklist refuses `union` unless shadow data proves false-positive, recall, latency, cost, and rollback targets.
<!-- /ANCHOR:phases -->
...
- [ ] Backfill completed or failed rows are explicitly reported; no partial rows are treated as ready.
- [ ] Shadow false-positive rate is at or below target in trigger goldens and representative production samples.
- [ ] Paraphrase recall lift is meaningful without lowering exact lexical precision.
- [ ] p95 latency remains inside the WARN budget with semantic lookup enabled.
- [ ] Runtime embed call count is zero for trigger matching.
- [ ] Rollback to lexical-only has been exercised from the same build.
<!-- /ANCHOR:rollback -->
