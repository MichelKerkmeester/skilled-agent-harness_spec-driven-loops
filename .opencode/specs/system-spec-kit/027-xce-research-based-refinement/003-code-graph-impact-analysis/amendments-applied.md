# Phase 003 amendments-applied — pt-02 cross-validation cycle

**Source**: `pt-02/sub-packet-amendments.md` §"Phase 003 amendments"
**Applied**: 2026-05-08
...
| **B-iter006-005** | Phase 003 references "layer-based criticality" from Phase 001 but Phase 001's classifier is undefined contract; would force redundant local classifier | NEW REQ-015 (layer fallback to `unavailable/null`, no second classifier) | iteration-006.md |
...
| **B-iter007-004** | LLM enrichment contract gap; cli-opencode subprocess inherits stdin-deadlock and lifecycle failure modes from pre-097 era | (Same as B-iter003-007 — REQ-014 + Phase 005 dispatcher reuse) | iteration-007.md |
...
| REQ-003 | **edited** | Was: `Risk score formula uses tunable weight constants`. Now: `…uses documented tunable weights and deterministic normalizers; default weights are labeled heuristic until Phase 005 calibration`. Closes B-iter003-002. |
...
| **REQ-015** | **NEW P1** | Layer fallback: emit `unavailable/null` if Phase 001 layer data missing; no second local classifier. Closes B-iter006-005. |
...
- T-003G: Layer fallback `unavailable/null` for Phase 001 absence
...
- Heuristic weights without calibration label can mislead consumers (mitigation: `weight_class: "heuristic"` + Phase 005 calibration).
...
- CLI enrichment inherits subprocess lifecycle failure modes (mitigation: Phase 005 dispatcher reuse).
...
- Calibrated risk weights (deferred to Phase 005 eval harness; weights labeled `heuristic` until then).
- LlmNarrativeProvider non-CLI implementations (HTTP/native API) — interface defined, only `none` and `cli` providers wired.

---

## Phase 003 readiness after amendments

- ✅ Implementable: yes — every BLOCKING finding has a remediation REQ.
- ⚠️ Conditional: REQ-015 depends on Phase 001's REQ-015 (`classifyFileRole()` exported); REQ-014 CLI provider depends on Phase 005's subprocess dispatcher contract.
