---
title: "Phase 009 — Retrieval Rerank Clients (Shared RerankClient + EmbeddingCacheClient Interfaces)"
description: "ADAPT pt-03 RQ-B5 client extraction: extract shared `RerankClient<T>` from memory's cross-encoder.ts so memory backend + coco-index share provider/cache/circuit-breaker semantics. Define `EmbeddingCacheClient` interface (memory adapter only — Coco adapter deferred). Cross-backend hit-rate overlap telemetry. SKIP shared indexing pipelines (binding boundary). ~250-420 prod LOC + ~120-220 tests."
trigger_phrases:
  - "027 phase 009"
...
_memory:
  continuity:
    ...
    recent_action: "Scaffolded 027/009 from pt-03 RQ-B5"
...
| **Depends on** | none (hard); existing `mcp_server/lib/search/cross-encoder.ts` |
