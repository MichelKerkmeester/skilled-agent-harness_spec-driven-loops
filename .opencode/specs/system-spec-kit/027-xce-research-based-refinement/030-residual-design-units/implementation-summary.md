...
1. **Unit B hash-bucket index is degenerate (flagged).** The SQLite `CAST('0x'||hex AS INTEGER)` expression resolves to 0, so seed selection is the first pool entry per intent. Privacy and class-collapse are unaffected; broadening per-class seed diversity is a future enhancement (it also requires adjusting the index-coupled privacy-test assertion).
