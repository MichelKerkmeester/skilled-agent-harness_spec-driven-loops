...
_memory:
  continuity:
    packet_pointer: ".opencode/specs/system-spec-kit/027-xce-research-based-refinement/006-coco-intent-steering"
...
### Adjacent precedents
- Phase-008 (RQ-A3) plans an active feedback rerank loop using `ccc_feedback` JSONL — this phase's `intent` field will become the cache key for Phase-008's reweight table (soft dep).
- Phase 004 plans a "MUST invoke FIRST" advisor mandate — REQ-009 hint integrates with that renderer surface.
- Phase 005 plans an eval harness — provides the gate for default-on promotion.
<!-- /ANCHOR:technical-context -->
...
Sub-Phase 3 is decoupled — runs in TypeScript advisor renderer. Connects via Phase-004 mandate brief OR feature-flag standalone fallback.
<!-- /ANCHOR:architecture -->
...
- Standalone fallback works without Phase 004 integration.
...
### Soft preconditions
- **Phase 004 (skill-advisor first-action mandate)** — REQ-009 advisor hint integrates with 004's mandate brief. Standalone fallback works without 004.
...
- **Phase 008** (feedback reducers) — reweight table will key on `(intent_tag, path_class)`; intent_tag comes from this phase's classifier.
- **Phase 010** (coco-memory context extras) — example bank may query intent for similar-prior-query lookup.
...
|----|------|----------|------------|------------|--------------|
| R-006-01 | Precision loss from over-expansion | P1 | Med | REQ-004 exact-intent suppression + REQ-002 3-embedding cap + Phase-005 eval before default-on | Phase-005 paired comparison vs. baseline |
...
| R-006-03 | Advisor hint coupling to Phase 004 | P2 | Med | Standalone fallback flag (REQ-009) | Test: render-coco-hint.vitest.ts standalone path |
...
| Advisor hint renders correctly (Phase-004 + standalone) | Both paths green | `render-coco-hint.vitest.ts` |
| Phase-005 eval lift (post-implementation) | Recall improvement on labeled task set | Phase-005 harness paired comparison |
<!-- /ANCHOR:success-metrics -->
...
If active expansion produces precision regressions in Phase-005 eval:
...
3. Re-run Phase-005 eval → re-promote to default-on only after lift confirmed.
