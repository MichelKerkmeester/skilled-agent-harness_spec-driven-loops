---
title: "Phase 006 — Coco-Index Intent Steering + Bounded Query Expansion"
description: "ADAPT XCE's intent-steering pattern into mcp-coco-index via (a) rule-based intent classifier shim before query embedding, (b) bounded query expansion (≤3 embeddings, exact-intent suppression), and (c) Phase-004 advisor first-action hint. Cross-language change (Python coco-index + TypeScript advisor renderer); feature-flagged default-off; Phase-005 eval gate before active rollout. ~250-350 LOC across intent classifier, query expander, advisor hint, telemetry."
trigger_phrases:
  - "027 phase 006"
...
_memory:
  continuity:
    ...
    recent_action: "Scaffolded 027/006 from pt-03 RQ-A1"
    ...
    session_dedup:
      ...
      session_id: "claude-opus-4-7-2026-05-09-027-006-scaffold"
    ...
    open_questions:
      ...
      - "Should expansion telemetry write to ccc_feedback JSONL (Phase-008 dependency) or stay in rankingSignals only?"
    answered_questions: []
---
...
1. **Intent-steering** — XCE's "use first" steering directive (`../external/README.md:101-119`, `../external/steering/CLAUDE.md:5-10`) maps to a Phase-004 advisor first-action hint that surfaces `mcp-coco-index` for code-location and concept-search intents (queries like "find code that does X", "where is the logic for", "how is X implemented").
...
- **Default-off behind `SPECKIT_COCOINDEX_INTENT_EXPAND=0`**; Phase-005 evaluation required before active rollout.
- **Soft dep on Phase 004** — advisor first-action hint integrates with the renderer when 004 ships; standalone fallback works without it.
...
| **Depends on** | none (hard); `027/004-skill-advisor-first-action-mandate` (soft — advisor wording integration) |
...
- Advisor first-action hint in `mcp_server/skill_advisor/lib/render.ts` (~30-50 LOC):
  ...
  - Soft dep on Phase 004 — feature-flag standalone fallback if 004 hasn't shipped.
...
- Active feedback loop for rerank weights (RQ-A3 territory; Phase 008).
- Few-shot exemplar surfacing (RQ-A4 territory; Phase 010).
...
| REQ-009 | Advisor first-action hint via `mcp_server/skill_advisor/lib/render.ts:124-158` (soft dep on Phase 004) | When 004 shipped, hint integrates cleanly with mandate brief; when 004 NOT shipped, hint emits behind feature-flag with deterministic fallback |
...
| Phase 004 not yet shipped at advisor hint integration | Fallback path emits hint behind feature flag; deterministic; no advisor crash |
<!-- /ANCHOR:edge-cases -->
...
- Phase-005 eval harness (when shipped) measures expansion lift over current single-shot behavior on 12-20 task set.
- Advisor first-action hint integrates with Phase 004 (when 004 ships) OR works standalone behind feature flag.
<!-- /ANCHOR:success -->
...
|------|----------|------------|
| Precision loss from over-expansion | P1 | Exact-intent suppression (REQ-004) + 3-embedding cap (REQ-002) + Phase-005 evaluation before default-on |
| Embedding cost from sub-queries | P1 | 3-embedding ceiling caps fanout (REQ-002) |
| Advisor hint coupling to Phase 004 | P2 | Feature-flag standalone fallback (REQ-009) |
