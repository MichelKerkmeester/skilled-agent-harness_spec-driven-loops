...
| 14 | [ ] | T014 | Standalone fallback flag `SPECKIT_COCOINDEX_FIRST_ACTION_HINT=0` for pre-Phase-004 ship | REQ-009 | `render.ts` (modified) | Flag-gated path renders hint without 004 dependency |
| 15 | [ ] | T015 | Author advisor render tests — Phase-004-integrated path + standalone fallback | REQ-009 | `mcp_server/__tests__/skill_advisor/render-coco-hint.vitest.ts` (NEW) | Both paths green |
...
- 19 tasks total; ~10-15 hours estimated wall (excluding Phase-005 eval gate).
