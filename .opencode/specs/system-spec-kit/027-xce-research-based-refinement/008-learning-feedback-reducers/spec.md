...
> **Note (iteration-036):** The P0 correctness split prescribed by pt-04 is already resolved architecturally — correctness fixes live in sibling packet `002-memory-write-safety` (hard dependency), not as a child of this phase. The `ccc-feedback.ts` JSONL handler cited in older research has been superseded by the SQLite-backed `feedback-ledger.ts`; `001-aggregator` correctly references `feedback_events` from that table.
<!-- /ANCHOR:phase-map -->
