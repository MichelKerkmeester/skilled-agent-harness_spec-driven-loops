---
title: "Phase 010 — Coco-Index Few-Shot Exemplars + Memory LLM-Curated Context (Presentation-Layer Extras)"
description: "ADAPT pt-03 RQ-A4 (coco few-shot example bank) + RQ-B2 (memory LLM-curated context packaging). Two presentation-layer features in one packet — neither alters scoring authority. Coco exemplars in separate response group; memory data.curatedContext as post-retrieval packaging plan with strict JSON schema validation. Both default-off + shadow-first + Phase-005 lift required for active mode. ~500-800 prod LOC + ~340-570 tests."
trigger_phrases:
  - "027 phase 010"
...
_memory:
  continuity:
    ...
    recent_action: "Scaffolded 027/010 from pt-03 RQ-A4 + RQ-B2"
...
Pt-03 RQ-A4 (coco few-shot exemplars; verdict ADAPT design / DEFER default-on, see `../research/027-xce-research-pt-03/research.md` §RQ-A4 and `iteration-004.md`) and RQ-B2 (memory LLM-curated context; verdict ADAPT shadow / DEFER active, see §RQ-B2 and `iteration-007.md`) bundle here as Phase 010 because both:
- Are presentation-layer additions (NOT ranking authorities).
- Default-off + shadow-first + Phase-005 lift required for active mode.
...
- Active mode requires Phase-005 eval lift evidence over deterministic baseline.
...
| Field | Value |
|-------|-------|
| **Level** | **3** (two independent presentation features in one packet; both nondeterministic in active mode; both require Phase-005 promotion gate; new local SQLite + vec0 table with TTL/reconciliation; introduces governed presentation surface; see `decision-record.md` ADR-001) |
...
| **Depends on** | none (hard); soft on Phase 007 (better trigger candidates → better curator input), Phase 008 (Consumer A signal quality for example-bank capture), Phase 005 (eval lift gate for both features) |
...
- Same Phase-005 gate; same default-off + shadow-first; same nondeterminism risk profile.
...
- Phase-005 eval gating documented.
...
- Active mode rollout (gated on Phase-005 lift).
<!-- /ANCHOR:scope -->
...
| REQ-018 | Active mode requires Phase-005 eval lift over deterministic profiles (`profile-formatters.ts:4-21`); lift criteria: task success ≥ deterministic baseline; cited-source correctness ≥ baseline; missed-critical-context rate ≤ baseline; latency budget; nondeterministic-variance bound | Promotion gate test |
<!-- /ANCHOR:requirements -->
...
- Phase-005 eval (when shipped) measures lift over deterministic baseline for both features.
<!-- /ANCHOR:success -->
...
|------|----------|------------|
| LLM curator nondeterminism affects retrieval trust | P1 | Default-off + strict timeout + deterministic fallback + JSON schema validation + Phase-005 lift requirement |
...
L3 designation rationale is in `decision-record.md` ADR-001. Cross-component change with feature-flag governance, telemetry contract, and Phase-005 eval gate.
...
- **US-003**: As a Phase-005 evaluator, I can compare baseline (flag-off) vs treatment (flag-on) on the labeled task set with paired comparison metrics.
