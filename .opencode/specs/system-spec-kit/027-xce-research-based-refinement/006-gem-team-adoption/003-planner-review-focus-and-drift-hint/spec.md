...
| **Depends on** | `001-typed-agent-io-adapter` (these fields live inside 001's I/O envelope) |
...
| Dependency | `001-typed-agent-io-adapter` | High | The two fields live inside 001's I/O envelope; implement 003 only after 001 lands. |
<!-- /ANCHOR:risks -->
...
- `target_files` / `affected_spec_docs` naming: align the drift field name with whatever 001's envelope settles on at implementation time.
- Confirm at implementation whether `self_assessed_quality` should carry a numeric band aligned to 001's confidence convention or stay a free-form short hint (default: short hint).
<!-- /ANCHOR:questions -->
