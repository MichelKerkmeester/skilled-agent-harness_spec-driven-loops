...
_memory:
  continuity:
    ...
    answered_questions:
      - "Proposal P1 confirmed as phase 001 — substrate child of 012-gem-team-adoption"
---
...
- A shared, versioned, advisory contract doc (`agent-io-contract.md`) with grouped optional sections (dispatch / result / handoff / pre_execution / advisory) so later children 013 and 014 reuse the same schema rather than diverging.
...
| **Depends on** | None (this is the substrate children 013 and 014 build on) |
...
1. **Shared advisory contract doc**
   ...
   - Versioned (`schema_version`), advisory only, with grouped OPTIONAL sections so 013 and 014 extend the same file rather than forking:
     ...
     - `handoff`: reserved for 013 (debug to implement) — defined as a placeholder grouping, not wired here.
     - `pre_execution`: reserved for 013 (pre-mortem, boundary contract) — placeholder grouping only.
     - `advisory`: reserved for 014 (reviewer_focus, quality_score, spec_drift) — placeholder grouping only.
...
- The P2 gates - debug-handoff schema, boundary contract-first, pre-mortem live in child 013.
- The P3 advisory fields - reviewer_focus, quality_score, spec_drift write-back live in child 014.
...
| `.opencode/agents/debug.md` | Modify | Accept the dispatch/handoff header (handoff fields are reserved for 013; Wave-1 adds only the header awareness) |
...
| REQ-010 | Group contract fields by purpose to prevent top-level bloat as 013/014 extend it. | The five groupings keep dispatch/result separate from the reserved handoff/pre_execution/advisory; later children add fields within a group, not at top level. |
<!-- /ANCHOR:requirements -->
...
- **SC-006**: Children 013 and 014 can record this contract as their reusable substrate (their fields slot into the reserved groupings).
<!-- /ANCHOR:success-criteria -->
...
| Risk | Contract bloat as 013 and 014 add their fields. | Medium | Group fields by purpose (dispatch / result / handoff / pre_execution / advisory); later children add within a group, never at top level. |
...
| Dependency | None. | None | This packet is the substrate; it has no upstream dependency. Children 013 and 014 depend on it. |
<!-- /ANCHOR:risks -->
...
- **NFR-M02**: Fields are grouped by purpose so 013 and 014 extend the contract without reshaping its top level.
...
| Contract consumed by a tool expecting 013/014 groupings before they ship | Reserved groupings exist but are empty placeholders; absent fields are simply not present. |
<!-- /ANCHOR:edge-cases -->
