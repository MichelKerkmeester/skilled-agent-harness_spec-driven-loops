...
| **Rollout wave** | Wave 2 — highest-risk transition (after 012's substrate lands) |
...
1. **Gate A — Typed debug-handoff schema** (predicate: `diagnosis_crosses_agents`, i.e. a `@debug` diagnosis is being handed to a `@code` implementer)
   - `@debug` emits a typed handoff carrying `root_cause`, `target_files`, `fix_recommendations`, and `confidence` (reusing 012's envelope `confidence`/`failure_type` fields).
...
- A full Gem-style failure taxonomy — `failure_type` is reused from 012, not expanded here.
- Adding raw `spec_drift`/advisory fields to the continuity schema — that is P3 / `014`, not this packet.
...
| REQ-002 | `@debug` emits a typed debug→implement handoff. | `debug.md` documents `root_cause`/`target_files`/`fix_recommendations`/`confidence` as the handoff payload for cross-agent diagnosis, reusing 012's envelope fields. |
...
| Dependency | `001-typed-agent-io-adapter` typed envelope. | Hard | This packet reuses 012's `confidence`/`failure_type` fields; 012 must land first (Wave 1 before Wave 2). |
...
- Depends on `012` landing its typed envelope first; if `012` slips, can Gate A ship with a local `confidence` field as a temporary fallback? (Default: hold for 012 per Wave 1→2 ordering.)
<!-- /ANCHOR:questions -->
