---
title: "Plan — Phase 009 Shared Feedback Reducers"
...
- `intent_tag` derived from RQ-A1 classifier (Phase 007 dependency for intent-tagged learning); path-class-only MVP uses `intent_tag='general'` if Phase 007 not shipped.
...
- **Phase 006** (adoption eval harness) — for promotion gate measurements (live mutation in C).
- **Phase 007** (coco intent steering) — for `intent_tag` dimension in Consumer A; path-class-only MVP works without 006.
...
| Phase 006 retention quality (live mutation gate) | prevented-then-cited deletes ≤ 0.05 | Phase-006 paired comparison |
| Phase 006 retention quality (shadow vs live) | extended-then-unused ratio < 0.20 | Phase-006 retention metrics |
<!-- /ANCHOR:success-metrics -->
