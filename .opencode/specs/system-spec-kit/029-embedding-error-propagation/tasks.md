...
trigger_phrases:
  - "029 embedding error propagation tasks"
...
- [x] T015 Run `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh .opencode/specs/system-spec-kit/029-embedding-error-propagation --strict`.
- [B] T016 Stage only the scoped 029 files if git index access is available. Blocked by `.git/index.lock` EPERM.
- [B] T017 Commit `fix(029): propagate embedding provider errors (bug-A)` if git index access is available. Blocked by `.git/index.lock` EPERM.
<!-- /ANCHOR:phase-3 -->
