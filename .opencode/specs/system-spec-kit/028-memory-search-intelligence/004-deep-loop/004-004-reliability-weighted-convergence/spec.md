...
- **The resilience cluster (failure-class taxonomy, transient/fatal retry, orphan reset, recover-vs-fresh gate)** — lives in the sibling sub-phase `003-003-fanout-failure-recovery`; that cluster is independent of D2 and is keyed only on exit-code/timeout/ledger state. Not re-implemented here.
