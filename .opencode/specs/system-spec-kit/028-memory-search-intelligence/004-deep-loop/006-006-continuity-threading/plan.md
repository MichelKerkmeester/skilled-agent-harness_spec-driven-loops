...
This sub-phase implements the 028/004 **continuity GO cluster** — the continuity / iterative-retrieval pair, decoupled from both the reliability-scoring cluster (D2/D3/Q2, NO-GO until benchmarked) and the resilience cluster (`003-003-fanout-failure-recovery`). The work fixes that the iteration loop threads forward "reconstruct-cold" (no self-owned thread) and that its next focus is hand-written free-text rather than derived from what the iteration found.
