...
RESEARCH-THEN-BUILD. This is Track-C `005-spec-data-quality`. The schedule below covers the original 28-phase research scaffold, children 001 through 028, where every child carries spec, plan, tasks, checklist and implementation-summary as a build plan for a later packet rather than work in progress, so the Count column reads scaffold. The lineage then continued and shipped. Children 029 through 040 added the vague-query and generated-JSON research, the generated-metadata build phases 033 through 038, the full-repo JSON migration 039 and the flag-graduation benchmark 040 that kept twelve flags and deleted one. A round-2 deep review on 2026-06-22 returned a CONDITIONAL verdict on the research scaffold and the same session remediated every P0 and P1 plus the actionable P2. The honest scope read from that review is single-digit. The buildable-now subset is `004-schema-warn-to-error`, `026-shared-safe-fix-engine`, `001-extend-quality-loop-authored` and `003-enum-constrain-schemas`, while the Tier-C retrieval slate and the thin novel items are deferred-until-measured behind a real prod@K read. No scaffold phase was deleted, all 28 are kept by operator intent. Two hard constraints drive the order. 026 the shared safe-fix engine ships first because the on-write and retroactive front doors reuse it. 015 the C2 prod-mode benchmark gate ships before every Tier-C retrieval phase and the 027 floor experiment because no retrieval candidate promotes without a prod-mode recall read across the @3, @5 and @8 columns the harness emits plus an order-sensitive NDCG@K with a top1 guard, so a promotion needs a recall rise and a ranking-quality hold.
...
| 2 | `015-prodmode-recall-gate` | 2 | 0 done, scaffold | CONDITIONAL. Prod-mode recall gate over the @3, @5 and @8 completeRecall columns plus an order-sensitive NDCG@K with a top1 guard, unblocks the whole retrieval tier and the 027 experiment |
| 3 | `004-schema-warn-to-error` | 2 | 0 done, scaffold | GO decision-unconditional, flip-gated. Promote the JSON-schema shape rules from warn to error, validation not ranking, no re-index, leads Tier A. The error flip waits on a backfill to zero since a real schema run still fails 24 files, 16 live |
| 4 | `001-extend-quality-loop-authored` | 2 | 0 done, scaffold | GO-on-cost. Keystone on-write seam, extends the live quality loop to authored docs, reuses 026 |
| 5 | `002-trigger-propagation-description` | 2 | 0 done, scaffold | GO-on-cost. Propagate trigger phrases into description.json |
| 6 | `003-enum-constrain-schemas` | 2 | 0 done, scaffold | GO-on-cost. Enum-constrain the two metadata JSON schemas, the producer guard covering three out-of-enum paths through a dual lenient and strict schema |
| 7 | `005-trigger-coherence-assertion` | 2 | 0 done, scaffold | GO-on-cost. Assert trigger coherence between spec.md and description.json |
| 8 | `006-hvr-style-autofix` | 2 | 0 done, scaffold | GO-on-cost. Autofix HVR style violations at write time |
| 9 | `007-ears-constraints-req-coverage` | 2 | 0 done, scaffold | GO-on-cost. EARS constraints and requirement-coverage checks |
| 10 | `008-surface-provenance-fields` | 2 | 0 done, scaffold | GO-on-cost. Surface provenance fields on each doc |
| 11 | `009-content-hash-integrity` | 2 | 0 done, scaffold | GO-on-cost. Content-hash integrity field so a stale or tampered doc errors |
| 12 | `010-per-surface-gates` | 2 | 0 done, scaffold | GO-on-cost. Per-surface gates so each doc family enforces only its checks |
| 13 | `011-scheduled-dq-sweep` | 2 | 0 done, scaffold | GO-on-cost. Standing scheduled DQ sweep with guarded auto-fix, reuses 026 |
| 14 | `012-doctor-dq-route` | 2 | 0 done, scaffold | GO-on-cost. /doctor data-quality auto-remediation route, reuses 026 |
| 15 | `013-retrieval-feedback-edge` | 2 | 0 done, scaffold | GO-on-cost. Retrieval-learning feedback edge from observed recall |
...
| 23 | `014-chunk-prefix` | 2 | 0 done, scaffold | CONDITIONAL. Header-path and global-identity chunk prefix, gated behind 015 |
| 24 | `016-answerable-questions-tags` | 2 | 0 done, scaffold | CONDITIONAL. Answerable-questions tags, gated behind 015 |
| 25 | `017-metadata-fusion` | 2 | 0 done, scaffold | CONDITIONAL. Metadata fused into the embedding, gated behind 015 |
| 26 | `018-llm-judge-scorer` | 2 | 0 done, scaffold | CONDITIONAL. LLM-judge scorer for floor-hidden candidates, gated behind 015 |
