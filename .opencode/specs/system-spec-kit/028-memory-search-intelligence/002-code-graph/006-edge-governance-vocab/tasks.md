...
- [x] T018 Pre-migration abort test: a seeded out-of-vocab row (incl. nullable `tombstone.edge_type`) makes the migration abort all-or-nothing — no half-rebuilt table (`mcp_server/tests/*.vitest.ts`) — Evidence: test seeds `LEGACY_EDGE`, nullable tombstone edge type, and `LEGACY_TOMBSTONE_EDGE`; migration throws before rebuild and leaves 2 edge rows intact.
...
- [x] T022 Run `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh .opencode/specs/system-spec-kit/028-memory-search-intelligence/002-code-graph/006-edge-governance-vocab --strict`; typecheck + focused code-graph schema/migration/indexer suites green — Evidence: typecheck exit 0; targeted run `3 files / 15 tests passed`; strict validation PASS with 0 errors and 0 warnings.
