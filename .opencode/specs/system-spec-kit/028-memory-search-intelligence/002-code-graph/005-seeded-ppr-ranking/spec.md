...
| 5 | **Q4-C2** multi-hop reliability decay | Reuse the rank-time `reliability` factor (`clamp(confidence) × evidenceClassFactor`) as the per-edge transition weight so an INFERRED 2-hop path ranks below an OBSERVED 1-hop path, folding into the PPR walk | `code-graph-context.ts` transition weights | M / M | **DONE (mechanism)**, deterministic decay test added, calibration **PENDING** on benchmark |
