...
- [ ] T002 Record that the cluster does NOT fix the real edge-staleness bug (dependency-transitivity) → route to sibling `002-002-edge-staleness-correctness`
...
- [ ] T013 [B] Reconcile the `code_edges` validity-window shape with Memory C3-B (`007-007-bitemporal-window`) — shared shape, no fork — BLOCKED on C3-B shape decision
...
- **Shared bi-temporal sibling**: `../../001-speckit-memory/007-007-bitemporal-window/` (Memory C3-B validity-window shape)
<!-- /ANCHOR:cross-refs -->
