...
**What changes**:
- The additive schema foundation shipped (commit `1c39235e36`): `code_edges.valid_at`/`invalid_at`, UP/DOWN/BACKFILL, fail-closed idempotent migration, default-off. The four consumers stay PENDING (gated), the real edge-staleness bug routes to the sibling phase.

**How to roll back**: the shipped schema foundation reverts via `rollbackCodeEdgeBitemporalSchema` plus reverting commit `1c39235e36`, the deferred consumers are non-coding, so their "reversal" is a future, evidence-backed decision to un-defer.
<!-- /ANCHOR:adr-001-impl -->
<!-- /ANCHOR:adr-001 -->
