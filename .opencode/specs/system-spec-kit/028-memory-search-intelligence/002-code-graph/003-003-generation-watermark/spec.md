...
### Out of Scope
- Bi-temporal `valid_at`/`invalid_at` columns (Q1-C1) and the `SCHEMA_VERSION` 5→6 migration — separate phase; this phase rides the column-free `code_graph_metadata` store.
- PPR-seeded impact ranking (Q3-C1), trust multiplier (Q4-C1, already shipped in 030 `e21caf5de6`), doc-lane extraction (Q5-C1) — other phases.
...
|------|------|--------|------------|
| Dependency | `code_graph_metadata` KV table (`code-graph-db.ts:193`/`:456`) | Q6-C2 storage substrate | Already present; KV stores strings only — `bumpCodeGraphGeneration` writes `String(n+1)` and reads back via `Number.parseInt` (the `parseInt`-with-fallback pattern at `:241`); no migration needed |
...
- **NFR-P02**: No new index, no table rebuild, no migration — Q6-C2 stays off the schema chokepoint (research iter-23 phase-0).
...
- The exact `scanPromotable` finalize line range in `handlers/scan.ts` drifts as the file changes — confirm the live block (currently ~`:666-679`, beside `setLastGitHead` `:672` and `setCodeGraphScope` `:679`) at implementation time.
