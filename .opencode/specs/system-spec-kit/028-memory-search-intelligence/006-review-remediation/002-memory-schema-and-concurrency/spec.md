...
|-----------|-------------|-------------|
| `mcp_server/lib/search/vector-index-schema.ts` | Modify later | Align v40 backfill rule_version with the live default; reconcile the skew |
...
| `mcp_server/lib/storage/consolidation.ts` | Modify later | Move the embedding pass out of `BEGIN IMMEDIATE`; add a refreshed maintenance handle |
...
| REQ-002 | Embedding pass outside the write lock | `runSemanticEdgeEmbeddingPass()` no longer runs inside `BEGIN IMMEDIATE`; embeddings are generated before or after the transaction |
...
| ID | Requirement | Acceptance Criteria |
|----|-------------|---------------------|
| REQ-005 | No default-on regression | All three features stay default-off; the fixes change gated paths only |
...
|------|------|--------|------------|
| Risk | Identity reconciliation rewrites existing ids | Migration could touch live rows | Backfill only `WHERE derived_id IS NULL` or reconcile skew explicitly; test on a copy |
