...
| `SPECKIT_GENERATED_METADATA_DRIFT_GATE` | STAY-OFF, awaits a migration re-run | A migration re-run that writes `source_doc_hashes` | Phase 039 restamped under identity-merge-safety and idempotent-writes only, so the field census reads `source_doc_hashes` on 0 of 2093 folders and the drift read has no baseline. |
| `SPECKIT_GENERATOR_HARDENING` | STAY-OFF, awaits a migration re-run | A migration re-run that writes `source_fingerprint` | The field census reads `source_fingerprint` on 0 of 2093 folders, and the verify pass with the flag forced on mass-failed 2049 of 2049 folders, all `SOURCE_FINGERPRINT_MISSING`. |
...
| `SPECKIT_LEXICAL_GROUNDING_V1` | GRADUATED default-ON | Run at phase 040 | The lead candidate cleared its bar, the off-corpus false-confirm rate fell from 0.833 to 0, so it flipped ON. |
...
| `SPECKIT_GROUNDING_SIGNAL_V1` | STAY-OFF, neutral | A fixture that exercises the field | Inert on the off-corpus fixture, the class does not exercise its input, so the false-confirm rate does not move. |
| `SPECKIT_NOISE_FLOOR_SUBTRACTION_V1` | GRADUATED default-ON | Run at phase 040 | The measured before-and-after drove the off-corpus false-confirm rate from 0.833 to 0, so the band-subtraction flipped ON. |
| `SPECKIT_CITE_WITH_CAVEAT_V1` | STAY-OFF, neutral | A fixture that exercises the tier | Inert on the off-corpus fixture for the same structural reason, no measured before-and-after move. |
| `SPECKIT_EVIDENCE_GAP_VERDICT_V1` | STAY-OFF, neutral | A fixture that exercises the gap signal | Inert on the off-corpus fixture, the off-corpus class does not exercise the Stage 4 gap signal. |
| `SPECKIT_ENVELOPE_FIDELITY_V1` | STAY-OFF, fixture-blocked | A captured render corpus | The replay proves the mechanics both ways, but no captured render corpus exists in the repo for a real grandfather report over live renders. |

The distinction from the original keep-offs is worth stating plainly. The ten the reckoning deleted led to no measurable win once their connection to live data was wired, so they were dead code. These thirteen have now been to that gate. Six earned a measured flip and ship graduated. Four stayed off on a neutral or fixture-blocked read, but each has a structural reason and a named fixture that would exercise it rather than the no-path-to-useful that condemned the original ten, so they stay off pending that fixture rather than being deleted. Two stayed off because phase 039 never wrote the field they enforce against, so their verdict waits on a migration re-run rather than a deletion. One ships default-ON by construction. A reader should treat the four neutral and the two re-migration flags as deferred reads with a named revisit condition, not a someday-maybe and not yet dead code.
