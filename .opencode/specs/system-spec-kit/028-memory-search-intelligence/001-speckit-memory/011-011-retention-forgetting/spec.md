...
> Confirmed against `030-memory-search-intelligence-impl/spec.md` §14 + `git log 1ecc531431..HEAD`: NONE of these 8 candidates appear in the Wave-0 shipped record (candidates 1-13 there are Q6-anchor / C9 / ANN-tiebreak / C5-B / C-X1+C6-A / two-primitive-id / gauges / skip-closed / CAS-guard / Deep-Loop-trio / Q4-C1; plus deferred C4-A and M-system-kind-exclusion). All 8 below are therefore PENDING.

| Candidate | STATUS | Gate | Seam (file:line) | Evidence |
|-----------|--------|------|------------------|----------|
| `M-spare-only-eligibility` | **PENDING** | shared-infra-dep (rides the existing `SPECKIT_FEEDBACK_RETENTION_LEARNING`+`mode=shadow` double-gate; finite-comparator hygiene) | `feedback-retention-reducer.ts:153` | [CONFIRMED] deltas/iter-016 `M-spare-only-feasibility` (S/FIX); deltas/iter-032 `H32-01` Rust reference `decay.rs:183-191`, `policy.rs:77-79`; Wave-0 ship-now in iter-037 J37-02 but NOT in 030 §14 |
| `forget-allowlist` | **PENDING** | schema-migration (needs a label column) + locate the exact allowlist symbol; coupled to the temporal/edge read path | `memory-retention-sweep.ts:142` | [CONFIRMED-seam] deltas/iter-012 `M-edge-allowlist-unreferenced` (M/BUILD); deltas/iter-032 `H32-02` 6-label list `forgetter.rs:41-48`; [INFERRED] deltas/iter-016 NEEDS-BENCHMARK (symbol not located) |
...
| `M-trust-gated-quarantine` | **PENDING** | shared-infra-dep (default-OFF flag like `SPECKIT_RECONSOLIDATION`; reuses reconsolidation merge/conflict atomicity + provenance + CONTRADICTS edge) | `reconsolidation-bridge.ts:121` | [CONFIRMED] deltas/iter-013 `M-trust-gated-quarantine` (M/BUILD); deltas/iter-018 `M-trust-quarantine-feasibility` (GO best-fit graft); survived the iter-021 procedural-cluster refutation |
