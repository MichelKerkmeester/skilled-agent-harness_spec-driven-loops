...
- [ ] CHK-002 [P0] Technical approach + sequencing defined in plan.md (spearhead → C3-B → GR-temporal → C3-D)
- [ ] CHK-003 [P1] Canonical event-time source decided (lineage) and recorded in decision-record.md
<!-- /ANCHOR:pre-impl -->
...
- [ ] CHK-010 [P0] Typecheck/build passes (`tsc` on mcp_server)
- [ ] CHK-011 [P0] No new lint warnings in `temporal-edges.ts` / `contradiction-detection.ts` / `vector-index-schema.ts`
- [ ] CHK-012 [P1] `invalidateEdge()` keeps its fail-open contract (warn, no throw)
- [ ] CHK-013 [P1] Additive columns nullable + forward-compatible (unread until a consumer opts in)
<!-- /ANCHOR:code-quality -->
...
- [ ] CHK-021 [P0] skip-closed regression: closed generated edge not re-touched (SHIPPED guard reconfirmed)
- [ ] CHK-022 [P1] C3-B additivity test: existing `IS NULL` readers byte-identical
...
- [ ] CHK-FIX-001 [P0] Each candidate has a finding class: spearhead = `algorithmic` (wrong-time correctness); C3-B = `schema/matrix`; GR-temporal = `algorithmic`; C3-D = decision note; skip-closed = `instance-only` (SHIPPED).
...
- [ ] CHK-FIX-007 [P1] Evidence pinned to a fix SHA / explicit diff range (skip-closed pinned to `e1c6a3c793`).
<!-- /ANCHOR:fix-completeness -->
...
- [ ] CHK-031 [P0] Temporal columns carry timestamps only — no recalled content surface added
- [ ] CHK-032 [P1] No new untrusted-content read path (out of scope; C8 render-escaper is a separate phase)
<!-- /ANCHOR:security -->
...
- [ ] CHK-041 [P1] C3-D separation-of-concerns note recorded (tombstone-sweep vs temporal-close)
...
- [ ] CHK-100 [P0] Architecture decisions documented in decision-record.md (ADR-001 lineage canonical, ADR-002 retention-TTL excluded, ADR-003 C3-D separation)
- [ ] CHK-101 [P1] All ADRs have status (ADR-001/002/003 = Accepted)
- [ ] CHK-102 [P1] Alternatives documented with rejection rationale (causal-projection source, fold-TTL-in, hard-gate skip-closed all scored + rejected)
- [ ] CHK-103 [P2] Migration path documented: C3-B columns additive/nullable, forward-compatible
<!-- /ANCHOR:arch-verify -->
...
- [ ] CHK-111 [P2] C3-B additive columns add no required index for the spearhead path
- [ ] CHK-112 [P2] No load regression in causal/temporal focused suite
...
- [ ] CHK-120 [P0] Rollback documented: per-candidate scoped commits; additive columns forward-compatible if writer reverts (plan.md §7)
- [ ] CHK-121 [P1] `SPECKIT_TEMPLATE_EDGES` flag already ON; spearhead does not depend on a flip
- [ ] CHK-122 [P2] No new monitoring surface introduced (timestamps only)
...
- [ ] CHK-130 [P1] No new untrusted-content read path (C8 render-escaper is a separate phase)
- [ ] CHK-131 [P2] No new external dependency / license surface
- [ ] CHK-132 [P2] Temporal columns carry timestamps only — no PII / content
- [ ] CHK-133 [P2] Data handling unchanged for non-temporal columns
<!-- /ANCHOR:compliance-verify -->
...
- [ ] CHK-141 [P1] Candidate status (SHIPPED vs PENDING) consistent across spec §3, tasks Phase 2, and 030 §14 reference
- [ ] CHK-142 [P2] C3-D separation note carried into both spec.md REQ-006 and decision-record ADR-003
- [ ] CHK-143 [P2] Research citations (file:line + [CONFIRMED]/[INFERRED]) preserved
<!-- /ANCHOR:docs-verify -->
