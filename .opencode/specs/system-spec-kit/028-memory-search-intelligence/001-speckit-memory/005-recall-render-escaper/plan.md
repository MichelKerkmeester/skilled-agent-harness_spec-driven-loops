...
description: "Approach, sequencing, and shared-infra dependencies for the six write→recall→prompt spine candidates: C8 render escaper, capture-side injection filter, CAS-guard P2 polish, substrate-kind recall exclusion, residual-retention disclosure."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Authored implementation plan from 028/001 research"
    next_safe_action: "Build the recall-trust spine (C8 + capture filter + probe) as one coherent change"
    blockers: []
    key_files:
      - "spec.md"
      - "plan.md"
...
### Overview
This sub-phase lands the highest-stakes spine — write → recall → prompt — plus two same-boundary recall-correctness candidates. The center of gravity is one coherent **recall-trust spine**: the C8 `source_kind`-gated render escaper at the recall content formatter, its non-destructive capture-side injection-marker filter at the shared `indexSingleFile` chokepoint, and a focused poison/injection probe vitest. Two candidates are already DONE in 030 (the Constitutional-CAS-guard, `e1c6a3c793`); its P2 polish, the substrate-kind recall exclusion, and the additive residual-retention field round out the phase. No candidate has a benchmarked benefit number — every change ships for correctness and reversibility (campaign caveat).
<!-- /ANCHOR:summary -->
...
### Definition of Done
- [ ] All P0 acceptance criteria met (C8 + injection-filter), each with a focused test
- [ ] CAS-guard P2 polish landed without touching the unconditional self-edit block
- [ ] Substrate-kind exclusion validated against the live DB (no canonical-spec-doc regression)
- [ ] `residual_retention` additive field on the existing sweep result, no deny-list registry
- [ ] `tsc` / build / focused vitest / `validate.sh --strict` green
- [ ] Each candidate committed in isolation on the 028 branch (no push without user go)
<!-- /ANCHOR:quality-gates -->
...
- **`indexSingleFile` chokepoint** (`context-server.ts:2190-2200`): the shared write path through which `memory_save`, ingest, file-watcher, and startup-scan all flow — where the capture-side injection-marker flag installs.
...
### Data Flow
Capture: content → `indexSingleFile` → (secrets redaction) + (NEW non-destructive injection-marker flag, metadata) → index, hash over cleaned content. Recall: stored row (+`source_kind`) → ranking → recall content formatter → (NEW untrusted-content wrapper + tag-escape, gated by `source_kind`; substrate-kind rows excluded unless opt-in) → MCP text block → agent loop.
<!-- /ANCHOR:architecture -->
...
| `indexSingleFile` (`context-server.ts:2190-2200`) | Shared write chokepoint (save/ingest/watcher/startup) | Update — install capture-side marker flag here | Test that an ingest-path write also flags markers (not just the after-tool hook) |
...
| `write-provenance.ts:7` (`SourceKind`) | Defines `'system'` (also canonical spec-docs) | Update — derive a real substrate-internal marker | Live-DB query proves canonical spec-docs + 29 constitutional rules are NOT hidden |
| `formatters/search-results.ts` default recall | Returns all kinds | Update — exclude substrate-internal by default; opt-in to surface | Default-hidden + opt-in-visible tests; live-DB validation |
...
### Phase 1: Setup
- [ ] Read the recall content formatter + `indexSingleFile` + `redaction-gate.ts` seams; confirm the single render seam (NOT `wrapForMCP`).
- [ ] Author the benign-corpus fixture + the anchored-phrase marker list (ported from `filter.rs:352-409`, re-validated on this corpus).
- [ ] Snapshot the live-DB `source_kind='system'` distribution to design the real substrate signal (avoid the ~49% canonical-spec-doc regression).

### Phase 2: Core Implementation
- [ ] **Recall-trust spine (one coherent change):** C8 `source_kind`-gated render escaper at the content formatter + non-destructive `detectInjectionMarkers` capture filter at `indexSingleFile` + the poison/injection probe vitest.
- [ ] **Constitutional-CAS-P2-polish:** remove the now-dead downgrade-audit branch; document the always-on-SELF_EDIT / opt-in-CAS posture.
- [ ] **M-system-kind-exclusion:** derive a real substrate-internal signal + constitutional/spec-doc short-circuit + opt-in surface path; default-recall filter.
- [ ] **M-residual-retention-report:** additive `residual_retention` field on `MemoryRetentionSweepResult` (reading-b scope only).

### Phase 3: Verification
- [ ] Poison/injection vitest passes (forged close-tag inert, zero-success ceiling, empty-probe-fails, full + compact recall).
- [ ] Benign-corpus zero-FP fixture passes (CI-gated).
- [ ] Live-DB validation: canonical spec-docs + 29 constitutional rules NOT hidden by the substrate-kind exclusion.
- [ ] `tsc` / build / existing search/crud/health/promoter suites green; `validate.sh --strict` on this folder green.
<!-- /ANCHOR:phases -->
...
| Benign-corpus fixture + anchored-phrase list | New artifact | Yellow | Required before the marker filter can be enabled (zero-FP gate) |
| Live-DB snapshot of `source_kind='system'` | Validation input | Yellow | Required before flipping the substrate-kind default (avoids the ~49% regression) |
...
- Each candidate is a separable diff with its own scoped commit on the 028 branch — revert the single commit to roll back that candidate without touching the others.
...
|-----------|-----------------|--------|
| C8 source_kind-gated render escaper | M (reference-impl backed) | PENDING |
| M-write-time-injection-filter | M (co-built with C8) | PENDING |
| Constitutional-CAS-guard | S | DONE (`e1c6a3c793`) |
| Constitutional-CAS-P2-polish | S (cleanup) | PENDING |
| M-system-kind-exclusion | S→ real build (re-scoped) | PENDING |
| M-residual-retention-report | S (additive field) | PENDING |
<!-- /ANCHOR:effort -->
...
- [ ] Each candidate is a separable scoped commit on the 028 branch
- [ ] No schema migration; all changes additive/reversible

### Rollback Procedure
1. **Per-candidate revert**: `git revert <candidate-commit>` rolls back exactly one candidate (separable diffs).
