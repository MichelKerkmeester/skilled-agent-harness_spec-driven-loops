...
| `mcp_server/handlers/memory-crud-update.ts` | Modify | Constitutional-CAS-P2-polish: remove the now-dead downgrade-audit branch; resolve opt-in-vs-always-on CAS posture |
...
> Effort/leverage tags are **structural inference, never benchmarked** (campaign-wide caveat, synthesis §B). Every claim is tagged [CONFIRMED] (file:line read directly) or [INFERRED]. Per-candidate STATUS: **DONE** (with 030 §14 commit) or **PENDING** (with gate).
