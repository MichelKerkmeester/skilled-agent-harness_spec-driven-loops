...
- The outcome emitter + f64 Beta primitive are reusable shared infra (the Beta primitive is shared with Advisor C4 — build once, wire twice).
...
**Summary**: Record bad-pattern as a net-new BUILD with two acceptable host paths, the choice deferred to a future build-time with these bounds: **Path A** — a `HAS_FAILURE` table-rebuild schema migration; **Path B** — reuse the `'deprecated'` tier + `contradicts` 0.8 dampener precedent, gated on a PREREQ audit of ALL retrieval-filter sites.
...
| Claim "reuse, no work" (pass-1) | — | REFUTED — `HAS_FAILURE` rejected at 2 layers | 0/10 |
...
**Rollback**: Planning-only; no migration run. A future Path A migration is reversible via down-migration; Path B is a default-OFF tier.
