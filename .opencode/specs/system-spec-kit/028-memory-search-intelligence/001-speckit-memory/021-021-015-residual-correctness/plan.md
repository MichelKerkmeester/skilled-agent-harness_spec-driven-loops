...
### Key Components
- **`handlers/memory-search.ts:494-508`** (A4): `resolveSearchScore` currently prefers `result.score` (RRF magnitude) and applies `> 1 ? /100`; `computeAverageScore` averages over it. Reroute to the absolute-relevance signal — ideally via the same `resolveAbsoluteRelevance` helper the 015 fix uses (`pipeline/types.ts:89-95`) — so the two paths cannot drift again.
