...
description: "Partial implementation summary for the per-edge semantic substrate: schema, edge-vector store, consolidation embedder hook, and shadow retrieval primitive implemented; semantic dedup/invalidation and benchmark remain pending."
trigger_phrases:
...
> **STATUS: PARTIAL IMPLEMENTATION.** The substrate/root work is delivered in this pass. Semantic dedup/merge, cross-pair invalidation, and post-reindex benchmark/promotion remain pending and default-off.
...
The additive v41 migration gives `causal_edges` a nullable `fact_text` column and creates `edge_vector_embeddings`. Existing edges get deterministic fact text backfilled from evidence when available, otherwise from the source/relation/target triplet. `runConsolidationCycle` accepts a flag-gated edge embedding provider; with `SPECKIT_SEMANTIC_EDGE_LAYER` off it does nothing and calls no provider.

### Shadow primitives

`CG-edge-vector-index` and `CG-edge-aware-triplet-search` are implemented as shadow primitives only. `findSemanticEdges` is behind `SPECKIT_EDGE_VECTOR_INDEX`; `rankEdgeTripletCandidates` is behind `SPECKIT_EDGE_TRIPLET_SEARCH`; neither is wired into live fused recall.
...
| `validate.sh --strict` on this packet (docs) | Pending for this implementation pass |
<!-- /ANCHOR:verification -->
...
1. **Partial implementation.** Semantic dedup/merge, cross-pair invalidation, and post-reindex benchmark/promotion remain pending.
