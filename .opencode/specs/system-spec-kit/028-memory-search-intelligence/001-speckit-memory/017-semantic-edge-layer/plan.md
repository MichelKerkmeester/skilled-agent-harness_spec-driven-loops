...
- [ ] Additive migration: nullable `fact_text` on `causal_edges` + dedicated edge-vector collection (mirror `ports/vector-store.ts`)
- [ ] `edge-vector-store.ts`: store edge embedding + nearest-edge lookup
...
| Edge-vector schema migration | Internal | Red (not built) | Whole layer blocked — substrate is gate-zero for the five consumers |
