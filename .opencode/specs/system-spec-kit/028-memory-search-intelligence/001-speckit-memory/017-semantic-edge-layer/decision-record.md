...
| 3 | **Sufficient?** | PASS | One root substrate + a consolidation embedder + a shadow retrieval primitive is the minimal shape that unlocks all five, and the deterministic core is untouched |
...
**How to roll back**: all flags are default-off, so deployed callers are unaffected and no live rollback is needed. To fully revert, drop the consumer modules and the consolidation embedding pass on the branch and optionally down-migrate the inert `fact_text` column and edge-vector collection. The synchronous insert path and live recall were never changed, so there is nothing to restore there.
<!-- /ANCHOR:adr-001-impl -->
<!-- /ANCHOR:adr-001 -->
