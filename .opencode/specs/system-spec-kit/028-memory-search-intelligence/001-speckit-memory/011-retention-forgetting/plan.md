...
| **Storage** | SQLite (`memory_index`, causal edges, retention rows); vector index |
...
- **Procedure**: spare-only and quarantine are flag-gated — flip the flag OFF to revert; C7-A and never-truncate are config-defaulted — set the cap to the limit (no-op) or revert the partition; git revert the touched module. No data migration is applied by the result-shaping candidates (allowlist label column, if added, is additive and reversible).
<!-- /ANCHOR:rollback -->
