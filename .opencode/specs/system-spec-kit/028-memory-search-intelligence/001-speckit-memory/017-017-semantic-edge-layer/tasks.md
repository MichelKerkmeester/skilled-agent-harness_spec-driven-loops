...
- [ ] T004 [P] Acknowledge the shared-infra dependency: the recall/dedup benchmark (Phase 5) is deferred until the gate-zero corpus reindex (`../001-001-corpus-reindex-gate-zero`) lands
<!-- /ANCHOR:phase-1 -->
...
- [ ] T018 [B] Build and run the post-reindex benchmark: edge-aware-triplet recall lift, dedup precision/recall, and false-merge rate; record numbers + the per-sub-candidate promote/keep-shadow/drop decision (BLOCKED on `../001-001-corpus-reindex-gate-zero`) (`mcp_server/__tests__/`)
...
- **Shared-infra dependency**: `../001-001-corpus-reindex-gate-zero` (gate-zero reindex — precondition for T018 benchmark)
