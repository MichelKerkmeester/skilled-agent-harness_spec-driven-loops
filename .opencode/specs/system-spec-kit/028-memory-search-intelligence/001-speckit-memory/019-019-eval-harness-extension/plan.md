...
The candidates follow a **forced linear build order** on a single precondition (the corpus reindex, gate-zero, owned by sibling `001-001-corpus-reindex-gate-zero`): C9-1 emits the diagnostic snapshot the runner already computes but throws away → C9-2 tags the golden labels three ways in one DB-join → C9-3 adds the three corpus-level metrics → A8 un-welds the promotion gate from `meanNdcgDelta` to a per-class panel and routes its labels through the golden set.
...
### Definition of Ready
- [ ] Gate-zero confirmed: sibling `001-001-corpus-reindex-gate-zero` reindex run + `assertEmbeddingCoverage` passes. Evidence: sibling phase status + `memory_health` coverage.
...
### Phase 1: Setup
- [ ] Confirm gate-zero green: sibling `001-001-corpus-reindex-gate-zero` reindex run + `assertEmbeddingCoverage` passes.
