...
**Why this one**: anchor-inclusion is mandatory; `rule_version` is the provenance value; the kind-tag prevents cross-class collisions.
<!-- /ANCHOR:adr-002-alternatives -->
...
Existing derived rows predate `rule_version`; the backfill must still produce a deterministic `derived_id` for them. Without a defined sentinel, legacy rows either fail the backfill or get a non-reproducible id.
