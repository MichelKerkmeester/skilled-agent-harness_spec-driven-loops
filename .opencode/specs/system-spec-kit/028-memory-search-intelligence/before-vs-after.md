...
The authored spec docs and the two metadata JSONs had no automated quality program. The live quality-loop, the memory-save scorers and the auto-fix that already ship default-on, ran only on the memory-save path and never touched the authored write surface. The truncation law was a measurement finding and nothing more. Section 7 had named it and Section 6 had priced flags against it, but no standing program had been built that treated the 3-result prod floor as the organizing constraint for a whole slate of data-quality work.

**After**

The research pass produced a tiered, dependency-ordered program of twenty-eight scaffolded recommendations, none of them built. The keystone is to extend the live default-on quality-loop, the same memory-save scorers and auto-fix, to the authored spec-doc and metadata-JSON write surface so the gates that already run at memory-save time also run when a doc or a metadata file is written. The program tiers by where a candidate sits against the truncation floor. Tier A is on-write reuse-first work across phases 001 through 010, and its one measured unconditional GO is A4, the promotion of the JSON-schema shape rules from warn to error, which is a validation tightening rather than a ranking gain. The rest of Tier A is scoped on cost rather than on a measured win. Tier B is retroactive automation across phases 011 through 013, a standing scheduled DQ sweep with guarded auto-fix, a `/doctor` data-quality auto-remediation route and a retrieval-learning feedback edge. Tier C is the retrieval slate across phases 014 through 018, every item default-off and gated on the prod-mode `completeRecall@3` benchmark in phase 015, which is the unblocker for the tier and has not been run. A separate novel slate across phases 019 through 025 carries seven floor-bypassing capabilities that earn a GO out of the box because they never depend on the truncated top-3, cross-doc contradiction detection, embedding-drift monitoring, auto example and test generation, a context-budget assembler, a typed-relation knowledge graph, a freshness-decay queue and per-doc quality SLAs. Three infra phases close the program, 026 the one shared safe-fix engine the on-write and retroactive front doors both use, 027 the operator-agreed experiment, proposed and not yet run, to raise the retrieval budget and measure whether results four through ten are signal or noise, and 028 the seventeen-stage rollout with its four-beat migration, safety model and measurement plan. The feature flags the program names, an authored-surface quality-loop flag, a scheduled-sweep flag, an EARS-lint flag, a retrieval-gap-detect flag and the C-tier retrieval flags, are proposed and not built. Nothing in any tier has shipped or been measured.

**Impact**

A future build packet does not inherit a pile of ad-hoc data-quality fixes. It inherits a tiered, dependency-ordered, truncation-law-aware program where every candidate already knows whether it can earn a prod-path keep or whether it sits behind the floor. The cheap real wins at the write surface are scoped and ready to build first, the one measured GO is isolated, the retrieval slate is fenced behind a single benchmark that has to go green before any of it runs, and the floor-bypassing novel work is marked as the part that can ship without waiting on the floor at all.

**Why**

The track holds the same discipline as the rest of 028. A retrieval feature earns a keep only by changing the composition of the truncated top-3, never by adding rows the floor cuts, exactly as the new-feature arc in Section 7 held its tail-additive seams. So the program puts its weight where the win is real today. The write-time gates run before truncation ever applies, and the one measured unconditional GO, A4, is a JSON-schema warn-to-error promotion that tightens validation rather than moves a rank. Everything that depends on the prod reader seeing past rank three is gated on the prod-mode benchmark and the 027 floor experiment, both proposed and neither run, so the retrieval slate waits behind a measurement instead of being forced through on an eval-mode number the production reader never sees.
