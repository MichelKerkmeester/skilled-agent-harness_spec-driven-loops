...
| **C5** | Skill Advisor | Runtime-empty lane elision in `liveTotal` (degrade-to-remaining, not skew-down) — fixes confirmed ~13% confidence skew when a lane runs but returns `[]` during graph rebuild | `fusion.ts:343-345, :388` | H | S | PROMOTE-fix |
...
| **C4** | Skill Advisor | Bounded Beta-posterior lane auto-tune — a GRADUATION of the already-shipped shadow estimator (durable outcome capture + bounded delta estimator + shadow-weight registry), not a new build; needs holdout promotion gate | `feedback-calibration.ts:154`; `lane-registry.ts:71-74`; outcomes `metrics.ts:234` | H | L | BUILD (shadow-first) |

**Note:** Deep Loop owns this spine — D2 is the keystone reliability signal that D1/D3/Q2/Q7 all consume (design once, use three times). Skill Advisor's C4 is the same Beta-posterior pattern but is a *graduation* of an end-to-end shadow pipeline that already ships (outcome → shadow-weight → shadow-recommendation), making it lower-build than it first appears. Both share aionforge's anti-overfit shape: cold-start neutrality, count floor, clamped deviation band.
...
- **Memory's two longest chains** are `C4-A → C4-C → C-G1` (idempotency → cursor → cadence) and `C3-A → C3-B → C3-C` (flip-on → four-timestamp → temporal modes). Wave 0 (C9, C3-A, C6-A, C5-B) is the independent, reversible, S-effort spearhead. C-X1 sequences BEFORE C2-B so per-class zero-weighting doesn't reshape the convergence bonus.
...
- The "import 001 `fuseResultsMulti`" reuse is **[CONFIRMED]** shape-compatible for Skill Advisor (`LaneMatch{skillId}` → `RrfItem{id}`, zero schema friction) but **[INFERRED]** for Code Graph (signature genericity for code-graph result shapes — unread).
...
- **Code Graph:** `SymbolKind` tolerance for new `heading`/`key` kinds (Q5-C1); `fuseResultsMulti` signature genericity (Q8); impact-walk run-stability (is impact output already stable across runs, or does it rely on SQLite row order); cross-file CALLS resolver vs supersede (trace dangling-prune contract `:957-968` before Q1-C1).
- **Skill Advisor:** re-confirm the C4 shadow linkage is ONE live pipeline (outcome record → delta estimator → shadow-weight registry → `_shadow` channel), not three disconnected scaffolds; QCR class taxonomy + per-class lane multipliers (keep explicit_author dominant); exact Beta prior + holdout thresholds for C4; how the negative `conflicts_with` mass stays outside RRF (C1, multiplicative vs subtractive bound); C5 before/after baseline capture (changes confidence magnitudes per regression-baseline rule).
