...
> This file is the **pass-1 (19-iter) parent synthesis** and is a truncated copy of `roadmap.md`. The campaign was broadened to **100 iterations** (001=38, 002=24, 003=18, 004=20). The authoritative cross-cutting corrections live in **`roadmap.md` → "BROADENING ADDENDUM (Iterations 20–100)"** — read that for the full record. Net effect: **deflationary** (~12 candidates refuted; both meta-spines tempered). Headline summary: "promote-off-state" is **0-of-4 clean flips** (only Memory C4-A is literal); "determinism strongest spine" is genuinely shared by **2-of-4**; Advisor **C4** is a build not a Beta graduation; Deep Loop **D2** is a wholly-absent net-new build (not a present keystone). No candidate has a measured before/after benefit number — the highest residual unknown at 100.
>
> **The campaign then ran a 5th child (`005-revisit-027`, +50 iters → 150 total)** reconciling these findings against packet 027's shipped code: **028 is net-additive to 027 (0 supersedes, 0 contradicts)**. **Consolidated, digestible synthesis: `research/synthesis/00-index.md`** (→ GO list `01`, 027↔028 ledger `02`, corrections/caveats `03`). Full cross-packet ledger: `005-revisit-027/research/research.md`.