...
- **The 006 round has no per-iteration file backup** — its findings live in one consolidated ledger + a 50-row state log (which is why `…/../../001-speckit-memory/research/sibling-cross-cutting-revisit/iterations/` is empty by design; re-validated during the final review).
