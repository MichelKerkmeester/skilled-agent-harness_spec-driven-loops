...
### Overview
Build the missing C4 machinery in dependency order on the campaign's critical path: a Phase-0 foundation (baseline + lane-health signal + the clean asymmetric-deltas FIX), then the disconnected estimator→registry **seam** (an out-of-process cron promoter), then a shared **Beta-posterior** f64 primitive (co-built with Deep-Loop D2), then the aionforge **promotion-gate family** (two-gate + held-out + cold-start + decay + content-fold) layered on top. Everything stays shadow-only behind `liveWeightsFrozen`; promotion to live is a documented NO-GO until a micro-benchmark earns it.
<!-- /ANCHOR:summary -->
