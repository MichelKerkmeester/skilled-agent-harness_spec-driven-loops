...
This rollup now reflects the child phase evidence instead of the raw generator output. Four phases shipped default-off or correctness-scoped scorer work, one phase shipped a shadow-only follow-on build and two phases are planning or substrate-gated closeouts. Detailed scope remains in the linked child changelogs.
...
| [`004-c4-shadow-seam-beta-posterior`](./changelog-003-004-c4-shadow-seam-beta-posterior.md) | Planning closeout | The reliability-learning path is specified as a shadow-only build with a promoter, shared Beta posterior and attestation gates. No code shipped in this child phase. |
