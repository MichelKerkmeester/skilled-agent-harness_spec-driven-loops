...
| `SPECKIT_EARS_LINT` | would-be OFF | Lints authored requirement lines against the EARS shape at write time and warns on requirements that do not parse. | PROPOSED, not built, not measured | Tier-A on-write (phases 001-010). Write-time validation, so it would bypass the prod truncation floor. Would ship on cost. |
| `SPECKIT_RETRIEVAL_GAP_DETECT` | would-be OFF | Watches retrieval results for gaps and feeds each miss back as a learning edge, so a query that should have surfaced a doc but did not would leave a trace the system can act on. | PROPOSED, not built, not measured | Tier-B retrieval-learning feedback edge (phase 013). Retrieval-facing, so it inherits the truncation law and is gated on the phase-015 prod-mode completeRecall@3 benchmark before any flip. |
| `SPECKIT_CHUNK_PREFIX` | would-be OFF | Prepends a context prefix to each chunk before embedding, so a chunk would carry its parent-doc identity into the vector. | PROPOSED, not built, not measured | Tier-C retrieval (phases 014-018), default-off. Gated on the phase-015 prod-mode completeRecall@3 benchmark before any flip. |
| `SPECKIT_ANSWERABLE_QUESTIONS` | would-be OFF | Generates the questions a doc can answer and indexes those next to the doc, so a question-shaped query would have a question-shaped target to match. | PROPOSED, not built, not measured | Tier-C retrieval. Gated on the phase-015 prod-mode completeRecall@3 benchmark before any flip. |
| `SPECKIT_METADATA_FUSION` | would-be OFF | Fuses structured metadata signals into the retrieval score, so doc type, freshness and provenance could lift or sink a candidate. | PROPOSED, not built, not measured | Tier-C retrieval. Gated on the phase-015 prod-mode completeRecall@3 benchmark before any flip. |
| `SPECKIT_LLM_JUDGE` | would-be OFF | Adds an LLM re-rank pass that judges candidate relevance before the result is returned. | PROPOSED, not built, not measured | Tier-C retrieval. Gated on the phase-015 prod-mode completeRecall@3 benchmark before any flip. |

The reason the whole retrieval tier sits behind a gate is the truncation law this packet already documents in [`benchmark-status.md`](./benchmark-status.md) and [`before-vs-after.md`](./before-vs-after.md). The prod retrieval path collapses every query to the `DEFAULT_MIN_RESULTS=3` floor through token-budget truncation, so any contribution past rank 3 is invisible to the prod reader. A retrieval-class candidate cannot earn a keep on eval-mode numbers. It can only earn one if it moves the truncated prod top-3. That is why every Tier-C flag above is gated on the phase-015 prod-mode completeRecall@3 benchmark (`015-c2-prodmode-recall-gate`) before any flip, and why the phase-027 floor experiment that would raise the retrieval budget and measure whether results 4 through 10 are signal or noise is itself only proposed, not run. Until that experiment runs there is no measured basis to flip any retrieval flag.

The one near-term real win in the whole slate is not a flag at all. It is the A4 JSON-schema warn-to-error promotion (`004-a4-schema-warn-to-error`), the slate's single measured unconditional GO, which would turn the existing JSON-schema shape rules from a warning into a hard error. It needs no environment switch because it tightens a check that already runs, and it is validation not ranking, so the prod truncation floor never touches it. Even so it has not shipped. Everything else in Track C either would ship on cost as an on-write or retroactive convenience or stays frozen behind the prod-mode gate, and none of it has been built or measured yet. The full program tiering, the per-phase plans and the truncation-law reasoning live under [`005-spec-data-quality/`](./005-spec-data-quality/).
