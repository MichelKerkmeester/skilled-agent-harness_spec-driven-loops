...
_memory:
  continuity:
    packet_pointer: "system-spec-kit/028-memory-search-intelligence/005-spec-data-quality/020-novel-embedding-drift-monitor"
...
### Out of Scope
- The C1 chunk-prefix re-embed and the strategy-version cache-key fold - that work is owned by `014-c1-chunk-prefix`. This phase only stamps and counts the regime, it does not re-embed.
...
- The prod-mode completeRecall@3 gate - that instrument is owned by `015-c2-prodmode-recall-gate`. This phase protects that read from a mixed-regime confound, it does not perform the read.
