...
**Evidence:**
- 015 modes read "the prod column" and "the prod profile" with no candidate parameter: `015-prodmode-recall-gate/spec.md:78`, `015-prodmode-recall-gate/spec.md:108-110`.
...
- Each consumer states a default-off posture but none defines how the flag goes on inside the gate: `016-answerable-questions-tags/spec.md:122`, `017-metadata-fusion/spec.md:122`, `018-llm-judge-scorer/spec.md:125`, `014-chunk-prefix/spec.md:117` (REQ-004).
...
- 014 names the raw harness as its metric source: `014-chunk-prefix/spec.md:127` (SC-002, "the prod-mode completeRecall@3 number from `run-eval-v2.mjs` dual-mode").
- 017 names the raw harness too: `017-metadata-fusion/spec.md:77`.
- 015 problem statement that the raw harness lacks both gate features: `015-prodmode-recall-gate/spec.md:65`. Gold set is the signal-bearing artifact: `015-prodmode-recall-gate/spec.md:128` (SC-003).
...
**Evidence:**
- 017 modifies the harness: `017-metadata-fusion/spec.md:92` (Change Type Modify, "Add an alpha-sweep mode").
- 015 freezes the same harness: `015-prodmode-recall-gate/spec.md:96` (Change Type "Verify (no change)"), reinforced at `015-prodmode-recall-gate/plan.md:106-107`.
...
- Both-keys requirement and High risk: `014-chunk-prefix/spec.md:110` (REQ-002), `014-chunk-prefix/spec.md:139` (Risk "Dual-cache-key no-op", High).
- Coverage-guard net-new verified: grep for `embedding_context_version|embeddingCoverage|coverageThreshold` across the skill tree (excluding specs) is empty, matching `014-chunk-prefix/spec.md:138`.
