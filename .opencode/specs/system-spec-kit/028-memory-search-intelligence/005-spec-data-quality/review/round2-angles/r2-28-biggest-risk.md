...
- **Internal contradiction that proves the overstatement:** `027-retrieval-floor-experiment/spec.md:67` describes the SAME mechanism accurately as "a 3-result floor layered with a gap-cliff at roughly 2x the median gap and a token budget". The program contradicts its own spine, the careful reading lives in 027 while the headline in research.md and the C2 spec (`015-prodmode-recall-gate/spec.md:65`) carry the fixed-3 framing.
