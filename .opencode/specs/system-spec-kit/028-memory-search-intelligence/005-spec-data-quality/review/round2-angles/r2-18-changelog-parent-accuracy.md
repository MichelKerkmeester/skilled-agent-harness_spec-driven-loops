...
- **Evidence:** `005-spec-data-quality/001-on-write-quality/001-extend-quality-loop-authored/graph-metadata.json` carries `"status": "pending"` while phases 002 through 028 carry `"status": "draft"` (confirmed on `003-enum-constrain-schemas/graph-metadata.json:35` and the full sweep). All 28 are in identical PLANNED scaffolded state. The root rollup faithfully echoes the split: `028-memory-search-intelligence/changelog/005-spec-data-quality/changelog-005-root.md:27` shows 001 as "PENDING" while lines 28-54 show 002-028 as "Draft".
