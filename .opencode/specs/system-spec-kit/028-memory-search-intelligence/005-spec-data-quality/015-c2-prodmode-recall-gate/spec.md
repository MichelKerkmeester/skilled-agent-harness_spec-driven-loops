...
- A minimal export surface on the unchanged `run-eval-v2.mjs` so the gate consumes its prod-lens profile and measurability classes without re-implementing the lenses.
...
| `.opencode/skills/system-spec-kit/mcp_server/scripts/evals/run-eval-v2.mjs` | Modify | Add a narrow export so the gate consumes the prod-lens profile and classes, lenses unchanged |
<!-- /ANCHOR:scope -->
...
| ID | Requirement | Acceptance Criteria |
|----|-------------|---------------------|
| REQ-005 | The gate SHALL reuse the `run-eval-v2.mjs` prod lens and measurability classes through an export rather than re-implementing them | The gate imports `buildSearchLenses`, `meanCompleteRecallProfile` and `MEASURABILITY_CLASSES` with no lens logic duplicated |
...
|------|------|--------|------------|
| Dependency | `run-eval-v2.mjs` dual-mode harness | The gate is a thin wrapper and breaks if the lens contract changes | Consume only the exported lens and class symbols and never fork the lens body |
...
|-----------|-------|-------|
| Scope | 9/25 | Two net-new files plus one narrow export on an existing 361-line harness |
...
Tier C unblocker. The verdict for the gate itself is GO-on-cost: it touches measurement not ranking, the dual-mode harness already ships and the build is two narrow files plus one export. Every Tier-C retrieval item (C1, C3, C4, C5) and every 027 retrieval item is conditional-C2-gated on this instrument, so this phase is the keystone that lifts that condition. The prod-mode completeRecall@3 read is non-negotiable: eval-mode @K and external @5/@10/@20 numbers are inadmissible as a pass condition because the K=3 floor hides exactly that band.
<!-- /ANCHOR:verdict -->
