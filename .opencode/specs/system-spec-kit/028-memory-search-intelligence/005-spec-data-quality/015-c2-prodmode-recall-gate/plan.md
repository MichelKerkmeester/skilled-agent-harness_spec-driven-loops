...
description: "Build a prod-mode completeRecall@3 instrument from a multi-target gold set, a PROMOTION and REGRESSION wrapper that reads only the prod column, a stored baseline, and a narrow export on the unchanged dual-mode harness."
trigger_phrases:
...
### Pattern
Thin gate wrapper over an unchanged measurement harness, plus two net-new data and script files and one narrow export. No ranking change and no new lens logic.
...
- **`run-eval-v2.mjs`**: the unchanged dual-mode harness, gaining only a narrow export so the gate reuses the prod lens and measurability classes.

### Data Flow
The harness builds the copy DB and runs the eval lens and the prod lens over it. The gate imports the prod-lens profile and the measurability classes through the new export, reads only the prod completeRecall@3 column, compares it to the stored baseline, and returns a recall-verdict exit code distinct from the existing crash handler.
<!-- /ANCHOR:architecture -->
...
|---------|--------------|--------|--------------|
| `run-eval-v2.mjs` lenses | Run the eval lens and the prod lens on one copy DB and report `evalVsProdDelta` at K of 3, 5, and 8 | not a consumer change, reuse the lenses verbatim, add a narrow export only | grep shows the new export and no edit to the lens bodies |
| `run-eval-v2.mjs` export surface | No symbol the gate can import | add an export for `buildSearchLenses`, `meanCompleteRecallProfile`, and `MEASURABILITY_CLASSES` | the gate imports those three names and duplicates no lens logic |
...
### Phase 1: Setup
- [ ] Confirm the prod lens, `meanCompleteRecallProfile`, and `MEASURABILITY_CLASSES` are reachable for export from `run-eval-v2.mjs` without touching the lens bodies
- [ ] Add the narrow export so the gate consumes the prod-lens profile and classes
...
| The exported `buildSearchLenses`, `meanCompleteRecallProfile`, and `MEASURABILITY_CLASSES` symbols | Internal | Yellow | The gate cannot reuse the lenses without the narrow export |
...
- **Procedure**: Remove the gate wrapper and the gold and baseline files, then revert the narrow export so the harness returns to its shipped state.
<!-- /ANCHOR:rollback -->
...
3. Revert the narrow export on `run-eval-v2.mjs`
4. Re-run the harness to confirm it returns to its shipped state

### Data Reversal
- **Has data migrations?** No
- **Reversal procedure**: N/A, the change adds eval-only data and script files and one narrow export
<!-- /ANCHOR:enhanced-rollback -->
