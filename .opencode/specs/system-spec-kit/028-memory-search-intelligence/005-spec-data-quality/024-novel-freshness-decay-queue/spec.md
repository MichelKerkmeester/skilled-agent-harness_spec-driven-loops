...
| Dependency | B1 sweep report (011-b1-scheduled-dq-sweep) and B3 governance precedent (013-b3-retrieval-feedback-edge) | The queue surfaces through the sweep and mirrors the `learned_feedback_audit` shape | Reuse the report channel and the audit governance, fork neither |
...
- **Sibling B1 and B3**: the queue surfaces through the B1 sweep report (011-b1-scheduled-dq-sweep) and mirrors the `learned_feedback_audit` governance the B3 feedback edge (013-b3-retrieval-feedback-edge) also reuses. It forks neither.
- **Not gated on 015-c2-prodmode-recall-gate**: the queue emits findings not vector rows, so the C2 prod-mode completeRecall@3 gate does not block it. This is floor-bypassing by construction.
