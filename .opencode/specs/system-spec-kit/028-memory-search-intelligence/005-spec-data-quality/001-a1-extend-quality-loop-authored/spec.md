...
### In Scope
- H1: score the two metadata JSONs at the `atomicWriteJson` seam in `generate-context.ts` using the PURE `computeMemoryQualityScore`, report-only, attach the score, never mutate the JSON body.
...
|-----------|-------------|-------------|
| `.opencode/skills/system-spec-kit/scripts/memory/generate-context.ts` | Modify later | H1: at the `atomicWriteJson` seam (`:398`) call `computeMemoryQualityScore` on the JSON payload and report the score, no body write |
...
|----|-------------|---------------------|
| REQ-001 | H1 scores the metadata JSONs without mutating them | WHEN `generate-context.ts` writes a metadata JSON via `atomicWriteJson`, THE SYSTEM SHALL compute `computeMemoryQualityScore` on the payload and report the score, and the written JSON body SHALL be byte-identical to the pre-scoring payload |
...
- The two metadata JSONs carry a reported quality score on write, and their bodies are unchanged.
...
- A metadata JSON that is malformed at the `atomicWriteJson` seam must still write, with the score reported as unavailable rather than blocking the write.
