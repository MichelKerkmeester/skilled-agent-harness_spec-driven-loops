...
| `029-substrate-sandbox-cleanup` | Complete | Test-infra hygiene. The substrate stress harness no longer leaves its `_sandbox/24--local-llm-query-intelligence/` scratch directory at the repository root. The throwaway code-graph DB is removed on every run, a `--clean` flag drops the whole sandbox for standalone runs and the test `afterAll` clears it after reading the summary TSV. No memory behavior changed. |
