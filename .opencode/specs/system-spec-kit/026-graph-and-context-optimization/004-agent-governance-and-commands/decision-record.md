...
_memory:
  continuity:
    ...
    recent_action: "Promoted 13 ADRs from former phase 002 to packet root under unified framing"
    next_safe_action: "Reference these ADRs when reviewing command-graph or agent-governance architecture"
    blockers: []
    key_files:
    ...
    session_dedup:
      fingerprint: "sha256:004-agent-governance-adr-merge-2026-04-24"
      session_id: "004-agent-governance-adr-merge-2026-04-24"
...
## Framing: How Workstream A relates to these ADRs

The 13 ADRs below were authored while delivering Workstreams B and C (canonical intake + middleware cleanup). Workstream A — the AGENTS execution-guardrail update — did not produce separate ADRs because it is the *instruction-surface* mirror of ADR-011's *agent-surface* decision: the same governance thesis that replaces `@speckit` exclusivity with templates + `validate.sh --strict` + `/memory:save` also relies on AGENTS files telling agents to plan first, act surgically, verify with evidence, and own issues instead of punting. Those eight guardrails are the operational behaviors ADR-011 assumes when it distributes spec-folder authoring across any agent rather than holding it inside one exclusive wrapper.
...
| 2 | **Beyond Local Maxima?** | PASS | Current diff formula, a lighter exact-line alternative, and a heavier semantic alternative were compared |
...
| 4 | **Fits Goal?** | PASS | Decision stays inside M7 remediation and does not expand into runtime changes |
...
**Decision**: Split the downstream sweep into forward-looking (authoritative docs) and historical (changelog entries, scratch snapshots, older packet internals). Only forward-looking docs are updated; historical records remain as-is.
...
**Note (Workstream A linkage)**: The lean execution-guardrail block in the three AGENTS files is the behavioral foundation this ADR depends on. Agents only safely hold distributed spec-folder authoring responsibility if they plan first, research first, reason from actual data, and own issues instead of punting — exactly the eight guardrails enforced by Workstream A.
...
<!--
Decision Record — 13 ADRs covering the full canonical-intake architecture + middleware cleanup:
- ADR-001: Structural overlap formula (M7 remediation)
- ADR-002 through ADR-009: Shared-module architecture decisions
- ADR-010: M15 deep-review remediation (explicit intake_only YAML gate)
- ADR-011: M9 distributed-governance rule (behavioral foundation enforced by Workstream A AGENTS guardrails)
