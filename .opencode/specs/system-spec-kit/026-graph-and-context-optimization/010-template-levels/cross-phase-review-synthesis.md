---
title: "Cross-Phase Deep-Review Synthesis (010-template-levels, 8/8 phases)"
description: "Aggregate findings across the 8 phase children of 010-template-levels. Single CONDITIONAL verdict cluster, ~40 P1 + ~23 P2, no P0. Captures cross-phase themes, per-phase results, operational events (rate-limit, sandbox blockage, accidental deletion), and recommended remediation packets."
trigger_phrases:
...
- Path Convention drift: `.opencode/agent/deep-review.md` is canonical per the YAML workflow (`spec_kit_deep-review_auto.yaml:71-72`), but `.claude/agents/deep-review.md`, `.gemini/agents/deep-review.md`, `.codex/agents/deep-review.toml` all label themselves as canonical Path Convention reference paths.
- 003 found `@create` agent missed in the cross-runtime agent vocabulary cleanup (`.opencode/agent/create.md:141` still uses `## 2. CAPABILITY SCAN` while siblings use `ROUTING SCAN`); the resource map also omits `create.md` from the agent cleanup list.
- 005 found the deep-review iteration prompt pack at `prompt_pack_iteration.md.tmpl:18` points to `.agents/skills/sk-code-review/references/review_core.md` (non-existent) while `.opencode/agent/deep-review.md:273` uses the correct `.opencode/skill/sk-code-review/references/review_core.md`. CLI and native executor severity classification will diverge.
...
**Fix scope**: remove the broad public-root allowlist OR replace it with narrow documented exceptions; add a default-public-surface sentinel proving banned vocabulary fails inside `.opencode/agent` or `.opencode/command/spec_kit`; add a golden test that executes `create.sh` phase-parent scaffolding and asserts no private vocabulary in rendered output.
...
- **Skill / command**: `/spec_kit:deep-review:auto` (`.opencode/skill/sk-deep-review/`, `.opencode/command/spec_kit/deep-review.md`).
...
- **Aggregate dispatch artifacts** preserved at `/tmp/dr-010-*.log` (per-phase stderr capture) — recommend archive to `010-template-levels/_review-logs/2026-05-04/` before /tmp cleanup.
...
|---|---|---|---|
| **RM-4a** | ✅ Applied | `sk-deep-review/assets/prompt_pack_iteration.md.tmpl:18` | Doctrine path corrected: `.agents/skills/sk-code-review/...` → `.opencode/skill/sk-code-review/...` (1-line fix) |
| **RM-4b** | ✅ Applied (×4 mirrors) | `.opencode/agent/create.md`, `.claude/agents/create.md`, `.gemini/agents/create.md`, `.codex/agents/create.toml` | `## 2. CAPABILITY SCAN` → `## 2. ROUTING SCAN` to align `@create` with the 8 sibling routing-scan agents (excluding `@code` and `@review` which use FAST PATH) |
...
- `001-template-consolidation-investigation/research/research.md` — original investigation that flagged the partial-consolidation path (later rejected; led to greenfield path).
