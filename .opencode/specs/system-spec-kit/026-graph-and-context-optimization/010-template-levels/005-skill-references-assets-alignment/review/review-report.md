...
```json
{
  ...
  "planSeed": [
    "Update .opencode/skill/sk-deep-review/assets/prompt_pack_iteration.md.tmpl line 18 to use .opencode/skill/sk-code-review/references/review_core.md or a rendered runtime variable.",
...
1. **P1-001: Iteration prompt pack points executor contexts at a non-existent review doctrine path**  
   - **Evidence:** `.opencode/skill/sk-deep-review/assets/prompt_pack_iteration.md.tmpl:18` instructs executors to load `.agents/skills/sk-code-review/references/review_core.md`, while `.opencode/agent/deep-review.md:273` uses the existing `.opencode/skill/sk-code-review/references/review_core.md` path. The auto YAML renders and dispatches this prompt pack through native and CLI executor paths at `.opencode/command/spec_kit/assets/spec_kit_deep-review_auto.yaml:638-680`.
   - **Impact:** Non-native executor severity classification can diverge or fail to load the required doctrine.
   - **Recommendation:** Replace the prompt-pack path with `.opencode/skill/sk-code-review/references/review_core.md` or render a runtime-resolved existing path.
...
1. **P2-001: `--path` boundary docs omit the live `/tmp` test-fixture exception**  
   - **Evidence:** `.opencode/skill/system-spec-kit/references/workflows/execution_methods.md:119` and `.opencode/skill/system-spec-kit/references/workflows/quick_reference.md:137` say traversal outside the repo is rejected, while `create.sh` explicitly permits `/tmp`/`${TMPDIR}` test fixtures before repo-boundary rejection at `.opencode/skill/system-spec-kit/scripts/spec/create.sh:751`.
...
3. **P2-003: Validation references are not registry-complete for the current validator surface**  
   - **Evidence:** `.opencode/skill/system-spec-kit/references/validation/validation_rules.md:16` claims complete rule coverage but omits current registry rules; `validate.sh` derives default rules from `.opencode/skill/system-spec-kit/scripts/lib/validator-registry.json` at `.opencode/skill/system-spec-kit/scripts/spec/validate.sh:357`, `:390`, and `:482`.
   - **Recommendation:** Generate or verify validation reference summaries from `validator-registry.json`.

4. **P2-004: Runtime mirrors re-label packaged mirror paths as canonical while YAML declares `.opencode/agent/deep-review.md` canonical**  
   - **Evidence:** `.opencode/command/spec_kit/assets/spec_kit_deep-review_auto.yaml:71-72` names `.opencode/agent/deep-review.md` canonical; mirror Path Convention lines point to `.claude/agents/*.md`, `.gemini/agents/*.md`, and `.codex/agents/*.toml` at `.claude/agents/deep-review.md:27`, `.gemini/agents/deep-review.md:27`, and `.codex/agents/deep-review.toml:20`.
   - **Recommendation:** Reword mirror Path Convention lines to say they are packaged mirrors while `.opencode/agent/deep-review.md` remains the canonical reference path.

## Active Finding Registry

| ID | Severity | Dimension | Primary Evidence | Disposition |
|----|----------|-----------|------------------|-------------|
| P1-001 | P1 | cross-runtime-mirror-consistency | `.opencode/skill/sk-deep-review/assets/prompt_pack_iteration.md.tmpl:18` | active |
| P2-001 | P2 | implementation-spec-alignment | `.opencode/skill/system-spec-kit/references/workflows/execution_methods.md:119` | active |
| P2-002 | P2 | template-rendering-correctness | `.opencode/skill/system-spec-kit/templates/manifest/spec-kit-docs.json:119` | active |
| P2-003 | P2 | validator-coverage | `.opencode/skill/system-spec-kit/references/validation/validation_rules.md:16` | active |
...
- T001 Update `.opencode/skill/sk-deep-review/assets/prompt_pack_iteration.md.tmpl` line 18 to the existing `.opencode/skill/sk-code-review/references/review_core.md` path or a rendered runtime variable.
