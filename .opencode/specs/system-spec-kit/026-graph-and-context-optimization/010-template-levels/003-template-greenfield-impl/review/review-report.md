...
- **DR-002-P1-001 [P1] Workflow-invariance test path-allowlists the public surfaces it claims to gate** (code-correctness, iteration 2)
  ...
  - Recommendation: Remove the broad public-root allowlist or replace it with narrow documented exceptions, then add a default-public-surface sentinel proving banned vocabulary fails inside .opencode/agent or .opencode/command/spec_kit.
  - Disposition: active
  - Finding class: cross-consumer
...
- **DR-003-P1-001 [P1] Golden scaffold snapshots miss create-time phase-parent rendering regressions** (template-rendering-correctness, iteration 3)
  - File: `.opencode/skill/system-spec-kit/scripts/tests/scaffold-golden-snapshots.vitest.ts:49`
...
- **DR-005-P1-001 [P1] Create agent prompt was missed by the cross-runtime agent vocabulary cleanup** (cross-runtime-mirror-consistency, iteration 5)
  - File: `.opencode/agent/create.md:141`
  - Evidence: The resource map says all AI-facing agent definitions under .opencode/agent/ are in scope for the heading rename, but omits create.md from the enumerated agent cleanup list. AGENTS.md and CLAUDE.md expose @create as a first-class agent, and create.md still contains ## 2. CAPABILITY SCAN while sibling agent prompts use ROUTING SCAN.
  - Recommendation: Rename the create.md section heading to ROUTING SCAN and add .opencode/agent/create.md to the agent cleanup/resource-map coverage so future mirror scans include it.
  - Disposition: active
  - Finding class: cross-consumer
...
```json
{
  ...
  "planSeed": [
    "T-001 Address DR-001-P1-001 at .opencode/skill/system-spec-kit/scripts/spec/create.sh:1123",
    "T-002 Address DR-002-P1-001 at .opencode/skill/system-spec-kit/scripts/tests/workflow-invariance.vitest.ts:93",
    "T-003 Address DR-003-P1-001 at .opencode/skill/system-spec-kit/scripts/tests/scaffold-golden-snapshots.vitest.ts:49",
    "T-004 Address DR-004-P1-001 at .opencode/skill/system-spec-kit/scripts/utils/template-structure.js:156",
    "T-005 Address DR-005-P1-001 at .opencode/agent/create.md:141"
  ],
  "findingClasses": [
    "cross-consumer"
  ],
...
1. **P1: Phase-parent scaffolding still emits private manifest vocabulary into generated public spec text** — Fix `.opencode/skill/system-spec-kit/scripts/spec/create.sh:1123`; Replace the generated scope text with taxonomy-neutral wording such as child phase list or phase map, and extend workflow-invariance coverage to rendered phase-parent output.
2. **P1: Workflow-invariance test path-allowlists the public surfaces it claims to gate** — Fix `.opencode/skill/system-spec-kit/scripts/tests/workflow-invariance.vitest.ts:93`; Remove the broad public-root allowlist or replace it with narrow documented exceptions, then add a default-public-surface sentinel proving banned vocabulary fails inside .opencode/agent or .opencode/command/spec_kit.
3. **P1: Golden scaffold snapshots miss create-time phase-parent rendering regressions** — Fix `.opencode/skill/system-spec-kit/scripts/tests/scaffold-golden-snapshots.vitest.ts:49`; Add a golden/sentinel test that executes create.sh phase-parent scaffolding (or the same post-render substitution function) and asserts the rendered spec.md contains no private vocabulary; remove the migration normalization for T-122/T-123 leak strings from the golden path once manifest templates are the source of truth.
4. **P1: Shell validator rules skip manifest-declared lazy docs** — Fix `.opencode/skill/system-spec-kit/scripts/utils/template-structure.js:156`; Split required-presence validation from present-file structure validation: keep check-files on required docs, but let template-source/header/section rules inspect existing manifest-declared lazy/optional docs as well, matching the MCP validator's document set.
5. **P1: Create agent prompt was missed by the cross-runtime agent vocabulary cleanup** — Fix `.opencode/agent/create.md:141`; Rename the create.md section heading to ROUTING SCAN and add .opencode/agent/create.md to the agent cleanup/resource-map coverage so future mirror scans include it.
...
- Require AI-facing agent cleanup to cover every exposed agent file, including `.opencode/agent/create.md`.

## Plan Seed

1. Update `.opencode/skill/system-spec-kit/scripts/spec/create.sh:1123` or its corresponding test/consumer surface to resolve `DR-001-P1-001`, then add regression evidence that covers same-class paths.
2. Update `.opencode/skill/system-spec-kit/scripts/tests/workflow-invariance.vitest.ts:93` or its corresponding test/consumer surface to resolve `DR-002-P1-001`, then add regression evidence that covers same-class paths.
3. Update `.opencode/skill/system-spec-kit/scripts/tests/scaffold-golden-snapshots.vitest.ts:49` or its corresponding test/consumer surface to resolve `DR-003-P1-001`, then add regression evidence that covers same-class paths.
4. Update `.opencode/skill/system-spec-kit/scripts/utils/template-structure.js:156` or its corresponding test/consumer surface to resolve `DR-004-P1-001`, then add regression evidence that covers same-class paths.
5. Update `.opencode/agent/create.md:141` or its corresponding test/consumer surface to resolve `DR-005-P1-001`, then add regression evidence that covers same-class paths.
...
| agent_cross_runtime | fail | Iteration 005 found `.opencode/agent/create.md` still uses legacy heading vocabulary. |
