...
5. Copied `.opencode/skills/system-spec-kit/templates` and `.opencode/skills/system-spec-kit/scripts/templates/compose.sh` into `/tmp/template-experiment/`, ran the composer there, and diffed generated `spec.md` outputs against committed `templates/level_N/spec.md` files.
6. Inspected `.opencode/skills/system-spec-kit/scripts/templates/compose.sh`, `.opencode/skills/system-spec-kit/templates/core/spec-core.md`, `.opencode/skills/system-spec-kit/templates/addendum/level3-arch/decision-record.md`, and `.opencode/skills/system-spec-kit/templates/addendum/level3-plus-govern/spec-level3plus-suffix.md` to locate exact repair sites.
...
- `.opencode/skills/system-spec-kit/templates/level_1/spec.md` contains a 22-line `_memory.continuity` block under frontmatter.
...
- File: `.opencode/skills/system-spec-kit/scripts/templates/compose.sh`
...
- `.opencode/skills/system-spec-kit/templates/core/spec-core.md:25` uses `| **Level** | [1/2/3/3+] |`.
- `.opencode/skills/system-spec-kit/scripts/templates/compose.sh:201-202` only replaces `[1/2/3]`.
...
- `.opencode/skills/system-spec-kit/scripts/templates/compose.sh:403-407` extracts core body from `^## 1.` and open questions from `^## 7.`, which drops `<!-- ANCHOR:metadata -->` and `<!-- ANCHOR:questions -->`.
- `.opencode/skills/system-spec-kit/templates/level_3/spec.md` has `<!-- ANCHOR:metadata -->` at line 62 and `<!-- ANCHOR:questions -->` at line 147.
- `.opencode/skills/system-spec-kit/templates/level_3+/spec.md` has the same metadata anchor and a broad `questions` anchor spanning lines 147-274.
...
- `.opencode/skills/system-spec-kit/templates/level_3+/spec.md` has `approval-workflow`, `compliance-checkpoints`, `stakeholder-matrix`, and `change-log` anchors.
- `.opencode/skills/system-spec-kit/templates/addendum/level3-plus-govern/spec-level3plus-suffix.md:71-107` has the matching sections but no anchor comments.
- `.opencode/skills/system-spec-kit/templates/addendum/level3-plus-govern/spec-level3plus.md:17-82` has anchors, but with different names for three sections (`compliance`, `stakeholders`, `changelog`) and is not the suffix file used by `compose.sh`.

Repair:

- File: `.opencode/skills/system-spec-kit/templates/addendum/level3-plus-govern/spec-level3plus-suffix.md`
- Areas: lines 71-107.
...
- `.opencode/skills/system-spec-kit/templates/addendum/level3-arch/decision-record.md:3` currently says `description: "not \"A decision was required regarding the selection of an appropriate approach.\" -->"`.
- `.opencode/skills/system-spec-kit/templates/level_3/decision-record.md` expects `description: "Decision record template for documenting architectural choices, alternatives, consequences, and implementation notes."`

Repair:

- File: `.opencode/skills/system-spec-kit/templates/addendum/level3-arch/decision-record.md`
- Area: line 3.
...
- `.opencode/skills/system-spec-kit/scripts/templates/compose.sh:52-58` hardcodes `$TEMPLATES_DIR/level_1`, `$TEMPLATES_DIR/level_2`, `$TEMPLATES_DIR/level_3`, and `$TEMPLATES_DIR/level_3+`.
...
1. Add temp output support in `.opencode/skills/system-spec-kit/scripts/templates/compose.sh`.
   - Edit lines 52-58, 794-825, and the help text.
   - Add `SPECKIT_TEMPLATE_OUT_ROOT` and `--out-root`.
   - Estimated diff: +30/-6.
   - Risk: infrastructure-low.

2. Add legacy continuity injection in `.opencode/skills/system-spec-kit/scripts/templates/compose.sh`.
...
3. Repair metadata level substitution in `.opencode/skills/system-spec-kit/scripts/templates/compose.sh`.
   - Edit lines 194-203.
...
4. Preserve Level 3 and Level 3+ anchor spans in `.opencode/skills/system-spec-kit/scripts/templates/compose.sh`.
...
5. Add Level 3+ governance anchors in `.opencode/skills/system-spec-kit/templates/addendum/level3-plus-govern/spec-level3plus-suffix.md`.
   - Edit lines 71-107.
   - Add eight anchor comment lines around sections 12-15.
   - Estimated diff: +8/-0.
   - Risk: semantic-medium.

6. Fix decision-record metadata in `.opencode/skills/system-spec-kit/templates/addendum/level3-arch/decision-record.md`.
   - Edit line 3.
   - Estimated diff: +1/-1.
   - Risk: cosmetic-low.

7. Add the parity gate from iteration 5.
   - Add `.opencode/skills/system-spec-kit/scripts/tests/template-rendered-parity.vitest.ts`.
   - Update `.opencode/skills/system-spec-kit/scripts/package.json` with `test:template-parity`.
...
- `compose.sh` currently hardcodes output dirs under `.opencode/skills/system-spec-kit/templates/level_N`.
