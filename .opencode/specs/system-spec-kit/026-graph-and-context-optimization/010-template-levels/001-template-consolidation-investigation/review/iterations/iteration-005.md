...
| File | Read | Purpose |
|---|---|---|
| `.opencode/agent/` (dir) | Glob | Canonical agent inventory (11 .md files) |
...
| `.opencode/agent/code.md` | Full read (492L) | Canonical code agent baseline |
...
| `.opencode/command/create/assets/*.yaml` | Grep + Read | Stale template-path search |
| `.opencode/skill/` (dir) | Grep | Verify manifest .tmpl `packet_pointer` references use `level_2/3/3+` |
...
1. **Code agent terminology drift — canonical `.opencode/` says "resolved route" / "router-selected guidance set"; mirrors say "detected stack" / "overlay"** — `.opencode/agent/code.md:179` vs `.claude/agents/code.md:179` / `.gemini/agents/code.md:179`; `.opencode/agent/code.md:311` vs `.claude/agents/code.md:311` / `.gemini/agents/code.md:311`
   ...
   **Scope proof**: Side-by-side diff: `.opencode/agent/code.md:179` vs `.claude/agents/code.md:179`; `.opencode/agent/code.md:311` vs `.claude/agents/code.md:311`; `.gemini/agents/code.md:179,311` match `.claude/`; `.codex/agents/code.toml:154,280` match `.opencode/`.
   ...
   **Claim Adjudication**:
   ```json
   {
     ...
     "evidenceRefs": [
       ".opencode/agent/code.md:179",
       ".claude/agents/code.md:179",
       ".gemini/agents/code.md:179",
       ".opencode/agent/code.md:311",
...
2. **Stale `templates/level_1/` paths in create-agent workflow YAML files** — `.opencode/command/create/assets/create_agent_auto.yaml:310-311` and `.opencode/command/create/assets/create_agent_confirm.yaml:343-344`
   ...
   **Affected surface hints**: [`.opencode/command/create/assets/create_agent_auto.yaml`, `.opencode/command/create/assets/create_agent_confirm.yaml`, `/create:agent` command]
   ...
   **Claim Adjudication**:
   ```json
   {
     ...
     "evidenceRefs": [
       ".opencode/command/create/assets/create_agent_auto.yaml:310-311",
       ".opencode/command/create/assets/create_agent_confirm.yaml:343-344"
     ],
...
3. **`.codex/agents/` uses TOML format while `.opencode/`, `.claude/`, `.gemini/` use Markdown** — `.codex/agents/code.toml:1-8` vs `.opencode/agent/code.md:1-20`
   ...
   **Scope proof**: `.codex/agents/code.toml:1-8` uses TOML; `.opencode/agent/code.md:1-20` uses YAML frontmatter + Markdown; `.claude/agents/code.md:1-20` same; `.gemini/agents/code.md:1-20` same.
...
4. **`.claude/agents/code.md:40` and `.gemini/agents/code.md:40` contain OpenCode-runtime-specific language** — `.claude/agents/code.md:40`, `.gemini/agents/code.md:40`
   ...
   **Scope proof**: `.claude/agents/code.md:40` says "OpenCode runtime layer"; `.gemini/agents/code.md:40` says "OpenCode runtime layer"; `.opencode/agent/code.md:40` is correct for its runtime.
...
| templates/level_1/ references (stale) | **FAIL** | 2 hits in `.opencode/command/create/assets/*.yaml` (P1-005-002) |
...
- **Command integration**: `.opencode/command/create/assets/create_agent_auto.yaml` and `create_agent_confirm.yaml` — stale template references (P1-005-002)
...
- **`sk-code` skill routing** consistently referenced as `.opencode/skill/sk-code/SKILL.md` across all 4 runtimes (correct canonical path)
