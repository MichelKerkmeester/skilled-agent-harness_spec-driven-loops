...
_memory:
  continuity:
    packet_pointer: "main/009-rm-8-prompt-hardening"
...
| **Parent Spec** | ../spec.md |
| **Phase** | 9 of 9 |
| **Predecessor** | 008-z-archive-marker-validation-sweep |
| **Successor** | None |
...
**Changelog**: When this phase closes, refresh the matching file in `../changelog/` using parent packet `010` plus this phase folder name `009-rm-8-prompt-hardening`.
<!-- /ANCHOR:phase-context -->
...
- **SC-001**: Post-edit dispatch on `013-doctor-update-orchestrator` phase parent under `cli-opencode + deepseek/deepseek-v4-pro` produces no writes outside `013-doctor-update-orchestrator/review/` (verified by `git status -- 013-doctor-update-orchestrator/` having no surprises during/after the 10-iteration run).
