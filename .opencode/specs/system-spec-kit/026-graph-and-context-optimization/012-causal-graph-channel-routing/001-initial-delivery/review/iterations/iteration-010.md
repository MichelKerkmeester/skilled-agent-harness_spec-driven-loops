...
- **Resolution:** The changelog file `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/changelog/changelog-026-012-causal-graph-channel-routing.md` exists on disk at review time (confirmed by resource-map status=OK and handover listing). The finding from iter 4 may have been valid at that point in time but was resolved by subsequent file creation.
