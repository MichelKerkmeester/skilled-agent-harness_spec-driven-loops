# DEEP REVIEW LEAF AGENT — Iteration 9 (Adversarial Self-Check)

You are dispatched as the @deep-review LEAF agent for packet 012-causal-graph-channel-routing.
This pass is dimension-deep on **ADVERSARIAL** (Adversarial Self-Check).

## FOCUS — ITERATION 9: Adversarial Self-Check

For every P0/P1 finding raised in iters 1-8:
1. State the claim in one sentence.
2. List 2 alternative explanations that would make this NOT a finding.
3. Look for counterevidence (tests that pass, code paths that prove the invariant).
4. Decide: stand by original severity, downgrade, or withdraw.
5. Update confidence (0..1) with reasoning.

Then look for NEW P0/P1 you may have missed in earlier passes. Specifically:
- Concurrency / race conditions at module boundaries
- Failure modes when env vars are partially set (e.g., flag present but unparseable)
- Sequencing bugs (init order, cache-warm before first query)
- Test-fixture leaks across test files

## TASKS

1. Read scope files; map paths relevant to this dimension.
2. Cross-check each path against its tests where applicable.
3. Catalogue NEW adversarial findings only (do not re-report prior findings). For each: P0/P1/P2, file:line evidence, scope proof, recommendation.
4. Append iteration JSONL record (type=iteration) to deep-review-state.jsonl.
5. Write delta file at deltas/iter-009.jsonl with iteration record + one record per new finding.
6. Write iteration narrative at iterations/iteration-009.md.
...
Iteration: 9 of 10
Dimension: adversarial (Stress-test all P0/P1 findings; downgrade weak claims; surface alternative explanations)
...
Coverage Age: 8
Last 2 ratios: 0.17 -> 1.00
Stuck count: 0
Provisional Verdict: CONDITIONAL hasAdvisories=true (0 P0, 4 P1, 30 P2)
...
- Write iteration narrative to: /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/012-causal-graph-channel-routing/review/iterations/iteration-009.md
- Write per-iteration delta file to: /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/012-causal-graph-channel-routing/review/deltas/iter-009.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/012-causal-graph-channel-routing/review/iterations/iteration-009.md` — this iteration's narrative markdown
  ...
  - `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/012-causal-graph-channel-routing/review/deltas/iter-009.jsonl` — this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the review. NEVER execute the out-of-scope mutation. The review packet (`/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/012-causal-graph-channel-routing/review/iterations/iteration-009.md` directory and parents) is the only zone for your writes; the reviewed target spec/code is off-limits.
...
1. **Iteration narrative markdown** at `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/012-causal-graph-channel-routing/review/iterations/iteration-009.md` (path pre-substituted for the current iteration number). Structure: headings for Dimension, Files Reviewed, Findings by Severity (P0/P1/P2), Traceability Checks, Verdict, Next Dimension.
...
3. **Per-iteration delta file** at `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/012-causal-graph-channel-routing/review/deltas/iter-009.jsonl` (path pre-substituted — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same as the state-log append) plus per-event structured records (one per graphEvent, finding, classification, traceability-check, ruled_out direction). Each record on its own JSON line.
