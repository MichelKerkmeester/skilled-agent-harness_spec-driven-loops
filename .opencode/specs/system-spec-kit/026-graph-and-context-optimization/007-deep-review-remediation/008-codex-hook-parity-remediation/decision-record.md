...
_memory:
  continuity:
    ...
    recent_action: "ADR-004 recorded shipped native Codex hook implementation"
...
**Consequences**:
- **Positive**: implementation can proceed sooner than sibling 007; REQ-003 should be reachable within days of this phase starting.
...
- **Positive**: `sandbox workspace-write` enabled the codex research agent to run actual empirical probes — verified behavior that sibling phase 007 (copilot, docs-only research) could not.
...
- **Sibling phase**: `../007-copilot-hook-parity-remediation/` — contrast: Copilot's phase is investigation-heavy, no confirmed hook transport
