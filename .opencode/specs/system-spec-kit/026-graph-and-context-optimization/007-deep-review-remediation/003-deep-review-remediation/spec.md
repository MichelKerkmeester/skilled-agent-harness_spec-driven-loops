...
| **DR-P1-005** | D7 | Fix workspace build command; correct Copilot integration model; regenerate playbook denominator; align Codex status; correct measurement artifact name | `../../../../../skill/system-spec-kit/references/hooks/skill-advisor-hook.md`, `README.md`, `../../../../../skill/system-spec-kit/mcp_server/skill-advisor/feature_catalog/feature_catalog.md`, `../../../../../skill/system-spec-kit/mcp_server/skill-advisor/manual_testing_playbook/manual_testing_playbook.md`, `../../../../../skill/system-spec-kit/scripts/observability/LIVE_SESSION_WRAPPER_SETUP.md` |
...
5. **DR-P1-005 — Operator docs**
   - Workspace build command updated to a command that works in this checkout (e.g., `npm --prefix ../../../../../skill/system-spec-kit/mcp_server run build`) OR the workspace is properly defined in root `package.json`
   - Copilot integration doc reflects callback-based model per `../../../../../skill/system-spec-kit/scripts/observability/LIVE_SESSION_WRAPPER_SETUP.md`
...
7. **AC-7 Docs executable**: Given a fresh checkout, when operator runs the command from `../../../../../skill/system-spec-kit/references/hooks/skill-advisor-hook.md` §Setup, then the build succeeds (or the doc is updated to a command that does succeed).
8. **AC-8 Test suite green**: All existing tests + new regression tests pass (`npm --prefix ../../../../../skill/system-spec-kit/mcp_server test`).
...
### Source (modify)
- `../../../../../skill/system-spec-kit/mcp_server/skill-advisor/scripts/skill_advisor.py` — add `--stdin` mode
- `../../../../../skill/system-spec-kit/mcp_server/lib/skill-advisor/subprocess.ts` — stdin-based spawn
- `../../../../../skill/system-spec-kit/mcp_server/lib/skill-advisor/skill-advisor-brief.ts` — tokenCap propagation, cache key maxTokens, provenance rebuild on hit
- `../../../../../skill/system-spec-kit/mcp_server/lib/skill-advisor/render.ts` — expose sanitizer OR re-export for envelope
- `../../../../../skill/system-spec-kit/mcp_server/lib/skill-advisor/prompt-cache.ts` — size cap + insertion eviction
- `../../../../../skill/system-spec-kit/mcp_server/lib/skill-advisor/normalize-adapter-output.ts` — remove alias OR JSDoc it
- `../../../../../skill/system-spec-kit/mcp_server/lib/context/shared-payload.ts` — strong label sanitizer at envelope boundary
- `../../../../../skill/system-spec-kit/mcp_server/hooks/{claude,gemini,copilot,codex}/user-prompt-submit.ts` — renderer call-site if tokenCap plumbing needed
- `../../../../../skill/system-spec-kit/scripts/observability/smart-router-measurement.ts` — separate static default stream
- `../../../../../skill/system-spec-kit/scripts/observability/live-session-wrapper.ts` — finalize API + skill-identity preservation
- `../../../../../skill/system-spec-kit/scripts/observability/smart-router-telemetry.ts` — session-level aggregation
- `../../../../../skill/system-spec-kit/scripts/observability/smart-router-analyze.ts` — promptId grouping + baseline-SKILL handling
- `.opencode/plugins/spec-kit-skill-advisor.js` — shared disable flag, SIGKILL escalation, source-signature cache key

### Tests (modify + add)
- `../../../../../skill/system-spec-kit/mcp_server/tests/advisor-privacy.vitest.ts` — spawn argv assertion
- `../../../../../skill/system-spec-kit/mcp_server/tests/advisor-subprocess.vitest.ts` — error-code + stdin coverage
- `../../../../../skill/system-spec-kit/mcp_server/tests/advisor-renderer.vitest.ts` — envelope instruction-label rejection
- `../../../../../skill/system-spec-kit/mcp_server/tests/advisor-brief.vitest.ts` — cache-key maxTokens, cache-hit provenance
- `../../../../../skill/system-spec-kit/mcp_server/tests/advisor-runtime-parity.vitest.ts` — plugin 5th path, end-to-end hook→builder
- `../../../../../skill/system-spec-kit/mcp_server/tests/spec-kit-skill-advisor-plugin.vitest.ts` — disable flag, SIGKILL, negative paths
- `../../../../../skill/system-spec-kit/mcp_server/tests/telemetry-*.vitest.ts` — path precedence, report writer

### Docs (modify)
- `../../../../../skill/system-spec-kit/references/hooks/skill-advisor-hook.md` — workspace command, Codex status
- `../../../../../skill/system-spec-kit/mcp_server/skill-advisor/README.md` — workspace command, integration note
- `../../../../../skill/system-spec-kit/mcp_server/skill-advisor/feature_catalog/feature_catalog.md` — measurement artifact name alignment
- `../../../../../skill/system-spec-kit/mcp_server/skill-advisor/manual_testing_playbook/manual_testing_playbook.md` — regenerate denominator, align Codex/Copilot status
- `../../../../../skill/system-spec-kit/scripts/observability/LIVE_SESSION_WRAPPER_SETUP.md` — if command names change
