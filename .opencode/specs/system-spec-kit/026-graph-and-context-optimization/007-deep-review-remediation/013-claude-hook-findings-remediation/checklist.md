...
- [x] CHK-050 [P1] Temp files in `scratch/` only during spike and `/tmp` during smokes; all were removed after evidence was summarized (Evidence: `find .../013-claude-hook-findings-remediation -maxdepth 2 -type f` has no scratch files) [EVIDENCE: implementation-summary.md]
