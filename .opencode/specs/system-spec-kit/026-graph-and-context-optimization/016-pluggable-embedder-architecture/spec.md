---
title: "016: Pluggable embedder architecture (phase parent)"
description: "Phase parent for making mk-spec-memory embedder-agnostic. End users swap models with one MCP call; no code changes, no manual schema migration. First concrete swap: mxbai-embed-large-v1 to close packet 008 cat-24/409."
trigger_phrases:
  - "016 pluggable embedder architecture"
  - "embedder adapter interface"
  - "any embedder zero migration"
  - "mxbai-embed-large swap"
importance_tier: "important"
contextType: "implementation"
_memory:
  continuity:
    packet_pointer: "system-spec-kit/026-graph-and-context-optimization/016-pluggable-embedder-architecture"
...
| Phase | Folder | Runtime | Outcome |
|-------|--------|---------|---------|
| 001 | `001-embedder-adapter-interface` | **Native Claude `@code`** (TypeScript design + same-context iteration) | `EmbedderAdapter` interface + `EmbedderRegistry` lookup. Pure types + small registry; no runtime wiring yet. |
| 002 | `002-ollama-backend-and-multi-dim-schema` | **cli-codex (gpt-5.5 reasoning_effort=high fast)** | Ollama HTTP-API adapter + lazy `vec_<dim>` table creation + active-embedder pointer in settings. The two-layer swap mechanism. |
| 003 | `003-embedder-mcp-tools-and-reindex` | **cli-devin (SWE-1.6, paired dispatch)** | `embedder_list` / `embedder_set` / `embedder_status` MCP tools + background re-index orchestrator. User-facing surface. |
| 004 | `004-mxbai-swap-and-008-closure` | **cli-opencode (`--model deepseek/deepseek-v4-pro --pure`)** | First concrete swap to mxbai-embed-large. Verify cat-24/409 reaches PASS. Verify 008's 56 PASS scenarios preserved. Close the 008/115 follow-on. |
<!-- /ANCHOR:sub-phase-list -->
