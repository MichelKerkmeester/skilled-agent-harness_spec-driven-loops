---
title: Deep Research Strategy — 011-post-stress-finding-remediation-research
...
**Source of evidence**:
- `../../010-stress-test-close-loop-measurement-rerun/findings.md` Recommendations §1-5
- `../../010-stress-test-close-loop-measurement-rerun/runs/{cell}/{cli}-N/score.md` per-cell scores (especially I1/cli-copilot for P0; Q1 cells for P1; pre-flight for P2; S2/cli-opencode for opportunity)
- `../../010-stress-test-close-loop-measurement-rerun/runs/preflight.log`
- Spec folders: `004-memory-save-rewrite/` (planner-first), `005-code-graph-fail-fast-routing/` (fallbackDecision matrix)
...
1. Read `../../010-stress-test-close-loop-measurement-rerun/findings.md` Recommendations §1-5 verbatim
2. Read `../../010-stress-test-close-loop-measurement-rerun/runs/I1/cli-copilot-1/score.md` (load-bearing for Q-P0)
3. Read `../../010-stress-test-close-loop-measurement-rerun/runs/I1/cli-codex-1/score.md` + `I1/cli-opencode-1/score.md` (compare correct-Gate-3 behavior)
4. Read `../../004-cocoindex-overfetch-dedup-rerank/spec.md` and the fork at `.opencode/skills/mcp-coco-index/mcp_server/cocoindex_code/` to ground Q-OPP
...
- `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/003-mcp-runtime-stress-remediation/010-stress-test-close-loop-measurement-rerun/findings.md`
- 30 score.md files in `010-stress-test-close-loop-measurement-rerun/runs/`

**Closely related closed packets** (read but don't modify):
- `003-memory-context-truncation-telemetry-contract` — token-budget envelope (Q-OPP indirect)
- `004-cocoindex-overfetch-dedup-rerank` — fork at version 0.2.3+spec-kit-fork.0.2.0; 7 new telemetry fields (Q-OPP primary)
- `005-code-graph-fail-fast-routing` — `fallbackDecision.nextTool` matrix shipped in `mcp_server/code_graph/handlers/query.ts:775-796` (Q-P1 primary)
- `006-causal-graph-relation-window-metrics` — production tuning deferred (Q-ARCH candidate)
- `007-intent-classifier-stability-telemetry` — sorted-token paraphrase grouping; v2 embedding-based deferred (Q-ARCH candidate)
- `008-mcp-daemon-rebuild-protocol` — daemon-readiness contract (Q-P2 indirect)
- `009-memory-search-citation-response-policy` — refusal contract for low-quality searches (Q-P0 indirect — policy ratified server-side)
