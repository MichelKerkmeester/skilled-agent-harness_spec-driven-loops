...
>
> **Frozen v1.0.1 baseline**: `../001-search-intelligence-stress-playbook/002-search-scenario-execution/findings.md` (do not modify above the trailing forward-pointer line).
>
...
- **Corpus**: 9 scenarios verbatim from `../001-…/001-search-scenario-design/spec.md` §Scenario Corpus.
...
3. **cli-copilot's `/memory:save` contract still bypassed by I1** (P0): copilot interprets "save the context for this conversation" as authorization to execute `memory_save` directly, selecting a target spec folder from session-bootstrap context (e.g., `004-legacy-phase-parent-migration`) without asking. This was less catastrophic than v1.0.1 (234s vs 9.6min, no 2.1MB output bloat), but the underlying Gate 3 HARD BLOCK bypass persists. The packet 004 planner-first contract is correctly enforced for codex and opencode.
...
1. **(P0) — cli-copilot /memory:save contract bypass**
   - Evidence: I1/cli-copilot 2/8 (Δ=+1 marginal recovery from 1/8). Mutated `004-legacy-phase-parent-migration/graph-metadata.json` without operator authorization.
...
3. **(P2) — Code-graph file-watcher debounce**
   ...
   - Owner: 005-code-graph-fail-fast-routing follow-up or 024-compact-code-graph maintenance packet.
