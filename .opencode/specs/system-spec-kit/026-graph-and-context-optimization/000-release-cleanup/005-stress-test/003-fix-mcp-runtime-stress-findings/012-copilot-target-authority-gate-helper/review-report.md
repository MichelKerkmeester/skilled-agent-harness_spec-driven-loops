...
- **Files:**
  - research.md §3.5 bullet 4 — `.opencode/specs/.../011-research-post-stress-finding-followups/research/research.md:95-98`
  - `.opencode/skills/system-spec-kit/mcp_server/tests/executor-config-copilot-target-authority.vitest.ts:37-243`
  - `.opencode/specs/.../012-copilot-target-authority-gate-helper/implementation-summary.md:136`
...
- **Recommended fix:** Add a temp-dir/stub-copilot replay test that uses the I1 "save context" prompt and asserts (a) no write-capable flag is present in argv when authority is missing, and (b) no files in the temp dir mutate. Alternatively, gate merge on a manual live-dispatch verification log captured into `010-stress-test-close-loop-measurement-rerun/runs/`.

### P1 — Out-of-scope changes present in the working tree

- **Files (out of packet 012 scope):**
  - `.opencode/specs/.../003-fix-mcp-runtime-stress-findings/spec.md:114-116` (parent references 013–015)
  - `.opencode/specs/.../003-fix-mcp-runtime-stress-findings/description.json` and `graph-metadata.json`
  ...
  - `.opencode/specs/.../013-code-graph-degraded-stress-cell/spec.md:1-4` (and 014/015 packet docs)
