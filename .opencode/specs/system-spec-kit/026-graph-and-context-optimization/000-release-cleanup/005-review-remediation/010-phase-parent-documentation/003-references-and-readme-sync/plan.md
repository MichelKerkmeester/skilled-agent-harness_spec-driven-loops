...
**Given** I run `/memory:save` against `003-references-and-readme-sync`, **when** I read `010/graph-metadata.json` afterwards, **then** `derived.last_active_child_id` is `system-spec-kit/026-graph-and-context-optimization/010-phase-parent-documentation/003-references-and-readme-sync` and `derived.last_active_at` is fresh.
<!-- /ANCHOR:testing -->
