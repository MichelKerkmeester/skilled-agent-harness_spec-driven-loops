...
|-----------|-------------|-------------|
| `../043-hook-plugin-per-runtime-testing/runners/common.ts` | Modify | Add sandbox detection, `SKIPPED_SANDBOX`, root resolution, and run-output path. |
| `../043-hook-plugin-per-runtime-testing/runners/run-all-runtime-hooks.ts` | Modify | Detect sandbox once, pass it to adapters, and print aggregate verdict. |
| `../043-hook-plugin-per-runtime-testing/runners/test-*.ts` | Modify | Emit direct-smoke and live-cli cells separately. |
| `../043-hook-plugin-per-runtime-testing/runners/README.md` | Modify | Document operator live mode and sandbox partial mode. |
| `.opencode/skill/system-spec-kit/mcp_server/package.json` | Modify | Add `hook-tests` script using the local TS loader. |
| `../043-hook-plugin-per-runtime-testing/findings.md` | Modify | Add corrected amendment while preserving original verdict. |
