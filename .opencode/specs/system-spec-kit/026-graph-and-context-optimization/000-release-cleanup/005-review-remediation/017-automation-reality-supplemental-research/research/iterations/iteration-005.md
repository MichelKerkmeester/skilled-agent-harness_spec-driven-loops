...
|----|---------|----------------|----------|--------|
| P1-1 / F2.9 | Structural code graph watcher claim lacks a structural source watcher; actual behavior is read-path freshness plus manual scan. | P1 | `.opencode/skill/system-spec-kit/mcp_server/lib/ops/file-watcher.ts:274`; `.opencode/skill/system-spec-kit/mcp_server/code_graph/handlers/query.ts:1087` | 032 |
| P1-2 / F4.11 | Retention metadata writes `delete_after`, but no sweep path over `memory_index.delete_after` was found. | P1 | `.opencode/skill/system-spec-kit/mcp_server/lib/session/session-manager.ts:737`; `.opencode/skill/system-spec-kit/mcp_server/handlers/memory-bulk-delete.ts:183` | 033 |
| F-013-011 | CCC command coverage maps CCC tools to `/memory:manage`, but the command cannot invoke them. | P1 | `.opencode/command/memory/README.txt:271-273`; `.opencode/command/memory/manage.md:1-4` | 031 |
| F-013-018 | Post-write spec validation is documented as an agent obligation but not enforced by runtime hooks. | P1 | `AGENTS.md:334`; `.opencode/skill/system-spec-kit/SKILL.md:63` | 031 |

### P2 findings

| ID | Finding | Final severity | Evidence | Packet |
|----|---------|----------------|----------|--------|
| F-013-012 | CCC architecture docs point to stale handler paths. | P2 | `.opencode/skill/system-spec-kit/ARCHITECTURE.md:306-308`; `.opencode/skill/system-spec-kit/mcp_server/code_graph/handlers/index.ts:9-11` | 031 |
| F-013-019 | Completion validation "auto-runs" wording is over-broad; actual enforcement is workflow plus CLI invocation. | P2 | `AGENTS.md:146`; `.opencode/skill/system-spec-kit/scripts/spec/validate.sh:87` | 031 |
| F-013-015 | Context-save auto-generation is not cross-runtime parity. | P2 | `.opencode/skill/system-spec-kit/mcp_server/hooks/claude/session-stop.ts:94`; `.opencode/skill/system-spec-kit/mcp_server/hooks/gemini/session-stop.ts:97` | 031 |
| NEW-013-001 | Session cleanup is real auto maintenance, but not retention sweeping. | P2 | `.opencode/skill/system-spec-kit/mcp_server/lib/session/session-manager.ts:202` | 033 |
| NEW-013-002 | Code graph read-path freshness can self-heal selective stale slices, but full scans remain manual. | P2 | `.opencode/skill/system-spec-kit/mcp_server/code_graph/handlers/context.ts:163` | 032 |
| NEW-013-003 | Skill graph scan has auto paths, while `skill_graph_validate` is manual diagnostic coverage. | P2 | `.opencode/skill/system-spec-kit/mcp_server/context-server.ts:1507` | 034 |
| NEW-013-004 | `memory_drift_why` is manual-only and requires a specific memory id. | P2 | `.opencode/skill/system-spec-kit/mcp_server/tools/causal-tools.ts:23` | 035 |
| NEW-013-005 | `memory_get_learning_history` is read-only; no automated feedback-loop consumer was found. | P2 | `.opencode/skill/system-spec-kit/mcp_server/handlers/session-learning.ts:659` | 035 |
| NEW-013-006 | `advisor_status` detects freshness but does not run its rebuild helper. | P2 | `.opencode/skill/system-spec-kit/mcp_server/skill_advisor/handlers/advisor-status.ts:86` | 034 |
| NEW-013-007 | `code_graph_context` self-checks readiness only after requested; routing recommends rather than invokes it. | P2 | `.opencode/skill/system-spec-kit/mcp_server/code_graph/tools/code-graph-tools.ts:60` | 032 |
| NEW-013-008 | `memory_health` is recommendation-driven and repairs only with explicit confirmation. | P2 | `.opencode/skill/system-spec-kit/mcp_server/handlers/memory-crud-health.ts:425` | 035 |
...
|----|---------|--------------|-----------|
| P1-1 / F2.9 | validated | `.opencode/skill/system-spec-kit/mcp_server/lib/ops/file-watcher.ts:274`; `.opencode/skill/system-spec-kit/mcp_server/code_graph/handlers/query.ts:1087` | Watchers found are markdown/spec-doc or skill graph scoped; structural graph freshness is read-path/manual. |
| P1-2 / F4.11 | validated | `.opencode/skill/system-spec-kit/mcp_server/lib/session/session-manager.ts:737`; `.opencode/skill/system-spec-kit/mcp_server/handlers/memory-bulk-delete.ts:183` | Cleanup infrastructure exists, but not for governed memory retention over `delete_after`. |
