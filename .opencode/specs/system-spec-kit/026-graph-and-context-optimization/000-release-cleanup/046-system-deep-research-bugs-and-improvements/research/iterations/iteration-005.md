...
## Focus
Audited schema validation gaps around `JSON.parse`, zod boundary validation, unsafe post-parse casts, and path traversal handling under `.opencode/skills/system-spec-kit/mcp_server/`. The emphasis was on public MCP boundaries and parse results that can affect reads, writes, routing, or output contracts.
