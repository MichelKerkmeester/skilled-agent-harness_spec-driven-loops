...
| Does Codex hook freshness smoke emit `stale: true, reason: "timeout-fallback"`? | Yes in the direct hook fallback path. Normal-shell live verdict remains a cross-runtime evidence concern from 045/005, not a failure of this marker code. | `hooks/codex/user-prompt-submit.ts:174-203`; `hooks-codex-freshness.vitest.ts:39-72`; `016-hook-plugin-per-runtime-testing/run-output/latest/codex-user-prompt-submit-freshness-direct-smoke.jsonl:1`. |
