...
- `.opencode/skills/system-spec-kit/scripts/rules/*.sh`
...
- `.opencode/skills/system-spec-kit/scripts/dist/spec/is-phase-parent.js`
...
Evidence:
- `.opencode/skills/system-spec-kit/scripts/utils/template-structure.js:190` extracts every `^##` header without tracking fenced code blocks.
- `.opencode/skills/system-spec-kit/scripts/utils/template-structure.js:549` loads anchors by regex from the full document body, again without fence awareness.
- `.opencode/skills/system-spec-kit/scripts/rules/check-anchors.sh:128` and `.opencode/skills/system-spec-kit/scripts/rules/check-anchors.sh:132` grep anchor tags from the whole file.
...
Evidence:
- `.opencode/skills/system-spec-kit/scripts/rules/check-template-headers.sh:73` records any extra header as a warning.
- `.opencode/skills/system-spec-kit/scripts/rules/check-template-headers.sh:125` returns `warn` when only extra headers exist.
- `.opencode/skills/system-spec-kit/scripts/spec/validate.sh:959` exits non-zero for warnings in strict mode.
...
Evidence:
- `.opencode/skills/system-spec-kit/scripts/spec/validate.sh:299` through `.opencode/skills/system-spec-kit/scripts/spec/validate.sh:318` append only `rule`, `status` and `message` to `RESULTS`.
- Rule details are printed separately through `log_detail`, for example `.opencode/skills/system-spec-kit/scripts/spec/validate.sh:574` and `.opencode/skills/system-spec-kit/scripts/spec/validate.sh:660`, but they are not serialized into JSON.
...
Evidence:
- The packet target lists `.opencode/skills/system-spec-kit/shared/lib/shell-common.sh`.
- The actual sourced file is `.opencode/skills/system-spec-kit/scripts/lib/shell-common.sh`; `validate.sh` sources it at `.opencode/skills/system-spec-kit/scripts/spec/validate.sh:31`.
- The detector implementation lives at `.opencode/skills/system-spec-kit/scripts/lib/shell-common.sh:48`.
...
Evidence:
- `.opencode/skills/system-spec-kit/scripts/rules/check-links.sh:86` defaults to `.opencode/skill`.
- `.opencode/skills/system-spec-kit/scripts/rules/check-links.sh:87` skips unless `SPECKIT_VALIDATE_LINKS=true`.
- `.opencode/skills/system-spec-kit/scripts/rules/check-links.sh:71` scans every skill markdown file, not the active packet or changed files.
- Probe: enabling `SPECKIT_VALIDATE_LINKS=true` while validating the otherwise passing 037/004 packet failed strict due unrelated broken links in `.opencode/skills/mcp-figma/nodes/*.md`.
...
Question answers:
- Do `is_phase_parent` and `isPhaseParent` always agree? In the real-folder sample, yes: 1,550 checked, 0 divergences. The two implementations also share the same child regex and populated-child condition at `.opencode/skills/system-spec-kit/scripts/lib/shell-common.sh:57` and `.opencode/skills/system-spec-kit/mcp_server/lib/spec/is-phase-parent.ts:12`.
...
- Does FRONTMATTER_MEMORY_BLOCK reject narrative violations? Yes. The rule rejects `because`, multiple sentences, URLs, markdown bullets and values over 96 characters at `.opencode/skills/system-spec-kit/mcp_server/lib/validation/spec-doc-structure.ts:612`.
