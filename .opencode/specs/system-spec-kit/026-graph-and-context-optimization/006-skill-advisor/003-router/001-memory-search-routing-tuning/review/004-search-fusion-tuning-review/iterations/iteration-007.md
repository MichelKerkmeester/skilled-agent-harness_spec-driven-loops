...
|----|----------|---------|----------|
| DRV-P1-005 | P1 | `description.json` still ends `parentChain` at `010-search-and-routing-tuning`, while `graph-metadata.json` resolves the packet under `001-memory-search-routing-tuning`. | [SOURCE: description.json:15-19]; [SOURCE: graph-metadata.json:3-5]; [SOURCE: description.json:31-38] |
