...
### 4.1 P0 (Blocker)
1. `bash .opencode/skill/system-spec-kit/scripts/spec/validate.sh .opencode/specs/.../027-.../002-daemon-freshness-foundation --strict --no-recursive` completes with exit 0 or 1 (no SyntaxError, no ReferenceError, no "Cannot use import", no "require is not defined")
...
1. **AC-1** Run `bash .opencode/skill/system-spec-kit/scripts/spec/validate.sh .opencode/specs/.../027-.../002-daemon-freshness-foundation --strict --no-recursive` under Node 25 → exits 0 or 1 with no SyntaxError/ReferenceError in stderr
...
4. **AC-4** `node dist/validation/evidence-marker-lint.js --folder .../027-.../002-daemon-freshness-foundation --json` exits 0 and prints valid JSON (or exits 1 with parseable error JSON)
