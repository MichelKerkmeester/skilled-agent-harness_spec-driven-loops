...
**Iteration 5 (audited).** Canonical invocation: `cd .opencode/skills/system-skill-advisor/mcp_server && npm exec -- vitest run tests/...` (verified: 49 tests / 4 files pass). The stale `npm --prefix .opencode/skills/system-spec-kit/mcp_server exec -- vitest run skill-advisor/tests/...` is a pre-extraction path (tests moved to `system-skill-advisor/mcp_server/tests/`). Audit found exactly **two** affected scenarios: NC-004 (`01--native-mcp-tools/ambiguous-brief-rendering.md:38`) and NC-005 (`lifecycle-redirect-metadata.md:36`).
