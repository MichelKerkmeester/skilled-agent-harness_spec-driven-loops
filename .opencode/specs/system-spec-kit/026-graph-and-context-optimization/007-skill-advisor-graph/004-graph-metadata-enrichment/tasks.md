...
- [x] T034 Run strict packet validation after the rewrite and confirm the packet closes without error status (packet verification) [Evidence: `bash .opencode/skill/system-spec-kit/scripts/spec/validate.sh .opencode/specs/system-spec-kit/026-graph-and-context-optimization/007-skill-advisor-graph/004-graph-metadata-enrichment --strict` exits without error status]
<!-- /ANCHOR:phase-3 -->
