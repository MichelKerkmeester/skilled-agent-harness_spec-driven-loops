...
You can now read this packet and get the live package shape directly: `feature_catalog/`, `manual_testing_playbook/`, and `scripts/` sit under `.opencode/skill/skill-advisor/`, while the packet metadata points at the root catalog, the root playbook, and the main runtime scripts by exact path.
