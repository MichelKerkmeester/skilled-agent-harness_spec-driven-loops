...
Assess whether External Project' `query`, `context`, and `impact` tools should influence Public Code Graph's MCP surface.

## Actions Taken

- Read External Project MCP tool definitions for query, context, and impact.
...
- External Project `query` is concept-oriented and returns process-grouped results, not just raw symbol or file matches. [SOURCE: external/src/mcp/tools.ts:49]
- External Project `context` is a 360-degree symbol view with callers, callees, references, and process participation, with disambiguation support. [SOURCE: external/src/mcp/tools.ts:172]
- External Project `impact` is explicitly framed as a pre-change blast-radius tool with depth groups, affected processes, modules, and risk levels. [SOURCE: external/src/mcp/tools.ts:285]
...
- Public `code_graph_context` already accepts CocoIndex/manual/graph seeds and emits LLM-oriented neighborhoods, which is a good foundation for External Project-style richer context. [SOURCE: .opencode/skill/system-spec-kit/mcp_server/code-graph/handlers/context.ts:166]
...
- What External Project ingestion, graph schema, process-flow, and query/context patterns are portable to Public's Code Graph package?
...
- external/src/mcp/tools.ts:49
- external/src/mcp/tools.ts:172
...
Evaluate External Project safety features: `detect_changes`, `impact`, and `rename`.
