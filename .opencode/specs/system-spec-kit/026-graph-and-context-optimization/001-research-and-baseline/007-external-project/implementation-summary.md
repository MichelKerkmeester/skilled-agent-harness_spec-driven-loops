...
title: "Implementation Summary: External Project Deep Research Closeout"
description: "Completed a 10-iteration External Project deep-research packet and converted the evidence into a decision matrix, ownership boundaries, and follow-up implementation packet proposals."
trigger_phrases:
  - "git nexus research summary"
  - "007-external-project summary"
...
_memory:
  continuity:
    ...
    key_files:
      ...
      - "research/007-external-project-pt-02/research.md"
      - "research/007-external-project-pt-02/deep-research-dashboard.md"
      - "research/007-external-project-pt-02/findings-registry.json"
      - "research/007-external-project-pt-02/resource-map.md"
      - "spec.md"
      - "plan.md"
      - "tasks.md"
      - "checklist.md"
    ...
    session_dedup:
      ...
      sibling_packets:
        - "research/007-external-project-pt-01 (Opus + cli-codex mix, status:complete, ratios 0.46→0.35→0.22)"
        - "research/007-external-project-pt-02 (cli-codex gpt-5.5 high fast, status:complete, ratios 0.84→...→0.34, 99 graph events, 267 SOURCE citations)"
    completion_pct: 95
    open_questions: []
    answered_questions:
      - "External Project should inform Public through selective adaptation, not direct reuse"
...
| Field | Value |
|-------|-------|
| **Spec Folder** | 007-external-project |
...
This packet now contains a completed 10-iteration External Project deep-research run. You can read the final synthesis to see which External Project graph ideas Public should adopt, adapt, reject, or defer, with separate recommendations for Code Graph, Spec Kit Memory, and Skill Graph.

### Research Artifacts

The workflow produced iteration notes, delta logs, a reducer dashboard, a findings registry, a resource map, and a final synthesis under `research/007-external-project-pt-01/`.
...
The workflow used `sk-deep-research` state conventions, two read-only sidecar agents, direct file reads, JSONL deltas, and reducer-generated observability. External Project and Public implementation source files stayed read-only.
<!-- /ANCHOR:how-delivered -->
...
| Decision | Why |
|----------|-----|
| Adapt External Project selectively | Public already has graph, memory, and skill systems; adaptation preserves ownership boundaries |
...
| Feed Skill Graph through existing graph-causal lane | Public already has Skill Advisor graph-causal scoring, so a new External Project-specific lane would duplicate responsibility |
...
| Graph metadata refresh after save | PASS: `backfill-graph-metadata.js --root .../007-external-project` refreshed `graph-metadata.json` |
<!-- /ANCHOR:verification -->
...
2. **No license/legal review was completed.** Treat External Project as architecture inspiration unless a later packet verifies copying constraints.
