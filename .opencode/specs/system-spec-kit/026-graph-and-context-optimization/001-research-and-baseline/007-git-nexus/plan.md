...
title: "Implementation Plan: GitNexus Deep Research Closeout"
description: "Plan for running and closing a 10-iteration GitNexus deep-research workflow that compares GitNexus graph patterns with Public Code Graph, Spec Kit Memory causal graph, and Skill Graph surfaces."
trigger_phrases:
  - "git nexus research plan"
  - "gitnexus deep research"
  - "007-git-nexus plan"
...
_memory:
  continuity:
    ...
    recent_action: "Closed out 10-iteration GitNexus deep research workflow"
    ...
    open_questions: []
    answered_questions:
      - "Which GitNexus patterns should Public adopt, adapt, reject, or defer?"
---
# Implementation Plan: GitNexus Deep Research Closeout
...
| **Storage** | Spec packet files under `.opencode/specs/.../007-git-nexus/` |
| **Testing** | JSONL parse checks, reducer pass, targeted spec validation, full packet validation |

### Overview

This plan covers a research-only workflow. The work runs 10 deep-research iterations over the downloaded GitNexus source, reduces the iteration state, synthesizes recommendations, and saves the packet for later implementation planning. No GitNexus or Public implementation source files are changed.
<!-- /ANCHOR:summary -->
...
- [x] Spec folder established at `007-git-nexus`.
- [x] GitNexus source confirmed under `external/`.
- [x] User requested `/spec_kit:deep-research:auto`, 10 iterations, and parallel research support.

### Definition of Done

- [x] Ten iteration notes exist under `research/007-git-nexus-pt-01/iterations/`.
- [x] Ten delta logs exist under `research/007-git-nexus-pt-01/deltas/`.
...
- [x] Keep GitNexus source read-only.
...
- **Iteration files**: `research/007-git-nexus-pt-01/iterations/iteration-001.md` through `research/007-git-nexus-pt-01/iterations/iteration-010.md` preserve evidence and reflection.
- **Delta files**: `deltas/iter-001.jsonl` through `iter-010.jsonl` preserve machine-readable findings.
- **Reducer outputs**: `research/007-git-nexus-pt-01/findings-registry.json`, `research/007-git-nexus-pt-01/deep-research-dashboard.md`, and `research/007-git-nexus-pt-01/resource-map.md` summarize convergence.
- **Final synthesis**: `research/007-git-nexus-pt-01/research.md` turns evidence into decisions and follow-up packet proposals.

### Data Flow

GitNexus and Public source evidence feeds iteration notes and deltas. The reducer reads JSONL state and iteration files to update registry, dashboard, strategy, and resource map. The final synthesis reads the reduced evidence and writes a compact summary back to `spec.md`.
<!-- /ANCHOR:architecture -->
...
- [x] Resolve artifact root to `research/007-git-nexus-pt-01/`.
...
|------------|------|--------|-------------------|
| GitNexus source under `external/` | External local source | Available | Research cannot cite GitNexus internals |
...
- **Procedure**: Remove or replace only the affected research packet artifacts in `007-git-nexus`, rerun JSONL parse checks, rerun reducer, and rerun validation. Do not modify `external/` source.
<!-- /ANCHOR:rollback -->
