...
title: "Decision Record: GitNexus Deep Research Closeout"
description: "Architecture decision record for adopting GitNexus ideas as selective research recommendations instead of direct code reuse."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Documented GitNexus research adoption decision"
    ...
    open_questions: []
    answered_questions:
      - "Selective adaptation chosen over direct GitNexus reuse"
---
# Decision Record: GitNexus Deep Research Closeout
...
<!-- ANCHOR:adr-001 -->
## ADR-001: Adapt GitNexus Patterns Selectively
...
We needed to decide whether GitNexus should influence Public's Code Graph, Spec Kit Memory causal graph, and Skill Graph systems. GitNexus has overlapping graph, impact, route/tool, shape, and group-contract capabilities, but Public already has working SQLite graph storage, memory causal edges, and Skill Advisor graph-causal scoring.

### Constraints

- This packet is research-only and must not implement changes.
- GitNexus source under `external/` remains read-only.
...
**We chose**: Adapt GitNexus architecture patterns selectively and reject direct wholesale reuse.
...
|--------|------|------|-------|
| **Selective adaptation** | Preserves Public ownership boundaries and uses strongest GitNexus ideas | Requires follow-up implementation work | 9/10 |
| Direct GitNexus storage or source reuse | Faster if compatible | Runtime mismatch, license risk, and duplicated graph responsibilities | 3/10 |
| Ignore GitNexus | Avoids scope expansion | Misses useful evidence-backed patterns | 2/10 |
| One shared graph for code, memory, and skills | Simple conceptual model | Blurs ownership and weakens retrieval semantics | 4/10 |

**Why this one**: Selective adaptation keeps useful GitNexus lessons while respecting Public's existing systems and validation surfaces.
<!-- /ANCHOR:adr-001-alternatives -->
...
|---|-------|--------|----------|
| 1 | **Necessary?** | PASS | User requested research to avoid missing useful GitNexus ideas |
...
- `research/007-git-nexus-pt-01/research.md` records final recommendations and follow-up packet proposals.
