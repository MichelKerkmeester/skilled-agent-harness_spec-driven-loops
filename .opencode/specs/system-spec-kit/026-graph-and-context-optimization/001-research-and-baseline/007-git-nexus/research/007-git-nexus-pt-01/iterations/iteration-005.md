...
Evaluate GitNexus route/API/tool affordances as candidates for Code Graph safety and Skill Graph entrypoint modeling.
...
- GitNexus `route_map` maps API routes to handlers and consumers, which is directly useful for pre-change API work. [SOURCE: external/gitnexus/src/mcp/tools.ts:400]
- GitNexus `tool_map` shows MCP/RPC tool definitions, handler files, and descriptions. [SOURCE: external/gitnexus/src/mcp/tools.ts:423]
- GitNexus `shape_check` compares route response keys against consumer property accesses, but sidecar evidence warns that unknown coverage can disappear when routes lack both shape and consumer data. [SOURCE: external/gitnexus/src/mcp/tools.ts:439]
- GitNexus `api_impact` combines route map, shape check, and impact data into one route-handler pre-change report. [SOURCE: external/gitnexus/src/mcp/tools.ts:462]
- Tool nodes and `HANDLES_TOOL` edges are especially relevant to Skill Graph because they can model command/tool entrypoints as graph nodes instead of passive text metadata. [SOURCE: external/gitnexus/src/core/ingestion/pipeline-phases/tools.ts:77]
...
- external/gitnexus/src/mcp/tools.ts:423
- external/gitnexus/src/mcp/tools.ts:439
- external/gitnexus/src/mcp/tools.ts:462
