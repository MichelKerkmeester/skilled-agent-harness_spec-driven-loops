# Deep-Research Iteration 5 — 020 Wave-3 Validation (cli-copilot)
...
Iteration: 5 of 20
Focus: **V5 Runtime-specific edge cases 006/007/008**

## TASKS

1. Read 006/007/008 spec.md §6 Risks + §8 Edge Cases.
2. Surface real-world edge cases NOT captured:
   - Claude: session resume after network interruption mid-hook?
   - Gemini: hook schema version mismatch across Gemini CLI versions?
   - Copilot: SDK availability matrix — which Copilot versions expose onUserPromptSubmitted return object? What if it was added/removed in a version?
   - Codex: defensive parse — stdin AND argv both empty? Both present with different content?
3. Check fixture coverage: does each adapter have fixtures for all its documented edge cases?
4. Write iteration-005.md with per-runtime edge-case gap list + severity tags.
...
- Iter narrative: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/research/020-skill-advisor-hook-surface-001-initial-research-003-implementation-plan-validation-copilot/iterations/iteration-005.md`
- Per-iter delta: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/research/020-skill-advisor-hook-surface-001-initial-research-003-implementation-plan-validation-copilot/deltas/iter-005.jsonl`

## CONSTRAINTS

- Soft cap 8 tool calls, hard max 12
- Iter 5 focus: V5 Runtime-specific edge cases 006/007/008 ONLY — do not drift
...
1. `iterations/iteration-005.md` — focused findings for iter 5 with severity-tagged items and file citations
2. Canonical JSONL appended to state.jsonl (exact shape):
   `{"type":"iteration","iteration":5,"newInfoRatio":0.80,"status":"in_progress","focus":"V5 Runtime-specific edge cases 006/007/008","findingsCount":N,"keyQuestions":10,"answeredQuestions":M,"timestamp":"2026-04-19T...Z","durationMs":N,"graphEvents":[]}`
   (Adjust newInfoRatio honestly based on actual novelty — 0.80 is a guidance target for this iter position.)
3. `deltas/iter-005.jsonl` — structured per-iter delta with findings list

## NOTE ON CONVERGENCE

This is iter 5 of 20. If validation findings have saturated and no new P0/P1 gaps surface, set `newInfoRatio` low (below 0.05). Be honest — don't force novelty.
