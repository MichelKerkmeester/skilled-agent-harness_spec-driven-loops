# Deep-Research Iteration 8 — 020 Wave-3 Validation (cli-copilot)
...
Iteration: 8 of 20
Focus: **V8 Migration compatibility**

## TASKS

1. Read wave-2 research-extended.md §X7 migration semantics.
2. Read 003 spec.md §3 (per-skill fingerprints, generation counter, rename/delete suppression).
3. Read 004 spec.md §3 (HMAC exact prompt cache).
4. Scenarios to validate:
   - Hook lands in an existing session where skill-graph.sqlite is stale — correct behavior?
   - User edits sk-code-opencode/SKILL.md mid-session — does generation increment + cache invalidate?
   - Skill renamed sk-git → sk-git-flow — does old slug get suppressed from cache immediately?
   - First session after hook lands: no prior cache — graceful cold start?
   - Prior generation counter file missing or corrupt — fallback?
5. Check if 003/004 specs address each scenario explicitly.
6. Write iteration-008.md with migration-scenario coverage table + severity tags.
...
- Iter narrative: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/research/020-skill-advisor-hook-surface-001-initial-research-003-implementation-plan-validation-copilot/iterations/iteration-008.md`
- Per-iter delta: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/research/020-skill-advisor-hook-surface-001-initial-research-003-implementation-plan-validation-copilot/deltas/iter-008.jsonl`

## CONSTRAINTS

- Soft cap 8 tool calls, hard max 12
- Iter 8 focus: V8 Migration compatibility ONLY — do not drift
...
1. `iterations/iteration-008.md` — focused findings for iter 8 with severity-tagged items and file citations
2. Canonical JSONL appended to state.jsonl (exact shape):
   `{"type":"iteration","iteration":8,"newInfoRatio":0.60,"status":"in_progress","focus":"V8 Migration compatibility","findingsCount":N,"keyQuestions":10,"answeredQuestions":M,"timestamp":"2026-04-19T...Z","durationMs":N,"graphEvents":[]}`
   (Adjust newInfoRatio honestly based on actual novelty — 0.60 is a guidance target for this iter position.)
3. `deltas/iter-008.jsonl` — structured per-iter delta with findings list

## NOTE ON CONVERGENCE

This is iter 8 of 20. If validation findings have saturated and no new P0/P1 gaps surface, set `newInfoRatio` low (below 0.05). Be honest — don't force novelty.
