# Test Prompts for 014-code-graph-upgrades — All CLIs
...
for t in 1 2 3 4 5 6 7; do
  codex exec --dangerously-bypass-approvals-and-sandbox -m gpt-5.4 \
    ...
    "$(sed -n "/^### T${t}-CX/,/^### T/p" .opencode/specs/system-spec-kit/026-graph-and-context-optimization/014-code-graph-upgrades/scratch/test-prompts-all-clis.md | head -n -1 | tail -n +3 | tr -d '`')" \
