...
- [ ] Existing `/doctor:code-graph.md` + 4 YAML assets fully read (canonical pattern source)
...
- [ ] All 21 YAML asset files load + canonical-path-validate
...
**Markdown entrypoint + per-mode YAML asset.** Established by `/doctor:code-graph`. Each command has one `*.md` file at `commands/doctor/` defining frontmatter, Execution Protocol, Constraints, and Unified Setup Phase. Setup Phase resolves all inputs (mode, scope, flags) in a single consolidated user prompt, then loads the corresponding YAML from `assets/`. The YAML executes a phased workflow with per-phase canonical-path validation, mutation boundaries, and rollback.

### Key Components

- **Markdown layer** (per-command `*.md`) — frontmatter + Execution Protocol + Constraints + Unified Setup Phase + Reference Sections.
- **YAML layer** (per-mode `*.yaml`) — phased workflow with `validate_targets`, `snapshot`, `mutate`, `post-verify`, `rollback` phases.
...
### Mode-Suffix Routing (council Q5)

| Mode | Behavior | Snapshot? | Default Prompts? |
|------|----------|-----------|------------------|
| `(no suffix)` | Tier-aware confirm | Mandatory | Tiered: short auto, medium combined, long-pole explicit |
| `:auto` | Autonomous | Mandatory (override `--no-snapshot`) | None |
| `:confirm` | Per-step prompt | Mandatory | Per-step |
| `:apply` | Autonomous + skip-status | Mandatory | None |
| `:apply-confirm` | Interactive + skip-status | Mandatory | Per-step |

### Data Flow

```
/doctor:update <mode> [flags]
   ↓
Markdown Setup Phase: resolve mode + flags + status preview
   ↓
...
- 4 YAML assets (`assets/doctor_<name>_{auto,confirm,apply,apply-confirm}.yaml`) — ~50 LOC each

**B1. `/doctor:memory`** — context-index FTS+vector orchestrator
**B2. `/doctor:causal-graph`** — causal-edges integrity + auto-link
...
- 5 YAML assets (`assets/doctor_update_{default,auto,confirm,apply,apply-confirm}.yaml`) — ~80 LOC each
...
7. Step failure: one retry → rollback (`:auto`) or prompt (`:confirm`)
8. SIGINT: graceful tx-commit + snapshot-restore
9. Post-run gold-battery + auto-rollback in `:auto`
...
| G2 | YAML canonical-path validator per new asset (21 files) |
| G3 | `validate.sh 013-... --strict` exit 0 |
| G4 | `/doctor:update :auto --no-snapshot` smoke on this repo |
| G5 | `/doctor:update :confirm` interactive UX with forced-failure injection |
| G6 | Concurrent `/doctor:update :auto` invocations refused |
...
- /Users/.../Public/.opencode/commands/doctor/assets/doctor_code-graph_auto.yaml (autonomous mode YAML)
- /Users/.../Public/.opencode/commands/doctor/assets/doctor_code-graph_confirm.yaml (interactive mode YAML)
- /Users/.../Public/.opencode/commands/doctor/assets/doctor_code-graph_apply.yaml (apply mode YAML)
- /Users/.../Public/.opencode/commands/doctor/assets/doctor_code-graph_apply-confirm.yaml (interactive apply YAML)
...
4. Snapshot before any mutation in :apply* modes
5. Post-verify gold-battery before claiming success
6. Auto-rollback on regression in autonomous mode
...
- **`/doctor:code-graph` YAML phases** (`assets/doctor_code-graph_*.yaml`) — `validate_targets`, `snapshot`, `mutate`, `post-verify`, `rollback`. Reuse pattern, customize per-subsystem MCP tool calls.
- **Canonical-path validator** (`doctor_skill-advisor_auto.yaml:49-70`) — realpath + glob match before any write. Reuse verbatim.
...
Phase 12 (causal-graph-channel-routing) shipped the `causal_edges` table at 1,328 edges with 46.14% coverage. The 60% target is therefore reachable but requires `/doctor:causal-graph :apply` to land first. This packet does NOT raise coverage — it ships the command that raises it. Coverage measurement is post-rebuild gold-battery only.
<!-- /ANCHOR:reuse -->
...
- Markdown + per-mode YAML pattern reused from /doctor:code-graph
