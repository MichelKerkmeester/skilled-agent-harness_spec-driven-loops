...
|------|---------------|-------|
| `013-doctor-update-orchestrator/` (parent listing) | PASS — lean trio | 3 lean files + 2 cross-cutting docs + 2 child dirs + review/. No heavy doc leak. |
| `013-doctor-update-orchestrator/spec.md:180` | PASS — phase parent spec | REQ-P-001 enforces lean trio; REQ-P-002 requires `last_active_child_id`. Phase map correct. |
| `013-doctor-update-orchestrator/graph-metadata.json:223` | SUSPECT | `last_active_child_id: null` (line 220) — already tracked as R2-P1-001. `last_save_at` set but child pointer never populated. |
| `013-doctor-update-orchestrator/description.json:30` | PASS | childTopology correct; parentChain complete. |
| `013-doctor-update-orchestrator/resource-map.md:206` | PASS | Parent aggregate lists only `.opencode/commands/doctor/` paths; no spurious cross-runtime claims. |
| `013-doctor-update-orchestrator/handover.md:276` | SUSPECT | 002-sandbox section line 100 false claim about last_active_child_id. |
...
| `001-doctor-commands/resource-map.md:168` | PASS | Correctly lists only `.opencode/commands/doctor/` paths. No cross-runtime claims. |
...
- **File**: `013-doctor-update-orchestrator/spec.md:88-95`
