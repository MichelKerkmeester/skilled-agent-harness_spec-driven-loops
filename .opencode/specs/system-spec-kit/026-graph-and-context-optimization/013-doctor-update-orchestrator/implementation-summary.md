---
title: "Implementation Summary: 013 Doctor Update Orchestrator (PARTIAL — Phase A + A.1 + Track B1 reference template complete)"
description: "Phase A scaffold + Phase A.1 tx-model verification + Track B1 reference template (/doctor:memory.md + auto.yaml) complete. Phase B2-B4, C, D, E pending — handoff instructions + dispatch design captured below."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Completed Phase A scaffold + Phase A.1 tx-model verification + Track B1 reference template (/doctor:memory.md + auto.yaml)"
    next_safe_action: "Dispatch Phase B2-B4 via cli-opencode using plan.md §5 design + doctor_memory_auto.yaml as template"
    blockers: []
    key_files:
      - "spec.md"
      - "plan.md"
...
<!-- SPECKIT_LEVEL: 2 -->
<!-- STATUS: PARTIAL — Phase A + A.1 complete; Track B1 reference template authored; B2-E pending -->
...
Six Level 2 packet docs authored under `026/013-doctor-update-orchestrator/`:
...
- `026/graph-metadata.json` `derived.last_active_child_id` updated to `013-doctor-update-orchestrator`
...
| `commands/doctor/assets/doctor_memory_auto.yaml` | ✅ AUTHORED | ~140 LOC; read-only diagnostic mode template |
| `commands/doctor/assets/doctor_memory_confirm.yaml` | ⏳ PENDING | Same flow as auto.yaml + interactive review gates |
| `commands/doctor/assets/doctor_memory_apply.yaml` | ⏳ PENDING | Adds Phase 2 (snapshot via VACUUM INTO) + Phase 3 (memory_index_scan) + Phase 4 (gold-battery) + Phase 5 (rollback-on-regression) |
| `commands/doctor/assets/doctor_memory_apply-confirm.yaml` | ⏳ PENDING | Same as apply.yaml + per-phase user-approval gates |
<!-- /ANCHOR:what-built -->
...
Author the remaining 3 YAML modes for `/doctor:memory` using `doctor_memory_auto.yaml` as the structural template:

- `doctor_memory_confirm.yaml` — copy auto.yaml; add `interactive_gates` block before each phase emitting a confirmation prompt
- `doctor_memory_apply.yaml` — start from auto.yaml; replace `intent: DIAGNOSE` with `intent: APPLY`; add Phase 2 snapshot, Phase 3 mutation, Phase 4 post-verify (gold-battery), Phase 5 rollback. Reference `doctor_code-graph_apply.yaml` for the snapshot/post-verify/rollback shape.
- `doctor_memory_apply-confirm.yaml` — copy apply.yaml; add interactive gates before each phase.
...
|-------|---------|------|-----------------------|
| B2 | `/doctor:causal-graph` | spec.md §3.2 | Add-only mutation boundary; coverage target ≥60%; auto-link confidence threshold ≥0.7 |
...
**Recommended approach**: Dispatch via cli-opencode tracks per plan.md §5 (Dispatch Design). Use `doctor_memory_auto.yaml` + `commands/doctor/memory.md` as the canonical template references in each dispatch prompt. Estimated wall-clock: 1-2 hours per track in parallel; ~6 hours serial.

### Phase C — `/doctor:update` orchestrator (6 files)
...
- `commands/doctor/update.md` — Markdown entrypoint with all 5 modes (default/auto/confirm/apply/apply-confirm) + 5 flags (--force, --no-snapshot, --cleanup-legacy, --migrate, --keep-snapshots) + tier-aware routing
- `assets/doctor_update_default.yaml` — tier-aware confirm (no suffix path)
- `assets/doctor_update_auto.yaml` — autonomous all-step
- `assets/doctor_update_confirm.yaml` — per-step prompt
- `assets/doctor_update_apply.yaml` — autonomous + skip-status (assume rebuild needed)
- `assets/doctor_update_apply-confirm.yaml` — interactive + skip-status

Each YAML implements the 10-phase orchestrator workflow per ADR-002..ADR-008 in `decision-record.md`. Estimated wall-clock: 4 hours.
...
Per checklist.md `<!-- ANCHOR:smoke-tests -->` and tasks.md T-047..T-055. Includes:
- G1 `validate_document.py --type command` per new `*.md` (5 files)
- G2 YAML canonical-path validator per asset (21 files)
...
TARGET: /doctor:<name> command + 4 YAML assets
...
- /Users/.../Public/.opencode/commands/doctor/assets/doctor_memory_auto.yaml
...
- /Users/.../Public/.opencode/commands/doctor/assets/doctor_code-graph_apply.yaml (canonical apply mode with snapshot+rollback)

IN_SCOPE_FILES:
- /Users/.../Public/.opencode/commands/doctor/<name>.md
- /Users/.../Public/.opencode/commands/doctor/assets/doctor_<name>_{auto,confirm,apply,apply-confirm}.yaml
...
4. Snapshot via VACUUM INTO before any mutation in :apply* modes
5. Post-verify gold-battery before claiming success
6. Auto-rollback on regression in :auto modes; prompt in :confirm modes
...
| G2 (YAML canonical-path validator) | 🟡 1 of 21 | `doctor_memory_auto.yaml` authored; canonical-path validation deferred to Phase E |
| G3 (strict spec-folder validate on 013) | ⏳ pending | Run after all 6 packet docs settle (post-implementation-summary update) |
| G4 (`/doctor:update :auto --no-snapshot` smoke) | ⏳ pending | Requires Phase C complete |
| G5 (`/doctor:update :confirm` UX) | ⏳ pending | Requires Phase C complete |
...
1. **Track B1 yaml completion** (3 files: confirm, apply, apply-confirm) — straightforward extension of `doctor_memory_auto.yaml`; estimated 1 hour.
...
| `.opencode/commands/doctor/assets/doctor_memory_auto.yaml` | 140 | Read-only diagnostic mode template (Track B1) |

### Modified (parent metadata)

| Path | Change |
|------|--------|
| `026/graph-metadata.json` | `derived.last_active_child_id` ← `013-doctor-update-orchestrator`; manual fields preserved |
<!-- /ANCHOR:files-touched -->
...
2. **Read** `commands/doctor/memory.md` and `assets/doctor_memory_auto.yaml` (canonical templates for the remaining authoring)
3. **Read** `commands/doctor/code-graph.md` and its 4 YAML assets (canonical write-mutation pattern source)
