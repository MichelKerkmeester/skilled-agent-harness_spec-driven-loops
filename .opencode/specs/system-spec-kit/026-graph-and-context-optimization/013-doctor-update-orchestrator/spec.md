...
_memory:
  continuity:
    ...
    next_safe_action: "Author plan.md, tasks.md, checklist.md, resource-map.md, decision-record.md"
    blockers: []
    key_files:
    ...
    open_questions:
      ...
      - "Should /doctor:update default mode be tier-aware confirm or :auto? (Council recommended tier-aware confirm.)"
    answered_questions:
...
| **Successor** | None |
| **Handoff Criteria** | All 5 new doctor commands pass `validate_document.py --type command` and exhibit clean YAML loads. `/doctor:update :auto` end-to-end smoke test on this repo detects 2 stale subsystems (causal-edges <60%, deep-loop-graph empty) and offers remediation. Concurrent dispatch refused via flock. Strict spec-folder validate exits 0. |
<!-- /ANCHOR:metadata -->
...
**Scope Boundary**: Author 5 new commands (4 isolated + 1 unified orchestrator) + 21 YAML assets (4 per isolated command + 5 for the orchestrator) + 1 migration manifest + (optional) per-version migration scripts. Does NOT modify any MCP server code beyond surfacing existing tools through the new commands.
...
- Existing canonical-path validator pattern (per `doctor_skill-advisor_auto.yaml:49-70`)

**Deliverables**:
- 4 isolated doctor commands: `/doctor:memory`, `/doctor:causal-graph`, `/doctor:deep-loop`, `/doctor:cocoindex`
- 1 unified orchestrator: `/doctor:update` with mode-aware tier prompts
- 1 migration manifest (`migration-manifest.json`) for users skipping versions
- 21 YAML asset files (16 per-mode for isolated + 5 for orchestrator)
...
4. **Honors operational safety** (council anchor lens) — flock-based concurrent dispatch protection, `VACUUM INTO` snapshots, graceful SIGINT cancellation, MCP-client-activity probe, tier-aware default mode.
<!-- /ANCHOR:problem -->
...
- **21 YAML assets** under `Public/.opencode/commands/doctor/assets/`:
  - 16 per-mode files for isolated commands (4 modes × 4 commands)
  - 5 per-mode files for orchestrator (default/auto/confirm/apply/apply-confirm)
...
| `.opencode/commands/doctor/assets/doctor_memory_{auto,confirm,apply,apply-confirm}.yaml` | Create | 4 per-mode YAML assets |
| `.opencode/commands/doctor/causal-graph.md` | Create | `/doctor:causal-graph` Markdown entrypoint |
| `.opencode/commands/doctor/assets/doctor_causal-graph_{auto,confirm,apply,apply-confirm}.yaml` | Create | 4 per-mode YAML assets |
| `.opencode/commands/doctor/deep-loop.md` | Create | `/doctor:deep-loop` Markdown entrypoint |
| `.opencode/commands/doctor/assets/doctor_deep-loop_{auto,confirm,apply,apply-confirm}.yaml` | Create | 4 per-mode YAML assets |
| `.opencode/commands/doctor/cocoindex.md` | Create | `/doctor:cocoindex` Markdown entrypoint |
| `.opencode/commands/doctor/assets/doctor_cocoindex_{auto,confirm,apply,apply-confirm}.yaml` | Create | 4 per-mode YAML assets |
| `.opencode/commands/doctor/update.md` | Create | `/doctor:update` orchestrator Markdown entrypoint |
| `.opencode/commands/doctor/assets/doctor_update_{default,auto,confirm,apply,apply-confirm}.yaml` | Create | 5 per-mode YAML assets |
...
| REQ-002 | All 21 YAML asset files load cleanly via the canonical-path validator pattern (per `doctor_skill-advisor_auto.yaml:49-70`) | Schema load + `realpath` validation exit 0 |
...
| REQ-006 | `VACUUM INTO` snapshot mechanism captures every `*.sqlite` before any mutation in `:apply*` modes | G7 smoke test passes (snapshot file count = sqlite file count post-`:apply`) |
...
| REQ-011 | SIGINT cancellation safety — graceful tx-commit + snapshot-restore in `:apply*` modes | G7 smoke test: SIGINT mid-rebuild produces exit 130 + snapshot restored |
...
| REQ-013 | Tier-aware default mode (no suffix) — short steps auto, medium combined-prompt, long-pole explicit-prompt-with-ETA | YAML asset routing logic; manual UX inspection |
...
| REQ-017 | Gold-battery validation per subsystem post-rebuild — regression auto-rolls back in `:auto` | Each isolated command's `:apply` YAML includes post-verify block; orchestrator chains them |
...
- **SC-002**: All 21 YAML asset files exist + load cleanly + canonical-path-validate.
- **SC-003**: `/doctor:update :auto --no-snapshot` smoke test on this repo: detects 2 stale subsystems (causal <60%, deep-loop empty), exits 0, prints dashboard.
- **SC-004**: Concurrent `/doctor:update :auto` invocations: second refused with helpful holding-PID message.
...
- **NFR-OS01**: No mutation without snapshot in `:apply*` modes (REQ-006).
...
- **NFR-I01**: Re-running `/doctor:update :auto` on a fresh repo is a no-op (status check only).
...
- **NFR-R01**: All 5 commands follow the canonical `/doctor:code-graph` shape (Markdown entrypoint + Execution Protocol + Constraints + Unified Setup Phase + per-mode YAML).
...
- 21 YAML assets matching canonical /doctor:code-graph pattern
