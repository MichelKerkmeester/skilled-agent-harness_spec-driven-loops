...
- [ADR-006: Default mode auto vs confirm (council Q5)](#adr-006-default-mode-auto-vs-confirm)
...
**Decision**: **One auto-retry with 5-sec backoff before rollback.** In `:auto` mode → restore snapshot of failed DB + downstream, log to `.doctor-update.last-run.json`, exit 1. In `:confirm` mode → prompt user with three options: `(a) retry from start, (b) restore snapshot and exit, (c) leave as-is (DANGEROUS)`.

**Rationale**: Single retry handles transient failures (disk full, OOM, race) without sacrificing safety. Auto-rollback to snapshot is mandatory in `:auto` because the cost of corrupt context-index breaking `memory_save` for a week dwarfs 5-15 min of lost rebuild work. State log enables forensic analysis after the fact.
...
## ADR-006: Default mode (auto vs confirm)

**Council question**: Should `/doctor:update` (no suffix) default to `:auto` or `:confirm`?

**Status**: Decided.

**Decision**: **Tier-aware confirm.** `/doctor:update` (no suffix) prompts only for medium and long-pole steps:
...
`:auto` opts out of all prompts (with snapshot still mandatory). `:confirm` prompts each step.
...
**Council convergence**: Lens C (UX advocate) introduced tiering after Lens A/B initial split between strict-confirm vs match-existing-pattern. Lens E (devil's advocate) wanted `:auto` default — rejected because this command is rare-use, not routine.

**Implementation**: Phase C T-028 (`assets/doctor_update_default.yaml`), T-037 (tier-aware execution).
