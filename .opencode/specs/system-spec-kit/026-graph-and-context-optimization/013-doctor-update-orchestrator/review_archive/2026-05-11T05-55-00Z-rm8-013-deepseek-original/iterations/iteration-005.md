...
Continue with **security — doctor command surface deep-scan** on the `001-doctor-commands` YAML assets (`assets/doctor_*.yaml`). The 21 YAML mode assets define `forbidden_targets` globs, field handling policies, and upstream asset declarations. Audit for:
