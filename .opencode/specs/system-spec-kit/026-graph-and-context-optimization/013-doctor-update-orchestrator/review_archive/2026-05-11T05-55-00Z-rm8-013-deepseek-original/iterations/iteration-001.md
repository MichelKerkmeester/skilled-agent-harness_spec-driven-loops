...
|------|----------------|------|
| `013-doctor-update-orchestrator/spec.md` | STABLE | Clean phase parent spec, lean trio, PHASE DOCUMENTATION MAP accurate |
| `013-doctor-update-orchestrator/description.json` | STABLE | Correct childTopology, parentChain, keywords |
| `013-doctor-update-orchestrator/graph-metadata.json` | SUSPECT | `last_active_child_id: null` (REQ-P-002 violation); `status: "planned"` vs spec "In Progress" |
| `013-doctor-update-orchestrator/handover.md` | SUSPECT | Title claims "001-doctor-commands" but file is at parent; primarily narrates v3.3 hardening on child 001 |
| `013-doctor-update-orchestrator/resource-map.md` | STABLE | Comprehensive 81-row parent aggregate, 140+ expanded paths, up-to-date |
| `001-doctor-commands/spec.md` | SUSPECT | `packet_pointer` missing `/001-doctor-commands` suffix (reads as parent); Status "Draft" vs impl "COMPLETE"; `completion_pct: 0` |
| `001-doctor-commands/plan.md` | STABLE | Well-structured 5-phase plan + dispatch design |
| `001-doctor-commands/tasks.md` | SUSPECT | All tasks still ⏳ Pending (authoring-time state), contradicting "all 5 phases delivered" claim |
| `001-doctor-commands/checklist.md` | SUSPECT | All 80+ checkboxes unchecked; no verification evidence despite COMPLETE claim |
| `001-doctor-commands/decision-record.md` | STABLE | 10 well-formed ADRs with evidence and cross-references |
| `001-doctor-commands/implementation-summary.md` | SUSPECT | Title "COMPLETE" vs body Metadata "PARTIAL (~30% complete)" — contradictory completion states |
| `001-doctor-commands/resource-map.md` | SUSPECT | All entries show PLANNED status (pre-implementation); not refreshed post-implementation |
| `001-doctor-commands/description.json` | SUSPECT | `specFolder` at parent level (`013-doctor-update-orchestrator`), should include `/001-doctor-commands` |
| `001-doctor-commands/graph-metadata.json` | STABLE | Correct child-level data; `packet_id` correctly includes `/001-doctor-commands` |
...
**Summary**: 14 STABLE, 9 SUSPECT, 1 GAP (the missing `001-doctor-commands/handover.md` documented in parent resource-map.md line 110 as intentionally removed)
...
| ai-council artifacts | parent handover.md | `001-doctor-commands/ai-council/` (10 files) |
| Parallel runtime mirrors | (cross-runtime) | `.claude/commands/doctor/`, `.codex/commands/doctor/`, `.gemini/commands/doctor/` (if they exist) |
| 001 dispatch prompts | 001 resource-map.md | `001-doctor-commands/dispatch/*.prompt.md` |
...
- **Evidence**: `derived.last_active_child_id: null`. REQ-P-002 requires this field to match the most recently saved child packet (`001-doctor-commands` or `002-sandbox-testing-playbook`). The 002 handover line 100 claims "last_active_child_id correctly tracks 002" but the field is null.
...
- **File**: `001-doctor-commands/implementation-summary.md:2,51`
...
- **File**: `001-doctor-commands/checklist.md` (all checkboxes)
...
- **File**: `001-doctor-commands/description.json:2`
- **Severity**: P1 (traceability — incorrect spec folder pointer)
- **Evidence**: `specFolder: "system-spec-kit/026-graph-and-context-optimization/013-doctor-update-orchestrator"` should be `".../013-doctor-update-orchestrator/001-doctor-commands"`. The child graph-metadata.json correctly has the child-level `spec_folder` but description.json has the parent-level. This causes spec-folder resolution to land on the parent instead of the child.
- **Finding class**: instance-only
...
- **File**: `001-doctor-commands/resource-map.md` (throughout)
