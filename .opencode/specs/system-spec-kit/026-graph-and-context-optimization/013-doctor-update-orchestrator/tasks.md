...
| T-012 | Author `assets/doctor_memory_auto.yaml` (read-only health check) | REQ-002 | `.opencode/commands/doctor/assets/doctor_memory_auto.yaml` | ⏳ Pending |
| T-013 | Author `assets/doctor_memory_confirm.yaml` (interactive health) | REQ-002 | same dir | ⏳ Pending |
| T-014 | Author `assets/doctor_memory_apply.yaml` (rebuild with snapshot + long-pole prompt) | REQ-002, REQ-006 | same dir | ⏳ Pending |
| T-015 | Author `assets/doctor_memory_apply-confirm.yaml` (interactive rebuild) | REQ-002, REQ-006 | same dir | ⏳ Pending |
...
| T-017 | Author `assets/doctor_causal-graph_auto.yaml` (stats + drift) | REQ-002 | same dir | ⏳ Pending |
| T-018 | Author `assets/doctor_causal-graph_confirm.yaml` (interactive drift) | REQ-002 | same dir | ⏳ Pending |
| T-019 | Author `assets/doctor_causal-graph_apply.yaml` (auto-link with add-only mutation boundary) | REQ-002 | same dir | ⏳ Pending |
| T-020 | Author `assets/doctor_causal-graph_apply-confirm.yaml` | REQ-002 | same dir | ⏳ Pending |
...
| T-022 | Author `assets/doctor_deep-loop_auto.yaml` (status check) | REQ-002 | same dir | ⏳ Pending |
| T-023 | Author `assets/doctor_deep-loop_confirm.yaml` | REQ-002 | same dir | ⏳ Pending |
| T-024 | Author `assets/doctor_deep-loop_apply.yaml` (rebuild from iteration set; lazy-init detection) | REQ-002 | same dir | ⏳ Pending |
| T-025 | Author `assets/doctor_deep-loop_apply-confirm.yaml` | REQ-002 | same dir | ⏳ Pending |

### B4: /doctor:cocoindex

| ID | Task | REQ | Path | Status |
|----|------|-----|------|--------|
| T-026 | Author `commands/doctor/cocoindex.md` + 4 YAML assets (4 modes; daemon coordination + idempotent restart) | REQ-001, REQ-002 | `.opencode/commands/doctor/cocoindex.md` + `assets/doctor_cocoindex_*.yaml` | ⏳ Pending |
<!-- /ANCHOR:phase-b -->
...
| T-028 | Author `assets/doctor_update_default.yaml` (tier-aware confirm; no suffix path) | REQ-002, REQ-013 | same dir | ⏳ Pending |
| T-029 | Author `assets/doctor_update_auto.yaml` (autonomous all-step) | REQ-002, REQ-004 | same dir | ⏳ Pending |
| T-030 | Author `assets/doctor_update_confirm.yaml` (per-step prompt) | REQ-002 | same dir | ⏳ Pending |
| T-031 | Author `assets/doctor_update_apply.yaml` (autonomous + skip-status; assume rebuild needed) | REQ-002 | same dir | ⏳ Pending |
| T-032 | Author `assets/doctor_update_apply-confirm.yaml` | REQ-002 | same dir | ⏳ Pending |
...
| T-048 | G2: YAML canonical-path validator per asset (21 files) → all load | REQ-002 | per-yaml validation | ⏳ Pending |
| T-049 | G3: Strict spec-folder validate on 013 packet → exit 0 | REQ-003 | `validate.sh 013-... --strict` | ⏳ Pending |
| T-050 | G4: `/doctor:update :auto --no-snapshot` smoke test → detects 2 stale subsystems (causal + deep-loop) | REQ-007 | smoke run | ⏳ Pending |
| T-051 | G5: `/doctor:update :confirm` UX test with forced-failure injection → tier prompts + retry/rollback/leave works | REQ-012, REQ-013 | smoke run | ⏳ Pending |
