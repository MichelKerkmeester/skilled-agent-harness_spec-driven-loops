...
- [ ] CHK-003 [P0] `/doctor:code-graph.md` + 4 YAML assets fully read (canonical pattern source)
...
- [ ] CHK-011 [P0] All 21 YAML assets load + canonical-path-validate (REQ-002)
...
- [ ] CHK-013 [P0] Mode-suffix routing implemented per command (`:auto`/`:confirm` minimum; `:apply`/`:apply-confirm` for mutating commands)
- [ ] CHK-014 [P0] Canonical-path validator pattern reused for every mutation boundary
- [ ] CHK-015 [P0] Pre-mutation snapshot in every `:apply*` mode (REQ-006)
- [ ] CHK-016 [P0] Post-rebuild gold-battery validation in every `:apply*` mode (REQ-017)
- [ ] CHK-017 [P0] Auto-rollback in `:auto` modes; prompt in `:confirm` (REQ-017)
...
- [ ] CHK-101 [P0] 4 YAML assets exist (auto/confirm/apply/apply-confirm)
- [ ] CHK-102 [P0] Long-pole ETA prompt (5-15 min) in `:apply` mode before rebuild
...
- [ ] CHK-105 [P1] `:auto` mode is read-only (`memory_health()` only); `:apply` mode triggers rebuild
...
- [ ] CHK-201 [P0] 4 YAML assets exist
...
- [ ] CHK-206 [P1] `:auto` mode is read-only (stats + drift report); `:apply` mode triggers auto-link
<!-- /ANCHOR:doctor-causal-graph -->
...
- [ ] CHK-301 [P0] 4 YAML assets exist
...
- [ ] CHK-305 [P1] `:auto` mode is read-only (status); `:apply` mode rebuilds from current iteration set
<!-- /ANCHOR:doctor-deep-loop -->
...
- [ ] CHK-401 [P0] 4 YAML assets exist
- [ ] CHK-402 [P0] Refuses if CocoIndex daemon unhealthy
- [ ] CHK-403 [P0] `:apply` mode triggers idempotent daemon restart per 3.4.1.0 fix
- [ ] CHK-404 [P0] Gold-battery: 3 representative semantic queries return ≥ 5 results post-rebuild
- [ ] CHK-405 [P1] `:auto` mode is read-only (status + drift); `:apply` mode reindexes via `ccc_reindex`
<!-- /ANCHOR:doctor-cocoindex -->
...
- [ ] CHK-501 [P0] 5 YAML assets exist (default/auto/confirm/apply/apply-confirm)
...
- [ ] CHK-511 [P0] Phase 9: post-run gold-battery; auto-rollback in `:auto` (REQ-017)
...
- [ ] CHK-701 [P0] G2: YAML canonical-path validator per asset exit 0 (21 files)
- [ ] CHK-702 [P0] G3: `validate.sh 013-... --strict` exit 0
- [ ] CHK-703 [P0] G4: `/doctor:update :auto --no-snapshot` smoke detects 2 stale subsystems on this repo (causal + deep-loop)
- [ ] CHK-704 [P1] G5: `/doctor:update :confirm` interactive UX with forced-failure injection works
