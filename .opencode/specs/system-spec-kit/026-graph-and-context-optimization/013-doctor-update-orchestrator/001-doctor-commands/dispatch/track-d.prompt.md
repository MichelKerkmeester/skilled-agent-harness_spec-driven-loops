...
1. **`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-doctor-update-orchestrator/spec.md` §3.5 + REQ-008** — manifest must declare 3.3.0.0, 3.4.0.0, 3.4.1.0 blocks.
2. **`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-doctor-update-orchestrator/decision-record.md` ADR-008** — detect-and-recommend semantics, never auto-delete; manifest is the source for `--cleanup-legacy` flag behavior.
...
```json
{
  ...
  "metadata": {
    ...
    "authored_by": "packet 026/013-doctor-update-orchestrator (Track D)",
