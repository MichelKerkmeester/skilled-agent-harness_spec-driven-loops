...
5. **`/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-doctor-update-orchestrator/decision-record.md`** — ADR-001 confirms `memory_index_scan` uses per-batch (per-file) transactions. SIGINT graceful cancel needs only ~5 sec settle window. Snapshot-restore is mandatory for cross-file consistency.
6. **`/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-doctor-update-orchestrator/spec.md` §3.1** — per-command specification for `/doctor:memory`.
