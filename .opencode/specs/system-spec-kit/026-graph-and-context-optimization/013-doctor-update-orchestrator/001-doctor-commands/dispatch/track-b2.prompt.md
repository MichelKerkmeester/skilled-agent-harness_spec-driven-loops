...
5. **`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-doctor-update-orchestrator/spec.md` §3.2** — per-command specification for `/doctor:causal-graph`.
6. **`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-doctor-update-orchestrator/decision-record.md`** — ADR-002..ADR-008 council answers.
7. **`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/012-causal-graph-channel-routing/spec.md`** — predecessor packet that ships causal-edges (1,328 edges, ~46% coverage). Coverage target ≥ 60% per CHK-065 from prior packet.
