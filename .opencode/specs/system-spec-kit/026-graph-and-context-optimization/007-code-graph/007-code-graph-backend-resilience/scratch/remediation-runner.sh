...
Spec folder: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/007-code-graph/007-code-graph-backend-resilience/
Skill routing: sk-code-opencode is preselected; do not re-evaluate.
You may write to: .opencode/skills/system-spec-kit/mcp_server/ and the spec folder above. Do NOT modify other files. Do NOT ask questions.
...
After applying patches, run `npm --prefix .opencode/skills/system-spec-kit/mcp_server run build` and confirm 0 TS errors. If errors appear, fix them and rebuild until clean. Do NOT mark tasks complete in tasks.md (the runner does that). Do NOT commit. Output a short summary of files modified + final build line.
...
Edit \`.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/verify.ts\` around lines 72-100:

1. Add canonical workspace containment for \`args.rootDir\`. Mirror the pattern used in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/scan.ts:135-168\` and \`.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/detect-changes.ts\`. Resolve via realpathSync; reject paths outside the workspace root with a structured error.

2. Restrict \`args.batteryPath\` to an approved asset directory. Allowed roots: \`<workspace>/.opencode/specs/**/assets/code-graph-gold-queries.json\` AND \`<workspace>/.opencode/skills/system-spec-kit/mcp_server/**/assets/code-graph-gold-queries.json\`. Anything else returns a structured error before any file read. Default battery path stays inside the 007 packet.

3. Update / add tests in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-verify.vitest.ts\` covering: (a) rootDir outside workspace returns error without inline-index, (b) batteryPath outside allowed roots returns error before fs read, (c) default battery path resolves correctly.
...
Edit \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts\` resolver code (around lines 1458-1474, 1500-1506, 1628-1638):
...
Add a unit test or integration assertion in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-indexer.vitest.ts\` proving an out-of-workspace alias target does not produce a cross-file edge.
...
1. \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/ensure-ready.ts:346-359\` — when state.action is 'none' AND verificationGate would otherwise fire, preserve verificationGate on the ReadyResult. The current code drops it because the early-return on action:'none' bypasses the gate-evaluation block. Restructure so verificationGate is computed independently of state.action and always attached. Add a regression test in \`.opencode/skills/system-spec-kit/mcp_server/tests/ensure-ready.vitest.ts\` that proves: action:'none' + failed last verification → ReadyResult has verificationGate:'fail'.

2. \`.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/status.ts:56-64\` — \`share_drift\` per edge type currently uses Math.abs() before surfacing. Keep the SIGNED delta in the surfaced summary (an edge type that lost share should report negative). Use absolute value ONLY for the threshold check. Update the type signature if needed.

3. \`.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/status.ts:40-67\` + \`.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/scan.ts:281-288\` — when the persisted baseline JSON parse fails, currently the drift summary defaults to all-zeros AND the auto-reseed path is also blocked. Fix: treat parse failure as 'baseline unavailable' (don't surface zero drift), AND allow the next full scan to overwrite the malformed metadata even when a baseline 'exists'.
...
1. \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/gold-query-verifier.ts:226-227\` — symbol comparison currently lowercases both sides. Make it case-sensitive by default (drop the .toLowerCase() calls). Identifiers in TypeScript are case-sensitive; matching must reflect that.

2. Same file (\`gold-query-verifier.ts:36-49,298-325\`) and \`.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/verify.ts:98-104\` — when a probe fails or partially matches, the persisted ProbeResult currently omits the original query text and source_file:line. Add fields: \`query: string, sourceFile: string, sourceLine?: number, expectedTopK: string[]\` to the persisted shape. Update \`setLastGoldVerification\` write site so failures preserve enough context for an operator to investigate without re-loading the battery.
...
1. \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/tree-sitter-parser.ts:364-380,388-392\` — inline 'import { type Foo, Bar }' currently stamps importKind:'value' for the whole statement. Detect the 'type' modifier per-specifier (named-import items can have an inline 'type' qualifier). Each specifier should carry its own importKind: 'type' or 'value'.

2. \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts:1500-1528\` — tsconfig 'extends' resolution is single-level. Walk the full chain with a \`Set<string>\` of canonical paths visited; on cycle detection, abort the extends walk and fall back to the partial parse (don't crash). Cap depth at 16 levels for safety.

3. \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts:531-559\` — regex fallback parse currently drops moduleSpecifier, importKind, exportKind from emitted RawCapture. Emit those fields from the regex path too so the resolver and edge classifier behave the same way regardless of whether tree-sitter or the regex fallback produced the capture.

Add tests covering each behavior in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-indexer.vitest.ts\`. Build must pass.
"
...
1. REQ-002 in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-scan.vitest.ts\` — assert that the scan handler calls \`isFileStale(filePath, { currentContentHash: result.contentHash })\` with the exact contentHash from ParseResult. Use a spy/mock on isFileStale to assert call shape, not just outcome.

2. REQ-003 in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-indexer.vitest.ts\` — write a real fixture file with 'import type { X }', 'export *', and 'import { Foo as Bar }', run the parser, and assert the captured RawCapture entries have the expected moduleSpecifier, importKind, exportKind values. No mocking the tree-sitter output.
...
1. REQ-005 — DECISION: implement as a metadata flag on existing IMPORTS edges (not a new edge type) AND amend \`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/007-code-graph/007-code-graph-backend-resilience/spec.md\` REQ-005 to reflect the metadata-tagging contract instead of a distinct TYPE_ONLY edge class. Then add tests in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-indexer.vitest.ts\` proving: a 'import type' results in an emitted edge whose metadata has importKind:'type' AND the effective weight is reduced (≤ 0.5) per the decision in 007 iter-009.

2. REQ-007 — add edge-weight unit tests. In \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-indexer.vitest.ts\` (or a new file): pass IndexerConfig.edgeWeights overriding CALLS to 0.99, run a small indexer pass, assert the emitted CALLS edges carry weight 0.99 and untouched edge types still use DEFAULT_EDGE_WEIGHTS.

3. REQ-008 + REQ-009 — add a unit test file \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/edge-drift.vitest.ts\` (NEW) that imports computeEdgeShare, computePSI, computeJSD from edge-drift.ts. Test cases: identical distributions → PSI=0 + JSD=0; one-edge-type drift of 30% share → PSI > 0.25 (flagged); zero-handling for missing edge types; negative case where neither threshold trips → flagged:false.

Plus an integration test in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-scan.vitest.ts\`: full scan persists \`edge_distribution_baseline\` in code_graph_metadata; subsequent status call surfaces edgeDriftSummary with computed values.
...
1. REQ-012 — add tests in \`.opencode/skills/system-spec-kit/mcp_server/tests/ensure-ready.vitest.ts\` exercising the real selective-reindex branches at \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/ensure-ready.ts:302-367\`. Cover all four observable outcomes: selfHealAttempted:true with selfHealResult:'ok', :'failed', :'skipped', and the no-op path. Each test must assert lastSelfHealAt is populated with a valid ISO string (and only on real attempts per F03 spec, not on skipped).

2. REQ-015 — add tests in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/detect-changes.test.ts\` proving the production-shape detect_changes contract. Mock ensureCodeGraphReady to return a ReadyResult with verificationGate:'fail' and freshness:'fresh', and assert detect_changes returns status:'blocked' (not ok) and never inline-runs scan/verify. Add a sibling test where verificationGate is undefined and freshness:'stale' — must also block.

3. Extract typed query transport adapter to close P1-12. Currently \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/gold-query-verifier.ts:234-330,344-361\` duplicates the parsing logic from \`.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/query.ts:1111-1131\` via \`any\`-typed parsing. Extract a shared module \`.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/query-result-adapter.ts\` exporting typed parse functions for outline/relationship payloads, and have BOTH the verifier and the query handler import from it. Remove the \`any\` casts.
