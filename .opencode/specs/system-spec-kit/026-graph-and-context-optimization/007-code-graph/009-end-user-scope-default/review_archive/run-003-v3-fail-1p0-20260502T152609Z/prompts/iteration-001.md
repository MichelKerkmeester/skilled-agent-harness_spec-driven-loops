...
# Deep-Review v3 Iteration 1 of 5 — VERIFICATION SWEEP
...
You are running iteration 1 of a 5-iteration RUN-3 verification sweep.
...
## DIMENSION FOCUS — CORRECTNESS (iter 1)

Iter 1 — REGRESSION verification on FIX-009-v2 P0 + P1 fixes:
- **R2-I7-P0-001 (data.errors leak)**: verify ALL error strings in scan response are relativized.
  - Check `scan.ts:340`: `errors.slice(0, 10).map(error => relativizeScanError(error, canonicalWorkspace))`
  - Check `scan.ts:296,301`: result.filePath relativized inline
  - Run grep for any remaining `result.filePath` or absolute-path string concatenation in error paths
  - Test `code-graph-scan.vitest.ts` for the new "data.errors" assertion — does it actually trigger the parser-failure path?
- **R2-I9-P1-001 (status payload)**: verify activeScope reflects per-call override.
  - Check `status.ts:174`: `activeScopePolicy = parseIndexScopePolicyFromFingerprint(storedScope) ?? resolveIndexScopePolicy()`
  - Verify the test in code-graph-scope-readiness or code-graph-scan covers env=true + scan({includeSkills:false}) → status.activeScope.includeSkills === false, source === 'scan-argument'
- **R2-I5-P1-001 (6-case matrix)**: verify all 6 cases pass and the (true, false, false, 'scan-argument') row exists.
...
```json
{"type":"iteration","iteration":1,"mode":"review","run":"run-001","status":"complete","focus":"correctness","dimensions":["correctness"],"filesReviewed":[<arr>],"findingsCount":<int>,"findingsSummary":{"P0":<int>,"P1":<int>,"P2":<int>},"findingsNew":[<arr>],"newFindingsRatio":<float>,"sessionId":"2026-05-02T15:03:06.606Z","generation":3,"lineageMode":"restart","timestamp":"<ISO>","durationMs":<int>}
```
