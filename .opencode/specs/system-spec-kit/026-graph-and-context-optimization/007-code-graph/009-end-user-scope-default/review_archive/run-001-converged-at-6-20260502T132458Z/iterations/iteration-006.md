...
- `.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/index-scope-policy.ts:5-49` (traceability context only; not used for new findings outside the stated in-scope list)
- `.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-scope-readiness.vitest.ts:67-92` (traceability context only; prior resource-map gap already recorded)
- `.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/context.ts:196` and `.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/query.ts:825` (traceability context only for CHK-G2-04)
...
- **Plan §4 Phase 3 line-cite ranges**: Verified README and ENV reference document the default `.opencode/skills/**` exclusion, `SPECKIT_CODE_GRAPH_INDEX_SKILLS=true`, `includeSkills:true`, and explicit full-scan migration. Evidence: `plan.md:251-260`, `code_graph/README.md:233-270`, `ENV_REFERENCE.md:255-265`.
...
- **ADR-001 five sub-decisions**: Verified the accepted defaults still match the code/docs: end-user default excludes `.opencode/skills/**`, env opt-in is `SPECKIT_CODE_GRAPH_INDEX_SKILLS=true`, per-call opt-in is `includeSkills:true`, `mcp-coco-index/mcp_server` stays excluded, and scope fingerprint/label require a full scan on mismatch. Evidence: `decision-record.md:70-80`, `indexer-types.ts:140-167`, `index-scope.ts:56-65`, `ensure-ready.ts:293-303`, `code_graph/README.md:250-270`.
