...
- P2 `.opencode/skill/system-spec-kit/mcp_server/ENV_REFERENCE.md:261` — the env reference describes `SPECKIT_CODE_GRAPH_INDEX_SKILLS` as a scan opt-in and cites scan files, but the query/readiness path also consumes the env through `ensure-ready`; remediation: update the docs/source column to include `code_graph/lib/ensure-ready.ts` and state that config changes require MCP server restart.

## EVIDENCE
Native rerun baseline:

- `../002-native-rerun/trials/trial-log.jsonl` recorded `N-CG-001` as a successful `includeSkills:true` scan with `filesIndexed:9280`, `totalNodes:56843`, and `totalEdges:36347`.
...
- `../002-native-rerun/synthesis-report-native-rerun.md` classifies the default-scope exclusion as P0 and says the default excludes the system code users work on.
- `research/iterations/iteration-001.md` already records the default `.opencode` exclusion as a P0 at `.opencode/skill/system-spec-kit/mcp_server/code_graph/lib/index-scope-policy.ts:14`, so this iteration treats that as prior context rather than a new finding.

Scope policy:

- `.opencode/skill/system-spec-kit/mcp_server/code_graph/lib/index-scope-policy.ts:75` parses unset/false skills env as `none`; line 80 parses `true` as `all`; line 98 reads `env[CODE_GRAPH_INDEX_SKILLS_ENV]`.
- `.opencode/skill/system-spec-kit/mcp_server/code_graph/lib/index-scope-policy.ts:140` derives `includeSkills` from `includedSkillsList !== "none"`; line 145 only emits skill exclude globs when the list is `none`.
- `.opencode/skill/system-spec-kit/mcp_server/code_graph/lib/index-scope-policy.ts:170` marks policy source as `env` when any env-derived include is enabled.
- `.opencode/skill/system-spec-kit/mcp_server/code_graph/lib/indexer-types.ts:149` documents the default `.opencode` exclusions; lines 159-164 append policy-driven excludes.
- `.opencode/skill/system-spec-kit/mcp_server/lib/utils/index-scope.ts:72` applies the code-graph policy; lines 80-85 include all skills when `includedSkillsList === "all"` and reject skills when it is `none`.

Scan path:

- `.opencode/skill/system-spec-kit/mcp_server/code_graph/handlers/scan.ts:256` calls `resolveIndexScopePolicy()` with per-call args; line 263 passes the resolved policy into `getDefaultConfig()`.
- `.opencode/skill/system-spec-kit/mcp_server/code_graph/handlers/scan.ts:342` persists the scope policy after scan, and line 426 builds the readiness block from the scan outcome.

Query/readiness path:

- `.opencode/skill/system-spec-kit/mcp_server/code_graph/handlers/query.ts:1089` calls `ensureCodeGraphReady()` before any query operation, with inline full scan disabled at line 1091.
- `.opencode/skill/system-spec-kit/mcp_server/code_graph/lib/ensure-ready.ts:293` resolves active scope from `process.env`; lines 302-310 compare active scope against stored scope and request a full scan on env/stored drift.
- `.opencode/skill/system-spec-kit/mcp_server/code_graph/lib/ensure-ready.ts:537` intentionally honors the last explicit scan's stored scope; lines 542-552 parse the stored scope and use it for inline indexing.
- `.opencode/skill/system-spec-kit/mcp_server/code_graph/lib/readiness-contract.ts:241` only wraps freshness into `canonicalReadiness` and `trustState`; it does not change scope.
...
- `.opencode/skill/system-spec-kit/mcp_server/ENV_REFERENCE.md:261` documents `SPECKIT_CODE_GRAPH_INDEX_SKILLS` defaulting to `false`, with only `index-scope-policy.ts` and `scan.ts` cited as sources.
