...
- `.opencode/skills/sk-code-review/references/fix-completeness-checklist.md:12-23` - R5 classification classes for cross-consumer and matrix/evidence fixes.
- `.opencode/skills/sk-code-review/references/fix-completeness-checklist.md:36-71` - R5 required producer, consumer, matrix, and hostile-state inventories.
- `.opencode/skills/sk-code-review/SKILL.md:319-322` - Review output producer schema includes `findingClass`, `scopeProof`, and `affectedSurfaceHints`.
- `.opencode/skills/sk-code-review/references/review_core.md:75-100` - Finding schema producer defines `affectedSurfaceHints` and shows the markdown output form.
...
- `.opencode/agents/deep-review.md:179` - Deep-review producer requires every actionable finding to include `affectedSurfaceHints`.
- `.opencode/agents/deep-review.md:211` - Deep-review JSONL consumer/producer contract includes `affectedSurfaceHints` inside `findingDetails`.
- `.opencode/skills/sk-deep-review/SKILL.md:425` - Skill-level deep-review state rule requires per-finding `affectedSurfaceHints`.
...
- `.opencode/commands/spec_kit/assets/spec_kit_plan_confirm.yaml:625` - Confirm plan writing rule maps `activeFindings[].affectedSurfaceHints` into the FIX ADDENDUM.
...
Same-class producer inventory: `rg -n "affectedSurfaceHints" .opencode/skills/sk-code-review .claude/skills/sk-code-review` found all same-class schema producers updated: `.opencode/skills/sk-code-review/SKILL.md:321`, `.opencode/skills/sk-code-review/references/review_core.md:88`, `.claude/skills/sk-code-review/SKILL.md:321`, and `.claude/skills/sk-code-review/references/review_core.md:88`.
