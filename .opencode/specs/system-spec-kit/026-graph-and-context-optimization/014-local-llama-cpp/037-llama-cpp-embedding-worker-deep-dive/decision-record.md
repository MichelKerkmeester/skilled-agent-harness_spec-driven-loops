...
The local-LLM Memory MCP substrate uses `node-llama-cpp@^3.15.1` (resolved at runtime to **3.17.1**) against the GGUF-quantized `unsloth/embeddinggemma-300m-GGUF` Q8_0 model for 768-dimension embeddings. Across the 014-local-llama-cpp phase, every save-heavy scenario in the 24-- query-intelligence playbook (411–415) accumulated chronic E081/E085 save failures. 214 historical `memory_index` rows sit with `embedding_status='failed'`.
