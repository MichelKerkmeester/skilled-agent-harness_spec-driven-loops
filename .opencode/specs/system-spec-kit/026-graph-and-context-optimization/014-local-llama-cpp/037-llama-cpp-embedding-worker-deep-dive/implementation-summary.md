...
_memory:
  continuity:
    packet_pointer: "system-spec-kit/026-graph-and-context-optimization/014-local-llama-cpp/037-llama-cpp-embedding-worker-deep-dive"
...
| Field | Value |
|-------|-------|
| **Spec Folder** | `system-spec-kit/026-graph-and-context-optimization/014-local-llama-cpp/037-llama-cpp-embedding-worker-deep-dive` |
| **Started** | 2026-05-14 |
...
| Parent spec.md phase map updated | PASS | row 37 + handoff criteria at lines 117 + 140 in 014-local-llama-cpp/spec.md |
...
| Live `memory_save` round-trip | BLOCKED by V8 sufficiency check | Save reaches the V-rule sufficiency gate cleanly (no embedding error). Then rejected with `V-rule hard block: V8` / `INSUFFICIENT_CONTEXT_ABORT`, even on ADR-003 (13873 chars, 3619 tokens, full anchors, file:line refs throughout, 4 decisions, evidence chain). `evidenceCounts.{primary, support, total, semanticChars, uniqueWords} = 0` across the board — the evidence-counter appears broken (zero unique words on a 14k-char file is impossible). This is UNRELATED to 037's embedding-worker scope. Worth opening 038-v-rule-sufficiency-evidence-count. |
| Scenario 401 replay | DEFERRED | belongs to 036's bootstrap once historical 214 failed rows are cleaned + V8 sufficiency gap resolved |
