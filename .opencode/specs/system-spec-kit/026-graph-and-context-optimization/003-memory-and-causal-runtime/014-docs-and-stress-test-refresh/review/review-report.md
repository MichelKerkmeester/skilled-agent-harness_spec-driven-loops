...
| R20-P1-001 | Error-code catalog documents active `E429` scan rejections after coalescing replaced them | `feature_catalog/08-…/070-error-code-reference.md:30` | traceability | `memory_index_scan` now returns `coalesced:true` success (`memory-index.ts:355-365`); frame `E429` as legacy. |
