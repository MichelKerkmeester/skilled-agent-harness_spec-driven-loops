...
- `.opencode/skills/system-code-graph/manual_testing_playbook/03--detect-changes/024-detect-changes-multi-file-diff.md`: 5 substitutions — 4 path refs `shared/embeddings/providers/llama-cpp.ts` → `shared/embeddings/providers/hf-local.ts`, 1 class-name `LlamaCppProvider` → `HfLocalProvider`
- `.opencode/skills/system-code-graph/manual_testing_playbook/06--mcp-tool-surface/022-code-graph-query-blast-radius.md`: 1 path substitution `llama-cpp.ts` → `hf-local.ts` in test scenario subject
<!-- /ANCHOR:what-built -->
