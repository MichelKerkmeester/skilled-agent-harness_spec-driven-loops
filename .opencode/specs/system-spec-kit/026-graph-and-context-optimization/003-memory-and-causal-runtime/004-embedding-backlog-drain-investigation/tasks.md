...
_memory:
  continuity:
    ...
    recent_action: "implemented-runtime-drain-fixes-T015-T020-build-and-tests-green"
    next_safe_action: "reconnect-mcp-and-verify-live-drain-T021"
    blockers:
      - "Live drain verification blocked on MCP reconnect (daemon not running)"
    key_files:
    ...
    completion_pct: 95
    open_questions:
      - "Does the live daemon honor the tuned retry env after reconnect (T021)?"
    answered_questions:
...
- [ ] T021 [B] Live drain verification — reconnect MCP, confirm startup `interval=5000ms/batch=100`, `ps eww` daemon env, `memory_health` queueDepth → 0
...
- [ ] Live backlog drained to 0 (`memory_health` queueDepth → 0) after MCP reconnect (T021)
