...
Iteration: 8 of 20
Dimension: correctness (UNDER-EXPLORED areas)
Prior Findings: P0=0 P1=7 P2=3 (all adjudicated + validated)
Dimension Coverage: all 4 covered; 7 P1 cluster in the single-writer/lease + supervision area
Last 2 ratios: 0.10 -> 0 (adjudication pass)
Stuck count: 0
Provisional Verdict: CONDITIONAL hasAdvisories=true

The single-writer/lease + supervision hotspot is SATURATED (7 P1 already). Do NOT re-mine it. This pass targets UNDER-EXPLORED ground for genuinely NEW correctness/data-safety findings: (a) cache lib (embedding-cache.ts) eviction/LRU/byte-budget + the shard-qualified cacheTable migration; (b) WAL durability ordering (vector-index-store.ts checkpoint/close sequencing, the 010 autocheckpoint + dual-schema TRUNCATE); (c) daemon shutdown sequencing (context-server.ts fatalShutdown ordering, the 009 fileWatcher-drain-then-closeDb invariant, the 012 boot integrity-check marker); (d) retention sweep (memory-retention-sweep.ts post-delete FTS optimize + incremental_vacuum guards); (e) embed batching/concurrency (execution-router byte+count chunking, hf-local.ts batch retry + dim adoption). Report only NEW findings with file:line evidence + a full claim-adjudication packet for any P0/P1. If an area is clean, state it explicitly (drives convergence).

Prior findings (do NOT re-report): the 7 P1 + 3 P2 from iters 1-6 (provider-override, reindex-cancel, tcp-exposure, workflow-lock-fail-open, daemon-child-ledger, respawn-lock-expiry, spawn-strand; direct-startup-socket-unlink, 005-idle-file-matrix, env-knobs-absent).

Review Iteration: 8 of 20
Mode: review
Dimension: correctness/data-safety (under-explored: cache eviction+migration, WAL ordering, daemon shutdown sequencing, retention sweep, embed batching/concurrency)
...
- Write iteration narrative to: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-008.md
- Write per-iteration delta file to: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-008.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-008.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-008.jsonl`, this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the review. NEVER execute the out-of-scope mutation. The review packet (`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-008.md` directory and parents) is the only zone for your writes; the reviewed target spec/code is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-008.md` (path pre-substituted for the current iteration number). Structure: headings for Dimension, Files Reviewed, Findings by Severity (P0/P1/P2), Traceability Checks, Verdict, Next Dimension.
...
3. **Per-iteration delta file** at `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-008.jsonl` (path pre-substituted, e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same as the state-log append) plus per-event structured records (one per graphEvent, finding, classification, traceability-check, ruled_out direction). Each record on its own JSON line.
