...
Iteration: 6 of 20
Dimension: correctness/security (single-writer-lease lifecycle deepening + final saturation sweep)
Prior Findings: P0=0 P1=6 P2=3
Dimension Coverage: all 4 (4/4)
Traceability: core=covered overlay=covered
Resource Map Coverage: resource-map.md not present; skipping coverage gate.
Coverage Age: 2
Last 2 ratios: 0.14 -> 0.11
Stuck count: 0
Provisional Verdict: CONDITIONAL hasAdvisories=true
SATURATION SWEEP: composite stop-score is ~0.70 (STOP candidate). This is likely the last productive pass. Deepen the single-writer/lease lifecycle (the iter-5 respawn-lock-expiry P1, the OR-R-01-class election race on the model-server side) and do a final correctness/security sweep for anything missed. Report only NEW findings; if the area is clean, say so explicitly (drives convergence). For EVERY new P0/P1 you MUST include the full claim-adjudication packet (claim, evidenceRefs, counterevidenceSought, alternativeExplanation, finalSeverity, confidence, downgradeTrigger).
Prior findings (do NOT re-report): provider-override-ignored; reindex-cancel-no-worker-stop; direct-startup-socket-unlink; tcp-embed-unauthenticated; workflow-lock-fail-open; daemon-child-ledger-conflict; 005-idle-eviction-file-matrix; env-knobs-absent-ENV_REFERENCE; respawn-lock-expiry-while-listener-owns.
...
- Write iteration narrative to: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-006.md
- Write per-iteration delta file to: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-006.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-006.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-006.jsonl`, this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the review. NEVER execute the out-of-scope mutation. The review packet (`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-006.md` directory and parents) is the only zone for your writes; the reviewed target spec/code is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-006.md` (path pre-substituted for the current iteration number). Structure: headings for Dimension, Files Reviewed, Findings by Severity (P0/P1/P2), Traceability Checks, Verdict, Next Dimension.
...
3. **Per-iteration delta file** at `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-006.jsonl` (path pre-substituted, e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same as the state-log append) plus per-event structured records (one per graphEvent, finding, classification, traceability-check, ruled_out direction). Each record on its own JSON line.
