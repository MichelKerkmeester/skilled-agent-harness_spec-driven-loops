...
- [ ] **P1.4** Add bounded global orphan sweep in `lib/search/incremental-index.ts`: ungated from the scan's discovery scope, paged by id/`updated_at`, per-tick cap, removes rows whose `file_path` AND `canonical_file_path` are absent on disk, via `delete_memory_from_database()` (`vector-index-mutations.ts:577-627`) — never a raw `memory_index` delete.
