---
title: "Phase 025 — Deep-Review Remediation Plan"
...
2. Update `skill-advisor-hook.md` Codex status to match shipped `hooks/codex/` adapter.
3. Update `manual_testing_playbook.md` §validation scope: count scenarios in file; replace denominator with actual count.
4. Reconcile measurement artifact name across `feature_catalog.md`, `manual_testing_playbook.md`, `smart-router-measurement.ts` — single canonical name.
5. `LIVE_SESSION_WRAPPER_SETUP.md` or `README.md`: clarify Copilot integration uses wrapper callbacks, not native hook.
...
### Phase H — Verify
1. Run full suite: `npm --prefix .opencode/skill/system-spec-kit/mcp_server test`
