...
## Focus
This iteration independently mapped process ownership across `/deep:start-research-loop`, adjacent deep-loop command assets, system-spec-kit subprocess helpers, embedder sidecars, daemon probes, and cli-X skill contracts. Prior Claude iterations were used only as context; findings below cite source lines from the current tree.

## Findings
1. `/deep:start-research-loop` defines the canonical state, prompt, iteration, delta, and lock paths under the resolved research artifact directory, so the workflow owns artifact placement before any executor runs. [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:100-115]
2. The workflow explicitly resolves a local research owner directory and local archive directory before initialization; child phases keep local research packets rather than sharing a parent-level output surface. [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:126-134]
3. The deep-research advisory lock contract is declarative: the YAML says runtimes must release the lock on halt, cancel, and workflow exit, then emits a final `lock_released` event after save/skip-save/cancel cleanup. [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:179-195] [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:1055-1058]
...
8. There is a real ownership-name drift: the config schema stores `kind`, the deep-research YAML branches on `config.executor.type`, the branch-local executor objects use `type`, and executor audit records read `executor.kind`. That can make branch selection and provenance depend on a non-schema field. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/deep-loop/executor-config.ts:21-27] [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:577-599] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/deep-loop/executor-audit.ts:10-16]
9. Deep-research wraps `cli-codex`, `cli-gemini`, and `cli-claude-code` through `runAuditedExecutorCommand`, passing command, args, cwd, timeout, state-log path, executor metadata, iteration number, and prompt input where needed. [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:584-724]
...
## Sources Consulted
- `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-embedder-testing-and-architecture/009-memory-leak-remediation/001-research-synthesis-and-remediation-map/research/source-research/020-cli-process-memory-leak-deep-research/research/prompts/iteration-contract.md:1-60`
...
- `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-embedder-testing-and-architecture/009-memory-leak-remediation/001-research-synthesis-and-remediation-map/research/source-research/020-cli-process-memory-leak-deep-research/research/deep-research-state.jsonl:1-6`
- `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/013-embedder-testing-and-architecture/009-memory-leak-remediation/001-research-synthesis-and-remediation-map/research/source-research/020-cli-process-memory-leak-deep-research/research/deep-research-strategy.md:1-30`
- `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:100-205`
- `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:540-825`
- `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:1055-1058`
- `.opencode/commands/deep/assets/deep_start-review-loop_auto.yaml:680-910`
