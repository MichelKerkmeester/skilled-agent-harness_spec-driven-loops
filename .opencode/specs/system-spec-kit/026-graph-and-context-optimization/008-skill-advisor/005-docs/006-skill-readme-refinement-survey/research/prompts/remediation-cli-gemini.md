REMEDIATE: cli-gemini/README.md per packet-005-style refinement rules.

# Goal

Modify exactly one file: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/cli-gemini/README.md`

Apply the refinements catalogued in the audit report for skill `cli-gemini`. Output the modified README directly in-place (the agent-config allows writes only to this one path).
...
Section 1 tables to remove or move:
  - lines 52-62 (3 cols, 6 rows): Key Statistics (Models, Built-in Tools, Free Tier, Context Window, References, Version) -> to-paragraph
  - lines 64-73 (5 cols, 5 rows): Comparison table with Claude Code, Codex, Copilot, Gemini for Web search, Architecture analysis, Context window, Cost, Open source, Models -> move-to-§3
  - lines 75-85 (2 cols, 8 rows): Key Features at a Glance (Google Search Grounding, Codebase Investigator, Save Memory, 1M+ Context, Free Tier, Open Source, Spec Kit handoff) -> to-bullets
...
Write the entire revised README to `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/cli-gemini/README.md`. Do not produce any other files. Do not edit any other file.
