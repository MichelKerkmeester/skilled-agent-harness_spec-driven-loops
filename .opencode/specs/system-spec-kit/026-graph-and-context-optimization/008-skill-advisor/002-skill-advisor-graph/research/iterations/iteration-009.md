...
This iteration tested regression risk for the current graph-integrated `skill_advisor.py` routing path, with emphasis on whether graph boosts can change the winning recommendation in harmful ways. The relevant integration points are `_apply_graph_boosts()` and `_apply_family_affinity()` in `.opencode/skills/skill-advisor/scripts/skill_advisor.py:81-143`, invoked before lexical scoring and ranking in `.opencode/skills/skill-advisor/scripts/skill_advisor.py:1496-1498`.
...
- That single top-1 flip was `P1-FULLSTACK-001`, where graph family affinity promoted `sk-code-full-stack` over `sk-code-opencode`. This is not currently a regression because the fixture explicitly allows either winner (`.opencode/skills/skill-advisor/scripts/fixtures/skill_advisor_regression_cases.jsonl:38`).
...
```bash
python3 .opencode/skills/skill-advisor/scripts/skill_advisor_regression.py --dataset .opencode/skills/skill-advisor/scripts/fixtures/skill_advisor_regression_cases.jsonl
```
...
```json
{
  "dataset": ".opencode/skills/skill-advisor/scripts/fixtures/skill_advisor_regression_cases.jsonl",
  "mode": "both",
  "thresholds": {
    "confidence": 0.8,
    "uncertainty": 0.35,
...
```bash
python3 .opencode/skills/skill-advisor/scripts/skill_advisor.py 'save context and create spec folder' --threshold 0
```
...
```bash
python3 .opencode/skills/skill-advisor/scripts/skill_advisor.py 'improve this agent evaluation' --threshold 0
```
...
```bash
python3 .opencode/skills/skill-advisor/scripts/skill_advisor.py 'delegate code review to external cli' --threshold 0
```
