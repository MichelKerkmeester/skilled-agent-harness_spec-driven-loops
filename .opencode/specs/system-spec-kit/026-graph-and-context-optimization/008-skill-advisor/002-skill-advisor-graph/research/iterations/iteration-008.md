...
This iteration validated the current `skill_advisor.py` routing behavior against the compiled graph generated on `2026-04-13T08:50:16Z` and focused on whether graph-derived reasons help or hurt real routing decisions. The advisor still applies graph boosts after lexical and phrase boosts, then surfaces them directly in the `reason` field (`.opencode/skills/skill-advisor/scripts/skill_advisor.py:81-143`, `.opencode/skills/skill-advisor/scripts/skill_advisor.py:1496-1498`, `.opencode/skills/skill-advisor/scripts/skill_advisor.py:1573-1579`).
...
- `system-spec-kit` still has zero declared edges in source metadata (`.opencode/skills/system-spec-kit/graph-metadata.json:1-15`) and therefore no compiled adjacency entry (`.opencode/skills/skill-advisor/scripts/skill-graph.json:39-145`).
- `mcp-coco-index` still has zero declared edges (`.opencode/skills/mcp-coco-index/graph-metadata.json:1-15`) and currently enters unrelated MCP results mainly through family affinity rather than an explicit workflow relationship.
- `sk-doc` still has zero declared edges (`.opencode/skills/sk-doc/graph-metadata.json:1-15`).
- `sk-improve-prompt` still compiles only a sibling edge to `sk-improve-agent` and no CLI-facing workflow edges (`.opencode/skills/sk-improve-prompt/graph-metadata.json:1-17`, `.opencode/skills/skill-advisor/scripts/skill-graph.json:136-145`).
...
```bash
python3 .opencode/skills/skill-advisor/scripts/skill_advisor.py 'review my opencode typescript module' --threshold 0
```
...
```bash
python3 .opencode/skills/skill-advisor/scripts/skill_advisor.py 'use chrome devtools to debug css layout' --threshold 0
```
...
```bash
python3 .opencode/skills/skill-advisor/scripts/skill_advisor.py 'deep review the spec folder' --threshold 0
```
...
- `sk-deep-review` is correctly first with or without graph, and `sk-code-review` is a legitimate supporting skill because `sk-deep-review` explicitly depends on it (`.opencode/skills/sk-deep-review/graph-metadata.json:6-19`, `.opencode/skills/sk-code-review/graph-metadata.json:6-20`).
...
2. Add explicit edges for the current zero-edge hub and orphan skills.
   - Evidence: `system-spec-kit`, `mcp-coco-index`, and `sk-doc` still declare no edges at all (`.opencode/skills/system-spec-kit/graph-metadata.json:1-15`, `.opencode/skills/mcp-coco-index/graph-metadata.json:1-15`, `.opencode/skills/sk-doc/graph-metadata.json:1-15`), and none of them appear in compiled adjacency (`.opencode/skills/skill-advisor/scripts/skill-graph.json:39-145`).
...
3. Add the missing CLI prompt-quality integration edges.
   - Evidence: `sk-improve-prompt` still compiles only a sibling edge to `sk-improve-agent` (`.opencode/skills/sk-improve-prompt/graph-metadata.json:6-16`, `.opencode/skills/skill-advisor/scripts/skill-graph.json:136-145`), so current CLI routing cannot use that relationship even though the prompt-quality card is a shared dependency surface.
...
2. Re-tune family affinity after the gating fix.
   - Evidence: `_apply_family_affinity()` currently adds `max_boost * 0.08` whenever a family member already exceeds `1.5` and the candidate has no score yet (`.opencode/skills/skill-advisor/scripts/skill_advisor.py:123-143`).
...
3. Improve reason-field ordering for audits.
   - Evidence: the advisor currently sorts the deduplicated reason set alphabetically and truncates to five entries (`.opencode/skills/skill-advisor/scripts/skill_advisor.py:1573-1579`).
