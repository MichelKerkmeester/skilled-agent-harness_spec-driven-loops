...
### P2
- **F004**: Canonical packet-doc preference can point an entity at an unrelated packet — `.opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:478` — `isCanonicalEntityPath()` treats any path ending in `/spec.md` or similar as canonical, and `prefersCandidate()` then upgrades path-like matches over basename-only local docs, which is why root 003 now maps its `spec.md` entity to `00--ai-systems/001-global-shared/spec.md` instead of the packet-local doc. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:478] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:488] [SOURCE: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/006-search-routing-advisor/001-search-and-routing-tuning/003-graph-metadata-validation/graph-metadata.json:78]
