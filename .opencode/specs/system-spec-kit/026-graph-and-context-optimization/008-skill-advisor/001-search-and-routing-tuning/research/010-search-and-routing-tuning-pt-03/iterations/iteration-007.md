...
## Findings
1. The parser reads status only from frontmatter scalars on canonical docs, in the order `implementation-summary.md`, `checklist.md`, `tasks.md`, `plan.md`, `spec.md`, and falls back to `planned` if none are present. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:346-353] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:498-510]
