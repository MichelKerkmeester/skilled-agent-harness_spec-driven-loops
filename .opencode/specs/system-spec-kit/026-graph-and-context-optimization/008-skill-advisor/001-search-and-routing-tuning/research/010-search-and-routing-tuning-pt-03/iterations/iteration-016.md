...
## Findings
1. The first essential check belongs in the key-files loop before `entities.set(filePath, ...)`, because duplicate-name rows are already created there. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:421-428]
2. The extracted-entity loop also needs the same guard before `entities.set(normalizedName, ...)`, otherwise doc-extracted entities can still collide with file-derived basenames. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:430-442]
