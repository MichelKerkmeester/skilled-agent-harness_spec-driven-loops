...
## Findings
1. The router exports a fully frozen Tier3 contract: prompt version `tier3-router-v1`, model `gpt-5.4`, reasoning effort `low`, temperature `0`, `200` max output tokens, and `2000ms` timeout. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:1128] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/tests/content-router.vitest.ts:472]
2. The prompt contract is strict about the eight categories, a refusal path via `refuse-to-route`, and merge-mode constraints, so Tier3 is designed as a bounded classifier, not a free-form router. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:1128] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/tests/content-router.vitest.ts:421]
3. The canonical save handler currently constructs `createContentRouter()` with no dependency injection, so `classifyWithTier3` defaults to `null` in the actual writer path. Tier3 exists in tests and the router contract, but not in the live save wiring studied here. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:1008] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:386]
4. The test suite proves Tier3 behavior only under injected classifiers or caches. That is valuable for contract safety, but it also means Tier3 threshold tuning alone cannot improve the current save path until the handler injects a real classifier. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/tests/content-router.vitest.ts:212] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/tests/content-router.vitest.ts:246]
...
- `.opencode/skills/system-spec-kit/mcp_server/tests/content-router.vitest.ts:212`
