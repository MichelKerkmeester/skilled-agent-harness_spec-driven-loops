...
## Findings
1. The router is already designed for dependency injection. `createContentRouter()` accepts `classifyWithTier3`, `cache`, `embedText`, and `now`, then threads those dependencies into `resolveNaturalDecision()` and `resolveTier3Decision()`. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:386] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:452]
2. The live save path never provides those Tier3 dependencies. `buildCanonicalAtomicPreparedSave()` simply calls `createContentRouter()` with no arguments, so the default dependency closes over `classifyWithTier3: async () => null`. That means the current runtime can only use Tier1, Tier2, and penalized Tier2 fallback. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:1008] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:391]
3. There is no production classifier implementation elsewhere in the repo. A search for `buildTier3Prompt`, `classifyWithTier3`, and `gpt-5.4` only finds the router contract and its tests. The nearest reusable production pattern is the OpenAI-compatible helper in `lib/search/llm-reformulation.ts`, which already handles env-based endpoint configuration, timeout, `fetch()`, and JSON parsing behind a fail-open interface. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:1128] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/search/llm-reformulation.ts:185]
4. Phase `003-wire-tier3-llm-classifier` therefore needs two concrete code changes, not one. First, add a real classifier adapter that calls the Tier3 prompt contract and returns `Tier3RawResponse | null`. Second, inject that adapter into the router constructor in `memory-save.ts` and, ideally, pass a cache implementation so repeated ambiguous saves can hit the existing session/spec-folder cache layer. [SOURCE: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/006-canonical-continuity-refactor/018-research-content-routing-accuracy/003-wire-tier3-llm-classifier/spec.md:10] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:630]
...
## Sources Consulted
- `.opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:386`
- `.opencode/skills/system-spec-kit/mcp_server/lib/routing/content-router.ts:630`
- `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:1008`
- `.opencode/skills/system-spec-kit/mcp_server/lib/search/llm-reformulation.ts:185`
