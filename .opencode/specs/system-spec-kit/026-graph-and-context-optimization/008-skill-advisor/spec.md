...
### Problem Statement
Skill advisor work was scattered across two phase wrappers (`006-search-routing-advisor/` for search/routing/graph/smart-router, and parts of `010-hook-package/` for hook surface, daemon unification, plugin hardening, standards alignment, and hook improvements). The split made it hard to see the advisor system as one coherent surface and forced cross-wrapper navigation when reasoning about advisor changes.
