...
- **Advisor lib transitive-import quirk**: `mcp_server/lib/skill-advisor-brief.ts` and `freshness.ts` use `../../../system-spec-kit/mcp_server/lib/context/shared-payload.js` which is correct at source-time but resolves wrongly when compiled JS is loaded from `dist/lib/` (needs `../../../../` instead). This is pre-existing and affects ALL runtime variants, not just Devin. The shim pattern mitigates it for cross-process invocation (we verified the inner advisor compiled hook produces the correct brief when invoked from repo root cwd). Out of scope for this packet — tracked as a follow-on packet candidate.
- **`read_config_from.claude` double-fire risk** between the explicit Devin hook and inherited Claude hook: not observed in smoke testing (Devin docs are silent on dedup semantics). Live Devin TUI verification via `/hooks` slash command + actual prompt submission is recommended before declaring full production readiness; not blocking acceptance because explicit Devin registration is the authoritative path.
<!-- /ANCHOR:limitations -->
