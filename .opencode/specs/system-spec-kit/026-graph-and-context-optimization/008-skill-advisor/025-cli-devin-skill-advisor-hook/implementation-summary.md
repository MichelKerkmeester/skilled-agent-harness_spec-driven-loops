...
- **`read_config_from.claude` double-fire risk** between the explicit Devin hook and inherited Claude hook: not observed in smoke testing (Devin docs are silent on dedup semantics). Live Devin TUI verification via `/hooks` slash command is recommended before declaring full production readiness; not blocking acceptance because explicit Devin registration is the authoritative path.
<!-- /ANCHOR:limitations -->
