...
Spec folder: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/005-code-graph-backend-resilience/
...
1. REQ-005 — DECISION: implement as a metadata flag on existing IMPORTS edges (not a new edge type) AND amend \`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/005-code-graph-backend-resilience/spec.md\` REQ-005 to reflect the metadata-tagging contract instead of a distinct TYPE_ONLY edge class. Then add tests in \`.opencode/skills/system-spec-kit/mcp_server/code_graph/tests/code-graph-indexer.vitest.ts\` proving: a 'import type' results in an emitted edge whose metadata has importKind:'type' AND the effective weight is reduced (≤ 0.5) per the decision in 007 iter-009.
