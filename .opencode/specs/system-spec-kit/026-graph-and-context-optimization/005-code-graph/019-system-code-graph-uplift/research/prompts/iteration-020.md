...
# Deep-Research Iteration Prompt Pack — Iteration 20 of 20
...
iter 018: {"type":"iteration","iteration":18,"newInfoRatio":0.85,"status":"insight","focus":"Q10 HVR pitfalls audit: 3 worst patterns to avoid in child-002 rewrite (semicolons in complex sentences, Oxford commas in lists, em dashes for emphasis)","graphEvents":[]}
iter 019: {"type":"iteration","iteration":19,"newInfoRatio":0.8,"status":"insight","focus":"Q2 sk-doc --type classification and mandatory requirements audit: 8 primary authored docs compliant with contracts; per-feature files deferred to Q7/Q8","graphEvents":[]}
...
- Iteration narrative target: `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/019-system-code-graph-uplift/research/iterations/iteration-020.md`
- Delta file target: `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/019-system-code-graph-uplift/research/deltas/iter-020.jsonl`
...
1. `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/019-system-code-graph-uplift/research/iterations/iteration-020.md` with sections: Focus, Actions Taken, Findings (file:line-cited per-file), Questions Answered (with question IDs from registry), Questions Remaining, Next Focus.
...
```json
{"type":"iteration","iteration":20,"newInfoRatio":<0..1>,"status":"<insight|partial|stuck|complete>","focus":"<short focus phrase>","graphEvents":[]}
```

3. Optional: write `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/019-system-code-graph-uplift/research/deltas/iter-020.jsonl` with the iteration record + per-finding records. Workflow will synthesize if absent.

## RESOLVING QUESTIONS

When a question is substantively answered, add to your iter-020.md a "Questions Answered" section listing question text + iter source citations. The reducer will resolve them at the next pass IF you also emit a delta record like:
```json
{"type":"question_resolved","questionId":"<id-from-registry>","resolvedAtIteration":20,"answer":"<one-line summary>"}
```
