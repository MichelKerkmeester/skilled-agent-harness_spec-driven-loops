...
- **Spec folder:** `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/004-code-graph-advisor-refinement`
...
| R3-P1-003 | P1 | Cross | Spec close-out summary still says the packet is planned and research-only | `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/004-code-graph-advisor-refinement/implementation-summary.md:4` | Update `implementation-summary.md` with: the implemented PR set, current deep-review verdict (CONDITIONAL), all 11 open P1 + 3 P2 findings by ID, validation run evidence, and the next remediation step. Update the `_memory.continuity` frontmatter to point to the remediation phase, not to `/spec_kit:deep-research:auto`. | S, 1 h |
