...
- **Spec folder:** `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/004-code-graph-and-advisor-refinement-research-and-fixes`
...
| R3-P1-003 | P1 | Cross | Spec close-out summary still says the packet is planned and research-only | `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/004-code-graph-and-advisor-refinement-research-and-fixes/implementation-summary.md:4` | Update `implementation-summary.md` with: the implemented PR set, current deep-review verdict (CONDITIONAL), all 11 open P1 + 3 P2 findings by ID, validation run evidence, and the next remediation step. Update the `_memory.continuity` frontmatter to point to the remediation phase, not to `/deep:start-research-loop:auto`. | S, 1 h |
...
.opencode/specs/system-spec-kit/026-graph-and-context-optimization/005-code-graph/004-code-graph-and-advisor-refinement-research-and-fixes/implementation-summary.md
```

**Pre-merge sanity check:** Confirm `find .opencode/skills/system-spec-kit/mcp_server/tests/promotion-positive-validation-semantics.vitest.ts` returns no result (if delete path taken). Confirm `implementation-summary.md` frontmatter status is no longer `planned`. Confirm `_memory.continuity` next-action points to remediation implementation, not `/deep:start-research-loop:auto`.
