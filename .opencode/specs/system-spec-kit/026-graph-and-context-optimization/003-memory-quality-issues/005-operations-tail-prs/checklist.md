...
- [x] CHK-520 [P0] PR-10 dry-run script exists with `--dry-run`, `--path`, `--report`, and `--per-defect` controls. [SOURCE: ../research/iterations/iteration-023.md:71-83] [EVIDENCE: `scripts/memory/migrate-historical-json-mode-memories.ts` was created and `npm run build` exited `0`.]
