...
- `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/006-search-routing-advisor/001-search-and-routing-tuning/003-graph-metadata-validation/002-sanitize-key-files/spec.md`
...
- `014-memory-save-rewrite/spec.md:175-181` stays aligned on deterministic target authority, no unsafe file-target exposure, and no silent widening from opt-in flags.
- `014-memory-save-rewrite/implementation-summary.md:57-69` still describes a non-mutating default, explicit `full-auto` fallback, and preserved `POST_SAVE_FINGERPRINT` safety parity without contradiction.
- `010-search-and-routing-tuning/003-graph-metadata-validation/002-sanitize-key-files/spec.md:14-23` remains narrowly scoped to rejecting non-path strings and explicitly avoids broadening into absolute-path resolution.
- `010-search-and-routing-tuning/003-graph-metadata-validation/006-key-file-resolution/checklist.md:18-21` still records bounded-root resolution and dropping unresolved candidates instead of broadening path lookup.
