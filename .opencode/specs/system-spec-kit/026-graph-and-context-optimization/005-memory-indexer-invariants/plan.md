...
- **`code-graph/lib/indexer-types.ts` + `code-graph/lib/structural-indexer.ts`** — Preserves legacy default excludes, adds `external`, and routes both recursive and `specificFiles` scans through `shouldIndexForCodeGraph()`. Walker caps (`.gitignore` 1MB, depth 20, ≤ 50,000 nodes) apply to both memory and code-graph walkers.
