...
- The second P1 concern is verification debt, not code debt: the packet still needs the live `memory_index_scan` rerun on `026/009-hook-package` in an embedding-capable runtime before the "0 `candidate_changed`" claim can be treated as fully closed. (`implementation-summary.md §Verification`, `checklist.md §Testing`)
