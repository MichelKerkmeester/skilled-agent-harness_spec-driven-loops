...
- Preserve legitimate current-reality references, including constitutional-memory file management in `/memory:learn`.
...
|--------|----------|---------|----------------|
| 016 `002-cmd-memory-and-speckit` reference map | 44 | 20 | 0 |
...
|----|-------------|---------------------|
| REQ-001 | Command save-path wording must match Phase 018 | Edited authoritative command docs point to `generate-context.js` and canonical packet continuity rather than hand-authored session memory files |
...
| REQ-003 | Deprecated active-state language must be removed | Edited command docs stop presenting archived-tier wording as part of the active continuity contract |
...
- The 20 editable drifted files are updated and re-read.
- Remaining unchanged references are limited to legitimate constitutional-memory management or documented sandbox-blocked mirror wrappers.
...
- **Given** a reviewed command target that legitimately manages constitutional-memory files, **When** it is evaluated during the revisit, **Then** it remains unchanged and is documented as intentional current-reality behavior.
...
| Risk | Over-correcting `/memory:learn` constitutional file management | Leave legitimate constitutional-memory file references intact and document why they remain |
