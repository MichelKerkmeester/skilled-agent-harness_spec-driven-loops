...
1. **Full vitest gate P1-04 is formally deferred.** Owner: parent `009-hook-parity` docs. Date: 2026-04-22. The blocker is outside the plugin-loader packet: `copilot-hook-wiring.vitest.ts` expects repo-local Copilot hook commands, while `.github/hooks/superset-notify.json` currently routes all events through Superset hook commands.
...
|------|--------|-------|------|--------|
| P1-04 full `npx vitest run` | Deferred | Parent `009-hook-parity` docs | 2026-04-22 | Out-of-scope Copilot hook wiring blocker in `copilot-hook-wiring.vitest.ts` against `.github/hooks/superset-notify.json`; focused plugin-loader verification remains green. |
<!-- /ANCHOR:limitations -->
