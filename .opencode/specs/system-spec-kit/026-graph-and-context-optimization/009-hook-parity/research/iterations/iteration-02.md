...
### Findings
- The remediated docs repeatedly say Codex native hooks are only active when `[features].codex_hooks = true` is enabled in `~/.codex/config.toml` and `~/.codex/hooks.json` is wired. Evidence: `AGENTS.md:92`, `.opencode/skills/system-spec-kit/references/config/hook_system.md:33`, `.opencode/skills/system-spec-kit/references/config/hook_system.md:67`, `.opencode/skills/system-spec-kit/mcp_server/INSTALL_GUIDE.md:137`, `.opencode/skills/system-spec-kit/README.md:118`
...
- `detectCodexHookPolicy()` reports `live` whenever `.codex/settings.json` is valid JSON, and it never checks either the `codex_hooks` feature flag or the documented `hooks.json` wiring. That lets the operator-status layer overstate Codex parity even when the stated activation contract is incomplete. Evidence: `.opencode/skills/system-spec-kit/mcp_server/lib/codex-hook-policy.ts:120`, `.opencode/skills/system-spec-kit/mcp_server/lib/codex-hook-policy.ts:125`, `.opencode/skills/system-spec-kit/mcp_server/lib/codex-hook-policy.ts:219`, `.opencode/skills/system-spec-kit/mcp_server/lib/codex-hook-policy.ts:222`, `.opencode/skills/system-spec-kit/mcp_server/lib/codex-hook-policy.ts:229`
- The runtime detection layer then forwards that optimistic Codex policy directly into cross-runtime recovery logic, so the same overstatement propagates beyond the Codex-specific hook files. Evidence: `.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/runtime-detection.ts:31`, `.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/runtime-detection.ts:39`, `.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/runtime-detection.ts:40`, `.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/runtime-detection.ts:115`, `.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/runtime-detection.ts:120`
