...
| L-009-007 | P2 | maintainability | .opencode/skills/system-spec-kit/feature_catalog/16--tooling-and-scripts/29-setup-native-module-health-and-mcp-installation.md:32 | "attempts to load `better-sqlite3` plus optional `onnxruntime-node` and `sharp` installs" | confirmed-residue | Regenerate the native-module health catalog entry so it no longer documents probing the rejected ONNX runtime backend. |
