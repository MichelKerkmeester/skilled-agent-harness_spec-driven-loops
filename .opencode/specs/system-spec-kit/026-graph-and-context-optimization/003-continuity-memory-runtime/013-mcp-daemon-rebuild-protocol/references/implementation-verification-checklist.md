...
1. **Daemon restart is out-of-band.** This packet patches source and rebuilds dist, but the running MCP daemon must be restarted by the MCP-owning client (OpenCode/Codex/Claude). See packet 013-mcp-daemon-rebuild-protocol for the canonical 4-part verification contract.
```
