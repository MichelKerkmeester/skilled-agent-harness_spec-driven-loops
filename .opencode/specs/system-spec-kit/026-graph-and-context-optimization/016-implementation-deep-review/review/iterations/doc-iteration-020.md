...
- `012-canonical-intake-and-middleware-cleanup` still keeps the intake lock scoped to Step 1 and keeps the deprecated command/agent deletion surfaces explicitly verified across spec, ADRs, and checklist evidence (`spec.md:98-104,250-273`; `decision-record.md:259-390`; `checklist.md:84-90,140-145,200-205`).
...
- `012-canonical-intake-and-middleware-cleanup`: Step-1-only intake lock, deleted middleware surfaces, and `/memory:save` handover ownership remain internally consistent at the packet-doc level.
