...
_memory:
  continuity:
    ...
    recent_action: "Completed ten deep-research iterations and final synthesis."
    ...
    answered_questions:
      ...
      - "Executor plan corrected to cli-claude-code Opus 4.7 for five iterations plus cli-codex GPT-5.5 xhigh fast for five iterations."
---
...
**Key Decisions**: split the ten research iterations into two sequential executor lanes; treat Apple Silicon swap and wired-memory pressure as first-class failure signals, not just orphan process RSS.
...
| **Created** | 2026-05-22 |
| **Parent Spec** | `../spec.md` |
| **Spec Folder** | `specs/system-spec-kit/026-graph-and-context-optimization/016-embedder-testing-and-architecture/004-code-index-stack/024-cli-deep-research-memory-leak-audit/` |
| **Primary Targets** | `.opencode/skills/mcp-coco-index/`, `.opencode/skills/system-code-graph/` |
| **Research Iterations** | 10 total: 5 Claude Code lane, 5 Codex lane |
<!-- /ANCHOR:metadata -->
...
### In Scope
- Ten `/spec_kit:deep-research` iterations against `.opencode/skills/mcp-coco-index/` and `.opencode/skills/system-code-graph/`.
- Lane A: five iterations using `cli-claude-code` with Opus 4.7 / Opus extended-thinking profile.
- Lane B: five iterations using `cli-codex` with `gpt-5.5`, `model_reasoning_effort="xhigh"`, and `service_tier="fast"`.
...
| REQ-002 | Use the corrected two-lane executor split. | Five iterations cite `cli-claude-code` Opus 4.7 / Opus profile; five iterations cite `cli-codex` `gpt-5.5` xhigh fast. |
...
- **SC-001**: Ten iteration artifacts exist or the loop halts early with a documented safety reason tied to memory pressure or repeated executor failure.
...
|------|------|--------|------------|
| Risk | Swap already saturated before research | Running ten iterations could worsen kernel pressure and make cleanup misleading. | Run `sysctl vm.swapusage` before the first iteration; stop if swap used is above 70 percent. |
...
| Research | 20/20 | Ten fresh-context iterations with broad code, workflow, and runtime investigation. |
...
|---------|-------------|--------|------------|------------|
| R-001 | Ten iterations create additional memory pressure. | High | Medium | Preflight swap, single dispatch, kill-between, halt thresholds. |
...
PRE-BOUND SETUP ANSWERS:
  ...
  spec_folder: specs/system-spec-kit/026-graph-and-context-optimization/016-embedder-testing-and-architecture/004-code-index-stack/024-cli-deep-research-memory-leak-audit
