...
description: "Architectural decisions for the two-lane deep-research memory-leak audit and Apple Silicon memory-pressure interpretation."
trigger_phrases:
...
The operator wants ten deep-research iterations but corrected the executor plan to use five Claude Code iterations and five Codex iterations. Prior sessions showed process buildup and high memory pressure when AI sessions used CLI skills to orchestrate other CLIs, so the research run must not create the same failure mode it investigates.
...
**We chose**: Run lane A with `cli-claude-code` Opus 4.7 / Opus profile for five iterations, then run lane B with `cli-codex` `gpt-5.5` xhigh fast for five iterations.
...
**What it costs**:
- The ten iterations take longer than parallel execution. Mitigation: accept wall-clock cost because memory safety is the primary objective.
...
| 3 | **Sufficient?** | PASS | Ten iterations plus synthesis is broad enough for discovery without immediate implementation. |
