...
_memory:
  continuity:
    ...
    recent_action: "Planned telemetry-gated deep research."
    next_safe_action: "Run preflight auth and memory checks before iteration 1."
    blockers:
      - "Research iterations not executed."
    key_files:
      - "spec.md"
      - "plan.md"
      - "tasks.md"
...
- [ ] Claude CLI auth, model id, and self-invocation route checked.
- [ ] Codex CLI auth and `gpt-5.5` xhigh fast route checked.
- [ ] `sysctl vm.swapusage` preflight is below the stop threshold.

### Definition of Done

- [ ] Ten iteration artifacts exist, or an early stop event explains why continuing would be unsafe.
- [ ] Every iteration has markdown, JSONL, telemetry, cleanup result, and source citations.
- [ ] `research/research.md` synthesizes leak classes, severity, owners, and follow-up fixes.
- [ ] No unexpected dispatcher process remains after the final iteration.
- [ ] Spec docs and memory metadata are refreshed after synthesis.
<!-- /ANCHOR:quality-gates -->
...
- [ ] Confirm the target path exists: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-spec-kit`.
- [ ] Run provider auth and model preflight for `cli-claude-code` and `cli-codex`.
- [ ] Capture baseline `sysctl vm.swapusage`, `vm_stat`, and process inventory.
- [ ] Initialize or resume the local deep-research packet under this spec folder's `research/` directory.

### Phase 2: Core Implementation

- [ ] Iteration 001, Claude Opus route: inventory process-spawn entrypoints and background execution patterns.
- [ ] Iteration 002, Claude Opus route: audit `/spec_kit:deep-research` state, locks, reducer writes, and failed-dispatch cleanup.
- [ ] Iteration 003, Claude Opus route: audit deep-review, AI council, and multi-agent workflow process behavior.
- [ ] Iteration 004, Claude Opus route: audit cli-X self-invocation guards, nested routing, and cross-CLI loop risks.
- [ ] Iteration 005, Claude Opus route: audit Apple Silicon swap, wired memory, sidecar, and daemon classification logic.
- [ ] Iteration 006, Codex GPT-5.5 xhigh fast route: independently map process owners and cleanup gaps.
- [ ] Iteration 007, Codex GPT-5.5 xhigh fast route: inspect MCP server, context server, reducers, and save workflows for long-lived handles.
- [ ] Iteration 008, Codex GPT-5.5 xhigh fast route: inspect `ccc search`, reranker sidecar, Ollama, embedder, `gtimeout`, and browser/devtools leftovers.
- [ ] Iteration 009, Codex GPT-5.5 xhigh fast route: inspect stale locks, pause files, resume behavior, and interrupted-session recovery.
- [ ] Iteration 010, Codex GPT-5.5 xhigh fast route: synthesize leak taxonomy and verify the remediation backlog against evidence.

### Phase 3: Verification

- [ ] Validate every iteration has markdown, JSONL, telemetry, and cleanup evidence.
- [ ] Confirm no unexpected dispatcher process remains after iteration 010 or early stop.
- [ ] Run strict spec validation after writing synthesis and continuity metadata.
