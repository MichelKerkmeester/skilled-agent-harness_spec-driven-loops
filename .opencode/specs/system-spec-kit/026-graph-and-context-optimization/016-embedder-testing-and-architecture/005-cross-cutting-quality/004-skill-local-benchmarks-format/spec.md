...
| Level | 2 |
| Priority | P1 |
| Status | IN PROGRESS — folder scaffolding + FORMAT.md shipped; benchmark_report.md + READMEs pending sk-doc agent dispatch |
| Branch | main |
...
**Convention layer:**
- Define `mcp_server/benchmarks/` folder structure (FORMAT.md)
...
- Required top-level files per skill: `README.md` (index) + `FORMAT.md` (single source via symlink across skills)
...
- `FORMAT.md` (single source in `system-spec-kit/...`; symlinked from `mcp-coco-index/...`)
...
|----|-------------|-----------|
| REQ-001 | `FORMAT.md` exists at `.opencode/skills/system-spec-kit/mcp_server/benchmarks/FORMAT.md` documenting the convention | file exists, lists 10-section `benchmark_report.md` structure + file purposes + date convention |
| REQ-002 | FORMAT.md is symlinked into `.opencode/skills/mcp-coco-index/mcp_server/benchmarks/FORMAT.md` | `readlink` confirms relative symlink to system-spec-kit copy |
...
| ID | Requirement | Acceptance |
|----|-------------|-----------|
| REQ-007 | All four primary docs (FORMAT.md, 2× README.md, 2× benchmark_report.md) include in-doc date prose as "May 17, 2026" / "May 18, 2026" (not ISO inline) | grep confirms long-form dates in prose |
...
- **SC-002:** Format is reusable — future MCPs (or forks) can adopt by copying the FORMAT.md and replicating the layout. No system-spec-kit-specific assumptions baked in.
...
- **Drift risk** — if the spec packet's `decision-record.md` adds new ADRs and the skill-local report doesn't get refreshed, readers in the skill folder see stale data. Mitigation: `SOURCE.md` cites the spec packet's `decision-record.md` directly + the authority hierarchy in FORMAT.md states explicitly that ADRs win.
...
- **Symlink rot** — the FORMAT.md symlink from `mcp-coco-index/.../FORMAT.md` to `system-spec-kit/.../FORMAT.md` breaks if either path moves. Mitigation: relative symlink (works from any worktree); if breakage occurs the validate.sh will surface it.
