...
| Daemon classifier sweep | Phase 005 is dry-run only. It marks eligible rows but sends no signals, even with `apply --confirmed`. | `node .opencode/skills/system-spec-kit/scripts/dist/ops/process-sweep.js plan --pretty` then `node .opencode/skills/system-spec-kit/scripts/dist/ops/process-sweep.js apply --confirmed <operator-token> --pretty` |
...
| 005 | Daemon classifier + dry-run sweep | `process-sweep.js fixture --pretty`; `apply --confirmed` remains non-destructive | `node mcp_server/node_modules/vitest/vitest.mjs run scripts/tests/process-memory-harness.vitest.ts scripts/tests/process-sweep.vitest.ts --config mcp_server/vitest.config.ts` |
