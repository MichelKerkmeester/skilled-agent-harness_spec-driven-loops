...
- **1 arc parent** (`006-mcp-launcher-concurrency-arc/spec.md` + `graph-metadata.json`): phase-map row for 011 plus backfill of 005, 007, 010 into `children_ids` (previous saves drifted).
...
- **Sideways**: arc parent `006-mcp-launcher-concurrency-arc` is the phase-parent control file; its `spec.md` phase-map and `graph-metadata.json` children-list need backfill.
