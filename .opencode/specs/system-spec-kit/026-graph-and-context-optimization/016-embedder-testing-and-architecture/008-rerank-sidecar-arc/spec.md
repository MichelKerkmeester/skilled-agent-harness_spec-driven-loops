...
| 005 | `005-promote-qwen-as-default/` | Planned | If 004 proves out, flip `cross-encoder.ts:55` default from `cross-encoder/ms-marco-MiniLM-L-6-v2` to `Qwen/Qwen3-Reranker-0.6B`. Set `SPECKIT_CROSS_ENCODER=true` in the 4 runtime configs. Update `ENV_REFERENCE.md`, `embedder_architecture.md`, and the cocoindex side's runtime configs to point at the shared skill instead of bundling a local copy of Qwen. |
<!-- /ANCHOR:phase-map -->
