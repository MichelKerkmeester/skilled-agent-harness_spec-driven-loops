...
## 2. TOPIC
Review target: 011-spec-memory-rerank-decision-arc (Level: phase-parent + 5 child packets). Scope includes the arc's terminal closure (011/005), the code patch landed at lib/search/search-flags.ts + lib/search/confidence-scoring.ts + tests/scoring-opt-in.vitest.ts, the supersede sweep across 7 sibling packets (011/002+003+004 + 008/005+007+008+009), docs at system-spec-kit/SKILL.md + system-rerank-sidecar/SKILL.md, and supporting evidence at 011/001-off-baseline-audit/evidence/off-baseline-2026-05-21.json + 011/002-bge-v2-m3-trial/evidence/bge-v2-m3-bench-2026-05-21.json + AI Council deliberation output. Verdict expected: PASS/CONDITIONAL/FAIL with hasAdvisories.
...
## 4. NON-GOALS
NOT in scope: cocoindex side (mcp-coco-index), sidecar Python source (system-rerank-sidecar/scripts/), Phase 1 evidence content (treated as historical reference), tests/scoring-opt-in.vitest.ts implementation details beyond contract correctness, broader spec-kit MCP server architecture beyond the touched files.

---

## 5. STOP CONDITIONS
1. P0 finding count plateaus AND coverage_age >= 1 AND all 4 dimensions reviewed at least twice. 2. Max iterations reached (10). 3. Stuck recovery fails after 2 consecutive no-progress passes.
...
```markdown
## 1. REVIEW CHARTER
- Target: .opencode/skills/deep-research (skill, v1.4.0)
...
- Files: .opencode/skills/deep-research/scripts/reduce-state.cjs, .opencode/skills/system-spec-kit/scripts/tests/deep-research-contract-parity.vitest.ts
