...
description: "Implementation plan for Expected Daemon Classifier and Process Sweep."
trigger_phrases:
...
### Overview
Provide a safe inventory-first cleanup surface for daemons, sidecars, browser tooling, and CLI leftovers. The phase must preserve source evidence from the research packets and avoid claiming memory relief without process and telemetry proof.
<!-- /ANCHOR:summary -->
...
- **Implementation surface**: `.opencode/skills/system-spec-kit/`, `.opencode/skills/mcp-coco-index/`, `.opencode/skills/system-rerank-sidecar/`.
...
|---------|--------------|--------|--------------|
| `.opencode/skills/system-spec-kit/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Fixture process tables for ancestors, EPERM, stale PIDs, sidecars, ccc daemons, browser sessions |
| `.opencode/skills/mcp-coco-index/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Fixture process tables for ancestors, EPERM, stale PIDs, sidecars, ccc daemons, browser sessions |
| `.opencode/skills/system-rerank-sidecar/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Fixture process tables for ancestors, EPERM, stale PIDs, sidecars, ccc daemons, browser sessions |
