...
### Overview
Bound rerank sidecar, local model, adapter, fallback, and HTTP client lifetimes. The phase must preserve source evidence from the research packets and avoid claiming memory relief without process and telemetry proof.
<!-- /ANCHOR:summary -->
...
- **Implementation surface**: `.opencode/skills/system-rerank-sidecar/`, `.opencode/skills/mcp-coco-index/mcp_server/cocoindex_code/rerankers/`, `.opencode/skills/system-spec-kit/`.
...
|---------|--------------|--------|--------------|
| `.opencode/skills/system-rerank-sidecar/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Healthy reuse, unknown-owner refusal, stale exact-PID cleanup, sidecar 5xx fallback RSS, adapter close idempotence |
| `.opencode/skills/mcp-coco-index/mcp_server/cocoindex_code/rerankers/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Healthy reuse, unknown-owner refusal, stale exact-PID cleanup, sidecar 5xx fallback RSS, adapter close idempotence |
| `.opencode/skills/system-spec-kit/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Healthy reuse, unknown-owner refusal, stale exact-PID cleanup, sidecar 5xx fallback RSS, adapter close idempotence |
