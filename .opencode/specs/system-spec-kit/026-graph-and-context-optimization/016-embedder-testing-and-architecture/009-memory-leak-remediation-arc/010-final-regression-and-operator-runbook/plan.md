...
description: "Implementation plan for Final Regression and Operator Runbook."
trigger_phrases:
...
### Overview
Prove the memory-leak remediation arc end-to-end and document operator recovery guidance. The phase must preserve source evidence from the research packets and avoid claiming memory relief without process and telemetry proof.
<!-- /ANCHOR:summary -->
...
- **Implementation surface**: `system-spec-kit/026-graph-and-context-optimization/016-embedder-testing-and-architecture/009-memory-leak-remediation-arc/`, `.opencode/skills/system-spec-kit/`, `.opencode/skills/mcp-coco-index/`, `.opencode/skills/system-code-graph/`.
...
|---------|--------------|--------|--------------|
| `system-spec-kit/026-graph-and-context-optimization/016-embedder-testing-and-architecture/009-memory-leak-remediation-arc/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Full harness replay, process baseline comparison, strict spec validation, memory index scan when available |
| `.opencode/skills/system-spec-kit/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Full harness replay, process baseline comparison, strict spec validation, memory index scan when available |
| `.opencode/skills/mcp-coco-index/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Full harness replay, process baseline comparison, strict spec validation, memory index scan when available |
| `.opencode/skills/system-code-graph/` | Phase implementation or evidence surface | Inspect and update only within this phase scope | Full harness replay, process baseline comparison, strict spec validation, memory index scan when available |
