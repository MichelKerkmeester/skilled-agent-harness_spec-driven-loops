...
| **P1-D3** | `doctor-apply-mode-stress.vitest.ts` has two skipped tests citing "026/000/002-vitest-recovery-followup requires missing fixture, daemon, auth, or offline-unavailable toolchain". These tests cover the read-only vs apply-mode boundary — safety-critical for doctor apply mode that mutates config files. | `stress_test/code-graph/doctor-apply-mode-stress.vitest.ts:125, 184` | Restore the missing fixtures/auth or replace with mocked alternatives. |
