# Deep Review Iteration 008 of 20 — system-code-graph: code_graph_context handler + code-graph-context.ts correctness (neighborhood retrieval)

## SITUATION

You are running as cli-devin SWE-1.6 in non-interactive `--print` mode, dispatched from Claude Code as iteration 008 of a 20-iteration deep-review campaign on the standalone `system-code-graph` skill (v1.0.0.0). Working directory: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public`.
...
**Focus this iteration:** code_graph_context handler + code-graph-context.ts correctness (neighborhood retrieval)

## SCOPE (this iteration only)

.opencode/skills/system-code-graph/mcp_server/handlers/context.ts, lib/code-graph-context.ts
