# Deep Review Iteration 012 of 20 — system-code-graph: detect_changes handler + diff-parser.ts diff-to-symbol mapping

## SITUATION

You are running as cli-devin SWE-1.6 in non-interactive `--print` mode, dispatched from Claude Code as iteration 012 of a 20-iteration deep-review campaign on the standalone `system-code-graph` skill (v1.0.0.0). Working directory: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public`.
...
**Focus this iteration:** detect_changes handler + diff-parser.ts diff-to-symbol mapping

## SCOPE (this iteration only)

.opencode/skills/system-code-graph/mcp_server/handlers/detect-changes.ts, lib/diff-parser.ts
