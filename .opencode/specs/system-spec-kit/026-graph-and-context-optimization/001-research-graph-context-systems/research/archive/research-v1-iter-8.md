...
```mermaid
graph TD
  C10["10 Honest token reporting"] --> C19["19 Pricing and cache normalization"]
  C10 --> C17["17 Runtime FTS capability cascade"]
  C21["21 Stop-time summaries"] --> C20["20 SessionStart fast path"]
  C21 --> C24["24 Pricing and cache normalization"]
  C15["15 Schema-boundary validator"] --> C11["11 Evidence/confidence tags"]
  C15 --> C18["18 Stable JSON artifact"]
  C15 --> C13["13 Two-layer cache invalidation"]
  C02["2 AST/regex confidence labels"] --> C07["7 SQLAlchemy extraction"]
  C02 --> C11
  C23["23 Consolidation contract"] --> C27["27 Placement rubric"]
  C23 --> C22["22 Auditor vs discoverer split"]
  C09["9 MCP scaffold"] --> C12["12 Graph-first hook"]
  C03["3 Profile overlays"] --> C04["4 Static artifacts + overlay"]
  C24 --> C25["25 Cache-cliff metric"]
  C24 --> C26["26 Dashboard JSON contracts"]
  C19 --> C28["28 Search/browse separation"]
  C18 --> C16["16 Leiden clustering"]
  C04 --> C01["1 Canonical ScanResult"]
  C20 --> C08["8 Bootstrap layering"]
  C13 --> C17["17 Rebuild policy"]
```
