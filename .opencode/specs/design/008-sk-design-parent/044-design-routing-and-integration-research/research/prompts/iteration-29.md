...
STATE SUMMARY (auto):
Iteration 29 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.67
Active angle: [D3-A12/D3] four-copy intent-vocabulary drift (graph-metadata, hub SKILL prose, mode-registry aliases, mode INTENT_SIGNALS) — a drift gate (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D3-A12 / D3] (corpus: local-repo):
four-copy intent-vocabulary drift (graph-metadata, hub SKILL prose, mode-registry aliases, mode INTENT_SIGNALS) — a drift gate
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D3-A12.
Last 3 Iterations Summary: run 26: [D3-A9/D3] sub-agent/small-model dispatch utilizat (0.64); run 27: [D3-A10/D3] standing routing self-test gold corpus (0.66); run 28: [D3-A11/D3] default-to-interface as a silent miss  (0.67)
...
- Write iteration narrative to: .opencode/specs/design/008-sk-design-parent/044-design-routing-and-integration-research/research/iterations/iteration-029.md
- Write per-iteration delta file to: .opencode/specs/design/008-sk-design-parent/044-design-routing-and-integration-research/research/deltas/iter-029.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/design/008-sk-design-parent/044-design-routing-and-integration-research/research/iterations/iteration-029.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/design/008-sk-design-parent/044-design-routing-and-integration-research/research/deltas/iter-029.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
