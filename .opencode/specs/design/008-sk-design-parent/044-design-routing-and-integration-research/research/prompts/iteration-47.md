...
STATE SUMMARY (auto):
Iteration 47 of 50 | NON-CONVERGING (convergence is a signal, not a stop) | last ratio: 0.68
Active angle: [D5-A2/D5] model the design manifest on the proven Gate-3 spec-folder pass-through (manifest present + sk-design loaded + register set, or ASK before dispatch) (corpus: local-repo)
...
THIS ITERATION'S ANGLE [D5-A2 / D5] (corpus: local-repo):
model the design manifest on the proven Gate-3 spec-folder pass-through (manifest present + sk-design loaded + register set, or ASK before dispatch)
Remaining Key Questions: - See strategy Section 3 (Q1-Q5).
Carried-Forward Open Questions:
Advancing the angle bank; current angle D5-A2.
Last 3 Iterations Summary: run 44: [D4-A13/D4] HTTP/fourth-surface escape - only daem (0.7); run 45: [MON-B3/D6] designer-skills-main 9-plugin command  (0.74); run 46: [D5-A1/D5] dispatch-contract asymmetry: cli-* have (0.68)
...
- Write iteration narrative to: .opencode/specs/design/008-sk-design-parent/044-design-routing-and-integration-research/research/iterations/iteration-047.md
- Write per-iteration delta file to: .opencode/specs/design/008-sk-design-parent/044-design-routing-and-integration-research/research/deltas/iter-047.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/design/008-sk-design-parent/044-design-routing-and-integration-research/research/iterations/iteration-047.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/design/008-sk-design-parent/044-design-routing-and-integration-research/research/deltas/iter-047.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
