---
title: "Decision Record: design-audit impeccable adoption"
description: "Binding decisions: scope-locked cli-codex dispatch with read-first skip-if-present + per-diff verification (ADR-001); adopt only as crosswalk refinements into design-audit with no new mode or parallel system, then independently fresh-Opus-verify (ADR-002)."
trigger_phrases:
  - "033-audit-impeccable-adoption decisions"
  - "impeccable audit ADRs"
  - "sk-design impeccable audit decisions"
...
_memory:
  continuity:
    ...
    recent_action: "Recorded the audit build ADRs"
    ...
    answered_questions: []
---
# Decision Record: design-audit impeccable adoption
...
### Context
The audit references are mature and prior corpus adoptions already pulled craft, so duplication is a real risk. The operator's goal named cli-codex gpt-5.5 high fast for the build and fresh Opus agents for verification.

### Constraints
- Edits additive and confined to the named audit files.
...
<!-- ANCHOR:adr-001-impl -->
### Implementation
**What changes**: the named audit files edited additively.
...
### Decision
**We chose**: adopt the audit items only as additive crosswalk refinements into the existing references; build no ruled-out system and no new mode.
<!-- /ANCHOR:adr-002-decision -->
...
<!-- ANCHOR:adr-002-impl -->
### Implementation
**What changes**: the audit references gain the named items.
