...
This iteration's focus (from strategy): Sweep the shared design laws in `skill/SKILL.src.md` (color, typography, layout, motion, interaction, the absolute-bans / match-and-refuse section, and the `<codex>`/`<gemini>` per-model defect blocks). Crosswalk each concrete/numeric rule onto its sk-design home (foundations or audit) and VERIFY against the real current sk-design file; mark each candidate net-new, already-covered, or out-of-scope-infra.
...
- Which of impeccable's numeric design laws (hero clamp ≤6rem, display tracking floor ≥-0.04em, body line-length 65-75ch, semantic z-index scale, anti-cream OKLCH band L0.84-0.97/C<0.06/hue40-100, ease-out-quart/quint/expo) are net-new thresholds versus already present in design-foundations after the 022/023 adoption? (init)
...
- Write iteration narrative to: .opencode/specs/design/008-sk-design-parent/028-impeccable-design-research/research/iterations/iteration-001.md
- Write per-iteration delta file to: .opencode/specs/design/008-sk-design-parent/028-impeccable-design-research/research/deltas/iter-001.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/design/008-sk-design-parent/028-impeccable-design-research/research/iterations/iteration-001.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/design/008-sk-design-parent/028-impeccable-design-research/research/deltas/iter-001.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
