...
- **Spec folder:** `146-mcp-figma-with-direct-cli-support` (phase parent)
...
## 4. Risk register (top)
npm binary collision (H/H — `figma-cli` npm is unrelated; canonicalize `figma-ds-cli`) · yolo patch reversibility (M/H — safe default, `unpatch`) · official-MCP http/OAuth/catalog unproven (H/M — mcp-remote bridge) · Framelink naming conflict (M/M — `figma_official_*` names) · MCP schema drift (M/M — verify via tool_info) · export overwrite (M/M — explicit path) · figma-cli doc drift (M/H — live `--help`) · token exposure (L/H — redact) · design-judgment scope creep (H/M — route to sk-design-interface) · cross-platform overclaim (H/L — macOS baseline). Full: `iterations/iteration-005.md`.
