...
description: "Make sk-design-interface a standalone skill by removing every mention of the internal mcp-open-design skill and the Open Design app from its live content. Split the shared parity doc: the generic real-UI loop stays here (renamed real_ui_loop.md, vendor-neutral); the Open Design transport mechanics move into mcp-open-design as design_parity_transport.md. The reverse coupling (mcp-open-design -> sk-design-interface) is preserved."
trigger_phrases:
  - "sk-design-interface decouple open design"
...
_memory:
  continuity:
    packet_pointer: "design/001-sk-design-interface/010-decouple-from-open-design"
    last_updated_at: "2026-06-17T00:00:00Z"
    last_updated_by: "claude-opus"
    recent_action: "Split parity doc, stripped Open Design naming from sk-design-interface, repointed consumers"
...
| **Handoff Criteria** | Zero `mcp-open-design`/Open Design mentions in sk-design-interface live content; the reverse coupling from mcp-open-design stays intact; all repointed links resolve; `validate.sh --strict` exits 0 |
<!-- /ANCHOR:metadata -->
...
This is **Phase 10** of the sk-design-interface specification.

**Scope Boundary**: Remove Open Design coupling from `sk-design-interface` so it reads as a standalone skill, and split the shared parity doc. Edits `sk-design-interface` (its content), plus cross-skill repoints in `mcp-open-design` and `sk-prompt`. No change to `mcp-open-design`'s mandatory `sk-design-interface` coupling logic. Mobbin/Refero stay (the routing upgrade is phase 011).
...
**Deliverables**:
- `real_ui_loop.md` (generic), `mcp-open-design/references/design_parity_transport.md` (transport), all Open Design naming stripped from sk-design-interface content, repointed consumers, version bump + changelog.

**Changelog**:
- `sk-design-interface/changelog/v1.4.0.0.md`.
<!-- /ANCHOR:phase-context -->
...
### Problem Statement
`sk-design-interface` is vendored from Anthropic's Apache-2.0 `frontend-design` skill and is meant to be reusable standalone, but it accumulated ~101 references to the internal `mcp-open-design` skill and the Open Design app across 23 files. An external user adopting it alone sees an internal sibling skill and an app they do not have. The shared `claude_design_parity.md` is the crux: it lives here but is the Open Design integration protocol.

### Purpose
Make `sk-design-interface` standalone: it names neither `mcp-open-design` nor the Open Design app. Keep the integration one-way — `mcp-open-design` still names this skill as its mandatory judgment partner (150/011), but this skill knows nothing about it.
<!-- /ANCHOR:problem -->
...
- Strip all `mcp-open-design`/Open Design naming from sk-design-interface content (SKILL.md, README, design_inventory.md, design_references_mcp.md, variation_diversity.md, feature_catalog, manual_testing_playbook, graph-metadata.json).
...
|-----------|-------------|-------------|
| `sk-design-interface/references/design-process/claude_design_parity.md` | Rename+rewrite | -> `real_ui_loop.md`, vendor-neutral, transport stripped |
...
| `sk-design-interface/{SKILL.md,README.md,graph-metadata.json}` + `references/**` + `feature_catalog/**` + `manual_testing_playbook/**` | Modify | Strip Open Design naming; repoint links |
...
|----|-------------|---------------------|
| REQ-001 | Standalone | Live-content grep over sk-design-interface (excl. changelog) finds zero `mcp-open-design` / "Open Design" / `od mcp` hits |
...
| REQ-003 | Reverse coupling intact | `mcp-open-design` SKILL.md still carries the MANDATORY PAIRING banner, hard gate, RULES, and success-criteria naming `sk-design-interface` |
...
- **SC-001**: `rg -i 'mcp-open-design|open design|\bod mcp\b' sk-design-interface --glob '!changelog/**'` returns zero hits.
- **SC-002**: The split docs exist and cross-reference correctly; `claude_design_parity.md` no longer exists.
- **SC-003**: `mcp-open-design` still mandates `sk-design-interface` (reverse coupling unbroken).
