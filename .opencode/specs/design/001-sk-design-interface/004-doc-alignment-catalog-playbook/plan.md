---
title: "Implementation Plan: sk-design-interface doc alignment, catalog, playbook"
description: "Plan to bring sk-design-interface to full sk-doc template alignment via fresh markdown agents: a feature catalog, a manual testing playbook, aligned references, documented assets, and accurate graph-metadata."
trigger_phrases:
  - "sk-design-interface doc alignment plan"
...
# Implementation Plan: sk-design-interface doc alignment, catalog, playbook
...
| **Storage** | `.opencode/skills/sk-design-interface/` (feature_catalog/, manual_testing_playbook/, references/, assets/) |
| **Testing** | sk-doc `validate_document.py`, `package_skill.py`, `validate.sh --strict` |

### Overview
Bring `sk-design-interface` to full sk-doc template alignment via fresh markdown agents using the /create workflows: a feature catalog, a manual testing playbook, template-aligned references, a self-documenting assets dir, and accurate graph-metadata.
<!-- /ANCHOR:summary -->
...
|---------|--------------|--------|--------------|
| `sk-design-interface/feature_catalog/` | New | create (agent) | validate_document + package_skill |
| `sk-design-interface/manual_testing_playbook/` | New | create (agent) | validate_document + package_skill |
| `sk-design-interface/references/*.md` | Existing | align (agent) | validate_document --type reference |
| `sk-design-interface/{SKILL.md,graph-metadata.json}` | Existing | reconcile (orchestrator) | package_skill, JSON parse |
| `sk-design-interface/assets/data/README.md` | New | add (orchestrator) | present + links resolve |
| `sk-design-interface/{LICENSE*,THIRD-PARTY-NOTICES}.{txt,md}` | Existing | restored from HEAD after a concurrent worktree deletion | on-disk present |
<!-- /ANCHOR:affected-surfaces -->
