...
# Research Specification: Claude Design parity for sk-design-interface and mcp-magicpath
...
| **Scope** | Two skills: `sk-design-interface` (148) and `mcp-magicpath` (147) |
<!-- /ANCHOR:metadata -->
...
### Problem Statement
The framework has two UI-design skills: `sk-design-interface` (design judgment plus the merged critique-against data) and `mcp-magicpath` (a MagicPath canvas-generation CLI that already depends on sk-design-interface). The operator wants their results to get closer to **Claude Design** (Anthropic Labs' conversational design tool): conversational generation of designs, prototypes, presentations, and more, with org design-system inheritance, canvas plus inline-comment iteration, context attachments, multi-format export, and Claude Code handoff. Where the two skills fall short of that experience is not yet mapped.
...
**In scope:**
- A parity gap analysis of `sk-design-interface` and `mcp-magicpath` against Claude Design's capabilities (design-system inheritance, iteration and visual feedback, context grounding, quality levers, output and handoff).
...
- R2: Per-skill improvements ranked ADOPT / ADAPT / SKIP with a stated reason and the philosophy guardrail (sk-design-interface stays anti-default; not a templated generator).
...
- How can sk-design-interface and mcp-magicpath be improved to get their UI-design results closer to Claude Design (Anthropic's conversational design tool): what concrete, prioritized improvements to each skill close the gap on design-system inheritance, iteration and visual feedback, context grounding, quality levers, and output and handoff, scored against Claude Design's capabilities, and what to leave out.
...
- Skill 1: `.opencode/skills/sk-design-interface/` (design_principles.md, ux_quality_reference.md, design_inventory.md, 9 data CSVs, design_search.py, feature_catalog, manual_testing_playbook).
- Skill 2: `.opencode/skills/mcp-magicpath/` (magicpath-ai CLI, Design Defaults, theme/@theme flow, the depends_on link to sk-design-interface; known gaps: no iteration loop, no token generation, no fidelity verification, Design Defaults canvas-only).
