# Research Synthesis — Claude Design Parity for sk-design-interface and mcp-magicpath
...
Claude Design (Anthropic's conversational design tool, seeded from the spec / support article): conversational generation of designs/prototypes/presentations; **org design-system inheritance**; **canvas + inline-comment iteration**; **context attachments**; **multi-format export**; **Claude Code handoff**; **named quality levers** [SOURCE: file:.opencode/specs/design/001-sk-design-interface/005-claude-design-parity-research/spec.md:141]. These map to the five R1 assessment dimensions used throughout.

---

## 4. Current-State Baseline

- `sk-design-interface` is a **judgment** skill, not a generation skill: ground → token system → critique-against-defaults → build → self-critique, owning the *look* and handing implementation to `sk-code` [SOURCE: file:.opencode/skills/sk-design-interface/SKILL.md:92] [SOURCE: file:.opencode/skills/sk-design-interface/SKILL.md:184]. Its differentiator is the anti-default philosophy [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:59].
- `mcp-magicpath` is a **CLI** over the `magicpath-ai` canvas tool: install (search/inspect/add → React/TS) + author (code start/submit revisions) + repo import, with themes and Design Defaults, and it `depends_on` `sk-design-interface` for the visual direction [SOURCE: file:.opencode/skills/mcp-magicpath/SKILL.md:250].
- **Key structural fact (F9):** the depends_on composition means judgment-side gaps belong to `sk-design-interface` and canvas/CLI-side gaps to `mcp-magicpath`; the recommendation stays per-skill plus one shared protocol.
...
- **sk-design-interface (score 1/3):** has token *content* (4–6 named hex + display/body/utility faces + scale) but only in thinking, with no ingest or emit — yet grounding already endorses using memory/brand context "as a hint." Inheritance is *endorsed-but-absent* (F3/F10) [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:61] [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:37].
...
- **Moves:** ADAPT `sk-design-interface` "inherit-if-present" Step 0.5 + optional token emit; ADAPT `mcp-magicpath` repo→theme bridge.
...
- **sk-design-interface (score 1/3):** iterates *privately* — "plan and iterate in your thinking, only show ideas at higher confidence," self-critique = "screenshot if possible, remove one accessory." This is the deliberate *inverse* of Claude Design's open, comment-driven loop (F14) [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:67].
- **mcp-magicpath (score 2/3):** has a real canvas with pending revisions (`code start`/`submit`, `--revision`, `selectedRevisionId`), but an **open** loop — `submit --wait` verifies the *build* compiled, never that the render *matches intent*; "no iteration loop, no fidelity verification" is a named gap (F6/F15) [SOURCE: file:.opencode/skills/mcp-magicpath/references/magicpath_operations.md:289] [SOURCE: file:.opencode/specs/design/001-sk-design-interface/005-claude-design-parity-research/spec.md:143].
- **Critical insight (F16):** the loop is a *missing protocol, not missing parts* — judgment (`sk-design-interface`) + canvas render (`mcp-magicpath` `view`/`share`) + screenshot (`mcp-chrome-devtools`, already named) compose into a closed loop with no new machinery [SOURCE: file:.opencode/skills/sk-design-interface/SKILL.md:188].
- **Moves:** **ADOPT `mcp-magicpath` fidelity verification (P1)**; ADOPT the shared loop protocol (P1); ADAPT `sk-design-interface` rendered self-critique loop; ADAPT `mcp-magicpath` selection→targeted-revision as the inline-comment analog.
...
- **sk-design-interface (score 1/3):** grounds in the subject + memory hints but has no structured attachment intake (F2/F12) [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:37].
...
- **Moves:** ADAPT `sk-design-interface` "context intake" framed inherit-from/critique-against (never copy); ADAPT `mcp-magicpath` image-as-visual-brief (route into the judgment skill, not just embed).
...
- **sk-design-interface (score 2/3):** has the *concepts* as fixed principles — a binary quality floor (responsive/focus/reduced-motion) plus complexity/density, restraint/boldness, and motion principles — but not as steerable dials (F4/F17) [SOURCE: file:.opencode/skills/sk-design-interface/SKILL.md:115] [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:51].
...
- **Moves:** ADAPT `sk-design-interface` to *name* boldness/density/motion as bounded intensity dials — guardrailed so they never become presets; ADAPT `mcp-magicpath` to surface target-device, and otherwise defer levers to `sk-design-interface`.
...
- **sk-design-interface (score 0/3):** produces no artifact; its handoff is prose intent to `sk-code` (F2/F19) [SOURCE: file:.opencode/skills/sk-design-interface/SKILL.md:184].
...
- **Moves:** ADAPT `sk-design-interface` to emit a structured handoff artifact (token system + signature + copy rules); **ADOPT `mcp-magicpath` design-token export (P2)**; frame share/view URLs as the explicit preview deliverable.
...
| # | Dimension | Target | sk-design-interface | mcp-magicpath | Headline move |
|---|---|---|:--:|:--:|---|
...
**Read:** `mcp-magicpath` leads on the mechanical dimensions (1,2,3,5) because it owns the canvas/CLI; `sk-design-interface` leads only on judgment (4). Close the gap by giving `mcp-magicpath` the missing *loop + export* and `sk-design-interface` the missing *inherit + emit*, wired by one shared protocol.
...
| P1 | ADOPT | Shared visual-feedback-loop protocol (F16) | Document intent→render→screenshot→compare→targeted-revision→stop, shared with sk-design-interface |
...
| Heavyweight visual-regression / diff engine for fidelity | Screenshot + judgment compare already afforded | sk-design-interface/SKILL.md:188 | 3 |
| **Style presets / "pick-a-vibe" lever menu in sk-design-interface** | **Reintroduces templated defaults — violates the skill's core anti-default philosophy (primary)** | sk-design-interface/SKILL.md:133 | 4 |
...
| Self-owned rendering pipeline in sk-design-interface | Judgment skill; rendering belongs to sk-code/magicpath/chrome-devtools | sk-design-interface/SKILL.md:184 | 4 |
| Duplicating quality levers inside magicpath | Splits the source of truth; levers live once in sk-design-interface | magicpath_operations.md:295 | 4 |
...
- **Watch for divergence:** (a) whether `sk-design-interface` should expose *any* quality levers (philosophy risk — sibling may rate ADOPT vs this lineage's cautious P3 ADAPT); (b) token emit/export shape (structured block vs full token file); (c) whether the feedback loop is one shared protocol (this lineage) or duplicated per skill.
...
- spec.md — topic, seeded Claude Design capabilities, R1–R5, success criteria, named magicpath gaps [SOURCE: file:.opencode/specs/design/001-sk-design-interface/005-claude-design-parity-research/spec.md:1]
- sk-design-interface/SKILL.md — process, quality floor, anti-default rules, integration boundary [SOURCE: file:.opencode/skills/sk-design-interface/SKILL.md:1]
- sk-design-interface/references/design_principles.md — grounding, token brainstorm, calibration, restraint, self-critique [SOURCE: file:.opencode/skills/sk-design-interface/references/design_principles.md:1]
...
- Lineage iterations 001–005 [SOURCE: file:.opencode/specs/design/001-sk-design-interface/005-claude-design-parity-research/research/lineages/opus48-claude2/iterations/iteration-001.md:1]
