...
1. **T2 — Constrain generation to a registered design system / component library (keystone).**
   ...
   - `sk-design-interface`: **ADAPT** — a critique check ("am I reinventing an existing system component?"); stays judgment.
...
3. **T1 — Direct-manipulation token/markup edits (no AI turn).**
   ...
   - `sk-design-interface`: **SKIP** — not its surface.
4. **T4 — Persistent project "Knowledge"/design-brief file.**
   - `sk-design-interface`: **ADAPT** — read a persisted design brief if present to ground Step 0 across sessions; read-if-present, never a chooser. (Lovable F9, v0 Sources F2.)
