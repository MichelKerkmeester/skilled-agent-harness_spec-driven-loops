...
## Focus
Survey **v0 by Vercel** (v0.app, formerly v0.dev) for distinctive, design-relevant capabilities — features, workflow, and quality mechanisms — that `sk-design-interface` or `mcp-magicpath` could adopt. Establish the cross-cutting frame for the lineage.
...
- **Relevance:** Multi-variant generation. DANGEROUS for `sk-design-interface` (a "pick-a-vibe" menu violates the anti-default mandate). Overlaps 005's "named bounded quality levers (variations)" ADAPT-cautious note. OVERLAP; carry the anti-default guardrail.
