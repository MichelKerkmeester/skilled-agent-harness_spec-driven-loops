...
Alignment Iteration: 1 of 10
Mode: alignment
Spec Folder: .opencode/specs/sk-design/012-style-database-and-interface-commands
Alignment Packet Root: .opencode/specs/sk-design/012-style-database-and-interface-commands/alignment
Session: 2026-07-19T11:57:36Z (generation 1, lineage new)
...
- Artifacts Slice (check exactly these this iteration): [{"path":".opencode/skills/sk-design/styles/_db/__tests__/adapter.test.mjs"},{"path":".opencode/skills/sk-design/styles/_db/__tests__/fixtures.mjs"},{"path":".opencode/skills/sk-design/styles/_db/__tests__/indexer.test.mjs"},{"path":".opencode/skills/sk-design/styles/_db/__tests__/retrieval.test.mjs"},{"path":".opencode/skills/sk-design/styles/_db/__tests__/schema.test.mjs"}]
...
- Config (read-only): .opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deep-alignment-config.json
- Corpus (read-only): .opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deep-alignment-corpus.json
- State Log (read + Bash-append): .opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deep-alignment-state.jsonl
- Findings Registry (read-only, reducer-owned): .opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deep-alignment-findings-registry.json
- Write this iteration's narrative to: .opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/iterations/iteration-001.md
- Write this iteration's delta file to: .opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deltas/iter-001.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to):**
  - `.opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/iterations/iteration-001.md`, this iteration's narrative markdown (Bash-create a new file)
  - `.opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deep-alignment-state.jsonl`, append-only JSONL state log (Bash `>>` append exactly ONE iteration record)
  - `.opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deltas/iter-001.jsonl`, this iteration's write-once delta JSONL (Bash-create once)
...
1. **Iteration narrative markdown** at `.opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/iterations/iteration-001.md` (Bash heredoc create; the file must not already exist). Include sections: Dispatcher, Lane, Artifacts Checked, Findings - New (P0/P1/P2 subsections), Verify-First Evidence, Known-Deviation Suppressions Applied, Edge Cases, Confirmed-Clean Artifacts, Ruled Out, Next Focus. `Next Focus` is a human-readable echo of what `partition-corpus.cjs` returns, never a hand-authored plan.

2. **Canonical JSONL iteration record** APPENDED (single line, newline-terminated) to `.opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deep-alignment-state.jsonl`. The record MUST use `"type":"iteration"` EXACTLY. Required route proof: `"iteration":<n>`, `"mode":"alignment"`, `"target_agent":"deep-alignment"`, `"agent_definition_loaded":true`, `"resolved_route":"Resolved route: mode=alignment target_agent=deep-alignment"`. Required alignment data: `run`, `status`, `laneId`, `authority`, `artifactClass`, `scope`, `artifactsChecked` (JSON array of the artifact paths you audited this iteration, not a bare count), `findingsCount`, `findingsSummary`, `findingsNew`, `findingDetails`, `newFindingsRatio`, `sessionId`, `generation`, `lineageMode`, `timestamp`, `durationMs`. Allowed `status`: `complete | timeout | error | stuck | insight | thought`. Append via `printf '%s\n' '<single-line-json>' >> .opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deep-alignment-state.jsonl`; do NOT pretty-print and do NOT print to stdout only.

3. **Per-iteration delta file** at `.opencode/specs/sk-design/012-style-database-and-interface-commands/alignment/deltas/iter-001.jsonl` (Bash-create once; never overwrite). Its first line MUST contain the same canonical iteration record as the state-log append, with `laneId` present, followed by structured `{"type":"finding","laneId":"...","finding":{...}}` lines (one per line) in the adapter's own raw finding shape.
