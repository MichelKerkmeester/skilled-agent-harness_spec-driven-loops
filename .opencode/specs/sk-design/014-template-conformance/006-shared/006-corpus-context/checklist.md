---
title: "Verification Checklist: sk-design shared corpus-context conformance"
...
# Verification Checklist: sk-design shared corpus-context conformance
...
- [x] CHK-010 [P0] All 8 in-scope file(s) read and diffed against .opencode/skills/sk-doc/create-skill/references/shared/overview.md directory rules + package_skill.py file-type gates (no authored corpus-context/ template)
  - **Evidence**: 8/8 files audited (README.md, 4 .mjs source files, tests/README.md, tests/validate-context-plan.test.mjs, tests/fixtures.mjs). Fixed: tests/README.md (--- separators added between the four numbered sections 2-5).
- [x] CHK-011 [P1] Audit outcome recorded per file, including 'no changes' results
  - **Evidence**: README.md is a short frontmatter-free contract doc with only a single '## 1. OVERVIEW' section — no numbering/casing defect possible with one section; already conformant. The 4 .mjs source files and 2 remaining test files are code, not governed by the doc-structure template — no changes. Cross-checked against `checklist.md` CHK-010 evidence.
<!-- /ANCHOR:code-quality -->
...
- [x] CHK-021 [P0] Confirmed defects fixed or explicitly deferred with owner
  - **Evidence**: `node --test shared/corpus-context/tests/*.test.mjs` -> 36/36 pass.
