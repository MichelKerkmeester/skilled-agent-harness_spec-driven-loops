...
### Definition of Ready
- [x] Phase 004 retrieval surface is available for one-bundle selection
- [x] Phase 005 schema contract exposes a locked target-facts digest / FACTS to bind against
- [x] The STUDY on/off switch and no-STUDY fallback contract are agreed

### Definition of Done
- [x] STUDY-disabled output is byte-identical to the pre-STUDY WRITE path
- [x] The de-literalized transformer emits no verbatim source values or phrases
- [x] The two-signal leak gate discards and retries without STUDY on every seeded leak
- [x] Provenance/rights/injection envelope accompanies every STUDY bundle
- [x] Adversarial + counterfactual fixtures pass
<!-- /ANCHOR:quality-gates -->
...
- [x] Build one-bundle STUDY selection over the phase-004 retrieval surface
- [x] Add generation-guarded hydration of the matched DESIGN.md + token artifacts

### Phase 2: De-literalization + binding + injection
- [x] Build the de-literalized observation transformer (structural observations only)
- [x] Bind observations to the locked target-facts digest
- [x] Inject the optional STUDY block after locked FACTS via `buildWritePrompt` / `buildPlan`
- [x] Attach the provenance / rights / injection envelope

### Phase 3: Leak gate + retry + fixtures
- [x] Add the two-signal (exact-value + normalized-span) leak gate in `runGuided`
- [x] Implement discard-and-retry-without-STUDY on any leak trip
- [x] Author adversarial + counterfactual fixtures
<!-- /ANCHOR:phases -->
...
- [x] STUDY switch defaults to off until reversibility is proven
- [x] STUDY-off byte-identity test is green
- [x] Leak-gate fixtures are green
