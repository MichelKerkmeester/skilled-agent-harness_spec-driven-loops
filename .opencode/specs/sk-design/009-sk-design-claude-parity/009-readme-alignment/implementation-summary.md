...
| `.opencode/skills/sk-design/design-motion/README.md` | Modified | Added a `procedures/` reference for its 1 card; corrected the VERIFICATION section from "Eight scenarios" across 5 categories to "Ten scenarios" across 6 categories, naming `06--advanced-craft`; version bumped 1.0.0.0 to 1.0.1.0. |
...
| Scenario count reconciliation | PASS - `design-motion/manual_testing_playbook/manual_testing_playbook.md` Section 1 lists exactly 10 `MOTION-*` rows across `01--strategy`, `02--presence`, `03--reduced-motion`, `04--micro-interactions`, `05--decision`, `06--advanced-craft`; the README's corrected claim matches exactly. |
