...
**Evidence**
The baseline report already identifies the relevant split: `deep-loop-runtime` owns backend primitives and scripts, while `deep-improvement` is the outlier with its own `loop-host.cjs`, reducer, promotion, and four lanes (`.opencode/specs/deep-loops/029-deep-loop-workflows/context/context-report.md:26-29`). It also flags this exact problem as high risk: improvement is really four modes, has no `loopType`, and Lane B writes to the foreign benchmark hub (`context-report.md:145-149`).
