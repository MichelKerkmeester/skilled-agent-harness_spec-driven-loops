...
_memory:
  continuity:
    ...
    blockers: []
    key_files:
      - ".opencode/specs/system-deep-loop/032-deep-alignment-mode/009-command-agent-advisor-cutover/spec.md"
      - ".opencode/specs/system-deep-loop/032-deep-alignment-mode/009-command-agent-advisor-cutover/tasks.md"
      - ".opencode/specs/system-deep-loop/032-deep-alignment-mode/009-command-agent-advisor-cutover/implementation-summary.md"
    session_dedup:
...
- **Cutover gates**: run `node .opencode/commands/doctor/scripts/parent-skill-check.cjs` in STRICT mode against the real `deep-alignment` skill directory once phase 003's skeleton is built out AND every adapter phase (005-007, 010) has landed, then `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh .opencode/specs/system-deep-loop/032-deep-alignment-mode --recursive --strict` across the full 10-phase packet.
