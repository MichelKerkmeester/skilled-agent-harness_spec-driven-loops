...
- [x] CHK-010 [P0] No shipped runtime or skill changes by this phase: zero writes outside `005-deep-loop-effectiveness-and-fanout/`. Evidence: scoped `git status` at close — the run wrote only under `005-.../research/` and `scratch/`; `runtime/scripts/fanout-run.cjs` untouched.
...
- [x] CHK-206 [P0] Zero shipped-runtime/skill changes by Run-2; all writes inside `005-deep-loop-effectiveness-and-fanout/`. Evidence: scoped `git status` — only `*-modes*` state/scratch + packet docs changed; `runtime/scripts/fanout-run.cjs` untouched.
