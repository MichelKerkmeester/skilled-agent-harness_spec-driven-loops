...
The current contract confirms three gaps:

- `standardSource()` returns references, validators, and deviations, while semantic rules remain reasoning-agent inputs rather than a typed rule set ([adapter](/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:46)).
- Convergence measures artifact coverage and finding stability, not authority-rule coverage ([state-machine wiring](/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:48)).
- Known deviations are machine-readable and evidenced, but lack ownership, expiry, authority digest, and explicit rule references ([deviation schema](/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:151)).

The transferable 2025–2026 mechanisms are stronger than ordinary verify-first auditing:

1. OPA validates policies before evaluation through parsing, strict compilation, JSON-Schema typing, capability constraints, policy tests, and coverage thresholds. It then packages rules into revisioned, scoped, optionally signed bundles and includes bundle/rule metadata in decision logs. Deep-alignment should similarly refuse authoritative findings when its compiled authority capsule is invalid or untested. ([OPA CLI](https://www.openpolicyagent.org/docs/cli), [bundles](https://www.openpolicyagent.org/docs/management-bundles), [decision logs](https://www.openpolicyagent.org/docs/management-decision-logs))

2. TypeSpec separates canonical source, typed intermediate representation, reusable libraries, configurable rulesets, emitters, code fixes, and isolated rule tests. That suggests compiling the formalizable subset of an authority into typed rules while explicitly retaining ambiguous prose as `reasoning-required` obligations. “Uncompiled” must be visible—not treated as compliant. ([TypeSpec](https://github.com/microsoft/typespec), [linter API](https://typespec.io/docs/extending-typespec/linters/))

3. Kyverno treats exceptions and outcomes as domain objects. Exceptions name policies, use CEL scope predicates, and remain visible as `skip` or configurable `pass`; reports distinguish `pass`, `fail`, `warn`, `error`, and `skip`. Its CLI also compares declared expected outcomes with actual evaluation and supports offline external-data mocks. Deep-alignment’s deviations should gain the same governed lifecycle and observable result semantics. ([exceptions](https://kyverno.io/docs/guides/exceptions/), [reports](https://kyverno.io/docs/guides/reports/), [CLI tests](https://kyverno.io/docs/kyverno-cli/reference/kyverno_test/))

4. Specmatic performs direction-sensitive compatibility: generate tests from the old specification, run them against a mock of the new specification, validate historical examples, then test the live implementation. Deep-alignment currently asks whether artifacts match today’s authority; it should also detect when an authority revision deletes, weakens, or incompatibly changes obligations. ([Specmatic compatibility](https://docs.specmatic.io/contract_driven_development/backward_compatibility.html))

5. Schemathesis compiles schemas into example, boundary, fuzz, negative, and stateful witnesses, then shrinks failures to minimal reproductions. Its 2026 adaptive loop learns repeated server constraints but does not promote a single observation. Deep-alignment can generate equivalent witness sets for rules and attach minimized replay receipts to findings. ([data generation](https://schemathesis.readthedocs.io/en/latest/explanations/data-generation/), [adaptive testing](https://schemathesis.readthedocs.io/en/stable/explanations/adaptive-testing/))

A suitable API shape is:

`compileAuthority(ref) -> {capsuleId, revision, sourceDigest, rules[], diagnostics[], tests, signature}`

Each rule should carry `ruleId`, source span, modality (`deterministic | schema | relational | reasoning`), scope selector, severity, checker, and witness plan. Each decision receipt should carry the capsule digest, rule ID, evidence hash, witness hash, result, exception ID if applicable, and exact replay command.

The uniqueness thesis is temporal, multi-authority certification. A linter evaluates already-formal local rules. A one-shot prompt lacks a frozen authority, coverage proof, exception history, and replayability. Deep-review asks whether something is good or correct; deep-improvement changes candidates. Deep-alignment can uniquely certify which version of which authority governed an artifact, prove which obligations were exercised, expose exceptions rather than erase them, detect authority weakening across epochs, and report conflicts between simultaneous authorities—all while remaining read-only.
...
```json
{
  "new_repos": [
    {
      ...
      "stars": "~12k",
      "what": "General policy compiler/evaluator; the release page showed v1.17.0 on 2026-05-28 with rule labels and published IR/bundle schemas.",
      "lesson": "Compile, type-check, capability-check, test, coverage-gate, revision, sign, and label an authority capsule before using it for findings.",
      "maps_to": [
        "deep-alignment",
    ...
    },
    {
      ...
      "stars": "~7.9k",
      "what": "Kubernetes policy platform; CEL policy types arrived in 2025 and became stable in v1.18, released in 2026.",
      "lesson": "Represent deviations as scoped policy-referencing objects and preserve pass/fail/warn/error/skip outcomes in background audit reports.",
      "maps_to": [
        "deep-alignment",
        "runtime/gauges-observability"
      ],
    ...
    },
    {
      ...
      "stars": "~5.8k",
      "what": "Typed specification compiler with reusable libraries, linter rulesets, emitters, code fixes, and rule testers; v1.14.0 released 2026-07-14.",
      "lesson": "Compile formalizable authority prose into a typed rule IR and expose residual ambiguous obligations as reasoning-required rather than silently omitting them.",
      "maps_to": [
        "deep-alignment"
      ],
      "confidence": "high"
    ...
    {
      "name": "specmatic/specmatic",
      "url": "https://github.com/specmatic/specmatic",
      "stars": "~389",
      "what": "Executable-contract and compatibility system; release 2.50.0 published 2026-07-06.",
      "lesson": "Replay old-authority witnesses against a new authority capsule, validate historical examples, then verify live artifacts to detect semantic weakening or breaking changes.",
      "maps_to": [
        "deep-alignment",
    ...
    },
    {
      ...
      "stars": "~3.5k",
      "what": "Schema-driven positive, negative, boundary, fuzz, and stateful conformance testing; release 4.22.4 published 2026-07-08.",
      "lesson": "Generate rule witnesses, explore relational behavior, shrink failures, and retain deterministic replay cases instead of recording prose-only findings.",
      "maps_to": [
        "deep-alignment",
  ...
  "insights": [
    {
      "insight": "Authority validity must be a prerequisite to artifact conformance: parse, type, capability, rule-test, coverage, and signature failures should produce AUTHORITY_INVALID rather than artifact PASS.",
      "evidence": "https://www.openpolicyagent.org/docs/cli and https://www.openpolicyagent.org/docs/management-bundles",
      "maps_to": [
        "deep-alignment",
        "runtime/state-jsonl-checkpointing"
      ],
    ...
    {
      "insight": "A typed rule IR should distinguish deterministic, schema, relational, and reasoning-required obligations so rule coverage cannot be confused with artifact coverage.",
      "evidence": "https://typespec.io/docs/extending-typespec/linters/",
      "maps_to": [
        "deep-alignment",
        "runtime/gauges-observability"
      ],
    ...
    {
      "insight": "Known deviations should be first-class, scope-predicated exception records whose use remains visible as SKIP or EXEMPT, not silent suppression.",
      "evidence": "https://kyverno.io/docs/guides/exceptions/ and https://kyverno.io/docs/guides/reports/",
      "maps_to": [
        "deep-alignment",
        "runtime/gauges-observability"
      ],
    ...
    {
      "insight": "Conformance is direction-sensitive across authority epochs: replaying witnesses from the old authority against the new one detects deleted or weakened obligations that a current-snapshot audit misses.",
      "evidence": "https://docs.specmatic.io/contract_driven_development/backward_compatibility.html",
      "maps_to": [
        "deep-alignment",
        "runtime/continuity-threading"
      ],
    ...
    {
      "insight": "Generated positive, negative, boundary, and stateful witnesses plus shrinking turn a semantic finding into a minimal replayable proof and reveal unexercised rule branches.",
      "evidence": "https://schemathesis.readthedocs.io/en/latest/explanations/data-generation/",
      "maps_to": [
        "deep-alignment",
        "runtime/convergence",
        "runtime/dedup-novelty"
  ...
  "recommendations": [
    {
      "rec": "Add compileAuthority(ref) and an immutable AuthorityCapsule containing revision, sourceDigest, compilerVersion, typed rules with source spans and modalities, compile diagnostics, capability profile, rule-test coverage, and signature. standardSource() should return a capsule reference; findings are forbidden when the capsule is invalid, stale, or has unacknowledged uncompiled obligations.",
      "target": "deep-alignment",
      "rationale": "The current adapter validates artifacts against paths and reasoning packets but does not prove that the authority was completely or consistently translated.",
      "uniqueness": "This makes deep-alignment a compiler-backed certifier of named authorities rather than another collection of lint rules.",
      "effort": "L",
      "impact": "high",
      "evidence": "https://www.openpolicyagent.org/docs/cli; https://www.openpolicyagent.org/docs/management-bundles; https://typespec.io/docs/extending-typespec/linters/"
    },
    {
      "rec": "Require a witness matrix per compiled rule: conforming example, violating example, boundary cases, and relational/stateful sequences where applicable. Shrink failures and store capsuleDigest, ruleId, witnessHash, evidenceHash, result, and replay command in each decision receipt; add rule and branch coverage gauges beside artifact coverage.",
      "target": "deep-alignment",
      "rationale": "Artifact coverage can reach 100% while important authority branches were never exercised; minimized witnesses make verify-first findings independently reproducible.",
      "uniqueness": "Replayable semantic proofs are stronger than a linter diagnostic or an LLM assertion and remain comparable across iterations.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://schemathesis.readthedocs.io/en/latest/explanations/data-generation/; https://www.openpolicyagent.org/docs/policy-testing"
    },
    {
      "rec": "Replace passive known-deviation entries with governed exceptions carrying owner, issuedAt, expiresAt, authorityDigest, ruleRefs, scopePredicate, justification evidence, and reportResult. On authority changes, replay prior witnesses, classify added/removed/weakened obligations, require an amendment decision, and report cross-lane authority conflicts separately from artifact failures.",
      "target": "deep-alignment",
      "rationale": "Exceptions otherwise become permanent blind spots, while current-only audits cannot detect standards weakened to make existing artifacts pass.",
      "uniqueness": "Temporal authority governance plus multi-authority conflict analysis is the clearest moat over linters, one-shot prompts, deep-review, and mutation-oriented deep-improvement.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://kyverno.io/docs/guides/exceptions/; https://docs.specmatic.io/contract_driven_development/backward_compatibility.html"
    }
  ],
  "contradictions": [
    {
      "claim": "Always reading the live authority gives the strongest verify-first result.",
      "counter": "Without an immutable authority revision and digest, the same audit cannot be reproduced and an authority can change mid-run; snapshot the live authority at INIT and compare it with head at REPORT.",
      "evidence": "https://www.openpolicyagent.org/docs/management-bundles"
    },
    {
      "claim": "Known deviations should be suppressed so accepted conventions do not appear as findings.",
      "counter": "Suppression should change the result to an observable SKIP or EXEMPT receipt tied to an exception, not erase the evaluation; otherwise stale exceptions and overbroad matches are invisible.",
      "evidence": "https://kyverno.io/docs/guides/exceptions/ and https://kyverno.io/docs/guides/reports/"
    },
    {
      "claim": "Authority compilation can eliminate reasoning-agent judgment.",
      "counter": "OPA, TypeSpec, and similar systems compile formal languages, not arbitrary normative prose. Ambiguous obligations must remain explicitly reasoning-required and undergo witness or amendment review rather than being guessed into deterministic rules.",
      "evidence": "https://www.openpolicyagent.org/docs/policy-language and https://github.com/microsoft/typespec"
    }
  ],
  "next_angles": [
    "Design an authority-capsule amendment protocol: authorized editors, evidence required to invalidate a rule, compatibility classes, baseline establishment, and honest cross-epoch comparability.",
    "Build a mutation benchmark for authority compilation: delete, weaken, invert, narrow, or exception-wrap canonical rules and measure whether capsule tests and historical witness replay catch each change.",
    "Specify a cross-authority conflict algebra for lanes: direct contradiction, precedence, narrower-scope override, incompatible severity, and exception-induced non-monotonicity."
  ],
  "notes": "Most mature engines surveyed predate 2025; the novel evidence here is their 2025-2026 evolution: OPA rule-labelled decisions and published drift-tested schemas, Kyverno's CEL policy family and exception reporting, Specmatic's current three-level compatibility workflow, and Schemathesis's adaptive four-phase witness generation. Coverage gap: no strong current system automatically and reliably compiles arbitrary prose standards into executable rules, so that boundary must remain explicit."
}
```
...
session id: 019f6527-d538-76a3-999e-5e0cc93eb86e
--------
user
You are iteration 21/40 of a per-MODE, NON-CONVERGING (deliberately broadening within the mode) research loop to make ONE mode of "system-deep-loop" both MORE EFFECTIVE and MORE UNIQUELY VALUABLE.
...
THIS ITERATION'S ANGLE (A1-sota): deep-alignment :: 2025-2026 state of the art
DIRECTIVE: Survey the NEWEST (2025-2026) automated conformance / policy-as-code / spec-conformance / compliance-audit systems (Semgrep, OPA/Rego, Conftest, constitutional classifiers, spec-conformance harnesses, guardrails frameworks). What do their authority-to-rule compilation and verification do that our verify-first lane audit does NOT? Real repos/URLs + adoptable mechanisms mapped to deep-alignment.
...
THIS run's new repos so far (81) — also do not repeat:
...
New insights so far: 86; recommendations: 57.
Open threads flagged: Calibrate regression-envelope false veto rates across sparse agent fixtures: paired exact tests versus bootstrap bounds, weak-segment minimum sizes, and MMD permutation tests on action traces. | Build a profile-substitution benchmark containing adversarial candidates that delete, rename, or weaken self-declared rules; compare current self-derived scoring against one frozen canonical capsule and measure false promotions. | Specify a metamorphic-relation DSL for agent prompts and tool traces, then audit relation validity and shrink quality across routing, coding, review, and safety targets. | Benchmark TextReg-style recurrence and scope penalties against held-out-only selection, measuring cross-domain correctness, prompt growth, narrow-rule accumulation, and rejected useful special cases. | Calibrate evaluation-context parity thresholds by testing whether real-versus-test-cued gaps predict shadow-production regressions better than ordinary held-out aggregates. | Define the evaluator-amendment protocol: who may change a capsule, which evidence invalidates, how a new baseline is established, and how histories remain comparable without pretending cross-epoch continuity.
...
hook: UserPromptSubmit Completed
codex
I’m treating this as a read-only research iteration, so the spec-folder write gate does not apply. I’ll verify live repositories, current architecture/docs, stars, and recency, then extract mode-specific mechanisms rather than repeat the shared-runtime recommendations.
exec
/bin/zsh -lc "node .opencode/bin/spec-memory.cjs memory_match_triggers --json '{\"prompt\":\"Iteration 21/40 deep-alignment 2025-2026 automated conformance policy-as-code spec-conformance compliance audit authority-to-rule compilation\"}' --format json --timeout-ms 3000" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 exited 75 in 3148ms:
{
  "status": "error",
...
exec
/bin/zsh -lc 'python3 .opencode/skills/system-skill-advisor/mcp_server/scripts/skill_advisor.py "Iteration 21/40 deep-alignment 2025-2026 automated conformance policy-as-code spec-conformance compliance audit authority-to-rule compilation" --threshold 0.8' in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 1840ms:
2026-07-15 11:42:51.166 xcodebuild[37518:207510445]  DVTFilePathFSEvents: Failed to start fs event stream.
2026-07-15 11:42:51.319 xcodebuild[37518:207510444] [MT] DVTDeveloperPaths: Failed to get length of DARWIN_USER_CACHE_DIR from confstr(3), error = Error Domain=NSPOSIXErrorDomain Code=5 "Input/output error". Using NSCachesDirectory instead.
python3: warning: confstr() failed with code 5: couldn't get path of DARWIN_USER_TEMP_DIR; using /tmp instead
python3: error: couldn't create cache file '/tmp/xcrun_db-KG2ETFWp' (errno=Operation not permitted)
2026-07-15 11:42:51.701 xcodebuild[37520:207510460]  DVTFilePathFSEvents: Failed to start fs event stream.
2026-07-15 11:42:51.807 xcodebuild[37520:207510459] [MT] DVTDeveloperPaths: Failed to get length of DARWIN_USER_CACHE_DIR from confstr(3), error = Error Domain=NSPOSIXErrorDomain Code=5 "Input/output error". Using NSCachesDirectory instead.
...
[
  {
    ...
    "reason": "Matched: !conformance(keyword), !deep-alignment(keyword), !deep-alignment(signal), alignment, deep(name)",
  ...
  },
  {
    ...
    "uncertainty": 0.23,
    "passes_threshold": true,
    "reason": "Matched: !audit(multi), !graph:depends(system-deep-loop,0.7), !graph:enhances(sk-code,0.3), !graph:enhances(system-deep-loop,0.6), !spec",
  ...
  },
  {
    ...
    "uncertainty": 0.31,
    "passes_threshold": true,
    "reason": "Matched: !audit, !audit(multi), !code(multi), !intent:review, code(name)",
    "_graph_boost_count": 0,
    "source": "local"
  },
  {
    ...
    "uncertainty": 0.35,
    "passes_threshold": true,
    "reason": "Matched: command_penalty, deep(name), review(name), spec(name)",
    "_graph_boost_count": 0,
    "source": "local"
  }
]
...
codex
Skill routing resolved to `system-deep-loop` (0.95); I’m loading that workflow before the survey because the iteration/state protocol is mode-owned. Memory trigger matching was unavailable through its warm daemon (`EPERM`, retryable exit 75), so no prior memory capsule is being assumed beyond the prompt.
exec
...
exec
/bin/zsh -lc "sed -n '1,240p' .opencode/skills/system-deep-loop/mode-registry.json && wc -l .opencode/skills/system-deep-loop/deep-alignment/SKILL.md && sed -n '1,280p' .opencode/skills/system-deep-loop/deep-alignment/SKILL.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
{
  "skill": "system-deep-loop",
  "version": "2.0.0.0",
...
exec
/bin/zsh -lc "sed -n '281,430p' .opencode/skills/system-deep-loop/deep-alignment/SKILL.md && wc -l .opencode/skills/system-deep-loop/deep-alignment/references/scoping_protocol.md && sed -n '1,260p' .opencode/skills/system-deep-loop/deep-alignment/references/scoping_protocol.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
### ⛔ NEVER
...
web search: 
web search: site:github.com/open-policy-agent/opa releases 2025 Rego compile bundle signatures decision logs ...
web search: 
web search: https://api.github.com/repos/open-policy-agent/opa
web search: 
web search: open-policy-agent/opa GitHub stars 2026 ...
web search: 
web search: site:github.com/specmatic/specmatic "Stars" ...
web search: 
web search: github specmatic/specmatic contract driven development repository ...
web search: 
web search: https://github.com/specmatic/specmatic
web search: 
web search: 
web search: 
web search: 
web search: 
web search: 'createLinterRule'
web search: 
web search: site:openpolicyagent.org/docs policy testing coverage schema type checking bundles signing compile wasm official ...
web search: 
web search: site:openpolicyagent.org/docs "JSON Schema" "opa eval --schema" Rego type check ...
web search: 
web search: site:kyverno.io/docs kyverno test manifest expected pass fail skip error policy exception official ...
web search: 
web search: site:kyverno.io/docs "kyverno test" "test.yaml" ...
web search: 
web search: site:docs.specmatic.io "Generative Testing" Specmatic ...
web search: 
web search: site:schemathesis.readthedocs.io checks negative data rejection schema conformance 2026 ...
web search: 
web search: 
codex
The strongest gap is emerging: deep-alignment verifies findings, but it does not yet compile an authority into a versioned executable capsule with its own type checks, rule tests, generated witnesses, or epoch-to-epoch compatibility proof. I’m checking the implementation details behind that gap now, including exceptions and replay semantics.
web search: 
web search: site:github.com/open-policy-agent/opa/releases/tag/v1.18.0 ...
exec
/bin/zsh -lc 'wc -l .opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md && rg -n "authority|standardSource|hash|digest|version|schema|test|witness|counterexample|deviation|provenance|exception|finding" .opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md' in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
     ...
     682 total
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:10:version: 1.0.0.1
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:36:- `discover_contract.md`, `scoping_protocol.md`, `lane_config_schema.md`, the upstream contracts this wiring consumes.
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:47:| `DISCOVER` | each lane's adapter `discover(scope)` (`deep-alignment/scripts/adapters/<authority>.cjs`), then `runtime/scripts/upsert.cjs --seed-source deep-alignment-discover --seed-confidence 1.0` | `deep-alignment-config.json` lanes | `alignment/deep-alignment-corpus.json` (one entry per lane: `{laneId, authority, artifactClass, scope, artifacts}`), coverage-graph `FILE` nodes via `upsert.cjs` |
...
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:50:| `REPORT` | `runtime/scripts/reduce-alignment-state.cjs <spec-folder>` | `deep-alignment-state.jsonl`, `deltas/`, `deep-alignment-config.json` | `alignment/deep-alignment-findings-registry.json`, `alignment/alignment-report.md` |
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:66:├── deep-alignment-findings-registry.json  # REPORT output (auto-generated, reducer-owned)
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:72:│   └── iter-NNN.jsonl                  # per-iteration finding deltas (write-once)
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:90:**Why AND, not OR**: full coverage with still-unstable findings is not a done run (something is still actively drifting), and stability with incomplete coverage is not a done run either (large parts of the corpus were never even looked at, a trivially "stable" zero-signal from untouched artifacts). Requiring both mirrors deep-review's own convergence philosophy of weighting coverage and stability as separate, simultaneously-necessary signals (`convergence.cjs`'s `computeCompositeScore` weights `dimensionCoverage` and `findingStability` as distinct additive terms, never alternatives).
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:125:**What this phase actually built instead**: `spec.md`'s own NFR-R01 names the fallback explicitly, *"If `convergence.cjs` cannot be reused as-is (loopType rejected), the plan's fallback is graceful degradation to a documented manual coverage check, not a silent skip of convergence detection."* `deep-alignment/scripts/check-convergence.cjs` **is** that documented manual coverage check: it implements REQ-004's coverage-AND-stability-AND-max-iterations formula directly against the reducer's own registry and the JSONL state log, without touching `convergence.cjs` or depending on any graph-node convention. This gives phase 008 a real, tested, runnable convergence decision today, while leaving Option A's enum extension as a clearly-scoped, ready-to-apply follow-up (add `'alignment'` to the validation line, add a lane-aware signals branch) for whichever phase is authorized to edit that shared file.
.opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md:147:- `discover_contract.md`, `scoping_protocol.md`, `lane_config_schema.md`, the upstream contracts this wiring consumes.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:3:description: The seeded sk-code known-deviation list for deep-alignment's ADR-005 suppression invariant, seeded from verify_alignment_drift.py's own live skip-path/severity-downgrade functions plus two adapter-specific conventions found while building this adapter, not invented.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:5:  - "sk-code known deviations"
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:6:  - "known deviation suppression list sk-code"
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:8:  - "context advisory path test heavy path motion dev overlay"
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:11:version: 1.0.0.2
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:16:The sk-code authority's known-deviation list for deep-alignment's ADR-005 suppression invariant: intentional repo conventions the mode must never flag as drift.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:24:ADR-005 (`.opencode/specs/system-deep-loop/059-deep-alignment-mode/002-architecture-decision/decision-record.md`, ANCHOR `adr-005`) requires every authority adapter's `standardSource` to carry a known-deviation list so a real repo-wide convention is never flagged as drift. Every entry below traces to real, already-live code: `verify_alignment_drift.py`'s own exemption functions, or a real classification outcome this adapter's own build surfaced by construction. It is not a hypothetical.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:28:This document is the single source of truth for sk-code's suppression rules. The fenced `json` block in Section 8 is parsed directly by `scripts/adapters/sk-code.cjs`'s `loadKnownDeviations()` at runtime. There is no separate, hand-synced copy of this list in code. Editing a deviation means editing it here, once.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:32:Unlike sk-doc's known-deviation list (which suppresses findings `validate_document.py`/`extract_structure.py` emit), every entry here is **native to the wrapped tool or the router itself**: `verify_alignment_drift.py` already downgrades severity or skips the check entirely for the four path categories in Sections 2-5 *before* it ever prints a finding, and the surface-precedence outcome in Section 6 is the router's own documented, correct behavior, not an adapter-level override. `sk-code.cjs`'s `suppressKnownDeviations()` therefore has nothing to actively filter for any entry here today: every `matchTypes` is `[]`. This list exists so a **reasoning-agent-layer-2** reviewer (which does not read `verify_alignment_drift.py`'s source before judging) does not independently re-flag a path or classification outcome the deterministic layer, or the router itself, already treats correctly. This mirrors `sk_doc_known_deviations.md`'s own "dormant" entries (Sections 4-5 there), which exist for the identical reason.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:40:**Why it is not a violation**: `verify_alignment_drift.py`'s `classify_severity()` (verify_alignment_drift.py:253-257) downgrades every rule's severity to `WARN`, never `ERROR`, for any path containing a `z_archive`, `scratch`, `memory`, `research`, `context`, `assets`, `examples` or `fixtures` path segment (`is_context_advisory_path()`, verify_alignment_drift.py:225-228, checking `CONTEXT_ADVISORY_SEGMENTS`, lines 75-84). plan.md's Architecture section names this exact function as one of two candidate seeds for this list ("`is_context_advisory_path`, `is_test_heavy_path`").
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:48:**Match rule**: none currently (`matchTypes: []`). See Section 1: `verify_alignment_drift.py`'s own output already reflects this downgrade. `sk-code.cjs`'s layer-1 translator (`checkOpencodeDeterministic()`) faithfully carries the tool's own `severity` field through to P0/P1, so a `z_archive/`-path finding already prints as P1 (WARN), never P0, with no adapter-side suppression required.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:58:**Why it is not a violation**: the same `classify_severity()` downgrade applies to any path under a `/tests/` segment, or whose basename ends in `.test.ts`/`.spec.ts`/`.vitest.ts`/`.test.tsx`/`.spec.tsx`/`.vitest.tsx` (`is_test_heavy_path()`, verify_alignment_drift.py:231-236, `TS_TEST_SUFFIXES`, lines 85-92). Test fixtures and specs are held to a lighter mechanical bar than shipped implementation code.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:62:- `.opencode/skills/sk-code/code-opencode/assets/scripts/verify_alignment_drift.py:231-236`: `is_test_heavy_path()`.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:63:- `007-adapter-sk-code/plan.md` Architecture section names `is_test_heavy_path` as the second seed candidate.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:67:**Live-Reality Check (2026-07-11)**: `is_context_advisory_path()` itself calls `is_test_heavy_path()` first (verify_alignment_drift.py:226). Every test-heavy path is also context-advisory, so this entry is a documented subset of Section 2's, kept separate because plan.md names both functions individually and a reasoning-agent reviewer benefits from the narrower, more specific rationale ("this is test code," not just "this is advisory").
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:84:**Live-Reality Check (2026-07-11)**: confirmed by reading `should_skip_ts_module_header()`'s two-line body directly (`is_test_heavy_path(path) or is_ts_pattern_asset(path)`). This is an unconditional skip, not a severity change, the one structurally different exemption shape among the four seeded here.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:92:**Why it is not a violation**: exactly one file, a deliberately-malformed JSON fixture used by an earlier spec-kit test suite, is exempt from the `JSON-PARSE` check by an exact-suffix match (`is_known_malformed_json_fixture()`, verify_alignment_drift.py:244-246, `KNOWN_MALFORMED_JSON_FIXTURE_SUFFIXES`, lines 94-97). The file is intentionally invalid JSON by design (it is a fixture for testing malformed-input handling), not a real drift instance.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:95:- `.opencode/skills/sk-code/code-opencode/assets/scripts/verify_alignment_drift.py:94-97`: the one hardcoded suffix, pointing at `.opencode/specs/system-spec-kit/z_archive/001-fix-command-dispatch/z_archive/044-speckit-test-suite/scratch/001-test-agent-08/malformed.json`.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:101:**Live-Reality Check (2026-07-11)**: confirmed the named path is a real, archived test-scratch file (both `z_archive` and `scratch` path segments present, so this single file is also covered by Section 2's context-advisory downgrade on every *other* rule type. This exemption is specifically and only about the `JSON-PARSE` check being skipped outright rather than merely downgraded).
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:113:- Live CLI dry-run, 2026-07-11: `node scripts/adapters/sk-code.cjs discover .opencode/skills/sk-code/code-webflow/assets/scripts` classified all three files (`minify-webflow.mjs`, `test-minified-runtime.mjs`, `verify-minification.mjs`) as `surface: "OPENCODE"`, `detectedFrom: "path"`. See `sk_code_adapter.md` Section 8 for the full transcript.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:115:**Match rule**: none currently (`matchTypes: []`). This is a classification-outcome deviation, not a finding-type deviation. It is recorded here for the reasoning-agent layer's benefit (Section 1) and for any future human reviewer who encounters the same "why is this WEBFLOW-named file marked OPENCODE?" question.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:125:**Why it is not a violation**: `smart_routing.md` §5 states Motion.dev (and by the same logic, GSAP/Lenis/Swiper/HLS, the same marker family `stack_detection.md`'s WEBFLOW content-grep checks) is *"a peer resource category... It is not a separate code surface. It supplements WEBFLOW, OPENCODE, or future surfaces."* A file that references one of these peer libraries is not thereby non-conformant to its own surface's standard. It is drawing on documented, expected cross-stack integration material (`code-webflow/references/animation/`), the exact resource `standardSource()`'s `motionOverlay` reference already surfaces to the reasoning-agent layer when `motionDevOverlay: true`.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:131:**Match rule**: none currently (`matchTypes: []`). `sk-code.cjs`'s layer 1 has no "wrong peer library" finding type to suppress. This entry exists purely to keep the reasoning-agent layer from independently inventing one.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:133:**Live-Reality Check (2026-07-11)**: `sk-code.cjs`'s own `MOTION_DEV_CONTENT_MARKER_RE` content-marker regex is a lightweight literal-text match, confirmed (by direct test against this adapter's own source file) to also fire on **prose that merely names** a Motion.dev API (for example a docstring quoting `"animate()"` as a keyword, not an actual `animate(...)` call). See `sk_code_adapter.md` Section 8 for the full reproduction. This is recorded as a known, accepted imprecision of the lightweight overlay signal (a false positive on *mentioning* the pattern, not *using* it), not silently smoothed over.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:139:**In scope**: sk-code authority only. Each other authority adapter (sk-doc phase 005, sk-git and sk-design phase 006) owns its own known-deviation list under its own `standardSource`, per ADR-005's per-authority requirement. This document does not attempt to anticipate their conventions.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:151:  "authority": "sk-code",
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:152:  "version": "1.0.0",
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:153:  "generatedFrom": "sk_code_known_deviations.md Section 9, hand-maintained alongside Sections 2-7",
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:154:  "deviations": [
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:170:      "id": "test-heavy-path-severity-downgrade",
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md:243:- [sk_code_adapter.md](./sk_code_adapter.md): the full `standardSource`/`discover`/`check` specification this list is loaded by.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:2:title: sk-code Hybrid Adapter - standardSource, discover, check
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:3:description: The concrete standardSource("sk-code")/discover(scope)/check(artifact,rules,options) specification for the hardest of the four v1 authorities: a two-layer HYBRID check() (ADR-008 LOCKED) wrapping the real shared surface router plus verify_alignment_drift.py and the Webflow verification scripts for the deterministic layer, and a structured dispatch-packet builder (not a self-judging script) for the reasoning-agent layer.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:12:version: 1.0.0.3
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:17:The concrete `standardSource("sk-code")` / `discover(scope)` / `check(artifact, rules, options)` specification for the sk-code authority, the hardest of the four v1 `deep-alignment` authorities (ADR-004), built last so the adapter contract was already proven against sk-doc's fully-deterministic reference shape (phase 005) before this phase's honestly-partial HYBRID shape.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:25:ADR-003 (`.opencode/specs/system-deep-loop/059-deep-alignment-mode/002-architecture-decision/decision-record.md`, ANCHOR `adr-003`) freezes the same three-method, authority-agnostic contract every adapter implements: `discover(scope) -> artifacts`, `standardSource(authority) -> {templates, rules}`, `check(artifact, rules) -> findings`. This document specifies sk-code's implementation of all three, and `scripts/adapters/sk-code.cjs` is the real, executable code behind it, built to match `scripts/adapters/sk-doc.cjs`'s exact file shape (imports → constants → classifier → discover → suppression → standardSource → subprocess wrappers → check → CLI → exports) per this phase's own brief.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:29:ADR-008 (decision-record.md, ANCHOR `adr-008`) LOCKS this adapter's `check()` as a two-layer HYBRID: **deterministic surface-detection** (reusing sk-code's shared router, never reimplemented: REQ-001) **plus reasoning-based pattern-conformance** for everything the deterministic layer does not cover, every finding honestly labeled by producing `layer`. This document and `sk-code.cjs` implement that locked frame. They do not re-litigate whether to use it (spec.md's Open Questions §10 confirms this explicitly).
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:36:2. `.opencode/skills/sk-code/shared/references/smart_routing.md`: the MOTION_DEV overlay's peer-category framing (§5) and the machine-readable `INTENT_SIGNALS`/`RESOURCE_MAP` this adapter's `standardSource()` points at.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:38:4. `.opencode/skills/sk-code/code-webflow/assets/scripts/{verify-minification.mjs,test-minified-runtime.mjs}`: the real, read-only WEBFLOW-surface verification scripts (Section 4.2's live-reality findings explain why `minify-webflow.mjs`, the third script in this directory, is explicitly excluded).
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:46:`standardSource('sk-code')` returns a single object naming every real source Section 1 lists, the excluded script and the parsed known-deviation list. This is the **live output** of `node scripts/adapters/sk-code.cjs standard-source` (2026-07-11, paths shown resolved for this checkout):
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:50:  "authority": "sk-code",
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:69:      "tool": "test-minified-runtime.mjs",
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:70:      "path": "<repo>/.opencode/skills/sk-code/code-webflow/assets/scripts/test-minified-runtime.mjs",
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:84:  "knownDeviations": [ /* 6 entries, parsed from sk_code_known_deviations.md Section 9 */ ]
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:88:Calling `standardSource()` with any authority other than `'sk-code'` throws. This file is the sk-code-specific implementation, not a cross-authority dispatcher (that dispatch belongs to phase 008's engine).
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:100:3. Collects every file matching `CODE_EXTENSIONS`: `verify_alignment_drift.py`'s own `SUPPORTED_EXTENSIONS` (11 extensions, 7 languages) **union** `.css`/`.html`, added because `stack_detection.md:28` names WEBFLOW as owning "Webflow / vanilla HTML, CSS, JavaScript" and `verify_alignment_drift.py` itself checks neither CSS nor HTML at all (confirmed: absent from its `SUPPORTED_EXTENSIONS` and from every `check_*` function). Only the narrower, OPENCODE-checkable subset gets real layer-1 coverage. CSS/HTML artifacts get a `deterministic-layer-not-applicable` finding (Section 4.1) and fall through to layer 2 only.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:123:`smart_routing.md` §5: "Motion.dev resources are a **peer category**... not a third surface." `detectMotionDevOverlay()` is therefore never a `surface` value: it is a boolean `metadata.motionDevOverlay` flag, tested against a Motion.dev-specific subset of the same marker family (`window\.Motion\b|Motion\.animate|\banimate\(|\binView\(|\bstagger\(|\bscroll\(\s*\{`), translated from `smart_routing.md`'s machine-readable `MOTION_DEV` `INTENT_SIGNALS` keyword list (`"animate()"`, `"inview"`, `"scroll()"`, `"stagger()"`) into a content regex, since `discover()` has file content available, not prompt text. Section 8 documents a real, honest limitation of this lightweight approach.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:129:`check()` runs two layers per artifact (deterministic, then reasoning-agent) and returns a flat findings array after known-deviation suppression, matching `sk-doc.cjs`'s exact shape. Per REQ-005, every call re-runs both layers fresh. Nothing is cached from `discover()` time or a prior `check()` call.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:137:1. If the artifact's extension is not in `verify_alignment_drift.py`'s own `SUPPORTED_EXTENSIONS` (for example a `.css`/`.html` file that classified OPENCODE by path), emit a single `P2` `deterministic-layer-not-applicable` finding. This tool structurally cannot check that extension, distinct from "ran and found nothing."
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:140:4. The tool's own findings (for the whole scanned directory) are filtered down to just the lines matching this one artifact's repo-relative path.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:142:6. Subprocess failure (`python3` missing, etc.) becomes a `P1` `adapter-error` finding, distinct from an artifact-level conformance finding, the same discipline as `sk-doc.cjs`'s subprocess wrappers.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:148:- **`minify-webflow.mjs` mutates the tree**. It shells out while doing it. `minify_file()` (lines 89-124) writes each `src/2_javascript/z_minified/*.min.js` output not via a direct file write but by shelling out to `execSync('npx terser "<source>" --compress --mangle -o "<output_path>"')` (line 104), a subprocess that can itself trigger an `npx` package fetch, a materially larger side effect than a plain file write. `save_manifest()` (line 146) separately writes `manifest.tsv` via `writeFileSync` (line 157). ADR-005/NFR-S01 require `check()` to be read-only by default. plan.md's Architecture section named this script as part of the deterministic chain to invoke. This adapter **excludes it from `check()` entirely** (recorded in `standardSource()`'s `excludedFromCheck`), a real, documented deviation from the plan's literal text, made because building the adapter surfaced a constraint (and a bigger one than initially assumed) the plan's prose had not accounted for, not silently reconciled.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:149:- **`verify-minification.mjs` and `test-minified-runtime.mjs` take no path argument.** Both hardcode `SOURCE_DIR = 'src/2_javascript'` / `OUTPUT_DIR = 'src/2_javascript/z_minified'` **relative to `process.cwd()`** (confirmed by reading both scripts in full). Unlike `verify_alignment_drift.py --root <path>`, there is no way to point either script at an arbitrary artifact. They must run with `cwd` set to the actual Webflow project root. `findWebflowProjectRoot()` walks upward from the artifact's directory (bounded at `REPO_ROOT`) looking for a `src/2_javascript` subdirectory.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:155:2. If the artifact sits under `<projectRoot>/src/2_javascript/z_minified/` (an output file), run `test-minified-runtime.mjs` with `cwd: projectRoot` the same way.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:156:3. A `FAIL` status from either script becomes a `P0` finding: both scripts' own summaries declare a hard block on `FAIL` ("⚠️ VERIFICATION FAILED - Do not deploy!" / "⚠️ RUNTIME TESTS FAILED - Do not deploy!"), the same "drives the wrapped tool's own hard block" logic behind the OPENCODE layer's `ERROR`→`P0` mapping (Section 7).
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:157:4. `SKIP` (no counterpart file yet) or `PASS`, or no entry at all for this artifact in the tool's own output, produces no finding.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:159:Given **no** project root is found (the default case in this repo today), or the artifact is WEBFLOW-surface but outside `src/2_javascript/` entirely (e.g. CSS/HTML): a single `P1` `deterministic-layer-unavailable` finding, per NFR-R01's own required language: "the adapter falls back to reasoning-agent-only judgment for that artifact and marks the deterministic layer as `unavailable` rather than silently treating it as clean."
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:167:Per spec.md's Data Boundaries edge case, a `P1` `surface-undetected` finding: never a silent skip, never a guessed surface.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:173:1. **`buildReasoningLayerDispatch(artifact, rules)`**, exported, independently callable (`node sk-code.cjs reasoning-dispatch <path>`), builds the well-formed input package a reasoning agent needs: the artifact path, its detected `surface` and `motionDevOverlay` flag, the exact `standardSource()` reference paths to read for that surface (plus the Motion overlay directory when the flag is set), the four conformance dimensions layer 1 cannot check (`naming-conventions-beyond-regex`, `architectural-pattern-conformance`, `cross-file-consistency`, `comment-hygiene-beyond-simple-patterns`, named directly from plan.md's Architecture section) and the exact `expectedFindingShape` the caller must produce. **This function never judges anything itself.** It is documentation-as-code for a follow-on step, not a hidden judgment engine.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:174:2. **`checkPatternConformance(artifact, options)`**: the actual layer-2 sub-check inside `check()`. Mirrors `sk-doc.cjs`'s `checkRealityAlignment()` exactly: accepts `options.verifiedFindings`, an array of already-judged `{dimension, claim, matchesStandard, evidence, severity}` records the caller (a future phase-008 ITERATE-state reasoning-agent dispatch step) produced by actually reading the artifact and the cited references. A finding is emitted **only** for entries where `matchesStandard === false` **and** `evidence` is present. No `verifiedFindings` supplied → no findings, never an invented one, the identical "no verifiedClaims → no findings" invariant `sk-doc_adapter.md` §4.2 documents for its own reality-alignment sub-check.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:182:No reasoning-layer finding is ever asserted without a cited `evidence` field (`checkPatternConformance()` drops any `verifiedFindings` entry missing one). No deterministic-layer finding is ever asserted without an actual, fresh subprocess run against the live artifact in the same `check()` call (REQ-005). It is never reused from `discover()` time or a prior iteration. This mirrors `sk_doc_adapter.md` §5's identical discipline, applied to a hybrid rather than fully-deterministic adapter.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:188:Every finding `check()` produces passes through `suppressKnownDeviations()` before being returned. The full seeded list, its structured evidence, and the machine-readable rule block `sk-code.cjs` actually parses live in [sk_code_known_deviations.md](./sk_code_known_deviations.md), not duplicated here. Unlike sk-doc's list, every current entry has `matchTypes: []`: the four `verify_alignment_drift.py`-native exemptions (context-advisory downgrade, test-heavy downgrade, TS pattern-asset skip, the one malformed-JSON fixture) are already reflected in that tool's own output before this adapter ever sees it, and the two adapter-specific entries (OPENCODE-precedence classification of Webflow-named tooling, Motion.dev peer-library references) are classification-outcome/reasoning-layer guidance, not finding-type suppressions. A suppression, when one is ever added, would only silence the matched deviation category on the matched artifact, never blanket-exempt the whole artifact from every other check (spec.md's Data Boundaries edge case).
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:196:| `verify_alignment_drift.py` | finding `severity === 'ERROR'` (drives the tool's own default exit-1 gate) | P0 | deterministic |
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:197:| `verify_alignment_drift.py` | finding `severity === 'WARN'` (present regardless of exit code, non-blocking by default) | P1 | deterministic |
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:199:| `verify-minification.mjs` / `test-minified-runtime.mjs` | `FAIL` (drives the script's own "Do not deploy!" hard block) | P0 | deterministic |
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:229:returned all three files (`minify-webflow.mjs`, `test-minified-runtime.mjs`, `verify-minification.mjs`) as `"surface": "OPENCODE"`, `"detectedFrom": "path"`, because they live under `.opencode/`, and `stack_detection.md`'s own Detection Order gives OPENCODE strict precedence, with a documented rationale (line 56) that names this exact scenario ("`.opencode/` system tools... may import vanilla animation libraries internally without being WEBFLOW-shipping artifacts"). Recorded as `sk_code_known_deviations.md` Section 6 so a reasoning-agent reviewer does not mistake this for a bug.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:233:Confirmed by reading the script: `minify_file()` (line 104) writes each `.min.js` output by shelling out to `execSync('npx terser ... -o <output_path>')`, and `save_manifest()` (line 157) calls `writeFileSync(MANIFEST_FILE, ...)`. Section 4.1.2 documents the exclusion. `standardSource()`'s `excludedFromCheck` records it machine-readably.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:237:Six live `verify_alignment_drift.py` dry-runs, increasing in corpus size, all returned `[alignment-drift] PASS` with zero findings:
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:255:(via `discover()`'s metadata, and independently via a direct regex test against this adapter's own source) reports `"motionDevOverlay": true`: not because the file calls a Motion.dev API, but because line 134 of its own source is a docstring quoting `smart_routing.md`'s keyword list: `// smart_routing.md's machine-readable MOTION_DEV keyword list ("animate()", ...)`. The literal text `animate(` matches `MOTION_DEV_CONTENT_MARKER_RE` regardless of context. This is a real, accepted limitation of a lightweight regex-based overlay signal. It detects *mentioning* the pattern, not *using* it. This is recorded in `sk_code_known_deviations.md` Section 7 rather than silently smoothed over or hidden by excluding the adapter's own source from the demonstration.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:269:**Honest fraction**: of the full conformance question, the deterministic layer's real, working coverage is a **minority slice restricted to mechanical hygiene** (12 OPENCODE rule types, WEBFLOW's real scripts gated on a project-root precondition presently unmet in this repo). The reasoning-agent layer covers everything else: the substantive "architectural/pattern conformance" the surface's own reference material (`code-webflow/references/implementation/*`, `code-opencode/references/shared/*`) actually prescribes. This is not a hedge. It is the concrete answer ADR-008 asked this phase to produce, and it is why every finding is layer-tagged rather than presented as one undifferentiated "conformance check."
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:275:`scripts/adapters/sk-code.cjs` implements every function this document specifies: `discover(scope)`, `standardSource(authority)`, `check(artifact, rules, options)`, plus `classifySurface`, `detectMotionDevOverlay`, the three subprocess wrappers, `buildReasoningLayerDispatch` and the suppression matcher. It exposes a CLI (`discover`, `check`, `standard-source`, `reasoning-dispatch` subcommands) for a manual dry-run without any engine wiring. Every command and output shown in Section 8 above is real, reproducible output from that CLI. `node --check scripts/adapters/sk-code.cjs` passes.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:281:- [sk_code_known_deviations.md](./sk_code_known_deviations.md): the structured, evidence-cited suppression list.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:285:- `../discover_contract.md`, `../lane_config_schema.md`: the real, live `discover(scope)->artifacts` contract this adapter's `discover()` conforms to.
.opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md:288:- `.opencode/skills/sk-code/code-webflow/assets/scripts/{verify-minification.mjs,test-minified-runtime.mjs,minify-webflow.mjs}`: the real WEBFLOW scripts (the third excluded from `check()`, Section 4.1.2).

exec
/bin/zsh -lc "sed -n '1,220p' .opencode/skills/system-deep-loop/deep-alignment/references/state_machine_wiring.md && sed -n '1,330p' .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
...
exec
/bin/zsh -lc "sed -n '1,120p' .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md && sed -n '121,240p' .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md && sed -n '241,330p' .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_adapter.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
title: sk-code Hybrid Adapter - standardSource, discover, check
description: The concrete standardSource("sk-code")/discover(scope)/check(artifact,rules,options) specification for the hardest of the four v1 authorities: a two-layer HYBRID check() (ADR-008 LOCKED) wrapping the real shared surface router plus verify_alignment_drift.py and the Webflow verification scripts for the deterministic layer, and a structured dispatch-packet builder (not a self-judging script) for the reasoning-agent layer.
...
exec
/bin/zsh -lc "sed -n '1,130p' .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md && sed -n '131,280p' .opencode/skills/system-deep-loop/deep-alignment/references/adapters/sk_code_known_deviations.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
...
web search: 
web search: site:github.com/kyverno/kyverno "pkg/engine" "PolicyException" ...
web search: 
web search: 
codex
The next moat is an authority compiler, not another checker. Deep-alignment already verifies findings well; it does not verify that the authority itself was translated completely, consistently, or reproducibly.
