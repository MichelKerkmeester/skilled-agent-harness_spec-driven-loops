...
**Moat thesis.** One-shot research agents produce a well-cited snapshot; deep-review audits an existing artifact, council deliberates alternatives, improvement optimizes a system, and alignment checks a standard. `deep-research` can uniquely maintain an outward-facing question–claim–evidence program across time: each fresh-context pass selects the most valuable unresolved verification, records exactly how evidence changed a claim, and later reopens only claims affected by new sources, corrections, or retractions. Sharpening that lifecycle—not adding more searches—is the moat: the output becomes inspectable, incrementally refreshable research whose history and negative knowledge survive model context and personnel changes.

### Highest-leverage changes

| Rank | Change | Impact / effort | Concrete mechanism |
|---|---|---:|---|
| 1 | Claim ledger plus promotion gate | High / M | Require structured `ClaimRecord` objects before findings enter synthesis: `claim_id`, canonical text, status, question IDs, evidence locators, support/refute/qualify relation, source-independence cluster, confidence, and version interval. Emit `claim_asserted`, `evidence_linked`, `claim_superseded`, and `claim_invalidated` JSONL events; let the reducer generate prose. |
| 2 | Dual-sided evidence synthesis | High / M | Separate retrieval from writing. For every decision-relevant claim, run support and contradiction probes, rank exact passages, cap evidence per ownership cluster, and promote only claims with locatable support and no unresolved refutation. Otherwise render `contested` or `evidence_gap`. PaperQA exposes this separation through search, `gather_evidence`, contextual scoring, `evidence_k`, and its `contracrow` configuration. [PaperQA](https://github.com/Future-House/paper-qa) |
| 3 | Affected-claim refresh with a two-snapshot test | High / S initially, M operationally | Store source identifiers, content hashes, last-seen timestamps, status, and reverse `dependent_claim_ids`. On resume, detect changed sources, compute the affected claim set, and dispatch targeted verification only. First validate this offline using injected additions, corrections, retractions, and syndicated duplicates; measure affected-claim precision/recall and unaffected-claim stability. |

W3C PROV supplies a ready vocabulary: claims and evidence spans are `Entity` objects, iterations and synthesis are `Activity` objects, and `wasDerivedFrom`, `wasGeneratedBy`, and invalidation edges preserve lineage. The Python `prov` library already serializes PROV-JSON/RDF and converts documents to NetworkX graphs, avoiding a bespoke provenance ontology. [W3C PROV-O](https://www.w3.org/TR/prov-o/), [trungdong/prov](https://github.com/trungdong/prov)

CoLRev adds the complementary process lesson: evidence synthesis should use explicit stages with validation and undo, not rely on prose conventions. Its current documentation describes Git-backed, fault-tolerant review workflows with data-quality, curation, validation, and rollback operations. [CoLRev documentation](https://colrev-environment.github.io/colrev/)

OpenAlex exposes `is_retracted` and incremental `from_updated_date` filtering, making it useful as a refresh trigger. The latter requires Premium access, so a cheaper initial implementation should re-fetch only registered identifiers. More importantly, `is_retracted` must not be treated as ground truth: published analysis found OpenAlex retraction misclassifications. Use it to enqueue verification against publisher, Crossref, or retraction-notice evidence. [OpenAlex filters](https://github.com/ourresearch/openalex-docs/blob/main/api-entities/works/filter-works.md), [retraction-field analysis](https://arxiv.org/abs/2403.13339)

The evaluation target should also move from “answer has citations” to claim-level citation recall, passage precision, contradiction recall, and grounded refusal. The newly published ResearchQA benchmark explicitly allows multiple valid supporting passages and rewards refusing unsupported answers—useful semantics for the promotion gate, though it is currently only a very recent preprint. [ResearchQA](https://arxiv.org/abs/2607.11074)

Remaining gaps are source-independence clustering, non-academic change feeds, prompt-injection-guard false positives, and choosing between value-of-information, bandit, and active-learning question schedulers. Those are the next divergent passes; they should follow the claim schema because each needs claim-level outcomes to measure.

```json
{
  "new_repos": [
    {
      "name": "Future-House/paper-qa",
      "url": "https://github.com/Future-House/paper-qa",
      "stars": "unknown",
      "what": "Scientific RAG with query-conditioned evidence gathering, passage scoring, contradiction search, and citation-linked answers; README documents substantial December 2025 updates.",
      "lesson": "Separate evidence acquisition from prose synthesis and expose support and contradiction retrieval as explicit claim-level operations.",
      "maps_to": [
        "deep-research",
    ...
    {
      "name": "CoLRev-Environment/colrev",
      "url": "https://github.com/CoLRev-Environment/colrev",
      "stars": "unknown",
      "what": "Git-backed collaborative literature-review environment; current documentation reports version 0.16.2 with validate, undo, curation, and explicit workflow stages.",
      "lesson": "Model research synthesis as reversible typed stages with validation rather than an accumulating prose document.",
      "maps_to": [
        "deep-research",
    ...
    },
    {
      ...
      "stars": "~134",
      "what": "Python implementation of W3C PROV with PROV-JSON, PROV-O/RDF, graph export, and release 2.1.1 dated 2025-06-24.",
      "lesson": "Represent claims, evidence spans, iterations, derivations, and invalidations with an established provenance model instead of inventing incompatible lineage fields.",
      "maps_to": [
        "deep-research",
        "runtime/state-jsonl-checkpointing",
    ...
    {
      "name": "ourresearch/openalex-docs",
      "url": "https://github.com/ourresearch/openalex-docs",
      "stars": "unknown",
      "what": "Official OpenAlex API documentation containing work retraction state and incremental from_updated_date filtering; documentation was current when crawled in 2026.",
      "lesson": "Maintain a source-refresh watermark and use metadata changes to enqueue targeted claim revalidation rather than rerunning the entire research topic.",
      "maps_to": [
        "deep-research",
        "runtime/continuity-threading",
        "runtime/budget-cost"
  ...
  "insights": [
    {
      "insight": "The synthesis artifact should be a materialized view of a versioned atomic-claim ledger, with evidence-span and derivation edges, rather than the primary mutable state.",
      "evidence": "https://www.w3.org/TR/prov-o/ and https://github.com/trungdong/prov",
      "maps_to": [
        "deep-research",
        "runtime/state-jsonl-checkpointing",
        "runtime/continuity-threading"
    ...
    {
      "insight": "Support retrieval, contradiction retrieval, and answer generation should be separate operations so a fluent synthesis cannot conceal missing or conflicting evidence.",
      "evidence": "https://github.com/Future-House/paper-qa",
      "maps_to": [
        "deep-research",
        "runtime/fan-out-fan-in"
      ],
    ...
    {
      "insight": "Claim evaluation should allow multiple valid evidence passages and score grounded abstention, not require one canonical citation or reward unsupported completeness.",
      "evidence": "https://arxiv.org/abs/2607.11074",
      "maps_to": [
        "deep-research",
    ...
    {
      "insight": "Retraction and update metadata are useful change signals but unsafe as automatic truth; they should reopen dependent claims for verification.",
      "evidence": "https://github.com/ourresearch/openalex-docs/blob/main/api-entities/works/filter-works.md and https://arxiv.org/abs/2403.13339",
      "maps_to": [
        "deep-research",
        "runtime/continuity-threading",
        "runtime/locks-recovery"
  ...
  "recommendations": [
    {
      "rec": "Add a reducer-owned ClaimRecord intermediate representation with versioned status, exact evidence locators, support/refute/qualify relations, independence clusters, source versions, and supersession links; emit claim lifecycle events to JSONL and generate research.md from the ledger.",
      "target": "deep-research + runtime/state-jsonl-checkpointing",
      "rationale": "This eliminates silent prose overwrite, makes contradictions explicit, and provides the reverse dependency index needed for targeted refresh.",
      "uniqueness": "It turns deep-research into a persistent evidence program whose claim history remains inspectable and updateable after the original run—something snapshot research agents and the other deep modes do not own.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://www.w3.org/TR/prov-o/ and https://github.com/trungdong/prov"
    },
    {
      "rec": "Insert a pre-synthesis claim-promotion gate that runs separate support and contradiction probes, requires exact passage locators, limits evidence dominance by source cluster, and emits contested or evidence_gap states instead of forcing an answer.",
      "target": "deep-research",
      "rationale": "Existing source-diversity gates operate at research-output level; claim-level acceptance catches citation drift, unsupported inference, and buried counterevidence before prose synthesis.",
      "uniqueness": "Deep-research becomes a system that preserves justified uncertainty and negative knowledge across passes, not merely a longer-form answer generator.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://github.com/Future-House/paper-qa and https://arxiv.org/abs/2607.11074"
    },
    {
      "rec": "Create a two-snapshot longitudinal fixture with additions, corrections, retractions, and ownership-linked duplicate sources; compute affected-claim precision and recall plus unaffected-claim stability before wiring incremental OpenAlex or Crossref refresh.",
      "target": "deep-research + runtime/gauges-observability",
      "rationale": "It directly tests whether targeted refresh is safer and cheaper than full reruns and exposes dependency or source-independence errors before live automation.",
      "uniqueness": "Passing this benchmark substantiates the living-research moat: the mode can revise exactly what changed while preserving valid prior work.",
      "effort": "S",
      "impact": "high",
      "evidence": "https://github.com/ourresearch/openalex-docs/blob/main/api-entities/works/filter-works.md and https://arxiv.org/abs/2403.13339"
    }
  ...
  "contradictions": [
    {
      "claim": "OpenAlex is_retracted can serve as automatic claim-invalidation ground truth.",
      "counter": "A published comparison reports retraction misclassifications; treat the field as a revalidation trigger and require confirmation from retraction notices, Crossref relations, or publishers.",
      "evidence": "https://arxiv.org/abs/2403.13339"
    }
  ],
  "next_angles": [
    "Define the ClaimRecord JSON schema, lifecycle event grammar, reducer invariants, and migration from the existing findings registry.",
    "Build the two-snapshot longitudinal benchmark and compare affected-claim propagation against full-run regeneration.",
    "Compare contextual-bandit, Bayesian value-of-information, and constrained active-learning schedulers using claim-resolution gain per cost.",
    "Benchmark source-independence clustering across DOI versions, syndication, shared ownership, review articles, and duplicated web content.",
    "Measure prompt-injection guard false positives on legitimate papers, code, and security documentation containing imperative language."
  ],
  "notes": "GitHub star counts were not reliably exposed for PaperQA, CoLRev, or openalex-docs, so they are marked unknown rather than estimated. ResearchQA is extremely recent and should be treated as a promising benchmark design, not established consensus. Non-academic source refresh, Crossref/Retraction Watch integration details, and source-ownership clustering remain uncovered."
}
```
...
session id: 019f64dd-3d03-71a0-9614-c40d938636ec
--------
user
You are iteration 5/40 of a per-MODE, NON-CONVERGING (deliberately broadening within the mode) research loop to make ONE mode of "system-deep-loop" both MORE EFFECTIVE and MORE UNIQUELY VALUABLE.
...
THIS ITERATION'S ANGLE (A5-synth): deep-research :: synthesis & highest-leverage recs
DIRECTIVE: Synthesis pass for deep-research. Across the prior 4 angles, produce the highest-leverage MODE-SPECIFIC recommendations (impact x effort) and a sharp one-paragraph 'why deep-research is uniquely valuable and how we make it more so' thesis. Surface remaining gaps and next_angles.
...
THIS run's new repos so far (12) — also do not repeat:
  assafelovic/gpt-researcher, langchain-ai/local-deep-researcher, GAIR-NLP/DeepResearcher, bytedance/deer-flow, asreview/asreview, ResearchObject/ro-crate-py, in-toto/attestation, imlrz/DeepResearch-Bench-II, markrussinovich/refchecker, amazon-science/RefChecker, amazon-science/RAGChecker, microsoft/BIPIA
Modes covered so far this run: deep-research
New insights so far: 13; recommendations: 9.
Open threads flagged: Build a longitudinal benchmark by introducing additions, corrections, retractions, and ownership-linked duplicate sources between two snapshots, then measure affected-claim precision and recall. | Compare contextual-bandit, Bayesian value-of-information, and constrained active-learning schedulers for question-DAG allocation using evidence-yield-per-cost and independent-source gain. | Measure guard false positives on legitimate sources containing imperative prose, code snippets, or security instructions that resemble prompt injection. | Compare result-set MinHash, source-cluster overlap, and atomic-claim delta as redundant-query-thrash detectors on long research traces. | Test sequential change-point detection on trusted evidence yield as an alternative to fixed three-iteration rolling windows. | Study correction and retraction feeds such as Crossref relation metadata and Retraction Watch for automatic claim invalidation after a completed run.
...
hook: UserPromptSubmit Completed
codex
This is a read-only synthesis pass, so the spec-folder write gate does not apply. I’m routing it through the existing deep-research iteration contract, then checking current primary sources for mechanisms and recency.
exec
/bin/zsh -lc 'python3 .opencode/skills/system-skill-advisor/mcp_server/scripts/skill_advisor.py "Iteration 5/40 deep-research synthesis and highest-leverage mode-specific recommendations with live source verification and unique-value moat" --threshold 0.8' in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 2054ms:
2026-07-15 10:21:14.212 xcodebuild[39087:206794312]  DVTFilePathFSEvents: Failed to start fs event stream.
2026-07-15 10:21:14.370 xcodebuild[39087:206794311] [MT] DVTDeveloperPaths: Failed to get length of DARWIN_USER_CACHE_DIR from confstr(3), error = Error Domain=NSPOSIXErrorDomain Code=5 "Input/output error". Using NSCachesDirectory instead.
python3: warning: confstr() failed with code 5: couldn't get path of DARWIN_USER_TEMP_DIR; using /tmp instead
python3: error: couldn't create cache file '/tmp/xcrun_db-O2UPbqpD' (errno=Operation not permitted)
2026-07-15 10:21:14.782 xcodebuild[39091:206794341]  DVTFilePathFSEvents: Failed to start fs event stream.
2026-07-15 10:21:14.888 xcodebuild[39091:206794338] [MT] DVTDeveloperPaths: Failed to get length of DARWIN_USER_CACHE_DIR from confstr(3), error = Error Domain=NSPOSIXErrorDomain Code=5 "Input/output error". Using NSCachesDirectory instead.
...
[
  ...
  },
  {
    ...
    "uncertainty": 0.2,
    "passes_threshold": true,
    "reason": "Matched: !verification, !verification(keyword), mode [disambiguation: deep-research reserved for this prompt]",
    "_graph_boost_count": 0,
    "source": "local"
  }
]
...
codex
Skill routing resolved at 0.95 to `system-deep-loop` in research mode. The memory trigger service is unavailable in this session, so there is no recovered continuity beyond the prior-work packet embedded in your prompt; I’m treating that packet as the source of truth.
exec
/bin/zsh -lc "rg --files .opencode/skills/system-deep-loop | rg '("'^|/)SKILL'"\\.md"'$|deep-research'"'" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
...
exec
/bin/zsh -lc "sed -n '1,170p' .opencode/skills/system-deep-loop/deep-research/SKILL.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
name: deep-research
description: "Autonomous deep-research loop: iterative investigation, externalized state, convergence detection, fresh context per pass."
...
exec
/bin/zsh -lc "sed -n '171,340p' .opencode/skills/system-deep-loop/deep-research/SKILL.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
    if resolved.suffix.lower() != ".md":
        raise ValueError(f"Only markdown resources are routable: {relative_path}")
    return resolved.relative_to(SKILL_ROOT).as_posix()
...
exec
/bin/zsh -lc "sed -n '341,520p' .opencode/skills/system-deep-loop/deep-research/SKILL.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
10. **Document ruled-out directions per iteration** -- Every iteration must include what was tried and failed
11. **Report newInfoRatio + 1-sentence novelty justification** -- Every JSONL iteration record must include both
12. **Quality guards must pass before convergence** -- Source diversity, focus alignment, and no single-weak-source checks must pass before STOP can trigger
...
codex
The mode contract makes synthesis a first-class phase, but its current guarantees are mostly artifact assembly and source-quality gates. I’m checking mechanisms that turn that phase into a claim-level, updateable research product—the strongest mode-specific gap after the runtime work already covered.
exec
/bin/zsh -lc 'wc -l .opencode/skills/system-deep-loop/deep-research/references/guides/quick_reference.md .opencode/skills/system-deep-loop/deep-research/references/convergence/convergence.md .opencode/skills/system-deep-loop/deep-research/references/state/state_outputs.md .opencode/skills/system-deep-loop/deep-research/references/protocol/spec_check_protocol.md .opencode/skills/system-deep-loop/deep-research/feature_catalog/research_output/progressive_synthesis.md' in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
     ...
     232 .opencode/skills/system-deep-loop/deep-research/references/protocol/spec_check_protocol.md
      60 .opencode/skills/system-deep-loop/deep-research/feature_catalog/research_output/progressive_synthesis.md
     851 total

exec
/bin/zsh -lc "sed -n '1,260p' .opencode/skills/system-deep-loop/deep-research/references/guides/quick_reference.md
sed -n '1,220p' .opencode/skills/system-deep-loop/deep-research/references/convergence/convergence.md
sed -n '1,220p' .opencode/skills/system-deep-loop/deep-research/references/state/state_outputs.md
sed -n '1,280p' .opencode/skills/system-deep-loop/deep-research/references/protocol/spec_check_protocol.md
sed -n '1,120p' .opencode/skills/system-deep-loop/deep-research/feature_catalog/research_output/progressive_synthesis.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
...
| File | Layer | Role |
|---|---|---|
| `.opencode/skills/system-deep-loop/deep-research/assets/deep_research_config.json` | Asset | Sets `progressiveSynthesis` to `true` by default in the live config template. |
| `.opencode/agents/deep-research.md` | Agent | Defines when the iteration agent may create or update `research/research.md`. |
| `.opencode/skills/system-deep-loop/deep-research/references/state/state_format.md` | Reference | Defines progressive update rules for `research/research.md`. |
| `.opencode/skills/system-deep-loop/deep-research/references/protocol/loop_protocol.md` | Reference | Defines the final synthesis cleanup pass that consolidates the document. |
...
| File | Type | Role |
|---|---|---|
| `.opencode/skills/system-deep-loop/deep-research/manual_testing_playbook/iteration_execution_and_state_discipline/progressive_synthesis_behavior_for_research_md.md` | Manual playbook | Verifies iteration-time updates respect the progressive-synthesis contract. |
| `.opencode/skills/system-deep-loop/deep-research/manual_testing_playbook/synthesis_save_and_guardrails/final_synthesis_memory_save_and_guardrail_behavior.md` | Manual playbook | Verifies final synthesis still owns the terminal research document. |
...
- Feature file path: `research-output/progressive-synthesis.md`
Related references:
- [negative-knowledge.md](../research_output/negative_knowledge.md) — Negative knowledge

web search: 
web search: Future-House paper-qa evidence citation implementation GitHub docs ...
web search: 
web search: site:github.com/Future-House/paper-qa evidence citation implementation ...
web search: 
web search: GitHub CoLRev-Environment colrev systematic review provenance stars ...
codex
The synthesis points to one priority: turn `deep-research` from an iterative answer generator into a living, claim-addressable evidence system. Its current progressive synthesis deduplicates prose and preserves gaps, while JSONL, reducers, and the findings registry already provide the right substrate. What is missing is an intermediate representation connecting atomic claims to evidence spans, contradictions, source versions, and later invalidation.
