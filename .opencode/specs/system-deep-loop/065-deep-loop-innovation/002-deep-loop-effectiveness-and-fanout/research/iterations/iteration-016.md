...
A suitable runtime event would persist `logicalBranchId`, phase, calibrated evidence mass, disagreement, predicted success, controller version, selected action, and budget delta. The action set should be `stop_accept | sample_peer | deepen_verifier | escalate_model | hotswap | abandon_with_receipt`. This extends R1/R4/R8 without weakening their path-coverage and replay requirements.

Three mechanisms stand out:

1. Separate planning from execution. Reward-guided dual-phase search scores and budgets plans independently from their executions, preventing weak plans from consuming full tool trajectories. This maps directly to council proposal selection followed by conditional fan-out. [Dual-phase search paper](https://arxiv.org/abs/2509.25420)

2. Replace vote counts with evidence sufficiency. ReASC first attempts a calibrated single-sample exit, then accumulates confidence-weighted support rather than treating every answer equally. For council use, this must be adjusted for correlated seats: effective-independence weights should discount shared model, prompt, evidence, and ancestry. [ReASC paper](https://arxiv.org/abs/2601.02970)

3. Scale verification selectively. GenPRM and ThinkPRM make the verifier itself a reasoning process that can receive parallel or sequential compute. The practical design is a judge ladder: deterministic checks, cheap scalar scorer, then generative process verification only for ambiguous or high-impact branches. [GenPRM](https://github.com/RyanLiu112/GenPRM), [ThinkPRM](https://github.com/mukhal/ThinkPRM)

Two adjacent techniques deserve guarded adoption. CATS uses reward-distribution and sparsity features to choose search hyperparameters through an actor-critic controller; this is a better template for budget allocation than fixed fan-out width. [CATS paper](https://arxiv.org/abs/2505.18065) Atropos predicts eventual agent-path failure from a merged inference graph and hotswaps the surviving context to a stronger model; for system-deep-loop, hotswap should be a salvage transition retaining the same logical branch and recording the original attempt as censored evidence. [Atropos paper](https://arxiv.org/abs/2604.15075)

The main caveat is calibration. Most evidence remains concentrated in math and code reasoning. Confidence-weighted stopping, generative verifiers, and online pseudo-rewards should enter through paired replay and shadow decisions before they control live termination.

```json
{
  "new_repos": [
    {
      "name": "RyanLiu112/GenPRM",
      "url": "https://github.com/RyanLiu112/GenPRM",
      "stars": "~100",
      "what": "AAAI 2026 codebase for generative process verification; its 2025 release uses relative-progress labels, verification rationales, and code-assisted judging.",
      "lesson": "Treat verification as an independently scalable reasoning phase, but invoke it only after cheap deterministic and scalar checks fail.",
      "maps_to": [
        "deep-review",
        "deep-ai-council",
    ...
    },
    {
      ...
      "stars": "~2",
      "what": "NeurIPS 2025 thinking-optimal test-time-scaling implementation with src/sft code, test data, and scripts/run_eval.sh; repository was still visible in June 2026.",
      "lesson": "Make reasoning allowance a controller-selected variable and evaluate quality against consumed compute, rather than assigning every branch the same token ceiling.",
      "maps_to": [
        "runtime/budget-cost",
        "runtime/convergence",
        "deep-improvement"
    ...
    },
    {
      ...
      "stars": "unknown",
      "what": "NeurIPS 2025 test-time reinforcement-learning implementation using majority-derived pseudo-rewards; its August 2025 verl integration exposes +ttrl.enable=True.",
      "lesson": "Use completed-loop consensus as offline training data for controller policies, but version and regression-test learned policies instead of mutating live policy inside a replayable lineage.",
      "maps_to": [
        "deep-improvement",
        "runtime/convergence",
    ...
    {
      "name": "mukhal/ThinkPRM",
      "url": "https://github.com/mukhal/ThinkPRM",
      "stars": "unknown",
      "what": "TMLR generative process-reward-model repository; test-time-scaling code was released July 2, 2025 and supports longer or parallel verifier reasoning.",
      "lesson": "Expose judge compute as its own typed budget and increase it only when cheap checks leave high decision uncertainty.",
      "maps_to": [
        "deep-review",
        "deep-ai-council",
  ...
  "insights": [
    {
      "insight": "Split each expensive branch into plan selection and execution, with separate reward distributions and budgets; execute only plans whose expected utility or upper confidence bound remains competitive.",
      "evidence": "Adaptive Test-Time Reasoning via Reward-Guided Dual-Phase Search — https://arxiv.org/abs/2509.25420",
      "maps_to": [
        "deep-ai-council",
        "runtime/fan-out-fan-in",
        "runtime/budget-cost"
    ...
    {
      "insight": "Replace count-based council quorum with a two-stage gate: calibrated single-leaf acceptance followed by confidence-weighted evidence accumulation, discounted by shared model, prompt, evidence, and ancestry.",
      "evidence": "Reliability-Aware Adaptive Self-Consistency — https://arxiv.org/abs/2601.02970",
      "maps_to": [
        "deep-ai-council",
        "runtime/convergence",
    ...
    {
      "insight": "A budget controller can consume reward-distribution shape, reward sparsity, remaining budget, and branch history, then emit sampling width, depth, verifier effort, or stop as explicit actions.",
      "evidence": "Reward Model Generalization for Compute-Aware Test-Time Reasoning — https://arxiv.org/abs/2505.18065",
      "maps_to": [
        "runtime/fan-out-fan-in",
        "runtime/budget-cost",
    ...
    {
      "insight": "Predicted failure need not mean termination: preserve the logical branch and hotswap its accumulated context to a stronger executor, while retaining the original attempt as censored evidence with a recovery receipt.",
      "evidence": "Atropos — https://arxiv.org/abs/2604.15075",
      "maps_to": [
    ...
    {
      "insight": "Verifier compute is a separate scaling axis: store raw cheap scores first, attach generative verification rationales later, and spend verifier tokens only near decision boundaries or on high-impact claims.",
      "evidence": "GenPRM https://github.com/RyanLiu112/GenPRM and ThinkPRM https://github.com/mukhal/ThinkPRM",
      "maps_to": [
        "deep-review",
        "deep-ai-council",
        "runtime/gauges-observability"
  ...
  "recommendations": [
    {
      "rec": "Add an append-only adaptive_compute_decision event containing logicalBranchId, phase, calibrated support and opposition mass, disagreement, predicted success, controllerVersion, action, typed budget delta, and replay fingerprint. Support stop_accept, sample_peer, deepen_verifier, escalate_model, hotswap, and abandon_with_receipt.",
      "target": "runtime/fan-out-fan-in + runtime/budget-cost + runtime/state-jsonl-checkpointing",
      "rationale": "This converts fixed fan-out and binary stop decisions into replayable compute allocation while preserving R1 path coverage, R2 receipts, and R8 typed budgets.",
      "effort": "L",
      "impact": "high",
      "evidence": "https://arxiv.org/abs/2509.25420 and https://arxiv.org/abs/2505.18065"
    },
    {
      "rec": "Implement a three-tier verification ladder: deterministic tests and schema checks, lightweight calibrated scorer, then generative process verifier. Persist every raw score before reduction and require verifier escalation only for uncertainty-band or high-impact cases.",
      "target": "deep-review + deep-ai-council + runtime/gauges-observability",
      "rationale": "It combines R5 cheap-first checking with newer verifier-time scaling without paying generative-judge cost on obvious passes or failures.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://github.com/RyanLiu112/GenPRM and https://github.com/mukhal/ThinkPRM"
    },
    {
      "rec": "Run controller learning in shadow mode over complete paired replays: log the action a learned controller would take, compare it with the live policy, and promote only versioned policies that pass quality-cost Pareto and replay-determinism gates.",
      "target": "deep-improvement + runtime/convergence",
      "rationale": "TTRL-style pseudo-rewards can improve allocation without allowing within-lineage online updates to invalidate deterministic replay or reinforce correlated council errors.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://github.com/PRIME-RL/TTRL"
    }
  ],
  "contradictions": [
    {
      "claim": "For deep-ai-council, more agreeing samples provide stronger evidence and can safely trigger quorum.",
      "counter": "ReASC shows that responses should contribute according to reliability, while R3 implies correlated seats are not independent evidence; quorum needs confidence and effective-independence weighting.",
      "evidence": "https://arxiv.org/abs/2601.02970"
    },
    {
      "claim": "For deep-review, cheap checks should always precede and largely replace expensive judges.",
      "counter": "GenPRM and ThinkPRM show that additional verifier-side reasoning can improve difficult judgments; the reconciled policy is conditional escalation rather than judge elimination.",
      "evidence": "https://github.com/RyanLiu112/GenPRM and https://github.com/mukhal/ThinkPRM"
    },
    {
      "claim": "For runtime/convergence, online adaptation from majority pseudo-rewards should make each successive iteration more effective.",
      "counter": "Live TTRL-style updates change the decision policy during replay and can amplify correlated majority errors; adaptation must be checkpoint-versioned, evaluated off-policy, and promoted through deep-improvement.",
      "evidence": "https://github.com/PRIME-RL/TTRL"
    }
  ],
  "next_angles": [
    "Calibrate confidence-weighted council stopping under correlated heterogeneous seats: compare conformal calibration, effective-sample-size corrections, and dependence-aware evidence accumulation.",
    "Specify an off-policy evaluator for adaptive_compute_decision events so fixed, learned, and oracle controllers can be compared on identical complete JSONL trajectories.",
    "Test context hotswap portability across model providers, tool schemas, and prompt versions, including what state must be summarized versus replayed verbatim.",
    "Derive decision boundaries for escalating from scalar scorer to generative verifier using expected value of information under a typed verifier budget."
  ],
  "notes": "No public code repository was verified for ReASC, CATS, dual-phase search, or Atropos, so their implementation-level transfer remains paper-derived. Star counts for TTRL and ThinkPRM were not reliably exposed and are marked unknown. Most adaptive-compute evidence remains math-heavy; Atropos is the strongest direct agent-loop evidence. The required next validation is paired offline replay on full tool-using system-deep-loop trajectories, with calibration segmented by model, prompt, role, evidence set, and replay fingerprint."
}
```
...
session id: 019f643b-ee97-7352-9613-97cf0f63c2d0
--------
user
You are iteration 16/20 of a TARGETED, NON-CONVERGING (deliberately broadening WITHIN its thread) research loop to make "system-deep-loop" MORE EFFECTIVE.
...
ANGLE: 2025-2026 SOTA techniques for more-effective agent loops
DIRECTIVE: What are the newest (2025-2026) techniques for making agentic/iterative loops more effective that 001 did NOT catalogue? Look for recent papers/repos on loop control, adaptive compute, test-time scaling, verification loops, self-consistency, process reward models. Extract concrete, adoptable mechanisms + recommendations for system-deep-loop.
...
New insights so far: 61; new contradictions: 49; recommendations: 44.
Open threads flagged: For runtime/budget-cost, specify exact sliding-log, bucketized sliding-window, token-bucket, and calendar-window semantics, including replay behavior across clock rollback, downtime, and reset-event duplication. | For deep-improvement and runtime/convergence, calibrate degradation order on a quality-cost Pareto frontier: reduce fan-out width, switch model, lower reasoning allowance, skip judges, or stop, while measuring which action causes the least quality regression. | Derive an anytime confidence sequence for evaluator-scored incremental utility under bounded evaluator drift, including explicit reset rules for model, prompt, evidence, and replay-fingerprint changes. | Build an offline paired-replay study comparing Stop-RAG-style learned Q stopping, fixed-confidence STOP-as-arm selection, and the current novelty-plus-gates rule on identical complete system-deep-loop trajectories. | Formalize a path-covering stopping-time theorem where one logical pull may contain retries, salvage, handoffs, and side-effect compensation, and determine which tail assumptions survive adaptive fan-out. | Specify and model-check the durable-step state machine, especially competing receipts, lease expiry, stale fencing epochs, compensation failure, and winner selection between divergent LLM attempts. | Test which LLM and tool providers support durable idempotency keys or request-status lookup, and define provider-specific recovery capabilities instead of one generic retry policy. | Design replay compatibility rules separating orchestration-code compatibility, prompt/model identity, tool-schema compatibility, and reducer projection compatibility.
...
codex
This is read-only research, so no spec-folder write gate applies. I’m routing it through the deep-research workflow named in the brief, then I’ll verify a small set of genuinely new 2025–2026 mechanisms against primary papers and repositories.
mcp: mk-spec-memory/memory_match_triggers started
mcp: mk-spec-memory/memory_match_triggers (failed)
user cancelled MCP tool call
exec
/bin/zsh -lc "node .opencode/bin/skill-advisor.cjs advisor_recommend --json '{\"prompt\":\"iteration 16 of a targeted non-converging deep-research loop for system-deep-loop effectiveness; research 2025-2026 agent-loop control, adaptive compute, test-time scaling, verification loops, self-consistency, and process reward models; output verified repos, papers, mechanisms, and recommendations\"}' --warm-only --format json --timeout-ms 3000" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 exited 75 in 0ms:
{
  "status": "error",
...
exec
/bin/zsh -lc 'python3 .opencode/skills/system-skill-advisor/mcp_server/scripts/skill_advisor.py "iteration 16 of a targeted non-converging deep-research loop for system-deep-loop effectiveness; research 2025-2026 agent-loop control, adaptive compute, test-time scaling, verification loops, self-consistency, and process reward models; output verified repos, papers, mechanisms, and recommendations" --threshold 0.8' in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 1791ms:
2026-07-15 07:24:56.756 xcodebuild[7332:205215709]  DVTFilePathFSEvents: Failed to start fs event stream.
2026-07-15 07:24:56.895 xcodebuild[7332:205215707] [MT] DVTDeveloperPaths: Failed to get length of DARWIN_USER_CACHE_DIR from confstr(3), error = Error Domain=NSPOSIXErrorDomain Code=5 "Input/output error". Using NSCachesDirectory instead.
python3: warning: confstr() failed with code 5: couldn't get path of DARWIN_USER_TEMP_DIR; using /tmp instead
python3: error: couldn't create cache file '/tmp/xcrun_db-32Ja5eVX' (errno=Operation not permitted)
2026-07-15 07:24:57.271 xcodebuild[7357:205215758]  DVTFilePathFSEvents: Failed to start fs event stream.
2026-07-15 07:24:57.371 xcodebuild[7357:205215757] [MT] DVTDeveloperPaths: Failed to get length of DARWIN_USER_CACHE_DIR from confstr(3), error = Error Domain=NSPOSIXErrorDomain Code=5 "Input/output error". Using NSCachesDirectory instead.
...
[
  ...
  },
  {
    ...
    "uncertainty": 0.23,
    "passes_threshold": true,
    "reason": "Matched: !test, !test(multi), !verification, !verification(keyword), models~ [disambiguation: deep-research reserved for this prompt]",
    "_graph_boost_count": 0,
    "source": "local"
  },
  {
  ...
  },
  {
    ...
    "uncertainty": 0.3,
    "passes_threshold": true,
    "reason": "Matched: command_penalty, deep(name), loop, loops~, spec(name)",
    "_graph_boost_count": 0,
    "source": "local"
  }
]
...
codex
SKILL ROUTING: `system-deep-loop` research mode (0.95 confidence); the memory trigger service was unavailable, while the advisor’s local fallback succeeded. The workflow contract now controls the research pass, including its non-converging iteration semantics and output shape.
exec
...
exec
/bin/zsh -lc "sed -n '1,240p' .opencode/skills/system-deep-loop/mode-registry.json && wc -l .opencode/skills/system-deep-loop/deep-research/SKILL.md && sed -n '1,320p' .opencode/skills/system-deep-loop/deep-research/SKILL.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
{
  "skill": "system-deep-loop",
  "version": "2.0.0.0",
...
exec
/bin/zsh -lc "sed -n '1,10000p' .opencode/skills/system-deep-loop/deep-research/references/protocol/loop_protocol.md .opencode/skills/system-deep-loop/deep-research/references/state/state_outputs.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
title: Loop Protocol Reference
description: Canonical specification for the deep research loop lifecycle with 4 phases, reference-only wave orchestration, and error handling.
...
exec
/bin/zsh -lc "sed -n '250,420p' .opencode/skills/system-deep-loop/deep-research/references/protocol/loop_protocol.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
...
web search: 
web search: site:arxiv.org 2025 adaptive test-time compute verification process reward model reasoning GitHub ...
codex
The first pass surfaced a useful cluster the prior survey missed: adaptive compute is moving from “sample more until agreement” to three finer controls—plan-versus-execution budget allocation, confidence-weighted evidence accumulation, and verifier-side reasoning. I’m checking the implementations and release signals now; weak or unverifiable star counts will stay `unknown`.
web search: 
web search: 
codex
The strongest new pattern is not another stopping score. It is a multi-action compute controller deciding where the next unit of budget goes: planning, execution, another independent sample, deeper verification, or a stronger model.
