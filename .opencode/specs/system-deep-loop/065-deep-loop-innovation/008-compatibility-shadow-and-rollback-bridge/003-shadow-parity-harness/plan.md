...
| Aspect | Value |
|--------|-------|
| **Surface** | system-deep-loop phase-008 migration bridge |
...
| **Primary inputs** | Phase-003 baseline, phase-006 replay fingerprints, phase-007 sealed artifacts, sibling-002 projections |
...
only after the complete case set is green. The certificate is evidence for phase 014, not authority to cut over.
<!-- /ANCHOR:summary -->
...
- **Case-manifest compiler**: closes the phase-003 scenario and state census into versioned parity rows. It rejects an
  unclassified behavior, state surface, reader, effect, or projection instead of inferring a comparison boundary.
- **Sealed-input preflight**: resolves one ordered artifact-reference set through phase 007, verifies every descriptor
...
- **Replay verifier**: calls the phase-006 registered API for each comparison transcript, retains the complete
...
- **Certificate issuer and verifier**: canonicalizes the complete pass set into one mode-bound certificate and exposes
  a fail-closed freshness check for phase-013 gates and phase 014. It cannot write an authority flag.
...
- Compile the phase-003 scenario, state, schema, behavior, and reader censuses into a closed mode-addressable manifest.
...
- Implement complete observation capture and phase-006 fingerprint verification for both comparison transcripts.
...
- Integrate certificate consumption into phase-013 mode gates and the phase-014 precondition without adding an authority write.
...
|-------------|--------------|
| REQ-001 | Manifest-closure test maps every executed phase-003 scenario, workstream, protected observation, state surface, reader, and projection row exactly once or fails with a named gap |
...
| REQ-005 | Fingerprint-version, registry, upcaster, reducer, projection-schema, and canonicalizer drift each fail through the phase-006 verifier; verified component equality passes |
...
| REQ-012 | Drift matrix mutates code/build, BASE, manifest, seals, replay, reducer, projection, adapter, and comparator identities and requires phase-014 verification failure |
...
`../../003-baseline-taxonomy-and-state-census/spec.md`; the registered fingerprint descriptor and verifier from
`../../006-transition-authorized-ledger-core/003-replay-fingerprints/spec.md`; verified artifact references from
`../../007-shared-evidence-and-control-services/002-sealed-reference-artifacts/spec.md`; exact legacy-shape contracts
...
those execution contracts exist. Downstream consumers are the per-mode gates in phase 013 and the authority preflight
in phase 014. Successor `004-inflight-state-classification` remains an adjacent independent planning contract.
<!-- /ANCHOR:dependencies -->

<!-- ANCHOR:rollback -->
## 7. ROLLBACK PLAN

The harness is additive and non-authoritative. Before phase 014, rollback disables the harness integration, revokes
...
Rollback must not delete divergence evidence, phase-006 attestations, sealed inputs under hold, or certificate audit
