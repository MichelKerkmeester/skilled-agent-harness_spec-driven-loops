---
title: "Feature Specification: Replay Fingerprints (003 phase 003)"
...
| **Origin** | Third child of the phase-003 transition-authorized ledger-core parent |
| **Depends on** | None (`[]`); sibling contracts compose at the phase-003 parent gate |
| **Consumers** | Phase 005 shadow-parity harness and phase 013 whole-system replay gate |
<!-- /ANCHOR:metadata -->
...
- Consumption gates that prevent mismatched or unverifiable replay output from becoming projection state, phase-005 shadow-parity evidence, cutover evidence, or a phase-013 whole-system pass.
...
- Implementing upcasters, compatibility adapters, legacy projections, the shadow-parity comparison policy, or in-flight-state migration owned by phase 005.
...
- Moving runtime authority from legacy state to the ledger; the fingerprint path remains additive, dark, and non-authoritative until the staged phase-011 cutover.
<!-- /ANCHOR:scope -->
...
| REQ-010 | Downstream consumers use the same verification API | Phase 005 consumes verified fingerprint results for shadow parity and phase 013 consumes them for mixed-version and whole-system replay; neither consumer implements an alternative digest or bypasses a typed mismatch |
| REQ-011 | Dark coexistence preserves legacy authority | Fingerprint generation or verification failure is observable and blocks later evidence, but it does not alter the current legacy result, state, schema, or authority before phase 011 |
...
- **SC-005**: Phase 005 and phase 013 consume the same verified result and cannot create or accept an expected fingerprint during the comparison path.
