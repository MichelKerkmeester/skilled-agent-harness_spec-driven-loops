---
title: "Feature Specification: Cross-Mode Closures (009 phase 002)"
description: "Hoist recurring evidence, receipt, adjudication, budget, and projection behavior into reusable closures so all phase-010 mode migrations share one implementation while retaining explicit mode-owned policies and state reducers."
trigger_phrases:
  - "cross-mode closures"
  - "shared deep-loop mode closures"
  - "phase 009 closure modules"
  - "deep-loop shared behavior hoisting"
importance_tier: "critical"
contextType: "planning"
parent: "system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/009-shared-mode-contracts-and-fixtures"
_memory:
  continuity:
    ...
    next_safe_action: "Define closure ports, override seams, and parity fixtures before phase 010"
    blockers: []
    key_files: []
    completion_pct: 0
    open_questions: []
...
> Phase adjacency under `009-shared-mode-contracts-and-fixtures` (grouping order, not a runtime dependency): predecessor `001-shared-mode-interfaces`; successor `003-mixed-version-fixtures`.
...
| Field | Value |
|-------|-------|
| **Packet** | system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/009-shared-mode-contracts-and-fixtures/002-cross-mode-closures |
| **Level** | 2 |
| **Priority** | P0 |
| **Status** | Planned |
| **Created** | 2026-07-15 |
| **Owner skill** | system-deep-loop |
| **Origin** | Phase 002 of the 009 shared-mode-contracts-and-fixtures parent |
| **Depends on** | `[]` in `manifest/phase-tree.json`; sibling contracts are navigation inputs |
| **Parent outcome** | Hoist cross-mode closures before the eight phase-010 migrations |
<!-- /ANCHOR:metadata -->

<!-- ANCHOR:problem -->
## 2. PROBLEM & PURPOSE

The phase-009 parent is the last shared gate before the eight phase-010 workstreams. Its parent spec requires the runtime to build the shared spine once and then give each mode a typed schema over it, while the phase-tree manifest explicitly assigns this child the cross-mode closure work. Without that hoisting, deep-research, deep-review, deep-ai-council, deep-improvement-common, agent-improvement, model-benchmark, skill-benchmark, and deep-alignment will each copy the same evidence, receipt, adjudication, budget, and projection behavior into separate migrations. The copied paths would drift in error handling, evidence shape, replay behavior, and write ownership.
...
- A receipt/effect closure that orders intent, execution, observed result, recovery, and boundary receipt emission through the phase-004 receipt and effect-recovery service; it must reuse the shipped executor-audit and post-dispatch evidence seams rather than issue parallel receipts.
- An adjudication closure that constructs blinded or counterfactual requests, invokes the phase-004 adjudication service, retains raw scores and probes, and returns the shared stable/unstable/inconclusive result without re-reducing it in each mode.
- A budget closure that performs typed admission, reservation, settlement, and exhaustion reporting through phase-004 hierarchical budgets while adapting current council cost guards and fan-out estimates as inputs.
...
- Implementing the phase-004 receipts, sealed artifacts, adjudication, budgets, gauges, locks/fencing, or continuity-identity services; this phase consumes those ports.
...
- Migrating any mode, removing legacy writers, changing authority, or selecting phase-011 cutover policy.
...
| REQ-003 | Receipt emission is one ordered closure | Boundary and effect receipts use the phase-004 intent/result/recovery contract, are idempotent, and cannot be emitted before authorized durable facts |
...
| REQ-008 | Shared implementations are reused by the complete phase-010 set | All eight manifest workstreams call the same closure implementations; deep-improvement-common runs once before its three variants, and deep-review/deep-alignment share only their declared loop behavior |
...
- **SC-001**: One closure catalog and implementation set covers evidence handling, receipt/effect emission, adjudication calls, typed budget checks, and projection updates for all eight phase-010 workstreams.
...
- **SC-003**: An override matrix proves mode-specific schemas and policies enter through typed seams; no mode can bypass the phase-003/004 safety ports.
...
- **SC-006**: Phase 010 receives a reusable closure package and adapter matrix without changing the frozen lifecycle contract or moving authority from the legacy path.
<!-- /ANCHOR:success-criteria -->
...
- **Repeated behavior survives under new names** — a mode can appear to use a closure while retaining a private receipt or budget path. Mitigation: require call-path inventory, one-owner rules, negative bypass fixtures, and write-set evidence before phase 010.
...
- **Shared write races** — deep-improvement variants and review/alignment may touch common state. Mitigation: expose closure write sets to `004-write-set-conflict-graph`, use phase-004 locks/fencing, and keep serialization decisions outside the closure implementation.
- **Version drift** — closure inputs may evolve while phase 010 proceeds. Mitigation: bind each closure to the frozen phase-001 interface version and hand mixed-version behavior to `003-mixed-version-fixtures`.
- **Dependencies**: `001-shared-mode-interfaces/spec.md`, the phase-004 parent and child contracts under `004-shared-evidence-and-control-services/`, the parent `../spec.md`, `manifest/phase-tree.json`, and the shipped runtime sources under `.opencode/skills/system-deep-loop/runtime/` are contract inputs. `003-mixed-version-fixtures` and `004-write-set-conflict-graph` are sibling handoff consumers, not hard dependencies.
<!-- /ANCHOR:risks -->
