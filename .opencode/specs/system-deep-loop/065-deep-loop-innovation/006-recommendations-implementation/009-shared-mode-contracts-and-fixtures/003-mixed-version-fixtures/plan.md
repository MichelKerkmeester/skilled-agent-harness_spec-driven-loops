...
| Aspect | Value |
|--------|-------|
| **Surface** | system-deep-loop phase-009 shared mode contracts and fixtures |
| **Change class** | Sealed fixture corpus, compatibility matrix, and deterministic replay planning |
| **Execution** | Freeze before phase 010; no authority change, state rewrite, or legacy-writer removal |
| **Primary inputs** | Phase-004 sealed artifacts, phase-005 upcasters and shadow parity, phase-009 interface, phase-tree manifest |

### Overview
Define one manifest-derived fixture envelope for all eight phase-010 workstreams. The corpus keeps pure-old and
pure-new controls beside mid-upgrade and interrupted-migration cases, composes independently versioned events and state,
seals every replay-affecting input through phase 004, and records expected upcaster, reducer, resume, replay, and parity
outcomes. The fixture runner is a consumer of phase-005 contracts, not a second upcaster or parity implementation.
<!-- /ANCHOR:summary -->
...
### Definition of Ready
- [ ] The parent `009-shared-mode-contracts-and-fixtures` outcome, phase adjacency, and phase-tree outcome are recorded.
- [ ] The phase-005 upcaster/dual-read contract and shadow-parity contract are reconciled into fixture inputs, expected
  outcomes, and fail-closed classifications.
- [ ] The phase-004 sealed-reference-artifact contract is available for canonical bytes, verified reads, retention roots,
...
- [ ] The eight phase-010 workstream rows and deep-improvement-common ordering are derived from `manifest/phase-tree.json`.
...
- [ ] The phase handoff gives phase 010 the fixture namespace and phase 005 the parity-ready case identities without
  moving authority.
<!-- /ANCHOR:quality-gates -->
...
- **Seal compiler**: Canonicalize and seal prompt sets, initial state, event rows, state snapshots, configuration,
  evaluator material, prior outputs, version policy, and restart metadata through phase 004. Publish one ordered digest
  set and refuse aliases, mutable paths, or partially published capsules.
- **Compatibility adapter**: Invoke the phase-005 registry and dual-read interfaces for each stored record, retaining
...
- **Replay and parity coordinator**: Run legacy and dark paths from isolated clones of one verified capsule, verify phase-
  ...
  blocking evidence. Phase-005 remains responsible for the harness and certificate semantics.
...
### Phase 1: Setup
- Extract the phase-009 interface version, phase-005 upcaster and parity contracts, and phase-004 seal obligations into
  one fixture requirement matrix.
- Derive the eight phase-010 workstream rows from `manifest/phase-tree.json` and record common/variant ordering.
...
- Seal every case capsule through phase-004 and verify the ordered digest set before the case is runnable.
- Invoke phase-005 upcasters and dual-read adapters for supported records, recording exact hop traces and typed failures.
- Execute reducer and resume expectations against isolated case roots and capture immutable observation evidence.
- Run the same verified capsule through phase-005 shadow parity, comparing replay components and legacy projections without
  changing authority.
...
| REQ-007 | Gap, cycle, future, ambiguous, lossy, malformed, and identity-mutating fixtures fail through phase-005 typed errors |
...
The fixture contract consumes the phase-004 sealed-reference-artifact descriptor and verified-read rules from
`../../004-shared-evidence-and-control-services/002-sealed-reference-artifacts/spec.md`. It consumes the phase-005
upcaster and dual-read rules from `../../005-compatibility-shadow-and-rollback-bridge/001-upcasters-and-dual-read-adapters/
spec.md` and the sealed-input comparison and certificate preconditions from
`../../005-compatibility-shadow-and-rollback-bridge/003-shadow-parity-harness/spec.md`. It also consumes the shared mode
...
need for the phase-004 and phase-005 contracts: the fixture gate cannot accept an implementation that bypasses their
seal, upcast, comparison, or additive-dark invariants. Phase 010 consumes the frozen corpus; phase 011 alone controls
authority cutover.
<!-- /ANCHOR:dependencies -->
...
or historical state. Before phase 011, rollback disables the fixture runner integration, supersedes any fixture-manifest
