...
fails the gate. No parity means no phase-011 cutover.
<!-- /ANCHOR:protocol -->

<!-- ANCHOR:pre-impl -->
## Pre-Implementation

- [ ] CHK-001 [P0] The phase-000 baseline is pinned to one immutable BASE and exposes stable scenario IDs, protected observables, initial-state fixtures, state surfaces, readers, effects, and rollback anchors
...
- [ ] CHK-003 [P0] Phase-003 replay verification, phase-004 verified sealed reads, and sibling-002 legacy projection contracts are available under registered versions
...
- [ ] CHK-006 [P0] One comparator path serves every mode; no mode bypasses sealed-input preflight, phase-003 verification, divergence recording, or certificate freshness
...
- [ ] CHK-014 [P0] Both path transcripts verify through the phase-003 API; unknown fingerprint, registry, upcaster, reducer, projection, or canonicalizer versions fail closed
...
- [ ] CHK-023 [P0] Mutating each bound identity independently makes the certificate stale; phase 011 rejects missing, partial, superseded, wrong-mode, unverifiable, and stale certificates
...
- [ ] CHK-028 [P1] Known-defect rows follow the versioned phase-000 disposition and are not silently normalized, fixed, or excluded by the harness
<!-- /ANCHOR:fix-completeness -->
...
- [ ] CHK-033 [P1] The implemented manifest, divergence taxonomy, observation boundary, certificate schema, and phase-011 freshness contract match `spec.md` and `plan.md`
- [ ] CHK-034 [P1] Verification evidence cites the phase-000 baseline, phase-003 fingerprints, phase-004 sealed artifacts, sibling-002 projections, and `manifest/phase-tree.json`
...
zero-divergence closure, verifies the mode certificate and phase-011 drift matrix, and confirms no tracked, live-state,
