...
description: "Implementation plan for producing and verifying the five-way in-flight state disposition manifest consumed by phase 011."
trigger_phrases:
...
_memory:
  continuity:
    ...
    next_safe_action: "Freeze the phase-000 census before assigning row dispositions"
    blockers: []
    key_files: []
    completion_pct: 0
    open_questions: []
...
| Aspect | Value |
|--------|-------|
| **Surface** | system-deep-loop compatibility and phase-011 cutover handoff |
...
### Overview
Build a deterministic classifier over the frozen phase-000 in-flight-state census, assign every row exactly one of
`UPCAST`, `PIN`, `FORK`, `MIGRATE`, or `BLOCK`, verify its class-specific prerequisites, and emit an immutable manifest
digest phase 011 can consume. The plan does not move state or authority. It converts the phase-000 family inventory,
phase-001 transition/rollback constraints, and program migration model into a total cutover decision surface whose
default is `BLOCK`.
<!-- /ANCHOR:summary -->
...
### Definition of Ready
- [ ] The executed phase-000 census is frozen with stable row IDs, state-family ownership, paths, lifecycle data, and evidence digests
- [ ] The phase-001 upcaster, authority, rollback, and cutover-certificate policy is ratified
...
- [ ] The manifest digest, row evidence, rollback anchors, and freshness contract are consumable by phase 011
...
- **Census adapter**: reads the frozen phase-000 state manifest and rejects unstable IDs, duplicate rows, unsupported
...
- **Phase-011 gate adapter**: re-reads current state and requires digest/epoch/lease/effect parity before returning the
...
### Phase 1: Setup
- Freeze and verify the executed phase-000 census, BASE identity, row IDs, state-family coverage, and rollback anchors.
- Pin the phase-001 policy revision and phase-tree migration model consumed by the classifier.
...
- Bind shadow forks to phase-003 parity cases and bind all non-blocked dispositions to sibling rollback-drill scenarios.
- Emit the canonical classification manifest and a read-only phase-011 gate adapter with freshness revalidation.
...
- Simulate post-classification state/epoch drift and prove phase 011 receives `BLOCK` before compare-and-swap.
<!-- /ANCHOR:phases -->
...
| REQ-010 / REQ-011 | Digest, authority-epoch, lease, effect-set, schema, and rollback-anchor drift all invalidate the manifest row and prevent phase-011 compare-and-swap |
<!-- /ANCHOR:testing -->
...
than hard runtime dependencies. Execution still requires the frozen phase-000 census as its data source, the
phase-001 transition/versioning/rollback policy as its normative decision boundary, phase-003 shadow parity for dark
fork evidence, sibling rollback drills for reversal evidence, and phase 011 as the sole state-migration and authority
...
`BLOCK`; legacy authority and the phase-000 rollback anchor remain intact until phase 011 completes a governed flip.
<!-- /ANCHOR:rollback -->
