...
description: "Plan a total, fail-closed classification of every phase-000 in-flight state row into upcast, pin, fork, migrate, or block before any phase-011 authority cutover."
trigger_phrases:
...
_memory:
  continuity:
    ...
    next_safe_action: "Classify each frozen phase-000 census row before phase-011 cutover"
    blockers: []
    key_files: []
    completion_pct: 0
    open_questions: []
...
> Phase adjacency under `005-compatibility-shadow-and-rollback-bridge` (navigation order, not a hard runtime dependency): predecessor `003-shadow-parity-harness`; successor `005-rollback-drills`.
...
| Field | Value |
|-------|-------|
| **Packet** | system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/005-compatibility-shadow-and-rollback-bridge/004-inflight-state-classification |
| **Level** | 2 |
| **Priority** | P0 |
| **Status** | Planned |
| **Created** | 2026-07-15 |
| **Owner skill** | system-deep-loop |
| **Origin** | In-flight-state child of the phase-005 compatibility, shadow, and rollback bridge |
<!-- /ANCHOR:metadata -->
...
The [phase-000 census specification](../../000-baseline-taxonomy-and-state-census/spec.md) requires a row-addressable
...
The [phase-001 transition and rollback policy](../../001-architecture-coverage-and-transition-contract/003-transition-versioning-and-rollback-policy/spec.md)
...
phase 011. This phase therefore plans one exhaustive five-way disposition: `UPCAST`, `PIN`, `FORK`, `MIGRATE`, or
...
- A required baseline class for every persisted state family named by the phase-000 census specification.
- A row-level classification manifest that assigns exactly one class to every frozen census row and records the state
  ...
  verifier evidence.
- Class-specific handling contracts for phase 011: logical upcast, legacy pin-to-completion, isolated dark fork,
...
### Out of Scope
- Executing the phase-000 census, fabricating census row IDs, or claiming that planned state families are observed
  runtime evidence.
...
- Running rollback drills or issuing cutover/rollback certificates; sibling phase 005 and program phase 011 own those
  actions.
- Retiring legacy writers or archival readers; phase 012 owns retirement after the rollback window and zero-use proof.
<!-- /ANCHOR:scope -->
...
|----|-------------|---------------------|
| REQ-001 | Classification is total over the frozen phase-000 in-flight-state census | Every census row appears exactly once in the classification manifest; the manifest reports zero missing, duplicate, or unrecognized row IDs |
...
| REQ-009 | Family baselines cover every persisted shape named by phase 000 | Every concrete census row inherits one baseline below or records an evidence-backed override reviewed by the owning state contract and rollback verifier |
| REQ-010 | Classification remains fresh through phase-011 compare-and-swap | Phase 011 re-verifies the row digest, authority epoch, leases, pending effects, prerequisites, parity evidence, and rollback anchor immediately before migration or cutover; drift invalidates the row and returns it to `BLOCK` |
...
### Phase-000 state-family baseline map
...
- **SC-001**: The frozen phase-000 census closes to a one-row/one-class manifest with zero coverage gaps.
...
- **SC-004**: Phase 011 can deterministically select handling from the manifest without inferring policy at runtime.
- **SC-005**: Classification freshness is rechecked immediately before state handling and authority compare-and-swap.
- **SC-006**: Legacy authority remains unchanged throughout phase 005.
...
**Given** a quiescent checkpoint with complete identity, ordering, pending-work, and rollback evidence, **When** phase 011
imports it, **Then** the row follows `MIGRATE`, verifies ledger equivalence, and retains a usable legacy anchor.

**Given** any row changes after classification, **When** phase 011 rechecks its digest or authority epoch, **Then** the
...
[phase-000 census](../../000-baseline-taxonomy-and-state-census/spec.md), inherits deterministic upcasting and
rollback constraints from the [phase-001 policy](../../001-architecture-coverage-and-transition-contract/003-transition-versioning-and-rollback-policy/spec.md),
and supplies the state-disposition digest required by phase 011 under the
...
timeouts, class-specific verifier commands, and phase-011 receipt schemas from the executed census and registered
runtime contracts. Those choices may not weaken one-row/one-class totality, `BLOCK` as the default, non-authoritative
forking, source preservation, or the rule that phase 005 moves no authority.
<!-- /ANCHOR:questions -->
