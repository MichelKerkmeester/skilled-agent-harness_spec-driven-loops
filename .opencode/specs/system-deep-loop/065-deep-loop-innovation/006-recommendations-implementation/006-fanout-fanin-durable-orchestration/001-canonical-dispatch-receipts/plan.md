...
| **Execution** | Additive-dark behind phase-003/004 interfaces; legacy execution remains authoritative |

### Overview
Promote the phase-002 executor adapter's resolved launch contract and invocation fingerprint into a versioned `lineage_dispatch_resolved` event. Resolve and validate the leaf first, authorize and append its canonical receipt through the phase-003 ledger second, and cross the subprocess spawn boundary only after the append receipt proves durability. Resume folds verified receipts into dispatch state, treats an exact match as already resolved, rejects identity/fingerprint conflicts, and routes receipt-without-result to recovery rather than guessing. The implementation composes with the phase-004 receipt/effect service and uses `receipt-crypto.ts` only within its declared canonicalization and verifier-trust limits.
<!-- /ANCHOR:summary -->
...
### Definition of Ready
- [ ] The phase-002 adapter return contract and invocation-fingerprint inputs are pinned by tests
- [ ] The phase-003 envelope registry, authorization proof, ledger append, and append-receipt interfaces are available
- [ ] The phase-004 intent-before-effect and durable-verifier profiles are available or represented by hermetic interfaces
...
- [ ] Legacy phase-002 command/pool/retry/budget baselines are captured before integration
...
- [ ] Legacy phase-002 behavior remains unchanged in additive-dark mode
<!-- /ANCHOR:quality-gates -->

<!-- ANCHOR:architecture -->
## 3. ARCHITECTURE

- **Resolution boundary**: phase-002 config expansion, capability validation, manifest expansion, and per-kind adapter resolution produce the effective launch contract plus `invocationFingerprint`. No canonical receipt is built from unvalidated requested values.
...
- **Integrity profile**: serialize with the registered canonical JSON profile. Reuse `canonicalReceiptJson`, `deriveReceiptKey`, `signReceipt`, and `verifyReceipt` for the dispatch-receipt MAC profile; persist no run-master secret and label MAC evidence advisory unless the phase-004 durable verifier can re-derive it across restart.
- **Authorization + append barrier**: obtain the phase-003 transition authorization proof, append under the ledger's exclusive lock and expected-head check, wait for the durable sequence/hash append receipt, and only then invoke the pool worker/spawn callback.
...
- **Crash semantics**: receipt absence permits first dispatch; receipt plus result is resolved; receipt without result is unresolved. The last state enters phase-004 reconciliation and successor salvage/result logic, never automatic success or blind respawn.
...
### Phase 1: Setup
- Pin the phase-002 adapter/fingerprint fixtures and the phase-003/004 interface versions used by this child.
...
- Add fingerprint-version metadata and invariant checks that the stored value equals the phase-002 adapter return.
...
- Re-run phase-002 legacy command, manifest, pool, retry, budget, and persisted-artifact fixtures with dark receipts disabled and enabled.
<!-- /ANCHOR:phases -->
...
| REQ-003 | Adapter/receipt parity fixtures compare the stored fingerprint byte-for-byte and mutate every phase-002 fingerprint input |
...
| REQ-013 | Phase-002 baseline snapshots prove legacy command, pool, retry, budget, and artifact parity in dark mode |
...
The child has no sibling planning dependency (`depends_on: []`). Program implementation inherits phase 006's prerequisites from `../../../manifest/phase-tree.json`: phase 002 supplies adapter resolution and the invocation fingerprint; phase 003 supplies the canonical envelope, authorization gateway, immutable typed ledger, append receipt, and replay verification; phase 004 supplies semantic receipt integrity and intent-before-effect recovery; phase 005 supplies compatibility/shadow/rollback posture. The concrete sources are `../../../002-fanout-live-tools-unblock/spec.md`, `../../../003-transition-authorized-ledger-core/002-typed-append-only-ledger/spec.md`, `../../../004-shared-evidence-and-control-services/001-receipts-and-effect-recovery/spec.md`, and `.opencode/skills/system-deep-loop/runtime/lib/deep-loop/receipt-crypto.ts`.
<!-- /ANCHOR:dependencies -->

<!-- ANCHOR:rollback -->
## 7. ROLLBACK PLAN

The implementation is additive-dark. Rollback disables canonical receipt emission/consumption at the compatibility boundary and returns resume decisions to the unchanged legacy path; it never deletes, truncates, rewrites, or re-signs committed ledger events. Existing dark receipts remain immutable historical evidence and are ignored by the legacy-authoritative projection. If rollback follows partial rollout, verify phase-002 command/pool/retry/budget parity and retain receipt schema readers so historical events remain replayable. A production authority rollback is owned by phase 011 and cannot be improvised here.
<!-- /ANCHOR:rollback -->
