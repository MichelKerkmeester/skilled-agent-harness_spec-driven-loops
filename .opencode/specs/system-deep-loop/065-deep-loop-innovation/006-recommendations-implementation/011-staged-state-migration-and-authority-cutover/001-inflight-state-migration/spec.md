...
> Phase adjacency under `011-staged-state-migration-and-authority-cutover` (navigation order, not a hard runtime dependency): predecessor `none` (first sibling); successor `002-per-mode-authority-flip`.
...
| Field | Value |
|-------|-------|
| **Packet** | system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/011-staged-state-migration-and-authority-cutover/001-inflight-state-migration |
| **Level** | 2 |
| **Priority** | P0 |
| **Status** | Planned |
| **Created** | 2026-07-15 |
| **Owner skill** | system-deep-loop |
| **Origin** | First sibling of the phase-011 staged state migration and authority cutover parent |
<!-- /ANCHOR:metadata -->

<!-- ANCHOR:problem -->
## 2. PROBLEM & PURPOSE

The phase-005 classification identifies what may happen to live state, but it does not perform the operation. A loop
can be between iterations, holding a lease, waiting for fan-out results, paused at a checkpoint, carrying pending
effects, or writing a JSON/JSONL projection when phase-011 prepares a mode for authority movement. A migration that
...
This phase plans the guarded execution of the phase-005 `UPCAST`, `PIN`, `FORK`, `MIGRATE`, and `BLOCK` dispositions.
...
The guards come from the phase-004 locks and fencing contract: a canonical resource key, a durable monotonic fencing
...
### In Scope
- A row-level migration envelope that binds the phase-005 class, state identity, source digest, schema/version, authority epoch, resource key, fencing token, migration ID, rollback anchor, and operation status.
...
### Out of Scope
- Reclassifying census rows or inventing state evidence; phase-005 owns the five-way classification contract.
- Implementing the shared lease/fencing service; phase-004 owns the service, while this phase consumes its guarded mutation boundary.
- Defining event envelopes, ledger schemas, replay fingerprints, or transition vocabulary owned by phase-003 and phase-001.
...
- Retiring legacy writers or archival readers; phase-012 owns retirement after zero-use telemetry and rollback proof.
<!-- /ANCHOR:scope -->
...
| REQ-007 | Each protected write uses the phase-004 fence atomically | The coordinator cannot complete a state write, ledger import, checkpoint transition, or migration commit marker after lease loss or token takeover |
...
- **SC-001**: Every eligible phase-005 row has one deterministic operation outcome, receipt, and final integrity status.
...
This child declares `depends_on: []` in the phase-011 planning tree; adjacency is navigation, not a hard runtime
dependency. Execution consumes the phase-005 classification manifest and its row-level freshness rules, the phase-004
locks/fencing service, the phase-001 transition and rollback policy, the phase-011 manifest entry, and the frozen
phase-000 state census. The [phase-005 classification contract](../../005-compatibility-shadow-and-rollback-bridge/004-inflight-state-classification/spec.md)
is the authority for disposition semantics. The [phase-004 locks and fencing contract](../../004-shared-evidence-and-control-services/006-locks-and-fencing/spec.md)
...
phase-004 fence at every mutation boundary, and classify any uncertainty as `BLOCK`.
...
post-verification equivalence command against the frozen phase-000 census and the phase-004 atomicity domain. Those
