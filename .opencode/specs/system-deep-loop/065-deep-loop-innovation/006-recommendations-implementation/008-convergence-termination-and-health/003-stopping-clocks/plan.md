---
title: "Implementation Plan: stopping clocks (006 phase 008 child 003)"
...
parent: "system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/008-convergence-termination-and-health/003-stopping-clocks"
_memory:
  continuity:
    packet_pointer: "system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/008-convergence-termination-and-health/003-stopping-clocks"
...
| Aspect | Value |
|--------|-------|
| **Surface** | system-deep-loop runtime + phase-008 termination contracts |
...
comes from the phase-004 typed budget authority; novelty decay from phase-007 concept/evidence projections; coverage from
...
### Definition of Ready
- [ ] The phase-004 budget event/projection boundary exposes typed exhaustion, scope ancestry, balances, and reconciliation state
- [ ] Phase-007 novelty inputs distinguish concept novelty from evidence novelty at one committed watermark
- [ ] Sibling 001 exposes a replay-stable coverage certificate and sibling 002 exposes confirmed/cleared cycle events
- [ ] The mode inventory and authoritative evaluation boundaries are frozen from the phase-000 baseline and phase-009 contract inputs
- [ ] Clock/profile schemas, termination classes, event namespace, and replay fingerprint policy are frozen against the phase-003 ledger contract
...
- **Termination writer** appends one idempotent `LoopTerminationDeclared` event through the phase-003 transition gateway. The event stores primary/co-causes, class, traces, source IDs, watermarks, fingerprint, remaining coverage/blockers, and in-flight settlement references.
...
- Build fixture adapters for budget, phase-007 novelty, sibling-001 coverage, monotonic wall time, and sibling-002 cycles before wiring runtime authority.
...
This child declares `depends_on: []`; sibling adjacency is navigational. Implementation consumes the phase-003 authorized
ledger and replay fingerprint, the phase-004 hierarchical typed-budget contract, phase-006 dispatch/receipt boundaries,
phase-007 semantic-community and next-focus novelty projections, sibling 001 coverage certificates, and sibling 002 cycle
