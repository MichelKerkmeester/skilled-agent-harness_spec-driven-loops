---
title: "Feature Specification: Legacy Writer Retirement (006 phase 012)"
description: "Remove the old live emitters and logic replaced by the evidence-ledger spine only after every mode has a clean phase-011 cutover certificate, a closed rollback window, and zero-use telemetry. Retain archival readers required for historical legacy packets."
trigger_phrases:
...
_memory:
  continuity:
    packet_pointer: "system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/012-legacy-writer-retirement"
...
> Phase adjacency under the 006 parent (navigation order, not a hard runtime dependency): predecessor `011-staged-state-migration-and-authority-cutover`; successor `013-whole-system-gate`.
...
| Field | Value |
|-------|-------|
| **Packet** | system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/012-legacy-writer-retirement |
| **Level** | 2 |
| **Priority** | P0 |
| **Status** | Planned |
| **Created** | 2026-07-15 |
| **Owner skill** | system-deep-loop |
| **Origin** | Phase 012 leaf in `manifest/phase-tree.json`; the last and only legacy-writer deletion phase |
<!-- /ANCHOR:metadata -->
...
phase 012 the one-time removal of old live emitters and replaced logic after authority has moved, while explicitly
requiring historical completed packets to remain readable (`.opencode/specs/system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/spec.md`).
The phase tree makes this leaf depend on `011-staged-state-migration-and-authority-cutover` and hands its result to
`013-whole-system-gate` (`.opencode/specs/system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/manifest/phase-tree.json`).

Deletion is unsafe if a legacy writer is merely quiet, if a dynamically constructed path was missed, or if a historical
reader is mistaken for a live authority path. The phase-000 census is therefore the starting inventory: every event
schema, reader, writer, persisted state surface, backend, repair path, and historical-read obligation must be reconciled
against the candidate before a deletion manifest is frozen (`../000-baseline-taxonomy-and-state-census/spec.md`).
...
Phase 011 supplies the hard authority boundary. Its per-mode flip contract keeps legacy available during the reversible
state, requires the eight workstreams to cut over in manifest order, and denies stale legacy writes by epoch
(`../011-staged-state-migration-and-authority-cutover/002-per-mode-authority-flip/spec.md`). Its certificate and window
contract requires the later of 14 calendar days and five successful authoritative executions, retains rollback assets,
and records clean closure before phase 012 evaluates retirement
(`../011-staged-state-migration-and-authority-cutover/003-cutover-certificate-and-rollback-window/spec.md`). This
...
### In Scope
- Reconcile the phase-000 reader/writer and state census with the exact candidate code, including dynamic emitters,
...
- Consume every mode's phase-011 cutover certificate, clean rollback-window closure evidence, final authority epoch,
  and retained rollback assets before deleting that mode's old live emitters or replaced logic.
- Retire in the phase-011 manifest order: `001-deep-research`, `002-deep-review`, `003-deep-ai-council`,
...
- Retain the phase-011 cutover, rollback, telemetry, and zero-use evidence as immutable handoff material for phase 013;
...
- Closing a rollback window early, deleting rollback evidence, or replacing phase-011's later-of-14-days-and-five-runs
...
- Adding new runtime capabilities, changing mode order, or retiring the `ai-system-improvement` mode excluded by the
  phase-tree manifest.
- Running the final whole-system gate; phase 013 consumes this phase's evidence and verifies the frozen post-retirement
  behavior.
<!-- /ANCHOR:scope -->
...
|----|-------------|---------------------|
| REQ-001 | The retirement inventory is closed against phase 000 | Every censused legacy producer, consumer, state surface, backend, dynamic path, and historical-read obligation is classified as delete, retain, or blocked; no unknown row remains. |
| REQ-002 | Every mode has completed phase-011 authority evidence | The eight manifest workstreams each have a matching cutover certificate, final authority epoch, clean rollback-window closure, retained rollback assets, and no unresolved revert signal. |
...
| REQ-005 | Retirement follows the required order | Modes retire in the phase-011 manifest order; each mode's legacy writers are removed before its replaced helpers, and shared legacy emitters are removed only after all mode rows pass. |
...
| REQ-010 | Phase 013 receives a complete handoff | The handoff binds the deletion diff, retention manifest, zero-use report, per-mode closure evidence, historical-read results, rollback evidence, candidate SHA, and verification commands. |
<!-- /ANCHOR:requirements -->
...
- **SC-002**: All eight modes have clean phase-011 closure evidence and retire in the declared order, with mode writers
...
- **SC-005**: Rollback evidence and a pre-delete restoration anchor remain retained through the deletion decision, and
  phase 013 receives an auditable handoff bound to the exact candidate SHA.
<!-- /ANCHOR:success-criteria -->
...
- **False zero from incomplete telemetry** - Dynamic path construction, retries, subprocesses, replay workers, or shared
  backends may bypass counters. Mitigation: reconcile the phase-000 census, static callsite inventory, runtime probes,
...
- **Archival reader orphaning** - A broad legacy sweep may delete a decoder or projection used only by old completed
  packets. Mitigation: classify reader events separately, retain every historical-read obligation from phase 000, and
...
- **Rollback evidence loss** - Removing adapters or evidence stores before the final gate could make a failed deletion
  unrecoverable. Mitigation: require closed phase-011 windows, retained anchors, a pre-delete candidate, and immutable
...
- **Dependencies**: the controlling sources are the 006 parent spec, `manifest/phase-tree.json`, phase-000 baseline and
  census, phase-011 authority-flip and cutover-window contracts, the retained phase-004 receipt semantics, and the phase
...
zero live-use evidence, phase-011 window closure, mode order, or archival-reader retention.
<!-- /ANCHOR:questions -->
