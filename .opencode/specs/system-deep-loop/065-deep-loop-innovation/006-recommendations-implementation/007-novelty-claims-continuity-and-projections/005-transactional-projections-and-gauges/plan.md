...
parent: "system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/007-novelty-claims-continuity-and-projections/005-transactional-projections-and-gauges"
_memory:
  continuity:
    packet_pointer: "system-deep-loop/065-deep-loop-innovation/006-recommendations-implementation/007-novelty-claims-continuity-and-projections/005-transactional-projections-and-gauges"
...
| **Execution** | Additive-dark implementation over the verified phase-003 ledger |

### Overview
Implement a versioned projection-bundle registry and a fenced coordinator that consumes verified ledger records in
sequence order. For each event, compute all affected phase-007 views and phase-004 gauges, validate their canonical
...
### Definition of Ready
- [ ] The phase-003 typed reader exposes verified ledger identity, sequence, record hash, canonical event bytes, and typed decode.
- [ ] Phase-007 siblings define the semantic-community, contradiction/supersession, claim-continuity, and next-focus events this bundle projects.
- [ ] The phase-004 gauge registry exposes versioned pure reducers, exact arithmetic, canonical serialization, and checkpoint provenance.
...
- **Verified input cursor**: the coordinator accepts records only from the phase-003 verified reader and binds progress to ledger ID, sequence, record hash, bundle version, and digests.
...
- **Atomic apply transaction**: begin; fence and compare expected watermark; validate the idempotency tuple; write all affected views and phase-004 gauge accumulators; insert the apply receipt; advance the watermark; commit. Any failure rolls back the unit.
...
### Phase 1: Setup
- Pin the implementation baseline and confirm the ledger reader, phase-004 gauge registry, phase-007 event contracts, legacy observability surfaces, and canonical storage capabilities.
...
- Compose the existing phase-004 gauge reducers into the same transaction as related phase-007 projections.
...
- Compare dark outputs with existing observability and phase-004 gauge surfaces; record mismatches as evidence only.
<!-- /ANCHOR:phases -->
...
| REQ-010 | Phase-004 gauge golden fixtures remain byte-identical when applied inside the projection transaction |
...
`../../003-transition-authorized-ledger-core/002-typed-append-only-ledger/spec.md`; deterministic gauge definitions
from `../../004-shared-evidence-and-control-services/005-stream-fold-gauges/spec.md`; phase-007 sibling events; and the
