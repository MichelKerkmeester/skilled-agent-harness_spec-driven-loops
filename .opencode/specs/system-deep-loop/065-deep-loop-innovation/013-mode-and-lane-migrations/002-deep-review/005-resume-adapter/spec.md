---
title: "Feature Specification: Deep Review resume adapter (013 phase 002/005)"
...
| Field | Value |
|-------|-------|
| **Packet** | system-deep-loop/065-deep-loop-innovation/013-mode-and-lane-migrations/002-deep-review/005-resume-adapter |
| **Level** | 2 |
| **Priority** | P1 |
| **Status** | Planned |
| **Created** | 2026-07-15 |
...
| **Planning authority** | Phase 009 shared review-loop contract, phase 012 mode contracts and write-set conflict graph |
<!-- /ANCHOR:metadata -->
...
The adapter consumes the review-loop backbone frozen in phase 009 and the shared mode contract emitted by phase 012. It does not fork the loop for Deep Review, move authority, or solve the other six sibling concerns.
<!-- /ANCHOR:problem -->
...
- Compatibility with the shared review-loop contract from phase 009 and the phase-012 dependency/write-set conflict graph; this phase consumes those contracts rather than defining a Deep Review fork.
...
| REQ-008 | The adapter shares the frozen loop backbone | The design references the phase-009 review-loop contract and phase-012 write-set ownership, with no mode-local lifecycle or conflicting event application path |
<!-- /ANCHOR:requirements -->
...
- **SC-005**: The adapter preserves the shared phase-009 loop semantics and the phase-012 write-set boundaries without introducing a Deep Review-only backbone.
...
- **Backbone fork** — a mode-local resume state machine could diverge from shared loop semantics. Mitigation: phase 009 is the lifecycle authority and phase 012 owns interface and write-set boundaries.
...
Dependencies are contract-level rather than sibling runtime dependencies: phase 009 must freeze the shared review-loop contract, phase 012 must publish mode interfaces and the conflict graph, and the Deep Review sibling concerns must expose the typed events, reducers, and certificates consumed by this adapter. The required adjacency to `004-certificates-and-receipts` and `006-shadow-parity` is navigation and ordering only.
<!-- /ANCHOR:risks -->

<!-- ANCHOR:questions -->
## 7. OPEN QUESTIONS

- Which exact sealed-frontier marker and chain-integrity receipt does phase 009 expose to mode adapters?
