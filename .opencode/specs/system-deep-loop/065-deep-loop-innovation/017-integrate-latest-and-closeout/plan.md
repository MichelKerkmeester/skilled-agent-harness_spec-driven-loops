---
title: "Implementation Plan: Integrate Latest & Closeout (065 phase 014)"
...
trigger_phrases:
  ...
  - "deep-loop phase 014 implementation plan"
...
### Overview
Phase 014 is the final landing loop for the 15-phase program. It takes the last green phase-013 candidate, integrates the latest origin, compares the resulting tree with the program's phase contracts and pinned baseline, and routes relevant drift back to the owning phase before any closeout claim. The final verification is the complete phase-013 gate on the final SHA, followed by append-only open-item, changelog, parent-rollup, and generated-metadata reconciliation.
<!-- /ANCHOR:summary -->
...
- [ ] The full phase-013 gate is green on the final SHA
...
- **Reopen router**: Reopen the owning phase first, then any downstream phase whose declared input is changed. Preserve prior receipts as historical evidence and require fresh phase evidence before phase 013 is accepted.
- **Final gate boundary**: Run the entire phase-013 contract against the final SHA, not a branch-relative range. The gate covers behavior and mode baselines, mixed-version replay, crash injection, counterfactual and degeneration tests, parity against 000, the blocking SOL review, and recursive strict validation.
...
### Phase 1: Setup
- Confirm phase-013's pre-integration receipt, the pinned 000 baseline, current parent status, and a clean isolated worktree.
...
- Run the complete phase-013 gate on the final SHA, including all mode gates, replay/crash/counterfactual/degeneration coverage, parity, SOL receipt, and recursive `validate.sh --strict`.
...
| REQ-004 | Phase-013 whole-system gate runs on the final SHA: exact-SHA behavior/mode baselines, mixed-version replay, crash injection, counterfactual and degeneration, parity against 000, SOL review, and recursive strict validation |
...
| `016-whole-system-gate` artifacts | Internal gate | Yellow | Final-SHA verification cannot be replayed until the phase-013 contract and receipts are available |
