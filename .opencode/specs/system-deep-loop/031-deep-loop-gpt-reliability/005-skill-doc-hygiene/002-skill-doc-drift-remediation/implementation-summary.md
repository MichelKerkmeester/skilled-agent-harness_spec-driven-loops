...
| Field | Value |
|-------|-------|
| **Spec Folder** | 002-skill-doc-drift-remediation |
...
Phase `016` (`016-mk-deep-loop-guard-hardening`, research complete as of the follow-up below): harden `.opencode/plugins/mk-deep-loop-guard.js` with orchestrate-specific loop-detection logic, mechanically blocking repeated/loop-like `orchestrate` → command-owned-loop-executor dispatches while allowing exactly one bounded hand-off — directly addressing phase 012's measured GPT-5.5 enforcement inconsistency (refused one direct `deep-research` dispatch citing the rule, allowed an identical direct `deep-review` dispatch in a separate run). This is new engineering (session-scoped dispatch-history tracking), not a doc fix, and needs its own implementation plan before coding.
<!-- /ANCHOR:followup -->

## Follow-Up: Post-Implementation Adversarial Review (2026-07-01)

Ran a 10-iteration dual-model deep-review (`cli-opencode openai/gpt-5.5-fast` reasoning=high + `cli-opencode zai-coding-plan/glm-5.2` reasoning=max, 5 iterations each) against this phase's own completed work. Both lineages reached synthesis (verdict CONDITIONAL, P0=0 both). Runner flagged both lineages `rejected` on cosmetic stop-reason/event-name string mismatches (same class of issue as phase 014's review lineage) — content was real and used as-is.

**Confirmed real findings, fixed:**
1. **GLM F001** — `cli-opencode/manual_testing_playbook/manual_testing_playbook.md:362` (the AGENT ROUTING category intro, outside the CO-017-specific range Cluster 1 originally patched) still called `ai-council` one of "4 repo-defined primaries." A genuine residual Cluster-1-class instance missed during the original sweep. Fixed: now correctly describes 3 primaries + `ai-council` as a subagent reachable via `orchestrate` or `/deep:ai-council`.
2. **GPT F002** — This phase's own `spec.md`/`plan.md` frontmatter and `**Status**` field still said "In Progress"/"Pending" for Cluster 6 and overall completion, despite `tasks.md`/`checklist.md`/`implementation-summary.md` all correctly showing complete. Fixed: `spec.md`/`plan.md` status fields, frontmatter continuity blocks, and all "Definition of Done"/phase checkboxes updated to reflect the actual completed state.

**Rejected on independent verification:**
- **GLM F002** ("CHK-010 cites the wrong interpreter, `python3` for a `.sh` file") — independently verified FALSE: `check-comment-hygiene.sh`'s actual shebang is `#!/usr/bin/env python3`; `python3` is the correct invocation (confirmed earlier this session via direct testing). GLM's one valid sub-point (the checklist entry didn't give a full path) was addressed anyway.
- **GPT F001** was a mixed finding: 2 of its 5 cited lines (`agent_delegation.md:197`, `:229`) were miscited as stale when independent re-read showed they already carry correct "Command-only/forbidden" framing. The other 3 citations (`:222`, `:225`, `:231`) do show a genuine internal inconsistency in `agent_delegation.md`'s own "AGENT ROUTING MATRIX" table (direct `--agent context/review/general` invocation patterns, contradicting stated rules elsewhere in the same file) — but this concerns agents unrelated to `ai-council`/the `mode: subagent` conversion, and appears to be a pre-existing documentation inconsistency predating packet 031, not something this packet's changes caused. Flagged for awareness; not fixed here (out of this phase's scope).

**Verification after fixes:** scoped re-grep confirms zero remaining "ai-council...primary" framing in `cli-opencode/`; zero remaining "In Progress"/"Pending" in phase 015's forward-facing status fields.
