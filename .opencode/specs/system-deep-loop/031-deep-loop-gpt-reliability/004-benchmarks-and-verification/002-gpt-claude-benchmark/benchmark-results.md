...
- **`/deep:context` and `/deep:ai-council`'s own command-level Phase-0 behavior** (as opposed to direct Task-dispatch, which was tested) was not separately re-verified in this run -- `/deep:research`'s command-level Phase-0 pass is treated as representative evidence for the shared Phase-0 mechanism (phase 008 applied the identical fix to all 7 surviving command files), not as proof for context/ai-council's command surfaces specifically.
