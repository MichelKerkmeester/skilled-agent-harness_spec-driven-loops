...
## Findings
- **mode-registry.json lists 4 runtime-loop modes (context/research/review/ai-council) and 3 improvement-family modes (agent-improvement, model-benchmark, skill-benchmark). Those improvement modes use backendKind=improvement-host, not the runtime-loop router.** [SOURCE: mode-registry.json:18-146]
...
- **Missed-seam check: the improvement commands (/deep:agent-improvement, /deep:model-benchmark, /deep:skill-benchmark) need a Resolved-route header at their own seams if GPT also mis-resolves them — but the operator symptoms (research-prompt.md:21) name only the 4 runtime modes, so improvement-family hardening is lower priority and can follow the same pattern later.** [SOURCE: mode-registry.json:81-145; research-prompt.md:21]
