...
| Skills — runtime | 8 | `artifact-root.cjs`, `fallback-router.ts`, `fanout-pool.cjs`, `cli-guards.cjs`, convergence-floor test, lane-d packaging test, runtime package/tsconfig. |
| Skills — workflows | 7 | `mode-registry.json`, reverse-coupling reducers/shims, `replay-graph-from-artifacts.cjs`, council test fixtures, deep-improvement named profile. |
...
| `deep-loop-runtime/tests/unit/meta-loop-lane-d-packaging.vitest.ts` | Asserts on named-profile filename |
...
| `deep-loop-workflows/deep-improvement/assets/non_dev_ai_system/profiles/deep-loop-runtime.json` | Ambiguous rename target |
