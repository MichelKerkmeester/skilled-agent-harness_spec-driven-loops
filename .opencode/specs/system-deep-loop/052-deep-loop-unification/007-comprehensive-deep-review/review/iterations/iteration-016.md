...
- Claim: The live model-benchmark feature catalog entry says `VALID_MODES` holds only `agent-improvement` and `model-benchmark`, but current `loop-host.cjs` exposes three valid modes and the adjacent Lane C catalog entry documents the newer mode.
- EvidenceRefs: `.opencode/skills/system-deep-loop/deep-improvement/feature_catalog/model-benchmark-mode/mode-switch.md:29`, `.opencode/skills/system-deep-loop/deep-improvement/scripts/shared/loop-host.cjs:45`, `.opencode/skills/system-deep-loop/deep-improvement/feature_catalog/skill-benchmark/mode-wiring.md:25`
- CounterevidenceSought: Checked the Lane C feature-catalog entry plus the three improvement-lane command markdown files; those surfaces describe the current Lane C route, so the issue appears isolated to this older Lane B feature entry rather than a command-level routing contradiction.
...
- Command/agent cross-reference: PASS with carried-forward context. The three improvement-lane commands reference current deep-improvement layout and ownership surfaces: Lane A/B owned YAML assets in `agent-improvement.md` and `model-benchmark.md`, and Lane C reads `deep-improvement/SKILL.md` plus `references/skill_benchmark/operator_guide.md`. DR-014-P1-001 remains a runtime wiring bug for shipped sweep profiles, but this traceability pass did not find a new command-file layout reference issue.
- `feature_catalog/` currency: PARTIAL. The Lane C sampled entry matches `loop-host.cjs`; the Lane B `mode-switch.md` entry has the P2 drift above.
...
Iteration 17 should continue the planned rotation with `deep-improvement` maintainability. Focus on whether Lane A/B/C maintainer instructions make cross-lane changes safe without re-counting DR-014-P1-001, DR-015-P1-001, or DR-016-P2-001 unless new evidence broadens them.
