...
_memory:
  continuity:
    ...
    recent_action: "Implemented and verified the additive-dark replay-fingerprint runtime"
...
| `fingerprint-version-registry.ts` | Independent positive version registry, current writer selection, and retained historical resolution |
| `replay-component-registry.ts` | Registered reducer, projection schema, and ledger-addressed replay-input contract |
| `derive-replay-fingerprint.ts` | Streaming derivation from the authorized ledger's verified ascending reader |
| `replay-fingerprint-attestation.ts` | Typed after-range attestation preparation, lookup, idempotency, and conflict refusal |
| `verify-replay-fingerprint.ts` | One fail-closed API shared by shadow parity and whole-system replay consumers |
...
| Missing or changed replay input/artifact digest | `replay_input` | Derivation or verification fails before trust |
...
Every failure result carries ledger ID, range, fingerprint version, responsible component, bounded expected/actual values, and the earliest deterministic sequence, hop, or stage available. The failure variant cannot contain a verified projection, parity result, cutover result, or whole-system gate result.
...
| Check | Receipt |
|---|---|
| Fingerprint unit suite | Runtime Vitest: `1` file, `24` tests passed, exit `0` |
| Envelope + ledger + fingerprint matrix | Runtime Vitest: `3` files, `100` tests passed, exit `0` |
| TypeScript | Runtime `tsconfig.json`, `tsc --noEmit`, exit `0` |
| Alignment drift | `8` runtime modules scanned, `0` findings, exit `0` |
| Comment hygiene | All `9` new TypeScript files passed, exit `0` |
| Falsifier | Disabling the projection-component comparison made the projection-drift fixture fail; restoring it returned exit `0` |
| Strict packet validation | `Errors: 0`, `Warnings: 0`, exit `0` |

Exact primary commands:

```text
.opencode/skills/system-spec-kit/mcp-server/node_modules/.bin/vitest run --config .opencode/skills/system-deep-loop/runtime/vitest.config.ts .opencode/skills/system-deep-loop/runtime/tests/unit/event-envelope.vitest.ts .opencode/skills/system-deep-loop/runtime/tests/unit/authorized-ledger.vitest.ts .opencode/skills/system-deep-loop/runtime/tests/unit/replay-fingerprint.vitest.ts
...
This leaf supplies derivation, attestation storage, and verification only. It does not modify the envelope, ledger, authorization gateway, or any shipped writer, and it does not move runtime authority. The current evidence covers the supported OpenCode/Node runtime; platform neutrality follows from the explicit byte encoding and forbidden-input checks rather than a second operating-system run in this worktree. Phase 008 and phase 016 can consume the shared API, but their policy and orchestration remain outside this leaf.
