...
_memory:
  continuity:
    ...
    next_safe_action: "Resolve canonicalization and dependency closure with the phase-007 sealing contract"
    blockers: []
    key_files: []
    completion_pct: 0
    open_questions: []
...
This phase makes the shared evaluator-first loop reproducible at its evidence boundaries. It does not create a local hash or storage protocol. It adapts the phase-007 sealing primitives into one artifact contract used by evaluator, canary, and promotion services: canonical bytes are dependency-closed and sealed before publication; reads verify the seal before returning data; and later reduction or policy changes create new derived references rather than mutating old evidence. The common service owns the contract for the three downstream variants, while `004-certificates-and-receipts` binds accepted service outcomes into certificates and receipts.
<!-- /ANCHOR:summary -->
...
- [ ] The phase-007 sealing primitive publishes its canonicalization, digest, seal-on-write, publication, and verification contract
...
- [ ] Every common artifact is content-addressed, sealed on write, immutable after publication, and verified on read through the phase-007 primitive
...
- **Single sealing adapter**: expose one common API over the phase-007 primitive for canonicalize, digest, seal, publish, inspect, and verify. The adapter owns no alternate hash, signature, chain, or storage semantics.
...
- Read the phase-007 sealing primitive contract and record its canonicalization, digest, dependency, publication, verification, and failure semantics; reject any design that duplicates them.
...
|-------------|--------------|
| REQ-001 | Contract tests prove every artifact kind routes through the phase-007 sealing adapter and reject alternate seal metadata or verification paths |
