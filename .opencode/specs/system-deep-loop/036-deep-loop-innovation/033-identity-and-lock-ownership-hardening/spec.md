...
_memory:
  continuity:
    ...
    completion_pct: 0
    open_questions:
      - "Exact root cause of the 451-test per-mode invalid_input regression — NOT isolated (see handover.md)"
    answered_questions:
      ...
      - "Fresh loop-lock records are published through a complete temporary inode and atomic hard-link claim."
---
# Feature Specification: Identity and Lock Ownership Hardening

> **⚠️ STATUS: DESIGN RETAINED, CODE REVERTED.** This spec captures four confirmed real
> gaps (F001-F004) surfaced by the 024 deep review and the intended remediation. The
> implementation was **attempted and rolled back** (451-test per-mode regression, root cause
> not isolated). This document is the plan of record for a re-attempt — see `handover.md`
> for the postmortem, hard lesson, and mandatory re-attempt conditions.
...
**Key Decisions**: reject closure-only policy state; use token-checked filesystem ownership; publish fresh loop-lock records atomically.
...
| **Status** | Blocked |
| **Created** | 2026-08-05 |
| **Branch** | `0129-system-deep-loop-036-remediation-execution` |
<!-- /ANCHOR:metadata -->
...
- Atomic fresh loop-lock ownership publication, plus deterministic three-process append-lock race coverage.
...
| `runtime/lib/deep-loop/loop-lock.ts` | Modify | Atomically publish a complete fresh owner record. |
...
| REQ-005 | Fresh loop-lock ownership must never expose a partial record as reclaimable. | A complete record is atomically claimed, the existing two-process falsifier has exactly one winner, and stale/release races remain green. |
...
- **SC-002**: F005 is either hardened or rejected with direct evidence; this packet records the hardened disposition.
...
| Risk | Atomic hard-link publication depends on same-filesystem paths. | A cross-device boundary would make the claim fail. | The lock and target are created in the same parent directory; treat claim failure as acquisition failure. |
<!-- /ANCHOR:risks -->
...
- Fresh loop-lock claim collision: return not acquired and parse the complete winner record.
<!-- /ANCHOR:edge-cases -->
...
| R-004 | A fresh loop-lock reader parses partial JSON. | High | Medium | Write and fsync a complete temporary record, then hard-link it into the claim path. |
...
**Acceptance Criteria**:
1. Given two fresh acquisitions released by a common barrier, When both claim the path, Then one and only one reports acquired and the winner record parses completely.
...
- None. F005 is dispositioned as hardened in ADR-001 based on direct code inspection and the deterministic two-process falsifier; F004 additionally has a deterministic three-process restore race.
