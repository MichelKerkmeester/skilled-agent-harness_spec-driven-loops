...
_memory:
  continuity:
    ...
    recent_action: "Recorded F001 identity-mode predicate and F004 compare-and-swap restore evidence"
    ...
    answered_questions:
      ...
      - "Append-lock restore uses a non-overwriting hard-link claim so two live racers cannot be erased by a reclaimer."
---
...
- [x] F005 has a documented hardened disposition and a passing deterministic two-process falsifier; F004 has a passing deterministic three-process restore race.
...
- **Loop lock**: Writes a complete owner record to a temporary inode and hard-links it into the exclusive claim path.
...
| Fresh loop-lock acquisition | Creates the first owner record | Publish complete temporary record through atomic hard-link claim | F005 falsifier plus loop-lock and CLI suites |
...
- Algorithm invariant: never delete or overwrite an owner whose token was not atomically claimed and revalidated; never expose a partial fresh owner record; never append a second canonical iteration line.
<!-- /ANCHOR:affected-surfaces -->
...
- [x] Atomically hard-link complete fresh loop-lock records.
...
| Node filesystem `open`, `link`, `rename`, `rm`, and `fsync` | Platform primitive | Green on current host | Ownership claims would need a platform-specific fallback. |
...
| Loop lock | Complete temporary record and hard-link claim | Fresh owner record | Loop process admission |
