...
### 1. Community novelty should be a vector

The current runtime already has useful seams: `CONTRADICTS`/`SUPERSEDES` relations in [coverage-graph-db.ts](/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:47), graph novelty in [coverage-graph-signals.ts](/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-signals.ts:716), and self-report corroboration in [convergence.cjs](/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/runtime/scripts/convergence.cjs:533). What is missing is stable semantic-community identity and temporal claim state.

Microsoft GraphRAG delegates hierarchical clustering to graspologic. The implementation accepts `starting_communities`, resolution, randomness and a seed, and returns a transition log containing node, cluster, parent, level and final-cluster state—not durable community identities. [GraphRAG operation](https://github.com/microsoft/graphrag/blob/main/graphrag/index/operations/cluster_graph.py), [graspologic implementation](https://github.com/graspologic-org/graspologic/blob/main/graspologic/partition/leiden.py). Leiden guarantees connected communities, but graph changes can still reorganize partitions; fixed seed only removes one source of instability. [Leiden paper](https://arxiv.org/abs/1810.08473)

Use two graph layers:

- A non-negative topical graph—semantic similarity, shared question, `SUPPORTS`, `CITES`—for Leiden.
- An epistemic overlay—`CONTRADICTS`, `SUPERSEDES`, validity and resolution events—excluded from Leiden weights. Treating contradiction as a positive clustering edge would incorrectly equate topical proximity with agreement.

After reclustering, preserve community identities through maximum-weight bipartite matching over weighted Jaccard overlap of canonical claim IDs. Emit explicit `community_born`, `community_split`, `community_merged` and `community_retired` events. Snapshot fingerprints should include graph hash, embedding/canonicalizer versions, Leiden version, seed, resolution and maximum cluster size.

The gauge should retain separate components:

`settledNovelty = coverageGain + communityBirthGain + resolutionGain - duplicatePenalty`

`pressure(c) = priority(c) × (1-coverage(c)) × (1+openCriticalContradictions(c)) × (1+boundaryUncertainty(c)) / ((1+visits(c)) × expectedCost(c))`

Normalize configured weights before collapsing `settledNovelty` into compatibility `newInfoRatio`. Select the next divergent focus by maximum pressure with a cooldown. Contradictions raise broadening pressure but do not falsely count as settled coverage.

### 2. Contradictions need bitemporal event semantics

Graphiti gives each fact a validity window and invalidates superseded facts instead of deleting them. XTDB sharpens this into two clocks: valid time—when the claim applies—and system time—when the runtime learned or revised it. [Graphiti](https://github.com/getzep/graphiti), [edge model](https://github.com/getzep/graphiti/blob/main/graphiti_core/edges.py), [XTDB bitemporality](https://docs.xtdb.com/intro/what-is-xtdb.html)

Recommended JSONL records:

- `claim_asserted.v1`: `claimVersionId`, `propositionId`, normalized proposition and scope, `validFrom/validTo`, `recordedAt`, source receipts, confidence.
- `contradiction_detected.v1`: both version IDs, contradiction kind (`logical|temporal|scope|source`), detector version, score and evidence receipts.
- `contradiction_resolved.v1`: outcome (`supersede|coexist|retract|unknown`), surviving versions, rationale receipts and policy version.
- `claim_validity_closed.v1`: old version, effective invalidation time and causative resolution event.

Nothing rewrites `claim_asserted`. The projector derives “current truth” at `(JSONL sequence, valid-time horizon)`. Continuity should persist `lastAppliedSeq`, active claim-version IDs, open contradiction IDs, community snapshot ID and projection fingerprint. A fingerprint mismatch forces replay instead of silently reusing stale continuity.

Mem0’s current pipeline reinforces the append-first half: it retrieves nearby memories, performs additive extraction, embeds candidate facts, applies exact-hash deduplication and records history. Exact hashes cannot resolve paraphrases or contradictions, so semantic resolution must remain a later explicit event rather than being hidden inside deduplication. [Mem0 `main.py`](https://github.com/mem0ai/mem0/blob/main/mem0/memory/main.py)

### 3. Recompute conditionally

Full hierarchical Leiden on every iteration would be wasteful and causes avoidable identity churn. Assign new claims provisionally to the strongest existing community, then recluster when edge/node drift, unmatched mass or contradiction-driven boundary change crosses a configured threshold. Persist the resulting snapshot so replay never reruns a nondeterministic clustering job. HIT-Leiden’s affected-region maintenance is the stronger future direction once graph scale warrants it. [HIT-Leiden paper](https://arxiv.org/abs/2601.08554)
...
```json
{
  "new_repos": [
    {
      ...
      "stars": "~1k",
      "what": "Graph-statistics library containing GraphRAG's hierarchical Leiden implementation; latest verified release v3.4.4 on 2025-09-08.",
      "lesson": "Persist the returned hierarchy as a versioned snapshot, warm-start with prior communities, fix the seed, and add stable-ID matching because native cluster integers are not continuity identifiers.",
      "maps_to": [
    ...
    },
    {
      ...
      "stars": "~3k",
      "what": "Immutable bitemporal SQL database with automatic valid-time and system-time history; latest verified release v2.1.0 on 2025-12-01.",
      "lesson": "Model a claim's domain validity separately from the JSONL sequence at which the runtime learned or resolved it, enabling reproducible as-of continuity projections.",
      "maps_to": [
    ...
    },
    {
      ...
      "stars": "~3.4k",
      "what": "Version-controlled document graph using immutable delta layers and deletion masks; latest verified server release v12.0.6 on 2026-06-24.",
      "lesson": "Resolution should append a delta or masking event over prior assertions, preserving provenance and permitting time-travel, diff and rollback without mutating evidence.",
      "maps_to": [
        "runtime/state-jsonl-checkpointing",
        "runtime/continuity-threading",
        "deep-alignment"
  ...
  "insights": [
    {
      "insight": "Use a two-layer graph: Leiden clusters unsigned topical affinity, while CONTRADICTS, SUPERSEDES and validity events form a separate epistemic overlay. Feeding contradiction edges into ordinary Leiden as positive weights would cluster opposition as agreement.",
      "evidence": "https://github.com/graspologic-org/graspologic/blob/main/graspologic/partition/leiden.py ; https://arxiv.org/abs/1810.08473",
      "maps_to": [
        "runtime/dedup-novelty",
        "runtime/convergence",
        "deep-research"
    ...
    {
      "insight": "Community IDs require a lifecycle independent of Leiden's integer labels: match adjacent snapshots by maximum-weight Jaccard overlap and append born, retained, split, merged and retired events with a clustering-input fingerprint.",
      "evidence": "https://github.com/graspologic-org/graspologic/blob/main/graspologic/partition/leiden.py ; https://github.com/microsoft/graphrag/blob/main/graphrag/index/operations/cluster_graph.py",
      "maps_to": [
        "runtime/dedup-novelty",
        "runtime/state-jsonl-checkpointing",
        "runtime/gauges-observability"
    ...
    {
      "insight": "Separate settledNovelty from divergencePressure. Coverage and resolved uncertainty advance the former; under-covered communities, boundary uncertainty and unresolved critical contradictions drive the latter. This prevents a loop from manufacturing novelty by accumulating unresolved conflict.",
      "evidence": "https://github.com/getzep/graphiti ; https://docs.xtdb.com/intro/what-is-xtdb.html",
      "maps_to": [
        "runtime/dedup-novelty",
        "runtime/convergence",
    ...
    {
      "insight": "Append assertions before resolving them, but make resolution an explicit bitemporal event. Exact-hash or embedding dedup is evidence consolidation, not truth maintenance, and cannot safely decide whether two scoped claims coexist or one supersedes another.",
      "evidence": "https://github.com/mem0ai/mem0/blob/main/mem0/memory/main.py ; https://github.com/getzep/graphiti/blob/main/graphiti_core/edges.py",
      "maps_to": [
        "runtime/dedup-novelty",
        "runtime/continuity-threading",
        "deep-review",
  ...
  "recommendations": [
    {
      "rec": "Add semantic-community-projection.ts behind coverage-graph-signals.ts. It should consume active claim versions, produce deterministic hierarchical snapshots, remap stable community IDs, and append raw coverageGain, communityBirthGain, resolutionGain, duplicatePenalty and per-community divergencePressure before deriving compatibility newInfoRatio.",
      "target": "runtime/dedup-novelty",
      "rationale": "The existing graph signal corroborates an agent scalar but cannot distinguish repeated claims, new semantic territory, partition churn or contradiction obligations.",
      "effort": "L",
      "impact": "high",
      "evidence": "https://github.com/graspologic-org/graspologic/blob/main/graspologic/partition/leiden.py"
    },
    {
      "rec": "Extend JSONL with claim_asserted.v1, contradiction_detected.v1, contradiction_resolved.v1, claim_validity_closed.v1 and community_snapshot.v1. Update continuity-thread.cjs to carry lastAppliedSeq, activeClaimVersionIds, openContradictionIds, communitySnapshotId and projectionFingerprint.",
      "target": "runtime/continuity-threading",
      "rationale": "Resume must reconstruct what was believed at a specific runtime sequence without deleting old evidence or flattening unresolved contradictions into current truth.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://docs.xtdb.com/intro/what-is-xtdb.html ; https://terminusdb.org/docs/immutability-explanation/"
    },
    {
      "rec": "Change deep-research next-focus selection from prose-only Next Focus to an auditable argmax over per-community divergencePressure, with visit cooldown, expected-cost normalization and a critical-contradiction obligation queue. Recluster only after configured graph-drift thresholds; otherwise use provisional assignments.",
      "target": "runtime/fan-out-automation",
      "rationale": "This turns semantic-community state into deliberate within-thread broadening while controlling recomputation cost and preventing oscillation between the same communities.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://arxiv.org/abs/2601.08554"
    }
  ],
  "contradictions": [
    {
      "claim": "A fixed-seed hierarchical Leiden partition provides stable semantic-community identifiers for continuity.",
      "counter": "A seed improves repeatability for an unchanged graph, but added nodes and edges can split, merge or relabel communities; durable IDs require explicit cross-snapshot matching and lifecycle events.",
      "evidence": "https://github.com/graspologic-org/graspologic/blob/main/graspologic/partition/leiden.py ; https://arxiv.org/abs/2601.08554"
    },
    {
      "claim": "Append-only memory is sufficient to preserve contradictions safely.",
      "counter": "Append-only storage preserves evidence but, without an as-of projector and explicit resolution events, retrieval can surface mutually incompatible versions and count both as novelty.",
      "evidence": "https://github.com/mem0ai/mem0/blob/main/mem0/memory/main.py ; https://docs.xtdb.com/intro/what-is-xtdb.html"
    },
    {
      "claim": "Every newly detected contradiction should increase the novelty score.",
      "counter": "A contradiction should increase divergent-search pressure, but it should increase settled novelty only when it opens genuinely new semantic territory or is resolved with evidence; otherwise loops are rewarded for unresolved conflict.",
      "evidence": "https://github.com/getzep/graphiti ; https://github.com/getzep/graphiti/blob/main/graphiti_core/edges.py"
    }
  ],
  "next_angles": [
    "Calibrate stable-community matching under splits and merges: compare weighted Jaccard plus Hungarian matching against centroid matching and lineage-aware overlap on replayed research graphs.",
    "Design signed epistemic subgraphs inside each topical community so SUPPORTS and CONTRADICTS form stance/version groups without corrupting Leiden's unsigned modularity objective.",
    "Specify reclustering triggers and provisional-assignment error bounds, comparing full hierarchical Leiden, warm starts and HIT-Leiden-style affected-region maintenance.",
    "Define contradiction-resolution policy precedence when source validity, temporal validity and logical entailment disagree, including who may close a critical contradiction."
  ],
  "notes": "GraphRAG, Graphiti and Mem0 were deepened as previously catalogued dependencies and therefore are not repeated as new repositories. The main coverage gap is empirical calibration: no runtime corpus was available to estimate community-matching thresholds, pressure weights, reclustering drift thresholds or the false-novelty rate caused by partition churn. Graspologic is Python with a native backend, while system-deep-loop is Node-oriented; adopt its snapshot contract first and keep the clustering engine behind a capability adapter rather than introducing a mandatory Python runtime."
}
```
...
session id: 019f6429-16d5-7c82-8585-1b505aae6aae
--------
user
You are iteration 11/20 of a TARGETED, NON-CONVERGING (deliberately broadening WITHIN its thread) research loop to make "system-deep-loop" MORE EFFECTIVE.
...
ANGLE: R6 semantic-community novelty + contradiction-as-version
DIRECTIVE: Deepen R6. Semantic-community coverage/novelty (GraphRAG Leiden), contradiction-as-versioned-event (Graphiti validity windows), append-then-resolve (mem0). Concrete algorithms + data models for a novelty gauge that drives divergent broadening. Actionable design for runtime/dedup-novelty + continuity-threading.
...
New insights so far: 41; new contradictions: 34; recommendations: 30.
Open threads flagged: Design a hierarchical dependence model for judges sharing provider, base model, fine-tuning lineage, retrieval corpus, or tool receipts, and compare it with the simple weighted N_eff estimator. | Design model-checking properties for the fan-in state machine: no merge before ACCEPT, one terminal decision per policy version, and invariant selected sets under every completion-order permutation. | Calibrate seed-stage agreement thresholds against false-consensus risk, comparing raw vote agreement with semantic-cluster agreement and critical-contradiction vetoes. | Specify atomic budget reservation and cancellation accounting for WIDEN, including races between deadline, late completion, and next-stage lease acquisition. | Calibrate the two weighted thresholds from logged ROC and cost curves, separately by artifact risk class, rather than choosing universal constants. | Specify an Eval-DAG cache-compatibility fingerprint covering candidate, dependency artifacts, evaluator code, prompt, model, policy, toolchain and environment. | Design sequential tests for flaky repair verification: compare fixed replay counts, confidence sequences and SPRT-style early stopping under asymmetric false-pass costs. | Measure judge-selection bias introduced by uncertainty-band routing, because judges will see a harder and distribution-shifted sample than the overall review stream.
...
codex
This is a read-only research iteration, so no spec-folder write gate applies. I’m using the named `system-deep-loop` workflow and will first load its contract, then verify a small set of adjacent implementations and papers against primary sources; every resulting mechanism will map to the requested runtime targets.
mcp: mk-spec-memory/memory_match_triggers started
mcp: mk-spec-memory/memory_match_triggers (failed)
user cancelled MCP tool call
exec
/bin/zsh -lc "rg --files .opencode/skills | rg 'system-deep-loop/.*/?SKILL\\.md"'$|system-deep-loop/SKILL'"\\.md"'$|system-deep-loop'"'" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
...
codex
The local contract confirms a sharp mismatch in the current gauge: `newInfoRatio` is an agent-assigned scalar, while JSONL already supports graph mutations and append-only lifecycle events. I’m checking the upstream implementations now to replace that scalar with a replayable community-coverage projection and to make contradictions first-class temporal events rather than destructive updates.
web search: 
web search: site:github.com/microsoft/graphrag hierarchical_leiden cluster_graph.py ...
web search: 
web search: graspologic hierarchical_leiden GitHub graspologic-org 2026 ...
web search: 
web search: https://github.com/graspologic-org/graspologic
web search: 
web search: 'def hierarchical_leiden'
exec
/bin/zsh -lc "rg --files .opencode/skills/system-deep-loop/runtime | rg '(dedup|novel|continuity|coverage|state|convergence)' | head -120 && rg -n \"newInfoRatio|novelty|continuity|CONTRADICT|SUPERSED|invalid_at|valid_at|community|cluster\" .opencode/skills/system-deep-loop/runtime/lib .opencode/skills/system-deep-loop/runtime/scripts | head -260" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
.opencode/skills/system-deep-loop/runtime/lib/deep-loop/continuity-thread.cjs
.opencode/skills/system-deep-loop/runtime/lib/deep-loop/atomic-state.ts
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/README.md
...
.opencode/skills/system-deep-loop/runtime/tests/unit/coverage-graph-query.vitest.ts
.opencode/skills/system-deep-loop/runtime/tests/unit/deep-research-convergence-floor.vitest.ts
.opencode/skills/system-deep-loop/runtime/scripts/fanout-merge.cjs:311:        relation: 'CONTRADICTS',
.opencode/skills/system-deep-loop/runtime/scripts/fanout-merge.cjs:887:    ?? latestIteration?.newInfoRatio
.opencode/skills/system-deep-loop/runtime/scripts/convergence.cjs:348:  const contradictionEdges = edges.filter((edge) => edge.relation === 'CONTRADICTS');
...
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:47:  | 'CONTRADICTS'
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:48:  | 'SUPERSEDES'
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:56:  | 'CONTRADICTS'
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:74:  | 'CONTRADICTS';
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:163:  CONTRADICTS: 0.8,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:164:  SUPERSEDES: 1.5,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:173:  CONTRADICTS: 0.8,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:194:  CONTRADICTS: 0.8,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:225:  research: ['ANSWERS', 'SUPPORTS', 'CONTRADICTS', 'SUPERSEDES', 'DERIVED_FROM', 'COVERS', 'CITES'] as const,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:226:  review: ['COVERS', 'EVIDENCE_FOR', 'CONTRADICTS', 'RESOLVES', 'CONFIRMS', 'ESCALATES', 'IN_DIMENSION', 'IN_FILE'] as const,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-db.ts:227:  context: ['CONTAINS', 'REFERENCES', 'IMPORTS', 'DEPENDS_ON', 'IMPLEMENTS', 'EXPOSES', 'REUSES', 'CONSTRAINS', 'COVERED_BY', 'CONFIRMS', 'CONTRADICTS'] as const,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:79:/** Consolidation discovery result split into clusters and singleton leftovers. */
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:81:  clusters: ConsolidationCluster[];
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:488: * Discover same-kind consolidation clusters without mutating graph rows.
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:491: * same-kind cluster, and returned as clusters plus singleton leftovers.
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:495: * @returns Consolidation clusters and nodes that did not match any cluster.
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:509:    for (const cluster of workingClusters) {
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:510:      if (cluster.kind !== candidate.kind) continue;
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:512:      for (const clusterNode of cluster.nodes) {
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:513:        if (clusterNode.kind !== candidate.kind) continue;
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:514:        const score = getMemoizedSimilarity(ns, candidate.kind, candidate.name, clusterNode.name);
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:516:          bestCluster = cluster;
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:535:  const clusters: ConsolidationCluster[] = [];
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:538:  for (const cluster of workingClusters) {
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:539:    if (cluster.nodes.length > 1) {
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:540:      clusters.push({
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:541:        kind: cluster.kind,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:542:        canonical: cluster.canonical,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:543:        nodes: cluster.nodes,
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:546:      leftovers.push(cluster.nodes[0]);
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:551:    clusters: clusters.sort(compareClusters),
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:606: * Find all CONTRADICTS edge pairs in a namespace.
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-query.ts:622:      AND e.relation = 'CONTRADICTS'
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-signals.ts:613: * Canonical research contradiction density: CONTRADICTS edges divided by all
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-signals.ts:626:    if (edge.relation === 'CONTRADICTS') contradictionCount++;
...
.opencode/skills/system-deep-loop/runtime/lib/coverage-graph/coverage-graph-signals.ts:852:          AND e.relation = 'CONTRADICTS'
.opencode/skills/system-deep-loop/runtime/lib/council/convergence.cjs:104:  const dissentEdges = edges.filter((edge) => edge.relation === 'CONTRADICTS' || edge.relation === 'ESCALATES').length;
.opencode/skills/system-deep-loop/runtime/lib/council/council-graph-db.ts:27:  | 'CONTRADICTS'
.opencode/skills/system-deep-loop/runtime/lib/council/council-graph-db.ts:107:  'CONTRADICTS',

web search: 
web search: https://github.com/getzep/graphiti/blob/main/graphiti_core/edges.py
codex
R6 should become a two-layer, version-aware projection. One scalar cannot safely represent both “we learned something settled” and “we found a contradiction that demands broader research.”
