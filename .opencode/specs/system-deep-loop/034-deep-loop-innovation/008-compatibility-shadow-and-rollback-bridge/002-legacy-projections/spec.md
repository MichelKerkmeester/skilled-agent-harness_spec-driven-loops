...
_memory:
  continuity:
    packet_pointer: "system-deep-loop/034-deep-loop-innovation/008-compatibility-shadow-and-rollback-bridge/002-legacy-projections"
...
> Phase adjacency under `008-compatibility-shadow-and-rollback-bridge` (navigation order, not a hard runtime dependency): predecessor `001-upcasters-and-dual-read-adapters`; successor `003-shadow-parity-harness`.
...
| Field | Value |
|-------|-------|
| **Packet** | system-deep-loop/034-deep-loop-innovation/008-compatibility-shadow-and-rollback-bridge/002-legacy-projections |
| **Level** | 2 |
| **Priority** | P0 |
| **Status** | Planned |
| **Created** | 2026-07-15 |
...
| **Depends on** | None (`[]`); sibling contracts compose at the phase-008 parent gate |
...
The phase-006 spine introduces an immutable, typed append-only ledger that records in parallel while the shipped
legacy JSONL and replace-style JSON paths remain authoritative. Existing consumers do not read that ledger. They read
the exact state files, checkpoints, dashboards, registries, and resume-facing artifacts catalogued by the phase-003
...
and never overwrites an authoritative legacy path in phase 008. Existing readers therefore continue unchanged against
the live legacy writer while the dark spine proves it can render their contracts faithfully.

The design is grounded in the phase-003 census contract, the phase-006 typed-ledger contract, the program phase tree,
...
### In Scope
- Derive a closed projection manifest from the executed phase-003 census. Every legacy JSONL/JSON surface records its
...
- Define projection as a pure fold from a verified phase-006 ledger prefix plus an explicit BASE anchor and versioned
...
- Defining the event envelope, ledger framing, authorization proof, or replay fingerprint owned by phase 006.
...
- Guessing legacy surfaces not yet resolved by the phase-003 census; an unclassified surface blocks manifest closure.
<!-- /ANCHOR:scope -->
...
|----|-------------|---------------------|
| REQ-001 | Projection coverage is census-complete | Every JSONL/JSON surface in `003-baseline-taxonomy-and-state-census/spec.md` maps to exactly one projection contract or an explicit non-projectable disposition with an owning later phase; zero reader or writer rows remain unclassified |
...
| REQ-007 | Shadow publication cannot alter legacy authority | All phase-008 projection writes resolve under an isolated shadow root, and path guards reject live legacy targets before opening a file |
...
| Resume and tooling inputs | The census-linked JSON/JSONL artifacts those readers already open | Never expose a head newer than the artifact's durable watermark; shadow paths only in phase 008 | Unmodified reader results against BASE and projected fixture trees |
<!-- /ANCHOR:requirements -->

<!-- ANCHOR:success-criteria -->
## 5. SUCCESS CRITERIA

- **SC-001**: The phase-003 census closes to a projection manifest with zero unclassified JSONL/JSON surfaces or readers.
...
phase-003 census and the verified phase-006 ledger reader. The highest risk is false byte parity from reserializing a
...
`006-transition-authorized-ledger-core/002-typed-append-only-ledger/spec.md`, `manifest/phase-tree.json`, and
