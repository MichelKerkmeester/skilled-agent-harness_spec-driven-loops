...
## Finding 3 — Propagation is all 7 command files, but priority is the 4 runtime modes

sonnet-critical noted the self-check appears in all 7 `/deep:*` command files. I confirm the shape is identical in `research.md:39-72`. Prioritization: the operator symptoms name the 4 runtime-loop modes (research/review/context/ai-council); the improvement-family 3 (skill-benchmark/agent-improvement/model-benchmark) share the pattern but are lower priority (glm-max `:113` deferral). Fix all 7 in one pass (identical edit) but gate acceptance on the 4 runtime modes.
