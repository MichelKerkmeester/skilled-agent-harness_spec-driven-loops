...
| Candidate score JSON contains `baselineScore`, `delta`, and `recommendation:"candidate-better"` or an evaluator artifact computes those fields | `promote-candidate.cjs` already requires `candidate-better` and `delta` (`.opencode/skills/sk-improve-agent/scripts/promote-candidate.cjs:156-163`). |
| `legal-stop-evaluation.json` or journal `legal_stop_evaluated` contains `improvementGate` with `weightedDelta>=thresholdDelta` evidence | Ties the stop gate to config `thresholdDelta` (`.opencode/skills/sk-improve-agent/assets/improvement_config.json:41-54`). |
| `session_end.details.stopReason:"converged"` appears only when `improvementGate.passed:true`; otherwise `blockedStop` | Matches the command taxonomy (`.opencode/commands/deep/start-agent-improvement-loop.md:306-316`). |

One existing fixture illustrates why this matters. The low-sample benchmark fixture records candidate iteration 2 as `recommendation:"keep-baseline"` (`.opencode/skills/sk-improve-agent/scripts/tests/fixtures/low-sample-benchmark/agent-improvement-state.jsonl:3`) and a gate decision of `keep-baseline` (`.opencode/skills/sk-improve-agent/scripts/tests/fixtures/low-sample-benchmark/improvement-journal.jsonl:7`), yet its session end uses `stopReason:"converged"` with `sessionOutcome:"keptBaseline"` (`.opencode/skills/sk-improve-agent/scripts/tests/fixtures/low-sample-benchmark/improvement-journal.jsonl:8`). The journal enum now supports `blockedStop` (`.opencode/skills/sk-improve-agent/scripts/improvement-journal.cjs:21-29`), so a future Call B should reject this shape as a legal-stop pass when `improvementGate` or `evidenceGate` fails.
...
- I ruled out "the scorer already computes delta from `--baseline`." The YAML passes `--baseline`, but the scorer main path does not read `args.baseline` and emits no `delta` (`.opencode/skills/sk-improve-agent/scripts/score-candidate.cjs:260-265`, `.opencode/skills/sk-improve-agent/scripts/score-candidate.cjs:347-361`).
- I ruled out "the reducer's bestPromptRecord is enough for improvementGate." It identifies the highest score, not a typed baseline/current delta comparison (`.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:514-522`).
- I ruled out "promotion_rules.md accurately reflects the executable promotion script." The reference says promotion enforces `candidate-acceptable` scoring above `thresholdDelta` (`.opencode/skills/sk-improve-agent/references/promotion_rules.md:35-39`), while the script requires `candidate-better` and `score.delta` above threshold (`.opencode/skills/sk-improve-agent/scripts/promote-candidate.cjs:156-163`).

## Sketched Diff (if any)

For `.opencode/skills/sk-improve-agent/scripts/score-candidate.cjs` around `main()`, current text:
...
For `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_auto.yaml`, current text:
...
| Call B | Disciplined `/deep:start-agent-improvement-loop` path with baseline scoring and legal-stop evaluator wired. |
