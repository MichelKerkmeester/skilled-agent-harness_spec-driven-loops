...
_memory:
  continuity:
    packet_pointer: "system-deep-loop/z_archive/012-sk-agent-improver-test-report-alignment"
    ...
    blockers: []
    key_files:
      - .opencode/specs/system-deep-loop/z_archive/012-sk-agent-improver-test-report-alignment/001-deep-research-recommendations/research/research.md
      ...
      - .opencode/specs/system-deep-loop/z_archive/012-sk-agent-improver-test-report-alignment/004-improve-agent-command-flow-stress-tests/test-report.md
      - .opencode/specs/system-deep-loop/z_archive/012-sk-agent-improver-test-report-alignment/005-improve-agent-executable-wiring/spec.md
    completion_pct: 100
    open_questions:
...
- **Cross-architecture validation** (added 2026-05-03 from 062 follow-on) — the 060/004 methodology was empirically applied to two more same-architecture LEAF agents in packet `../062-deep-loop-command-flow-stress-tests/`. Result: `@deep-research` PASS 6/0/0 R1 clean (clean-substrate confirmation); `@deep-review` PASS 0/5/1 across R1+R2 (real discipline gaps surfaced — setup-binding emission, refusal-discipline wording). The asymmetric outcome is the methodology working as designed: it correctly distinguishes well-disciplined agents from those with hidden gaps. Recommendations applied to sk-deep-review SKILL.md ALWAYS rules 13+14 (BINDING + REFUSE canonical wording) close the surfaced gaps; expected R3 ≈ PASS 5+/PARTIAL 0-1.
- **Executor failover pattern** (added 2026-05-03 from 062 follow-on) — when cli-copilot account budget caps out mid-campaign, switch to `cli-opencode` with `github-copilot` provider via `opencode run "PROMPT" -m github-copilot/gpt-5.5 --dangerously-skip-permissions`. Different OAuth pool, fresh credentials. Verified working in 062/004 R2 (6 sequential CPs) — opencode handled what cli-copilot timed out on (CP-053: 600s timeout → 140s clean run).
<!-- /ANCHOR:reusable-artifacts -->
