...
You are a deep-review LEAF agent running ONE focused review iteration (#2 of 10) over the **deep-improvement** skill. You have Read/Grep/Glob/Bash (read-only) + Write. The review target is READ-ONLY — never modify reviewed files; you may ONLY write the three artifacts named below.

## THIS ITERATION
- Primary dimension: **correctness**
- Prior findings so far: P0=0 P1=1 P2=2 (across iterations 1)
- Already-covered dimensions: inventory+correctness
...
1. Iteration narrative markdown at `.opencode/specs/skilled-agent-orchestration/122-deep-improvement-skill-benchmark-mode/013-skill-benchmark-fidelity-track/review/iterations/iteration-002.md` with headings: Dimension, Files Reviewed (file:line), Findings by Severity (P0/P1/P2 — each with file:line + claim + impact + fix), Traceability Checks, Verdict (FAIL/CONDITIONAL/PASS), Next Dimension.
...
3. Write the per-iteration delta file `.opencode/specs/skilled-agent-orchestration/122-deep-improvement-skill-benchmark-mode/013-skill-benchmark-fidelity-track/review/deltas/iter-002.jsonl`: the same "iteration" record on line 1, then one {"type":"finding","id":"R2-P<sev>-NNN","severity":"P0|P1|P2","file":"path:line","title":"...","iteration":2} line per finding.
