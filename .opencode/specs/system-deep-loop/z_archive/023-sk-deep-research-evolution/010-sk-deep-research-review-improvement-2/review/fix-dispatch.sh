...
1. .opencode/skills/sk-deep-review/scripts/reduce-state.cjs:
...
2. .opencode/skills/sk-deep-research/scripts/reduce-state.cjs:
...
3. .opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:
...
4. .opencode/skills/sk-improve-agent/scripts/trade-off-detector.cjs:
   - Emit scored dimension array into journal row, or change getTrajectory() to load from scoreOutputPath

5. .opencode/skills/sk-improve-agent/scripts/improvement-journal.cjs:
...
1. .opencode/skills/system-spec-kit/mcp_server/lib/coverage-graph/coverage-graph-query.ts:
...
2. .opencode/skills/system-spec-kit/mcp_server/lib/coverage-graph/coverage-graph-signals.ts:
   - Thread full namespace through every nested edge lookup

3. .opencode/skills/system-spec-kit/mcp_server/handlers/coverage-graph/query.ts, status.ts, convergence.ts:
   - Make sessionId required for all non-admin reads

4. .opencode/skills/system-spec-kit/scripts/optimizer/promote.cjs:
   - Restrict savePromotionReport() to dedicated audit directory, reject symlinks
   - Make manifest mandatory for evaluateCandidate()

5. .opencode/skills/system-spec-kit/scripts/optimizer/search.cjs:
   - Derive search space from canonical manifest, reject unknown/locked fields

6. .opencode/skills/system-spec-kit/scripts/optimizer/replay-corpus.cjs:
...
1. .opencode/skills/system-spec-kit/scripts/lib/wave-segment-planner.cjs:
...
2. .opencode/skills/system-spec-kit/scripts/lib/wave-lifecycle.cjs:
   - Enforce adjacent phase transitions with an allowed-transition matrix

3. .opencode/skills/system-spec-kit/scripts/lib/wave-coordination-board.cjs:
...
4. .opencode/skills/system-spec-kit/scripts/lib/coverage-graph-core.cjs:
...
6. .opencode/skills/system-spec-kit/scripts/optimizer/replay-runner.cjs + replay-corpus.cjs:
...
4. .opencode/agents/deep-review.md — Collapse iteration skeleton to one canonical template matching reduce-state.cjs
5. .opencode/agents/deep-research.md — Make config an explicit Step 1 input

6. .opencode/skills/sk-deep-research/assets/deep_research_config.json — Remove fork/completed-continue
7. .opencode/skills/sk-deep-review/assets/review_mode_contract.yaml — Update to 3 supported modes only
8. .opencode/skills/sk-improve-agent/assets/improvement_config.json — Strip dormant resume fields
9. .opencode/skills/sk-deep-review/references/convergence.md — Fix to match actual blocked_stop event shape
10. .opencode/skills/sk-deep-research/references/loop_protocol.md — Fix wave section and delta-vs-full replay
11. .opencode/skills/sk-deep-review/references/loop_protocol.md — Fix delta-vs-full replay description
...
1. .opencode/skills/system-spec-kit/scripts/tests/coverage-graph-integration.vitest.ts:
   - Replace shadow-copied contract with real TS source import
   - Replace tautological checks with executable assertions

2. .opencode/skills/system-spec-kit/scripts/tests/coverage-graph-stress.vitest.ts:
   - Move wall-clock budgets to opt-in benchmark guards
   - Fix no-contradictions test to build contradiction-free graph

3. .opencode/skills/system-spec-kit/scripts/tests/session-isolation.vitest.ts:
   - Add colliding-ID fixtures for higher-level read paths

4. .opencode/skills/system-spec-kit/scripts/tests/optimizer-replay-runner.vitest.ts:
   - Replace alternate-config test with threshold-straddling fixtures

5. .opencode/skills/sk-improve-agent/scripts/tests/benchmark-stability.vitest.ts:
   - Assert heuristic outcomes directly

6. .opencode/skills/sk-improve-agent/scripts/tests/improvement-journal.vitest.ts:
   - Add session_end alias tests

7. Create .opencode/skills/system-spec-kit/scripts/tests/coverage-graph-contradictions.vitest.ts with session-scoped tests

8. Move stale test suites to archive:
   - mv .opencode/skills/system-spec-kit/mcp_server/tests/coverage-graph-db.vitest.ts to tests/archive/
   - mv .opencode/skills/system-spec-kit/mcp_server/tests/coverage-graph-tools.vitest.ts to tests/archive/
...
7. .opencode/skills/sk-improve-agent/manual_testing_playbook/07--runtime-truth/034-replay-consumer.md:
   - Rewrite dashboard claims to registry-only outputs

8. .opencode/skills/sk-deep-research/manual_testing_playbook/03--iteration-execution-and-state-discipline/029-graph-events-emission.md:
