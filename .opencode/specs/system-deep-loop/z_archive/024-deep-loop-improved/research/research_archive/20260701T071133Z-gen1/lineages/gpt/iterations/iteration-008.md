...
4. Existing comment-hygiene infrastructure exists in `sk-code`, but it is not preventing the checked-in workflow YAML markers. The system-spec-kit manual playbooks identify `check-comment-hygiene.sh` as the baseline enforcement script [SOURCE: `.opencode/skills/system-spec-kit/manual_testing_playbook/ux-hooks/A--comment-hygiene-checker-baseline.md:72`], and `sk-code` requires the checker before commit [SOURCE: `.opencode/skills/sk-code/SKILL.md:245`]. Recommendation: include `.yaml` command assets in the pre-commit/CI scan or add an explicit command-asset hygiene gate.
