...
Replaced scaffold-template placeholder bodies and scaffold-signature frontmatter in all 18 `plan.md` and 18 `tasks.md` files under `.opencode/specs/system-deep-loop/024-deep-loop-improved/002-deep-loop-runtime/`. Each rewrite was grounded in the corresponding leaf's own `spec.md`, preserved the existing template anchors, marked the shipped implementation phases/tasks complete, and set each remediated file's completion metadata to 100 percent.
