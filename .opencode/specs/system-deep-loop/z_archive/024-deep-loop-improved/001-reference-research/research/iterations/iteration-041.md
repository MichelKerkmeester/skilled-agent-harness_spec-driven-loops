...
1. Author the ADR now: convergence knobs are already policy, not incidental implementation details.
   - Reference mechanism: Kasper declares bounded config fields for `scoring_threshold`, `weakness_decay_days`, and `min_observations_for_update` in one schema, then merges normalized values over defaults [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/config.ts:26] [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/config.ts:189]. Its defaults live in one typed object [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/types.ts:31].
...
2. The ADR should define a declarative convergence profile, with per-loop semantics but shared shape.
   - Reference mechanism: loop-cli-main centralizes loop operator inputs in `LoopOptions`, IPC payloads, and a single `buildLoopOptions()` adapter before those values reach the daemon/UI boundary [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/types.ts:20] [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/types.ts:90] [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/loop-cli-main/src/loop-config.ts:86].
...
3. The shared parity test is mandatory; otherwise the ADR becomes documentation drift.
   - Reference mechanism: Kasper tests threshold clamping for `scoring_threshold` and `weakness_decay_days` [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/tests/config.test.ts:43] [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/tests/config.test.ts:110], tests min-observation gating [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/tests/utils.test.ts:47], and tests decay changing observed ranking [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/tests/state.test.ts:178].
...
4. Fold graph novelty floors into the same contract, but keep signal computation in coverage graph code.
   - Reference mechanism: Kasper applies half-life weighting directly in aggregate statistics with `weight = 0.5 ** (ageDays / decayDays)` [SOURCE: .opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:74].
