...
description: "Normalize all 27 deep-improvement reference/asset docs to the canonical frontmatter contract: enum fixes, tier judgment, and net-new authoring for 9 docs."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Executed: 27 docs normalized and checks green"
...
| **Storage** | `.opencode/skills/deep-improvement/references/*.md` + `assets/*.md` (27 docs, 7 asset READMEs exempt) |
| **Testing** | `check-skill-doc-frontmatter.sh --skill deep-improvement --coverage` + Python local-mode advisor smoke |

### Overview
deep-improvement is the largest phase so far: 27 in-scope docs across four lanes (agent_improvement, model_benchmark, non_dev_ai_system, skill_benchmark plus shared). Drift spans four states — 5 fully-detailed docs with `contextType: reference` outside the enum, 12 partial docs missing tier and contextType, 8 docs with title+description only, and 1 doc with no frontmatter at all — so the phase mixes pure normalization with net-new phrase authoring.
<!-- /ANCHOR:summary -->
...
3. One guarded Python pass patches all 27 leading fences
...
### Phase 1: Setup
- [x] Frontmatter state captured for all 27 docs (5 full-with-enum-drift, 12 partial, 8 title-only, 1 bare)
...
- [x] Net-new authoring: 8 title-only docs gain phrases + tier + contextType; `heldout_and_gold_sets.md` gains the full block
- [x] Tier judgment: 8 formal contract docs at `important` (4 kept, 4 added); 2 Lane C guides demoted from `important` to `normal`

### Phase 3: Verification
- [x] Coverage check green for the skill (docs=27, violations=0)
...
|-----------|-------|-------|
| Contract | All 27 docs, coverage mode | `check-skill-doc-frontmatter.sh --skill deep-improvement --coverage` |
