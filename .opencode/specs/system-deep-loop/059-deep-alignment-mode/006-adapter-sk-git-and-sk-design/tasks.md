...
- [x] T001 Confirm phase 005 adapter contract signature is available or fall back to the design-brief-locked contract (`.opencode/specs/system-deep-loop/059-deep-alignment-mode/005-adapter-sk-doc/`) — read `sk-doc.cjs` in full; both new adapters copy its `discover`/`standardSource`/`check`/`loadKnownDeviations` module shape.
