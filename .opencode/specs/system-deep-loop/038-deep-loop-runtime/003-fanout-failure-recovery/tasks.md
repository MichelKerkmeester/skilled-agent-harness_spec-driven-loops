...
**Candidate map**: C1 = DL-failure-class-taxonomy · C2 = Q3-fanout-recovery (transient/fatal classification) · C3 = Q3-fanout-transient-fatal-retry · C4 = DL-orphan-lineage-reset · C5 = DL-recover-vs-fresh-gate. All five are DONE, and the prerequisite 030 infra is pre-checked `[x]` with commit evidence.
...
- [x] T011 Guarantee count-correctness: a retry-success flips the lineage to succeeded and is NOT counted in `summary.failed`, and a retry-exhaustion surfaces as a genuine failure with the correct run exit-code [45m]
...
- [x] T013 [P] Add a validate-existing-state resume gate that REFUSES a missing/empty/corrupt expected JSONL state instead of silently fresh-initializing. Implemented as explicit `--require-existing-state` / `requireExistingState`, and the legitimate fresh reducer path remains unchanged (`reduce-state.cjs`) [1h]
...
### Unit Tests
- [x] T014 C1: each of timeout / exit / salvage_miss produces the right bounded label, and the `buildPoolSummary` per-class rollup counts are correct (`fanout-pool.vitest.ts`) [30m]
...
- [x] T019 [P] C5: refuse-on-missing, refuse-on-empty/zero-byte, refuse-on-corrupt, and a legitimate first-ever fresh start is unaffected (`deep-research-reduce-state.vitest.ts`) [30m]
...
- [x] T023 `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh .opencode/specs/system-deep-loop/038-deep-loop-runtime/003-fanout-failure-recovery --strict` passes with 0 errors / 0 warnings [10m]
