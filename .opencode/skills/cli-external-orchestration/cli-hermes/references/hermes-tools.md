...
- A whole-tree symlink from `./.hermes/skills` to `.opencode/skills` is followed, flattened and **scanned in full at every session start**: observed 2026-09-14, the static scanner ran at full CPU for over ten minutes and quarantined every hub as dangerous with thousands of findings (scripts and node modules under the packets). Do not link the tree. Link one skill directory per need under `.hermes/skills/<name>` and preload it with `-s <name>`; `hermes skills trust` reported 56 loadable project skills for the tree, and `hermes skills list` shows no project rows either way.
...
- Link whole skill directories, never single files: the static scanner flags a symlink resolving outside a skill directory as a critical traversal, and quarantine is per skill directory and fail-closed.
