...
- Evidence: The symlink listing proving the project skill is present on disk, the line count of the listing, the grep count, the exit code, and the head of the table showing the Source column.
- Desired user-visible outcome: a concise verdict naming the observed signal and the evidence behind it.
- Pass/fail: PASS when the symlink exists and the listing does not mention it; FAIL when the command errors, or when the listing does show the project skill, which would mean this documented boundary has changed and the packet's guidance needs updating; SKIP only when a named environment blocker prevents the check, such as an unreachable provider or a missing credential, including a missing `hermes` binary.
