...
- Prompt: `Quote verbatim the bound packet path and the Objective line from the goal section a project plugin froze into your system prompt. If there is no bound packet, reply exactly NO_BOUND_PACKET.`
...
perl -e 'alarm 300; exec @ARGV' -- hermes chat -Q --oneshot --ignore-rules --source tool --provider llmgateway --model glm-5.3-flash --reasoning none \
  ...
  -q "Quote verbatim the bound packet path and the Objective line from the goal section a project plugin froze into your system prompt. If there is no bound packet, reply exactly NO_BOUND_PACKET." \
