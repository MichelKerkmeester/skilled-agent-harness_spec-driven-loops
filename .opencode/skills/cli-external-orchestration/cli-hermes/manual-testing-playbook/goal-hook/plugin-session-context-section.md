...
- Section split: the plugin renders the goal as its own `repo-guards-goal` section. This scenario checks `repo-guards-session-context` alone, which carries no packet binding and stays in the hundreds of characters. The goal slice and its size are `HERMES-020`'s to assert, and this command deliberately sets no `HERMES_SPEC_FOLDER`.
