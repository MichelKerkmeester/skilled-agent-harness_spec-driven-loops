...
| Project skills | One directory symlink per curated skill under `.hermes/skills/<name>`, preloaded with `-s <name>` after `hermes skills trust` |
...
| Path | What it holds |
|---|---|
| `.hermes/skills/<name>/` | One directory symlink per curated skill. Never link the whole `.opencode/skills` tree: Hermes's static scanner walks everything it reaches, costs ten minutes per session start and quarantines every hub |
