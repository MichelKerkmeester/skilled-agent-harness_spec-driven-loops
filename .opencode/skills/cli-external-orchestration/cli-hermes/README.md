...
| Generated prompt templates | `.hermes/prompts/*.md` mirror `.opencode/commands/**`, regenerated by `sync-prompts-hermes.cjs` |
...
```text
.opencode/skills/cli-external-orchestration/cli-hermes/
...
| `.hermes/prompts/*.md` | Generated pointer stubs, one per `.opencode/commands/**` file, named by the flattened command path |
| `.hermes/SYNC.md` | The sync manifest: what derives from `.opencode`, what can drift and how to regenerate it |
...
| Session start takes ten minutes and every hub is quarantined | The whole `.opencode/skills` tree is linked under `.hermes/skills` | Link one skill directory at a time and preload it with `-s <name>` |
