...
description: The dedicated per-mode catalog of the Cursor provider, the enforced 21-id model allowlist, defaults, the suffix-baked effort lever and dispatch shape reachable through the cli-cursor mode.
trigger_phrases:
...
The single catalog of the provider, model ids, defaults, effort lever, and dispatch shape the cli-cursor mode can reach. Unlike sibling modes, cli-cursor dispatch is scoped to an ENFORCED 21-id allowlist — this file lists all twenty-one inline because that list is a safety contract.
...
### Core Principle
One place to answer "which provider, which model, which effort, how to dispatch" for cli-cursor. This mode reaches exactly one backing provider — Cursor — and is deliberately narrow: dispatch is scoped to a hard-enforced 21-id allowlist, and Cursor's own `auto` router is excluded.
...
cli-cursor has ONE backing provider — **Cursor** — reached through the `cursor-agent` binary. Cursor's live roster spans 150+ hosted-frontier ids (GPT/Claude/Gemini/Grok/GLM/Kimi families), but **cli-cursor dispatch is scoped to exactly these 21 ids — this is an ENFORCED allowlist, not a reference list.** Dispatching any off-list id HARD-FAILS before a command is built (see §6). `auto` (Cursor's own router) is deliberately EXCLUDED — it can silently resolve to a model outside this set, defeating the point of enforcing one.
