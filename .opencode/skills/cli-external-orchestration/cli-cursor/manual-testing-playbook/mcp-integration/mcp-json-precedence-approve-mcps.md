...
- Expected signals: Existence of project/user `mcp.json` is checked and reported honestly (present or absent for each). `cursor-agent -p "say hi" --model auto --output-format text --approve-mcps </dev/null` exits 0 with no flag-rejection error.
