---
title: "EC-008 -- cli-cursor missing transport"
description: "Hermetic operator check for cli-cursor:EC-008: fails before cli-cursor execution when its transport is unavailable."
matrix_cell: "cli-cursor:EC-008"
test_file: "tests/stress/cli-adapter/cli-cursor.vitest.ts"
test_name: "fails before cli-cursor execution when its transport is unavailable"
playbook_path: "cli-external-orchestration/cli-cursor/manual-testing-playbook/stress/transport-missing.md"
id: "cli-cursor-EC-008"
version: 1.0.0.0
---

# EC-008 -- cli-cursor missing transport

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-cursor:EC-008`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-cursor:EC-008` ("fails before cli-cursor execution when its transport is unavailable"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-cursor.vitest.ts` reports `fails before cli-cursor execution when its transport is unavailable` as passing under the shipped stress-matrix
  cell `cli-cursor:EC-008`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-cursor:EC-008 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `fails before cli-cursor execution when its transport is unavailable` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-cursor:EC-008 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "fails before cli-cursor execution when its transport is unavailable" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `fails before cli-cursor execution when its transport is unavailable` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
- **ledger/artifacts**: A non-zero result and no cli-cursor provider capture when its transport is removed from `PATH`.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-cursor:EC-008` |
