---
title: "EC-009 -- cli-cursor budget rejection"
description: "Hermetic operator check for cli-cursor:EC-009: rejects an over-budget cli-cursor lineage before process spawn."
matrix_cell: "cli-cursor:EC-009"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-cursor:EC-009`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-cursor:EC-009` ("rejects an over-budget cli-cursor lineage before process spawn"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-cursor.vitest.ts` reports `rejects an over-budget cli-cursor lineage before process spawn` as passing under the shipped stress-matrix
  cell `cli-cursor:EC-009`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-cursor:EC-009 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `rejects an over-budget cli-cursor lineage before process spawn` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-cursor:EC-009 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "rejects an over-budget cli-cursor lineage before process spawn" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `rejects an over-budget cli-cursor lineage before process spawn` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-cursor:EC-009` |
