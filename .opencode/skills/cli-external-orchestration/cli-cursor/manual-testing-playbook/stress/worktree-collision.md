---
title: "EC-012 -- cli-cursor worktree collision"
description: "Hermetic operator check for cli-cursor:EC-012: runs cli-cursor from each isolated worktree cwd."
matrix_cell: "cli-cursor:EC-012"
test_file: "tests/stress/cli-adapter/cli-cursor.vitest.ts"
test_name: "runs cli-cursor from each isolated worktree cwd"
playbook_path: "cli-external-orchestration/cli-cursor/manual-testing-playbook/stress/worktree-collision.md"
id: "cli-cursor-EC-012"
version: 1.0.0.0
---

# EC-012 -- cli-cursor worktree collision

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-cursor:EC-012`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-cursor:EC-012` ("runs cli-cursor from each isolated worktree cwd"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-cursor.vitest.ts` reports `runs cli-cursor from each isolated worktree cwd` as passing under the shipped stress-matrix
  cell `cli-cursor:EC-012`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-cursor:EC-012 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `runs cli-cursor from each isolated worktree cwd` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-cursor:EC-012 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "runs cli-cursor from each isolated worktree cwd" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `runs cli-cursor from each isolated worktree cwd` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-cursor:EC-012` |
