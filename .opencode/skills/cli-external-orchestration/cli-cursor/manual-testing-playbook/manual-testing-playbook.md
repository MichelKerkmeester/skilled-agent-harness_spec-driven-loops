...
7. **Enforced model allowlist.** `composer-2.5` (Cursor's own native model, confirmed live at Pro tier) is the documented default; dispatch is otherwise scoped to exactly 20 ids — `composer-2.5`/`composer-2.5-fast`, `cursor-grok-4.5-{low,medium,high}[-fast]` (6 ids), `cursor-grok-4.6-{low,medium,high,xhigh}[-fast]` (8 ids), `glm-5.2-{high,max}`, and `gpt-5.6-luna-{max,max-fast}` (2 ids) — enforced by `CURSOR_SUPPORTED_MODELS` in `executor-config.ts` and checked in both `fanout-run.cjs` and `dispatch-model.cjs`. `auto` and every hosted GPT/Claude/Gemini/Kimi id are OUT OF SCOPE for this skill, not merely undocumented. Effort tiers are reached by exact enumerated id, never a `[effort=...]` bracket (confirmed rejected: `Error: Cannot use this model`). Use the model a scenario names; do not substitute IDs outside this allowlist.
