...
| **Model allowlist** | Enforce the exact 20 Cursor model ids across Composer, GLM 5.2, Grok (4.5 and 4.6) and GPT-5.6 Luna Max, rejecting `auto` and parameterized `model[effort=...]` brackets |
...
Dispatch is scoped to an enforced 20-id allowlist. Cursor's own `auto` router is deliberately excluded, since it can silently resolve outside the set. **Composer** (`composer-2.5` and `composer-2.5-fast`) is Cursor's native model and the skill default. **Grok** adds both live versions — 4.5's 6 tiers (`cursor-grok-4.5-{low,medium,high}[-fast]`) and 4.6's 8 tiers (`cursor-grok-4.6-{low,medium,high,xhigh}[-fast]`) — **GLM 5.2** (`glm-5.2-high`, `glm-5.2-max`), and **GPT-5.6 Luna Max** (`gpt-5.6-luna-max`, `gpt-5.6-luna-max-fast`) round out the allowlist. Effort tiers are baked directly into a model id. There is no `--reasoning-effort` flag. The parameterized `model[effort=...]` bracket some sibling CLIs support is rejected outright by Cursor CLI.
...
| `cli-cursor model '<id>' is not in the enforced allowlist` | The requested `--model` is outside the 20-id allowlist (SKILL.md §3) | Pick one of the 20 allowed ids or ask the operator to extend the allowlist |
...
A: Pick `composer-2.5` or `composer-2.5-fast` (the default) when the task specifically wants Cursor's own house model or when no other model is named. Pick `cursor-grok-4.5-{low,medium,high}[-fast]`, `cursor-grok-4.6-{low,medium,high,xhigh}[-fast]`, `glm-5.2-{high,max}`, or `gpt-5.6-luna-{max,max-fast}` when the task or user explicitly names Grok, GLM, or GPT-5.6 Luna Max. Dispatch is enforced to exactly these 20 ids. Every other hosted GPT persona/tier, plus Claude, Gemini and Kimi ids and `auto`, are all out of scope for this skill.
...
| [`references/providers-and-models.md`](./references/providers-and-models.md) | Single-source catalog of the Cursor provider and the enforced 20-id model allowlist |
