...
| Model id | Notes |
|----------|-------|
| `deepseek-v4-flash-vision-exp` | Latency-optimized reasoning model pinned to `--thinking max` by policy. **Direct-dispatch route only since 2026-09-07:** the bare literal now maps to `llmgateway` in `PI_MODEL_PROVIDERS`, so the deep-loop fan-out reaches DeepSeek V4 Flash through DevPass; select this route by hand with `--provider opencode-go`. A live `opencode run --model opencode-go/deepseek-v4-flash` turn completed 2026-08-07; the gateway's monthly usage window returned 429 on 2026-09-07 |
