...
| Model id | Notes |
|----------|-------|
| `deepseek-v4-flash-vision-exp` | Latency-optimized reasoning model pinned to `--thinking max` by policy. **Direct-dispatch route only since 2026-09-07:** the fan-out's bare DeepSeek literal maps to `llmgateway` in `PI_MODEL_PROVIDERS`, so the deep-loop fan-out reaches DeepSeek Flash through DevPass and not here; select this route by hand with `--provider opencode-go`. The DevPass literal is `deepseek-v4.1-flash` since 2026-09-10, so the two routes no longer name the same model. A live `opencode run --model opencode-go/deepseek-v4-flash` turn completed 2026-08-07; the gateway's monthly usage window returned 429 on 2026-09-07 |
