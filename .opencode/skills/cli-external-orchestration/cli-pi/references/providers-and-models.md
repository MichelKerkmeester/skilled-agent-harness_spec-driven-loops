...
Cline provider (Cline Pass account, base `https://api.cline.bot/api/v1`, OpenAI-compatible), added to Pi **by config** — a `providers.cline-pass` block in `.pi/models.json` (`api: openai-completions`, env-keyed `CLINE_API_KEY`) plus `enabledModels` entries in `.pi/settings.json`. It is not a Pi builtin; full setup and removal live in [.pi/custom-providers.md](../../../../../.pi/custom-providers.md). Select flash with `--provider cline-pass --model cline-pass/cline-pass/deepseek-v4-flash`, pro with `--model cline-pass/cline-pass/deepseek-v4-pro`, or the free Ox Alpha tune with `--model cline-pass/x-ai/ox-alpha`. pi's default here is `defaultProvider: cline-pass` with `defaultModel` set in `.pi/settings.json` (currently `x-ai/ox-alpha`; point it at any cline-pass model).

Model-id gotcha: every reference here is **three-segment** (`<provider>/<modelType>/<model>`) because the model `id` in `.pi/models.json` keeps its `modelType/` prefix. The DeepSeek entries use `cline-pass/` (`cline-pass/cline-pass/deepseek-v4-flash`); **Ox Alpha uses the vendor prefix `x-ai/`** (`cline-pass/x-ai/ox-alpha`, id = `x-ai/ox-alpha`) — do NOT assume `cline-pass/ox-alpha`, which the Cline API 404s. Cline requires the `modelType/model` shape: a bare id returns `400 "invalid model format"`, a wrong one returns `404 "model not found"`, and both hide from `pi --list-models` / `pi auth check` (which never send a completion), surfacing only on the first real dispatch.

Policy: the DeepSeek V4 Flash, DeepSeek V4 Pro, and Ox Alpha entries here are reasoning models whose Cline thinking tiers top out at `xhigh` — there is **no `max` tier** on this provider. Dispatch them **only at `--thinking xhigh`**; do not request `max`. Pi's global `defaultThinkingLevel` is already `xhigh`, so an unqualified dispatch lands on the correct tier, but pass `--thinking xhigh` explicitly in fan-out for clarity. Ox Alpha's interactive picker additionally offers the lower tiers, but the config entry mirrors the DeepSeek `xhigh` ceiling for a single consistent cline-pass policy.
...
| `cline-pass/x-ai/ox-alpha` | Ox Alpha via the Cline provider — Cline's **free** tune (limited usage, separate from the ClinePass quota). Reasoning model dispatched **only at `--thinking xhigh`** (its top tier; no `max` here). Config-only provider, not a Pi builtin; context 1M, output 131K. Three-segment reference (model `id` = `x-ai/ox-alpha`, the **`x-ai/` vendor prefix**, not `cline-pass/`); live dispatch verified 2026-08-24 with the stored `cline-pass` credential (real `PONG`). The **same underlying model** as `openrouter/stealth/ox-alpha` (packet 052), reached through a different provider — pick the route deliberately. Also wired into the deep-loop cli-pi fan-out roster |
