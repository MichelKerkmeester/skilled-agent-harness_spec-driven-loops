---
title: "EC-010 -- cli-pi partial lineage death"
description: "Hermetic operator check for cli-pi:EC-010: reports a signal-killed cli-pi lineage as failed."
matrix_cell: "cli-pi:EC-010"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-pi:EC-010`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-pi:EC-010` ("reports a signal-killed cli-pi lineage as failed"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-pi.vitest.ts` reports `reports a signal-killed cli-pi lineage as failed` as passing under the shipped stress-matrix
  cell `cli-pi:EC-010`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-010 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `reports a signal-killed cli-pi lineage as failed` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-010 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "reports a signal-killed cli-pi lineage as failed" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `reports a signal-killed cli-pi lineage as failed` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-pi:EC-010` |
