...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-pi:EC-006`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-pi:EC-006` ("passes child gate variables through the full cli-pi runtime"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-pi.vitest.ts` reports `passes child gate variables through the full cli-pi runtime` as passing under the shipped stress-matrix
  cell `cli-pi:EC-006`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-006 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `passes child gate variables through the full cli-pi runtime` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-006 and report PASS, FAIL, or SKIP with the exact blocker.`
...
The command exits `0`, and the verbose Vitest reporter names `passes child gate variables through the full cli-pi runtime` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-pi:EC-006` |
