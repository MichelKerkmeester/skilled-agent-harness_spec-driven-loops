---
title: "EC-013 -- cli-pi node modules integrity"
description: "Hermetic operator check for cli-pi:EC-013: preserves independent worktree node_modules boundaries during cli-pi dispatch."
matrix_cell: "cli-pi:EC-013"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-pi:EC-013`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-pi:EC-013` ("preserves independent worktree node_modules boundaries during cli-pi dispatch"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-pi.vitest.ts` reports `preserves independent worktree node_modules boundaries during cli-pi dispatch` as passing under the shipped stress-matrix
  cell `cli-pi:EC-013`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-013 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `preserves independent worktree node_modules boundaries during cli-pi dispatch` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-013 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "preserves independent worktree node_modules boundaries during cli-pi dispatch" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `preserves independent worktree node_modules boundaries during cli-pi dispatch` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-pi:EC-013` |
