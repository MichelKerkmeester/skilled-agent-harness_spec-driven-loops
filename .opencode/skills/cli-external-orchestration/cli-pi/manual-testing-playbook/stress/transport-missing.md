---
title: "EC-008 -- cli-pi missing transport"
description: "Hermetic operator check for cli-pi:EC-008: fails before cli-pi execution when its transport is unavailable."
matrix_cell: "cli-pi:EC-008"
test_file: "tests/stress/cli-adapter/cli-pi.vitest.ts"
test_name: "fails before cli-pi execution when its transport is unavailable"
playbook_path: "cli-external-orchestration/cli-pi/manual-testing-playbook/stress/transport-missing.md"
id: "cli-pi-EC-008"
version: 1.0.0.0
---

# EC-008 -- cli-pi missing transport

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-pi:EC-008`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-pi:EC-008` ("fails before cli-pi execution when its transport is unavailable"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-pi.vitest.ts` reports `fails before cli-pi execution when its transport is unavailable` as passing under the shipped stress-matrix
  cell `cli-pi:EC-008`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-008 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `fails before cli-pi execution when its transport is unavailable` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-008 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "fails before cli-pi execution when its transport is unavailable" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `fails before cli-pi execution when its transport is unavailable` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
- **ledger/artifacts**: A non-zero result and no cli-pi provider capture when its transport is removed from `PATH`.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-pi:EC-008` |
