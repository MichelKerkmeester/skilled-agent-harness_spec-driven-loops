---
title: "EC-007 -- cli-pi sandbox or permission"
description: "Hermetic operator check for cli-pi:EC-007: forwards the asserted cli-pi sandbox and permission flags."
matrix_cell: "cli-pi:EC-007"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-pi:EC-007`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-pi:EC-007` ("forwards the asserted cli-pi sandbox and permission flags"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-pi.vitest.ts` reports `forwards the asserted cli-pi sandbox and permission flags` as passing under the shipped stress-matrix
  cell `cli-pi:EC-007`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-007 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `forwards the asserted cli-pi sandbox and permission flags` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-pi:EC-007 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "forwards the asserted cli-pi sandbox and permission flags" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `forwards the asserted cli-pi sandbox and permission flags` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
- **ledger/artifacts**: A read-only command succeeds with the restricted tools list and disables extensions, skills, and prompt templates.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-pi:EC-007` |
