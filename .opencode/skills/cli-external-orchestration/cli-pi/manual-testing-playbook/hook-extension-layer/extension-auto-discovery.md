...
This scenario checks that seven project-local extension factories load from `.pi/extensions/` without an extension path in `.pi/settings.json`.
...
- Expected signals: Seven extension files exist; settings contains packages but no extension path; live startup reaches `No API key found for the selected model` without an extension factory error.
...
| File | Role |
|---|---|
| `.pi/extensions/*.ts` | Seven project-local extension factories |
