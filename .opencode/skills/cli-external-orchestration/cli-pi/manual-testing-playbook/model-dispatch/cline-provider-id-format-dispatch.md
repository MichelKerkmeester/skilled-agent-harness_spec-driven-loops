...
- Expected execution process: Grep `.pi/models.json` for the cline-pass model ids -> confirm both are slashed (`cline-pass/deepseek-v4-flash`, `cline-pass/deepseek-v4-pro`), not bare -> confirm `.pi/settings.json` `enabledModels` carry the three-segment `cline-pass/cline-pass/...` references and `defaultModel` is `cline-pass/deepseek-v4-flash` -> if credentials exist, run the isolated positive-control dispatch and inspect the output text.
...
- Confirm `cline-pass/cline-pass/deepseek-v4-pro` dispatches under the same contract when a Pro turn is needed.
