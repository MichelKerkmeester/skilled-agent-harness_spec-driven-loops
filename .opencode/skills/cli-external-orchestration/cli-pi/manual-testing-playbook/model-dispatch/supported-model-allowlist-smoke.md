...
- Objective: Confirm the seven real Pi model IDs and prove `PI_DEFAULT_MODEL` is not `auto`.
...
- Expected signals: Seven listed IDs are present; default is `deepseek-v4-pro`; no `auto` value occurs in the allowlist/default; a live smoke response is optional only when credentials exist.
