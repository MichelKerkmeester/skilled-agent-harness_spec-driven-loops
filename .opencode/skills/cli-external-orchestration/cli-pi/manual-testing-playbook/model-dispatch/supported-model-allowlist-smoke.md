...
- Objective: Confirm the eleven real Pi model IDs and prove `PI_DEFAULT_MODEL` is not `auto`.
...
- Expected signals: Eleven listed IDs are present; default is `deepseek-v4-flash`; no `auto` value occurs in the allowlist/default; a live smoke response is optional only when credentials exist.
