---
title: "EC-012 -- cli-opencode worktree collision"
description: "Hermetic operator check for cli-opencode:EC-012: runs cli-opencode from each isolated worktree cwd."
matrix_cell: "cli-opencode:EC-012"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-opencode:EC-012`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-opencode:EC-012` ("runs cli-opencode from each isolated worktree cwd"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-opencode.vitest.ts` reports `runs cli-opencode from each isolated worktree cwd` as passing under the shipped stress-matrix
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-opencode:EC-012 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "runs cli-opencode from each isolated worktree cwd" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `runs cli-opencode from each isolated worktree cwd` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-opencode:EC-012` |
...
- Group: Stress Matrix
- Playbook ID: cli-opencode-EC-012
