---
title: "EC-003 -- cli-opencode rate limit"
description: "Hermetic operator check for cli-opencode:EC-003: retains cli-opencode rate-limit diagnostics without retry delay."
matrix_cell: "cli-opencode:EC-003"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-opencode:EC-003`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-opencode:EC-003` ("retains cli-opencode rate-limit diagnostics without retry delay"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-opencode.vitest.ts` reports `retains cli-opencode rate-limit diagnostics without retry delay` as passing under the shipped stress-matrix
  cell `cli-opencode:EC-003`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-opencode:EC-003 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `retains cli-opencode rate-limit diagnostics without retry delay` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-opencode:EC-003 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "retains cli-opencode rate-limit diagnostics without retry delay" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `retains cli-opencode rate-limit diagnostics without retry delay` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-opencode:EC-003` |
...
- Group: Stress Matrix
- Playbook ID: cli-opencode-EC-003
