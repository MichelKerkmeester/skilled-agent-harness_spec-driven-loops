---
title: "EC-008 -- cli-opencode missing transport"
description: "Hermetic operator check for cli-opencode:EC-008: fails before cli-opencode execution when its transport is unavailable."
matrix_cell: "cli-opencode:EC-008"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-opencode:EC-008`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-opencode:EC-008` ("fails before cli-opencode execution when its transport is unavailable"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-opencode.vitest.ts` reports `fails before cli-opencode execution when its transport is unavailable` as passing under the shipped stress-matrix
  cell `cli-opencode:EC-008`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-opencode:EC-008 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `fails before cli-opencode execution when its transport is unavailable` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-opencode:EC-008 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "fails before cli-opencode execution when its transport is unavailable" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `fails before cli-opencode execution when its transport is unavailable` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
- **ledger/artifacts**: A non-zero result and no cli-opencode provider capture when its transport is removed from `PATH`.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-opencode:EC-008` |
...
- Group: Stress Matrix
- Playbook ID: cli-opencode-EC-008
