...
The cli-opencode skill defaults to `--format json` because external runtimes parse the stream incrementally (per ALWAYS rule 4 in SKILL.md and `references/cli-reference.md` §7). If `--format default` and `--format json` are not both functional, every downstream pattern that relies on the JSON event schema (CO-003 through CO-031) silently degrades. This test proves both surfaces work for the same prompt.
...
| `../../references/cli-reference.md` (§7 Output Format and Event Stream) | JSON event schema and line-delimited contract |
...
| `../../references/cli-reference.md` | §4 `opencode run` flag table, §7 event stream documentation |
