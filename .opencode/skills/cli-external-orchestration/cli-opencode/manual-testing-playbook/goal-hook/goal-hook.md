...
description: "This scenario validates the OpenCode-native mk-goal plugin for `CO-039`. It focuses on the /goal:goal-opencode action set, mk_goal/mk_goal_status tool behavior, per-OpenCode-session state, native token accounting, and the experimental.chat.system.transform injection, driven directly in-process against the shipped plugin."
version: 1.0.0.0
---
...
- Prompt: `As an OpenCode plugin validator, exercise the mk-goal plugin's tool, event, and experimental.chat.system.transform hooks directly against a scratch session-state directory. Verify the full /goal:goal-opencode action set (set, show, history, doctor, health, pause, resume, complete, clear) returns the documented STATUS envelope, per-session goal state stays isolated by session id, native OpenCode token accounting attributes tokens correctly from a message.updated event, and the transform renders a well-formed [active_goal] injection block. Return the exact envelope lines and a PASS/FAIL verdict.`
...
|| `.opencode/commands/goal/goal-opencode.md` | The `/goal:goal-opencode` command router whose action set this plugin implements |
