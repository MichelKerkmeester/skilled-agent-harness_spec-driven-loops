...
This scenario validates External dispatch from Claude Code into OpenCode for `CO-006`. It focuses on confirming the canonical use case 1 path documented in `references/integration-patterns.md` §2: an external Anthropic runtime (Claude Code) dispatches into a fresh OpenCode session that loads the full plugin / skill / MCP / Spec Kit Memory runtime in a single one-shot invocation.
