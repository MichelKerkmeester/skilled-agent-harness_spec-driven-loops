...
- Expected signals: Layer 1 (env var) detection trips on `OPENCODE_CONFIG_DIR`. The documented refusal message in `references/integration-patterns.md` §5 surfaces with all three remediation options (sibling cli-* skill, fresh shell, parallel-session keywords). No actual `opencode run` dispatch is constructed.
...
4. Confirm the refusal message text in `references/integration-patterns.md` §5 includes all three remediation options.
...
| `../../references/integration-patterns.md` (§5 SMART-ROUTER DECISION TREE + REFUSAL MESSAGE) | Documents the layered detection and exact refusal text |
...
| `../../references/integration-patterns.md` | §5 decision tree + refusal message body |
...
- Feature file path: `external-dispatch/self-invocation-refusal.md`
