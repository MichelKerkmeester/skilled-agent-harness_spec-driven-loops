...
**Core Principle**: The calling AI stays the conductor. Delegate to OpenCode for what it does best — the full plugin, skill, and MCP runtime in a one-shot dispatch. Validate and integrate the output.
...
- **Full plugin / skill / MCP runtime** (use case 1) — calling AI is Claude Code / OpenCode / Copilot / raw shell AND the task needs the project's spec folders and continuity docs, or every plugin/skill/MCP tool in a one-shot dispatch. Includes `@deep-research` / `@deep-review` agent loops with externalized state under `.opencode/specs/`.
...
INTENT_SIGNALS = {
    ...
    "CROSS_AI_HANDBACK":  {"weight": 4, "keywords": ["spec kit", "spec-kit", "spec_kit", "code graph", "skill advisor", "cross-ai handback", "hand back to opencode", "bridge back to opencode", "opencode as a bridge", "non-anthropic cli", "project's spec folders", "structural code index", "use the skill advisor"]},
...
Run `opencode providers list` to confirm credentials and `opencode models <provider>` for live choices. Default: `opencode-go/deepseek-v4-flash --variant max` (Go gateway; flash is max-tier-pinned). Common alternates (all omit `--agent`): `opencode-go/glm-5.3-flash`, `opencode-go/qwen3.8-max`, `minimax/MiniMax-M3`, `xiaomi/mimo-v2.5-pro` (+`xiaomi/mimo-v2.5-pro-ultraspeed`), the GPT-5.6 sol/luna slugs (via the `openai` provider), the OpenRouter-routed `openrouter/deepseek/deepseek-v4-flash-latest` (OpenRouter routes exactly three models: DeepSeek V4 Flash, `openrouter/z-ai/glm-5.3-flash`, and `openrouter/google/gemini-3.8-flash` — no other), the Cline-routed `cline-pass/cline-pass/deepseek-v4-flash`, and the five DevPass routes on the `llmgateway` provider (`llmgateway/deepseek-v4-flash`, `…/deepseek-v4-flash-vision-exp` — one of three image-capable entries, with Luna and Gemini, `…/glm-5.3-flash`, `…/gpt-5.6-luna`, `…/gemini-3.8-flash`; **bare two-segment ids**, the inverse of the Cline form) — full roster, GPT-5.6 slug grid, OpenRouter + Cline rows, per-provider `--variant` mapping, and model-specific operational caveats in [references/providers-and-models.md](references/providers-and-models.md). **Closed roster: only models catalogued there may be dispatched — any model not in that catalog is FORBIDDEN for cli-opencode** (a hard discipline rule; cli-opencode has no code-enforced allowlist).
...
3. Pin model + variant + format + dir explicitly — **no `--agent`** (see the Default Invocation note: current opencode rejects a top-level `--agent general`; put any agent-profile request in the prompt body). Default: `--model opencode-go/deepseek-v4-flash --variant max --format json --dir <repo-root>`. Honor user overrides verbatim (e.g. `minimax/MiniMax-M3`, `xiaomi/mimo-v2.5-pro`, `xiaomi/mimo-v2.5-pro-ultraspeed`, `openai/gpt-5.6-sol-pro`, `openrouter/deepseek/deepseek-v4-flash-latest`, `cline-pass/cline-pass/deepseek-v4-flash`). OpenRouter routes exactly three models — DeepSeek V4 Flash, `openrouter/z-ai/glm-5.3-flash`, and `openrouter/google/gemini-3.8-flash`; do not route any other (e.g. GPT-5.6 Luna) through `openrouter/`.
...
7. **Prompt construction & model-craft (cli-* family precedence).** Compose every dispatch prompt via the 2-tier rule canonical in `../../sk-prompt/assets/cli-prompt-quality-card.md`:
   1. **Fast path (default).** Build from the local `assets/prompt-quality-card.md`, which delegates the framework table + CLEAR check to the canonical card. Framework selection uses the canonical card's cross-model defaults; there are no per-model prompt-craft profiles.
