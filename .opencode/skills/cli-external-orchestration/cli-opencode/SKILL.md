...
**Six providers are reachable:** `opencode-go` (default), `openai` (the GPT-5.6 sol/luna grid), `cline-pass` (Cline Pass), `llmgateway` (**DevPass**, the operator's LLM Gateway plan, metered per token with a 3x credit bonus), `minimax` and `xiaomi`. Which models each one carries, their thinking ceilings, and their per-provider caveats live in [references/providers-and-models.md](references/providers-and-models.md) — that file is the roster, and this one deliberately does not restate it, because two copies of a model list is how the two disagree.
