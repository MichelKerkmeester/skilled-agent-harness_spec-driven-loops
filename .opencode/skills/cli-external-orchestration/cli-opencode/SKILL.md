...
3. Pin model + variant + format + dir explicitly — **no `--agent`** (see the Default Invocation note: current opencode rejects a top-level `--agent general`; put any agent-profile request in the prompt body). Default: `--model deepseek/deepseek-v4-pro --variant high --format json --dir <repo-root>`. Honor user overrides verbatim (e.g. `deepseek/deepseek-v4-pro`, `minimax/MiniMax-M3`, `xiaomi/mimo-v2.5-pro`, `xiaomi/mimo-v2.5-pro-ultraspeed`, `openai/gpt-5.6-sol-pro`, `openrouter/deepseek/deepseek-v4-flash-latest`, `cline-pass/cline-pass/deepseek-v4-flash`). OpenRouter routes DeepSeek V4 Flash only — do not route another model (e.g. GPT-5.6 Luna) through `openrouter/`.
