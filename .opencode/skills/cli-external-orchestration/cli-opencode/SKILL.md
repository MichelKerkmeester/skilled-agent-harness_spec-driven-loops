...
Run `opencode providers list` to confirm credentials and `opencode models <provider>` for live choices. Default: `deepseek/deepseek-v4-pro --variant high` (direct DeepSeek API). Common alternates (all omit `--agent`): `deepseek/deepseek-v4-flash`, `minimax/MiniMax-M3`, `xiaomi/mimo-v2.5-pro` (+`xiaomi/mimo-v2.5-pro-ultraspeed`), the GPT-5.6 sol/terra/luna slugs, and the OpenRouter-routed `openrouter/deepseek/deepseek-v4-flash-latest` / `openrouter/openai/gpt-5.6-luna` — full roster, GPT-5.6 slug grid, OpenRouter rows, per-provider `--variant` mapping, and model-specific operational caveats in [references/providers-and-models.md](references/providers-and-models.md).
...
3. Pin model + variant + format + dir explicitly — **no `--agent`** (see the Default Invocation note: current opencode rejects a top-level `--agent general`; put any agent-profile request in the prompt body). Default: `--model deepseek/deepseek-v4-pro --variant high --format json --dir <repo-root>`. Honor user overrides verbatim (e.g. `deepseek/deepseek-v4-pro`, `minimax/MiniMax-M3`, `xiaomi/mimo-v2.5-pro`, `xiaomi/mimo-v2.5-pro-ultraspeed`, `openai/gpt-5.6-sol-pro`, `openrouter/deepseek/deepseek-v4-flash-latest`, `openrouter/openai/gpt-5.6-luna`).
