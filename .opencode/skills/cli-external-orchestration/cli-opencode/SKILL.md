...
Run `opencode providers list` to confirm credentials and `opencode models <provider>` for live choices. Default: `deepseek/deepseek-v4-pro --variant high` (direct DeepSeek API). Common alternates (all omit `--agent`): `deepseek/deepseek-v4-flash`, `minimax/MiniMax-M3`, `xiaomi/mimo-v2.5-pro` (+`xiaomi/mimo-v2.5-pro-ultraspeed`), and the GPT-5.6 sol/terra/luna slugs — full roster, GPT-5.6 slug grid, per-provider `--variant` mapping, and model-specific operational caveats in [references/providers-and-models.md](references/providers-and-models.md).
...
13. **Design Standards Loading (surface-aware contract)** — When dispatching for design or UI work, instruct the dispatched session to: (1) load `sk-design` (the hub); (2) let the hub resolve a `workflowMode` through `mode-registry.json` (`sk-design-interface` / `sk-design-md-generator` / `sk-design-mcp-open-design`); (3) load the selected mode packet, set the design register, and run that mode's design verification; (4) if the work feeds Open Design, carry the `sk-design-mcp-open-design` pairing (a nested transport packet of the hub) — the transport never decides taste. Fallback: if the design mode cannot be determined confidently, ask for the surface and design intent. NEVER treat `mcp-figma` or `sk-design-mcp-open-design` as the taste authority, or hardcode obsolete flat design skills in dispatch prompts.
