...
Build the prompt directly from the canonical card (`../../../sk-prompt/sk-prompt-models/assets/cli-prompt-quality-card.md`). Select a framework from the table in §2, apply the task-to-framework map in §3, run the CLEAR pre-dispatch check in §4, and dispatch. No additional skill loading required for routine work.

**Tier 2 — Model override (mandatory when a per-model profile exists)**
If the target Devin model has a per-model profile at `../../../sk-prompt/sk-prompt-models/references/models/<id>.md`, that profile OVERRIDES the cross-model defaults from the canonical card. The **sk-prompt/sk-prompt-models** packet owns per-model prompt-craft (framework + scaffold + gotchas, mirroring `sk-prompt/sk-prompt-models/assets/model-profiles.json` `recommended_frameworks`); consult it before composing for any small model dispatched through Devin (e.g. SWE-1.6, DeepSeek, Kimi, GLM).

**Tier 3 — Deep path (escalation)**
Dispatch `@prompt-improver` via the Task tool (never load full `sk-prompt` inline) when any canonical **Tier 3** trigger applies — the trigger list lives in `../../../sk-prompt/sk-prompt-models/assets/cli-prompt-quality-card.md` under "Tier 3 — Deep path"; do not re-enumerate it here.
...
| `swe` / `swe-1-6-fast` | **Yes — consult sk-prompt/sk-prompt-models** | Cognition SWE model; a profiled small model that needs model-specific prompt-craft |
| `deepseek` | **Yes — consult sk-prompt/sk-prompt-models** | Open-source model; profiled small model |
| `kimi` | **Yes — consult sk-prompt/sk-prompt-models** | Open-source model; profiled small model |
| `glm` | **Yes — consult sk-prompt/sk-prompt-models** | Open-source model; profiled small model |

When dispatching a profiled small model (SWE-1.6, DeepSeek, Kimi, GLM), read the model's profile at `../../../sk-prompt/sk-prompt-models/references/models/<id>.md` before composing the prompt. Skipping this step is the leading cause of underperformance on profiled models.

---

## 5. DELEGATION / PRECEDENCE

The 3-tier precedence rule (fast path → model override → deep path) is canonical in `../../../sk-prompt/sk-prompt-models/assets/cli-prompt-quality-card.md` and restated in §2 above and in `../SKILL.md`.
