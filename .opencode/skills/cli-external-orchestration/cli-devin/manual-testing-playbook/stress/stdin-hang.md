...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-devin:EC-005`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-devin:EC-005` ("closes stdin for the headless cli-devin process"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-devin.vitest.ts` reports `closes stdin for the headless cli-devin process` as passing under the shipped stress-matrix
  cell `cli-devin:EC-005`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-devin:EC-005 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `closes stdin for the headless cli-devin process` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-devin:EC-005 and report PASS, FAIL, or SKIP with the exact blocker.`
...
The command exits `0`, and the verbose Vitest reporter names `closes stdin for the headless cli-devin process` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-devin:EC-005` |
