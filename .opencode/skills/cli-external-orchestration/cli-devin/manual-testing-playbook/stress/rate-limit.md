---
title: "EC-003 -- cli-devin rate limit"
description: "Hermetic operator check for cli-devin:EC-003: retains cli-devin rate-limit diagnostics without retry delay."
matrix_cell: "cli-devin:EC-003"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-devin:EC-003`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-devin:EC-003` ("retains cli-devin rate-limit diagnostics without retry delay"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-devin.vitest.ts` reports `retains cli-devin rate-limit diagnostics without retry delay` as passing under the shipped stress-matrix
  cell `cli-devin:EC-003`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-devin:EC-003 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `retains cli-devin rate-limit diagnostics without retry delay` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-devin:EC-003 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "retains cli-devin rate-limit diagnostics without retry delay" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `retains cli-devin rate-limit diagnostics without retry delay` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-devin:EC-003` |
