---
title: "EC-006 -- cli-devin child spec gate"
description: "Hermetic operator check for cli-devin:EC-006: passes child gate variables through the full cli-devin runtime."
matrix_cell: "cli-devin:EC-006"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-devin:EC-006`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-devin:EC-006` ("passes child gate variables through the full cli-devin runtime"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-devin.vitest.ts` reports `passes child gate variables through the full cli-devin runtime` as passing under the shipped stress-matrix
  cell `cli-devin:EC-006`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-devin:EC-006 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `passes child gate variables through the full cli-devin runtime` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-devin:EC-006 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "passes child gate variables through the full cli-devin runtime" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `passes child gate variables through the full cli-devin runtime` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-devin:EC-006` |
