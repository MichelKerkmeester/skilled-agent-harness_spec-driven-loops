---
title: "EC-002 -- cli-claude-code model or balance failure"
description: "Hermetic operator check for cli-claude-code:EC-002: retains cli-claude-code model-or-balance diagnostics in bounded lineage output."
matrix_cell: "cli-claude-code:EC-002"
test_file: "tests/stress/cli-adapter/cli-claude-code.vitest.ts"
test_name: "retains cli-claude-code model-or-balance diagnostics in bounded lineage output"
playbook_path: "cli-external-orchestration/cli-claude-code/manual-testing-playbook/stress/model-or-balance.md"
id: "cli-claude-code-EC-002"
version: 1.0.0.0
---

# EC-002 -- cli-claude-code model or balance failure

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-claude-code:EC-002`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-claude-code:EC-002` ("retains cli-claude-code model-or-balance diagnostics in bounded lineage output"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-claude-code.vitest.ts` reports `retains cli-claude-code model-or-balance diagnostics in bounded lineage output` as passing under the shipped stress-matrix
  cell `cli-claude-code:EC-002`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-002 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `retains cli-claude-code model-or-balance diagnostics in bounded lineage output` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-002 and report PASS, FAIL, or SKIP with the exact blocker.`
...
The command exits `0`, and the verbose Vitest reporter names `retains cli-claude-code model-or-balance diagnostics in bounded lineage output` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-claude-code:EC-002` |
...
- Group: Stress Matrix
- Playbook ID: cli-claude-code-EC-002
