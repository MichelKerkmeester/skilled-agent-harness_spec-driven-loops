---
title: "EC-005 -- cli-claude-code stdin closure"
description: "Hermetic operator check for cli-claude-code:EC-005: closes stdin for the headless cli-claude-code process."
matrix_cell: "cli-claude-code:EC-005"
...
This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-claude-code:EC-005`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-claude-code:EC-005` ("closes stdin for the headless cli-claude-code process"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-claude-code.vitest.ts` reports `closes stdin for the headless cli-claude-code process` as passing under the shipped stress-matrix
  cell `cli-claude-code:EC-005`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-005 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `closes stdin for the headless cli-claude-code process` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-005 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "closes stdin for the headless cli-claude-code process" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `closes stdin for the headless cli-claude-code process` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-claude-code:EC-005` |
...
- Group: Stress Matrix
- Playbook ID: cli-claude-code-EC-005
