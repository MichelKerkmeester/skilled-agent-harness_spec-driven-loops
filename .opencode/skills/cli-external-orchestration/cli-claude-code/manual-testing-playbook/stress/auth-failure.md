---
title: "EC-001 -- cli-claude-code authentication failure"
description: "Hermetic operator check for cli-claude-code:EC-001: retains cli-claude-code authentication diagnostics in bounded lineage output."
matrix_cell: "cli-claude-code:EC-001"
test_file: "tests/stress/cli-adapter/cli-claude-code.vitest.ts"
test_name: "retains cli-claude-code authentication diagnostics in bounded lineage output"
playbook_path: "cli-external-orchestration/cli-claude-code/manual-testing-playbook/stress/auth-failure.md"
id: "cli-claude-code-EC-001"
version: 1.0.0.0
---

# EC-001 -- cli-claude-code authentication failure

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-claude-code:EC-001`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-claude-code:EC-001` ("retains cli-claude-code authentication diagnostics in bounded lineage output"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-claude-code.vitest.ts` reports `retains cli-claude-code authentication diagnostics in bounded lineage output` as passing under the shipped stress-matrix
  cell `cli-claude-code:EC-001`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-001 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `retains cli-claude-code authentication diagnostics in bounded lineage output` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-001 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "retains cli-claude-code authentication diagnostics in bounded lineage output" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `retains cli-claude-code authentication diagnostics in bounded lineage output` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
- **ledger/artifacts**: A non-zero fan-out result and the bounded lineage log contains the cli-claude-code OAuth-unavailable diagnostic.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-claude-code:EC-001` |
...
- Group: Stress Matrix
- Playbook ID: cli-claude-code-EC-001
