---
title: "EC-011 -- cli-claude-code orphan cleanup"
description: "Hermetic operator check for cli-claude-code:EC-011: terminates the captured detached cli-claude-code process group after timeout."
matrix_cell: "cli-claude-code:EC-011"
test_file: "tests/stress/cli-adapter/cli-claude-code.vitest.ts"
test_name: "terminates the captured detached cli-claude-code process group after timeout"
playbook_path: "cli-external-orchestration/cli-claude-code/manual-testing-playbook/stress/orphan-cleanup.md"
id: "cli-claude-code-EC-011"
version: 1.0.0.0
---

# EC-011 -- cli-claude-code orphan cleanup

This document captures the hermetic stress-matrix contract, execution process, source anchors, and metadata for `cli-claude-code:EC-011`.

---

## 1. OVERVIEW

This scenario runs the manifest-indexed hermetic stress test for `cli-claude-code:EC-011` ("terminates the captured detached cli-claude-code process group after timeout"). It
...
- Objective: confirm `tests/stress/cli-adapter/cli-claude-code.vitest.ts` reports `terminates the captured detached cli-claude-code process group after timeout` as passing under the shipped stress-matrix
  cell `cli-claude-code:EC-011`.
- Operator prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-011 and report PASS, FAIL, or SKIP with the exact blocker.`
...
- Expected signals: the verbose Vitest reporter names the exact test `terminates the captured detached cli-claude-code process group after timeout` as passed, and the
...
Prompt: `Run the manifest-indexed hermetic stress test for cli-claude-code:EC-011 and report PASS, FAIL, or SKIP with the exact blocker.`
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "terminates the captured detached cli-claude-code process group after timeout" </dev/null
```

### Expected

The command exits `0`, and the verbose Vitest reporter names `terminates the captured detached cli-claude-code process group after timeout` as passed. Capture the verbose Vitest line for the exact test name and the final file/test count.
...
| [matrix-manifest.ts](../../../../system-deep-loop/runtime/tests/stress/cli-adapter/matrix-manifest.ts) | Stress matrix cell manifest defining `cli-claude-code:EC-011` |
...
- Group: Stress Matrix
- Playbook ID: cli-claude-code-EC-011
