---
title: "EC-009 -- fanout-run budget rejection"
description: "Hermetic operator check for fanout-run:EC-009: rejects lineage and aggregate budgets before provider process spawn."
matrix_cell: "fanout-run:EC-009"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-009`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "rejects lineage and aggregate budgets before provider process spawn" </dev/null
```
...
- **ledger/artifacts**: Both lineage and aggregate over-budget runs exit 3, create no provider captures, and record the matching budget-cap ledger events.
