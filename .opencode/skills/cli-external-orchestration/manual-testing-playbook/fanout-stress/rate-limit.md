---
title: "EC-003 -- fanout-run rate limit"
description: "Hermetic operator check for fanout-run:EC-003: rejects a rate-limited fan-out lineage without a retry spawn."
matrix_cell: "fanout-run:EC-003"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-003`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "rejects a rate-limited fan-out lineage without a retry spawn" </dev/null
```
...
- **ledger/artifacts**: Exit 3, exactly one provider capture, and no `retry_scheduled` ledger event.
