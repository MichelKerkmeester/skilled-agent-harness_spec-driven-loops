---
title: "EC-014 -- fanout-run self invocation"
description: "Hermetic operator check for fanout-run:EC-014: blocks same-kind recursive fan-out before provider process spawn."
matrix_cell: "fanout-run:EC-014"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-014`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "blocks same-kind recursive fan-out before provider process spawn" </dev/null
```
...
- **ledger/artifacts**: The dispatch audit denies the repeated executor kind at the stack layer; fan-out exits 3 and no provider capture exists.
