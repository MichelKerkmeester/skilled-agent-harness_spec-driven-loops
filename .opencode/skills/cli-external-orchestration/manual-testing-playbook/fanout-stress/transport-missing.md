---
title: "EC-008 -- fanout-run missing transport"
description: "Hermetic operator check for fanout-run:EC-008: fails before fan-out execution when the executor transport is unavailable."
matrix_cell: "fanout-run:EC-008"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-008`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "fails before fan-out execution when the executor transport is unavailable" </dev/null
```
...
- **ledger/artifacts**: Exit 3 and no provider capture when the executor transport is removed from `PATH`.
