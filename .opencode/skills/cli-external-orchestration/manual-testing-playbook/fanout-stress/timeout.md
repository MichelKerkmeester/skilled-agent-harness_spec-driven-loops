---
title: "EC-004 -- fanout-run timeout"
description: "Hermetic operator check for fanout-run:EC-004: times out and reaps the captured fan-out lineage process."
matrix_cell: "fanout-run:EC-004"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-004`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "times out and reaps the captured fan-out lineage process" </dev/null
```
...
- **ledger/artifacts**: Exit 3, at least one captured lineage PID, and every captured PID is dead after the bounded wait.
