---
title: "EC-013 -- fanout-run node modules integrity"
description: "Hermetic operator check for fanout-run:EC-013: preserves independent node_modules realpaths while fan-out is active."
matrix_cell: "fanout-run:EC-013"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-013`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "preserves independent node_modules realpaths while fan-out is active" </dev/null
```
...
- **ledger/artifacts**: Two concurrent fan-out runs succeed; each `node_modules` realpath is distinct, equals its owning worktree path, and remains inside that worktree.
