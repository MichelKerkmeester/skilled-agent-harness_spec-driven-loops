---
title: "EC-005 -- fanout-run stdin closure"
description: "Hermetic operator check for fanout-run:EC-005: closes stdin for the headless fan-out lineage process."
matrix_cell: "fanout-run:EC-005"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-005`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "closes stdin for the headless fan-out lineage process" </dev/null
```
...
- **ledger/artifacts**: Exit 0, one provider capture, and the captured stdin payload is empty.
