---
title: "EC-012 -- fanout-run worktree collision"
description: "Hermetic operator check for fanout-run:EC-012: preserves exclusive ownership when concurrent worktrees collide."
matrix_cell: "fanout-run:EC-012"
...
This snippet runs the manifest-indexed hermetic test for `fanout-run:EC-012`.
...
cd .opencode/skills/system-deep-loop/runtime && \
  ...
  -t "preserves exclusive ownership when concurrent worktrees collide" </dev/null
```
...
- **ledger/artifacts**: Two concurrent fan-out runs preserve distinct real CWDs; a pre-existing ownership claim remains with the first worktree and a second claim throws `EEXIST`.
