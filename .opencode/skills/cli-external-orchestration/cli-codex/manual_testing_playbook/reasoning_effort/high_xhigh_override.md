...
SKILL.md §3 + `integration_patterns.md` §5 explicitly recommend `high` / `xhigh` for architecture, security and complex planning. If these levels silently degrade to `medium` behavior, every routing recommendation that depends on depth (`@review`, `@ai-council`, `@deep-research`) loses its quality edge.
...
- Feature file path: `reasoning_effort/high_xhigh_override.md`
