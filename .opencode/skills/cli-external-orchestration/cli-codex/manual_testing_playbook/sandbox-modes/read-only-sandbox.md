...
- Expected execution process: Operator snapshots `git status --porcelain .opencode/skills/cli-external-orchestration/cli-codex/` before dispatch -> dispatches read-only analysis of SKILL.md -> captures stdout -> re-snapshots git status -> diffs the two snapshots.
...
|---|---|---|---|---|---|---|---|---|
| CX-005 | read-only sandbox (analysis) | Verify --sandbox read-only blocks writes during analysis | `Summarize the cli-codex SKILL.md with --sandbox read-only and confirm Codex exits 0 without modifying files.` | 1. `bash: git status --porcelain .opencode/skills/cli-external-orchestration/cli-codex/ > /tmp/cli-codex-cx005-pre.txt` -> 2. `codex exec --model gpt-5.5 -c model_reasoning_effort="medium" -c service_tier="fast" --sandbox read-only "@./.opencode/skills/cli-external-orchestration/cli-codex/SKILL.md Summarize this skill in three bullet points: (1) primary purpose, (2) the most critical ALWAYS rule, (3) the most critical NEVER rule." > /tmp/cli-codex-cx005.txt 2>&1` -> 3. `bash: cat /tmp/cli-codex-cx005.txt` -> 4. `bash: git status --porcelain .opencode/skills/cli-external-orchestration/cli-codex/ > /tmp/cli-codex-cx005-post.txt && diff /tmp/cli-codex-cx005-pre.txt /tmp/cli-codex-cx005-post.txt` | Step 1: pre-snapshot captured; Step 2: exit 0; Step 3: stdout contains 3 bullets covering purpose + ALWAYS rule + NEVER rule; Step 4: pre/post snapshots are identical (no diff output) | Pre-snapshot, post-snapshot, captured stdout, dispatched command line, exit code | PASS if exit 0 AND structured 3-bullet summary present AND pre/post snapshots match AND `--sandbox read-only` is in the dispatched command; FAIL if any check misses or pre/post diff is non-empty | (1) Confirm SKILL.md path resolves; (2) re-run with `2>&1 \| tee` for stderr inline; (3) inspect `/tmp/cli-codex-cx005.txt` for any "permission denied" message |
...
- Feature file path: `02--sandbox-modes/read-only-sandbox.md`
