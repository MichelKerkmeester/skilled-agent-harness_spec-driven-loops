...
The exact reproducible command sequence is the 9-column table's "Exact Command Sequence" cell
below: registration grep, scratch-repo setup, the shared-hook dispatch, and the suppressed re-run.
...
`.codex/hooks.json` excerpt, captured advisory JSON, silence confirmation, and the terminal
...
- **Pass**: the advisory names `commit-scope-drops-untracked` AND no denial field is present AND
  the suppressed re-run (step 4) prints nothing.
