...
- Expected execution process: Operator inspects `~/.codex/hooks.json` -> runs the documented manual smoke checks from `hook-contract.md` §6 -> dispatches `codex --enable codex_hooks exec --full-auto` with the smoke-test prompt -> captures stdout -> verifies the hook scripts are reachable.
...
| `../../references/cli-reference.md` (§4 Command-Line Flags - --full-auto) | Documents --full-auto |
