...
This scenario validates native Codex hook integration for `CX-016` with the current `hooks` feature and an explicit headless approval policy. It confirms `codex --enable hooks` (or `[features].hooks = true` in config) runs the documented Spec Kit hooks at `SessionStart` and `UserPromptSubmit`, while the write-bearing dispatch runs through an authorized child.

### Why This Matters

`references/hook-contract.md` documents the native-hook surface (Codex CLI 0.122.0+) and Spec Kit's hook wiring at `.opencode/skills/system-spec-kit/runtime/dist/hooks/codex/{session-start,user-prompt-submit}.js`. If the hooks fail to load, every session in this skill loses startup context and the Skill Advisor brief - a major regression for the AGENTS.md Gate 2 routing surface.
...
- Objective: Verify `codex --enable hooks` plus `-c approval_policy=never --sandbox workspace-write` runs the Spec Kit hooks at SessionStart and UserPromptSubmit and that the advisor brief is injected.
- Real user request: `Confirm Codex's native hooks are wired into the Spec Kit hook contract and the skill advisor still surfaces.`
- RCAF Prompt: `Spec folder: /tmp/cli-codex-playbook (pre-approved, skip Gate 3). As a cross-AI orchestrator validating Codex hook parity, FIRST verify ~/.codex/hooks.json contains entries for SessionStart and UserPromptSubmit pointing at .opencode/skills/system-spec-kit/runtime/dist/hooks/codex/{session-start,user-prompt-submit}.js, THEN dispatch AI_SESSION_CHILD=1 SYSTEM_SPEC_GATE_ENFORCE=0 codex --enable hooks -c approval_policy=never --sandbox workspace-write exec "Implement a tiny TypeScript hook smoke test in /tmp/cli-codex-playbook-cx016/hook.ts" with --model gpt-5.6-luna -c service_tier="fast". Verify the hook stdout contract is satisfied (session-start emits {} or hookSpecificOutput.additionalContext; user-prompt-submit emits an Advisor: brief). Return a verdict naming the hook script paths and confirming the advisor brief surfaced.`
...
| `../../references/hook-contract.md` (§1 Activation, §2 Registration, §7 Spec Kit Hook Wiring) | Authoritative hook contract |
...
| `.opencode/skills/system-spec-kit/runtime/dist/hooks/codex/session-start.js` | Spec Kit SessionStart hook |
| `.opencode/skills/system-spec-kit/runtime/dist/hooks/codex/user-prompt-submit.js` | Spec Kit UserPromptSubmit hook |
