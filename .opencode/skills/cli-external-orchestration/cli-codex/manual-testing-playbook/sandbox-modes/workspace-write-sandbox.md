...
Workspace-write is the load-bearing sandbox for every code-generation, refactoring and test-generation flow in the skill (SKILL.md §4 ALWAYS rule 3, `references/cli-reference.md` §10). Without it, the documented "workspace-write for generation" pattern silently produces zero file changes.
...
- Prompt: `As an authorized child with AI_SESSION_CHILD=1 and MK_SPEC_GATE_ENFORCE=0, generate /tmp/cli-codex-playbook-cx006/hello.ts with workspace-write and confirm only that temp folder changed.`
- Expected execution process: Operator pre-creates the temp directory -> snapshots `git status --porcelain` for the workspace -> dispatches the write-bearing generation through production fanout or an authorized child with `AI_SESSION_CHILD=1` and `MK_SPEC_GATE_ENFORCE=0` -> verifies the file exists and has the expected content -> re-snapshots git status and confirms no unrelated files moved.
...
3. Dispatch the workspace-write generation prompt through production fanout or an authorized child. that names the target file.
...
| `../../references/cli-reference.md` (§10 Sandbox Modes) | Documents workspace-write semantics |

### Implementation And Test Anchors

| File | Role |
|---|---|
| `../../references/cli-reference.md` | §10 Sandbox Mode Values |
| `../../references/codex-tools.md` | §5 Sandbox Modes capability matrix |
...
- Feature file path: `sandbox-modes/workspace-write-sandbox.md`
