...
- Prompt: `As an authorized child with AI_SESSION_CHILD=1 and SYSTEM_SPEC_GATE_ENFORCE=0, generate /tmp/cli-codex-playbook-cx006/hello.ts with workspace-write and confirm only that temp folder changed.`
- Expected execution process: Operator pre-creates the temp directory -> snapshots `git status --porcelain` for the workspace -> dispatches the write-bearing generation through production fanout or an authorized child with `AI_SESSION_CHILD=1` and `SYSTEM_SPEC_GATE_ENFORCE=0` -> verifies the file exists and has the expected content -> re-snapshots git status and confirms no unrelated files moved.
