...
- RCAF Prompt: `Spec folder: /tmp/cli-codex-playbook (pre-approved, skip Gate 3). As a cross-AI orchestrator constructing a non-trivial dispatch, FIRST take a deliberately weak prompt ("Fix auth"), score it with the prompt-quality-card.md CLEAR 5-check (Correctness, Logic, Expression, Arrangement, Reusability), THEN escalate it to a structured prompt by applying the RCAF framework from the canonical `cli-prompt-quality-card.md` §3. Dispatch the improved prompt through an authorized child with AI_SESSION_CHILD=1 and MK_SPEC_GATE_ENFORCE=0 against /tmp/cli-codex-playbook-cx022/auth.ts with --model gpt-5.6-luna --sandbox workspace-write -c model_reasoning_effort="high" -c service_tier="fast". Verify the operator records the CLEAR scores for both versions, names the framework selected, and Codex produces a meaningfully better implementation from the improved prompt than the weak prompt would have. Return a verdict including both CLEAR score sets and the framework selected.`
