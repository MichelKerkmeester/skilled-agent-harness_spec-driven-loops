...
18. **Never dispatch `codex exec` from inside a running codex session.** A codex process cannot spawn another codex from inside its own sandbox. The nested child dies at startup with `failed to initialize in-process app-server client: Operation not permitted` (observed 2026-09-02 on codex-cli 0.151.0 under `--sandbox workspace-write`), and the parent reads that as a failed iteration rather than a route violation. So when codex is itself the executor of a deep-loop fan-out lineage or any other orchestrating session, it must do each iteration's work in-process with its own tools and never dispatch a second codex. Grep that exact message to confirm you are hitting this and not a model or network fault.
