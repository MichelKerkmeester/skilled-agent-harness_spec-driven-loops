...
| Executor / model | codex / gpt-5.6-luna (agent-routing) |
| CLI version | — |
| Flag state | `—` |
| Runtime digest | `—` |
| Run revision | — |
...
| -------- | ----- | ----- | ----- | ------------------- |
| CX-014 | manual | agent-routing | — | PASS: profile-v2 + Gate-3 preamble re-run: disposable home + auth + debug.config.toml; exit 0, file fixed (i<=arr.length -> i<arr.length), root cause cited |
