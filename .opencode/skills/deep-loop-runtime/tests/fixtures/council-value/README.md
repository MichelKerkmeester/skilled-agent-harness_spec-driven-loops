...
- The consumer test `tests/integration/council-graph-value-scenarios.vitest.ts` compares `graph.runtimeCalls` against `baseline.fileReads` and generates a performance metrics report at `tests/council-graph-value-report.json`.
