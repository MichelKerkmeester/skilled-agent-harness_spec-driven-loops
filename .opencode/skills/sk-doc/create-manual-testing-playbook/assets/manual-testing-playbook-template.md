...
├── manual-testing-playbook.md          # Root directory page, review rules, and orchestration guidance
...
```text
manual-testing-playbook/{CATEGORY_DIR}/{FEATURE_SLUG}.md
```
...
Copy this scaffold to create `{SKILL_PATH}/manual-testing-playbook/manual-testing-playbook.md`:
...
> **Catalog:** [{CATALOG_PATH_1}](../feature-catalog/{CATALOG_PATH_1})
...
> **Catalog:** [{CATALOG_PATH_2}](../feature-catalog/{CATALOG_PATH_2})
...
Copy this scaffold to create `manual-testing-playbook/{CATEGORY_DIR}/{FEATURE_SLUG}.md`. Both placeholder values must be kebab-case:
...
```markdown
Structure:
- [ ] Canonical playbook package exists in `manual-testing-playbook/`
- [ ] Category directory and per-feature filename values are kebab-case with no underscore path segments
