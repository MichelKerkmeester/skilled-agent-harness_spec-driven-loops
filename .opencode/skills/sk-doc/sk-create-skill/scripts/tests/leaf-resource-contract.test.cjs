...
function testDualReadDoesNotGenericallyStripUnrecognizedPrefixes() {
  ...
  const traversal = contract.dualReadLegacyResource({
    raw: '../../sk-create-skill/references/../../etc/passwd',
    declaredModes,
    aliasEntries: [],
  });
...
function testDualReadOfRealLegacyFixtureString() {
  ...
  const qualified = expectedResources.find((r) => r.startsWith('sk-create-quality-control/'));
  assert.ok(qualified, 'expected a sk-create-quality-control/-qualified legacy string in doc_quality.md gold');
  ...
  assert.equal(result.pair.workflowMode, 'sk-create-quality-control');
  assert.equal(result.pair.leafResourceId, qualified.slice('sk-create-quality-control/'.length));
...
function testQualifiedIdToLeafParsesAndResolves() {
  ...
  assert.equal(resolved.workflowMode, 'quality');
...
function testQualifiedIdToLeafFailsClosedOnOrphansAndMismatch() {
  ...
  assert.equal(orphan.code, 'ORPHAN_WORKFLOW_MODE');
  ...
  // Malformed and empty ids report a code rather than throwing.
  assert.equal(contract.qualifiedIdToLeaf('', { modeIndex }).code, 'EMPTY_QUALIFIED_ID');
  assert.equal(contract.qualifiedIdToLeaf('sk-code//packet/x/y', { modeIndex }).code, 'MALFORMED_QUALIFIED_ID');
}
