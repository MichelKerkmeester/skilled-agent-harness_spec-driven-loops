...
- **Path contract (nested packets):** inside a parent hub, a packet's `references/`/`assets/` paths become packet-qualified `leafResourceId`s in the hub's second-layer surface router at `shared/references/smart-routing.md`. A raw path converts to the canonical `(workflowMode, leafResourceId)` pair at exactly one boundary — the leaf-resource contract — via a packet-qualified prefix or an authored shared alias, never by generic prefix stripping. See [parent-hub-router-schema.md](../../references/parent-skill/parent-hub-router-schema.md) §8 and scaffold the surface router from [parent-skill-smart-routing-template.md](../parent-skill/parent-skill-smart-routing-template.md).
