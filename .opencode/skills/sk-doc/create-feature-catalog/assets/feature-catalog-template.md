...
```text
feature-catalog/
├── feature-catalog.md                 # Root inventory and summary catalog
...
Copy this scaffold to create `{SKILL_PATH}/feature-catalog/feature-catalog.md`:
...
Copy this scaffold to create `feature-catalog/{CATEGORY_DIR}/{FEATURE_SLUG}.md`. Both placeholder values must be kebab-case:
...
```markdown
Structure:
- [ ] `feature-catalog.md` exists with frontmatter and H1 intro paragraph
- [ ] Root catalog uses numbered all-caps H2 section headers (no Table of Contents)
- [ ] Category directories and per-feature files use descriptive kebab-case names such as `category-name/feature-name.md` (no numeric prefix)
...
- [manual-testing-playbook-template.md](../../../create-manual-testing-playbook/assets/manual-testing-playbook-template.md) - Parallel pattern for manual validation packages
