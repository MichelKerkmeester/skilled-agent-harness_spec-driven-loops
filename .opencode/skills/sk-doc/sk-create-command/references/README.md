...
Routing hub for the command-creation overflow set. `sk-create-command/SKILL.md` is the authoritative numbered workflow; the depth that would bloat it lives here as single-concern files, and this map points each concern to its file.
...
*End of Command Creation reference map — the authoritative workflow lives in `sk-create-command/SKILL.md`.*
