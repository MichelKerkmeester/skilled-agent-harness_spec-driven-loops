...
User types: /command-name arguments
                ...
                │
                └─► OpenCode finds .opencode/commands/command-name.md
                    │
...
- In `allowed-tools`, MCP tools use the fully-qualified `mcp__<server>__<tool>` form (e.g. `mcp__code_mode__call_tool_chain`), matching the `opencode.json` MCP namespace. Bare tool IDs (e.g. `advisor_recommend`) belong in instruction prose, never in `allowed-tools`.
...
Namespace and action segments must match `^[a-z0-9]+(?:-[a-z0-9]+)*$`. Reject underscore-bearing or ambiguous names, and keep each emitted `.md` stem identical to its validated action.
...
- [command-contract.json](command-contract.json) / [command-contract.schema.json](command-contract.schema.json) - Machine-readable behavioral contract and its schema
