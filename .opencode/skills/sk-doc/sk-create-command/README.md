...
description: "Turns a repeatable workflow into a validated OpenCode slash command with the frontmatter, input gates and router or presentation structure its type needs."
trigger_phrases:
  - "create command"
  - "slash command"
version: 1.0.2.0
...
> A repeatable workflow earns a repeatable entry point. create-command turns it into a validated slash command with the right frontmatter and gates for its type.
...
A slash command looks simple until you hit its edges. The `argument-hint` must summarize the invocation shape without ballooning past its budget. A required argument needs a gate that stops the command cold instead of letting the model guess at missing input. A mode-based command that grows dashboards, startup prompts, result templates and next-step wording inline turns into an unreadable router the moment someone adds a second mode. Get any of these wrong and the command either breaks silently or becomes the file nobody wants to touch.

### What It Does

create-command is the `sk-doc` workflow behind `/create:command`. It decides whether a command is even the right component, then classifies the command type from simple through router and namespace. Next it walks the file through frontmatter and mandatory input gates, then hands mode-based commands the router/presentation split. It does not own reusable domain knowledge or named runtime personas. Those live in `create-skill` and `create-agent`.

### The Command-Type Layer

| Type | What the skill produces |
|---|---|
| **Simple** | one `.md` file with direct steps for a single action |
| **Workflow** | one `.md` file with an overview, instructions and recovery for a multi-step process |
| **Mode-based** | `:auto` and `:confirm` routing, with a thin router plus owned assets when the command grows |
...
**Step 1: Decide the command type.** A one-off task does not need a command. A repeatable, multi-step workflow with defined steps does.
...
The full command-type table in `SKILL.md` lists seven shapes, summarized above. Pick the smallest one that fits. A single action with few arguments stays a simple command in one file. A destructive command, one that deletes data or makes an irreversible change, needs confirmation by default, an affected-state display before execution, execution logging and recovery guidance, whatever type it otherwise is.
...
Reach for create-command when a workflow is repeatable and needs `$ARGUMENTS` parsing, mode routing for `:auto` and `:confirm`, a confirmation gate for something destructive or a router/presentation split once display text outgrows the router body. Skip it for reusable domain knowledge with no invocation shape. Skip it too when the workflow only needs to run once.
...
| `create-skill` | Owns reusable knowledge and standards. A command stays executable and workflow-oriented while it links out to a skill rather than becoming a reference manual. |
| `sk-git` | Owns git branch, commit, merge and PR mechanics. A command can prepare content but never substitutes for git workflow ownership. |
...
| `allowed-tools` is broader than what runs | Tools were added in case they are needed later | List only the tools the command actually calls, add more only when a real step needs them |
...
A: A skill is reusable knowledge a session reads. A command is an executable entry point a user or session triggers directly, with its own arguments, gates, tool restrictions and execution contract. If nobody invokes it repeatably, it is not a command yet.

**Q: When do I need the router/presentation split?**

A: When a mode-based command grows dashboards, startup prompt sequences, result templates or next-step wording inline. Below that size, a single `.md` file with clear sections is simpler and easier to maintain.
...
A: The command still runs, but you have no proof the frontmatter, gates, section vocabulary and tool restrictions are correct until it breaks in a live session. Run the validator every time before calling a command done.
