...
**A sidecar file (`risk-analysis-rerank-nondeterminism.md`) carried the follow-on context** that did not fit inside the report's ten-section structure. This is the pattern for depth that does not fit the report shape: drop it next to `benchmark-report.md` as a focused sidecar with its own frontmatter and anchors.
