...
How the packet validator actually inspects a file, how to keep box widths consistent, the mistakes that most often fail validation, and the judgment the validator cannot supply. This is overflow detail; `../SKILL.md` §8 is the authoritative validator contract for exact thresholds and exit behavior, and `../scripts/validate_flowchart.sh` is the source of truth.
