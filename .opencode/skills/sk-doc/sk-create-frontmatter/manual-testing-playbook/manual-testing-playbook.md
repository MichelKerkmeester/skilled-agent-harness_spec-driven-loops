...
Two gates read this package and they disagree about one frontmatter key, so the shape is deliberate rather than incidental. Every scenario file carries `id`, `stage`, `expected_intent`, `expected_resources` and a block-form `expected_leaf_resources`, and none of them carries `expected_workflow_mode`.

The reason is that `validate-playbook-package.cjs` treats a file carrying a non-empty `expected_workflow_mode` **and** a valid `expected_leaf_resources` block as a routing-gold fixture and filters it out of the operator-scenario contract entirely. With that scalar present on every file, no file is left to validate, and the package reports `SKIP` with a zero exit rather than `PASS`. Dropping the scalar is the maximum that can be given up: the benchmark loader parses `expected_leaf_resources` independently of it, so every scenario keeps its full typed leaf gold and its `expected_resources`, and every file stays inside the operator contract where it belongs. This is an operator-scenario package, not a routing-gold fixture, so being classified as the former is also the correct outcome.

The cost is worth stating plainly. Without the scalar the loader leaves `scenario.expected` undefined, which leaves `requireRouteDeclaration` false in the live executor, so a run does not mark a missing route declaration as a gold miss. The mode itself is not lost: it is carried inside every typed pair as `workflow_mode`, and again as the prefix on every `expected_resources` entry. Do not re-add `expected_workflow_mode`. Adding it to one file silently removes that file from validation, and adding it to all of them returns the whole package to `SKIP` while still exiting zero, which no gate will report as a regression.
