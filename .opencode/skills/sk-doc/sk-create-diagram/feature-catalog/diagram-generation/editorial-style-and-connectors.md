...
This is the visual contract behind every diagram the packet produces. Colors and typography are referred to by semantic role and resolved in `references/foundations/style-guide.md`, the single source of truth for tokens, so changing that file re-skins every future diagram without touching type-specific logic. The caller is the agent drawing the diagram, and the failure modes are the ones the taste gate catches: off-grid coordinates, over-accented focal signals, and connectors that a reader cannot trace.
...
| `references/foundations/style-guide.md` | Reference | Anchor for the design-system rules the taste gate checks against |
