#!/usr/bin/env python3
# ───────────────────────────────────────────────────────────────
# COMPONENT: MERMAID IR EXTRACTOR
# ───────────────────────────────────────────────────────────────
...
@dataclass
class Node:
    """A node with optional container/parent/depth bookkeeping."""
...
@dataclass
class Edge:
    """A directed or undirected connection with style and arrowhead."""
...
@dataclass
class Diagram:
    """One parsed Mermaid diagram with nodes, edges, fragments, and notes."""
    ...
    def node_map(self) -> dict[str, Node]:
        """Return the id -> Node index for this diagram."""
    ...
    def add_node(
        ...
        """Add or update a node, merging labels, shapes, and parents.

        Args:
            node_id: The node identifier.
            label: The display label; ignored when empty or already set.
            shape: The normalized shape family.
            parent: Optional parent node id; sets depth and attachment.
            container: Whether the node is a container.

        Returns:
            The created or updated Node.

        Raises:
            SystemExit: With status 2 when MAX_NODES is exceeded.
        """
    ...
    def _depth_for(self, parent: str | None) -> int:
        """Return the depth for a parent node id (0 when parent is None)."""
        if parent is None:
            return 0
    ...
    def _attach(self, parent: str, child: str) -> None:
        """Attach a child id to a parent's children list if not already present."""
        parent_node = self._nodes_by_id.get(parent)
    ...
    def add_edge(
        ...
        """Append an edge to the diagram with an auto-incrementing id.

        Args:
            source: Source node id.
            target: Target node id.
            label: Optional edge label.
            style: solid, dashed, or thick.
            arrowhead: arrow, open, cross, or cardinality marker.
            bidirectional: Whether both ends carry markers.
            undirected: Whether the edge has no arrowheads.

        Returns:
            The created Edge.

        Raises:
            SystemExit: With status 2 when MAX_EDGES is exceeded.
        """
...
@dataclass
class SourceBlock:
    """One mermaid source block with its file-relative start line."""
...
def _read_bounded(path: Path) -> str:
    """Read the source file up to MAX_SOURCE_BYTES, decoding it as UTF-8."""
    try:
...
def load_blocks(path: Path) -> list[SourceBlock]:
    """Split a Mermaid or Markdown file into source blocks.

    Args:
        path: Path to a .mmd, .mermaid, or Markdown file with mermaid fences.

    Returns:
        A list of SourceBlock: one entry for .mmd/.mermaid, one per fenced block.

    Raises:
        SystemExit: With status 2 for unsupported files or missing/empty blocks.
    """
...
def _prepared_lines(block: SourceBlock) -> list[tuple[int, str]]:
    """Mask directives and frontmatter so the parser sees only diagram statements."""
...
def _kind_and_direction(
    ...
    """Return the diagram kind, declared direction, and header line position."""
...
def _split_top_level(text: str, delimiter: str) -> list[str]:
    """Split text on a delimiter only at top-level (unquoted/unbracketed) positions."""
...
def _parse_node_expression(expression: str) -> tuple[str, str, str] | None:
    """Parse one node expression into (node_id, label, shape) or None."""
...
@dataclass
class _Operator:
    """A link operator match with its label and style flags."""
...
def _operator_style(token: str) -> tuple[str, str, bool, bool]:
    """Map a link operator token to style, arrowhead, and direction flags."""
...
def _edge_operators(text: str) -> list[_Operator]:
    """Find link operators in a statement, carrying their inline labels."""
    mask = _top_level_mask(text)
...
def _endpoint_group(
    ...
    """Parse an ampersand-joined endpoint group into node identifiers or None."""
...
def _discard_nonsemantic(diagram: Diagram, text: str) -> bool:
    """Count and drop style directives and click handlers from a statement."""
...
def _parse_flowchart(
    ...
    """Parse flowchart/graph statements into nodes, edges, and containers."""
    containers: list[str] = []
...
def _parse_sequence(
    ...
    """Parse sequenceDiagram statements into participants, messages, and fragments."""
...
def _state_endpoint(
    ...
    """Resolve a state endpoint token to a node id, creating start/end markers."""
...
def _parse_state(
    ...
    """Parse stateDiagram-v2 statements into states and transitions."""
    containers: list[str] = []
    for line_number, raw in lines[header_position + 1 :]:
        text = raw.strip()
        if not text:
...
def _parse_er(
    ...
    """Parse erDiagram statements into entities, fields, and relationships."""
...
def parse_block(block: SourceBlock) -> Diagram:
    """Parse one source block into a Diagram by its detected grammar.

    Args:
        block: The source block to parse.

    Returns:
        A Diagram with nodes, edges, fragments, and analysis-ready degrees.
    """
...
def _finalize_degrees(diagram: Diagram) -> None:
    """Accumulate in/out degrees over all edges after parsing."""
...
def _has_cycle(nodes: list[Node], edges: list[Edge]) -> bool:
    """Detect a cycle in the edge graph via iterative DFS coloring."""
...
def shape_family(shape: str) -> str:
    """Return the family name for a normalized shape ("container" stays as-is)."""
    return "container" if shape == "container" else shape


def analyze(diagram: Diagram) -> dict[str, Any]:
    """Compute the structural signals the skill uses to pick a type and detail.

    Args:
        diagram: The parsed diagram to analyze.

    Returns:
        A dict with counts, hubs, type candidates, budget flags, and collapsible groups.
    """
...
def _escape_markdown(text: str) -> str:
    """Escape markdown-sensitive characters in digest output."""
    encoded = html.escape(text, quote=False)
    return re.sub(r"([\\`*{}\[\]()#+\-.!_|>])", r"\\\1", encoded)


def _escape_table(text: str) -> str:
    """Escape text for a Markdown table cell, flattening newlines."""
...
def digest(
    ...
    """Render a compact Markdown digest of the selected diagrams.

    Args:
        path: The source file path, used for the header line.
        diagrams: All diagrams in the file, used for the summary line.
        selected: The diagrams to render in detail.
        max_rows: Cap on rows per table.

    Returns:
        The Markdown digest as a string.
    """
...
def to_json(path: Path, diagrams: list[Diagram], selected: list[Diagram]) -> str:
    """Serialize the full IR of the selected diagrams as JSON.

    Args:
        path: The source file path, recorded in the payload.
        diagrams: All diagrams in the file.
        selected: The diagrams to serialize.

    Returns:
        A JSON string with analysis, nodes, edges, and fragments per diagram.
    """
    return json.dumps(
        {
            "source": str(path),
...
def select_diagrams(diagrams: list[Diagram], selector: str | None) -> list[Diagram]:
    """Resolve a --diagram selector to a concrete diagram list.

    Args:
        diagrams: All parsed diagrams.
        selector: None, "all", or a diagram index.

    Returns:
        The matching diagrams.

    Raises:
        SystemExit: With status 2 when the selector matches nothing.
    """
    if selector is None:
        return diagrams[:1]
    if selector == "all":
...
def main(argv: list[str] | None = None) -> int:
    """Run the Mermaid extractor CLI.

    Args:
        argv: Command-line arguments; defaults to sys.argv[1:].

    Returns:
        The process exit code (0 on success, 2 on unreadable input).
    """
