...
def _svg_embedded_xml(text: str) -> str | None:
    """Pull the ``mxfile`` XML out of a draw.io-exported SVG ``content`` attribute."""
...
def parse_style(style: str | None) -> dict[str, str]:
    """Parse a draw.io semicolon-separated style string into a key/value dict.

    Args:
        style: The raw style string, or None.

    Returns:
        A dict of style key to value; bare keys map to "1".
    """
    out: dict[str, str] = {}
    if not style:
        return out
...
def classify_shape(style: dict[str, str]) -> str:
    """Map a parsed style to a canonical shape name.

    Args:
        style: The parsed style dict from parse_style.

    Returns:
        The canonical shape name (e.g. "rect", "swimlane", "icon:aws").
    """
    raw = style.get("shape", "")
    if raw:
...
def shape_family(shape: str) -> str:
    """Return the family name for a canonical shape, stripping icon/shape prefixes.

    Args:
        shape: The canonical shape name.

    Returns:
        The part after "icon:" or "shape:", or the shape itself.
    """
...
@dataclass
class Node:
    """A vertex with absolute geometry resolved from its parent chain."""
...
@dataclass
class Edge:
    """A connection between two nodes with style and topology signals."""
...
@dataclass
class Page:
    """One draw.io diagram page: a list of nodes and edges."""
    ...
    def node_map(self) -> dict[str, Node]:
        """Return the id -> Node index for this page."""
        return {n.id: n for n in self.nodes}


def _num(geom: ET.Element | None, key: str) -> float:
    """Read a float attribute from an mxGeometry element, defaulting to 0.0."""
    if geom is None:
        return 0.0
    try:
...
def parse_page(diagram: ET.Element, index: int) -> Page:
    """Parse one ``<diagram>`` element into a Page of absolute-positioned nodes.

    Args:
        diagram: The ``<diagram>`` XML element (or a bare ``<mxGraphModel>``).
        index: The page's position in the file, used for naming.

    Returns:
        A Page with resolved nodes and edges; empty when no model is found.
    """
...
def parse_file(path: Path) -> list[Page]:
    """Decode any supported draw.io input into a list of pages.

    Args:
        path: Path to a .drawio, .xml, .drawio.png, or .drawio.svg file.

    Returns:
        A list of Page objects, one per diagram page.

    Raises:
        SystemExit: With status 2 for unsupported or unreadable input.
    """
...
def analyze(page: Page) -> dict[str, Any]:
    ...
    Args:
        page: The parsed page to analyze.
...
def page_bounds(page: Page) -> tuple[float, float, float, float]:
    """Return the (x0, y0, x1, y1) bounds of all positioned nodes."""
...
def digest(path: Path, pages: list[Page], selected: list[Page], max_rows: int) -> str:
    """Render a compact Markdown digest of the selected pages.

    Args:
        path: The source file path, used for the header line.
        pages: All pages in the file, used for the summary line.
        selected: The pages to render in detail.
...
def to_json(path: Path, pages: list[Page], selected: list[Page]) -> str:
    """Serialize the full IR of the selected pages as JSON.

    Args:
        path: The source file path, recorded in the payload.
        pages: All pages in the file.
        selected: The pages to serialize.

    Returns:
        A JSON string with bounds, analysis, nodes, and edges per page.
    """
    payload = {
        "source": str(path),
...
def select_pages(pages: list[Page], selector: str | None) -> list[Page]:
    """Resolve a --page selector to a concrete page list.

    Args:
        pages: All parsed pages.
        selector: None, "all", a page index, or a page name.
