...
How the packet validator actually inspects a file, how to keep box widths consistent, the mistakes that most often fail validation, and the judgment the validator cannot supply. This is overflow detail; `../../SKILL.md` §4 rule 8 is the authoritative validator requirement, and this file is the notation/mechanics detail, and `../../scripts/validate-flowchart.sh` is the source of truth.
...
The validator is lightweight and runs each check across the whole file, not per node. Knowing the mechanism explains why a diagram fails. For the exact numeric thresholds and exit behavior, see `../../SKILL.md` §4 rule 8 and the script itself.
...
- `../../SKILL.md` - authoritative workflow contract; §4 rule 8 for the validator requirement (this file carries the notation/mechanics detail directly, no separate SKILL.md notation section exists post-merge)
