...
description: "Parametric before-picture layout for legacy IT landscapes — orientation, zones, focal pain-points, file-based hand-off labels, and footer bars."
trigger_phrases:
  - "current it landscape"
  - "legacy modernization before"
  - "pain point diagram"
  - "file based handoffs"
  - "pre platform tooling"
  ...
  - name: "COLLECTION"
    components:              # 1..5 components per zone
      - id: survey-solutions
        ...
        sub:  "CAPI · PostgreSQL"
        icon: postgres               # any id from references/primitives/primitive-icons.md
...
- `components[i][k].icon` — any id from `references/primitives/primitive-icons.md`. If missing → no icon, the name shifts left. (Catalog has 41 icons; `mail` is currently missing — use `icon: file` as fallback for email hand-offs.)
...
**Icon placement** (24×24, monochrome via `currentColor` — see `references/primitives/primitive-icons.md`):
