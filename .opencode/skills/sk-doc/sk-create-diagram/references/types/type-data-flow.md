...
- `assets/examples/example-data-flow.html` — minimal light (the platform, 4-role × 5-step: Admin, Engineers, Scientists, Consumers). Gallery default.
- `assets/examples/example-data-flow-dark.html` — same, dark skin.
- `assets/examples/example-data-flow-full.html` — same, editorial-card frame.
- `assets/examples/example-data-flow-extended.html` — exercises §4 color override: Access Control node in rust-red (governance), Clean & Stage node in slate-blue (data quality). Focal accent on Analyze step + Explore & Model node + anon-data arrow unchanged.
- `assets/examples/example-data-flow-extended-dark.html` — extended pattern, dark skin.
- `assets/examples/example-data-flow-extended-full.html` — extended pattern, editorial-card frame.
