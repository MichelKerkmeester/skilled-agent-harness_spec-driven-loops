...
`sk-create-flowchart/scripts/` holds the single validator for the `/create:flowchart` workflow. `validate-flowchart.sh` runs five checks against one flowchart file and reports errors and warnings without needing any other file.
...
```bash
.opencode/skills/sk-doc/sk-create-flowchart/scripts/validate-flowchart.sh <flowchart.md>
```
