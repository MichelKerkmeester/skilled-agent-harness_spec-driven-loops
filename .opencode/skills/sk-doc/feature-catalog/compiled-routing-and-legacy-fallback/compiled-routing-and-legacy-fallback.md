...
description: "How sk-doc resolves the compiled per-hub router contract ahead of its own registry-driven routing, and the legacy fallback that keeps it inert until explicitly authorized."
trigger_phrases:
...
`sk-doc`'s `SKILL.md` carries an opt-in, flag-gated, additive directive that asks the compiled per-hub router contract to resolve the mode before falling through to the mode-registry-driven routing described in [`packet-authored-registry-routing.md`](../packet-authored-registry-routing/packet-authored-registry-routing.md).

The directive is off by default and byte-identical to today's behavior when unset: the compiled front door returns a `{"servingAuthority":"legacy"}` sentinel (or is never meaningfully consulted), and `sk-doc` falls back to its registry-driven routing every time.
...
| File | Layer | Role |
|---|---|---|
| `.opencode/skills/sk-doc/SKILL.md` | Shared | Carries the opt-in compiled-routing directive `sk-doc` follows. |
