...
| Assets | `assets/**/*.md` |
| Feature catalogs | `feature_catalog/**/*.md` (roots **and** per-feature leaves) |
| Testing playbooks | `manual_testing_playbook/**/*.md` (roots **and** per-feature leaves) |

### Out of scope (do NOT add `version` in this standard)

`.opencode/commands/*.md`, `.opencode/agents/*.md`, and standalone `.opencode/install_guides/`. These also carry frontmatter but are governed separately; a follow-up packet may bring them in.
