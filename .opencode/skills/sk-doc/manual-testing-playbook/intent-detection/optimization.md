...
description: "This scenario validates OPTIMIZATION intent detection for SD-016."
stage: routing
...
This document captures the routing-gold contract, current behavior, execution notes, source anchors, and metadata for `SD-016`.
...
- Objective: Verify sk-doc routes the scenario to `OPTIMIZATION` with the expected resources.
...
- Expected signals: Intent resolves to `OPTIMIZATION`; loaded resources match `expected_resources`.
...
Intent resolves to `OPTIMIZATION`; loaded resources match `expected_resources`.
