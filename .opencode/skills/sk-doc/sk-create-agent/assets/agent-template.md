...
The `name` value and emitted filename stem must both match `^[a-z0-9]+(?:-[a-z0-9]+)*$`. Reject underscore-bearing or ambiguous names rather than converting them after the frontmatter and path diverge.
...
| `@design` | `design.md` | LEAF write-capable design subagent | routes between `sk-design` (decide values) and `sk-design-md-generator` (measure a surface), applies the path, then verifies |
...
- [ ] `name` and filename stem are lowercase kebab-case with no underscores.
