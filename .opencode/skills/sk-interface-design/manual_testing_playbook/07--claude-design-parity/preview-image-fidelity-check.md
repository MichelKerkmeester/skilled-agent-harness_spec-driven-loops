...
- Expected execution process: Use the runId of a completed Open Design generation run (for example one produced by the RUN-001 recipe `od run start --agent opencode --model <explicit>`, or a prior ID-008 build). Follow `references/design-process/claude_design_parity.md` Section 5, confirm via `mcp-open-design` `get_run` that the run completed rather than sitting `awaiting_input`, fetch the `previewUrl` and the written files with `get_artifact`, and judge the render against the `ux_quality_reference.md` floor and the anti-default critique.
...
- Pass/fail: PASS if the verdict judges the completed run's render against both gates per `references/design-process/claude_design_parity.md` Section 5; FAIL if completion is claimed from a run left `awaiting_input`, from an `od artifacts create` file-add, or from a screenshot alone
...
4. Compare the produced verdict against `references/design-process/claude_design_parity.md` Section 5 and the two cited gates.
...
| File | Role |
|---|---|
| `../../references/design-process/claude_design_parity.md` | Section 5 fidelity check: the `previewUrl` mechanism, the two-gate bar, and the no-screenshot-alone caveat |
