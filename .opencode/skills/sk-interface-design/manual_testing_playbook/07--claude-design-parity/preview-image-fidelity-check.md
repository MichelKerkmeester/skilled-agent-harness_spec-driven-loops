...
- Expected execution process: Follow `references/claude_design_parity.md` Section 5, confirm via `mcp-open-design` `get_run` that the run completed rather than sitting `awaiting_input`, fetch the `previewUrl` and the written files with `get_artifact`, and judge the render against the `ux_quality_reference.md` floor and the anti-default critique.
