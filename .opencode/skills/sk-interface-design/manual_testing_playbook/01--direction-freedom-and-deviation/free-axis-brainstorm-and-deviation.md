...
- Expected execution process: Load `references/design_principles.md`, ground the subject, brainstorm a 4-6 hex token system with type and a signature, run the critique against the three default clusters, and revise free axes with a stated reason.
...
- Pass/fail: PASS if the plan names the three default clusters and states a justified deviation per `references/design_principles.md` Section 4; FAIL if the plan ships cream-serif-terracotta, near-black-plus-acid-accent, or broadsheet-hairlines on a free axis with no stated reason
...
| `../../references/design_principles.md` | The two-pass brainstorm-critique process and the three AI-default clusters |
