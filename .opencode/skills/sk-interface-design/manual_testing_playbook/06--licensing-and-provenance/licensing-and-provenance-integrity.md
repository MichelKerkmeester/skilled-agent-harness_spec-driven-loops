...
- Expected execution process: Confirm `design_principles.md` declares the Apache-2.0 upstream and carries no local edits to its guidance, confirm `LICENSE.txt` exists, and grep the skill tree for the legacy de-vendor tokens (the vendored data directory, the removed search script, the vendored MIT license file, and the removed third party notices) to prove none remains.
