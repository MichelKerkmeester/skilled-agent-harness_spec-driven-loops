...
|---|---|---|---|---|---|---|---|---|
| CU-029 | Timer Lifecycle — start, status, stop | Verify timer start/status/stop lifecycle: start exits 0, status shows running timer, stop exits 0 and clears timer | `Start timer on TASK_ID, check timer status, then stop it.` | 1. `cupt time start TASK_ID`  # → exit 0 2. `cupt time status`  # → 'Timer running on: Task Name (TASK_ID), Elapsed: 0:00:XX' 3. `cupt time stop`  # → exit 0, logs elapsed time 4. `cupt time status`  # → 'No timer running' | Step 1: exit 0. Step 2: running timer shown. Step 3: exit 0, elapsed logged. Step 4: 'no timer' message. | Terminal output of the command sequence above | PASS if all 4 steps exit 0 AND final status shows no timer; FAIL if any step exits non-zero OR final status still shows running timer | See [`../../references/troubleshooting.md`](../../references/troubleshooting.md) |
