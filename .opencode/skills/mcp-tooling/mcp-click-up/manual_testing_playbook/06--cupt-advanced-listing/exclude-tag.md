...
|---|---|---|---|---|---|---|
| CU-013 | Exclude by Tag — --no-tag | Verify `cupt list --no-tag <name> --json` excludes task | `List tasks that do NOT have the 'processed' tag.` | JSON array; no task in result has 'processed' in its tags array; exit 0. | PASS if no task in result has the excluded tag AND exit 0; FAIL if excluded tag appears in any returned task OR exit non-z | See `../references/troubleshooting.md` |
...
| `../feature_catalog/02--cupt-task-listing/exclude-tag.md` | Feature catalog source |
