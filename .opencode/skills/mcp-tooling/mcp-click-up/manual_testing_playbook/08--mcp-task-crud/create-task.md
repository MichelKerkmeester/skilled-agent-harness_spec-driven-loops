...
1. Code Mode: `clickup_official.clickup_official_clickup_create_task({list_id: 'LIST_ID', name: 'MCP Playbook Test Task', priority: 3})`
2. Verify response includes `id` field
3. `cupt show TASK_ID --json | jq .name`  # → 'MCP Playbook Test Task'

| Feature ID | Feature Name | Scenario Objective | Exact Prompt | Exact Command Sequence | Expected Signals | Evidence | Pass/Fail Criteria | Failure Triage |
|---|---|---|---|---|---|---|---|---|
| MCP-H001 | Create Task via MCP | Verify `clickup_create_task` creates a task in LIST_ID and returns a task_id | `Create a task named 'MCP Playbook Test Task' in list LIST_ID.` | 1. Code Mode: `clickup_official.clickup_official_clickup_create_task({list_id: 'LIST_ID', name: 'MCP Playbook Test Task', priority: 3})` 2. Verify response includes `id` field 3. `cupt show TASK_ID --json \| jq .name`  # → 'MCP Playbook Test Task' | MCP response includes `id`; `cupt show` confirms name; exit 0. | Code Mode response + terminal output of the verification step(s) above | PASS if response includes `id` AND task visible via cupt show; FAIL if `id` missing from response OR task not found via cupt | See [`../../references/troubleshooting.md`](../../references/troubleshooting.md) |
...
| [`../../feature_catalog/11--mcp-high-priority/create-task.md`](../../feature_catalog/11--mcp-high-priority/create-task.md) | Feature catalog source |
