...
Note: no top-level `get_document`/`update_document` tool was found in the last inventory — only creation and page-level operations. Treat those two capabilities as unsupported until proven otherwise (see `feature_catalog/12--mcp-medium-priority/{get-document,update-document}.md`).
