...
| Callable name or schema mismatch at discovery | Provider or adapter drift against the inferred `mobbin.mobbin_search_screens` form, or the prediction was simply wrong (it is Inferred, never observed) | Re-run discovery; **fail closed**; update the reviewed packet docs rather than improvising a call |
