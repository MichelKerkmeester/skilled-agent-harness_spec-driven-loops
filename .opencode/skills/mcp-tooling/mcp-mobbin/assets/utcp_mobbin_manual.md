...
**Key Points**:
- Manual `name` is `mobbin`, so convention predicts callables of the form `mobbin.mobbin_search_screens(...)` (dotted discovery `mobbin.mobbin.search_screens`) — **Inferred until live `tool_info` confirms**.
...
- [ ] Run discovery: `list_tools()` filtered to the `mobbin` manual, then `tool_info()` on the exact dotted name. Record the **actual** callable name and schema on the validation date — the predicted `mobbin.mobbin_search_screens` form is Inferred until this step. [SKIP-valid: inside Code Mode run `list_tools()` then `tool_info({ tool_name: "mobbin.mobbin_search_screens" })`]
- [ ] Confirm the live tool set against the documented single-tool baseline (`search_screens`); **fail closed** on missing/renamed/extra tools, and refuse any mutation-capable tool outright. [SKIP-valid: compare the `list_tools()` output against `references/tool_surface.md` §1]
- [ ] Check the live schema for the open items: whether `deep` is a client-settable input, and any optional fields beyond `query`/`platform`/`limit`. [SKIP-valid: inspect the `tool_info()` JSON Schema for a `deep` property and extra fields]
...
- [mcp_wiring.md](../references/mcp_wiring.md) - the full wiring reference: bridge behavior, OAuth/DCR/PKCE, auth state, inferred naming, and discovery.
- [tool_surface.md](../references/tool_surface.md) - the single-tool contract the manual exposes.
