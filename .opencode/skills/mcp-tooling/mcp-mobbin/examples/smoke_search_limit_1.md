...
The minimum end-to-end verification of the Mobbin transport: one confirmed callable, one call, one screen. This is the same smoke check the post-registration checklist in `../assets/utcp_mobbin_manual.md` carries as its live item, worked as a full walkthrough. Contract authority: [`../references/tool_surface.md`](../references/tool_surface.md) §1 (the one tool, hard constraints).
...
Check the response against the documented baseline (tool_surface.md §1):
