...
description: "Operator-facing scenarios for headless notesmd-cli operations, the official app-backed obsidian CLI, cyanheads MCP round-trips, and file-layer tie-ins for two community plugins and the Obsidian theme system."
version: 0.2.0.0
---
...
This playbook is the operator directory for the `mcp-obsidian` mode. It validates the headless `notesmd-cli` profile, the official app-backed `obsidian` CLI, the cyanheads `obsidian_*` MCP surface, and file-layer operations for two community plugins and the Obsidian theme system — Health.md Visualizations (`health-md`) and Iconic (`iconic`) at the plugin layer, plus the theme system (`references/themes/`).
...
| Community-plugin tie-ins | `OBS-014`, `OBS-015`, `OBS-021` | File-layer fixtures; app reload is required only for the render/activation check |
...
| 7 | `OBS-014`, `OBS-015`, `OBS-021` | Health.md data fixture, Iconic rulebook fixture, and theme activation; reload only after file-layer evidence |
...
## 12. PLUGIN AND THEME FILE-LAYER TIE-INS (`OBS-014`, `OBS-015`, `OBS-021`)
