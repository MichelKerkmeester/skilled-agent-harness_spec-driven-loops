...
- A portal is `portal: true` on a `file` node. The serialized shape of a cross-portal ("interdimensional") edge is the one item flagged `VERIFY` — do not hand-author it.
