...
- Never author `.claude/mcp.json` or invent a Claudian MCP registry — Claudian removes that legacy file at init; MCP is declared in the provider CLI's own native MCP config. For Claude Code, Claudian writes no MCP file — it passes an in-memory `mcpServers` array to the runtime, and the positive add-a-server surface is UNKNOWN from the installed build.
