...
Use [`brat-data-entry.example.json`](../../../assets/plugins/obsidian42-brat/brat-data-entry.example.json) as a valid populated example, not as a replacement for an existing vault file. Merge into the existing BRAT settings while preserving unrelated keys and entries.
