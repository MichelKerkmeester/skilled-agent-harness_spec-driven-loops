...
> Ships in the `mcp-obsidian` mode at `references/plugins/`. BRAT is the **installer** for beta plugins not in the community list (e.g. `obsidian-flat-financing`). It is the enabling piece for the other two plugins.
