...
- **A portal is `portal: true` on a `file` node.** A *cross-portal edge* (an edge whose endpoint is a node inside a portal) lives in an `interdimensionalEdges` array on the portal `file` node, using composite `portalId-nodeId` endpoints (`data-model.md` §5) — never place it in the top-level `edges` array. The exact endpoint encoding is confirmed from the plugin's own serialization code but not yet byte-verified against a captured portal `.canvas` file; if you want a fully byte-verified path, let the plugin manage portal-internal edges after a reload.
