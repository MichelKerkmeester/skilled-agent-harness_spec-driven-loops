...
Full style-reference retrieval for UUIDs shortlisted by [`refero_search_styles`](search_styles.md). READ-ONLY. Canonical callable: `refero.refero_refero_get_style({ style_id | style_ids, response_format? })` (doubled prefix; confirm via `tool_info` first).
...
| File | Role |
|---|---|
| `../../references/tool_surface.md` | The exactly-one-of ID union, batching bound, and result shape (Section 1) |
| `../../manual_testing_playbook/read_only/styles_funnel.md` | The bounded-batch funnel scenario exercising this tool |
...
- Feature file path: `styles/get_style.md`
