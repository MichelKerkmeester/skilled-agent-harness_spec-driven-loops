...
- Expected signals: the update line exits 0 with `does not match the task-branch naming grammar (update allowed — migration tolerance)`; the new-branch line for the same name exits 1 with the `[gate:naming]` new-branch rejection.
...
- Feature file path: `owner-first-worktree-tooling/prepush-migration-tolerance.md`
