...
- Expected signals: the new-branch line is rejected (exit 1, a `[gate:naming]` rejection reading `new branch '<name>' does not match the task-branch naming grammar`); the update line with the identical name exits 0 with a migration-tolerance notice, not a rejection.
...
- Feature file path: `owner-first-worktree-tooling/prepush-gates-only-new-branches.md`
