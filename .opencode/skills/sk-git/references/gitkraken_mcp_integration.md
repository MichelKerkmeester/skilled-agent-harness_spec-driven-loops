...
The GitKraken MCP server (`@gitkraken/gk`) exposes 31 tools spanning local git operations, GitLens AI workflows, and cross-platform issue/PR management across GitHub, GitLab, Azure DevOps, Bitbucket, and Jira. Unlike [github_mcp_integration.md](./github_mcp_integration.md), which is GitHub-only, GitKraken MCP is the tool to reach for when a repository lives on a non-GitHub provider, or when the task benefits from GitLens' AI-assisted workflows (commit composition, PR review, launchpad triage).
...
**Internal References**:
- [github_mcp_integration.md](./github_mcp_integration.md) - GitHub-only MCP reference; prefer it for GitHub-only tasks
- [quick_reference.md](./quick_reference.md) - Command cheat sheet
- [shared_patterns.md](./shared_patterns.md) - Reusable git patterns
