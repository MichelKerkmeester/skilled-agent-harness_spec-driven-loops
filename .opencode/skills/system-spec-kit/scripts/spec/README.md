...
| `validate.sh` | Runs the modular validation gate used before completion claims. `resolve_orchestrator()` checks compiled `mcp_server` dist freshness (via `../lib/dist-freshness.cjs`) before trusting it and fails closed with exit `3` when stale — no silent auto-rebuild. |
