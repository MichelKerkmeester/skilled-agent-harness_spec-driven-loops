...
`scripts/lib/` contains shared helpers for the script package. TypeScript modules compile to `scripts/dist/lib/`, while shell helpers and standalone CommonJS modules are sourced or required directly, with no build step.
...
```bash
node -e "console.log(typeof require('./.opencode/skills/system-spec-kit/scripts/lib/completion-state.cjs').computeCompletionState)"
node -e "console.log(typeof require('./.opencode/skills/system-spec-kit/scripts/lib/coverage-graph-core.cjs').createGraph)"
```
