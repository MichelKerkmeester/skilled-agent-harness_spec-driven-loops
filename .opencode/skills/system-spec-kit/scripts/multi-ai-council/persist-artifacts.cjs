...
function roundLabel(round) {
  const parsed = Number(round || 1);
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 99) {
    throw new Error('[multi-ai-council] --round must be an integer from 1 to 99');
  }
...
function assertInside(baseDir, targetPath) {
  ...
  if (relative.startsWith('..') || path.isAbsolute(relative)) {
    throw new Error(`[multi-ai-council] Refusing to write outside ${baseDir}: ${targetPath}`);
  }
}

function writeFileScoped(aiCouncilRoot, relativePath, content) {
...
function writeArtifacts(packetSpecFolder, rendered) {
  ...
  const files = [
    ['ai-council-config.json', rendered.config],
    ['ai-council-strategy.md', rendered.strategy],
    ['ai-council-state.jsonl', rendered.stateLog],
    [rendered.deliberationPath || 'deliberations/round-001.md', rendered.deliberation],
    ['council-report.md', rendered.councilReport],
    ...rendered.seats.map((seat) => [seat.path, seat.content]),
  ];

  for (const [relativePath, content] of files) {
    try {
      written.push(writeFileScoped(aiCouncilRoot, relativePath, content));
    } catch (error) {
      conflicts.push(`${relativePath}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
