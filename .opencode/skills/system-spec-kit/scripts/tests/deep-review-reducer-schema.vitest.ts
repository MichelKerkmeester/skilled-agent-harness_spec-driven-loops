...
describe('deep-review reducer and schema contract', () => {
  it('keeps reducer, severity, and release-readiness schemas stable in the canonical assets', () => {
    const contract = yaml.load(
      readWorkspaceFile('.opencode/skills/sk-deep-review/assets/review_mode_contract.yaml'),
    ) as Record<string, any>;
    const config = JSON.parse(
      readWorkspaceFile('.opencode/skills/sk-deep-review/assets/deep_review_config.json'),
    ) as Record<string, any>;
  ...
  it('wires reducer refresh and machine-owned report guidance into both review workflows', () => {
    ...
    for (const [docPath, content] of [
      ...
      expect(content, `${docPath} workflow should invoke the reducer script`).toContain(
        'node .opencode/skills/sk-deep-review/scripts/reduce-state.cjs',
      );
  ...
  it('documents reducer metrics and machine-owned report boundaries in the review references', () => {
    const stateFormat = readWorkspaceFile('.opencode/skills/sk-deep-review/references/state_format.md');
    const loopProtocol = readWorkspaceFile('.opencode/skills/sk-deep-review/references/loop_protocol.md');
    const convergence = readWorkspaceFile('.opencode/skills/sk-deep-review/references/convergence.md');
