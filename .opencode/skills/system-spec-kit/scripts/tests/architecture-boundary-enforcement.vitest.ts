...
// workspace (mcp-server vs scripts) vitest treats as process.cwd().
...
describe('Architecture Boundary Enforcement', () => {
  ...
  function createFixtureRoot(): string {
    ...
    fs.mkdirSync(path.join(root, 'mcp-server', 'scripts'), { recursive: true });
  ...
  it('T39: GAP A detects shared -> mcp-server/scripts imports across syntax variants', () => {
    ...
    writeFixtureFile(root, 'shared/dynamic-import.ts', [
      ...
      "  return import('../mcp-server/dynamic-runtime');",
      '}',
    ].join('\n'));
  ...
  it('parses export-from, import type, and require() forms when checking shared neutrality', () => {
    ...
    writeFixtureFile(root, 'shared/export-from.ts', [
      "export { runtimeA } from '../mcp-server/runtime';",
    ].join('\n'));
  ...
  it('T40: GAP B flags wrappers exceeding 50 substantive lines', () => {
    ...
    writeFixtureFile(root, 'mcp-server/scripts/too-long.ts', [
    ...
    const violation = violations.find((v) => v.file.endsWith(path.join('mcp-server', 'scripts', 'too-long.ts')));
  ...
  it('T41: GAP B flags wrappers missing child_process import', () => {
    ...
    writeFixtureFile(root, 'mcp-server/scripts/no-child-process.ts', [
      "const targetScript = '../../scripts/dist/memory/cli.js';",
    ...
    const violations = checkWrapperOnly(root);
    const violation = violations.find((v) => v.file.endsWith(path.join('mcp-server', 'scripts', 'no-child-process.ts')));
  ...
  it('T42: GAP B flags wrappers missing scripts/dist reference', () => {
    ...
    writeFixtureFile(root, 'mcp-server/scripts/no-dist-reference.ts', [
      "import { spawnSync } from 'child_process';",
    ...
    const violations = checkWrapperOnly(root);
    const violation = violations.find((v) => v.file.endsWith(path.join('mcp-server', 'scripts', 'no-dist-reference.ts')));
  ...
  it('T43: GAP B catches wrapper bypasses (barrel re-exports and scripts import without spawn/exec)', () => {
    ...
    const violations = checkWrapperOnly(root);
    const barrel = violations.find((v) => v.file.endsWith(path.join('mcp-server', 'scripts', 'barrel-export.ts')));
    const noSpawn = violations.find((v) => v.file.endsWith(path.join('mcp-server', 'scripts', 'no-spawn-usage.ts')));
  ...
  it('T45: valid mcp-server/scripts -> shared imports are not false positives', () => {
    const root = createFixtureRoot();
    ...
    writeFixtureFile(root, 'mcp-server/lib/consumer.ts', [
