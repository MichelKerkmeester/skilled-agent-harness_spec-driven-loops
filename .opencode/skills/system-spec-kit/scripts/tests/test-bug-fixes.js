...
const LIB_PATH = path.join(ROOT, 'runtime', 'dist', 'lib');
...
const DB_PATH = path.join(ROOT, 'runtime', 'database');
...
const DB_STATE_PATH = path.join(ROOT, 'runtime', 'dist', 'core', 'db-state.js');
...
async function testBug001() {
  ...
  try {
    ...
    // Test 1: Notification file mechanism now lives in runtime/
...
async function testBug007() {
  ...
  try {
    ...
    const validators = fs.readFileSync(
      path.join(ROOT, 'runtime', 'dist', 'utils', 'validators.js'),
      'utf8'
    );
