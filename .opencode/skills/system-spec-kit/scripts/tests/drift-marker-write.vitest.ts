...
describe('drift marker write entrypoint', () => {
  ...
  it('recognizes a symlinked entrypoint as the executing module', () => {
    const root = makeTempDir();
    const realEntrypoint = path.join(root, 'drift-marker-write.js');
    const linkedEntrypoint = path.join(root, 'linked-drift-marker-write.js');
    fs.writeFileSync(realEntrypoint, '');
    fs.symlinkSync(realEntrypoint, linkedEntrypoint);

    expect(isDriftMarkerMainModule(pathToFileURL(realEntrypoint).href, linkedEntrypoint)).toBe(true);
  });
