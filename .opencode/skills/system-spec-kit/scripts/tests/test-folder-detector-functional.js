...
const BETTER_SQLITE3_PATH = path.join(SKILL_ROOT, 'runtime/node_modules/better-sqlite3');
const REAL_DB_PATH = path.join(SKILL_ROOT, 'runtime/database/context-index__<provider>__<safe-model>__<dim>__<dtype>.sqlite');
...
async function testConfigProjectRootAlignment() {
  ...
  try {
    ...
    const constructedDbPath = path.join(
      CONFIG.PROJECT_ROOT,
      '.opencode/skills/system-spec-kit/runtime/database/context-index__<provider>__<safe-model>__<dim>__<dtype>.sqlite'
    );
    ...
    const constructedSqlitePath = path.join(
      CONFIG.PROJECT_ROOT,
      '.opencode/skills/system-spec-kit/runtime/node_modules/better-sqlite3'
    );
    try {
