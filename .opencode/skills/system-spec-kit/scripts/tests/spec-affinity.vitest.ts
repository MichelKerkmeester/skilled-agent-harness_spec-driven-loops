...
describe('spec affinity evaluation', () => {
  ...
  it('treats exact file-target references inside command text as valid spec anchors', () => {
    ...
    expect(matchesSpecAffinityText(
      "sed -n '680,760p' .opencode/skills/system-spec-kit/scripts/core/workflow.ts",
      targets,
    )).toBe(true);
  });
      ...
      observations: [
        {
          ...
          files: ['.opencode/skills/system-spec-kit/scripts/core/config.ts'],
        },
      ],
      FILES: [
        {
          FILE_PATH: '.opencode/skills/system-spec-kit/scripts/core/config.ts',
