...
function parseArguments(argv) {
  ...
  throw new Error('Usage: node sync-prompts-pi.cjs [--check]');
}
...
function renderPrompt(relativePath) {
  ...
  return [
    ...
    `# Pi entry: /${flatName}`,
    '',
    `This is the Pi runtime entry point for the \`/${flatName}\` command. The canonical, authoritative command definition lives in the OpenCode tree at:`,
    '',
...
function checkOutputs(expectedOutputs) {
  ...
  if (drift.length > 0) {
    console.error('[pi-prompt-sync] Drift detected:');
    for (const item of drift) {
      console.error(`[pi-prompt-sync] ${item}`);
    }
    return false;
  }

  console.log(`[pi-prompt-sync] PASS: ${expectedOutputs.size} prompts are in sync.`);
  return true;
}
...
try {
  ...
  console.error(`[pi-prompt-sync] ERROR: ${error.message}`);
  process.exitCode = 1;
}
