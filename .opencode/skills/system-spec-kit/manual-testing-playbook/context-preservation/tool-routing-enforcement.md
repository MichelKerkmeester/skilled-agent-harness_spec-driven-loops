...
version: 3.6.0.18
id: context-preservation-tool-routing-enforcement
...
1. cd .opencode/skills/system-spec-kit && rg -n "## Tool Routing|buildServerInstructions" mcp-server/context-server.ts && node -e "const fs=require('fs'); const p='mcp-server/context-server.ts'; const s=fs.readFileSync(p,'utf8'); console.log(s.includes('## Tool Routing'))"
...
Check `mcp-server/context-server.ts` around `buildServerInstructions()` and confirm routing rules are not gated off unexpectedly
...
Check `mcp-server/hooks/memory-surface.ts` prime assembly and confirm the session is actually fresh before testing
...
Check `mcp-server/hooks/response-hints.ts` code-search detection patterns and confirm the query is classified as code-search intent
...
1. cd <repo-root> && rg -n "Code Graph|semantic search|Tool Routing Enforcement|SEARCH ROUTING" AGENTS.md
