...
The mechanical tests in `mcp-server/tests/local-llm-features/performance/*.bench.ts` measure single-embedding latency under controlled conditions. This scenario measures end-to-end SEARCH latency — including embedding the query, running the vector search, ranking, and returning hits — under a realistic mixed workload.
