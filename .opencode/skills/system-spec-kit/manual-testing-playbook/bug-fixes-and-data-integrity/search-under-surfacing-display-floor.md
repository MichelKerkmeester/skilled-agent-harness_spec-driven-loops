...
1. Open `mcp-server/tests/token-budget-constitutional-sync.vitest.ts`
...
`mcp-server/tests/token-budget-constitutional-sync.vitest.ts` source evidence from `describe('T207: display floor + compact overflow')`, including `T207-A1` and `T207-A2` assertions.
...
- Feature file path: `bug-fixes-and-data-integrity/search-under-surfacing-display-floor.md`
