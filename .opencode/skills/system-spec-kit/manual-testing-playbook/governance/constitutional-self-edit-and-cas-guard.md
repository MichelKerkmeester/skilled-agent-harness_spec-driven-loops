...
| File | Role |
|---|---|
| `mcp-server/handlers/memory-crud-update.ts` | Self-edit assertion and `expectedHash` compare-and-swap precondition |
| `mcp-server/handlers/memory-crud-types.ts` | Update args type carrying `expectedHash` |
| `mcp-server/schemas/tool-input-schemas.ts` | Additive optional `expectedHash` field |
| `mcp-server/tests/memory-crud-update-constitutional-guard.vitest.ts` | Self-edit, stale-hash, and matching-hash safe-update cases |
