...
version: 3.6.0.2
id: governance-stale-exclusion-audit-and-tool-ownership-lint
...
3. Run the focused suite from `.opencode/skills/system-spec-kit/mcp-server`: `npx vitest run tests/stale-audit-tool-ownership.vitest.ts`.
4. Run the clean lint command from `.opencode/skills/system-spec-kit/mcp-server`: `node tests/tool-ownership-lint-runner.mjs`.
...
6. Run `git diff -- .opencode/skills/system-spec-kit/mcp-server/tests/fixtures/tool-ownership-map.json .opencode/skills/system-spec-kit/mcp-server/tests/tool-ownership-lint-runner.mjs` and confirm no source fixture was changed.
...
Focused suite from `.opencode/skills/system-spec-kit/mcp-server`:
...
⎯⎯⎯⎯⎯⎯⎯ Failed Tests 1 ⎯⎯⎯⎯⎯⎯⎯
 ...
 FAIL  mcp-server/tests/stale-audit-tool-ownership.vitest.ts > stale/status hard-exclusion audit > flags deprecated relevant rows while recall output stays byte-identical
...
+   2,
 ...
 ❯ mcp-server/tests/stale-audit-tool-ownership.vitest.ts:111:32
    109|
...
Clean lint command from `.opencode/skills/system-spec-kit/mcp-server`:
...
Unreadable definitions drift check from `.opencode/skills/system-spec-kit/mcp-server` after creating a temp directory and copying `tests/fixtures/tool-ownership-map.json` to `<temp>/tool-ownership-map.json`:
...
```text
$ git diff -- .opencode/skills/system-spec-kit/mcp-server/tests/fixtures/tool-ownership-map.json .opencode/skills/system-spec-kit/mcp-server/tests/tool-ownership-lint-runner.mjs
```
...
| File | Role |
|---|---|
| `mcp-server/handlers/memory-crud-health.ts` | Health audit surface |
| `mcp-server/tests/stale-audit-tool-ownership.vitest.ts` | Audit and lint regression coverage |
| `mcp-server/tests/tool-ownership-lint-runner.mjs` | Source-derived lint runner |
| `mcp-server/tests/fixtures/tool-ownership-map.json` | Committed ownership fixture |
