...
6. Targeted Vitest: `cd .opencode/skills/system-spec-kit/mcp-server && npm exec -- vitest run tests/community-search.vitest.ts`.
...
```text
$ sqlite3 -readonly ".opencode/skills/system-spec-kit/mcp-server/database/context-index.sqlite" "SELECT name FROM sqlite_master WHERE type='table' AND name='community_summaries'; SELECT community_id, member_count, substr(summary,1,500) AS summary, member_ids FROM community_summaries LIMIT 10;"
community_summaries
...
- Source: `.opencode/skills/system-spec-kit/mcp-server/lib/search/community-search.ts`
- Regression tests: `.opencode/skills/system-spec-kit/mcp-server/tests/community-search.vitest.ts`
