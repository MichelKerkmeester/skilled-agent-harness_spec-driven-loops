...
- **Pass**: Each mode behaves as documented and the kill-switch reverts.
- **Fail**: Modes return identical output, auto fallback never fires, or the kill-switch has no effect.
...
- Source: `.opencode/skills/system-spec-kit/mcp-server/handlers/memory-search.ts`, `.opencode/skills/system-spec-kit/mcp-server/lib/search/search-flags.ts`
