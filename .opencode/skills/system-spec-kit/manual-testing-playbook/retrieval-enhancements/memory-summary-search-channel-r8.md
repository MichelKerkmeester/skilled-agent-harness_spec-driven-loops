...
Command: `sqlite3 ".opencode/skills/system-spec-kit/mcp-server/database/context-index.sqlite" "SELECT 'memory_index_success_count', COUNT(*) FROM memory_index WHERE embedding_status = 'success'; SELECT 'memory_summaries_count', COUNT(*) FROM memory_summaries; SELECT 'scale_gate_gt_5000', CASE WHEN (SELECT COUNT(*) FROM memory_index WHERE embedding_status = 'success') > 5000 THEN 'true' ELSE 'false' END;"`
...
Command: `node ".opencode/bin/spec-memory.cjs" memory_search --json '{"query":"memory summary search channel","limit":5}' --format json --timeout-ms 10000`
