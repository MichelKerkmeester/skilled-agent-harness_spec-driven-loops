...
This scenario validates that the local `/goal-opencode` OpenCode plugin owns session-goal state, exposes the expected tool status, and renders the same active-goal context the model receives.

---

## 2. SCENARIO CONTRACT

- Objective: Verify `/goal-opencode set` persists a goal, generates `goalPrompt` metadata, and exposes an `[active_goal]` injection preview through plugin status.
...
- Prompt: `Validate the /goal-opencode plugin active-goal injection and status surface.`
- Expected execution process: Restart OpenCode after plugin edits, run `/goal-opencode bind <packet-path>` against a packet that carries a `goal.md`, run `/goal-opencode show` and confirm `packet_bound=true` and an injection preview whose `objective:` starts with `Execute <packet>/goal.md.`, whose `criteria:` lines carry the packet's completion criteria one per line, and which carries no frontmatter, run `/goal-opencode resent` and confirm `resend_pending=false`, run `/goal-opencode packet <packet-path>` and confirm `chat_slice=` carries no `title:` line, then run `/goal-opencode set <objective> --budget N`, run `/goal-opencode show`, run `/goal-opencode history`, run `/goal-opencode doctor` or `/goal-opencode health`, run `/goal-opencode pause <reason>`, run `/goal-opencode resume`, inspect each status envelope, validate `OPENCODE_GOAL_VERIFIER=heuristic` and optional `OPENCODE_GOAL_VERIFIER=llm`, and compare it with the plugin unit tests when a direct runtime check is unavailable.
- Expected signals: `STATUS=OK ACTION=bind` with `mutation=bound`, `packet_bound=true`, `resend_pending=true`, `STATUS=OK ACTION=resent` with `resend_pending=false`, `STATUS=OK ACTION=packet` with `packet_slice_hash=`, `STATUS=OK ACTION=set`, `STATUS=OK ACTION=show`, `STATUS=OK ACTION=history`, `STATUS=OK ACTION=doctor` or `STATUS=OK ACTION=health`, `STATUS=OK ACTION=resume`, `goal_prompt=`, `prompt_framework="CRAFT+TIDD-EC"`, `prompt_max_chars=4000`, `token_budget=`, `remaining_auto_turns=`, `remaining_wall_ms=`, `provider_retry_after_ms=`, `verifier_source=none|injected|default-heuristic|default-llm`, `mutation=created|refreshed|replaced` on set, `store_health=` on status/set output, and an injection preview containing `[active_goal:<goalId>]` plus `goal_prompt:`.
- Desired user-visible outcome: A concise pass/fail verdict with the exact status lines or unit-test evidence.
- Pass/fail: PASS if tool status and injection preview include the active goal plus prompt metadata; FAIL if `/goal-opencode` reads state directly from command markdown, omits `goal_prompt`, or requires MCP daemon state.
...
4. `/goal-opencode set Finish the goal plugin docs integration and validate it --budget 1234`
5. `/goal-opencode history`
6. `/goal-opencode doctor`
...
Live OpenCode goal plugin tool output from `/goal-opencode show` equivalent:
...
- **Fail**: a bound goal's injection preview or `chat_slice` carries a `---` fence or a `session_id:` line, `bind` accepts a path outside the workspace, `resent` does not clear `resend_pending`, command markdown reads state directly, status lacks prompt metadata, set output omits `mutation=`, status/set output omits `store_health=` or `verifier_source=`, `/goal-opencode history` omits `archive_count=`, `/goal-opencode doctor` or `/goal-opencode health` omits `orphan_candidate_count=`, `/goal-opencode resume` cannot reactivate a paused goal, `OPENCODE_GOAL_VERIFIER=heuristic` false-completes weak evidence, `OPENCODE_GOAL_VERIFIER=llm` cannot reach `ctx.client.session.promptAsync` when enabled, `OPENCODE_GOAL_MAX_AUTO_TURNS` or `OPENCODE_GOAL_MAX_WALL_MS` is ignored, or injection preview omits `goal_prompt:`.
