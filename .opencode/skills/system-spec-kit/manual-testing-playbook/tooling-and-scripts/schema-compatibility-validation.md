...
- Real user request: `Please validate Schema compatibility validation against cd .opencode/skills/system-spec-kit/mcp-server and tell me whether the expected signals are present: Targeted suite passes; transcript shows missing-table reporting and minimal-compatible schema success coverage.`
...
```
Validate Schema compatibility validation against cd .opencode/skills/system-spec-kit/mcp-server and report cited pass/fail evidence.
```
