...
- **FAIL**: `spec-memory` exited 69 with `@spec-kit/mcp-server dist is stale. Run: cd .opencode/skills/system-spec-kit/mcp-server && npm run build` instead of shell exit 75, so the scenario did not satisfy the all-three-calls exit-75 requirement; launcher counts stayed unchanged at 11 and no `daemon-ipc.sock` appeared in the sandbox socket directory.
...
| `../../feature-catalog/tooling-and-scripts/spec-memory-cli-daemon-backed-surface.md` | Feature-catalog source for the warm-only contract |
