...
- **BLOCKED**: `node .opencode/bin/spec-memory.cjs list-tools --format json` returned no payload because `@spec-kit/mcp-server dist is stale. Run: cd .opencode/skills/system-spec-kit/mcp-server && npm run build`; observed `payload bytes: 0 (must be > 65536)`, `PARSE-FAIL` for both formats, and empty-payload hash `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`, while the dual-client vitest suite passed.
