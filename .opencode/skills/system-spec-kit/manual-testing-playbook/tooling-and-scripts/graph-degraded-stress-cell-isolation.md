...
```
Validate the graph degraded stress cell isolation contract against cd .opencode/skills/system-spec-kit/mcp-server && npx vitest run tests/code-graph-degraded-sweep.vitest.ts and report cited pass/fail evidence.
```
...
include: mcp-server/tests/**/*.{vitest,test}.ts, ../runtime//tests/**/*.{vitest,test}.ts, scripts/tests/**/*.{vitest,test}.ts
exclude:  mcp-server/tests/memory-save.vitest.ts, mcp-server/tests/archive/**
```
...
- Sibling (rebuild + restart + live probe): [mcp-daemon-rebuild-restart-live-probe.md](../../manual-testing-playbook/tooling-and-scripts/mcp-daemon-rebuild-restart-live-probe.md)
