...
`search-decisions.jsonl` (under `mcp-server/data/`) carries `routingReasons` containing `graph-preserved-by-intent` for each query classified as find_decision or find_spec; entity-density activations also emit `graph-preserved-by-entity-density`.

### Evidence

- Captured `memory_health` response JSON before and after flag toggle
- Tail of `mcp-server/data/search-decisions.jsonl` showing the new reason strings
...
- Health diagnostics: [discovery/health-diagnostics-memoryhealth.md](../../feature-catalog/discovery/health-diagnostics-memoryhealth.md)
