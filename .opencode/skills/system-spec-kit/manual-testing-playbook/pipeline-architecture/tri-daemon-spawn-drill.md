...
```bash
# Gated run (the drill)
(cd .opencode/skills/system-skill-advisor/mcp-server && SPECKIT_RUN_TRI_DAEMON_DRILL=1 npx vitest run tests/tri-daemon-drill.vitest.ts)

# Ungated control: must skip, not run
(cd .opencode/skills/system-skill-advisor/mcp-server && npx vitest run tests/tri-daemon-drill.vitest.ts)
```
...
| `../../feature-catalog/tooling-and-scripts/spec-memory-cli-daemon-backed-surface.md` | Feature-catalog source for the CLI program surface |
...
| `.opencode/bin/system-spec-memory-launcher.cjs` | Launcher with transparent-recycle SIGTERM reap |
| `.opencode/bin/system-skill-advisor-launcher.cjs` | Launcher that exits on child SIGTERM |
...
- Feature file path: `pipeline-architecture/tri-daemon-spawn-drill.md`
