...
version: 3.6.0.2
id: pipeline-architecture-front-proxy-reconnect-and-backend-only
...
2. Confirm the server skips `new StdioServerTransport()` / `server.connect(transport)` (backend mode), per `mcp-server/context-server.ts`.
...
Inspect the `SPECKIT_BACKEND_ONLY` branch in `mcp-server/context-server.ts` (around the `backendOnly` guard) if the server still connects its own transport under the flag.
