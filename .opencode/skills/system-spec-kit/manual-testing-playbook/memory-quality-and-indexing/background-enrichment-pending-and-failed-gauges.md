...
Command run from `.opencode/skills/system-spec-kit/mcp-server` using the built handler and in-memory vector index sandbox:
...
Schema/read-side check command run from `.opencode/skills/system-spec-kit/mcp-server`:
...
| File | Role |
|---|---|
| `mcp-server/handlers/memory-save.ts` | `getBackgroundEnrichmentStats` pending and failed gauge fields |
| `mcp-server/handlers/memory-crud-health.ts` | Per-status backlog aggregation and `backgroundEnrichment` health block |
| `mcp-server/tests/handler-memory-health-edge.vitest.ts` | Asserts pending and failed gauge values |
