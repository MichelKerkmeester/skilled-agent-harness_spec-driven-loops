...
version: 3.6.0.3
id: memory-quality-and-indexing-graph-metadata-and-lineage-repair-runner
...
4. From repo root:
   ```
   node .opencode/skills/system-spec-kit/mcp-server/scripts/repair-graph-metadata.mjs --dry-run --scan-log /tmp/scan-before.log
   ```
...
6. Run the same command without `--dry-run`:
   ```
   node .opencode/skills/system-spec-kit/mcp-server/scripts/repair-graph-metadata.mjs --scan-log /tmp/scan-before.log
   ```
...
- Source files: `mcp-server/scripts/repair-graph-metadata.mjs`, `mcp-server/lib/graph/graph-metadata-schema.ts`, `shared/embeddings/factory.ts`
