...
version: 3.6.0.16
id: memory-quality-and-indexing-canonical-continuity-save-substrate
...
The first command requires a real canonical writer save, and commands 7-9 require reading files produced or refreshed by that save. The invocation constraints for this run allowed writes only to `.opencode/skills/system-spec-kit/manual-testing-playbook/memory-quality-and-indexing/canonical-continuity-save-substrate.md`, so executing the save would modify files outside the allowed write path. No `memory_save`, canonical writer CLI, continuity-freshness validator, or resume ladder command was run because the playbook cannot be executed under the allowed write-path constraint.
...
- Feature file path: `memory-quality-and-indexing/canonical-continuity-save-substrate.md`
