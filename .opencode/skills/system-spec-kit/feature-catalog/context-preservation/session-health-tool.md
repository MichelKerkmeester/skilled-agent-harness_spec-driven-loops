...
mcp-server/handlers/session-health.ts
...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/handlers/session-health.ts` | Handler | Session status computation with time-based thresholds |
| `mcp-server/lib/session/context-metrics.ts` | Lib | QualityScore computation with 4-factor scoring |
| `mcp-server/hooks/memory-surface.ts` | Hook | Provides isSessionPrimed(), getSessionTimestamps(), getCodeGraphStatusSnapshot() |
| `mcp-server/tools/lifecycle-tools.ts` | Dispatch | Tool dispatch registration for session_health |
