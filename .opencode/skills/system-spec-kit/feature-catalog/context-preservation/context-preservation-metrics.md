...
mcp-server/lib/session/context-metrics.ts
...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/session/context-metrics.ts` | Lib | Event recording and QualityScore computation |
| `mcp-server/handlers/session-health.ts` | Handler | Consumer: includes QualityScore in session_health response |
| `mcp-server/lib/session/session-manager.ts` | Lib | Session state management (complements metrics) |
