...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/parsing/memory-parser.ts` | Lib | Added `/z_archive/` exclusion in `isMemoryFile()` during test-alignment fixes |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/memory-save-extended.vitest.ts` | Automated test | Timeout hardening (5000ms -> 15000ms) |
| `mcp-server/tests/entity-linker.vitest.ts` | Automated test | Added `db.close()` in `afterEach` to prevent handle leaks |
| `mcp-server/tests/integration-search-pipeline.vitest.ts` | Automated test | Reworked flag tests to validate behavior deltas |
...
Related references:
- [full-reporting-and-ablation-study-framework.md](../../feature-catalog/evaluation-and-measurement/full-reporting-and-ablation-study-framework.md) — Full reporting and ablation study framework
