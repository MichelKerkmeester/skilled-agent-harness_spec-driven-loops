...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/eval/memory-state-baseline.ts` | Lib | Baseline metric capture and optional persistence |
| `mcp-server/lib/eval/eval-db.ts` | Lib | Eval DB initialization and schema support for persisted snapshots |
| `mcp-server/lib/config/capability-flags.ts` | Lib | Memory-roadmap phase and flag metadata snapshot |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/memory-state-baseline.vitest.ts` | Automated test | Snapshot capture, persistence path selection, and missing-DB fallbacks |
