...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/llm-reformulation.ts` | Lib | Seed retrieval, LLM reformulation, step-back abstraction, variant generation |
| `mcp-server/lib/search/llm-cache.ts` | Lib | Shared LLM result cache (used by both reformulation and HyDE) |
| `mcp-server/lib/search/sqlite-fts.ts` | Lib | FTS5/BM25 keyword search for seed retrieval |
| `mcp-server/lib/search/search-flags.ts` | Lib | `isLlmReformulationEnabled()` flag accessor |
| `mcp-server/lib/search/pipeline/stage1-candidate-gen.ts` | Lib | Stage 1 integration: invokes reformulation in deep mode |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/search-flags.vitest.ts` | Automated test | Flag behavior for LLM reformulation |
...
- Feature file path: `query-intelligence/llm-query-reformulation.md`
Related references:
- [query-expansion.md](../../feature-catalog/query-intelligence/query-expansion.md) — Query expansion
- [hyde-hypothetical-document-embeddings.md](../../feature-catalog/query-intelligence/hyde-hypothetical-document-embeddings.md) — HyDE (Hypothetical Document Embeddings)
