...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/dynamic-token-budget.ts` | Lib | Token budget computation |
| `mcp-server/lib/search/hybrid-search.ts` | Lib | Budget adjustment with header overhead (CHK-060) |
...
| `mcp-server/tests/token-budget.vitest.ts` | Automated test | Token budget tests + CHK-023 adjustedBudget formula |
