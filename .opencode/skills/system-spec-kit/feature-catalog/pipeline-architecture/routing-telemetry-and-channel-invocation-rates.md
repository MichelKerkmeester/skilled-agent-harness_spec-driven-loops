...
`mcp-server/lib/search/routing-telemetry.ts` records each routing decision with channel attribution and writes a structured `routingReasons` field to `mcp-server/data/search-decisions.jsonl`. The health handler reads the in-process counters and reports them on the `data.routing` envelope.
...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/routing-telemetry.ts` | Library | Rolling window, channel attribution, rate computation |
| `mcp-server/lib/search/query-router.ts` | Library | Routing decisions, graph-channel preservation gating |
| `mcp-server/handlers/memory-crud-health.ts` | Handler | Surfaces `data.routing` envelope on health responses |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/query-router.vitest.ts` | Automated test | Routing decisions and telemetry recording shape |
| `mcp-server/tests/query-router-channel-interaction.vitest.ts` | Automated test | Channel-interaction matrix |
| `mcp-server/tests/routing-telemetry-stress.vitest.ts` | Automated test | Window stability under load and reset behavior |
...
- Feature file path: `pipeline-architecture/routing-telemetry-and-channel-invocation-rates.md`
Related references:
