...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/storage/lineage-state.ts` | Lib | Lineage append, active projection resolution, and `asOf` lookup |
| `mcp-server/lib/search/vector-index-schema.ts` | Lib | Lineage schema creation and compatibility checks |
| `mcp-server/handlers/memory-save.ts` | Handler | Save-path integration for lineage writes and projection updates |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/memory-lineage-state.vitest.ts` | Automated test | Active projection and deterministic `asOf` selection |
| `mcp-server/tests/memory-lineage-backfill.vitest.ts` | Automated test | Backfill planning, idempotent apply, and rollback restore behavior |
