...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/pipeline/stage1-candidate-gen.ts` | Lib | D1: Summary quality bypass fix — summary candidates now pass through `minQualityScore` filter |
| `mcp-server/lib/search/sqlite-fts.ts` | Lib | D2: FTS5 tokenization — refactored to shared `sanitizeQueryTokens()` function |
| `mcp-server/lib/search/bm25-index.ts` | Lib | D2: BM25 tokenization — now consumes shared `sanitizeQueryTokens()` from `sqlite-fts.ts` |
| `mcp-server/lib/search/channel-representation.ts` | Lib | D3: `QUALITY_FLOOR` lowered from 0.2 to 0.005 for RRF-range compatibility |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/channel-representation.vitest.ts` | Automated test | Quality floor and channel representation tests |
| `mcp-server/tests/sqlite-fts.vitest.ts` | Automated test | SQLite FTS5 tokenization tests |
| `mcp-server/tests/bm25-index.vitest.ts` | Automated test | BM25 tokenization tests |
...
- Feature file path: `pipeline-architecture/search-pipeline-safety.md`
Related references:
- [learned-relevance-feedback.md](../../feature-catalog/pipeline-architecture/learned-relevance-feedback.md) — Learned relevance feedback
- [performance-improvements.md](../../feature-catalog/pipeline-architecture/performance-improvements.md) — Performance improvements
