...
| File | Type | Role |
|---|---|---|
| `mcp-server/tests/launcher-session-proxy.vitest.ts` | Automated test | Session-proxy reattach, replay, keepalive, and protocol-mismatch fail-closed behavior |
| `mcp-server/tests/launcher-ipc-bridge-probe.vitest.ts` | Automated test | IPC bridge probe and socket-path resolution |
| `mcp-server/tests/launcher-watchdog.vitest.ts` | Automated test | Watchdog / recycle trigger behavior |
| `mcp-server/stress-test/durability/daemon-recycle-transparency-stress.vitest.ts` | Automated test | Daemon-recycle transparency under load (secondary clients survive a recycle) |
