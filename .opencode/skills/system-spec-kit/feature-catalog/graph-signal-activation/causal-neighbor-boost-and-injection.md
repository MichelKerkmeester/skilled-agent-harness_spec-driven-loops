...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/causal-boost.ts` | Lib | Causal neighbor graph walk and boost application |
| `mcp-server/lib/search/pipeline/stage2-fusion.ts` | Lib | Stage 2 fusion invoking causal boost |
| `mcp-server/lib/search/pipeline/types.ts` | Lib | Pipeline type definitions |
| `mcp-server/lib/search/session-boost.ts` | Lib | Session boost (shared ceiling with causal) |
...
| `mcp-server/tests/phase2-integration.vitest.ts` | Automated test | Phase 2 integration tests |
...
- Feature file path: `graph-signal-activation/causal-neighbor-boost-and-injection.md`
Related references:
