...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/causal-boost.ts` | Lib | `isTypedTraversalEnabled()`, sparse-first policy, intent-aware edge traversal, CTE walk scoring |
...
| File | Type | Role |
|---|---|---|
| `mcp-server/tests/causal-boost.vitest.ts` | Automated test | Causal boost with typed traversal, sparse-first gating, intent-aware scoring |
| `mcp-server/tests/sparse-first-graph.vitest.ts` | Automated test | Sparse-first policy coverage for traversal depth and community gating |
