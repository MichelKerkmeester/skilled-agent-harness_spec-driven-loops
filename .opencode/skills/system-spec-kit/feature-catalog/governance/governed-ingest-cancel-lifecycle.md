...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/handlers/memory-ingest.ts` | Handler | Start, status, and cancel surface plus job store wiring |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/handler-memory-ingest.vitest.ts` | Automated test | Job lifecycle including cancel acknowledgement and terminal state |
...
- Feature file path: `governance/governed-ingest-cancel-lifecycle.md`
Related references:
- [session-resume-caller-binding-and-unicode-sanitization.md](../../feature-catalog/governance/session-resume-caller-binding-and-unicode-sanitization.md) — Session-resume caller binding and Unicode sanitization
