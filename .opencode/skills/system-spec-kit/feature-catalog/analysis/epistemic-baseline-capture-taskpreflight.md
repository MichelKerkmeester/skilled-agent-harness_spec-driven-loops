...
| File | Role |
|------|------|
| `mcp-server/handlers/session-learning.ts` | Session learning handler: preflight baseline capture, UNIQUE constraint on (spec_folder, task_id) |
...
| `mcp-server/schemas/tool-input-schemas.ts` | Zod input schemas for `task_preflight` arguments |
| `mcp-server/tool-schemas.ts` | MCP-visible JSON schema for `task_preflight` |
...
- Feature file path: `analysis/epistemic-baseline-capture-taskpreflight.md`
...
- [post-task-learning-measurement-taskpostflight.md](../../feature-catalog/analysis/post-task-learning-measurement-taskpostflight.md) — Post-task learning measurement (task_postflight)
