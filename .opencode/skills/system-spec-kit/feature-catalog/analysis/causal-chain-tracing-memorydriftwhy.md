...
| File | Role |
|------|------|
| `mcp-server/handlers/causal-graph.ts` | MCP handler for `memory_drift_why`: depth-limited traversal, direction, cycle detection, relation weighting |
| `mcp-server/lib/storage/causal-edges.ts` | Causal edge storage: traversal queries for incoming/outgoing edges |
...
| `mcp-server/schemas/tool-input-schemas.ts` | Zod input schemas for drift_why arguments |
| `mcp-server/tool-schemas.ts` | MCP-visible JSON schema for `memory_drift_why` |
...
- Feature file path: `analysis/causal-chain-tracing-memorydriftwhy.md`
...
Related references:
- [causal-edge-deletion-memorycausalunlink.md](../../feature-catalog/analysis/causal-edge-deletion-memorycausalunlink.md) — Causal edge deletion (memory_causal_unlink)
