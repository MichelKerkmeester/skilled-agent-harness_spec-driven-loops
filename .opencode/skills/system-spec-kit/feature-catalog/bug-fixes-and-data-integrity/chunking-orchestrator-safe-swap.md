...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/handlers/chunking-orchestrator.ts` | Handler | Re-chunk swap logic with staged indexing |
| `mcp-server/handlers/memory-save.ts` | Handler | Chunked-save finalize path that records PE supersede state after chunked indexing succeeds |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/chunking-orchestrator-swap.vitest.ts` | Automated test | Staged-swap success, rollback, partial-embedding, and cache-key normalization regressions |
| `mcp-server/tests/handler-memory-save.vitest.ts` | Automated test | Chunked-save finalize rollback when PE supersede finalization fails after chunk indexing |
