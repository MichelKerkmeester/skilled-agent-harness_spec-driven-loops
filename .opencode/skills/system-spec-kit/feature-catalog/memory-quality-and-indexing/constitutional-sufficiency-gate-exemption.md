...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/handlers/memory-index.ts` | Scan handler | Line 474 OR-chain extended with `isConstitutional`. Rationale comment block names packet 018 and the policy-not-evidence reasoning |
| `mcp-server/handlers/memory-save.ts` | Save handler | Receives `qualityGateMode` from the scan path. Warn-only mode suppresses the `INSUFFICIENT_CONTEXT_ABORT` hard-reject |
...
- Feature file path: `memory-quality-and-indexing/constitutional-sufficiency-gate-exemption.md`
...
- [graph-metadata-and-lineage-repair-runner.md](../../feature-catalog/memory-quality-and-indexing/graph-metadata-and-lineage-repair-runner.md) — Graph-metadata and lineage repair runner
