...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/handlers/quality-loop.ts` | Handler | Quality scoring, retry loop, auto-fix logic, and actual-attempt reporting |
| `mcp-server/handlers/memory-save.ts` | Handler | Save-path integration, rejected status handling, fixed-content persistence, sufficiency handoff, and template-contract enforcement |
| `mcp-server/handlers/v-rule-bridge.ts` | Handler | Loads compiled V-rule validation and disposition logic consumed by save-time quality enforcement |
...
| File | Type | Role |
|---|---|---|
| `mcp-server/tests/quality-loop.vitest.ts` | Automated test | Attempt counting, retry behavior, and rejection semantics |
| `mcp-server/tests/handler-memory-save.vitest.ts` | Automated test | Rejected-save rollback semantics plus post-loop insufficiency hard-block coverage |
...
- Feature file path: `memory-quality-and-indexing/verify-fix-verify-memory-quality-loop.md`
- Source list updated 2026-03-25 per deep review
Related references:
- [signal-vocabulary-expansion.md](../../feature-catalog/memory-quality-and-indexing/signal-vocabulary-expansion.md) — Signal vocabulary expansion
