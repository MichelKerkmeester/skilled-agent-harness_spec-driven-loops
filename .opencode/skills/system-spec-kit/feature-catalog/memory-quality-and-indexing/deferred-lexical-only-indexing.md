...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/vector-index-mutations.ts` | Lib | Vector index mutations with deferred path |
| `mcp-server/lib/search/vector-index-schema.ts` | Lib | Vector index schema including embedding_status |
| `mcp-server/handlers/save/embedding-pipeline.ts` | Handler | Embedding generation pipeline with fallback |
