...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/validation/save-quality-gate.ts` | Lib | Quality gate layers, short-critical exception logic, structural signal counting |
| `mcp-server/lib/search/search-flags.ts` | Lib | `isSaveQualityGateExceptionsEnabled()` flag accessor |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/short-critical-quality-gate.vitest.ts` | Automated test | Short-critical exception behavior, structural signal thresholds, flag gating |
