...
| `runtime/cli/spec-folder/generate-description.ts` | CLI | Per-folder description.json generator |
...
| `runtime/cli/utils/slug-utils.ts` | Utils | ensureUniqueMemoryFilename collision prevention |
| `runtime/cli/core/workflow.ts` | Core | Memory tracking integration (`memorySequence` increment, `memoryNameHistory` ring buffer, and invalid-value hardening via `Number()` coercion) |
| `runtime/cli/spec/create.sh` | Shell | Auto-generation on folder creation |
...
| `runtime/tests/slug-utils-boundary.vitest.ts` | Automated test | Slug derivation and collision boundaries |
