...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/extraction/entity-extractor.ts` | Lib | Five regex extraction rules, entity dedup, `memory_entities` storage |
| `mcp-server/lib/extraction/entity-denylist.ts` | Lib | 64-word denylist filter (common nouns, tech stop words, generic modifiers) |
| `mcp-server/lib/search/entity-linker.ts` | Lib | Canonical `normalizeEntityName()` (Unicode-aware) and `computeEdgeDensity()` |
| `mcp-server/lib/extraction/extraction-adapter.ts` | Lib | Extraction adapter coordinating entity extraction at save time |
| `mcp-server/lib/search/search-flags.ts` | Lib | `isAutoEntitiesEnabled()` flag accessor (`SPECKIT_AUTO_ENTITIES`) |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/entity-extractor.vitest.ts` | Automated test | Entity extraction rule coverage |
| `mcp-server/tests/entity-linker.vitest.ts` | Automated test | Entity linking and normalization tests |
| `mcp-server/tests/extraction-adapter.vitest.ts` | Automated test | Extraction adapter tests |
