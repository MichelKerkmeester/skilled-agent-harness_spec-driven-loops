...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/parsing/trigger-matcher.ts` | Lib | Trigger phrase matching — CORRECTION and PREFERENCE signal categories added here |
| `mcp-server/configs/cognitive.ts` | Core | Cognitive configuration defining signal category vocabulary |
...
| `mcp-server/tests/trigger-config-extended.vitest.ts` | Automated test | Trigger config extended coverage |
...
- Feature file path: `memory-quality-and-indexing/signal-vocabulary-expansion.md`
Related references:
- [verify-fix-verify-memory-quality-loop.md](../../feature-catalog/memory-quality-and-indexing/verify-fix-verify-memory-quality-loop.md) — Verify-fix-verify memory quality loop
