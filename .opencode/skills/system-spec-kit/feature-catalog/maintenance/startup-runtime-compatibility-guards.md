...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/startup-checks.ts` | Core | Startup compatibility validation |
| `mcp-server/context-server.ts` | Core | Server initialization invoking checks |
| `mcp-server/cli.ts` | Core | CLI entry point with startup checks |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/startup-checks.vitest.ts` | Automated test | Runtime mismatch logic and SQLite version guard tests |
| `mcp-server/tests/context-server.vitest.ts` | Automated test | Context server tests |
| `mcp-server/tests/modularization.vitest.ts` | Automated test | Modularization tests |
...
Related references:
- [workspace-scanning-and-indexing-memoryindexscan.md](../../feature-catalog/maintenance/workspace-scanning-and-indexing-memoryindexscan.md) — Workspace scanning and indexing (memory_index_scan)
