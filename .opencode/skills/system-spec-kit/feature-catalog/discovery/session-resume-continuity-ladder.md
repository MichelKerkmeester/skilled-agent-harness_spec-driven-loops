...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/handlers/session-resume.ts` | Handler | Walks the continuity ladder, handles phase-parent redirect, returns layered response |
| `mcp-server/lib/spec/is-phase-parent.ts` | Library | Phase-parent detection rule (single source of truth) |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/session-resume.vitest.ts` | Automated test | Layer ordering, phase-parent redirect, missing-redirect listing |
...
- [session-health-shared-payload.md](../../feature-catalog/discovery/session-health-shared-payload.md) — Session health shared payload
