...
| File | Role |
|------|------|
| `mcp-server/handlers/memory-crud-list.ts` | Handler implementation, validation, pagination, and response shaping |
...
| `mcp-server/utils/json-helpers.ts` | Parses `trigger_phrases` for `triggerCount` |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/handler-memory-list-edge.vitest.ts` | Automated test | Sort fallback, validation envelopes, pagination clamps, and empty-string `specFolder` parity against seeded rows |
| `mcp-server/tests/handler-memory-crud.vitest.ts` | Automated test | Public export and direct input-validation coverage |
| `mcp-server/tests/memory-crud-extended.vitest.ts` | Automated test | Happy-path list structure, chunk inclusion, and trigger count parsing |
