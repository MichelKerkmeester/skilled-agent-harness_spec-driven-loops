...
| File | Role |
|------|------|
| `mcp-server/handlers/memory-crud-stats.ts` | Handler validation, aggregate queries, folder ranking, and response shaping |
| `mcp-server/handlers/memory-crud.ts` | Public CRUD exports and snake_case compatibility aliases |
| `mcp-server/lib/scoring/folder-scoring.ts` | Archive detection and folder scoring for non-count rankings |
| `mcp-server/lib/search/hybrid-search.ts` | Supplies graph channel metrics |
...
| File | Type | Role |
|---|---|---|
| `mcp-server/tests/handler-memory-stats-edge.vitest.ts` | Automated test | Limit behavior, validation envelopes, archived/excluded folder filtering, ranking modes, and `totalSpecFolders` regression coverage |
| `mcp-server/tests/handler-memory-crud.vitest.ts` | Automated test | Public export and direct validation coverage |
| `mcp-server/tests/memory-crud-extended.vitest.ts` | Automated test | Happy-path stats structure, scoring fallback, and composite-score payload coverage |
...
- Feature file path: `discovery/system-statistics-memorystats.md`
Related references:
- [memory-browser-memorylist.md](../../feature-catalog/discovery/memory-browser-memorylist.md) — Memory browser (memory_list)
