...
The handler lives at `.opencode/skills/system-code-graph/mcp-server/handlers/detect-changes.ts` and is exported from `.opencode/skills/system-code-graph/mcp-server/handlers/index.ts` alongside the other Code Graph handlers. It is registered as a code-graph MCP tool through `.opencode/skills/system-code-graph/mcp-server/tools/code-graph-tools.ts` and declared in `.opencode/skills/system-code-graph/mcp-server/tool-schemas.ts`. It accepts `{ diff: string, rootDir?: string }` and returns `{ status, affectedSymbols[], affectedFiles[], blockedReason?, timestamp, readiness }`.
...
| File | Layer | Role |
|------|-------|------|
| `.opencode/skills/system-code-graph/mcp-server/handlers/detect-changes.ts` | Handler | Read-only preflight; orchestrates readiness probe, diff parse, and overlap attribution |
| `.opencode/skills/system-code-graph/mcp-server/lib/diff-parser.ts` | Lib | Custom unified-diff parser (`parseUnifiedDiff`) plus `rangesOverlap` helper |
| `.opencode/skills/system-code-graph/mcp-server/handlers/index.ts` | Handler | Exports `handleDetectChanges` alongside the other Code Graph handlers |
| `.opencode/skills/system-code-graph/mcp-server/tools/code-graph-tools.ts` | Dispatcher | `'detect_changes'` in `TOOL_NAMES`; switch case dispatches to `handleDetectChanges` after validating `diff` is a non-empty string (010/007 T-A) |
| `.opencode/skills/system-code-graph/mcp-server/tool-schemas.ts` | JSON schema | `detect_changes` ToolDefinition with `{ diff: string, rootDir?: string }` declared inputSchema |
| `.opencode/skills/system-code-graph/mcp-server/lib/ensure-ready.ts` | Lib (existing) | Source of `ensureCodeGraphReady` readiness probe (consumed read-only) |
| `.opencode/skills/system-code-graph/mcp-server/lib/readiness-contract.ts` | Lib (existing) | Source of `buildReadinessBlock` envelope shape mirrored in the response |
| `.opencode/skills/system-code-graph/mcp-server/lib/code-graph-db.ts` | Lib (existing) | Source of `queryOutline` and `resolveSubjectFilePath` (consumed read-only) |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `.opencode/skills/system-code-graph/mcp-server/tests/detect-changes.test.ts` | Automated test | Blocked-on-stale safety invariant, parse_error contract, symbol attribution by line-range overlap, output-shape contract, diff-parser unit cases |
