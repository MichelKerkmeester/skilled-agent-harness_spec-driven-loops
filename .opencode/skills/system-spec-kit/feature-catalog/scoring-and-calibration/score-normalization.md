...
|----------|----------------------|-----------------|
| RRF normalization gate and transform (`SPECKIT_SCORE_NORMALIZATION`, `isScoreNormalizationEnabled`, `normalizeRrfScores`) | `shared/algorithms/rrf-fusion.ts` | `mcp-server/tests/score-normalization.vitest.ts` |
| Cross-variant RRF normalization path | `shared/algorithms/rrf-fusion.ts` (`fuseResultsCrossVariant`) | `mcp-server/tests/score-normalization.vitest.ts` |
| Composite normalization gate and transform (`isCompositeNormalizationEnabled`, `normalizeCompositeScores`) | `mcp-server/lib/scoring/composite-scoring.ts` | `mcp-server/tests/score-normalization.vitest.ts` |
...
Related references:
- [interference-scoring.md](../../feature-catalog/scoring-and-calibration/interference-scoring.md) — Interference scoring
