...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/eval/k-value-analysis.ts` | Lib | `isKExperimentalEnabled()`, `runJudgedKSweep()`, K-value sensitivity analysis, NDCG/MRR computation |
...
| File | Type | Role |
|---|---|---|
| `mcp-server/tests/k-value-judged-sweep.vitest.ts` | Automated test | Judged K sweep, per-intent selection, flag gating |
| `mcp-server/tests/k-value-optimization.vitest.ts` | Automated test | K-value optimization and flag behavior |
...
- [usage-weighted-ranking.md](../../feature-catalog/scoring-and-calibration/usage-weighted-ranking.md) — Usage-weighted ranking signal
