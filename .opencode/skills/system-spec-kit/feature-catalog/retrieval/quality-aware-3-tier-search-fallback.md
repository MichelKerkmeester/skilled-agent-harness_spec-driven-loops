...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/hybrid-search.ts` | Lib | Multi-channel search orchestration: `executeFallbackPlan()`, `searchWithFallbackTiered()`, `enrichFusedResults()`, `checkDegradation()`, 3-tier fallback chain |
| `mcp-server/lib/search/search-flags.ts` | Lib | Feature flag registry (`SPECKIT_SEARCH_FALLBACK` gate) |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/search-fallback-tiered.vitest.ts` | Automated test | Tiered fallback chain behavior |
| `mcp-server/tests/hybrid-search.vitest.ts` | Automated test | Hybrid search orchestration |
| `mcp-server/tests/hybrid-search-flags.vitest.ts` | Automated test | Hybrid search flag behavior |
...
- Feature file path: `retrieval/quality-aware-3-tier-search-fallback.md`
