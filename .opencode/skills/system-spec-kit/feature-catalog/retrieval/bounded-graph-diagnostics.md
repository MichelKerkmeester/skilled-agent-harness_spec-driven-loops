...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/formatters/search-results.ts` | Formatter | Attaches `trace.graphContribution` envelope to each result when tracing is on |
| `mcp-server/lib/search/hybrid-search.ts` | Library | Computes raw, normalized, and applied bonus values; emits `capApplied` flag |
| `mcp-server/handlers/memory-search.ts` | Handler | Surfaces the trace envelope to callers when `includeTrace` is true |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/search-results-format.vitest.ts` | Automated test | Trace envelope shape, ordering stability, cap behavior |
| `mcp-server/tests/hybrid-search.vitest.ts` | Automated test | Bonus math, normalization, and cap thresholds |
...
- [memory-search-command-routing.md](../../feature-catalog/retrieval/memory-search-command-routing.md) — /memory:search command routing
