...
3. `install.sh` is the environment bootstrap for the bundled `system-spec-memory` MCP. It requires Node.js >= 20.11, `npm`, and `npx`; logs the current Node version, module ABI version, and binary path; and verifies that the expected Spec Kit workspace, MCP server folder, and both `package.json` files exist before proceeding.
