...
The module tracks exactly 7 dist-producing packages in its `DIST_PACKAGES` registry: `system-spec-kit/shared`, `system-spec-kit/scripts`, `system-spec-kit/mcp-server` (with named `spec-memory-cli` and `validation-orchestrator` sub-entries in addition to its default entry), `mcp-code-mode/mcp-server`, `system-skill-advisor/mcp-server` (with a `skill-advisor-cli` sub-entry), `system-code-graph/mcp-server` (with a `code-index-cli` sub-entry), and `sk-design/sk-design-md-generator/backend`.
