...
Integration happens in `runtime/cli/core/workflow.ts` at Step 7.6, where rendered file changes are transformed into thinning inputs, processed through `applyTreeThinning()` and then applied back to the effective file set before downstream retrieval/scoring logic.
...
| File | Layer | Role |
|------|-------|------|
| `runtime/cli/core/tree-thinning.ts` | Core script (primary) | Tree thinning model and implementation (`applyTreeThinning()`, thresholds, parent merges) |
| `runtime/cli/core/workflow.ts` | Core workflow (integration) | Step 7.6 integration (`applyTreeThinning()` and `applyThinningToFileChanges()`) |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `runtime/cli/tests/tree-thinning.vitest.ts` | Automated test | Tree-thinning thresholds, merge behavior, boundary handling and invariants |
...
- Feature file path: `tooling-and-scripts/tree-thinning-for-spec-folder-consolidation.md`
Related references:
