...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/ops/file-watcher.ts` | Lib | Filesystem watcher |
| `mcp-server/context-server.ts` | Server | Starts and stops the watcher during MCP server lifecycle |
| `mcp-server/lib/search/search-flags.ts` | Lib | Exposes the `SPECKIT_FILE_WATCHER` gate via `isFileWatcherEnabled()` |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/file-watcher.vitest.ts` | Automated test | Coverage for path filtering, debounce/retry/delete behavior, rename integration (unlink+add lifecycle, old-entry removal), burst/concurrent rename handling and watcher metrics (`filesReindexed`, `avgReindexTimeMs`) |
...
- [standalone-admin-cli.md](../../feature-catalog/tooling-and-scripts/standalone-admin-cli.md) — Standalone admin CLI
