...
| File | Layer | Role |
|---|---|---|
| `mcp-server/hooks/spec-memory-cli-fallback.ts` | Hook helper | Shared warm-only spec-memory CLI fallback |
| `mcp-server/hooks/code-index-cli-fallback.ts` | Hook helper | Shared warm-only code-index CLI fallback |
...
Related references:
- [spec-memory-cli-daemon-backed-surface.md](../../feature-catalog/tooling-and-scripts/spec-memory-cli-daemon-backed-surface.md) — Daemon-backed spec-memory CLI surface
