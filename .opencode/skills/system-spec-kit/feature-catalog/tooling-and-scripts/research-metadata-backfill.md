...
Commit `88063287b` introduced `runtime/cli/continuity/backfill-research-metadata.ts`.
...
| File | Layer | Role |
|------|-------|------|
| `runtime/cli/continuity/backfill-research-metadata.ts` | Script | Creates missing metadata files for research iteration folders |
| `runtime/cli/core/workflow.ts` | Orchestrator | Invokes the research backfill step from the canonical save workflow when research children are present |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `runtime/cli/tests/backfill-research-metadata.vitest.ts` | Automated test | Backfill behavior for missing-vs-complete research iteration folders |
