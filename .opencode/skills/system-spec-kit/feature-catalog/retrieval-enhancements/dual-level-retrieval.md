...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/handlers/memory-search.ts` | Handler | `retrievalLevel` parameter handling and mode dispatch |
| `mcp-server/lib/search/search-flags.ts` | Lib | Flag registration and `retrievalLevel` parameter definition |
