...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/hybrid-search.ts` | Lib | Injects contextual headers after Stage 4 truncation and builds the cached tail-description map |
| `mcp-server/lib/search/search-flags.ts` | Lib | Exposes the `SPECKIT_CONTEXT_HEADERS` runtime gate |
...
- [session-boost-graduated.md](../../feature-catalog/retrieval-enhancements/session-boost-graduated.md) — Session attention boost (graduated default-ON)
