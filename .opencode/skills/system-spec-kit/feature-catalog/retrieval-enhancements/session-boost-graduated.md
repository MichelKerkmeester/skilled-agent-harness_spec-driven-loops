...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/session-boost.ts` | Lib | Session attention score application |
| `mcp-server/lib/search/search-flags.ts` | Lib | `isSessionBoostEnabled()` flag check |
| `mcp-server/lib/search/pipeline/stage2-fusion.ts` | Lib | Stage 2 fusion invoking session boost |
