...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/entity-linker.ts` | Lib | Builds the entity catalog, finds cross-document matches, and creates `supports` edges with density-guard enforcement |
| `mcp-server/handlers/save/post-insert.ts` | Handler | Save-time enrichment path that invokes entity linking after entity extraction succeeds |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/entity-linker.vitest.ts` | Automated test | Unit coverage for catalog build, match discovery, link creation, and density-guard behavior |
| `mcp-server/tests/deferred-features-integration.vitest.ts` | Automated test | End-to-end entity-linking integration through `runEntityLinking()` |
...
- Feature file path: `retrieval-enhancements/cross-document-entity-linking.md`
Related references:
- [memory-summary-search-channel.md](../../feature-catalog/retrieval-enhancements/memory-summary-search-channel.md) — Memory summary search channel
