...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/handlers/memory-crud-update.ts` | Handler | Update handler |
| `mcp-server/lib/providers/embeddings.ts` | Lib | Embedding regeneration for title changes |
| `mcp-server/lib/search/bm25-index.ts` | Lib | BM25 re-index when title or trigger phrases change |
| `mcp-server/lib/search/vector-index.ts` | Lib | Persistent memory row updates |
| `mcp-server/lib/storage/history.ts` | Lib | Update history writes |
| `mcp-server/lib/storage/mutation-ledger.ts` | Lib | Update audit ledger |
| `mcp-server/lib/storage/transaction-manager.ts` | Lib | Transaction wrapper for the mutation path |
...
| `mcp-server/tests/memory-crud-extended.vitest.ts` | Automated test | Update-path rollback and partial-update scenarios |
...
- Feature file path: `mutation/memory-metadata-update-memoryupdate.md`
Related references:
- [memory-indexing-memorysave.md](../../feature-catalog/mutation/memory-indexing-memorysave.md) — Memory indexing (memory_save)
