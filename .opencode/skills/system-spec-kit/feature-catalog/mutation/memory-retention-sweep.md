...
The MCP schema registers the tool under L4 mutation and the memory-tool dispatcher routes `memory_retention_sweep` to the handler. The tool contributes to the current `mk-spec-memory` server's tool count (canonical source: `TOOL_DEFINITIONS.length` in `mcp-server/tool-schemas.ts`).
