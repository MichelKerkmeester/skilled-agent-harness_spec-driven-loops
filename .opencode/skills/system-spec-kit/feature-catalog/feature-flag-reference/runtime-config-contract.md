...
|---|---|
| Loader path | `runtime/cli/core/config.ts` reads `config/config.jsonc`, strips JSONC comments with `stripJsoncComments()`, parses the file, shallow-merges user values onto defaults, then validates the resulting `WorkflowConfig`. |
...
| File | Type | Role |
|---|---|---|
| `runtime/cli/tests/runtime-memory-inputs.vitest.ts` | Automated test | Legacy `qualityAbortThreshold` normalization and explicit config-input failure handling in the script runtime |
