...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/hooks/index.ts` | Hook | Module barrel export for all UX hook modules |
| `mcp-server/hooks/mutation-feedback.ts` | Hook | Dedicated mutation feedback hook module |
| `mcp-server/hooks/response-hints.ts` | Hook | Dedicated response hint hook module |
| `mcp-server/hooks/memory-surface.ts` | Hook | Dedicated auto-surface UX hook module |
| `mcp-server/handlers/memory-crud-types.ts` | Handler | Shared `MutationHookResult` contract consumed by hook modules |
