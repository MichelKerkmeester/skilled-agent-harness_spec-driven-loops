...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/pipeline/stage2-fusion.ts` | Lib | Graph evidence population during fusion |
| `mcp-server/formatters/search-results.ts` | Formatter | Graph evidence serialization in response envelope |
| `mcp-server/lib/search/pipeline/types.ts` | Lib | `graphEvidence` type definition |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `mcp-server/tests/provenance-envelope.vitest.ts` | Automated test | Provenance envelope structure and graph evidence content |
...
- Feature file path: `ux-hooks/result-provenance.md`

- Kill switch: SPECKIT_RESULT_PROVENANCE=false
Related references:
- [result-confidence.md](../../feature-catalog/ux-hooks/result-confidence.md) — Result confidence scoring
