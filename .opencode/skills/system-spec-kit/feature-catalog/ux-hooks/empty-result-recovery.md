...
| File | Layer | Role |
|------|-------|------|
| `mcp-server/lib/search/recovery-payload.ts` | Lib | Recovery status classification, reason inference, suggested query generation, payload construction |
...
| File | Type | Role |
|---|---|---|
| `mcp-server/tests/d5-recovery-payload.vitest.ts` | Automated test | Recovery payload generation, status classification, flag gating |
