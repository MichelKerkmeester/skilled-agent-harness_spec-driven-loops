...
|----------|---------|---------|
| `MEMORY_DB_PATH` | Unset | Explicit file override for the active memory database |
...
| `SPECKIT_STRICT_SCHEMAS` | `true` | Enforce strict Zod MCP tool input validation for the 41-tool `mk-spec-memory` MCP surface (`TOOL_DEFINITIONS.length` in `mcp_server/tool-schemas.ts`; `false` allows unknown passthrough keys and is development-only) |
...
| `SPECKIT_MEMORY_*` | `SPECKIT_MEMORY_ROADMAP_PHASE`, `SPECKIT_MEMORY_ADAPTIVE_RANKING` | Memory roadmap and adaptive ranking controls |
...
| `SPECKIT_PROGRESSIVE_DISCLOSURE` | ON | R-011 | Cursor-based pagination with progressive detail levels |
| `SPECKIT_SESSION_RETRIEVAL_STATE` | ON | R-011 | Cross-turn session state with seen-dedup and goal alignment |
...
| `SPECKIT_EMPTY_RESULT_RECOVERY` | ON | R-011 | Structured recovery payloads for empty/weak search results. Classifies outcome, diagnoses cause, suggests alternatives |
| `SPECKIT_RESULT_CONFIDENCE` | ON | R-011 | Per-result calibrated confidence from 3 weighted factors: margin (0.35), channel agreement (0.30), anchor density (0.15) |
...
| `SPECKIT_RESULT_EXPLAIN` | ON | R-011 | Two-tier result explainability: slim (summary + topSignals) and debug (per-channel breakdown). Set `false` to disable |
| `SPECKIT_RESPONSE_PROFILE` | ON | R-011 | Mode-aware response profiles: quick, research, resume, debug. Set `false` to disable. Original response when profile omitted |
...
| ~~`SPECKIT_HYDRA_*`~~ | - | - | Removed in commit `6f2c2c939`. All HYDRA aliases, SCOPE_ENFORCEMENT, and GOVERNANCE_GUARDRAILS flags deleted with the shared-memory infrastructure. |
