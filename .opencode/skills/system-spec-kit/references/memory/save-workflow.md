...
├── spec.md                      # vision + Phase Documentation Map (no continuity block)
├── description.json             # discovery metadata
├── graph-metadata.json          # children rollup + derived.last_active_child_id pointer
├── 001-phase/                   # phase children carry implementation-summary.md + heavy docs
└── 002-phase/
```

The parent has NO `implementation-summary.md`, NO `plan.md`, NO `tasks.md`, NO `acceptance-criteria.md`, NO `decision-record.md`. Continuity for a phase parent is the pointer in `graph-metadata.json`, refreshed every save by the generator. The lean phase-parent template at `templates/packet-types/phase-parent.spec.md.tmpl` is the canonical scaffold.
