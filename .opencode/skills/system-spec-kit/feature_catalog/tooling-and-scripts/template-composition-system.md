...
4. Level 1 packets receive the baseline four documents. Level 2 adds `checklist.md`. Level 3 and Level 3+ add `decision-record.md`, with Level 3+ retaining the extended governance sections. Phase-parent packets stay lean and write only the parent control documents. The review-record level (public label Review Record, resolver value `review`, template `review.spec.md.tmpl`) is a lean packet type that requires only `spec.md` plus `review/review-report.md` and waives the numbered-level plan, tasks and implementation-summary documents.
...
| File | Layer | Role |
|------|-------|------|
| `.opencode/skills/system-spec-kit/mcp_server/lib/templates/level-contract-resolver.ts` | Resolver | Returns the document contract for each public Level |
...
Related references:
- [setup-native-module-health-and-mcp-installation.md](setup-native-module-health-and-mcp-installation.md) — Setup, Native Module Health, and MCP Installation
- [evaluation-benchmark-and-import-policy-tooling.md](evaluation-benchmark-and-import-policy-tooling.md) — Evaluation, benchmark, and import-policy tooling
