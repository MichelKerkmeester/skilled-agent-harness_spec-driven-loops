...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/cli.vitest.ts` | Automated test | CLI integration tests: stats success, bulk-delete execution with checkpoint creation, reindex success, schema-downgrade guardrails (`--confirm` required + safe failure for unsupported downgrade path), unknown command/missing-arg errors and database-path failure handling |
...
| `mcp_server/tests/mutation-ledger.vitest.ts` | Automated test | Mutation ledger tests |
...
- Feature file path: `tooling-and-scripts/standalone-admin-cli.md`
- Playbook reference: 113
Related references:
- [real-time-filesystem-watching-with-chokidar.md](real-time-filesystem-watching-with-chokidar.md) — Real-time filesystem watching with chokidar
- [watcher-delete-rename-cleanup.md](watcher-delete-rename-cleanup.md) — Watcher delete/rename cleanup
