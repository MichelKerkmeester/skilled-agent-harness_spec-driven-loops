---
title: "Memory roadmap flags (SPECKIT_MEMORY_*)"
description: "Feature flag reference for the live memory roadmap resolvers."
trigger_phrases:
  - "memory roadmap flags"
  - "SPECKIT_MEMORY_*"
...
This document captures the implemented behavior, source references, and validation scope for the spec-doc record roadmap flag resolvers. The roadmap metadata contract stays separate from live runtime gates such as `SPECKIT_GRAPH_UNIFIED`.

---

## 1. OVERVIEW

This feature-reference file mirrors the feature-catalog snippet pattern: a concise overview of the roadmap flag family, the live current-reality table, and the source metadata needed to keep docs and runtime behavior aligned.
...
Roadmap flag entries are evaluated at call time, not frozen import-time, and the roadmap snapshot stays distinct from live runtime defaults.
...
- Feature file path: `19--feature-flag-reference/memory-roadmap-capability-flags.md`
Related references:
- [filter-config-contract.md](filter-config-contract.md) — Filter Config Contract
- [launcher-idle-timeout.md](launcher-idle-timeout.md) — Launcher Idle Timeout
