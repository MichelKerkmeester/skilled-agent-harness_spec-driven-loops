...
The MCP schema registers the tool under L4 mutation and the memory-tool dispatcher routes `memory_retention_sweep` to the handler. The tool contributes to the current `mk-spec-memory` server's 41-tool count.
...
- Feature file path: `02--mutation/026-memory-retention-sweep.md`

- Packet source: `005-memory-retention-sweep`
Related references:
- [025-reconsolidation-conflict-transaction-helper.md](025-reconsolidation-conflict-transaction-helper.md) — Reconsolidation conflict transaction helper
