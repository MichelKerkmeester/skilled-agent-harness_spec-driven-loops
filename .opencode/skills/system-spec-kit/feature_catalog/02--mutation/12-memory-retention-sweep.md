...
The MCP schema registers the tool under L4 mutation and the memory-tool dispatcher routes `memory_retention_sweep` to the handler. The tool contributes to the current `spec_kit_memory` server's 41-tool count.
