...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/memory-save-ux-regressions.vitest.ts` | Automated test | Atomic save parity: `postMutationHooks` contract fields, duplicate no-op suppression |
| `mcp_server/tests/memory-save-integration.vitest.ts` | Automated test | Save integration covering both standard and atomic paths |
| `mcp_server/tests/memory-save-extended.vitest.ts` | Automated test | Extended save scenarios including partial-indexing guidance |
...
Related references:
- [duplicate-save-no-op-feedback-hardening.md](duplicate-save-no-op-feedback-hardening.md) — Duplicate-save no-op feedback hardening
