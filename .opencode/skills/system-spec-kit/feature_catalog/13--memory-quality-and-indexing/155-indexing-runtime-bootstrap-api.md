...
**Index-scope invariants:** file collection is governed by the shared path-policy module `mcp_server/lib/utils/index-scope.ts`. Its `shouldIndexForMemory()`, `shouldIndexForCodeGraph()`, and `isIndexableConstitutionalMemoryPath()` predicates permanently exclude `z_future/` and `/external/` subtrees across memory discovery, spec-doc classification, parser admissibility, code-graph recursive scanning, and storage-layer tier mutation surfaces. `resolveCanonicalPath()` hardens the exclusion check against symlink escapes before policy evaluation. The constitutional tier is restricted to real rule files under `.opencode/skills/system-spec-kit/constitutional/` — the constitutional `README.md` stays excluded per ADR-005 and storage-boundary remediation. Defense-in-depth runs at save time: `memory-save.ts` rejects excluded paths with `E_MEMORY_INDEX_SCOPE_EXCLUDED` and downgrades non-indexable constitutional `importanceTier: constitutional` to `important` before persistence, recording a `tier_downgrade_non_constitutional_path` row in the governance audit. Walker DoS caps surface structured `warnings` and `capExceeded` fields. Operator maintenance for pre-existing pollution lives in `scripts/dist/memory/cleanup-index-scope-violations.js` (`--apply` / `--verify`); verify counts should be `constitutional_total=2`, `z_future_rows=0`, `external_rows=0`, `invalid_constitutional_rows=0`.

### Async & Safety

**Scan-originated `fromScan` contract:** when `handleMemoryIndexScan` delegates a file to `memory_save`, it tags the save with `fromScan: true`. The save handler uses that flag to skip only the transactional complement-recheck path that previously fired `candidate_changed` false-positives on scan-originated saves. Interactive (non-scan) saves retain the full recheck. In parallel, the PE orchestration path now compares `SimilarMemory.canonical_file_path` to the current target: cross-file `UPDATE` / `REINFORCE` candidates are downgraded to `CREATE`, which prevents sibling packet docs from driving lineage-bearing updates across files (the prior `E_LINEAGE` class). The default `BATCH_SIZE` was restored — an earlier `scanBatchSize = 1` serialization approach was rolled back in favor of the `fromScan` bypass.
---
## 3. SOURCE FILES
### Implementation
| File | Layer | Role |
...
- Feature file path: `13--memory-quality-and-indexing/155-indexing-runtime-bootstrap-api.md`
Related references:
- [154-save-quality-gate-exceptions.md](154-save-quality-gate-exceptions.md) — Save quality gate exceptions
- [156-spec-doc-structure-validator.md](156-spec-doc-structure-validator.md) — Spec-doc structure validator
