...
The pre-storage quality gate validates structural integrity, content quality and semantic deduplication before a spec-doc record enters the index.
...
The pre-storage quality gate still validates structure, content quality, and semantic deduplication before a spec-doc record enters the index, but it is no longer the first semantic stop in the save pipeline.
...
The three-layer pre-storage quality gate then handles the spec-doc records that are already semantically sufficient:

- Layer 1: structural validity
- Layer 2: content quality scoring across five dimensions
- Layer 3: semantic deduplication against existing spec-doc records in the same spec folder
...
| `mcp_server/tests/handler-memory-save.vitest.ts` | Automated test | Save handler ordering and insufficiency hard-block coverage |
...
- Feature file path: `13--memory-quality-and-indexing/138-pre-storage-quality-gate.md`
Related references:
- [137-spec-folder-description-discovery.md](137-spec-folder-description-discovery.md) — Spec folder description discovery
- [139-reconsolidation-on-save.md](139-reconsolidation-on-save.md) — Reconsolidation-on-save
