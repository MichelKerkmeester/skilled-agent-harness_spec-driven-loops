...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/folder-discovery.vitest.ts` | Automated test | PerFolderDescription schema, generation, required-field/array validation, path-containment hardening, `slugifyFolderName`, CRUD/staleness, and CRLF frontmatter stripping |
| `mcp_server/tests/folder-discovery-integration.vitest.ts` | Automated test | Mixed-mode aggregation, fresh-vs-stale preference, corrupt/missing fallback, cache staleness, regeneration behavior, and performance checks |
| `scripts/tests/slug-uniqueness.vitest.ts` | Automated test | Filename collision resolution |
| `mcp_server/tests/workflow-memory-tracking.vitest.ts` | Automated test | Load-mutate-save cycle for `memorySequence` increment and `memoryNameHistory` ring buffer |
...
- Feature file path: `13--memory-quality-and-indexing/137-spec-folder-description-discovery.md`
Related references:
- [136-pre-flight-token-budget-validation.md](136-pre-flight-token-budget-validation.md) — Pre-flight token budget validation
- [138-pre-storage-quality-gate.md](138-pre-storage-quality-gate.md) — Pre-storage quality gate
