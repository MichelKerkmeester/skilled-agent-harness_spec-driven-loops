...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/co-activation.vitest.ts` | Automated test | Co-activation spreading tests, including batched lookup coverage for related-memory and causal-neighbor hydration |
| `mcp_server/tests/rrf-degree-channel.vitest.ts` | Automated test | Fan-effect divisor behavior in `boostScore()` and co-activation boost interactions |
| `mcp_server/tests/stage2-fusion.vitest.ts` | Automated test | Stage 2 adjacent scoring-path coverage, including one-shot co-activation neighbor-count precomputation |
...
- Feature file path: `08--bug-fixes-and-data-integrity/058-co-activation-fan-effect-divisor.md`
Related references:
- [057-chunk-collapse-deduplication.md](057-chunk-collapse-deduplication.md) — Chunk collapse deduplication
- [059-sha-256-content-hash-deduplication.md](059-sha-256-content-hash-deduplication.md) — SHA-256 content-hash deduplication
