...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/hybrid-search.vitest.ts` | Automated test | `T031-LEX-05` and `T031-BASIC-04` canonical-ID dedup regression coverage for lexical and legacy hybrid flows |
| `mcp_server/tests/unit-rrf-fusion.vitest.ts` | Automated test | `T006b` canonical numeric/string/`mem:` fusion coverage for multi-source result merging |
| `mcp_server/tests/content-hash-dedup.vitest.ts` | Automated test | `T320-1` and `T320-2` coverage for two direct save-path probes and exact scope clauses |
| `mcp_server/tests/vector-index-schema-migration-refinements.vitest.ts` | Automated test | Verifies `idx_save_parent_canonical_path` and `idx_save_parent_content_hash_scope` exist with `WHERE parent_id IS NULL` |
...
- Feature file path: `08--bug-fixes-and-data-integrity/canonical-id-dedup-hardening.md`
Related references:
- [guards-and-edge-cases.md](guards-and-edge-cases.md) — Guards and edge cases
- [error-response-credential-sanitization.md](error-response-credential-sanitization.md) — Error response credential sanitization
