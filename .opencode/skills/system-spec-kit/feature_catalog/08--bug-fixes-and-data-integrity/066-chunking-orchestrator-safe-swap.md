...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/chunking-orchestrator-swap.vitest.ts` | Automated test | Staged-swap success, rollback, partial-embedding, and cache-key normalization regressions |
| `mcp_server/tests/handler-memory-save.vitest.ts` | Automated test | Chunked-save finalize rollback when PE supersede finalization fails after chunk indexing |
...
- Feature file path: `08--bug-fixes-and-data-integrity/066-chunking-orchestrator-safe-swap.md`
Related references:
- [065-session-manager-transaction-gap-fixes.md](065-session-manager-transaction-gap-fixes.md) — Session-manager transaction gap fixes
- [067-working-memory-timestamp-fix.md](067-working-memory-timestamp-fix.md) — Working Memory Session Cleanup Timestamp Fix
