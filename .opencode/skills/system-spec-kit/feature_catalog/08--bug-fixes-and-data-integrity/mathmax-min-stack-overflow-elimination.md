...
trigger_phrases:
  ...
  - "stack overflow spread pattern fix"
...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/reporting-dashboard.vitest.ts` | Automated test | Dashboard reporting tests (covers fixed Math.max/min sites) |
| `mcp_server/tests/k-value-optimization.vitest.ts` | Automated test | K-value analysis tests (covers residual site) |
| `mcp_server/tests/causal-boost.vitest.ts` | Automated test | Causal boost tests (covers fixed site) |
| `mcp_server/tests/prediction-error-gate.vitest.ts` | Automated test | PE gate tests (covers fixed sites) |
...
- Feature file path: `08--bug-fixes-and-data-integrity/mathmax-min-stack-overflow-elimination.md`
Related references:
...
- [session-manager-transaction-gap-fixes.md](session-manager-transaction-gap-fixes.md) — Session-manager transaction gap fixes
