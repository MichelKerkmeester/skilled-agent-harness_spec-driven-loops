...
Stage 3 (Rerank and Aggregate) handles cross-encoder reranking (optional, gated by `SPECKIT_CROSS_ENCODER`) and MPAB chunk collapse with parent reassembly. The reranker is only invoked when at least 4 candidates reach Stage 3. MMR still runs after reranking when enabled, and the intent lambda map now includes continuity at `0.65`. Chunks are grouped by parent ID, the best chunk per group is elected by score, and full parent content is loaded from the database. The reassembly helper now accepts both snake_case and camelCase chunk metadata (`parent_id`/`parentId`, `chunk_index`/`chunkIndex`, `chunk_label`/`chunkLabel`), so formatter-style rows still collapse back to their parent correctly. On DB failure, the best-chunk row is emitted as a fallback. The retained `applyLengthPenalty` option is now a compatibility no-op (`1.0` multiplier), and `getRerankerStatus()` exposes reranker latency plus cache `hits`, `misses`, `staleHits`, and `evictions`. Non-chunks and reassembled parents merge and sort descending by effective score.
