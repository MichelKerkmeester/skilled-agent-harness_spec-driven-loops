...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/tool-input-schema.vitest.ts` | Automated test | Public/runtime schema validation for memory_search args |
| `mcp_server/tests/pipeline-v2.vitest.ts` | Automated test | Stage metadata contract including constitutional passthrough |
| `mcp_server/tests/stage1-expansion.vitest.ts` | Automated test | Stage 1 embedding, scope filtering and deep-mode merge guardrails |
| `mcp_server/tests/handler-memory-search.vitest.ts` | Automated test | Chunk collapse path behavior |
| `mcp_server/tests/bm25-index.vitest.ts` | Automated test | Stemmer regression coverage |
| `mcp_server/tests/search-extended.vitest.ts` | Automated test | Additional simpleStem doubled-consonant checks |

### Validation And Tests — Mutation hardening

| File | Type | Role |
|---|---|---|
| `mcp_server/tests/memory-crud-extended.vitest.ts` | Automated test | Update/delete handler integration behavior |
| `mcp_server/tests/deferred-features-integration.vitest.ts` | Automated test | Cross-table cleanup checks for mutation-side delete paths |
| `mcp_server/tests/transaction-manager.vitest.ts` | Automated test | Atomic-save and pending-file transaction behavior |
| `mcp_server/tests/preflight.vitest.ts` | Automated test | Preflight error-code contract used by save handler responses |
| `mcp_server/tests/chunking-orchestrator-swap.vitest.ts` | Automated test | Safe-swap finalization success/failure, staged-chunk cleanup, and old-child rollback coverage |
| `mcp_server/tests/handler-memory-save.vitest.ts` | Automated test | Chunked PE finalize rollback and compensating chunk-tree cleanup coverage |
...
- Feature file path: `pipeline_architecture/pipeline_and_mutation_hardening.md`
Related references:
- [legacy-v1-pipeline-removal.md](legacy_v1_pipeline_removal.md) — Legacy V1 pipeline removal
- [dbpath-extraction-and-import-standardization.md](dbpath_extraction_and_import_standardization.md) — DB_PATH extraction and import standardization
