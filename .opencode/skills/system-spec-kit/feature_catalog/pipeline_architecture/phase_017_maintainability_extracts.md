...
trigger_phrases:
  ...
  - "assertNever helper"
  - "advisoryPreset rename"
  - "reconsolidation transaction helper"
  - "post-insert extraction helper"
version: 3.6.0.5
---
...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/exhaustiveness.vitest.ts` | Automated test | `assertNever()` coverage across representative unions |
| `mcp_server/tests/run-enrichment-step.vitest.ts` | Automated test | Extracted post-insert helper behavior |
| `mcp_server/tests/reconsolidation.vitest.ts` | Automated test | Shared conflict transaction behavior |
| `mcp_server/tests/graph-first-routing-nudge.vitest.ts` | Automated test | `advisoryPreset` rename coverage |
...
- Feature file path: `pipeline_architecture/phase_017_maintainability_extracts.md`
Related references:
- [embeddings-and-retry-api.md](embeddings_and_retry_api.md) — Embeddings and Retry API
