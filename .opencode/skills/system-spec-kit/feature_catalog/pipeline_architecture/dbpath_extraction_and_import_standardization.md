...
| File | Type | Role |
|---|---|---|
| `scripts/tests/test-folder-detector-functional.js` | Automated test | Functional DB-path construction checks for script resolver usage |
| `scripts/tests/test-cleanup-orphaned-vectors.js` | Automated test | Script-level DB_PATH consumer coverage for cleanup flow |
| `scripts/tests/import-policy-rules.vitest.ts` | Automated test | Import-standardization policy checks for @spec-kit alias usage |
...
- Feature file path: `pipeline_architecture/dbpath_extraction_and_import_standardization.md`
Related references:
- [pipeline-and-mutation-hardening.md](pipeline_and_mutation_hardening.md) — Pipeline and mutation hardening
- [strict-zod-schema-validation.md](strict_zod_schema_validation.md) — Strict Zod schema validation
