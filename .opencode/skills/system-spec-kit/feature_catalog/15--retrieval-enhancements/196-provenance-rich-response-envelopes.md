...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/search-results-format.vitest.ts` | Automated test | Formatter-level validation for trace channels, query complexity fallback and envelope payload shaping. |
| `mcp_server/tests/mcp-response-envelope.vitest.ts` | Automated test | Protocol-level MCP envelope validation with `includeTrace` on/off provenance assertions. |
| `mcp_server/tests/envelope.vitest.ts` | Automated test | Core response envelope contract validation (`content[]`, `meta` and wrapper behavior). |
...
- Feature file path: `15--retrieval-enhancements/196-provenance-rich-response-envelopes.md`
Related references:
- [195-tier-2-fallback-channel-forcing.md](195-tier-2-fallback-channel-forcing.md) — Tier-2 fallback channel forcing
- [197-contextual-tree-injection.md](197-contextual-tree-injection.md) — Contextual tree injection
