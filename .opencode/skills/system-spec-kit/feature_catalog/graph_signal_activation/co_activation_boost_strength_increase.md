...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/co-activation.vitest.ts` | Automated test | Co-activation spreading tests, including batched `WHERE id IN (...)` hydration and single-query causal-neighbor coverage |
| `mcp_server/tests/stage2-fusion.vitest.ts` | Automated test | Verifies co-activation neighbor counts are precomputed once per boosted batch |
...
- Feature file path: `graph_signal_activation/co_activation_boost_strength_increase.md`
Related references:
- [typed-weighted-degree-channel.md](typed_weighted_degree_channel.md) — Typed-weighted degree channel
- [edge-density-measurement.md](edge_density_measurement.md) — Edge density measurement
