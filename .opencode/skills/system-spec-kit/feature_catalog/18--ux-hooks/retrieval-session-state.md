...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/session-state.vitest.ts` | Automated test | Flag behavior, session CRUD, deduplication, goal refinement, TTL eviction, LRU eviction |
| `mcp_server/tests/memory-search-ux-hooks.vitest.ts` | Automated test | Live handler integration for `data.sessionState` and `data.goalRefinement` |
...
- Feature file path: `18--ux-hooks/retrieval-session-state.md`
Related references:
- [progressive-disclosure.md](progressive-disclosure.md) — Progressive disclosure with cursor pagination
- [empty-result-recovery.md](empty-result-recovery.md) — Empty result recovery
