...
| File | Layer | Role |
|------|-------|------|
| `.opencode/skills/system-spec-kit/scripts/evals/run-ablation.ts` | Eval CLI | Runs channel ablation studies against the production hybrid-search stack and persists report artifacts |
| `.opencode/skills/system-spec-kit/scripts/evals/run-bm25-baseline.ts` | Eval CLI | Measures BM25-only retrieval quality, computes contingency output, and records eval snapshots |
| `.opencode/skills/system-spec-kit/scripts/evals/run-performance-benchmarks.ts` | Benchmark CLI | Executes synthetic latency and concurrency benchmarks for session boost, causal boost, and extraction hooks |
| `.opencode/skills/system-spec-kit/scripts/evals/map-ground-truth-ids.ts` | Provenance CLI | Audits or previews ground-truth parent-memory ID mappings against the production DB |
| `.opencode/skills/system-spec-kit/scripts/evals/check-architecture-boundaries.ts` | Policy checker | Enforces `shared/` neutrality and `mcp_server/scripts/` wrapper-only rules |
| `.opencode/skills/system-spec-kit/scripts/evals/check-handler-cycles-ast.ts` | Policy checker | Detects circular import and re-export dependencies inside `mcp_server/handlers` |
| `.opencode/skills/system-spec-kit/scripts/evals/import-policy-rules.ts` | Shared rule module | Normalizes candidate import paths and decides whether they violate the eval import policy |
| `.opencode/skills/system-spec-kit/scripts/evals/README.md` | Documentation | Documents script inventory, preferred import boundaries, and the allowlist-based exception process |
...
Related references:
- [template-composition-system.md](template_composition_system.md) — Template Composition System
- [memory-quality-kpi-reporting.md](memory_quality_kpi_reporting.md) — Memory Quality KPI Reporting
