...
**Cross-reference**: See `16--tooling-and-scripts/221-template-compliance-contract-enforcement.md` for the 3-layer template compliance architecture (agent contracts + post-write validation + runtime schema enforcement).

---

## 3. SOURCE FILES

- `.opencode/skills/system-spec-kit/mcp_server/lib/cognitive/rollout-policy.ts` - Canonical runtime flag helper and rollout-percentage enforcement, including default-on/explicit-opt-out semantics and fail-closed identity handling for partial rollout.
- `.opencode/skills/system-spec-kit/mcp_server/lib/config/capability-flags.ts` - Memory roadmap phase and flag governance, including `scope-governance` fallback and intentionally default-off dormant flags such as adaptive ranking.
...
- Feature file path: `17--governance/249-feature-flag-governance.md`
Related references:
