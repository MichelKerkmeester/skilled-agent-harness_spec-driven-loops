...
| File | Type | Role |
|---|---|---|
| `mcp_server/tests/hybrid-search.vitest.ts` | Automated test | Tiered fallback and token-budget pipeline behavior |
| `mcp_server/tests/memory-summaries.vitest.ts` | Automated test | TF-IDF summary generation and summary query behavior |
| `mcp_server/tests/mutation-ledger.vitest.ts` | Automated test | Mutation ledger counting/query performance guards |
| `mcp_server/tests/entity-linker.vitest.ts` | Automated test | Entity linker batch edge-count logic coverage |
| `mcp_server/tests/causal-edges-unit.vitest.ts` | Automated test | Causal edge upsert/update unit behavior |
| `mcp_server/tests/causal-edges.vitest.ts` | Automated test | Causal edge storage integration paths |
| `mcp_server/tests/spec-folder-hierarchy.vitest.ts` | Automated test | Hierarchy cache/TTL behavior and traversal checks |
...
- Feature file path: `14--pipeline-architecture/performance-improvements.md`
Related references:
- [search-pipeline-safety.md](search-pipeline-safety.md) — Search pipeline safety
- [legacy-v1-pipeline-removal.md](legacy-v1-pipeline-removal.md) — Legacy V1 pipeline removal
