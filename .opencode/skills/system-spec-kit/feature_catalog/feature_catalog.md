...
| `SPECKIT_CROSS_ENCODER` | `false` | boolean | `lib/search/search-flags.ts` | Opt-in cross-encoder reranking in Stage 3 of the 4-stage pipeline. Default OFF; set to `true` or set a cloud API key to enable. When enabled, the reranker rescores candidates using a more expensive cross-attention model, but only when at least 4 candidates reach the Stage 3 gate. Disabling falls back to vector-only ranking from fusion. |
