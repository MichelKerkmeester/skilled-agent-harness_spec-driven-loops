...
3. `install.sh` is the environment bootstrap for the bundled `mk-spec-memory` MCP. It requires Node.js 18+, `npm`, and `npx`; logs the current Node version, module ABI version, and binary path; and verifies that the expected Spec Kit workspace, MCP server folder, and both `package.json` files exist before proceeding.
...
- Feature file path: `16--tooling-and-scripts/232-setup-native-module-health-and-mcp-installation.md`
Related references:
- [231-spec-folder-detection-and-description.md](231-spec-folder-detection-and-description.md) — Spec-Folder Detection and Description Metadata
- [233-template-composition-system.md](233-template-composition-system.md) — Template Composition System
