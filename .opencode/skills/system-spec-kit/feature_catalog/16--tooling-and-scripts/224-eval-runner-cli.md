...
2. The script targets the active Memory MCP database, resolving the profile-derived filename via `shared/embeddings/profile.ts` (provider, model, dimension, and dtype keyed). It refuses to proceed if that database file does not exist or if `vectorIndex.initializeDb(PROD_DB_PATH)` returns `null`. After DB rebuilds or imports, operators should preview or refresh ground-truth mappings with `scripts/evals/map-ground-truth-ids.ts` before treating a new ablation run as comparable to older history.
...
| File | Layer | Role |
|------|-------|------|
| `.opencode/skills/system-spec-kit/scripts/evals/run-ablation.ts` | CLI | Runtime entry point for ablation studies, channel parsing, report printing, and JSON artifact writing |
...
| `.opencode/skills/system-spec-kit/mcp_server/api/index.ts` | Public API | Re-export boundary that supplies eval, search, embedding, and vector-index surfaces to the CLI |

## 4. SOURCE METADATA
- Group: Tooling And Scripts
- Canonical catalog source: `feature_catalog.md`
- Feature file path: `16--tooling-and-scripts/224-eval-runner-cli.md`
Related references:
- [223-ops-self-healing-runbooks.md](223-ops-self-healing-runbooks.md) — Ops Self-Healing Runbooks
- [225-phase-system-knowledge-node.md](225-phase-system-knowledge-node.md) — Phase-System Knowledge Node
