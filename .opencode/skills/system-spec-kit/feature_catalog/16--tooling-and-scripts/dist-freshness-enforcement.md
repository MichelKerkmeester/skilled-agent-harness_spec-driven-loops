...
`checkPackageFreshness(packageId, options)` resolves a package's watched source files (filtered by extension and per-package exclusions such as `tests/`, `node_modules/`, `dist/`), computes the newest mtime among them, and compares it against the mtime of the package's compiled dist entry. If `newestSourceMtime > distMtime`, the result is `stale` and carries the package's `rebuildCommand`. A SHA256 content hash over the same watched source files is written lazily to a small cache file next to the dist entry purely as a same-session performance short-circuit — if a later check finds the same hash already cached, it reports fresh without re-touching every file's mtime. This cache is never pre-warmed by a build script; the authoritative signal is always the mtime comparison, not the hash.
...
| `.opencode/skills/sk-code/scripts/check-dist-staleness.sh` | Handler | PostToolUse warn-only checker (Python, `.sh` extension) |
| `.opencode/skills/sk-code/scripts/hooks/claude-posttooluse.sh` | Handler | Dispatches the dist-staleness checker on every `Write`/`Edit` |
...
| `16--tooling-and-scripts/cli-dist-freshness-guard.md` | Manual playbook | CLI-shim guard trip, override, and restore (playbook ID 429) |
| `16--tooling-and-scripts/validate-sh-dist-freshness-backstop.md` | Manual playbook | `validate.sh` hard backstop trip and restore (playbook ID 455) |

## 4. SOURCE METADATA

- Group: Tooling And Scripts
- Canonical catalog source: `feature_catalog.md`
- Feature file path: `16--tooling-and-scripts/dist-freshness-enforcement.md`
...
Related references:
- [spec-memory-cli-daemon-backed-surface.md](spec-memory-cli-daemon-backed-surface.md) — Daemon-backed spec-memory CLI surface that consumes this module's freshness guard
- [source-dist-alignment-enforcement.md](source-dist-alignment-enforcement.md) — Related but distinct concern: detects orphaned dist artifacts with no source, not source-newer-than-dist staleness
