...
description: "Runtime integrations for the 028 CLI program: prompt-time hooks in Claude and Codex adapters gain a warm-only CLI fallback (socket probe first, fast fail-open, no prompt-time cold spawn), and the OpenCode plugins route over CLI/IPC with zero in-process DB imports."
trigger_phrases:
...
A CLI nobody's runtime calls does not close the transport-down incident class, so every 028 CLI workstream shipped paired runtime integrations. Prompt-time hooks for Claude Code and Codex gained a shared warm-only CLI fallback helper per system: the hook probes the daemon socket first, uses the CLI when the daemon is warm, and fails open in about a millisecond when no socket exists. Cold spawn stays confined to SessionStart, explicit prewarm, cron, or non-prompt maintenance contexts.
...
Claude adapters `session-prime.ts`, `compact-inject.ts`, and `session-stop.ts` plus Codex `session-start.ts` use the spec-memory and code-index helpers; the Claude and Codex `user-prompt-submit.ts` advisor hooks use the skill-advisor helper. The one-shot native bridge (measured 824.8 ms) stays banned from the prompt path.

### Allowlists and guidance

`.codex/settings.json` allowlists the CLI invocations for Codex (the Claude allowlist lives in local-only settings by decision), and `AGENTS.md` carries the transport-down fallback and maintenance-tool policy guidance.
...
| `mcp_server/hooks/codex/session-start.ts` | Hook adapter | Codex session-start with CLI warm path |
...
| `.opencode/skills/system-skill-advisor/hooks/codex/user-prompt-submit.ts` | Hook adapter | Codex advisor hook with CLI fallback |
...
| `.codex/settings.json` | Runtime config | Codex allowlist for CLI use |
...
- Feature file path: `16--tooling-and-scripts/cli-runtime-warm-only-fallbacks.md`
Related references:
