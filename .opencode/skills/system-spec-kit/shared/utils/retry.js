...
export const PERMANENT_HTTP_STATUS_CODES = new Set([
    400, // Bad Request
    401, // Unauthorized
    403, // Forbidden
    404, // Not Found
...
export const TRANSIENT_NETWORK_ERRORS = new Set([
...
export function getBackoffSequence(config = DEFAULT_CONFIG) {
    const delays = [];
...
export async function retryWithBackoff(fn, options = {}) {
    const { operationName = 'operation', maxRetries = DEFAULT_CONFIG.maxRetries, baseDelayMs = DEFAULT_CONFIG.baseDelayMs, maxDelayMs = DEFAULT_CONFIG.maxDelayMs, exponentialBase = DEFAULT_CONFIG.exponentialBase, onRetry = null, shouldRetry = null, } = options;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            if (isRetry) {
                console.error(`[retry] ${operationName} succeeded on attempt ${attemptNum}/${maxRetries + 1}`);
            }
            return result;
        }
        catch (error) {
            ...
            console.error(`[retry] ${operationName} attempt ${attemptNum}/${maxRetries + 1} failed: ` +
                `[${errorType}] ${lastError.message}`);
            // CHK-184: Permanent errors fail fast (no retry)
            if (errorType === 'permanent') {
                console.error(`[retry] ${operationName} failed permanently (${reason}), not retrying`);
                const permanentError = new Error(`${operationName} failed permanently: ${lastError.message}`);
            ...
            if (attempt >= maxRetries || !willRetry) {
                console.error(`[retry] ${operationName} exhausted retries (${attemptNum} attempts)`);
                const exhaustedError = new Error(`${operationName} failed after ${attemptNum} attempts: ${lastError.message}`);
            ...
            console.error(`[retry] ${operationName} will retry in ${delay}ms ` +
