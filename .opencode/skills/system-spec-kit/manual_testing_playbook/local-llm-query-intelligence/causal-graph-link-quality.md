...
- The exact AI-to-CLI handoff prompt + the CLI's verbatim response.
  - Handoff prompt was not sent to an external CLI because step 1 requires writing three canonical research-doc files, while the active run constraint allowed writes only to `.opencode/skills/system-spec-kit/manual_testing_playbook/local-llm-query-intelligence/causal-graph-link-quality.md`.
