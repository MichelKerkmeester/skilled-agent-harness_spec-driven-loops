...
- RCAF Prompt: `As a query-intelligence validation operator, read manual_testing_playbook/local-llm-query-intelligence/409-fixture.json, query each fixture paraphrase with Memory MCP, and verify the expected source memory appears in top-3. Return a pass/fail verdict with the per-sample table.`
...
1. Load the deterministic fixture:
   ```bash
   jq -c '.[]' .opencode/skills/system-spec-kit/manual_testing_playbook/local-llm-query-intelligence/409-fixture.json
   ```
