...
BLOCKED: The unknown-job edge returned structured `not_found` output, but the no-job active-pointer command could not be completed because the native MCP wrapper rejects a missing/empty `jobId` and the CLI fallback is blocked by stale `.opencode/skills/system-spec-kit/mcp_server` dist output.
