...
- Targeted Vitest run: `cd .opencode/skills/system-spec-kit/mcp_server && npm exec -- vitest run tests/temporal-edges.vitest.ts`
...
Inspect `mcp_server/lib/graph/temporal-edges.ts` for the `invalidateEdge` and `getValidEdges` implementations. Confirm `SPECKIT_TEMPORAL_EDGES` is not disabled in the runtime env. Check `mcp_server/lib/search/causal-boost.ts` for the call site that should filter by `getValidEdges`.
...
- Feature file path: `10--graph-signal-activation/196-temporal-edges.md`
