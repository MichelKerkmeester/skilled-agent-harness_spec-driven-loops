...
2. Confirm snapshots are enabled and record pre-run checksums for `context-index.sqlite` and the vector index database.
...
- Pre-run checksums for `context-index.sqlite` and the vector DB.
...
- Feature file path: `23--doctor-commands/340-doctor-update-G7-sigint.md`
