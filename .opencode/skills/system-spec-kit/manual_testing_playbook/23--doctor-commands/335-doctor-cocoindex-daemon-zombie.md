---
title: "DOC-335 -- Doctor code_graph daemon zombie"
description: "Manual scenario validating /doctor code_graph behavior for an unresponsive Code Graph daemon and the 3.4.1.0 idempotent restart contract."
---

# DOC-335 -- Doctor code_graph daemon zombie

## 1. OVERVIEW

This scenario validates the daemon-resilience branch for `/doctor code_graph` when a Code Graph daemon process exists but is unresponsive, CPU-bound, or otherwise zombie-like.
...
- Playbook ID: DOC-335.
- Real user request: `Reindex code_graph. The daemon has been spinning at 90% CPU since this morning.`
- Prompt: `Reindex code_graph. The daemon has been spinning at 90% CPU since this morning.`
- Preconditions: A disposable sandbox can simulate a zombie Code Graph daemon: a `ccc run-daemon` process exists, but `ccc_status({})` or `ccc daemon status` reports unresponsive behavior, stale pid/socket/lock state, or CPU spin.
- Expected execution process: Capture pre-run daemon process count and CPU, run `/doctor code_graph`, verify idempotent restart behavior, then confirm reindex and gold-battery success when restart recovers the daemon.
...
```
Reindex code_graph. The daemon has been spinning at 90% CPU since this morning.
```

### Commands

1. In a disposable sandbox, reproduce or attach to a real zombie-like Code Graph daemon state without touching active user data.
...
4. Capture `ccc_status({})` and `.opencode/skills/system-code-graph/mcp_server/.venv/bin/ccc daemon status`.
5. Capture recent `~/.mk_code_index/daemon.log` metadata and BrokenPipeError count without modifying the log.
6. Run `/doctor code_graph` through the real runtime.
7. Capture the YAML asset load for `.opencode/commands/doctor/assets/doctor_code_graph.yaml`, daemon-health classification, restart command, and `ccc_reindex({full: true})` result.
...
- **SKIP**: Code Graph is not installed or the sandbox forbids daemon process inspection.
...
If the command refuses before restart, compare the observed behavior against `.opencode/commands/doctor/assets/doctor_code_graph.yaml` daemon coordination. The current asset lists duplicate processes, unreachable daemon status, BrokenPipeError bursts, and stale pid/socket/lock state as unhealthy signals; this scenario intentionally catches whether the intended 3.4.1.0 idempotent restart recovery is implemented for a restartable zombie state or whether the command has become refuse-only.
...
- Read-only diagnostic YAML asset: [.opencode/commands/doctor/assets/doctor_code_graph.yaml](../../../../commands/doctor/assets/doctor_code_graph.yaml)
...
- Playbook ID: DOC-335
- Feature name: Doctor code_graph daemon zombie
...
- Feature file path: `23--doctor-commands/335-doctor-code_graph-daemon-zombie.md`
