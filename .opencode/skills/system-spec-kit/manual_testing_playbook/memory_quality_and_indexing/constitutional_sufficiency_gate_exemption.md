...
4. `rg -n "isConstitutional.*useWarnOnly|useWarnOnly.*isConstitutional|Packet 018" .opencode/skills/system-spec-kit/mcp_server/handlers/memory-index.ts`. Assert the OR-chain and rationale comment are both present.
...
ALLOWED WRITE PATHS
- .opencode/skills/system-spec-kit/manual_testing_playbook/memory_quality_and_indexing/constitutional_sufficiency_gate_exemption.md (this file only)
```
...
- Block A failures: rebuild `mcp_server/dist/`, restart the daemon, re-run the scan. Confirm the daemon process loaded the new dist (check PID and process tree).
...
- Feature catalog: [memory_quality_and_indexing/constitutional_sufficiency_gate_exemption.md](../../feature_catalog/memory_quality_and_indexing/constitutional_sufficiency_gate_exemption.md)
