...
| `SHADOW` | `SPECKIT_SEMANTIC_TRIGGERS=true`; mode unset | blocked: `@spec-kit/mcp-server dist is stale. Run: cd .opencode/skills/system-spec-kit/mcp_server && npm run build` | live MCP fallback returned `[23213, 16437, 8545, 6819, 6646]` | none; no semantic shadow stats or trace metadata surfaced |
| `UNION` | `SPECKIT_SEMANTIC_TRIGGERS=true`, `SPECKIT_SEMANTIC_TRIGGERS_MODE=union` | blocked: `@spec-kit/mcp-server dist is stale. Run: cd .opencode/skills/system-spec-kit/mcp_server && npm run build` | live MCP fallback returned `[6646, 8650, 6645, 8670, 7155]` | none; no semantic union metadata surfaced and all matches reported lexical `matchedPhrases` |
| `DISABLED` | both flags unset; `printenv` produced `(no output)` | blocked: `@spec-kit/mcp-server dist is stale. Run: cd .opencode/skills/system-spec-kit/mcp_server && npm run build` | live MCP fallback returned `[23213, 16437, 8545, 6819, 6646]` | none; response fields were `data.matchType: "trigger-phrase"`, `data.degradedMatching: null`, `meta.triggerSignals: []` |
...
- **BLOCKED**: the required fresh daemon/handler restart boundary is unavailable because `node ".opencode/bin/spec-memory.cjs" ...` exits with `@spec-kit/mcp-server dist is stale. Run: cd .opencode/skills/system-spec-kit/mcp_server && npm run build`; rebuilding is outside the allowed write paths, so shadow and union mode assertions could not be verified in a process that loaded the requested env changes.
...
| `feature_flag_reference/semantic_trigger_shadow_and_union.md` | Scenario contract |
...
- Feature file path: `feature_flag_reference/semantic_trigger_shadow_and_union.md`
