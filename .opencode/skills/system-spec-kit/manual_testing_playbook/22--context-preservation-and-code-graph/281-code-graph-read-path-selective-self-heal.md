...
- Prompt: `Validate code graph read-path selective self-heal against the documented validation surface.`
...
```bash
printf '\n// manual playbook self-heal marker %s\n' "$(date +%s)" >> .opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/README.md
```

3. Run a structural read:

```text
code_graph_query({ "operation": "outline", "subject": ".opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/README.md", "limit": 20 })
```
...
```bash
find .opencode/skills/system-spec-kit/mcp_server/code_graph -name '*.ts' | head -60 | while read -r file; do printf '\n// broad stale marker %s\n' "$(date +%s)" >> "$file"; done
```

```text
code_graph_query({ "operation": "outline", "subject": ".opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/query.ts", "limit": 20 })
```
...
- Source: `.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/ensure-ready.ts`
- Source: `.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/query.ts`
- Source: `.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/detect-changes.ts`
