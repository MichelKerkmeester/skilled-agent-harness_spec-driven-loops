...
- Prompt: `Validate code_graph_query fallbackDecision routing for empty, broad-stale, readiness-exception, and fresh graph states.`
...
1. `cd .opencode/skills/system-spec-kit/mcp_server && npx vitest run tests/code-graph-query-fallback-decision.vitest.ts`
2. `cd .opencode/skills/system-spec-kit/mcp_server && npx vitest run tests/code-graph-degraded-sweep.vitest.ts`
3. `shasum -a 256 .opencode/skills/system-spec-kit/mcp_server/database/code-graph.sqlite` before and after the two suites
