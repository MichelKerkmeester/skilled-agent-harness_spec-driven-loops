...
- The real `/doctor memory --incremental=false` workflow contract in `.opencode/commands/doctor/assets/doctor_memory.yaml` is diagnostic-only, so the required full rebuild path, ETA prompt, snapshot creation, forced `memory_index_scan`, Phase 4 gold battery, and Phase 6 state-log path are not present in the current command implementation. Actual file content observed:
  ...
  mutation_boundaries:
    allowed_targets:
      - "mcp_server/database/context-index.sqlite"  # canonical memory DB
...
- The exact snapshot existence command from the playbook was run:
  ...
  Output:
  ```
  zsh:1: no matches found: .opencode/skills/system-spec-kit/mcp_server/database/context-index__*.sqlite.pre-doctor-memory.*.bak
  ```
...
If no ETA prompt appears, inspect `.opencode/commands/doctor/speckit.md` setup handling for full rebuilds. If no snapshot exists, inspect Phase 2 in `doctor_memory.yaml`. If gold battery fails, compare baseline and post counts from the state log and confirm rollback behavior did not trigger.
...
- Feature file path: `doctor_commands/doctor_memory_long_pole_rebuild.md`
