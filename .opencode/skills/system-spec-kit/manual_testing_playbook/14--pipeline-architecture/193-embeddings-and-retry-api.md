...
Public export capture + annotated snippets or notes from `shared/embeddings.ts` showing provider cascade and local model IDs + retry-manager evidence showing queue/backoff/circuit-breaker behavior + import smoke-test transcript or equivalent script proof

### Pass / Fail

- **Pass**: the stable API surface, shared embedding substrate, and retry orchestration layers align with the documented contract
- **Fail**: public exports are incomplete, shared embedding lifecycle helpers or provider defaults are missing, or retry recovery behavior is not present
...
- Feature file path: `14--pipeline-architecture/193-embeddings-and-retry-api.md`
