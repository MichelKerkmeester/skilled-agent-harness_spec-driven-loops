...
```bash
sqlite3 -header -column ".opencode/skills/system-spec-kit/mcp_server/database/context-index.sqlite" "SELECT memory_id, COUNT(*) AS entity_count, GROUP_CONCAT(DISTINCT entity_type) AS entity_types FROM memory_entities WHERE memory_id BETWEEN 38625 AND 38631 GROUP BY memory_id ORDER BY memory_id;"
```
...
Default configuration evidence from `.opencode/skills/system-spec-kit/mcp_server/lib/search/search-flags.ts`:
...
Default rollout policy evidence from `.opencode/skills/system-spec-kit/mcp_server/lib/cognitive/rollout-policy.ts`:
...
- Feature catalog: [13--memory-quality-and-indexing/auto-entity-extraction.md](../../feature_catalog/13--memory-quality-and-indexing/auto-entity-extraction.md)
...
- Feature file path: `13--memory-quality-and-indexing/implemented-auto-entity-extraction-r10.md`
