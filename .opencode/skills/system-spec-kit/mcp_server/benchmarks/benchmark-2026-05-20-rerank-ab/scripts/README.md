...
- `run_arm.py` and `run_arm.sh` execute a single benchmark arm against `memory_search`.
...
└──────┬───────┘      └────────┬─────────┘      └──────────────────┘
       ...
       ▼                       ▼
┌──────────────────────────────────────────┐
│ generate_report.py                       │ ───▶ benchmark_report.md
└──────────────────────────────────────────┘
...
+-- run_arm.sh          # Shell wrapper that invokes run_arm.py for one arm
...
| `run_arm.sh` | Shell wrapper that runs `run_arm.py` for a single arm with the arm-specific environment. |
...
Expected result: the run writes refreshed `results.csv`, `per-probe.jsonl`, and a `benchmark_report.md` whose verdict matches the decision rule above.
