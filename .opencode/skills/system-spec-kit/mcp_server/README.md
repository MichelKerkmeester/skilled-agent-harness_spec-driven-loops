...
| Daemon re-election (022, experimental) | `SPECKIT_DAEMON_REELECTION` (default off) spawns the daemon detached and releases (not kills) it on owner shutdown so the shared backend can outlive its owner. Default-off is byte-identical to prior behavior; enabling needs runtime validation of secondary adoption (spec packet 022). |
