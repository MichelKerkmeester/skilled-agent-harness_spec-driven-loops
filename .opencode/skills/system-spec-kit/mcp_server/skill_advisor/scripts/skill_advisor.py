...
# This script lives in .opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/.
...
PHRASE_INTENT_BOOSTERS = {
    ...
    "agent improvement": [("deep-agent-improvement", 2.8)],
    "recursive agent": [("deep-agent-improvement", 2.8)],
    "improvement loop": [("deep-agent-improvement", 2.8)],
    "agent improvement loop": [("deep-agent-improvement", 3.2)],
    "proposal-only improvement": [("deep-agent-improvement", 2.6)],
    "proposal only improvement": [("deep-agent-improvement", 2.6)],
    "proposal only": [("deep-agent-improvement", 1.4)],
    "evaluator-first": [("deep-agent-improvement", 2.4)],
    "bounded mutator": [("deep-agent-improvement", 2.2)],
    "candidate scoring": [("deep-agent-improvement", 2.3)],
    "promotion gate": [("deep-agent-improvement", 2.0)],
    "handover target": [("deep-agent-improvement", 2.0)],
    "deep-agent-improvement": [("deep-agent-improvement", 3.2)],
    "/deep-agent-improvement": [("deep-agent-improvement", 3.2)],
    "sk-agent-improvement-loop": [("deep-agent-improvement", 3.0)],
    "/sk-agent-improvement-loop": [("deep-agent-improvement", 3.0)],
    "5-dimension": [("deep-agent-improvement", 1.8)],
    "5-dimension agent scoring": [("deep-agent-improvement", 2.8)],
    "5-dimension evaluation": [("deep-agent-improvement", 2.8)],
    ...
    "score agent dimensions": [("deep-agent-improvement", 2.8)],
    "agent integration surface": [("deep-agent-improvement", 2.6)],
    "/improve:agent": [("deep-agent-improvement", 3.2)],
    "/improve:prompt": [("sk-prompt", 3.2)],
    "improve agent": [("deep-agent-improvement", 2.8)],
    "score agent": [("deep-agent-improvement", 2.6)],
    "evaluate agent": [("deep-agent-improvement", 2.6)],
    ...
    "/mcp-coco-index": [("mcp-coco-index", 2.8)],
    ".opencode/skills/mcp-coco-index": [("mcp-coco-index", 3.0)],
...
_SKILL_PATH_RE = re.compile(r'\.opencode/skills/([^/]+)/')
...
def _resolve_skill_from_path(
    ...
    ``.opencode/skills/sk-git/SKILL.md`` and matches against loaded skills.
    """
...
def _parse_ccc_output(stdout: str, limit: int = COCOINDEX_LIMIT) -> List[Dict[str, Any]]:
    ...
    ``.opencode/skills/<name>/`` references, mapping each to a skill hit.
    """
        ...
        if file_match:
            ...
            if skill_match:
                ...
                if folder not in seen_folders:
                    ...
                    hits.append({"path": f".opencode/skills/{folder}/SKILL.md", "score": min(current_score, 1.0)})
            continue
...
Examples:
  ...
  python skill_advisor.py "deploy to production" --semantic-hits '[{"path":".opencode/skills/sk-git/SKILL.md","score":0.92}]'
  python skill_advisor.py "deploy to production" --cocoindex-hits '[{"path":".opencode/skills/sk-git/SKILL.md","score":0.92}]'
        '''
    )
