...
import { isCritical } from '../../../lib/council-graph/council-graph-query.js';
...
export type CouncilGraphDb = ReturnType<typeof initDb>;
...
export interface ScenarioFixture {
  ...
  graph: {
    runner: (db: CouncilGraphDb, specFolder: string, sessionId: string) => Promise<{ mcpCalls: number; answer: unknown }>;
    expectedAnswer: unknown;
  };
}
...
export function openTempCouncilGraphDb(): { db: CouncilGraphDb; dbPath: string; cleanup: () => void } {
  closeDb();
  const dbDir = fs.mkdtempSync(path.join(os.tmpdir(), 'speckit-council-value-'));
  const db = initDb(dbDir);
  const dbPath = path.join(dbDir, 'council-graph.sqlite');
...
export async function upsertFixtureGraph(
  ...
  const response = await handleCouncilGraphUpsert({
    specFolder,
    sessionId,
    nodes: graphSeed.nodes.map((node) => ({
      id: node.id,
      kind: node.kind,
    ...
    edges: graphSeed.edges.map((edge) => ({
      id: edge.id,
      sourceId: edge.from,
      targetId: edge.to,
      relation: edge.kind,
...
export function buildScenarioFixture(scenarioId: string): ScenarioFixture {
  ...
  return {
    ...
    graph: {
      runner: (_db, specFolder, sessionId) => runGraph(data.scenarioId, specFolder, sessionId),
...
async function runGraph(
  ...
  specFolder: string,
  sessionId: string,
): Promise<{ mcpCalls: number; answer: unknown }> {
  switch (scenarioId) {
    case 'DAC-027': {
      const data = parseData(await handleCouncilGraphQuery({
        specFolder,
        sessionId,
      ...
      return { mcpCalls: 1, answer: disagreements.map((item) => item.id).sort() };
    }
    case 'DAC-028': {
      const data = parseData(await handleCouncilGraphQuery({
        specFolder,
    ...
    case 'DAC-029': {
      const data = parseData(await handleCouncilGraphConvergence({ specFolder, sessionId }));
    ...
    case 'DAC-030': {
      const data = parseData(await handleCouncilGraphQuery({
        specFolder,
        sessionId,
        queryType: 'convergence_blockers',
        limit: 5,
      }));
    ...
    case 'DAC-031': {
      ...
      return { mcpCalls: 1, answer: hotNodes.map((item) => item.id) };
    }
    case 'DAC-032': {
...
function parseData(response: HandlerResponse): Record<string, unknown> {
  ...
  if (envelope.status !== 'ok' || !envelope.data) {
    throw new Error(envelope.error ?? 'Council graph handler returned an invalid response');
  }
