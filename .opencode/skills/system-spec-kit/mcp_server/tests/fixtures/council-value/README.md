...
description: "Test fixtures for council graph value scenarios, providing scenario fixtures and shared helper utilities that compare council graph MCP performance against baseline file reads."
trigger_phrases:
...
> Layered fixture directory that provides scenario fixtures and shared seeding utilities for validating council graph query performance against baseline file reading operations.
...
`tests/fixtures/council-value/` owns scenario fixtures and helper utilities for the council graph value demonstration system. Each scenario measures the performance improvement of council graph MCP queries over baseline file reading approaches. The fixtures cover six council graph query types: unresolved disagreements, decision support, convergence, convergence blockers, hot nodes and status queries.
...
- `seed-helpers.ts` is the shared utility module providing database seeding, artifact tree creation, baseline file counting and `buildScenarioFixture` construction.
...
- Fixtures import from `lib/council-graph/` for database operations and from `handlers/council-graph/` for convergence, query, status and upsert handlers.
- The consumer test `tests/council-graph-value-scenarios.vitest.ts` compares `graph.mcpCalls` against `baseline.fileReads` and generates a performance metrics report at `council-graph-value-report.json`.
- External imports include `node:fs`, `node:os`, `node:path`, `node:url` and `node:module` for filesystem, temporary directory and CommonJS interop operations.
...
| `seed-helpers.ts` | Shared utilities for temporary database creation, artifact tree seeding, graph upsert and scenario fixture construction. |
...
| `seed-helpers.ts` | Utility | Shared module providing `openTempCouncilGraphDb`, `seedArtifactTree`, `upsertFixtureGraph` and `buildScenarioFixture`. |
...
- [Council Graph Lib: lib/council-graph/](../../../lib/council-graph/README.md)
