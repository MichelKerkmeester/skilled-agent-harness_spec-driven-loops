---
title: "system spec kit mcp server tests graph: Code README"
description: "Code-facing README for .opencode/skills/system-spec-kit/mcp_server/tests/graph."
trigger_phrases:
  - "system-spec-kit mcp_server/tests/graph"
  - "code README"
---

# system spec kit mcp server tests graph
...
This README documents the code-bearing folder `.opencode/skills/system-spec-kit/mcp_server/tests/graph` so operators can understand its role without opening every source file first. It follows the sk-doc skill README structure while staying focused on code navigation.
...
```bash
rg --files .opencode/skills/system-spec-kit/mcp_server/tests/graph
```
...
|---|---|
| Folder boundary | Documents direct code files under `mcp_server/tests/graph`. |
...
| Path | Purpose |
|---|---|
| `graph-metadata-lineage.vitest.ts` | TS source file in this folder. |
...
```text
User request: Check .opencode/skills/system-spec-kit/mcp_server/tests/graph for sk-code and README coverage.
