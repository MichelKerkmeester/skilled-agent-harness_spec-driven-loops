...
const ACKNOWLEDGED_UNCEILINGED_FLAGS: string[] = [
  ...
  // Additive tail-lane recall flags (028 memory-search-intelligence). Opt-in,
  // tail-only appenders that never evict a baseline hit — deliberately not part
  // of the ceiling soak (they add candidates rather than re-rank), so they are
  // acknowledged here.
  ...
  // Lexical-floor and scoring-hardening opt-in flags read in search-flags.ts.
  // They reshape relevance scoring rather than join the boolean ceiling soak,
  // so they are acknowledged here rather than activated by the ceiling test.
