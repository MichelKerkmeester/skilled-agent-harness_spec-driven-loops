...
describe('dist freshness — build cache bootstrap', () => {
  it('pre-warms the checker hash cache so content-identical mtime skew reports fresh', () => {
    ...
    const packageRoot = join(workspaceRoot, '.opencode', 'skills', 'system-spec-kit', 'mcp_server');
    ...
    const before = distFreshness.checkPackageFreshness('system-spec-kit/mcp_server', {
      workspaceRoot,
      entry: 'spec-memory-cli',
    });
    expect(before.status).toBe('stale');

    const cacheWrite = distFreshness.writePackageSourceHashCache('system-spec-kit/mcp_server', {
      workspaceRoot,
      entry: 'spec-memory-cli',
    });
    ...
    const after = distFreshness.checkPackageFreshness('system-spec-kit/mcp_server', {
      workspaceRoot,
      entry: 'spec-memory-cli',
    });
    ...
    const stale = distFreshness.checkPackageFreshness('system-spec-kit/mcp_server', {
      workspaceRoot,
      entry: 'spec-memory-cli',
    });
  ...
  it('uses per-entry cache paths so one entry cannot vouch for another', () => {
    ...
    const cliCache = distFreshness.writePackageSourceHashCache('system-spec-kit/mcp_server', {
      workspaceRoot,
      entry: 'spec-memory-cli',
    });
    const orchestratorCache = distFreshness.writePackageSourceHashCache('system-spec-kit/mcp_server', {
      workspaceRoot,
