...
import { TreeSitterParser } from '../../../system-code-graph/mcp_server/lib/tree-sitter-parser.js';
import type { CodeEdge, CodeNode, ParseResult } from '../../../system-code-graph/mcp_server/lib/indexer-types.js';
...
describe('structural-indexer tree-sitter readiness integration', () => {
  it('reconciles missing grammars even when a parser instance was already cached', async () => {
    ...
    vi.resetModules();
    vi.doMock('../../../system-code-graph/mcp_server/lib/tree-sitter-parser.js', () => ({
    ...
    const { getParser } = await import('../../../system-code-graph/mcp_server/lib/structural-indexer.js');
...
describe('finalizeIndexResults dedupe reconciliation', () => {
  it('drops edges whose source nodes were removed by cross-result dedup', async () => {
    const { finalizeIndexResults } = await import('../../../system-code-graph/mcp_server/lib/structural-indexer.js');
...
describe('capturesToNodes dedupe (Bug #2 regression)', () => {
  it('drops duplicate (filePath, fqName, kind) captures, preserving first and module node', async () => {
    const { capturesToNodes } = await import('../../../system-code-graph/mcp_server/lib/structural-indexer.js');
  ...
  it('regression: parsing structural-indexer.ts produces no duplicate symbolIds', async () => {
    ...
    const { parseFile } = await import('../../../system-code-graph/mcp_server/lib/structural-indexer.js');
