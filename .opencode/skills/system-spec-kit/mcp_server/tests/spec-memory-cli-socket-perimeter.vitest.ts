// -----------------------------------------------------------------------------
// MODULE: Spec Memory CLI Socket Perimeter Tests
// -----------------------------------------------------------------------------
// The IPC client must refuse to probe/connect under a socket directory it does
// not own, a symlinked directory, or a group/world-writable directory, and must
// reject a symlinked socket node — otherwise a pre-planted fake daemon on a
// shared host can answer initialize and spoof CLI results.
...
import { __testing } from '../spec-memory-cli.js';

const SOCKET_FILE_NAME = 'daemon-ipc.sock';
const tempDirs: string[] = [];

function makeRoot(): string {
  const dir = mkdtempSync(join(tmpdir(), 'spec-memory-perimeter-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('spec-memory CLI socket perimeter', () => {
