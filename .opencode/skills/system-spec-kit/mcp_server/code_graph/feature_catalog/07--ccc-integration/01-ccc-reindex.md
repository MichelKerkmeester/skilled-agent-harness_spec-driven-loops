...
| File | Layer | Role |
|---|---|---|
| `.opencode/skills/system-spec-kit/mcp_server/code_graph/handlers/ccc-reindex.ts:27-54` | Handler | locates the `ccc` binary and runs `index` or `index --full` |
| `.opencode/skills/system-spec-kit/mcp_server/code_graph/tools/code-graph-tools.ts:86-90` | Tool surface | dispatches the requested tool |
