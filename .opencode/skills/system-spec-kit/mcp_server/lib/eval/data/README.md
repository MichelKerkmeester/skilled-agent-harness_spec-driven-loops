...
```bash
python3 .opencode/skills/sk-doc/scripts/validate_document.py .opencode/skills/system-spec-kit/mcp_server/lib/eval/data/README.md
```

Check JSON syntax when editing fixtures:

```bash
node -e "JSON.parse(require('node:fs').readFileSync('.opencode/skills/system-spec-kit/mcp_server/lib/eval/data/ground-truth.json', 'utf8'))"
```
