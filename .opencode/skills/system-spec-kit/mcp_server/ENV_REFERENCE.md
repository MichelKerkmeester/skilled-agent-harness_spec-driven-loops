...
| `SPECKIT_CODE_GRAPH_INDEX_SKILLS` | `false` (committed configs); see `SPECKIT_CODE_GRAPH_MAINTAINER_MODE` for one-shot maintainer override | boolean or csv | Maintainer opt-in for structural code graph scans to include `.opencode/skills/**`. Set `true` for all skills, or a comma-separated list such as `sk-code-review,sk-doc` for selected skills. Overridden by per-call `includeSkills` when provided. | `code_graph/lib/index-scope-policy.ts`, `code_graph/handlers/scan.ts` |
| `SPECKIT_CODE_GRAPH_INDEX_AGENTS` | `false` (committed configs); see `SPECKIT_CODE_GRAPH_MAINTAINER_MODE` for one-shot maintainer override | boolean | Maintainer opt-in for structural code graph scans to include `.opencode/agents/**`. Overridden by per-call `includeAgents` when provided. | `code_graph/lib/index-scope-policy.ts`, `code_graph/handlers/scan.ts` |
| `SPECKIT_CODE_GRAPH_INDEX_COMMANDS` | `false` (committed configs); see `SPECKIT_CODE_GRAPH_MAINTAINER_MODE` for one-shot maintainer override | boolean | Maintainer opt-in for structural code graph scans to include `.opencode/commands/**`. Overridden by per-call `includeCommands` when provided. | `code_graph/lib/index-scope-policy.ts`, `code_graph/handlers/scan.ts` |
| `SPECKIT_CODE_GRAPH_INDEX_SPECS` | `false` (committed configs); see `SPECKIT_CODE_GRAPH_MAINTAINER_MODE` for one-shot maintainer override | boolean | Maintainer opt-in for structural code graph scans to include `.opencode/specs/**`. Overridden by per-call `includeSpecs` when provided. | `code_graph/lib/index-scope-policy.ts`, `code_graph/handlers/scan.ts` |
| `SPECKIT_CODE_GRAPH_INDEX_PLUGINS` | `false` (committed configs); see `SPECKIT_CODE_GRAPH_MAINTAINER_MODE` for one-shot maintainer override | boolean | Maintainer opt-in for structural code graph scans to include `.opencode/plugins/**`. Overridden by per-call `includePlugins` when provided. | `code_graph/lib/index-scope-policy.ts`, `code_graph/handlers/scan.ts` |
| `SPECKIT_CODE_GRAPH_MAINTAINER_MODE` | unset (treated as false) | boolean | Maintainer-mode one-shot override. When set to `"true"` in `.env.local` (gitignored), the `system-code-graph-launcher.cjs` forces all 5 `SPECKIT_CODE_GRAPH_INDEX_*` to `"true"` at startup, overriding whatever the runtime's MCP config injected. Use this on a dev machine to index `.opencode/{skills,agents,commands,specs,plugins}` without leaking maintainer state into committed configs. Per-call `code_graph_scan` args still override env. | `.opencode/bin/system-code-graph-launcher.cjs` |
