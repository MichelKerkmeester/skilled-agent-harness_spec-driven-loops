...
| Acceptance coverage enforcement | OFF | `SPECKIT_AC_COVERAGE_ENFORCE` | Reserved promotion switch for a later strict coverage gate. The current shipped rule remains advisory | acceptance coverage gate |
...
Total unique variables documented: 305, counted as unique backticked names in first table columns (legacy HYDRA aliases removed, dual-stack CLI front-door variables included, see the "CLI front door" section). Recount with that method when adding rows. Multi-variable cells count once per cell here.
...
| Query-time existence filter | OFF | `SPECKIT_QUERY_TIME_EXISTENCE_FILTER` | Default-off query-time filter on the `memory_search` hot path: excludes already-ranked top-k rows whose backing file no longer exists and queues their ids as drift suspects instead of deleting them inline. Off preserves legacy query behavior byte-identical | `mcp_server/lib/config/capability-flags.ts`, `mcp_server/handlers/memory-search.ts` |
| Metadata disk-path consistency enforce | OFF (advisory) | `SPECKIT_METADATA_DISK_CONSISTENCY_ENFORCE` | Advisory→enforce toggle for the `METADATA_DISK_PATH_CONSISTENCY` rule: a `description.json`/`graph-metadata.json` path mismatch against the on-disk folder surfaces in `--strict` output but the rule still passes until this is set | `scripts/rules/check-metadata-disk-consistency.sh` |
| Status cross-doc consistency enforce | OFF (advisory) | `SPECKIT_STATUS_CROSS_DOC_ENFORCE` | Advisory→enforce toggle for the `STATUS_CROSS_DOC_CONSISTENCY` rule: a classified-status disagreement between `spec.md` and `implementation-summary.md` surfaces in `--strict` output but the rule still passes until this is set | `scripts/rules/check-status-cross-doc-consistency.sh` |
...
| `SPECKIT_RELEVANCE_AWARE_GAP` | `true` | boolean | Replaces the `detectEvidenceGap` Z-score peakedness check with the noise-floor-subtracted absolute top relevance the request-quality banding already computes, banded at the same `LOW_THRESHOLD`. A 043 benchmark proved the Z-score measures peakedness not relevance. When ON the gap fires when subtracted relevance falls below the band floor. When no embedder noise-floor resolves it fails closed to the Z-score path. Default OFF returns the Z-score result unchanged, so flag-off is byte-identical. Set `true` to enable. | `lib/search/evidence-gap-detector.ts`, `lib/search/search-flags.ts` |
...
| `SPECKIT_CODE_GRAPH_INDEX_SKILLS` | `false` (committed configs). See `SPECKIT_CODE_GRAPH_MAINTAINER_MODE` for one-shot maintainer override | boolean or csv | Maintainer opt-in for structural code graph scans to include `.opencode/skills/**`. Set `true` for all skills, or a comma-separated list such as `sk-code,sk-doc` for selected skills. Overridden by per-call `includeSkills` when provided. | `.opencode/skills/system-code-graph/mcp_server/lib/index-scope-policy.ts`, `.opencode/skills/system-code-graph/mcp_server/handlers/scan.ts` |
...
| `SPECKIT_METADATA_DISK_CONSISTENCY_ENFORCE` | `false` | boolean | Advisory→enforce toggle for the `METADATA_DISK_PATH_CONSISTENCY` rule. **Default OFF, advisory**: a `description.json`/`graph-metadata.json` path mismatch against the folder's actual on-disk path surfaces in `--strict` output but the rule still passes. Set `true`/`1` to fail `--strict` on a mismatch. | `scripts/rules/check-metadata-disk-consistency.sh` |
| `SPECKIT_STATUS_CROSS_DOC_ENFORCE` | `false` | boolean | Advisory→enforce toggle for the `STATUS_CROSS_DOC_CONSISTENCY` rule. **Default OFF, advisory**: a classified-status disagreement between `spec.md` and `implementation-summary.md` surfaces in `--strict` output but the rule still passes. Set `true`/`1` to fail `--strict` on a disagreement. | `scripts/rules/check-status-cross-doc-consistency.sh`, `scripts/lib/status-classifier.sh` |
...
| `SPECKIT_QUERY_TIME_EXISTENCE_FILTER` | `false` | boolean | Default-off query-time existence guard on the `memory_search` hot path. When enabled, already-ranked top-k rows whose backing file no longer exists are excluded from results and their ids are queued as drift suspects (`memory-drift-healing.ts`) for confirmation during the next `memory_index_scan`, instead of being deleted inline. Each response reports its `checked`/`excluded` values in `extraData.queryTimeExistenceFilter`; `extraData.queryTimeExistenceFilterAggregate` reports the process-lifetime cumulative totals. The aggregate is in-memory, resets on daemon restart, and is also available through `getQueryTimeExistenceFilterAggregateStats()` for in-process consumers. Reads through the shared opt-in helper (`isOptInEnabled()`, `lib/search/search-flags.ts`); set `true`/`1`/`yes`/`on`/`enabled` to opt in, unset preserves legacy query behavior byte-identical. Latency evidence: `020-query-time-filter-benchmark/implementation-summary.md`. | `lib/config/capability-flags.ts`, `lib/search/search-flags.ts`, `handlers/memory-search.ts`, `lib/storage/memory-drift-healing.ts` |
| `SPECKIT_SKIP_MEMORY_DRIFT_GIT_HOOK` | `0` (bypass off) | boolean (`"1"`) | Bypass for the shared git-hook memory-drift marker sourced by `post-commit`, `post-merge`, and `post-rewrite`. When unset, a spec-doc rename/delete recorded by the completed git operation is appended to `.memory-drift-dirty-paths.json`, which `startup-checks.ts` consumes on the next MCP boot to seed drift-suspect confirmation. Set `1` to skip the marker write for that git operation. | `.opencode/scripts/git-hooks/lib/memory-drift-marker.sh`, `mcp_server/startup-checks.ts` |
...
| `EMBEDDER_REINDEX_BATCH_SIZE` | `50` | number | Texts per write batch during an embedder reindex job (bounded 1 to 1000). Larger batches raise reindex throughput at higher peak memory. | `mcp_server/lib/embedders/reindex.ts` |
...
| `SPECKIT_SPEC_MEMORY_CLI_PROMPT_TIME` | unset (off) | flag | Marks the spec-memory CLI invocation as prompt-time, which implies warm-only. | `mcp_server/spec-memory-cli.ts` |
...
| `MK_SKILL_ADVISOR_CLI_PROMPT_TIME` | unset (off) | flag | Marks the skill-advisor CLI invocation as prompt-time, which implies warm-only. Alias: `SPECKIT_SKILL_ADVISOR_CLI_PROMPT_TIME`. | `.opencode/skills/system-skill-advisor/mcp_server/skill-advisor-cli.ts` |
...
| `MK_SKILL_ADVISOR_TRUST_DEFAULT` | unset (fail-closed untrusted) | string (`"trusted"`) | **Daemon-side** trust default. The advisor daemon fails closed (untrusted) when a caller's transport `_meta` is absent. Setting `trusted` in the daemon's own environment restores default-trusted behavior for native MCP surfaces whose clients send no `_meta`. Set in the committed MCP registrations (`.mcp.json`, `opencode.json`, `opencode.json`), and callers cannot forge it. | `.opencode/skills/system-skill-advisor/mcp_server/advisor-server.ts` |
...
*Generated from source code analysis. Last updated: 2026-07-02.*
