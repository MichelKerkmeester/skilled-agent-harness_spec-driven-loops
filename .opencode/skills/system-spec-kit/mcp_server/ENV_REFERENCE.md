...
| `SPECKIT_DETERMINISTIC_RANKING` | `false` | boolean | Removes wall-clock inputs from the ranking score so a fixed query is reproducible. When ON it passes `useDecay=false` into the vector lane and zeroes the `timestampBoost` and `computeRecencyScore` recency contributions on the hybrid and fusion paths. The trigger SQL id tie-break is independent and always on. Default OFF keeps the decay and recency terms exactly as before, so flag-off ranking is byte-identical. Set `true` to enable. | `lib/search/search-flags.ts`, `lib/search/hybrid-search.ts`, `lib/search/pipeline/stage2-fusion.ts` |
| `SPECKIT_RELEVANCE_AWARE_GAP` | `false` | boolean | Replaces the `detectEvidenceGap` Z-score peakedness check with the noise-floor-subtracted absolute top relevance the request-quality banding already computes, banded at the same `LOW_THRESHOLD`. A 043 benchmark proved the Z-score measures peakedness not relevance. When ON the gap fires when subtracted relevance falls below the band floor; when no embedder noise-floor resolves it fails closed to the Z-score path. Default OFF returns the Z-score result unchanged, so flag-off is byte-identical. Set `true` to enable. | `lib/search/evidence-gap-detector.ts`, `lib/search/search-flags.ts` |
