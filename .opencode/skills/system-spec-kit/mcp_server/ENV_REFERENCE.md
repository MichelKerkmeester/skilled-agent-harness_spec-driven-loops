...
| `SPECKIT_QUERY_TIME_EXISTENCE_FILTER` | `false` | boolean | Default-off query-time existence guard on the `memory_search` hot path. When enabled, already-ranked top-k rows whose backing file no longer exists are excluded from results and their ids are queued as drift suspects (`memory-drift-healing.ts`) for confirmation during the next `memory_index_scan`, instead of being deleted inline. Reads through the shared opt-in helper (`isOptInEnabled()`, `lib/search/search-flags.ts`); set `true`/`1`/`yes`/`on`/`enabled` to opt in, unset preserves legacy query behavior byte-identical. | `lib/config/capability-flags.ts`, `lib/search/search-flags.ts`, `handlers/memory-search.ts`, `lib/storage/memory-drift-healing.ts` |
