...
| `SPECKIT_ORPHAN_SWEEP_REFRESH_CADENCE_MS` | `20000` | number | Rate-gate for re-firing the maintenance-marker refresh during a long orphan sweep. Clamped to `[1, effective budget - 1]` so at least one refresh can fire per budget window; checked at bounded enqueue-page boundaries. A clamp logs a warning. | `mcp_server/handlers/memory-index.ts` |
