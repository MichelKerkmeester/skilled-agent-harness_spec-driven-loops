...
| `SPECKIT_ORPHAN_SWEEP_TIME_BUDGET_MS` | `45000` | number | Wall-clock budget for one global orphan-sweep invocation. Clamped to `[2, 90000]` — the upper bound is the maintenance-marker refresh threshold, so a sweep can never starve the marker. Budget-expired sweeps exit with a resumable cursor and report `partial`, never `complete`. A clamp logs a warning. | `mcp_server/handlers/memory-index.ts` |
| `SPECKIT_ORPHAN_SWEEP_REFRESH_CADENCE_MS` | `20000` | number | Rate-gate for re-firing the maintenance-marker refresh during a long orphan sweep. Clamped strictly below the effective time budget so at least one refresh can fire per budget window; checked between bounded 25-row deletion chunks, not only at page boundaries. A clamp logs a warning. | `mcp_server/handlers/memory-index.ts` |
