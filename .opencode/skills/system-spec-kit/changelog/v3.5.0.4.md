...
Turning it on for everyone is safe because the downside is bounded. If a live secondary adopts the released daemon, the backend survives. If none adopts it, the released daemon is ownerless and the existing idle self-exit reaps it, which lands back on the prior behavior rather than anything worse. The release-vs-kill decision is covered by a hermetic integration test that drives the real decision functions and real process semantics: flag-on releases the detached daemon and it survives, flag-off kills it.
...
Full multi-session secondary adoption is still under live observation rather than proven by a CI test, because a real launcher cannot be spawned in a test without touching the shared production lease and database. The shipped test validates the real decision functions and the OS release semantics with a hermetic stand-in, and the idle-timeout bound covers the unadopted case.
...
Spec packets: `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/007-mcp-daemon-reliability/017` through `027`.
