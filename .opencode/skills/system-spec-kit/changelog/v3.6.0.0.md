...
No migration is required for the common path. If you set `SPECKIT_SKILL_ADVISOR_HOOK_DISABLED`, rename it to `SYSTEM_SKILL_ADVISOR_HOOK_DISABLED`. The `mcp-magicpath` skill was removed, so switch any design work to `mcp-open-design`. The advisor recovery fix takes effect on your next fresh session. The new structural protections are on by default, while the flag-gated features stay off until you enable them.
