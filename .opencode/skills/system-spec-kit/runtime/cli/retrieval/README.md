...
| `lookup-trigger-index.mjs` | Loads the committed index and ranks candidates for one prompt, optionally scoped to a spec folder. The Gate 1 instruction that calls it lives once, in the root `AGENTS.md` (`CLAUDE.md` is a symlink to it); no runtime-specific instruction file carries a copy. |
