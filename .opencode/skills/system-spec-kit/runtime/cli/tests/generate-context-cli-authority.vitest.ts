...
import { ZERO_CONTINUITY_FINGERPRINT } from '../../api';
...
describe('generate-context CLI authority', () => {
  ...
  it('passes JSON-mode data and explicit CLI spec-folder override through main()', async () => {
    ...
    process.argv = ['node', path.join('runtime', 'cli', 'dist', 'continuity', 'generate-context.js'), dataFile, explicitSpecFolder];
  ...
  it('forwards explicit --session-id to workflow', async () => {
    ...
    process.argv = [
      'node',
      path.join('runtime', 'cli', 'dist', 'continuity', 'generate-context.js'),
      '--session-id',
      sessionId,
      dataFile,
      explicitSpecFolder,
  ...
  it('does not accumulate SIGINT or SIGTERM handlers across repeated module imports', async () => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      vi.resetModules();
      await import('../continuity/generate-context');
    }
