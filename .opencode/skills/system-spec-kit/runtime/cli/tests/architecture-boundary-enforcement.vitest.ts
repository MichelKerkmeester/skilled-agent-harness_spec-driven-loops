// ───────────────────────────────────────────────────────────────────
// MODULE: Architecture Boundary Enforcement Tests
// ───────────────────────────────────────────────────────────────────
...
// workspace (runtime vs scripts) vitest treats as process.cwd().
...
describe('Architecture Boundary Enforcement', () => {
  ...
  function createFixtureRoot(): string {
    ...
    fs.mkdirSync(path.join(root, 'runtime', 'cli'), { recursive: true });
    return root;
  }
  ...
  it('T39: GAP A detects shared -> runtime/scripts imports across syntax variants', () => {
    ...
    writeFixtureFile(root, 'shared/default-import.ts', [
      "import tool from '../runtime/cli/tool-default';",
      'export default tool;',
    ].join('\n'));

    writeFixtureFile(root, 'shared/namespace-import.ts', [
      "import * as tooling from '../runtime/cli/tool-namespace';",
      'export { tooling };',
    ].join('\n'));

    writeFixtureFile(root, 'shared/dynamic-import.ts', [
      'export async function loadRuntime() {',
      "  return import('../runtime/dynamic-runtime');",
      '}',
    ].join('\n'));
    ...
    expect(importPaths).toEqual(expect.arrayContaining([
      '../runtime/runtime',
      '../runtime/cli/tool-default',
      '../runtime/cli/tool-namespace',
  ...
  it('parses export-from, import type, and require() forms when checking shared neutrality', () => {
    ...
    writeFixtureFile(root, 'shared/export-from.ts', [
      "export { runtimeA } from '../runtime/runtime';",
    ].join('\n'));

    writeFixtureFile(root, 'shared/import-type.ts', [
      "import type { RuntimeConfig } from '../runtime/cli/types';",
      'export type { RuntimeConfig };',
    ].join('\n'));

    writeFixtureFile(root, 'shared/require-call.ts', [
      "const runtime = require('../runtime/cli/legacy-loader');",
      'export { runtime };',
    ].join('\n'));
  ...
  it('T40: GAP B flags wrappers exceeding 50 substantive lines', () => {
    ...
    writeFixtureFile(root, 'runtime/scripts/too-long.ts', [
      "import { spawnSync } from 'child_process';",
      "const targetScript = '../../runtime/cli/dist/continuity/cli.js';",
      body,
      "spawnSync(process.execPath, [targetScript], { stdio: 'inherit' });",
    ].join('\n'));

    const violations = checkWrapperOnly(root);
    const violation = violations.find((v) => v.file.endsWith(path.join('runtime', 'scripts', 'too-long.ts')));
  ...
  it('T41: GAP B flags wrappers missing child_process import', () => {
    ...
    writeFixtureFile(root, 'runtime/scripts/no-child-process.ts', [
      "const targetScript = '../../runtime/cli/dist/continuity/cli.js';",
    ...
    const violations = checkWrapperOnly(root);
    const violation = violations.find((v) => v.file.endsWith(path.join('runtime', 'scripts', 'no-child-process.ts')));
  ...
  it('T42: GAP B flags wrappers missing scripts/dist reference', () => {
    ...
    writeFixtureFile(root, 'runtime/scripts/no-dist-reference.ts', [
      "import { spawnSync } from 'child_process';",
    ...
    const violations = checkWrapperOnly(root);
    const violation = violations.find((v) => v.file.endsWith(path.join('runtime', 'scripts', 'no-dist-reference.ts')));

    expect(violation).toBeDefined();
    expect(violation?.reasons).toContain('no reference to cli/dist/');
  });

  it('T43: GAP B catches wrapper bypasses (barrel re-exports and scripts import without spawn/exec)', () => {
    const root = createFixtureRoot();

    writeFixtureFile(root, 'runtime/scripts/barrel-export.ts', [
      "export * from '../../runtime/cli/dist/continuity/generate-context.js';",
    ].join('\n'));

    writeFixtureFile(root, 'runtime/scripts/no-spawn-usage.ts', [
      "import { spawnSync } from 'child_process';",
      "import '../../runtime/cli/dist/continuity/generate-context.js';",
      'const forwarded = spawnSync;',
      'void forwarded;',
    ].join('\n'));

    const violations = checkWrapperOnly(root);
    const barrel = violations.find((v) => v.file.endsWith(path.join('runtime', 'scripts', 'barrel-export.ts')));
    const noSpawn = violations.find((v) => v.file.endsWith(path.join('runtime', 'scripts', 'no-spawn-usage.ts')));
  ...
  it('T44: legitimate thin wrappers pass GAP B checks', () => {
    ...
    writeFixtureFile(root, 'runtime/scripts/valid-wrapper.ts', [
      "import { spawnSync } from 'child_process';",
      "import path from 'path';",
      "const targetScript = path.resolve(__dirname, '../../runtime/cli/dist/continuity/generate-context.js');",
  ...
  it('T45: valid runtime/scripts -> shared imports are not false positives', () => {
    const root = createFixtureRoot();
    ...
    writeFixtureFile(root, 'runtime/lib/consumer.ts', [
