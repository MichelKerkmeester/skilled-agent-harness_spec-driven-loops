...
# contains 37 rule entries and the fixture tree contains 76 numbered
...
" 2>/dev/null || printf '%s\n' "False")
    ...
    if [[ "$actual" = "$expect" ]] && [[ "$json_valid" = true ]] && [[ "$has_fields" = "True" ]]; then
        ...
        if [[ "$VERBOSE" = true ]]; then
            ...
            echo "$json_output" | python3 -m json.tool 2>/dev/null | sed 's/^/    /' | head -10
        fi
    else
        echo -e "${RED}✗${NC} $name ${DIM}[${time_display}]${NC}"
...
# The 053 fixture is frozen and pinned; it declares a phrase that repeats a token of
# its own folder name, so the grep-convention rule reports it as a warning. The base
# suite documents this under its FOLDER-TOKEN NOTE; every row here expects the same.
    run_test "053-template-compliant-level2 passes cleanly" "053-template-compliant-level2" "warn"
fi
...
if begin_category "Exit Code Verification"; then
    run_test "Exit 0: Compact compliant fixture returns pass" "053-template-compliant-level2" "warn"
...
if begin_category "CLI Options"; then
    ...
    run_test_quiet "--quiet: minimal output (compact pass)" "053-template-compliant-level2" "warn"
...
if begin_category "Edge Cases: Anchor Scenarios"; then
    run_test "Compliant anchor order passes cleanly" "053-template-compliant-level2" "warn"
...
if begin_category "Edge Cases: Evidence Patterns"; then
    run_test "Compliant evidence citations pass cleanly" "053-template-compliant-level2" "warn"
fi

if begin_category "Edge Cases: Placeholder Contexts"; then
    run_test "Compliant fixture stays placeholder-free cleanly" "053-template-compliant-level2" "warn"
...
if begin_category "Edge Cases: Level Detection"; then
    run_test "Explicit level on compliant fixture passes cleanly" "053-template-compliant-level2" "warn"
...
if begin_category "Edge Cases: Special Directories"; then
    run_test "Compliant fixture stays stable without special directories" "053-template-compliant-level2" "warn"
fi
