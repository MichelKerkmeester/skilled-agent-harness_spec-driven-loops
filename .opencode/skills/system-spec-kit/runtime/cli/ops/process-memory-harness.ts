...
function isExternalMcpProcess(command: string): boolean {
  ...
  // runtime/ directory from being misread as an external, preserve-forever process.
  return /(?:^|[\s/])mcp-[^\s/]+(?=\s|$)/.test(command) || /(?:^|\s)--runtime(?:\s|=|$)/.test(command);
}
...
USAGE:
  node runtime/cli/dist/ops/process-memory-harness.js snapshot [--pretty]
  node runtime/cli/dist/ops/process-memory-harness.js fixture [--pretty]
