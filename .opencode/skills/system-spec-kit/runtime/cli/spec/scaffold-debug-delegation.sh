...
description: "Pre-filled handoff for @debug after 3+ failed task attempts. Operator must review and manually dispatch — never auto-dispatched."
trigger_phrases:
  - "debug"
  - "delegation"
  - "scaffold"
...
_memory:
  continuity:
    ...
    next_safe_action: "Operator review + manual @debug dispatch with this file as the handoff"
    blockers: []
    key_files: []
    session_dedup:
...
Pre-filled handoff for the @debug agent after 3+ failed task attempts. The operator must review and dispatch manually. Never auto-dispatched.
...
1. Operator: review this scaffold, fill any \`[Operator: ...]\` placeholders, then manually dispatch @debug with this file path as the structured handoff.
...
<!--
SCAFFOLD GENERATED BY: .opencode/skills/system-spec-kit/runtime/cli/spec/scaffold-debug-delegation.sh
SCHEMA SOURCE: .opencode/agents/debug.md (Debug Context Handoff format, lines 60-89)
HARD CONSTRAINT: workflow does NOT auto-dispatch @debug — operator dispatches manually.
