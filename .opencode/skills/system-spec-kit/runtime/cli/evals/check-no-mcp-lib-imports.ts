...
function resolveAllowlistPath(): string | null {
  const candidates = [
    ...
    // Compiled layout (node): runtime/cli/dist/evals/check-no-mcp-lib-imports.js
    path.resolve(moduleDir, '../../evals/import-policy-allowlist.json'),
    // CWD fallbacks
    path.resolve(process.cwd(), 'evals/import-policy-allowlist.json'),
    path.resolve(process.cwd(), 'runtime/cli/evals/import-policy-allowlist.json'),
  ];
...
function isAllowlisted(filePath: string, importPath: string, allowlist: Allowlist): boolean {
  ...
  const normalizedFile = `cli/${relativeFile}`;
