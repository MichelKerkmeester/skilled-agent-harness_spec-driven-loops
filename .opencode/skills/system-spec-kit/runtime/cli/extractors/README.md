...
The legacy module check (`npm run test:legacy`) additionally runs `tests/test-extractors-loaders.js`. Expected result: the extractor suites pass and the build emits `runtime/cli/dist/extractors/`.
...
- Runtime imports should use compiled modules under `runtime/cli/dist/extractors/`.
