...
description: "Code-facing README for .opencode/skills/system-spec-kit/runtime/tests/graph."
trigger_phrases:
  - "system-spec-kit runtime/tests/graph"
  - "code README"
---
...
This README documents the code-bearing folder `.opencode/skills/system-spec-kit/runtime/tests/graph` so operators can understand its role without opening every source file first. It follows the sk-doc skill README structure while staying focused on code navigation.
...
```bash
rg --files .opencode/skills/system-spec-kit/runtime/tests/graph
```
...
|---|---|
| Folder boundary | Documents direct code files under `runtime/tests/graph`. |
...
```text
User request: Check .opencode/skills/system-spec-kit/runtime/tests/graph for sk-code and README coverage.
