...
`templates/` (the `core/`, `addons/` and `packet-types/` folders plus `spec-kit-docs.json`) is private implementation infrastructure for Spec Kit template contracts. It maps public Level values to the document set, lazy document lifecycle and section gates consumed by scaffolding and validation code.
