...
| `MK_GIT_WORKTREE_GUARD_DISABLED` | unset (enabled) | truthy disable flag | Disables the worktree warning. Aliases: `MK_WORKTREE_GUARD_DISABLED` (pre-rename), `SPECKIT_WORKTREE_GUARD=off` (caller-side, exact `off` grammar). | `.opencode/bin/worktree-guard.sh`, `hooks/pi/session-start-advisories.ts` |
...
| `MK_LIVE_SYNC_DISABLED` | unset (enabled) | truthy disable flag | Master switch for the whole live-sync loop: post-commit autosync publish, SessionStart primary reconcile, IDE follower auto-start, and self-heal hook auto-install. No aliases. | `.opencode/scripts/git-hooks/post-commit`, `.opencode/bin/check-git-hooks.sh`, `.opencode/bin/git-primary-reconcile.sh`, `.opencode/bin/git-live-follow.sh` |
| `MK_PRIMARY_RECONCILE_DISABLED` | unset (enabled) | truthy disable flag | Disables SessionStart reconciliation of the clean primary live checkout only. Publish, follower, and self-heal legs keep running. No aliases. | `.opencode/bin/git-primary-reconcile.sh` |
...
| `MK_SCOPE_CHANGED_FILES` | (unset) | string | Opt-in change-set for the advisory `SCOPE_ADHERENCE` rule: whitespace/newline-separated repo-relative changed paths. Takes precedence over `MK_SCOPE_BASE`. The rule is a no-op when neither is set. | `scripts/rules/check-scope-adherence.sh` |
| `MK_SCOPE_BASE` | (unset) | string | Opt-in git ref for `SCOPE_ADHERENCE`; when `MK_SCOPE_CHANGED_FILES` is empty the change-set is `git diff --name-only <MK_SCOPE_BASE>`. Packet-owned docs are always in-scope. | `scripts/rules/check-scope-adherence.sh` |
