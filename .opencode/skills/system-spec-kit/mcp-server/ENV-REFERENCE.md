...
| `SPECKIT_AC_CLOSURE` | `true` | boolean | Default-on closure gate (ERROR) for Levels 2/3/3+. Unmet acceptance criteria block a completion claim and a waiver must cite an ADR that exists in `decision-record.md`. Set `false` to opt out. | `scripts/rules/check-ac-closure.sh` |
| `SPECKIT_AC_CLOSURE_CUTOFF` | `2026-08-29T00:00:00Z` | ISO date | Forward-only rollout boundary. Packets whose `spec.md` `Created` date precedes it, or cannot be read, stay advisory. | `scripts/rules/check-ac-closure.sh` |
