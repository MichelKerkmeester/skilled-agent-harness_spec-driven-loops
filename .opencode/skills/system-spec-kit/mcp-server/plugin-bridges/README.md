...
`plugin-bridges/` contains Node `.mjs` helper scripts that sit outside `.opencode/plugins/`. The scripts let plugin entrypoints call built MCP server and advisor surfaces while keeping OpenCode plugin discovery limited to real plugin files.
