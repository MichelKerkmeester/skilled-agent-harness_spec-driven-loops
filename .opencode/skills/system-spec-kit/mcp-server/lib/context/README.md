...
`mcp-server/lib/context/` owns the typed context contracts shared by MCP startup, resume, health, bootstrap, compaction, and OpenCode hook delivery paths. It keeps payload shape, trust metadata, transport rendering, caller identity, and publishability checks in one small package.
...
```bash
node .opencode/skills/sk-doc/scripts/extract_structure.py .opencode/skills/system-spec-kit/mcp-server/lib/context/README.md
node .opencode/skills/sk-doc/scripts/validate_document.py .opencode/skills/system-spec-kit/mcp-server/lib/context/README.md
```
