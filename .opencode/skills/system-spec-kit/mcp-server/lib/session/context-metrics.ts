...
export function computeQualityScore(): QualityScore {
  ...
  //
  //   recency (0.44)       — Highest weight because stale sessions are the primary
  //                          degradation signal. A 60-min gap almost certainly means
  //                          the LLM's working context has drifted from the codebase.
  //
  //   continuity (0.31)    — Second-highest. Frequent spec folder switches indicate
  //                          context fragmentation; stable focus correlates with quality.
  //
  //   recovery (0.25)      — A memory_context({ mode: "resume" }) call is the most
  //                          reliable signal that the session has recovered prior state.
  //                          Binary (done/not-done) so lower weight than continuous factors.
  //
  //   A fourth structural-freshness factor once carried 0.20 here. Its weight was
  //   redistributed proportionally across the three survivors rather than dropped,
  //   so the relative ordering the original weights encoded is preserved and the
  //   score stays on a 0..1 scale — leaving the level thresholds below meaningful.
  //
  //   Total: 0.44 + 0.31 + 0.25 = 1.00
  const score = (
    factors.recency * 0.44 +
    factors.recovery * 0.25 +
    factors.continuity * 0.31
  );
