...
`mcp-server/lib/` contains the shared TypeScript implementation used by MCP handlers and scripts. It owns retrieval pipelines, cognitive state, continuity parsing, scoring, persistence helpers, governance checks, response envelopes and evaluation support.
...
(cd .opencode/skills/system-spec-kit/mcp-server && ../scripts/node_modules/.bin/vitest run --reporter=default)
```
