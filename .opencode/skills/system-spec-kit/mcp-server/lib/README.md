...
`mcp-server/lib/` contains the TypeScript implementation behind the package's public API. It owns spec folder validation, the two generated metadata files a packet carries, per-folder description generation, continuity records and resume ladders, and the small shared helpers those depend on.
...
There is no stable `lib/` barrel for external callers. External consumers import `@spec-kit/mcp-server/api`; package-internal code imports the concrete module.
...
(cd .opencode/skills/system-spec-kit/mcp-server && npm run test:core)
```
