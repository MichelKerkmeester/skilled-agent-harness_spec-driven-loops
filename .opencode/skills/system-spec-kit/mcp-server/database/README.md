...
- Hold launcher runtime files: the owner lease, and (when `SPECKIT_LAUNCHER_LOG` is enabled, the default) the persistent launcher log `.system-spec-memory-launcher.log` plus its rotated `.prev.log` generation. These are gitignored runtime artifacts, not source.
