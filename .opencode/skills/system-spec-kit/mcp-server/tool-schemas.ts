...
const memoryUpdate: ToolDefinition = {
  ...
  inputSchema: { type: 'object', additionalProperties: ALLOW_UNKNOWN_PARAMETERS, properties: { id: { type: 'number', minimum: 1, description: 'Spec-doc record ID to update' }, title: { type: 'string', description: 'New title' }, triggerPhrases: { type: 'array', items: { type: 'string' }, description: 'Updated trigger phrases' }, importanceWeight: { type: 'number', minimum: 0, maximum: 1, description: 'New importance weight (0-1)' }, importanceTier: { type: 'string', enum: ['constitutional', 'critical', 'important', 'normal', 'temporary', 'archived', 'deprecated'], description: 'Set importance tier.' }, allowPartialUpdate: { type: 'boolean', default: false, description: 'Allow update to succeed even if embedding regeneration fails. When true, metadata changes are applied and the embedding is marked for re-index. When false (default), the entire update is rolled back on embedding failure.' }, expectedHash: { type: 'string', minLength: 1, description: 'Optional compare-and-swap precondition for constitutional rows. When provided, the current content hash must match before the update is accepted.' } }, required: ['id'] },
};
