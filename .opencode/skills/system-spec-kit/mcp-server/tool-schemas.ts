...
memoryContextProperties.includeConstitutional = {
  type: 'boolean',
  default: false,
  description: 'Deprecated: the constitutional tier is retired. Opt-in only; defaults to false and surfaces nothing when the tier is empty.',
};

// L2: Core - Primary operations (Token Budget: 3500)
const memorySearch: ToolDefinition = {
  name: 'memory_search',
  description: '[L2:Core] Search indexed spec-doc continuity semantically using vector similarity. Returns ranked results with similarity scores. Constitutional tier rules are ALWAYS included at the top of results (~2000 tokens max), regardless of query. Requires query (string), concepts (array of 2-5 strings), or cursor (string) for continuation pagination. Supports intent-aware retrieval (REQ-006) with task-specific weight adjustments. When implicit feedback logging is enabled, searches also emit shadow-only feedback signals such as search_shown and, for includeContent runs, result_cited. Token Budget: 3500. For code search, use Grep for exact text or token searches.',
  inputSchema: {
    type: 'object',
...
const memoryUpdate: ToolDefinition = {
  ...
  inputSchema: { type: 'object', additionalProperties: ALLOW_UNKNOWN_PARAMETERS, properties: { id: { type: 'number', minimum: 1, description: 'Spec-doc record ID to update' }, title: { type: 'string', description: 'New title' }, triggerPhrases: { type: 'array', items: { type: 'string' }, description: 'Updated trigger phrases' }, importanceWeight: { type: 'number', minimum: 0, maximum: 1, description: 'New importance weight (0-1)' }, importanceTier: { type: 'string', enum: ['constitutional', 'critical', 'important', 'normal', 'temporary', 'archived', 'deprecated'], description: 'Set importance tier. Constitutional tier rules always surface at top of results.' }, allowPartialUpdate: { type: 'boolean', default: false, description: 'Allow update to succeed even if embedding regeneration fails. When true, metadata changes are applied and the embedding is marked for re-index. When false (default), the entire update is rolled back on embedding failure.' }, expectedHash: { type: 'string', minLength: 1, description: 'Optional compare-and-swap precondition for constitutional rows. When provided, the current content hash must match before the update is accepted.' } }, required: ['id'] },
};
