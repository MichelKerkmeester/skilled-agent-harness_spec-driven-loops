...
`matrix-runners/templates/` owns the 14 prompt template files that define standardized test payloads for the matrix runner system. Each template corresponds to a feature cell (F1 through F14) and is loaded by `matrix-manifest.json` and executed by `run-matrix.ts` through executor-specific adapters.
...
```bash
cd .opencode/skills/system-spec-kit/mcp-server && npx vitest run tests/matrix-adapter
```
