...
describe('W7 empty code-graph readiness stress cell', () => {
  it('preserves harness metrics for empty fallback envelopes', async () => {
    const measurement = await runMeasurement({ workstream: 'W7', variant: 'variant' });
    const testCase = measurement.cases.find((item) => item.caseId === 'w7-code-graph-empty');
