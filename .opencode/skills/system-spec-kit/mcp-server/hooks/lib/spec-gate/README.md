...
| `evaluateMutation(request)` | Returns `allow`/`advise`/`deny` for a Write, Edit or Bash call against the cached gate state. Deny is opt-in via `SYSTEM_SPEC_GATE_ENFORCE`. |
