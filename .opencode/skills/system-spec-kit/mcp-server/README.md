...
- **The `mcp_*` tool surface.** Direct MCP callers (other agents, scripts, tests) reach the tools through `mcp__system_spec_memory__*` after the server registers them.
...
- **The plugin bridge.** The OpenCode plugin (`.opencode/plugins/system-spec-memory.js`) routes through `plugin-bridges/system-spec-memory-bridge.mjs` using CLI transport only — it shells the same `spec-memory.cjs` front door rather than holding a second MCP connection.
- **CLI scripts.** Maintenance, evaluation, and migration scripts live under `scripts/`.
- **The compiled backend artifact.** `dist/context-server.js` is built by `npm run build` and spawned by the launcher. MCP client configs point at `.opencode/bin/system-spec-memory-launcher.cjs`, which supervises the backend and owns the client-facing transport. The MCP registrations themselves are unchanged by the CLI: dual-stack means the CLI is additive.
...
**Default scan scope includes Spec Kit docs.** `memory_index_scan` defaults `includeSpecDocs=true` for `specs/` documents, with `background:true` jobs polled by `memory_index_scan_status` and stopped by `memory_index_scan_cancel`.
...
| `dist/context-server.js` | Runtime artifact | Compiled backend server spawned and supervised by the launcher. Client configuration points at `.opencode/bin/system-spec-memory-launcher.cjs`. |
...
| Rebuild + recycle helper | `bash .opencode/skills/system-spec-kit/scripts/deploy-mcp.sh` is the canonical helper to rebuild every MCP server `dist/` (system-spec-memory + `@spec-kit/shared`, code-graph, advisor) after a source change. `dist/` is gitignored, so rebuild after pulling source changes. Passing `--recycle` drives the *MCP front-proxy recycle* row above for system-spec-memory (transparent); code-graph and advisor are not SIGTERM-recycled (their launchers exit, so fresh dist loads on next start). Launcher `.cjs` changes need a FRESH session and cannot be picked up by a recycle. |
