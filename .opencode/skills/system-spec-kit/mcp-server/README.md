...
> `@spec-kit/mcp-server` is the compiled TypeScript engine behind spec folder validation, generated packet metadata, and the per-runtime hook adapters. It is consumed as a library, not run as a service.
...
| Boundary | Rule |
|---|---|
| Public API | External callers import `@spec-kit/mcp-server/api`. Reaching into `lib/`, `core/`, or `handlers/` needs a governed allowlist entry in `../scripts/evals/import-policy-allowlist.json`. |
...
|---|---|---|
| `@spec-kit/mcp-server/api` | Package export | The supported import surface, resolved to `dist/api/index.js`. |
