  ...
  describe('spec-folder mutex hardening', () => {
    it('retries through a filesystem-backed lock when another process already holds the spec-folder lock', async () => {
      ...
      const mkdirSyncMock = vi.fn((targetPath: fs.PathLike, options?: fs.MakeDirectoryOptions & { recursive?: boolean }) => {
        ...
        if (String(targetPath).includes('system-spec-memory-save-locks')) {
          lockAttempts += 1;
          if (lockAttempts === 1) {
  ...
  describe.skip('atomic-save failure injection', () => {
    ...
    function createCanonicalRoutingFixture(options: {
      ...
      writeCanonicalFixtureDoc(path.join(specFolder, 'implementation-summary.md'), {
        ...
        body: 'Updated `mcp-server/handlers/memory-save.ts` to support routed canonical writes.',
        extraAnchors: [
          {
            id: 'verification',
            heading: '## Verification',
    ...
    it('routes canonical atomic saves into the target implementation summary document', async () => {
      ...
      const result = await harness.module.atomicSaveMemory(
        {
          ...
          content: 'Implemented routed canonical writer integration in `mcp-server/handlers/memory-save.ts`.',
      ...
      expect(fs.readFileSync(fixture.targetPath, 'utf8')).toContain('Implemented routed canonical writer integration in `mcp-server/handlers/memory-save.ts`.');
    ...
    it('returns a non-mutating planner result for routed canonical saves by default', async () => {
      ...
      const result = await harness.module.atomicSaveMemory(
        {
          file_path: fixture.sourcePath,
          content: 'Implemented planner-first routed canonical writer integration in `mcp-server/handlers/memory-save.ts`.',
          routeAs: 'narrative_progress',
        },
        { force: true }
      );
    ...
    it('promotes template-contract violations to planner blockers for routed canonical saves', async () => {
      ...
      const result = await harness.module.atomicSaveMemory(
        {
          file_path: fixture.sourcePath,
          content: 'Implemented planner-first blocker classification in `mcp-server/handlers/memory-save.ts`.',
          routeAs: 'narrative_progress',
        },
        { force: false }
      );
