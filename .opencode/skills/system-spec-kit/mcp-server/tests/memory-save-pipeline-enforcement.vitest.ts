...
function buildValidPipelineMemory(overrides: PipelineMemoryOverrides = {}): string {
  const {
    ...
    fileTableEntries = [
      { path: 'mcp-server/handlers/memory-save.ts', description: 'Coordinates atomic save, sufficiency enforcement, duplicate detection, and post-mutation feedback for the memory save path.' },
      { path: 'mcp-server/lib/validation/save-quality-gate.ts', description: 'Three-layer quality gate that validates structure, content density, and semantic deduplication before persisting.' },
    ],
    observations = [
      {
...
afterAll(() => {
  ...
  // Remove the entire mkdtemp root (was: only FIXTURE_ROOT subdir)
...
describe('Cat 0: Golden Path', () => {
  ...
  it('dry-run accepts canonical spec docs that satisfy the spec-doc health contract', async () => {
    ...
    // The shared fixture carries its own packet id throughout its body, and this
    // test relocates it into a different host folder. Left as-is, those ids read
    // as foreign-packet contamination relative to the host and trip the cross-packet
    // hard block before the template-contract path the assertions below exercise.
    // Rewriting the id keeps the copied docs self-consistent with where they live.
