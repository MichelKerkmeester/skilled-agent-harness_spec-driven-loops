...
describe('handleMemoryDelete - Happy Path', () => {
  ...
  it('EXT-D1: Single ID delete returns deleted=1', async () => {
    ...
    const result = await handler.handleMemoryDelete({ id: 42, confirm: true });
  ...
  it('EXT-D2: Delete non-existent returns deleted=0', async () => {
    ...
    const result = await handler.handleMemoryDelete({ id: 999, confirm: true });
    const parsed = parseResponse(result);
  ...
  it('EXT-D3: String ID "7" parsed to numeric 7', async () => {
    ...
    await handler.handleMemoryDelete({ id: '7', confirm: true });
  ...
  it('EXT-D4: Successful delete clears caches', async () => {
    if (!handler?.handleMemoryDelete || !vectorIndex) { throw new Error('Test setup incomplete: memory-crud handler or vector-index unavailable'); }
    const calls = installDeleteMocks({ deleteResult: true });
    await handler.handleMemoryDelete({ id: 1, confirm: true });
  ...
  it('EXT-D5: Failed delete does not clear caches', async () => {
    ...
    await handler.handleMemoryDelete({ id: 999, confirm: true });
...
describe('handleMemoryDelete - Causal Edge Cleanup', () => {
  ...
  it('EXT-CE1: Causal edges cleaned up on single delete', async (ctx) => {
    ...
    if (!handler?.handleMemoryDelete || !vectorIndex) { throw new Error('Test setup incomplete: memory-crud handler or vector-index unavailable'); }
    const calls = installDeleteMocks({ deleteResult: true, dbAvailable: true });
    await handler.handleMemoryDelete({ id: 5, confirm: true });
  ...
  it('EXT-CE2: Causal edge cleanup failure aborts delete transaction', async (ctx) => {
    ...
    await expect(handler.handleMemoryDelete({ id: 3, confirm: true })).rejects.toThrow('Mock edge cleanup error');
  });
...
describe.skip('handleMemoryHealth - Happy Path', () => {
  ...
  it('EXT-H8: Health includes alias conflict summary', async () => {
    ...
    installHealthMocks({
      dbAvailable: true,
      aliasRows: [
        { file_path: '/workspace/specs/system-spec-kit/001-test/memory/a.md', content_hash: 'hash-1' },
        { file_path: '/workspace/.opencode/specs/system-spec-kit/001-test/memory/a.md', content_hash: 'hash-1' },
      ],
    });
    const result = await handler.handleMemoryHealth({});
    const parsed = parseResponse(result);
  ...
  it('EXT-H9: Health hints include divergent alias warning', async () => {
    ...
    installHealthMocks({
      dbAvailable: true,
      aliasRows: [
        { file_path: '/workspace/specs/system-spec-kit/002-test/memory/b.md', content_hash: 'hash-1' },
        { file_path: '/workspace/.opencode/specs/system-spec-kit/002-test/memory/b.md', content_hash: 'hash-2' },
      ],
    });
    const result = await handler.handleMemoryHealth({});
    const parsed = parseResponse(result);
  ...
  it('EXT-H10: divergent_aliases mode returns compact divergent-only payload', async () => {
    ...
    installHealthMocks({
      dbAvailable: true,
      aliasRows: [
        { file_path: '/workspace/specs/system-spec-kit/010-test/memory/a.md', content_hash: 'hash-1', spec_folder: 'system-spec-kit/010-test' },
        { file_path: '/workspace/.opencode/specs/system-spec-kit/010-test/memory/a.md', content_hash: 'hash-2', spec_folder: 'system-spec-kit/010-test' },
      ],
    });
    ...
    expect(parsed?.data?.groups?.[0]?.normalizedPath).toBe('specs/system-spec-kit/010-test/memory/a.md');
    expect(parsed?.data?.groups?.[0]?.variants?.map((variant: { filePath: string }) => variant.filePath)).toEqual([
      '.opencode/specs/system-spec-kit/010-test/memory/a.md',
      'specs/system-spec-kit/010-test/memory/a.md',
    ]);
  ...
  it('EXT-H10b: divergent_aliases mode normalizes Windows alias paths without leaking absolute prefixes', async () => {
    ...
    installHealthMocks({
      dbAvailable: true,
      aliasRows: [
        { file_path: 'C:\\workspace\\specs\\system-spec-kit\\011-test\\memory\\b.md', content_hash: 'hash-1', spec_folder: 'system-spec-kit/011-test' },
        { file_path: 'C:\\workspace\\.opencode\\specs\\system-spec-kit\\011-test\\memory\\b.md', content_hash: 'hash-2', spec_folder: 'system-spec-kit/011-test' },
      ],
    });
    const result = await handler.handleMemoryHealth({ reportMode: 'divergent_aliases', limit: 20 });
    const parsed = parseResponse(result);
    expect(parsed?.data?.totalDivergentGroups).toBe(1);
    expect(parsed?.data?.returnedGroups).toBe(1);
    expect(parsed?.data?.groups?.[0]?.normalizedPath).toBe('specs/system-spec-kit/011-test/memory/b.md');
    expect(parsed?.data?.groups?.[0]?.variants?.map((variant: { filePath: string }) => variant.filePath)).toEqual([
      '.opencode/specs/system-spec-kit/011-test/memory/b.md',
      'specs/system-spec-kit/011-test/memory/b.md',
    ]);
  });
  ...
  it('EXT-H10c: divergent_aliases mode detects relative specs aliases and groups them correctly', async () => {
    ...
    installHealthMocks({
      dbAvailable: true,
      aliasRows: [
        { file_path: 'specs/system-spec-kit/012-test/memory/c.md', content_hash: 'hash-1', spec_folder: 'system-spec-kit/012-test' },
        { file_path: '.opencode/specs/system-spec-kit/012-test/memory/c.md', content_hash: 'hash-2', spec_folder: 'system-spec-kit/012-test' },
      ],
    });
    const result = await handler.handleMemoryHealth({ reportMode: 'divergent_aliases', limit: 20 });
    const parsed = parseResponse(result);
    expect(parsed?.data?.totalDivergentGroups).toBe(1);
    expect(parsed?.data?.returnedGroups).toBe(1);
    expect(parsed?.data?.groups?.[0]?.normalizedPath).toBe('specs/system-spec-kit/012-test/memory/c.md');
    expect(parsed?.data?.groups?.[0]?.variants?.map((variant: { filePath: string }) => variant.filePath)).toEqual([
      '.opencode/specs/system-spec-kit/012-test/memory/c.md',
      'specs/system-spec-kit/012-test/memory/c.md',
    ]);
  });
  ...
  it('EXT-H11: divergent_aliases mode respects limit', async () => {
    ...
    installHealthMocks({
      dbAvailable: true,
      aliasRows: [
        { file_path: '/workspace/specs/system-spec-kit/020-test/memory/a.md', content_hash: 'hash-1', spec_folder: 'system-spec-kit/020-test' },
        { file_path: '/workspace/.opencode/specs/system-spec-kit/020-test/memory/a.md', content_hash: 'hash-2', spec_folder: 'system-spec-kit/020-test' },
        { file_path: '/workspace/specs/system-spec-kit/021-test/memory/b.md', content_hash: 'hash-3', spec_folder: 'system-spec-kit/021-test' },
        { file_path: '/workspace/.opencode/specs/system-spec-kit/021-test/memory/b.md', content_hash: 'hash-4', spec_folder: 'system-spec-kit/021-test' },
      ],
    });
  ...
  it('EXT-H14: autoRepair marks partial success when FTS consistency check throws but orphan cleanup succeeds', async (ctx) => {
    ...
    const result = await handler.handleMemoryHealth({ reportMode: 'full', autoRepair: true, confirmed: true });
    const parsed = parseResponse(result);
  ...
  // drift: verified against shipped behavior during Unit H
  it('EXT-H15: autoRepair reports no cleanup when the vector channel is unavailable', async (ctx) => {
    if (
      !handler?.handleMemoryHealth ||
      !vectorIndex ||
      !embeddingsSourceMod ||
    ...
    expect(parsed?.data?.repair?.attempted).toBe(false);
    expect(parsed?.data?.repair?.actions).not.toContain('orphan_vectors_cleaned:3');
    ...
    expect(parsed?.hints).toEqual(
      expect.arrayContaining([
        expect.stringContaining('vector channel unavailable'),
      ])
    );
...
describe('MCP Response Envelope Structure', () => {
  ...
  it('EXT-ENV1: Success response envelope structure', async () => {
    if (!handler?.handleMemoryDelete || !vectorIndex) { throw new Error('Test setup incomplete: memory-crud handler or vector-index unavailable'); }
    const calls = installDeleteMocks({ deleteResult: true });
    const result = await handler.handleMemoryDelete({ id: 1, confirm: true });
...
describe('Mutation ledger wiring', () => {
  ...
  it('EXT-ML1: single delete logs a delete mutation', async (ctx) => {
    ...
    await handler.handleMemoryDelete({ id: 42, confirm: true });
