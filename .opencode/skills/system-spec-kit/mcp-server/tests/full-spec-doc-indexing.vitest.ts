...
describe('Spec 126 Phase 4: Parser Enhancements', () => {
  ...
  describe('T069: extractDocumentType() filename mapping', () => {
    ...
    it('Returns "memory" for readme.md', () => {
      expect(extractDocumentType('/p/.opencode/skills/kit/README.md')).toBe('memory');
    });

    it('Returns "memory" for readme.txt', () => {
      expect(extractDocumentType('/p/.opencode/commands/kit/README.txt')).toBe('memory');
    });
  ...
  describe('T067: isMemoryFile() recognizes spec documents', () => {
    ...
    it('Rejects spec.md in /z-future/ directory', () => {
      expect(isMemoryFile('/p/.opencode/specs/003/100/z-future/spec.md')).toBe(false);
    });
    ...
    it('Rejects constitutional README.md files', () => {
      expect(isMemoryFile('/p/.opencode/skills/kit/constitutional/README.md')).toBe(false);
    });
  ...
  describe('T068: extractSpecFolder() handles non-memory paths', () => {
    ...
    it('Extracts folder from plan.md path (non-memory)', () => {
      const result = extractSpecFolder('/p/.opencode/specs/system-spec-kit/100-feature/plan.md');
      expect(result).toBe('system-spec-kit/100-feature');
    });

    it('Extracts folder from decision-record.md path (non-memory)', () => {
      const result = extractSpecFolder('/p/.opencode/specs/system-spec-kit/100-feature/decision-record.md');
      expect(result).toBe('system-spec-kit/100-feature');
    });
