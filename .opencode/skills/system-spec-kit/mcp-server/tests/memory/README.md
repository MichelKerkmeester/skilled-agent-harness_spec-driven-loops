...
description: "Code-facing README for .opencode/skills/system-spec-kit/mcp-server/tests/memory."
trigger_phrases:
...
This README documents the code-bearing folder `.opencode/skills/system-spec-kit/mcp-server/tests/memory` so operators can understand its role without opening every source file first. It follows the sk-doc skill README structure while staying focused on code navigation.
...
```bash
rg --files .opencode/skills/system-spec-kit/mcp-server/tests/memory
```
...
|---|---|
| Folder boundary | Documents direct code files under `mcp-server/tests/memory`. |
...
```text
User request: Check .opencode/skills/system-spec-kit/mcp-server/tests/memory for sk-code and README coverage.
