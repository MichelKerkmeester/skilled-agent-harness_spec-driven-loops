...
describe('index-scope helper', () => {
  ...
  it('rejects z_future and external for memory indexing; z_archive stays indexed', () => {
    ...
    expect(shouldIndexForMemory('/workspace/.opencode/specs/system-spec-kit/z-future/001-research/spec.md')).toBe(false);
...
describe('memory discovery respects index scope invariants', () => {
  it('skips z_future and external spec docs and graph metadata; discovers z_archive', () => {
    ...
    writeFixture(tempRoot, '.opencode/specs/system-spec-kit/z-future/001-future/spec.md');
    writeFixture(tempRoot, '.opencode/specs/system-spec-kit/z-future/001-future/graph-metadata.json', '{"packet_id":"001-future"}\n');
...
describe('code-graph scope policy helper', () => {
  it('skips external and z_future paths before code-graph scanning', () => {
    ...
    const futureFile = writeFixture(tempRoot, 'z-future/future.ts', 'export const future = true;\n');
