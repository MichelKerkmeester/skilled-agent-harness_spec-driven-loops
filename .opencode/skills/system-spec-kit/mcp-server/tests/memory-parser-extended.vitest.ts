...
describe('MEMORY PARSER EXTENDED TESTS', () => {
  ...
  describe('isMemoryFile', () => {
    ...
    it('T21: rejects README.md in constitutional dir', () => {
      expect(mod.isMemoryFile('/project/.opencode/skills/my-skill/constitutional/README.md')).toBe(false);
    });

    it('T21b: rejects z_future spec docs', () => {
      expect(mod.isMemoryFile('/project/.opencode/specs/003-auth/z-future/spec.md')).toBe(false);
    });

    it('T21c: rejects external spec docs', () => {
      expect(mod.isMemoryFile('/project/.opencode/specs/003-auth/external/spec.md')).toBe(false);
    });
