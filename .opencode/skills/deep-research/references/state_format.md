...
Research mode stores its runtime packet under the canonical `{artifact_dir}` resolved by `resolveArtifactRoot(specFolder, 'research')` from `.opencode/skills/system-spec-kit/shared/review-research-paths.cjs`. For root packets this resolves to `{spec_folder}/research/`. For child phases and sub-phases the resolver uses **flat-first**: a first run with an empty `research/` resolves directly to `{spec_folder}/research/` (no `pt-NN` wrapper). A `pt-NN` packet is allocated only when prior content for a non-matching target already exists. Continuation runs reuse the existing flat artifact (or matching `pt-NN` packet). Prompts, state, deltas, iteration findings, and canonical synthesis all live under the same `{artifact_dir}`. `{artifact_dir}/research.md` is workflow-owned canonical synthesis output.
...
- Machine-readable matrix: `.opencode/skills/deep-research/assets/runtime_capabilities.json`
...
{
  "mode": "review",
  "reviewTarget": "specs/030-deep-research-review-mode",
