...
Fully shipped in both `deep_start-research-loop_auto.yaml` and `confirm.yaml`. Parity
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml` | `step_resolve_artifact_root` (if_present/if_absent branches), `step_fanout_spawn`, `step_fanout_merge` |
| `.opencode/commands/deep/assets/deep_start-research-loop_confirm.yaml` | Same steps (confirm variant) |
| `.opencode/commands/deep/start-research-loop.md` | `--executor`/`--executors`/`--concurrency` flag docs, default policy, fan-out examples |
...
- Catalog source: `feature_catalog/01--loop-lifecycle/007-fanout-dispatch.md`
- Primary source files: `deep_start-research-loop_auto.yaml`, `start-research-loop.md`
Related references:
