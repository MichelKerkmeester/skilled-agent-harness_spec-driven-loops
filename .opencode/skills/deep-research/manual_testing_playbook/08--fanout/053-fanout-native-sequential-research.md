...
- `deep_start-research-loop_auto.yaml` present.

### Steps

1. `bash: grep -n "step_fanout_spawn_native\|agent: deep-research\|fanout_lineage_artifact_dir" .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml`
...
5. `bash: grep -n "native.*count\|count.*native\|native fan-out" .opencode/commands/deep/start-research-loop.md` — confirm native fan-out note in examples section.
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml` | `step_fanout_spawn_native`, `agent: deep-research` dispatch, `fanout_lineage_artifact_dir` context |
| `.opencode/commands/deep/start-research-loop.md` | Native fan-out note in EXAMPLES section |
...
- Scenario file path: `manual_testing_playbook/08--fanout/053-fanout-native-sequential-research.md`
