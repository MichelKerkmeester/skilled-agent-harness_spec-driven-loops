...
- Given: A research packet with at least one `blocked_stop` event, using `.opencode/skills/deep-research/scripts/tests/fixtures/interrupted-session/` once T048 lands or a hand-constructed minimal example that includes `blockedBy`, `gateResults`, `recoveryStrategy`, and `timestamp`.
- When: The operator runs `node .opencode/skills/deep-research/scripts/reduce-state.cjs <spec-folder>`.
...
- Feature catalog status: No `feature_catalog/` package exists under `.opencode/skills/deep-research/` as of 2026-04-11.
