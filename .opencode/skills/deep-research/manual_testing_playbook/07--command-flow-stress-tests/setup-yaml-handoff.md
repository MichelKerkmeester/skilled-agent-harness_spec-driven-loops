...
description: "Validate that /deep:start-research-loop resolves setup inputs before loading the auto YAML workflow and leaves grep-checkable state evidence."
---
...
This scenario sends a bounded one-iteration research request through `/deep:start-research-loop:auto`. The differential is whether the command resolves topic, spec folder, mode, max iterations, convergence and artifact root before the YAML workflow starts.
...
- Expected process: create sandbox, seed `/tmp/cp-046-spec/spec.md`, capture repo tripwire, run Call A as generic task, reset sandbox, run Call B through `/deep:start-research-loop:auto`, then grep artifacts and diffs.
...
4. Reset the sandbox.
5. Run Call B as `/deep:start-research-loop:auto ... --spec-folder=/tmp/cp-046-spec --max-iterations=1`.
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-research/manual_testing_playbook/07--command-flow-stress-tests/setup-cp-sandbox.sh --sandbox-dir /tmp/cp-046-sandbox
...
In /tmp/cp-046-sandbox/, run /deep:start-research-loop:auto for setup-binding evidence.
...
| File | Anchor |
|---|---|
| `.opencode/commands/deep/start-research-loop.md:7-23` | Markdown setup must resolve inputs before YAML load |
| `.opencode/commands/deep/start-research-loop.md:46-149` | single setup prompt and required bindings |
| `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:143-150` | preflight rejects missing bindings |
| `.opencode/skills/deep-research/SKILL.md:46-64` | command-owned workflow is mandatory |
