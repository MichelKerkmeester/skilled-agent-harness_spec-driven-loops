---
title: "MagicPath CLI Reference"
description: "Complete magicpath-ai command reference with flags, JSON output shapes, and the code subcommands for canvas authoring."
trigger_phrases:
  - "magicpath cli reference"
  - "magicpath-ai commands"
  - "magicpath inspect add"
  - "magicpath code subcommands"
  - "magicpath search list"
...
### Core Principle
Always pass `-o json` for structured output and `-y` to skip prompts when running from an agent.

### When to Use
- Look up the exact flags, arguments, and JSON shape for any `magicpath-ai` command.
- Confirm how `inspect`, `add`, and the `code` subcommands behave before running them.

### Key Sources
- [magicpath_operations.md](magicpath_operations.md) — the workflow that ties these commands together.
