---
title: "MagicPath Operations Guide"
description: "Full operational playbook for the magicpath-ai CLI: first-step auth, teams and people, the discover/understand/install/integrate workflow, theme application, design-to-production adaptation, canvas authoring with the code subcommands and Design Defaults, repository import, and a complete command quick reference."
trigger_phrases:
  - "magicpath workflow"
  - "magicpath code subcommands"
  - "magicpath canvas authoring"
  - "magicpath teams themes"
importance_tier: normal
contextType: general
---

# MagicPath Operations Guide

> Deep-dive operational guidance for the `mcp-magicpath` skill. `SKILL.md` routes here for full workflow detail. Sibling references: [CLI Reference](cli_reference.md), [Working with repositories](working_with_repositories.md), [Working with embedded browsers](working_with_embedded_browsers.md).
...
MagicPath canvas components can also be created and edited directly from local code via the `npx -y magicpath-ai code ...` subcommands — see [Edit or create canvas components from code](#10-edit-or-create-canvas-components-from-code). That path is strict: only `src/App.tsx`, `src/index.css`, files under `src/components/generated/`, and temporary image assets under `assets/` in the code working directory are editable.

When this skill runs inside an agent host with an embedded browser, use a MagicPath project as a persistent visual canvas beside the agent when appropriate. If you create a project for canvas authoring, open that project in the embedded browser immediately after creation and before `code start`; see [Working with embedded browsers](working_with_embedded_browsers.md).
...
### Core Principle
MagicPath is a design tool: a component tells you WHAT to build; your job is to make it work, responsively and fully wired.

### When to Use
- Execute the full install workflow: discover, understand, install, integrate.
- Author or edit canvas components with the `code` subcommands under the Design Defaults.
- Apply themes, navigate teams and people, or import a repository onto the canvas.

### Key Sources
- [cli_reference.md](cli_reference.md) — exact command flags and JSON shapes.
- [working_with_repositories.md](working_with_repositories.md) — repository-to-canvas import.
- [working_with_embedded_browsers.md](working_with_embedded_browsers.md) — embedded-canvas hosts.
...
## 5. DESIGN-TO-PRODUCTION MINDSET
...
If the user also asked for a design inside the new project, take the `id` from the response and continue with the canvas-component creation flow described under [Edit or create canvas components from code](#10-edit-or-create-canvas-components-from-code) (`code start --project <id> --name "..."`, fill in the scaffolded files, `code submit --wait`). Do not re-create the project per design — one project holds many components.
...
## 8. USE A MAGICPATH PROJECT AS AN EMBEDDED CANVAS WHEN AVAILABLE
...
Do not open a project automatically for background work such as `info`, `whoami`, listing/searching data, retrieving themes, inspecting source, or installing a component into an application. Full decision guidance and recipes live in [Working with embedded browsers](working_with_embedded_browsers.md).

## 9. BRING AN EXISTING REPOSITORY INTO MAGICPATH
...
Full step-by-step guidance — the styling-translation table, edge cases (monorepos, non-React sources, server components, Tailwind v3→v4), and quick recipes for "bring the sidebar of my app" and "render this project" — lives in [Working with repositories](working_with_repositories.md).

## 10. EDIT OR CREATE CANVAS COMPONENTS FROM CODE
...
- [Working with repositories](working_with_repositories.md) — bring an existing local or online Git repository's UI onto the MagicPath canvas (e.g. "render this project in MagicPath", "bring the sidebar of my app into MagicPath")
- [Working with embedded browsers](working_with_embedded_browsers.md) — use a MagicPath project as the persistent canvas inside Codex, Cursor, or another host with an in-app browser
