...
description: "Recreate UI from a local path or Git repo onto the MagicPath canvas via the code start to submit flow, the inverse of add and inspect."
trigger_phrases:
  - "import repo into magicpath"
  - "recreate ui on canvas"
  - "bring app into magicpath"
  - "render project in magicpath"
...
### Core Principle
The source of truth is the user's repository; the destination is the MagicPath canvas. Recreate, do not install.

### When to Use
- "Bring the sidebar of my app into MagicPath", "render this project in MagicPath", "recreate my landing page here".
- Importing UI from a local path or a GitHub/GitLab/Bitbucket repo onto the canvas.

### Key Sources
- [magicpath_operations.md](magicpath_operations.md) — the broader skill workflow and Design Defaults.
- [cli_reference.md](cli_reference.md) — `code start` / `code submit` command details.

---

## 2. WHEN THIS APPLIES (TRIGGERS)
...
## 5. PHASE 1 — MAP THE PROJECT (READ THE DESIGN FOUNDATION FIRST)
...
## 8. PHASE 4 — BUILD IT ON THE CANVAS

Use the **create** path of the `code` flow. (Full command details live in `cli_reference.md`.)
