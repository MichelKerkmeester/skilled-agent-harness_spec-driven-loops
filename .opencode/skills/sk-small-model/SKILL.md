...
Operator prompt mentions small-model name or pattern
    ...
    +-- enhances edge pulls sk-small-model into top-3
```
...
TASK CONTEXT
    ...
    +- STEP 1: Surface sk-small-model alongside the executor skill
...
def surface_sk_small_model(prompt, advisor):
    if matches_trigger(prompt, SMALL_MODEL_TRIGGERS):
        # Lexical lane match
        return ("sk-small-model", advisor.lexical_score(prompt))
    if "cli-devin" in advisor.top_k(prompt, k=3) or "cli-opencode" in advisor.top_k(prompt, k=3):
        ...
        return ("sk-small-model", 0.5 * advisor.enhances_weight)
    return None
```
...
1. **Discovery** — Operator mentions a small-model name or pattern; the advisor surfaces `sk-small-model` alongside the relevant CLI skill.
...
```text
sk-small-model (this skill)         ← discovery + index only
    |
