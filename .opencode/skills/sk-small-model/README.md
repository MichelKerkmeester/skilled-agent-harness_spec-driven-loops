---
title: "sk-small-model: Small-Model Optimization Sentinel"
...
Routing is automatic. Mention any small-model name (`SWE-1.6`, `Kimi-k2.6`, `DeepSeek-v4-pro`, `Qwen3.6`, `Claude Haiku`, `Gemini Flash`) or pattern (`context budget`, `output verification`, `permissions matrix`, `quota fallback`, `model profile`, `tool scoring`) in a prompt and the skill-advisor will surface `sk-small-model` alongside the matching CLI skill.

**Step 2: Read the pattern index.**

```bash
cat .opencode/skills/sk-small-model/references/pattern-index.md
```
...
Expected result: `sk-small-model` appears in the top-3 with confidence ≥ 0.8.
<!-- /ANCHOR:quick-start -->
...
| Routing co-surfacing | Operator prompt with small-model signals | `sk-small-model` in advisor top-3 | [`graph-metadata.json`](./graph-metadata.json) |
...
|---|---|---|
| `enhances → cli-devin` | weight 0.5 | Co-surface sk-small-model when cli-devin is already high-confidence |
| `enhances → cli-opencode` | weight 0.5 | Co-surface sk-small-model when cli-opencode is already high-confidence |
...
Resources loaded: sk-small-model/references/pattern-index.md
...
Skill routing: cli-devin scores high → enhances edge surfaces sk-small-model
Resources loaded: cli-devin/SKILL.md + sk-small-model/SKILL.md + sk-small-model/references/pattern-index.md
...
Resources loaded: sk-small-model/references/pattern-index.md § 4 (Adopting a New Provider)
...
Resources loaded: sk-small-model/references/pattern-index.md § 5 (Staleness Policy)
...
|---|---|---|
| `sk-small-model` not surfacing on small-model prompts | Trigger phrases stale or advisor not re-indexed | Run `skill_advisor.py --force-refresh`; verify with a sample prompt |
