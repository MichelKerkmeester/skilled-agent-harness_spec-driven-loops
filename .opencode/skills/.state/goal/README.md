...
description: Stores session goal state, continuation decisions and archived goal snapshots for the opencode-goal plugin.
trigger_phrases:
  - "goal runtime state"
...
> Runtime storage for active and archived session goals managed by `opencode-goal.js`.
...
|---|---|---|
| `OPENCODE_GOAL_STATE_ACTIVE_RETENTION_DAYS` | `2` days | Controls when inactive goal files move into `.archive/`. |
| `OPENCODE_GOAL_STATE_ARCHIVE_RETENTION_DAYS` | `90` days | Controls archive and log retention. |
| `OPENCODE_GOAL_STATE_SWEEP_INTERVAL_MS` | `3600000` | Controls how often cleanup may run. |
| `OPENCODE_GOAL_JSONL_MAX_BYTES` | `5242880` bytes | Controls when owned JSON Lines logs rotate. |
| `OPENCODE_GOAL_DEBUG` | Disabled | Enables `.goal-events.log` records and debug diagnostics. |

The plugin prunes expired archive files and old log segments. It retains one active log plus timestamped rotated segments.

The `doctor` and `health` actions inspect active and archived file counts, owned log sizes, the last sweep time and stale active-file candidates. `OPENCODE_GOAL_DEBUG=1` enables bounded `.goal-events.log` entries and debug diagnostics. The plugin otherwise keeps that debug log inactive.

---

## 6. SYSTEM BOUNDARIES

`opencode-goal.js` is a standalone local OpenCode plugin. It is not a Spec Kit Memory MCP tool and does not use a daemon-backed plugin bridge. The plugin owns its file store, event hook, system transform and tools directly.
...
Other skills can invoke the plugin tools through the command surface, but they do not own these files. The plugin remains the only writer, while `opencode_goal_status`, `history`, `doctor` and `health` provide supported read paths for operators and workflows.
