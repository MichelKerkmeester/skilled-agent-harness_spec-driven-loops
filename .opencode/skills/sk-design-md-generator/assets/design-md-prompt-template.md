...
Then fill the three placeholders (`<DOMAIN>`, `<TOKENS_JSON_PATH>`, `<DESIGN_MD_PATH>`) and hand the prompt below to the writing agent. Load `references/design-md-format.md` (the section spec) and `references/writing-style-guide.md` (voice) alongside it.
...
working material. Read references/design-md-format.md for the section order and
references/writing-style-guide.md for voice before writing.
...
- [cardinal-rules-card.md](./cardinal-rules-card.md) - the fidelity rules as a standalone checklist.
- [../references/extraction-workflow.md](../references/extraction-workflow.md) - the full three-phase workflow.
