...
1. NEGATIVE CONTROL: `cd .opencode/skills/sk-design-md-generator/tool && npx ts-node scripts/extract.ts https://www.cloudflare.com --fast`  # -> exits non-zero or reports zero pages crawled; stderr/stdout contains 403/429/blocked errors
2. inspect the output directory: `bash: ls output/cloudflare.com/ 2>/dev/null \|\| echo "output directory not created"`  # -> tokens.json absent, empty, or has zero-page token arrays
3. if tokens.json exists: `bash: node -e "const t = require('./output/cloudflare.com/tokens.json'); console.log('colorTokens:', t.colorTokens?.length ?? 0, 'total pages:', t.extractionReport?.pagesCrawled ?? 0)"` (run from `tool/`)  # -> colorTokens length 0, pagesCrawled 0
...
| File | Role |
|---|---|
| `../../tool/scripts/extract.ts` | Extraction orchestrator — handles crawl errors, emits exit codes |
| `../../tool/scripts/crawl.ts` | Playwright crawler — produces the 403/429/empty-page errors |
| `../../tool/scripts/cluster.ts` | Token classifier — never invoked when zero pages are crawled |
