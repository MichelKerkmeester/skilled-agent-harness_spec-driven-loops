...
1. NEGATIVE CONTROL: `cd .opencode/skills/sk-design-md-generator/tool && npx ts-node scripts/extract.ts https://www.cloudflare.com --fast --output .opencode/specs/<track>/<packet>/output`  # -> exits non-zero or reports zero pages crawled; stderr/stdout contains 403/429/blocked errors
...
3. if tokens.json exists: `bash: node -e "const t = require('./<--output>/tokens.json'); console.log('colorTokens:', t.colorTokens?.length ?? 0, 'total pages:', t.extractionReport?.pagesCrawled ?? 0)"` (run from `tool/`)  # -> colorTokens length 0, pagesCrawled 0
