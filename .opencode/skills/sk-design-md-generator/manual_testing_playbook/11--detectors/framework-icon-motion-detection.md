...
3. `cd .opencode/skills/sk-design-md-generator/backend && npx ts-node scripts/extract.ts <url> --fast --output .opencode/specs/<track>/<packet>/output`  # -> exits 0, crawl progress on stdout
4. `bash: node -e "const t = require('./<--output>/tokens.json'); console.log('framework:', JSON.stringify(t.framework, null, 0)); console.log('iconSystem:', JSON.stringify(t.iconSystem, null, 0)); console.log('motion:', JSON.stringify(t.motion, null, 0)); console.log('designBoundary:', JSON.stringify(t.designBoundary, null, 0))"` (run from `backend/`)  # -> each field present, populated or null
