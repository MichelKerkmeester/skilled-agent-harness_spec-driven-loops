...
1. `bash: node -e "const t = require('./output/<domain>/tokens.json'); console.log('darkMode present:', !!t.darkMode); if(t.darkMode) { console.log('detectionMethod:', t.darkMode.detectionMethod); console.log('has variableDiff:', !!t.darkMode.variableDiff?.length); console.log('has darkTokens:', !!t.darkMode.darkTokens?.length) }"` (run from `tool/`)  # -> darkMode metadata printed
...
| `../../tool/resources/design-md-format.md` | v2 section specification — §2.5 Dark Mode placement and content rules |
