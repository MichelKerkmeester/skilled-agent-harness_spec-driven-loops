...
1. `cd .opencode/skills/sk-design-md-generator/tool && npx ts-node scripts/validate.ts <path-to-faithful-style-reference.md> <--output>/tokens.json`  # -> exit 0, values 100, claims >= 80, failures array empty
...
4. `cd tool && npx ts-node scripts/validate.ts /tmp/planted.md <--output>/tokens.json`  # -> exit non-zero or failures non-empty, at least one phantom-color finding for ff0000
...
| `../../tool/resources/design_md_format_v3.md` | v3 Style Reference section specification — defines the named token tables, Components, Quick Start, and Similar Brands sections |
