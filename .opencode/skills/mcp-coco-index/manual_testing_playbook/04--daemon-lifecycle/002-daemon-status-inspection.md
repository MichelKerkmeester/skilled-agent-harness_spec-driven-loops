...
- Expected signals: Step 1 output includes version or uptime information. Step 2 `daemon.pid`, `daemon.sock`, `daemon.lock` and `daemon.spawn-lock` files exist. 0-byte lock files are normal
...
|---|---|---|---|---|---|---|---|---|
| DMN-002 | Daemon status inspection | Verify `ccc daemon status` shows version and uptime. PID and socket files exist | `Verify ccc daemon status reports runtime info and the daemon PID/socket files exist.` | 1. `bash: ccc daemon status` -> 2. `bash: ls ~/.cocoindex_code/daemon.pid ~/.cocoindex_code/daemon.sock ~/.cocoindex_code/daemon.lock ~/.cocoindex_code/daemon.spawn-lock` | Step 1 output includes version or uptime information. Step 2 PID, socket, lifetime lock and spawn-lock files exist. 0-byte lock files are normal | Daemon status output, file listing | PASS if daemon status reports running AND PID, socket and lock files exist. PARTIAL if status reports running but one file missing. FAIL if daemon not running or all files are missing | Run `ccc search "test" --limit 1` or `ccc index` to trigger daemon auto-start, then re-check status. Inspect `daemon.log` for errors. Verify `~/.cocoindex_code/` permissions |
...
Operators can inspect `~/.cocoindex_code/daemon.lock` to confirm the daemon lifetime lock file exists. A 0-byte size is normal.
Operators can inspect `~/.cocoindex_code/daemon.spawn-lock` to confirm the client spawn coordination file exists. A 0-byte size is normal.
PID-file readers do not need lock awareness because `daemon.pid` is informational.
