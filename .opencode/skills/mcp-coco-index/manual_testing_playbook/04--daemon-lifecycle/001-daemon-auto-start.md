...
- Expected signals: Step 1 daemon stops or reports already stopped. Step 3 search returns at least 1 result. Step 5 daemon reports running. After healthy startup `~/.cocoindex_code/` contains `daemon.pid`, `daemon.sock`, `daemon.log`, `daemon.lock` and `daemon.spawn-lock`
...
After healthy startup, `~/.cocoindex_code/` contains 5 runtime files: `daemon.pid`, `daemon.sock`, `daemon.log`, `daemon.lock` and `daemon.spawn-lock`.
...
`RuntimeError: daemon already running at PID N; refusing to start` is the expected exit message when a sibling daemon is already alive.
