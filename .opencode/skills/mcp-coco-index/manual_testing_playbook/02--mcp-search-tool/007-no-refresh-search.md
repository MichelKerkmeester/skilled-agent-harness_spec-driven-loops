...
Patch 2 added socket-level safety for client disconnects.
`mcp_server/cocoindex_code/daemon.py:62` defines `_safe_send_bytes`.
All 6 daemon response-send sites now call that wrapper.
The wrapper swallows `BrokenPipeError` and `ConnectionResetError`.
It logs one INFO line when the client disconnects before a response send.
Operators should see zero stack traces in `~/.cocoindex_code/daemon.log` from client disconnects during this scenario.
