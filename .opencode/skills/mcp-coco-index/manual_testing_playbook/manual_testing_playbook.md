...
- The shared skill advisor exists at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py`.
...
#### Scenario Contract
Prompt summary: As a manual-testing orchestrator, search CocoIndex for "skill" only under .opencode/skills/ against the current CocoIndex CLI, daemon, and MCP surfaces in this repository. Verify All result paths begin with .opencode/skills/; no results from other directories. Return a concise user-facing pass/fail verdict with the main reason.
