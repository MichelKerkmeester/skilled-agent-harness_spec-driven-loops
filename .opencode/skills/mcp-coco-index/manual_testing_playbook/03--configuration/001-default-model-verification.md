...
- Prompt: `Verify global CocoIndex settings include a documented embedding model; return pass/fail with reason.`
...
- Expected signals: Settings file exists; `embedding.provider` is `sentence-transformers`; `embedding.model` is the current EmbeddingGemma default (`google/embeddinggemma-300m` in settings or `sbert/google/embeddinggemma-300m` in environment defaults); the query prompt resolves to `InstructionRetrieval`.
...
|---|---|---|---|---|---|---|---|---|
| CFG-001 | Default model verification | Verify global settings contain the documented default embedding model | `Verify global CocoIndex settings include a documented embedding model; return pass/fail with reason.` | 1. `bash: cat ~/.cocoindex_code/global_settings.yml` -> 2. Locate `embedding.provider` and `embedding.model` fields -> 3. Confirm EmbeddingGemma query prompt resolves to `InstructionRetrieval` | Settings file exists; `embedding.provider` is `sentence-transformers`; `embedding.model` is the current EmbeddingGemma default (`google/embeddinggemma-300m` in settings or `sbert/google/embeddinggemma-300m` in environment defaults); query prompt resolves to `InstructionRetrieval` | Contents of `global_settings.yml` with embedding fields highlighted plus prompt-registry evidence | PASS if embedding provider/model and query prompt match the current defaults; FAIL if file missing, model field absent, or stale MiniLM/Voyage defaults are reported as current | Check `COCOINDEX_CODE_DIR` env var; verify `~/.cocoindex_code/` directory exists; run `ccc init` if needed |
