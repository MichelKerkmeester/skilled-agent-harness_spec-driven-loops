---
title: "mcp coco index mcp server tests: Code README"
description: "Code-facing README for .opencode/skills/mcp-coco-index/mcp_server/tests."
trigger_phrases:
  - "mcp-coco-index mcp_server/tests"
  - "code README"
---

# mcp coco index mcp server tests
...
This README documents the code-bearing folder `.opencode/skills/mcp-coco-index/mcp_server/tests` so operators can understand its role without opening every source file first. It follows the sk-doc skill README structure while staying focused on code navigation.
...
```bash
rg --files .opencode/skills/mcp-coco-index/mcp_server/tests
```
...
|---|---|
| Folder boundary | Documents direct code files under `mcp_server/tests`. |
...
| `test_observability.py` | PY source file in this folder. |
| `test_refresh_split.py` | PY source file in this folder. |
...
```text
User request: Check .opencode/skills/mcp-coco-index/mcp_server/tests for sk-code and README coverage.
