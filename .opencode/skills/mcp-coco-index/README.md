...
If that exploration feeds into Spec Kit packet recovery, `/speckit:resume` remains the canonical surface. Packet continuity still comes from `handover.md`, then `_memory.continuity`, then the remaining spec docs, while generated memory artifacts stay secondary.
