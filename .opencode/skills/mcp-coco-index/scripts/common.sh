...
compute_recommended_next_step() {
    ...
    if [[ " ${READINESS_BLOCKING_ISSUES[*]-} " == *" $EXIT_HELPER_PAYLOAD_INCOMPLETE "* ]]; then
        printf '%s\n' "Restore the CocoIndex helper scripts under .opencode/skills/mcp-coco-index/scripts."
        return 0
    fi
    if [[ " ${READINESS_BLOCKING_ISSUES[*]-} " == *" $EXIT_BINARY_MISSING "* ]]; then
        printf 'Run bash .opencode/skills/mcp-coco-index/scripts/ensure_ready.sh --root "%s"\n' "$READINESS_PROJECT_ROOT"
        return 0
    fi
    if [[ " ${READINESS_BLOCKING_ISSUES[*]-} " == *" $EXIT_INDEX_MISSING "* ]]; then
        printf 'Run bash .opencode/skills/mcp-coco-index/scripts/ensure_ready.sh --root "%s" --refresh-index\n' "$READINESS_PROJECT_ROOT"
        return 0
    fi
    if [[ " ${READINESS_BLOCKING_ISSUES[*]-} " == *" $EXIT_REQUIRED_CONFIG_MISSING "* ]] || [[ " ${READINESS_WARNINGS[*]-} " == *" $EXIT_REQUIRED_CONFIG_MISSING "* ]]; then
        printf '%s\n' "Follow .opencode/skills/mcp-coco-index/references/downstream_adoption_checklist.md and assets/config_templates.md to add cocoindex_code to the required configs."
        return 0
    fi
