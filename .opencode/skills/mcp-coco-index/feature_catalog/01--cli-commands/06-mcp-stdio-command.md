...
| File | Layer | Role |
|------|-------|------|
| `.opencode/skills/mcp-coco-index/mcp_server/cocoindex_code/cli.py:473` | CLI | Defines `ccc mcp`. |
| `.opencode/skills/mcp-coco-index/mcp_server/cocoindex_code/server.py:74` | MCP server | Creates the FastMCP server. |
| `.opencode/skills/mcp-coco-index/references/tool_reference.md:185` | Reference | Documents `ccc mcp` and the CLI-only boundary. |

### Validation And Tests

| File | Type | Role |
|------|------|------|
| `.opencode/skills/mcp-coco-index/tests/test_e2e.py:189` | End-to-end | Covers CLI help and daemon status around the full session. |
| `.opencode/skills/mcp-coco-index/tests/test_protocol.py:47` | Protocol | Covers search request defaults used under the daemon protocol. |
