...
1. `bash: rg -n "^function (writeConfig|writeStrategyMd|writeStateJsonl|writeSeat|writeDeliberation|writeCritique|writeReport)" .opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js`
2. `bash: rg -n "artifact_written" .opencode/skills/sk-ai-council/scripts/lib/`
...
|---|---|---|---|---|---|---|---|---|
| DAC-013 | Library writer call sequence | Verify seven writers and audit events | `Inspect lib/persist-artifacts.js named exports and verify the canonical writer call sequence with artifact_written events.` | `bash: rg -n "^function (writeConfig\|writeStrategyMd\|writeStateJsonl\|writeSeat\|writeDeliberation\|writeCritique\|writeReport)" .opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js` -> `bash: rg -n "artifact_written" .opencode/skills/sk-ai-council/scripts/lib/` | Seven writers and audit event references across lib/ | Grep output | PASS if all writers and audit emission are present | Check renamed writer contract |
...
| File | Role |
|---|---|
| `.opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js` | Writer library and audit event implementation |
| `.opencode/agents/sk-ai-council.md` | Canonical writer sequence in invocation contract |
...
- Feature file path: `07--writer-library-contract/001-library-writer-call-sequence.md`
