...
1. Read `.opencode/agents/sk-ai-council.md` §16.
2. Inspect `.opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js` path guard logic.
...
1. `bash: rg -n "OUT_OF_SCOPE_WRITE|ai-council" .opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js | head -20`
2. `bash: rg -n "OUT_OF_SCOPE_WRITE" .opencode/agents/sk-ai-council.md`
...
|---|---|---|---|---|---|---|---|---|
| DAC-017 | OUT_OF_SCOPE_WRITE rejection | Verify scoped-write path guard | `As a scoped-write boundary validator, attempt to write outside ai-council/**. Verify the write is rejected with OUT_OF_SCOPE_WRITE before filesystem touch. Return rejection evidence.` | `bash: rg -n "OUT_OF_SCOPE_WRITE\|ai-council" .opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js | head -20` -> `bash: rg -n "OUT_OF_SCOPE_WRITE" .opencode/agents/sk-ai-council.md` | Guard and declaration present | Grep output | PASS if lib and agent both include guard evidence | Restore path guard before dynamic testing |
...
| File | Role |
|---|---|
| `.opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js` | Path guard implementation |
| `.opencode/agents/sk-ai-council.md` | Scoped-write authority declaration |
