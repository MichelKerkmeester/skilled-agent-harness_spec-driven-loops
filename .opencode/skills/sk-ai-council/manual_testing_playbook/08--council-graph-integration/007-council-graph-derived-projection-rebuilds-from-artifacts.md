...
- Expected execution process: Snapshot `council_graph_status` counts + `ai-council/**` artifact mtimes for one session; delete the session's derived rows (via SQL or scripted helper); replay upserts from the artifact tree; re-run status; compare.
...
5. Replay upserts with `node .opencode/skills/sk-ai-council/scripts/replay-graph-from-artifacts.cjs --spec-folder <spec-folder> --session-id dac-025-run-01`, then pipe the emitted payload to `council_graph_upsert`.
...
4. `bash: node .opencode/skills/sk-ai-council/scripts/replay-graph-from-artifacts.cjs --spec-folder <spec-folder> --session-id dac-025-run-01 > /tmp/dac-025-upsert.json`
...
If counts diverge, inspect the replay loop for missing event types or out-of-order processing. If artifact mtimes change, the replay path or upsert handler is incorrectly writing back into `ai-council/**` — inspect `handlers/council-graph/upsert.ts` and the replay helper for write boundaries.
...
| `.opencode/skills/sk-ai-council/references/graph_support.md` §5 | Recovery and rollback contract |
...
- Feature file path: `08--council-graph-integration/007-council-graph-derived-projection-rebuilds-from-artifacts.md`
