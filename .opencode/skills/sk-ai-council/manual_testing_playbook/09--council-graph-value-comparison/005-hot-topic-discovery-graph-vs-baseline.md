...
description: "Real-world scenario for surfacing the most-contested claims/decisions in a multi-round council. Graph returns ranked hot nodes by edge-degree; baseline requires manual cross-referencing. Anchors to `council_graph_query mode='hot_nodes'`."
---
...
This scenario validates that `council_graph_query mode='hot_nodes'` materially beats the no-graph baseline for surfacing the most-contested or most-supported topics in a council session — useful for post-deliberation analysis, retrospective synthesis, and identifying where seat conflict concentrates.
...
> **Automated test anchor:** `mcp_server/tests/council-graph-value-scenarios.vitest.ts` test name `DAC-031 graph beats no-graph baseline`. Measured baseline-vs-graph ratios live in `mcp_server/tests/council-graph-value-report.json`.
...
- Prompt: `As a council-graph integration validator, identify the top 3 hot nodes in a seeded 4-round council session — first via the no-graph baseline (operator counts cross-references manually), then via council_graph_query with mode hot_nodes — and confirm the with-graph path returns a ranked list backed by edge-degree counts in one MCP call.`
- Expected execution process: Seed graph with known edge-degree distribution. Run baseline cross-reference tally. Run `council_graph_query mode='hot_nodes' limit:3`. Compare.
...
3. Run **with-graph workflow**: `council_graph_query mode='hot_nodes' limit:3`.
...
3. `tool: council_graph_upsert({ specFolder: 'sandbox/dac-031', sessionId: 'dac-031-run-01', nodes: [...5 claims, 2 decisions, 3 evidence...], edges: [...wire edge-degree per contract...] })`
4. `tool: council_graph_query({ specFolder: 'sandbox/dac-031', sessionId: 'dac-031-run-01', mode: 'hot_nodes', limit: 3 })`
...
| `../08--council-graph-integration/004-council-graph-query-five-modes-prompt-safe-context.md` | Functional anchor for `hot_nodes` mode |
...
| `.opencode/skills/system-spec-kit/mcp_server/lib/council-graph/council-graph-query.ts` | `getHotNodes` ranking helper |
| `.opencode/skills/sk-ai-council/references/graph_support.md` §3 | Documents edge kinds counted toward hotness |
...
- Feature file path: `09--council-graph-value-comparison/005-hot-topic-discovery-graph-vs-baseline.md`
