...
description: "Real-world scenario where a council hits max_rounds without convergence. Operator needs to know what blocked progress and what to fix first. Graph returns ranked blockers; baseline requires unranked artifact survey. Anchors to `council_graph_query mode='convergence_blockers'`."
---
...
This scenario validates that `council_graph_query mode='convergence_blockers'` materially beats the no-graph baseline for the post-stall triage task: "the council hit max_rounds without converging — what are the top blockers and in what order should I address them?"
...
> **Automated test anchor:** `mcp_server/tests/council-graph-value-scenarios.vitest.ts` test name `DAC-030 graph beats no-graph baseline`. Measured baseline-vs-graph ratios live in `mcp_server/tests/council-graph-value-report.json`.
...
- Prompt: `As a council-graph integration validator, triage a stalled 4-round council session — first via the no-graph baseline (read all critiques + disagreements), then via council_graph_query with mode convergence_blockers — and confirm the with-graph path returns a ranked blocker list naming the critical disagreement first.`
- Expected execution process: Seed graph with 3 known blockers as above. Run baseline survey workflow. Run `council_graph_query mode='convergence_blockers'`. Compare prioritization.
...
3. Run **with-graph workflow**: `council_graph_query mode='convergence_blockers' limit:5`.
...
4. `tool: council_graph_upsert({ specFolder: 'sandbox/dac-030', sessionId: 'dac-030-run-01', nodes: [...c5 low evidence, d1 critical, d2 medium, claims, decisions...], edges: [...CONTRADICTS, SUPPORTS, EVIDENCE_FOR...] })`
5. `tool: council_graph_query({ specFolder: 'sandbox/dac-030', sessionId: 'dac-030-run-01', mode: 'convergence_blockers', limit: 5 })`
...
If `d1` is not first, inspect `lib/council-graph/council-graph-query.ts` `getConvergenceBlockers` ranking — severity should dominate evidence-depth and centrality. If reason fields missing, inspect handler response shape.
...
| `../08--council-graph-integration/004-council-graph-query-five-modes-prompt-safe-context.md` | Functional anchor for `convergence_blockers` mode |
| `../04--convergence-and-rollback/002-max-rounds-without-convergence-emits-non-converged.md` | Baseline `convergence:false` behavior |
...
| `.opencode/skills/system-spec-kit/mcp_server/lib/council-graph/council-graph-query.ts` | `getConvergenceBlockers` helper |
| `.opencode/skills/sk-ai-council/references/convergence_signals.md` | Documents `max_rounds` escape hatch |
...
- Feature file path: `09--council-graph-value-comparison/004-stalled-council-blocker-ranking-graph-vs-baseline.md`
