...
`@sk-ai-council` is planning-only. It returns a council report and cannot write, edit, patch, or run shell commands. The dispatching parent owns persistence by invoking `.opencode/skills/sk-ai-council/scripts/persist-artifacts.cjs` after the report is captured.
...
A user or top-level assistant dispatches `@sk-ai-council`, stores the returned report as `council-report.md`, then invokes the helper.
...
`@orchestrate` dispatches `@sk-ai-council` as a LEAF. The LEAF returns report text only. `@orchestrate` then persists artifacts using the helper from its own write-capable context.
...
steps:
  ...
  - id: step_persist_council
    uses: shell.exec
    command: >
      node .opencode/skills/sk-ai-council/scripts/persist-artifacts.cjs
      "${packet.spec_folder}"
...
- Agent body §16: `.opencode/agents/sk-ai-council.md`
- Output schema: `.opencode/skills/sk-ai-council/references/output_schema.md`
- State format: `.opencode/skills/sk-ai-council/references/state_format.md`
- Persistence helper: `.opencode/skills/sk-ai-council/scripts/persist-artifacts.cjs`
