...
The shipped surface is anchored by `sk-ai-council`. The playbook scenario `07--writer-library-contract/001-library-writer-call-sequence.md` defines the operator prompt, command sequence, expected signals, evidence, and pass/fail criteria for DAC-013.

Current behavior is grounded in `.opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js`, which the scenario identifies as writer library and audit event implementation. Validation is anchored by `manual_testing_playbook/07--writer-library-contract/001-library-writer-call-sequence.md`, covering manual scenario contract.
...
| File | Layer | Role |
|------|-------|------|
| `.opencode/skills/sk-ai-council/scripts/lib/persist-artifacts.js` | Library | Writer library and audit event implementation |
| `.opencode/agents/sk-ai-council.md` | Runtime Mirror | Canonical writer sequence in invocation contract |
