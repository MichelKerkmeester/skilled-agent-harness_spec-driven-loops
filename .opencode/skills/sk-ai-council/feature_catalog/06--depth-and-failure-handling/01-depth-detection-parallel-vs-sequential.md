...
The shipped surface is anchored by `sk-ai-council`. The playbook scenario `06--depth-and-failure-handling/001-depth-detection-parallel-vs-sequential.md` defines the operator prompt, command sequence, expected signals, evidence, and pass/fail criteria for DAC-014.

Current behavior is grounded in `.opencode/agents/sk-ai-council.md`, which the scenario identifies as authoritative depth 0 and depth 1 source. Validation is anchored by `manual_testing_playbook/06--depth-and-failure-handling/001-depth-detection-parallel-vs-sequential.md`, covering manual scenario contract.
...
| File | Layer | Role |
|------|-------|------|
| `.opencode/agents/sk-ai-council.md` | Runtime Mirror | Authoritative Depth 0 and Depth 1 source |
| `.opencode/skills/sk-ai-council/references/depth_dispatch.md` | Reference | Reference mirror for operator use |
