...
The shipped surface is anchored by `council_graph_upsert`, `council_graph_query`, `sk-ai-council`. The playbook scenario `09--council-graph-value-comparison/005-hot-topic-discovery-graph-vs-baseline.md` defines the operator prompt, command sequence, expected signals, evidence, and pass/fail criteria for DAC-031.
...
| `.opencode/skills/system-spec-kit/mcp_server/lib/council-graph/council-graph-query.ts` | Library | `getHotNodes` ranking helper |
| `.opencode/skills/sk-ai-council/references/graph_support.md 3` | Reference | Documents edge kinds counted toward hotness |
...
| `mcp_server/tests/council-graph-value-scenarios.vitest.ts` | Automated test name: DAC-031 graph beats no-graph baseline |
