...
The shipped surface is anchored by `sk-ai-council`. The playbook scenario `05--scope-boundaries/001-graph-support-derived-and-scoped.md` defines the operator prompt, command sequence, expected signals, evidence, and pass/fail criteria for DAC-011.

Current behavior is grounded in `.opencode/skills/sk-ai-council/SKILL.md`, which the scenario identifies as skill scope rules. Validation is anchored by `manual_testing_playbook/05--scope-boundaries/001-graph-support-derived-and-scoped.md`, covering manual scenario contract.
...
| File | Layer | Role |
|------|-------|------|
| `.opencode/skills/sk-ai-council/SKILL.md` | Skill | Skill scope rules |
