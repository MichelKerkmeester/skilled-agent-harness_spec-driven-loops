...
description: Private procedure card for design-interface interaction states, feedback, transitions, and reduced-motion expectations.
trigger_phrases:
...
Private procedure card for applying the existing design-interface interaction-state workflow.

## 1. REQUIRED FIELDS

| Field | Value |
|---|---|
| Purpose | Let `design-interface` specify complete interaction states, feedback, transitions, and reduced-motion expectations. |
...
If the request is primarily an accessibility release claim, `interface`'s pre-delivery gate (`sk-design-interface/assets/interface-preflight-card.md`) owns the verdict while this card supplies the interaction-state standard.
