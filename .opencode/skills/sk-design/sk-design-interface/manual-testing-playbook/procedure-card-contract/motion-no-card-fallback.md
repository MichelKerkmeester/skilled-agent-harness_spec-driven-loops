...
```text
Advise whether this transition should be faster or slower, and state whether a private procedure card applies before answering.
```
...
- Prompt: `Advise whether this transition should be faster or slower, and state whether a private procedure card applies before answering.`
- Expected execution process: Read fallback line; confirm no card trigger; state exact fallback; answer using the baseline interface workflow.
...
- Feature file path: `procedure-card-contract/motion-no-card-fallback.md`
