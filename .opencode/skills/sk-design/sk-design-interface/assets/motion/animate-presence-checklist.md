...
Copy the section, mark each box and record the file and line for any fail. A checklist with an open fail is not ready to hand off. The reasoning and code for each rule lives in `references/motion/animate-presence-patterns.md`, so this checklist stays terse and points there when a fix is needed.
...
- [ ] Every fail has a recorded `file:line` and a fix from `references/motion/animate-presence-patterns.md`.

Report any open fail in the audit findings format from `references/motion/animate-presence-patterns.md` Section 7, for example `components/modal.tsx:42 - [exit-requires-wrapper] conditional motion.div is not wrapped`.
