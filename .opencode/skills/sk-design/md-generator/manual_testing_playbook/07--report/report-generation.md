...
PRE: Wave 1 (SETUP-001) must be PASS. The `backend/node_modules/` directory must exist and `npx playwright install chromium` must have completed. An existing `tokens.json` and `DESIGN.md` from a prior extraction are required; extract from `https://example.com --fast` first if they do not exist.
...
4. `cd .opencode/skills/sk-design/md-generator/backend && npx ts-node scripts/report-gen.ts <--output>/tokens.json <--output>/ <--output>/DESIGN.md`  # -> exits 0, writes report.html
5. `bash: ls -la <--output>/report.html`  # -> file exists, > 1 KB
6. `cd .opencode/skills/sk-design/md-generator/backend && npx ts-node scripts/preview-gen.ts <--output>/tokens.json <--output>/`  # -> exits 0, writes preview.html
7. `bash: ls -la <--output>/preview.html`  # -> file exists, > 1 KB
8. `cd .opencode/skills/sk-design/md-generator/backend && npx ts-node scripts/proof.ts https://example.com <--output>/tokens.json <--output>/`  # -> exits 0, writes proof.html + proof-data.json
...
| File | Role |
|---|---|
| `../../backend/scripts/report-gen.ts` | Token-to-section HTML report generator -- takes tokens.json as first arg, optional output dir and DESIGN.md |
| `../../backend/scripts/preview-gen.ts` | Visual CSS preview renderer -- takes tokens.json as first arg, optional output dir |
| `../../backend/scripts/proof.ts` | Fidelity proof generator -- takes URL as first arg, tokens.json as second arg, optional output dir and preview.html |
| `../../backend/scripts/validate.ts` | DESIGN.md validator -- consumed by report-gen to embed quality score and per-check results |
