...
```bash
npx ts-node .opencode/skills/sk-design/design-md-generator/backend/scripts/extract.ts https://example.com --fast --wait-for networkidle --output .opencode/specs/<track>/<packet>/output
```
