...
PRE: Waves 1 (SETUP-001 PASS) and 2 (EXTRACT-001 PASS) must be complete. A faithful v3 Style Reference must exist, written from the Wave 2 tokens.json per the v3 format specification in `references/design_md_format.md`. Load `assets/source_of_truth_router_card.md` as the sorting tool.
...
| File | Role |
|---|---|
| `../../assets/source_of_truth_router_card.md` | The fill-in card that sorts each value by origin and records the doubtful values |
| `../../references/authoring_boundary.md` | The four origins, the source-of-truth labels and the forward-authoring out-of-scope rule the card enforces |
...
| `../../references/design_md_format.md` | v3 Style Reference section specification and the cardinal rules the card protects |
