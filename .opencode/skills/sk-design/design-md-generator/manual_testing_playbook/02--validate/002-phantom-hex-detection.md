...
1. `cd .opencode/skills/sk-design/design-md-generator/backend && npx ts-node scripts/validate.ts <path-to-faithful-style-reference.md> <--output>/tokens.json`  # -> exit 0, values 100, claims >= 80, failures array empty
...
4. `npx ts-node .opencode/skills/sk-design/design-md-generator/backend/scripts/validate.ts /tmp/planted.md <--output>/tokens.json`  # -> exit non-zero or failures non-empty, at least one phantom-color finding for ff0000
...
6. check token naming against `references/quality_checklist.md`: primitive tokens may use raw values or scale positions such as `blue-600`, while semantic tokens must name roles such as `color-action`, `color-surface`, or `color-danger`
