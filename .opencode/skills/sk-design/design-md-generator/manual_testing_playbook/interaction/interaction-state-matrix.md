...
3. `npx ts-node .opencode/skills/sk-design/design-md-generator/backend/scripts/extract.ts https://example.com --fast --output .opencode/specs/<track>/<packet>/output`  # -> exits 0, crawl progress on stdout, interaction capture messages on stdout (capture is default-on; `--fast` still captures interaction)
...
| `../../feature_catalog/interaction-capture/interaction-capture.md` | Feature catalog entry for interaction-capture.ts |
