...
```
Render a visual HTML report and preview with a diff report from the tokens.json and DESIGN.md.
```
...
4. `cd .opencode/skills/sk-design/design-md-generator/backend && npx ts-node scripts/report-gen.ts <--output>/tokens.json <--output>/ <--output>/DESIGN.md`  # -> exits 0, writes report.html
5. `bash: ls -la <--output>/report.html`  # -> file exists, > 1 KB
6. `cd .opencode/skills/sk-design/design-md-generator/backend && npx ts-node scripts/preview-gen.ts <--output>/tokens.json <--output>/`  # -> exits 0, writes preview.html
7. `bash: ls -la <--output>/preview.html`  # -> file exists, > 1 KB
8. `cd .opencode/skills/sk-design/design-md-generator/backend && npx ts-node scripts/proof.ts https://example.com <--output>/tokens.json <--output>/`  # -> exits 0, writes proof.html + proof-data.json
...
- Feature file path: `report/report-generation.md`
