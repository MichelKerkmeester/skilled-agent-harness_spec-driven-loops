...
description: "The /design:diagram router - a thin router that loads a presentation contract, resolves :auto or :confirm mode, and executes the bound workflow YAML."
trigger_phrases:
...
# create-diagram command (/design:diagram)

<!-- sk-doc-template: skill_asset_feature_catalog -->

## 1. OVERVIEW

The `/design:diagram` router — a thin router that loads a presentation contract, resolves `:auto` or `:confirm` mode, and executes the bound workflow YAML.

The command is a thin router, not the workflow itself. It separates execution routing from user-facing presentation: the router loads the presentation contract, resolves the execution mode, and binds a workflow YAML, while every prompt, setup/status dashboard, and result display comes from the presentation contract. The caller is a user invoking `/design:diagram <target>.html [description] [--import source] [--type type] [:auto|:confirm]`, and the main failure mode is a missing owned asset, which stops the router with the missing path reported.
...
Related references:
- [hub-registration.md](hub-registration.md) — the hub registration that binds the `/design:diagram` command to this packet
