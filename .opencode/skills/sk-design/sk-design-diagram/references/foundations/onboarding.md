---
title: "Style Guide Onboarding"
description: "Agent-mediated workflow for generating a diagram skin from a website URL, an installed skill, or a local design folder."
trigger_phrases:
  - "onboard style guide"
  - "extract palette from url"
  - "customize diagram colors"
  - "extract tokens from skill"
  - "design tokens from folder"
...
**Goal:** point the skill at a design source — a website, an installed skill, or a local folder — and have it extract the palette + typography, then rewrite `style-guide.md` so every future diagram inherits that skin. Takes about 60 seconds.

Three source methods are supported. Jump to the relevant section:

- [§2 URL](#2-url-source) — a live website the user provides
- [§3 Skill](#3-skill-source) — an installed Agent Skill that carries design tokens
- [§4 Folder](#4-folder-source) — a local design-system directory (CSS, JSON, Markdown)
...
Source you provide (URL / skill name / folder path)
      ↓
[1] read the source — for a URL, ask the user for it; any live network
    fetch is performed by the calling AI session's own tools, because
    this packet's tool surface has no network-fetch tool
      ↓
...
> *"Onboard the diagram skill to my site — `https://example.com`"*

### Step 1 — obtain the page

Ask the user for the site URL. Fetching the page is performed by the calling AI session's own tools — this packet's tool surface declares no network-fetch tool. If the site has multiple pages worth sampling (landing + blog + product), fetch 2–3 and merge the palette signals.
...
1. Open the gallery and confirm the new palette feels coherent across all 27 types.
...
> *"Onboard the diagram skill from my `acme-design` skill"*
...
> *"Onboard the diagram skill from my design system at `~/projects/brand/design-tokens/`"*
...
Read every discovered file. Apply the same extraction logic as the Skill method (§3 → Step 3). CSS custom properties and JSON tokens take priority over inferred values from prose.
