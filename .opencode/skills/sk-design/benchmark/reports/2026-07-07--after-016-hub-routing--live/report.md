...
> **Scope note:** PASS is computed over the four weighted, scored dimensions (D1intra, D2, D3, D5); D1-inter and D4 remain unscored this run (see Methodology below). It does **not** certify the browser-class scenarios — see the P1 `funnel_attrition` bottleneck under Ranked bottlenecks: 5 of 6 browser scenarios first-fail at the `browser` stage because the harness has no per-URL export/motion/video probe for 3 scenario types (`PARTIAL-NEEDS-ARTIFACT`) and no recipe at all for the other 2 (`SKIP-NO-BROWSER`). That is a benchmark-harness instrumentation gap, not an observed routing/discovery defect in the skill; treat it as an open item tracked via the bottlenecks table, not resolved by this headline.
...
| -------- | ----- | ----- | ------- |
| P1 | funnel_attrition | browser | 5 scenario(s) first fail at stage 'browser' |
