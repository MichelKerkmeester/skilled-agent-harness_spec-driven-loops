...
`design-md-generator` has one private procedure card selected after the public mode is chosen: `design-system-extraction.md`.
...
The card grants no additional permission beyond the mode's existing extraction pipeline; it does not grant Write, Edit, or Bash to the read-only sibling `sk-design` mode. When no measurable source exists and the user wants values decided from a brief instead, the card routes to the sibling authoring skill `sk-design`.
...
- Feature file path: `procedure-cards/md-generator-procedure-card-inventory.md`

Related references:
- [../extract/extract.md](../extract/extract.md) - Extraction pipeline the card's tool boundary is scoped to.
- [../validate/validate.md](../validate/validate.md) - Validation gate the card's output must still pass.
