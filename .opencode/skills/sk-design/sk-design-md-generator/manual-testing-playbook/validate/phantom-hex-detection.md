...
1. `cd .opencode/skills/sk-design/sk-design-md-generator/backend && npx ts-node scripts/validate.ts <path-to-faithful-style-reference.md> <--output>/tokens.json`  # -> exit 0, values 100, claims >= 80, failures array empty
...
4. `npx ts-node .opencode/skills/sk-design/sk-design-md-generator/backend/scripts/validate.ts /tmp/planted.md <--output>/tokens.json`  # -> exit non-zero or failures non-empty, at least one phantom-color finding for ff0000
