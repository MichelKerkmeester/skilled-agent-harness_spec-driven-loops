...
- **foundations** owns static-system nouns: color, type, spacing, grid, layout rhythm, visual hierarchy, information hierarchy, responsive adaptation, and design-token system design. `tokens.json` and `DESIGN.md` as measured artifacts route to `md-generator`, not foundations, unless the prompt asks to design or adapt a token system.
