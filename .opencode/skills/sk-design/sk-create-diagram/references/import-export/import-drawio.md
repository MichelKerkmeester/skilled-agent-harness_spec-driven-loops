...
description: "Redraw a .drawio source (raw XML, deflate+base64, PNG/SVG-embedded mxfile) into an editorial-quality diagram via the drawio_extract.py IR."
trigger_phrases:
  - "import drawio diagram"
  - "convert drawio file"
  - "redraw existing diagram"
  - "drawio extract intermediate representation"
  - "drawio png svg embedded mxfile"
...
Turn a `.drawio` file into an editorial-quality diagram at the format, size, and detail level the destination needs. This is a redraw, not a conversion: you read the source for its *content* — components, relationships, grouping, direction — and then draw a new diagram in this skill's design system.
...
`<skill-dir>` is `.opencode/skills/sk-design/sk-create-diagram/` in this repo, or the skill's own directory when it's installed standalone or as a plugin. If the path isn't obvious, glob for `**/sk-create-diagram/scripts/drawio_extract.py`.
...
|---|---|---|
| `lifeline` shapes, tall vertical bars | Sequence | [type-sequence.md](../types/type-sequence.md) |
| `table` / `er` shapes, rows of fields | ER / data model | [type-er.md](../types/type-er.md) |
| ≥2 aligned `swimlane` containers (`type candidates: swimlane`) | Swimlane | [type-swimlane.md](../types/type-swimlane.md) |
| `rhombus` present, single entry point, labeled yes/no edges | Flowchart | [type-flowchart.md](../types/type-flowchart.md) |
| Mostly `ellipse`, self-loops, `has_cycle: True` | State machine | [type-state.md](../types/type-state.md) |
| `icon:aws` / `icon:azure` / `icon:gcp` / `icon:kubernetes` families | Architecture | [type-architecture.md](../types/type-architecture.md) |
| Nested containers, depth ≥2, few edges | Nested | [type-nested.md](../types/type-nested.md) |
| One entry point, no cycle, fan-out only | Tree or Org chart | [type-tree.md](../types/type-tree.md), [type-org-chart.md](../types/type-org-chart.md) |
| Boxes stacked vertically, edges only between neighbours | Layer stack | [type-layers.md](../types/type-layers.md) |
| Dated labels on a single axis | Timeline or Gantt | [type-timeline.md](../types/type-timeline.md), [type-gantt.md](../types/type-gantt.md) |
| Anything else with edges | Architecture | [type-architecture.md](../types/type-architecture.md) |
...
2. **Apply the detail level.** Walk [`output-spec.md` §3](../foundations/output-spec.md) degrade ladder until you're under the node ceiling. The digest's *collapsible groups* section is step 3 of that ladder, pre-computed.
...
| `actor` | Input/User treatment, or the user icon from [primitive-icons.md](../primitives/primitive-icons.md) |
| `cloud` | External/Cloud treatment |
| `note` | Annotation callout ([primitive-annotation.md](../primitives/primitive-annotation.md)), max 2 — or drop |
| `icon:aws` / `icon:azure` / `icon:gcp` / `icon:kubernetes` | The matching monochrome icon from [primitive-icons.md](../primitives/primitive-icons.md), inheriting `currentColor` |
