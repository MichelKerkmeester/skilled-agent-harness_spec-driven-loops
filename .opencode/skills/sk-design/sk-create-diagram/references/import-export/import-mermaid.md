...
description: "Redraw .mmd, .mermaid, or fenced Mermaid blocks into an editorial-quality diagram via the mermaid_extract.py IR."
trigger_phrases:
  - "import mermaid diagram"
  - "convert mermaid source"
  - "redraw mermaid flowchart"
  - "mermaid extract intermediate representation"
  - "mermaid fenced block markdown"
  - "simplify mermaid diagram"
...
Turn Mermaid source into an editorial-quality diagram at the format, size, and detail level the destination needs. This is a redraw, not a render or conversion: Mermaid supplies content and declared direction, not coordinates — discard its computed renderer layout, theme, classes, and shape styling, and create a fresh layout in this skill's design system.
...
`<skill-dir>` is `.opencode/skills/sk-design/sk-create-diagram/` in this repo, or the skill's own directory when installed standalone or as a plugin. If the path isn't obvious, glob for `**/sk-create-diagram/scripts/mermaid_extract.py`.
...
|---|---|---|
| `flowchart`, decision rhombus, labeled branches | Flowchart | [type-flowchart.md](../types/type-flowchart.md) |
| `flowchart` with service/container topology and no decisions | Architecture | [type-architecture.md](../types/type-architecture.md) |
| `sequenceDiagram` | Sequence | [type-sequence.md](../types/type-sequence.md) |
| `stateDiagram-v2` | State machine | [type-state.md](../types/type-state.md) |
| `erDiagram` | ER / data model | [type-er.md](../types/type-er.md) |
| Nested subgraphs, depth ≥2, few edges | Nested | [type-nested.md](../types/type-nested.md) |
