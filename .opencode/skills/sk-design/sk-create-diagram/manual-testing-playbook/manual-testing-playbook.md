...
1. Working directory is the repository root, so `.opencode/skills/sk-design/sk-create-diagram/` subpaths and the `.opencode/commands/create/` assets resolve.
...
4. Generated artifacts (`.html`, `.svg`, `.png`) are written to a scratch or docs output directory outside the packet. The one documented in-package mutation is `references/foundations/style-guide.md` during the onboarding scenario (DIA-003), which runs on a scratch checkout or is reverted with `git checkout -- .opencode/skills/sk-design/sk-create-diagram/references/foundations/style-guide.md`.
...
- Repo-relative paths are written as `.opencode/skills/sk-design/sk-create-diagram/...`; `<skill-dir>` means the packet root wherever the packet is installed.
