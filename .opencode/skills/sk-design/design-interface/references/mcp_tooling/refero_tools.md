...
Refero is a large library of real shipped UI, organized into three layers: **styles** (visual direction from marketing/product pages), **screens** (concrete in-app UI patterns), and **flows** (multi-step journeys). Inside interface these are a CRITIQUE-AGAINST reference; the discipline (one reference, read live, never copy) lives in `design_references_mcp.md`, and this file is the tool catalog.
