...
**Required load-and-prove loop:** `../shared/register.md`, `references/design_process/brief_to_dials.md`, and `assets/interface_preflight_card.md` are not optional for interface work; load the first two before decisions and prove the third before delivery.
...
- `references/design_process/ux_quality_reference.md` is the objective quality-floor gate; apply it after the direction is set.
...
- When the Mobbin or Refero subscriptions are connected, they are an OPTIONAL real-world critique-against reference (via Code Mode) for naming the category default to deviate from, never a chooser and never copied. `references/design_grounding/design_references_mcp.md` owns the rules and `design_principles.md` stays the authority.
