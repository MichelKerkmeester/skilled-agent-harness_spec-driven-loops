...
Drag, swipe and pointer-driven motion. This card records the spec only. The behavior rules for thresholds, velocity, damping and accessible alternatives live in `references/motion/micro-interactions.md` Section 4, so cite them here rather than restating them.
...
| Behavior rules source | `references/motion/micro-interactions.md` Section 4 |
...
| Timing and easing | `__________` (cite `references/motion/motion-strategy.md`; no custom values) |
