...
`audit` is a permanent command subworkflow owned by the registered `interface` mode. It reviews built or planned interfaces for accessibility, performance, responsive behavior, theming, anti-patterns, design quality, and production hardening. `/interface:audit` invokes this contract directly without creating a nested skill identity or a peer registry mode.
...
This is an independently invokable command workflow within `design-interface`. It owns review, scoring, severity, risk surfacing, and hardening. It may cite sibling standards but does not replace them.
...
DESIGN QA TASK
    |
    +- Full audit / score / release readiness -> ../references/audit/audit-contract.md
    +- A11y or motion/load perf -> ../references/audit/accessibility-performance.md
    +- Critique / cognitive load / persona / polish -> ../references/audit/critique-hardening.md
    +- Edge cases / copy / pseudo-elements / production -> ../references/audit/anti-patterns-production.md
```
...
- `../references/audit/audit-contract.md` defines the P0-P3 severity model, 5-dimension `/20` scoring contract, output format, and evidence rules.
- `../references/audit/accessibility-performance.md` covers accessible names, keyboard, focus, semantics, forms, announcements, contrast, motion performance, Core Web Vitals, and measurement.
- `../references/audit/critique-hardening.md` covers critique workflow, cognitive load, Nielsen heuristics, personas, polish, edge cases, i18n, error states, and resilience.
- `../references/audit/anti-patterns-production.md` covers slop detection, theming drift, token misuse, pseudo-elements, View Transitions, copy clarity, and production hardening.
- `../assets/audit/ai-fingerprint-registry.json` mirrors the model-specific tell catalog as one machine-checkable row per tell.
- `../assets/audit/ai-fingerprint-self-defect-card.md` carries one self-audit prompt per registry row.
...
- `../references/audit/corpus-map.md` maps the source corpus.
...
| --- | --- | --- |
| ALWAYS | Any audit or critique | `../references/audit/corpus-map.md`, `../references/audit/audit-contract.md` |
...
| ALWAYS | Any audit or design-readiness task | `../../shared/context-loading-contract.md` (register-first gate, context manifest, the four proof fields including audit evidence, and the hard gates that block release/accessibility claims) |
| CONDITIONAL | Accessibility or performance concern | `../references/audit/accessibility-performance.md` |
| CONDITIONAL | Holistic critique, UX score, persona, polish, hardening | `../references/audit/critique-hardening.md` |
| CONDITIONAL | Slop, theming, pseudo-elements, copy clarity, production details | `../references/audit/anti-patterns-production.md` |
| CONDITIONAL | Detecting model-specific AI tells | `../references/audit/ai-fingerprint-tells.md`, `../assets/audit/ai-fingerprint-registry.json`, `../assets/audit/ai-fingerprint-self-defect-card.md` |
| CONDITIONAL | Routing a bolder, quieter or simpler request | `../references/audit/transform-remediation.md` |
| CONDITIONAL | Resolving the target and labeling evidence | `../references/audit/evidence-capture.md` |
| CONDITIONAL | Carrying confirmed, inferred and not-assessed labels into findings and scores | `../assets/audit/audit-evidence-worksheet.md` |
| CONDITIONAL | Calibrating the Anti-Patterns dimension | `../assets/audit/anti-patterns-score-rubric.md` |
| CONDITIONAL | Production-readiness and edge-case probes | `../references/audit/hardening-edge-cases.md` |
| CONDITIONAL | Producing the audit report | `../assets/audit/audit-report-template.md` |
...
| CONDITIONAL | Citing the snippet-level accessibility fix | `../assets/audit/a11y-quick-fixes.md` |
| CONDITIONAL | Routing accepted findings to sk-code | `../../shared/sk-code-handoff.md` |
...
The private procedure-card selection table in Section 3 is part of this routing contract: after `/interface:audit` selects this subworkflow, choose at most one card from `procedures/` or `../../shared/procedures/` and cite its relative path in the plan or proof line.
...
The full implementation lives in [`smart-router-pseudocode.md`](../references/audit/smart-router-pseudocode.md). Read it before implementing or modifying routing logic; this section is the contract, that file is the code.
...
1. Resolve the target and the register: source files, URL, screenshot, design plan or rendered UI evidence, then read `../../shared/register.md` so the audit-severity dial weights findings by Brand-vs-Product posture. `../references/audit/evidence-capture.md` owns target resolution.
   **Ready-claim gate:** before any ready, release, or accessibility claim, complete the five-dimension score with evidence labels (`confirmed`, `inferred`, `not-assessed`) per `../references/audit/audit-contract.md`; reuse `../assets/audit/audit-evidence-worksheet.md` to carry those labels into findings and scores.
...
3. Score the five audit dimensions 0-4 each:
   ...
   - Responsive Design.
   - Theming.
   - Anti-Patterns. Detect model-specific tells with `../references/audit/ai-fingerprint-tells.md` and probe production readiness with `../references/audit/hardening-edge-cases.md`.
4. Produce a findings-first report ordered by P0, P1, P2, P3. Use `../assets/audit/audit-report-template.md` for the fill-in skeleton.
5. Map each finding to the owning sibling command or implementation skill. Route a bolder, quieter, distill or redesign direction through `../references/audit/transform-remediation.md` first, because the correct direction depends on the register.
6. End with recommended next actions; do not silently implement fixes during a review-only request.

### Procedure Card Selection

After the command router selects the audit subworkflow, choose at most one primary private procedure card and cite it by relative path in the plan or proof line. These cards support audit focus; they are not public routes.
...
Record the context basis before findings: owner mode `interface`, subworkflow `audit`, loaded references, selected procedure card or no-procedure fallback, target artifact, available evidence, missing evidence, audit bar, and whether the score is full or focused. Before a ready, release, or accessibility claim, include proof naming the selected procedure card, evidence labels, dimensions covered, and what remains not assessed.

This subworkflow must run directly with Read, Glob, and Grep only. If subagents are unavailable or disallowed, do not dispatch; execute the same procedure selection, context capture, and proof checks in the current session. The fallback keeps the same proof bar and cannot rely on Write, Edit, Bash, or Task. The deterministic report scripts need Bash and therefore run only downstream through a human reviewer, CI job, or Bash-capable implementation workflow.
...
The audit subworkflow remains Read/Glob/Grep-only. Execution belongs to a Bash-capable
...
typed comparison envelope, and returns the validated JSON result to this subworkflow.
...
- [`audit-contract.md`](../references/audit/audit-contract.md) - Severity, `/20` score, findings schema, and audit output contract.
- [`accessibility-performance.md`](../references/audit/accessibility-performance.md) - Accessibility and performance checks.
- [`critique-hardening.md`](../references/audit/critique-hardening.md) - Holistic critique, cognitive load, personas, polish, and hardening.
- [`anti-patterns-production.md`](../references/audit/anti-patterns-production.md) - Anti-slop, theming, pseudo-elements, copy clarity, and production details.
...
- [`transform-remediation.md`](../references/audit/transform-remediation.md) - Register-gated directional verbs mapped to findings and owners.
- [`evidence-capture.md`](../references/audit/evidence-capture.md) - Target resolution, browser and deterministic evidence, screenshot and overlay notes, fallback labels.
- [`hardening-edge-cases.md`](../references/audit/hardening-edge-cases.md) - Production-readiness matrix of extreme inputs, errors, permissions, concurrency, i18n and RTL, text expansion, CJK and emoji.
- [`corpus-map.md`](../references/audit/corpus-map.md) - Source traceability for the distilled corpus.
- [`smart-router-pseudocode.md`](../references/audit/smart-router-pseudocode.md) - Implementation behind Section 2's smart router.
...
- [`audit-report-template.md`](../assets/audit/audit-report-template.md) - Fill-in findings-first report with the five-dimension `/20` score, anti-pattern verdict, owner mapping, and evidence caveats.
- [`a11y-quick-fixes.md`](../assets/audit/a11y-quick-fixes.md) - Snippet-level accessibility fixes the report cites by reference; the audit names the fix, `sk-code` applies it.
- [`audit-evidence-worksheet.md`](../assets/audit/audit-evidence-worksheet.md) - Fill-in worksheet that carries confirmed, inferred and not-assessed labels into findings and scores.
- [`anti-patterns-score-rubric.md`](../assets/audit/anti-patterns-score-rubric.md) - Anti-Patterns 0 to 4 calibration ladder for full audits.
...
- The `interface` mode and `/interface:audit` route audit, critique, and hardening prompts here.
