...
- UNKNOWN. The md-generator router guards paths inside `sk-design-md-generator/`.
...
1. `sk-design-md-generator/backend/node_modules` and Playwright Chromium are installed if the operator executes the full run.
...
- **PASS** iff advisor top-1 is `sk-design`, mode is `md-generator`, packet is `sk-design-md-generator/SKILL.md`, the response names EXTRACT, WRITE, and VALIDATE in order, and any writes are confined to `/tmp/skd-MG001/`.
...
2. If stage names are missing, inspect `sk-design-md-generator/SKILL.md` `The Three-Phase Pipeline`.
