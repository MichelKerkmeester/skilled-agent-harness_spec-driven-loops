...
description: "Verify a code-correctness review request using review/audit-adjacent wording still routes to sk-code's code-review mode, not sk-design."
version: 1.0.0.0
id: AI-004
...
3. Record top-1 skill, score, and whether `sk-design` (or `design-interface`) appeared in top-3.

### Pass/Fail Criteria

- **PASS** iff top-1 is `sk-code` (routed to its code-review mode), and `sk-design` is not top-1 at confidence `>= 0.80`.
- **FAIL** iff `sk-design` wins, `design-interface` loads a packet or procedure card, or the review-vocabulary overlap on "review" pulls the request into the design family.
