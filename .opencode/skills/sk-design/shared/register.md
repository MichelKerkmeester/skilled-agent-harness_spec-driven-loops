...
Set the register at the start of any design task, before the mode-specific workflow runs. Each mode (interface, md-generator) reads the register and applies the dial settings for the chosen posture. The register is shared vocabulary, not an additional mode and not a replacement for mode workflows.
