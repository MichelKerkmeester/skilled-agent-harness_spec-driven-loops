---
title: Layered Body
description: Structural fingerprint that deepens evidence through distinct passes rather than repeating equal panels.
trigger_phrases:
  - "layered body structure"
  - "progressive evidence composition"
...
- **Divider language:** mark changes in evidence depth, not every section boundary.
- **Button voice:** delay consequential action until the body has supplied the information needed to choose; local actions remain contextual.
...
**Reach for it when:** readers need progressively stronger evidence, the page contains genuinely different content jobs, and a linear argument improves confidence.
...
- Equal panels are relabeled as layers even though their depth and job never change.
...
Visible proof identifies each body's distinct job and shows how specificity increases across the sequence. Record `cardId: "layered-body"` in the shared `structuralFingerprintSelections` envelope with the body-composition emphasis, applied regions, responsive-gate result, and evidence that the closing action is earned by prior content.
