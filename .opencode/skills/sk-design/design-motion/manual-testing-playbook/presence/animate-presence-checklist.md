...
1. Load `assets/animate-presence-checklist.md` and walk every section in order: exit wiring, keys, first render, mode, presence hooks, nested exits.
2. Mark each box pass or fail and record a `file:line` for any fail.
3. Point each fail to its fix in `references/animate-presence-patterns.md` rather than restating the reasoning inline.
