...
Decide whether an interaction earns motion before any timing, easing or material is chosen. This reference owns the restraint gate. Timing, easing and material live in `motion-strategy.md`, and this gate runs before them. If the gate says do not animate, no later choice matters.
...
A motion choice that survives all four steps is worth choreographing. Hand it to `motion-strategy.md` for timing, easing and material. A choice that fails any step ships as an instant state change, which is the right outcome, not a downgrade.
