...
Nine more enforce clauses the contract states outside its numbered table. Six run per file. The last two do not: `pointer-contract-coverage` compares the whole template directory against the contract in one pass, while `card-readout` and `pointer-reach` run only under `--render`, and only on the files that carry a card. Each of the six existed as a written rule before it existed as a check, and a rule the tooling does not check is a wish:
...
- `legend` requires every multi-series form to render one keyed HTML legend per declared series, with the series name as its label and one swatch carrying the matching palette token. A plot can carry the right colours and still leave a reader guessing which line or bar is which; the legend is the contract's named bridge from series data to the picture.
- `tooltip-card` requires every tooltip-bearing form to keep the readout in a positioned HTML card, with a per-series indicator, label and value read from `READOUT`. SVG text is rejected for the card because it cannot provide the card's padding, boundary, narrow-viewport clamp and pointer-safe presentation.
...
- `pointer-reach` also runs under `--render` only, and it is the rule that asks whether a pointer can reach a mark at all rather than whether a mark exists. Several forms draw marks a few pixels across, and a mark too small to aim at is unreachable however correct its card is. The rule walks an 11 by 11 grid across each drawing, dispatches a real `pointermove` at each position, and requires two things: that a position inside the plot answers with a card, and that the card belongs to the mark nearest the aim rather than a neighbour. Answering with the wrong mark fails outright, because a reading attributed to the wrong point is worse than no reading. Dead space fails only past a tenth of the grid, because an honest chart has corners its marks do not reach and demanding every pixel would fail it. Like `card-readout` it works on a temporary instrumented copy and never touches a corpus file.
- Both `--render` rules force every animation to its end state before reading any geometry. These transforms are compositor-driven, so the painted picture reaches its end state while the main thread still reports the pre-animation box: `getBoundingClientRect()` returns a zero height and `getAnimations()` reports the animation unfinished, right through the end of the virtual-time budget. The screenshot is right and the geometry API is wrong. A probe written without `document.getAnimations().forEach(a => a.finish())` measures the animation instead of the chart, and one written here did exactly that.
