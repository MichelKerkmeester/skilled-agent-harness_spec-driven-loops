...
**Expected procedure card**: `design-md-generator/procedures/design_system_extraction.md`.
...
- `design_system_extraction.md` requires measured source evidence, visible gaps, and source-traceable values rather than silently filled design prose.
...
- `design-md-generator/procedures/design_system_extraction.md`
...
2. `design-md-generator/procedures/design_system_extraction.md` exists and requires source-traceable extraction evidence.
...
- **PASS** iff advisor top-1 is `sk-design`, resolved mode is `md-generator`, the response names `design-md-generator/procedures/design_system_extraction.md`, it confirms only `md-generator` may write extraction artifacts, and live execution either writes sandboxed `/tmp/skd-PB003/` outputs or explicitly records that operator execution is required.
...
2. If measured values are fabricated, inspect `design_system_extraction.md` and `source_of_truth_router_card.md`.
