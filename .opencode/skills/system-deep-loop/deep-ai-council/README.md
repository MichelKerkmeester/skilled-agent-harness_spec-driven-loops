...
The council shares the `runtime/` with the other active deep-loop families. The current family and lane roster is authoritative in [`mode-registry.json`](../mode-registry.json); each entry owns a different phase of the planning-to-implementation pipeline. Use `@context` or `/speckit:plan` for codebase context before council work.
...
| `deep-improvement` | Runs evaluator-first improvement across the `agent-improvement` and `model-benchmark` command lanes. |
