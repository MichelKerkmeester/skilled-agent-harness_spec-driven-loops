...
**Rationale.** The most realistic cell — a natural-language ask that says "our standards" but names no authority, no artifact-class, and no scope. An alignment lane is a `(authority, artifactClass, scope)` tuple (`references/scoping_protocol.md` §2), and none of the three axes is resolvable here. `SKILL.md` ALWAYS #1 forbids guessing a scope, so the correct behavior is a targeted lane-resolution question that pins the missing axes — above all *whose* standards ("our standards" is not an authority the registry knows), never a guess.
