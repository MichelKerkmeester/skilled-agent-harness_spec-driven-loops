...
- Expected execution process: Build a fixture, drive it through a coverage-met-but-unstable state (expect CONTINUE), then to a coverage-met-and-stable state (expect CONVERGED), reading the `converged = coverageMet && stability.stable` line and `state-machine-wiring.md` §4.
...
| `.opencode/skills/system-deep-loop/deep-alignment/references/state-machine-wiring.md` | §4 "AND, not OR" formula and rationale |
