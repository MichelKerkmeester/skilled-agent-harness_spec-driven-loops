...
### Commands
1. `bash: node -e "for(const n of ['sk-doc','sk-git','sk-code']){const m=require('./.opencode/skills/system-deep-loop/deep-alignment/scripts/adapters/'+n+'.cjs'); const list=m.loadKnownDeviations(); console.log(n, 'deviations='+list.length, list.map(d=>d.id+':'+(d.status||'?')).join(', '));}"`
