...
- Expected execution process: Require each of the five adapter modules and assert the three exports are functions; inspect each `discover`'s arity and node shape; read `discover-contract.md` §2 (single-parameter signature) and §6 (extensibility guarantee).
...
### Commands
1. `bash: node -e "for(const n of ['sk-doc','sk-doc-command','sk-git','sk-code']){const m=require('./.opencode/skills/system-deep-loop/deep-alignment/scripts/adapters/'+n+'.cjs'); console.log(n, ['discover','standardSource','check'].map(k=>typeof m[k]).join('/'), 'discover.arity='+m.discover.length);}"`
...
### Failure Triage
If any adapter reports `discover.arity` > 1, it has widened the signature (an ADR-003 violation). If a seed node's `kind` is not `FILE`, cross-check against `discover-contract.md` §4.2's NodeKind requirement.
---
...
| `.opencode/skills/system-deep-loop/deep-alignment/references/discover-contract.md` | §2 single-parameter signature; §4 FILE seed-node shape; §6 extensibility guarantee |
