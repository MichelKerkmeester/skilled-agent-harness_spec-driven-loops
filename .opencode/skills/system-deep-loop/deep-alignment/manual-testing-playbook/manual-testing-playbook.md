...
- Four registered adapters exist under `.opencode/skills/system-deep-loop/deep-alignment/scripts/adapters/`: `sk-doc.cjs`, `sk-doc-command.cjs`, `sk-git.cjs`, `sk-code.cjs`.
...
- **Not yet built (out of scope, see BUILD-STATE NOTE):** `/deep:alignment` command, `deep-alignment-auto.yaml` / `deep-alignment-confirm.yaml`, `@deep-alignment` agent. (The mode `README.md` is now built and is a precondition above, not a gap.)
...
2. Referenced per-feature files under `manual-testing-playbook/<category-name>/`
...
#### Test Execution
> **Feature File:** [DAL-001](../manual-testing-playbook/entry-points-and-modes/invocation-contract-and-forbidden-patterns.md)
...
#### Test Execution
> **Feature File:** [DAL-002](../manual-testing-playbook/entry-points-and-modes/parameter-surface-modes-and-tuning.md)
...
Expected signals: `scoping.cjs main()` requires `--lane-config` and exits with an input-validation error (exit `3`) and an explicit message pointing at the interactive fallback when the flag is absent; `scoping-protocol.md` §6 and `lane-config-schema.md` §1 lock the config-file-only rule; `resolveLanesFromSelections()` is the interactive path the invoking command calls, not a readline loop.

#### Test Execution
> **Feature File:** [DAL-003](../manual-testing-playbook/entry-points-and-modes/config-file-only-non-interactive-path.md)

---

## 8. LANE RESOLUTION AND SCOPING

This category covers 6 scenario summaries while the linked feature files remain the canonical execution contract. All anchor to `scripts/scoping.cjs` and the `scoping-protocol.md` / `lane-config-schema.md` references.
...
Expected signals: Equivalent input through both entrypoints yields identical `{authority, artifactClass, scope}` tuples; both call `validateLane` -> `validateScope`; `scoping-protocol.md` §4 states "one shape, two producers".

#### Test Execution
> **Feature File:** [DAL-004](../manual-testing-playbook/lane-resolution-and-scoping/dual-path-identical-lane-shape.md)
...
#### Test Execution
> **Feature File:** [DAL-005](../manual-testing-playbook/lane-resolution-and-scoping/authority-artifact-class-registry.md)
...
#### Test Execution
> **Feature File:** [DAL-006](../manual-testing-playbook/lane-resolution-and-scoping/scope-shape-and-repo-root-validation.md)
...
#### Test Execution
> **Feature File:** [DAL-007](../manual-testing-playbook/lane-resolution-and-scoping/empty-lane-config-zero-lanes.md)
...
#### Test Execution
> **Feature File:** [DAL-008](../manual-testing-playbook/lane-resolution-and-scoping/fail-closed-error-contract.md)

### DAL-009 | Multi-authority single run

#### Description
Verify that one run resolves N lanes (not a cross-product) — the "sk-code and sk-git and/or sk-doc in one pass" precedent resolves to exactly the named combinations, with no hard-coded lane ceiling.
...
#### Test Execution
> **Feature File:** [DAL-009](../manual-testing-playbook/lane-resolution-and-scoping/multi-authority-single-run.md)
...
#### Test Execution
> **Feature File:** [DAL-010](../manual-testing-playbook/discovery-and-adapters/sk-doc-adapter.md)
...
#### Test Execution
> **Feature File:** [DAL-011](../manual-testing-playbook/discovery-and-adapters/sk-git-adapter.md)
...
Expected signals: all five adapter modules export `discover`, `standardSource`, `check`; `discover(scope)` is single-parameter (no authority arg); each `discover` returns `{artifacts, nodes}` with `kind:'FILE'` nodes carrying `authority`/`artifactClass` metadata; `discover-contract.md` §6 states a fifth authority needs no change to the contract, `scoping.cjs`, or the loop.

#### Test Execution
> **Feature File:** [DAL-015](../manual-testing-playbook/discovery-and-adapters/authority-agnostic-adapter-contract.md)
...
#### Test Execution
> **Feature File:** [DAL-017](../manual-testing-playbook/verify-first-and-known-deviations/known-deviation-suppression.md)
...
#### Test Execution
> **Feature File:** [DAL-018](../manual-testing-playbook/verify-first-and-known-deviations/sk-git-exempt-precheck-vs-suppression.md)
...
#### Test Execution
> **Feature File:** [DAL-019](../manual-testing-playbook/iteration-and-convergence/corpus-partitioning-round-robin.md)
...
Expected signals: `checkConvergence` computes `converged = coverageMet && stability.stable`; a run with 100% coverage but a recent new-finding iteration returns `CONTINUE`; a stable-but-incompletely-covered run returns `CONTINUE`; `state-machine-wiring.md` §4 states "AND, not OR" with the rationale.

#### Test Execution
> **Feature File:** [DAL-020](../manual-testing-playbook/iteration-and-convergence/coverage-and-stability-and-semantics.md)
...
#### Test Execution
> **Feature File:** [DAL-021](../manual-testing-playbook/iteration-and-convergence/max-iterations-hard-stop.md)
...
#### Test Execution
> **Feature File:** [DAL-022](../manual-testing-playbook/iteration-and-convergence/dry-run-stability-fail-closed.md)
...
#### Test Execution
> **Feature File:** [DAL-023](../manual-testing-playbook/iteration-and-convergence/nothing-to-converge-and-vacuous-lane.md)
...
#### Test Execution
> **Feature File:** [DAL-024](../manual-testing-playbook/read-only-and-gated-remediation/read-only-default-surface.md)
...
#### Test Execution
> **Feature File:** [DAL-025](../manual-testing-playbook/read-only-and-gated-remediation/gated-remediation-hook-noop.md)
...
#### Test Execution
> **Feature File:** [DAL-026](../manual-testing-playbook/report-emission-per-lane/one-report-per-lane.md)
...
#### Test Execution
> **Feature File:** [DAL-027](../manual-testing-playbook/report-emission-per-lane/worst-verdict-overall-rollup.md)
...
#### Test Execution
> **Feature File:** [DAL-028](../manual-testing-playbook/report-emission-per-lane/finding-dedup-and-fail-closed-severity.md)
...
#### Test Execution
> **Feature File:** [DAL-030](../manual-testing-playbook/state-and-fault-tolerance/malformed-jsonl-corruption-warnings.md)
...
Expected signals: `node scripts/tests/state-machine-wiring.test.cjs` runs all four tests and prints the pass line; `state-machine-wiring.md` §2's state-to-script map matches the scripts exercised; §5 documents that `convergence.cjs`'s loopType enum (research/review/council/context) does not accept "alignment", so `check-convergence.cjs` is the NFR-R01-sanctioned self-contained coverage check, and Option A (extend the enum) is a scoped future recommendation, not done here.

#### Test Execution
> **Feature File:** [DAL-031](../manual-testing-playbook/state-and-fault-tolerance/state-machine-wiring-regression.md)
...
- `Adapters`: `.opencode/skills/system-deep-loop/deep-alignment/scripts/adapters/{sk-doc,sk-doc-command,sk-git,sk-code}.cjs`
...
- `State Machine Wiring`: `.opencode/skills/system-deep-loop/deep-alignment/references/state-machine-wiring.md`
- `Adapter specs + known deviations`: `.opencode/skills/system-deep-loop/deep-alignment/references/adapters/`
- `Config template`: `.opencode/skills/system-deep-loop/deep-alignment/assets/deep-alignment-config-template.json`
...
- DAL-001: [Invocation contract and forbidden patterns](../manual-testing-playbook/entry-points-and-modes/invocation-contract-and-forbidden-patterns.md)
- DAL-002: [Parameter surface: modes and loop tuning](../manual-testing-playbook/entry-points-and-modes/parameter-surface-modes-and-tuning.md)
- DAL-003: [Config-file-only non-interactive path](../manual-testing-playbook/entry-points-and-modes/config-file-only-non-interactive-path.md)

### LANE RESOLUTION AND SCOPING

- DAL-004: [Two resolution paths produce one identical lane shape](../manual-testing-playbook/lane-resolution-and-scoping/dual-path-identical-lane-shape.md)
- DAL-005: [Authority x artifact-class registry enforcement](../manual-testing-playbook/lane-resolution-and-scoping/authority-artifact-class-registry.md)
- DAL-006: [Scope-shape validation and repo-root containment](../manual-testing-playbook/lane-resolution-and-scoping/scope-shape-and-repo-root-validation.md)
- DAL-007: [Empty lane-config resolves to zero lanes](../manual-testing-playbook/lane-resolution-and-scoping/empty-lane-config-zero-lanes.md)
- DAL-008: [Fail-closed error contract](../manual-testing-playbook/lane-resolution-and-scoping/fail-closed-error-contract.md)
- DAL-009: [Multi-authority single run](../manual-testing-playbook/lane-resolution-and-scoping/multi-authority-single-run.md)

### DISCOVERY AND ADAPTERS

- DAL-010: [sk-doc adapter: discover / standardSource / check](../manual-testing-playbook/discovery-and-adapters/sk-doc-adapter.md)
...
- DAL-016: [Verify-first: no finding without a live re-probe](../manual-testing-playbook/verify-first-and-known-deviations/verify-first-no-finding-without-reprobe.md)
- DAL-017: [Known-deviation suppression](../manual-testing-playbook/verify-first-and-known-deviations/known-deviation-suppression.md)
- DAL-018: [sk-git exempt-subject pre-check vs post-hoc suppression](../manual-testing-playbook/verify-first-and-known-deviations/sk-git-exempt-precheck-vs-suppression.md)

### ITERATION AND CONVERGENCE

- DAL-019: [Corpus partitioning round-robin](../manual-testing-playbook/iteration-and-convergence/corpus-partitioning-round-robin.md)
- DAL-020: [Coverage AND dry-run stability convergence](../manual-testing-playbook/iteration-and-convergence/coverage-and-stability-and-semantics.md)
- DAL-021: [Max-iterations independent hard stop](../manual-testing-playbook/iteration-and-convergence/max-iterations-hard-stop.md)
- DAL-022: [Dry-run stability window fails closed on a fresh run](../manual-testing-playbook/iteration-and-convergence/dry-run-stability-fail-closed.md)
- DAL-023: [Nothing-to-converge and vacuous-lane exclusion](../manual-testing-playbook/iteration-and-convergence/nothing-to-converge-and-vacuous-lane.md)

### READ-ONLY AND GATED REMEDIATION

- DAL-024: [Read-only default: audited artifacts are never modified](../manual-testing-playbook/read-only-and-gated-remediation/read-only-default-surface.md)
- DAL-025: [Gated remediation hook is an enterable no-op](../manual-testing-playbook/read-only-and-gated-remediation/gated-remediation-hook-noop.md)

### REPORT EMISSION PER LANE

- DAL-026: [One report section per lane, never blended](../manual-testing-playbook/report-emission-per-lane/one-report-per-lane.md)
- DAL-027: [Worst-verdict overall rollup](../manual-testing-playbook/report-emission-per-lane/worst-verdict-overall-rollup.md)
- DAL-028: [Finding dedup and fail-closed severity](../manual-testing-playbook/report-emission-per-lane/finding-dedup-and-fail-closed-severity.md)

### STATE AND FAULT TOLERANCE

- DAL-029: [alignment/ state-file layout and file protection](../manual-testing-playbook/state-and-fault-tolerance/alignment-state-file-layout.md)
- DAL-030: [Malformed JSONL surfaces corruptionWarnings, never crashes](../manual-testing-playbook/state-and-fault-tolerance/malformed-jsonl-corruption-warnings.md)
- DAL-031: [End-to-end state-machine wiring regression](../manual-testing-playbook/state-and-fault-tolerance/state-machine-wiring-regression.md)
