...
**In scope**: sk-code authority only. Each other authority adapter (sk-doc — phase 005; sk-git, sk-design — phase 006) owns its own known-deviation list under its own `standardSource`, per ADR-005's per-authority requirement; this document does not attempt to anticipate their conventions.
