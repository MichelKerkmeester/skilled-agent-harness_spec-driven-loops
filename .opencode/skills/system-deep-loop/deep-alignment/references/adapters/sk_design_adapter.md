...
`check(artifact, rules, options)` runs a deterministic structural sub-check for both artifact kinds, plus an optional reasoning-agent audit-rubric sub-check, then returns a flat findings array after known-deviation suppression (Section 5).
...
## 5. KNOWN-DEVIATION SUPPRESSION (ADR-005)
...
## 6. VERIFY-FIRST BEHAVIOR (ADR-005)
...
## 7. LIVE-REALITY FINDINGS (SURFACED DURING THIS ADAPTER'S OWN CONSTRUCTION)
...
- `check` against all 4 real example `DESIGN.md` files (`vercel`, `linear`, `stripe`, `supabase`) — all clean after Section 7's two fixes (vercel/linear were the two that surfaced the fixes).
