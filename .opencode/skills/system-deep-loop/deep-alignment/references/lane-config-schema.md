...
- `scoping-protocol.md`, the interactive tree that produces the identical lane-tuple shape this file's `authority`/`artifactClass`/`scope` triple maps onto 1:1.
- `discover-contract.md`, what `DISCOVER` does with a resolved lane's `scope` next.
