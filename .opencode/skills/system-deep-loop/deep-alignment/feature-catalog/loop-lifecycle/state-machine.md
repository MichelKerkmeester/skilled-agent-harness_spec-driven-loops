...
| File | Layer | Role |
|---|---|---|
| `references/state-machine-wiring.md` | Reference | The state-to-script map, the `alignment/` layout, the loopType decision, and the corpus-partitioning/remediate rationale. |
...
| `assets/deep-alignment-config-template.json` | Template | The full run-config shape (`fileProtection`, script paths, convergence defaults) the states read. |
...
- Primary sources: `references/state-machine-wiring.md`, `runtime/scripts/loop-lock.cjs`, `runtime/scripts/upsert.cjs`
Related references:
...
- [../alignment-contract/gated-remediation.md](../../feature-catalog/alignment-contract/gated-remediation.md) — Gated remediation
