...
sk-git is a special case with two distinct mechanisms: its Git-generated-subject and `work/` launch-wrapper exemptions are structural pre-checks that return early before any finding is even produced, while its legacy pre-hook packet-path scope rule uses the same post-hoc JSON suppression the other adapters use.
