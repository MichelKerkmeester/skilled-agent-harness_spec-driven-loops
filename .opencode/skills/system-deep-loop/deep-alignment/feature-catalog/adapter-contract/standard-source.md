...
The returned shape is authority-specific because standards differ: sk-doc returns its two validator paths (`validate_document.py`, `extract_structure.py`), template dirs, and `core-standards.md`; sk-git returns its commit-grammar/branch-naming rule anchors in `SKILL.md` and the enforcement hook path; sk-code returns its surface router, per-surface validators, an `excludedFromCheck` record for the tree-mutating minifier, and its reference dirs. Every one also carries `knownDeviations`, loaded from that authority's `sk_*_known_deviations.md` fenced JSON block.
...
| File | Layer | Role |
|---|---|---|
| `scripts/adapters/sk-doc.cjs` | Adapter | `standardSource('sk-doc')` returns validator paths, template dirs, `core-standards.md`, and loaded deviations. |
...
| `references/discover-contract.md` | Reference | Names `standardSource` as the second method of the ADR-003 three-method contract. |
