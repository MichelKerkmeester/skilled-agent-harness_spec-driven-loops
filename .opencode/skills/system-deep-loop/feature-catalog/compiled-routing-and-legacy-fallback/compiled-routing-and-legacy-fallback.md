...
description: "How system-deep-loop resolves the compiled per-hub router contract ahead of its own registry-driven routing, and the legacy fallback that keeps it inert until explicitly authorized."
trigger_phrases:
...
`system-deep-loop`'s `SKILL.md` carries an opt-in, flag-gated, additive directive that asks the compiled per-hub router contract to resolve the mode before falling through to the mode-registry-driven routing described in [`deep-loop-mode-classification.md`](../deep-loop-mode-classification/deep-loop-mode-classification.md).

The directive is off by default and byte-identical to today's behavior when unset: the compiled front door returns a `{"servingAuthority":"legacy"}` sentinel (or is never meaningfully consulted), and `system-deep-loop` falls back to its registry-driven routing every time.
...
| File | Layer | Role |
|---|---|---|
| `.opencode/skills/system-deep-loop/SKILL.md` | Shared | Carries the opt-in compiled-routing directive `system-deep-loop` follows. |
...
Related references:
- [deep-loop-mode-classification.md](../deep-loop-mode-classification/deep-loop-mode-classification.md) — the registry-driven routing this directive resolves ahead of.
