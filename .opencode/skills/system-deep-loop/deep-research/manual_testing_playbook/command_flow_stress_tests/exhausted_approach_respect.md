...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/deep-research/manual_testing_playbook/command_flow_stress_tests/setup_cp_sandbox.sh --sandbox-dir /tmp/cp-051-sandbox
...
opencode run "$(cat /tmp/cp-051-prompt-A.txt)" --model deepseek/deepseek-v4-pro --dangerously-skip-permissions --dir /tmp/cp-051-sandbox </dev/null 2>&1 | tee /tmp/cp-051-A-task.txt; echo "EXIT_A=${PIPESTATUS[0]}" | tee /tmp/cp-051-A-exit.txt
...
| `.opencode/commands/deep/assets/deep_research_auto.yaml:197-217` | resume classification and event |
...
- Feature file path: `command-flow-stress-tests/exhausted-approach-respect.md`
