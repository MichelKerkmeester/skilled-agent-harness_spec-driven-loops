...
Signal rules live in `../convergence/convergence-signals.md`.
...
Graph convergence event shape lives in `../convergence/convergence-graph.md`.
...
Malformed lines are handled by the reducer according to its strict/lenient mode. Fault tolerance and reconstruction are documented in `state-reducer-registry.md`.
