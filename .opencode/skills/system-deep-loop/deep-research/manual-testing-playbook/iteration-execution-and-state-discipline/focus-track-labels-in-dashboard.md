...
- Pass/fail posture: PASS if focusTrack is defined in state-format.md as optional on iteration records AND the dashboard Progress table includes a Track column; FAIL if the field is missing from either location or the two definitions contradict.
...
### Evidence
Capture the state-format.md focusTrack field definition, the dashboard Progress table header row, and any README mention of focus track grouping.
### Pass/Fail
PASS if focusTrack is defined in state-format.md as optional on iteration records AND the dashboard Progress table includes a Track column; FAIL if the field is missing from either location or the two definitions contradict.
### Failure Triage
Privilege state-format.md as the canonical schema; use the dashboard asset and README only as secondary confirmation of propagation.
---
