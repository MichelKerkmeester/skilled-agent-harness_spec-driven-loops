...
- Expected signals: `graphEvents` documented as iteration-record input for graph-aware convergence; the state-format example uses flat `type` values; live graph tests seed `QUESTION`, `FINDING`, and `SOURCE` node kinds rather than `question_node` / `finding_node` wrappers.
...
### Failure Triage
Privilege `references/state/state-format.md` for the payload contract, `references/convergence/convergence.md` for consumption semantics, and the active graph tests for implementation truth. If they diverge, treat the active tests plus state-format doc as current reality and flag any remaining nested-schema wording as drift.
---
