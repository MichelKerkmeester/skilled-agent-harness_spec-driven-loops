...
This scenario invokes `/deep:research:auto` with `--no-resource-map`. Call B must persist the disabled setting and avoid emitting a convergence-time `resource-map.md` while still producing normal research artifacts.
...
4. Reset the sandbox.
5. Run Call B through `/deep:research:auto ... --no-resource-map`.
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/deep-research/manual-testing-playbook/command-flow-stress-tests/setup_cp_sandbox.sh --sandbox-dir /tmp/cp-048-sandbox
...
In /tmp/cp-048-sandbox/, run /deep:research:auto with --no-resource-map.
...
- Feature file path: `command-flow-stress-tests/resource-map-toggle.md`
