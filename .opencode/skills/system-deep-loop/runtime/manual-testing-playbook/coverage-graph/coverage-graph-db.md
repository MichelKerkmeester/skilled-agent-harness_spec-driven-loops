...
- Feature catalog entry exists at `feature-catalog/coverage-graph/coverage-graph-db.md`.
...
- Source excerpts from `lib/coverage-graph/coverage-graph-db.ts` showing the anchors named in the commands above, read from the current files rather than recalled.
- Captured stdout and exit status for every command run in this section.
- Output from `tests/lifecycle/db-open-close.vitest.ts` naming the assertions that carry the expected signals.
...
- Playbook ID: DLR-011
- Feature catalog entry: `feature-catalog/coverage-graph/coverage-graph-db.md`
- Scenario file path: `manual-testing-playbook/coverage-graph/coverage-graph-db.md`
