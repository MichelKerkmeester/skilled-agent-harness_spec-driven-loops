...
- Feature catalog entry exists at `feature-catalog/prompt-rendering/prompt-pack.md`.
...
- Source excerpts from `lib/deep-loop/prompt-pack.ts` showing the anchors named in the commands above, read from the current files rather than recalled.
- Captured stdout and exit status for every command run in this section.
- Output from `tests/unit/prompt-pack.vitest.ts` naming the assertions that carry the expected signals.
...
- Playbook ID: DLR-004
- Feature catalog entry: `feature-catalog/prompt-rendering/prompt-pack.md`
- Scenario file path: `manual-testing-playbook/prompt-rendering/prompt-pack.md`
