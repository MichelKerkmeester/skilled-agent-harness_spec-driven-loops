// ───────────────────────────────────────────────────────────────────
// MODULE: Agent Improvement Rollback Switch
// ───────────────────────────────────────────────────────────────────

import { parseAgentImprovementResumeDecision } from '../agent-improvement-resume-adapter/index.js';
...
import { AgentImprovementModeMigrationGate } from './mode-gate.js';
import { AGENT_IMPROVEMENT_ROLLBACK_GATE_SCHEMA_VERSION } from './types.js';
...
import type {
  ...
  AgentImprovementRollbackCertificate,
  AgentImprovementRollbackDecision,
  AgentImprovementRollbackDenialReasonCode,
  AgentImprovementRollbackOperation,
  AgentImprovementRollbackRequest,
  AgentImprovementRollbackSwitchOptions,
} from './types.js';
...
const ALLOWED_ROLLBACK_OPERATIONS: ReadonlySet<AgentImprovementRollbackOperation> = new Set([
  'rollback',
  'unquarantine',
  'verifier-replacement',
  'authority-restoration',
]);
const AGENT_IMPROVEMENT_LEDGER_WRITER_RESOURCE = canonicalizeProtectedResource({
...
function denied(
  reasonCode: AgentImprovementRollbackDenialReasonCode,
): AgentImprovementRollbackDecision {
...
function validateResumeEvidence(
  input: NonNullable<AgentImprovementRollbackRequest['resumeEvidence']>,
): Readonly<{
  value: NonNullable<AgentImprovementRollbackRequest['resumeEvidence']>;
  digest: string;
}> {
  ...
  const legacyDecision = parseAgentImprovementResumeDecision(input.legacyDecision);
  const ledgerDecision = parseAgentImprovementResumeDecision(input.ledgerDecision);
  for (const field of [
    'legacyEventTailDigest',
    'ledgerEventTailDigest',
    'legacyFreshProjectionFingerprint',
...
async function reverifyMigrationCertificate(
  certificate: AgentImprovementModeMigrationCertificate | null | undefined,
  gateInput: AgentImprovementRollbackRequest['gateInput'],
): Promise<AgentImprovementModeMigrationCertificate | null> {
  ...
  try {
    ...
    const internallyValid = certificate.schemaVersion
      ...
      && certificate.mode === 'agent-improvement'
      ...
      && certificate.candidateDispatched === false
    ...
    const reverified = await new AgentImprovementModeMigrationGate().evaluate(gateInput);
...
export class AgentImprovementRollbackSwitch {
  readonly #options: AgentImprovementRollbackSwitchOptions;

  public constructor(options: AgentImprovementRollbackSwitchOptions) {
    this.#options = options;
  }

  public async requestRollback(
    input: AgentImprovementRollbackRequest,
  ): Promise<AgentImprovementRollbackDecision> {
    let prepared: Readonly<{
      operation: AgentImprovementRollbackOperation;
    ...
    try {
      ...
      if (
        authorizationRequest.mode !== 'agent-improvement'
        || authorizationRequest.authorityEpoch !== input.currentAuthority.epoch
        || authorizationRequest.evidenceDigest !== boundEvidenceDigest
      ) return denied('EVIDENCE_INCOMPLETE');
      if (
        writerResource.resourceDigest !== AGENT_IMPROVEMENT_LEDGER_WRITER_RESOURCE.resourceDigest
    ...
    if (
      authorization.decision.mode !== 'agent-improvement'
    ...
    try {
      const rollbackLease = await this.#options.fencingCoordinator.acquire({
        ...
        ownerId: 'agent-improvement-external-rollback',
    ...
    const certificate: AgentImprovementRollbackCertificate = Object.freeze({
      ...certificateCore,
