---
title: "agent-improvement-ledger-schema: Ledger Schema"
description: "Agent Improvement ledger schema layer for the durable runtime event spine."
trigger_phrases:
  - "agent-improvement-ledger-schema"
  - "agent-improvement ledger schema"
---
...
This folder defines the lane's durable event stems, payloads and compatibility types for the Agent Improvement runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| Primary implementation | `agent-improvement-ledger-schema.ts` | event definitions, payload construction and schema validation. |
...
This folder owns the ledger schema step for Agent Improvement. It does not replace the shared substrate or decide consumer-facing workflow policy.
