// ───────────────────────────────────────────────────────────────────
// MODULE: Deep Review Certificate Validation
// ───────────────────────────────────────────────────────────────────
...
const TRANSITION_KINDS = new Set(Object.values(DeepReviewTransitionKinds));
...
const ARTIFACT_KINDS = new Set(Object.values(DeepReviewArtifactKinds));
...
function parseReceiptFacts(value: unknown, location: string): DeepReviewTransitionReceiptFacts {
  const candidate = record(value, location);
  ...
  return candidate as unknown as DeepReviewTransitionReceiptFacts;
}

/** Parse one receipt with closed field coverage and no free-string fallback. */
export function parseDeepReviewTransitionReceipt(
  value: unknown,
  location = 'receipt',
): DeepReviewTransitionReceipt {
  const candidate = record(value, location);
...
function parseArtifactClaim(value: unknown, location: string): DeepReviewCertificateArtifactClaim {
  const candidate = record(value, location);
  exactFields(candidate, [
    'binding', 'descriptorDigest', 'contentDigest', 'canonicalizationVersion',
  ], location);
  const binding = parseDeepReviewSealedArtifactBinding(candidate.binding);
...
function parseClosureRule(
  ...
  const expectedArtifactKinds = candidate.expectedArtifactKinds.map((artifactKind, index) => {
    ...
    return artifactKind as DeepReviewNamedDigestClosureRule['containingArtifactKind'];
  });
  validateField(
  ...
  return Object.freeze({
    containingArtifactKind: candidate.containingArtifactKind as
      DeepReviewNamedDigestClosureRule['containingArtifactKind'],
...
function parseConvergenceEvidence(
  ...
  return candidate as unknown as DeepReviewCertificateConvergenceEvidence;
}
...
function parseStatusEvidence(
  ...
  return candidate as unknown as DeepReviewCertificateStatusEvidence;
}
...
function parseCertificateBody(value: unknown, location: string): DeepReviewRunCertificateBody {
  const candidate = record(value, location);
  ...
  return candidate as unknown as DeepReviewRunCertificateBody;
}

/** Parse a run certificate without trusting any of its evidence claims. */
export function parseDeepReviewRunCertificate(
  value: unknown,
...
export function parseDeepReviewCertificateBundle(value: unknown): DeepReviewCertificateBundle {
  ...
  const receipts = candidate.receipts.map((receipt, index) => (
    parseDeepReviewTransitionReceipt(receipt, `bundle:receipts:${index}`)
  ));
  return Object.freeze({
    bundleVersion: 1,
    certificate: parseDeepReviewRunCertificate(candidate.certificate),
