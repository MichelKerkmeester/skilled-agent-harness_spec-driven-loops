// ───────────────────────────────────────────────────────────────────
// MODULE: Deep AI Council Certificate Validation
// ───────────────────────────────────────────────────────────────────
...
const TRANSITION_KINDS = new Set(Object.values(DeepAiCouncilTransitionKinds));
...
const CONVERGENCE_DECISIONS = new Set([
  'blocked',
  'continue',
  'converged',
  'incomplete',
...
const GATE_VERDICTS = new Set(['blocked', 'fail', 'pass', 'unknown']);
const ARTIFACT_COMPLETENESS = new Set(['complete', 'incomplete', 'unknown']);
  ...
  switch (rule.kind) {
    ...
    case 'number':
      if (typeof value !== 'number' || value < rule.minimum || value > rule.maximum) {
        invalid(location, 'Expected a bounded number');
      }
      return;
    case 'prose':
      if (
...
function parseReceiptFacts(value: unknown, location: string): DeepAiCouncilTransitionReceiptFacts {
  const candidate = record(value, location);
  ...
  return candidate as unknown as DeepAiCouncilTransitionReceiptFacts;
}

/** Parse one receipt with closed field coverage and no free-string fallback. */
export function parseDeepAiCouncilTransitionReceipt(
  value: unknown,
  location = 'receipt',
): DeepAiCouncilTransitionReceipt {
  const candidate = record(value, location);
...
function parseArtifactClaim(value: unknown, location: string): DeepAiCouncilCertificateArtifactClaim {
  const candidate = record(value, location);
  exactFields(candidate, [
    'binding', 'descriptorDigest', 'contentDigest', 'canonicalizationVersion',
  ], location);
  const binding = parseDeepAiCouncilSealedArtifactBinding(candidate.binding);
...
function parseConvergenceEvidence(
  ...
  const candidate = record(value, location);
  exactFields(candidate, [
    'outcome', 'eligible', 'evaluationEventId', 'decision', 'rawAgreement',
    'calibratedSupport', 'effectiveSeatCount', 'blockerIds',
  ], location);
  validateField(candidate.outcome, { kind: 'enum', values: CONVERGENCE_OUTCOMES }, `${location}:outcome`);
  validateField(candidate.eligible, { kind: 'boolean' }, `${location}:eligible`);
  validateField(candidate.evaluationEventId, { kind: 'token' }, `${location}:evaluationEventId`);
  validateField(candidate.decision, { kind: 'enum', values: CONVERGENCE_DECISIONS }, `${location}:decision`);
  validateField(candidate.rawAgreement, { kind: 'number', minimum: 0, maximum: 1 }, `${location}:rawAgreement`);
  validateField(candidate.calibratedSupport, { kind: 'number', minimum: 0, maximum: 1 }, `${location}:calibratedSupport`);
  validateField(candidate.effectiveSeatCount, { kind: 'integer', minimum: 0 }, `${location}:effectiveSeatCount`);
  scalarArray(candidate.blockerIds, { kind: 'token' }, `${location}:blockerIds`);
  return candidate as unknown as DeepAiCouncilCertificateConvergenceEvidence;
}
...
function parseStatusEvidence(
  ...
  return candidate as unknown as DeepAiCouncilCertificateStatusEvidence;
}

function parseTestGateEvidence(
  value: unknown,
  location: string,
): DeepAiCouncilCertificateTestGateEvidence {
  const candidate = record(value, location);
  exactFields(candidate, [
    'verdict', 'evaluationEventId', 'testSuiteDigest', 'fixtureManifestDigest',
    'artifactCompleteness', 'criticalFailureRefs',
  ], location);
  validateField(candidate.verdict, { kind: 'enum', values: GATE_VERDICTS }, `${location}:verdict`);
  validateField(candidate.evaluationEventId, { kind: 'token' }, `${location}:evaluationEventId`);
  validateField(candidate.testSuiteDigest, { kind: 'digest' }, `${location}:testSuiteDigest`);
  validateField(candidate.fixtureManifestDigest, { kind: 'digest' }, `${location}:fixtureManifestDigest`);
  validateField(
    candidate.artifactCompleteness,
    { kind: 'enum', values: ARTIFACT_COMPLETENESS },
    `${location}:artifactCompleteness`,
  );
  scalarArray(candidate.criticalFailureRefs, { kind: 'token' }, `${location}:criticalFailureRefs`);
  return candidate as unknown as DeepAiCouncilCertificateTestGateEvidence;
}

const CERTIFICATE_BODY_FIELDS = Object.freeze([
  'certificateVersion', 'authority', 'runId', 'roundId', 'generation',
  'lifecycleResult', 'startHead', 'finalHead', 'artifactClaims', 'artifactSetDigest',
  'orderedDependencyClosureDigest', 'receiptDigests', 'receiptChainDigest', 'replayFingerprint',
  'replayFingerprintVersion', 'projectionIntegrityDigest', 'convergenceEvidence',
  'statusEvidence', 'testGateEvidence', 'outputArtifactQualifiedDigests', 'openObligationIds',
]);

function parseCertificateBody(value: unknown, location: string): DeepAiCouncilRunCertificateBody {
  const candidate = record(value, location);
  ...
  validateField(candidate.roundId, { kind: 'token' }, `${location}:roundId`);
  ...
  parseTestGateEvidence(candidate.testGateEvidence, `${location}:testGateEvidence`);
  return candidate as unknown as DeepAiCouncilRunCertificateBody;
}

/** Parse a run certificate without trusting any of its evidence claims. */
export function parseDeepAiCouncilRunCertificate(
  value: unknown,
...
export function parseDeepAiCouncilCertificateBundle(value: unknown): DeepAiCouncilCertificateBundle {
  ...
  const receipts = candidate.receipts.map((receipt, index) => (
    parseDeepAiCouncilTransitionReceipt(receipt, `bundle:receipts:${index}`)
  ));
  return Object.freeze({
    bundleVersion: 1,
    certificate: parseDeepAiCouncilRunCertificate(candidate.certificate),
