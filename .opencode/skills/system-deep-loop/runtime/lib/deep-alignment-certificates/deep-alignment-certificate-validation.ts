// ───────────────────────────────────────────────────────────────────
// MODULE: Deep Alignment Certificate Validation
// ───────────────────────────────────────────────────────────────────
...
const TRANSITION_KINDS = new Set(Object.values(DeepAlignmentTransitionKinds));
...
const ARTIFACT_KINDS = new Set(Object.values(DeepAlignmentArtifactKinds));
...
function parseReceiptFacts(value: unknown, location: string): DeepAlignmentTransitionReceiptFacts {
  const candidate = record(value, location);
  ...
  return candidate as unknown as DeepAlignmentTransitionReceiptFacts;
}

/** Parse one receipt with closed field coverage and no free-string fallback. */
export function parseDeepAlignmentTransitionReceipt(
  value: unknown,
  location = 'receipt',
): DeepAlignmentTransitionReceipt {
  const candidate = record(value, location);
...
function parseArtifactClaim(value: unknown, location: string): DeepAlignmentCertificateArtifactClaim {
  const candidate = record(value, location);
  exactFields(candidate, [
    'binding', 'descriptorDigest', 'contentDigest', 'canonicalizationVersion',
  ], location);
  const binding = parseDeepAlignmentSealedArtifactBinding(candidate.binding);
...
function parseClosureRule(
  ...
  const candidate = record(value, location);
  exactFields(candidate, [
    'containingArtifactKind', 'field', 'expectedArtifactKinds', 'cardinality',
    'allowEmpty',
  ], location);
  validateField(
    candidate.containingArtifactKind,
    { kind: 'enum', values: ARTIFACT_KINDS },
    `${location}:containingArtifactKind`,
  );
  validateField(candidate.field, { kind: 'token' }, `${location}:field`);
  if (!Array.isArray(candidate.expectedArtifactKinds) || candidate.expectedArtifactKinds.length === 0) {
    invalid(`${location}:expectedArtifactKinds`, 'Closure rule requires expected artifact kinds');
  }
  const expectedArtifactKinds = candidate.expectedArtifactKinds.map((artifactKind, index) => {
    validateField(
      artifactKind,
      { kind: 'enum', values: ARTIFACT_KINDS },
      `${location}:expectedArtifactKinds:${index}`,
    );
    return artifactKind as DeepAlignmentNamedDigestClosureRule['containingArtifactKind'];
  });
  validateField(
    candidate.cardinality,
    { kind: 'enum', values: new Set(['array', 'scalar']) },
    `${location}:cardinality`,
  );
  validateField(candidate.allowEmpty, { kind: 'boolean' }, `${location}:allowEmpty`);
  return Object.freeze({
    containingArtifactKind: candidate.containingArtifactKind as
      DeepAlignmentNamedDigestClosureRule['containingArtifactKind'],
    field: candidate.field as string,
    expectedArtifactKinds: Object.freeze(expectedArtifactKinds),
    cardinality: candidate.cardinality as 'array' | 'scalar',
    allowEmpty: candidate.allowEmpty as boolean,
  });
}
...
function parseAuthorityEvidence(
  ...
  const candidate = record(value, location);
  exactFields(candidate, [
    'authorityEpochId', 'authorityId', 'authorityCapsuleRef',
    'validationEventId', 'validationDigest',
  ], location);
  for (const field of [
    'authorityEpochId', 'authorityId', 'authorityCapsuleRef', 'validationEventId',
  ] as const) {
    validateField(candidate[field], { kind: 'token' }, `${location}:${field}`);
  }
  validateField(candidate.validationDigest, { kind: 'digest' }, `${location}:validationDigest`);
  return candidate as unknown as DeepAlignmentCertificateAuthorityEvidence;
}

function parseApplicabilityEvidence(
  value: unknown,
  location: string,
): DeepAlignmentCertificateApplicabilityEvidence {
  const candidate = record(value, location);
  exactFields(candidate, [
    'coverageDigest', 'unresolvedRuleIds', 'blockedRuleIds',
  ], location);
  validateField(candidate.coverageDigest, { kind: 'digest' }, `${location}:coverageDigest`);
  scalarArray(candidate.unresolvedRuleIds, { kind: 'token' }, `${location}:unresolvedRuleIds`);
  scalarArray(candidate.blockedRuleIds, { kind: 'token' }, `${location}:blockedRuleIds`);
  return candidate as unknown as DeepAlignmentCertificateApplicabilityEvidence;
}
...
function parseConformanceEvidence(
  ...
  validateField(
    ...
    { kind: 'enum', values: new Set([
      'PASS', 'FAIL', 'WARN', 'INCONCLUSIVE', 'NOT_APPLICABLE', 'SKIP', 'EXEMPT',
    ]) },
    `${location}:overallVerdict`,
  );
  scalarArray(candidate.activeFindingIds, { kind: 'token' }, `${location}:activeFindingIds`);
  scalarArray(candidate.hardVetoFindingIds, { kind: 'token' }, `${location}:hardVetoFindingIds`);
  return candidate as unknown as DeepAlignmentCertificateConformanceEvidence;
}
...
function parseConvergenceEvidence(
  ...
  return candidate as unknown as DeepAlignmentCertificateConvergenceEvidence;
}
...
function parseStatusEvidence(
  ...
  return candidate as unknown as DeepAlignmentCertificateStatusEvidence;
}

const CERTIFICATE_BODY_FIELDS = Object.freeze([
  'certificateVersion', 'authority', 'runId', 'sessionId', 'generation',
  'lifecycleResult', 'startHead', 'finalHead', 'artifactClaims', 'artifactSetDigest',
  'namedDigestClosureRules', 'orderedDependencyClosureDigest',
  'receiptDigests', 'receiptChainDigest', 'replayFingerprint',
  'replayFingerprintVersion', 'projectionIntegrityDigest', 'authorityEvidence',
  'applicabilityEvidence', 'conformanceEvidence', 'convergenceEvidence',
  'statusEvidence', 'outputArtifactQualifiedDigests', 'openObligationIds',
]);

function parseCertificateBody(value: unknown, location: string): DeepAlignmentRunCertificateBody {
  const candidate = record(value, location);
  ...
  validateField(candidate.sessionId, { kind: 'token' }, `${location}:sessionId`);
  ...
  for (const field of [
    'artifactSetDigest', 'orderedDependencyClosureDigest', 'receiptChainDigest', 'replayFingerprint',
  ...
  parseAuthorityEvidence(candidate.authorityEvidence, `${location}:authorityEvidence`);
  parseApplicabilityEvidence(
    candidate.applicabilityEvidence,
    `${location}:applicabilityEvidence`,
  );
  parseConformanceEvidence(candidate.conformanceEvidence, `${location}:conformanceEvidence`);
  ...
  if (!Array.isArray(candidate.namedDigestClosureRules) || candidate.namedDigestClosureRules.length === 0) {
    invalid(`${location}:namedDigestClosureRules`, 'Certificate requires the named-digest closure map');
  }
  candidate.namedDigestClosureRules.forEach((rule, index) => (
    parseClosureRule(rule, `${location}:namedDigestClosureRules:${index}`)
  ));
  return candidate as unknown as DeepAlignmentRunCertificateBody;
}

/** Parse a run certificate without trusting any of its evidence claims. */
export function parseDeepAlignmentRunCertificate(
  value: unknown,
...
export function parseDeepAlignmentCertificateBundle(value: unknown): DeepAlignmentCertificateBundle {
  ...
  const receipts = candidate.receipts.map((receipt, index) => (
    parseDeepAlignmentTransitionReceipt(receipt, `bundle:receipts:${index}`)
  ));
  return Object.freeze({
    bundleVersion: 1,
    certificate: parseDeepAlignmentRunCertificate(candidate.certificate),
