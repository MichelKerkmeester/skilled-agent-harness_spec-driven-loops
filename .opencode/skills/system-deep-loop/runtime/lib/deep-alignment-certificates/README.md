---
title: "deep-alignment-certificates: Certificates"
description: "Deep Alignment certificates layer for the durable runtime event spine."
trigger_phrases:
  - "deep-alignment-certificates"
  - "deep-alignment certificates"
---
...
This folder issues and validates transition receipts and run certificates for the Deep Alignment runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-alignment-certificate-types.ts` | Lane-specific types, constants and contract values. |
| `deep-alignment-certificate-validation.ts` | Strict parsing and validation for the lane's durable records. |
| `deep-alignment-certificates.ts` | certificate issuance and verification logic |
...
| Primary implementation | `deep-alignment-certificates.ts` | certificate issuance and verification logic. |
...
This folder owns the certificates step for Deep Alignment. It does not replace the shared substrate or decide consumer-facing workflow policy.
