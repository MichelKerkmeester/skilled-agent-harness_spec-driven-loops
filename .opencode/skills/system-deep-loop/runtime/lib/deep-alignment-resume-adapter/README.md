---
title: "deep-alignment-resume-adapter: Resume Adapter"
description: "Deep Alignment resume adapter layer for the durable runtime event spine."
trigger_phrases:
...
This folder reconstructs lane continuity from durable ledger, projection and artifact evidence for the Deep Alignment runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-alignment-resume-adapter.ts` | resume request parsing, continuity decisions and fingerprint binding |
...
| Primary implementation | `deep-alignment-resume-adapter.ts` | resume request parsing, continuity decisions and fingerprint binding. |
...
This folder owns the resume adapter step for Deep Alignment. It does not replace the shared substrate or decide consumer-facing workflow policy.
