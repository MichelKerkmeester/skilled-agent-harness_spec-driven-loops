// ───────────────────────────────────────────────────────────────────
// MODULE: Skill Benchmark Rollback Switch
// ───────────────────────────────────────────────────────────────────
...
import { parseSkillBenchmarkResumeDecision } from '../skill-benchmark-resume-adapter/index.js';

import { SkillBenchmarkModeMigrationGate } from './mode-gate.js';
import { SKILL_BENCHMARK_ROLLBACK_GATE_SCHEMA_VERSION } from './types.js';
...
import type {
  ...
  SkillBenchmarkRollbackCertificate,
  SkillBenchmarkRollbackDecision,
  SkillBenchmarkRollbackDenialReasonCode,
  SkillBenchmarkRollbackOperation,
  SkillBenchmarkRollbackRequest,
  SkillBenchmarkRollbackSwitchOptions,
} from './types.js';
...
const ALLOWED_ROLLBACK_OPERATIONS: ReadonlySet<SkillBenchmarkRollbackOperation> = new Set([
  'rollback',
  'unquarantine',
  'verifier-replacement',
  'authority-restoration',
]);
const SKILL_BENCHMARK_LEDGER_WRITER_RESOURCE = canonicalizeProtectedResource({
...
function denied(
  reasonCode: SkillBenchmarkRollbackDenialReasonCode,
): SkillBenchmarkRollbackDecision {
...
function validateResumeEvidence(
  input: NonNullable<SkillBenchmarkRollbackRequest['resumeEvidence']>,
): Readonly<{
  value: NonNullable<SkillBenchmarkRollbackRequest['resumeEvidence']>;
  digest: string;
}> {
  ...
  const legacyDecision = parseSkillBenchmarkResumeDecision(input.legacyDecision);
  const ledgerDecision = parseSkillBenchmarkResumeDecision(input.ledgerDecision);
  for (const field of [
    'legacyEventTailDigest',
    'ledgerEventTailDigest',
    'legacyFreshProjectionFingerprint',
...
async function reverifyMigrationCertificate(
  certificate: SkillBenchmarkModeMigrationCertificate | null | undefined,
  gateInput: SkillBenchmarkRollbackRequest['gateInput'],
): Promise<SkillBenchmarkModeMigrationCertificate | null> {
  ...
  try {
    ...
    const internallyValid = certificate.schemaVersion
      ...
      && certificate.mode === 'skill-benchmark'
      ...
      && certificate.effectCertificateApplied === false
    ...
    const reverified = await new SkillBenchmarkModeMigrationGate().evaluate(gateInput);
...
export class SkillBenchmarkRollbackSwitch {
  readonly #options: SkillBenchmarkRollbackSwitchOptions;

  public constructor(options: SkillBenchmarkRollbackSwitchOptions) {
    this.#options = options;
  }

  public async requestRollback(
    input: SkillBenchmarkRollbackRequest,
  ): Promise<SkillBenchmarkRollbackDecision> {
    let prepared: Readonly<{
      operation: SkillBenchmarkRollbackOperation;
    ...
    try {
      ...
      if (
        authorizationRequest.mode !== 'skill-benchmark'
        || authorizationRequest.authorityEpoch !== input.currentAuthority.epoch
        || authorizationRequest.evidenceDigest !== boundEvidenceDigest
      ) return denied('EVIDENCE_INCOMPLETE');
      if (
        writerResource.resourceDigest !== SKILL_BENCHMARK_LEDGER_WRITER_RESOURCE.resourceDigest
    ...
    let authorization;
    try {
      // The gateway owns event kind, epoch, state, causal order, capability, and policy checks.
    ...
    if (
      authorization.decision.mode !== 'skill-benchmark'
    ...
    try {
      const rollbackLease = await this.#options.fencingCoordinator.acquire({
        ...
        ownerId: 'skill-benchmark-external-rollback',
      ...
      try {
        ...
        // Active lease ownership and the durable high-water mark, not set membership, prove supersession.
        superseded = prepared.staleWriterLease.fenceToken > 0
          ...
          && durable.activeLease.ownerId === 'skill-benchmark-external-rollback'
          && durable.activeLease.correlationId === authorization.decision.decision_id
    ...
    // The switch emits evidence only; the authority coordinator performs any actual restoration.
    const certificate: SkillBenchmarkRollbackCertificate = Object.freeze({
      ...certificateCore,
