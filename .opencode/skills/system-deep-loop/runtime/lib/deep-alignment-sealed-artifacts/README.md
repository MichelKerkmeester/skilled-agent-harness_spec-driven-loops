---
title: "deep-alignment-sealed-artifacts: Sealed Artifacts"
description: "Deep Alignment sealed artifacts layer for the durable runtime event spine."
trigger_phrases:
...
This folder canonicalizes, seals and reads lane-owned evidence artifacts for the Deep Alignment runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-alignment-artifact-material.ts` | Canonical artifact material and kind registration. |
| `deep-alignment-sealed-artifact-types.ts` | Lane-specific types, constants and contract values. |
| `deep-alignment-sealed-artifacts.ts` | artifact materialization, sealing, binding parsing and reads |
...
| Primary implementation | `deep-alignment-sealed-artifacts.ts` | artifact materialization, sealing, binding parsing and reads. |
...
This folder owns the sealed artifacts step for Deep Alignment. It does not replace the shared substrate or decide consumer-facing workflow policy.
