---
title: "deep-improvement-common-certificates: Certificates"
description: "Deep Improvement Common certificates layer for the durable runtime event spine."
trigger_phrases:
...
This folder issues and validates transition receipts and run certificates for the Deep Improvement Common runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-improvement-common-certificate-types.ts` | Lane-specific types, constants and contract values. |
| `deep-improvement-common-certificate-validation.ts` | Strict parsing and validation for the lane's durable records. |
| `deep-improvement-common-certificates.ts` | certificate issuance and verification logic |
...
| Primary implementation | `deep-improvement-common-certificates.ts` | certificate issuance and verification logic. |
...
This folder owns the certificates step for Deep Improvement Common. It does not replace the shared substrate or decide consumer-facing workflow policy.
