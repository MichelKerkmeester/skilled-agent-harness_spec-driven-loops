---
title: "model-benchmark-reducers: Reducers"
description: "Model Benchmark reducers layer for the durable runtime event spine."
trigger_phrases:
  - "model-benchmark-reducers"
  - "model benchmark reducers"
---
...
This folder folds verified lane events into deterministic projection state for the Model Benchmark runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `model-benchmark-projection-schema.ts` | Projection field ownership, schema versions and guards. |
| `model-benchmark-projection-types.ts` | Projection state and reducer contract types. |
| `model-benchmark-reducer.ts` | Deterministic event folding and legacy projection logic. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `model-benchmark-reducer.ts` |
...
This folder owns the reducers step for Model Benchmark. It does not replace the shared substrate or decide consumer-facing workflow policy.
