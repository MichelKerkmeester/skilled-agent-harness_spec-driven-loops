---
title: "skill-benchmark-ledger-schema: Ledger Schema"
description: "Skill Benchmark ledger-schema layer for the durable runtime event spine."
trigger_phrases:
  - "skill-benchmark-ledger-schema"
  - "skill benchmark ledger-schema"
---
...
This folder defines the lane's durable event and ledger payload contract for the Skill Benchmark runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `skill-benchmark-ledger-schema.ts` | Lane event definitions, payload builders and schema guards. |
| `skill-benchmark-ledger-types.ts` | Lane-specific event, payload and compatibility types. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `skill-benchmark-ledger-schema.ts` |
...
This folder owns the ledger schema step for Skill Benchmark. It does not replace the shared substrate or decide consumer-facing workflow policy.
