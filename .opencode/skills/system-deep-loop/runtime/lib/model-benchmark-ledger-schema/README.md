---
title: "model-benchmark-ledger-schema: Ledger Schema"
description: "Model Benchmark ledger-schema layer for the durable runtime event spine."
trigger_phrases:
  - "model-benchmark-ledger-schema"
  - "model benchmark ledger-schema"
---
...
This folder defines the lane's durable event and ledger payload contract for the Model Benchmark runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `model-benchmark-ledger-schema.ts` |
...
This folder owns the ledger schema step for Model Benchmark. It does not replace the shared substrate or decide consumer-facing workflow policy.
