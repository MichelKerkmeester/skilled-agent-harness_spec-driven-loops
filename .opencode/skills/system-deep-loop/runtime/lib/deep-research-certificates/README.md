---
title: "deep-research-certificates: Certificates"
description: "Deep Research certificates layer for the durable runtime event spine."
trigger_phrases:
  - "deep-research-certificates"
  - "deep research certificates"
---
...
This folder issues and validates transition receipts and run certificates for the Deep Research runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-research-certificate-types.ts` | Lane-specific types, constants and contract values. |
| `deep-research-certificate-validation.ts` | Strict parsing and validation for the lane's durable records. |
| `deep-research-certificates.ts` | Certificate issuance and verification logic. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `deep-research-certificates.ts` |
...
This folder owns the certificates step for Deep Research. It does not replace the shared substrate or decide consumer-facing workflow policy.
