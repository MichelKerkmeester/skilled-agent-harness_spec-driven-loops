// ───────────────────────────────────────────────────────────────────
// MODULE: Deep AI Council Independent Mode Gate
// ───────────────────────────────────────────────────────────────────
...
import type { DeepAiCouncilResumeDecision } from '../deep-ai-council-resume-adapter/index.js';
import type {
  DeepAiCouncilGateDisposition,
  DeepAiCouncilGateInputDisposition,
  DeepAiCouncilGateInputKind,
  DeepAiCouncilGateReasonCode,
  DeepAiCouncilGateVerdict,
  DeepAiCouncilLifecycleEvidenceKind,
  DeepAiCouncilModeGateInput,
  DeepAiCouncilModeGateResult,
  DeepAiCouncilModeMigrationCertificate,
  DeepAiCouncilRollbackWindowEvaluation,
  DeepAiCouncilRollbackWindowInput,
} from './types.js';
import type { DeepAiCouncilParityReceipt } from '../deep-ai-council-shadow-parity/index.js';
...
const REQUIRED_LIFECYCLE: readonly DeepAiCouncilLifecycleEvidenceKind[] = Object.freeze([
  'normal-completion',
...
const INPUT_ORDER: readonly DeepAiCouncilGateInputKind[] = Object.freeze([
  'shadow_parity',
  'sealed_artifacts',
...
const SAFE_RESUME_COMPATIBILITY_OUTCOMES = new Set<
  DeepAiCouncilResumeDecision['compatibilityOutcome']
>(['exact', 'compatible', 'migrate']);
const SAFE_RESUME_MANIFEST_DISPOSITIONS = new Set<
  DeepAiCouncilResumeDecision['manifestDisposition']
>(['original', 'restart']);
const SAFE_RESUME_BRANCH_DISPOSITIONS = new Set<
  DeepAiCouncilResumeDecision['branches'][number]['disposition']
>(['reuse', 'reexecute']);
const SAFE_RESUME_EFFECT_DISPOSITIONS = new Set<
  DeepAiCouncilResumeDecision['effects'][number]['disposition']
>(['reexecute', 'reconcile']);
const SAFE_RESUME_DECISION_DISPOSITIONS = new Set<
  DeepAiCouncilResumeDecision['disposition']
...
function resumeDecisionSemanticView(decision: DeepAiCouncilResumeDecision): JsonObject {
  const {
  ...
  const invalidation = {
    ...
    invalidatedLogicalBranchIds: [...decision.invalidation.invalidatedLogicalBranchIds].sort(),
    invalidatedArtifactIds: [...decision.invalidation.invalidatedArtifactIds].sort(),
  };
  return {
    ...semanticDecision,
    compatibility,
...
function isReadyResumeDecision(decision: DeepAiCouncilResumeDecision): boolean {
  return SAFE_RESUME_DECISION_DISPOSITIONS.has(decision.disposition)
...
function disposition(
  input: DeepAiCouncilGateInputKind,
  value: DeepAiCouncilGateDisposition,
  reasonCode: DeepAiCouncilGateReasonCode | null,
...
function fail(
  input: DeepAiCouncilGateInputKind,
  reasonCode: DeepAiCouncilGateReasonCode,
...
function overallVerdict(
  dispositions: readonly DeepAiCouncilGateInputDisposition[],
): DeepAiCouncilGateVerdict {
...
function validateTopLevel<TState extends JsonObject>(input: DeepAiCouncilModeGateInput<TState>): boolean {
...
async function evaluateParity<TState extends JsonObject>(
  input: DeepAiCouncilModeGateInput<TState>,
): Promise<Readonly<{
  disposition: DeepAiCouncilGateInputDisposition;
  receipts: readonly DeepAiCouncilParityReceipt[];
  ...
  try {
    const reported = parseDeepAiCouncilModeGateInput(input.parity.modeGateInput);
    ...
    const receipts = input.parity.receipts.map((entry) => (
      parseDeepAiCouncilParityReceipt(entry, input.parity!.manifest)
    ));
    ...
    const allowed = audit.entries.filter((entry) => (
      ...
      && entry.decision.mode === 'deep-ai-council'
...
async function evaluateSealed<TState extends JsonObject>(
  ...
  try {
    const verified = await Promise.all(input.sealedArtifacts.bindings.map((binding) => (
      readDeepAiCouncilArtifact(input.sealedArtifacts!.store, binding)
    )));
    const lifecycleByKind = new Map(DEEP_AI_COUNCIL_ARTIFACT_KIND_REGISTRY.map(
...
async function evaluateCertificates<TState extends JsonObject>(
  ...
  try {
    ...
    const bundle = parseDeepAiCouncilCertificateBundle(
...
async function evaluateRollback<TState extends JsonObject>(
  input: DeepAiCouncilModeGateInput<TState>,
): Promise<DeepAiCouncilGateInputDisposition> {
...
export function evaluateDeepAiCouncilRollbackWindow(
  input: DeepAiCouncilRollbackWindowInput,
): DeepAiCouncilRollbackWindowEvaluation {
  ...
  const minimumsMet = elapsedCalendarDays >= DEEP_AI_COUNCIL_ROLLBACK_MINIMUM_DAYS
    && successful >= DEEP_AI_COUNCIL_ROLLBACK_MINIMUM_SUCCESSFUL_EXECUTIONS;
  ...
  const core = Object.freeze({
    ...
    minimumCalendarDays: DEEP_AI_COUNCIL_ROLLBACK_MINIMUM_DAYS,
...
export class DeepAiCouncilModeMigrationGate {
  public async evaluate<TState extends JsonObject>(
    input: DeepAiCouncilModeGateInput<TState>,
  ): Promise<DeepAiCouncilModeGateResult> {
    try {
      return await this.#evaluate(input);
    } catch {
      const dispositions = Object.freeze(INPUT_ORDER.map((inputKind) => (
        fail(inputKind, 'EVIDENCE_MALFORMED')
      )));
      return Object.freeze({
  ...
  async #evaluate<TState extends JsonObject>(
    ...
    let window: DeepAiCouncilRollbackWindowEvaluation | null = null;
    try {
      window = evaluateDeepAiCouncilRollbackWindow(input.rollbackWindow);
    } catch {
      window = null;
    }
    ...
    const certificateCore = Object.freeze({
      schemaVersion: DEEP_AI_COUNCIL_ROLLBACK_GATE_SCHEMA_VERSION,
      certificateKind: 'mode-migration-readiness' as const,
      mode: 'deep-ai-council' as const,
    ...
    const certificate: DeepAiCouncilModeMigrationCertificate = Object.freeze({
      ...certificateCore,
