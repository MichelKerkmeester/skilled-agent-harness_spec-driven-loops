// ───────────────────────────────────────────────────────────────────
// MODULE: Deep AI Council Rollback Switch
// ───────────────────────────────────────────────────────────────────
...
import { DeepAiCouncilModeMigrationGate } from './mode-gate.js';
import {
...
import type {
  ...
  DeepAiCouncilRollbackCertificate,
  DeepAiCouncilRollbackDecision,
  DeepAiCouncilRollbackDenialReasonCode,
  DeepAiCouncilRollbackOperation,
  DeepAiCouncilRollbackRequest,
  DeepAiCouncilRollbackSwitchOptions,
} from './types.js';
...
const ALLOWED_ROLLBACK_OPERATIONS: ReadonlySet<DeepAiCouncilRollbackOperation> = new Set([
  'rollback',
  'unquarantine',
...
const DEEP_AI_COUNCIL_LEDGER_WRITER_RESOURCE = canonicalizeProtectedResource({
...
function denied(reasonCode: DeepAiCouncilRollbackDenialReasonCode): DeepAiCouncilRollbackDecision {
...
async function reverifyMigrationCertificate(
  certificate: DeepAiCouncilModeMigrationCertificate | null | undefined,
  gateInput: DeepAiCouncilRollbackRequest['gateInput'],
): Promise<DeepAiCouncilModeMigrationCertificate | null> {
  if (!certificate) return null;
  try {
    const { certificateDigest, ...core } = certificate;
    const internallyValid = certificate.schemaVersion === DEEP_AI_COUNCIL_ROLLBACK_GATE_SCHEMA_VERSION
      && certificate.certificateKind === 'mode-migration-readiness'
      && certificate.mode === 'deep-ai-council'
    ...
    const reverified = await new DeepAiCouncilModeMigrationGate().evaluate(gateInput);
...
function resumeEvidenceDigest(input: NonNullable<DeepAiCouncilRollbackRequest['resumeEvidence']>): string {
  return digest(input);
}
...
export class DeepAiCouncilRollbackSwitch {
  readonly #options: DeepAiCouncilRollbackSwitchOptions;

  public constructor(options: DeepAiCouncilRollbackSwitchOptions) {
    this.#options = options;
  }

  public async requestRollback(input: DeepAiCouncilRollbackRequest): Promise<DeepAiCouncilRollbackDecision> {
    let prepared: Readonly<{
      operation: DeepAiCouncilRollbackOperation;
      authorizationRequest: NonNullable<DeepAiCouncilRollbackRequest['authorizationRequest']>;
    ...
    try {
      ...
      if (
        input.authorizationRequest.mode !== 'deep-ai-council'
      ...
      if (
        writerResource.resourceDigest !== DEEP_AI_COUNCIL_LEDGER_WRITER_RESOURCE.resourceDigest
    ...
    try {
      const rollbackLease = await this.#options.fencingCoordinator.acquire({
        ...
        ownerId: 'deep-ai-council-external-rollback',
    ...
    const certificate: DeepAiCouncilRollbackCertificate = Object.freeze({
      ...certificateCore,
