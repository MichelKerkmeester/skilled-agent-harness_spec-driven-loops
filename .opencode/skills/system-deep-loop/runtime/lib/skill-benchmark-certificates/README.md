---
title: "skill-benchmark-certificates: Certificates"
description: "Skill Benchmark certificates layer for the durable runtime event spine."
trigger_phrases:
  - "skill-benchmark-certificates"
  - "skill benchmark certificates"
---
...
This folder issues and validates transition receipts and run certificates for the Skill Benchmark runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `skill-benchmark-certificate-types.ts` | Lane-specific types, constants and contract values. |
| `skill-benchmark-certificate-validation.ts` | Strict parsing and validation for the lane's durable records. |
| `skill-benchmark-certificates.ts` | Certificate issuance and verification logic. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `skill-benchmark-certificates.ts` |
...
This folder owns the certificates step for Skill Benchmark. It does not replace the shared substrate or decide consumer-facing workflow policy.
