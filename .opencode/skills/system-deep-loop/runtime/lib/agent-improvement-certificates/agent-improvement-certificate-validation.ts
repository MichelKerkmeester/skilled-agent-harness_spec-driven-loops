// MODULE: Agent Improvement Certificate Validation
...
const TRANSITIONS = new Set(Object.values(AgentImprovementTransitionKinds));
const OUTCOMES = new Set(['completed', 'inconclusive', 'rejected']);
const DISPOSITIONS = new Set(['ABORT', 'FAIL', 'INSUFFICIENT_EVIDENCE', 'PASS']);
const ARTIFACT_ROLES = new Set(['benchmark-evidence', 'proposal', 'scoring-evidence']);
...
function parseIdentity(value: unknown, location: string): AgentImprovementReceiptIdentity {
  const candidate = record(value, location);
  exactFields(candidate, [
    'identityVersion', 'runId', 'transitionKind', 'logicalOperationId',
    'effectIdempotencyKey', 'digest',
  ...
  field(candidate.digest, { kind: 'digest' }, `${location}:digest`);
  return candidate as unknown as AgentImprovementReceiptIdentity;
}
...
function parseReceiptFacts(
  ...
  return candidate as unknown as AgentImprovementTransitionReceiptFacts;
}

export function parseAgentImprovementTransitionReceipt(
  value: unknown,
  location = 'receipt',
): AgentImprovementTransitionReceipt {
  const candidate = record(value, location);
...
function parseArtifactClaim(
  ...
  const binding = parseAgentImprovementSealedArtifactBinding(candidate.binding);
  ...
  }) as AgentImprovementCertificateArtifactClaim;
}

function parseBody(value: unknown, location: string): AgentImprovementRunCertificateBody {
  const candidate = record(value, location);
  exactFields(candidate, [
    'certificateVersion', 'authority', 'mode', 'runId', 'lineageId', 'generation',
    'candidateId', 'parentCandidateId', 'agentIrDigest', 'changeContractDigest',
    'mutationProposalDigest', 'evaluationEpochId', 'canaryEpochId', 'disposition',
    'artifactClaims', 'artifactSetDigest', 'namedDigestClosureRules',
    'orderedDependencyClosure', 'commonCertificateDigest', 'commonReceiptIdentities',
  ...
  if (candidate.mode !== 'agent-improvement') invalid(`${location}:mode`, 'Certificate belongs to another mode');
  for (const name of [
    'runId', 'lineageId', 'candidateId', 'evaluationEpochId', 'canaryEpochId',
  ] as const) field(candidate[name], { kind: 'token' }, `${location}:${name}`);
  field(candidate.parentCandidateId, { kind: 'token', nullable: true }, `${location}:parentCandidateId`);
  field(candidate.generation, { kind: 'integer', minimum: 1 }, `${location}:generation`);
  for (const name of [
    'agentIrDigest', 'changeContractDigest', 'mutationProposalDigest',
    'artifactSetDigest', 'commonCertificateDigest', 'receiptChainDigest',
  ...
  if (!Array.isArray(candidate.namedDigestClosureRules) || candidate.namedDigestClosureRules.length !== 0) {
    invalid(`${location}:namedDigestClosureRules`, 'Agent Improvement declares an empty plain-digest closure map');
  }
  ...
  return candidate as unknown as AgentImprovementRunCertificateBody;
}

export function parseAgentImprovementRunCertificate(
  value: unknown,
...
export function parseAgentImprovementCertificateBundle(
  value: unknown,
): AgentImprovementCertificateBundle {
  ...
  return Object.freeze({
    bundleVersion: 1,
    certificate: parseAgentImprovementRunCertificate(candidate.certificate),
    receipts: Object.freeze(candidate.receipts.map((receipt, index) => (
      parseAgentImprovementTransitionReceipt(receipt, `bundle:receipts:${index}`)
    ))),
