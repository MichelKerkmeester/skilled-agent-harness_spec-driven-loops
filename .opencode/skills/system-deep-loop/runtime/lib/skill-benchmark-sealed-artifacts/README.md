---
title: "skill-benchmark-sealed-artifacts: Sealed Artifacts"
description: "Skill Benchmark sealed-artifacts layer for the durable runtime event spine."
trigger_phrases:
  - "skill-benchmark-sealed-artifacts"
  - "skill benchmark sealed-artifacts"
---
...
This folder materializes canonical, sealed artifacts from lane evidence for the Skill Benchmark runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `skill-benchmark-artifact-material.ts` | Canonical materialization of lane artifact content. |
| `skill-benchmark-sealed-artifact-types.ts` | Sealed artifact kinds, envelopes and storage contract types. |
| `skill-benchmark-sealed-artifacts.ts` | Sealing, parsing, storage and verification helpers. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `skill-benchmark-sealed-artifacts.ts` |
...
This folder owns the sealed artifacts step for Skill Benchmark. It does not replace the shared substrate or decide consumer-facing workflow policy.
