// MODULE: Model Benchmark Certificate Validation
...
const TRANSITIONS = new Set(Object.values(ModelBenchmarkTransitionKinds));
...
const SELECTION_STATES = new Set(['BLOCKED', 'INCONCLUSIVE', 'TIE', 'WINNER']);
const RANKING_STATES = new Set(['blocked', 'ranked', 'unranked']);
...
function parseIdentity(value: unknown, location: string): ModelBenchmarkReceiptIdentity {
  const candidate = record(value, location);
  exactFields(candidate, [
    'identityVersion', 'runId', 'transitionKind', 'logicalOperationId',
    'effectIdempotencyKey', 'digest',
  ...
  field(candidate.digest, { kind: 'digest' }, `${location}:digest`);
  return candidate as unknown as ModelBenchmarkReceiptIdentity;
}
...
function parseReceiptFacts(
  ...
  return candidate as unknown as ModelBenchmarkTransitionReceiptFacts;
}

export function parseModelBenchmarkTransitionReceipt(
  value: unknown,
  location = 'receipt',
): ModelBenchmarkTransitionReceipt {
  const candidate = record(value, location);
...
function parseArtifactClaim(
  ...
  const binding = parseModelBenchmarkSealedArtifactBinding(candidate.binding);
  ...
  }) as ModelBenchmarkCertificateArtifactClaim;
}

function parseBody(value: unknown, location: string): ModelBenchmarkRunCertificateBody {
  const candidate = record(value, location);
  exactFields(candidate, [
    'certificateVersion', 'authority', 'mode', 'runId', 'lineageId', 'generation',
    'evaluatorEpochId', 'canaryEpochId', 'matrixProfileId', 'matrixDigest',
    'workloadProfileDigest', 'disposition', 'selectionState', 'winnerModelId',
    'matrixCoverage', 'rankingState', 'blockingCellKeys', 'blockingVetoCodes',
    'unresolvedEvidenceRefs',
    'artifactClaims', 'artifactSetDigest', 'namedDigestClosureRules',
    'orderedDependencyClosure', 'recipeQualifiedDigest', 'runManifestQualifiedDigest',
    'modelCellInputQualifiedDigests', 'rawObservationQualifiedDigests',
    'scoringMatrixQualifiedDigest', 'commonAnchorQualifiedDigest',
    'diagnosticSelectionQualifiedDigest', 'validityEvidenceQualifiedDigests',
    'contaminationEvidenceQualifiedDigests', 'workloadEvidenceQualifiedDigests',
    'selectionEvidenceQualifiedDigest', 'commonCertificateDigest', 'commonReceiptIdentities',
  ...
  if (candidate.certificateVersion !== 1) invalid(`${location}:certificateVersion`, 'Unsupported certificate version');
  if (candidate.authority !== 'dark-evidence-only') invalid(`${location}:authority`, 'Certificate cannot carry authority');
  if (candidate.mode !== 'model-benchmark') invalid(`${location}:mode`, 'Certificate belongs to another mode');
  for (const name of [
    'runId', 'lineageId', 'evaluatorEpochId', 'canaryEpochId', 'matrixProfileId',
  ] as const) field(candidate[name], { kind: 'token' }, `${location}:${name}`);
  field(candidate.winnerModelId, { kind: 'token', nullable: true }, `${location}:winnerModelId`);
  field(candidate.generation, { kind: 'integer', minimum: 1 }, `${location}:generation`);
  for (const name of [
    'matrixDigest', 'workloadProfileDigest', 'artifactSetDigest',
  ...
  field(candidate.selectionState, { kind: 'enum', values: SELECTION_STATES }, `${location}:selectionState`);
  field(candidate.rankingState, { kind: 'enum', values: RANKING_STATES }, `${location}:rankingState`);
  field(candidate.matrixCoverage, { kind: 'ratio' }, `${location}:matrixCoverage`);
  ...
  for (const name of [
    'recipeQualifiedDigest', 'runManifestQualifiedDigest',
    'scoringMatrixQualifiedDigest', 'commonAnchorQualifiedDigest',
    'diagnosticSelectionQualifiedDigest', 'selectionEvidenceQualifiedDigest',
  ] as const) field(candidate[name], { kind: 'qualified-digest' }, `${location}:${name}`);
  for (const name of [
    'modelCellInputQualifiedDigests', 'rawObservationQualifiedDigests',
    'validityEvidenceQualifiedDigests', 'contaminationEvidenceQualifiedDigests',
    'workloadEvidenceQualifiedDigests',
  ] as const) {
    scalarArray(candidate[name], { kind: 'qualified-digest' }, `${location}:${name}`, false);
  }
  for (const name of [
    'blockingCellKeys', 'blockingVetoCodes', 'unresolvedEvidenceRefs',
  ...
  if (!Array.isArray(candidate.namedDigestClosureRules) || candidate.namedDigestClosureRules.length !== 0) {
    invalid(`${location}:namedDigestClosureRules`, 'Model Benchmark declares an empty plain-digest closure map');
  }
  ...
  return candidate as unknown as ModelBenchmarkRunCertificateBody;
}

export function parseModelBenchmarkRunCertificate(
  value: unknown,
...
export function parseModelBenchmarkCertificateBundle(
  value: unknown,
): ModelBenchmarkCertificateBundle {
  ...
  return Object.freeze({
    bundleVersion: 1,
    certificate: parseModelBenchmarkRunCertificate(candidate.certificate),
    receipts: Object.freeze(candidate.receipts.map((receipt, index) => (
      parseModelBenchmarkTransitionReceipt(receipt, `bundle:receipts:${index}`)
    ))),
