---
title: "model-benchmark-certificates: Certificates"
description: "Model Benchmark certificates layer for the durable runtime event spine."
trigger_phrases:
  - "model-benchmark-certificates"
  - "model benchmark certificates"
---
...
This folder issues and validates transition receipts and run certificates for the Model Benchmark runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `model-benchmark-certificate-types.ts` | Lane-specific types, constants and contract values. |
| `model-benchmark-certificate-validation.ts` | Strict parsing and validation for the lane's durable records. |
| `model-benchmark-certificates.ts` | Certificate issuance and verification logic. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `model-benchmark-certificates.ts` |
...
This folder owns the certificates step for Model Benchmark. It does not replace the shared substrate or decide consumer-facing workflow policy.
