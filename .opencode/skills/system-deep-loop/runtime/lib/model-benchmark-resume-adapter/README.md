---
title: "model-benchmark-resume-adapter: Resume Adapter"
description: "Model Benchmark resume-adapter layer for the durable runtime event spine."
trigger_phrases:
...
This folder maps durable lane evidence into resumable continuity decisions for the Model Benchmark runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `model-benchmark-resume-adapter.ts` | Resume request parsing, continuity decisions and adapter implementation. |
...
|---|---|
| Public API | `index.ts` |
| Primary implementation | `model-benchmark-resume-adapter.ts` |
...
This folder owns the resume adapter step for Model Benchmark. It does not replace the shared substrate or decide consumer-facing workflow policy.
