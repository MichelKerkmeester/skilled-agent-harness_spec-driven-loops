---
title: "deep-ai-council-ledger-schema: Ledger Schema"
description: "Deep Ai Council ledger schema layer for the durable runtime event spine."
trigger_phrases:
  - "deep-ai-council-ledger-schema"
  - "deep-ai-council ledger schema"
---
...
This folder defines the lane's durable event stems, payloads and compatibility types for the Deep Ai Council runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-ai-council-ledger-schema.ts` | event definitions, payload construction and schema validation |
| `deep-ai-council-ledger-types.ts` | Lane-specific types, constants and contract values. |
...
| Primary implementation | `deep-ai-council-ledger-schema.ts` | event definitions, payload construction and schema validation. |
...
This folder owns the ledger schema step for Deep Ai Council. It does not replace the shared substrate or decide consumer-facing workflow policy.
