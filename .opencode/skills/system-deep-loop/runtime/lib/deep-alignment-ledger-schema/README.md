---
title: "deep-alignment-ledger-schema: Ledger Schema"
description: "Deep Alignment ledger schema layer for the durable runtime event spine."
trigger_phrases:
  - "deep-alignment-ledger-schema"
  - "deep-alignment ledger schema"
---
...
This folder defines the lane's durable event stems, payloads and compatibility types for the Deep Alignment runtime lane. It keeps lane-specific contracts beside the shared ledger, event-envelope and replay primitives, then publishes the supported surface through the public barrel.
...
| File | Responsibility |
|---|---|
| `deep-alignment-ledger-schema.ts` | event definitions, payload construction and schema validation |
| `deep-alignment-ledger-types.ts` | Lane-specific types, constants and contract values. |
...
| Primary implementation | `deep-alignment-ledger-schema.ts` | event definitions, payload construction and schema validation. |
...
This folder owns the ledger schema step for Deep Alignment. It does not replace the shared substrate or decide consumer-facing workflow policy.
