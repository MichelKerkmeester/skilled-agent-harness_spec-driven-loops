// ───────────────────────────────────────────────────────────────────
// MODULE: Deep Alignment Independent Mode Gate
// ───────────────────────────────────────────────────────────────────
...
import type { DeepAlignmentResumeDecision } from '../deep-alignment-resume-adapter/index.js';
import type { DeepAlignmentParityReceipt } from '../deep-alignment-shadow-parity/index.js';
import type { JsonObject } from '../event-envelope/index.js';
import type {
  DeepAlignmentGateDisposition,
  DeepAlignmentGateInputDisposition,
  DeepAlignmentGateInputKind,
  DeepAlignmentGateReasonCode,
  DeepAlignmentGateVerdict,
  DeepAlignmentLifecycleEvidenceKind,
  DeepAlignmentModeGateInput,
  DeepAlignmentModeGateResult,
  DeepAlignmentModeMigrationCertificate,
  DeepAlignmentRollbackWindowEvaluation,
  DeepAlignmentRollbackWindowInput,
  DeepAlignmentRollbackWindowResult,
} from './types.js';
...
const ROLLBACK_WINDOW_KEYS = Object.freeze([
  'openedAt',
  'evaluatedAt',
  'executions',
...
const ROLLBACK_EXECUTION_KEYS = Object.freeze([
  'executionId',
  'authorityState',
  'authorityEpoch',
  'result',
...
const REQUIRED_LIFECYCLE: readonly DeepAlignmentLifecycleEvidenceKind[] = Object.freeze([
  'init',
  'authority',
  'lane-scope',
  'applicability',
...
const INPUT_ORDER: readonly DeepAlignmentGateInputKind[] = Object.freeze([
  'shadow_parity',
  'sealed_artifacts',
...
function resumeDecisionSemanticView(decision: DeepAlignmentResumeDecision): JsonObject {
  const {
  ...
  const invalidation = {
    ...
    reopenedLaneIds: [...decision.invalidation.reopenedLaneIds].sort(),
    ...
    reopenedProofIds: [...decision.invalidation.reopenedProofIds].sort(),
  };
  return {
    ...semanticDecision,
    compatibility,
...
function isReadyResumeDecision(decision: DeepAlignmentResumeDecision): boolean {
  return SAFE_RESUME_REUSE_DISPOSITIONS.has(decision.reuseDisposition)
    && SAFE_RESUME_COMPATIBILITY_OUTCOMES.has(decision.compatibilityOutcome)
...
function disposition(
  input: DeepAlignmentGateInputKind,
  value: DeepAlignmentGateDisposition,
  reasonCode: DeepAlignmentGateReasonCode | null,
...
function fail(
  input: DeepAlignmentGateInputKind,
  reasonCode: DeepAlignmentGateReasonCode,
...
function malformedResult(): DeepAlignmentModeGateResult {
  const dispositions = Object.freeze(INPUT_ORDER.map((input) => (
    fail(input, 'EVIDENCE_MALFORMED')
  )));
...
function overallVerdict(
  dispositions: readonly DeepAlignmentGateInputDisposition[],
): DeepAlignmentGateVerdict {
...
function validateTopLevel<TState extends JsonObject>(
  input: DeepAlignmentModeGateInput<TState>,
): boolean {
  return hasExactKeys(input, GATE_INPUT_KEYS)
...
async function evaluateParity<TState extends JsonObject>(
  ...
  disposition: DeepAlignmentGateInputDisposition;
  receipts: readonly DeepAlignmentParityReceipt[];
  ...
  try {
    ...
    const reported = parseDeepAlignmentModeGateInput(parity.modeGateInput);
    const requiredFixtureIds = parity.manifest.cases
      .filter((entry) => entry.mode === 'deep-alignment')
      .map((entry) => entry.caseId)
      .sort();
    const receipts = parity.receipts.map((entry) => (
      parseDeepAlignmentParityReceipt(entry, parity.manifest)
    ));
    ...
    if (
      ...
      || parity.manifest.baseSha !== input.baseSha
      || reported.manifestDigest !== parity.manifest.manifestDigest
      || receipts.some((entry) => (
    ...
    try {
      audit = await readAuthorizationAudit(
        parity.authorizationAuditRootDirectory,
        parity.authorizationAuditLedgerId,
      );
    } catch {
      return {
    ...
    const allowed = audit.entries.filter((entry) => (
      ...
      && entry.decision.mode === 'deep-alignment'
    ...
    // The audit authenticates evidence admission, not a production authority transfer.
...
async function evaluateSealed<TState extends JsonObject>(
  ...
  try {
    const sealedArtifacts = input.sealedArtifacts;
    const verified = await Promise.all(sealedArtifacts.bindings.map((binding) => (
      readDeepAlignmentArtifact(sealedArtifacts.store, binding)
    )));
    const required = new Set(DEEP_ALIGNMENT_ARTIFACT_KIND_REGISTRY.map(
    ...
    // Store reads prove immutable bytes and binding ownership, not semantic completion.
...
async function evaluateCertificates<TState extends JsonObject>(
  ...
  try {
    ...
    const bundle = parseDeepAlignmentCertificateBundle(
    ...
    // Offline replay proves this evidence bundle only; it cannot activate a writer.
...
function evaluateLifecycle<TState extends JsonObject>(
  ...
  try {
    ...
    // Resume parity cannot prove that an unrelated live process has quiesced.
...
async function evaluateRollback<TState extends JsonObject>(
  input: DeepAlignmentModeGateInput<TState>,
): Promise<DeepAlignmentGateInputDisposition> {
  ...
  try {
    ...
    if (
      ...
      || facts.bindings.receiptContract !== input.sharedContractDigest
      || facts.bindings.candidate !== input.writeSetDigest
    ) return fail('rollback_readiness', 'EVIDENCE_STALE');
    // A drill verifies reversible evidence in isolation; it performs no live rollback.
...
function validRollbackExecution(value: unknown): boolean {
    return hasExactKeys(value, ROLLBACK_EXECUTION_KEYS)
    && isToken(value.executionId)
    && typeof value.authorityState === 'string'
    && AUTHORITY_STATES.has(value.authorityState)
    && Number.isSafeInteger(value.authorityEpoch)
    && (value.authorityEpoch as number) > 0
    && typeof value.result === 'string'
    && EXECUTION_RESULTS.has(value.result)
    && isDigest(value.certificateDigest);
}

export function evaluateDeepAlignmentRollbackWindow(
  input: DeepAlignmentRollbackWindowInput,
): DeepAlignmentRollbackWindowResult {
  try {
    if (
      !hasExactKeys(input, ROLLBACK_WINDOW_KEYS)
      || typeof input.openedAt !== 'string'
      || typeof input.evaluatedAt !== 'string'
      || !Array.isArray(input.executions)
      || input.executions.some((entry) => !validRollbackExecution(entry))
    ...
    for (const entry of validExecutions) {
      const executionIds = executionIdsByCertificate.get(entry.certificateDigest)
        ?? new Set<string>();
      executionIds.add(entry.executionId);
      executionIdsByCertificate.set(entry.certificateDigest, executionIds);
      const certificateDigests = certificateDigestsByExecution.get(entry.executionId)
        ?? new Set<string>();
    ...
    const minimumsMet = elapsedCalendarDays >= DEEP_ALIGNMENT_ROLLBACK_MINIMUM_DAYS
      && successful >= DEEP_ALIGNMENT_ROLLBACK_MINIMUM_SUCCESSFUL_EXECUTIONS;
    const extended = input.lowTraffic || input.unresolvedEvidenceCount > 0;
    const core = Object.freeze({
      state: extended
        ? 'extended' as const
        : minimumsMet
          ? 'eligible_to_close' as const
          : 'open' as const,
      elapsedCalendarDays,
      successfulAuthoritativeExecutions: successful,
      minimumCalendarDays: DEEP_ALIGNMENT_ROLLBACK_MINIMUM_DAYS,
    ...
    // Eligibility is evidence only; this substrate cannot close the window or change authority.
    return Object.freeze({
      ...core,
...
export class DeepAlignmentModeMigrationGate {
  public async evaluate<TState extends JsonObject>(
    input: DeepAlignmentModeGateInput<TState>,
  ): Promise<DeepAlignmentModeGateResult> {
    try {
  ...
  async #evaluate<TState extends JsonObject>(
    input: DeepAlignmentModeGateInput<TState>,
  ): Promise<DeepAlignmentModeGateResult> {
    const windowResult = evaluateDeepAlignmentRollbackWindow(input.rollbackWindow);
    const window: DeepAlignmentRollbackWindowEvaluation | null = windowResult.state === 'invalid'
      ? null
      : windowResult;
    ...
    const topLevelValid = validateTopLevel(input);
    if (!topLevelValid) verdict = 'blocked';
    if (topLevelValid && input.unresolvedRiskIds.length > 0 && verdict === 'pass') {
      verdict = 'blocked';
    }
    if (verdict !== 'pass' || window === null || input.rollback === null) {
      return Object.freeze({ verdict, dispositions, certificate: null });
    }
    if (certificates.certificateDigest === null || certificates.replayFingerprint === null) {
      return malformedResult();
    }
    const certificateCore = Object.freeze({
      ...
      streamDigests: Object.freeze(
        parity.receipts.map((entry) => entry.ledgerStreamDigest).sort(),
      ),
    ...
    const certificate: DeepAlignmentModeMigrationCertificate = Object.freeze({
      ...certificateCore,
