// ───────────────────────────────────────────────────────────────────
// MODULE: Deep Improvement Common Rollback Switch
// ───────────────────────────────────────────────────────────────────

import { parseDeepImprovementCommonResumeDecision } from '../deep-improvement-common-resume-adapter/index.js';
...
import { DeepImprovementCommonModeMigrationGate } from './mode-gate.js';
import { DEEP_IMPROVEMENT_COMMON_ROLLBACK_GATE_SCHEMA_VERSION } from './types.js';
...
import type {
  ...
  DeepImprovementCommonRollbackCertificate,
  DeepImprovementCommonRollbackDecision,
  DeepImprovementCommonRollbackDenialReasonCode,
  DeepImprovementCommonRollbackOperation,
  DeepImprovementCommonRollbackRequest,
  DeepImprovementCommonRollbackSwitchOptions,
} from './types.js';
...
const ALLOWED_ROLLBACK_OPERATIONS: ReadonlySet<DeepImprovementCommonRollbackOperation> = new Set([
  'rollback',
  'unquarantine',
  'verifier-replacement',
  'authority-restoration',
]);
const DEEP_IMPROVEMENT_COMMON_LEDGER_WRITER_RESOURCE = canonicalizeProtectedResource({
...
function hasOnlyRequestKeys(value: object): boolean {
  return Object.keys(value).every((key) => REQUEST_KEYS.has(key));
}

function denied(
  reasonCode: DeepImprovementCommonRollbackDenialReasonCode,
): DeepImprovementCommonRollbackDecision {
...
async function reverifyMigrationCertificate(
  certificate: DeepImprovementCommonModeMigrationCertificate | null | undefined,
  gateInput: DeepImprovementCommonRollbackRequest['gateInput'],
): Promise<DeepImprovementCommonModeMigrationCertificate | null> {
  ...
  try {
    ...
    const internallyValid = certificate.schemaVersion
      ...
      && certificate.mode === 'deep-improvement-common'
    ...
    const reverified = await new DeepImprovementCommonModeMigrationGate().evaluate(gateInput);
...
function validateResumeEvidence(
  input: NonNullable<DeepImprovementCommonRollbackRequest['resumeEvidence']>,
): Readonly<{
  value: NonNullable<DeepImprovementCommonRollbackRequest['resumeEvidence']>;
  digest: string;
}> {
  ...
  const legacyDecision = parseDeepImprovementCommonResumeDecision(input.legacyDecision);
  const ledgerDecision = parseDeepImprovementCommonResumeDecision(input.ledgerDecision);
  for (const field of [
    'legacyEventTailDigest',
    'ledgerEventTailDigest',
    'legacyFreshProjectionFingerprint',
...
export class DeepImprovementCommonRollbackSwitch {
  readonly #options: DeepImprovementCommonRollbackSwitchOptions;

  public constructor(options: DeepImprovementCommonRollbackSwitchOptions) {
    this.#options = options;
  }

  public async requestRollback(
    input: DeepImprovementCommonRollbackRequest,
  ): Promise<DeepImprovementCommonRollbackDecision> {
    let prepared: Readonly<{
      operation: DeepImprovementCommonRollbackOperation;
      authorizationRequest: NonNullable<DeepImprovementCommonRollbackRequest['authorizationRequest']>;
    ...
    try {
      if (!isPlainRecord(input) || !hasOnlyRequestKeys(input)) {
      ...
      if (
        authorizationRequest.mode !== 'deep-improvement-common'
        || authorizationRequest.authorityEpoch !== input.currentAuthority.epoch
        || authorizationRequest.evidenceDigest !== boundEvidenceDigest
      ) return denied('EVIDENCE_INCOMPLETE');
      if (
        writerResource.resourceDigest
          !== DEEP_IMPROVEMENT_COMMON_LEDGER_WRITER_RESOURCE.resourceDigest
    ...
    if (
      authorization.decision.mode !== 'deep-improvement-common'
    ...
    try {
      const rollbackLease = await this.#options.fencingCoordinator.acquire({
        ...
        ownerId: 'deep-improvement-common-external-rollback',
      ...
      try {
        ...
        superseded = Number.isSafeInteger(prepared.staleWriterLease.fenceToken)
          && prepared.staleWriterLease.fenceToken > 0
    ...
    const certificate: DeepImprovementCommonRollbackCertificate = Object.freeze({
      ...certificateCore,
