...
constructs the per-kind CLI command (`opencode run` + stdin, `claude -p`, `opencode run` +
`</dev/null`), sets
...
- Group: Fan-Out
- Feature ID: F025
- Catalog source: `feature_catalog/fanout/fanout_run.md`
- Primary source files: `scripts/fanout-run.cjs`
Related references:
- [fanout-pool.md](fanout_pool.md) — Fan-out worker pool
- [fanout-salvage.md](fanout_salvage.md) — Fan-out write-failure salvage
