...
import type { ModelBenchmarkProjectionState } from '../../lib/model-benchmark-reducers/index.js';
import type {
...
describe('model benchmark certificates', () => {
  it('issues and independently verifies one complete dark evidence bundle', async () => {
    const fixture = await validModelScenario();
    const result = await verifyModelBenchmarkCertificateOffline(fixture.verification);
    ...
    expect(fixture.bundle.receipts.map((receipt) => receipt.facts.transitionKind)).toEqual([
      'benchmark_started',
      'model_cell_started',
      'model_cell_completed',
      'score_matrix_reduced',
      'judge_calibrated',
      'contamination_checked',
      'diagnostic_tail_allocated',
      'selection_proposed',
    ]);
    expect(fixture.bundle.receipts.every((receipt) => (
      receipt.facts.commonReceiptIdentities.length
      === fixture.bundle.commonBundle.receipts.length
    ))).toBe(true);
  });

  it('declares no untyped plain-digest closure fields', () => {
    expect(MODEL_BENCHMARK_NAMED_DIGEST_CLOSURE_RULES).toEqual([]);
  });

  it.each(ownedReferenceCases)(
    'rejects a wrong-kind sealed reference at $field through the offline verifier',
    async (testCase) => {
      const fixture = await validModelScenario();
      const result = await verifyOwnedReferenceTamper(fixture, testCase, 'wrong-kind');
  ...
  it.each(ownedReferenceCases)(
    'rejects a never-sealed expected-kind reference at $field through the offline verifier',
    async (testCase) => {
      const fixture = await validModelScenario();
      const result = await verifyOwnedReferenceTamper(fixture, testCase, 'missing');
      expect(result).toMatchObject({
        verdict: 'invalid',
        code: ModelBenchmarkCertificateFailureCodes.ARTIFACT_CLOSURE_INVALID,
      });
    },
  );
  ...
  it('returns unverifiable when a separate offline store lacks referenced bytes', async () => {
    const fixture = await validModelScenario();
    const prunedStore = createModelBenchmarkSealedArtifactStore({
      rootDirectory: temporaryRoot('pruned-offline-store'),
    });
    ...
    expect(result).toMatchObject({
      ...
      code: ModelBenchmarkCertificateFailureCodes.ARTIFACT_MISSING,
    });
  });

  it('rejects a forged wrong-kind artifact claim through verified store reads', async () => {
    const fixture = await validModelScenario();
    const recipe = fixture.artifacts.bindings.find(
      (binding) => binding.artifactKind === ModelBenchmarkArtifactKinds.BENCHMARK_RECIPE,
    );
    if (!recipe) throw new Error('Missing recipe binding');
    const bundle = bundleWithArtifactBinding(
      fixture,
    ...
    expect(result.verdict).toBe('invalid');
    expect([
      ModelBenchmarkCertificateFailureCodes.SCHEMA_INCOMPATIBLE,
  ...
  it('returns unverifiable for a forged never-published artifact claim', async () => {
    const fixture = await validModelScenario();
    const material = fixture.artifacts.materials.get(
      ModelBenchmarkArtifactKinds.SELECTION_EVIDENCE,
    );
    const original = fixture.artifacts.bindings.find(
      (binding) => binding.artifactKind === ModelBenchmarkArtifactKinds.SELECTION_EVIDENCE,
    );
    if (!material || !original) throw new Error('Missing selection fixture');
    const derived = fixture.common.store.derive(
      ModelBenchmarkArtifactKinds.SELECTION_EVIDENCE,
      { ...material, artifactId: 'selection-never-published' },
      { canonicalizationVersion: original.reference.canonicalization_version },
    );
    const fabricated: ModelBenchmarkSealedArtifactBinding = {
      ...original,
  ...
  it('rejects sealed artifact evidence from a different evaluator epoch', async () => {
    const fixture = await validModelScenario();
    const artifactKind = ModelBenchmarkArtifactKinds.BENCHMARK_RECIPE;
    const material = fixture.artifacts.materials.get(artifactKind);
    if (!material) throw new Error('Missing benchmark recipe material');
    const mismatchedBinding = await sealModelBenchmarkArtifact(
      fixture.common.store,
      artifactKind,
      {
        ...material,
    ...
    expect(result).toMatchObject({
      verdict: 'invalid',
      code: ModelBenchmarkCertificateFailureCodes.EPOCH_MISMATCH,
    });
  });

  it('resolves external typed evidence references through the real store', async () => {
    const fixture = await validModelScenario();
    const material = fixture.artifacts.materials.get(
      ModelBenchmarkArtifactKinds.VALIDITY_EVIDENCE,
    );
    if (!material) throw new Error('Missing validity material');
    const neverPublished = fixture.common.store.derive(
      InitialArtifactKinds.FIXTURE,
      { fixture: 'never-published-judge-calibration' },
    ).reference;
    const altered = {
      ...material,
      artifactId: 'validity-missing-judge-calibration',
      judgeCalibrationReference: neverPublished,
    } as ModelBenchmarkArtifactMaterial;
  ...
  it('rejects reordered transition receipts', async () => {
    const fixture = await validModelScenario();
    const [first, second, ...rest] = fixture.bundle.receipts;
    if (!first || !second) throw new Error('Missing transition receipts');
    const bundle = {
      ...fixture.bundle,
      receipts: [second, first, ...rest],
    };
    const result = await verifyModelBenchmarkCertificateOffline(
      verificationWithBundle(fixture, bundle),
    );
    expect(result).toMatchObject({
      verdict: 'invalid',
      code: ModelBenchmarkCertificateFailureCodes.RECEIPT_CHAIN_INVALID,
    });
  });
    ...
    const receipts = fixture.bundle.receipts.map((receipt, index) => (
      index === 1
        ? {
          ...
          facts: {
            ...receipt.facts,
            predecessorReceiptDigests: [digest('fabricated-predecessor')],
          },
        }
        : receipt
    ));
    const result = await verifyModelBenchmarkCertificateOffline(
      verificationWithBundle(fixture, { ...fixture.bundle, receipts }),
    );
    expect(result).toMatchObject({
      verdict: 'invalid',
      code: ModelBenchmarkCertificateFailureCodes.RECEIPT_CHAIN_INVALID,
  ...
  it('rejects a receipt whose result event was never authorized', async () => {
    const fixture = await validModelScenario();
    const receipts = fixture.bundle.receipts.map((receipt, index) => (
      index === 0
        ? {
          ...
          facts: {
            ...receipt.facts,
            resultEventId: 'event-never-authorized',
          },
        }
        : receipt
    ));
    ...
    expect(result).toMatchObject({
      verdict: 'invalid',
      code: ModelBenchmarkCertificateFailureCodes.TRANSITION_UNAUTHORIZED,
    });
  });

  it('rejects a changed replay input instead of trusting the certificate fingerprint', async () => {
    const fixture = await validModelScenario();
    const replay = {
      ...
      replay: {
        ...
        replayInputDigests: {
          ...fixture.verification.replay.replay.replayInputDigests,
          initial_state: digest('changed-initial-state'),
        },
      },
    };
    const result = await verifyModelBenchmarkCertificateOffline({
    ...
    expect(result).toMatchObject({
      verdict: 'invalid',
      code: ModelBenchmarkCertificateFailureCodes.FINGERPRINT_MISMATCH,
    });
  });

  it('rejects stale sealed evidence at verification time', async () => {
    const fixture = await validModelScenario();
    const result = await verifyModelBenchmarkCertificateOffline({
      ...fixture.verification,
      verificationTime: '2026-07-23T11:00:00.000Z',
    });
    expect(result).toMatchObject({
      verdict: 'invalid',
      code: ModelBenchmarkCertificateFailureCodes.ARTIFACT_STALE,
    });
  });

  it.each([
    ['blocked hard floor', { hardFloorStatus: 'fail' }],
    ['unresolved validity finding', { includeValidityUnknown: true }],
    ['contaminated evidence', { contaminationStatus: 'confirmed' }],
    ['late superseding contamination', { lateContaminationStatus: 'confirmed' }],
    ['aborted terminal run', { terminalOutcome: 'aborted' }],
    ['incomplete max-iteration run', { omitRunClosed: true }],
  ] as const)(
    'refuses trusted completion for %s',
    async (_label, options) => {
      const fixture = await validModelScenario();
      await expect(modelScenario(options, fixture.common)).rejects.toMatchObject({
        code: ModelBenchmarkCertificateFailureCodes.LIFECYCLE_INVALID,
      });
    },
  );

  it.each([
    ['rejected trial', { trialOutcome: 'rejected' }],
    ['unknown-effect trial', { trialOutcome: 'unknown' }],
  ] as const)(
    'fails closed for a %s lifecycle',
    async (_label, options) => {
      const fixture = await validModelScenario();
      try {
        await modelScenario(options, fixture.common);
        throw new Error('Expected non-happy lifecycle rejection');
      } catch (error: unknown) {
        expect(error).toMatchObject({
          code: expect.stringMatching(
            /^(LIFECYCLE_INVALID|PROJECTION_INVALID|cell-transition-invalid)$/,
          ),
        });
      }
    },
  ...
  it('rejects mutated sealed bytes through the offline verifier', async () => {
    const fixture = await validModelScenario();
    const selection = fixture.artifacts.bindings.find(
      (binding) => binding.artifactKind === ModelBenchmarkArtifactKinds.SELECTION_EVIDENCE,
    );
    if (!selection) throw new Error('Missing selection binding');
    const paths = fixture.common.store.inspectPaths(selection.reference);
    chmodSync(paths.blobPath, 0o600);
    writeFileSync(paths.blobPath, '{"mutated":true}', 'utf8');
    const result = await verifyModelBenchmarkCertificateOffline(fixture.verification);
    expect(result).toMatchObject({
      verdict: 'invalid',
      code: ModelBenchmarkCertificateFailureCodes.ARTIFACT_MUTATED,
    });
  });
});
