...
} from '../../lib/skill-benchmark-resume-adapter/index.js';
...
async function buildSkillScenario(
  options: { readonly blockedGold?: boolean } = {},
...
function validSkillScenario(): Promise<SkillScenario> {
  validSkillFixture ??= buildSkillScenario();
  return validSkillFixture;
}

const skillScenario = validSkillScenario;
...
function commonResumeFacts(): readonly SkillBenchmarkResumeComponentFact[] {
  return Object.freeze([
    Object.freeze({
...
function placeholderResumeFacts(): readonly SkillBenchmarkResumeComponentFact[] {
  return Object.freeze([
    ...commonResumeFacts(),
...
function migrationRegistry(
  entries: SkillBenchmarkMigrationRegistry['entries'] = Object.freeze([]),
): SkillBenchmarkMigrationRegistry {
  ...
  return Object.freeze({
    ...body,
    registryDigest: skillBenchmarkMigrationRegistryDigest(body),
  });
}
...
function resumeAdapter(
  ...
  void ignored;
  return new SkillBenchmarkResumeAdapter({
    verification,
...
function resumeRequest(
  fixture: SkillScenario,
  currentInputs: readonly SkillBenchmarkResumeComponentFact[],
  registry: SkillBenchmarkMigrationRegistry,
  checkpoint: SkillBenchmarkProjectionCheckpoint | null = null,
): SkillBenchmarkResumeRequest {
  const body = fixture.bundle.certificate.body;
  return Object.freeze({
    runId: RUN_ID,
    idempotencyKey: 'skill-benchmark-resume:' + digest({
      inputs: currentInputs,
      registry: registry.registryDigest,
      checkpoint,
    }),
    requestedAt: RESUME_REQUEST_TIME,
    resumeReason: 'recover the offline-verified Skill Benchmark run',
    currentInputs,
    migrationRegistry: registry,
    lease: {
      runId: RUN_ID,
      leaseId: 'skill-benchmark-resume-lease',
...
async function persistedResumeFacts(
  fixture: SkillScenario,
  effects: EffectHarness,
): Promise<readonly SkillBenchmarkResumeComponentFact[]> {
...
function changedFact(
  facts: readonly SkillBenchmarkResumeComponentFact[],
...
function registryFor(
  prior: readonly SkillBenchmarkResumeComponentFact[],
  current: readonly SkillBenchmarkResumeComponentFact[],
  component: SkillBenchmarkResumeCompatibilityComponent,
...
async function appendEffectEvent(
  ...
  const prepared = prepareEventWrite({
    ...
    producer: { name: 'skill-benchmark-resume-effect-fixture', version: '1' },
    authority_epoch: 1,
...
function rewriteSkillEvent(
  ...
  return prepareSkillBenchmarkEvent({
    stem: event.payload.stem,
    scope: event.payload.scope,
    prevEventHash: event.payload.prevEventHash,
    replay: event.payload.replay,
  ...
  createSkillBenchmarkEventRegistry()).envelope;
}
...
async function withReplayEvents(
  ...
  const { ledger, registry } = await authorizedSkillLedger(events);
  return Object.freeze({
    ...fixture,
    events,
    verification: {
...
describe('skill benchmark resume adapter', () => {
  it.each([
    {
      label: 'exact-reuse',
      component: null,
    ...
    async ({ label, component, outcome, expected }) => {
      const fixture = await skillScenario();
  ...
  it('classifies real divergence and rejects caller and registry assertions', async () => {
    ...
    expect(() => parseSkillBenchmarkResumeRequest({
  ...
  it('commits reducer, adapter, schema, and codec versions to the fingerprint', async () => {
    ...
    expect(fingerprint.reducerVersion).toBe(SKILL_BENCHMARK_REDUCER_VERSION);
    expect(fingerprint.adapterVersion).toBe(SKILL_BENCHMARK_RESUME_ADAPTER_VERSION);
    expect(fingerprint.schemaVersion).toBe(SKILL_BENCHMARK_PROJECTION_SCHEMA_VERSION);
    ...
    for (const field of [
      ...
      expect(skillBenchmarkResumeFingerprintDigest({
        ...fingerprint,
  ...
  it('rejects a self-consistent checkpoint with the wrong authenticated cursor', async () => {
    ...
    const changedTail = checkpoint.sourceTails.find(
      (tail) => tail.streamId === SKILL_STREAM_ID,
    );
    if (changedTail === undefined || changedTail.sequence < 2) {
      throw new Error('Expected a non-empty Skill Benchmark checkpoint tail');
    }
    const earlier = fixture.events.find((event) => (
      event.stream_id === changedTail.streamId
      && event.stream_sequence === changedTail.sequence - 1
    ));
    if (earlier === undefined) throw new Error('Expected earlier stream event');
    const sourceTails = checkpoint.sourceTails.map((tail) => (
      tail.streamId === changedTail.streamId
        ? {
          streamId: tail.streamId,
          sequence: earlier.stream_sequence,
          eventId: earlier.event_id,
          eventDigest: digest(earlier),
        }
        : tail
    ));
    const forgedCheckpoint: SkillBenchmarkProjectionCheckpoint = {
  ...
  it('rejects a certificate frontier that differs from the real ledger tail', async () => {
    ...
    const body = {
      ...
      finalHeadHash: digest('different-final-frontier'),
    };
    ...
    const result = await resumeAdapter(
      ...
      ...resumeRequest(fixture, prior, registry),
      priorRunBundle,
      lease: {
        ...resumeRequest(fixture, prior, registry).lease,
        certificateDigest: digest(body),
      },
    });
    expect(result.decision.disposition).toBe('rebuild-required');
  ...
  it.each([
    {
      ...
      rewrite: (event: SkillBenchmarkLedgerEvent) => rewriteSkillEvent(event, {
    ...
    async ({ label, rewrite }) => {
      ...
      if (last === undefined) throw new Error('Expected final event');
      const divergent = await withReplayEvents(
        fixture,
        Object.freeze([
  ...
  it('exports the full continuity ladder and authenticated resume frontier', async () => {
    expect(SKILL_BENCHMARK_CONTINUITY_LADDER.map((row) => row.step)).toEqual([
      'run-identity',
      'treatment-design',
      'scenario-setup',
      'skill-path',
    ...
    expect(result.continuity?.streamTails).toEqual(result.checkpoint?.sourceTails);
