...
function dataFor<TStem extends DeepReviewEventStem>(
  ...
  const data: Readonly<Record<DeepReviewEventStem, JsonObject>> = {
    ...
    'deep_review.migration': {
      mode: 'review',
      legacyArtifacts: ['deep-research-config.json', 'deep-research-state.jsonl'],
      canonicalArtifacts: ['deep-review-config.json', 'deep-review-state.jsonl'],
    },
    ...
    'deep_review.synthesis_incomplete': {
      ...
      invariantFailures: ['missing_synthesis_artifacts'],
      missingArtifacts: [{ name: 'registry', path: '/tmp/review/deep-review-findings-registry.json' }],
      registryFindingCount: null,
      iterationFindingCount: 1,
      identifiableFindingCount: 1,
      missingStructuredFindingCount: 0,
      stateParseFailureCount: 0,
    },
