// ───────────────────────────────────────────────────────────────────
// MODULE: Agent Improvement Rollback Gate Tests
// ───────────────────────────────────────────────────────────────────
...
import {
  ...
  AgentImprovementRollbackSwitch,
...
} from '../../lib/agent-improvement-rollback-gate/index.js';
import {
...
import type {
  ...
  AgentImprovementRollbackRequest,
  AgentImprovementRollbackWindowExecution,
} from '../../lib/agent-improvement-rollback-gate/index.js';
import type {
...
function temporaryRoot(label: string): string {
  const root = mkdtempSync(join(tmpdir(), `agent-improvement-rollback-${label}-`));
...
function successfulExecutions(count = 5): AgentImprovementRollbackWindowExecution[] {
...
function emptyModeGateInput(): AgentImprovementModeGateInput<JsonObject> {
  ...
  return {
    ...
    versions: {
      eventEnvelopeVersion: 1,
      eventSchemaVersion: 'agent-improvement-event@1',
      reducerVersion: 'agent-improvement-reducer@1',
      projectionVersion: 'agent-improvement-projection@1',
    },
...
describe('agent improvement rollback window', () => {
  it('inherits the common minimums without forking policy', () => {
    expect(AGENT_IMPROVEMENT_ROLLBACK_MINIMUM_DAYS).toBe(14);
    expect(AGENT_IMPROVEMENT_ROLLBACK_MINIMUM_SUCCESSFUL_EXECUTIONS).toBe(5);
    expect(AGENT_IMPROVEMENT_ROLLBACK_MINIMUM_DAYS)
      .toBe(DEEP_IMPROVEMENT_COMMON_ROLLBACK_MINIMUM_DAYS);
    expect(AGENT_IMPROVEMENT_ROLLBACK_MINIMUM_SUCCESSFUL_EXECUTIONS)
  ...
  it('requires both calendar days and distinct successful executions', () => {
    const tooEarly = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
    ...
    const tooFew = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
    ...
    const eligible = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
      executions: successfulExecutions(),
      unresolvedEvidenceCount: 0,
  ...
  it('deduplicates repeated rows before the success threshold', () => {
    const execution = successfulExecutions(1)[0]!;
    const result = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
  ...
  it('deduplicates shared execution identities across changed certificate digests', () => {
    const result = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
  ...
  it('deduplicates shared certificates across changed execution labels', () => {
    const result = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
  ...
  it('collapses transitive execution and certificate aliases into one component', () => {
    const executions = [
      { ...successfulExecutions(1)[0]!, executionId: 'execution-a', certificateDigest: hash('certificate-a') },
      { ...successfulExecutions(1)[0]!, executionId: 'execution-a', certificateDigest: hash('certificate-b') },
      { ...successfulExecutions(1)[0]!, executionId: 'execution-b', certificateDigest: hash('certificate-b') },
      { ...successfulExecutions(1)[0]!, executionId: 'execution-c', certificateDigest: hash('certificate-c') },
    ];
    const result = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
  ...
  it.each([
    ...
    ['abstained', 'abstained'],
  ] as const)('does not count a %s execution', (_label, result) => {
    const executions: AgentImprovementRollbackWindowExecution[] = [
      ...successfulExecutions(4),
      {
        ...
        authorityEpoch: 9,
        result,
        certificateDigest: hash(`non-success-${result}`),
      },
    ];
    const evaluation = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
      executions,
      unresolvedEvidenceCount: 0,
  ...
  ] as const)('extends for %s after both minimums are met', (_label, lowTraffic, unresolvedEvidenceCount) => {
    const result = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
      executions: successfulExecutions(),
  ...
  it('commits the complete evidence input rather than only the summary', () => {
    const first = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
      executions: successfulExecutions(),
      unresolvedEvidenceCount: 0,
    ...
    const second = evaluateAgentImprovementRollbackWindow({
      openedAt: '2026-07-01T00:00:00.000Z',
      evaluatedAt: '2026-07-15T00:00:00.000Z',
      executions: successfulExecutions().map((entry, index) => ({
        ...entry,
  ...
  it.each([
    ...
    [{ openedAt: 'not-a-date', evaluatedAt: '2026-07-15T00:00:00Z', executions: [], unresolvedEvidenceCount: 0, lowTraffic: false }, 'invalid opening'],
    [{ openedAt: '2026-07-15T00:00:00Z', evaluatedAt: '2026-07-01T00:00:00Z', executions: [], unresolvedEvidenceCount: 0, lowTraffic: false }, 'reverse time'],
    [{ openedAt: '2026-07-01T00:00:00Z', evaluatedAt: '2026-07-15T00:00:00Z', executions: [], unresolvedEvidenceCount: -1, lowTraffic: false }, 'negative unresolved count'],
    [{ openedAt: '2026-07-01T00:00:00Z', evaluatedAt: '2026-07-15T00:00:00Z', executions: [], unresolvedEvidenceCount: 0, lowTraffic: 'false' }, 'wrong low-traffic type'],
  ] as const)('rejects malformed standalone window input: %s', (input) => {
    expect(() => evaluateAgentImprovementRollbackWindow(input as never)).toThrow(TypeError);
  });
});

describe('independent fail-closed gate', () => {
  it('maps every absent evidence bucket to its typed disposition', async () => {
    const result = await new AgentImprovementModeMigrationGate().evaluate(emptyModeGateInput());
  ...
  it('drives the real offline verifier and preserves its failure as a typed denial', async () => {
    ...
    const result = await new AgentImprovementModeMigrationGate().evaluate(input);
  ...
  it('does not adopt a self-reported passing parity handoff', async () => {
    const manifest = compileParityCaseManifest({
      ...
      cases: [{
        ...
        projectionIds: ['agent-improvement'],
        timeoutMs: 1_000,
        terminationPolicy: 'bounded',
      }],
    });
    const computed = createAgentImprovementModeGateInput({
      manifest,
  ...
  it('still rejects forged green evidence when a real gateway audit contains an allow', async () => {
    ...
    const request = await createFixtureRequest(
      ...
      { mode: 'agent-improvement', evidenceDigest: hash('forged-attestation') },
    );
    expect((await harness.gateway.authorize(request)).verdict).toBe('allow');
    const manifest = compileParityCaseManifest({
      baseSha: BASE_SHA,
      baselineRows: [{ scenarioId: 'forged', mode: 'agent-improvement', contractDigest: hash('contract'), disposition: 'protected' }],
      cases: [{ caseId: 'forged', scenarioId: 'forged', mode: 'agent-improvement', contractDigest: hash('contract'), requiredObservations: ['ordered-transitions'], projectionIds: ['agent'], timeoutMs: 1_000, terminationPolicy: 'bounded' }],
    });
    const result = await new AgentImprovementModeMigrationGate().evaluate({
      ...emptyModeGateInput(),
      parity: {
  ...
  ] as const)('turns malformed top-level %s into typed denials without throwing', async (field, value) => {
    const input = { ...emptyModeGateInput(), [field]: value };
    await expect(new AgentImprovementModeMigrationGate().evaluate(input))
  ...
  it.each([
    ...
    await expect(new AgentImprovementModeMigrationGate().evaluate(input as never))
      .resolves.toMatchObject({ certificate: null });
  });
  ...
  it('rejects unknown top-level fields instead of treating them as inert', async () => {
    ...
    } as unknown as AgentImprovementModeGateInput<JsonObject>;
    const result = await new AgentImprovementModeMigrationGate().evaluate(input);
    expect(result.certificate).toBeNull();
    expect(result.dispositions.every((entry) => entry.reasonCode === 'EVIDENCE_MALFORMED'))
      .toBe(true);
  ...
  it('converts circular evidence into typed malformed dispositions', async () => {
    const input = emptyModeGateInput() as AgentImprovementModeGateInput<JsonObject> & {
      circular?: unknown;
    };
...
describe('rollback switch caller boundary', () => {
  async function rollbackSwitch(authority: AuthoritySnapshot = FIXTURE_AUTHORITY) {
    ...
    return {
      harness,
      fencingCoordinator,
      rollbackSwitch: new AgentImprovementRollbackSwitch({
  ...
  ] as const)('returns the typed denial %s without throwing', async (request, reasonCode) => {
    const fixture = await rollbackSwitch();
    await expect(fixture.rollbackSwitch.requestRollback(request as AgentImprovementRollbackRequest))
  ...
  it('rejects unknown request fields before any gateway decision', async () => {
    ...
    } as unknown as AgentImprovementRollbackRequest);
  ...
  it('proves the real coordinator advances a durable high-water mark above a stale token', async () => {
    ...
    const resource = {
      ...
      components: { writerId: 'agent-improvement-ledger-writer' },
      atomicityDomain: AtomicityDomains.SINGLE_HOST_FILESYSTEM,
    } as const;
    const stale = await fixture.fencingCoordinator.acquire({
      resource,
      ownerId: 'stale-agent-improvement-writer',
      correlationId: 'stale-agent-improvement-writer',
      ttlMs: 60_000,
    ...
    const current = await fixture.fencingCoordinator.acquire({
      resource,
      ownerId: 'rollback-agent-improvement-writer',
      correlationId: 'rollback-agent-improvement-writer',
      ttlMs: 60_000,
      acquireTimeoutMs: 1_000,
    });
  ...
  it('rejects a stale authority token at the real transition gateway', async () => {
    ...
    const request: TransitionAuthorizationRequest = await createFixtureRequest(
      ...
      'stale-agent-improvement-token',
      { mode: 'agent-improvement', authorityEpoch: 1 },
    );
  ...
  it('keeps external policy denial distinct from malformed caller denial', async () => {
    ...
    const request = await createFixtureRequest(
      ...
      event,
      policies,
      'self-authorized-agent-improvement-recovery',
      { mode: 'agent-improvement', capabilityId: 'self-authorized-recovery' },
    );
