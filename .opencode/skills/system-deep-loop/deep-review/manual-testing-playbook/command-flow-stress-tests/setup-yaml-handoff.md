...
This scenario checks the command-owned entrypoint. The differentiator is whether Call B binds target, mode, dimensions, spec folder, max iterations, and convergence before handing off to `deep-review-auto.yaml`.
...
{
  field "auto_yaml_loaded" "deep-review-auto.yaml|AUTONOMOUS|auto mode" /tmp/cp-052-B-combined.txt
