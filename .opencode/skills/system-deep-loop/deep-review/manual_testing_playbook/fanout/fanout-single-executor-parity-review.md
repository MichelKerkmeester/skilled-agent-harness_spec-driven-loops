...
1. `bash: grep -n "if_absent\|skip_when\|resolveArtifactRoot" .opencode/commands/deep/assets/deep_review_auto.yaml | head -20`
...
5. Repeat for `deep_review_confirm.yaml`.
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_review_auto.yaml` | `step_resolve_artifact_root` if_absent branch, `step_fanout_spawn` skip_when, `step_fanout_merge` skip_when |
| `.opencode/commands/deep/assets/deep_review_confirm.yaml` | Same (confirm variant) |
...
- Scenario file path: `manual_testing_playbook/fanout/fanout-single-executor-parity-review.md`
