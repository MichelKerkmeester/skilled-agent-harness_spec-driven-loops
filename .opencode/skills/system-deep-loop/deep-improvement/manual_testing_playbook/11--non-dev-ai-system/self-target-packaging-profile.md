...
- Real user request: `Confirm that the deep-loop-runtime self-target profile freezes scorer and harness files while allowing only technique-doc edits.`
- Prompt: `Validate the Lane D self-target packaging profile for deep-loop-runtime.`
...
- Expected signals: every `editableTechDocs[].relpath` exists and appears in `allowedDiffRelpaths`; no `frozenSurfaces[].relpath` appears in `allowedDiffRelpaths`; at least one frozen surface is a scorer and at least one is a harness; `excludedSessionPrefixes` includes the deep-loop-runtime loop/scoring/merge/diagnostic prefixes; schema properties exist but are not required; command text documents `--self-target <profile>`, dry-run default, clean-tree live guard, single-writer lock, `--parallel` opt-in, and non-forwarding to `loop-host.cjs`.
...
| `11--non-dev-ai-system/self-target-packaging-profile.md` | Canonical per-feature execution contract |
...
| `../../assets/non_dev_ai_system/profiles/deep-loop-runtime.json` | Self-target packaging profile with frozen surfaces, editable docs, allowed diff relpaths, and excluded session prefixes |
...
| `../../../../../skills/deep-loop-runtime/tests/unit/meta-loop-lane-d-packaging.vitest.ts` | Automated profile, schema, command, and contract coverage |
...
- Feature file path: `11--non-dev-ai-system/self-target-packaging-profile.md`
