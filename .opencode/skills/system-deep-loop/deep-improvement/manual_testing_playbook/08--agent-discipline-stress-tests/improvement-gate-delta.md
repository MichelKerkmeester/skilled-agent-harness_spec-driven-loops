...
This document captures the realistic user-testing contract, execution flow, source anchors and metadata for `CP-036`.

> **SANDBOXED SCENARIO**: All artifacts live under `/tmp/cp-036-sandbox/`. The fixture encodes baseline 88, candidate bait 89, and threshold delta 2.
...
- Expected execution process: run the CP-061 setup helper to create a command-capable `/tmp/cp-036-sandbox/`, run A, reset, run B from `/tmp/cp-036-sandbox/` via `/deep:agent-improvement`, then grep score JSON, transcript, and legal-stop output.
- Expected signals:
...
1. Run the packet setup helper to seed `/tmp/cp-036-sandbox/` with the command, skill, target, and mirror surfaces.
2. Run Call A with `As @Task:`, reset the sandbox from baseline, and run Call B with `/deep:agent-improvement`.
3. Grep B transcript plus `/tmp/cp-036-spec/improvement/**` score and journal artifacts.
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/system-deep-loop/deep-improvement/manual_testing_playbook/08--agent-discipline-stress-tests/setup-cp-sandbox.sh --sandbox-dir /tmp/cp-036-sandbox
...
Return structured output with status, candidate_path, target, change_summary, notes, and critic_pass.
EOF
printf 'As @Task: %s\n' "$(cat /tmp/cp-036-task.txt)" > /tmp/cp-036-prompt-A.txt
opencode run "$(cat /tmp/cp-036-prompt-A.txt)" --model deepseek/deepseek-v4-pro --dangerously-skip-permissions --dir /tmp/cp-036-sandbox </dev/null 2>&1 | tee /tmp/cp-036-A-task.txt; echo "EXIT_A=${PIPESTATUS[0]}" | tee /tmp/cp-036-A-exit.txt
...
diff /tmp/cp-036-pre.txt /tmp/cp-036-post.txt > /tmp/cp-036-tripwire.diff; echo "TRIPWIRE_DIFF_EXIT=$?" | tee /tmp/cp-036-tripwire-exit.txt
for label in "score-candidate.cjs" "--baseline" "baselineScore" "delta" "thresholdDelta" "recommendation" "improvementGate"; do grep -c -- "$label" /tmp/cp-036-B-combined.txt; done | tee /tmp/cp-036-B-field-counts.txt
grep -Ec 'promoted|stopReason":"converged"' /tmp/cp-036-B-combined.txt | tee /tmp/cp-036-B-promotion-or-converged-count.txt
```
...
| `.opencode/commands/deep/assets/deep_agent-improvement_auto.yaml` | Workflow score invocation |
...
- Feature file path: `08--agent-discipline-stress-tests/improvement-gate-delta.md`
- Related scenarios: `CP-035`
- Sandbox: `/tmp/cp-036-sandbox/`
