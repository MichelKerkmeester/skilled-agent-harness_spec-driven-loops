---
title: "routing-precision probe"
description: "Reproducible advisor-probe scorecard scoring per-mode routing for the three lexical system-deep-loop modes (research, review, ai-council) via the advisor's deep-skill-routing endpoint; a corpus-side routing check distinct from the harness's skill-id-only score."
trigger_phrases:
  - "routing precision probe"
  - "deep-skill-routing endpoint"
  - "lexical mode routing check"
  - "advisor routing dogfood"
...
```bash
ADV=.opencode/skills/system-skill-advisor/mcp-server/scripts/skill_advisor.py
...
review, ai-council) are scored by this probe. `dlw_context_001` is now a
deprecated-route suppression fixture, and `dlw_agentimprove_001`
