...
Mode resolution is closed-set. `VALID_MODES` introduced this two-mode split (`agent-improvement` and `model-benchmark`); the set has since grown additively to four — `agent-improvement`, `model-benchmark`, `skill-benchmark`, and `non-dev-ai-system-refine` — with the two newer modes documented in [mode-wiring.md](../skill_benchmark/mode_wiring.md) and [guarded-refine-loop.md](../non_dev_ai_system/guarded_refine_loop.md). `resolveMode()` treats an undefined mode as `agent-improvement`. An unknown mode value writes a warning to stderr and falls back to `agent-improvement`, so a typo degrades to the safe default rather than failing the run. The model-benchmark plan also requires `--profile` and `--outputs-dir`, and the host aborts remaining steps when an earlier step exits non-zero.
