...
description: "Verify a plain code-edit prompt routes to sk-code and not to the deep-loop-workflows hub."
version: "1.1.0.0"
---
...
This scenario verifies that the deep-loop hub does not capture ordinary implementation work. A plain code-edit prompt should route to `sk-code`, not to `deep-loop-workflows`.
...
- Hub source: deep-loop-workflows is for active deep-loop workflows, not single quick read/edit work.
...
2. `.opencode/skills/deep-loop-workflows/SKILL.md` contains the When NOT to Use rule.
...
| Step | Signal |
|---|---|
| 1 | Advisor top skill is `sk-code`, not `deep-loop-workflows`. |
...
- **FAIL** iff `deep-loop-workflows` wins, a deep-loop mode is selected, or the AI starts a loop for the plain code edit.
...
- `.opencode/skills/deep-loop-workflows/SKILL.md` - When NOT to Use and hub scope.
- `.opencode/skills/deep-loop-workflows/mode-registry.json` - finite list of deep-loop modes.
