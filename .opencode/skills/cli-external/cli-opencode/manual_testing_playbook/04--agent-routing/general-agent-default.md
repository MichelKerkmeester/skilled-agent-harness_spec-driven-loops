...
- Expected signals: Dispatch exits 0. JSON event stream includes a `tool.call` event whose payload references the Read tool. Session.completed or session metadata references `general` as the agent slug. Response references content from `.opencode/skills/cli-external/cli-opencode/SKILL.md`. Runtime under 90 seconds.
