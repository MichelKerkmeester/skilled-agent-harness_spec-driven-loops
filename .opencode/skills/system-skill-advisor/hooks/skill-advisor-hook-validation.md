...
Load this reference after changes to runtime hook registration, `advisor_recommend`, `advisor_validate`, the OpenCode bridge, shared rendering, thresholds, diagnostics, or rollback behavior.

### Core Principle

Hook validation must prove shared render parity, prompt-safe telemetry, disable-flag rollback, and durable diagnostics without exposing raw prompt text.
...
Durable JSONL writes only occur when `SKILL_ADVISOR_DEBUG=1` is set; otherwise the sink early-returns.

```bash
export SKILL_ADVISOR_DEBUG=1
ls -la "${TMPDIR:-/tmp}"/speckit-skill-advisor-metrics/*.jsonl
head -3 "${TMPDIR:-/tmp}"/speckit-skill-advisor-metrics/*.jsonl
```
