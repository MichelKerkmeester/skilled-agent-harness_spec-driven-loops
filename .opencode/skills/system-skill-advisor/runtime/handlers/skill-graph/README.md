...
description: "Skill-graph handlers for scanning, relationship queries, status and validation."
trigger_phrases:
...
`skill-graph/` owns the handler layer for the SQLite-backed skill graph. The handlers parse tool arguments, call the package-local graph library and return redacted response envelopes.
...
Keep response formatting in `response-envelope.ts`. Database and traversal logic belongs in `../../lib/skill-graph/`. Handler modules stay thin and do not write graph tables directly.
