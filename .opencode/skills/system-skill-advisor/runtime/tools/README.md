---
title: "Skill Advisor Tools: Command Descriptors"
description: "Tool descriptor modules for the advisor and skill graph command surface."
trigger_phrases:
  - "advisor tools"
...
# Skill Advisor Tools: Command Descriptors
...
`runtime/tools/` contains `ToolDefinition` descriptors for the advisor command surface. Each descriptor defines a command name, prompt-safe description and JSON Schema input contract.
...
| `skill-graph-tools.ts` | Defines `skill_graph_scan`, `skill_graph_query`, `skill_graph_status`, `skill_graph_validate` and `skill_graph_propagate_enhances`. |
...
```bash
python3 .opencode/skills/sk-doc/scripts/validate_document.py .opencode/skills/system-skill-advisor/runtime/tools/README.md
```
