...
`stress-test/skill-advisor/` contains pressure tests for the skill-advisor package. The suites cover recommendation handlers, scorer fusion, daemon lifecycle, derived metadata sync, generation cache invalidation, hooks parity, compatibility, Python runners and advisor diagnostics.
...
| `mcp-diagnostics-stress.vitest.ts` | Test file | Advisor diagnostics stress coverage. |
...
```bash
python3 .opencode/skills/sk-doc/scripts/validate_document.py .opencode/skills/system-skill-advisor/runtime/stress-test/skill-advisor/README.md
```
