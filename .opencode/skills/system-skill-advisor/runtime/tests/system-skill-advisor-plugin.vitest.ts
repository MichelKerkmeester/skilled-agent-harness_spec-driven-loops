...
function mockBridgeSuccess(stdout = cliResponse()) {
...
describe('system-skill-advisor OpenCode plugin', () => {
  ...
  it('status tool returns prompt-safe fields only', async () => {
    ...
    expect(status).toContain('plugin_id=system-skill-advisor');
  ...
  it('bridge timeout returns the standalone hygiene context and never throws', async () => {
    vi.useFakeTimers();
    mockedBridge.spawn.mockImplementation(() => makeChild(cliResponse(), 5000));
  ...
  it('nonzero bridge close fail-opens to standalone hygiene context even when stdout looks ok', async () => {
    mockedBridge.spawn.mockImplementation(() => makeChild(cliResponse(), 0, 2));
    const hooks = await makePlugin();
  ...
  it('dedups concurrent identical requests into one advisor spawn', async () => {
    mockedBridge.spawn.mockImplementation(() => makeChild(cliResponse(), 10));
    const hooks = await makePlugin({ cacheTTLMs: 5000 });
  ...
  it('clamps oversized prompt payload before the CLI spawn', async () => {
    const hooks = await makePlugin({ cacheTTLMs: 5000 });
  ...
  it('applies configurable prompt, brief, and cache caps', async () => {
    mockBridgeSuccess(cliResponse([{ skillId: 'c'.repeat(500), confidence: 0.91, uncertainty: 0.23 }]));
    const hooks = await makePlugin({
      cacheTTLMs: 5000,
      maxPromptBytes: 4096,
      maxBriefChars: 64,
    ...
    expect(Buffer.byteLength(cliPromptAt(0), 'utf8')).toBeLessThanOrEqual(4096);
  ...
  it('keeps separate cache entries for identical prompts across workspace roots', async () => {
    ...
    const spawnCwds = mockedBridge.spawn.mock.calls.map((call) => (call[2] as { cwd?: string } | undefined)?.cwd);
    expect(spawnCwds).toEqual([workspaceA, workspaceB]);
