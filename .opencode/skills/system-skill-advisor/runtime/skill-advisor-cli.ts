...
// A cold call starts the daemon and waits for it, but never past this. The measured
...
// The CLI's own last resort when no daemon answers: the checked-in Python scorer
// still routes locally, so an unreachable daemon degrades a recommendation
// instead of failing the call and leaving every caller to build this path again.
const LOCAL_SCORER_TIMEOUT_MS = 5_000;
const LOCAL_SCORER_SOURCE = 'local-scorer';
// The same values are passed to the scorer and reported back, so the degraded
// envelope cannot advertise a threshold the scorer did not apply.
const LOCAL_SCORER_THRESHOLDS = {
  confidenceThreshold: SKILL_ADVISOR_COMPAT_CONTRACT.defaults.confidenceThreshold,
  uncertaintyThreshold: SKILL_ADVISOR_COMPAT_CONTRACT.defaults.uncertaintyThreshold,
...
interface InitializeResult {
  readonly advisorProtocol?: string;
}
...
function extractToolPayload(result: unknown): ToolPayloadResult {
  return {
    payload: result,
    isError: isErrorPayload(result),
  };
}
...
function isProtocolMismatch(error: unknown): boolean {
  if (error instanceof JsonRpcError && (error.code === -32002 || error.code === -32600)) return true;
  const message = error instanceof Error ? error.message : String(error);
...
function isRetryableError(error: unknown): boolean {
  ...
  if (error instanceof JsonRpcError && (error.code === -32001 || error.code === -32000)) return true;
  const message = error instanceof Error ? error.message : String(error);
...
function exitCodeForError(error: unknown): number {
  ...
  if (error instanceof JsonRpcError && error.code === -32601) return EXIT_USAGE;
...
function spawnLauncher(paths: RepoPaths): ChildProcess {
  ...
  child.unref();
  // Handed back so the wait below can end the moment the launcher proves it cannot
  // bring the socket up, instead of spending the rest of the budget on a dead process.
  return child;
}
...
async function waitForDaemon(
  ...
  // A launcher that has already died cannot bring the socket up, so the wait ends there
  // rather than burning the rest of the budget on a guaranteed failure. It arrives as an
  // event rather than a return value, so it is tracked as state.
  const launcherState: { exited: boolean; reason: string | null } = { exited: false, reason: null };
  if (launcher) {
    launcher.once('error', (error: Error) => {
      launcherState.exited = true;
      launcherState.reason = `daemon launcher failed to start: ${error.message}`;
    });
    launcher.once('exit', (code: number | null, signal: NodeJS.Signals | null) => {
      launcherState.exited = true;
      launcherState.reason = signal
        ? `daemon launcher exited on ${signal}`
        : `daemon launcher exited with code ${code ?? 'unknown'}`;
    });
  }

  while (Date.now() <= deadline) {
    const remaining = Math.max(1, deadline - Date.now());
    // Probe before reading the launcher state: a launcher that bridged to a daemon which
    // was already running exits on success, so its exit only matters while the socket is
    // still not answering.
    ...
    if (launcherState.exited) {
      throw new CliRetryableError(`backend unavailable: ${launcherState.reason ?? lastReason}`);
    }
...
// own timeout. Every other reason — absent socket, refused connection, a handshake that
// errored or closed before replying — is a daemon that is simply not running yet, so the
// call starts it and waits, bounded: a caller that can also answer without the daemon
// must not be held for the whole tool timeout by one that never accepts a connection.
function probeTimedOut(probe: BridgeProbeResult): boolean {
  return /timeout/i.test(probe.reason ?? '');
}
...
async function ensureDaemonReady(
  ...
  const waitMs = hasLocalFallback && !probeTimedOut(initialProbe)
    ? Math.min(timeoutMs, COLD_START_WAIT_MS)
    : timeoutMs;
  await waitForDaemon(socketPath, bridge, waitMs, spawnLauncher(paths));
}
...
async function callTool(toolName: string, args: Record<string, unknown>, timeoutMs: number, trusted: boolean, warmOnly: boolean): Promise<unknown> {
  ...
  // advisor_recommend carries a local-scorer fallback, so it starts the daemon and waits
  // for it only within the bounded cold-start window; the local scorer still answers if
  // that window closes. Tools without a fallback keep the caller's full timeout: for them
  // the daemon is the only answer.
  await ensureDaemonReady(socketPath, bridge, paths, timeoutMs, warmOnly, toolName === 'advisor_recommend');
  ...
  try {
    ...
    if (initialize.advisorProtocol !== ADVISOR_PROTOCOL_VERSION) {
      const daemonVersion = initialize.advisorProtocol ?? 'missing advisorProtocol';
      throw new CliProtocolError(
        `backend protocol mismatch: daemon "${daemonVersion}", client "${ADVISOR_PROTOCOL_VERSION}"`,
      );
    }
    return await client.request('advisor.call', {
      command: toolName,
      args,
...
async function runLocalScorer(prompt: string): Promise<readonly AdvisorRecommendation[] | null> {
  const forceLocalEnv = SKILL_ADVISOR_COMPAT_CONTRACT.forceLocalEnv;
  const previous = process.env[forceLocalEnv];
  // Force-local for this spawn alone: the scorer would otherwise spend the fallback
  // budget retrying the daemon that just proved unreachable.
  process.env[forceLocalEnv] = '1';
  try {
    const result = await runAdvisorSubprocess(prompt, {
      workspaceRoot: findRepoPaths().repoRoot,
      timeoutMs: LOCAL_SCORER_TIMEOUT_MS,
      thresholdConfig: LOCAL_SCORER_THRESHOLDS,
    });
    return result.ok ? result.recommendations : null;
  } finally {
    if (previous === undefined) {
      delete process.env[forceLocalEnv];
    } else {
      process.env[forceLocalEnv] = previous;
    }
  }
}

function degradedRecommendations(entries: readonly AdvisorRecommendation[]): Array<Record<string, unknown>> {
  return entries
    .filter((entry) => entry.passes_threshold === true)
    .map((entry) => ({
      skillId: entry.skill,
...
async function invokeAdvisorRecommendPayload(
  args: Record<string, unknown>,
  timeoutMs: number,
  trusted: boolean,
  warmOnly: boolean,
): Promise<ToolPayloadResult> {
  try {
    ...
    return {
      payload: {
        ...
        data: {
          recommendations: degradedRecommendations(recommendations),
          effectiveThresholds: LOCAL_SCORER_THRESHOLDS,
        },
      },
      isError: false,
    };
...
export async function runSkillAdvisorCli(argv: string[], io: CliIo = { stdout: process.stdout, stderr: process.stderr }): Promise<number> {
  try {
    ...
    const invoked = validated.tool.name === 'advisor_recommend'
      ? await invokeAdvisorRecommendPayload(validated.args, parsed.timeoutMs, parsed.trusted, parsed.warmOnly)
      : validated.tool.name === 'skill_graph_scan'
