...
This cookbook gives you a worked example for every query type the `skill_graph_query` MCP tool supports. The tool reads the package-local SQLite skill graph at `.opencode/skills/system-skill-advisor/mcp_server/database/skill-graph.sqlite` (see [`db-path-policy.md`](./db-path-policy.md)) and returns relationship traversals over the indexed skill metadata. For the tool signature see [`tool-ids-reference.md`](./tool-ids-reference.md) §3.

All examples assume `mk_skill_advisor` is registered (verify via `mcp__mk_skill_advisor__advisor_status({})`) and the graph is `live` (per the [freshness contract](./freshness-contract.md)).
...
**Gotcha**: Asymmetric `enhances` declarations are common. A skill can declare it enhances another without that other declaring `enhanced_by`. The propagate-enhances internal tool (see [`propagate-enhances.md`](./propagate-enhances.md)) detects and proposes the missing reciprocals.
...
- [`tool-ids-reference.md`](./tool-ids-reference.md) §3, full `skill_graph_query` schema
- [`freshness-contract.md`](./freshness-contract.md), trust state must be `live` for reliable results
