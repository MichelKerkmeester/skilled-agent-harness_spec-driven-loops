...
The freshness contract defines how the advisor reports the trustworthiness of its skill graph to callers, what callers must do at each trust state, plus what the daemon does to keep state consistent. Without this contract, callers cannot tell when a recommendation reflects current skill metadata vs. cached state from a prior generation.

This contract covers the advisor's freshness behavior. For skill-graph integrity validation see [`tool-ids-reference.md`](./tool-ids-reference.md) `skill_graph_validate`. For lane attribution mechanics see [`advisor-scorer.md`](./advisor-scorer.md).
...
- [`advisor-scorer.md`](./advisor-scorer.md), confidence calibration depends on trust state for some routes.
- [`db-path-policy.md`](./db-path-policy.md), where the SQLite file lives.
