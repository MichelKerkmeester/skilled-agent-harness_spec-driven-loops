...
Operator contract for the native-first Skill Advisor hooks across Claude Code, Copilot CLI, OpenCode and the OpenCode plugin bridge.
...
- Installing or debugging Claude, Copilot, OpenCode or OpenCode prompt-submit hooks.
...
- `mcp-server/hooks/`
- `mcp-server/lib/skill-advisor-brief.ts`
...
| Copilot CLI | `mcp-server/hooks/copilot/user-prompt-submit.ts` | managed block in `$HOME/.copilot/copilot-instructions.md`. Hook stdout remains `{}` | Copilot advisor is NEXT-PROMPT freshness: current prompt sees PRIOR turn's brief. |
