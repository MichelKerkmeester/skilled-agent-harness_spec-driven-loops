...
Current state and roadmap for lib/skill-graph/ ownership inside system-skill-advisor after migration from system-spec-kit.
...
Records the current ownership state of `lib/skill-graph/` inside `system-skill-advisor` after extraction from `system-spec-kit`.

### When to Use

- Checking whether skill graph code still needs migration.
- Explaining old documentation that mentioned `system-spec-kit` ownership.
- Confirming that reference cleanup must not move runtime code.

### Core Principle

Extraction is complete; future work here is documentation alignment unless a new ADR reopens runtime ownership.
