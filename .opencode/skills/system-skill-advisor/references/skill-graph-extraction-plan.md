...
Prior to packet 058, `SKILL.md:189` stated that `lib/skill-graph/` database and query logic remained in `system-spec-kit` until a pending packet 011 cleanup.

That statement is outdated. The extraction completed earlier and no `skill-graph` folder exists under `system-spec-kit/mcp_server/lib/`. File search across the spec-kit tree confirms the absence. The pending-packet reference is obsolete and is removed by edit A-020 in this packet's verified delta.
...
- Update `SKILL.md:189` to state that extraction is complete. (Tracked as edit A-020 in the 058 verified delta.)
- Remove the obsolete `pending packet 011 cleanup` reference from any operator or maintainer docs that still cite it.
