...
This doc captures the live baseline numbers for `advisor_validate` plus the troubleshooting playbook for slice regressions. Use it before promoting a scorer change (see [`lane-weight-tuning.md`](./lane-weight-tuning.md)) and during incident triage when routing accuracy drops in production.

Source-of-truth for the canonical baseline values is `feature_catalog/feature_catalog.md` §1 plus `mcp_server/lib/scorer/`. This doc consolidates the numbers in one place plus adds operator-facing remediation steps.
...
- [`tool-ids-reference.md`](./tool-ids-reference.md) §2, `advisor_validate` input plus output schema
...
- [`freshness-contract.md`](./freshness-contract.md), trust state affects validate outcomes
