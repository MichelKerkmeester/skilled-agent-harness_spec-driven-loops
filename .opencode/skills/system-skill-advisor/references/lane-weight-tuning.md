...
This guide tells you when, how, plus under what evidence to change scorer lane weights in `mcp_server/lib/scorer/lane-registry.ts`. Lane weights control how the advisor fuses signals from 5 lanes (explicit_author, lexical, graph_causal, derived_generated, semantic_shadow). A wrong change can regress routing across the entire skill catalog, so this guide enforces measurement before mutation.

For lane attribution mechanics see [`advisor-scorer.md`](./advisor-scorer.md). For the current canonical weights see [`feature_catalog/04--scorer-fusion/06-weights-config.md`](../feature_catalog/04--scorer-fusion/06-weights-config.md).
...
Any "Defer" condition blocks the change. If improvements on some axes come with regressions on others, document the tradeoff in `advisor-scorer.md` plus get explicit sign-off before shipping.
