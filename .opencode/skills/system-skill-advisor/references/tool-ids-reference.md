...
|---|---|---|
| `skill_graph_propagate_enhances` | Internal authoring helper for missing `enhances` edges. See [propagate-enhances.md](./propagate-enhances.md). | `requireTrustedCaller` plus workspace-escape guard |
...
Stable IDs survive migrations. The MCP server namespace may rename across packets, but the per-tool IDs do not change. See [legacy-tool-bridge.md](./legacy-tool-bridge.md) for the bridge policy.
