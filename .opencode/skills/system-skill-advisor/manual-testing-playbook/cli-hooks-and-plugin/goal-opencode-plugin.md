...
| LLM verifier cannot call client | `OPENCODE_GOAL_VERIFIER=llm` never reaches `ctx.client.session.promptAsync` | Inspect `client: ctx?.client` threading into `maybeVerifyGoal`. |
| Live tool path unavailable | OpenCode cannot invoke `opencode_goal`/`opencode_goal_status` in the run | Mark live portion `SKIP` with the blocker; keep direct plugin test evidence. |
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/commands/goal_opencode.md:
  ...
  Line 40: - `set <objective>` routes to `opencode_goal` with `action: "set"` and `objective: REST`.

  Line 41: - Bare text routes to `opencode_goal` with `action: "set"` and `objective: QUERY`.

  Line 42: - `clear`, `complete`, and `pause [reason]` route to `opencode_goal`.

  Line 43: - Any other non-empty `QUERY` is treated as bare goal text and routes to `opencode_goal` with `action: "set"` and `objective: QUERY`.

  Line 49: Your FIRST and ONLY action is the single tool call selected below. Do NOT read files, glob, grep, or explore the repository - the `opencode_goal` / `opencode_goal_status` tools own all goal state and session resolution. Make the call immediately, then print its result verbatim.

  Line 53: 1. If `ARGS_PRESENT=false`, call `opencode_goal_status({})` and print its result exactly.

  Line 54: 2. If `FIRST` is `show`, call `opencode_goal_status({})` and print its result exactly.

  Line 55: 3. If `FIRST` is `set`, require non-empty `REST`, then call `opencode_goal({ action: "set", objective: REST })`.

  Line 56: 4. If `FIRST` is `clear`, call `opencode_goal({ action: "clear" })`.

  Line 57: 5. If `FIRST` is `complete`, call `opencode_goal({ action: "complete" })`.

  Line 58: 6. If `FIRST` is `pause`, call `opencode_goal({ action: "pause", reason: REST })`.

  Line 59: 7. For any other non-empty `QUERY`, call `opencode_goal({ action: "set", objective: QUERY })`.

  Line 83: - Reads go through `opencode_goal_status`; mutations go through `opencode_goal`.
```
...
```text
opencode-goal tool-path tests passed
```
...
Set via `opencode_goal({ action: "set", objective: "Finish the documentation propagation task" })`:
...
Show via `opencode_goal_status({})` while active:
...
Pause via `opencode_goal({ action: "pause", reason: "waiting for user approval" })`:
...
Complete via `opencode_goal({ action: "complete" })`:
...
Show via `opencode_goal_status({})` after complete:
...
Clear via `opencode_goal({ action: "clear" })`:
...
```text
Found 20 matches
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/plugins/tests/opencode-goal-continuation.test.cjs:
  Line 38:   const originalAutonomy = process.env.OPENCODE_GOAL_AUTONOMY;
  Line 41:     delete process.env.OPENCODE_GOAL_AUTONOMY;
  Line 45:         async promptAsync() {
  Line 62:     assert.equal(result.reason, 'autonomy_disabled');
  Line 68:       reason: 'autonomy_disabled',
  Line 72:     process.env.OPENCODE_GOAL_AUTONOMY = 'passive';
  Line 87:     process.env.OPENCODE_GOAL_AUTONOMY = 'smoke';
  Line 98:     assert.equal(result.decision, 'would_fire');
  Line 121:       decision: 'would_fire',
  Line 126:     process.env.OPENCODE_GOAL_AUTONOMY = 'active';
  ...
  Line 433:     restoreEnv('OPENCODE_GOAL_AUTONOMY', originalAutonomy);
```
...
```text
Found 12 matches
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/plugins/tests/opencode-goal-lifecycle.test.cjs:
...
```text
Found 11 matches
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/plugins/tests/opencode-goal-state.test.cjs:
