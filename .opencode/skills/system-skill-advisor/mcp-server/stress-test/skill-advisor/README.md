...
`stress-test/skill-advisor/` contains pressure tests for the skill-advisor package. The suites cover recommendation handlers, scorer fusion, daemon lifecycle, derived metadata sync, generation cache invalidation, hooks parity, compatibility, Python runners and MCP diagnostics.
