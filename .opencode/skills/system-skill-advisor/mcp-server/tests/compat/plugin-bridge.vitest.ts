...
const bridgePath = resolve(repoRoot, '.opencode/skills/system-skill-advisor/mcp-server/plugin-bridges/system-skill-advisor-bridge.mjs');
...
describe('system-skill-advisor plugin bridge compat path', () => {
  it('publishes one effective-threshold contract for the native bridge route', async () => {
    ...
    expect(parsed.brief).toContain('Comment hygiene [HARD BLOCK]:');
  ...
  it('renders native uncertainty from the recommendation instead of a literal zero', async () => {
    ...
    const contractPath = resolve(repoRoot, '.opencode/skills/system-skill-advisor/mcp-server/schemas/compat-contract.json');
