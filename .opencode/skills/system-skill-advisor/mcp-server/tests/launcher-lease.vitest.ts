...
describe('mk-skill-advisor launcher lease', () => {
  ...
  function createWorkspace(options: { ignoreChildSigterm?: boolean } = {}): Workspace {
    ...
    const advisorServer = join(root, '.opencode/skills/system-skill-advisor/mcp-server/dist/mcp-server/advisor-server.js');
    const leaseModule = join(root, '.opencode/skills/system-skill-advisor/mcp-server/dist/mcp-server/lib/daemon/lease.js');
  ...
  // Skill-advisor must reject concurrent launcher spawns; closes the 3-zombie observation from 2026-05-18T16:47Z.
  ...
  // Different workspaces sharing one resolved DB directory share one lease.
  ...
  // Symlink aliases to one real DB directory share one lease boundary.
  ...
  // Live legacy daemon lease blocks rolling-start duplicates.
