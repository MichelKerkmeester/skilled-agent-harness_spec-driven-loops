...
function sanitizeAdvisorErrorMessage(value) {
...
export function classifyAdvisorException(error) {
...
export function classifyAdvisorFailure(errorCode, errorDetails) {
    ...
    if (normalizedCode === 'JSON_PARSE_FAILED'
        || normalizedCode === 'INVALID_JSON_SHAPE'
        || normalizedCode === 'PARSE_FAIL') {
        return { errorClass: 'parse', ...(errorMessage ? { errorMessage } : {}) };
    }
    if (normalizedCode === 'PYTHON_MISSING'
