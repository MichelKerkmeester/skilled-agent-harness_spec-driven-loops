// ───────────────────────────────────────────────────────────────
// MODULE: Embedders — Ollama adapter
// ───────────────────────────────────────────────────────────────

import type { EmbedderAdapter, EmbedderInputType, EmbedderOptions } from '../adapter.js';
...
export class OllamaAdapter implements EmbedderAdapter {
  ...
  async embed(texts: ReadonlyArray<string>, options: EmbedderOptions = {}): Promise<Float32Array[]> {
    ...
    if (rows.length !== texts.length) {
      throw new OllamaAdapterError(`Ollama returned ${rows.length} embeddings for ${texts.length} inputs`);
    }
  ...
  private applyPrefix(text: string, inputType: EmbedderInputType): string {
    const prefix = inputType === 'query' ? this.prefixQuery : this.prefixDocument;
    return prefix ? `${prefix}${text}` : text;
  }

  private prepareInput(text: string, inputType: EmbedderInputType): string {
