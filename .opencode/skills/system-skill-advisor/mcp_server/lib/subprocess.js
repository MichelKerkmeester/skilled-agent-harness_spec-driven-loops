...
function thresholdArgs(thresholdConfig) {
...
function parseRecommendations(stdout) {
...
export async function runAdvisorSubprocess(prompt, options) {
    const timeoutMs = options.timeoutMs ?? defaultTimeoutMs();
    const scriptPath = options.scriptPath
        ?? join(options.workspaceRoot, '.opencode', 'skills', 'system-skill-advisor', 'mcp_server', 'scripts', 'skill_advisor.py');
    const startedAt = performance.now();
