...
- Expected execution process: Read `.opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` for `step_classify_session`; read `.opencode/commands/deep/start-context-loop.md` for lineage mode documentation; verify `assets/deep_context_config.json` template exists; read SKILL.md §3 for session lifecycle.
...
|---|---|---|---|---|---|---|---|---|
| FS-001 | Frontier Initialization | Verify session classification gates file writes | `Validate that the deep-context initialization classifies session state correctly and only writes files on a fresh run.` | 1. `rg "step_classify_session" .opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` -> 2. `rg "fresh\|resume\|completed-session\|invalid" .opencode/commands/deep/start-context-loop.md` -> 3. `ls .opencode/skills/deep-context/assets/deep_context_config.json` -> 4. `rg "step_acquire_lock" .opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` | Step 1: `step_classify_session` found in YAML; Step 2: all four session outcomes mentioned; Step 3: template file exists; Step 4: lock step found | Grep outputs from all four commands | PASS if steps 1-4 all return matches; FAIL if any step returns no output | 1. Check YAML path is correct. 2. Search for alternative session-classification wording in the YAML. 3. Confirm `deep_start-context-loop_auto.yaml` is the current YAML filename. |
...
```bash
rg "resumed\|skip.*phase_loop\|skip.*loop" .opencode/commands/deep/start-context-loop.md .opencode/skills/deep-context/SKILL.md
```
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` | Primary implementation: phase_init steps including session classification and state-file creation |
| `.opencode/skills/deep-context/assets/deep_context_config.json` | Default config template written during fresh initialization |
| `.opencode/commands/deep/start-context-loop.md` | Command entrypoint documenting lineage modes and setup phase |
| `.opencode/skills/deep-context/SKILL.md` | Skill overview documenting session lifecycle in §3 HOW IT WORKS |
