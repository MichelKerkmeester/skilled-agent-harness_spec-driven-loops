...
```bash
rg "stateLogRepaired\|stateLogDroppedBytes" .opencode/skills/deep-context/scripts/reduce-state.cjs
```
...
| File | Role |
|---|---|
| `.opencode/skills/deep-context/scripts/reduce-state.cjs` | `repairJsonlTailInline` (fallback), `loadStateSafety` (loader); `stateSafety.repairJsonlTail(stateLogPath)` call; `registry.stateLogRepair` assignment |
