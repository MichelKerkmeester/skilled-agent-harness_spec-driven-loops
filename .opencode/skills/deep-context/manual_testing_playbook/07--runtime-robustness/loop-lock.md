...
This scenario validates Loop Lock for `RUNTIME-004`. It focuses on `loop-lock.cjs` passing syntax check, exposing the `acquire`/`refresh`/`release` CLI contract, and being correctly wired via `step_acquire_lock` and `step_release_lock` in both `deep_start-context-loop_auto.yaml` and `deep_start-context-loop_confirm.yaml`.
...
```bash
rg "acquireLoopLock\|refreshLoopLock\|releaseLoopLock" .opencode/skills/deep-context/scripts/loop-lock.cjs
```
...
| File | Role |
|---|---|
| `.opencode/skills/deep-context/scripts/loop-lock.cjs` | CLI wrapper: `acquire`, `refresh`, `release` actions; loads `loop-lock.ts` via tsx CJS register |
...
| `.opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` | `step_acquire_lock` and `step_release_lock` invocations |
| `.opencode/commands/deep/assets/deep_start-context-loop_confirm.yaml` | `step_acquire_lock` and `step_release_lock` invocations |
