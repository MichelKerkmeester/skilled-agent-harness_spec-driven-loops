...
| File | Layer | Role |
|---|---|---|
| `.opencode/skills/deep-context/scripts/reduce-state.cjs` | Script | Full reducer: reads JSONL + seat files, computes agreement-weighted registry, writes dashboard |
| `.opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` | Workflow | `step_update_registry` — in-loop equivalent of reduce-state.cjs per iteration |
...
| File | Type | Role |
|---|---|---|
| `.opencode/skills/deep-context/manual_testing_playbook/05--context-report-synthesis/reduce-state-merge.md` | Manual playbook | Verifies reduce-state.cjs syntax, runs it with node --check, and verifies registry bucket structure after a simulated sweep |
