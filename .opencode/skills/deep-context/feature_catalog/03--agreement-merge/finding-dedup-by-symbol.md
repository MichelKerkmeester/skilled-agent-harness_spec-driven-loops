...
| File | Layer | Role |
|---|---|---|
| `.opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` | Workflow | `step_merge_findings` — full dedup algorithm, attribution union, agreement computation |
| `.opencode/skills/deep-context/scripts/reduce-state.cjs` | Script | Standalone reducer: same dedup + attribution + agreement logic over JSONL + seat files |
...
| File | Type | Role |
|---|---|---|
| `.opencode/skills/deep-context/manual_testing_playbook/03--agreement-merge/finding-dedup-by-symbol.md` | Manual playbook | Verifies unit_id computation, attribution union, and agreement count across two seats finding the same symbol |
