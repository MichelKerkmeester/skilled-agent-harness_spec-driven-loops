...
| File | Layer | Role |
|---|---|---|
| `.opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` | Workflow | `step_render_seat_prompts` — per-seat prompt rendering and framework resolution |
...
| `.opencode/commands/deep/start-context-loop.md` | Command | Per-model prompt framing note in PRE-BOUND SETUP ANSWERS schema and pool resolution section |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `.opencode/skills/deep-context/manual_testing_playbook/02--by-model-parallel-sweep/per-model-prompt-framework.md` | Manual playbook | Verifies framework assignment per model and four-part lineage contract presence in rendered prompts |
