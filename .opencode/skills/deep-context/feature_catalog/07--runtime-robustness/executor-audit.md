...
Both `deep_start-context-loop_auto.yaml` and `deep_start-context-loop_confirm.yaml` include a `cli_contract` block under `step_sweep_cli_pool` that mandates:
...
| File | Layer | Role |
|---|---|---|
| `.opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` | Workflow | `cli_contract` requirement in `step_sweep_cli_pool`: mandates `SPECKIT_CLI_DISPATCH_STACK` + `buildExecutorDispatchEnv` per seat |
| `.opencode/commands/deep/assets/deep_start-context-loop_confirm.yaml` | Workflow | Same `cli_contract` requirement |
...
| File | Type | Role |
|---|---|---|
| `.opencode/skills/deep-context/manual_testing_playbook/07--runtime-robustness/executor-audit.md` | Manual playbook | Verifies `SPECKIT_CLI_DISPATCH_STACK` and `executor-audit` references exist in the auto YAML |
