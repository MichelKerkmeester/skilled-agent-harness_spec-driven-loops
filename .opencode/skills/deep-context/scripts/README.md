...
description: "Code-facing README for .opencode/skills/deep-context/scripts — documents reduce-state.cjs, the agreement-weighted context reducer."
trigger_phrases:
...
This README documents the code-bearing folder `.opencode/skills/deep-context/scripts` so operators can understand its role without opening every source file first. It follows the sk-doc skill README structure while staying focused on code navigation. Behavior details live in source comments; higher-level workflow details live in the owning `SKILL.md`.
...
Start with `.opencode/skills/deep-context/SKILL.md` for runtime routing and workflow boundaries.

**Step 2: Inspect the local code.**

```bash
rg --files .opencode/skills/deep-context/scripts
```
...
```bash
node --check .opencode/skills/deep-context/scripts/reduce-state.cjs && echo "SYNTAX OK"
```
...
```bash
node -e "
const { reduceContextState } = require('./.opencode/skills/deep-context/scripts/reduce-state.cjs');
