...
| Command | Description |
|---------|-------------|
| `/deep:start-context-loop:auto "scope"` | Run autonomous deep context (no approval gates) |
| `/deep:start-context-loop:confirm "scope"` | Run with approval gates at setup, each iteration, and synthesis |
| `/deep:start-context-loop "scope"` | Ask which mode to use |
...
| `--executor=<type>` | default pool | Repeatable seat (`native`/`cli-opencode`/`cli-codex`/`cli-claude-code`/`cli-devin`); accepts `--model`, `--reasoning-effort`, `--prompt-framework`, `--label` |
...
| Scenario | Use |
|----------|-----|
| Map existing code before planning/implementing, multi-model confidence needed | `/deep:start-context-loop` |
| Single-pass codebase search, one lens | `@context` (not iterative) |
| Check prior work / saved memory only | `memory_context()` |
| Outward/web knowledge discovery | `/deep:start-research-loop` (different skill) |
| Code audit / defect finding | `/deep:start-review-loop` (different skill) |
| Exhaustive critical mapping | `/deep:start-context-loop --max-iterations 12 --convergence 0.05` |
...
```text
/deep:start-context-loop  -->  YAML workflow  -->  @deep-context seats (LEAF) + CLI seats
    |                    |                      |
...
| `/deep:start-research-loop` | Outward/web research (different skill) |
| `/deep:start-review-loop` | Code audit / defect finding (different skill) |
