...
| Owner | Writes |
|-------|--------|
| Host (`/deep:start-context-loop` command / `@deep-context` orchestrator) | config, state-log appends, strategy, iteration markdown, deltas, collected seat files, the Context Report and its JSON companion, coverage-graph upserts |
