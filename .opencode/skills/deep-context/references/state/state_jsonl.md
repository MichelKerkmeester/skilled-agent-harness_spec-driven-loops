...
- `.opencode/commands/deep/assets/deep_start-context-loop_auto.yaml` — every `append_jsonl` / `append_to_jsonl` / `append_iteration_jsonl` record (the authoritative JSON shapes).
- `.opencode/skills/deep-context/scripts/reduce-state.cjs` — how the reducer consumes these records (`parseJsonlDetailed`, the `type === 'iteration'` / `type === 'event'` split).
