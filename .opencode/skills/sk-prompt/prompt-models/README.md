...
**Step 4: Cross to the executor.** Open `references/pattern_index.md`. It locates each mechanic (context budget, output verification, quota fallback, permissions schema) by path in `sk-prompt-models` or `cli-opencode`. Load the mechanic your dispatch needs and combine it with the prompt-craft from step 3.
...
Reach for `sk-prompt-models` before you dispatch any small model through `cli-opencode`. Reach for it when you need to know which framework a model wants or what its gotchas are. Reach for it when you are authoring a prompt-pack and need the scaffold shape.
