...
| --- | --- | --- | --- |
| Context budget engine (per-model defaults, fit-to-budget truncation, priority eviction) | `prompt-models` | [`./context_budget.md`](./context_budget.md) | shipped |
| Output verification pipeline (compile → execute → smoke-test → lint) | `prompt-models` | [`./output_verification.md`](./output_verification.md) | shipped |
| Confidence-scoring rubric (verification scoring formula) | `prompt-models` | [`../assets/confidence_scoring_rubric.md`](../assets/confidence_scoring_rubric.md) | shipped |
| Per-model budget defaults (3 required + 1 optional stub) | `prompt-models` | [`../assets/per_model_budgets.json`](../assets/per_model_budgets.json) | shipped |
| Quota-pool-aware fallback (no same-pool retry; fail-fast when no different-pool target) | `prompt-models` | [`./quota_fallback.md`](./quota_fallback.md) | shipped |
...
1. **Populate the registry entry** at `prompt-models/assets/model_profiles.json` — set the executor entry's `provider` + `quota_pool` (nested under `executors[]`), plus the model-level `context_length`, `tool_calling`, `primary_quota_pool`, and `recommended_frameworks` fields.
