...
const RIG_ROOT = path.resolve(PACKET_ROOT, '..', 'eval-rig');
...
async function scoreVariantFixture(opts) {
  ...
  if (process.env.EVAL_LOOP_EXTRACT === 'true') {
    const extractScriptPath = path.resolve(PACKET_ROOT, '..', '005-extraction-rerun', 'scripts', 'extract-files-from-markdown.cjs');
