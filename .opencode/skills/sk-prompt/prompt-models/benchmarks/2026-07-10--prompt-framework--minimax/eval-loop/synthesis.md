...
- `cli-opencode/assets/prompt-templates.md` — add a MiniMax M2.7 section using the **TIDD-EC** scaffold with **dense** pre-planning (Task / Instructions / Do's / Don'ts / Examples / Context + a 4-5 step pre-plan).
- `cli-opencode/assets/prompt-quality-card.md` — mark TIDD-EC as the empirical default for `minimax/MiniMax-M2.7`; RCAF as fallback.
- `sk-prompt-models/references/pattern-index.md` — add a MiniMax prompt-framework row → cli-opencode canonical location.
- `sk-prompt/assets/cli-prompt-quality-card.md` — note MiniMax → TIDD-EC + dense pre-plan.
