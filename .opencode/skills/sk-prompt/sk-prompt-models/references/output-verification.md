...
Recipe-level opt-in via a `verification_enabled` field depends on the executor's `--agent-config` parser accepting unknown top-level fields; where the parser is strict (rejecting unknown fields), recipe-level opt-in is not available. `post-dispatch-validate.ts` treats absent config as `verification_enabled: false` — the only behavior today. The pipeline below describes the intended design for when the config is supplied through a supported channel.
...
Phase 004 adapts that idea in two layers:
...
SmallCode's source pipeline is five stages because it includes structural validation before compile. The operator-facing pipeline is documented as four delivery gates:
...
The validator does not execute arbitrary code directly. That keeps post-dispatch validation safe by default and avoids running untrusted model output outside a controlled sandbox.
...
Use this template for future retry loops:
...
```text
.opencode/skills/system-spec-kit/mcp-server/lib/deep-loop/post-dispatch-validate.ts
```
