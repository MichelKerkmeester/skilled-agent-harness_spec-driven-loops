 ...
 *
 * Inputs:
 *   - 113/003/state/eval-loop-state.jsonl (v1 — text-only, mocked grader)
 *   - 113/005/state/eval-loop-state-v2.jsonl  (v2 — extraction + live grader)
 *
 * Output:
 *   - 113/005/per-fixture-analysis.md
 */
...
const STATE_V1 = path.resolve(PACKET_ROOT, '..', '003-eval-loop', 'state', 'eval-loop-state.jsonl');
