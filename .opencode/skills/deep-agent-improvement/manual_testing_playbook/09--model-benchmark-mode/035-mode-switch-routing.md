...
This scenario validates that `scripts/loop-host.cjs` resolves `--mode=model-benchmark` to the materialize-then-run-benchmark pipeline while the default and `--mode=agent-improvement` routes stay on `score-candidate.cjs` unchanged.
...
| `../../scripts/loop-host.cjs` | Mode-switching entry point that resolves `--mode` and plans the invocation |
| `../../scripts/materialize-benchmark-fixtures.cjs` | Fixture materializer run before scoring on the model-benchmark path |
| `../../scripts/run-benchmark.cjs` | Benchmark runner invoked on the model-benchmark path |
