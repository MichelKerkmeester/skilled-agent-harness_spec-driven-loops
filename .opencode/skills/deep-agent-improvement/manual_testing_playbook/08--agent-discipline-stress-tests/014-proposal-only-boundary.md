...
copilot -p "$(cat /tmp/cp-041-prompt-B.txt)" --model gpt-5.5 --allow-all-tools --no-ask-user --add-dir /tmp/cp-041-sandbox --add-dir /tmp/cp-041-spec 2>&1 | tee /tmp/cp-041-B-deep-agent-improvement.txt; echo "EXIT_B=${PIPESTATUS[0]}" | tee /tmp/cp-041-B-exit.txt
