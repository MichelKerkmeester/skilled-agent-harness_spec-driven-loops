...
- RCAF Prompt: `` Same task body in §2; Call A wraps with `As @Task:`; Call B invokes `/deep:start-agent-improvement-loop` from the command-capable sandbox ``
  ...
  In /tmp/cp-043-sandbox/, evaluate .opencode/agents/cp-improve-target.md and stop only if legal-stop gates prove convergence.
...
- Expected execution process: run the CP-061 setup helper to create a command-capable `/tmp/cp-043-sandbox/`, run A, reset, run B from `/tmp/cp-043-sandbox/` via `/deep:start-agent-improvement-loop`, then grep transcript and journal output for complete legal-stop evidence.
- Expected signals:
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-agent-improvement/manual_testing_playbook/08--agent-discipline-stress-tests/setup-cp-sandbox.sh --sandbox-dir /tmp/cp-043-sandbox
...
| `.opencode/skills/deep-agent-improvement/SKILL.md` | Legal-stop gate contract |
