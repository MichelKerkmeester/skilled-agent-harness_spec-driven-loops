...
description: "Static state files that drive the deep-agent-improvement vitest suites without running a live loop."
trigger_phrases:
  - "test fixtures"
...
`fixtures/` holds static input files that the vitest suites in `scripts/tests/` read instead of running a live improvement or benchmark loop. Each fixture pins a known ledger and config shape so a test can assert exact reducer, scorer, and gate output.
...
- [`scripts/tests/README.md`](../README.md)
