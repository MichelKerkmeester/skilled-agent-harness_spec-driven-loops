...
| File | Layer | Role |
|---|---|---|
| `.opencode/skills/deep-agent-improvement/scripts/model-benchmark/dispatch-model.cjs` | Dispatcher | Routes a prompt to one of five executor CLIs, forwards `cwd`, and applies non-busy rate-limit backoff. |
| `.opencode/skills/deep-agent-improvement/scripts/model-benchmark/run-benchmark.cjs` | Benchmark runner | Consumes dispatcher output on the model-benchmark path before scoring. |
| `.opencode/skills/deep-agent-improvement/scripts/shared/loop-host.cjs` | Mode router | Gates the dispatcher behind `--mode=model-benchmark` so it loads only on that path. |
