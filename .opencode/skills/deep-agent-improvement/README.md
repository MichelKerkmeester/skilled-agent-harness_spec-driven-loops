...
- A target agent file in `.opencode/agents/` (any agent)
...
```text
node .opencode/skills/deep-agent-improvement/scripts/score-candidate.cjs --candidate=.opencode/agents/review.md
```
...
Dynamic mode is the only path. The profile generator derives scoring checks from the agent's own ALWAYS/NEVER rules, output verification checklist, capability scan, and anti-patterns. Any agent in `.opencode/agents/` is a valid target.
