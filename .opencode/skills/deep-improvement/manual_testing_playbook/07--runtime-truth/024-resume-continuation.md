---
title: "RT-024 -- Fresh-Session Continuation After Archive"
...
This document captures the canonical manual-testing contract for `RT-024`.

---

## 1. OVERVIEW

This scenario validates that the current release uses standalone `new`-mode sessions, so continuation happens by archiving the previous run and starting a fresh `/deep:start-agent-improvement-loop` session.
...
- Real user request: `` Validate that the current release uses standalone `new`-mode sessions, so continuation happens by archiving the previous run and starting a fresh `/deep:start-agent-improvement-loop` session. ``
- Prompt: `Validate that new-mode continuation archives the previous run and starts a fresh session.`
...
| `07--runtime-truth/024-resume-continuation.md` | Canonical per-feature execution contract |
...
- Feature file path: `07--runtime-truth/024-resume-continuation.md`
