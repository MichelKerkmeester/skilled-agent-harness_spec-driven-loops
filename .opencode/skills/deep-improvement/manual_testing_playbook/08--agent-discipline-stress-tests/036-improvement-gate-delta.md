...
- Expected execution process: run the CP-061 setup helper to create a command-capable `/tmp/cp-036-sandbox/`, run A, reset, run B from `/tmp/cp-036-sandbox/` via `/deep:start-agent-improvement-loop`, then grep score JSON, transcript, and legal-stop output.
- Expected signals:
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-improvement/manual_testing_playbook/08--agent-discipline-stress-tests/setup-cp-sandbox.sh --sandbox-dir /tmp/cp-036-sandbox
...
opencode run "/deep:start-agent-improvement-loop \".opencode/agents/cp-improve-target.md\" :auto --spec-folder=/tmp/cp-036-spec --iterations=1" --model deepseek/deepseek-v4-pro --dangerously-skip-permissions --dir /tmp/cp-036-sandbox </dev/null 2>&1 | tee /tmp/cp-036-B-command.txt; echo "EXIT_B=${PIPESTATUS[0]}" | tee /tmp/cp-036-B-exit.txt
...
- Feature file path: `08--agent-discipline-stress-tests/036-improvement-gate-delta.md`
