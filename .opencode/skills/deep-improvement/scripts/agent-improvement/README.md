...
```bash
node .opencode/skills/deep-improvement/scripts/agent-improvement/generate-profile.cjs --agent=.opencode/agents/code.md
```

Expected result: JSON profile with a `derivedChecks` block printed to stdout.

```bash
npx vitest run --config .opencode/skills/deep-improvement/scripts/vitest.config.mjs
```
