...
node -e 'for (const f of process.argv.slice(1)) JSON.parse(require("fs").readFileSync(f,"utf8"))' \
  .opencode/skills/deep-improvement/assets/model_benchmark/benchmark-fixtures/*.json \
  .opencode/skills/deep-improvement/assets/model_benchmark/benchmark-profiles/*.json && echo OK
```
