...
```bash
node -e "for (const f of ['default_profile.json','remediation_taxonomy.json','fixtures/deep-improvement/agent-improve-001.public.json','fixtures/deep-improvement/agent-improve-001.private.json']) { JSON.parse(require('fs').readFileSync('.opencode/skills/deep-improvement/assets/skill_benchmark/'+f,'utf8')); console.log('ok',f); }"
