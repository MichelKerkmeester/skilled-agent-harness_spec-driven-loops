---
title: "E2E-017 -- Full Pipeline Loop with Debug Target"
...
This document captures the canonical manual-testing contract for `E2E-017`.

---

## 1. OVERVIEW

This scenario validates the complete `/deep:agent-improvement` loop end-to-end using the debug agent as the target.
...
- Real user request: `` Validate that the complete `/deep:agent-improvement` loop end-to-end using the debug agent as the target. ``
- Prompt: `Validate the full /deep:agent-improvement loop against the debug agent.`
...
| `06--end-to-end-loop/full-pipeline.md` | Canonical per-feature execution contract |
...
- Feature file path: `06--end-to-end-loop/full-pipeline.md`
