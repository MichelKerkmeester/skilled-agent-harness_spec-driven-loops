---
title: "E2E-018 -- Full Pipeline Loop with Non-Standard Agent (Debug)"
...
This document captures the canonical manual-testing contract for `E2E-018`.

---

## 1. OVERVIEW

This scenario validates the complete `/deep:agent-improvement` loop targeting a non-standard agent (debug.md) to confirm the pipeline is not hardcoded to specific agents.
...
- Real user request: `` Validate that the complete `/deep:agent-improvement` loop targeting a non-standard agent (debug.md) to confirm the pipeline is not hardcoded to specific agents. ``
- Prompt: `Validate that the improvement loop works against a non-standard debug.md agent.`
...
| `06--end-to-end-loop/any-agent.md` | Canonical per-feature execution contract |
...
- Feature file path: `06--end-to-end-loop/any-agent.md`
