...
- `.opencode/skills/deep-loop-workflows/context/scripts/reduce-state.cjs` — how the reducer consumes these records (`parseJsonlDetailed`, the `type === 'iteration'` / `type === 'event'` split).
