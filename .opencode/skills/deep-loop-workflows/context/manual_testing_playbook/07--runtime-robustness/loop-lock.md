...
```bash
rg "acquireLoopLock\|refreshLoopLock\|releaseLoopLock" .opencode/skills/deep-loop-workflows/context/scripts/loop-lock.cjs
```
...
| File | Role |
|---|---|
| `.opencode/skills/deep-loop-workflows/context/scripts/loop-lock.cjs` | CLI wrapper: `acquire`, `refresh`, `release` actions; loads `loop-lock.ts` via tsx CJS register |
