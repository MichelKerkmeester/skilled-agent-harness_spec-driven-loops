...
```bash
rg "stateLogRepaired\|stateLogDroppedBytes" .opencode/skills/deep-loop-workflows/context/scripts/reduce-state.cjs
```
...
| File | Role |
|---|---|
| `.opencode/skills/deep-loop-workflows/context/scripts/reduce-state.cjs` | `repairJsonlTailInline` (fallback), `loadStateSafety` (loader); `stateSafety.repairJsonlTail(stateLogPath)` call; `registry.stateLogRepair` assignment |
