...
- `.opencode/skills/deep-loop-workflows/deep-context/scripts/reduce-state.cjs` — the reducer. Exports `reduceContextState`, `dedupByUnit`, `detectContradictions`, `buildRegistry`, `collectAllSeatFindings`, `validateSeatFinding`, `loadStateSafety`, plus `parseJsonl` / `parseJsonlDetailed` and `unitId`.
