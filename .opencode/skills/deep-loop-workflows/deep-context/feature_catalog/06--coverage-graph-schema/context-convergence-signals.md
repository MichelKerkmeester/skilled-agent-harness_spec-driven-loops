...
| File | Type | Role |
|---|---|---|
| `.opencode/skills/deep-loop-workflows/deep-context/manual_testing_playbook/06--coverage-graph-schema/context-convergence-signals.md` | Manual playbook | Verifies ContextConvergenceSignals interface fields exist in coverage-graph-signals.ts, evaluateContext returns parseable JSON with all five fields, createSnapshot upserts correctly |
