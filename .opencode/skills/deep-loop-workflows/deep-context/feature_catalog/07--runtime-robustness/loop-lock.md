...
| File | Layer | Role |
|---|---|---|
| `.opencode/skills/deep-loop-workflows/deep-context/scripts/loop-lock.cjs` | Script | CLI wrapper: `acquire`, `refresh`, `release` actions; loads `loop-lock.ts` via tsx CJS register |
...
| File | Type | Role |
|---|---|---|
| `.opencode/skills/deep-loop-workflows/deep-context/manual_testing_playbook/07--runtime-robustness/loop-lock.md` | Manual playbook | Verifies `loop-lock.cjs` syntax; confirms `step_acquire_lock`/`step_release_lock` references exist in both YAML files |
