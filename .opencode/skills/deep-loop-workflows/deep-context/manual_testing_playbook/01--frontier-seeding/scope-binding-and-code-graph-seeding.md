...
- Expected execution process: Read the auto YAML for `step_seed_frontier`; read `loop_protocol.md` §4 for frontier seeding rules; read SKILL.md §3 step 1 for code_graph_query usage; read `context.md` section 5 for background frontier seeding; confirm Glob+Grep fallback is documented.
...
```bash
rg "whole.*repo\|never.*repo\|whole-repo" .opencode/skills/deep-loop-workflows/deep-context/SKILL.md .opencode/skills/deep-loop-workflows/deep-context/references/protocol/loop_protocol.md
```
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_context_auto.yaml` | Primary: `step_seed_frontier` in phase_init |
| `.opencode/skills/deep-loop-workflows/deep-context/references/protocol/loop_protocol.md` | Loop protocol §4: frontier seeding rules, code_graph_query usage, Glob+Grep fallback |
| `.opencode/skills/deep-loop-workflows/deep-context/SKILL.md` | SKILL.md §3 step 1 describing frontier seeding and scope anchors |
| `.opencode/commands/deep/context.md` | Command section 5: background read-only frontier seeding before setup prompt |
