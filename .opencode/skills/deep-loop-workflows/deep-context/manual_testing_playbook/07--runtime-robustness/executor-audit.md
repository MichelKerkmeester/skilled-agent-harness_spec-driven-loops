...
This scenario validates Executor Audit for `RUNTIME-005`. It focuses on `SPECKIT_CLI_DISPATCH_STACK` and the `executor-audit` recursion-guard contract being wired into `deep_context_auto.yaml` via the `cli_contract` requirement, and `buildExecutorDispatchEnv` being the runtime function that stamps the dispatch stack into each CLI seat's environment.
...
- Objective: Verify that `SPECKIT_CLI_DISPATCH_STACK` and `executor-audit` are referenced in `deep_context_auto.yaml`, confirming the recursion-guard wiring for CLI seats.
...
- Prompt: `As a manual-testing orchestrator, validate the executor-audit recursion-guard contract for deep-context against the auto YAML cli_contract block. Verify SPECKIT_CLI_DISPATCH_STACK and executor-audit appear in deep_context_auto.yaml. Return a concise verdict.`
- Expected execution process: Grep `deep_context_auto.yaml` for `SPECKIT_CLI_DISPATCH_STACK` and `executor-audit`; grep `executor-audit.ts` for `CLI_DISPATCH_STACK_ENV` and `buildExecutorDispatchEnv`.
...
```bash
rg "SPECKIT_CLI_DISPATCH_STACK\|executor-audit" .opencode/commands/deep/assets/deep_context_confirm.yaml
```
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_context_auto.yaml` | `cli_contract` block in `step_sweep_cli_pool`: mandates `SPECKIT_CLI_DISPATCH_STACK` + `buildExecutorDispatchEnv` |
