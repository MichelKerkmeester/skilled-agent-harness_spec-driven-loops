...
```bash
node -e "for (const f of ['default_profile.json','remediation_taxonomy.json','fixtures/deep-improvement/agent-improve-001.public.json','fixtures/deep-improvement/agent-improve-001.private.json']) { JSON.parse(require('fs').readFileSync('.opencode/skills/deep-loop-workflows/improvement/assets/skill_benchmark/'+f,'utf8')); console.log('ok',f); }"
npx vitest run .opencode/skills/deep-loop-workflows/improvement/scripts/model-benchmark/tests/remediation.vitest.ts
```
