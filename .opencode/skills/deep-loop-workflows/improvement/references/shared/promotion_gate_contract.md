...
```bash
node .opencode/skills/deep-loop-workflows/improvement/scripts/shared/promote-candidate.cjs \
...
```bash
node .opencode/skills/deep-loop-workflows/improvement/scripts/agent-improvement/rollback-candidate.cjs \
  --target={canonical_target_path} \
  --backup={backup_path}
```
