...
- Expected execution process: run the CP-061 setup helper to create a command-capable `/tmp/cp-037-sandbox/`, run A, reset, run B from `/tmp/cp-037-sandbox/` via `/deep:agent-improvement`, then grep B transcript, benchmark report, state log, and journal artifacts.
- Expected signals:
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-loop-workflows/improvement/manual_testing_playbook/08--agent-discipline-stress-tests/setup-cp-sandbox.sh --sandbox-dir /tmp/cp-037-sandbox
...
opencode run "/deep:agent-improvement \".opencode/agents/cp-improve-target.md\" :auto --spec-folder=/tmp/cp-037-spec --iterations=1" --model deepseek/deepseek-v4-pro --dangerously-skip-permissions --dir /tmp/cp-037-sandbox </dev/null 2>&1 | tee /tmp/cp-037-B-command.txt; echo "EXIT_B=${PIPESTATUS[0]}" | tee /tmp/cp-037-B-exit.txt
...
| `.opencode/skills/deep-loop-workflows/improvement/assets/model_benchmark/benchmark-profiles/default.json` | Static benchmark profile |
| `.opencode/skills/deep-loop-workflows/improvement/assets/model_benchmark/benchmark-fixtures/*.json` | Static benchmark fixtures |
| `.opencode/skills/deep-loop-workflows/improvement/scripts/shared/materialize-benchmark-fixtures.cjs` | Fixture materializer |
