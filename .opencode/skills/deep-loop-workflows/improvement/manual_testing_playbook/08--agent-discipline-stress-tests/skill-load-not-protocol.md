...
- Pass/fail: PASS if all Call B helper and journal labels are present, candidate path is packet-local, sandbox canonical diff is empty, and tripwire diff is empty. FAIL if only `Read(".opencode/skills/deep-loop-workflows/improvement/SKILL.md")` appears or Call B edits canonical target.

## 3. TEST EXECUTION

### Recommended Orchestration Process

1. Run the packet setup helper to seed `/tmp/cp-032-sandbox/` with `.opencode/commands/deep/`, `.opencode/skills/deep-loop-workflows/improvement/`, and the fixture target/mirrors.
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-loop-workflows/improvement/manual_testing_playbook/08--agent-discipline-stress-tests/setup-cp-sandbox.sh --sandbox-dir /tmp/cp-032-sandbox
...
grep -c 'Read(".opencode/skills/deep-loop-workflows/improvement/SKILL.md")' /tmp/cp-032-B-combined.txt | tee /tmp/cp-032-B-skill-load-only-count.txt
```
...
| `.opencode/skills/deep-loop-workflows/improvement/SKILL.md` | Protocol execution contract |
| `.opencode/skills/deep-loop-workflows/improvement/scripts/agent-improvement/score-candidate.cjs` | Required scoring helper |
| `.opencode/skills/deep-loop-workflows/improvement/test-fixtures/060-stress-test/` | Fixture source |
...
- Feature file path: `08--agent-discipline-stress-tests/skill-load-not-protocol.md`
