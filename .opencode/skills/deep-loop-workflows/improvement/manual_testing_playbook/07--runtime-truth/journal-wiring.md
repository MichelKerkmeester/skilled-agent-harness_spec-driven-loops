...
This scenario validates that the `/deep:agent-improvement` autonomous workflow wires `improvement-journal.cjs` at every required boundary: `session_start`, per-iteration lifecycle checkpoints, nested `legal_stop_evaluated.details.gateResults`, and `session_end`. Given: a fresh `/deep:agent-improvement` `:auto` session. When: the operator runs the session end-to-end or inspects `.opencode/commands/deep/assets/deep_agent-improvement_auto.yaml`. Then: journal events appear in `improvement-journal.jsonl` for every boundary, the CLI example in `.opencode/commands/deep/agent-improvement.md` executes as written against a temp target, and the frozen `STOP_REASONS` / `SESSION_OUTCOMES` enums match the helper validator.
...
- Real user request: `` Validate that the `/deep:agent-improvement` autonomous workflow wires `improvement-journal.cjs` at every required boundary: `session_start`, per-iteration lifecycle checkpoints, nested `legal_stop_evaluated.details.gateResults`, and `session_end`. Given: a fresh `/deep:agent-improvement` `:auto` session. When: the operator runs the session end-to-end or inspects `.opencode/commands/deep/assets/deep_agent-improvement_auto.yaml`. Then: journal events appear in `improvement-journal.jsonl` for every boundary, the CLI example in `.opencode/commands/deep/agent-improvement.md` executes as written against a temp target, and the frozen `STOP_REASONS` / `SESSION_OUTCOMES` enums match the helper validator. ``
- Prompt: `Validate that /deep:agent-improvement wires improvement-journal.cjs at every required workflow boundary.`
...
| `.opencode/commands/deep/agent-improvement.md` | Implementation or verification anchor referenced by this scenario |
| `.opencode/commands/deep/assets/deep_agent-improvement_auto.yaml` | Implementation or verification anchor referenced by this scenario |
