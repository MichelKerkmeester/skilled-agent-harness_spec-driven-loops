...
This scenario validates that the current release uses standalone `new`-mode sessions, so continuation happens by archiving the previous run and starting a fresh `/deep:agent-improvement` session.
...
- Real user request: `` Validate that the current release uses standalone `new`-mode sessions, so continuation happens by archiving the previous run and starting a fresh `/deep:agent-improvement` session. ``
...
| `07--runtime-truth/resume-continuation.md` | Canonical per-feature execution contract |
...
- Feature file path: `07--runtime-truth/resume-continuation.md`
