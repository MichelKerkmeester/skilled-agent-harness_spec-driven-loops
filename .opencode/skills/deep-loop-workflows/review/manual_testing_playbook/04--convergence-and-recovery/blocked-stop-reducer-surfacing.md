...
- When: The operator runs `node .opencode/skills/deep-loop-workflows/review/scripts/reduce-state.cjs <spec-folder>`.
...
### Commands
1. `bash: node .opencode/skills/deep-loop-workflows/review/scripts/reduce-state.cjs {spec_folder}`
...
| File | Role |
|---|---|
| `.opencode/skills/deep-loop-workflows/review/scripts/reduce-state.cjs` | Canonical reducer implementation, promotes `blocked_stop` into `blockedStopHistory`, renders `BLOCKED STOPS`, and rewrites `ANCHOR:next-focus` |
| `.opencode/skills/deep-loop-workflows/review/references/state/state_format.md` | Review state contract, defines `blockedStopHistory`, dashboard `BLOCKED STOPS`, strategy next-focus override, and review-specific gate payload shape |
| `.opencode/skills/deep-loop-workflows/review/references/protocol/loop_protocol.md` | Review legal-stop workflow contract, defines the review-specific gate names used in `blocked_stop` events |
