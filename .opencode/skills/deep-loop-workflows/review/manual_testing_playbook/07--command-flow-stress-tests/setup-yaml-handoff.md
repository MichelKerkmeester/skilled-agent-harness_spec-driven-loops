...
This scenario checks the command-owned entrypoint. The differentiator is whether Call B binds target, mode, dimensions, spec folder, max iterations, and convergence before handing off to `deep_review_auto.yaml`.
...
- Expected execution process: build sandbox, snapshot baseline, invoke `/deep:review:auto`, then inspect transcript plus review artifacts.
...
/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-loop-workflows/review/manual_testing_playbook/07--command-flow-stress-tests/setup-cp-sandbox.sh --sandbox-dir /tmp/cp-052-sandbox
...
{
  field "auto_yaml_loaded" "deep_review_auto.yaml|AUTONOMOUS|auto mode" /tmp/cp-052-B-combined.txt
...
| `.opencode/skills/deep-loop-workflows/review/SKILL.md` | 43-61, 253-346 | Command-only invocation and three-layer workflow |
