...
description: "Code-facing README for .opencode/skills/deep-loop-workflows/research/scripts."
trigger_phrases:
...
This README documents the code-bearing folder `.opencode/skills/deep-loop-workflows/research/scripts` so operators can understand its role without opening every source file first. It follows the sk-doc skill README structure while staying focused on code navigation.
...
Start with `.opencode/skills/deep-loop-workflows/research/SKILL.md` for runtime routing and workflow boundaries.

**Step 2: Inspect the local code.**

```bash
rg --files .opencode/skills/deep-loop-workflows/research/scripts
```
...
```text
User request: Check .opencode/skills/deep-loop-workflows/research/scripts for sk-code and README coverage.
