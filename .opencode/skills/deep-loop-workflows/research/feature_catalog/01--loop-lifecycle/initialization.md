...
| `.opencode/skills/deep-loop-workflows/research/references/protocol/loop_protocol.md` | Reference | Documents session classification, canonical names, and the resumed or restarted lifecycle event contract. |
| `.opencode/skills/deep-loop-workflows/research/assets/deep_research_config.json` | Asset | Supplies the default config shape written during initialization. |
| `.opencode/skills/deep-loop-workflows/research/assets/deep_research_strategy.md` | Asset | Supplies the initial strategy structure and anchor layout. |

### Validation And Tests

| File | Type | Role |
|---|---|---|
| `.opencode/skills/deep-loop-workflows/research/manual_testing_playbook/02--initialization-and-state-setup/fresh-initialization-creates-canonical-state-files.md` | Manual playbook | Verifies fresh initialization creates the canonical packet files. |
| `.opencode/skills/deep-loop-workflows/research/manual_testing_playbook/02--initialization-and-state-setup/resume-classification-from-valid-prior-state.md` | Manual playbook | Verifies resume classification and restart-safe continuation from valid state. |
| `.opencode/skills/deep-loop-workflows/research/manual_testing_playbook/02--initialization-and-state-setup/invalid-or-contradictory-state-halts-for-repair.md` | Manual playbook | Verifies contradictory packet state halts instead of guessing. |
| `.opencode/skills/deep-loop-workflows/research/manual_testing_playbook/02--initialization-and-state-setup/research-charter-validation.md` | Manual playbook | Verifies the research charter sections exist before the loop proceeds. |
