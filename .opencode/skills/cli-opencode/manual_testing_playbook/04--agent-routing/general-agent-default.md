...
- Objective: Confirm `--agent general` loads the project's general agent definition from `.opencode/agents/general.md` and the dispatched session demonstrates implementation-style behavior (file reads, optional writes, multi-step reasoning).
...
- Expected signals: Dispatch exits 0. JSON event stream includes a `tool.call` event whose payload references the Read tool. Session.completed or session metadata references `general` as the agent slug. Response references content from `.opencode/skills/cli-opencode/SKILL.md`. Runtime under 90 seconds.
...
- Feature file path: `04--agent-routing/general-agent-default.md`
