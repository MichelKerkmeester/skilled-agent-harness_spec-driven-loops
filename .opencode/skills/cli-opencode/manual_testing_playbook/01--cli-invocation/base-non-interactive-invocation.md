...
- Objective: Confirm that the canonical default invocation `opencode run --model deepseek/deepseek-v4-pro --agent general --variant high --format json --dir "$(pwd)" "<prompt>" 2>&1` returns a parseable JSON event stream and exits 0.
...
- Expected execution process: External-AI orchestrator (Claude Code, Codex, Copilot or raw shell) confirms self-invocation guard does NOT trip, runs `command -v opencode`, dispatches with the default shape, captures stdout+stderr via `2>&1` and parses the newline-delimited JSON events with `jq`.
