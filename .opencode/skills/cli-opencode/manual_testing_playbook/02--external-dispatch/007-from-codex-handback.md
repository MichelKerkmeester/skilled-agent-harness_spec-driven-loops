...
- Real user request: `From Codex, use cli-opencode to dispatch a general question to OpenCode that uses the project plugin runtime but does NOT touch the spec-kit. Have OpenCode confirm it has access to the Code Graph structural query + Grep MCP.`
- RCAF Prompt: `As Codex (or any non-Anthropic external runtime) dispatching from a fresh shell into a new OpenCode session via cli-opencode use case 1, dispatch a one-shot prompt against /tmp/co-007-events.jsonl asking the session to call the Code Graph structural query + Grep MCP for "self-invocation guard" across the .opencode/skills/cli-opencode/ folder and return the top 3 results. Verify the JSON event stream contains a tool.call event for the Code Graph search MCP, the search returns at least one hit referencing actual cli-opencode content, the spec-kit memory database is not touched, and the smart router selected use case 1 (not use case 3). Return a one-line pass/fail verdict naming the Code Graph tool called and confirming use case 1 was selected.`
- Expected execution process: External-AI orchestrator (Codex or any non-Anthropic runtime, including raw shell as a stand-in) confirms self-invocation guard does NOT trip, dispatches with the cli-opencode default shape and a prompt explicitly naming Code Graph (NOT spec-kit/memory/code-graph) and verifies a `tool.call` for the Code Graph search MCP appears in the JSON event stream.
- Expected signals: Dispatch exits 0. JSON event stream includes a `tool.call` event whose payload references the Code Graph search MCP tool. Session.completed payload references the search result (file paths, snippets or similar). Runtime under 120 seconds.
- Desired user-visible outcome: Verdict naming the Code Graph tool that was called and confirming use case 1 was selected (not use case 3).
- Pass/fail: PASS if exit 0 AND Code Graph `tool.call` appears AND search result references actual files. FAIL if dispatch fails, Code Graph tool is not called or the smart router incorrectly routed to use case 3.
...
3. Dispatch with a prompt explicitly naming Code Graph (NOT memory/spec-kit/code-graph) so use case 1 is selected.
4. Parse the JSON event stream for the Code Graph `tool.call` event.
5. Verify session.completed references the search result.
6. Return a verdict naming the Code Graph tool and the search snippet.

| Feature ID | Feature Name | Scenario Name / Objective | Exact Prompt | Exact Command Sequence | Expected Signals | Evidence | Pass/Fail Criteria | Failure Triage |
|---|---|---|---|---|---|---|---|---|
| CO-007 | External dispatch from Codex into OpenCode (use case 1) | Confirm use case 1 dispatch from a non-Anthropic external runtime reaches OpenCode for general full-runtime tasks | `As Codex (or any non-Anthropic external runtime) dispatching from a fresh shell into a new OpenCode session via cli-opencode use case 1, dispatch a one-shot prompt against /tmp/co-007-events.jsonl asking the session to call the Code Graph structural query + Grep MCP for "self-invocation guard" across the .opencode/skills/cli-opencode/ folder and return the top 3 results. Verify the JSON event stream contains a tool.call event for the Code Graph search MCP, the search returns at least one hit referencing actual cli-opencode content, the spec-kit memory database is not touched, and the smart router selected use case 1 (not use case 3). Return a one-line pass/fail verdict naming the Code Graph tool called and confirming use case 1 was selected.` | 1. `bash: env \| grep -q '^OPENCODE_' && echo "ABORT: in-OpenCode" \|\| echo "OK: external runtime"` -> 2. `bash: opencode run --model opencode-go/deepseek-v4-pro --agent general --variant high --format json --dir "$(pwd)" "Use the Code Graph MCP semantic search tool. Search for 'self-invocation guard' across the .opencode/skills/cli-opencode/ folder and return the top 3 results in one short sentence." > /tmp/co-007-events.jsonl 2>&1` -> 3. `bash: echo "Exit: $?"` -> 4. `bash: jq -r 'select(.type == "tool.call") \| .payload.name' /tmp/co-007-events.jsonl \| sort -u` -> 5. `bash: jq -r 'select(.type == "tool.call" and (.payload.name \| test("(coco\|search)";"i"))) \| .payload' /tmp/co-007-events.jsonl \| wc -l` -> 6. `bash: jq -r 'select(.type == "session.completed") \| .payload' /tmp/co-007-events.jsonl \| grep -ciE '(self-invocation\|guard\|cli-opencode)'` | Step 1: prints `OK: external runtime`; Step 2: events captured; Step 3: exit 0; Step 4: tool.call names include the Code Graph search MCP slug (case-insensitive match for "coco" or "search"); Step 5: count of Code Graph tool calls >= 1; Step 6: count of guard/self-invocation/cli-opencode mentions in session.completed >= 1 | `/tmp/co-007-events.jsonl`, terminal transcript with tool.call enumeration | PASS if exit 0 AND Code Graph tool.call appears AND search result references actual cli-opencode content; FAIL if any check fails | 1. If Code Graph tool.call is missing, the MCP may not be registered — check `opencode.json` for the code_graph MCP entry; 2. If the search returns no hits, lower the case sensitivity or broaden the query; 3. If session.completed is missing, the session may have crashed mid-run — re-run with `--print-logs --log-level DEBUG` and inspect stderr; 4. If the smart router picked use case 3 instead, the prompt may have leaked a spec-kit keyword — re-prompt avoiding "memory", "spec-kit", "code-graph" |
...
- Feature file path: `02--external-dispatch/007-from-codex-handback.md`
