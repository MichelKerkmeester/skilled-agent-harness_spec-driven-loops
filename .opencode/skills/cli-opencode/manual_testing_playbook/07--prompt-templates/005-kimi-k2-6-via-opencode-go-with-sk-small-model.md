---
title: "CO-036 -- Kimi-k2.6 via opencode-go through sk-small-model + sk-prompt"
...
Kimi-k2.6 has two dispatch paths declared in `sk-prompt/assets/model-profiles.json`: cli-devin via Cognition Pro (primary) and cli-opencode via opencode-go. This scenario validates the opencode-go path through the three-skill flow: `sk-small-model` surfaces the dispatch matrix, the operator overrides the primary path to opencode-go for long-context work, `sk-prompt` composes with an extended-context framework, and `cli-opencode` dispatches `--model opencode-go/kimi-k2.6 --variant high`.
...
- Expected execution process: Advisor surfaces sk-small-model + cli-opencode -> read model-profiles.json kimi-k2.6 entry to confirm opencode-go path -> compose through sk-prompt -> opencode dispatches.
- Expected signals: Advisor confidence ≥ 0.85 for sk-small-model AND ≥ 0.80 for cli-opencode. model-profiles.json kimi-k2.6 shows 2 executors. `opencode run --model opencode-go/kimi-k2.6 --variant high` exits 0. Output references ≥ 5 distinct input files.
...
1. Run the advisor smoke test: `python3 .opencode/skills/system-skill-advisor/mcp_server/scripts/skill_advisor.py "Kimi-k2.6 long-context analysis via opencode-go"` and confirm `sk-small-model` + `cli-opencode` both above 0.8.
2. Read `.opencode/skills/sk-prompt/assets/model-profiles.json` kimi-k2.6 entry. Confirm 2 executor paths (cli-devin cognition-pro + cli-opencode opencode-go) and document why opencode-go is chosen.
3. Pick a long-context task: "summarize the architecture across `.opencode/skills/sk-small-model/`, `.opencode/skills/cli-devin/references/`, `.opencode/skills/cli-opencode/references/`, `.opencode/skills/sk-prompt/assets/`, and `.opencode/skills/system-spec-kit/mcp_server/lib/deep-loop/`. Identify cross-cutting patterns and unresolved gaps."
...
| `../../../sk-small-model/SKILL.md` | Dispatch matrix table |
| `../../../sk-prompt/assets/model-profiles.json` | kimi-k2.6 entry: 2 executor paths |
...
- Feature file path: `07--prompt-templates/005-kimi-k2-6-via-opencode-go-with-sk-small-model.md`
