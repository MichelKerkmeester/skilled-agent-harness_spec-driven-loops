...
The `--agent <slug>` flag loads an agent definition from `.opencode/agents/<slug>.md`. The frontmatter pins the model, tool permissions, and system prompt. Specialized agents (`deep-research`, `deep-review`, `context`, `review`, `multi-ai-council`, etc.) provide domain-specific behavior the calling AI can target.
