...
test("file, model, content, and entity helpers classify inputs", () => {
  ...
  assert.equal(listEmbeddingModels().length, 17);
  assert.equal(
...
test("glob matching is stable across reuse and cache eviction", () => {
  // Compiled patterns are cached, so repeated calls must agree with the first
  // result and the two case variants of one pattern must not share an entry.
  for (let round = 0; round < 3; round++) {
    assert.equal(pathPatternMatches("src/**/*.ts", "src/nested/file.ts"), true);
    assert.equal(
      pathPatternMatches("src/**/*.ts", "src/nested/file.js"),
      false,
    );
    assert.equal(pathPatternMatches("*.MD", "docs/readme.md"), false);
    assert.equal(
      pathPatternMatchesCaseInsensitive("*.MD", "docs/readme.md"),
      true,
    );
  ...
  // Push distinct patterns past the cache bound so the eviction path runs.
  for (let index = 0; index < 5000; index++) {
    pathPatternMatches(`generated-${index}/**/*.ts`, "generated-0/a/file.ts");
  }

  // Recompiled-after-eviction matchers must behave exactly as before.
  assert.equal(pathPatternMatches("src/**/*.ts", "src/nested/file.ts"), true);
  assert.equal(pathPatternMatches("src/**/*.ts", "src/nested/file.js"), false);
  assert.equal(pathPatternMatches("*.MD", "docs/readme.md"), false);
  assert.equal(
    pathPatternMatchesCaseInsensitive("*.MD", "docs/readme.md"),
    true,
