...
test("direct-mode stdio refuses a listen address instead of ignoring it", async () => {
  const home = await mkdtemp(join(tmpdir(), "zvec-grep-direct-listen-"));
  try {
    await assert.rejects(
      execFileAsync(
        ...
        { env: { ...process.env, ZVEC_GREP_HOME: home, NO_COLOR: "1" } },
      ),
      (error) => {
        assert.match(error.stderr, /does not start a listener; drop --listen/);
        return true;
      },
    );
    // The failure must not have started a daemon behind the client's back.
