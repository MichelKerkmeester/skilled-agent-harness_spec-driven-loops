...
function validateCliShape(
  ...
  if (command === "server") {
    ...
    if (options.mode !== undefined && !options.serverStdio) {
      throw new Error("--mode can only be used with zg server --stdio");
    }
  ...
  if (
    ...
    throw new Error(
      "--mode can only be used with query, index, status, zg server --stdio, or zg install",
    );
  }
  if (
    options.checkReady &&
