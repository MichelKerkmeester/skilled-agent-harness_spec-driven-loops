...
const ENVIRONMENT_VARIABLES = {
  ...
  ZVEC_GREP_OLLAMA_URL:
    "Ollama server base URL; defaults to http://127.0.0.1:11434",
  OLLAMA_BASE_URL: "Ollama server base URL fallback after ZVEC_GREP_OLLAMA_URL",
...
${formatEnvironmentVariables([
  ...
  "ZVEC_GREP_AUTHORIZATION_KEY_FILE",
    ...
    case "server":
      return `Usage:
  zg server --stdio [--mode <auto|server|direct>] [--token-file <path>] [--mcp-toolset <agent|full>]
...
--stdio --mode direct instead serves the same MCP tools from this process and
never starts a daemon, for clients that must not leave a background process
running. Direct mode has no file watcher and no background refresh, so index
freshness is the caller's responsibility. ZVEC_GREP_MODE=direct selects the
same behavior; any other mode keeps the daemon bootstrap.
...
See zg help environment for daemon startup scope.`;
    case "install":
      return `Usage:
  zg install [--target codex|claude|qwen|qoder|opencode|cursor|all|auto] [--mcp-transport stdio|http] [--mode auto|server|direct] [--mcp-toolset agent|full] [--yes] [--force]
...
Options:
  ...
  --mode <auto|server|direct>       stdio launch mode written into the agent configuration (default: the daemon bootstrap)
...
--mode direct writes a "server --stdio --mode direct" launch instead, so the
agent runs zvec-grep in its own process and no daemon is started or required.
It applies to stdio only and leaves index refresh to the caller.
...
function modelsHelp(): string {
  const models = listEmbeddingModels().sort((left, right) => {
    return (
      runtimeRank(left) - runtimeRank(right) ||
...
Ollama models need a reachable Ollama server that has already pulled the tag,
by default http://127.0.0.1:11434. Set ZVEC_GREP_OLLAMA_URL or --endpoint for
another address. They embed without a credential, and without Remote Embedding
authorization, because the request stays between zvec-grep and that server.
...
function runtimeRank(model: EmbeddingCatalogEntry): number {
  if (model.provider === "local") return 0;
  return model.backend === "ollama" ? 1 : 2;
}
...
function formatEmbeddingModels(
  ...
  const rows = models.map((model) => {
    // An Ollama server is reached over HTTP but is normally the same machine,
    // so calling it remote would misdescribe where the content goes.
    const runtime =
...
Precedence:
  ...
  Ollama server URL                Embedding runtime endpoint > ZVEC_GREP_OLLAMA_URL > OLLAMA_BASE_URL > http://127.0.0.1:11434
