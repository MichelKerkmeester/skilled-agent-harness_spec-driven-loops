...
function fullMcpInstructions(options: { serverStatusTool: boolean }): string {
  return formatPromptRules(
...
export const ZVEC_GREP_FULL_MCP_INSTRUCTIONS = fullMcpInstructions({
  serverStatusTool: true,
});

/**
 * Direct mode runs the tools in the calling process, so it never registers the
 * daemon-only server-status tool and must not instruct an agent to call it.
 */
export const ZVEC_GREP_FULL_DIRECT_MCP_INSTRUCTIONS = fullMcpInstructions({
  serverStatusTool: false,
});
...
export type ZvecGrepMcpServerOptions = {
  ...
  /**
   * Register `zvec_grep_server_status` with the `full` toolset. Backends
   * without a daemon set this to false so the surface never advertises a tool
   * that cannot report anything.
   */
  includeServerStatusTool?: boolean;
...
export function createZvecGrepMcpServer(
  ...
  const server = new McpServer(
    { name: "zvec-grep", version },
    {
      instructions:
        toolset === "full"
          ? options.includeServerStatusTool === false
...
export function registerZvecGrepTools(
  ...
  if (full && options.includeServerStatusTool !== false) {
    server.registerTool(
