...
// A directory walk tests the same handful of ignore rules against every path it
// visits, so compiling on each call makes regexp construction scale with paths
// times rules and dominate the walk. Compiled expressions never carry a /g or
// /y flag, so they hold no match state and are safe to share across calls.
const GLOB_REGEXP_CACHE_LIMIT = 4096;
...
function globToRegExp(pattern: string, caseInsensitive = false): RegExp {
  ...
  // Real rule sets are small, so exceeding the limit means patterns are being
  // generated rather than reused. Drop the map wholesale instead of growing
  // without bound; the next walk repopulates only what it actually uses.
  if (globRegExpCache.size >= GLOB_REGEXP_CACHE_LIMIT) {
    globRegExpCache.clear();
  }
  globRegExpCache.set(cacheKey, compiled);
