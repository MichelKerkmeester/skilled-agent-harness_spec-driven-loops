...
Fully shipped in both `deep_start-review-loop_auto.yaml` and `confirm.yaml`. Parity gate:
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_start-review-loop_auto.yaml` | `step_resolve_artifact_root` (if_present/if_absent), `step_fanout_spawn`, `step_fanout_merge` (with `bind_from_output` for p0/p1/p2 counts) |
| `.opencode/commands/deep/assets/deep_start-review-loop_confirm.yaml` | Same steps (confirm variant) |
| `.opencode/commands/deep/start-review-loop.md` | `--executor`/`--executors`/`--concurrency` flag docs, strongest-restriction note, fan-out examples |
...
- Catalog source: `feature_catalog/01--loop-lifecycle/fanout-dispatch.md`
- Primary source files: `deep_start-review-loop_auto.yaml`, `start-review-loop.md`
Related references:
- [executor-selection-contract.md](executor-selection-contract.md) — Executor Selection Contract
