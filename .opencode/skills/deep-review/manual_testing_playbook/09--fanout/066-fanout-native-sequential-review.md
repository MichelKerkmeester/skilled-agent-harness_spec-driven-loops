...
- `deep_start-review-loop_auto.yaml` present.

### Steps

1. `bash: grep -n "step_fanout_spawn_native\|agent: deep-review\|agent: deep-research\|fanout_lineage_artifact_dir" .opencode/commands/deep/assets/deep_start-review-loop_auto.yaml`
...
5. `bash: grep -n "native.*count\|native fan-out" .opencode/commands/deep/start-review-loop.md` — confirm native fan-out note in examples.
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_start-review-loop_auto.yaml` | `step_fanout_spawn_native`, `agent: deep-review` dispatch, `fanout_lineage_artifact_dir` context |
| `.opencode/commands/deep/start-review-loop.md` | Native fan-out note in EXAMPLES section |
...
- Scenario file path: `manual_testing_playbook/09--fanout/066-fanout-native-sequential-review.md`
