...
- Real user request: `/deep:start-review-loop:auto "skill:deep-research" --executor=cli-codex --model=o4-mini --label=codex --executor=cli-claude-code --model=claude-opus-4-8 --label=claude --concurrency=2`
...
- `deep_start-review-loop_auto.yaml` and `start-review-loop.md` present.

### Steps

1. `bash: grep -n "step_fanout_spawn\|step_fanout_merge\|bind_from_output\|active_p0\|active_p1\|skip_when" .opencode/commands/deep/assets/deep_start-review-loop_auto.yaml | head -25`
...
4. `bash: grep -n "fanout_executors\|--executor\|strongest.restriction\|merged FAIL" .opencode/commands/deep/start-review-loop.md | head -15`
...
| File | Role |
|---|---|
| `.opencode/commands/deep/assets/deep_start-review-loop_auto.yaml` | `step_fanout_spawn`, `step_fanout_merge` (with `bind_from_output`), `step_resolve_artifact_root` |
| `.opencode/commands/deep/start-review-loop.md` | `--executor` flag docs, strongest-restriction note |
...
- Scenario file path: `manual_testing_playbook/09--fanout/064-fanout-cli-lineages-review.md`
