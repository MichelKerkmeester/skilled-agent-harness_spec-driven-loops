...
# Layer 3: best-effort state lock-file probe
...
def detect_self_invocation():
    ...
    try:
        ...
        pass
    # Layer 3: best-effort state lock-file probe. Absence is not proof that no Devin session is active.
...
> **SWE-1.6 Prompt-Quality Contract (REQUIRED)**: SWE-1.6 is fast and coding-specialized but smaller than the complex-task models — it relies on prompt clarity for reliable output. The framework choice and the mandatory caller-side pre-planning contract are OWNED by [`../sk-prompt-small-model/references/models/swe-1.6.md`](../sk-prompt-small-model/references/models/swe-1.6.md) (reached via the tier-2 model-override step of the 3-tier precedence rule in §4 ALWAYS). EVERY cli-devin dispatch with `--model swe-1.6` MUST: (1) honor that swe-1.6 profile plus the 3-tier precedence rule when composing the final `--prompt-file` payload, AND (2) include an explicit pre-planning block in the prompt body — the calling AI decomposes the task into ordered steps with acceptance criteria BEFORE handing it to SWE-1.6, rather than asking SWE-1.6 to figure out structure on its own. See `assets/prompt_templates.md` §2 for the executor-owned dispatch template that wraps the profile scaffold, `../sk-prompt-small-model/assets/cli_prompt_quality_card.md` §4 for canonical CLEAR, and `assets/prompt_quality_card.md` §3 for local precedence. Skipping this contract is the single largest cause of underwhelming SWE-1.6 output and must be treated as a soft-block in operator review. **Composition guidance** (set by the profile): use medium-density pre-planning (3-4 ordered steps with per-step acceptance and verification); do NOT pair BUILD with strict bundle-gate wording — verbose constraint language pushes SWE 1.6 most strongly toward defensive output rather than direct code, and the same pattern holds on `deepseek-v4` and `kimi-k2.6`, so keep bundle-gate language at "standard" for all cli-devin model presets.
...
Per-model context budget defaults live in `assets/per-model-budgets.json`; the budget pattern is documented in `references/context-budget.md`, and the opt-in output-verification pipeline is documented in `references/output-verification.md`. Shared model facts, quota pools, and fallback targets live in `../sk-prompt-small-model/assets/model-profiles.json`.
...
8. **Load `assets/prompt_quality_card.md` before building any dispatch prompt.** For Tier 1, compose directly via the canonical card it delegates to: select the framework, apply the CLEAR 5-check, and tag the framework in the Bash invocation comment. Reserve `ENHANCED_PROMPT` for Tier-3 `@prompt-improver` output when a canonical Tier 3 trigger applies.
9. **Prompt construction & model-craft (cli-* family precedence).** Compose every dispatch prompt via the 3-tier rule canonical in `../sk-prompt-small-model/assets/cli_prompt_quality_card.md`:
...
10. **Operator-confirmation gate before cloud handoff.** No cli-devin invocation that mentions "cloud handoff", "hand off to cloud", or `devin cloud` proceeds without the operator's explicit confirmation in the same turn. See `references/cloud_handoff.md`.
...
Related skills: `cli-claude-code` for Anthropic Claude Code, `cli-codex` for OpenAI Codex, `cli-opencode` for full plugin/skill/MCP/Spec-Kit-Memory runtime dispatch, `sk-code` for surface-aware code quality contracts, `mcp-code-mode` for external MCP work, and `system-spec-kit` for packet handback.
