...
> **SWE-1.6 Prompt-Quality Contract (REQUIRED, v1.0.2.0 — updated v1.0.5.0)**: SWE-1.6 is fast and coding-specialized but smaller than the complex-task models — it relies on prompt clarity for reliable output. EVERY cli-devin dispatch with `--model swe-1.6` MUST: (1) be composed through `sk-prompt` (apply a structured framework, **default RCAF — Role/Context/Action/Format**; fall back to STAR for narrative tasks or BUILD for multi-file refactors; run the CLEAR 5-check) before composing the final `--prompt-file` payload, AND (2) include an explicit pre-planning block in the prompt body — the calling AI decomposes the task into ordered steps with acceptance criteria BEFORE handing it to SWE-1.6, rather than asking SWE-1.6 to figure out structure on its own. See `assets/prompt_templates.md` §2 for the canonical SWE-1.6 pre-planning template and `assets/prompt_quality_card.md` §3 for the framework selection rules. Skipping this contract is the single largest cause of underwhelming SWE-1.6 output and must be treated as a soft-block in operator review. **Empirical basis (v1.0.5.0)**: 42-dispatch optimization run (`specs/skilled-agent-orchestration/114-cli-devin-swe16-prompt-optimization/003-eval-loop/synthesis.md`) found RCAF + medium pre-plan + 5-thought sequential_thinking threshold + standard bundle-gate language scored 33% higher than STAR baseline; strict bundle-gate wording HURT scores by pushing SWE 1.6 toward defensive output.
...
12. **SWE-1.6 Prompt-Quality Contract** (v1.0.2.0+ — empirically tuned v1.0.5.0) — EVERY dispatch with `--model swe-1.6` MUST (a) be composed through `sk-prompt` with a chosen framework (**default RCAF**; STAR for narrative-heavy tasks, BUILD for well-defined multi-file refactors) and a passing CLEAR 5-check, AND (b) include an explicit pre-planning block in the prompt body: ordered steps + per-step acceptance criteria + stop conditions + verification approach. SWE-1.6 is coding-specialized but smaller than the complex-task models — it relies on the calling AI doing the structural decomposition upfront. The 003-eval-loop run (42 dispatches, 6 iters) showed RCAF + medium pre-plan beats STAR baseline by 33%; do NOT tighten bundle-gate language beyond "standard" (verbose constraints pushed SWE 1.6 toward defensive output and HURT scores). See `assets/prompt_templates.md` §2 for the canonical pre-planning template and `specs/skilled-agent-orchestration/114-.../003-eval-loop/synthesis.md` for the data.
