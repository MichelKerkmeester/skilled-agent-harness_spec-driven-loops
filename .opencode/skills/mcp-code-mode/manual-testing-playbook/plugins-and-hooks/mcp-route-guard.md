...
```bash
node .opencode/hooks/mcp-route-guard/lib/mcp-route-guard.test.cjs
```
...
```bash
node -e "
const guardCore = require('./.opencode/hooks/mcp-route-guard/lib/mcp-route-guard.cjs');
...
```bash
printf '%s' '{"tool_name":"mcp__claude_ai_ClickUp__clickup_create_task","tool_input":{},"cwd":"'"$PWD"'"}' | node .opencode/hooks/mcp-route-guard/claude/mcp-route-guard.cjs
```

4. Pipe the same payload shape for the unrouteable Webflow case:

```bash
printf '%s' '{"tool_name":"mcp__claude_ai_Webflow__data_cms_tool","tool_input":{},"cwd":"'"$PWD"'"}' | node .opencode/hooks/mcp-route-guard/claude/mcp-route-guard.cjs
```
