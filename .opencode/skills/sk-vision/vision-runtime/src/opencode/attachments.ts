...
/**
 * Auto-injects sk-vision evidence (structured scene read + caption + exact OCR)
    ...
    if (notes.length > 0) {
      extra.push(
        "\n<SK-VISION Atlas>\nPasted/clipboard images were materialized to disk so you can inspect them directly:\n" +
          notes.map((n) => `- ${n}\n`).join("") +
          "Use sk_vision_inspect / sk_vision_ocr / sk_vision_detect with the path above if you need a closer look.\n</SK-VISION>\n",
      );
    }
    ...
    try {
      ...
      return {
        ...
        warn: `Automatic vision analysis could not run right now: ${msg}. ` +
          "The image is materialized to a path below — use sk_vision_inspect(path=...) to analyze it manually.",
        at: Date.now(),
      };
    }
  }
