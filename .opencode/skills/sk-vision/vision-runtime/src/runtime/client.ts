...
export type SkVisionMessage = {
  title?: string;
  message: string;
  variant: "info" | "success" | "warning" | "error";
};

/** Debug logging: only shown when SK_VISION_DEBUG=1. */
function log(...args: unknown[]): void {
  if (process.env.SK_VISION_DEBUG !== "1") return;
...
/**
 * A line-delimited JSON-RPC client over the stdin/stdout of the sk-vision
 * Python runtime.
 *
...
export class RuntimeClient {
  ...
  private readonly uv: string = process.env.SK_VISION_UV ?? "uv";
  private readonly notify: (m: SkVisionMessage) => void;

  constructor(
    opts: {
      pythonPath?: string;
      timeoutMs?: number;
      notify?: (m: SkVisionMessage) => void;
    } = {},
  ) {
    this.pythonPath =
      opts.pythonPath ??
      process.env.SK_VISION_PYTHON ??
      this.resolvePython();
  ...
  private message(m: SkVisionMessage): void {
    try {
      this.notify(m);
    } catch {
   ...
   * use. Manual overrides (SK_VISION_PYTHON, or an existing .venv next to the
   * runtime) win; auto-provision is disabled with SK_VISION_DISABLE_AUTO_PROVISION.
   */
  private async ensurePython(): Promise<string> {
    ...
    if (process.env.SK_VISION_PYTHON) return current;
    ...
    if (process.env.SK_VISION_DISABLE_AUTO_PROVISION) {
      throw new SkVisionError(
        ...
        `interpreter '${current}' lacks 'moondream' and auto-provision is disabled. ` +
          `Set SK_VISION_PYTHON to an environment with moondream installed.`,
      );
    }

    // 3. Auto-provision into a user-scoped venv (survives npm cache rewrites).
    const venv = resolvePath(
      process.env.SK_VISION_VENV_DIR ?? resolvePath(homedir(), ".cache", "sk-vision", "venv"),
    );
      ...
      try {
        ...
        throw new SkVisionError(
          ...
          `uv provisioning failed: ${(err as Error).message}. ` +
            `Remove ${venv} and retry, or set SK_VISION_PYTHON manually.`,
        );
      }
    } else {
  ...
  /** Whether the uv binary is usable (overridable with SK_VISION_UV). */
  ...
  private requestOnce(
    ...
    return new Promise<RpcResponse>((resolve, reject) => {
      ...
      if (res.error) throw new SkVisionError(res.error.code, res.error.message);
      return res.result ?? {};
    });
  }
...
export class SkVisionError extends Error {
