...
import { SkVisionError } from "../runtime/client.js";
...
function cacheDir(): string {
  return process.env.SK_VISION_CACHE_DIR ?? joinPath(homedir(), ".cache", "sk-vision");
}
...
/**
 * VisionProvider backed by the sk-vision Python runtime (default model: moondream2).
 *
...
export class PhotonProvider implements VisionProvider {
  ...
  async caption(request: CaptionRequest): Promise<CaptionResult> {
    ...
    if (typeof res["caption"] !== "string") {
      throw new SkVisionError("handler.no_result", "caption returned no text", true);
    }
  ...
  async scene(request: SceneRequest): Promise<SceneResult> {
    ...
    if (typeof res["scene"] !== "string" || res["scene"].trim() === "") {
      throw new SkVisionError("handler.no_result", "scene analysis returned no text", true);
    }
  ...
  async crop(request: CropRequest): Promise<CropResult> {
    ...
    if (typeof res["path"] !== "string") {
      throw new SkVisionError("EMPTY_RESULT", "crop produced no file", true);
    }
  ...
  async zoom(request: ZoomRequest): Promise<ZoomResult> {
    ...
    if (typeof res["path"] !== "string") {
      throw new SkVisionError("EMPTY_RESULT", "zoom produced no file", true);
    }
    return {
      path: res["path"],
      width: num(res["width"]),
  ...
  async annotate(request: AnnotateRequest): Promise<AnnotateResult> {
    ...
    if (typeof res["path"] !== "string") {
      throw new SkVisionError("EMPTY_RESULT", "annotate produced no file", true);
    }
