...
Moondream vision runtime (default model: moondream2). The OpenCode plugin spawns this process and
...
# the user explicitly asked for debug logs via SK_VISION_DEBUG=1.
if os.environ.get("SK_VISION_DEBUG") != "1":
...
def _cache_dir() -> str:
    base = os.environ.get("SK_VISION_CACHE_DIR") or os.path.join(
        os.path.expanduser("~"), ".cache", "sk-vision"
    )
...
def _ensure_model(model_name: str | None = None) -> Any:
    ...
    model_name = model_name or os.environ.get("SK_VISION_MODEL", DEFAULT_MODEL)
    started = time.time()
    ...
    if os.environ.get("SK_VISION_KV_CACHE_PAGES"):
        try:
            kwargs["kv_cache_pages"] = int(os.environ["SK_VISION_KV_CACHE_PAGES"])
        except ValueError:
            pass
    else:
...
def _yandex_reverse(image_path: str) -> dict:
    ...
    boundary = "----SkVisionBoundary" + secrets.token_hex(8)
    body = io.BytesIO()
