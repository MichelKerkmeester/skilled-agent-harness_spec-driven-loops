...
Prefer CSS first for simple hover/focus states, one-off transitions, and static keyframes that do not need JS state or sequencing. Existing Webflow guidance already encodes this as "CSS first; Motion.dev when you need programmatic control" (Repo: `.opencode/skills/sk-code/webflow/references/implementation/animation_workflows.md`).
