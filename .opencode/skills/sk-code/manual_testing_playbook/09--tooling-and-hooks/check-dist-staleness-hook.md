...
2. The dist-staleness checker resolves: `bash: test -f .opencode/skills/sk-code/code-quality/scripts/check-dist-staleness.sh`.
...
2. **Invoke the checker directly** to confirm the underlying banner logic independent of the hook's subprocess call:
   ```bash
   python3 .opencode/skills/sk-code/code-quality/scripts/check-dist-staleness.sh "$(pwd)/$SOURCE"; echo "direct_exit=$?"
   ```
...
1. **Banner missing, stderr shows `Permission denied` on `check-dist-staleness.sh`**: this is the known current gap (§1). Fix: `chmod +x .opencode/skills/sk-code/code-quality/scripts/check-dist-staleness.sh` (mirrors `check-comment-hygiene.sh`'s existing `-rwxr-xr-x`), then re-run step 3.
...
- `.opencode/skills/sk-code/code-quality/scripts/hooks/claude-posttooluse.sh` — Dispatches both the comment-hygiene and dist-staleness checkers on every `Write`/`Edit`; always exits 0.
