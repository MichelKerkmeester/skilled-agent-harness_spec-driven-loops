...
3. **Introduce an orphan**:
   ```
   bash: mkdir -p .opencode/skills/sk-code/code-opencode/references/zzz-fake-language
   ```
...
6. **Restore**:
   ```
   bash: rmdir .opencode/skills/sk-code/code-opencode/references/zzz-fake-language
   ```
...
- **Created**: 2026-06-13
- **Critical path**: No
- **Destructive**: No (creates and removes a temporary `references/zzz-fake-language/` folder; recovery is the `rmdir` in step 6).
