...
3. The skill advisor at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py` is callable and `skill-graph.json` includes the `sk-code` entry with `signals` array intact.
...
- Skill advisor probe: `python3 .opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py "<prompt>" --threshold 0.8`.
- Bash commands: `bash: <command>`.
- AI agent prompts: `As @<agent>: <instruction>`.
- Resource path notation: paths shown relative to `.opencode/skills/sk-code/` (e.g. `references/router/code_surface_detection.md`).
