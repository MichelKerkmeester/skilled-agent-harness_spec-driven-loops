...
description: "How sk-code resolves the compiled per-hub router contract ahead of its two-axis registry-driven routing, and the legacy fallback that keeps it inert until explicitly authorized."
trigger_phrases:
...
`sk-code`'s `SKILL.md` carries an opt-in, flag-gated, additive directive that asks the compiled per-hub router contract to resolve the mode before falling through to the two-axis registry-driven routing described in [`two-axis-registry-driven-routing.md`](../two-axis-registry-driven-routing/two-axis-registry-driven-routing.md).

The directive is off by default and byte-identical to today's behavior when unset: the compiled front door returns a `{"servingAuthority":"legacy"}` sentinel (or is never meaningfully consulted), and `sk-code` falls back to its registry-driven routing every time.
...
| File | Layer | Role |
|---|---|---|
| `.opencode/skills/sk-code/SKILL.md` | Shared | Carries the opt-in compiled-routing directive `sk-code` follows. |
