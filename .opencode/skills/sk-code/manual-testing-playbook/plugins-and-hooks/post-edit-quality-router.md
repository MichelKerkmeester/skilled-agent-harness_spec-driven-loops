...
- Preconditions:
  ...
  - `.opencode/runtime-hooks/post-edit-quality/claude/claude-posttooluse.cjs` exists (Claude adapter).
  - `.opencode/runtime-hooks/post-edit-quality/lib/post-edit-router.cjs` exists (shared core).
...
printf '%s' "{\"tool_name\":\"Write\",\"tool_input\":{\"file_path\":\"$PWD/$TMPDIR_LIVE/edited.ts\"},\"cwd\":\"$PWD\"}" \
  | node .opencode/runtime-hooks/post-edit-quality/claude/claude-posttooluse.cjs
echo "EXIT_CODE=$?"
rm -rf "$TMPDIR_LIVE"
```
...
printf '%s' "{\"tool_name\":\"Write\",\"tool_input\":{\"file_path\":\"$PWD/$TMPDIR_KILL/edited.ts\"},\"cwd\":\"$PWD\"}" \
  | MK_POST_EDIT_QUALITY_DISABLED=1 node .opencode/runtime-hooks/post-edit-quality/claude/claude-posttooluse.cjs
echo "EXIT_CODE=$?"
rm -rf "$TMPDIR_KILL"
```
...
OK  .opencode/skills/sk-doc/create-flowchart/scripts/validate-flowchart.sh
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "bash -c 'cd \"${CLAUDE_PROJECT_DIR:-$PWD}\" && node .opencode/runtime-hooks/post-edit-quality/claude/claude-posttooluse.cjs'",
      "timeout": 10
    },
    {
      "type": "command",
...
- Claude PostToolUse adapter: `.opencode/runtime-hooks/post-edit-quality/claude/claude-posttooluse.cjs`
- Shared runtime-neutral core: `.opencode/runtime-hooks/post-edit-quality/lib/post-edit-router.cjs`
...
- Checkers dispatched by the router:
  ...
  - `.opencode/skills/sk-doc/create-flowchart/scripts/validate-flowchart.sh`
