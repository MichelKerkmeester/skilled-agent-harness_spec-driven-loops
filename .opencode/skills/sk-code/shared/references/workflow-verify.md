...
- Rebuild before verifying behavior that depends on generated output. For system-spec-memory, the server package documents `dist/context-server.js` as the compiled backend artifact and `npm run build` as the command that builds TypeScript into `dist/`.
