...
> Cross-language commenting rules (WHY-not-WHAT principle, no commented-out code, platform-specific `WEBFLOW:`/`MOTION:`/`LENIS:`/`HLS.JS:` prefixes) live in [`../../shared/cross-language-rules.md`](../../shared/cross-language-rules.md). This section covers JavaScript-specific comment **layout, frequency, and content** patterns observed in production.
...
- [`../quality-standards/init-dom-error-and-async.md`](../quality-standards/init-dom-error-and-async.md) — initialization, DOM safety, error handling, async, observers, validation, performance, animation, state, cleanup, document listener, WeakMap caching
- [`../quick-reference.md`](../quick-reference.md) — JS workflows, code snippets, one-liners
