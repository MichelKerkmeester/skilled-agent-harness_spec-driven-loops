...
semantic role, scoped to one surface's own CSS custom-property block instead of `:root`. Retinting a
component token changes only the surface that declares it. `style.css` carries exactly three such
families — this reference is the full list, what each token resolves to, and the exact blast radius of
retinting one.

---

## 1. `--model-sheet-*` (surface `model-effort-sheet`)
...
panel, the diff view, artifacts, or the composer (measured; see `retint-recipes.md` §2).

---

## 2. `--slash-*` (surface `slash-autocomplete`)
...
## 3. `--diff-add` / `--diff-remove` (consumed by two surfaces, not per-surface-aliased)
