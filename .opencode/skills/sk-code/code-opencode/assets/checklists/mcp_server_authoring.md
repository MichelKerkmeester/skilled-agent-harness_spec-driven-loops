...
- Use this when authoring a new MCP server under an existing skill-local `mcp_server/` directory, such as `.opencode/skills/system-spec-kit/mcp_server/` or `.opencode/skills/system-code-graph/mcp_server/`.
...
- [ ] Read native MCP examples at `.opencode/skills/system-spec-kit/mcp_server/`, `.opencode/skills/system-code-graph/mcp_server/`, `.opencode/skills/system-skill-advisor/mcp_server/`, and `.opencode/skills/mcp-code-mode/mcp_server/`.
- [ ] Identify the startup pattern in `opencode.json`.
  ...
  - Direct-dist pattern: `code_mode` runs `/Users/michelkerkmeester/.nvm/versions/node/v24.9.0/bin/node` with `.opencode/skills/mcp-code-mode/mcp_server/dist/index.js` directly.
...
- [ ] Run the owning package build after TypeScript edits. Verified package build scripts include `.opencode/skills/system-spec-kit/mcp_server/package.json`, `.opencode/skills/system-spec-kit/scripts/package.json`, `.opencode/skills/system-code-graph/package.json`, `.opencode/skills/system-skill-advisor/mcp_server/package.json`, and `.opencode/skills/mcp-code-mode/mcp_server/package.json`.
...
- Direct-dist example: `.opencode/skills/mcp-code-mode/mcp_server/dist/index.js`
- MCP server examples: `.opencode/skills/system-spec-kit/mcp_server/`, `.opencode/skills/system-code-graph/mcp_server/`, `.opencode/skills/system-skill-advisor/mcp_server/`, `.opencode/skills/mcp-code-mode/mcp_server/`
- Verification recipes: `.opencode/skills/sk-code/code-opencode/assets/checklists/typescript_checklist.md`, `.opencode/skills/sk-code/code-opencode/assets/checklists/python_checklist.md`
