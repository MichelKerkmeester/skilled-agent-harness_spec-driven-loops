---
title: opencode
description: Read-only OPENCODE surface evidence for the sk-code hub — TypeScript/Python/shell/config standards, language-agnostic patterns, hooks, alignment verification, and authoring checklists. Bundled alongside a workflow mode; never a primary.
trigger_phrases:
...
| **Carries** | System-code evidence for the `.opencode/` tree: TypeScript/Python/shell/config standards, language-agnostic patterns, hook contracts, alignment verification, and skill/agent/command/MCP authoring checklists. |
...
- `references/` — `typescript/`, `python/`, `shell/`, `config/`, `javascript/` (language standards); `shared/` (language-agnostic patterns, hooks, alignment verification).
