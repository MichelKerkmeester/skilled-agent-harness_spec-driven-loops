...
> **IMPORTANT:** The canonical binary is `figma-ds-cli` (silships, MIT). The npm package literally named `figma-cli` is an UNRELATED tool (unic/figma-cli, bin `figma`, "export and scaffold from Figma into style guides") — **NEVER `npm i -g figma-cli`**. The `figma-cli` command only exists when installed from the silships repo, and even then it may shadow the unic package on PATH; this skill invokes `figma-ds-cli` throughout.
...
### Core Principle
figma-ds-cli drives the live Figma **Desktop** session over a local HTTP daemon — there is no Figma API key and nothing renders without Figma Desktop open on a file. Identify the binary first, confirm Figma Desktop is open and the daemon is healthy, then act, gating every command by its mutation class.
...
- The only version published to npm is **`figma-ds-cli@1.0.0`** (versions array = `["1.0.0"]`); its `bin` exposes **only `figma-ds-cli`** → `src/index.js`. **[CONFIRMED — `npm view`, npm 11.9.0]**
- The repo `main` branch is **1.2.0**, whose `package.json` exposes **both** `figma-ds-cli` and `figma-cli`, both mapped to `src/index.js`. This version is **unpublished** — `figma-ds-cli@1.2.0` returns E404 on npm. So "install the pinned `figma-ds-cli@1.2.0` from npm" is **impossible**; the newer 1.2.0 only comes from the silships git repo. **[CONFIRMED — `npm view` E404 + repo `package.json`]**
...
| **OS** | **macOS is the supported baseline.** Source carries Windows/Linux platform helpers, but the docs treat those as "coming" / unverified — do not promise non-macOS behavior. | **[CONFIRMED — `docs/ARCHITECTURE.md`, `src/platform.js`]** |
...
```bash
figma-ds-cli unpatch            # illustrative; rollback — restores the original app.asar string
```
...
| Idle timeout | 60 minutes in executable code (a header comment says 10 minutes — the code value wins). Not reboot-persistent. | **[CONFIRMED — `src/daemon.js`]** |
...
All illustrative; verify with `figma-ds-cli daemon --help`. A token mismatch surfaces as "Unauthorized: Invalid or missing token" — run `daemon diagnose` then `daemon restart` (never auto-delete the token). **[CONFIRMED — `REFERENCE.md`]**
...
| **ARBITRARY** | `eval` / `raw` / `run` — can do anything. | **Treat as mutating; review the code/command first** even when the prompt sounds exploratory. |
...
`eval [code] [-f file]`, `run <file>` (eval-file alias), `raw <command...>`, and `fj eval` are arbitrary Figma Plugin API / figma-use execution — **review first**. Generated agent rules explicitly say never use `eval` to create visual nodes (it bypasses positioning/guards); use `render` / `render-batch` instead. **[CONFIRMED — `CLAUDE.md`, per-command table]**

---

## 6. GROUPED COMMAND REFERENCE

From the source-verified per-command table (`iter-001.out`, ~130 rows). Examples are **illustrative — verify exact flags with `<bin> <group> --help`** before running. Class tags: **RO** read-only · **MUT** mutating · **DESTR** destructive.
...
- **`extract`** writes a `DESIGN.md` with **11 sections** (Identity, Structure, Color, Typography, Spacing & Layout, Depth & Motion, Components, States, Rules, Extending this system, **Machine-readable tokens**). It **auto-splits** a structure section above ~50k tokens into a `DESIGN-structure/` directory — handle multi-file output. Section 11 is a ```json design-tokens fence (color / typography / spacing / radius / shadow / fonts / meta). **[CONFIRMED — `src/design-extract.js`, `src/design-md.js`, `docs/COMMANDS.md`]**
- **`import`** accepts `DESIGN.md`, `tailwind.config.{js,cjs,ts}`, CSS (`globals.css` / `styles.css`), W3C / Style-Dictionary `tokens.json`, a Storybook URL, or `./storybook-static/`. A Storybook import yields component names/variants only (**no variables**) — combine with a Tailwind/CSS import for tokens. **[CONFIRMED — `docs/COMMANDS.md`, `src/commands/setup.js`]**
...
> **No-overwrite rule:** `extract` (and every `export*`) writes local files — always pass an explicit output path and never silently overwrite an existing file.
...
> **Primitive drift:** `REFERENCE.md` lists some primitives as `create rect/circle/...`; the source scan proved `create frame/icon/image` **plus** the top-level aliases above. `CLAUDE.md` also mentions `voice` / `chat` / `prop combine`, which the source scan did **not** find. Prefer verified commands and confirm with `--help` before emitting any of these. **[CONFIRMED-as-uncertain — `REFERENCE.md` vs source scan]**
...
**`var:name` binding syntax.** Token references use `var:name` in `render` / `create` / `set` (e.g. `bg="var:card"`, `color="var:foreground"`, `figma-ds-cli set fill "var:primary"`). `var:collection:name` pins a specific collection; `render` / `render-batch` accept `--collection <name>` to pin the whole render. **[CONFIRMED — `CLAUDE.md`]**
...
> Read-only, but these write files — require an explicit `-o` path and never overwrite silently.
...
These groups mix classes per verb — see [tool_surface.md](tool_surface.md) for the full per-verb tags.
...
> The **official Figma Dev Mode MCP is OUT OF SCOPE for this 0.1.0 release** — do not document it as a supported path. It is at most a future option (e.g. via an `mcp-remote` stdio bridge), not part of this skill's contract.
...
1. **Published-npm `figma-cli` binary behavior.** npm latest is `1.0.0` (only `figma-ds-cli` bin); repo `main` is `1.2.0` (both bins) and unpublished. Whether a `figma-cli` command exists after install depends entirely on the install source — verify the binary on PATH and confirm silships, not unic. **[CONFIRMED gap]**
2. **`voice` / `chat` / `prop combine` commands.** `CLAUDE.md` references them; the source command-registration scan did **not** find them. Likely docs ahead of registered CLI — verify with `--help` before emitting. **[CONFIRMED gap]**
...
6. **Exact command examples.** Every `figma-ds-cli …` example here is illustrative and unrun — confirm flags with `<bin> <group> --help` before use. **[CONFIRMED gap]**
...
- Upstream: [silships/figma-cli](https://github.com/silships/figma-cli) (npm `figma-ds-cli`, MIT). This skill documents driving the installed tool from the terminal; it does not vendor or redistribute it.
