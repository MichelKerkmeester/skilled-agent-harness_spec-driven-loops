...
description: "Per-tool contract reference for the 11 MCP tools exposed by mk-code-index: purpose, handler file, key preconditions, and token budget."
trigger_phrases:
...
The 11 MCP tools registered by `mk-code-index` with the handler file, primary purpose, key preconditions, and token budget.
...
- **CCC bridge (3):** `ccc_status`, `ccc_reindex`, `ccc_feedback` — coordinate with CocoIndex (semantic search).
...
| 4 | `code_graph_context` | Read | Build LLM-oriented compact graph neighborhoods. Accepts CocoIndex seeds, manual seeds, or graph seeds. Modes: `neighborhood`, `outline`, `impact`. | `readiness === "fresh"`. Returns `blocked` with `requiredAction: "code_graph_scan"` otherwise. | 1200 |
...
| 9 | `ccc_status` | CCC | Check CocoIndex availability: `available`, `binaryPath`, `indexExists`, `indexSize`, `recommendation`. | None — probe is non-destructive. | n/a |
| 10 | `ccc_reindex` | CCC | Trigger CocoIndex incremental (or full) re-index. | CocoIndex binary must be installed and on PATH. | n/a |
| 11 | `ccc_feedback` | CCC | Submit quality feedback on CocoIndex search results (`helpful` / `not_helpful` / `partial`). | `query` and `rating` are required. | n/a |
...
| `ccc_status` | `tool-schemas.ts` (`cccStatus`) | `tools/code-graph-tools.ts` → `lib/ccc/` |
| `ccc_reindex` | `tool-schemas.ts` (`cccReindex`) | `tools/code-graph-tools.ts` → `lib/ccc/` |
| `ccc_feedback` | `tool-schemas.ts` (`cccFeedback`) | `tools/code-graph-tools.ts` → `lib/ccc/` |
