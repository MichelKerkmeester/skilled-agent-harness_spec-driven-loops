...
Cross-skill consumers reach the data through two paths: in-process imports from `system-spec-kit` handlers and hooks, and the warm daemon — which serves both the standalone MCP namespace used by agents and commands and the additive daemon-backed `code-index` CLI shim used as a warm-only fallback by the OpenCode plugin bridge. SessionStart hooks remain under `system-spec-kit/mcp_server/hooks/` due to 110-plus file references. This asymmetry is intentional and any future move is planned as a separate packet with build and config redesign scope.
