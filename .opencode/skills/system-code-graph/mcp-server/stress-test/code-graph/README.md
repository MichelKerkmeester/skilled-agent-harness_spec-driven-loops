...
`stress-test/code-graph/` contains Vitest stress coverage for scan breadth, context generation, degraded-mode behavior, change detection, manual diagnostics and denial-of-service caps. These tests exercise paths that are broader or more adversarial than normal unit tests.
...
```bash
.opencode/skills/system-code-graph/node_modules/.bin/vitest --config .opencode/skills/system-code-graph/vitest.config.ts --run stress-test/code-graph
python3 .opencode/skills/sk-doc/scripts/validate_document.py --type readme .opencode/skills/system-code-graph/mcp-server/stress-test/code-graph/README.md
```
