...
`mcp-server/tools/` owns the MCP tool dispatch surface for code graph operations. It maps incoming tool names to handler functions, performs shallow required-argument checks and keeps the server export small.
...
| `index.ts` | Re-exports the tool dispatch module and provides `dispatch()` for `mcp-server/index.ts`. |
...
| Exports | `index.ts` is the public module consumed by `mcp-server/index.ts`. |
