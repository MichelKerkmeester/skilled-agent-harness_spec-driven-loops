 ...
 * mcp-server/{lib,dist/...}`, so anchoring on the on-path `.opencode` segment
...
export function defaultExcludeRuleConfidencePath(): string {
  ...
  for (let i = 0; i < 12; i += 1) {
    if (basename(current) === '.opencode') {
      return join(
        dirname(current), '.opencode', 'skills', 'system-code-graph', 'mcp-server', 'data',
  ...
  return join(process.cwd(), '.opencode', 'skills', 'system-code-graph', 'mcp-server', 'data', DEFAULT_EXCLUDE_RULE_CONFIDENCE_FILENAME);
}
