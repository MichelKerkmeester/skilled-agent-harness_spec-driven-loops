...
import type {
  ...
  WeightedWalkResult,
} from '../../../system-spec-kit/mcp-server/dist/lib/graph/bfs-traversal.js';

type MemoryWeightedWalkModule = typeof import('../../../system-spec-kit/mcp-server/dist/lib/graph/bfs-traversal.js');
...
export function resolveMemoryWeightedWalkModuleUrl(): URL {
  const candidates = [
    new URL('../../../system-spec-kit/mcp-server/dist/lib/graph/bfs-traversal.js', import.meta.url),
    new URL('../../../../system-spec-kit/mcp-server/dist/lib/graph/bfs-traversal.js', import.meta.url),
  ];
...
function shouldUseSeededPprRanking(mode: QueryMode): boolean {
  return mode === 'impact' && seededPprRankingEnabled();
}
...
function contextEdgeTransitionWeight(edge: graphDb.CodeEdgeTargetResult['edge'] | graphDb.CodeEdgeSourceResult['edge']): number {
  const reliability = contextEdgeReliability(edge);
  if (reliability > 0) {
