...
function createSymlinkFixtures() {
  ...
  const blockedTarget = writeFixture(root, 'z-future/evil.md', '# blocked\n');
...
function createCodeGraphSymlinkFixtures() {
  ...
  const blockedTarget = writeFixture(root, 'z-future/blocked.ts', 'export const blocked = true;\n');
...
describe('realpath hardening for symlinked paths', () => {
  it('blocks memory_save when a safe-looking spec path resolves into z_future', async () => {
    ...
    try {
      vi.doMock('../../../system-spec-kit/mcp-server/core/index.js', async (importOriginal) => {
        const actual = await importOriginal<typeof import('../../../system-spec-kit/mcp-server/core/index.js')>();
        return {
          ...actual,
      ...
      vi.doMock('../../../system-spec-kit/mcp-server/utils/validators.js', async (importOriginal) => {
        const actual = await importOriginal<typeof import('../../../system-spec-kit/mcp-server/utils/validators.js')>();
        return {
          ...actual,
      ...
      vi.doMock('../../../system-spec-kit/mcp-server/utils/index.js', async (importOriginal) => {
        const actual = await importOriginal<typeof import('../../../system-spec-kit/mcp-server/utils/index.js')>();
        return {
          ...actual,
      ...
      const { handleMemorySave } = await import('../../../system-spec-kit/mcp-server/handlers/memory-save.js');
      ...
      expect(envelope.data.details.canonicalPath).toContain('/z-future/');
    } finally {
      database.close();
