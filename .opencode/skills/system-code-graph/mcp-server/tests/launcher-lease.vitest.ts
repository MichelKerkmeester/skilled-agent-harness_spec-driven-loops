...
const pidFileRelativePath = '.opencode/skills/system-code-graph/mcp-server/database/.mk-code-index-launcher.json';
const ownerLeaseRelativePath = '.opencode/skills/system-code-graph/mcp-server/database/.code-graph-owner.json';
...
function createWorkspace(options: { ignoreChildSigterm?: boolean } = {}): { root: string; launcherPath: string; pidFilePath: string } {
  ...
  const directServer = join(root, '.opencode/skills/system-code-graph/mcp-server/dist/index.js');
