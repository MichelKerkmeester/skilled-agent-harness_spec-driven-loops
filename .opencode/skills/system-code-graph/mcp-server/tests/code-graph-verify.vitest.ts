...
describe('code-graph verify', () => {
  ...
  describe('handleCodeGraphVerify', () => {
    ...
    it('resolves the default gold-battery path inside the skill, never a spec folder', async () => {
      ...
      // the skill's own mcp-server/assets/ directory. It must never resolve into a
      ...
      expect(DEFAULT_VERIFY_BATTERY_PATH).toContain('mcp-server/assets/code-graph-gold-queries.json');
