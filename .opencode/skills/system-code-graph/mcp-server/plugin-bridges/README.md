...
`mcp-server/plugin-bridges/` owns the prompt-safe CLI bridge that connects OpenCode plugin injection to the warm code-index daemon. It lives outside the `.opencode/plugins/` directory to avoid automatic OpenCode plugin discovery while still enabling targeted plugin injection during context compaction.
...
```bash
node .opencode/skills/system-code-graph/mcp-server/plugin-bridges/mk-code-graph-bridge.mjs --minimal --spec-folder <spec-folder>
```
