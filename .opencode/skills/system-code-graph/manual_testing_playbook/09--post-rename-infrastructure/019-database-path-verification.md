...
Verify the code-graph SQLite database lives at the correct active path (.opencode/skills/system-code-graph/mcp_server/database/code-graph.sqlite) and no legacy database paths are in use.
...
- Expected execution process: Check `.opencode/skills/system-code-graph/mcp_server/database/code-graph.sqlite` exists and is non-empty. Verify code_graph_status reports consistent dbFileSize.
...
1. `ls -la .opencode/skills/system-code-graph/mcp_server/database/code-graph.sqlite`.
2. Call `code_graph_status({})` and verify `dbFileSize` matches approximately.
3. Verify no legacy `.opencode/skills/system_code_graph/` or `.opencode/system-code-graph/database/` paths exist.
