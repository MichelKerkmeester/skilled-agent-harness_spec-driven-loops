...
import { EDGE_DRIFT_JSD_THRESHOLD, EDGE_DRIFT_PSI_THRESHOLD, EDGE_DRIFT_SHARE_THRESHOLD, buildEdgeDistribution, computeEdgeShare, computeJSD, computePSI, } from '../lib/edge-drift.js';
import { getGraphFreshness, getGraphReadinessSnapshot } from '../lib/ensure-ready.js';
import { parseIndexScopePolicyFromFingerprint, resolveIndexScopePolicy, } from '../lib/index-scope-policy.js';
...
function getParseDiagnosticsForStatus() {
...
function buildEdgeDriftSummary(edgesByType) {
    ...
    const shareDrift = Object.fromEntries(Object.keys(currentShare).map((edgeType) => [
        edgeType,
        currentShare[edgeType]
            - baselineShare[edgeType],
    ]));
    const psi = computePSI(currentShare, baselineShare);
    const jsd = computeJSD(currentShare, baselineShare);
    const maxShareDrift = Object.values(shareDrift).reduce((max, value) => Math.max(max, Math.abs(value)), 0);
    return {
...
function getVerificationPassPolicy(verification) {
    ...
    if (typeof overall_pass_rate !== 'number'
...
function getVerificationTimestampMs(verification, lastPersistedAt) {
...
function getGoldVerificationTrust(verification, freshness, lastPersistedAt) {
    if (!verification) {
        return 'absent';
    }
    if (freshness !== 'fresh') {
...
export async function handleCodeGraphStatus() {
    ...
    try {
        ...
        const goldVerificationTrust = getGoldVerificationTrust(lastGoldVerification, freshness, stats.lastScanTimestamp);
