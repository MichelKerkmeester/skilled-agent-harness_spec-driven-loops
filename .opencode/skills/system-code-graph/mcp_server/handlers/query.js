...
import { attachGraphEdgeEnrichment, attachStructuralTrustFields, } from '../../../system-spec-kit/mcp_server/lib/context/shared-payload.js';
import { buildQueryTrustMetadata, buildReadinessBlock, } from '../lib/readiness-contract.js';
...
const SUPPORTED_EDGE_TYPE_SET = new Set(SUPPORTED_EDGE_TYPES);
...
const SUPPORTED_RELATIONSHIP_OPERATION_SET = new Set(SUPPORTED_RELATIONSHIP_OPERATIONS);
/**
...
function isSupportedEdgeType(edgeType) {
    return SUPPORTED_EDGE_TYPE_SET.has(edgeType);
}
function isSupportedRelationshipOperation(operation) {
    return SUPPORTED_RELATIONSHIP_OPERATION_SET.has(operation);
}
function buildUnknownOperationResponse(operation) {
    return {
        content: [{
                type: 'text',
...
function resolveRequestedEdgeType(args) {
...
function querySubjectMatches(field, subject) {
...
function relationshipEdgeCount(candidate, operation, edgeType) {
    if (!candidate.symbolId) {
        return 0;
    }
    const resolvedEdgeType = edgeType ?? (operation.startsWith('calls')
        ? 'CALLS'
...
function callableKindRank(kind) {
    switch (kind) {
        case 'function':
        case 'method':
            return 0;
...
function pickOperationAwareCandidate(candidates, operation, edgeType) {
    const rankedCandidates = candidates
        ...
        .sort((left, right) => ((right.operationEdgeCount ?? 0) - (left.operationEdgeCount ?? 0)
...
function buildAmbiguousSubjectWarning(subject, matchField, matchResult, selectedCandidate, selectionReason) {
...
function resolveSubject(subject, operation, edgeType) {
    const d = graphDb.getDb();
...
// R22-001, R23-001).
function buildGraphQueryPayload(payload, readiness, label, graphEdgeEnrichment) {
    const { readiness: _incomingReadiness, ...rest } = payload;
...
function excludeDanglingEdges(entries, limit, operation, missingEndpoint, isResolved, getDanglingId) {
    const resolvedEntries = [];
    const danglingSymbolIds = [];
...
function transitiveTraversal(startId, edgeType, direction, maxDepth, limit, operation) {
    const visited = new Set();
    const resultSymbolIds = new Set();
    const results = [];
    const danglingSymbolIds = [];
    let frontier = [{ id: startId, depth: 0 }];
    const missingEndpoint = direction === 'from' ? 'target' : 'source';
...
function classifyEdgeEvidenceClass(edgeType, metadata) {
    switch (edgeType) {
        case 'IMPORTS':
        case 'EXPORTS':
            return 'import';
...
function clampNumericConfidence(value) {
...
function sanitizeEdgeMetadataReadString(value) {
    if (typeof value !== 'string')
        return null;
    if (value.length === 0)
        return null;
    if (value.length > EDGE_METADATA_REASON_MAX_LENGTH)
        return null;
    if (EDGE_METADATA_REASON_BLOCKED.test(value))
        return null;
    return value;
}
function edgeMetadataOutput(edge) {
    return {
        ...
        edgeEvidenceClass: classifyEdgeEvidenceClass(edge.edgeType, edge.metadata),
    };
}
function mapOutboundRelationshipEdge(entry, options) {
    return {
...
function mapInboundRelationshipEdge(entry, options) {
    return {
...
function extractOutboundFilePaths(entries) {
    return entries
        .map((entry) => entry.targetNode?.filePath)
        .filter((value) => typeof value === 'string');
}
function extractInboundFilePaths(entries) {
    return entries
        .map((entry) => entry.sourceNode?.filePath)
        .filter((value) => typeof value === 'string');
}
const EDGE_EVIDENCE_CLASS_WEAKNESS = {
...
function summarizeWeakestGraphEdgeEnrichment(edges) {
    let weakest = null;
    for (const edge of edges) {
        ...
        if (edge.numericConfidence === weakest.numericConfidence
...
function buildFallbackDecision(readiness) {
...
function buildBlockedReadPayload(readiness, operation, subject) {
...
function buildHotFileBreadcrumbs(filePaths) {
...
function parseEdgeMetadataConfidence(metadataJson, weight) {
...
function queryImportDependentsForBlastRadius(minConfidence) {
    if (minConfidence <= 0) {
...
function buildDepthGroups(affectedFiles, maxDepth) {
    const depthGroups = {};
 ...
 */
function classifyBlastRadiusRisk(depthGroups, ambiguityCandidates) {
...
function buildPartialBlastRadiusResult(sourceFiles, nodes, affectedFiles, depthGroups) {
    return {
        sourceFiles,
        nodes,
        affectedFiles,
...
function computeBlastRadius(sourceFiles, maxDepth, limit, minConfidence = 0, ambiguityCandidates = []) {
    ...
    const breadcrumbByFile = new Map(buildHotFileBreadcrumbs([
        ...normalizedSources,
        ...
        ...(overflowed
            ? {
                failureFallback: {
                    ...
                    code: 'limit_reached',
                    partialResult: buildPartialBlastRadiusResult(normalizedSources, [...seedNodes, ...affectedFiles], affectedFiles, depthGroups),
                },
            }
            : {}),
    };
...
export async function handleCodeGraphQuery(args) {
    let readiness = {
        freshness: 'empty',
        action: 'none',
    ...
    if (operation === 'outline') {
        ...
        const outlineNodes = limited.map(n => ({
            name: n.name,
            kind: n.kind,
            fqName: n.fqName,
            line: n.startLine,
    ...
    if (operation === 'blast_radius') {
        ...
        for (const candidate of rawSubjects) {
            ...
            let resolvedPathCandidate = null;
                            ...
                            text: JSON.stringify({
                                ...
                                data: buildGraphQueryPayload({
                                    ...
                                    failureFallback: {
                                        ...
                                        partialResult: buildPartialBlastRadiusResult(sourceFiles, preservedSeedNodes, [], preservedDepthGroups),
                                    },
        ...
        try {
            ...
            console.warn(`[code-graph-query] blast_radius compute failure: ${reason} (sources=${sourceFiles.length})`);
            try {
                if (isSpeckitMetricsEnabled()) {
                    speckitMetrics.incrementCounter('spec_kit.graph.blast_radius_failure_total', { code: 'compute_error' });
                }
            }
            catch (_metricErr) {
    ...
    if (args.includeTransitive) {
        ...
        const transitive = graphDb.getDb().transaction(() => transitiveTraversal(symbolId, requestedEdgeType ?? 'CALLS', direction, maxDepth, limit, operation))();
    ...
    switch (operation) {
        ...
        case 'imports_from': {
            ...
            const edges = graphDb.getDb().transaction(() => graphDb.queryEdgesFrom(symbolId, requestedEdgeType))();
            const { resolvedEntries, warnings } = excludeDanglingEdges(edges, limit, operation, 'target', (entry) => entry.targetNode != null, (entry) => entry.edge.targetId);
            result = {
                operation,
                symbolId,
                ...(resolvedSubject.selectedCandidate ? { selectedCandidate: resolvedSubject.selectedCandidate } : {}),
        ...
        case 'imports_to': {
            ...
            const edges = graphDb.getDb().transaction(() => graphDb.queryEdgesTo(symbolId, requestedEdgeType))();
            const { resolvedEntries, warnings } = excludeDanglingEdges(edges, limit, operation, 'source', (entry) => entry.sourceNode != null, (entry) => entry.edge.sourceId);
            result = {
                operation,
                symbolId,
                ...(resolvedSubject.selectedCandidate ? { selectedCandidate: resolvedSubject.selectedCandidate } : {}),
