...
import { parseUnifiedDiff, rangesOverlap } from '../lib/diff-parser.js';
...
function blockedResponse(reason, readiness) {
...
function errorResponse(reason, readiness) {
    return buildResponse({
        status: 'error',
        affectedSymbols: [],
        affectedFiles: [],
 ...
 */
function readinessRequiresBlock(readiness) {
...
function checkPathByteSafety(candidate) {
...
function checkPathContainment(candidate, canonicalRootDir) {
...
function resolveCandidatePath(diffFile, canonicalRootDir) {
...
export async function handleDetectChanges(args) {
    ...
    if (readinessRequiresBlock(readiness)) {
        const blockedFreshness = readiness.verificationGate === 'fail'
            ...
            : readiness.freshness;
        return blockedResponse(`graph readiness is "${blockedFreshness}" (action: ${readiness.action}); run code_graph_scan before detect_changes. Reason: ${readiness.reason}`, readinessBlock);
    }
    ...
    const affectedSymbolsById = new Map();
    ...
    for (const diffFile of parsed.files) {
        ...
        for (const hunk of diffFile.hunks) {
            ...
            for (const node of fileNodes) {
                ...
                const overlapsPostImage = hunk.newLines > 0 && rangesOverlap(node.startLine, nodeLines, hunk.newStart, hunk.newLines);
                const overlapsPreImage = hunk.oldLines > 0 && rangesOverlap(node.startLine, nodeLines, hunk.oldStart, hunk.oldLines);
                if (!overlapsPostImage && !overlapsPreImage)
                    continue;
                if (affectedSymbolsById.has(node.symbolId))
                    continue;
