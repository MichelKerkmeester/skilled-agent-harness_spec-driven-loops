...
import { hasCrossFileCallResolutionActivity, resolveCrossFileCallEdges, } from '../lib/cross-file-edge-resolver.js';
...
import { DEFAULT_GOLD_BATTERY_PATH, executeBattery, loadGoldBattery, } from '../lib/gold-query-verifier.js';
...
function summarizeDetectorProvenance(results) {
    const counts = {};
...
function summarizeGraphEdgeEnrichment(results) {
    let best = null;
    for (const result of results) {
...
function classifyParseErrorFiles(results) {
...
function countPersistableNodes(results) {
    return results.reduce((total, result) => (result.parseHealth === 'error' ? total : total + result.nodes.length), 0);
}
function computeParseErrorRatio(parseErrorCount, totalFiles) {
    if (totalFiles === 0) {
        return 0;
    }
    return parseErrorCount / totalFiles;
}
function formatParseDiagnosticMessage(result) {
    return result.parseErrors.length > 0
        ? result.parseErrors.join('; ')
        : 'Parser returned parseHealth="error" without diagnostics';
}
function recordParseDiagnosticsForResults(results) {
    for (const result of results) {
...
function cleanupMissingTrackedFiles(filePaths) {
...
function summarizeEdgeDistribution(results) {
...
function hasUsablePersistedEdgeDistributionBaseline() {
...
function relativize(absPath, workspaceRoot) {
...
function relativizeScanMessage(message, workspaceRoot) {
...
function relativizeScanWarning(warning, workspaceRoot) {
    return relativizeScanMessage(warning, workspaceRoot);
}
export function relativizeScanError(error, workspaceRoot) {
    return relativizeScanMessage(error, workspaceRoot);
}
function relativizeParseDiagnostics(summary, workspaceRoot) {
    return {
...
export async function handleCodeGraphScan(args) {
    ...
    if (scopeChangePromotionBlocked) {
        ...
        console.warn(`[code-graph-scan] Blocked scope-change full scan promotion over existing graph (${priorNodeCount} prior node(s)); stored scope ${storedScope.fingerprint} differs from candidate scope ${candidateFingerprint}; pass forceScopeChange: true to allow scope replacement.`);
    ...
    if (zeroNodePromotionBlocked) {
        ...
        console.warn(`[code-graph-scan] Blocked zero-node full scan promotion over existing graph (${priorNodeCount} prior node(s)); pass forceZeroNodeReset:true to allow destructive reset.`);
        const parseDiagnostics = relativizeParseDiagnostics(graphDb.getParseDiagnosticsSummary(), canonicalWorkspace);
        const skipListSummary = getSkipListSummary();
        const readinessBlock = buildReadinessBlock({
            freshness: 'stale',
    ...
    for (const result of results) {
        ...
        if (effectiveIncremental
            && !graphDb.isFileStale(result.filePath, result.contentHash ? { currentContentHash: result.contentHash } : undefined)) {
            filesSkipped++;
            continue;
        }
        try {
