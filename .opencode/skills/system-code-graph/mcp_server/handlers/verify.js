...
function buildScopeMismatchInfo(preflight) {
...
function applyCategoryFilter(battery, category) {
    if (!category) {
        return battery;
    }
...
function isAllowlistedBatteryPath(canonicalWorkspace, canonicalBatteryPath) {
    const allowlistedBases = [
        ...
        resolve(canonicalWorkspace, '.opencode/skills/system-code-graph/mcp_server'),
    ];
    return allowlistedBases.some((basePath) => {
        const candidateRelative = relative(basePath, canonicalBatteryPath);
        if (candidateRelative.length === 0
...
function resolveVerifyPaths(args) {
...
export async function handleCodeGraphVerify(args) {
