...
const DEFAULT_EXCLUDED_FIXTURES = [
  { label: 'skills', modeFlag: 'includedSkills', path: '.opencode/skills/sk-doc/SKILL.md' },
  { label: 'agents', modeFlag: 'includeAgents', path: '.opencode/agents/context.md' },
  { label: 'commands', modeFlag: 'includeCommands', path: '.opencode/commands/doctor/code-graph.md' },
...
function isIncludedByDoctorScope(path: string, policy: DoctorActiveScopePolicy): boolean {
  ...
  if (path.startsWith('.opencode/agents/')) return policy.includeAgents;
  if (path.startsWith('.opencode/commands/')) return policy.includeCommands;
  ...
  if (!path.startsWith('.opencode/skills/')) return true;
  if (policy.includedSkills === 'all') return true;
  if (policy.includedSkills === 'none') return false;
  return policy.includedSkills.some(skill => path.startsWith(`.opencode/skills/${skill}/`));
}
...
describe('cg-017 — doctor apply mode', () => {
  ...
  it('cg-017 documents activeScope, granular includeSkills, and default-exclude rejection across doctor assets', () => {
    ...
    expect(applyYaml).toContain('.opencode/agents/**');
    expect(applyYaml).toContain('.opencode/commands/**');
