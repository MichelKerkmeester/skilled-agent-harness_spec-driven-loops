// ───────────────────────────────────────────────────────────────────
// MODULE: Ccc Integration Stress tests
// ───────────────────────────────────────────────────────────────────
...
import { closeDb, initDb, replaceNodes, upsertFile } from '../../lib/code-graph-db.js';
...
const cccSkipReason = 'skipped: ccc binary unavailable at .opencode/skills/mcp-coco-index/mcp_server/.venv/bin/ccc';
...
describe('cg-014,015,016 — CCC integration', () => {
  ...
  cccIt(cccName('cg-015 appends helpful feedback JSONL inside the disposable workspace'), async () => {
    const parsed = parseResponse(await handleCccFeedback({
      ...
      resultFile: 'mcp_server/handlers/query.ts',
      rating: 'helpful',
      comment: 'stress path',
    }));
