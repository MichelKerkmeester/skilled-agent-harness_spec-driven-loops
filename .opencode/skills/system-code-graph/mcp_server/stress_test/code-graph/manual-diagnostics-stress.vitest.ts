...
import { recordCandidateManifest } from '../../lib/ensure-ready.js';
import { executeBattery, loadGoldBattery, type GoldBattery } from '../../lib/gold-query-verifier.js';
import { generateContentHash, type CodeEdge, type CodeNode } from '../../lib/indexer-types.js';
...
const BATTERY_PATH = join(TEST_DIR, '..', '..', 'tests', 'assets', 'code-graph-gold-queries.json');
...
function seedFile(relativePath: string, content: string, nodes: CodeNode[], edges: CodeEdge[] = []): string {
  ...
  setCodeGraphScope(resolveIndexScopePolicy());
  recordCandidateManifest([filePath]);
  return filePath;
}
