...
// ───────────────────────────────────────────────────────────────
// The sibling code-graph-seeded-ppr-ranking suite proves the multi-hop ranking
// behaviour with the flag stubbed on. This suite closes a narrower gap: it pins
// the GATE itself — that SPECKIT_CODE_GRAPH_SEEDED_PPR_RANKING flips the impact
// path from the flat single-hop baseline to the seeded-PPR walk, so the same
// controlled graph yields a DIFFERENT (not byte-identical) caller ordering.
//
// Mocked graph keeps the assertion deterministic and side-effect-free; the live
// ON-DB delta is measured separately by scripts/eval/score-seeded-ppr-retrieval.
...
// A two-hop caller fan-in: direct -> alpha, middle -> alpha, central -> middle.
// Flat ranking only sees the DIRECT callers; the seeded-PPR walk also surfaces
// the deeper `central` caller.
...
describe('seeded-PPR impact ranking — flag gate flips the path', () => {
  ...
  it('flag OFF stays on the flat single-hop path (only direct callers, never walks them)', () => {
    ...
    expect(callers).not.toContain('Central.caller'); // two-hop caller never surfaced
    // Flat baseline never expands a direct caller's own callers.
  ...
  it('flag ON switches to the seeded-PPR walk and surfaces the deeper caller', () => {
    ...
    expect(callers).toContain('Central.caller'); // NEW: two-hop caller surfaced
    // The PPR walk expands a direct caller's own callers — proof the path changed.
