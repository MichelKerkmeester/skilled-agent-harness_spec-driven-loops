...
| Query and context handlers | `code-graph-query-handler.vitest.ts`, `code-graph-context-handler.vitest.ts`, `code-graph-context-cocoindex-telemetry-passthrough.vitest.ts` | MCP handler output contracts, seed telemetry and graph context shaping. |
