// ───────────────────────────────────────────────────────────────
// MODULE: Code Graph Boundary Contracts (local copy for system-code-graph)
// ───────────────────────────────────────────────────────────────
...
export function normalizeStructuralReadiness(graphFreshness: GraphFreshness): StructuralReadiness {
  if (graphFreshness === 'fresh') return 'ready';
  if (graphFreshness === 'stale') return 'stale';
  return 'missing';
}
...
export function buildCodeGraphOpsContract(args: {
  ...
  return {
    ...
    doctor: {
      supported: true,
      surface: 'memory_health',
      checks: ['fts_consistency', 'trigger_cache_refresh', 'orphan_edges', 'orphan_vectors', 'orphan_chunks'],
      repairModes: ['confirmation-gated autoRepair', 'best-effort partial success reporting'],
...
export function createMetadataOnlyPreview(input: {
  ...
  const kind = input.mimeType?.startsWith('text/') ? 'text' : input.mimeType ? 'binary' : 'unknown';
  return {
    mode: 'metadata-only',
    path: normalizedPath,
    fileName,
