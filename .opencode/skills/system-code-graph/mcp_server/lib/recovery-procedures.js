...
function resolveDbDir(dbDir) {
    const resolved = resolve(dbDir ?? DATABASE_DIR);
    ensureDir(resolved);
    return resolved;
}
function tripletPath(dbDir, fileName) {
...
function copyTriplet(dbDir, targetDir) {
    ensureDir(targetDir);
...
function moveTriplet(dbDir, targetDir) {
    ensureDir(targetDir);
    let moved = 0;
    for (const fileName of DB_TRIPLET) {
        const source = tripletPath(dbDir, fileName);
...
function restoreTriplet(sourceDir, dbDir) {
    let restored = 0;
    for (const fileName of DB_TRIPLET) {
...
function runIntegrityCheck(sqlitePath) {
...
function findLatestKnownGood(dbDir, auditDir) {
...
export function snapshotKnownGoodTriplet(options = {}) {
...
export async function recoverSqliteCorruption(options = {}) {
    const dbDir = resolveDbDir(options.dbDir);
...
export async function recoverPartialScanFailure(options = {}) {
    const dbDir = resolveDbDir(options.dbDir);
    const errors = [];
    ...
    try {
        ...
        if (existsSync(sqlitePath)) {
            const db = new Database(sqlitePath, { readonly: true, fileMustExist: true });
            try {
                const rows = db.prepare('SELECT file_path FROM code_files WHERE file_mtime_ms = 0 ORDER BY file_path LIMIT 50').all();
...
export async function rollbackBadApply(options = {}) {
    const dbDir = resolveDbDir(options.dbDir);
    const auditDir = ensureDir(resolve(options.auditDir ?? join(dbDir, 'apply-audit')));
    assertInside(dbDir, auditDir);
    const ts = timestamp(options.now ?? (() => new Date()));
...
export function relativizeRecoveryPath(path, root = process.cwd()) {
    const rel = relative(root, path);
    return rel.startsWith('..') ? path : rel;
}
//# sourceMappingURL=recovery-procedures.js.map