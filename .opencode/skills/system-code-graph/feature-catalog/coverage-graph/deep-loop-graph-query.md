...
| `.opencode/skills/system-deep-loop/runtime/references/integration-points.md` | Call site | `query.cjs` is a read-only query entry with no `*_auto.yaml` auto-step; it is invoked directly via the deep-loop runtime CLI (see integration-points.md), consistent with the no-auto-fire note above |
...
- [02-deep-loop-graph-status.md](../../feature-catalog/coverage-graph/deep-loop-graph-status.md)
