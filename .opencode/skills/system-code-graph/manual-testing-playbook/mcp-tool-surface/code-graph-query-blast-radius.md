...
Tests the path in `mcp-server/handlers/query.ts`. Multi-subject union logic is in the blast-radius assembler; transitive BFS uses the maxDepth bound to prevent runaway traversal. Trace breadcrumbs are built from `.opencode/skills/system-code-graph/mcp-server/lib/graph/bfs-traversal.ts` and are covered by `.opencode/skills/system-code-graph/mcp-server/tests/code-graph-query-handler.vitest.ts`.
