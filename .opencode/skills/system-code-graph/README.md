...
A tree-sitter parser walks your workspace and converts each source file into a graph node. Symbols, calls, imports and definitions become typed edges in a SQLite database. The indexer skips unchanged files by content hash, so incremental scans stay fast. Parser crash cohorts classified as B1/B2/OTHER can land in a skip-list surfaced through `code_graph_status` output under `parserHealth` and `parserSkipList`. Transient failures such as WASM memory errors and timeouts remain eligible for later parse attempts until the retry ceiling is reached (default 5, configurable with `SPECKIT_PARSER_SKIP_LIST_MAX_RETRIES`); a later clean parse removes the transient skip-list entry. Fatal failures and exhausted transient entries stay quarantined for manual review. Syntax-error partial parses surface parse diagnostics without adding a skip-list row. They do not poison the graph but they do reduce coverage.

The default scan globs include JSON, JSONC, YAML, YML and TOML as the `doc` lane, but deliberately omit Markdown/prose docs unless they are re-added with explicit `includeGlobs`. The doc lane records file rows and now performs lightweight structural extraction: config-format files produce `key` symbol nodes, Markdown files produce `heading` symbol nodes when included, and nested document structure is represented with `CONTAINS` edges. Treat doc-lane coverage as config/document structure, not as full source-code AST extraction.
...
| `parserHealth` shows quarantined files | A parser failure is fatal, or a transient failure reached the retry ceiling | Inspect `parserSkipList.sample` from `code_graph_status`. Transient rows are retried and self-heal after a clean parse until `SPECKIT_PARSER_SKIP_LIST_MAX_RETRIES` is exhausted; fatal rows need manual review |
