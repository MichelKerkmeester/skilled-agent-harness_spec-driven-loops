...
- Pass/fail: PASS if the output is a call tree or equivalent structural diagram and every protected identifier and path is byte-identical to the prompt; FAIL if the output is prose instead of a structural visual, or any identifier or path is altered, abbreviated, or paraphrased; SKIP only if the current session cannot execute the `/rewrite:explain-visually` slash command.
