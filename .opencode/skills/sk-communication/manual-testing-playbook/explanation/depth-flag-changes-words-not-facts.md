...
- Pass/fail: PASS if the prose differs between depths and every protected span is byte-identical across both replies; FAIL if the prose is unchanged between depths, or any identifier, path, or number differs, is dropped, or is altered between the two replies; SKIP only when the `/rewrite:explain-visually` slash command is unavailable in the current session, naming that missing runtime as the blocker.
