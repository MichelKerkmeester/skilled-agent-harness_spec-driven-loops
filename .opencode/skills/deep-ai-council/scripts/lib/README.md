...
description: "Code-facing README for .opencode/skills/deep-ai-council/scripts/lib."
trigger_phrases:
...
This README documents the code-bearing folder `.opencode/skills/deep-ai-council/scripts/lib` so operators can understand its role without opening every source file first. It follows the sk-doc skill README structure while staying focused on code navigation.
...
Start with `.opencode/skills/deep-ai-council/SKILL.md` for runtime routing and workflow boundaries.

**Step 2: Inspect the local code.**

```bash
rg --files .opencode/skills/deep-ai-council/scripts/lib
```
...
```text
User request: Check .opencode/skills/deep-ai-council/scripts/lib for sk-code and README coverage.
