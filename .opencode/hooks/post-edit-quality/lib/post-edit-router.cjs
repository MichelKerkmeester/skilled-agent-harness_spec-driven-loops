 ...
 * lives under sk-design-diagram's ASCII-pattern output dir) rather than also
 ...
 * sk-design-diagram's `assets/`, since that dir also holds HTML/SVG
...
function isFlowchartCandidate(basename, segments) {
  ...
  return segments.includes('sk-design-diagram') && segments.includes('ascii-patterns');
}
