...
for (const fname of ['.env.local', '.env']) {
  ...
  if (fs.existsSync(p)) {
    ...
    if (n > 0) process.stderr.write(`[system-skill-advisor-launcher] loaded ${n} env(s) from ${fname}\n`);
  }
}
...
function log(message) {
  process.stderr.write(`[system-skill-advisor-launcher] ${message}\n`);
}
...
// Gate verbose error/path logging behind SYSTEM_SKILL_ADVISOR_DEBUG.
// Operationally-important events keep using log(); error stacks + DB-path traces use debug().
function debug(message) {
  if (process.env.SYSTEM_SKILL_ADVISOR_DEBUG === '1') {
    process.stderr.write(`[system-skill-advisor-launcher] [debug] ${message}\n`);
  }
}
...
async function bridgeOrReportLeaseHeld(leaseResult) {
  ...
  const decision = await maybeBridgeLeaseHolder({
    ...
    leaseResult,
    loggerPrefix: 'system-skill-advisor-launcher',
...
function resolveSessionProxySocketPath() {
  ...
  return getIpcSocketPath('system-skill-advisor', { dbDir: resolvedAdvisorDbDir() });
}
...
function writeSharedModelServerPid(pid) {
  ...
  fs.writeFileSync(tmp, JSON.stringify({
    ...
    ownerLauncher: 'system-skill-advisor',
    socketPath,
  }, null, 2), { mode: 0o600 });
...
async function main() {
  ...
  try {
    ...
    const strictSingleWriter = !isStrictModeDisabled(process.env.SYSTEM_SKILL_ADVISOR_STRICT_SINGLE_WRITER);
    ownerLeaseRequired = strictSingleWriter;
    if (strictSingleWriter) {
      if (await checkStrictSingleWriter()) return;
    } else {
      log('SYSTEM_SKILL_ADVISOR_STRICT_SINGLE_WRITER is disabled; skipping lease check');
    }
    ...
    if (lockHeld) {
      ...
      writeState({
        command: 'system-skill-advisor-launcher',
        start: started,
        end: now(),
        status: 'ready',
        actions,
