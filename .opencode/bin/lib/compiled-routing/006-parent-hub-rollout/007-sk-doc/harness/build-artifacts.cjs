...
function sourceBytes() {
  ...
  for (const packet of [...new Set(registry.modes.map((mode) => mode.packet))]) {
    bytes[sourceKeyForPacket(packet)] = fs.readFileSync(path.join(SKILL_ROOT, packet, 'SKILL.md'));
  }
  return bytes;
}
...
function compiledLeafPairsForDecision(snapshot, decision) {
  ...
  return decision.route.targets.flatMap((entry) => {
    const mode = entry.destinationId.workflowMode;
    if (!selections.has(mode)) throw new TypeError(`compiled leaf selection missing for ${mode}`);
