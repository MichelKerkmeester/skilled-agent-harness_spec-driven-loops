...
**Dist freshness.** Each shim calls the shared `checkPackageFreshness()` (`system-spec-kit/scripts/lib/dist-freshness.cjs`) to compare its package's watched source files against the built dist entrypoint, exiting `69` with a rebuild instruction when stale. Dev overrides: `SPECKIT_SPEC_MEMORY_CLI_DEV_ALLOW_STALE=1`, `SPECKIT_CODE_INDEX_CLI_DEV_ALLOW_STALE=1`, `SYSTEM_SKILL_ADVISOR_CLI_DEV_ALLOW_STALE=1`.
