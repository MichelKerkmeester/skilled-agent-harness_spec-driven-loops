...
trigger_phrases:
  ...
  - "skill-advisor cli"
  - "compiled route cli"
  - "compiled routing runtime"
---

# bin: MCP Launchers, Daemon CLIs, Compiled Routing and Repo Tooling
...
+-- worktree-status.sh             # Reports worktree, branch and sync state
...
+-- compiled-route-status.cjs      # Per-hub serving status with a cause code separating drift from breakage
+-- compiled-route-guard.cjs       # Freshness gate: serving matches inputs, runtime matches source
+-- compiled-route-manifest.cjs    # CLI over the activation-manifest library in lib/
...
The same shared module tracks 7 dist-producing packages total (the 3 CLI shims' own `mcp_server`s plus `system-spec-kit/{shared,scripts}`, `mcp-code-mode/mcp-server`, and `sk-design/sk-design-md-generator/backend`) and backs three other consumers: `validate.sh`'s `run_node_orchestrator()` fails closed (exit `3`, no auto-rebuild) when the compiled spec-validation orchestrator it depends on is stale. the `sk-code` `claude-posttooluse.sh` PostToolUse hook prints a non-blocking `STALE DIST WARNING` banner when an edit lands in a watched source tree. and the `mk-dist-freshness-guard` OpenCode plugin (`.opencode/plugins/`) surfaces stale packages to the agent via bounded system-context injection (`experimental.chat.system.transform`) plus an append-only `.opencode/logs/dist-freshness-guard.log`, never stdout/stderr, which the TUI would paint over the chat input, refreshed before a Bash call matching `opencode run`/`validate.sh` and once per session on `session.created`. Root cause and full design: `system-speckit/028-memory-search-intelligence/002-spec-data-quality/050-validate-sh-dist-freshness-and-repo-remediation/001-dist-freshness-enforcement` spec folder.
