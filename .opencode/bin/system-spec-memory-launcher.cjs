#!/usr/bin/env node
// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ COMPONENT: system-spec-memory Launcher                                       ║
// ╠══════════════════════════════════════════════════════════════════════════╣
...
let lockDir = path.join(dbDir, '.system-spec-memory-launcher.lockdir');
const PID_FILE_NAME = '.system-spec-memory-launcher.json';
...
function log(message) {
  process.stderr.write(`[system-spec-memory-launcher] ${message}\n`);
...
function resolveLauncherLogPath(env = process.env, baseDir = dbDir) {
  ...
  return typeof override === 'string' && override.trim()
    ? override.trim()
    : path.join(baseDir, '.system-spec-memory-launcher.log');
}
...
function loadProjectEnvFiles() {
  for (const fname of ['.env.local', '.env']) {
    ...
    if (fs.existsSync(p)) {
      ...
      if (n > 0) process.stderr.write(`[system-spec-memory-launcher] loaded ${n} env(s) from ${fname}\n`);
    }
  }
}
...
function resolveSessionProxySocketPath() {
  ...
  return getIpcSocketPath('system-spec-memory', { dbDir: resolvedDbDir() });
}
...
function refreshPaths() {
  ...
  lockDir = path.join(dbDir, '.system-spec-memory-launcher.lockdir');
...
function writeLeaseHeldJsonRpcError(leaseResult, reason) {
  process.stdout.write(`${JSON.stringify({
    ...
    error: {
      code: -32001,
      message: `system-spec-memory: lease held by pid ${leaseResult.ownerPid} but session bridge unavailable (${reason}); reconnect`,
      data: { retryable: true },
    },
  })}\n`);
}
...
function bridgeReadiness(leaseResult) {
  ...
  const socketPath = getIpcSocketPath('system-spec-memory', { dbDir: resolvedDbDir() });
...
function respawnLockPath() {
  return path.join(resolvedDbDir(), '.system-spec-memory-respawn.lock');
}
...
async function bridgeOrReportLeaseHeld(leaseResult) {
  ...
  const decision = await maybeBridgeLeaseHolder({
    ...
    leaseResult,
    loggerPrefix: 'system-spec-memory-launcher',
    dbDir: resolvedDbDir(),
...
function writeSharedModelServerPid(pid) {
  ...
  return writeAtomicJsonFile(pidPath, {
    ...
    ownerLauncher: 'system-spec-memory',
    socketPath,
...
async function main() {
  ...
  try {
    ...
    const strictSingleWriter = !isStrictModeDisabled(process.env.SYSTEM_SPEC_MEMORY_STRICT_SINGLE_WRITER);
    ...
    if (strictSingleWriter) {
      ...
      log('SYSTEM_SPEC_MEMORY_STRICT_SINGLE_WRITER is disabled; skipping lease check');
    }
    ...
    if (lockHeld) {
      ...
      writeState({
        command: 'system-spec-memory-launcher',
        start: started,
        end: now(),
        status: 'ready',
        actions,
