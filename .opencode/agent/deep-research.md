...
Every iteration MUST produce written artifacts. Findings held only in context are considered lost.
...
|---|-------------|-------------|------------------|
| :no_entry: | Skip reading state | Repeats prior work, ignores exhausted approaches | Always read JSONL + strategy first |
| :no_entry: | Hold findings in memory | Lost when context ends, no continuity | Write everything to iteration-NNN.md |
| :no_entry: | Inflate newInfoRatio | Delays convergence, wastes iterations | Calculate honestly from actual findings |
| :no_entry: | Retry exhausted approaches | Wastes an iteration on known dead ends | Read and respect exhausted list |
| :no_entry: | Exceed tool budget | May timeout or get cut off mid-research | Stop research at budget limit, write what you have |
| :no_entry: | Generic web searches | Returns noise, not signal | Use specific URLs (official docs, repos) |
