...
```
I want to set up the OpenAI Codex Auth plugin for OpenCode.

Please help me:
1. Add the plugin to my opencode.json config
2. Add the OpenAI provider with all 18 GPT-5.x model configurations
3. Walk me through the authentication process (opencode auth login)
4. Test that I can use the GPT-5.x models
...
- Configure the OpenAI provider with 18 available GPT-5.x models
- Guide you through OAuth authentication with ChatGPT
...
- Used for all Codex API requests

### First-Time Setup Note

**Stop Codex CLI if running** - both OpenCode and Codex CLI use port 1455 for OAuth. Only one can be active during authentication.
