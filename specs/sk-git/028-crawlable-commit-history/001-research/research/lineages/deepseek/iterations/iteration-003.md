...
F. real commit 2d356893a8: `Refs: specs/hooks/016-...` sits in its own paragraph BEFORE the
...
4. **Trailer extraction is a structural parse, not a text scan — and it is lossy today.** `%(trailers:key=Refs)` extracts 119 of the 349 literal `Refs:` lines, because git only treats a **final, contiguous paragraph** as the trailer block: commit `2d356893a8` places `Refs:` in its own paragraph before the Co-Authored-By block and its Refs value is invisible to the extraction format. Keys are matched case-insensitively (119 = 119), and `valueonly` returns the bare value. [SOURCE: command:%(trailers:key=Refs) → 119 vs 349; commit 2d356893a8]
