...
Create a standard, non-phased **Level 3** packet at `.opencode/specs/sk-git/009-skill-scoped-worktree-naming` (sibling of `001-continuous-integration-workflow`), not a child of 137. Level 3 is warranted by the architecture decision and shared-repository blast radius even though the net code footprint is moderate. The recommended branch for the implementation work obeys the new grammar: `sk-git/{NNNN}-skill-scoped-worktree-naming` (number from the live allocator).
