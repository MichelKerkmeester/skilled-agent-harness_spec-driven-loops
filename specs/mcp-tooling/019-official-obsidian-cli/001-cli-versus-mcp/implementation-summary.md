...
6. **A concurrent session shares this checkout and interfered twice.** It moved `HEAD` from `ce826ccc45` to `b7247763e0` mid-run, which reverted every uncommitted edit to the six tracked skill files. They were re-applied and re-staged. Its commit `aa11c2b622` also swept this packet's staged scaffold into itself, so the empty templates are committed under another session's message while the authored content sits staged. Nothing was rewound to fix that, because rewriting shared history was not authorized.
