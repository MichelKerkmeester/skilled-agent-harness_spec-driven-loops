...
Evidence: `regression/post-ws2-replay-{hub,packets}.json` — hub 13/13 intent + 13/13 resource, packets 49/49 + 49/49 (`scratch/replay-driver.cjs`, loader-enumerated, gold from raw frontmatter); benchmark re-run `mcp-tooling/benchmark/reports/2026-07-16--after-routing-remediation--router/` PASS 98 with routeGold `{auto, enforced, rows 13, matches 13, violations 0}`; enforcement proof (scratch-corpus injection) BLOCKED-BY-ROUTE-GOLD exit 3 and gate-off PASS; new harness suite `tests/route-gold-gate.vitest.ts` 39/39.
