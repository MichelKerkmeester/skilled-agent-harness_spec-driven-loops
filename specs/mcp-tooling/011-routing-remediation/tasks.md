...
- [x] T027 Re-run the Lane-C benchmark into a NEW run-label folder; verify baseline/ untouched and route-gold rows above 0 (.opencode/skills/mcp-tooling/benchmark/) [evidence: benchmark/2026-07-16--after-routing-remediation--router/ — verdict PASS, aggregate 98, routeGold mode auto ENFORCED, rows 13, matches 13, violations 0, parseFailures 0; `git status` shows baseline/ untouched (only the new untracked run-label folder); run table updated in benchmark/README.md]
