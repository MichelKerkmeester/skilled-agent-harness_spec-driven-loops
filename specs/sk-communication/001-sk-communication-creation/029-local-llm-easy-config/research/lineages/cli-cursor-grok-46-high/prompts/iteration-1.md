---
title: "Deep-Research Iteration Prompt Pack"
trigger_phrases: []
---
DEEP-RESEARCH
Resolved route: mode=research; target_agent=@deep-research; execution=single_iteration; state_source=externalized_files; do_not_switch_mode=true

# Deep-Research Iteration Prompt Pack

## STATE

STATE SUMMARY (auto-generated):
Segment: 1 | Iteration: 1 of 5
Questions: 0/5 answered | Last focus: none
Last 2 ratios: none | Stuck count: 0
Recovery: none
Next focus: Map the shipped config-discovery surfaces

Research Topic: Local LLM easy-config and automatic projection activation
Iteration: 1 of 5
Focus Area: Config discovery format
Remaining Key Questions: Q1-Q5
CONSTRAINT: LEAF agent -- do NOT dispatch sub-agents
ALLOWED WRITE PATHS: this lineage artifact_dir only
