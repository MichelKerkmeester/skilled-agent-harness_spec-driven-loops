...
| **Testing** | The harness is the test, and its own correctness is checked by a negative control |
...
per-dimension delta against a baseline that was captured during phase 003's setup. A negative control
comes first: a case the adopted rules should not affect must score the same under both conditions, or
the harness is measuring the label rather than the reply.

Four things follow from the two research runs, and each one changes what the harness has to do.

1. **The baseline belongs to phase 003's setup.** A baseline taken after the rules change cannot
   support a regression claim, so the capture happens before phase 003's first edit. This phase
   inherits the recorded baseline and the frozen cases and rubric. It confirms their provenance and
...
3. **Every recorded result names its provider.** Both provider profiles now use provider-default
   ...
   in this measurement and it is recorded per result rather than assumed away.
4. **A no-op is separated from a rewrite.** Phase 004 adds the change-kind field. While a no-op row is
   indistinguishable from a rewrite, a quality delta describes the mix of rows rather than the rules.
<!-- /ANCHOR:summary -->
...
which the pre-change rule set is still observable.
...
command or the artifact that produces it, and the gate carries no condition that rests on evidence
...
Both provider profiles use provider-default thinking mode, which was the decision that kept a provider
from being refused for a capability it cannot prove. The cost of that decision lands here: two
providers can apply different defaults, so a quality delta cannot be fully attributed to the rules.

The harness records the provider that produced every result, in the same row as the score, so the
variable is visible even though it is not controlled. Where a dimension moves and the cases do not
...
no-op value rather than hiding inside a pass. The harness reads that field and reports no-op rows
...
score can move without any rule having changed the reply, so counting it inside the rule delta would
...
- **Runner**: produces one scored result file per condition, refuses to run on a malformed case set, and records the provider and the change kind on every row.
...
Cases plus a condition produce replies. Replies plus the rubric produce scores. Every scored row carries
the provider that produced the reply and, where the engine lane produced it, the change kind from the
accept record. Scores plus the baseline produce the delta. The delta is what the program's completion
claim rests on, so it is recorded as an artifact rather than quoted from memory.
<!-- /ANCHOR:architecture -->
...
- Adversarial cases: a judge that can infer the condition from formatting, a case set edited after the first run, a run that records a default score on a scoring failure, and a run whose rows carry no provider.
<!-- /ANCHOR:affected-surfaces -->
...
The order carries one hard constraint. Setup confirms the phase 003 baseline and the frozen cases, so
...
| Two-condition run | Baseline provenance, frozen cases and rubric | Delta report |
| Delta report | Two-condition run, phase 004's change-kind field | Mechanism and regeneration |
...
|-------|------------|------------------|
| Setup | Medium | 2 hours confirming the baseline's provenance and the frozen cases rather than drafting either |
