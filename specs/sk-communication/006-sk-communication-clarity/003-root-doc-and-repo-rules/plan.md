...
twenty-nine adopted candidates, the largest single allocation in the program, and it records its own
length ceiling. It is therefore split before anything is added to it. The seam is the unit a rule
governs: sentence and paragraph mechanics on one side, whole-reply shape on the other.

The split lands a new half under `repo-rules/`. Its trigger row and its index row land in the same
change as the file, because a file with no trigger row is a rule that never loads and a row pointing
at nothing is a broken router. No rule sentence is added, removed or reworded by the move.
...
Neither baseline can be reconstructed after the edit, so both are captured first.

The root doc is not in this phase's diff. Phase 002 found no clause that cannot live below it, so
`AGENTS.md` keeps its two binding clauses and its three pointers to the reply-shape rule. The router
carries the reach of the new half instead.
<!-- /ANCHOR:summary -->
...
- [ ] Both halves exist with every rule sentence from the pre-change file appearing exactly once
- [ ] Both halves exist with every rule sentence from the pre-change file appearing exactly once
<!-- /ANCHOR:quality-gates -->
...
bind when the router's trigger matches the action about to be taken. General judgment fills the rest.
The split happens inside the third tier and moves nothing between tiers.
...
- **`repo-rules/communication.md`**: the whole-reply half after the split, which is where the ten
  candidates phase 006 writes land.
- **A new half file under `repo-rules/`**: sentence and paragraph mechanics, carrying the standard
  routed-from line, the bounded-by statement, a fires-when list and a self-check.
- **`REPO RULES.md`**: the trigger row and the index row that make the new half reachable, landing in
  the same change as the file they name.
...
same way. The two rows may fire on the same action, which the router allows and expects. The
...
record. The move touches two rule files and one router. Every other surface stays as it is.

| Surface | Current Role | Action | Verification |
|---------|--------------|--------|--------------|
| `repo-rules/communication.md` | Reply shape, broadest trigger in the set | Keep the whole-reply rules and move the sentence mechanics out | Compare both halves against the pre-change file and confirm every sentence appears once |
| The new half under `repo-rules/` | Does not exist yet | Create it with the standard header, its own fires-when list and its own self-check | Read the header and the closing self-check. Confirm the routed-from line names the router |
...
- Matrix axes: governed unit by half. The required rows are the sections of the pre-change file. Each one lands on exactly one side of the seam.
...
| Baseline | Each half's size and every mark's instruction before and after the move | `wc`, plus a read of the pre-change file |
...
|------------|------|--------|-------------------|
| Phase 002's decision record | Internal | Green, eight decisions Accepted | The punctuation scope is unset without it. The colon decision is what keeps a mark's instruction out of this phase's diff |
...
- **Trigger**: A half loads on the wrong action. The other trigger is a move that changed the instruction for a mark rather than relocating it.
- **Procedure**: The change is one diff across two rule files plus one router. Rejoin the two halves into `repo-rules/communication.md`. Remove the new half's trigger row and index row in the same change rather than before it. Then re-run the trigger walk, so no row names a file that no longer exists. The recorded baselines are evidence rather than state, so they stay as the record of what the pre-split rule looked like.
<!-- /ANCHOR:rollback -->
