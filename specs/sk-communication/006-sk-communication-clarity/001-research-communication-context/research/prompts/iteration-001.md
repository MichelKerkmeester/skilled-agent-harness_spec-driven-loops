...
Iteration 1 of 10. Convergence mode is "off", so convergence is telemetry only and never stops this run. Iterations completed so far: 0. Key findings recorded: 0.
...
Iteration 1: inventory `context/clarity.md`. Extract each of its eighteen rules as a discrete
recommendation, then classify each against the current stack with a cited line. Start here rather
than with the other two sources because clarity's first four rules concern decisions made before
any sentence is written, and the stack's documents all begin at the sentence, so this is where the
coverage question is most likely to return a real gap.
Remaining Key Questions: ## 3. KEY QUESTIONS (remaining)
Generated from the reducer registry. Add external or late questions through `research/inbox.jsonl`; direct edits are imported as compatibility input and may be replaced on the next reduce step.
...
- Write iteration narrative to: specs/sk-communication/006-sk-communication-clarity/001-research-communication-context/research/iterations/iteration-001.md
- Write per-iteration delta file to: specs/sk-communication/006-sk-communication-clarity/001-research-communication-context/research/deltas/iter-001.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `specs/sk-communication/006-sk-communication-clarity/001-research-communication-context/research/iterations/iteration-001.md`, this iteration's narrative markdown
  - `specs/sk-communication/006-sk-communication-clarity/001-research-communication-context/research/deltas/iter-001.jsonl`, this iteration's delta JSONL
...
1. **Iteration narrative markdown** at `specs/sk-communication/006-sk-communication-clarity/001-research-communication-context/research/iterations/iteration-001.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
