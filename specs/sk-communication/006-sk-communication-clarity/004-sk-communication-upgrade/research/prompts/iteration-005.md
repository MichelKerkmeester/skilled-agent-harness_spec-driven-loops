...
Iteration 5 of 5. Convergence mode is "off", so convergence is telemetry only and never stops this run. Iterations completed so far: 4. Key findings recorded: 32.
...
Iteration: 5 of 5
Focus Area: Final iteration. Consolidate, do not re-investigate. Budget at most 4 reads and 12 tool calls. Produce a single ranked change list for the engine, where every row carries: the change, the exact file and line it touches, the failure it prevents, whether it is mechanically verifiable or needs judgment, and its blast radius on the frozen invariants (canonical bytes preserved, privacy before ranking, exact-original on every failed path, one home for the wording standard). Rank by value over risk, cheapest safe win first. Then state explicitly which rows are edits the operator can take without a design decision, and which rows require a decision first and what that decision is. Close with the one thing you would NOT do and why, chosen from the candidates the earlier iterations raised. Every row cites file:line.
...
Last 3 Iterations Summary: - iteration-002.md: Two questions, in the priority order the dispatch names:
- iteration-003.md: Two questions in the order the dispatch names them.
- iteration-004.md: Answer the last open key question: given the clarity source's rule 17, is a re-render in plainer
...
- Write iteration narrative to: specs/sk-communication/006-sk-communication-clarity/004-sk-communication-upgrade/research/iterations/iteration-005.md
- Write per-iteration delta file to: specs/sk-communication/006-sk-communication-clarity/004-sk-communication-upgrade/research/deltas/iter-005.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `specs/sk-communication/006-sk-communication-clarity/004-sk-communication-upgrade/research/iterations/iteration-005.md`, this iteration's narrative markdown
  - `specs/sk-communication/006-sk-communication-clarity/004-sk-communication-upgrade/research/deltas/iter-005.jsonl`, this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the research. NEVER execute the out-of-scope mutation. The research packet (`specs/sk-communication/006-sk-communication-clarity/004-sk-communication-upgrade/research/iterations/iteration-005.md` directory and parents) is the only zone for your writes; the researched target/topic surface is off-limits.
...
1. **Iteration narrative markdown** at `specs/sk-communication/006-sk-communication-clarity/004-sk-communication-upgrade/research/iterations/iteration-005.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
