// Claude Code function hook: replace the built-in compaction summary with
// verbatim Jev-guided compaction. Adapted from fast-jev-compaction
// (https://github.com/tamaratran/fast-jev-compaction, MIT) to run inside the
// jev plugin. Imports the self-contained copy in ./compaction/ (kept in sync
// with src/vendor/compaction/ by `npm run sync:hooks`).
//
// Function hooks are an early-access Claude Code feature. Enable with
// CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1 in the environment or in settings.json
// `env`. The checked-in type reference in ./types/claude-code.d.ts was written
// by Claude Code 2.1.274; regenerate with /plugin-types after upgrades.
...
import { compact, reductionRatio } from './compaction/compact.ts';
import { buildJevRequest, DEFAULT_MODEL, parseJevResponse } from './compaction/request.ts';
import type { CompactOptions, CompactResult, JevAsker, Message, ToolResult, ToolUse } from './compaction/types.ts';
...
/**
 * Maps the library's output back onto session messages. Untouched objects are
 * returned as the engine's own (handle included); rebuilt ones are fresh.
 */
...
export function decisionLogLines(result: CompactResult, maxChars: number = UI_LOG_MAX_CHARS): string[] {
  ...
  if (entries.length === 0) return ['jev compact decisions: (none)'];
  const chunks: string[] = [];
  let current = '';
  for (const entry of entries) {
    const next = current ? `${current} ${entry}` : entry;
    if (current && next.length > maxChars - 32) {
      chunks.push(current);
      current = entry;
    } else current = next;
  }
  ...
  return chunks.map((chunk, index) =>
    chunks.length === 1 ? `jev compact decisions: ${chunk}` : `jev compact decisions (${index + 1}/${chunks.length}): ${chunk}`,
  );
}
  ...
  on('session.compact', async ($, event, next) => {
    try {
      ...
      if (reductionRatio(result) < config.minReductionRatio) {
        notify($, `jev compact: falling back to the built-in summary (below ${percent(config.minReductionRatio)} minimum: ${summarize(result)})`);
        return next(event);
      }
      notify($, `jev compact: kept ${messages.length}/${event.messages.length} messages verbatim, no summary (${summarize(result)})`);
      return { messages };
    } catch (error) {
      notify($, `jev compact: falling back to the built-in summary (${error instanceof Error ? error.message : String(error)})`);
      return next(event);
    }
  });

  if (configured.compactAtPercent > 0) {
    on('turn.complete', async ($, event: TurnCompleteInput, next) => {
      ...
      try {
        ...
        $.ui.log(`jev compact: auto-compact skipped (${error instanceof Error ? error.message : String(error)})`);
      } finally {
        compacting = false;
      }
      return next(event);
