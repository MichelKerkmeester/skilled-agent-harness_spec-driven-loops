...
8. **Three phase folders still carry the scaffold description.** `003-decouple-and-rewire`, `004-compiled-fleet-onboarding` and `005-playbook-reverification-and-closeout` keep the placeholder `description` text and its junk keywords in `description.json`, while `001` and `002` carry derived ones. The derivation writer is `.skilled/skills/system-spec-kit/runtime/cli/spec-folder/generate-description.ts`, and it cannot run in this tree: its package build under `.skilled/skills/system-spec-kit/scripts/dist/` is absent and its imports resolve to `.js` specifiers that do not exist next to the source. No validator reads the field (both packets validate green with it), so it is recorded rather than guessed at by hand.
<!-- /ANCHOR:limitations -->
