...
_memory:
  continuity:
    ...
    recent_action: "Reconciled the plan with the eight live runbooks"
...
Turns implemented contracts and observed operations into accurate API, security, setup, maintenance, incident, and rollback documentation. The eight live runbooks are implemented under `.pi/pi-remote/docs/`.
<!-- /ANCHOR:summary -->
...
### Key Components
- **`.pi/pi-remote/docs/architecture.md`**: System boundaries, data flow, failure semantics, and the canonical protocol/API compatibility narrative alongside `.pi/pi-remote/packages/pi-rpc-protocol/`
- **`.pi/pi-remote/docs/security.md`**: Threat model, authentication, authorization, approval, containment, redaction, and retention
- **`.pi/pi-remote/docs/setup.md` and `operations.md`**: Installation, configuration, startup, health, devices, mutation, migrations, and maintenance
- **`.pi/pi-remote/docs/incident-playbooks.md` and `rollback.md`**: Incident handling, authority drain, backup/restore, down-migration, and rollback
- **`.pi/pi-remote/docs/platform-support.md`**: PWA install, permissions, notifications, offline behavior, and platform limitations
- **`.pi/pi-remote/docs/release-verification.md`**: Machine gates, operator-only evidence, thresholds, compatibility limits, and rollout readiness
...
The implementation was relocated into `.pi/pi-remote/docs/`. This table maps the planned document set to the live split runbooks.

| Surface | Current Role | Action | Verification |
|---------|--------------|--------|--------------|
| `.pi/pi-remote/docs/architecture.md` | Architecture plus protocol/API compatibility narrative | Implemented | Source-to-doc contract review |
| `.pi/pi-remote/packages/pi-rpc-protocol/` | Machine-readable protocol/API compatibility surface | Implemented | Typecheck and protocol guard tests |
| `.pi/pi-remote/docs/security.md` | Security boundaries and operator-only mutation checks | Implemented | Source-to-doc contract review |
| `.pi/pi-remote/docs/setup.md` and `operations.md` | Setup and daily operations | Implemented | Command/runbook review |
| `.pi/pi-remote/docs/incident-playbooks.md` and `rollback.md` | Incident and recovery operations | Implemented | Rollback drill plus operator guidance review |
| `.pi/pi-remote/docs/platform-support.md` | Mobile and push limitations | Implemented | Platform contract review; physical devices pending |
| `.pi/pi-remote/docs/release-verification.md` | Release evidence and rollout contract | Implemented | `release:verify` evidence review |
