...
_memory:
  continuity:
    ...
    recent_action: "Reconciled the implemented protocol and supported baseline"
...
Pins the live Pi, host, ingress, browser, storage, and extension contracts and freezes the threat model before production implementation. The contracts and selected baseline are implemented in the relocated `.pi/pi-remote/` monorepo.
...
|-----------|-------------|-------------|
| `.pi/pi-remote/packages/pi-rpc-protocol/` | Relocated and implemented | Versioned Pi command, response, event, envelope, approval, and command contracts |
| `.pi/pi-remote/apps/pi-remote-relay/src/fixtures/` | Relocated and implemented | Sanitized recorded RPC fixtures |
| `.pi/pi-remote/apps/pi-remote-relay/tests/security/` | Relocated and implemented | Authorization and negative-control evidence |
| `.pi/pi-remote/docs/architecture.md`, `.pi/pi-remote/docs/security.md`, `.pi/pi-remote/docs/platform-support.md` | Implemented | Supported baseline, trust boundaries, containment choice, and platform limits |
<!-- /ANCHOR:scope -->
...
| REQ-005 | The credential model is unified and owned. | The relationship between device-enrollment keys and any WebAuthn/passkey step-up is frozen — which grants authorization, the recovery and fallback paths, and revocation ownership — so no orphan or ambiguous credential system reaches implementation. |
