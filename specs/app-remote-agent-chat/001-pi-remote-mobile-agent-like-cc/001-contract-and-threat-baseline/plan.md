...
_memory:
  continuity:
    ...
    recent_action: "Reconciled the implemented baseline with passing machine evidence"
...
Pins the live Pi, host, ingress, browser, storage, and extension contracts and freezes the threat model. The implementation now follows this plan under `.pi/pi-remote/`; rollout evidence remains separate.
<!-- /ANCHOR:summary -->
...
### ADR-002: Fixed PWA and TypeScript stack

The accepted stack decision is recorded in [decision-record.md](decision-record.md#adr-002-ship-an-installable-pwa-not-a-native-app-on-a-fixed-typescript-stack): installable React 19/Vite PWA, TypeScript relay and shared protocol, better-sqlite3 durability, React Aria/Tailwind UI foundations, and Web Push. The live implementation follows this decision under `.pi/pi-remote/{apps,packages}`.

### ADR-003: First supported baseline

The accepted baseline is recorded in [decision-record.md](decision-record.md#adr-003-pin-the-first-supported-baseline--macos-host-sandbox-exec-better-sqlite3-ios-pwa): macOS host, `sandbox-exec` containment profile, numbered better-sqlite3 migrations, and installed iOS PWA. The baseline is implemented, while real containment and physical iOS evidence remain operator-verification pending.
