...
_memory:
  continuity:
    ...
    recent_action: "Reconciled the implemented auth boundary and Serve deployment assets"
...
Adds private HTTPS/WSS ingress assets, application authentication, default-deny authorization, revocation, and a read-only remote API. The relay boundary is implemented and integration-tested; real Tailscale Serve ingress remains operator-verification pending.

**Key Decisions**: Expose a loopback relay through Tailscale Serve and require a separate short-lived application session plus per-action authorization. Device enrollment uses a single-use QR challenge bound to a device key rather than a reusable application ticket, and forwarded Tailscale-Serve identity is authenticated at the loopback boundary rather than trusted by network position.
...
|-----------|-------------|-------------|
| `.pi/pi-remote/apps/pi-remote-relay/src/http/` | Relocated and implemented | Bootstrap, session, ticket, Origin, command, and rate-limit endpoints |
| `.pi/pi-remote/apps/pi-remote-relay/src/auth/` | Relocated and implemented | Device proof, principal, session, action, and revocation policy |
| `.pi/pi-remote/deploy/setup-tailscale-serve.sh` and `serve.env.example` | Relocated and implemented | Loopback service and Tailscale Serve configuration |
| `.pi/pi-remote/apps/pi-remote-relay/tests/auth.test.ts`, `authority-loop.test.ts`, and `tests/security/` | Relocated and implemented | Spoof, bypass, ticket, Origin, revocation, and authority-drain evidence |
<!-- /ANCHOR:scope -->
...
| REQ-006 | Device enrollment is a single-use QR ceremony. | Enrollment binds origin, pairing id, host fingerprint, a single-use challenge, and a device public key over tailnet HTTPS/WSS; no reusable application ticket or public fallback exists; revocation invalidates the device's capabilities and leases and disconnects its sockets. |
| REQ-007 | Forwarded Serve identity is authenticated, not positional. | The relay accepts the deployment identity signal only from the loopback Serve proxy, strips client-supplied identity headers, and proves that a direct-loopback or alternate-tailnet path cannot spoof the trusted boundary. |
| REQ-008 | The Serve-to-relay trust anchor is a concrete mechanism. | The relay authenticates the Serve proxy by a non-forgeable local channel — peer credential over a unix domain socket, or a per-deployment ephemeral secret injected out of band — not by header presence or bind address; a direct local connection lacking the anchor is rejected. |
| REQ-009 | Tickets and enrollment challenges have bounded lifetimes. | One-use WebSocket tickets and QR enrollment challenges carry a short TTL, single-use consumption, defined reconnect/renewal semantics, and partial-enrollment recovery; expiry and reuse fail closed. |
...
### State Transitions
- The implementation is built; real Tailscale Serve acceptance remains operator-verification pending.
...
- Subsequent implementation is recorded in `implementation-summary.md`; real ingress acceptance and capability enablement remain pending.
<!-- /ANCHOR:change-log -->
