...
Questions: 0/6 answered | Last focus: Q5: Compose box affordances
Last 2 ratios: 0.74 -> 0.79 | Stuck count: 0
Resource map: resource-map.md not present; skipping coverage gate.
Memory context refresh: none loaded yet.
Next focus: Q2: What information architecture and navigation patterns make multi-surface mobile agent control (home sessions, live session, review queue, attention inbox) feel coherent and low-effort?
...
Iteration: 4 of 10
Focus Area: Q2: What information architecture and navigation patterns make multi-surface mobile agent control (home sessions, live session, review queue, attention inbox) feel coherent and low-effort, especially under foreground authority and push hinting? Define one coherent mobile navigation model that moves from a content-free attention hint to the authoritative session or exact-action review, then returns the operator without losing place.
Remaining Key Questions: Q2, Q3, Q6 (Q1, Q4, Q5 answered at pattern level in iterations 1-3)
...
Last 3 Iterations Summary: run 1: Q1 transferable patterns (0.88) | run 2: Q4 review flows (0.74) | run 3: Q5 compose box (0.79)
...
- Write iteration narrative to: specs/cli-external-orchestration/044-pi-mobile-ui-ux-research/research/iterations/iteration-004.md
- Write per-iteration delta file to: specs/cli-external-orchestration/044-pi-mobile-ui-ux-research/research/deltas/iter-004.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `specs/cli-external-orchestration/044-pi-mobile-ui-ux-research/research/iterations/iteration-004.md`, this iteration's narrative markdown
  ...
  - `specs/cli-external-orchestration/044-pi-mobile-ui-ux-research/research/deltas/iter-004.jsonl`, this iteration's delta JSONL
- **BANNED OPERATIONS (NEVER execute against any path)**: `rm`, `rm -rf`, `git rm`, `mv`, `sed -i` (including `sed -i ''`), `rmdir`, `find ... -delete`, shell output-redirect truncate `>` against any file not in the allowed-write list, and any tool call whose effect is to delete, rename, or replace a file outside the allowed-write list. Reading is unrestricted; **writing, renaming, and deleting are scoped**.
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the research. NEVER execute the out-of-scope mutation. The research packet (`specs/cli-external-orchestration/044-pi-mobile-ui-ux-research/research/iterations/iteration-004.md` directory and parents) is the only zone for your writes; the researched target/topic surface is off-limits.
...
1. **Iteration narrative markdown** at `specs/cli-external-orchestration/044-pi-mobile-ui-ux-research/research/iterations/iteration-004.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `specs/cli-external-orchestration/044-pi-mobile-ui-ux-research/research/deltas/iter-004.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
