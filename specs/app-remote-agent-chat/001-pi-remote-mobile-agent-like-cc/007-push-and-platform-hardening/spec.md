...
Adds privacy-minimized Web Push hints and hardens installation, lifecycle, revocation, and stale-state behavior across supported mobile platforms. The generic payload and lifecycle code are implemented and machine-tested; physical iOS delivery remains operator-verification pending.

**Key Decisions**: Send generic hints only after committed state transitions and require authenticated fetch-on-open for all details and actions. Hints carry only a bounded attention class — `needs_input`, `finished`, or `error` — never a free-form category, and the same three classes drive an in-app Attention Inbox so push-denied users keep the attention-to-context workflow.
...
|-----------|-------------|-------------|
| `.pi/pi-remote/apps/pi-remote-relay/src/push/` | Relocated and implemented | Encrypted subscriptions, preferences, deduplication, and generic hints |
| `.pi/pi-remote/apps/pi-remote-web/public/service-worker.js` | Relocated and implemented | Notification handling and authenticated fetch-on-open |
| `.pi/pi-remote/apps/pi-remote-web/src/App.tsx`, `auth.ts`, and `attention.ts` | Relocated and implemented | Preferences, unsubscribe/logout, revocation, and Attention Inbox behavior |
| `.pi/pi-remote/apps/pi-remote-relay/tests/push.test.ts` and `.pi/pi-remote/docs/platform-support.md` | Relocated and implemented | Payload/lifecycle evidence and explicit platform limitations |
<!-- /ANCHOR:scope -->
...
|----|-------------|---------------------|
| REQ-001 | Push carries no authority or sensitive content. | Payload inspection shows only an opaque lookup identifier and one bounded attention class from {needs_input, finished, error}; no transcript, tool, workspace, approval, path, decision, or free-form category text exists. |
...
| REQ-005 | Attention is a bounded class driving push and an in-app inbox. | `attention.changed` carries only an opaque id and one class from {needs_input, finished, error}; push and the in-app Attention Inbox render the same three classes; generation/nonce dedup drops stale hints; opening any hint reauthenticates and fetches authoritative state before exposing an action. |
...
### State Transitions
- The implementation is built; physical iOS push acceptance remains operator-verification pending.
