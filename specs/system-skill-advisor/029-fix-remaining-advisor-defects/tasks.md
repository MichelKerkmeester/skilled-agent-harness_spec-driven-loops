...
- [x] T029 Lane L main integration (merge 2dbaa8fd66): merge main into the fix branch on a detached HEAD, take main's side on the four conflicting `graph-metadata.json` files and regenerate `skill-graph.json` (.skilled/skills/cli-external-orchestration/graph-metadata.json, .skilled/skills/cli-jev/graph-metadata.json, .skilled/skills/mcp-tooling/graph-metadata.json, .skilled/skills/sk-git/graph-metadata.json, .skilled/skills/system-skill-advisor/runtime/scripts/skill-graph.json)
...
- [x] T034 Merged-tree checks: before the re-capture the battery failed 3 tests, all ratchets reporting improvements (full-corpus top-1 152 against 151, memory_save 27 against 26, four changed ledger entries), with the Pi dedup suite at 15 of 15 and the OpenCode plugin suite at 29 of 29. After the re-capture it ran 891 passed, 0 failed, 7 skipped of 898, 123.9 s
