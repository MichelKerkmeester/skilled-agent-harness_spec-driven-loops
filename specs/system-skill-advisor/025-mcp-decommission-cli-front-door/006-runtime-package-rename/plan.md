---
title: "Implementation Plan: Phase 6: runtime-package-rename"
...
# Implementation Plan: Phase 6: runtime-package-rename
...
### Pre-Task Checklist
- [ ] Read `goal.md`, then `spec.md`, this plan and `tasks.md` before the first edit
- [ ] Confirm the target files match this phase's declared scope
...
| Rule | Requirement |
|------|-------------|
| TASK-SEQ | Execute tasks in dependency order; nothing in a later phase starts before this one hands off |
| TASK-SCOPE | Touch only the files this phase names; anything else is a finding, recorded not fixed |
| TASK-VERIFY | Run the task's verification and read its output before marking it complete |
| TASK-EXEC | Build work runs on the executor the parent directive names; substituting one is an amendment |
...
1. Mark the task BLOCKED with the blocking fact, not a summary of it
