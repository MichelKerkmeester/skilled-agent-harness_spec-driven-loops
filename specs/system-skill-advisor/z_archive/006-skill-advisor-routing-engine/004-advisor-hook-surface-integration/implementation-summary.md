---
title: "...-kit/026-graph-and-context-optimization/006-skill-advisor/004-advisor-hook-surface-integration/implementation-summary]"
...
| Field | Value |
|-------|-------|
| **Spec Folder** | 004-advisor-hook-surface-integration |
...
| 2026-04-19T11:00:00Z | Wave-3 synthesis written | — | (pending commit) | research-validation (111 lines, per-child delta + severity action list) under `research/020-skill-advisor-hook-surface-pt-03/` |
...
**Status:** Complete (2026-04-19) — HARD GATE lifted for 006/007/008. Shipped `render.ts` pure renderer (whitelist fields only, canonical-fold sanitization, instruction-regex deny), `normalize-adapter-output.ts` cross-runtime comparator, `metrics.ts` (`speckit_advisor_hook_*` namespace, `AdvisorHookDiagnosticRecord` JSONL schema, alert-threshold env overrides, `advisor-hook-health` section builder). 10 canonical JSON fixtures + 5 Vitest harnesses (renderer/observability/200-prompt corpus parity/timing/privacy). Key gate: 200/200 corpus parity PASS; cache-hit lane p95 0.016ms; 30-turn replay hit rate 66.7%.

### 006-claude-hook-wiring
**Status:** Complete (2026-04-19). Shipped `hooks/claude/user-prompt-submit.ts` reading Claude JSON from stdin, delegating to `buildSkillAdvisorBrief(..., {runtime:'claude'})`, rendering via `renderAdvisorBrief()`, emitting JSON `hookSpecificOutput.additionalContext`. Fail-open returns `{}` (exit 0) on all error paths. Registered in `.claude/settings.local.json`. Tests: 9/9 PASS. Real interactive smoke deferred to T9 gauntlet.

### 007-gemini-copilot-hook-wiring
**Status:** Complete (2026-04-19). Shipped Gemini `BeforeAgent` adapter (`hooks/gemini/user-prompt-submit.ts`) emitting JSON `additionalContext`; Copilot adapter (`hooks/copilot/user-prompt-submit.ts`) with SDK `onUserPromptSubmitted` preferred + wrapper fallback when SDK local-unavailable. 3-runtime parity harness (Claude/Gemini/Copilot) covering 5 canonical fixtures — identical `additionalContext` after transport normalization. Gemini registered in `.gemini/settings.json`. Copilot wrapper-fallback produces `promptWrapper` when brief exists; notification-only `{}` explicitly separated.
