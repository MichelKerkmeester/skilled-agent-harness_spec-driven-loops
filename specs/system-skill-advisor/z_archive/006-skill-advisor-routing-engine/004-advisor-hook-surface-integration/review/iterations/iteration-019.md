...
- `.opencode/skills/system-spec-kit/mcp_server/tests/spec-kit-skill-advisor-plugin.vitest.ts:239-274` → regression coverage proves session isolation and source-signature invalidation, but there is no assertion that separate plugin `ctx.directory` values get separate cache entries.
...
- `.opencode/skills/system-spec-kit/mcp_server/hooks/claude/user-prompt-submit.ts:120-129` → Claude hook still short-circuits on `SPECKIT_SKILL_ADVISOR_HOOK_DISABLED`.
- `.opencode/skills/system-spec-kit/mcp_server/hooks/gemini/user-prompt-submit.ts:134-143` → Gemini hook still short-circuits on `SPECKIT_SKILL_ADVISOR_HOOK_DISABLED`.
- `.opencode/skills/system-spec-kit/mcp_server/hooks/copilot/user-prompt-submit.ts:160-169` → Copilot hook still short-circuits on `SPECKIT_SKILL_ADVISOR_HOOK_DISABLED`.
- `.opencode/skills/system-spec-kit/mcp_server/hooks/codex/user-prompt-submit.ts:217-226` → Codex hook still short-circuits on `SPECKIT_SKILL_ADVISOR_HOOK_DISABLED`.
- `.opencode/skills/system-spec-kit/mcp_server/tests/spec-kit-skill-advisor-plugin.vitest.ts:153-194` → plugin tests still assert shared-env disable behavior and SIGTERM-to-SIGKILL timeout escalation.
...
- `.opencode/skills/system-spec-kit/mcp_server/tests/advisor-runtime-parity.vitest.ts:203-218` → parity harness still includes `opencode-plugin` alongside Claude, Gemini, Copilot, Codex, and the Copilot wrapper.
- `.opencode/skills/system-spec-kit/mcp_server/tests/advisor-corpus-parity.vitest.ts:76-85` → corpus parity test still asserts a 200-prompt corpus before comparing direct CLI vs hook top-1 outputs.
