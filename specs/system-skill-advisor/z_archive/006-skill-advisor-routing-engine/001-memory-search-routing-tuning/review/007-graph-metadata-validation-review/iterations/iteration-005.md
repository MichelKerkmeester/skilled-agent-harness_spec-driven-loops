...
## Findings
### P1 - Required
- **F004**: Documented derived-field caps are not enforced by the schema validator - `spec.md:27` - The packet documents max sizes for `trigger_phrases`, `key_topics`, `key_files`, and `entities`, but the live `graphMetadataDerivedSchema` accepts unbounded arrays for all four fields, so oversized payloads still validate. [SOURCE: spec.md:27-30; .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-schema.ts:36-49]
