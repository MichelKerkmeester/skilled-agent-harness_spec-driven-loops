...
## Findings
1. The graph parser builds `triggerPhrases` with `normalizeUnique(...)` and writes the full array into `derived.trigger_phrases` without any `.slice(0, 12)`. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:529-545]
2. The graph-metadata schema only requires `trigger_phrases` to be a non-empty string array; it does not impose a max-length bound. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-schema.ts:29-39]
...
4. This is a direct omission rather than a disputed contract. `key_topics` already slices to 12, so the minimal patch is simply to slice `triggerPhrases` the same way before storing them. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:474-480]
...
## Sources Consulted
- `.opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:474-545`
- `.opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-schema.ts:29-39`
