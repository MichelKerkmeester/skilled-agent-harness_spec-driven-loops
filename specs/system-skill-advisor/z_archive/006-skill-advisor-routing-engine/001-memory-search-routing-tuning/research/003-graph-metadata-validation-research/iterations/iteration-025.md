...
1. Phase `001-fix-status-derivation` should stay inside `graph-metadata-parser.ts`:
   ...
   - only use `implementation-summary.md` as a completion signal when `checklist.md` is absent or complete [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:498-510] [SOURCE: .opencode/skills/system-spec-kit/scripts/spec/check-completion.sh:204-235]
2. Phase `002-sanitize-key-files` should also stay parser-local:
   ...
   - append `docs.map((doc) => doc.relativePath)` after filtering, then `normalizeUnique(...).slice(0, 20)` as today [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:318-334] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:463-471]
3. Phase `003-deduplicate-entities` needs a slightly richer patch than the earlier wave implied:
   ...
   - prefer canonical packet-doc paths (`spec.md`, `plan.md`, etc.) over non-canonical collision paths such as `specs/.../spec.md` [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:418-446] [SOURCE: .opencode/specs/system-spec-kit/023-hybrid-rag-fusion-refinement/graph-metadata.json]
4. Phase `004-normalize-legacy-files` is still stale on the active branch because the live corpus contains `0` legacy text files. The safest concrete action is to rewrite the phase as a guarded no-op helper in `backfill-graph-metadata.ts`:
   ...
   - otherwise emit a no-op summary and leave the tree untouched [SOURCE: .opencode/skills/system-spec-kit/scripts/graph/backfill-graph-metadata.ts:139-177] [SOURCE: live filesystem scan over `.opencode/specs` on 2026-04-13]
5. The trigger-cap fix is implementation-ready but is not currently owned by a child phase:
   ...
   - leave schema hardening (`.max(12)`) as an optional second step after regenerated metadata proves clean [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:523-545] [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-schema.ts:32-44]
...
## Sources Consulted
- `.opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:318-545`
