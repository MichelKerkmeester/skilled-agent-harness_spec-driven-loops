...
## Findings
1. `deriveEntities()` first seeds entities from every `key_files` entry using `path.basename(filePath)`, then adds up to six extracted doc entities per source doc, and finally truncates the combined list to 16 items. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts:418-446]
