...
Codex gpt-5.5 xhigh impact analysis of these constraints lives at `001-extraction-design-and-adr/research/standalone-mcp-discussion.md`. The discussion narrows the candidate architectural shapes from 4 down to 2 and **recommends "Standalone Advisor MCP With Legacy Tool Bridge"** — preserving the `advisor_*` tool ids on the new server, with `spec_kit_memory` either exposing deprecated proxy tools or fail-fast hints during a migration window.
...
- `002-semantic-routing-lane` shipped (commit `48d5470bc` and earlier on main). The 015 line stabilized advisor behavior so a structural migration is safe.
