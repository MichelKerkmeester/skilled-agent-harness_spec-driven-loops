...
- Hunter: confirmed. `readAdvisorStatus()` catches failures and returns `freshness: 'unavailable'` with errors at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/handlers/advisor-status.ts:151`; the direct recommendation path only fails open for `absent` at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/handlers/advisor-recommend.ts:161`; it then scores at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/handlers/advisor-recommend.ts:195`. The compat probe treats this same freshness as unavailable through `AVAILABLE_FRESHNESS.has(status.freshness)` at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/lib/compat/daemon-probe.ts:53`.
...
- Hunter: confirmed. `skill_graph_scan` is exposed as a public maintenance tool at `.opencode/skills/system-spec-kit/mcp_server/tool-schemas.ts:673` with only `skillsRoot` at line 679. The server builds `callerContext` at `.opencode/skills/system-spec-kit/mcp_server/context-server.ts:925` and passes it through `runWithCallerContext()` at line 1010, but the generic dispatcher calls `dispatcher.handleTool(name, validatedArgs)` at `.opencode/skills/system-spec-kit/mcp_server/tools/index.ts:107`. The skill-graph dispatcher routes scan directly at `.opencode/skills/system-spec-kit/mcp_server/tools/skill-graph-tools.ts:53`, and the handler indexes plus publishes generation at `.opencode/skills/system-spec-kit/mcp_server/handlers/skill-graph/scan.ts:40` and line 42.
...
- Hunter: confirmed. Existing advisor tests cover disabled fail-open at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/tests/handlers/advisor-recommend.vitest.ts:238`, absent fail-open at line 249, and stale warnings at line 338, but not direct unavailable fail-open. `skill_graph_scan` tests assert direct routing at `.opencode/skills/system-spec-kit/mcp_server/tests/skill-graph-schema.vitest.ts:53` and pass-through at line 133, but not untrusted rejection. Daemon tests cover busy retry at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/tests/daemon-freshness-foundation.vitest.ts:187` and lease exclusivity at line 307, but not concurrent corruption rebuild. Diagnostic tests check prompt redaction at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/tests/handlers/advisor-status.vitest.ts:97`, not filesystem path redaction.
...
- T1: Add `unavailableOutput()` branch in `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/handlers/advisor-recommend.ts:161`; add regression `tests/handlers/advisor-recommend-unavailable.vitest.ts`.
