...
| `lib/scorer/skill-outcome-store.ts` | DELETED | Built at `3682ec59da` as a 364-line durable append-only store, wired live-adjacent at `f3a1231c66`, deleted at `6b99eb68d2`; current tree absence is expected after the measured NO-GO |
| `lib/scorer/outcome-weighted-rerank.ts` | DELETED | Built at `3682ec59da` as a 124-line shadow re-rank, wired live-adjacent at `f3a1231c66`, deleted at `6b99eb68d2`; current tree absence is expected after MRR delta +0.005 to +0.008 versus SD 0.0237 and right-skill@3 = 0.000 across all 90 runs |
...
| `scripts/skill-outcome-fold-tick.mjs` | Created (confirmed present) | Out-of-process cron/maintenance fold-tick runner - exists but orphaned; its store dependency (`skill-outcome-store.ts`) was created at `3682ec59da` then deleted at `6b99eb68d2`, not "never created" |
| `tests/scorer/outcome-weighted-ranking.vitest.ts` | DELETED | One of the 4 test files deleted at `6b99eb68d2` after the measured NO-GO: MRR delta +0.005 to +0.008 versus SD 0.0237, right-skill@3 = 0.000 across all 90 runs |
...
**Superseded by the pass-2 correction above - the limitations below described the state as of the original shadow-only build, before it was measured and deleted at `6b99eb68d2`.** Preserved for historical context only:
