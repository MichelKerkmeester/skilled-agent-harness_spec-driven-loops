...
### Overview
The C4 machinery shipped default-off shadow in `7cd16f70fa`: `beta-reliability.ts` (shared f64 posterior + advisor adapter), the standalone `shadow-weight-promoter.ts` module as 339 lines alongside `beta-reliability.ts`, and the `feedback-calibration.ts`/`lane-registry.ts` integration wiring (estimator→registry seam, asymmetric deltas, content-fold, decay channel). Git history shows `shadow-weight-promoter.ts` was later deleted in `6b99eb68d2`, the same commit that removed outcome-weighted-rerank, its only consumer. Everything stays shadow-only behind `liveWeightsFrozen`, only the live-flip micro-benchmark remains NO-GO. Everything stays shadow-only behind `liveWeightsFrozen`, promotion to live is a documented NO-GO until a micro-benchmark earns it.

**CORRECTION (2026-07-01, drift-audit remediation -- pass 2 / git-history reconciliation): the 'never committed' claim above is factually wrong -- see full correction.** Git history shows the standalone `shadow-weight-promoter.ts` module was committed in `7cd16f70fa` as a 339-line module alongside `beta-reliability.ts`. Git history also shows `shadow-weight-promoter.ts` was deleted in `6b99eb68d2`, the same commit that removed the outcome-weighted-rerank feature; in `6b99eb68d2`, outcome-weighted-rerank was the promoter's only consumer. The promoter's sole purpose was feeding that feature's live-adjacent rerank, so when `6b99eb68d2` removed outcome-weighted-rerank for measured-negative reasons, `shadow-weight-promoter.ts` was removed in `6b99eb68d2` alongside it as dead weight. Q-002 (daemon reload semantics) remains a separate, still-unresolved architecture blocker independent of the `6b99eb68d2` deletion. Reviving `shadow-weight-promoter.ts` was considered and explicitly not approved by the operator, so this is a documentation correction only; no code changes accompany it.
<!-- /ANCHOR:summary -->
