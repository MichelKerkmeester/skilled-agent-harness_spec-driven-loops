...
**Scope:** `.opencode/skills/system-skill-advisor/mcp_server/lib/scorer` and the `016-skill-advisor-tuning` spec subtree it documents (8 workstream children).
...
2. **WS1 "empirically falsified" claim has no evidence trail** — the parent spec asserts it, but packet 001's own docs say "not started / GATED" and contain zero falsification records despite a calibration commit (`875884b80a`). Reconstruct the experiment, verify the claim, resolve the 001 status contradiction.
