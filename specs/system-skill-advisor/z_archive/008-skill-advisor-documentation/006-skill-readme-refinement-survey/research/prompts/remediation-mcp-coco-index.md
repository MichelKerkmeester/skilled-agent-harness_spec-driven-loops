REMEDIATE: mcp-coco-index/README.md per packet-005-style refinement rules.

# Goal

Modify exactly one file: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/mcp-coco-index/README.md`

Apply the refinements catalogued in the audit report for skill `mcp-coco-index`. Output the modified README directly in-place (the agent-config allows writes only to this one path).
...
Section 1 tables to remove or move:
  - lines 71-82 (2 cols, 9 rows): Key Statistics table (Version, MCP tools exposed, Supported languages, Default embedding model, Unified with Memory MCP, Vector storage, Chunk size, Similarity metric) -> to-paragraph
  - lines 84-90 (3 cols, 5 rows): How This Compares table (ccc search vs code_graph_query vs Grep vs Glob vs Read for Use When, Limitation) -> move-to-§3
Em dashes: 2 total
  - line 33: 
  - line 35: 
Cross-skill coupling: 1 hits (review)
...
Write the entire revised README to `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/mcp-coco-index/README.md`. Do not produce any other files. Do not edit any other file.
