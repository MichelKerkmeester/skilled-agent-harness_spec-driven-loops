REMEDIATE: deep-review/README.md per packet-005-style refinement rules.

# Goal

Modify exactly one file: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-review/README.md`

Apply the refinements catalogued in the audit report for skill `deep-review`. Output the modified README directly in-place (the agent-config allows writes only to this one path).
...
Section 1 tables to remove or move:
  - lines 40-52 (2 cols, 10 rows): Key Statistics table (Default max iterations, Default convergence threshold, Stuck threshold, Review dimensions, Finding severities, Quality gates, Verdicts, State files, Tool budget per iteration, Output document) -> to-paragraph
  - lines 54-63 (2 cols, 6 rows): When to Use / Do not use table -> to-paragraph
  - lines 69-78 (2 cols, 6 rows): How This Compares table (sk-code-review single-pass vs deep-review iterative multi-pass) -> move-to-§3
...
Write the entire revised README to `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-review/README.md`. Do not produce any other files. Do not edit any other file.
