REMEDIATE: sk-code/README.md per packet-005-style refinement rules.

# Goal

Modify exactly one file: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/sk-code/README.md`

Apply the refinements catalogued in the audit report for skill `sk-code`. Output the modified README directly in-place (the agent-config allows writes only to this one path).
...
Section 1 tables to remove or move:
  - lines 48-57 (2 cols, 7 rows): Key Statistics table (Skill version, SKILL.md size, Surfaces shipped, Smart Router pseudocode, Reference directories, OPENCODE language sub-detection, Verification scripts) -> to-paragraph
  - lines 59-68 (3 cols, 5 rows): How This Compares table (sk-code vs sk-code-review vs sk-doc for Code implementation, Surface auto-detection, Verification evidence, Findings-first review, Documentation quality, Template customization surface) -> move-to-§3
  - lines 70-81 (2 cols, 9 rows): Key Features table (Surface auto-detection, Intent classification, Resource loading, Phase lifecycle, OPENCODE language sub-detection, Cross-stack motion.dev, Iron Law verification, Template customization surface) -> to-bullets
...
Write the entire revised README to `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/sk-code/README.md`. Do not produce any other files. Do not edit any other file.
