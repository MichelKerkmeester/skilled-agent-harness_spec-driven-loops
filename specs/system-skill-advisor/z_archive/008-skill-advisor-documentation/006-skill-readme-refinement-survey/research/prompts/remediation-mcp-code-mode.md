REMEDIATE: mcp-code-mode/README.md per packet-005-style refinement rules.

# Goal

Modify exactly one file: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/mcp-code-mode/README.md`

Apply the refinements catalogued in the audit report for skill `mcp-code-mode`. Output the modified README directly in-place (the agent-config allows writes only to this one path).
...
Section 1 tables to remove or move:
  - lines 50-58 (4 cols, 6 rows): Key Statistics comparison table (Traditional vs Code Mode for Version, Context tokens, Execution time, API round trips, Simple workflows, Complex workflows) -> move-to-§3
  - lines 60-66 (3 cols, 4 rows): How This Compares table (Direct tool calling vs Code Mode vs Native MCP tools for Context Cost, Multi-tool State, Tool Discovery) -> move-to-§3
  - lines 72-78 (2 cols, 6 rows): Key Features table (Progressive discovery, State persistence, Type safety, Built-in observability, Configurable timeouts, Parallel execution, Error isolation) -> to-bullets
...
Write the entire revised README to `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/mcp-code-mode/README.md`. Do not produce any other files. Do not edit any other file.
