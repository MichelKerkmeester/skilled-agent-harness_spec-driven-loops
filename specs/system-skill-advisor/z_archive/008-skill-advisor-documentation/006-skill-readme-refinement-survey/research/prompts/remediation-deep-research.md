REMEDIATE: deep-research/README.md per packet-005-style refinement rules.

# Goal

Modify exactly one file: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-research/README.md`

Apply the refinements catalogued in the audit report for skill `deep-research`. Output the modified README directly in-place (the agent-config allows writes only to this one path).
...
Section 1 tables to remove or move:
  - lines 81-99 (2 cols, 16 rows): Key Features table (Fresh context per iteration, Lineage-aware lifecycle, Reducer synchronization, Packet-first recovery, Runtime capability matrix, spec.md anchoring, Folder-state contract, Advisory lock + generated fence, Progressive synthesis, Negative knowledge, Quality guards, 3-signal convergence model, Graph-aware legal-stop checks, Semantic coverage graph, Graph convergence guards, Question coverage tracking, Fail-closed corruption handling, Graph convergence fallback, Terminal stop metadata) -> to-bullets
...
Write the entire revised README to `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/deep-research/README.md`. Do not produce any other files. Do not edit any other file.
