REMEDIATE: cli-codex/README.md per packet-005-style refinement rules.

# Goal

Modify exactly one file: `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/cli-codex/README.md`

Apply the refinements catalogued in the audit report for skill `cli-codex`. Output the modified README directly in-place (the agent-config allows writes only to this one path).
...
Section 1 tables to remove or move:
  - lines 50-61 (3 cols, 7 rows): Key Statistics (Model, Default Dispatch, Sandbox Modes, Reasoning Levels, Agent Profiles, References, Version) -> to-paragraph
  - lines 63-73 (5 cols, 6 rows): Comparison table with Claude Code, Gemini, Copilot, Codex for Web search, Code review, Sandbox control, Image input, Session management, Spec Kit handoff, Cloud execution -> move-to-§3
  - lines 75-86 (2 cols, 8 rows): Key Features at a Glance (Web Search, Diff-Aware Review, Sandbox Modes, Reasoning Effort, Image Input, Session Fork, Agent Profiles, Native Hooks) -> to-bullets
...
Write the entire revised README to `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/skills/cli-codex/README.md`. Do not produce any other files. Do not edit any other file.
