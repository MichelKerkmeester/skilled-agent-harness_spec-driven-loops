...
YOUR SCOPE — LINEAGE 4b (RUNTIME CODE: MCP front-proxy reconnect engine):
- .opencode/bin/lib/launcher-session-proxy.cjs (frame splitter, pendingRequests tracker, replaySnapshot, idempotency classifyFrame, -32001 RETRYABLE_RECYCLE_ERROR / -32002 PROTOCOL_MISMATCH terminal CLOSED, socket drain flag, keepalive id-reservation, reattach loop)
- .opencode/bin/mk-spec-memory-launcher.cjs (recycleDaemonInPlace in-place child respawn, bridgeStdioThroughSessionProxy multi-client)
- .opencode/bin/lib/launcher-ipc-bridge.cjs (maybeBridgeLeaseHolder await of async bridge)
FOCUS: replay/dedup correctness across a recycle, protocol-drift fail-closed handling, socket detach/drain/keepalive race conditions, reattach-loop bounds, multi-client transparency, frame-parsing edge cases. Hunt for REAL bugs/edge-cases/security in the deployed implementation. Be concise — output findings + VERDICT only.
