...
YOUR SCOPE — LINEAGE 2 (the durability STRESS TESTS + their fidelity to real code):
- .opencode/skills/system-spec-kit/mcp_server/stress_test/durability/{checkpoint-v2-contention-stress,daemon-recycle-transparency-stress,enrichment-marker-backfill-stress,index-scan-coalescing-stress}.vitest.ts and README.md
- cross-check against the code they exercise: lib/storage/checkpoints.ts, handlers/save/enrichment-state.ts, handlers/memory-index.ts (+ db-state lease), .opencode/bin/lib/launcher-session-proxy.cjs
FOCUS: test correctness; ISOLATION (throwaway temp DBs/sockets only — confirm NO access to production ~/.mk-spec-memory or the live daemon-ipc socket); assertion strength (post-remediation the snapshotFormat assertion is a hard check; the recycle-stress was honestly re-scoped to test the classifier/partition decision — is that re-scoping now ACCURATE and not still overclaiming?); fidelity (does each test actually validate what its name/README claims?). Flag masked/weak assertions, isolation leaks, overclaims, or flaky constructs.
