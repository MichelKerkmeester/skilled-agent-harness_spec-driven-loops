...
CONTEXT: The "013 memory-index-scan" roadmap shipped + deployed to main (self-maintaining index, checkpoint-v2 file snapshots, MCP front-proxy in-place recycle, memory_save enrichment schema v30, post-restore .needs-rebuild sentinel). A gpt-5.5-fast 20-iteration deep review found 18 findings (P0=0), remediated in commit c41dcff894 (one finding — R16-P1-001 includeEmbeddings — was a verified FALSE POSITIVE, left unchanged). You are the independent Opus cross-check.
...
YOUR SCOPE — LINEAGE 4a (RUNTIME CODE: checkpoint-v2 durability):
- .opencode/skills/system-spec-kit/mcp_server/lib/storage/checkpoints.ts (checkpoint-v2 VACUUM INTO create, restoreCheckpointV2 whole-file swap, restore journal swap-pending/swap-done crash-safety, .needs-rebuild sentinel write/clear/repair, mergeCheckpointCatalogRows, enforceMaxCheckpointsInTransaction pruning + snapshot-dir removal)
- .opencode/skills/system-spec-kit/mcp_server/lib/search/vector-index-store.ts (shard attach/swap, reopenActiveDatabase)
- .opencode/skills/system-spec-kit/mcp_server/context-server.ts (SPECKIT_BACKEND_ONLY stdio gate, serverInfo 1.8.0, init / cache invalidation)
FOCUS: restore atomicity + two-phase journal recovery on crash, snapshot-dir vs catalog-row pruning correctness, sentinel set/clear race conditions, shard reopen safety, SQL/transaction correctness. Hunt for REAL bugs/edge-cases/crash-safety gaps in the deployed implementation. Be concise — output findings + VERDICT only.
