...
The caps are present and graceful, but operator visibility is weak. `memory-index-discovery.ts` flips `maxNodesExceeded` and calls `console.warn` at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-index-discovery.ts:36`, `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-index-discovery.ts:42`, and depth warnings at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-index-discovery.ts:103`; the discovery API still returns only `results` at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-index-discovery.ts:158`. `memory_index_scan` calls those discovery helpers at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-index.ts:224` and returns response data at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-index.ts:646` without cap-hit metadata. Code graph has the same shape: cap constants are defined at `.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts:78`, warning-only exits happen at `.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts:1377` and `.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts:1393`, and `findFiles()` returns only files/exclusion counts at `.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts:1425`.
...
The rejection happens before the indexing write path, but it is emitted as a generic thrown `Error`. The path guard rejects excluded realpaths at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:2714` with `throw new Error(...)` at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:2715`; the local catch only translates `VRuleUnavailableError` into an MCP error envelope at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:3047` and rethrows other errors at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:3061`.
...
The normal `memory_save` path prepares and downgrades parsed memory before chunking: `runWithinSpecFolderLock` uses `prepareParsedMemoryForIndexing()` at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:2113`, and chunking is invoked with the guarded `applyPostInsertMetadata` wrapper at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:2304` through `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:2308`. However, `indexChunkedMemoryFile()` itself defaults to `applyPostInsertMetadataFallback` at `.opencode/skills/system-spec-kit/mcp_server/handlers/chunking-orchestrator.ts:161`; that fallback dynamically updates `importance_tier` with no `isConstitutionalPath()` guard at `.opencode/skills/system-spec-kit/mcp_server/handlers/chunking-orchestrator.ts:101` through `.opencode/skills/system-spec-kit/mcp_server/handlers/chunking-orchestrator.ts:119`. Its safe-swap parent update also writes `importance_tier = ?` directly from `parsed.importanceTier` at `.opencode/skills/system-spec-kit/mcp_server/handlers/chunking-orchestrator.ts:510` through `.opencode/skills/system-spec-kit/mcp_server/handlers/chunking-orchestrator.ts:529`.
...
|-------|--------|----------|
| S1 cleanup transaction + TOCTOU | Pass | Apply path opens `database.transaction(...)` and rebuilds the plan inside the transaction at `.opencode/skills/system-spec-kit/scripts/memory/cleanup-index-scope-violations.ts:429` through `.opencode/skills/system-spec-kit/scripts/memory/cleanup-index-scope-violations.ts:435`. Cleanup audit inserts happen inside `applyCleanup()` at `.opencode/skills/system-spec-kit/scripts/memory/cleanup-index-scope-violations.ts:343` through `.opencode/skills/system-spec-kit/scripts/memory/cleanup-index-scope-violations.ts:358`, so an uncaught audit insert failure aborts the transaction rather than committing an unaudited cleanup. |
| S1 action-string contract | Pass | Shared constants define `tier_downgrade_non_constitutional_path` and `tier_downgrade_non_constitutional_path_cleanup` at `.opencode/skills/system-spec-kit/mcp_server/lib/governance/scope-governance.ts:117` through `.opencode/skills/system-spec-kit/mcp_server/lib/governance/scope-governance.ts:120`; cleanup tests assert the cleanup action at `.opencode/skills/system-spec-kit/mcp_server/tests/cleanup-script-audit-emission.vitest.ts:121` through `.opencode/skills/system-spec-kit/mcp_server/tests/cleanup-script-audit-emission.vitest.ts:140`. |
...
| S2 symlink / realpath bypass | Pass | `resolveCanonicalPath()` uses `fs.realpathSync()` with fail-open fallback at `.opencode/skills/system-spec-kit/mcp_server/lib/utils/canonical-path.ts:36` through `.opencode/skills/system-spec-kit/mcp_server/lib/utils/canonical-path.ts:41`; `memory-save` evaluates `shouldIndexForMemory()` and `isConstitutionalPath()` against the canonical path at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:309` through `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:313`; code-graph specific file handling rejects by canonical path at `.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts:1431` through `.opencode/skills/system-spec-kit/mcp_server/code_graph/lib/structural-indexer.ts:1445`. Symlink regression tests cover both surfaces at `.opencode/skills/system-spec-kit/mcp_server/tests/symlink-realpath-hardening.vitest.ts:51` through `.opencode/skills/system-spec-kit/mcp_server/tests/symlink-realpath-hardening.vitest.ts:109`. |
| S3 walker DoS caps | Partial | Caps exist and stop gracefully, but only log warnings; see P2-003. |
| S4 constitutional promotion / downgrade paths | Partial | `memory_update`, vector-index mutations, and post-insert metadata guard non-constitutional paths, with tests at `.opencode/skills/system-spec-kit/mcp_server/tests/memory-crud-update-constitutional-guard.vitest.ts:145` through `.opencode/skills/system-spec-kit/mcp_server/tests/memory-crud-update-constitutional-guard.vitest.ts:223`. The exported chunking helper fallback remains a defense-in-depth gap; see P2-005. |
| S5 governance audit durability under failure | Pass | `recordGovernanceAudit()` writes via `INSERT INTO governance_audit` at `.opencode/skills/system-spec-kit/mcp_server/lib/governance/scope-governance.ts:350` through `.opencode/skills/system-spec-kit/mcp_server/lib/governance/scope-governance.ts:369`; save/update/post/vector callers catch audit write failures and preserve the invariant at `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-save.ts:326`, `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-crud-update.ts:202`, `.opencode/skills/system-spec-kit/mcp_server/lib/storage/post-insert-metadata.ts:117`, and `.opencode/skills/system-spec-kit/mcp_server/lib/search/vector-index-mutations.ts:93`. Cleanup intentionally fails closed by leaving the audit insert uncaught inside the cleanup transaction. |
