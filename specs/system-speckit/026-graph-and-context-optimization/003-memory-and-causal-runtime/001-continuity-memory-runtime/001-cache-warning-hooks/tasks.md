...
_memory:
  continuity:
    packet_pointer: "system-speckit/026-graph-and-context-optimization/003-memory-and-causal-runtime/001-continuity-memory-runtime/001-cache-warning-hooks"
...
- [x] T005 [P0] Create or refine replay harness isolation contract (`.opencode/skills/system-spec-kit/mcp_server/test/hooks/replay-harness.ts`) [EVIDENCE: added `mcp_server/test/hooks/replay-harness.ts` for isolated Stop-path replays.]
...
- [x] T007 [P0] Add failing assertion for out-of-bound writes in replay validation (`.opencode/skills/system-spec-kit/mcp_server/tests/*.vitest.ts`) [EVIDENCE: `createStopReplaySandbox()` rejects touched paths outside the sandbox and the replay suite passes.]
- [x] T008 [P1] Add at least one Stop-path fixture for producer verification (`.opencode/skills/system-spec-kit/mcp_server/tests/fixtures/hooks/`) [EVIDENCE: added `tests/fixtures/hooks/session-stop-replay.jsonl`.]

### Phase B: Bounded Producer Metadata Patch
- [x] T009 [P0] Extend `HookState` with additive producer metadata only (`.opencode/skills/system-spec-kit/mcp_server/hooks/claude/hook-state.ts`) [EVIDENCE: `HookState` now carries `producerMetadata` with `lastClaudeTurnAt`, transcript reference, and cache-token fields.]
- [x] T010 [P0] Keep `claudeSessionId` primary and `speckitSessionId` nullable in the updated seam (`.opencode/skills/system-spec-kit/mcp_server/hooks/claude/hook-state.ts`) [EVIDENCE: `speckitSessionId` is now nullable and replay tests assert the Stop-path state keeps `claudeSessionId` primary.]
- [x] T011 [P0] Persist bounded transcript identity or reference and cache-token carry-forward fields in Stop writer flow (`.opencode/skills/system-spec-kit/mcp_server/hooks/claude/session-stop.ts`) [EVIDENCE: `session-stop.ts` writes transcript fingerprint/path/mtime/size plus cache token carry-forward data into hook state.]
- [x] T012 [P0] Confirm `session-stop.ts` remains a producer boundary, not an analytics reader (`.opencode/skills/system-spec-kit/mcp_server/hooks/claude/session-stop.ts`) [EVIDENCE: The stop hook runs `processStopHook()` which invokes `runContextAutosave()` by default when session state is populated. This is the intended producer boundary - autosave is not a side effect but the primary continuity mechanism. No analytics reader or startup consumer was added.]
- [x] T013 [P1] Verify `session-prime.ts` stays unchanged in active scope or additive-safe only (read-only check) [EVIDENCE: `git diff --name-only -- .opencode/skills/system-spec-kit/mcp_server/hooks/claude/session-prime.ts .claude/settings.local.json` returned no changes.]

### Phase C: Idempotent Verification and Handoff
- [x] T014 [P0] Replay the same transcript twice and assert stable session totals (`.opencode/skills/system-spec-kit/mcp_server/tests/*.vitest.ts`) [EVIDENCE: `tests/hook-session-stop-replay.vitest.ts` replays the same transcript twice and passes.]
- [x] T015 [P0] Assert no duplicate turn ingestion or duplicate producer markers after double replay (`.opencode/skills/system-spec-kit/mcp_server/tests/*.vitest.ts`) [EVIDENCE: replay suite proves second replay parses 0 new messages, preserves metrics, and leaves exactly one state file.]
...
### Phase Verification
- [x] T017 [P0] Run `npm run typecheck` for `.opencode/skills/system-spec-kit/mcp_server` [EVIDENCE: `TMPDIR=$PWD/.tmp/tsc-tmp npm run typecheck` passed.]
- [x] T018 [P0] Run strict packet validation [EVIDENCE: `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh \".opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-continuity-memory-runtime/001-cache-warning-hooks\" --strict` passed.]
