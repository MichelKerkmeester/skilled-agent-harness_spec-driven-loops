---
title: "Iteration 19 — Domain 1: Silent Fail-Open Patterns (9/10)"
trigger_phrases: []
---
...
- **Evidence:** `transitiveTraversal()` pushes result rows with `targetNode?.fqName ?? null`, `targetNode?.filePath ?? null`, and `targetNode?.startLine ?? null` (and the symmetric `sourceNode` fallbacks) at `.opencode/skills/system-spec-kit/mcp_server/handlers/code-graph/query.ts:127-166`. The DB layer explicitly permits those null joins by returning `{ edge, targetNode: null }` / `{ edge, sourceNode: null }` when the node lookup misses at `.opencode/skills/system-spec-kit/mcp_server/lib/code-graph/code-graph-db.ts:503-507,521-525`. The transitive branch then returns that payload unchanged inside `status: "ok"` at `.opencode/skills/system-spec-kit/mcp_server/handlers/code-graph/query.ts:417-436`. Direct coverage only locks the happy path where the traversed node exists: `.opencode/skills/system-spec-kit/mcp_server/tests/code-graph-query-handler.vitest.ts:87-107`.
