...
6. Used `view` on `shared/review-research-paths.cjs` and `command/deep/assets/deep_start-research-loop_auto.yaml`; confirmed packet selection happens before `step_acquire_lock`, so the advisory lock cannot serialize `pt-{NN}` allocation.
