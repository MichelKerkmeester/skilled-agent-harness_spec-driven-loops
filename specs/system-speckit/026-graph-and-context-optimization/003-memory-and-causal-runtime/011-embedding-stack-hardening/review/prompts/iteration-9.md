...
Iteration: 9 of 20
Dimension: FINAL broad discovery sweep (all dimensions, whole scope)
Prior Findings: P0=0 P1=8 P2=3 (validated)
Coverage: all 4 dims; iter-8 ruled clean: cache lib/migration, fatalShutdown ordering, retention sweep, embed batching
Last 2 ratios: 0 -> 0.1
Stuck count: 0
Provisional Verdict: CONDITIONAL hasAdvisories=true

FINAL discovery sweep before convergence. The review has found 8 P1 + 3 P2 (clustered in single-writer/lease + WAL durability) and ruled several areas clean. This is the LAST-CHANCE pass for anything genuinely missed across the WHOLE scope — especially any P0/data-corruption path not yet seen. Probe: WAL connection-lifecycle (the iter-8 non-active-connection TRUNCATE gap — are there related untracked-connection or shard-detach ordering issues?), the embed dim-adoption/normalization correctness, the auto-select cascade ordering under partial-provider availability, and any error path that could drop/duplicate data. Report only genuinely NEW findings with file:line + full claim-adjudication packet for P0/P1. If you find nothing new, say so explicitly — that confirms convergence.

Prior findings (do NOT re-report): the 8 P1 + 3 P2 (lease/supervision cluster, provider-override, reindex-cancel, tcp-exposure, workflow-lock, daemon-child-ledger, WAL non-active-connection TRUNCATE; direct-startup-unlink, 005-idle-matrix, env-knobs).

Review Iteration: 9 of 20
Mode: review
Dimension: final broad discovery sweep (all dimensions; last-chance for missed P0/data-safety across the whole scope)
...
- Write iteration narrative to: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-009.md
- Write per-iteration delta file to: .opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-009.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-009.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-009.jsonl`, this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the review. NEVER execute the out-of-scope mutation. The review packet (`.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-009.md` directory and parents) is the only zone for your writes; the reviewed target spec/code is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/iterations/iteration-009.md` (path pre-substituted for the current iteration number). Structure: headings for Dimension, Files Reviewed, Findings by Severity (P0/P1/P2), Traceability Checks, Verdict, Next Dimension.
...
3. **Per-iteration delta file** at `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/003-memory-and-causal-runtime/011-embedding-stack-hardening/review/deltas/iter-009.jsonl` (path pre-substituted, e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same as the state-log append) plus per-event structured records (one per graphEvent, finding, classification, traceability-check, ruled_out direction). Each record on its own JSON line.
