...
## Focus
Scanned correctness-sensitive code and config surfaces for post-022 residue that can change runtime behavior: provider/profile resolution, provider-keyed SQLite path derivation, migration/eval callers, committed runtime config, and dependency manifests. I treated the Voyage -> OpenAI -> llama-cpp -> hf-local cascade as canonical and skipped frozen review/history/evidence artifacts, legacy model registries, and vitest temp-database idioms.
...
- Notes: Saturation not reached. The main correctness residue is a split-brain resolver: `factory.ts` correctly auto-selects llama-cpp, while `profile.ts` still resolves no-key profiles directly to hf-local. Several eval/migration callers inherit that mismatch through `resolveActiveProfileDbPath()`. I did not flag the canonical cloud-key cascade, llama-cpp default DB notes in runtime configs, legacy model lookup registries, vitest `context-index.sqlite` temp patterns, `test_backward_compat.py`, frozen review/spec artifacts, or historical evidence/log files.
