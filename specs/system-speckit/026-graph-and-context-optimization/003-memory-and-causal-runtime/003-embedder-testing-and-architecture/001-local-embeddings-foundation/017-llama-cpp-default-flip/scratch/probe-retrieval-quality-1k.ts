...
const CORPUS_PATH = path.join(__dirname, 'probe-1k-corpus.json');
const QUERIES_PATH = path.join(__dirname, 'probe-1k-queries.json');
const EMBEDDINGS_PATH = path.join(__dirname, 'probe-1k-embeddings.json');
const RESULTS_PATH = path.join(__dirname, 'probe-1k-results.json');
const RESULTS_MD_PATH = path.join(__dirname, 'probe-1k-results.md');
...
function sampleCorpus(): CorpusDoc[] {
  ...
  try {
    ...
    if (docs.length < CORPUS_SIZE) {
      throw new Error(`Expected ${CORPUS_SIZE} usable corpus rows, got ${docs.length}`);
    }
    return docs;
  } finally {
    db.close();
...
function writeMarkdown(results: ResultsJson, corpus: CorpusDoc[]): void {
  ...
  const lines: string[] = [
    ...
    '| Metric | Value | Target |',
    '|--------|-------|--------|',
    `| Corpus size | ${results.corpus_size} | 1000 |`,
    `| Query count | ${results.query_count} | 100 |`,
...
async function run(): Promise<void> {
  ...
  if (queries.length !== QUERY_COUNT) {
    throw new Error(`Expected ${QUERY_COUNT} derived queries, got ${queries.length}`);
  }
  ...
  const oneLine = verdict === 'EQUIVALENT'
    ? 'The 1k/100-query rerun preserves retrieval ordering closely enough to keep llama-cpp as the default.'
    : verdict === 'MILD_DIVERGENCE'
      ? 'The 1k/100-query rerun mostly agrees but shows enough movement to treat the default flip as conditional.'
      : 'The 1k/100-query rerun diverges materially; restore hf-local as the default.';
