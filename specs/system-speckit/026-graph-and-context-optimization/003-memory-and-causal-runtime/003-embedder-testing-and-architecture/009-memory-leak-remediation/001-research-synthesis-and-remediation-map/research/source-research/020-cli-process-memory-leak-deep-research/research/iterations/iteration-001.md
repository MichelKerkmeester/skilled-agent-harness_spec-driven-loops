...
2. The same wrapper is the only caller for `cli-codex`, `cli-gemini`, and `cli-claude-code` branches in **deep-research** dispatch — `if_cli_codex` (line 584-629), `if_cli_gemini` (line 630-674), `if_cli_claude_code` (line 675-723). All three import `runAuditedExecutorCommand` and call `process.exit(...)` from a `node --experimental-strip-types --input-type=module` heredoc. [SOURCE: .opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:586-715]
...
- Contradictory evidence: `loop_protocol.md` says cli-claude-code, cli-gemini, cli-codex dispatch is "all branches share … 3. Executor audit append via `appendExecutorAuditToLastRecord`" — but the live `deep_start-review-loop_auto.yaml` branches for those kinds run raw shell, not the audited spawnSync wrapper. The protocol doc reflects the deep-research path; deep-review diverged. Flag for follow-on documentation reconciliation. [SOURCE: .opencode/skills/deep-research/references/loop_protocol.md:221-226]
...
- `.opencode/commands/deep/assets/deep_start-research-loop_auto.yaml:120-918`
- `.opencode/commands/deep/assets/deep_start-review-loop_auto.yaml:670-870`
- `.opencode/commands/deep/start-research-loop.md:100-205`
