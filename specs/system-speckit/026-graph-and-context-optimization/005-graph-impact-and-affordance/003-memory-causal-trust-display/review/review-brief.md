...
Conduct **7 independent review iterations** of sub-phase `005-memory-causal-trust-display` of phase `010-graph-impact-and-affordance-uplift`. Each iteration is a focused audit pass on a different dimension. Track findings as P0/P1/P2 with file:line evidence. Produce a single `review-report.md` at the end.

## Scope

**Sub-phase root:** `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/010-graph-impact-and-affordance-uplift/005-memory-causal-trust-display/`

**Implementation surface (read in full before reviewing):**
- mcp_server/formatters/search-results.ts (trustBadges field)
- mcp_server/lib/response/profile-formatters.ts
- mcp_server/tests/memory/trust-badges.test.ts (currently .skip — known follow-up)
- mcp_server/tests/response-profile-formatters.vitest.ts

**Spec context:**
- `010/005-memory-causal-trust-display/spec.md` (requirements)
- `010/005-memory-causal-trust-display/checklist.md` (verification items)
- `010/005-memory-causal-trust-display/implementation-summary.md` (what was built)
...
Write `010/005-memory-causal-trust-display/review/review-report.md` (create directory if needed):
...
- Stay scoped to `005-memory-causal-trust-display` — don't audit other sub-phases.
