...
- Read `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/fixtures/skill_advisor_regression_cases.jsonl` in full.
- Read the regression harness in `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor_regression.py`, especially `run_case_inprocess()`, `evaluate_case()`, and `compute_metrics()`.
...
|----|----------|-----------|-------------|----------------|
| F-013-C3-01 | P1 | `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/fixtures/skill_advisor_regression_cases.jsonl:26` | The regression fixture expects `sk-code-review` for `review and update this`, and the advisor returns it with high confidence (`0.93`, uncertainty `0.28`). That is a bad recommendation for a write/edit prompt: `sk-code-review` says review use cases are findings/gate passes and explicitly excludes feature implementation without review intent at `.opencode/skills/sk-code-review/SKILL.md:30` and `.opencode/skills/sk-code-review/SKILL.md:36`. The scorer cause is visible in `analyze_request()`: token boosts are applied directly at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py:2759`, `review` has a stronger single-skill boost at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py:1256`, while `update` only gives `sk-code` `0.2` in the multi-skill map at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py:1442`. `calculate_confidence()` then lets boosted scores cap at `0.95` at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py:2359`, so the false positive confidently passes `filter_recommendations()` at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py:2547`. | Change fixture line 26 to expect `sk-code` or to mark the case ambiguous, then add a write-intent override/penalty: when `review` co-occurs with edit verbs such as `update`, `fix`, `modify`, or `implement`, prefer `sk-code` unless the prompt also contains review-only terms like `findings`, `diff`, `PR review`, `security review`, or `no edits`. |

## Questions Answered
- Where does the advisor confidently recommend the wrong skill? The clearest checked-in example is fixture `P1-REVIEW-007` at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/fixtures/skill_advisor_regression_cases.jsonl:26`.
- Where does it correctly hedge with low confidence? `P0-UNC-001` abstains for `api chain mcp` at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/fixtures/skill_advisor_regression_cases.jsonl:7`; the paired confidence-only case at line 8 returns `mcp-code-mode` despite uncertainty `0.39`, which is expected because `filter_recommendations()` bypasses uncertainty in confidence-only mode at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill_advisor.py:2556`.
