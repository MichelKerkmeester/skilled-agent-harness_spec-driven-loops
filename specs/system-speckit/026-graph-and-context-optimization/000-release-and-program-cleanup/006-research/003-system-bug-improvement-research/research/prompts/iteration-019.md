---
title: "Deep-Research Iteration 019 — Angle D4: Spec-kit folder topology sustainability"
trigger_phrases: []
---
# Deep-Research Iteration 019 — Angle D4: Spec-kit folder topology sustainability

You are the deep-research LEAF agent for iteration 019 of 20. The packet root is `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/003-system-bug-improvement-research/`.

**Workflow position**: Iteration 019 of 20. Convergence threshold: 0.01. Executor: yourself (cli-codex gpt-5.5 high, normal speed).

## Focus

**Angle D4 — Spec-kit folder topology sustainability**
...
### 1. Iteration narrative — write to `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/003-system-bug-improvement-research/research/iterations/iteration-019.md`
...
|----|----------|-----------|-------------|----------------|
| F-019-D4-01 | P0/P1/P2/none | path/file.ext:LINE | What's wrong | What to do |

## Questions Answered
[Which sub-questions of D4 did this iteration answer?]

## Questions Remaining
[Open D4 questions for follow-on packets]
...
### 2. Per-iteration delta JSONL — write to `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/003-system-bug-improvement-research/research/deltas/iter-019.jsonl`

One JSON record per line. MUST start with the canonical iteration record:

```json
{"type":"iteration","iteration":19,"newInfoRatio":0.85,"status":"insight","focus":"D4: Spec-kit folder topology sustainability","angle":"D4"}
```

Then one `{"type":"finding",...}` record per finding:

```json
{"type":"finding","id":"f-iter019-001","severity":"P1","label":"<short label>","file":"path/file.ext","line":NN,"iteration":19,"angle":"D4"}
```

The first record's `type` MUST be exactly `"iteration"` (not `"iteration_delta"`). Use `newInfoRatio` between 0 and 1.

### 3. State log append — append the iteration record to `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/003-system-bug-improvement-research/research/deep-research-state.jsonl`
