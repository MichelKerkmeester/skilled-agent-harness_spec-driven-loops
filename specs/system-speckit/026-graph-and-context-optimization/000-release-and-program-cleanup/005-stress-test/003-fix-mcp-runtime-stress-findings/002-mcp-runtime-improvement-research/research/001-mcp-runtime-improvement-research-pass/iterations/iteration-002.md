...
005 records the exact broken response: `memory_context({input:"Semantic Search", mode:"auto"})` returned `meta.tokenBudgetEnforcement.{enforced:true, truncated:true, returnedResultCount:2}` while `data.content[0].text` contained `{"count":0,"results":[]}` (`005-memory-search-runtime-bugs/spec.md:113-114`). The 006 live probes repeated the same shape with `actualTokens:65 / 3500`, `returnedResultCount:3`, and empty payload (`001-search-intelligence-stress-playbook/scratch/live-probes-2026-04-26.md:9-17`).
