...
The packet folder is: `.opencode/specs/system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/020-cocoindex-feature-catalog/` — write ALL packet files there. Do NOT ask for the spec folder.

### Goal

Author a complete `feature_catalog/` for `.opencode/skills/mcp-coco-index/` following the canonical sk-doc shape. The skill currently has SKILL.md, references/, scripts/, tests/, mcp_server/, manual_testing_playbook/, assets/ — but **no feature_catalog/**. This packet creates one.

### Source of truth

- Per-feature shape: `.opencode/skills/sk-doc/assets/documentation/feature_catalog/feature_catalog_snippet_template.md` (4 sections: OVERVIEW / CURRENT REALITY / SOURCE FILES / SOURCE METADATA)
- Root-index shape: `.opencode/skills/sk-doc/assets/documentation/feature_catalog/feature_catalog_template.md` (TOC + OVERVIEW + per-category sections)
- Reference example: `.opencode/skills/sk-improve-agent/feature_catalog/` (cleanly conformant)
- Evergreen rule: `.opencode/skills/sk-doc/references/global/evergreen_packet_id_rule.md` (no packet IDs in evergreen content)
- Standards: `.opencode/skills/sk-doc/references/global/{core_standards.md,hvr_rules.md}`

### Phase 1: Inventory the surface

Read these and build the feature inventory:
- `.opencode/skills/mcp-coco-index/SKILL.md` (smart routing, when to use, activation triggers)
- `.opencode/skills/mcp-coco-index/README.md`
- `.opencode/skills/mcp-coco-index/references/tool_reference.md` (CLI commands + MCP tool spec)
- `.opencode/skills/mcp-coco-index/references/search_patterns.md`
- `.opencode/skills/mcp-coco-index/references/settings_reference.md`
- `.opencode/skills/mcp-coco-index/references/cross_cli_playbook.md`
- `.opencode/skills/mcp-coco-index/references/downstream_adoption_checklist.md`
- `.opencode/skills/mcp-coco-index/scripts/{install.sh,doctor.sh,ensure_ready.sh,update.sh,common.sh}`
- `.opencode/skills/mcp-coco-index/tests/test_*.py` (skim for what behaviors are validated)
- `.opencode/skills/mcp-coco-index/manual_testing_playbook/manual_testing_playbook.md`
- `.opencode/skills/mcp-coco-index/mcp_server/pyproject.toml` (entry points)

Also inspect the upstream patches: SKILL.md mentions REQ-001..006 (mirror dedup + path-class reranking). Find the actual implementation files under `.opencode/skills/mcp-coco-index/mcp_server/` (look for cocoindex_code/ source).
...
For EACH feature, create a file at `.opencode/skills/mcp-coco-index/feature_catalog/<NN>--category/<NN>-feature-slug.md` matching the canonical 4-section shape:
...
At `.opencode/skills/mcp-coco-index/feature_catalog/feature_catalog.md`, author the root index following `feature_catalog_template.md`:
...
bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh \
  .opencode/specs/system-spec-kit/026-graph-and-context-optimization/000-release-cleanup/005-review-remediation/020-cocoindex-feature-catalog --strict
...
grep -rnE '\b\d{3}-[a-z-]+\b|\bpacket [0-9]{3}|\bphase [0-9]{3}|\bfrom packet|\bin packet|\bvia packet' \
  .opencode/skills/mcp-coco-index/feature_catalog/ || true

# Build sanity (no code changes expected; doc-only)
cd .opencode/skills/system-spec-kit/mcp_server && npm run build && cd -
```
...
**Trigger phrases**: `["020-cocoindex-feature-catalog","mcp-coco-index feature catalog","cocoindex catalog","semantic search catalog"]`.
