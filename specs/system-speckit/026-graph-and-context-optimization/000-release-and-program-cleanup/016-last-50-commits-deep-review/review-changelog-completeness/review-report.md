...
- **HEAD:** `81009f395a`
- **Scope (user-chosen):** full range `v3.4.2.0 changelog (820e788e1b) -> HEAD` (258 commits), checking whether `.opencode/changelog/system-spec-kit/v3.5.0.0.md` plus the nested 026 changelog tree captured all work since `v3.4.2.0.md`, accurately.
...
|---|---|---|
| 240 commits in `820e788e1b..270fe50627` | 240 | EXACT |
...
| MCP serverInfo 1.7.2 -> 1.8.0 (`5bafb0c7e3`) | package.json = 1.8.0 | VERIFIED |
| checkpoint schema v29 (`9a6ffaa1a9`) -> v30 (`06bf89895c`) | commits present, match | VERIFIED |
| causal coverage 39.91% -> 43.59%, `caused` 3 -> 103 (`55dadc234a`) | sourced in `changelog-026-relation-backfill-review-remediation.md` (not commit body) | VERIFIED (see UNCERTAIN-1) |
| 694 changelogs centralized (`e06cb0d15a`) vs current 763 | +69 from later backfills/authoring | RECONCILABLE |
| 77 stale paths (`4c1c5ec09f`) + 92 remapped (`2214486ddc`) | commits exist with exact subjects | VERIFIED |
| timeline section D link index non-empty | 695 changelog links | VERIFIED |

**Bottom line on the release changelog itself: clean.** Its window (`820e788e1b..270fe50627`) is internally complete and every metric is honest.
...
- **Evidence:** commit `20e03b8cc1` (25 files: runtime configs, READMEs, feature catalog, playbook). The only matching changelog is `changelog-000-015-docs-drift-review.md`, which is the read-only *review* and states the reviewed docs were left untouched. `20e03b8cc1` is referenced in no changelog body.
...
- **Evidence:** commit `1711954c95` deletes 291 checked-in `.gemini` surface files (+27 config edits). No changelog references `1711954c95`; nested files only mention gemini in passing (doctor/install docs).
...
- **Evidence:** timeline line 878 lists `003-memory-and-causal-runtime/027-launcher-concurrency-spawn-and-bridge-investigation` with `(none)` changelog. Created by `9135941012`.
- **Re-verification:** `9135941012` is a **deep-research** packet. Per the distributed-governance rule, workflow-owned research markdown is changelog-exempt, and the code it produced (`795cfb8a07`) IS covered by `changelog-006-015-socket-server-reconvergence-and-hardening.md`. **Expected, not a defect.**
...
- **Evidence:** `changelog/003-memory-and-causal-runtime/changelog-016-004-fix-investigation-p1s-for-ts-cjs-rerank-twin-parity.md:63` shows `strict validation | TBD | Pending final packet validation`. Commit `4260d91f9d` body records `validate.sh phase 010/002/004 --strict: PASSED (0 errors, 0 warnings)`.
...
- **P2-1 (`v3.5.0.0.md:63`):** "single 605-line change in the checkpoint storage layer" uses total insertions; `8148f5f834` numstat shows the storage file at 413+/25- plus a separate 192-line suite. Wording, not a factual error. (i03, 0.92)
- **P2-2 (`v3.5.0.0.md:124`):** "`5a4fe38a29 / f7acab456b` ... 12 cli-*/sk-* skills" over-compresses scope; `f7acab456b` specifically covers skill-advisor, ai-council, mcp-click-up. Work exists; narrative slightly imprecise. (i04, 0.86)
...
2. Fix the one stale `TBD` cell in `changelog-016-004-*` to `PASSED` per `4260d91f9d` (P1-6).
3. Decide on the post-window commits: either author nested changelogs for `20e03b8cc1` (drift remediation) and `1711954c95` (gemini removal), or fold them into a follow-up release rollup. (P1-1, P1-2)
4. Optional polish: P2-1/P2-2/P2-4/P2-5 wording + Oxford-comma sweep on the 020-026 cohort.

**Release-level judgment (from i01, endorsed):** do not extend v3.5.0.0 — its range is fixed at `820e788e1b..270fe50627`. The ~17 post-window commits belong in a separate follow-up release changelog when one is cut.
