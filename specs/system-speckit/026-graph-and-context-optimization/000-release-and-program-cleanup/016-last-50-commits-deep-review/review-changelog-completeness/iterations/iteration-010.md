...
- '694 live changelogs centralized' (e06cb0d15a) vs current count: find .opencode/specs/system-spec-kit/026-graph-and-context-optimization/changelog -name '*.md' | wc -l  (expect ~763 now; explain the delta if reconcilable: later additions/backfills).
- '12 sidecar leaf changelogs marked later-removed' (270fe50627): grep -rl 'later-removed' .opencode/specs/system-spec-kit/026-graph-and-context-optimization/changelog | wc -l
- '77 stale paths fixed' (4c1c5ec09f) + '92 remapped' (2214486ddc): confirm commits exist with those subjects.
...
The current tree count is 763 markdown changelogs, while the release cites the historical `e06cb0d15a` centralization of 694. I’m checking whether the 69-file delta is explained by later changelog additions/removals in the same bounded tree.
The 694→763 current-count delta is not directly comparable to the quoted “live changelogs” wording because the tree also contains README/rollup files and post-centralization adds/deletes/renames. I’m checking snapshot counts at `e06cb0d15a` and exact sidecar-note counts to distinguish true drift from naming/counting convention differences.
