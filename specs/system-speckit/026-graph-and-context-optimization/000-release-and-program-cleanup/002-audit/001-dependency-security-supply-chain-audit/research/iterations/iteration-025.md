...
| H-ADJ-02 | Live ClickUp + Figma API keys in `.env` | `/Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.env:4-8`: `clickup_CLICKUP_API_KEY=pk_1234567890_...` and `figma_FIGMA_API_KEY=figd_...`. Discovered iter-021 (H-002). | Rotate both keys at providers; move to `.env.local` or secrets manager. |
