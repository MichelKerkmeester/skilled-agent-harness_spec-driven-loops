...
- The Gemini prompt-hook implementation agrees with that config, explicitly describing itself as running on Gemini's "prompt-equivalent BeforeAgent surface" rather than a literal `UserPromptSubmit` event. Evidence: `.opencode/skills/system-spec-kit/mcp_server/hooks/gemini/user-prompt-submit.ts:3`, `.opencode/skills/system-spec-kit/mcp_server/hooks/gemini/user-prompt-submit.ts:5`, `.opencode/skills/system-spec-kit/mcp_server/hooks/gemini/user-prompt-submit.ts:43`, `.opencode/skills/system-spec-kit/mcp_server/hooks/gemini/user-prompt-submit.ts:45`
- The docs do not currently mirror those Gemini names. The runtime matrix still calls the Gemini prompt hook `UserPromptSubmit`, compaction hook `PreCompact`, and stop hook `Stop`, which is now a naming mismatch against the checked-in Gemini surface. Evidence: `.opencode/skills/system-spec-kit/references/config/hook_system.md:26`, `.opencode/skills/system-spec-kit/references/config/hook_system.md:59`, `.opencode/skills/system-spec-kit/references/config/hook_system.md:62`
- The test layer is also behind the checked-in runtime: `runtime-detection.ts` would report Gemini hooks as enabled when `.gemini/settings.json` contains `hooks` or `hooksConfig`, but the fixtures and fallback tests still hard-code Gemini as `unavailable` / `tool_fallback`. Evidence: `.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/runtime-detection.ts:60`, `.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/runtime-detection.ts:69`, `.opencode/skills/system-spec-kit/mcp_server/code-graph/lib/runtime-detection.ts:74`, `.opencode/skills/system-spec-kit/mcp_server/tests/fixtures/runtime-fixtures.ts:58`, `.opencode/skills/system-spec-kit/mcp_server/tests/cross-runtime-fallback.vitest.ts:58`, `.opencode/skills/system-spec-kit/mcp_server/tests/cross-runtime-fallback.vitest.ts:102`
