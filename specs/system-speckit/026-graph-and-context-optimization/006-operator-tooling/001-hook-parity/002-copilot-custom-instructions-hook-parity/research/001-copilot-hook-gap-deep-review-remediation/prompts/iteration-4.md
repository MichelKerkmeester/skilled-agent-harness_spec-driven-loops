---
title: "Iteration 4: <focus-topic>"
trigger_phrases: []
---
You are executing iteration 4 of 10 of a deep-research loop against the Copilot CLI hook parity question. Operate as a LEAF agent — do not dispatch sub-agents.
...
Last 3 iterations:
  run 1: documented injection surfaces (ratio=0.85)
  run 2: custom agents and ACP surface (ratio=0.65)
  run 3: hooks and agent startup surface (ratio=0.6)
...
Write to: /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/research/001-copilot-hook-gap-deep-review-remediation/iterations/iteration-4.md

Structure:
# Iteration 4: <focus-topic>
## Focus
<1-2 sentences>
## Actions Taken
...
## Next Focus
<one-sentence recommendation for iteration 5>

### 2. JSONL iteration record

APPEND (do not overwrite) one line to: /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/research/001-copilot-hook-gap-deep-review-remediation/deep-research-state.jsonl

Record schema (use type "iteration" exactly):
{"type":"iteration","run":4,"status":"insight"|"evidence"|"thought"|"dead-end","focus":"<short focus label>","findingsCount":<int>,"newInfoRatio":<0..1>,"citationCount":<int>,"primarySourceCount":<int>,"timestamp":"<ISO8601>","graphEvents":[]}

newInfoRatio guidance: 1.0 = all findings new; 0.0 = all redundant with prior iterations. Be honest.

### 3. Per-iteration delta file

Write to: /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public/.opencode/specs/system-spec-kit/026-graph-and-context-optimization/research/001-copilot-hook-gap-deep-review-remediation/deltas/iter-4.jsonl
...
Begin iteration 4 now. Produce the three artifacts.
