...
- [x] RT029 Update `.opencode/skills/system-spec-kit/mcp_server/tests/modularization.vitest.ts` so the extended limit for `formatters/search-results.js` is `536` and the adjacent actual-count note is `536`, keeping the modularization guard aligned to the narrowly landed split. Evidence: `npx vitest run tests/modularization.vitest.ts`; `timeout 180 npm run test:core`; `python3 .opencode/skills/sk-code-opencode/scripts/verify_alignment_drift.py --root .opencode/skills/system-spec-kit`. `[Supports: narrow modularization/test-budget slice only; no canonical verdict change]`
...
- [x] RT034 Convert `.opencode/skills/system-spec-kit/mcp_server/scripts/map-ground-truth-ids.ts` into a thin compatibility wrapper that delegates to `../../scripts/dist/evals/map-ground-truth-ids.js`, fixing the wrapper-only architecture-boundary failure for that script surface. Evidence: `npm run check --workspace=scripts` in `.opencode/skills/system-spec-kit`; `python3 .opencode/skills/sk-code-opencode/scripts/verify_alignment_drift.py --root .opencode/skills/system-spec-kit`. `[Supports: narrow wrapper-alignment slice only; no canonical verdict change]`
