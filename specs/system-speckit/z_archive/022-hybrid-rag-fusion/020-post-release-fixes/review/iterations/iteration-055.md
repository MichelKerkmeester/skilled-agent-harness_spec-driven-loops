...
## Evidence
- `feature_catalog/16--tooling-and-scripts/13-constitutional-memory-manager-command.md:37-45` still lists `.opencode/commands/memory/README.txt` as an active implementation surface.
- `scripts/tests/memory-learn-command-docs.vitest.ts:24-38` still reads `.opencode/commands/memory/README.txt` as part of the active-doc alignment regression.
- Fresh targeted run failed: `tests/memory-learn-command-docs.vitest.ts` -> `1 failed, 1 passed` with `ENOENT` on `.opencode/commands/memory/README.txt`, while `.opencode/commands/memory/README.md:1-16` is the live file that exists.
