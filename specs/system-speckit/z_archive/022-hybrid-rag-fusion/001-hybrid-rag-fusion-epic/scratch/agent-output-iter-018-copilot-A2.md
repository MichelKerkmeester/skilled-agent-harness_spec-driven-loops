...
● Run skill advisor for research task (shell)
  │ python3 .opencode/skills/scripts/skill_advisor.py "Deep research iteration 18/20 on
  │ hybrid-rag-fusion system at .opencode/skills/system-spec-kit/mcp_server/lib/search/. Focus:
...
● Search (grep)
  ...
  │ " in *.ts (.opencode/skills/system-spec-kit/mcp_server/lib/search)
  └ 53 files found
...
● Explore Cross-check search subsystem
  ...
  │   │ Env/Public/.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage2-fusion.ts"
  │   └ 4 lines...
...
● Read search-types.ts
  │ .opencode/skills/system-spec-kit/mcp_server/lib/search/search-types.ts
  └ L1:260 (10 lines read)

● Search (grep)
  │ "executePipeline\(|searchWithFallback\(|syncScoreAliasesInPlace\(|applyValidationSignalScoring\(
  │ |resolveRerankOutputScore\(" in **/*.ts (.opencode/skills/system-spec-kit/mcp_server)
  └ 32 lines found
...
● Search (grep)
  │ "collapseAndReassembleChunkResults\(" in **/*.ts (.opencode/skills/system-spec-kit/mcp_server)
  └ 27 lines found

● Search (grep)
  │ "\bhybridSearch\(" in **/*.ts (.opencode/skills/system-spec-kit/mcp_server)
  └ 23 lines found
...
● Search (grep)
  │ "formatSearchResults\(" in **/*.ts (.opencode/skills/system-spec-kit/mcp_server)
  └ 31 lines found
...
- **Stage 1 is not actually “raw candidate generation.”** It calls `searchWithFallback()`/`hybridSearchEnhanced()`, which already does fusion, channel enforcement, confidence truncation, MMR, co-activation, and token-budget truncation, then Stage 2/3 apply co-activation/MMR again. That breaks the Stage 1 contract and double-applies ranking logic. [`handlers/memory-search.ts:929-968`](.opencode/skills/system-spec-kit/mcp_server/handlers/memory-search.ts#L929-L968) [`pipeline/stage1-candidate-gen.ts:179-190`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage1-candidate-gen.ts#L179-L190) [`pipeline/stage1-candidate-gen.ts:257-326`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage1-candidate-gen.ts#L257-L326) [`hybrid-search.ts:732-999`](.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts#L732-L999) [`pipeline/stage2-fusion.ts:619-647`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage2-fusion.ts#L619-L647) [`pipeline/stage3-rerank.ts:159-229`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage3-rerank.ts#L159-L229)

- **Chunk-collapse is split across two incompatible places.** Stage1’s hybrid path collapses chunk hits into bare parent rows `{id, score, source, _chunkHits}`, but Stage3 can only reassemble rows that still carry `parent_id`/`parentId`. Result: collapsed hits can no longer get `precomputedContent`/`file_path`, so `includeContent` can degrade to `"File not found"`. [`hybrid-search.ts:761-792`](.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts#L761-L792) [`pipeline/stage3-rerank.ts:448-460`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage3-rerank.ts#L448-L460) [`pipeline/stage3-rerank.ts:563-607`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage3-rerank.ts#L563-L607) [`formatters/search-results.ts:441-460`](.opencode/skills/system-spec-kit/mcp_server/formatters/search-results.ts#L441-L460)

- **Deep/R12 variant merging can duplicate the same memory.** Stage1 dedups merged variant results with `Set<number>()` on raw `row.id`, but BM25 emits string IDs and RRF preserves the original ID shape from the first contributing item, so `"42"` and `42` can survive as separate rows across variants. [`pipeline/stage1-candidate-gen.ts:292-300`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage1-candidate-gen.ts#L292-L300) [`pipeline/stage1-candidate-gen.ts:378-386`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage1-candidate-gen.ts#L378-L386) [`hybrid-search.ts:279-299`](.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts#L279-L299) [`shared/algorithms/rrf-fusion.ts:248-257`](.opencode/skills/system-spec-kit/shared/algorithms/rrf-fusion.ts#L248-L257)

- **Tier-3 structural fallback ignores `includeArchived`.** `structuralSearch()` filters only on `importance_tier`, not `is_archived`, and Stage1 only reapplies archive filtering to the summary-embedding side channel, not to general fallback results. Degraded searches can therefore surface archived memories even when `includeArchived=false`. [`hybrid-search.ts:1134-1185`](.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts#L1134-L1185) [`hybrid-search.ts:1452-1516`](.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts#L1452-L1516) [`pipeline/stage1-candidate-gen.ts:96-107`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage1-candidate-gen.ts#L96-L107) [`pipeline/stage1-candidate-gen.ts:621-623`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage1-candidate-gen.ts#L621-L623)
