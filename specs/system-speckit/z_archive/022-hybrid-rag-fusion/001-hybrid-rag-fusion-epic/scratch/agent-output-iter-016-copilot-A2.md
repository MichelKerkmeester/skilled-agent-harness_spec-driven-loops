...
● Run skill advisor (shell)
  │ python3 .opencode/skills/scripts/skill_advisor.py "Deep research iteration 16/20 on
  │ hybrid-rag-fusion system at .opencode/skills/system-spec-kit/mcp_server/lib/search/. Focus: Q1
...
● Search (glob)
  │ "**/*.ts" in .opencode/skills/system-spec-kit/mcp_server/lib/search
  └ 56 files found
...
● Explore Explore search architecture
  ...
  │   │
  │   Env/Public/.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/ranking-contract.ts"
  │   └ 11 lines...
...
● Search (grep)
  ...
  │ hybridSearch\(" (.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts)
  └ 5 lines found
...
● Search (grep)
  ...
  │ tage4\(" in **/*.ts (.opencode/skills/system-spec-kit/mcp_server/lib/search)
  └ 17 lines found
...
● Search (grep)
  ...
  │ rch\s*," (.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts)
  └ 3 lines found
...
● Read pipeline-integration.vitest.ts
  │ .opencode/skills/system-spec-kit/mcp_server/tests/pipeline-integration.vitest.ts
  └ L1:90 (90 lines read)
...
● Show numbered export lines (shell)
  ...
  │ Env/Public/.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts' | tail -n 30
  └ 31 lines...
...
● Show export block lines (shell)
  │ nl -ba '/Users/michelkerkmeester/MEGA/Development/Opencode
  │ Env/Public/.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts' | sed -n
  │ '1700,1738p'
  └ 40 lines...

Q1 synthesis: the live pipeline is `executePipeline()` → Stage 1 candidate generation → Stage 2 fusion/scoring → Stage 3 rerank/MPAB aggregation → Stage 4 filtering/annotation. Stage 1 can also inject constitutional rows before later stages treat them uniformly. [`pipeline/orchestrator.ts:42-77`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/orchestrator.ts), [`pipeline/stage1-candidate-gen.ts:192-217`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage1-candidate-gen.ts), [`pipeline/stage2-fusion.ts:555-579`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage2-fusion.ts), [`pipeline/stage3-rerank.ts:127-156`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage3-rerank.ts), [`pipeline/stage4-filter.ts:243-345`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage4-filter.ts)

Findings:

- `compareDeterministicRows()` still hardcodes its own score fallback order (`score → intentAdjustedScore → rrfScore`) while the canonical resolver is `intentAdjustedScore → rrfScore → score`, clamped. That means sorting can diverge from the shared scoring contract used elsewhere. [`pipeline/ranking-contract.ts:32-58`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/ranking-contract.ts), [`pipeline/types.ts:48-68`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/types.ts)

- Stage 4 TRM/evidence-gap checks use `rrfScore` before final `score`, but Stage 3 reranking overwrites `score` and preserves prior Stage 2 state separately. So post-rerank confidence analysis can run on stale pre-rerank scores. [`pipeline/stage4-filter.ts:203-216`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage4-filter.ts), [`pipeline/stage4-filter.ts:269-284`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage4-filter.ts), [`pipeline/stage3-rerank.ts:373-380`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage3-rerank.ts)

- Degraded hybrid fallback is semantically inconsistent: Stage 1 can catch hybrid failure and substitute pure vector candidates, but Stage 2 still decides behavior from `config.searchType === 'hybrid'`, so fallback-vector results still get hybrid-only treatment and skip non-hybrid intent weighting. [`pipeline/stage1-candidate-gen.ts:409-441`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage1-candidate-gen.ts), [`pipeline/stage2-fusion.ts:578-617`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage2-fusion.ts), [`pipeline/stage2-fusion.ts:713-730`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage2-fusion.ts)

- The Stage 4 immutability check is weaker than its comments imply: it snapshots all rows, but `verifyScoreInvariant()` skips any row filtered out later, so score mutations on rows removed during filtering would go undetected. [`pipeline/stage4-filter.ts:247-253`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage4-filter.ts), [`pipeline/stage4-filter.ts:306-312`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/stage4-filter.ts), [`pipeline/types.ts:384-397`](.opencode/skills/system-spec-kit/mcp_server/lib/search/pipeline/types.ts)

- I did **not** confirm true dead code in the core path. The deprecated naive `hybridSearch()` is still exported, so it’s legacy surface area rather than unreachable code. [`hybrid-search.ts:413-419`](.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts), [`hybrid-search.ts:1083-1121`](.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts), [`hybrid-search.ts:1712-1721`](.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts)
