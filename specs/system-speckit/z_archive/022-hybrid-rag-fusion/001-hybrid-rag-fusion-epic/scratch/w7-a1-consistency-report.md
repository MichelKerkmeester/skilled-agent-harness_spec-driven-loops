...
Audit scope: `001`, `002`, `005`, `006`, `007`, `009`, `010`, and `012` under `.opencode/specs/system-spec-kit/022-hybrid-rag-fusion/`.
...
| `006-ux-hooks-automation` | `in-progress` | `in-progress` | Yes | Yes | warn |
| `008-architecture-audit` | `complete` | `complete` | Yes | Yes | fail |
| `009-spec-descriptions` | `in-progress` | `in-progress` | Yes | Yes | warn |
...
- `006-ux-hooks-automation`: frontmatter says `status: in-progress`, but the metadata table says `Status: Implemented` (`spec.md:3`, `spec.md:32`).
- `009-spec-descriptions`: frontmatter says `status: in-progress`, but the metadata table says `Status: Ready` (`spec.md:3`, `spec.md:31`).
...
- `006-extra-features`: `| **Parent Spec** | ../spec.md |` is broken because there is no `spec.md` at the `022-hybrid-rag-fusion/` root (`spec.md:404-408`). The predecessor note also says historical continuity was folded from retired `008-architecture-audit`, but `008-architecture-audit` still exists as an active complete folder (`spec.md:407-408`).
- `006-ux-hooks-automation`: no explicit backlink to the current epic spec (`001-hybrid-rag-fusion-epic`) was found in the canonical `spec.md`/`checklist.md` pair.
- `008-architecture-audit`: `| **Parent Spec** | ../spec.md |` is broken because there is no `spec.md` at the `022-hybrid-rag-fusion/` root (`spec.md:320-323`).
- `009-spec-descriptions`: no explicit backlink to the current epic spec (`001-hybrid-rag-fusion-epic`) was found in the canonical `spec.md`/`checklist.md` pair.
...
- **006-ux-hooks-automation**: warn — status vocabulary drifts from frontmatter and no clean epic backlink is present.
- **008-architecture-audit**: fail — explicit parent-spec link is broken.
- **009-spec-descriptions**: warn — status vocabulary drifts from frontmatter and no clean epic backlink is present.
