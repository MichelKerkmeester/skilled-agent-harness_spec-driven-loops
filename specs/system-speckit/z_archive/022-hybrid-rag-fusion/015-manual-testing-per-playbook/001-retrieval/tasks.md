...
| T021 | 185 | /memory:search command routing | DONE | PASS | `.opencode/commands/memory/search.md` documents the no-args prompt, routing gates, analysis quick reference, and retrieval-plus-analysis tool surface for the renamed command. |
| T022 | 187 | Quick search (memory_quick_search) | DONE | PASS | .opencode/skills/system-spec-kit/mcp_server/tools/memory-tools.ts:47-65 (delegates to handleMemorySearch with defaults), schemas/tool-input-schemas.ts:389-485 (schema + allowed params), tests/memory-tools.vitest.ts:41-64 (governed-scope forwarding) |
<!-- /ANCHOR:phase-2 -->
