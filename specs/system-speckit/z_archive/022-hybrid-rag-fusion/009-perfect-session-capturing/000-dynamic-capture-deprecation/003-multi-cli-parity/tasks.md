...
_memory:
  continuity:
    packet_pointer: "system-speckit/z_archive/022-hybrid-rag-fusion/009-perfect-session-capturing/000-dynamic-capture-deprecation/003-multi-cli-parity"
...
- [x] T004 Add a direct `View -> Research` regression in `.opencode/skills/system-spec-kit/scripts/tests/phase-classification.vitest.ts`.
- [x] T005 Create `.opencode/skills/system-spec-kit/scripts/tests/content-filter-parity.vitest.ts` for Copilot lifecycle noise, Codex reasoning markers, and empty XML wrappers.
- [x] T006 Extend `.opencode/skills/system-spec-kit/scripts/tests/runtime-memory-inputs.vitest.ts` to prove `Read ...` titles and `_provenance: 'tool'` on CLI-derived `FILES`.
...
- [x] T009 Run `.opencode/skills/system-spec-kit/scripts/tests/test-extractors-loaders.js` and record the final totals.
- [x] T010 Run `npm run typecheck` and `npm run build` from `.opencode/skills/system-spec-kit`.
- [x] T011 Run `.opencode/skills/system-spec-kit/scripts/spec/validate.sh` on the phase-016 folder and reconcile the remaining template/evidence findings.
...
- **Parity tests**: `.opencode/skills/system-spec-kit/scripts/tests/phase-classification.vitest.ts`, `.opencode/skills/system-spec-kit/scripts/tests/content-filter-parity.vitest.ts`, `.opencode/skills/system-spec-kit/scripts/tests/runtime-memory-inputs.vitest.ts`
<!-- /ANCHOR:cross-refs -->
