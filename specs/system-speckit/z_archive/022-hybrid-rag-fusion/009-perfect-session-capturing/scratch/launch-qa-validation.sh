...
2. Find 3 spec folders with existing memory/ subdirectories (that have real session data):
   ls -d $PROJECT_ROOT/.opencode/specs/system-spec-kit/022-hybrid-rag-fusion/*/
...
4. $PROJECT_ROOT/.opencode/specs/system-spec-kit/022-hybrid-rag-fusion/manual_testing_playbook/manual_testing_playbook.md (first 50 lines for table format)
...
2. $PROJECT_ROOT/.opencode/specs/system-spec-kit/022-hybrid-rag-fusion/feature_catalog/feature_catalog.md (first 130 lines for format/structure)
3. $SCRIPTS_DIR/core/config.ts (configurable values)
