...
● List directory .opencode/skills/system-spec-kit/shared
  └ 20 files found
...
● Search (glob)
  │ ".opencode/skills/system-spec-kit/shared/parsing/*.ts"
  └ 6 files found

● Search (glob)
  │ ".opencode/skills/system-spec-kit/shared/utils/*.ts"
  └ 4 files found
...
● Search (grep)
  │ "\bany\b" in **/*.ts (.opencode/skills/system-spec-kit/shared)
  └ 6 lines found

● Search (grep)
  │ "export\s+\*|export\s+type\s+\*" in **/*.ts (.opencode/skills/system-spec-kit/shared)
  └ 4 lines found
...
● Search (glob)
  │ ".opencode/skills/system-spec-kit/shared/lib/*.ts"
  └ 1 file found
...
● Read memory-template-contract.ts
  │ .opencode/skills/system-spec-kit/shared/parsing/memory-template-contract.ts
  └ 388 lines read
...
● Read spec-doc-health.test.ts
  │ .opencode/skills/system-spec-kit/shared/parsing/spec-doc-health.test.ts
  └ 223 lines read
...
● Read jsonc-strip.ts
  │ .opencode/skills/system-spec-kit/shared/utils/jsonc-strip.ts
  └ 95 lines read

● Read path-security.ts
  │ .opencode/skills/system-spec-kit/shared/utils/path-security.ts
  └ 107 lines read

● Read token-estimate.ts
  │ .opencode/skills/system-spec-kit/shared/utils/token-estimate.ts
  └ 14 lines read
...
● Search (grep)
  ...
  │ **/*.ts (.opencode/skills/system-spec-kit/shared)
  └ No matches found
...
● Search (grep)
  │ "catch \([^)]*\) \{\s*// skip" in **/*.ts (.opencode/skills/system-spec-kit/shared)
  └ No matches found

● Search (grep)
  │ "export\s*\{[\s\S]*\}" (.opencode/skills/system-spec-kit/shared/embeddings.ts)
  └ 45 lines found

● Search (grep)
  │ "catch \{" in **/*.ts (.opencode/skills/system-spec-kit/shared)
  └ 6 lines found
...
● Search (grep)
  │ "graphCausalBias|graphWeight" in **/*.ts (.opencode/skills/system-spec-kit/shared)
  └ 16 lines found

● Search (grep)
  │ "interface IEmbeddingProvider|getProfile\(|getProviderMetadata\(|getEmbeddingProfile\(|getEmbedd
  │ ingProfileAsync\(" in **/*.ts (.opencode/skills/system-spec-kit/shared)
  └ 12 lines found
...
● Search (grep)
  │ "semanticChunk\(" in **/*.{ts,js} (.opencode/skills/system-spec-kit)
  └ 4 lines found
...
1. `.opencode/skills/system-spec-kit/shared/embeddings/factory.ts:191-223,233-262` — `createEmbeddingsProvider()` silently swaps a failed cloud provider for `hf-local`, even when the embedding dimension changes.
...
2. `.opencode/skills/system-spec-kit/shared/parsing/spec-doc-health.ts:78-80,116-119,136-139,159-162,207-209,259-261` — the health evaluator swallows filesystem exceptions and continues as if checks were simply skipped.
...
1. `.opencode/skills/system-spec-kit/shared/algorithms/adaptive-fusion.ts:17-28,60-76,135-176,191-237,344-419` — the public adaptive-fusion API advertises `graphWeight` and `graphCausalBias`, but the actual fusion entrypoints only accept semantic and keyword result lists.
...
2. `.opencode/skills/system-spec-kit/shared/types.ts:22-57`, `.opencode/skills/system-spec-kit/shared/embeddings.ts:708-726`, and provider `getProfile()` implementations at `embeddings/providers/hf-local.ts:285-291`, `openai.ts:285-292`, `voyage.ts:302-309` — embedding profile/provider typing is broader than it needs to be.
...
3. `.opencode/skills/system-spec-kit/shared/scoring/folder-scoring.ts:23,204-218,251-265,312-317` with `.opencode/skills/system-spec-kit/shared/types.ts:307-325` — folder scoring intentionally accepts `Partial<Memory> & Record<string, unknown>` and the canonical `FolderScore` output has an open-ended `[key: string]: unknown` index signature.
...
4. `.opencode/skills/system-spec-kit/shared/index.ts:168`, `.opencode/skills/system-spec-kit/shared/algorithms/index.ts:5-7`, `.opencode/skills/system-spec-kit/shared/embeddings.ts:752-796` — export hygiene is looser than the rest of the shared layer.
...
5. `.opencode/skills/system-spec-kit/shared/chunking.ts:15-27,37-110` vs `.opencode/skills/system-spec-kit/shared/lib/structure-aware-chunker.ts:10-24,37-40,119-220` — these are not direct duplicates, but they are overlapping chunking APIs with separate heuristics and constants.
