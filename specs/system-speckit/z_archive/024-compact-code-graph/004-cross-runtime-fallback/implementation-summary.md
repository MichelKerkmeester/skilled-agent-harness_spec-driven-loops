...
_memory:
  continuity:
    packet_pointer: "system-speckit/z_archive/024-compact-code-graph/004-cross-runtime-fallback"
...
**`AGENTS.md`** (root): Added equivalent compaction recovery instructions using the same two primitives (`memory_match_triggers` + `memory_context(resume)`) for tool-based recovery without hooks.
...
| `CLAUDE.md` | New | Claude-specific hook-aware recovery instructions |
| `AGENTS.md` | Modified | Added compaction recovery instructions for Codex CLI |
...
| Separate `CLAUDE.md` over root-only | Claude Code loads both root and `.claude/` instructions. The private file handles Claude-specific hook awareness without cluttering the universal root file. |
...
| `CLAUDE.md` created | Verified — hook-aware recovery, SessionStart payload handling |
