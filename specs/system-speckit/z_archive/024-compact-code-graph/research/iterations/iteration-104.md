...
OpenCode custom agents are defined as Markdown files under `.opencode/agents/*.md`, and the repo documents them as the canonical runtime source. They are invoked either by automatic routing rules or manually via `@agent_name` syntax. [SOURCE: .opencode/README.md:94-116] [SOURCE: .opencode/README.md:302-330]

The actual OpenCode agent schema is frontmatter-driven. In this repo, agents use fields like `name`, `description`, `mode`, `temperature`, `permission`, and optional `mcpServers`; `@context` is read-only and mounts `spec_kit_memory` plus `cocoindex_code`. [SOURCE: .opencode/agents/context.md:1-23] `@orchestrate` routes all exploration through `@context` and treats it as the exclusive retrieval leaf. [SOURCE: .opencode/agents/orchestrate.md:87-100]

`@context` already performs most of the desired priming behavior conceptually: memory-first retrieval, then `code_graph_status()` once per session before structural exploration. [SOURCE: .opencode/agents/context.md:43-53] But it is a broad exploration agent, not a startup/bootstrap specialist.

No documented OpenCode session-start hook or auto-agent invocation surface exists in `opencode.json` or `.opencode/README.md`; the config exposes permissions and MCP servers only. [SOURCE: opencode.json:1-61] By contrast, Claude has a real `SessionStart` hook (`session-prime.ts`) that injects startup guidance and checks code-graph freshness. [SOURCE: .opencode/skills/system-spec-kit/mcp_server/hooks/claude/session-prime.ts:84-120]

### Problem

OpenCode has strong custom-agent infrastructure, but no documented zero-touch session-priming mechanism. That means context recovery currently depends on either the user/LLM manually invoking `@context` or `/spec_kit:resume`, or on the model remembering first-turn instructions like `memory_context(...resume...)` and `code_graph_status()`. [SOURCE: .opencode/commands/spec_kit/resume.md:1-17]

## Proposals

### Proposal A: @context-prime Agent with Manual Invocation

- Description: Create a new `.opencode/agents/context-prime.md` as a narrow bootstrap agent. Its job would be: `memory_context({ input: "resume previous work continue session", mode: "resume", profile: "resume" })`, `code_graph_status({})`, optionally `ccc_status()`, then return a short "Prime Package" with active spec folder, current task, blockers, next steps, and retrieval-health status. This works with the existing custom-agent system because OpenCode already supports new agent files plus manual `@agent_name` invocation.
- LOC estimate: 60-140
- Files to change: `.opencode/agents/context-prime.md`, `.opencode/README.md`, optionally `.opencode/agents/orchestrate.md`
...
- LOC estimate: 25-80
- Files to change: `.opencode/agents/context.md`, `.opencode/agents/orchestrate.md`, `.opencode/README.md`
...
- LOC estimate: 50-160
- Files to change: new `.opencode/commands/.../prime.md` or extend `.opencode/commands/spec_kit/resume.md`, plus `.opencode/README.md`
...
- LOC estimate: 20-70
- Files to change: `.opencode/README.md`, `.opencode/agents/orchestrate.md`, `CLAUDE.md`, `GEMINI.md`, `CODEX.md` (and `AGENTS.md` only if that file is an active runtime instruction entrypoint)
...
1. Add `.opencode/agents/context-prime.md` with a narrow "bootstrap only" contract.
...
4. Update `.opencode/agents/orchestrate.md` and `.opencode/README.md` to use `@context-prime` on first turn / after `/clear`.
