...
5. The exact data flow from the memory database to final context output has two parallel branches:
   ...
   In Branch A, constitutional rows come directly from `memory_index` via SQL in `getConstitutionalMemories()`, while trigger matches come from a cached loader query over `memory_index` in `trigger-matcher.ts`. In Branch B, the search pipeline formats result rows into a `memory_search` envelope, then the resume profile rewrites that inner envelope into `{ state, nextSteps, blockers, topResult }`, and only after that does `handleMemoryContext()` wrap the nested search envelope in the outer `memory_context` response. Recovery payloads from `recovery-payload.ts` can appear on the search side when results are empty or weak, but they are not part of `autoSurfaceAtCompaction()` itself. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:90-124`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:195-223`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/parsing/trigger-matcher.ts:176-195`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/parsing/trigger-matcher.ts:337-440`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/parsing/trigger-matcher.ts:621-692`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-context.ts:688-717`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-context.ts:1215-1280`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-search.ts:784-835`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-search.ts:1066-1084`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/formatters/search-results.ts:394-427`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/formatters/search-results.ts:593-680`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/response/profile-formatters.ts:356-370`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/response/profile-formatters.ts:447-498`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/search/recovery-payload.ts:179-224`]

6. `api/` is a stable script-facing surface, but it does not currently expose the compaction hook helpers. The package export map only exposes `"." -> ./dist/context-server.js`, `"./api"`, and `"./api/*"`. `api/README.md` further says external consumers should prefer `@spec-kit/mcp-server/api` or `@spec-kit/mcp-server/api/<module>` and avoid direct imports from `lib/`, `handlers/`, or `core/` unless there is a deliberate exception. Because there is no `./hooks` export, a hook script that wants `autoSurfaceAtCompaction()` cannot get it through the public package API today; it must either import a relative compiled file under `dist/hooks/` or fall back to an MCP tool call. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/package.json:6-13`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/api/README.md:22-24`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/api/README.md:49-53`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/api/index.ts:11-117`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/index.ts:4-18`] [SOURCE: `.opencode/specs/system-spec-kit/024-compact-code-graph/decision-record.md:27-36`]
...
7. The exact import-path guidance splits into two valid buckets:
   ...
   Importing `@spec-kit/mcp-server` itself is not the right hook-helper surface because that resolves to `dist/context-server.js`, which is the stdio MCP server entrypoint. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/package.json:6-18`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/api/README.md:51-53`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/dist/hooks/index.d.ts:1-4`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/dist/hooks/memory-surface.d.ts:20-65`] [SOURCE: `.opencode/specs/system-spec-kit/024-compact-code-graph/001-precompact-hook/spec.md:35-48`] [SOURCE: `.opencode/specs/system-spec-kit/024-compact-code-graph/002-session-start-hook/spec.md:29-53`]
...
10. The dependency graph between the live modules is:
    ...
    The important architectural consequence is that the compaction hook helper and the nested resume retrieval payload only meet again at `context-server.ts`, where the outer response envelope is finalized. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/context-server.ts:20-117`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/index.ts:4-18`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/tools/index.ts:15-35`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/tools/context-tools.ts:10-17`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-context.ts:15-39`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-search.ts:784-835`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/formatters/search-results.ts:394-680`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/search/hybrid-search.ts:1-44`] 

11. The main error boundaries are already layered and mostly non-fatal:
    - `getConstitutionalMemories()` catches DB/SQL failures and returns `[]` with a warning. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:97-124`]
    - `trigger-matcher` cache load failures degrade to `[]` and store degraded-state metadata instead of throwing. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/parsing/trigger-matcher.ts:346-440`]
    - `autoSurfaceMemories()` catches surfacing failures and returns `null`. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:224-228`]
    - `context-server.ts` treats auto-surface failures as non-fatal and continues the tool call. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/context-server.ts:334-355`]
    - `handleMemoryContext()` has hard envelopes for database-state failure (`E_INTERNAL`), untrusted session scope (`E_SESSION_SCOPE`), and strategy execution failure (`E_STRATEGY`). [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-context.ts:984-997`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-context.ts:1041-1059`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-context.ts:1163-1185`]
    - `appendAutoSurfaceHints()` swallows parse/shape failures and leaves the original response untouched. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/response-hints.ts:59-110`]
    - `enforceTokenBudget()` and `enforceAutoSurfaceTokenBudget()` truncate or drop payloads rather than emitting malformed JSON. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:136-186`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-context.ts:286-543`]
    - Planned external hook scripts add additional failure points not yet implemented in code: stdin parse, transcript-tail read, cache write/read/delete, and stdout contamination. The stdout rule is especially strict because MCP transport must keep stdout clean; any diagnostics belong on stderr. [SOURCE: `.opencode/specs/system-spec-kit/024-compact-code-graph/001-precompact-hook/spec.md:43-49`] [SOURCE: `.opencode/specs/system-spec-kit/024-compact-code-graph/001-precompact-hook/spec.md:58-66`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/utils/logger.ts:4-6`] [SOURCE: `.opencode/skills/system-spec-kit/references/config/environment_variables.md:401-403`]
...
Key proof points: `.opencode/skills/system-spec-kit/mcp_server/context-server.ts:325-399`, `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:188-317`
...
Key proof points: `.opencode/skills/system-spec-kit/mcp_server/package.json:6-13`, `.opencode/skills/system-spec-kit/mcp_server/api/README.md:49-53`, `.opencode/skills/system-spec-kit/mcp_server/dist/hooks/index.d.ts:1-4`
...
1. Add a small public `api/hooks.ts` export if hook scripts are expected to import compaction helpers as supported runtime consumers. Right now the packet wants direct import, but the package export map does not make that a first-class contract. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/package.json:6-13`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/api/README.md:49-53`]

2. Treat `autoSurfaceAtCompaction()` as a retrieval primitive only. Build a separate hook-side renderer that converts `AutoSurfaceResult` into compact human text for stdout and cache storage, instead of assuming the hook helper already returns a ready-to-inject brief. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:26-36`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/hooks/memory-surface.ts:300-317`]
...
5. Document the two-layer output model in the implementation notes: compaction auto-surface lives in outer envelope metadata, while the resume brief lives inside the nested `memory_search` envelope. That distinction matters for both hook rendering and debugging. [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/context-server.ts:393-399`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-context.ts:1247-1280`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/response/profile-formatters.ts:356-370`] [SOURCE: `.opencode/skills/system-spec-kit/mcp_server/lib/response/profile-formatters.ts:447-498`]
