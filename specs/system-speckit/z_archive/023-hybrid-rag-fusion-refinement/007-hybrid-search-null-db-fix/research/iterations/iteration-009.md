...
3. The exact orphan query returns 8 rows in `memory_index` that are not present in `active_memory_projection`, but they are all historical superseded versions, not active search losses. SQL against `.opencode/skills/system-spec-kit/mcp_server/database/context-index.sqlite` returned:
