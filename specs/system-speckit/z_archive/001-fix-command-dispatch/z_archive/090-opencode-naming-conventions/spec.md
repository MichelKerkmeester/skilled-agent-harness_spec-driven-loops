---
title: "Feature Specific [system-spec-kit/z_archive/001-fix-command-dispatch/z_archive/090-opencode-naming-conventions/spec]"
...
Align JavaScript naming conventions in the OpenCode framework with ecosystem standards (camelCase) instead of the current non-standard snake_case enforcement. Migrate all ~206 JS files in `.opencode/skills/system-spec-kit/` and update the `sk-code-opencode` skill documentation.
...
### Problem Statement
The `sk-code-opencode` skill enforces `snake_case` for JavaScript functions, parameters, and exports. This contradicts JS ecosystem standards (MDN, Airbnb, Node.js all use camelCase). All ~206 JS files in `.opencode/skills/system-spec-kit/` follow snake_case, creating friction and inconsistency.
...
- Part B: Migrate ~206 JS files in `.opencode/skills/system-spec-kit/` from snake_case to camelCase
...
|-----------|-------------|-------------|
| `.opencode/skills/sk-code-opencode/` (9 files) | Modify | Update naming convention docs |
| `.opencode/skills/system-spec-kit/**/*.js` (~206 files) | Modify | Rename snake_case to camelCase |
