---
title: "Iteration 37 (Round S): SA3-decay-importance"
trigger_phrases: []
---
# Iteration 37 (Round S): SA3-decay-importance

> **Reconstructed file.** Child 006 consolidated its per-iteration findings directly into `../research.md` during the original sprint (a documented deviation). This file was regenerated from `../deep-research-state.jsonl` + `../research.md` to restore structural parity with siblings 001–005. **Authoritative record: `../research.md`** plus this iteration’s row in `../deep-research-state.jsonl`. Prose is condensed (the original per-seat narrative was not separately retained); claims are the ledger one-liners.

## Focus
`SA3-decay-importance` — round S, read-only claude2 Opus seat. (newInfoRatio 0.3, findingsCount 1, status insight, ~300s)

## Findings
- **SA3** — surfaced (`upsert_finding`): Decay-importance: aionforge importance is an author-assigned anchor + time-decay, not learned; Spec-Kit FSRS is already richer (adds centrality).

## Verdict / disposition
NO-TRANSFER

## Full detail
See `../research.md` → Deflations. Key questions: PC. Answered: PC.
