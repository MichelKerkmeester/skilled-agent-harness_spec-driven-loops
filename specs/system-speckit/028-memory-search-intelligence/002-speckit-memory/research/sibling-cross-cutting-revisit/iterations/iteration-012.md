---
title: "Iteration 12 (Round P): PD4-namespace-auth"
trigger_phrases: []
---
# Iteration 12 (Round P): PD4-namespace-auth
...
## Focus
`PD4-namespace-auth` — round P, read-only claude2 Opus seat. (newInfoRatio 0.6, findingsCount 1, status insight, ~300s)

## Findings
- **PD4** — surfaced (`upsert_finding`): Namespace-authorization: recall scoping is opt-in not enforced, but cross-namespace recall is a feature in a single-tenant store — only an opt-in strict-isolation mode is worth flagging.
