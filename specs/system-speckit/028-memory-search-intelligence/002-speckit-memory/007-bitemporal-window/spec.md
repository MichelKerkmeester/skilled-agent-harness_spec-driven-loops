...
_memory:
  continuity:
    packet_pointer: "system-speckit/028-memory-search-intelligence/002-speckit-memory/007-bitemporal-window"
...
| **Parent Spec** | ../spec.md |
| **Predecessor** | ../006-redteam-probe-gate/spec.md |
| **Successor** | ../008-search-index-integrity-sweep/spec.md |
<!-- /ANCHOR:metadata -->
...
|-----------|-------------|-------------|
| `.opencode/skills/system-spec-kit/mcp_server/lib/graph/temporal-edges.ts` | Modify | `invalidateEdge()` accepts/derives an event-time close timestamp from lineage instead of `now()`, and adds the four-timestamp columns (`valid_from`/`valid_to`/`ingested_at`/`expired_at`) additively. |
