...
_memory:
  continuity:
    packet_pointer: "system-speckit/028-memory-search-intelligence/003-spec-data-quality/004-novel-research/007-novel-per-doc-quality-slas"
...
| **Parent Spec** | ../spec.md |
| **Predecessor** | ../006-novel-freshness-decay-queue/spec.md |
<!-- /ANCHOR:metadata -->
...
|------|------|--------|------------|
| Dependency | A host queue (freshness decay auto-refresh queue OR B3 `refinement_queue` from `013-retrieval-feedback-edge`) | Hard blocker. The SLA files a ticket and has nowhere to file until one queue exists. | Build this phase only after a host queue ships. The evaluator degrades to a no-op until then per REQ-006. |
