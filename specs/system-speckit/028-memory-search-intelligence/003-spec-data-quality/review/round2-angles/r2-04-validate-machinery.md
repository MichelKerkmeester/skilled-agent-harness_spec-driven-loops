...
A4 spec.md §3 Files-to-Change for `validate.sh` says delete `detect_legacy_grandfathered` (175-183), its call site (1044), and "the `LEGACY_GRANDFATHERED` read in the strict RESULT branch (line 927)" (`004-schema-warn-to-error/spec.md:97`). The live tree reads the flag at `validate.sh:912, 927, 935, 1062` plus init at `:41`, function at `:175-183`, call at `:1044`. The scope names only `927` and omits `912, 935, 1062`. `validate.sh` runs `set -euo pipefail` (`validate.sh:8`), so a surviving `! $LEGACY_GRANDFATHERED` after the init line is deleted throws an unbound-variable error and crashes every strict run that carries warnings. REQ-003's "grep returns zero matches" acceptance is the real backstop and would catch this, but the FROZEN scope table is materially incomplete and a literal build introduces a P0 crash. Separately confirmed: A4's "0 grandfathered packets" census premise holds. A scan of every `graph-metadata.json` under `.opencode/specs` found zero with `legacy_grandfathered === true` (the lone key hit is A4's own `causal_summary` prose in `004-a4.../graph-metadata.json:111`), so the bypass removal itself is safe on blast radius. The defect is the seam enumeration, not the census.
