...
- [x] T001 Trace the mislabeled rollup rows (009/011/017/018/020) to their commits and child summaries. Traced to `a00803e724`, `478bebf387`, `508af65780` and each child implementation-summary. 008 confirmed no-code per its child.
- [x] T002 Enumerate the commits omitted by `timeline.md` and `before-vs-after.md`. Full `git log main..HEAD` enumerated. The omitted window runs `8b07538178` through `62394395fb`.
- [x] T003 Confirm phase 001 has applied its `benchmark-status.md` re-run update. Confirmed committed-clean at `62394395fb` (criterion-4 re-derived).
<!-- /ANCHOR:phase-1 -->
...
- [x] T006 [P] Advance `before-vs-after.md` CURRENT STATE and correct release-cleanup claims. CURRENT STATE advanced to `bd66487db2`. Section 6 release-cleanup corrected from PENDING to executed. Benchmark present-tense and measured-benefit framing reconciled to the criterion-4 run.
