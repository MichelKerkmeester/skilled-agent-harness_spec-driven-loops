...
- `ipc-client-cap-fanout-stress.vitest.ts` — added 2026-06-11 (`8eb78e87d1`)
- `shard-repair-persistence-stress.vitest.ts` — added 2026-06-11 (`8eb78e87d1`)
- `metadata-edge-promoter-stress.vitest.ts` — added 2026-06-12 (`4f5237d4fb`)
- `release-cleanup-new-surfaces-stress.vitest.ts` — added 2026-06-10 (`5d3982197c`)

The README's own git history (`git log -- .../durability/README.md`) shows its most recent edit is `490da351a1`, dated 2026-06-19, titled **"docs+fix(028): align stale code READMEs + strip pre-existing artifact-ID comments"** — a commit whose explicit purpose was to fix stale READMEs, landing eight days after the last of the four missing files existed. All four files pre-date that alignment pass; none were added by it. This is the strongest finding in the audit: a dedicated stale-doc-remediation commit still left over a third of the domain's real test files undocumented.
...
This describes the harness as it existed before `.opencode/specs/system-speckit/028-memory-search-intelligence/001-speckit-memory/029-substrate-sandbox-cleanup` shipped (COMPLETE, 2026-06-23, commit `b547368ffd`, and re-touched `2026-07-04`). That packet added, and I independently confirmed present in the current source (`mcp_server/stress_test/substrate/run-substrate-stress-harness.mjs:13,105,115-116,793-805,887`):
...
None of this is reflected in the README's "SCENARIO BOUNDARY" section, which is the exact section that should describe it. `git log` confirms the README's last touch is `9ac8b0b045` (2026-06-01) — three weeks before the cleanup feature shipped — so this is a clean pre/post gap, not a misread. Separately, the same README's §2 SCOPE table (lines 17-24) documents 5 of the substrate directory's 7 real files; missing are `idempotency-receipt-race-stress.vitest.ts` and `secret-scrub-save-flood-stress.vitest.ts` (both added 2026-06-11, also after the README's last touch).
...
**CONFIRMED.** `SKILL.md:5` declares `version: 3.7.1.0`, which is an exact match for the newest file in `.opencode/skills/system-spec-kit/changelog/` (`v3.7.1.0.md`). `git log` on `SKILL.md` shows edits as recently as 2026-07-05 (`95e3152d32`) and 2026-07-04 (`ef5e347261`, `e8b36a3a66`, `f064e40582`) — same-week maintenance activity.
