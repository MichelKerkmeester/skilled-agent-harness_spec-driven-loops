...
- [x] T005 If Option B is chosen, open the follow-on execution packet under Gate 3, seeded with this phase's blast-radius table and `runtime/hooks/claude/session-stop.ts:73-76`'s explicit inclusion (REQ-004) — **done, with a scope amendment the operator approved directly**: rather than a separately Gate-3'd packet, Stage B executed inside `specs/system-speckit/054-decommission-debt-fixes/002-scripts-into-runtime-nesting/` (the scripts -> runtime/cli move already in flight there), since both renames touch the same tree and the same `session-stop.ts` fallback candidates. `scratch/code-path-followups.md` served as the seed; see that packet's `implementation-summary.md` for the executed change and evidence.
