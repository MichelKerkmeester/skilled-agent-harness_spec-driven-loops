...
| V-C12 | P1 | eleven remaining private repo-root walk-ups in scripts outside the consolidation | Reported | Fixed: the resolver now lives in `shared/workspace/repo-root.mjs` (hooks re-export it), and every CommonJS entry point requires it through the shared package export; the shell script already used `git rev-parse` |
| V-C13 | P2 | `shared/predicates/boolean-expr.ts`, `runtime/lib/search/folder-discovery.ts` catches | Unconsumed module; uncommented catches | The predicate module is a public shared export with its own test and stays; each discovery catch now states why it stays quiet; `auto-select` gained a test; the JavaScript style guide now documents the divider header the tree actually uses |
