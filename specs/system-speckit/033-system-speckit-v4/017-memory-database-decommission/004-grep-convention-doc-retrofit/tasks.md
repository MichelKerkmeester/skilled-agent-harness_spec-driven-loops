...
- [x] T004 Write the convention document, with `spec.md` sections 13 through 15 as its normative source (`.opencode/skills/system-spec-kit/references/structure/grep-convention.md`) (done: committed at `c39d613f23`, registered in the skill `SKILL.md` and README, sk-doc validator 0 issues; the seven points it surfaced are resolved in `spec.md` section 13.7)
...
- [x] CHK-011 [P0] No console errors or warnings — the 14 track runs wrote 10,210 documents with 0 failures and no unhandled diagnostic; the post-commit drift-marker hook did overflow the environment limit on the 10k-file commit and was fixed to stream the diff at `cdfa247b14`
...
- [x] CHK-FIX-007 [P1] Evidence is pinned to a fix SHA or explicit diff range, not a moving branch-relative range. — the convention is `c39d613f23`, the tooling `265d9a206f` and `cdfa247b14`, and the corpus passes `2f58acfb6e`, `f138cd5487`, `396c0f3f0e` and `2896c6ac68`
<!-- /ANCHOR:fix-completeness -->
