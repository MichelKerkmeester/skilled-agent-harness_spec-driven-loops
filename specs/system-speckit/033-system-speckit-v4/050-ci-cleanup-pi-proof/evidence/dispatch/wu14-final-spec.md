...
EDIT 2 (INSERT two rows directly after this ANCHOR line)
ANCHOR: | `recursive-child-manifest.vitest.ts` | Modify | Spec-kit CLI tests. Two hardcoded .opencode/specs paths are corrected to the tracked specs/ tree |
NEW:
...
VERIFY - run these, paste each command with its result line
  grep -c 'In Progress' specs/system-speckit/033-system-speckit-v4/050-ci-cleanup-pi-proof/spec.md   # expect 0
  grep -c '.github/workflows/' specs/system-speckit/033-system-speckit-v4/050-ci-cleanup-pi-proof/spec.md   # expect 2
