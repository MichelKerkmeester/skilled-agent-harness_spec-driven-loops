...
[![Discord](https://img.shields.io/discord/1423675330749399203?style=for-the-badge&logo=discord&logoColor=white&label=Discord&color=7289da)](https://discord.gg/wJQfZKNfVH)
...
AI coding assistants have amnesia. Every session starts from zero.

You explain your architecture Monday. By Wednesday, it is gone. Decisions, trade-offs, the carefully reasoned choices behind them, all lost the moment the conversation window closes.

And memory is only the first casualty:

- **Changes ship undocumented.** Nothing records why a decision was made, so the next session - or the next teammate - has to reverse-engineer it.
- **"Done" is whatever the agent claims.** No gate stands between a confident answer and a verified one.
- **Every task starts cold.** The assistant reloads the same context, asks the same questions and repeats the same mistakes.
- **One model does every job.** The same weights plan the work, write the code and review their own output.
...
- A validator checks the record before the work can be called complete
- Every packet is plain markdown in your repo, so the history is yours to read, diff and keep

Powered by the **Spec Kit Framework**.

* * *

### "Done" only when verified

Completion is gated. Validation rules, acceptance criteria and fresh evidence decide, not the agent's say-so.

- Mandatory gates run before file changes and after execution
- Reviews rank findings P0/P1/P2 and re-challenge critical findings before they stick
- An open blocker forces another pass instead of a quiet sign-off

Powered by the gate system, `validate.sh` and the review loops.
...
Powered by **Autonomous Deep Loops**.
...
Powered by the **Agent Network** and **cross-CLI orchestration**.
...
**`/speckit:complete`** - the headline command
...
`/deep:research` investigates a question for you, one focused pass at a time, until the answers hold up.
...
- Anchored to a real `spec.md`, with `new`/`resume`/`restart` lifecycle. Modes: `:auto`, `:confirm`

* * *

### Audit code until it is clean

`/deep:review` audits your code in passes and never edits it.
...
`@ai-council` runs the seats, and `/deep:ai-council` handles multi-topic sessions.
...
`@context` maps the existing codebase before you plan, so you extend what's already there instead of rewriting it. Use it for one-shot lookup and continuity recovery. Use `/deep:research` or `/deep:review` when iterative work needs a bounded context snapshot.
...
How the loops run under the hood: [Inside the Deep Loops](#inside-the-deep-loops).
...
- See [A goal that survives resets](#a-goal-that-survives-resets) above for the full contract
...
### Skills that load when you need them
...
Two skills power the autonomous loops described in [Inside the Deep Loops](#inside-the-deep-loops):
...
These skills let you run **cross-CLI agent teams from supported runtimes**.
...
- Dispatches distinct reasoning lenses across cli-opencode, cli-claude-code and native for multi-round deliberation. Planning-only. See [Inside the Deep Loops](#inside-the-deep-loops)

**Deep Research** - one research iteration at a time, state on disk

- Executes a single LEAF pass. The `/deep:research` command owns the loop. See [Inside the Deep Loops](#inside-the-deep-loops)

**Deep Review** - audits one review pass, read-only on code

- Produces `file:line` findings. The `/deep:review` command owns the loop. See [Inside the Deep Loops](#inside-the-deep-loops)

**Deep Improvement** - proposes one agent improvement, safely

- Writes a single candidate to packet-local runtime and never scores or promotes it. The `/deep:agent-improvement` command handles that. See [Inside the Deep Loops](#inside-the-deep-loops)

---

## 7. WORKS WHERE YOU WORK

* * *

### OpenCode and Claude Code, plus five more

OpenCode and Claude Code are the primary runtimes. Codex, Cursor, Devin, Pi and Hermes join through their own CLI bridges, each one availability-gated and fails closed.

- Every bridge is a one-shot non-interactive call that streams structured output back to the caller
- The conducting AI stays in charge. The dispatched CLI handles the part it is best at and returns
- Per-bridge model rosters and probe commands live in the [Cross-AI CLI](#cross-ai-cli) toolkit entries

* * *

### External tools without the context cost

`code_mode` gives the AI access to external tools (Figma, GitHub, Chrome DevTools, ClickUp, Webflow) through a single TypeScript execution interface.
...
The full tool inventory lives under [Inside Code Mode MCP](#inside-code-mode-mcp).

* * *

### Parallel sessions that stay in sync
...
Everything below is reference material. You never need it to use the framework, but it is here when you want the mechanics.
...
- **`scaffold`**, written when the packet is created - `spec.md`, `plan.md`, `tasks.md`, `implementation-summary.md` (after implementation starts), `acceptance-criteria.md` (Level 2 and above). Lives in `core/` and `addons/`
- **`--with-lazy-addons`** on `create.sh` - `before-after.md`, `timeline.md`, `roadmap.md`, `decision-record.md`. Lives in `addons/`
- **`--with-goal`** on `create.sh` - `goal.md`. Lives in `addons/`
- **The inline gate renderer**, run by hand - `resource-map.md`, or any add-on for an existing packet. Lives in `addons/`
- **Command or agent owned** - `handover.md` (memory save), `debug-delegation.md` (`@debug`), `research.md` (`/deep:research`). Lives in `addons/`
- **Packet type**, not a level - `phase-parent.spec.md`, `review.spec.md`, `research.spec.md`, `review-report.md`. Lives in `packet-types/`

Beyond the four numbered levels, the contract defines three packet types with their own required documents:

- **`phase`** - a phase parent, needs only `spec.md`
- **`review`** - needs `spec.md` plus `review/review-report.md`
- **`research`** - needs `spec.md` plus `research/research.md`
...
### How memory and search work

Spec memory and retrieval are packet-local and file-based.
...
It works with OpenCode and Claude Code, and the same rules reach Codex, Cursor, Devin, Pi and Hermes through their own adapters.

- OpenCode uses plugin surfaces
- Claude Code uses hook adapters
- Hermes runs the repo's guards through the `.hermes/plugins/repo-guards` project plugin
...
- The `code_mode` server registers seven tools, listed under [Inside Code Mode MCP](#inside-code-mode-mcp)
