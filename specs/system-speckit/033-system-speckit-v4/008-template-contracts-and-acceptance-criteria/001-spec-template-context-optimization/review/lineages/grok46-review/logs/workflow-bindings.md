---
trigger_phrases: []
---
PRE-BOUND SETUP ANSWERS:
review_target: specs/system-speckit/034-spec-template-context-optimizations
review_target_type: spec-folder
review_dimensions: all
spec_folder: specs/system-speckit/034-spec-template-context-optimizations
execution_mode: AUTONOMOUS
lineage_mode: auto
session_id: fanout-grok46-review-1786566874028-sks2q2
maxIterations: 5
convergenceThreshold: 0.1
stop_policy: max-iterations
config.fanout_lineage_artifact_dir: specs/system-speckit/034-spec-template-context-optimizations/review/lineages/grok46-review

BINDING: review_target=specs/system-speckit/034-spec-template-context-optimizations
BINDING: review_target_type=spec-folder
BINDING: review_dimensions=all
BINDING: spec_folder=specs/system-speckit/034-spec-template-context-optimizations
BINDING: execution_mode=AUTONOMOUS
BINDING: lineage_mode=auto
BINDING: session_id=fanout-grok46-review-1786566874028-sks2q2
BINDING: executor=cli-cursor model=cursor-grok-4.6-xhigh
BINDING: loop_type=review
BINDING: config.stopPolicy=max-iterations
BINDING: config.maxIterations=5
BINDING: config.convergenceThreshold=0.1
BINDING: artifact_dir=specs/system-speckit/034-spec-template-context-optimizations/review/lineages/grok46-review
BINDING: artifact_dir_source=config.fanout_lineage_artifact_dir override
BINDING: resolveArtifactRoot=skipped per fanout_lineage_artifact_dir override
BINDING: resource_map_present=false
BINDING: goal_file_manifest=absent
