...
WHY: the spec gate no longer exempts /tmp as a location. It exempts paths outside the project, so a
fixture project may live anywhere, /tmp included. The step still says the core treats /tmp as
exempt scratch space.

EDIT 1 (OLD occurs exactly once: step 3 of the scenario)
OLD: 3. Build a disposable, non-exempt fixture project (deliberately NOT under `/tmp` or `/private/tmp`, which the core always treats as exempt scratch space -- `mktemp -d` with no path argument resolves to `$TMPDIR`, e.g. `/var/folders/.../T/...` on macOS):
NEW: 3. Build a disposable fixture project. Any location works, `/tmp` included, because the core exempts only paths outside the project, never the project's own location. `mktemp -d` with no path argument resolves to `$TMPDIR`, e.g. `/var/folders/.../T/...` on macOS:

VERIFY - run these, paste each command with its result line
  grep -c 'always treats as exempt' .skilled/skills/system-spec-kit/manual-testing-playbook/plugins-and-hooks/spec-mutation-gate-enforce.md   # expect 0
