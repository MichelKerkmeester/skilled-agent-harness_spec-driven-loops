...
STEP 1 - fill specs/system-speckit/033-system-speckit-v4/054-ci-cleanup-follow-ups/implementation-summary.md
Shape model (read-only): specs/system-speckit/033-system-speckit-v4/050-ci-cleanup-pi-proof/implementation-summary.md
- Title in frontmatter and H1: "Implementation Summary: CI Cleanup Follow-ups".
...
- Metadata: Spec Folder 054-ci-cleanup-follow-ups. Completed 2026-09-23. Level 2.
...
- Known Limitations: the observation in evidence.md about unbuilt dists in a fresh worktree. Then one line naming the two out-of-scope items and where they are recorded.
Accept when: only specs/system-speckit/033-system-speckit-v4/054-ci-cleanup-follow-ups/implementation-summary.md changed and every rule above holds.


VERIFY - run these three, paste each command with its result line
  bash .skilled/skills/system-spec-kit/runtime/cli/spec/check-placeholders.sh specs/system-speckit/033-system-speckit-v4/054-ci-cleanup-follow-ups --verbose 2>&1 | grep "implementation-summary.md" ; echo "placeholder_lines=$?"   # placeholder_lines=1 (no lines) is the pass
  grep -c '<!-- /\?ANCHOR:' specs/system-speckit/033-system-speckit-v4/054-ci-cleanup-follow-ups/implementation-summary.md      # must equal 12
  grep -c -e '—' -e ';' specs/system-speckit/033-system-speckit-v4/054-ci-cleanup-follow-ups/implementation-summary.md          # report the count; 0 is the goal outside code spans
