...
description: "Move every tracked file under .opencode into .skilled through one verified commit of exact renames that also adds the .opencode link phase 004 chose, with followable history, without pushing or touching the main checkout."
trigger_phrases:
...
- C2, one commit that moves all 16 tracked top-level entries as exact renames and adds the `.opencode -> .skilled` link, as phase 004's cutover step 11 has it. The operator chose this shape on 2026-09-17 over a separate link commit.
- C0, only if no earlier phase added them: `.skilled/` twins of the 56 `.gitignore` rules anchored at `.opencode/`. Not needed, because phase 006's twins already cover every ignored entry.
...
- Moving the 16 tracked top-level entries of `.opencode/`, one `git mv` each: `skills/` (17,185 tracked files), `hooks/` (179), `commands/` (162), `bin/` (98), `changelog/` (49), `plugins/` (41), `scripts/` (30), `agents/` (13), `install-guides/` (9), `logs/` (1), the files `package.json`, `package-lock.json`, `bun.lock` and `vitest.config.bin.ts`, plus the links `manual-testing-playbook` and `specs`. That is 17,773 tracked files in all at `048d16d725` (`git ls-files .opencode | wc -l`), six more than at authoring because phase 006 added test files. The parent's scope list names ten of these entries (`../spec.md:80`). The other six move by default and 004's keep-list can hold them back.
- Accounting for the 18 ignored entries under `.opencode/` in this worktree at the move: eight `node_modules/` trees, four `dist/` trees, two SQLite files, one `.state` file, one `.node-version-marker`, the test sandboxes and a Python bytecode cache (`git ls-files --others --ignored --exclude-standard --directory .opencode`). The authoring census found 12.
- Creating the `.opencode` link 004 chose, in the rename commit.
...
| `.opencode/**` (17,773 tracked files) | Move | C2, to `.skilled/**` at the same relative paths |
| `.opencode` | Create | C2, the link to `.skilled` that 004 chose |
| `.gitignore` | Modify (conditional, not needed) | C0, `.skilled/` twins of the 56 rules anchored at `.opencode/`. Phase 006's twins already cover every ignored entry |
...
| REQ-002 | Move every tracked top-level entry of `.opencode/`, except entries 004 keeps in place, into `.skilled/` in one commit whose every rename line is `R100` and whose only other line adds the `.opencode` link. |
...
| REQ-004 | Leave `.opencode` resolvable in 004's shape through the link, committed together with the renames. |
...
- **SC-001**: `git show -M --name-status --format= <C2>` prints 17,773 `R100` lines and one `A` line for the `.opencode` link.
...
| Risk | Broad staging sweeps untracked fan-out containment snapshots under `specs/` into C2 | High | `git mv` stages the renames, other paths are staged by name and a name-status gate runs before every commit. The runbook's `git add -A` (`.opencode/skills/sk-git/references/large-reorg-playbook.md:76`) is not used |
...
- Duplicate content: 10 files under `.opencode/` are the empty blob, which is also the placeholder's blob. 114 blob ids are shared by more than one path (17,773 entries, 17,173 distinct blobs at the move). Git can pair a rename with a same-content file at another path, so the path map is the authority and `--follow` samples use unique-content files.
- Modes: 199 executable files and 208 symlinks keep their modes, which the path map compares.

### Error Scenarios
- A `git mv` fails: the entry stays where it was. Read `git status`, then fix the cause or roll back to R1.
- A hook blocks C1 or C2: halt and record the gate with its output. The fix goes to 005 or 006, never through a bypass.
...
- Renames staged without the link: the gates would not find `.opencode`, so C2 commits only when the staged set holds the 17,773 renames and the link's `A` line together.
- `skilled/v4.0.0.0` moved after the move: re-run the scripted move on the new base rather than rebasing 17,773 renames.
<!-- /ANCHOR:edge-cases -->
...
|-----------|-------|-------|
| Scope | 20/25 | 17,773 tracked files in 16 top-level entries, two commits, 18 ignored entries |
...
- Does a directory-level `git mv` carry the ignored trees inside it here? Answered: yes. 17 of the 18 ignored entries travelled inside their entry, and only the root `node_modules` stayed behind (T030). The repository's caveat that `git mv` leaves ignored files behind (`.opencode/skills/sk-git/references/worktree-workflows.md:561-567`) did not hold for this move: every destination was new, and git renamed each source directory on disk as one unit.
- Will the main checkout's hook bodies carry 005's dual-root changes when this phase runs? Answered: yes. The main checkout's `pre-commit` names `skilled` 16 times (T014), against zero at authoring.
- Where must opencode's plugin dependencies live after the move? Answered: phase 003 found that plugins importing packages load through the whole-directory link, so under option A `.opencode/node_modules` relocated to `.skilled/node_modules`.
- What does git do with a dangling hook under `core.hooksPath`? Answered by phase 003: it skips the hook silently, for commits and pushes alike, so phase 010 reinstalls the hooks before the main checkout takes the move.
<!-- /ANCHOR:questions -->
