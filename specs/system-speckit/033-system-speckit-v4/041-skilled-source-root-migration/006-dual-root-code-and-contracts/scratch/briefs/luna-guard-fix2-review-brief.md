...
CONTRACT UNDER REVIEW. The no-spec-import guard `.opencode/bin/check-no-spec-imports.cjs` counts a file toward its scan only when the file is not allowlisted, exits 2 when no such file exists, lists every violation it found, and exits 2 when it could not read a file it scanned, even beside a violation. It exits 1 only when it read every scanned file and found a violation, and 0 when it read every scanned file and found none.

TASK. Review commit `dc9fcbb6e9`, which answers F6 and F7 of `scratch/briefs/luna-guard-compiler-fix-review-return.md`. Read `git show dc9fcbb6e9`, the guard and `.opencode/bin/compiled-routing-foundation.vitest.ts` whole at that commit, and the three calls in `.github/workflows/runtime-no-spec-import.yml`. Look for: an input where the exit code differs from the contract; a workflow call whose result changes; a test row that would pass without the change; and comment text that is inaccurate or names a spec path, packet number or task id. Report a defect only with the concrete inputs that reproduce it.
