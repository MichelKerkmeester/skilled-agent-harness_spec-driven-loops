...
TASK (unit t065j-ledger-workspace-root-test): Read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/006-dual-root-code-and-contracts/scratch/briefs/t065j-ledger-workspace-root-test-payload.md`. It names each new file and its CONTENT block; a block is the text between its two fence lines, without the fence lines. Create each file with its CONTENT block exactly, byte for byte, keeping every space, quote and backslash. The block is already exact, so pass it to the write tool as it stands rather than re-deriving or re-checking it in your reasoning. Only these files may change: `.opencode/skills/system-skill-advisor/runtime/tests/parity/capture-ledger-workspace-root.vitest.ts`.

RETURN: one line per file, `file <n>: created` or `file <n>: not created, <reason>`. Nothing else.
