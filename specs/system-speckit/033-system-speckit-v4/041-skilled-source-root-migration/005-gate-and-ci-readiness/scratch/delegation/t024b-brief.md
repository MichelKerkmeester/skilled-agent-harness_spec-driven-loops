...
TASK (unit t024b): Read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/005-gate-and-ci-readiness/scratch/delegation/t024b-payload.md`. It names one new file and its CONTENT block; a block is the text between its two fence lines, without the fence lines. Create that file with the CONTENT block exactly, byte for byte, keeping every space, quote and backslash, and ending with a single newline. The block is already exact, so pass it to the write tool as it stands rather than re-deriving or re-checking it in your reasoning. Only these files may change: `.opencode/bin/tests/check-git-hooks.test.sh`.

RETURN: one line, `file 1: created` or `file 1: not created, <reason>`. Nothing else.
