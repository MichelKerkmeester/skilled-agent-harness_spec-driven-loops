...
TASK (unit luna-fix2-check): Read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/005-gate-and-ci-readiness/scratch/delegation/luna-fix2-check-payload.md`. It names one file and its CONTENT block; a block is the text between its two fence lines, without the fence lines. Write the file with its CONTENT block exactly, replacing the whole of its current content, byte for byte, keeping every space, quote and backslash. The block is already exact, so pass it to the write tool as it stands rather than re-deriving or re-checking it in your reasoning. Only these files may change: `.github/scripts/check-gate-inputs.sh`.
