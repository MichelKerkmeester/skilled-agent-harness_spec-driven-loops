...
- [B] T012 Review T009 to T011, and fix or answer every finding [GPT-5.6, then Orchestrator]
...
- [x] T014 H01 and H02: kill switch and comment hygiene (`pre-commit:17-28`, `:45-54`) [DeepSeek]
- [x] T015 H03 and H06: agent mirror-sync checker and mirror parity scripts (`pre-commit:90-106`, `:167-191`) [DeepSeek]
- [x] T016 H07 and H08: card-sync and mutation-class triggers and guards (`pre-commit:199-234`) [DeepSeek]
- [x] T017 H09: route re-mint pathspecs, modules, runtime root and `--skill-root` (`pre-commit:253-410`) [DeepSeek]
- [x] T018 H10: spec re-mint tool lookup (`pre-commit:503`) [DeepSeek]
- [x] T019 H13, H14 and H16: skill detector, skill-gate path and routing-byte twins (`.opencode/scripts/git-hooks/pre-push:121`, `:208-231`, `:280-287`) [DeepSeek]
- [x] T020 H11, H12 and H15: mass-deletion library, permission script and route guard (`pre-push:34-69`, `:136-139`, `:250-268`) [DeepSeek]
- [x] T021 H17: allocator lookup and warning (`.opencode/scripts/git-hooks/prepare-commit-msg:39-50`) [DeepSeek]
...
- [B] T025 Review the rule diffs from T014 to T016, T018 and T020 to T024, and fix or answer every finding [GPT-5.6, then Orchestrator]
...
- [x] T027 [P] C01: `.skilled/` twins for the 56 `paths:` entries, eight briefs (`chart-corpus.yml`, `diagram-corpus.yml`, `markdown-link-integrity.yml`, `repo-rules-corpus.yml`, `routing-registry-drift.yml`, `runtime-no-spec-import.yml`, `skill-doc-frontmatter.yml`, `spec-kit-check.yml`) [DeepSeek]
...
- [B] T031 Review T030, and fix or answer every finding [GPT-5.6, then Orchestrator]
...
- [B] T034 Review T033, and fix or answer every finding [GPT-5.6, then Orchestrator]

### Naming guard, a move keeps its name
- [x] T045 Write the three REQ-012 cases in the guard's pytest suite and watch the first fail against today's guard (`.opencode/skills/sk-doc/scripts/tests/test_no_new_snake_case_guard.py`) [DeepSeek]
- [x] T046 Skip the destination basename of an `R` or `C` record when it equals the source basename, still checking every other destination component (`.opencode/skills/sk-doc/shared/scripts/check_no_new_snake_case.py`) [DeepSeek]
- [B] T047 Review T045 and T046, and fix or answer every finding [GPT-5.6, then Orchestrator]
...
- [x] T035 Run every hook test script and compare with the T003 baseline: every case passes and none was removed [Orchestrator]
- [x] T036 Run `bash -n` on every changed shell file, and run every new harness under `/bin/bash` [Orchestrator]
- [x] T037 Parse every workflow with `ruby -ryaml`, then run `bash .github/scripts/check-gate-inputs.sh` and its test, each ending in `RESULT: PASSED` [Orchestrator]
- [x] T038 Run `bash .github/scripts/tests/broken-move-drill.sh`: `RESULT: PASSED`, with the pre-change control showing exit 0 and no gate output [Orchestrator]
...
- [B] T042 Push the tip that carries this phase and read each workflow run's conclusion, `gate-inputs.yml` included [Orchestrator]
...
- [x] CHK-010 [P0] `bash -n` passes for every changed hook, library and script
- [x] CHK-011 [P0] Every shell change runs under `/bin/bash` 3.2.57
- [x] CHK-012 [P1] `check-comment-hygiene.sh` exits 0 or 2 for every changed file
- [x] CHK-013 [P1] The naming guard passes with `--changed-since` the phase base, so every new file name is kebab-case
<!-- /ANCHOR:code-quality -->
...
- [x] CHK-020 [P0] The six existing hook test scripts pass at or above 126 cases with none removed
- [x] CHK-021 [P0] Each new test case was seen failing against the unchanged file before its change
...
- [x] CHK-FIX-001 [P0] Every change row carries its class: the silent skip is `class-of-bug`, a gate that looks its script up at one root, and every instance appears in plan §3.
...
- [x] CHK-FIX-003 [P0] Consumer inventory for the missing-script rule complete: `git grep -l '_in_toolchain_repo()'` lists exactly the eight gate files that call a script, the seven hooks less `commit-msg` plus the legacy helper and the SessionStart check.
- [x] CHK-FIX-004 [P0] The rule's adversarial cases are exercised: a dangling `.opencode` link, a space in the repository path, a repository with no sentinel under either root and the linked layout.
- [x] CHK-FIX-005 [P1] Matrix axes and row count listed in `goal.md` before completion is claimed: layout by script state by identity.
- [x] CHK-FIX-006 [P1] Hostile environment variant executed: the hook test scripts pass when the caller has `GIT_DIR`, `GIT_INDEX_FILE` and a global `core.hooksPath` set.
- [x] CHK-FIX-007 [P1] Evidence pinned to the unit commit SHAs logged in `goal.md`, not to a moving branch range.
<!-- /ANCHOR:fix-completeness -->
...
- [x] CHK-030 [P0] No gate gains a new bypass variable and no bypass name changes
- [x] CHK-031 [P0] The check only reads, and no gate change writes outside the paths the gate already wrote
- [x] CHK-032 [P1] `gate-inputs.yml` declares `contents: read` and no other permission
<!-- /ANCHOR:security -->
...
- [x] CHK-040 [P1] Spec, plan, tasks and acceptance criteria agree on counts and cited lines
- [x] CHK-041 [P1] The git-hooks README, its tests README and the workflows README describe the missing-script rule, the new cases and `gate-inputs.yml`
- [x] CHK-042 [P1] The phase 006 handoff list in `implementation-summary.md` names each script the gates call whose own root literal remains
<!-- /ANCHOR:docs -->
...
- [x] CHK-050 [P1] Disposable clones and drill output live in temporary directories outside the repository
- [x] CHK-051 [P1] `scratch/` holds only the delegation record in `scratch/delegation/` before completion
<!-- /ANCHOR:file-org -->
