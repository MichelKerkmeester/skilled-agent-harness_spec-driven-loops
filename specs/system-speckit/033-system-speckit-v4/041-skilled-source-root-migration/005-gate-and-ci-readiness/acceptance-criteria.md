...
|-------|-----|---------------------|--------------|--------|--------|
| AC-001 | REQ-001 | Given a disposable clone with `.opencode/` renamed to `.skilled/` and each gate script replaced by a stub that records its own path, When each hook runs with one staged trigger per gate, Then every recorded path is under `.skilled/` | `bash .github/scripts/tests/broken-move-drill.sh` prints a PASS line per gate in its moved-and-whole section, and `bash .opencode/scripts/git-hooks/tests/pre-commit.test.sh` passes its `.skilled/` fixture cases | Unmet | - |
...
| AC-005 | REQ-005 | Given every job in the 19 existing workflows, When a guard or run path is missing under the chosen root, Then the step fails with `::error::` rather than exiting 0, and every executable path goes through `$SOURCE_ROOT` | `grep -c 'source-root.sh export'` summed over the 19 existing workflows prints 21, and the check reports no `workflow-inputs` or `escaped-roots` failure. None of the five converted workflows keeps an `exit 0` after a missing-guard test | Unmet | - |
