...
| Independent check | Kept, with five rules: gate-files, hook-inputs (every literal `$REPO_ROOT/.opencode/<path>` or `${REPO_ROOT}/.opencode/<path>` in a gate file resolves, and so does every variable assigned a literal `.opencode/<path>`), workflow-inputs (every literal executable `.opencode/<path>` in a workflow resolves, a glob needs a match, the directory that installs or builds a `node_modules` or `dist` path must exist, and `echo` or `printf` text is not an input), filter-twins (a twin sits in the same workflow filter, dependabot update entry, hook array or hook command) and parser-miss. The block-copies and escaped-roots rules go with the block. The rules in parentheses beyond the first sentence came from the GPT-5.6 Luna review of 2026-09-17 |
| Agent mirror checker path pattern | Added on 2026-09-17 by the operator. The checker's pattern admits `.skilled/agents/`, with Vitest cases that run it against an in-sync and a drifted mirror (T050 to T052). Without it every agent edit under `.skilled` reaches the checker and is dropped with "nothing verified" |
