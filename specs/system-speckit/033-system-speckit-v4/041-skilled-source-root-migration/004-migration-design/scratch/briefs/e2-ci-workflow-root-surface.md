...
Read-only. For each file in `.github/workflows/` that names `.opencode`, list its `paths:` lines, guard path lines and any line that exits 0 when a file is missing. One markdown table: workflow, line number, literal text, kind. No commentary.
