...
Read-only. List every line of `.gitignore` in the repository root that names `.opencode`, with its line number and whether it is a comment, an ignore rule or a negation. One markdown table. No commentary.
