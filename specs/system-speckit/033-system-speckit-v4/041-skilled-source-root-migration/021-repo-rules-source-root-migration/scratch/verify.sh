...
expect() { # expect <name> <expected-substring> <command...>
  ...
  local out
  out="$("$@" 2>&1)"
  # A here-string rather than a pipe: under pipefail a large capture makes the
  # writer die of SIGPIPE the moment grep matches, which reads as a failed check.
...
expect 'agent mirror sync' 'all mirrors in sync' node .skilled/skills/system-deep-loop/deep-improvement/scripts/check-agent-mirror-sync.cjs --all
...
# Two exceptions are planned writing, not drift: the new packet itself, the
# parent packet's phase map, which every appended child phase registers in, and
# the compiled-routing activation manifests, which the commit gate re-mints and
# stages whenever a skill document changes. Those manifests are asserted on
# their own row below.
frozen="$(git diff --diff-filter=MD "$BASE" HEAD -- specs \
  ...
  ':!specs/**/activation/**/manifest.json' \
...
# The activation manifests are derived from the routing inputs, and a skill-doc
# edit invalidates the hub's pin. The commit gate re-mints and stages them, so
# this row asserts that every manifest changed since base differs by its policy
# hash rather than by hand.
minted="$(git diff --name-only "$BASE" HEAD -- ':(glob)**/activation/**/manifest.json')"
if [ -z "$minted" ]; then
  pass 'derived re-mints' 'no activation manifest changed since base'
elif git diff "$BASE" HEAD -- ':(glob)**/activation/**/manifest.json' | grep -q 'effectivePolicyHash'; then
  pass 'derived re-mints' "$(printf '%s' "$minted" | wc -l | tr -d ' ') manifest(s) re-minted by the commit gate"
else
  fail 'derived re-mints' 'a manifest changed without a policy hash'
fi
...
# The check is only worth its verdict if it fails on the drift it exists for: a
# missing public entry is invisible to a reader who never opens the farm, so the
# mutated fixture proves the check names the uncovered rule rather than passing
# on whatever happens to be there.
fixture="$(mktemp -d)"
mkdir -p "$fixture/.skilled/repo-rules" "$fixture/repo-rules"
cp .skilled/repo-rules/*.md "$fixture/.skilled/repo-rules/"
for file in "$fixture"/.skilled/repo-rules/*.md; do
  ln -s "../.skilled/repo-rules/$(basename "$file")" "$fixture/repo-rules/$(basename "$file")"
done
rm "$fixture/repo-rules/blast-radius.md"
farm_out="$(node "$PKT/scratch/check-farm.cjs" --root "$fixture" 2>&1)" || true
if grep -Fq 'blast-radius.md: no farm entry reaches it' <<<"$farm_out"; then
  pass 'farm gap detected' 'a deleted public entry is named'
else
  fail 'farm gap detected' 'the missing entry was not reported'
fi
rm -rf "$fixture"
