...
This unit covers runtime instruction files that every agent session loads. Rewrite a path to this repository's own tree, such as a script or skill path. Keep text that states a runtime's own directory, such as the directory OpenCode reads its agents from.

TASK (unit manual-agents-md-01): read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/009-reference-rewrite/scratch/briefs/manual-agents-md-01-payload.md`. It lists 20 rows.
