...
This unit covers the READMEs of the git hooks. By the gate readiness decision each hook locates its scripts through literal `.opencode/` paths on purpose, because that path keeps resolving through the link in linked worktrees on older branches, and hook pathspecs name both roots. Keep `.opencode` where the text quotes a path a hook actually invokes, sources or tests for (open the hook under `.skilled/scripts/git-hooks/` or `.skilled/hooks/git/` to check). Rewrite where the text names where the hook sources live in this repository.

TASK (unit manual-git-hook-readmes-01): read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/009-reference-rewrite/scratch/briefs/manual-git-hook-readmes-01-payload.md`. It lists 25 rows.
