...
This unit covers files that already name both `.opencode` and `.skilled`. Most were written to support both roots, so read each file's intent before deciding. A plain path constant or command that names this repository's real tree is still a `rewrite`.

TASK (unit dual-03): read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/009-reference-rewrite/scratch/briefs/dual-03-payload.md`. It lists 128 rows.
