...
TASK (unit restore-projection): Read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/009-reference-rewrite/scratch/briefs/restore-projection-payload.md`. It lists numbered edits. Each names a file, an OLD block and a NEW block; a block is the text between its two fence lines, without the fence lines. Apply the edits in order: in the named file, replace the OLD block with the NEW block exactly, byte for byte, keeping every space, quote and backslash. The blocks are already exact, so pass each one to the edit tool as it stands rather than re-deriving or re-checking it in your reasoning. Only these files may change: `.skilled/skills/system-skill-advisor/runtime/lib/scorer/projection.ts`.
