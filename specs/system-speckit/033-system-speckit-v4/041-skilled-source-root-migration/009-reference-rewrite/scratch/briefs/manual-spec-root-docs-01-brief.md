...
This unit covers documents about spec-root resolution: the canonical `specs/` root and the legacy `.opencode/specs` alias, which stays as a compatibility link. Keep references to the alias and to legacy-root behavior. Rewrite references to this repository's tree, such as scripts, skills and commands.

TASK (unit manual-spec-root-docs-01): read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/009-reference-rewrite/scratch/briefs/manual-spec-root-docs-01-payload.md`. It lists 28 rows.
