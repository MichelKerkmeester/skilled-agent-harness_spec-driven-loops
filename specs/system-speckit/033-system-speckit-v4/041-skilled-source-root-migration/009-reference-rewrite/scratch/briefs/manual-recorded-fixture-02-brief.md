...
This unit covers test fixtures. For each fixture, find the test that reads it by searching for its file name. Rewrite when the fixture supplies a path into this repository's tree that the test resolves, builds or compares against current output. Keep when the fixture records a past capture compared as recorded, or when the path is sample text whose handling does not depend on the root name. Name the reading test in the reason.

TASK (unit manual-recorded-fixture-02): read `specs/system-speckit/033-system-speckit-v4/041-skilled-source-root-migration/009-reference-rewrite/scratch/briefs/manual-recorded-fixture-02-payload.md`. It lists 39 rows.
