...
_memory:
  continuity:
    ...
    blockers: []
    key_files:
      - "specs/system-speckit/033-system-speckit-v4/017-memory-database-decommission/goal.md"
    session_dedup:
...
| D3 | Every reference, README or asset created or changed since 576c37b72e conforms to the sk-create-skill template for its class, checked by that skill's own validators, not by eye |
...
**Read before working:** `specs/system-speckit/033-system-speckit-v4/017-memory-database-decommission/goal.md` and its phase goals; the sk-create-skill template contract under `.opencode/skills/sk-doc/sk-create-skill`; the 049 review report under `049-memory-decommission/review/lineages/luna-max`.
...
- [x] Every document created or changed since 576c37b72e under references/, README files and command assets passes the sk-create-skill validators for its class with zero errors
...
| Item | State | Evidence |
|------|-------|----------|
| Merge v4 into the branch | Done | `6b06d6c892`: 34 conflicts, 12 deleted-engine files kept deleted, command frontmatter takes v4's contract hints with memory tools removed; `8509a66fc7` merges the operator's four DevPass commits, regenerated 049 metadata |
| Template alignment | Done | 187 changed reference and README documents validated; two fixed at `e1c1e03df8`; three class defects recorded below |
...
| CI: Routing Registry Drift Guard | Fixed | the scorer-eval ratchet failed from `d7d28d2ff3`: two golden prompts naming `/memory:save` routed to the spec-kit skill because its hand-curated derived vocabulary still said memory and the metadata refresh reset the age haircut that had been muting it. Vocabulary curated to continuity terms; the corpus and ambiguity counts each rose by one over the pinned baseline, so the baseline was recaptured. Green again on origin at `5b6fc075d0` |
| README reframing (D7) | Done | `464bc84eed`: root README plus nine skill, script and runtime READMEs describe continuity and retrieval; the store-era feature-flag, token-budget and decay-model passages and the five-target doctor list are gone; ten of ten still pass their template class; a residual grep for the memory-framing phrases returns nothing |
| Fast-forward v4 and main | Done | `main` = `skilled/v4.0.0.0` = `426c837b0b`; no memory registration in five config roots, no hook, plugin or launcher on disk; sweep live 0 on the main checkout after one stale launcher state file was removed |
...
| Deep review loop, pass 2 | Done | ten iterations on the landed head, lineage `luna-max-pass2`, verdict CONDITIONAL on four P1s: wrapper parity, HF-local model identity, the packet's own summary, and corpus symlinks outside the root. All four are fixed on the branch (`92683c97c2`, the summary at `a39561b9f5`, `bceadb694c`, `18dba056c3`); five P2s handed to owners under D5, three fixed. Pass 3 is the rename phase's own review on the tree that carries every fix |
| Rename phase (D8) | Done | `0db44e44c0`, one commit of 708 files: `system-spec-kit/mcp-server` moved to `system-spec-kit/runtime` as `@spec-kit/runtime`; the MCP SDK and every dependency without an importer dropped after a resolution audit; freshness table, hook symlinks, vitest config, doctor assets, compiled routing and every live reference follow. Old-path grep outside `specs/`, dist, archive and benchmark returns nothing; routing guard, dist freshness, doctor routes, both audits and validate on 049 recursive, 052 and 053 pass on the committed tree |
| Fast-forward v4 and main after the rename | Done | `main` = `skilled/v4.0.0.0` = `0db44e44c0` on origin; v4 then carries `f80c7e709e` through live-sync |
...
| Deep review loop, pass 3, attempt 2 | Done, rejected by the runner | relaunched at `945ded30d9` with the runner log outside the repository; codex exited with code 1 after 11 seconds and no stdout. Reproduced by hand: the Codex account had hit its usage limit, retry allowed from 00:55 local. The runner drains the child's stderr without keeping it, so the lineage log is empty; recorded as an observation below. Kept as `luna-max-pass3.attempt-2-usage-limit` |
| Deep review loop, pass 3, attempt 3 | Done | ten iterations at `945ded30d9` in 34 minutes, verdict CONDITIONAL: 2 P1, 2 P2, all raised in the first three iterations and none new after; fixed below at `90ca479473` and `1b0e512425`. Kept as `luna-max-pass3.attempt-3-conditional` |
| Deep review loop, pass 3, attempt 4 | Done | ten iterations at `4b894ebda0`, verdict PASS, 0 P0, 0 P1, 2 P2 (a stale `[mcp-server/]` label in the root README and the bin README's env-reference sentence; the 053 summary's dependency arithmetic) fixed at `a4e85f6ad4`. Lineage `luna-max-pass3` committed under 053 with the three earlier attempts beside it. The runner itself marked the lineage rejected: the leaf wrote three iteration files to a cwd-relative `iterations/` at the worktree root as well as into the lineage, containment reverted the strays, and the copies were byte-identical; recorded below |
| Remediation phases under 054 | Done | 001 claim binding `3a9d374c9b`; 003 retrieval coverage `426fff2771`; 004 save and resume freshness `322fb7161e`; 005 hook fallback signal `2fa9210f46`; 006 dead code `fe1a5f60c6`; 007 command family renamed to `/speckit:save`, `/speckit:search`, `/doctor speckit-retrieval` with a hard cutover `d6a5df774a` (supersedes D7 for command names by the operator's decision of 2026-09-05); 002 `scripts/` nested as `runtime/cli/` with the continuity writer at `runtime/cli/continuity` `f65b8f1e5b`, review pass running |
...
| Nesting review remediation and the CLI package rename | Done | review of the `scripts/` to `runtime/cli/` move (packet 054 phase 002) returned CONDITIONAL, 1 P0 and 8 P1; F001 fixed at `015a946df8`, F002 to F009 at `069c223966` (package renamed `@spec-kit/cli`, build wrapped in the freshness pair, CI install step, script and spec-root registries, skill-root vitest projects, hoisted tsx and vitest), install strategy hoisted at `76d1585e59`, disposition table and level reconcile at `ccbff09cfb`. Gates at `069c223966`: typecheck 0, dist fresh, 002 and control packet PASSED, route-validate OK, guard fresh, sweep live 0 |
...
| Metadata regeneration and parser edges, packet 057 | Done | drift census 114 packets (1 dirty) and 13 track roots; 113 clean packets regenerated (`c38aa31117`), 4 remain (1 dirty, 3 declaring children gone from disk), track roots stay with the sweep since no generator writes them. Dependency edges: deep-loop runtime manifest force-tracked and depends on `@spec-kit/shared` (`b95162c166`); sk-doc gains a skill-root manifest; one GLM lane adopted the shared parser in the deep-loop drift checker and six sk-doc scripts, tests rerun green; 38 hand-rolled sites remain with reasons in the packet |
| Parser adoption, second lane, and the specs deletion incident | Done | a deep-loop skill-root manifest (`85a5c28289`) gave the mode scripts the shared edge; one GLM lane adopted the parser at 14 deep-loop and 13 spec-kit sites with index parity proven and mirrors unchanged (`bedf5691b7`); the Cursor hooks README's fallback note had been inserted inside its frontmatter and made the index generator refuse to publish, fixed and the index regenerated in the same commit. During the lane the operator accidentally deleted 25 packet folders under `specs/system-speckit` in Finder; the undo restored only empty directories and the Trash emptied, so every tracked file (2,365) was restored from HEAD with the one staged edit re-applied from the index; 16 files git never tracked and the pre-deletion working-tree edits to those folders could not be recovered locally (MEGA's cloud rubbish bin may still hold them). A stale unmerged index entry for the council database, left by another session, blocked commits and was cleared from the index without touching the file |
| Consolidation into `033-system-speckit-v4` | Done | operator directive 2026-09-06: the twenty-four packets 033 to 050 and 052 to 057 moved under one phase parent as children 001 to 024 ordered by first commit, with literal names and 041's numbering gap closed; every live reference repointed by map (495 files), pointers repaired, metadata regenerated for 82 packet folders, the track root and the global descriptions cache rebuilt, `timeline.md` generated from git history, and every document given five to eight specific trigger phrases by two DeepSeek V4 Flash lanes through pi (169 files, audited: only phrase lines changed) plus a deterministic pass adding each folder's current and former name; the trigger index regenerates identically twice with 0 malformed documents and ten sampled folder-name lookups all resolve into the tree. This packet is now child 019 of that parent; its former id `052-memory-decommission-landing` stays searchable |
| Follow-up packets after closeout | Done | `system-speckit/033-system-speckit-v4/022-shared-containment-helper` centralizes the CLI write-boundary check (`6e20a1b467`); `system-deep-loop/043-review-leaf-protocol` states the verbatim-path and stop-reason duties in the lineage prompt and agent contract (`266debc441`) and repoints three determinism tests whose nested vitest path the hoist removed (`5210fe3566`); the nine 'environment-dependent' deep-loop files were partly this path defect. Both packets validate strict PASSED |
...
|---------|----------|-----|
| F001 free-text ripgrep recipes omit `--hidden`, so dotted documentation under `.opencode` is a silent miss | P1 | `2827031fb4`: `--hidden` in the lane's base flags, a `.git` exclusion glob, every recipe in the conventions, a hidden-directory test |
| F002 lookup `--limit` accepts `2junk` and `1.9` and truncates silently | P2 | `2827031fb4`: the raw value must be a whole decimal number; six malformed cases tested |
| F003 the HF model server's remote-bind opt-in requires a token that no request ever checks | P1 | `4ad10a7e42`: the configured token is required as a bearer credential on every request, compared in constant time, and the client sends it; live probe 401, 401, pass-through. This edits the preserved set, recorded below as a D5 deviation |
| F005 the cli-codex skill's default non-interactive example omits the stdin redirect its own hard rule requires | P2 | `464bc84eed`: the example carries `</dev/null` and names the rule that explains it |
| F006 forced-depth completion accepts a gapped iteration file set because it checks only the count | P1 | `bccdeae207`: the runner requires the contiguous set 1..cap on disk and in the state log and names the set it found; three regression cases |
| F007 post-dispatch write containment can be bypassed through a symlink beneath the artifact directory | P1 | `bccdeae207`: every descendant path is canonicalized before it counts as inside, an escape is fatal rather than an advisory, the review workflow's dirty-path filter applies the same rule; six regression tests, four of which fail against the old module |
...
| F010 plugin documentation still advertises the retired memory-plugin kill switch | P2 | `464bc84eed` removed the flags from the configuration lists; this commit removes the last mentions in the plugin test guidance and the shared-library rows |
...
| F004 the memory doctor route names a checklist artifact that does not exist and a gold-battery pass policy from the retired search engine | P1 | `5f8353bc97`: the route points at the 049 phase 001 acceptance criteria, the frozen parity baseline and the latency report, and its pass policy states the lookup exit contract, the 200 ms cold budget, byte-identical regeneration and zero unexplained parity differences |

### Review pass 2 findings and fixes

| Finding | Severity | Fix |
|---------|----------|-----|
| P2-F001 the public ripgrep wrapper builds its own recipes and never gained `--hidden` or the `.git` exclusion, so it diverged from the lane it fronts | P1 | `92683c97c2`: the wrapper's recipes open with the same head flags and glob set as the lane; the parity assertion is green again and every recipe is asserted to carry both |
| P2-F002 the copyable recipes in the conventions omit the `.git` exclusion the prose calls mandatory | P2 | `92683c97c2`: all six recipes carry it and the glob-order note names three exclusions |
| P2-F003 the wrapper drops a third positional silently, so an unquoted phrase searches its first word | P2 | `92683c97c2`: extra positionals are refused with a message that says to quote the phrase |
| P2-F004 the HF-local availability probe reports a ready server without proving it serves the requested model, so the mismatch surfaces later as a 404 | P1 | `18dba056c3` under the D5 security exception: the probe resolves the requested model and refuses a ready server that serves another; readiness throws on the mismatch instead of latching; four identity tests |
...
| P2-F007 the packet's implementation summary asserted completion over scaffold text | P1 | `a39561b9f5`: the summary describes what is true now and says it is in progress; the closure reconciles summary, continuity metadata, spec status, acceptance rows and tasks together |
| P2-F012 the corpus walker indexed a symlinked Markdown file whose target resolves outside the repository | P1 | `bceadb694c`: a link resolving outside the resolved root is refused and reported; regression test with an out-of-tree target |
| P2-F010 the forced-depth check de-duplicated state records before comparing, so a full range plus a duplicate passed | P2 | `b34f39694e`: duplicates are a violation in their own right, with a test |
...
| Finding | Severity | Fix |
|---------|----------|-----|
| DR-001 the scripts freshness scan followed the `scripts/runtime` symlink into the engine's dist, so every runtime rebuild flagged the scripts package stale and the hash covered another package's outputs | P1 | `68d9854f2e`: symlinked directories are not descended; source count 210 to 150; touching a runtime dist file leaves scripts fresh |
| DR-003 live operational docs still presented the runtime package as an MCP server (titles, diagrams, boundaries, FAQ, hook matrix, trigger guide) | P1 | `945ded30d9`: 58 documents and the leaf manifest reframed around the runtime engine; the context README rewritten to the one module it holds; recovery routes point at the trigger-index lookup; template validators 0 issues before and after; trigger index regenerated and deterministic |
| DR-002 the Claude shim overrides honored any existing path, including a relative one resolved against the caller's cwd or a directory | P2 | `68d9854f2e`: an override must be absolute and a regular file; four shim tests |
| DR-004 stale `mcp_server` vocabulary in code comments, test constants and identifiers | P2 | `68d9854f2e`: 34 code and test files renamed to runtime wording; found alongside and fixed: the import-policy guard still matched `mcp_server/(lib|core|handlers)` so relative imports into the runtime internals passed unchecked (its test went 2 failed to 4 passed); the source-dist alignment eval keyed on the old directory |
| DR-005 the freshness shell test cleaned a cache file under the old package id | P2 | `68d9854f2e`: glob follows the runtime id; test 2 of 2 |
| Found alongside: `scripts/doctor.sh` probed the removed MCP SDK and sqlite-vec, so the doctor route would fail strict on any fresh clone | P1 (own finding) | `68d9854f2e`: probes better-sqlite3 and zod; strict run passes |

### Review pass 3, attempt 3 findings and fixes

| Finding | Severity | Fix |
|---------|----------|-----|
| DR-001 the symlink guard added for attempt 1's finding called `statSync` on the link, so a dangling `scripts/runtime` (fresh checkout, runtime not built) turned the scripts freshness check into an error | P1 | `90ca479473`: a link whose target is missing is skipped like a directory link; walker unit tests cover a regular tree, a directory link and a dangling link |
| DR-004 the runtime manifest kept `chokidar` on the strength of an advisor fallback candidate that the advisor's own installed copy always shadows | P1 | `90ca479473`: dropped from the manifest, the tsconfig path mapping and the lockfile (29 lockfile lines); the advisor's candidate list is preserved-set and untouched, its second entry now never resolves; `1b0e512425` records the reversed decision in the 053 summary |
| DR-002 the shim override accepts any absolute regular file; asked for it to be documented as trusted input and tested for a dangling link | P2 | `1b0e512425` documents it as a trusted operator or test input, not a hardening boundary; `90ca479473` adds the dangling-link test (5 shim tests) |
| DR-003 the fixture READMEs, the root README's doctor lock path and the Devin fallback text still named `mcp_server` or `mcp-server` | P2 | `1b0e512425` and `90ca479473`: runtime wording; the lock path now names the advisor package that actually owns it |
...
| Changed docs pass their class validators | 187 references and READMEs plus 23 command, install-guide and root documents checked; two real failures fixed at `e1c1e03df8`; the remaining failures are the three validator class defects below, each failing identically at `576c37b72e` |
| validate.sh --strict on 049 recursive, 050, 051 | 049: 8 of 8 PASSED after the merge and again at `a4e85f6ad4`; 050 and 051 were retired with the zvec lane at the operator's request, so 052 and 053 stand in their place and both PASS |
| Trigger index byte-identical on main; stashed edits accounted for | Regenerated at `426c837b0b` for the 210 documents the merge brought in; two runs hash identically. Re-run at `0db44e44c0`: two runs give index sha `3c666220d84d76ef` and manifest `d5fb5e1a23657ead`, no tracked diff. The operator committed their work on v4 before the landing, so no stash was needed |
| Deep review 10 iterations, no P0 or P1 | Landing tree: pass 1 CONDITIONAL (5 P1 fixed), pass 2 CONDITIONAL (4 P1 fixed). Rename tree: attempt 3 CONDITIONAL (2 P1 fixed), attempt 4 PASS with 0 P0, 0 P1 at `4b894ebda0`. Nesting tree (054 phase 002): passes 1, 3, 5 and 6 accepted, passes 2 and 4 rejected by the runner; every reproducible P0 and P1 across them is fixed and names its commit in the phase summary; the six P1 the read-only seat raises for gates it cannot run are replayed green on the landed tree. Read per the decision row below: no reproducible P0 or P1 remains |
...
| Validator class defect: compiled deep-loop contracts | `commands/deep/assets/compiled/*.contract.md` are generated by the contract compiler and fail the `command` template at `576c37b72e` exactly as they do now. Owner: system-deep-loop contract compiler or sk-doc's type detection |
| Validator class defect: install-scripts folder README | `install-guides/install-scripts/README.md` is a folder index classified as an `install_guide` by its path and fails the same five sections at `576c37b72e`. Owner: sk-doc type detection |
...
| Dist freshness counts retrieval fixtures as source | Every trigger-index run rewrites `scripts/retrieval/fixtures/*.json` byte-for-byte, and the freshness table counts them as scripts-package sources, so the package reads stale until `prepare-build`, `npm run build`, `record-build` are run. Re-stamped that way at `f80c7e709e`; a plain `npm run build` from the package directory does not clear it. Owner: spec-kit scripts. Not fixed here |
