...
- [x] T021 Validate strict on the packet and recursively on the parent tree (`validate.sh`) — **Evidence**: `validate.sh 048-gate-3-mutation-time-delivery --strict` → `RESULT: PASSED` 0 errors; `validate.sh 033-system-speckit-v4 --recursive --strict` passes on the parent and on 48 of the 49 children, with the single failure being `030-spec-kit-simplification-research`, whose `goal.md` durable slice (6498 characters) exceeds the 4000-character ceiling and which fails identically from the pre-change tree.
