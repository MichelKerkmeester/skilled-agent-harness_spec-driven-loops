...
**Session:** `fanout-mimo-1780735927714-4462h3`
...
Architecture (a) — a pure per-invocation CLI — fails the zero-feature-loss bar: 6 STATE-WATCHER tools and 5 daemon services cannot function without a long-lived process. Architecture (c) — auto-spawning hybrid — achieves zero loss but adds unnecessary complexity for marginal gain over (b).
...
|-------------|--------|----------|
| KQ1: Parity matrix — CLI equivalent per tool? | All 37 tools classified: 22 STATELESS (trivially CLI-portable), 9 STATE-EMBED (CLI-portable with cold-embed latency), 6 STATE-WATCHER (need daemon). Zero MCP-ONLY tools. | Iteration 001 |
| KQ2: Daemon-dependency audit — what dies per architecture? | Architecture (a) loses 6 tools + 5 services. Architecture (b) preserves all. Architecture (c) preserves all with cold-latency caveat on daemon restart. | Iteration 002 |
...
|---|------|-------|----------------|-------|
| 1 | `memory_search` | STATELESS | `spec-memory search --query "..." --limit 10` | Pure vector+BM25+FTS5 query [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/tools/memory-tools.ts:67] |
| 2 | `memory_quick_search` | STATELESS | `spec-memory quick-search "..."` | Thin wrapper with preset flags [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/tools/memory-tools.ts:68-88] |
| 3 | `memory_match_triggers` | STATELESS | `spec-memory match-triggers "..."` | Trigger phrase matching |
| 4 | `memory_save` | STATE-EMBED | `spec-memory save --file path.md` | Needs embedding. Prior art: `generate-context.js` |
...
| 7 | `memory_health` | STATELESS | `spec-memory health` | DB integrity + FTS5 validation |
...
| 12 | `memory_retention_sweep` | STATELESS | `spec-memory retention-sweep --dry-run` | Governed record cleanup |
| 13 | `memory_embedding_reconcile` | STATE-EMBED | `spec-memory embedding-reconcile` | Vector status reconciliation |
| 14 | `embedder_list` | STATE-EMBED | `spec-memory embedder-list` | Registered backends |
| 15 | `embedder_set` | STATE-EMBED | `spec-memory embedder-set --name <name>` | Backend selection with re-index |
| 16 | `embedder_status` | STATE-EMBED | `spec-memory embedder-status` | Re-index progress |

### Group B: Context (1)

| # | Tool | Class | CLI Equivalent | Notes |
|---|------|-------|----------------|-------|
| 17 | `memory_context` | STATELESS | `spec-memory context --input "..." --mode deep` | Unified entry point [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/tools/context-tools.ts:16] |

### Group C: Causal (4)

| # | Tool | Class | CLI Equivalent | Notes |
|---|------|-------|----------------|-------|
| 18 | `memory_drift_why` | STATELESS | `spec-memory drift-why --id 42` | Causal graph traversal [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/tools/causal-tools.ts:34] |
...
| # | Tool | Class | CLI Equivalent | Notes |
|---|------|-------|----------------|-------|
| 22 | `checkpoint_create` | STATELESS | `spec-memory checkpoint-create --name "pre"` | [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/tools/checkpoint-tools.ts:34] |
...
| # | Tool | Class | CLI Equivalent | Notes |
|---|------|-------|----------------|-------|
| 26 | `memory_index_scan` | STATE-EMBED | `spec-memory index-scan --spec-folder "028-..."` | [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/tools/lifecycle-tools.ts:57] |
| 27 | `task_preflight` | STATELESS | `spec-memory task-preflight --spec-folder "..."` | [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/tools/lifecycle-tools.ts:58] |
...
| 30 | `memory_ingest_start` | STATE-EMBED | `spec-memory ingest-start --paths file1.md` | Job-queue in-process [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/lib/ops/job-queue.ts:46-60] |
...
| 33 | `eval_run_ablation` | STATE-EMBED | `spec-memory eval-ablation` | |
| 34 | `eval_reporting_dashboard` | STATELESS | `spec-memory eval-dashboard` | |
| 35 | `session_health` | STATE-WATCHER | `spec-memory session-health` | **Needs daemon** [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/handlers/session-health.ts:8-12] |
| 36 | `session_resume` | STATE-WATCHER | `spec-memory session-resume` | **Needs daemon** |
| 37 | `session_bootstrap` | STATE-WATCHER | `spec-memory session-bootstrap` | **Needs daemon** [SOURCE: file:.opencode/skills/system-spec-kit/mcp_server/handlers/session-bootstrap.ts:260-276] |
...
**Note:** MiMo classifies `memory_ingest_start` as STATE-EMBED (needs embedding per file + uses in-process job queue for async dispatch) and counts 6 STATE-WATCHER tools. This is a minor divergence from the DeepSeek lane (10 STATE-EMBED, 5 STATE-WATCHER) — the total remains 37 and the architecture verdict is identical.
...
|-------------|----------------|------------|---------|
| (a) Pure CLI | 31 | 6 (ingest_status, ingest_cancel, session_health, session_resume, session_bootstrap, ingest_start loses async) | **Fails zero-feature-loss bar** |
| (b) CLI-over-Daemon | 37 | 0 | **Meets zero-feature-loss bar** |
| (c) Hybrid | 37 (temporarily degraded after daemon crash) | 0 | **Meets zero-feature-loss bar with availability caveat** |
...
| Runtime permissioning | `opencode.json` `permissions.allow` for shell | Adapted — runtime change needed |
...
|-----------|-------------|---------------------|-----------|
| Tools preserved | 31/37 | 37/37 | 37/37 |
...
|----------|------------------|----------|-------------|
| Architecture (a): Pure per-invocation CLI | Fails zero-feature-loss bar: 6 STATE-WATCHER tools (ingest_status, ingest_cancel, session_health, session_resume, session_bootstrap, ingest_start async behavior) non-functional without daemon. 5 services lost (file-watcher, retry queue, ingest queue, warm embedder, session state). | Iterations 001, 002 | 1-5 |
...
- `mcp_server/handlers/session-bootstrap.ts:260-276` — Composite session handler
- `mcp_server/handlers/session-health.ts:8-12` — Session health with daemon-resident state
- `mcp_server/lib/ops/job-queue.ts:46-60` — In-process async ingest job queue
- `.opencode/bin/mk-spec-memory-launcher.cjs` — Launcher with owner-lease, RSS watchdog, crash-loop guard
- `.opencode/bin/lib/launcher-ipc-bridge.cjs:80-122` — JSON-RPC IPC bridge over daemon-ipc.sock
- `.opencode/bin/lib/launcher-session-proxy.cjs:33-58` — Reconnecting MCP proxy with replay classification
...
**Research completed 2026-06-06. Prepared by MiMo-V2.5-Pro lineage under `fanout-mimo-1780735927714-4462h3`.**
