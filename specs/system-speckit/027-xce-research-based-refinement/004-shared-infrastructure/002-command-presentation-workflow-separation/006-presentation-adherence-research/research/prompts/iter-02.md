...
## YOUR ANGLE (#2)
The /memory:search render path specifically: trace how a dispatched model gets from search.md step 7 to assets/search_presentation.md section 2, identify every hop where the template can be dropped, and propose the minimal restructuring that makes the parseable format unmissable.

## OUTPUT
ONLY a fenced ```json block:
{"angle": 2, "findings": [{"class": "DOC-DRIFT|REFINEMENT|NEW-FEATURE|BUG", "severity": "P1|P2|P3", "title": "<short>", "evidence": "<file:line or concrete comparison>", "detail": "<2-3 sentences>", "fix_sketch": "<one sentence>"}], "summary": "<2 sentences>"}