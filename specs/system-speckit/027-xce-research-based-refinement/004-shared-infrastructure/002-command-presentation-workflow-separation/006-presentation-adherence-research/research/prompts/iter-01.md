...
## YOUR ANGLE (#1)
Inline-vs-referenced render templates: audit every command family (memory, speckit, create, doctor, deep) for where the render/display template physically lives (inline in the command .md vs a pointed-to presentation asset) and correlate with observed adherence (doctor menu rendered verbatim; /memory:search format ignored).

## OUTPUT
ONLY a fenced ```json block:
{"angle": 1, "findings": [{"class": "DOC-DRIFT|REFINEMENT|NEW-FEATURE|BUG", "severity": "P1|P2|P3", "title": "<short>", "evidence": "<file:line or concrete comparison>", "detail": "<2-3 sentences>", "fix_sketch": "<one sentence>"}], "summary": "<2 sentences>"}