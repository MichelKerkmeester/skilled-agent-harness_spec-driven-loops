...
- **REQ-001..004 all resolve to shipped code** (commit `da09d7c69e`), verified at file:line. Transaction-yield boundary is correct (yields strictly between self-contained 200-row chunk transactions, never inside one); cancel checks, timer teardown in `finally`, and `onPhase`→`maintenance.refresh()` wiring all confirmed.
