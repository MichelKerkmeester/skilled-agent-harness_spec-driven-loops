...
1. **`mcp_server/handlers/save/spec-folder-mutex.ts:37` — save mutex reaps live owners.** `isStaleLock` uses only mtime age; `owner.json` pid is never read back, the lock dir mtime is set once at mkdir and never refreshed, and acquire reaps purely on age → a still-alive long save (chunked embedding / slow provider) has its lock stolen by a second writer. This is the **twin** of the lock fixed in `generate-context.ts` by commit `6aab9d31b1` ("save-lock liveness") — that fix never reached this module. *(Verified by Kimi.)*
