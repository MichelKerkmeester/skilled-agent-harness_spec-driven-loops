...
3. **Orphan check** — All 6 presentation files are referenced by their router. **No orphaned `.txt`.** The router without a presentation asset (`skill-benchmark.md`) intentionally follows a different "shared deep-loop command contract" / packaging-owned-loop pattern and does **not** reference any `deep/assets/*_presentation.txt` — so no dangling reference (`skill-benchmark.md:114`).
