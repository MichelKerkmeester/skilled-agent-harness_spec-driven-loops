...
- [x] 006-T022 · `.opencode/skills/system-spec-kit/mcp_server/handlers/memory-triggers.ts:525` — Pass a larger or unbounded max to matchSemanticTriggers (e.g. limit*4) and let filterSemanticMatchesByScope reduce to the final scoped set, then slice to limit — mirroring the lexical limit*2 over-fet _[P2 — FIXED in commit 33df05f455: `max: limit * 4` over-fetch then scope-filter]_
...
- [x] 006-T024 · `.opencode/skills/system-spec-kit/mcp_server/lib/embedders/reindex.ts:663` — After the detach attempt, assert get_attached_vector_path(jobDb)===null before renaming (re-detach or throw otherwise) instead of swallowing all errors as 'not attached'; and/or add a file-identity (f _[P2 — FIXED in commit 33df05f455: post-detach assertion + source-kind carry]_
...
- [x] 006-T041 · `.opencode/specs/system-spec-kit/027-xce-research-based-refinement (commit 16f51bcc29)` — Future bulk snapshots should be committed as a neutral chore (no per-epic scope prefix) or split by owning packet; explicitly tag any 'not independently verified' content so a later verification pass  _[P2]_
