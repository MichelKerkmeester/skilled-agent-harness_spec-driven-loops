...
## Closed in code wave 9 (commit 53bb925804, Fable-verified)
...
## Adjudicated by-design / honest-doc (round-2)
tri-173 (ambiguity FP/FN slice): `ambiguityStable()` is a single synthetic two-skill stability check, not an empirical false-positive/false-negative rate — the labeled corpus carries no expected-ambiguous advisor labels, so an FP/FN slice cannot be computed without a hand-labeled ambiguity set. Documented at the function + the `ambiguity_slice_stable` metric so it is not read as ambiguity-rate coverage; building the labeled set is future eval data, not a code fix.
tri-180 (harvestable misroute payload): outcome telemetry persists only sanitized skill ids + accept/correct/ignore, never the prompt — deliberate prompt-safety. Real misroutes therefore cannot be reconstructed into gold cases from telemetry; capturing prompts would breach the prompt-safety invariant. Documented at the schema + record builder as an accepted tradeoff (gold cases are authored, not harvested). No capture path added by design.

## Code queue (open)
None remaining in L5. tri-138 deferred pending consumer-aware design (memory_health budget enforcement; doctor flows read data.routing).
