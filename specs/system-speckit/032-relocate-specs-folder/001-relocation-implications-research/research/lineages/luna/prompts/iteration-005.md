...
Questions: 4/5 answered | Last focus: Spec Kit Memory MCP path resolution | Evidence iterations: 4 | Graph coverage: unavailable (better-sqlite3 Node ABI mismatch) | Direct-mode fallback: active and bounded to this detached lineage | New-info ratios: 1.00, 0.85, 0.75, 0.65
...
Iteration: 5 of 5
Focus Area: Measure the remaining production reference surface and derive the safest migration sequence under the Git, mirror, and Memory MCP constraints
...
Which canonical-only and default-root callers remain after the tooling, Git, mirror, and Memory MCP audits?
What compatibility bridge and validation matrix minimize migration risk?
Last 3 Iterations Summary: Iteration 2: Git evidence confirmed a tracked mode-120000 root symlink and a source/downstream ignore split driven by the global excludes file.
...
Iteration 4: Memory MCP source audit found dual-root identity and explicit-path support, but canonical-first discovery and canonical-only recovery/repair blind spots.
