...
| Nesting review, pass 5 | Running | launched at `072da7777c` on DevPass GPT-5.6 LUNA max (cli-opencode), the only LUNA route with quota left; the packet's review-scope section now names the exact `specs/system-speckit/` write path so the leaf's earlier track typo has a documented rule against it |
