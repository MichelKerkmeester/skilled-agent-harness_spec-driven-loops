...
| AC-003 | REQ-003 | Given a pushed commit with a packet its track does not list, When the hook runs, Then the push is blocked, and a clean commit passes whatever the working tree holds | `pre-push.test.sh`, 43 passing, two of them real `git push` runs from a linked worktree; reading the working tree instead of the commit fails two cases | Met | - |
