...
**Packet:** 035-spec-kit-simplification-research/013-trigger-phrase-quality-enforcement
**Level:** 2
**Status:** Complete
**Date:** 2026-09-07
<!-- /ANCHOR:metadata -->
...
|-------|-----|---------------------|--------------|--------|--------|
| AC-001 | REQ-001 | Given the judge, When `retrieval`, `2026-05-14`, `session`, `memory`, `the` and `spec folder question` are judged, Then the classes are single-token, numeric-only, editor-fallback, generic-workflow-word, stop-word-only and admitted | the phrase-judge cases in `grep-convention.vitest.ts` passed | Met | - |
| AC-002 | REQ-002 | Given the corpus, When the generator runs twice, Then the diagnostics carry `phraseQuality` with phrase and document counts per class and the index hash is identical | printed bucket: single-token 314 phrases / 825 documents, numeric-only 42 / 121, generic 4 / 171, editor-fallback 2 / 4, stop-word-only 3 / 12, prose-sentence 27 / 18, ok 35,087; two runs, one hash | Met | - |
| AC-003 | REQ-003 | Given the doctor asset, When its pollution and pair activities are read, Then they name the committed diagnostics bucket and all four artifacts | the asset parses and the three edited lines carry the names | Met | - |
| AC-004 | REQ-004 | Given the presentation asset, README and conventions, When they are searched for the retired labels, field names, script count, recipe count and missing rows, Then nothing contradicts the code | the search returned only the rewritten lines; sk-doc validator exit 0 on both documents; the parity suite passed | Met | - |
