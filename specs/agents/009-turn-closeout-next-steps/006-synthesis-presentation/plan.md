...
# Implementation Plan: Phase 1: synthesis-presentation
...
### Overview
Four research iterations deciding whether presenting a synthesis earns its own rule, and what each deep-loop mode owes the reader.
<!-- /ANCHOR:summary -->
...
### Key Components
- **The per-mode table**: Six rows, because six modes produce different artifacts
- **The decision tests**: Decide whether the content may be a rule at all

### Data Flow
The brief requires a distinct row per mode, so a single generic answer across six modes fails rather than passes.
<!-- /ANCHOR:architecture -->
