...
# Implementation Plan: Phase 4: agents-md-integration
...
### Overview
Wire the rule at all three integration points, and record the scope widening the operator authorised.
<!-- /ANCHOR:summary -->
...
### Key Components
- **`REPO RULES.md`**: Trigger row loads the rule, index row makes it browsable
- **`AGENTS.md`**: Pointers make it findable at the moment of need

### Data Flow
Gate 5 reads the router, the trigger table matches on the action about to be taken, and only then does the rule file load.
<!-- /ANCHOR:architecture -->
