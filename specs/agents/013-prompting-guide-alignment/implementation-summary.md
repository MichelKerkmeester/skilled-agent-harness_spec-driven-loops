...
_memory:
  continuity:
    ...
    recent_action: "Applied the operator decisions and closed the packet"
    next_safe_action: "None: complete. The emphasis-label follow-up is packet 014"
    blockers: []
    key_files:
    ...
    open_questions: []
    answered_questions:
      - "Stop-for-yes: narrow AGENTS.md:116 to irreversible actions and installs"
      - "Improvement cycles: one retry, everywhere"
      - "Agent emphasis labels: a new packet for the template and all agents"
      - "scratch/: delete sources, briefs and logs"
...
You then settled the three decisions the lenses left open. The stop-for-yes in `AGENTS.md` now covers irreversible actions and installs, and points to the reversibility ladder for what counts as irreversible. `sk-prompt` retries once everywhere, matching what `@prompt-improver` already did. The agent template's emphasis labels moved to their own packet, `specs/agents/014-agent-emphasis-register`, because the template and all twelve agents have to change together.
...
| File | Action | Purpose |
|------|--------|---------|
| `AGENTS.md` | Modified | Law 4, question timing, Violation Recovery, targeted recheck, name-the-source bullet, stop-for-yes narrowed to irreversible actions and installs |
...
| `.skilled/repo-rules/blast-radius.md` | Modified | Finish reversible work before a tier-3 stop; an overwritten or deleted untracked file is tier 3; an install still waits for a yes (1.0.1.2) |
...
| `.skilled/skills/sk-prompt/SKILL.md` | Modified | Matrix, ALWAYS 1, NEVER 1 and 4, one-retry cap (3.0.1.0) |
| `.skilled/skills/sk-prompt/README.md` | Modified | One-retry cap |
| `.skilled/skills/sk-prompt/references/patterns-evaluation.md` | Modified | Matrix, chain-of-thought format, few-shot tags |
| `.skilled/skills/sk-prompt/references/depth-framework.md` | Modified | Single-point interaction, selection criteria, RICCE, one-retry cap |
| `sk-prompt/manual-testing-playbook/` SP-011, SP-017 and the index | Modified | The scenarios test the one-retry cap |
...
| `cli-devin/SKILL.md`, `README.md`, `references/providers-and-models.md`, `references/cli-reference.md`, `changelog/v1.4.4.0.md` | Modified / Created | Roster docs (1.4.4.0; another session took 1.4.3.0 on main first, so this release was renumbered at merge) |
| `.pi/models.json` (main checkout) | Modified | Comma and space restored in a line another session had joined |
| `scratch/sources/`, `scratch/briefs/`, `scratch/logs/` | Deleted | Vendor page text stays out of the repository; briefs and logs backed up outside it first |
<!-- /ANCHOR:what-built -->
...
| Decision | Why |
|----------|-----|
| Narrow `AGENTS.md:116` to irreversible actions and installs | Your call. The ladder in `blast-radius.md` §2 already priced reversible work as proceed, so `AGENTS.md` now points to it instead of repeating a wider list |
| Move the agent emphasis labels to packet 014 | Your call. `sk-create-agent`'s template prescribes them for all twelve agents, so the template and the agents change together there |
| One retry, everywhere | Your call. It matches what `@prompt-improver` already did, and scenarios SP-011 and SP-017 now test it |
...
| Write two files the frozen scope table does not list | `cli-devin/changelog/v1.4.4.0.md` follows from the version rule (a `SKILL.md` version equals its newest changelog), and `.hermes/skills/sk-prompt/SKILL.md` is regenerated by the mirror script; `scope-discipline.md` §2 counts both as the same change |
...
| Check | Result |
|-------|--------|
| `check-repo-rules.cjs` | PASS 9/9, exit 0, rerun at closeout |
| sk-doc `validate_document.py`, sk-prompt files and changelog | VALID on all nine edited sk-prompt docs, rerun at closeout |
| sk-doc `validate_document.py`, `AGENTS.md`, `REPO RULES.md`, four rules | One issue each (missing "overview", read as README type), identical at base `c70180f373`; rechecked at closeout for `AGENTS.md` and `blast-radius.md` |
...
| Hermes skill mirrors | PASS 70/70, rerun at closeout |
| sk-prompt playbook package | 28 scenarios, 0 errors, 3 advisory warnings that main also shows, exit 0 |
...
| `validate.sh --strict` | RESULT: PASSED at closeout, after re-deriving `graph-metadata.json` with the main checkout's built tools |
<!-- /ANCHOR:verification -->
...
1. **The agent emphasis labels are still in place.** Packet `014-agent-emphasis-register` plans their removal from the template and all agents.
2. **The worktree has no shared dependencies.** Post-edit hooks and `check-agent-mirror-sync.cjs` cannot load `@spec-kit/shared` here, so those checks ran from the main checkout's copy.
3. **Adjacent defects, reported and not fixed.** The drift-guard errors in `cli-jev` probe scripts and old containment copies; `child-dispatch-preamble.md:61`'s files-on-disk completion line; the cli-devin docs' default permission mode, default model, Gemini note and unlisted Grok; em dashes in `answer-the-actual-request.md` against `communication-prose.md`; the hub `ROUTER.md:61` roster line, which omits GPT-6 Luna and feeds compiled routing; nine `fanout-run` tests that failed in the loaded baseline run and passed on the quiet rerun, so they look load-sensitive rather than broken.
<!-- /ANCHOR:limitations -->
