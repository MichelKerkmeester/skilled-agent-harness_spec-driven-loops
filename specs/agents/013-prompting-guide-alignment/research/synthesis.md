...
## 3. Settled by the operator

These three survived verification but were not mine to settle. The operator decided each one on 2026-09-24.

**LOGIC-SYNC: how many improvement cycles `@prompt-improver` runs.** `sk-prompt/SKILL.md:464` (§7 Deterministic Agent Rules) allowed "up to 3 total improvement cycles", and two playbook scenarios asserted that cap (`manual-testing-playbook/depth-clear-loop/depth-iteration-cap.md:30`, `clear-scoring/forty-of-fifty-threshold.md:30`). The agent itself says "retry exactly once" (`prompt-improver.md:83`) and "Retry at most once" (`:189`). Luna and MiMo both found it. **Decision: one retry, everywhere.** `SKILL.md`, `README.md`, `references/depth-framework.md`, scenarios SP-011 and SP-017, the playbook index and the `v3.0.1.0` changelog now state one retry, and the agent was already right.

**LOGIC-SYNC: the mandatory stop-for-yes against the reversibility ladder.** `AGENTS.md:116` required a rollback and a wait before any delete, overwrite, migrate, deploy, send or install. `blast-radius.md:61-62` priced a deleted tracked file as "Proceed" and an installed dependency or reversible migration as "Say what undoing costs, then proceed". Luna and MiMo both proposed narrowing `AGENTS.md` to the irreversible tier (`openai-gpt-6.md:71` says reversible work needs no permission). **Decision: narrow `AGENTS.md`.** `AGENTS.md:116` now names irreversible actions and any install, and points to the ladder's bottom tier for what counts. `blast-radius.md` (1.0.1.2) moved an overwritten or deleted untracked file into the irreversible tier and says an install still waits for a yes.

**The agent template's emphasis labels.** `prompt-improver.md` uses CRITICAL ×2, IMPORTANT, HARD BLOCK ×2 and MANDATORY ×5, the register `claude-prompting-best-practices.md:489` says now over-triggers and that `cli-prompt-quality-card.md:59` already measured as underperforming. Opus, Luna and MiMo all flagged it. The labels come from `sk-create-agent/assets/agent-template.md` (`:155`, `:157`, `:428`, `:433`, `:438`), and the template's own checklist at `:814` requires the CRITICAL and IMPORTANT statements, so rewriting one agent would break its conformance with the other eleven. **Decision: a new packet for the template and all agents.** `specs/agents/014-agent-emphasis-register` holds the planned change.
