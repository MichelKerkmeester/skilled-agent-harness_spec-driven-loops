...
| AC-012 | REQ-012 | Given the prioritised fix list, When the remediation set lands and every gate is re-run, Then each change carries its proving check and no gate that passed at baseline now fails | Applied diffs plus `scratch/gate-results-final-review.md` read gate by gate against `scratch/gate-results-baseline-review.md`: `FAILING GATES: 0`, the sixteen baseline gates unchanged and the added router-probe gate passing | Met | - |
| AC-013 | REQ-013 | Given the eight playbook scenarios, When they run in wave order, Then each records a verdict, a transcript, an exit status and a reason | `.skilled/skills/cli-orca/benchmark/reports/2026-09-20--playbook-post-remediation/**`, 4 PASS / 0 FAIL / 4 SKIP, each scenario carrying its exact prompt, transcript, exit status and one-sentence reason | Met | - |
