...
## 0.1 MODEL ELIGIBILITY (HARD BLOCK)

Run this agent only on an eligible (model, route) pair. Eligibility is a property of the (model, route) pair, never of the model alone.
...
| `cli-hermes` | `deepseek-v4.1-flash` or `glm-5.3-flash` through `llmgateway` | Yes | The Hermes roster is closed at these two ids; neither is Luna, Opus or Fable |
...
The route list above is a list of executors. The `cli-usage` transport — the only mode of the `cli-jev` hub — is deliberately absent: it answers one typed judgment and runs nothing, so no prompt-improvement task can be dispatched to it and it needs no eligibility row.
...
| ----- | -------------- | ------ | -------- | ------------ |
| `sk-prompt` | `INT-SKILL-PROMPT-IMPROVER` | Prompt engineering | High-stakes prompt improvement is needed | 7 frameworks, DEPTH, CLEAR scoring |
