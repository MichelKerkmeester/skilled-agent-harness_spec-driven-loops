...
_memory:
  continuity:
    ...
    recent_action: "Superseded AC-004 via ADR-001 after the operator dropped the Problem section from the root README"
...
Nine criteria are Met with observed evidence and AC-004 is Superseded by ADR-001
after the operator removed the Problem section from the root README. The
two-register emoji policy, feature-named headings, per-item inventory
descriptions and the paragraph bound are written into the files authors load, and
the shared `core-standards.md` wording was tightened in the same direction.
Scanner enforcement was deliberately excluded because these are guidance-level
rules.
<!-- /ANCHOR:closure -->
