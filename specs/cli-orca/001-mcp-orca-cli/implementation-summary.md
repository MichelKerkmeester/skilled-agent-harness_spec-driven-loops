...
## Supersession Addendum (2026-09-20)

This packet moved from `specs/mcp-tooling/021-mcp-orca-cli` to `specs/cli-orca/001-mcp-orca-cli`. The subject it ported left the `mcp-tooling` parent hub and now lives as a standalone class-S skill at `.skilled/skills/cli-orca`, so the hub-mode framing in the documents above is historical. The mode entry, its router signals, its vocabulary classes, its stage-two resource map and its manifest leaves were removed from the hub in version 1.8.0.0.

The promoted skill supersedes this packet's runtime surface. It carries the routing contract, the four CLI references carried forward from the former packet, the eight official Orca skill references, the verbatim snapshot assets, its own feature catalog and manual testing playbook. The extraction and upgrade are recorded in `specs/cli-orca/002-consolidate-official-orca-skills`, and the hub-side evidence for the former mode moved to `benchmark/reports/hub-routing-archived/` in this packet.

Verification note for this addendum: the packet documents remain as written for the hub-mode era, because they are the historical record of that work. Only the metadata in `description.json` and `graph-metadata.json` was re-pointed to the new packet identity.
