---
title: "Deep-Review Iteration Prompt Pack (2 of 20)"
trigger_phrases: []
---
DEEP-REVIEW
...
1. **Routing logic correctness**: does `SKILL.md`'s described routing rule actually match what `mode-registry.json` + `hub-router.json` contain? E.g., does every `workflowMode` mentioned in `SKILL.md` prose actually exist as a registered mode? Does the described three-tier discriminator (workflowMode/runtimeLoopType/backendKind) match the actual field values in `mode-registry.json` for every mode entry (especially the 3 improvement lanes claimed to share one packet with `runtimeLoopType: null`)?
