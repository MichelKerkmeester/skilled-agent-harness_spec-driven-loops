...
- Scope proof: The routing registry and router remain internally consistent: six modes are registered in `.opencode/skills/system-deep-loop/mode-registry.json:29-198`, and all six are represented in `.opencode/skills/system-deep-loop/hub-router.json:15-50`; the issue is confined to README wording.
...
- Routing logic correctness: PASS. `SKILL.md` names six public workflow modes across four families, and `mode-registry.json` contains the same six `workflowMode` entries. Research, review, and ai-council have `backendKind: runtime-loop-type`; all three improvement lanes have `runtimeLoopType: null`.
- `hub-router.json` internal consistency: PASS. `routerSignals` and `routerPolicy.tieBreak` cover all six registry modes. Every referenced vocabulary class exists under `vocabularyClasses`.
- `description.json` accuracy: PASS. It describes six modes across four workflow families and names the three improvement lanes.
...
- Cross-file count consistency: PASS. Counted six `workflowMode` entries, six `routerSignals`, six `tieBreak` entries, and one `graph-metadata.json` under `.opencode/skills/system-deep-loop/`.
