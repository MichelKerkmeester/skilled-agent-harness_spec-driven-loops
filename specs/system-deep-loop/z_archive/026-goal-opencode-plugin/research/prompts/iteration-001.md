...
Iteration 1: Confirm the live command filename (`goal_opencode.md` vs `goal.md`) via direct filesystem check, then grep-sweep the whole repo for `mk-goal`, `mk_goal`, `/goal `, `goal_opencode`, `recordProviderUsageLimit`, `archiveGoalStateFile`, `pruneArchive`, `sweepOrphanedActiveStates` to build the full candidate list of files mentioning the plugin, excluding the already-updated docs list.
...
- Write iteration narrative to: .opencode/specs/deep-loops/032-goal-opencode-plugin/research/iterations/iteration-001.md
- Write per-iteration delta file to: .opencode/specs/deep-loops/032-goal-opencode-plugin/research/deltas/iter-001.jsonl
...
1. **Iteration narrative markdown** at `.opencode/specs/deep-loops/032-goal-opencode-plugin/research/iterations/iteration-001.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `.opencode/specs/deep-loops/032-goal-opencode-plugin/research/deltas/iter-001.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
...
This is iteration 1 of 10 of a DOCUMENTATION-STALENESS AUDIT ONLY (read-mostly; no implementation changes).
...
- .opencode/specs/deep-loops/032-goal-opencode-plugin/research/iterations/iteration-001.md (create)
- .opencode/specs/deep-loops/032-goal-opencode-plugin/research/deep-research-state.jsonl (append-only, one JSON line)
- .opencode/specs/deep-loops/032-goal-opencode-plugin/research/deltas/iter-001.jsonl (create)
