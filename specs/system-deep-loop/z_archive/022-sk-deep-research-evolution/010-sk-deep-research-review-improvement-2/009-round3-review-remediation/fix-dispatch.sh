...
3. .opencode/skills/sk-deep-review/scripts/reduce-state.cjs: In the veto freshness check, re-validate ALL active P0/P1 findings (not just new ones from latest iteration) before emitting a stop verdict.
...
5. .opencode/skills/sk-improve-agent/scripts/trade-off-detector.cjs: Coerce numeric strings for details.iteration in getTrajectory() — the journal stores iteration as string but trajectory math needs numbers.
...
3. .opencode/skills/sk-improve-agent/scripts/scan-integration.cjs: Before every new RegExp(agentName) construction, escape regex metacharacters in the agent name string.

4. .opencode/skills/sk-improve-agent/scripts/generate-profile.cjs + scan-integration.cjs: Add path containment checks verifying targets resolve under .opencode/agents/ before processing.

5. .opencode/skills/system-spec-kit/scripts/optimizer/replay-corpus.cjs: Apply the same approved-root containment policy to saveCorpus() and loadCorpus() that buildCorpus() already uses.

6. .opencode/skills/system-spec-kit/scripts/optimizer/promote.cjs: Validate all path segments before any mkdirSync call — reject paths that traverse through symlink parents.

7. .opencode/skills/system-spec-kit/references/templates/template_mapping.md + level_specifications.md: Replace any residual sign-off wording with approval-tracking terminology.

8. .opencode/skills/system-spec-kit/references/templates/level_specifications.md: Add stakeholder tracking to the Level 3+ definition alongside approval and compliance.

9. .opencode/skills/system-spec-kit/mcp_server/lib/coverage-graph/coverage-graph-db.ts: Fix schema_version migration edge case — treat missing version row as legacy state requiring drop/recreate.
...
1. .opencode/skills/sk-deep-research/references/convergence.md: Rewrite ALL legalStop examples to the emitted blocked_stop schema with actual gate names.

2. .opencode/skills/sk-deep-research/references/loop_protocol.md: Change delta-only reducer refresh description to full-history replay as actually implemented.

3. .opencode/skills/sk-deep-review/assets/review_mode_contract.yaml: Update the reducer input model from delta to fullJSONLHistory/iterationFiles contract.
...
1. .claude/agents/deep-review.md: Sync iteration skeleton Step 5/7 from canonical .opencode/agents/deep-review.md template including findingId in claim-adjudication packets.
...
1. .opencode/skills/system-spec-kit/scripts/lib/wave-lifecycle.cjs: Require sessionId, generation, and loopType explicitly at the joinWave() API boundary. Remove the wave-join fallback default.

2. .opencode/skills/system-spec-kit/scripts/lib/wave-coordination-board.cjs: Add an exported finalizeBoard() function that transitions board.status to completed through the allowed transition path. Fix createCanonicalMergeBoard() defaults.

3. .opencode/skills/system-spec-kit/scripts/lib/coverage-graph-contradictions.cjs: Replace private session comparison functions with imports from coverage-graph-session.cjs.

4. Consolidate .opencode/skills/system-spec-kit/scripts/tests/coverage-graph-integration.vitest.ts and coverage-graph-cross-layer.vitest.ts into one canonical contract suite.

5. .opencode/skills/system-spec-kit/vitest.config.ts: Add mcp_server/tests/archive/** to exclusion pattern.
...
7. .opencode/skills/sk-deep-review/README.md + sk-deep-research/README.md + sk-improve-agent/README.md: Add version footer with changelog link.

8. .opencode/skills/sk-deep-review/scripts/reduce-state.cjs: Isolate repeatedFindings to a legacy adapter path. Remove from canonical contract init and dashboard rendering.

9. .opencode/skills/system-spec-kit/mcp_server/lib/graph/graph-metadata-parser.ts + lib/extraction/entity-extractor.ts: Consolidate entity-length policy to one shared constant (pick 100 or 80 and use everywhere).
...
1. .opencode/skills/sk-deep-review/manual_testing_playbook/03--iteration-execution-and-state-discipline/015-graph-events-review.md: Rewrite around flat event schema (dimension, file, finding, evidence, edge) instead of retired _node taxonomy. Point to active coverage-graph test sources, not archived ones.

2. .opencode/skills/sk-improve-agent/manual_testing_playbook/06--end-to-end-loop/022-mutation-coverage-graph-tracking.md: Rename scenario around actual flat coverage artifact (mutations, exhausted, trajectory) instead of node/edge graph model.

3. .opencode/skills/sk-improve-agent/manual_testing_playbook/07--runtime-truth/027-resume-continuation.md: Change verification to use archive separation and iteration reset instead of asserting lineageMode/generation fields the runtime doesn't emit. Fix any corrupted prompt text.

4. .opencode/skills/sk-improve-agent/manual_testing_playbook/07--runtime-truth/030-benchmark-stability-surface.md: Reword stability description from all deltas to last-N-samples staying within per-dimension score range.
