...
FOCUS_AREAS=(
  ...
  "Phase 002 coverage-graph SQLite: session isolation, namespace scoping, SQL injection surface in .opencode/skills/system-spec-kit/mcp_server/lib/coverage-graph/coverage-graph-db.ts and all 4 MCP handlers in .opencode/skills/system-spec-kit/mcp_server/handlers/coverage-graph/. Check composite primary keys, parameterized queries, and input validation."
  ...
  "Phase 001-002 code structure: review 4 CJS shared libraries in .opencode/skills/system-spec-kit/scripts/lib/coverage-graph-*.cjs for module boundaries, error handling, function complexity, and export surface area. Check vitest coverage in scripts/tests/coverage-graph-*.vitest.ts files."

  # Round 2: Phase 003-004 scale and optimization
  "Phase 003 wave-executor: fan-out/join logic, segment identity determinism, activation gates in .opencode/skills/system-spec-kit/scripts/lib/wave-*.cjs (5 modules). Check wave-lifecycle.cjs for state transitions, wave-segment-planner.cjs for deterministic ordering, wave-convergence.cjs for per-segment convergence."
  "Phase 004 offline-optimizer: replay corpus extraction, config search bounds, audit trail integrity in .opencode/skills/system-spec-kit/scripts/optimizer/replay-corpus.cjs and replay-runner.cjs. Check for path traversal in corpus loading, bounds validation in config search, and advisory-only promotion gate enforcement."
  ...
  "Phase 005 agent-improver alignment: verify .opencode/skills/sk-improve-agent/scripts/ (13 CJS modules) correctly implement stop-reason taxonomy, journal wiring, and advisory optimizer contract. Focus on improvement-journal.cjs, mutation-coverage.cjs, trade-off-detector.cjs, candidate-lineage.cjs, benchmark-stability.cjs."
  "Phase 005-006 MCP handler auth: check all coverage-graph MCP handlers for consistent parameter validation, error responses, and namespace enforcement. Verify .opencode/skills/system-spec-kit/mcp_server/handlers/coverage-graph/index.ts registration and tool schema correctness."
  ...
  "Phase 005-006 test quality: review .opencode/skills/sk-improve-agent/scripts/tests/ (5 vitest files) and .opencode/skills/system-spec-kit/scripts/tests/coverage-graph-integration.vitest.ts plus coverage-graph-stress.vitest.ts for assertion quality, edge cases, and coverage of the documented contract."

  # Round 4: Phase 007-008 closeout and runtime truth
  "Phase 007-008 regression risk: check .opencode/skills/sk-deep-review/scripts/reduce-state.cjs for fail-closed corruption handling, claim-adjudication gate implementation, and the split between persistent same-severity findings and severity changes. Verify against .opencode/skills/sk-deep-review/references/state_format.md §9 and loop_protocol.md §Step 4a."
  "Phase 007-008 database migrations: verify .opencode/skills/system-spec-kit/mcp_server/lib/coverage-graph/coverage-graph-db.ts v2 schema migration (drop-and-recreate) is safe, composite primary key correctness, and that .opencode/skills/system-spec-kit/scripts/tests/session-isolation.vitest.ts covers shared-ID collision regression."
  ...
  "Phase 007-008 cross-cutting quality: review .opencode/agents/deep-review.md iteration skeleton alignment with reduce-state.cjs:186, review_dimensions_json pre-serialization in both review workflows, and Signal 3 convergence vote correctness. Check .opencode/agents/deep-research.md for matching runtime truth."
  ...
  "Cross-phase command workflow integrity: review all 6 YAML workflow assets in .opencode/commands/speckit/assets/ and 2 in .opencode/commands/deep/assets/. Verify step sequences, variable references, conditional branches, and error handling paths match the SKILL.md documented workflows."
