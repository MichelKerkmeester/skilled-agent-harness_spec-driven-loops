...
description: "Replace the 7 command files' self-classification gate with a deterministic dispatch-context check, and land the ai-council identity fix across orchestrate-topic.cjs and deep_ai-council_auto.yaml in the same change."
trigger_phrases:
  - "implementation"
  - "plan"
  - "mode d gate fix"
...
- [x] Current state of all 7 command files' Phase-0 blocks confirmed structurally identical (not byte-identical — each substitutes its own loop name/command).
...
### Definition of Done
- [x] All 7 command files' Phase-0 block replaced with the deterministic check.
...
- Same-class producers: all 7 command files share the identical Phase-0 block text — confirm via diff before editing to catch any that have already drifted.
...
### Phase 1: Setup
- [x] Read all 7 command files' Phase-0 blocks; confirmed structurally identical (not byte-identical).
...
- [x] Replaced the Phase-0 block in all 7 command files with the deterministic check.
...
- [x] grep all 7 files: zero remaining self-classification prose confirmed.
...
2. Revert any of the 7 command files independently if only the Mode-D fix regresses.
