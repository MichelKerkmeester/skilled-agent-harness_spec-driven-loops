...
Reference mechanism: kasper computes `ageDays` from `Date.now() - e.timestamp`, then applies `weight = 0.5 ** (ageDays / decayDays)` when `decayDays > 0`; weakness and strength frequency maps accumulate that fractional weight before ranking. [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:74`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:79`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:82`]
...
Reference mechanism: kasper keeps `total_sessions` and `avg_score` raw while applying decay only to weakness/strength frequency evidence. The helper's denominator for average score is still unweighted session count, but the stale weakness ranking fades. [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:58`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:69`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:90`]
...
Reference mechanism: kasper exposes `weakness_decay_days`, documents `0` as decay-disabled, and defaults it to 30 days. [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/config.ts:227`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/README.md:82`]
...
Reference mechanism: the tested `computeStats` helper applies decay, and the regression test proves an old repeated weakness drops below a recent issue. [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/tests/state.test.ts:178`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/tests/state.test.ts:203`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/tests/state.test.ts:209`]

Reference caveat: the current running aggregate path rebuilds and increments weakness frequencies with weight `1`, then ranks direct counts. That means the formula is reliable as a mechanism, but the live wiring should be treated as a cautionary example. [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:690`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:694`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:740`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:744`] [SOURCE: `.opencode/specs/deep-loops/030-agent-loops-improved/external/kasper/src/state.ts:804`]
