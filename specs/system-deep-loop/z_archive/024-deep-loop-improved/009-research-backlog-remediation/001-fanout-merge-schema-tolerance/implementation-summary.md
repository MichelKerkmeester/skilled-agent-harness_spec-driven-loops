...
| Field | Value |
|-------|-------|
| **Spec Folder** | `system-deep-loop/024-deep-loop-improved/009-research-backlog-remediation/001-fanout-merge-schema-tolerance` |
...
3. **Real-data validation (the actual bug reproduction case)**: re-ran `node fanout-merge.cjs --loop-type research --artifact-dir .opencode/specs/system-deep-loop/024-deep-loop-improved/research` against the real glm/gpt lineage registries from the 2026-07-01 deep-research fan-out. stderr now emits `{"type":"schema_mismatch",...,"lineage":"glm","message":"Registry uses non-canonical key \"findings\" instead of \"keyFindings\"; coerced 18 entries."}`. The on-disk `research/deep-research-findings-registry.json` now shows **26 `keyFindings`** (previously silently 8) and carries the `schema_mismatch` field. This matches `research/research.md`'s own reported true total (18 glm + 8 gpt = 26).
<!-- /ANCHOR:verification -->
