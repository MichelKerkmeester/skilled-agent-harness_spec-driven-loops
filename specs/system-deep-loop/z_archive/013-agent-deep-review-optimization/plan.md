...
_memory:
  continuity:
    packet_pointer: "system-deep-loop/z_archive/013-agent-deep-review-optimization"
...
`@deep-review` is consumed via a specific spec_kit command flow. Improvements ship as canonical `.opencode/agents/deep-review.md` updates plus 3 mirror updates. Score profile is dynamic (generated per-target via `generate-profile.cjs`).
<!-- /ANCHOR:architecture -->
...
| **P2** Dispatch | `/deep:start-agent-improvement-loop .opencode/agents/deep-review.md` | `<packet>/candidate.md` |
...
`rollback-candidate.cjs --target=.opencode/agents/deep-review.md --backup=<pre-promote-backup>` reverts canonical. Mirror revert via git revert of the sync commit. 
<!-- /ANCHOR:rollback -->
