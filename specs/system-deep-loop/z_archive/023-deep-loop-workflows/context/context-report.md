...
The system is a **partially-realized two-skill split**: one shared backend (`deep-loop-runtime`) plus five sibling workflow skills, all dispatched through 7 `/deep:*` commands and per-runtime native agents.
...
- **Council orchestration** (`orchestrate-*.cjs`, `persist-artifacts.cjs`, `advise-council-completion.cjs`) and **improvement lanes** (the entire `agent-improvement/`, `model-benchmark/`, and `skill-benchmark/` subtrees + `loop-host.cjs` mode-router).
...
### A. Commands (7 commands / 12 YAML + 6 presentation.txt = 18 asset files)

- **Markdown routers**: thin; only **3 literal skill-path strings in 2 `.md` files** need rewriting (start-research-loop.md:89; start-skill-benchmark-loop.md:36,76). **1 frontmatter `skill: deep-improvement`** value (start-skill-benchmark-loop.md:3) → `skill: deep-loop-workflows`.
...
- **deep-improvement backs 3 commands** → largest rename footprint; phase its 3-command cluster as one migration phase.
...
- **Repoint volume per agent** (if agents keep names): deep-context = **1 string** (caller-table name only, no Read() loads); deep-research = 1 path (loop_protocol.md) ×3; deep-review = 2 self-paths + LEAVE the out-of-set `sk-code-review/references/review_core.md`; deep-improvement = skill-name string + verify 3-lane script paths; **ai-council = ~9 paths ×3 ≈ 27 edits** (executes `persist-artifacts.cjs` from literal paths — highest effort, lockstep contract with `output_schema.md`).
...
- **No common category schema**: lifecycle (research/review) vs sweep (context) vs 3-lane (improvement) vs council taxonomy → realistic shape is per-MODE top-level sections each keeping its own subtree, NOT a unified category list.
...
1. **Single multi-mode SKILL.md vs thin SKILL.md + per-mode reference packets?** deep-improvement already proves a 564-line multi-lane SKILL.md works, but the merged skill is N+2 modes. Decide: one SKILL.md with mode sections + smart-router (deep-improvement §2 has a ~120-line router with 10 weighted intents), OR a thin hub SKILL.md routing to `references/<mode>/`. Resolve the keyword-collision risk: deep-improvement's generic `benchmark` intent already routes to 3 lanes; unioning 5 skills' intents needs de-confliction.
...
10. **Hoist deep-improvement's frozen stop-reason/journal taxonomy into the runtime as the shared convergence/journal contract for all modes?** It overlaps research/review convergence vocabulary. Risk: a conflicting second contract if the runtime already owns one — confirm runtime ownership.
...
12. **How does Lane C fit as a mode?** Lane C is diagnostic-only, benchmarks a skill, and has a 14-script subtree with self-referential risk. Decide whether it remains a first-class mode or becomes a nested sub-mode.
