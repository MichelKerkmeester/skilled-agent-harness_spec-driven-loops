...
For `/deep:start-agent-improvement-loop`, the command markdown is the user-facing lifecycle authority: each loop scans integration, dispatches `@improve-agent`, scores, benchmarks, appends the ledger, reduces state, and checks stop conditions [SOURCE: `.opencode/commands/deep/start-agent-improvement-loop.md:266-280`]. The YAML is the executable producer for runtime paths, helper invocations, candidate/scoring/journal events, and reducer calls under `{spec_folder}/improvement` [SOURCE: `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_auto.yaml:96-110`, `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_auto.yaml:150-198`]. The reducer is the structured consumer authority for legal-stop visibility because it only populates `latestLegalStop.gateResults` from `details.gateResults` [SOURCE: `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:173-218`].
