...
The 059 Coder Acceptance Rubric is a useful ancestor but not the right direct rubric for `/deep:start-agent-improvement-loop`. Its five dimensions are **Correctness**, **Scope-Adherence**, **Verification-Evidence**, **Stack-Pattern-Compliance**, and **Integration**; they grade whether an implementation task can return `DONE`, with P0/P1 blockers overriding score averages [SOURCE: `.opencode/agents/code.md:128-140`, `.opencode/agents/code.md:142-157`]. That matches `@code` because the 059 Call B literally prepended `.opencode/agents/code.md`; the stress harness was testing an agent-body envelope whose discipline lived in that body [SOURCE: `.opencode/specs/skilled-agent-orchestration/z_archive/047-agent-implement-code/test-report.md:81-98`].

`/deep:start-agent-improvement-loop` needs a different rubric because the unit under test is a command-owned pipeline. The command says its loop scans integration surfaces, dispatches `@improve-agent`, scores the candidate, runs benchmark fixtures, appends ledger data, reduces state, and checks stop conditions [SOURCE: `.opencode/commands/deep/start-agent-improvement-loop.md:266-280`]. The skill says the legal-stop contract requires five gate bundles and may not claim `converged` unless all pass [SOURCE: `.opencode/skills/sk-improve-agent/SKILL.md:279-292`]. The auto YAML is the executable evidence producer for helper calls and journal events [SOURCE: `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_auto.yaml:125-143`, `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_auto.yaml:150-198`].
...
This preflight is stronger than iteration 1's list because it makes "natural entry point" and "consumer check" blocking. Those two gates directly target the R1 failure and the newly observed legal-stop producer/consumer seam: YAML emits `legal_stop_evaluated` with flat gate fields, while the reducer's latest legal-stop summary only reads `details.gateResults` [SOURCE: `.opencode/commands/deep/assets/deep_start-agent-improvement-loop_auto.yaml:192-198`, `.opencode/skills/sk-improve-agent/scripts/reduce-state.cjs:213-218`].
...
2. **Baseline/current scoring output** is complete enough to test. The scorer now computes baseline score, total/per-dimension delta, `thresholdDelta`, and a split between `candidate-better`, `candidate-acceptable`, and `keep-baseline` [SOURCE: `.opencode/skills/sk-improve-agent/scripts/score-candidate.cjs:376-450`].
