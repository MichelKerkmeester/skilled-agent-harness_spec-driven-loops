...
Iteration: 1 of 5
Dimension: traceability (primary), correctness (secondary) — Angle 6
...
Review Scope Files: .opencode/commands/deep/assets/deep-review-auto.yaml, .opencode/commands/deep/assets/deep-review-confirm.yaml, the presentation assets they render, the prompt packs and runtime counterparts they reference (see strategy §13 Angle 6 pointer); the four in-scope trees bound the blast radius (system-deep-loop, sk-code, cli-external-orchestration, sk-doc)
Prior Findings: P0=0 P1=0 P2=0
...
`{artifact_dir}/iterations/iteration-001.md` MUST end with exactly one of these plain-text lines as the **absolute final line** (no trailing whitespace, no variation), and this iteration MUST emit exactly one parseable verdict:
...
- Write iteration narrative to: {artifact_dir}/iterations/iteration-001.md
- Write per-iteration delta file to: {artifact_dir}/deltas/iter-001.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `{artifact_dir}/iterations/iteration-001.md`, this iteration's narrative markdown
  - `{artifact_dir}/deltas/iter-001.jsonl`, this iteration's delta JSONL
  - the strategy file (in-place updates only)
  - the lane's recorded event sidecar `logs/iter-001-events.jsonl` (this lane's recorded mechanism for persisted iteration events; the state log itself is written by the lane's recorded legacy-direct writer — see strategy §13 deviations)
