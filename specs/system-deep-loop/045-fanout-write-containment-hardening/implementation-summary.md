...
| Manual uncommitted-packet worktree run | Not run. The phase is built and off by default; producing the evidence the default flip needs means one full fan-out with the flag on, reading the created, published and degraded events off the ledger |<!-- /ANCHOR:verification -->
...
8. **The staging-residue sweep exists but is not called in production** A publisher that dies mid-transaction can still leave staging directories behind. The function is built and tested; wiring it is an open task.
