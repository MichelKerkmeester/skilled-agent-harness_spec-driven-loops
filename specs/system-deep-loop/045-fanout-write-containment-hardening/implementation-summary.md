...
| Manual uncommitted-packet worktree run | Not run. The phase is built and off by default; producing the evidence the default flip needs means one full fan-out with the flag on, reading the created, published and degraded events off the ledger |
<!-- /ANCHOR:verification -->
