...
13. **The forced-depth rejection has recurred, with its evidence kept** A LUNA lane completed three iterations and was rejected for duplicate state records. The log held each record twice: once with millisecond timestamps from the append gateway, once with round, backdated ones the model wrote directly, against the agent contract. The validator counts and rejects where it should deduplicate by iteration and prefer the gateway's records. The lineage directory is retained under `research/lineages/`.
