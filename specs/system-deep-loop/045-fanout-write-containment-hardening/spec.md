...
| REQ-005 | Each CLI lineage runs in its own git worktree created from HEAD for the run, so containment inside that worktree is exact and the main checkout is never written by a lane. The packet specifies worktree location, naming, cleanup, behaviour when the target packet is not yet committed, how the lineage directory is copied back into the main checkout, and how every path handed to the executor is rewritten. The choice between sk-git's allocator and a detached ephemeral worktree is recorded as a decision with its alternative. |
...
| Dependency | Gitignored `node_modules` and `dist` inside a fresh worktree | A bare worktree makes the spec-kit and deep-loop generators crash or silently no-op, which is sk-git's own documented trap | Mirror the launch wrapper: symlink the shared `node_modules` and `dist` into each lineage worktree at creation |
