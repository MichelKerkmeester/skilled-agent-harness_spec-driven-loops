...
| AC-009 | REQ-005 | Given a fan-out with the worktree option on and a packet that is not yet committed, When the run completes, Then every lineage directory is published into the main checkout and no worktree **of this run** remains except those in a retained state | `runtime/tests/unit/fanout-run.vitest.ts` asserts no worktree of this run remains after the lane is copied back, scoped to this run rather than to the shared prefix. Suite: 156 files, 2643 passed | Met | - |
...
| AC-012 | REQ-005 | Given run A with live lineages, When run B starts and performs its startup reclamation, Then every one of A's worktrees survives with byte-identical contents and B records a keep decision naming the reason for each | `runtime/tests/unit/fanout-run.vitest.ts` plants a live peer run's worktree and asserts it survives byte-identical with its keep decision and reason on the ledger, paired with an otherwise identical peer whose owner cannot answer being reclaimed. Survival alone would pass for a pass that never considered the peer; the pair pins the liveness check | Met | - |
| AC-013 | REQ-005 | Given a completed lane, When its lineage directory is published into the main checkout, Then the published contents hash-equal the worktree source | `runtime/tests/unit/fanout-run.vitest.ts` re-hashes every published file against the entry the manifest recorded for it, because a removed worktree proves nothing about whether its contents arrived | Met | - |
| AC-014 | REQ-005 | Given a lane running inside its own worktree, When it writes outside its lineage directory but inside that worktree, Then containment still reports a violation naming the worktree root; and when a neighbouring session writes in the main checkout during that run, Then no violation is attributed to the lane and those files stay byte-identical | `runtime/tests/unit/fanout-run.vitest.ts` asserts both halves in one run: the lane writing outside its lineage directory inside its own worktree is still caught there, and a neighbour writing to the main checkout mid-lane through a ready-and-go handshake is neither attributed nor altered. The handshake matters because a write made before dispatch lands in the baseline and is ignored by construction | Met | - |
| AC-015 | REQ-005 | Given a lane whose worktree has diverged from the main checkout, When code inside the lane imports a first-party workspace package, Then the bytes loaded are the lane's own and not the main checkout's | `runtime/tests/unit/worktree-lifecycle.vitest.ts` spawns a child process rooted in the lane and asserts on the value the module exported, with a negative control at the main checkout proving the probe can distinguish the two. Asserted on bytes rather than on the resolved path, because a path is spellable: preserved symlinks report the lane's spelling for a file the main checkout owns. Verified end to end against this repository as well, for all four workspace self-links and both consumer trees | Met | - |
| AC-016 | REQ-005 | Given a generator the runner invokes after a run settles, When the build directory it lives in is reached through a link, Then the generator still performs its work rather than exiting 0 having done nothing | `runtime/tests/unit/fanout-run.vitest.ts` asserts the planned invocation path is canonical and does not retain the linked spelling, and that the checkout to act on still travels in the arguments. The defect was reproduced first: the same generator, same arguments, wrote its output from the main checkout and produced nothing from a worktree whose build was linked, exiting 0 both times | Met | - |
