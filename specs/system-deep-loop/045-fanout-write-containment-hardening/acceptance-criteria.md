...
| AC-009 | REQ-005 | Given a fan-out with the worktree option on and a packet that is not yet committed, When the run completes, Then every lineage directory is published into the main checkout and no worktree **of this run** remains except those in a retained state | A manual run recorded in `implementation-summary.md`, plus a Vitest case over the worktree lifecycle helpers asserting create, seed, publish and run-scoped removal. Teardown is scoped to this run because a sweep that removes every worktree satisfies an unscoped criterion perfectly while destroying a peer's | Unmet | - |
