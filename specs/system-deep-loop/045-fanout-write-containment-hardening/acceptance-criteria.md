...
| AC-009 | REQ-005 | Given a fan-out with the worktree option on and a packet that is not yet committed, When the run completes, Then every lineage directory is published into the main checkout and no worktree **of this run** remains except those in a retained state | `runtime/tests/unit/fanout-run.vitest.ts:4193` runs the lane in its own worktree and asserts no worktree of this run remains after publication, scoped to this run rather than to the shared prefix.  | Met | - |
...
| AC-017 | REQ-007 | Given a default-configured fan-out with no worktree flag, When its lanes run, Then each lane runs in its own worktree and the run summary reports the per-attempt isolation tally; and when a lane cannot be isolated, Then the attempt is counted degraded and the run still completes | `runtime/tests/unit/fanout-run.vitest.ts:4145` spawns with no flag and asserts summary `isolation { enabled: true, isolated: 1, degraded: 0 }` alongside a tree created under the configured worktree base; `:4177` pins the explicit-off case as `{ enabled: false, isolated: 0, degraded: 0 }`; `:4312` pins a lane whose tree could not be made as `{ enabled: true, isolated: 0, degraded: 1 }`, as does the interrupted-run summary at `:3130`; `runtime/tests/unit/executor-config.vitest.ts:450` covers the schema default, the partial containment object and both opt-out shapes | Met | - |
