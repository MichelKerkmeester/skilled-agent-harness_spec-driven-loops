...
- [ ] A review lane fixture with open findings in its registry and finding rows in its deltas raises no empty-registry warning, and a research lane with none still does
- [ ] A fan-out config carrying containment.worktrees is rejected with a message naming the key
- [ ] The retained review run's two lineage_registry_empty events are shown to be false positives by re-running the check over them
