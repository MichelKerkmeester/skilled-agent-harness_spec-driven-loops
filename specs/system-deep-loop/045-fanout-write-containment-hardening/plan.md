...
- **Quarantine writer**: writes `containment/quarantine/<iteration>[-attempt-<n>]/` inside the lineage directory, one directory per containment pass that is never overwritten by a later pass, holding `manifest.json` (one entry per path with its status code, kind, blob hash and whether content was truncated), `content/<repo-relative path>` (the bytes as the lane left them), `head.patch` (the diff against HEAD) and, when a baseline exists for the path, `baseline.patch`. The combined revert patch lives under the same pass directory, in `containment-out-of-scope/` under preserve and `containment-reverted/` only when restore mode actually reverts.
