...
1. Leaf 1 — pi extension isolation: add the three `--no-*` flags to the read-only pi builder; update the exact-arg assertions in the fan-out, model-benchmark, and ai-council suites.
2. Leaf 2 — combo coverage matrix: enumerate every kind × model × mode; assert construction or log a skip with a reason.
3. Leaf 3 — cursor/devin/MCP isolation: neutralize repo hooks + unapproved MCP for read-only cursor/pi; isolate devin config; verify hermetic against ambient config.
<!-- /ANCHOR:phases -->
