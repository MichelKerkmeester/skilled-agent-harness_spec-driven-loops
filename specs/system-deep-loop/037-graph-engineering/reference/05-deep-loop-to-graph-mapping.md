...
**Core Principle**: Graph engineering is an orchestration migration, not a big-bang rewrite; authority remains with the governed ledger, gateway, sealed artifacts, receipts, fingerprints, and adjudication boundary.

This document defines the hybrid target, maps deep-loop concepts to control and work graphs, preserves the existing authority model, and specifies parity gates and a staged migration path.

The recommended target is hybrid:
...
- Control topology and per-run execution topology are distinct.
- Graph state supports resumability; the evidence ledger remains the authority and audit plane.
- Migration promotion requires exact parity and operator gates.
...
### 2.3 Iteration focus as a typed node
...
- The registry remains the routing authority.
- The adapter preserves typed writes, citations, and reducer ownership.
- Mode subgraphs are a target design, not an assertion of existing production implementations.

## 3. STATE, REDUCERS, AND EVENT BOUNDARIES

### 3.1 Proposed research graph state
...
### 3.3 Knowledge-node and relation vocabulary
...
- Typed execution state is distinct from the knowledge graph it emits.
- Reducers remain independently testable and comparison-oriented.
- Projection omissions cannot alter convergence authority.

## 4. CONVERGENCE AS GUARDED ROUTING
...
### 4.2 Preserve the inline three-signal contract
...
### 4.3 Quality and convergence parity assertions
...
- Guarded routing represents the existing convergence contract; it does not replace it.
- The inline vote, quality gates, and minimum requirements remain authoritative.
- Any parity mismatch blocks promotion.

## 5. THE AUTHORITY BOUNDARY

### 5.1 Fail-closed transition authorization stays in the ledger plane
...
### 5.2 Sealed receipts and replay fingerprints stay outside mutable graph state
...
### 5.3 Blinded adjudication stays isolated
...
### 5.4 Why the authority boundary is non-negotiable
...
- The graph may request and explain transitions, but only the gateway authorizes durable effects.
- Receipts and replay fingerprints remain outside mutable graph authority.
- Isolated branches do not prove adjudication properties without gateway-mediated evidence.

## 6. FAN-OUT, FAN-IN, AND SERIALIZATION

### 6.1 Fan-out: parallel branches, not shared authority
...
### 6.2 Fan-in: deterministic and contradiction-preserving
...
### 6.3 Loop-lock is serialization, not topology
...
- Flat-pool branch semantics and filesystem-enforced lineages remain explicit.
- Fan-in must be deterministic and preserve contradictions.
- Scheduler topology does not replace lock fencing or serialization.

## 7. EVIDENCE LEDGER BESIDE CHECKPOINTS
...
- Checkpoints support resume; the ledger records durable transition history.
- Event bridges expose enough independent data for parity reconstruction.
- Receipt and fence references are supplied by the authority boundary.

## 8. HYBRID TARGET ARCHITECTURE
...
- Stable control graphs are governed; work graphs are run-specific.
- The first adapter decomposes research into narrow, independently checkable subgraphs.
- Ephemeral graph state cannot replace ledger authority.

## 9. FOUR-PHASE MIGRATION PATH

### 9.1 Phase A — additive-dark research adapter
...
### 9.2 Phase B — exact shadow parity
...
### 9.3 Phase C — per-mode cutover behind 036 discipline
...
### 9.4 Phase D — convergence-graph enrichment
...
- Phase A is additive-dark and DB-independent.
- Phase B requires exact parity, including failure behavior and graph-off execution.
- Phase C is gated by 036 authority preconditions.
- Phase D is optional enrichment and cannot authorize early stop.

## 10. PARITY FIXTURE AND OPERATOR RUNBOOK

### 10.1 Deterministic fixture corpus
...
### 10.3 Operator decision sequence
...
- The fixture corpus makes artifacts, ordering, replay, and failure behavior deterministic.
- Unsupported dependencies fail or block a gate rather than becoming `N/A`.
- Promotion and rollback are explicit operator decisions.

## 11. CURRENT BLOCKERS AND OWNERSHIP

### 11.1 014 cutover blockers
...
|---|---|---|
| **F001 identity resolution** | Production gateway `identityResolver` wiring remains optional/conditional. | No graph transition may become authoritative until identity is mandatory and bound. |
...
| **024 append fencing** | Core fence is recorded as genuinely unbuilt; the `appendAuthorized` primitive remains public in the cited current evidence. | No graph adapter may bypass or become the durable append authority. |
...
### 11.2 Coverage-graph database ABI mismatch
...
### 11.3 034 and 036–046 ownership gap
...
- Authority cutover is not ready because F001, F002, F005, 022 residual evidence, and 024 remain relevant.
- The ABI mismatch blocks projection work, not Phase A/B correctness proof.
- Ownership records must be current and owner-approved before closeout claims.

## 12. GOTCHAS AND REVIEW CHECKLIST
...
- Review must reject authority conflation, nondeterministic joins, and graph-only early stops.
- Every change needs explicit ownership, parity evidence, operator gating, and rollback.
