...
Questions: 5/5 answered; residuals: direct runtime verification, fixture execution, 036-046 ownership | Last focus: cross-check (timeout)
Last 2 ratios: 0.90 -> 0.45 | Stuck count: 1
Resource map: resource-map.md not present; skipping coverage gate.
Memory context refresh: none loaded yet.
Next focus: when-not-to-use validation + direct verification of the cross-check residuals.
...
Iteration: 17 of 20
Focus Area: When-not-to-use validation + residual verification: (1) from the corpus articles and LangGraph/GraphARC evidence, extract the concrete when-NOT-to-use-graphs criteria (simple linear flows, low state, single-pass tasks, latency/overhead, no branching needed) and validate them against OUR loop: which of our seven modes and which iteration phases would lose more than they gain from graph structure; (2) complete the residual direct checks from iteration 16: read transition-authorization-gateway.ts (identityResolver wiring), loop-lock.ts (F005 partial-record window), and inventory graph-engineering-master/dist/ + graph-engineering/ — cite file:line; (3) record CONTRADICTS/SUPERSEDES edges for anything that overturns prior findings
...
- Write iteration narrative to: specs/system-deep-loop/037-graph-engineering/research/iterations/iteration-017.md
- Write per-iteration delta file to: specs/system-deep-loop/037-graph-engineering/research/deltas/iter-017.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `specs/system-deep-loop/037-graph-engineering/research/iterations/iteration-017.md`, this iteration's narrative markdown
  - `specs/system-deep-loop/037-graph-engineering/research/deep-research-state.jsonl`, append-only JSONL state log
  - `specs/system-deep-loop/037-graph-engineering/research/deltas/iter-017.jsonl`, this iteration's delta JSONL
...
1. **Iteration narrative markdown** at `specs/system-deep-loop/037-graph-engineering/research/iterations/iteration-017.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
