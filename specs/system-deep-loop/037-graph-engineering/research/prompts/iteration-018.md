...
Questions: 5/5 answered; residuals: fixture execution, F001/F005 remediation, 036-046 ownership | Last focus: when-not-to-use + residuals
Last 2 ratios: 0.45 -> 0.90 | Stuck count: 0
Resource map: resource-map.md not present; skipping coverage gate.
Memory context refresh: none loaded yet.
Next focus: migration path and sequencing.
...
Iteration: 18 of 20
Focus Area: Migration path and sequencing: (1) design the staged path to land graph-engineering-based loops aligned with our constraints — phase A: research-mode graph adapter in additive-dark shadow (typed graph state over existing JSONL, graphEvents as the bridge, DB-independent), phase B: shadow-parity gates (iteration artifacts, reducer output, convergence decisions identical), phase C: per-mode cutover behind the 036 gateway discipline (one mode at a time, rollback window, operator gate), phase D: convergence-graph enrichment (contradiction/coverage/source-diversity signals) once the coverage-graph DB dependency is resolved; (2) sequence against the 036 epic: what must land first (014 cutover, 024 fencing) and what can proceed in parallel (adapter research/ design); (3) name the operator gates and rollback per phase; cite the 036 handover sequencing and our iteration-005/006/012 findings
...
- Write iteration narrative to: specs/system-deep-loop/037-graph-engineering/research/iterations/iteration-018.md
- Write per-iteration delta file to: specs/system-deep-loop/037-graph-engineering/research/deltas/iter-018.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `specs/system-deep-loop/037-graph-engineering/research/iterations/iteration-018.md`, this iteration's narrative markdown
  - `specs/system-deep-loop/037-graph-engineering/research/deep-research-state.jsonl`, append-only JSONL state log
  - `specs/system-deep-loop/037-graph-engineering/research/deltas/iter-018.jsonl`, this iteration's delta JSONL
...
1. **Iteration narrative markdown** at `specs/system-deep-loop/037-graph-engineering/research/iterations/iteration-018.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
