...
4. **Correction (added later, by measurement).** The grading note above — that confirm-first grading found every other authority edge already implemented per mode — does not hold. The `rollback_pending -> legacy_authoritative` restoration does exist, but nothing writes `cutover_ready`, so the readiness *gate* was mistaken for a transition that *produces* the state. Because this packet's forward flip requires `cutover_ready`, it cannot fire in production. Measured and machine-confirmed in `specs/system-deep-loop/036-deep-loop-innovation/012-runtime-enablement/006-enablement-closeout/scratch/claim-sweep.md`. Nothing else here is affected: the flip mechanism is built and unit-verified, and limitation 3 above was already accurate.
<!-- /ANCHOR:limitations -->
