---
title: "Implementation Summary: Agent Improvement Rollback and Mode Gate"
description: "Delivered the additive-dark Agent Improvement migration gate and rollback switch as an extension of the shared deep-improvement-common gate, with gateway-re-derived verdicts and never-throw typed denials."
trigger_phrases:
  - "Agent Improvement rollback gate implementation"
  - "agent improvement mode migration gate"
  - "agent improvement rollback switch"
...
---
# Implementation Summary: Agent Improvement Rollback and Mode Gate
...
The runtime now exposes the Agent Improvement mode-migration gate and rollback switch over the landed event schema, reducers, sealed
...
agent-improvement certificate, parity, sealed, and lifecycle evidence gates on top. It consumes the shadow-parity receipt as required
...
| File | Action | Purpose |
|---|---|---|
| `runtime/lib/agent-improvement-rollback-gate/mode-gate.ts` | Created | Gateway-re-derived migration verdicts layered on the shared common gate |
| `runtime/lib/agent-improvement-rollback-gate/rollback-switch.ts` | Created | Rollback window evaluation, re-derived-certificate anchor cross-check, fence supersession |
| `runtime/lib/agent-improvement-rollback-gate/types.ts` | Created | Lane request, verdict, window, and denial contracts extending the common types |
| `runtime/lib/agent-improvement-rollback-gate/index.ts` | Created | Stable public exports |
| `runtime/tests/unit/agent-improvement-rollback-gate.vitest.ts` | Created | Re-derivation, field-binding, forged-receipt, window, supersession, and never-throw tests |
...
the already-verified shared base, and adds the agent-improvement-specific evidence gates. Every consequential request field is
