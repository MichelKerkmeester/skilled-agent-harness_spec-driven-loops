...
trigger_phrases:
  - "Deep AI Council retained count boundary"
  - "rollback count authorization binding"
  - "phase-014 deletion verification wiring"
  - "Deep AI Council lifecycle semantic binding"
  - "Deep AI Council health aggregate authenticity boundary"
  - "Deep AI Council complete evidence authentication audit"
  - "Deep AI Council rollback request field binding audit"
...
_memory:
  continuity:
    ...
    recent_action: "Mirrored seven authenticated substrate boundaries"
    next_safe_action: "Correlate bounded evidence before any authority decision"
    blockers: []
    key_files:
    ...
    answered_questions:
      ...
      - "The Deep AI Council ledger writer has one fixed canonical protected-resource identity in this leaf."
...
| **Status** | Accepted |
| **Date** | 2026-07-28 |
| **Deciders** | Deep AI Council mode-gate implementation owner |
...
|---|---|
| `schemaVersion` | Implementation constant `DEEP_AI_COUNCIL_ROLLBACK_GATE_SCHEMA_VERSION` |
| `certificateKind` | Implementation constant `mode-migration-readiness` |
| `mode` | Implementation constant `deep-ai-council`; parity and rollback verifiers independently require the same mode |
...
The lifecycle matrix requires ten unique rows and authenticates each exact `(eventDigest, receiptDigest)` identity against ready parity receipts, verified transition receipts, or verified sealed artifacts. That proves evidence identity and uniqueness. It does not prove that a cited identity originated from the behavior named by the row's `kind` label.

The Council fixture vocabulary names normal completion, multi-round deliberation, seat timeout, seat error, unresolved contradiction, maximum-round non-convergence, partial artifact persistence, rollback, resume boundaries, and blinded adjudication. The available transition receipts and sealed-artifact lifecycles authenticate enough distinct evidence identities, but they do not provide a one-to-one semantic proof for every fixture label. Binding labels to unrelated transition or artifact kinds would therefore invent evidence semantics.

### Decision

Accept the same boundary as the golden gate: every lifecycle row must use a distinct authenticated evidence identity, but fixture labels are not semantically bound to dedicated behavior evidence where the substrate supplies no such relation. Do not infer a per-kind mapping from unrelated transition, artifact, or shadow-stage labels.
...
|---|---|
| Parity manifest | Parsed parity receipts are verified against the manifest; required Deep AI Council case IDs must equal receipt fixture IDs |
| Parity mode-gate input | `parseDeepAiCouncilModeGateInput` verifies its closed shape and digest; BASE and manifest digest are cross-checked |
| Parity receipts | `parseDeepAiCouncilParityReceipt` verifies receipt and parity-certificate structure; every stream/attestation pair must match a real allowed authorization-audit entry |
...
| Sealed store and bindings | `readDeepAiCouncilArtifact` verifies every binding, descriptor, content digest, and required lifecycle artifact |
| Certificate verification input | `verifyDeepAiCouncilCertificateOffline` verifies receipts, artifact bindings, and deterministic replay; parsed receipt and event facts cross-check epoch and envelope version |
...
| Resume evidence | Legacy and ledger must have equal canonical semantic digests over decision constants and dispositions, complete compatibility/branch/effect structures, all Council invalidation fields, the complete lease, receipt and artifact digest sets, certificate and resume fingerprints, event-tail digest, and fresh-projection fingerprint. Arrays are sorted by canonical stable keys. Path-local decision IDs/digests, branch attempt IDs, and explanatory `decisionReason` prose are excluded because the two adapters may generate different envelopes or wording for the same action. Both sides still pass the Council migration-ready allowlist, and the complete original evidence remains bound into the lifecycle disposition. No signed resume reference exists in the bundle, so origin authenticity is deferred to phase 014 |
...
|---|---|
| `schemaVersion` | Derived constant `DEEP_AI_COUNCIL_ROLLBACK_GATE_SCHEMA_VERSION` |
| `certificateKind` | Derived constant `non-destructive-rollback` |
| `mode` | Derived constant `deep-ai-council`; the authorization request is independently required to use the same mode |
...
| `writerResourceDigest` | Canonical digest of the fixed Deep AI Council ledger-writer identity; the request value is gateway-bound and cross-checked before coordinator access |
...
| `DeepAiCouncilRollbackRequest` field | Binding, authentication, or constant |
|---|---|
...
| `gateInput` | Authenticated by re-executing `DeepAiCouncilModeMigrationGate`; only an exact certificate reproduction is accepted |
...
| `writerResource` | Canonicalized, required to equal the fixed Deep AI Council ledger writer, bound by `resourceDigest`, matched to the stale lease, and recorded in the certificate |
...
No declared request field is inert. Every field is now externally bound, authenticated or cross-checked against real evidence, or restricted to a validated closed value. Structurally malformed caller evidence resolves to a denied `DeepAiCouncilRollbackDecision`; it does not escape as an uncaught serializer or validator exception.
