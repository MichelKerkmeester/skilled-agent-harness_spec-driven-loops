...
description: "Implemented plan for the additive-dark Deep AI Council rollback switch and independently re-derived mode gate."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Implemented the authenticated evidence chain"
...
| **Execution** | Additive-dark typed path; legacy authority retained until phase-014 |

### Overview
The implementation defines a mode-owned rollback switch and gate evaluator over the shared event, reducer, seal,
...
### Definition of Ready
- [x] The shared event, reducer, seal, receipt, certificate, replay, resume, and shadow-parity contracts are version-pinned
- [x] The legacy authority anchor and typed shadow frontier are identified for every gate fixture
- [x] The switch state transitions and default-deny behavior are reviewed against the phase-014 cutover contract
- [x] The Deep AI Council gate fixture matrix covers the full lifecycle and required failure dispositions
- [x] Required artifact kinds, certificate fields, and rollback evidence are named without creating mode-local primitives

### Definition of Done
- [x] The switch rejects malformed, stale, unauthorized, expired, mixed-version, and wrong-mode requests
- [x] The rollback window is bounded, observable, and restorable to the pinned legacy anchor
- [x] Shadow parity is green for all required Deep AI Council fixtures and records non-green dispositions explicitly
- [x] Sealed artifacts, receipt chain, replay fingerprint, and mode certificate verify independently
- [x] The handoff is deterministic and phase-014 receives no authority permission without its own cutover receipt
<!-- /ANCHOR:quality-gates -->
...
- **State progression**: `legacy_authoritative -> shadow_eligible -> cutover_armed -> ledger_authoritative_within_window`.
  ...
  window_closed`. Phase-014 owns the authority transition; this phase defines the admissible evidence and state semantics.
...
- Define the default-deny switch configuration, authorized transition table, fail-closed error taxonomy, and mode-bound policy
  fingerprint. Keep `ledger_authoritative_within_window` inaccessible without a later phase-014 cutover receipt.
...
| REQ-008 | Inspect every gate and rollback fixture for authority writes; assert legacy remains authoritative until phase-014's own receipt |
...
task-conditional protocol selection. Phase-011 convergence and health evidence is consumed as an exit predicate; phase-014
...
This phase is additive-dark and does not change runtime authority. Any implementation commits remain git-reversible and must
...
For a post-cutover incident, phase-014 invokes the mode-defined window: fence new ledger-authoritative writes, append a
