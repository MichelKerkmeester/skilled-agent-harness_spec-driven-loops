...
The 021 RED baseline is `148 files / 3,992 tests / 3,986 pass / 6 fail in 3 files`. The confirmed pre-existing failing files (owned by child `009-silent-failure-and-harness-repair` or content-drift, verified NOT caused by 024):
...
3. Land via the leak-guard tree-to-tree lander:
   - Paths file (`/tmp/ks/paths-036-024.txt` — recreate if scratch wiped): the three prefixes
     ...
     `.opencode/specs/system-deep-loop/036-deep-loop-innovation/004-durable-write-boundaries`.
