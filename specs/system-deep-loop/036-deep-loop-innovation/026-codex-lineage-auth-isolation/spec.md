...
_memory:
  continuity:
    ...
    recent_action: "The fix was reverted after a fresh security-and-correctness review returned FAIL"
    next_safe_action: "None inside this packet. A correct attempt belongs in a new packet"
    blockers: []
    key_files:
      - "spec.md"
...
| **Status** | Withdrawn |
| **Created** | 2026-08-30 |
| **Packet ID** | `system-deep-loop/036-deep-loop-innovation/026-codex-lineage-auth-isolation` |
<!-- /ANCHOR:metadata -->
...
## 4. WHY THE PRE-FLIGHT DID NOT CATCH IT
...
## 5. WHY THE SIBLING EXECUTORS DO NOT HIT IT
...
## 7. SECONDARY DEFECTS FOUND ALONGSIDE
...
live in the caller rather than the adapter. They are real and recorded in section 7, SECONDARY DEFECTS FOUND ALONGSIDE. Neither is
...
<!-- ANCHOR:risks -->
## 11. WITHDRAWN — the fix could not have fixed the reported failure
...
and exits status 1 in about nineteen seconds. It does not hang. This document's section 3, THE ROOT CAUSE, claimed the
...
- **Where the relocation actually comes from is still open.** Section 3 established that no
  component in this repository sets a lineage's `CODEX_HOME`. Section 11's own review found that
  the search had missed the dispatch env allowlist forwarding an ambient value whose name starts
  with the executor's prefix. That propagation path was proposed, not confirmed, so the setter
  remains unidentified.
- **The unexplained timeout is still open.** Section 11 found that the 401 reconnect loop is
  bounded at five attempts and exits status 1 in about nineteen seconds, which does not hang the
  way section 3 originally claimed. The timeout actually recorded in AC-1 and AC-3 is therefore
  left unexplained, and the 401 remains, in section 11's own words, a real but different defect.
<!-- /ANCHOR:questions -->
