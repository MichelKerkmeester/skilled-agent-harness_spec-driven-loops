...
- **The ten merged packets, 041 to 050.** These lived as separate top-level packets until 2026-09-20, when they became children of five of this program's group parents. `041-cli-pi-devpass-glm-route` and `044-cli-pi-devpass-deepseek-route` put GLM-5.3-Flash and DeepSeek V4 Flash on the operator's flat-price DevPass plan by remapping their bare model literals to `llmgateway` and allowlisting the credential prefixes `cli-pi` needed, leaving opencode-go direct-dispatch only. `045-fanout-write-containment-hardening` stopped the containment guard from clobbering a concurrent session's 1,858 tracked paths: preserve-and-quarantine is the default, restore targets pre-dispatch bytes, and a finished lane stays finished — it brought its 20 children with it. `046-synthesis-chat-presentation` recorded what a completion message owes its reader and stayed `draft`; `047-deprecate-skill-benchmark` removed the skill-benchmark lane from every reachable surface and left the hub with five workflow modes. `048-fanout-convergence-mode-flag` made the runner read the `--convergence-mode` flag it had documented but ignored, and `050-spec-protocol-ledger-events` gave the deep-research ledger its own spec-protocol event stems so a run's spec writes reach the log. `042-deep-loop-test-debt` cleared the deep-loop-owned red tests in the spec-kit CLI suite and took the runtime typecheck from 53 errors to zero, and `049-deep-loop-alignment-review` read the whole system as one architecture, turning the confirmed findings into seventeen phases — it brought its 17 children with it. `043-review-leaf-protocol` told the review leaf the verbatim-path and stop-reason duties the runner already enforced.

**Why it matters.** The substrate work, the ledger, the gateway and the evidence services, was the hard part of this program. Everything from 013 onward is the ordinary, unglamorous work that follows any real migration: making six runtimes of leaf agents actually use the one door, making the runtime survive a spaced path and a symlinked checkout, recognizing a gate as unfixable instead of patching around it and cleaning up the credential and nesting bugs that only real fan-out lineages, running on real operators' machines, could surface. The ten packets merged in on 2026-09-20 continue that list: routing two fan-out models onto the flat-price plan, making containment preserve instead of clobber, reading a documented flag, letting a run's spec writes reach the ledger, telling the review leaf its duties, and reading the whole system as one architecture.
