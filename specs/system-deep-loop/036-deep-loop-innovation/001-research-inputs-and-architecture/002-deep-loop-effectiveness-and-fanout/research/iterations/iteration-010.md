...
### 1. Eval-DAG: hard vetoes, weighted triage, then judges

Use three ordered lanes:

1. **Hard deterministic gates:** schema, scope, syntax, build, security invariants, original reproducer. `FAIL` rejects immediately; `UNKNOWN` suspends or retries. Neither is judge-overridable.
2. **Cheap weighted checks:** retain every raw score and compute  
   `Q = Σ(wᵢqᵢ) / Σwᵢ`.
   - `Q ≤ τ_reject`: reject or repair without a judge.
   - `Q ≥ τ_accept`: accept without a judge only for policy-declared low-risk artifacts.
   - Middle band, explicit ambiguity, or high-risk class: invoke a judge.
3. **Expensive judges:** judge only reachable candidates and attach evidence; never erase preceding receipts.

This is stricter than Promptfoo’s current aggregate-threshold behavior, where a test-level threshold can determine the final pass despite component failures. Promptfoo nevertheless supplies useful weighted-score and raw-component machinery. Its assertions currently run through a concurrency-limited queue, so environment-level “short circuit” is not equivalent to a genuinely cost-ordered DAG: an expensive check may already be running. ([Promptfoo assertion accumulator](https://github.com/promptfoo/promptfoo/blob/main/src/assertions/assertionsResult.ts), [assertion runner](https://github.com/promptfoo/promptfoo/blob/main/src/assertions/index.ts))

OpenCompass demonstrates the cost benefit: sequential `CascadeEvaluator` runs rules first and sends rule failures to the LLM evaluator. But its final OR merge lets the LLM overturn a rule failure. That is correct for permissive answer matching, not for builds, schemas, security checks, or regression tests. Its cached LLM details also appear to be validated by result count rather than a complete input/model/policy fingerprint. ([documentation](https://doc.opencompass.org.cn/advanced_guides/llm_judge.html), [implementation](https://github.com/open-compass/opencompass/blob/main/opencompass/evaluator/cascade_evaluator.py))

Each node should declare:

```text
{id, dependencies, cost_class, hard, weight, timeout,
 on_fail, on_unknown, evaluator_version, cache_key_fields}
```

Each execution appends an immutable `eval_node_completed` receipt containing candidate hash, policy version, environment fingerprint, verdict, raw score, cost, duration and artifact hash. Aggregates remain derived projections.  
**Maps:** `deep-review`, `runtime/state-jsonl-checkpointing`, `runtime/budget-cost`, `runtime/gauges-observability`.

### 2. Repair-DAG: promotion requires fresh proof

Before repair, establish a baseline and classify checks as:

- `target_failure`: must change from fail to pass.
- `baseline_pass`: must remain passing.
- `baseline_known_failure`: must not be misreported as a new regression.
...
cheap hard gates
→ original reproducer
→ impacted baseline-pass tests
→ risk-selected full gate
→ semantic judge, only for remaining ambiguity
→ promotion
```

Promotion requires fresh receipts for the exact candidate, environment and policy:

```text
target repaired
AND no new baseline failures
AND every required hard receipt is PASS
AND no required receipt is UNKNOWN or stale
```

SWE-bench’s `FAIL_TO_PASS` versus `PASS_TO_PASS` split is a useful starting model, but issue #280 documents that selected `PASS_TO_PASS` tests can miss failures in unchanged tests. Deep-improvement therefore needs an impacted/full-suite baseline delta, not the claim “no regression” from a curated subset. ([grading implementation](https://github.com/SWE-bench/SWE-bench/blob/main/swebench/harness/grading.py), [coverage limitation](https://github.com/SWE-bench/SWE-bench/issues/280))

For flaky failures, absence is weaker evidence than presence. Syzkaller’s bisection repeats each revision ten times and treats one reproduced crash as sufficient evidence of “bad”; it also delays closure until a fix reaches all tracked builds. Deep-improvement should make replay count policy-driven, record every trial, and classify insufficient or non-reproducible evidence as `UNKNOWN`, never `PASS`. ([syzbot workflow](https://github.com/google/syzkaller/blob/master/docs/syzbot.md))

OSS-Fuzz and ClusterFuzz add the missing pre-repair step: minimize the failing input and, when possible, bisect its regression range. That gives every repair a stable reproducer receipt and constrains what contract changed. OSS-Fuzz’s LLM target work also measures compilation, API reachability, immediate crashes and coverage mechanically before treating generated code as useful. ([OSS-Fuzz target generation](https://google.github.io/oss-fuzz/research/llms/target_generation/), [ClusterFuzz](https://github.com/google/clusterfuzz))

**Maps:** `deep-improvement`, `runtime/convergence`, `runtime/locks-recovery`, `runtime/state-jsonl-checkpointing`.
...
```json
{
  "new_repos": [
    {
      ...
      "stars": "~5.6k",
      "what": "Crash triage backend with testcase minimization, deduplication and regression bisection; GitHub showed release v2.34.0 on 2026-07-14.",
      "lesson": "Minimize and fingerprint the original failure before repair, then bind every repair receipt to that reproducer and its regression range.",
      "maps_to": [
        "deep-improvement",
    ...
    {
      "name": "google/syzkaller",
      "url": "https://github.com/google/syzkaller",
      "stars": "~6.3k",
      "what": "Kernel fuzzer and automated bug tracker with reproducer testing, cause/fix bisection and multi-build fix tracking; master showed commits in June 2026.",
      "lesson": "Use asymmetric flaky evidence: one observed regression can veto promotion, while absence requires repeated trials and coverage across required branches.",
      "maps_to": [
        "deep-improvement",
        "runtime/convergence",
    ...
    },
    {
      ...
      "stars": "~5.4k",
      "what": "Software-repair evaluation harness separating FAIL_TO_PASS resolution from PASS_TO_PASS maintenance; main showed a commit on 2026-03-19.",
      "lesson": "Represent target repair, preserved behavior and baseline-known failures separately; selected regression tests are not proof of whole-system non-regression.",
      "maps_to": [
        "deep-review",
        "deep-improvement",
        "runtime/convergence"
    ...
    },
    {
      ...
      "stars": "~47.4k",
      "what": "Coding-agent loop with automatic lint/test feedback and bounded reflection; main showed commits on 2026-05-22.",
      "lesson": "Feed deterministic failures back into a bounded repair loop, but make successful re-run receipts mandatory because a retry cap alone proves nothing.",
      "maps_to": [
        "deep-improvement",
        "runtime/budget-cost",
        "runtime/convergence"
  ...
  "insights": [
    {
      "insight": "Use a three-region weighted policy: deterministic reject, deterministic low-risk accept, and an uncertainty band routed to judges; keep hard vetoes outside the weighted average.",
      "evidence": "https://github.com/promptfoo/promptfoo/blob/main/src/assertions/assertionsResult.ts; https://doc.opencompass.org.cn/advanced_guides/llm_judge.html",
      "maps_to": [
        "deep-review",
        "runtime/budget-cost"
      ],
    ...
    {
      "insight": "A real cheap-first cascade must defer scheduling expensive nodes until prerequisites resolve; merely throwing after concurrent assertions start does not guarantee cost short-circuiting.",
      "evidence": "https://github.com/promptfoo/promptfoo/blob/main/src/assertions/index.ts",
      "maps_to": [
        "deep-review",
    ...
    {
      "insight": "Promotion proof is a receipt lattice: target-failure repair, preservation of baseline-pass behavior, explicit baseline-known failures, and environment-policy freshness are independent requirements.",
      "evidence": "https://github.com/SWE-bench/SWE-bench/blob/main/swebench/harness/grading.py; https://github.com/SWE-bench/SWE-bench/issues/280",
      "maps_to": [
        "deep-improvement",
        "runtime/state-jsonl-checkpointing",
        "runtime/convergence"
    ...
    {
      "insight": "For nondeterministic regressions, positive reproduction and clean evidence have different evidentiary strength; UNKNOWN must remain distinct from PASS throughout reduction and promotion.",
      "evidence": "https://github.com/google/syzkaller/blob/master/docs/syzbot.md; https://google.github.io/oss-fuzz/advanced-topics/bug-fixing-guidance",
      "maps_to": [
        "deep-improvement",
        "runtime/convergence",
        "runtime/gauges-observability"
  ...
  "recommendations": [
    {
      "rec": "Add a versioned Eval-DAG policy with hard, weighted-soft and judge nodes; schedule nodes topologically by cost and append an immutable receipt for every verdict while retaining raw component scores.",
      "target": "deep-review/runtime/state-jsonl-checkpointing",
      "rationale": "This provides actual cheap-check short-circuiting, prevents weighted scores or judges from hiding invariant failures, and makes every aggregate replayable.",
      "effort": "M",
      "impact": "high",
      "evidence": "https://github.com/promptfoo/promptfoo/blob/main/src/assertions/assertionsResult.ts; https://github.com/open-compass/opencompass/blob/main/opencompass/evaluator/cascade_evaluator.py"
    },
    {
      "rec": "Make candidate promotion require a fingerprint-matched verification bundle containing original-reproducer, target-fix, impacted-regression and risk-selected full-gate receipts; timeout, flakiness or missing evidence yields UNVERIFIED.",
      "target": "deep-improvement/runtime/convergence",
      "rationale": "A repair is only established by rerunning the failing behavior and proving a baseline delta; a generated patch or exhausted retry loop is not evidence.",
      "effort": "L",
      "impact": "high",
      "evidence": "https://github.com/SWE-bench/SWE-bench/blob/main/swebench/harness/grading.py; https://github.com/google/syzkaller/blob/master/docs/syzbot.md"
    },
    {
      "rec": "Expose deterministic-reject rate, judge-escalation rate, judge cost per promoted candidate, stale-receipt count, flaky-unknown count and regression delta as stream-fold gauges.",
      "target": "runtime/gauges-observability",
      "rationale": "These gauges reveal whether the cascade is saving judge cost without silently increasing unverified promotions or regression escapes.",
      "effort": "S",
      "impact": "med",
      "evidence": "https://doc.opencompass.org.cn/advanced_guides/llm_judge.html"
    }
  ],
  "contradictions": [
    {
      "claim": "[deep-review] A weighted aggregate above threshold is sufficient for promotion.",
      "counter": "Hard schema, build, security and regression failures must remain vetoes; otherwise strong soft scores can conceal a broken invariant.",
      "evidence": "https://github.com/promptfoo/promptfoo/blob/main/src/assertions/assertionsResult.ts"
    },
    {
      "claim": "[deep-review] A rule failure should always be sent to an LLM judge that may rescue it.",
      "counter": "Rescue is appropriate for ambiguous answer matching, but an LLM must not overturn deterministic hard gates; only soft or uncertain nodes should be judge-rescuable.",
      "evidence": "https://github.com/open-compass/opencompass/blob/main/opencompass/evaluator/cascade_evaluator.py"
    },
    {
      "claim": "[deep-improvement] Passing FAIL_TO_PASS and a selected PASS_TO_PASS set proves no regression.",
      "counter": "SWE-bench issue #280 shows unchanged tests can still regress outside the selected set, so promotion needs an impacted or full-suite baseline delta.",
      "evidence": "https://github.com/SWE-bench/SWE-bench/issues/280"
    },
    {
      "claim": "[runtime/convergence] Failure to reproduce means the defect is fixed.",
      "counter": "OSS-Fuzz and syzkaller explicitly account for nondeterministic and environment-sensitive failures; non-reproduction without sufficient trials is UNKNOWN.",
      "evidence": "https://google.github.io/oss-fuzz/advanced-topics/bug-fixing-guidance; https://github.com/google/syzkaller/blob/master/docs/syzbot.md"
    }
  ],
  "next_angles": [
    "Calibrate the two weighted thresholds from logged ROC and cost curves, separately by artifact risk class, rather than choosing universal constants.",
    "Specify an Eval-DAG cache-compatibility fingerprint covering candidate, dependency artifacts, evaluator code, prompt, model, policy, toolchain and environment.",
    "Design sequential tests for flaky repair verification: compare fixed replay counts, confidence sequences and SPRT-style early stopping under asymmetric false-pass costs.",
    "Measure judge-selection bias introduced by uncertainty-band routing, because judges will see a harder and distribution-shifted sample than the overall review stream."
  ],
  "notes": "Star counts are approximate GitHub values observed on 2026-07-15. OpenCompass documents cascade statistics but I found no published calibration of cost savings versus false rescues. Its visible cache path appears to check result count rather than a complete compatibility fingerprint. Aider clearly exposes bounded reflection plus lint/test hooks, but automatic testing is opt-in by default; its design should not be copied as a promotion guarantee."
}
```
...
session id: 019f6423-e5dc-78c2-b2cf-9a481abd8cb0
--------
user
You are iteration 10/20 of a TARGETED, NON-CONVERGING (deliberately broadening WITHIN its thread) research loop to make "system-deep-loop" MORE EFFECTIVE.
...
ANGLE: R5 cheap-checks-before-judges + regression-gated self-repair
DIRECTIVE: Deepen R5. Eval-DAG cascades (cheap deterministic checks short-circuit before expensive judges), weighted-threshold promotion, regression-gated self-repair (every fix proven by re-run). Look at promptfoo, OpenCompass CascadeEvaluator, oss-fuzz. Actionable design for deep-review + deep-improvement.
...
THIS run's new repos so far (32) — also do not repeat:
  openai/codex, google-gemini/gemini-cli, anomalyco/opencode, anthropics/claude-agent-sdk-typescript, microsoft/conductor, massgen/MassGen, lastmile-ai/mcp-agent, etcd-io/etcd, delta-io/delta-rs, holepunchto/autobase, cadence-workflow/cadence, nextflow-io/nextflow, snakemake/snakemake, jshiv/cronicle, MinishLab/semhash, allenai/olmes, smartyfh/LLM-Uncertainty-Bench, browser-use/browser-use, SahilShrivastava-Dev/semantic-halting-problem, grpc/grpc, temporalio/sdk-go, restatedev/sdk-typescript, Azure/azure-functions-durable-extension, scikit-learn-contrib/DESlib, Toloka/crowd-kit, ArjunPanickssery/self_recognition, lm-sys/FastChat, kubernetes/kubernetes, volcano-sh/volcano, tektoncd/pipeline, lm-sys/RouteLLM, taskflow/taskflow
Angles already covered this run: Per-leaf CLI flag + model + live-tool parametrization | Heterogeneous multi-model fan-out orchestration | Cross-iteration shared state for parallel research leaves | Resumable externalized-state fan-out with salvage/merge | Bias-free reduction of heterogeneous multi-model leaves | R1 multi-signal path-covering termination — mechanisms | R2 side-effect-receipt resume contract — mechanisms | R3 effective-independence + 5-role evaluator separation | R4 conditional budget-aware fan-in + determinism
New insights so far: 37; new contradictions: 30; recommendations: 27.
Open threads flagged: Specify how effect leases interact with hierarchical budgets: prepared effects reserve cost, verified results commit it, provably absent reconciliations refund it, and unknown effects remain charged and block automatic lease reuse. | Build a factorial calibration experiment that independently varies model family, prompt template, evidence set, context snapshot, decoding seed, and role to decompose error correlation into provider, prompt, context, and task-difficulty variance components. | Compare static three-seat councils with task-conditional judge routing: select the next detector by expected marginal log-loss reduction per token, then test whether this improves N_eff-per-dollar without suppressing rare dissent. | Study online drift detection for judge independence: rolling error-correlation matrices, confidence sequences for marginal gain, and explicit invalidation when a model or prompt version changes. | Design a hierarchical dependence model for judges sharing provider, base model, fine-tuning lineage, retrieval corpus, or tool receipts, and compare it with the simple weighted N_eff estimator. | Design model-checking properties for the fan-in state machine: no merge before ACCEPT, one terminal decision per policy version, and invariant selected sets under every completion-order permutation. | Calibrate seed-stage agreement thresholds against false-consensus risk, comparing raw vote agreement with semantic-cluster agreement and critical-contradiction vetoes. | Specify atomic budget reservation and cancellation accounting for WIDEN, including races between deadline, late completion, and next-stage lease acquisition.
...
codex
I’m using the repository’s `system-deep-loop` deep-research workflow because the request explicitly freezes that route. This is read-only research, so no spec-folder choice is needed; I’ll first load its contract and prior-state hooks, then verify every source and keep the final JSON as the last output.
mcp: mk-spec-memory/memory_match_triggers started
mcp: mk-spec-memory/memory_match_triggers (failed)
user cancelled MCP tool call
exec
/bin/zsh -lc "node .opencode/bin/skill-advisor.cjs advisor_recommend --json '{\"prompt\":\"Iteration 10/20 targeted non-converging deep-research deepening R5 cheap-checks-before-judges and regression-gated self-repair for system-deep-loop; inspect promptfoo, OpenCompass CascadeEvaluator, and oss-fuzz; produce verified sources and final JSON\"}' --warm-only --format json --timeout-ms 3000" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 exited 75 in 0ms:
{
  "status": "error",
...
codex
The memory trigger lookup was cancelled and the warm Skill Advisor daemon is unavailable (retryable exit 75). The user-specified route is still unambiguous, so I’m proceeding with the named deep-research contract rather than substituting a manual workflow.
exec
...
exec
/bin/zsh -lc "sed -n '1,900p' .opencode/skills/system-deep-loop/deep-research/references/state/state_outputs.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
...
exec
/bin/zsh -lc "sed -n '1,900p' .opencode/skills/system-deep-loop/deep-research/references/guides/quick_reference.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
...
exec
/bin/zsh -lc "sed -n '1,900p' .opencode/skills/system-deep-loop/deep-research/references/convergence/convergence_signals.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
...
exec
/bin/zsh -lc "sed -n '1,900p' .opencode/skills/system-deep-loop/deep-research/references/protocol/loop_protocol.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
---
...
exec
/bin/zsh -lc "sed -n '140,390p' .opencode/skills/system-deep-loop/deep-research/references/protocol/loop_protocol.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
  "continuedFromRun": <number or null>,
  "archivedPath": "<path or null>",
  "timestamp": "<ISO 8601>"
...
exec
/bin/zsh -lc "sed -n '391,646p' .opencode/skills/system-deep-loop/deep-research/references/protocol/loop_protocol.md" in /Users/michelkerkmeester/MEGA/Development/Code_Environment/Public
 succeeded in 0ms:
   - `repetitive` - iterations keep rediscovering the same information
   - `exhausted` - the topic area has been thoroughly explored with diminishing returns
3. Select a targeted recovery prompt based on the classification (see convergence.md §4 for category-specific strategies)
...
codex
The contract’s relevant constraint is clear: this leaf may research and report but not implement, and every mechanism needs an authoritative source. I’m now testing the R5 design against concrete evaluator cascade and fuzz-repair implementations; repository popularity will be reported only where GitHub exposes it reliably.
web search: 
web search: site:github.com/promptfoo/promptfoo threshold assertion weight source code scoring evaluate ...
web search: 
web search: opencompass "CascadeEvaluator" GitHub ...
web search: 
web search: site:github.com/Aider-AI/aider auto test cmd fix lint test source base_coder.py ...
web search: 
web search: https://api.github.com/repos/google/clusterfuzz
web search: 
web search: github google/clusterfuzz stars latest commit ...
web search: 
web search: site:github.com/google/syzkaller "Stars" "Releases" ...
web search: 
web search: https://github.com/SWE-bench/SWE-bench
web search: 
web search: 
web search: 
web search: "class CascadeEvaluator" opencompass ...
web search: 
web search: https://github.com/open-compass/opencompass/blob/main/opencompass/evaluator/cascade_evaluator.py
web search: 
web search: 'if not self.parallel'
web search: 
web search: 'run_tests'
web search: 
web search: site:github.com/Aider-AI/aider "auto_test" "test_cmd" ...
web search: 
web search: https://github.com/promptfoo/promptfoo/blob/main/src/assertions/assertionsResult.ts
web search: 
web search: 'self.auto_lint'
web search: 
web search: site:github.com/google/syzkaller syzbot patch testing fix commit bisection docs ...
web search: 
web search: site:github.com/google/clusterfuzz "regression_task" ...
web search: 
web search: site:google.github.io/oss-fuzz verify bug fix reproducer no longer crashes ...
web search: 
web search: site:github.com/google/oss-fuzz "LLM Code Fixer" ...
web search: 
web search: 'totalWeight'
web search: 
web search: 
web search: 
web search: site:github.com/Aider-AI/aider "reflected_message =" ...
web search: 
web search: 
context compacted
codex
The evidence now points to a sharper R5 design: deterministic checks need both a hard-veto lane and a weighted soft lane. I’m doing one final recency check, then I’ll turn that into an Eval-DAG and a receipt-gated repair protocol rather than another repository survey.
web search: 
web search: 
codex
R5 should become two linked state machines: an Eval-DAG decides whether an artifact merits an expensive judge; a Repair-DAG decides whether a proposed fix merits promotion. A single aggregate score is unsafe because it can conceal invariant failures.
