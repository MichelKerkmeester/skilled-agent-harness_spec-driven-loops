...
- [x] T008 Inspect F005, retain the deterministic fresh-acquisition falsifier (proves single-winner admission), and harden the loop-lock release/reclaim path via rename-based claim-then-owned-unlink; the fresh-acquisition partial-record window remains open as a documented per-mode 014-cutover precondition (`loop-lock.ts`, `loop-lock.vitest.ts`).
