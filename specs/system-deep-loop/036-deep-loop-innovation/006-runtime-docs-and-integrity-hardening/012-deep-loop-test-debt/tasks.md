...
- [x] T004 [P] Review reducer on a missing machine-owned strategy anchor — the committed contract (016 audit remediation, `235f1f6f49`) is warn-and-keep-output, not throw; the CLI-tree test encoded the superseded contract and now asserts the warning plus the computed registry. Producer unchanged
...
- [x] T006 Clear the runtime typecheck errors without changing runtime behavior (`.opencode/skills/system-deep-loop/runtime`) — `9c962fe7a3`: a readonly-array JSON carrier for the legacy projection states, the gateway allow result's optional reason code, a minimal ambient declaration for the sqlite binding; no compiler option changed
- [x] T007 Fix the remaining red files from the T001 baseline at their producers — the contract-drift files were red on stale compiled contracts, regenerated at `2fc15dbf00` (drift check OK, 3 commands); the rest are locale and collation child-process tests that depend on the host environment and are recorded as a limitation
<!-- /ANCHOR:phase-2 -->
