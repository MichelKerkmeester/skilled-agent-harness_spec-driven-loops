...
- [x] CHK-006 [P0] Each coordinator call requests exactly one mode (REQ-006) [EVIDENCE: `runStep` is awaited once per mode with a single mode string — `enablement-driver.ts:137`; asserted by `requests exactly one mode per call`. No coordinator request is constructed at all — but note this is because the step has no write path, not because it refuses before one. For a `cutover_ready` mode the step returns ok without constructing anything, which is why CHK-008 now fails]
