...
- [x] CHK-009 [P0] The runtime suite ran at the candidate and is reported as a delta (REQ-004, SC-003) [EVIDENCE: re-measured at the current candidate over 2h04m and reported as a delta, never a bare number — `failed 15 vs 15`, `passed 4188 vs 4111`, `skipped 39 vs 39`, `total 4242 vs 4165`, `files 188 vs 182`. The failing count matching is not the proof; the 21 failure lines are byte-identical between the two runs, so the same tests fail by name and nothing regressed. An earlier capture was discarded rather than reported: it ran while the tree was being edited, so it measured something that no longer existed]
