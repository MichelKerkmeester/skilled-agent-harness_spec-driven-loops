...
- [ ] CHK-002 [P0] Both command variants proven by execution to reach one shared composition seam (REQ-001, SC-001) [PARTIAL: the declared command was executed verbatim and succeeded end to end — `exit 0`, `ok: true`, a ledger frame, an audit-ledger authorization decision, the projected `deep-research-state.jsonl`, and a projection watermark at `ledger_sequence: 1`. The seam is proven real and proven to work pre-flip (no authority record was written during the run). Still open because neither workflow variant was run as an agent, so the step from a manifest directive to that command remains declared. Evidence: `scratch/seam-executed.md`, `scratch/seam-execution-receipt.json`]
...
- [ ] CHK-005 [P0] The seam routes through the gateway with no variant retaining a private write path (REQ-001) [PARTIAL: the gateway no longer accepts caller-asserted authority and the CLI reads the durable record, both proven by probe; the declared route was additionally executed end to end, producing a receipt, an audit decision and a projected legacy file (`scratch/seam-executed.md`). The manifests retain their `append_to_jsonl` directives under one `state_write_protocol` block, and `check-protocol-append-sites.cjs` now fails any asset whose appends are undeclared or uncounted. Still open because the directive-to-command step is declared rather than agent-executed]
