...
3. **Wave 3**: F2 four value files; retain `mode-contract-types.ts`, make `index.ts` types-only. **Pre-step:** relocate `strict-gate-validator.ts`'s `matchesPreparedAuthorizationDecision` (+ its `digest`/`isRecord`/`isDigest` helpers) into a new `lib/authorized-ledger/` module and repoint `tests/unit/authorized-ledger.vitest.ts` — that kept 13-case test calls it, so it cannot be deleted with the file.
