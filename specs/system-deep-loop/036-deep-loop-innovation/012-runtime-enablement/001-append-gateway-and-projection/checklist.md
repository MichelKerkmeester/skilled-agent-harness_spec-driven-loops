...
_memory:
  continuity:
    ...
    answered_questions: []
---
# Verification Checklist: Append Gateway and Legacy Projection
...
- [x] CHK-009 [P0] Envelope refusal test passes and was observed red with its guard removed (REQ-002, SC-002) — the earlier deferral said the gateway does not validate envelopes. That was wrong, and was measured wrong: a bare unprefixed stem returns `Envelope field must be a bounded non-empty string` and a record with neither stem nor event_type returns `Unrecognized event format`, both `exit 1`, with zero files written to the run directory. Negative control on the bounded-string guard in `lib/event-envelope/event-envelope.ts`: baseline 57 passed, guard neutered 5 failed and 52 passed, restored 57 passed, tree byte-identical. Evidence: `scratch/envelope-refusal-nc.md`
