...
- [ ] CHK-010 [P0] Tree-wide search finds no direct-append instruction in any mode's protocol documents (REQ-001, SC-001) [NOT MET: 8 of 10 mode-facing command assets now declare the gateway as the sanctioned mechanism via `state_write_protocol`, but 2 literal shell appends survive by design (`deep-review-auto.yaml:284`, `deep-review-confirm.yaml:249`), so a tree-wide search still finds direct-append instructions. The earlier deferral reason was wrong: `mode-append-gateway.vitest.ts:38` pins authority at `legacy_authoritative` and the suite passes 11/11, so the gateway is a working pre-flip write path. Evidence: `scratch/protocol-retirement-baseline.md`]
