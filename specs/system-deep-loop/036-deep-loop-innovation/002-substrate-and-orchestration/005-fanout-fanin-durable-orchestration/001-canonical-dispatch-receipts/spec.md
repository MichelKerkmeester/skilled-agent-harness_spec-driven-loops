...
description: "Promote each phase-005 resolved leaf invocation into a canonical, authorized, durable pre-spawn ledger receipt so resume can detect prior dispatch intent without duplicating work."
trigger_phrases:
...
> Phase adjacency under `005-fanout-fanin-durable-orchestration` (navigation order, not a hard runtime dependency): predecessor none (first sibling); successor `002-result-envelopes-and-resume-salvage`.
...
| Field | Value |
|-------|-------|
| **Packet** | system-deep-loop/036-deep-loop-innovation/005-fanout-fanin-durable-orchestration/001-canonical-dispatch-receipts |
| **Level** | 2 |
| **Priority** | P0 |
...
Phase 005 deliberately stops at the dispatch boundary. Its executor adapters resolve `{ command, args, input, effectiveConfig, invocationFingerprint }`, including executor kind, model, effort, web-search policy, executable version, and prompt digest, but its specification states that the fingerprint is an adapter return value rather than a ledger event. A process crash therefore loses the durable fact that a particular leaf launch contract was resolved, and resume cannot distinguish a leaf that was never planned from one whose launch crossed the persistence boundary (`../../001-fanout-live-tools-unblock/spec.md`).

The phase-006 typed ledger already plans immutable, conflict-detecting idempotent append, authorized event linkage, monotonic sequence, and a durable append receipt (`../../002-transition-authorized-ledger-core/002-typed-append-only-ledger/spec.md`). Phase 007 adds semantic receipts and intent-before-effect recovery, including the rule that an unresolved external effect must be reconciled rather than replayed speculatively (`../../003-shared-evidence-and-control-services/001-receipts-and-effect-recovery/spec.md`). The shipped `runtime/lib/deep-loop/receipt-crypto.ts` supplies deterministic per-dispatch key derivation, recursively canonical JSON, HMAC signing that excludes the `mac` field, and constant-time verification, but its run-master secret remains outside the receipt and its process-local MAC cannot by itself establish restart-verifiable authority.
...
### In Scope
- Register a versioned canonical `lineage_dispatch_resolved` event in the phase-006 envelope/type registry and define it as a dispatch receipt recording durable intent, not proof that a process started or completed.
- Carry the phase-005 adapter's `invocationFingerprint` unchanged, with an explicit fingerprint version/algorithm, alongside the independently normalized resolved launch facts that produced it.
...
- Reuse phase-006 exact-repeat idempotency: the same dispatch identity and canonical event bytes return the original ledger append receipt; the same identity with changed launch facts or fingerprint fails closed.
...
### Out of Scope
- Changing the phase-005 capability matrix, adapter command shapes, manifest Cartesian expansion, model selection, effort policy, or invocation-fingerprint inputs.
- Redefining the phase-006 envelope, ledger frame/hash chain, transition vocabulary, replay fingerprint, append durability, or authorization gateway.
- General boundary receipts, external-effect adapter policy, or durable certification-provider design owned by phase 007.
...
- Activating ledger-derived resume decisions for authoritative production paths before the staged phase-014 cutover.
<!-- /ANCHOR:scope -->
...
| REQ-003 | The phase-005 invocation fingerprint is promoted without semantic drift | The stored fingerprint equals the adapter return value byte-for-byte, carries a version/algorithm, and changes whenever the phase-005 contract says it must change |
...
| REQ-011 | The receipt is replay-deterministic and projection-safe | The phase-006 verified stream rebuilds byte-identical dispatch-state projections; mutable checkpoint caches cannot override ledger evidence |
...
| REQ-013 | Legacy behavior remains authoritative during dark rollout | Existing phase-005 command, pool, retry, budget, and persisted-artifact behavior is unchanged when canonical receipt consumption is not enabled |
...
|-------------|----------------|----------|
| Envelope | event type/version, event ID, run ID, correlation/causation IDs, authority epoch | Supplied by the phase-006 canonical envelope and authorization gateway |
...
| Fingerprint | invocation fingerprint, fingerprint version, algorithm/namespace | Fingerprint is copied from the phase-005 adapter result and checked against the normalized facts |
...
- **SC-001**: Every leaf spawn is preceded by one durable, authorized `lineage_dispatch_resolved` ledger event carrying its phase-005 invocation fingerprint.
...
- **SC-005**: Canonicalization, MAC labeling, secret exclusion, and ledger integrity claims match `receipt-crypto.ts` and the phase-007 durable-verifier policy.
- **SC-006**: Legacy phase-005 commands, pool behavior, retries, budgets, and persisted artifacts remain unchanged during additive-dark rollout.

**Given** a phase-005 adapter has resolved a valid launch contract, **When** fan-out reaches the execution boundary, **Then** the authorized receipt is durably committed before the spawn stub is called.
