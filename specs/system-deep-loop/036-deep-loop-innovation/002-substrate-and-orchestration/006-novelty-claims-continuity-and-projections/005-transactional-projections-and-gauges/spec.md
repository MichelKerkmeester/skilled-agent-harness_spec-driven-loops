...
> Phase adjacency under `006-novelty-claims-continuity-and-projections` (navigation order, not a hard runtime dependency): predecessor `004-next-focus-semantics`; successor none (last sibling).
...
| Field | Value |
|-------|-------|
| **Packet** | system-deep-loop/036-deep-loop-innovation/006-novelty-claims-continuity-and-projections/005-transactional-projections-and-gauges |
| **Level** | 2 |
| **Priority** | P1 |
| **Status** | Implemented |
| **Created** | 2026-07-15 |
...
The phase-010 parent requires deterministic transactional projections and gauges as the final intelligence-layer
contract before convergence can consume novelty, claim, continuity, and next-focus signals. Its role and ordering are
fixed by `../../spec.md` and `../../manifest/phase-tree.json`: phase 010 produces coherent derived signals over the
ledger and durable fan-in substrate, while phase 011 owns the policies that interpret those signals.

The source contracts solve only part of that problem. The typed append-only ledger in
`../../002-transition-authorized-ledger-core/002-typed-append-only-ledger/spec.md` provides immutable verified order,
idempotent append, integrity linkage, and deterministic reduction. The stream-fold gauge contract in
`../../003-shared-evidence-and-control-services/005-stream-fold-gauges/spec.md` defines versioned pure folds and
...
computes all affected dashboard rows, registries, claim tables, indexes, and phase-007 gauges against one prior
...
- Transactional composition of the phase-007 stream-fold gauges without redefining their gauge IDs, arithmetic, event ownership, or pure reducer semantics.
...
### Out of Scope
- Defining the phase-006 envelope, ledger writer, sequence allocation, integrity chain, transition authorization, or replay fingerprint.
- Redefining semantic communities, contradiction/supersession events, claim continuity, or next-focus semantics owned by phase-010 siblings 001-004.
- Redefining progress, novelty, cost, or health gauge folds owned by phase 007; this phase owns their transactional application with related views.
...
- Choosing convergence thresholds, health verdicts, degeneration policy, or stop decisions owned by phase 011.
...
| REQ-010 | Gauges retain the phase-007 fold contract | Gauge definitions, exact arithmetic, event filters, and canonical output remain versioned pure folds; this phase changes only their transaction and publication boundary |
...
- **SC-006**: Dark comparison covers the shipped observability-event surface and phase-007 gauge outputs without changing legacy authority, schemas, or control flow.
<!-- /ANCHOR:success-criteria -->
...
reader and immutable order from phase 006, the versioned gauge reducers from phase 007, and the domain events planned
by phase-010 siblings. The main risk is false atomicity: committing each table independently, or updating a cursor
...
or reducer output. Ledger sequence, canonical event hashes, registered versions, and exact phase-007 numeric rules are
...
batch size, lease mechanism, rebuild retention count, and publication transport after the phase-006 ledger and
phase-010 event schemas are materialized. Any choice must provide one atomic boundary for related projections,
