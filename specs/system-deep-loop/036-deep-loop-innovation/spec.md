...
After grouping, the parent has nine thematic group parents from the consolidation (`001` through `009`), then flat leaf children appended in creation order as later work landed, with ten further children, `041` through `050`, merged in on 2026-09-20 inside five of those group parents. Two of those later additions, `012-runtime-enablement` and `019-risky-followup-remediation`, are themselves phase parents with their own children, but neither is one of the original nine consolidation groups. The full creation order of every child, including the former top-level numbering of the thirteen children merged in from separate packets, is in the root `timeline.md`.
...
Ten more children arrived by merge alone on 2026-09-20: each kept its number and moved inside one of the five thematic group parents, so this packet's direct-child count is unchanged at 28.
- `007-executor-and-cli-hardening` gained `008-cli-pi-devpass-glm-route` (was `system-deep-loop/041-cli-pi-devpass-glm-route`), `009-cli-pi-devpass-deepseek-route` (was `system-deep-loop/044-cli-pi-devpass-deepseek-route`) and `010-fanout-write-containment-hardening` (was `system-deep-loop/045-fanout-write-containment-hardening`, bringing its 20 children).
- `003-mode-contracts-migration-and-cutover` gained `005-synthesis-chat-presentation` (was `system-deep-loop/046-synthesis-chat-presentation`) and `006-deprecate-skill-benchmark` (was `system-deep-loop/047-deprecate-skill-benchmark`).
- `002-substrate-and-orchestration` gained `008-fanout-convergence-mode-flag` (was `system-deep-loop/048-fanout-convergence-mode-flag`) and `009-spec-protocol-ledger-events` (was `system-deep-loop/050-spec-protocol-ledger-events`).
- `006-runtime-docs-and-integrity-hardening` gained `012-deep-loop-test-debt` (was `system-deep-loop/042-deep-loop-test-debt`) and `013-deep-loop-alignment-review` (was `system-deep-loop/049-deep-loop-alignment-review`, bringing its 17 children).
- `008-review-and-rollback-followup` gained `005-review-leaf-protocol` (was `system-deep-loop/043-review-leaf-protocol`).

Three statuses are kept honest rather than smoothed. `046-synthesis-chat-presentation` reads `draft` because its tasks are unchecked and its lone acceptance criterion is still an unmet placeholder. `041-cli-pi-devpass-glm-route` reads `complete` on its own declared status while two verification tasks in its `tasks.md` remain open. `049-deep-loop-alignment-review` reads `complete` on its own phase map and checked goal criteria while its `graph-metadata.json` derives `in_progress` from unevidenced rows. In all three, the gate is the closure authority, the derived record is left as it is, and neither is reconciled by back-filling evidence.
...
Direct children after grouping: nine thematic group parents from the consolidation, then flat leaf children appended in creation order as later work landed; the direct-child count did not change when ten more, `041` through `050`, were merged in on 2026-09-20 inside five of those group parents. Two of the later additions, `012-runtime-enablement/` and `019-risky-followup-remediation/`, are themselves phase parents with their own children. The full lineage of every child, including the former top-level numbering of the thirteen children merged in from separate packets, is in the root `timeline.md`.
