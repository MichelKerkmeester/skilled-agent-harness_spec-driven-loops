...
- `.opencode/skills/system-deep-loop/runtime/scripts/render-command-contract.cjs` (6342c5822f7 removed the DISPATCH-CONTEXT prefix, 8 lines)
- `.opencode/skills/system-deep-loop/runtime/tests/unit/render-command-contract.vitest.ts` (6342c5822f7 removed the gate-resolution test, 22 lines)
- `git show 6342c5822f7` — Phase-0 gate retirement across 33 files (8 deep/* commands + prompt/improve + 4 legacy bodies + 3 presentations + 023 spec packet)
- `git show 3029aad3c34` — single-dispatch branches for cli-cursor/devin/pi in deep-alignment-auto.yaml (and research/review auto YAMLs)
- `git show 94ab276bfd` — 3-row edit to frozen `state-backend-census.json` (removed alignment-config-corpus, alignment-state-deltas, alignment-projections)
...
## Traceability Checks
- **spec_code:** REQ-010 (frozen auditable handoff with hashes) vs shipped state — PARTIAL: the handoff manifest exists and hashes artifacts, but the census hash is stale post-94ab276bfd and unenforced (P2-012). The evidence-exists invariant (implied by validate-evidence.cjs) IS satisfied by the row removal.
