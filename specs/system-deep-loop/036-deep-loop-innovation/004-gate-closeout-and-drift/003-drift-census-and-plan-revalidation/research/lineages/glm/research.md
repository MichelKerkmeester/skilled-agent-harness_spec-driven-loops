...
|-------|---------|-------------|------------------------|-----------------|
| **003** baseline-taxonomy-and-state-census | **needs refinement** | FIRST-ORDER (two engines) | `b052f329a73` (kebab migration) + `69aee63cb8a` (renumber) | `003/plan.md:81,82`; `003/spec.md:93,144`; `003/plan.md:153`; `003/spec.md:57,119` |
...
| **009** fanout-fanin-durable-orchestration | **still valid** | NO DRIFT | `739b85ac57` + `1accee48991` (both orthogonal) | cited paths resolve; `fanout-run.cjs:331-334` confirms premise (`FLAT_POOL_ASSIGNMENT_MODEL` + `WAVE_*_REJECTION` strings); deliverables unshipped |
...
| **012** shared-mode-contracts-and-fixtures | **needs refinement** | SECOND-ORDER | `fa74e868615` + `4b2c351cc1e` + `d60cc2c7553` | `mode-registry.json:resourceContractVersion` (added); `hub-router.json:defaultMode` (research→null); `hub-router.json:routerSignals.{model-benchmark,skill-benchmark}.classes` (restricted); `shared/references/smart_routing.md` (new); `leaf-manifest.json` (new) |
| **013** mode-and-lane-migrations | **needs refinement** | SECOND-ORDER | `4b2c351cc1e` + `d60cc2c7553` + `fa74e868615` | `hub-router.json:defaultMode` (research→null); `hub-router.json:routerSignals` `hub-identity` dropped from all 7 modes; `model-benchmark`+`skill-benchmark` signals command-bridge-only. **Mode count UNCHANGED** (OPEN QUESTION A) |
...
| **016** whole-system-gate | **still valid** | NO DRIFT (transitive dependency on phase-003 refinement documented) | `37c589e08e4` orthogonal (touches skill-benchmark harness not whole-system gate) | surfaces checked: `016/spec.md:59,68,88,100,115,135,151,164`; cites phase-003 baseline at 8 sites but not "Packet 033" directly |
| **017** integrate-latest-and-closeout | **still valid** | NO DRIFT | `d5db5ecad3d` + `fe261503031` (both reference-completion fixes) | surfaces checked: `017/spec.md:52,60,61,66,80,81,91`; the drift-handling charter is exactly what this census is executing (pulled forward as packet 018 per `018/spec.md:72`) |
...
|----------------------|-------------------|-------------------------|--------------|-------------|
| `.opencode/skills/system-deep-loop/runtime/references/state_format.md` (`003/plan.md:81`) | **NO** | `state-format.md` YES | snake_case PRESENT at baseline `60b9ed8bc2` | `b052f329a73` |
| `.opencode/skills/system-deep-loop/runtime/references/integration_points.md` (`003/plan.md:82`) | **NO** | `integration-points.md` YES | snake_case PRESENT at baseline | `b052f329a73` |
| `.opencode/skills/system-deep-loop/*/behavior_benchmark/` glob (`003/spec.md:93,144`; `003/plan.md:153`) | **0 dirs match** | `behavior-benchmark/` matches 6 dirs | snake_case PRESENT at baseline | `b052f329a73` |
...
|---------------------|-------------------|---------------------------|-------------------------|-------------|
| "Packet 033" (`003/spec.md:57,119 REQ-007`) — "behavior evidence becomes an eight-workstream baseline" | AMBIGUOUS | `033-post-sync-verification-fixes` (different packet) | `z_archive/027-deep-loop-behavior-benchmarks` (where the behavior-evidence packet was relocated) | `69aee63cb8a` (renumber commit, 12.5 hours after baseline) |

This second drift engine is NOT the kebab migration. It's a renumber commit that's INSIDE the census range (`60b9ed8bc2..739b85ac57`) but OUTSIDE the runtime subtree (touches `.opencode/specs/system-deep-loop/`, not `.opencode/skills/system-deep-loop/`). The census's scope explicitly includes such commits (`spec.md §3 In-Scope`: "the one confirmed drift hit to date originated outside the runtime").
...
> Did `d60cc2c7553` / `fa74e868615` / `4b2c351cc1e` change the registered-mode count phase 013's eight workstreams assume?

**Resolution (iteration 2):** **NO. The count is UNCHANGED at 7 routing modes.**

At both baseline `60b9ed8bc2` and HEAD `739b85ac57`:
...
|--------|--------|-----------|
| `defaultMode` flip | `4b2c351cc1e` | `"research"` → `null` |
| `hub-identity` dropped from all 7 modes | `d60cc2c7553` | every mode carried `hub-identity` → none do |
| `model-benchmark` + `skill-benchmark` signal restriction | `d60cc2c7553` | bare advisor alias → command-bridge-only (`/deep:model-benchmark`, `/deep:skill-benchmark`) |
| typed-pair routing surface added | `fa74e868615` | (none) → `shared/references/smart_routing.md` + `leaf-manifest.json` + `resourceContractVersion` |
...
The renumber commit `69aee63cb8a` (2026-07-16 18:35:55, "refactor(system-deep-loop): compact archive to 001-029, renumber active to 030-036") is:
- **In the census range** (`git merge-base --is-ancestor 69aee63cb8a 60b9ed8bc2` returns NON-ancestor).
- **12.5 hours AFTER baseline** (`60b9ed8bc2` is 2026-07-16 06:09:33).
...
|---|-------|------------------|-------------------|--------------------------|-------------|
| 1 | 003 | `runtime/references/state_format.md` (`003/plan.md:81`) | NO | `state-format.md` resolves | `b052f329a73` |
| 2 | 003 | `runtime/references/integration_points.md` (`003/plan.md:82`) | NO | `integration-points.md` resolves | `b052f329a73` |
| 3 | 003 | `behavior_benchmark/` glob (`003/spec.md:93,144`; `003/plan.md:153`) | 0 dirs match | `behavior-benchmark/` matches 6 dirs | `b052f329a73` |
| 4 | 003 | "Packet 033" reference (`003/spec.md:57,119`) | AMBIGUOUS (points to wrong packet) | `z_archive/027-deep-loop-behavior-benchmarks` holds the actual content | `69aee63cb8a` |
...
|---|-------|-------------------|---------------------|-------------|
| 1 | 012 | "freeze a mode-registry.json without `resourceContractVersion`" | `resourceContractVersion: 1` is now part of the boundary | `fa74e868615` |
| 2 | 012 | "freeze a hub-router.json with `defaultMode: 'research'`" | `defaultMode: null` is now part of the boundary | `4b2c351cc1e` |
| 3 | 012 | "freeze signal restrictions allowing bare advisor alias for model/skill-benchmark" | command-bridge-only is now part of the boundary | `d60cc2c7553` |
| 4 | 012 | "freeze a boundary without `shared/references/smart_routing.md`" | `smart_routing.md` is now part of the boundary | `fa74e868615` |
| 5 | 012 | "freeze a boundary without `leaf-manifest.json`" | `leaf-manifest.json` is now part of the boundary | `fa74e868615` |
| 6 | 013 | "shadow-parity tests can assume `defaultMode: 'research'` for hub-generic prompts" | hub-generic prompts now defer to the routing helper | `4b2c351cc1e` |
| 7 | 013 | "shadow-parity tests can assume `hub-identity` fans out hub-generic prompts to all 7 modes" | hub-identity is dropped; hub-generic prompts no longer co-fire all modes | `d60cc2c7553` |
| 8 | 013 | "model-benchmark and skill-benchmark are reachable via bare advisor alias for shadow-parity" | they are reachable only via `/deep:model-benchmark` and `/deep:skill-benchmark` command bridge | `d60cc2c7553` |
...
The most consequential single finding is that **two different drift engines hit phase 003 from two different commit categories**. Iteration 1 caught the kebab migration (`b052f329a73`, which IS in `.opencode/skills/system-deep-loop/`'s commit count); iteration 7 caught the renumber (`69aee63cb8a`, which is NOT in `.opencode/skills/system-deep-loop/`'s commit count — it touches `.opencode/specs/system-deep-loop/` instead). The spec's "22 system-deep-loop commits" header under-counts because it filters by skills path; the renumber lives in the specs path. The census's scope explicitly covers the full range, so the renumber was always in scope — but a triage that filtered to `-- .opencode/skills/system-deep-loop` would have missed it.
...
### Range and HEAD pin
- Baseline: `60b9ed8bc2` (2026-07-16 06:09:33) — the merge that landed the normalized 036 packet on v4.
...
- Range: `git log --oneline 60b9ed8bc2..739b85ac57 | wc -l` = **205** total (195 non-merge).
- Runtime subset: `git log --oneline 60b9ed8bc2..739b85ac57 -- .opencode/skills/system-deep-loop | wc -l` = **25** commits (spec said 22; branch moved past spec authoring).
...
The `69aee63cb8a` renumber commit was discovered at iteration 7 by searching phase-003 spec.md for the literal string "033" — not by filtering the commit range by path.
...
|----------|-------------------|----------|--------------|
| Trust the spec's commit counts (204/22) | Live range at pinned HEAD is 205/25; spec snapshot drifted post-authoring | `git log --oneline 60b9ed8bc2..739b85ac57 \| wc -l` = 205 | 1 |
...
| Carry iteration 1's "low-medium" risk rating for phase 007 into a "needs refinement" verdict | Typed-pair series touches skill-benchmark harness (phase 016/003 surface), not phase 007 shared services | `git show --stat 05d309d0d96`, `268fa65a558`, `536484dc28d` — all touch `deep-improvement/` or `shared/behavior-benchmark/` paths only | 4 |
...
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/003-drift-census-and-plan-revalidation/{spec.md, plan.md, tasks.md, checklist.md}` — the census charter.
...
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/001-fanout-live-tools-unblock/{spec.md, plan.md}`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/002-transition-authorized-ledger-core/spec.md`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/003-shared-evidence-and-control-services/spec.md`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/004-compatibility-shadow-and-rollback-bridge/spec.md`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/005-fanout-fanin-durable-orchestration/spec.md`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/006-novelty-claims-continuity-and-projections/spec.md`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/007-convergence-termination-and-health/spec.md`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/001-shared-mode-contracts-and-fixtures/spec.md`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/002-mode-and-lane-migrations/spec.md` (+ 8 mode children)
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/003-staged-state-migration-and-authority-cutover/spec.md`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/004-legacy-writer-retirement/{spec.md, plan.md}`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/001-whole-system-gate/{spec.md, plan.md}`
- `.opencode/specs/system-deep-loop/036-deep-loop-innovation/002-integrate-latest-and-closeout/{spec.md, plan.md}`

### Drift-engine commits
- `b052f329a73` — kebab-case migration (~3,700 renames; renamed `state_format.md`, `integration_points.md`, `behavior_benchmark.md` family).
- `69aee63cb8a` — packet renumber (compacted archive to 001-029, renumbered active to 030-036; rebound "Packet 033").
- `4b2c351cc1e` — flipped four hubs (incl. `system-deep-loop`) to `defaultMode: null`.
- `d60cc2c7553` — corrected mode routing (dropped `hub-identity` from all 7 modes; restricted `model-benchmark`/`skill-benchmark` to command-bridge-only).
- `fa74e868615` — implemented typed-pair routing surface (added `smart_routing.md`, `leaf-manifest.json`, `resourceContractVersion`).
- `739b85ac57` — cli-opencode fan-out dispatch unblock (orthogonal 17-line fix).
- `1accee48991` — `goal_opencode` → `goal-opencode` rename.
- `37c589e08e4` — de-skill-specific harness classifier + manifest freshness gate.
- `d5db5ecad3d`, `fe261503031` — reference-completion fixes.
...
- Artifact dir: `.opencode/specs/system-deep-loop/036-deep-loop-innovation/003-drift-census-and-plan-revalidation/research/lineages/glm/`
