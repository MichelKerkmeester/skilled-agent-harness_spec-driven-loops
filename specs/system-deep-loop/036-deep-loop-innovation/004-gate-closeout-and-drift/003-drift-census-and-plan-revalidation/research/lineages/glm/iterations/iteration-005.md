---
title: "Iteration 5: Phases 010-012 drift"
trigger_phrases: []
---
...
Iteration 5's focus was Q-005: drift over phases 010 (novelty claims + continuity + projections — flagged medium risk from `b052f329a73`'s continuity-reference renames), 011 (convergence + termination + health — flagged low-medium risk from `b052f329a73`'s convergence-reference renames), and 012 (shared mode contracts + fixtures — flagged HIGH risk from `d60cc2c7553`/`fa74e868615`/`4b2c351cc1e` plus the skill-benchmark typed-pair series). The highest-risk pending batch; most likely to surface additional drift beyond phases 003 and 013.
...
6. Verified `b052f329a73`'s scope on deep-loop continuity/convergence reference files via `git show --stat b052f329a73 | grep -E "(system-deep-loop|convergence|continuity|novelty|projection)"`.
...
`b052f329a73` did NOT rename `convergence.cjs` — it was already kebab-safe (no underscore). The kebab migration DID rename related deep-loop coverage-graph files (`coverage-graph/deep-loop-graph-convergence.md` and `deep-loop-graph-convergence-yaml-fire.md`), but phase 011 doesn't cite either.
...
|---------------------|----------------|------------|----------------------|
| `mode-registry.json` | 7 modes, no `resourceContractVersion` | 7 modes + `resourceContractVersion: 1` (added by `fa74e868615`) | The freeze must now include the typed-pair resource contract version. |
| `hub-router.json` `defaultMode` | `"research"` | `null` (flipped by `4b2c351cc1e`) | The freeze must capture the post-flip default (null), not the baseline default. |
| `hub-router.json` `routerSignals.{model-benchmark,skill-benchmark}` | bare advisor alias OK | command-bridge-only (restricted by `d60cc2c7553`) | The freeze must capture the restricted signal surface, not the baseline surface. |
| `shared/references/smart_routing.md` | ABSENT | PRESENT (added by `fa74e868615`) | The freeze must include this new file as part of the boundary. |
| `leaf-manifest.json` | ABSENT | PRESENT (added by `fa74e868615`) | The freeze must include the leaf-manifest as part of the boundary. |
...
**Phase 012 = needs refinement** (second-order: shared mode boundary grew by 5 components since baseline; freeze scope and mixed-version fixtures must expand to cover them; write-set conflict graph must add the new typed-pair surface). [SOURCE: `012/spec.md:47,55,65`; `git diff 60b9ed8bc2..739b85ac57 -- .opencode/skills/system-deep-loop/mode-registry.json` (resourceContractVersion added); `git diff 60b9ed8bc2..739b85ac57 -- .opencode/skills/system-deep-loop/hub-router.json` (defaultMode flipped, signals restricted); `git show --stat fa74e868615` adds `shared/references/smart_routing.md` + `leaf-manifest.json`.]
...
- `git show --stat b052f329a73 | grep -E "(system-deep-loop|convergence|continuity|novelty|projection)"` (renames touched session-continuity and coverage-graph docs, NOT phase 010/011 surfaces)
- Grep for snake_case paths across `010/**`, `011/**`, `012/**` — 0 hits each
- `git diff 60b9ed8bc2..739b85ac57 -- mode-registry.json` (resourceContractVersion added)
- `git diff 60b9ed8bc2..739b85ac57 -- hub-router.json` (defaultMode + signal restrictions)
- `git show --stat fa74e868615` (smart_routing.md + leaf-manifest.json added)
...
Iteration 6: Phases 014-015 drift. Phase 014 (staged state migration + authority cutover — touched indirectly by `1accee48991` path refs and mode-routing churn). Phase 015 (legacy writer retirement — flagged low risk; verify writer paths still exist). After iteration 6, only phases 016-017 + open question B remain before synthesis.
