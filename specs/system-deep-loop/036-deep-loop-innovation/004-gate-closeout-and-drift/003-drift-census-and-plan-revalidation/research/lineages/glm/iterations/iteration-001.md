...
Iteration 1's focus was Q-001: triage the commit range `60b9ed8bc2..739b85ac57` by phase surface and produce a per-phase hit-list, plus an opportunistic first pass at the Q-002 positive control (phase 003's renamed reference files and its `behavior_benchmark/` glob).

Pinned HEAD: `739b85ac57` ("fix(deep-loop): unblock cli-opencode fan-out lineage dispatch"). Baseline: `60b9ed8bc2` (2026-07-16, the merge that landed normalized 036 on v4).

## Actions Taken

1. `git log --oneline 60b9ed8bc2..HEAD` → **205** commits in range (spec.md said 204; the spec was off by one — `739b85ac57` itself landed after the spec was authored on 2026-07-19, see `git show -s --format=%ci 739b85ac57`). 195 non-merge commits.
2. `git log --oneline 60b9ed8bc2..HEAD -- .opencode/skills/system-deep-loop` → **25** commits touching the runtime substrate (spec.md said 22; three more landed between spec authoring and HEAD pin).
3. `git log --name-only --pretty=format: --no-merges 60b9ed8bc2..HEAD | awk -F'/' '{print $1"/"$2}' | sort | uniq -c | sort -rn` → top-level churn by category: `.opencode/specs` 62559 lines, `.opencode/skills` 15832, `.opencode/commands` 278, with long-tail edits to `.opencode/{bin,plugins,scripts,agents,install-guides,changelog,hooks,logs}`, `.claude/agents`, `.codex/{agents,prompts,hooks.json,config.toml}`, `.github/{workflows,hooks}`, and root configs.
4. `git show --stat b052f329a73` and rename-extraction grep → confirmed `b052f329a73` renamed `state_format.md`→`state-format.md`, `integration_points.md`→`integration-points.md`, and the `behavior_benchmark.md` family across all eight `system-deep-loop` modes.
5. Path resolution against pinned HEAD: `state_format.md` and `integration_points.md` under `runtime/references/` **DO NOT RESOLVE**; the kebab-case siblings `state-format.md` and `integration-points.md` do. `state_format.md` was PRESENT at baseline `60b9ed8bc2`, `state-format.md` was ABSENT — proving the rename is entirely post-baseline.
...
- `003/plan.md:81` names `.opencode/skills/system-deep-loop/runtime/references/state_format.md` — file no longer resolves. Kebab sibling `state-format.md` resolves. Renamed by `b052f329a73` (`{state_format.md => state-format.md}` under three separate subtrees including `runtime/references/`). [SOURCE: 003/plan.md:81, `git show --stat b052f329a73`, `ls runtime/references/`]
- `003/plan.md:82` names `integration_points.md` — same pattern. Kebab sibling `integration-points.md` resolves. Renamed by `b052f329a73` (`{integration_points.md => integration-points.md}`). [SOURCE: 003/plan.md:82]
...
|-------|--------------------------------|---------------------------|------------|
| 003 | Baseline taxonomy + state census (runtime references, behavior benchmarks) | `b052f329a73` (renames), `37c589e08e4` (de-skill-specific harness classifier + manifest freshness gate), `05d309d0d96` (typed resource pairs), skill-benchmark typed-pair series | **HIGH (confirmed)** |
| 004 | Architecture coverage + transition contract | typed-pair gold (`536484dc28d`, `268fa65a558`), `fa74e868615` (typed-pair routing surface) | medium |
| 005 | Fanout live-tools unblock | `739b85ac57` (cli-opencode fan-out dispatch unblock), `1accee48991` (goal-opencode rename + path refs) | medium |
| 006 | Transition authorized-ledger core | `d60cc2c7553` (mode routing + stale route gold), `fa74e868615` (typed-pair routing) | medium |
| 007 | Shared evidence + control services | `05d309d0d96`, `268fa65a558` (typed pairs in skill-benchmark), shared mode contract churn | low-medium |
| 008 | Compatibility shadow + rollback bridge | none directly; the rollback-bridge paths may share names with 015 | low |
| 009 | Fanout fan-in durable orchestration | `739b85ac57`, `1accee48991`; durable-orchestration script paths possibly touched by `1accee48991` | medium |
| 010 | Novelty claims + continuity + projections | `b052f329a73` renamed continuity references, `fe261503031` (deferred-integration reference completion) | medium |
| 011 | Convergence + termination + health | convergence references renamed by `b052f329a73`; possibly affected by mode-routing fixes | low-medium |
| 012 | Shared mode contracts + fixtures | `d60cc2c7553`, `fa74e868615`, `4b2c351cc1e` all directly touch mode registries and routing defaults; **also the skill-benchmark typed-pair series (`05d309d0d96`, `536484dc28d`, `268fa65a558`, `51260439e1b`, `ba279c75773`, `145e0b8ce35`, `ac1bfb15fde`, `37c589e08e4`)** | **HIGH** |
| 013 | Mode + lane migrations (8 workstreams) | `d60cc2c7553`, `fa74e868615`, `4b2c351cc1e` (OPEN QUESTION A) | **HIGH (open question A)** |
| 014 | Staged state migration + authority cutover | `1accee48991` (path refs), mode-routing churn | low-medium |
| 015 | Legacy writer retirement | none directly named; check writer paths still exist | low |
| 016 | Whole-system gate (incl. behavior-benchmark harness) | `37c589e08e4` (de-skill-specific harness classifier + manifest freshness gate) — directly touches the harness | medium |
| 017 | Integrate latest + closeout (drift-handling charter) | `d5db5ecad3d` (017 hyphen refs in benchmark suite), `fe261503031` (deferred-integration reference completion) | low |

**Cross-cutting:** `b052f329a73` (kebab migration) is a first-order-drift engine across every phase that names a snake_case path; `1accee48991` (goal_opencode rename) is a smaller first-order engine for any phase that names the old command file; the skill-benchmark typed-pair series is a second-order engine for any phase whose premises assume the old skill-authoritative single-source contract.

### F1.4 — Two open questions are tractable in this loop

- **OPEN QUESTION A (mode count):** commits `d60cc2c7553`, `fa74e868615`, `4b2c351cc1e` all touch routing defaults and mode registries. Phase 013 plans eight workstreams against a specific registered-mode count. To resolve: count the registered modes at baseline vs HEAD in the typed-pair routing manifest and the four flipped hubs.
...
- **Q-002** (phase 003 positive control): ANSWERED at the path-resolution level. Phase 003 has first-order drift on three named paths (two reference files + the `behavior_benchmark/` glob); attribution confirmed to `b052f329a73`; the kebab siblings exist and preserve content. Final verdict locked in iteration 8 synthesis, but evidence is complete.
...
- `git log --oneline 60b9ed8bc2..739b85ac57` (full range, 205 commits)
- `git log --oneline 60b9ed8bc2..739b85ac57 -- .opencode/skills/system-deep-loop` (25 runtime commits)
- `git log --name-only --pretty=format: --no-merges 60b9ed8bc2..739b85ac57` (top-level path churn)
- `git show --stat b052f329a73` (kebab-case migration squash, ~3000 files)
- `git cat-file -e 60b9ed8bc2:...state_format.md` (present at baseline) and `60b9ed8bc2:...state-format.md` (absent at baseline)
...
- Treating the kebab-case migration `b052f329a73` as a first-class "drift engine" rather than one of 25 commits made the per-phase triage collapse fast: every snake_case path cited by a phase is a candidate first-order hit.
...
- _Approach:_ "trust the spec's commit counts". _Reason ruled out:_ spec said 204/22; live range is 205/25. The discrepancy is post-spec-authoring drift on the branch itself. Verdicts must cite the live counts against the pinned HEAD, not the spec's snapshot. _Evidence:_ `git log --oneline 60b9ed8bc2..739b85ac57 | wc -l` = 205.

## Recommended Next Focus

Iteration 2: Open question A's anchor — resolve the registered-mode count. Read `003-baseline-taxonomy-and-state-census` and `002-mode-and-lane-migrations` plan.md to enumerate the modes they assume, then count registered modes in the typed-pair routing manifest and the four hubs `4b2c351cc1e` flipped to `defaultMode: null` at HEAD vs baseline. This both advances Q-006 (phase 013 verdict) and produces the most load-bearing open-question evidence early, before budget pressure forces breadth over depth.
