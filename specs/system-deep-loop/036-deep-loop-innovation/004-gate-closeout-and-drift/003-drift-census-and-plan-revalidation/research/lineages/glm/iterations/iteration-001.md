...
Iteration 2: Open question A's anchor — resolve the registered-mode count. Read `003-baseline-taxonomy-and-state-census` and `002-mode-and-lane-migrations` plan.md to enumerate the modes they assume, then count registered modes in the typed-pair routing manifest and the four hubs `908efde8d8f` flipped to `defaultMode: null` at HEAD vs baseline. This both advances Q-006 (phase 013 verdict) and produces the most load-bearing open-question evidence early, before budget pressure forces breadth over depth.
