...
- [~] CHK-006 [P2] **UNVERIFIABLE FROM THE RECEIPTS — terminal for this packet.** Audited rather than left vague. Both lineage configs set `maxToolCallsPerIteration: 12`, but `deep-research-state.jsonl` records `toolsUsed` as a list of distinct tool *names*, never call counts, so budget adherence cannot be derived from the run's own evidence for either lineage. On the sub-agent half the two lineages differ: sol logs `toolsUsed` on all 7 iterations and no entry is a dispatch tool (`skill`, `read`, `glob`, `grep`, `bash`, `apply_patch`, `memory_match_triggers` only), with `target_agent=deep-research` throughout — evidence of leaf-only execution, though absence of a dispatch tool name is not proof none was spawned mid-iteration; glm omits `toolsUsed` entirely on all 7 iterations, so nothing about it is auditable. Closing this properly needs per-iteration call counts in the state ledger, which is an observability change, not a re-reading of these artifacts
<!-- /ANCHOR:code-quality -->
...
- [~] CHK-017 [P1] **FAILED — forced depth did not apply. Terminal for this run; root cause since closed.** `stopPolicy` was passed inside the fan-out config JSON, but `fanoutConfigSchema` has no such key and zod stripped it silently; `fanout-run.cjs:1512` reads it only from the `--stop-policy` CLI flag. Both lineages ran 7/10 and stopped on `converged`. Impact is asymmetric: sol's stop was legitimate (composite 0.70 > 0.60, coverage 1.00, all guards passed); glm's stopping point is unverifiable. The run cannot be retroactively given the depth it did not run at, so this stays a recorded failure. The defect that caused it is now closed: the shared fan-out control shape rejects a `stopPolicy` key outright rather than letting object parsing drop it, so the same mistake fails loudly instead of producing artifacts that read as a legitimate convergence. Both branches of the config union are covered, with a regression test proven red before the guard and green after
...
| Forced depth | CHK-017 | **FAIL** — recorded, not smoothed over; sol unaffected, glm's stop unverifiable. Permanent for this run, not closeable without a re-run. The underlying defect is now fixed: a `stopPolicy` smuggled into fan-out config is rejected instead of silently dropped |
| Leaf tool budget | CHK-006 | **UNVERIFIABLE** — the budget is 12 calls per iteration, but the state ledger records distinct tool names rather than call counts, so neither lineage can be audited against it from its own receipts |
