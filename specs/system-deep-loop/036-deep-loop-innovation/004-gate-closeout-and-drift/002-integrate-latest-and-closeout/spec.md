...
_memory:
  continuity:
    packet_pointer: "system-deep-loop/036-deep-loop-innovation/002-integrate-latest-and-closeout"
...
- Rerunning the full `001-whole-system-gate` contract on the final SHA, including exact-SHA behavior and mode baselines, mixed-version replay, crash injection, counterfactual and degeneration tests, parity against 000, blocking SOL review, and recursive strict validation.
- Reconciling 036 open items append-only, updating packet and phase changelogs, regenerating `description.json` and `graph-metadata.json` through deterministic tooling, and rolling the final state up to the 036 parent.

### Out of Scope
- Implementing or silently patching an earlier phase after drift; affected work returns to that phase's own contract.
- Rewriting 036/001 or 036/002 research artifacts, deleting open-item history, or inventing recommendations outside the frozen 178-row ledger.
...
| REQ-005 | Reconcile the program and generated packet state append-only | Every 036 open item has a disposition or explicit carried-forward owner, changelogs record the integration and gate result, and deterministic metadata is regenerated across the affected packet tree |
...
| REQ-007 | Reconcile parent and child statuses | The 036 phase map, child status fields, completion percentages, open-item ledger, changelog entries, and generated metadata agree on the final state |
...
- **SC-003**: The full `001-whole-system-gate` verification is green on the exact final SHA, including SOL review and recursive strict validation.
- **SC-004**: All 036 open items, changelogs, parent rollup fields, and deterministic packet metadata are reconciled append-only and mutually consistent.
<!-- /ANCHOR:success-criteria -->
...
| Dependency | `001-whole-system-gate` contract and receipts | The final program claim cannot be reproduced | Treat the phase-016 outcome in the parent and manifest as the minimum gate contract; require the phase-016 artifact set before execution |
...
- Which 036 open items remain carried forward after the final gate, with an owner and next action?
<!-- /ANCHOR:questions -->
