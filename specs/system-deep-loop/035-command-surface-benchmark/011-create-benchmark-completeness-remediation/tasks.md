...
- [x] T001 — Fold the remediation into packet 066 as this child; author spec, plan, tasks, checklist; generate metadata. (`105d6a283a`)
...
- [x] T003 (P1) — **Router fallback + family keying.** `DEFAULT_RESOURCE` → `references/shared/README.md`; `FAMILY_DISK_KEY` maps `mcp_promotion`→`shared`. Shipped `7a4ff81f0b`; re-review confirmed CLOSED (Sol P1-5).
- [x] T004 (P1) — **Behavior schema-v2 scaffold.** Scenario template carries a v2 block; index schemaVersion wording conditional; guide §6 teaches one-block-per-file. Shipped `7a4ff81f0b`. Re-review found scaffold defects (v1 block lacked evidence_kind/min_seats for ai-council/improvement; postcondition/command_topology gaps) — all corrected in `1c80093ea3`.
- [x] T005 (P1) — **Conformance exemplar completion.** Authored the family README + `command-surface/conformance_benchmark.md`; corrected the "planned adapter" wording. Shipped `718404ef3a`. Re-review found a false "no run" status and a broken verify command — both corrected in `1c80093ea3`.

**P2 (create-benchmark-local)**

- [x] T006 (P2) — **Command-benchmark first-classing.** Added `references/shared/command_benchmark_composition.md` (matrix-manifest field shape), composition note, routing keys, unknown-fallback line. Shipped `8026e536b9`; router-overclaim + resultPointer gaps corrected in `1c80093ea3`.
- [x] T007 (P2) — **Route-map refresh.** `references/shared/README.md` now lists conformance (§12), Lane A (§14), and the composition guide. Shipped `8026e536b9`.
- [x] T008 (P2) — **Phantom Lane D + guide wording.** Removed the "Lane A/D" phantom. The scorer/seed reconciliation was over-corrected and re-fixed in `1c80093ea3` (author does NOT select scorer/seed; lane-owned). Shipped `8026e536b9`.
- [x] T009 (P2) — **Deep-command links.** `/deep:command-benchmark` real link in both guides; Lane A `/deep:agent-improvement` retargeted to the command; direct `/deep:model-benchmark` link added. Shipped `8026e536b9`.
- [x] T010 (P2) — **Lane C fixture-authoring link.** Linked `scenario_authoring.md` + the `create-manual-testing-playbook` corpus handoff. Shipped `8026e536b9`.

**P2 (cross-tree exemplar sweep — full-sweep, hyphen-pilot-aware)**

- [x] T011 (P2) — **MCP consumer README + template href.** Validator path → `sk-doc/shared/scripts/`; dangling "Section 6" → create-benchmark SKILL §4–§8; template href `../../README.md`→`../README.md`. Non-promoted folders already carried HOLD/UNREPORTED status (no redundant "legacy" tag). Shipped `7969aa0bc4`.
- [x] T012 (P2) — **Lane C exemplar READMEs.** Added run-label index (system-deep-loop), authored the missing deep-improvement index, labeled sk-code legacy sidecars (no rename, no hyphen reversion). Shipped `0792b1d633`; sk-code headline contradiction re-fixed in `1c80093ea3`.
- [x] T013 (P2) — **Behavior-index back-pointers.** Added create-benchmark back-pointers to the four deep-mode behavior_benchmark.md indexes. Shipped `0792b1d633`.
<!-- /ANCHOR:phase-2 -->

<!-- ANCHOR:phase-3 -->
## Phase 3: Verification

- [x] T014 (P1) — **GPT-5.6 Sol Ultra Fast re-review.** Dispatched against the patched tree (tip `0792b1d633`); report archived at `evidence/review-sol-ultra-rereview.md`. Verdict FAIL: 10 new issues (4 P1) plus surviving PARTIAL findings. Every load-bearing claim was independently verified against real files; the confirmed regressions were closed in `1c80093ea3`, and the deeper structural findings are recorded below (T016).
- [x] T015 (P1) — **Validate + ship.** `validate_document.py` 0 issues on every edited doc; `validate.sh --strict` on this child is Errors:0 Warnings:0 RESULT:PASSED; commits pathspec-scoped + FF-pushed to `skilled/v4.0.0.0` (`7a4ff81f0b`, `718404ef3a`, `8026e536b9`, `7969aa0bc4`, `0792b1d633`, `1c80093ea3`); completion metadata reconciled here.

**Re-review regressions (all CLOSED in `1c80093ea3`)**
...
- N4 = the `718404ef3a` commit-sweep of 16 concurrent-session files (disclosed to operator; not un-swept — needs force-push, forbidden). N10 = this metadata reconciliation.
