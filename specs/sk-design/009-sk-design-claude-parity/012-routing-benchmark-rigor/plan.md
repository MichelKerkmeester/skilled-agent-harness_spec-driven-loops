...
This phase plans an update to the `sk-design` Lane-C benchmark corpus and scenarios for Phases 006-011's changes, the freezing of a new `baseline-post-011/` comparison anchor once those phases land, and a rigorous, materially larger routing-accuracy benchmark covering skill-advisor top-1 confidence and gap-to-second, hub mode-resolution accuracy, and procedure-card selection accuracy per mode, in both router and live trace modes. Numeric "near-perfect routing" floors and a remediation loop are defined as release-gate decisions in `decision-record.md`. **This plan was not executed as originally written.** Per `decision-record.md` ADR-003 (Accepted, 2026-07-06), Phase 012 closes instead on the existing, real `benchmark/2026-07-06--after-012-routing-rigor--router/report.{json,md}` Mode A router-mode evidence (verdict PASS, aggregate 100/100), which scores the same non-expanded 24-scenario corpus as `benchmark/2026-07-06--after-009--router/`. The corpus expansion, `07--procedure-card-selection/` and `08--advisor-confidence-battery/` categories, `advisor-probe.cjs`/`score-skill-benchmark.cjs` harness extension, live-mode rerun, and `benchmark/baseline-post-011/` promotion described in the remainder of this plan were never built; they remain descoped, open work for a future phase, not silently claimed as done. The sections below are retained as historical planning context.
<!-- /ANCHOR:summary -->
...
- [ ] `benchmark/baseline/` and `benchmark/2026-07-06--after-009--router/` have been read and their scenario ids, dimension scores, and conventions are understood as the immutable comparison history. **Not executed for this reconciliation** — this pass reads `benchmark/2026-07-06--after-012-routing-rigor--router/` directly instead; no comparison against `baseline/`/`after-009/` scenario ids was performed.
...
- [ ] `benchmark/baseline-post-011/` exists only after explicit release-owner confirmation and promotion; `benchmark/baseline/` and `benchmark/2026-07-06--after-009--router/` are untouched. **Not executed** — no `baseline-post-011/` folder was created; `baseline/` and `after-009/` remain untouched (confirmed, no promotion attempted).
...
- **Post-011 baseline**: A new, explicitly promoted `benchmark/baseline-post-011/` folder that becomes the comparison anchor for Phase 013 onward, appended alongside (not replacing) `benchmark/baseline/` and `benchmark/2026-07-06--after-009--router/`.
...
- [ ] Read `benchmark/baseline/`, `benchmark/2026-07-06--after-009--router/`, and `benchmark/README.md` to capture the existing scenario ids, dimension scores, and conventions.
