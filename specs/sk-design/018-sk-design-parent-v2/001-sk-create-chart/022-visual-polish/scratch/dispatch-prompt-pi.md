...
You are a non-interactive dispatched worker. `AI_SESSION_CHILD=1` and `SYSTEM_SPEC_GATE_ENFORCE=0`
are set in your environment, which this repository's AGENTS.md defines as the autonomous
child-dispatch exemption: the spec-folder question is pre-resolved and MUST NOT be asked. Nobody is
at a prompt; no answer can reach you. Your write authority is already bound to two roots:
  .opencode/skills/sk-design/sk-design-chart/                                     (the chart skill)
  specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/022-visual-polish/   (this packet)
Write nothing outside them. Proceed directly to the work; your task is complete only when files
exist on disk and the verification commands below have been run and their output read.

# RUNTIME AND GUARDS

- Runtime: pi CLI, print mode, one shot. You are the leaf. Never dispatch pi, codex, another CLI,
  or a sub-agent; do everything yourself in this process.
- Do not run any git command that changes state (no add, commit, stash, checkout, reset). The
  conductor commits. Read-only git (status, diff, log) is fine.
- No external resource may enter a template: no CDN, no font URL, no import. The checker's
  `no-external` family errors on any, and that rule is a constraint, not a trade-off.
- Do not loosen, skip, or delete any existing assertion, gate or threshold in
  `scripts/check-corpus.cjs`. Retune values where the packet says so; never remove a check. Per-family
  assertion counts must not fall below the baseline you record first.
- `toLocaleString`, `Intl.NumberFormat` and `Intl.DateTimeFormat` stay forbidden by the
  `number-format` family. Write formatters by hand.
- Comment hygiene is a hard rule: no spec paths, task ids, requirement ids or decision ids inside
  code comments. Write the durable why.
- Palette values in `assets/color/palettes.json` and every numeric gate are frozen.
...
# TASK: visual polish pass on the chart corpus

Read first, in this order:
1. `specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/022-visual-polish/spec.md` (section 3 lists the six moves; sections 4 and 5 are the bar), `goal.md` (D1 to D5 are frozen), `plan.md`, `tasks.md`, `acceptance-criteria.md`.
2. `specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/021-visual-upgrade-research/research/research.md` for the evidence behind each move.
3. `.opencode/skills/sk-design/sk-design-chart/references/template-contract.md`, `scripts/check-corpus.cjs` in full, `scripts/README.md`, `changelog/v1.5.0.0.md`.
4. One template end to end, `assets/templates/daily-line.html`: the `CHART_DATA`, palette, geometry, `READOUT` and `CURVE` blocks, the card markup with its `data-chart-part` attributes, the footer, the table, the tooltip card and the drawing code.

The six moves, applied to all 26 templates and all 7 deliveries under `assets/examples/`:

A. **Source line.** The sentence "Replace the data block at the top of the script and nothing else." leaves every visible source line (the element with `data-chart-part="source"`) and moves into a script comment directly above `CHART_DATA`. Write a `source-line` assertion in the checker that fails a file whose source text contains "Replace the data block"; prove it on a mutated copy first; record the failing line in `scratch/mutations.md`.

B. **Plot proportion.** Survey every form's viewBox into `scratch/proportions.md` (width, height, ratio, target). For cartesian forms raise the viewBox height so the plot is 50 to 56 percent of frame width, moving the baseline, tick and legend geometry with it; ordered and non-cartesian forms (calendar-grid, heat-matrix, unit-grid, unit-ring, treemap, progress-single, bullet, funnel) keep their proportions where a taller frame would only add empty ground, and the survey says which and why. The `GEOMETRY DEFAULTS` block is a shared record compared across files: update it in every file identically if a shared value changes, and run the checker after every few forms. The narrow-viewport pan floor must still hold.

C. **Table disclosure.** Wrap the data table in a native `details` element with a `summary` reading "Show the data"; the `details` is open for inert forms (those with no `data-chart-tooltip`) and closed for tooltip forms. Keep `data-chart-table` on the table itself so the accessibility and card-readout checks read it unchanged. Write a `table-disclosure` assertion (a details element wraps the table, a summary is present, the open state matches the form's tooltip register); prove it on a mutated copy first. Test the render gate on one tooltip form before sweeping: the card-readout driver reads the DOM, but confirm it.

D. **Finding direction cue.** Add a `FINDING` sentinel block beside `READOUT` (same comment-delimited shape) with a `trend` field of `up`, `down` or `none` and a one-line rationale. The footer finding draws an inline SVG arrow before the sentence when trend is up or down, 12px, in the emphasis colour, `aria-hidden`, and nothing when none. Choose the trend per form from its data and finding sentence. Write a `finding-cue` assertion (block present on every form with a footer finding, trend in the set, arrow present when up or down and absent when none); prove it on a mutated copy first.

E. **Compact numbers and units.** Add a `unit` field to every `READOUT` block (a short string or empty). The tooltip card prints the unit after the value with a thin space. A hand-written `compact()` formatter renders axis ticks at or above five digits as `12k`, `1.2M`, one decimal at most, and is used by the tick code; values in the card and the table keep full digits with thousands separators. Extend the `number-format` family so a `READOUT` without `unit` fails; prove it on a mutated copy.

F. **Contract text.** In `references/template-contract.md` correct the single-area fade figures to what ships (0.35 to 0.04 for a neutral single series; 0.4 flat for stacks; the reference to 0.8 to 0.1 stays only as the frozen shadcn measurement it came from), and document `FINDING`, `READOUT.unit`, the disclosure and the compact tick rule in the contract's own voice. Add `changelog/v1.6.0.0.md` in the shape of `v1.5.0.0.md`; bump `version` in `SKILL.md` and `README.md` to 1.6.0.0.

Order of work: baseline counts into `scratch/baseline.md`; the four assertions with their mutations; `daily-line.html` through all six moves and the static gate; then the other 25 forms one at a time; then the 7 deliveries; then `node scripts/build-gallery.cjs`; then F; then the packet close-out: fill `implementation-summary.md` from its anchors, set each acceptance row's Verification cell to observed evidence and its Status to Met or Unmet, tick `tasks.md`, update `goal.md` progress and criteria, set `spec.md` and the acceptance metadata Status, and regenerate metadata and validate:
    node .opencode/skills/system-spec-kit/runtime/cli/dist/spec-folder/generate-description.js specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/022-visual-polish "$PWD"
    node .opencode/skills/system-spec-kit/runtime/cli/dist/graph/backfill-graph-metadata.js specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/022-visual-polish
    NODE_PRESERVE_SYMLINKS=1 bash "$(realpath .opencode)/skills/system-spec-kit/runtime/cli/spec/validate.sh" specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/022-visual-polish --strict --no-recursive
...
# VERIFICATION (run all; paste the summary lines in the handback)
    ...
    rg -c "Replace the data block" .opencode/skills/sk-design/sk-design-chart/assets/templates .opencode/skills/sk-design/sk-design-chart/assets/examples
    rg -c "FINDING:BEGIN" .opencode/skills/sk-design/sk-design-chart/assets/templates
    git diff --stat -- .opencode/skills/sk-design/sk-design-chart/assets/color
    validate.sh as above

The render gate and the screenshot pass need the installed Google Chrome; if Chrome cannot launch from your process, say so plainly and leave both to the conductor. Otherwise also run
    cd .opencode/skills/sk-design/sk-design-chart && node ../shared/scripts/render-screenshots.cjs ./assets ./screenshots
...
files_changed: one line per file group
assertions_added: name, what it errors on, the mutation's exact failure line; per-family counts before and after
verification: every command above with its summary line; render and screenshot status
failures: anything you could not verify or had to do differently, and why. Never claim completion without the command outputs above.
