...
Your write authority is already bound. The spec folder is:
  specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/016-design-md-theming
...
- Write only under these two roots. Anything else is out of scope, however tempting:
    ...
    specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/016-design-md-theming/   (this packet's docs and scratch/)
...
# TASK: theme the chart corpus from a DESIGN.md Style Reference

Read first, in this order:
1. `specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/016-design-md-theming/spec.md` (section 3 is the scope, seven items; sections 4 and 5 are the bar), `goal.md` (decisions D1 to D5 are frozen), `plan.md`, `tasks.md`, `acceptance-criteria.md`.
2. `.opencode/skills/sk-design/sk-design-md-generator/references/design-md-format.md` sections 3, 4 and 5 (the v3 parse contract) and the four bundled examples under `.opencode/skills/sk-design/sk-design-md-generator/references/examples/{stripe,vercel,linear,supabase}/` (`DESIGN.md` plus `tokens.json`). These are your fixtures. Never fetch anything.
3. `.opencode/skills/sk-design/sk-design-chart/scripts/check-corpus.cjs` in full, with attention to the palette blocks (`canonicalBlock`, `canonicalDarkBlock`, `checkPaletteSource`, the `THEMES` table, the `gates` read from `assets/color/palettes.json`), `assets/color/palettes.json`, `references/color-system.md`, `references/template-contract.md`, `SKILL.md`, `scripts/README.md`, `changelog/v1.3.0.0.md`.
4. One template end to end, `assets/templates/daily-line.html`, so you know the two palette blocks, the `data-scheme` pin, the font stacks and the corner ladder you will rewrite.

What to build (tasks T001 to T013 in `tasks.md`; tick each with one line of evidence):

A. `scripts/apply-design-md.cjs`, a CommonJS script with no dependency. CLI: `node apply-design-md.cjs <DESIGN.md> --out <dir> (--forms a,b | --all) [--tokens tokens.json] [--scheme light|dark|both]`. A URL argument is refused. It parses the `## Tokens — Colors` table (Name, Value, Token, Role), the primary typeface heading and its Substitute line under `## Tokens — Typography`, and the `### Border Radius` table, by the headings the format document names; a missing section is reported by name and the run stops. It maps roles as spec section 3 item 2 says: surface, ink, muted, rule, series one to four, emphasis, for the light ground and the dark ground (dark from a declared dark theme or `tokens.json` `darkMode` when present, else the stock dark chrome with series re-derived against it). It computes every gate in `palettes.json` `gates` on the derived values with the same contrast arithmetic the checker uses (reuse the checker's functions by requiring the module if they are exported, else lift them into a shared file both scripts require; do not duplicate the maths) and refuses to write on any failure, printing role, ratio, gate and the nearest table colour that clears. It prints one mapping line per role with the token name and ratio, and ends with `RESULT: PASSED` or `RESULT: FAILED`. On pass it writes each chosen form into the output directory with: both palette blocks rewritten as `system=design-md`, a provenance comment directly under the begin marker carrying the `DESIGN.md` path as given, its SHA-256 and the script's version, the body and mono font stacks replaced by the measured face followed by the substitute stack, and the corner ladder mapped to the nearest measured radius per rung without exceeding the stock rung. Everything else byte-identical. Two runs on the same input produce identical output.

B. `check-corpus.cjs`: a `design-md` branch beside the source-equality path, never inside it. For a block whose system is `design-md`, require the provenance comment with a well-formed 64-hex hash and compute the same gates on the inline values in both themes; every other family runs unchanged. Add `--extra <dir>` to scan an outside directory of themed files with the file-level families. Prove the branch with two mutations recorded in `scratch/mutations.md`: a themed copy with a malformed hash, and one with an inline series value pushed below the mark gate; and prove stock equality is intact by a one-byte palette edit on a stock form. Restore every mutated file.

C. The proof delivery: run the script on the stripe example for one form that shows the palette well (`grouped-bars` or `stacked-area`) into `assets/examples/` with a delivery file name in the corpus' own naming style, give it real data and a real headline the way the other deliveries have, and make the full corpus pass static. Try `--render`; if Chrome aborts in this sandbox, say so and leave it to the conductor.

D. Docs: `references/design-md-theming.md` (mapping rules, gates, refusal, provenance, in the corpus' voice); `template-contract.md` and `color-system.md` record the fourth system and that it is accepted by provenance plus gates; `scripts/README.md` documents the script and `--extra`; `changelog/v1.4.0.0.md` in the shape of `v1.3.0.0.md`; bump the version wherever `SKILL.md` and `README.md` carry it. `SKILL.md`: add the activation trigger and keyword triggers for a request naming a `DESIGN.md`, a style reference or a measured site's look, add the routing branch to the script, and rewrite the boundary sentence so extraction goes to `sk-design-md-generator` and application stays here.

E. Tests: `scripts/tests/apply-design-md.test.cjs` runnable with `node --test`: the four examples parse and derive a passing palette (or a documented refusal, if one example genuinely cannot clear a gate; say which and why), one refusal fixture you author under `scripts/tests/fixtures/`, and one byte-identity check on a themed copy limited to the four rewritten regions.

F. Packet close-out: fill `implementation-summary.md` from its anchors; set each acceptance row's Verification cell to observed evidence and Status to Met or Unmet; tick `tasks.md`; update `goal.md` progress and criteria; set `spec.md` and acceptance metadata Status. Regenerate metadata and validate:
    node .opencode/skills/system-spec-kit/runtime/cli/dist/spec-folder/generate-description.js specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/016-design-md-theming "$PWD"
    node .opencode/skills/system-spec-kit/runtime/cli/dist/graph/backfill-graph-metadata.js specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/016-design-md-theming
    NODE_PRESERVE_SYMLINKS=1 bash "$(realpath .opencode)/skills/system-spec-kit/runtime/cli/spec/validate.sh" specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/016-design-md-theming --strict --no-recursive
  Require the literal `RESULT: PASSED`. Acceptance cells must not contain a bare pipe inside inline code.

Rules that bind throughout: write only under the two roots named above; `assets/color/palettes.json` and every stock template and the three stock systems do not change; no external reference enters any file; comment hygiene is hard; every claim in the packet docs is backed by a command you ran.

# VERIFICATION (run all, paste the summary lines in the handback)

    node --check .opencode/skills/sk-design/sk-design-chart/scripts/apply-design-md.cjs
    node --test .opencode/skills/sk-design/sk-design-chart/scripts/tests/
    node .opencode/skills/sk-design/sk-design-chart/scripts/apply-design-md.cjs .opencode/skills/sk-design/sk-design-md-generator/references/examples/stripe/DESIGN.md --forms bar-columns,daily-line --out specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/016-design-md-theming/scratch/themed
    node .opencode/skills/sk-design/sk-design-chart/scripts/check-corpus.cjs --extra specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/016-design-md-theming/scratch/themed
    ...
    git diff --stat -- .opencode/skills/sk-design/sk-design-chart/assets/color .opencode/skills/sk-design/sk-design-chart/assets/templates
...
MAPPING: the mapping lines the stripe run printed.
ASSERTIONS ADDED: the design-md branch and --extra, each with its mutation's exact failure line; the stock one-byte edit's failure line.
VERIFICATION: every command above with its summary line; render status (PASSED or UNKNOWN with the reason).
