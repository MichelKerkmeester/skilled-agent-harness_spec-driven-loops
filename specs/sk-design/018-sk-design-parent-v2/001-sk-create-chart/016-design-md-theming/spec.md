...
2. **Role mapping.** Surface is the lightest background-role colour; ink the darkest text-role colour; muted the text-role colour nearest the muted ratio the stock chrome holds; rule is ink at the stock alpha. Series one to four are the chromatic tokens ranked by prominence in the table, filtered so each clears the mark gate on the derived surface and each adjacent pair clears the separation floor; emphasis is the most saturated accent that clears the emphasis-against-first-series floor. When the `DESIGN.md` declares a dark theme or `tokens.json` carries `darkMode`, the dark ground is derived the same way from those values; otherwise the stock dark chrome is kept and the series are re-derived against it, following the theme-boundary rule in `color-system.md`. Every mapping decision is printed with the token name it chose and the ratio it measured.
3. **Gate before write.** The script computes the corpus gates from `palettes.json` (`textOnSurface`, `markOnSurface`, ramp and separation floors, emphasis floor) on the derived palette and refuses to write when one fails, naming the failing role, the ratio, the gate and the nearest table colour that would clear it. A `--force` flag is not provided.
4. **Themed output.** Each chosen form is copied with its palette blocks rewritten as `system=design-md`, a provenance comment carrying the `DESIGN.md` path, its SHA-256 and the generator's version, the body and mono font stacks set from the typeface section with the substitute stack preserved, and the corner ladder mapped from the radius table (nearest measured value per rung, never above the stock rung's role). `CHART_DATA`, `READOUT`, `CURVE`, geometry and everything else stay byte-identical.
