...
- [x] `validate.sh specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/007-fidelity-and-library-research --strict` prints `RESULT: PASSED`
<!-- /ANCHOR:completion -->
...
| T5 series-mapping descriptions | Partial | The five forms with a detached key now map each series to its swatch in the `<desc>`, against key and stack order rather than colour names. `stacked-bars.html` names platform, services and support bottom upward. Partial rather than done, on the verification ledger's stricter reading: `scatter.html:77`, `heat-matrix.html:82` and `waterfall.html:77` still state facts read off the data block that no edit to that block will update |
...
| T8 gradient ramp legends | Partial, by decision | `heat-matrix` had no legend at all and gained a stepped ramp reading `from 6 … up to 101`. Neither shaded form got a gradient: both band into five steps, and a continuous bar would claim a resolution the encoding lacks. The ledger reads `calendar-grid.html:229-239` and `heat-matrix.html:219-223` as discrete swatches rather than a ramp, which is correct and is the decision ADR-006 records, not a gap |
...
| C1 narrow-viewport assertion | Partial, and stated | `narrow-viewport` in `check-corpus.cjs`, 87 assertions, rule 14 in the contract. Proven to fail three ways on a mutated `scatter.html`. Asserted from the stylesheet because a headless run returns the DOM and the answer lives in layout, and both documents say so. `scripts/check-corpus.cjs:476-485` states the limit in the script itself, which is why the ledger reads it as partial |
...
| Citation fold-back into the research prose | Done | All ten corrections are inline in `research/lineages/deepseek-flash-max/research.md`, each marked `CORRECTED 2026-09-03`, and both unreachable sources are marked `UNVERIFIABLE` beside the claim rather than deleted. The mode's references needed no change, because none of them cites an upstream URL |
...
| The verification ledger reads three items more strictly than this phase did | T5, T8 and C1 were recorded as done and are each partial. T5 and C1 are partial in fact, and T8 is partial by the decision in ADR-006. The rows above now say so, and `implementation-summary.md` carries the three-row reconciliation |
| Two corrected claims were softened rather than re-sourced | The Vega ARIA-bloat concern rested on a release note that would not serve, and the Plotly hover default was read off an axes page that never carried it. Neither has another source, so the research prose now claims less rather than citing a page that does not say it |
<!-- /ANCHOR:log -->
