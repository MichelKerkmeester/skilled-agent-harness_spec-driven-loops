...
- **Corpus verdict:** The pinned corpus follows the same data/config separation at the delivery level: one literal data block precedes drawing code, and the contract makes that block the auditable source for the figure. It intentionally does not generalize to a reusable 2D-array component because each standalone file has one named data shape. `[CORPUS@3ce648be44: .opencode/skills/sk-doc/sk-create-chart/references/template-contract.md:85-106; .opencode/skills/sk-doc/sk-create-chart/references/catalog.md:71-76]`
