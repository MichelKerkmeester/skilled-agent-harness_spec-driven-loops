...
_memory:
  continuity:
    packet_pointer: "sk-design/018-sk-design-parent-v2/001-sk-create-chart/009-chart-visual-overhaul/002-chrome-rollout"
...
**Packet:** sk-design/018-sk-design-parent-v2/001-sk-create-chart/009-chart-visual-overhaul/002-chrome-rollout
**Level:** 3
**Status:** Complete
**Date:** 2026-09-03
<!-- /ANCHOR:metadata -->
...
Every per-file count below was taken against the corpus this phase closed on, which held
29 asset files. Phase 007 added a twenty-first form, `templates/bar-line-composed.html`,
so a re-run today returns one more wherever the number counts files. That one file carries
the dashed grid, the muted tick and the mono stack, which is the whole of the difference,
and it rounds its columns on the same rung AC-020 checks.
...
| AC-018 | REQ-005 | Given a fill sits under a mark that carries the value, When A9 rolls out, Then it fades toward the baseline | `grep -rl 'id="area-fade"' $CHART/assets/` returns `templates/daily-line.html` and `examples/orders-after-the-price-change.html`, and each paints its stops from `--chart-series-1` at 0.18 down to 0 through `.fade-top` and `.fade-base`, at `assets/templates/daily-line.html:112-113` and `assets/examples/orders-after-the-price-change.html:97-98`. The row first named the broader `grep -rl 'linearGradient'`, which returns three files today and did return two when the row was written. The third is `templates/progress-single.html`, which gained a `progress-ramp` in phase 006 at `assets/templates/progress-single.html:166`: a hue ramp along the fill from `--chart-series-4` to `--chart-series-1` at `:119-120`, not an opacity fade to a baseline and not an area fill, so the row never reached it. `daily-range` and `stacked-area` are out of the row's reach and ADR-006 says why | Met | - |
...
| AC-020 | REQ-008 | Given bar marks draw square corners, When the mark rung lands, Then a bar leaves square every edge that meets something and takes the mark rung on every edge that meets nothing, with two forms whose bars meet nothing at more than one edge named below | The rung is `--chart-radius-mark: 2px`, declared once per file at `assets/templates/bar-columns.html:16` and identical everywhere in the corpus: `grep -rho -- '--chart-radius-mark: [^;]*' $CHART/assets/ \| sort -u` prints a single line, and each form reads it into `MARK_R` before drawing. Four of the six named forms round one edge at that value through a path: `bar-columns`, `grouped-bars` and the top segment of `stacked-bars` round the end away from the baseline through `topRounded` (`assets/templates/bar-columns.html:248`, applied at `:282`), and `bar-rows` rounds the end away from the axis through `endRounded` (`assets/templates/bar-rows.html:229`, applied at `:257`). The delivery `staff-hours-by-service` matches `bar-rows`. The two exceptions obey the same rule rather than escaping it. `waterfall` draws a total bar as a `topRounded` path at `assets/templates/waterfall.html:316` and a floating step bar as a rect carrying `rx: var(--chart-radius-mark)` at `assets/templates/waterfall.html:112`, so that step takes 2px on all four corners, because a bar hanging between two running totals meets nothing at either end. `progress-single` is a pill: its track and fill share `rx: var(--chart-radius-pill)` at 8px at both ends (`assets/templates/progress-single.html:111`), because that bar meets a track rather than a baseline. `grouped-bars` also declares `rx: var(--chart-radius-mark)` on `.past, .current` at `assets/templates/grouped-bars.html:108`, which paints the legend swatch rect it assigns at `:311` and is inert on the path bars it draws at `:340`. Each was read as a rendered image | Met | - |
...
that an `rx` attribute cannot satisfy on a column, because it rounds all four corners while a
column meets the baseline on one of them. The replacements are narrower in the files they name and
stricter in what they check. AC-020 carries the two forms where an `rx` is the right answer, and
says why each one is the rule holding rather than an exception to it.
