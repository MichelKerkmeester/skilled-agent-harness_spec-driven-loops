...
- Write only under these two roots. Anything else is out of scope, however tempting:
    ...
    specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/014-shadcn-adoptions/   (this packet's docs and scratch/)
...
Implement phase 014 of the chart packet: land three decisions the shadcn research judged worth adopting, as checker-held contracts across the 26 standalone chart templates. Read these first, in this order, before editing anything:

1. specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/014-shadcn-adoptions/spec.md, plan.md, tasks.md, acceptance-criteria.md, goal.md  (the frozen scope; tasks T001 to T013 are your order of work)
2. specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/013-shadcn-reference-research/research/lineages/luna/research.md  (the evidence; the Final Synthesis names the three adoptions and what to keep)
3. .opencode/skills/sk-design/sk-design-chart/SKILL.md, references/template-contract.md, references/catalog.md, references/color-system.md
4. .opencode/skills/sk-design/sk-design-chart/scripts/check-corpus.cjs  (the binding contract; 31 assertions; read how sentinel blocks such as CHART_DATA and the palette block are parsed and reported)
5. Two or three templates under assets/templates/ of each kind: a multi-series form (stacked-area.html, grouped-bars.html), a tooltip-bearing form, a time-path form (daily-line.html)

The three adoptions, each done assertion first:

A. Series-key token indirection. Every multi-series template declares its series once, by key, beside CHART_DATA, and each key resolves to exactly one palette token. Extend the series-mapping assertion so it holds the mapping by key as well as by index: a key with no token, two tokens for one key, or a paint that uses a token its key does not own is an error. Prove the assertion fails on a mutated copy under scratch/ first, then bring every multi-series template to pass.

B. Local readout knobs. Every template that carries a tooltip or card readout gets one READOUT sentinel block beside CHART_DATA declaring its label formatter, value formatter and key alias, and its readout code reads from that block instead of formatting ad hoc. Extend the number-format assertion so a tooltip-bearing form without READOUT is an error. Mutation first, then templates.

C. Declared curve intent. Every template that draws a path over time declares CURVE as one of linear, step or monotone, default linear, with a one-line rationale beside it saying why that curve is honest for the data; the path code honours the declaration. Add a curve-contract assertion: undeclared, out of set, or natural is an error. Mutation first, then templates. Linear is the default for measured trends; step only for discrete states; monotone only with a rationale.

Then: write the three contracts into references/template-contract.md in that document's own voice; record the kept decisions in references/catalog.md (radar and pie stay out, cite the research) and references/color-system.md (the measured palette comparison: shadcn light 16.0 degrees and 1.72:1 versus standalone categorical 92.9 degrees and 3.37:1, cite the research). Bring the six deliveries under assets/examples/ to their templates' new blocks. If scripts/build-gallery.cjs reads templates, regenerate the gallery.

Do NOT do any of these: add a chart form; add radar, pie, area variants or natural interpolation; port or wrap Recharts; implement the four policy-gated items (keyboard gate, colour-vision-deficiency threshold, data-accuracy metadata, retargetability manifest), which stay operator decisions in spec.md section 10.

# VERIFICATION (run these; paste the real output into the summary)

- node .opencode/skills/sk-design/sk-design-chart/scripts/check-corpus.cjs        must end with RESULT: PASSED and zero errors
- one mutation per new assertion, on a copy under specs/.../014-shadcn-adoptions/scratch/, showing RESULT: FAILED and the assertion's error line; revert or leave the copies under scratch/ only
- node .opencode/skills/sk-design/sk-design-chart/scripts/check-corpus.cjs --render   only if a browser is available; if it is not, write that render-dependent checks are UNKNOWN, never inferred
- grep -c READOUT over the tooltip forms and grep -n CURVE over the time-path forms; the counts must equal the forms the contract names
- bash .opencode/skills/system-spec-kit/runtime/cli/spec/validate.sh specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/014-shadcn-adoptions --strict   must print RESULT: PASSED

# CLOSE THE PACKET DOCS

Update tasks.md (tick T001 to T013 with evidence), acceptance-criteria.md (each AC row Met with the file:line or command that proves it; Status Complete), goal.md (completion criteria ticked; log rows with evidence; deviations recorded honestly), implementation-summary.md (What Was Built with a Files Changed table, How It Was Delivered, Key Decisions, Verification table with the real command outputs, Known Limitations), and spec.md Status to Complete. Then regenerate the packet's metadata twice:
  node .opencode/skills/system-spec-kit/runtime/cli/dist/spec-folder/generate-description.js <packet> "$PWD"
  node .opencode/skills/system-spec-kit/runtime/cli/dist/graph/backfill-graph-metadata.js <packet>
and run validate.sh --strict on the packet again.

# HANDBACK (your final message, in this shape)

SUMMARY: three to six sentences on what landed.
FILES CHANGED: one line per file, with what changed.
ASSERTIONS ADDED: name, what it errors on, the mutation's exact error line.
VERIFICATION: the check-corpus summary line, the validate.sh RESULT line, the grep counts, render status (PASSED or UNKNOWN with the reason).
