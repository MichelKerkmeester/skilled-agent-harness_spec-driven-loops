...
Your write authority is already bound. The spec folder is:
  specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/015-shadcn-visual-upgrade
...
- Runtime: Codex CLI, non-interactive exec, one shot, sandbox workspace-write. You are the leaf. Never dispatch codex, another CLI, or a sub-agent; do everything yourself.
- Do not run any git command that changes state (no add, commit, stash, checkout, reset). The conductor commits.
- Write only under these two roots. Anything else is out of scope, however tempting:
    .opencode/skills/sk-design/sk-design-chart/            (templates, checker, references, examples)
    specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/015-shadcn-visual-upgrade/   (this packet's docs and scratch/)
- No external resource may enter a template: no CDN, no font URL, no import. `checkNoExternalResources` errors on any, and that rule is a constraint, not a trade-off.
- Do not loosen, skip, or delete any existing assertion, gate, or threshold in `scripts/check-corpus.cjs`. Add; never weaken.
- Comment hygiene is a hard rule: no spec paths, task ids, requirement ids or decision ids inside code comments. Write the durable why.

# PERSONA

You act as the repository's code implementer. Its contract, verbatim, is the agent file below; where it speaks of an orchestrator, the operator dispatched you directly, which satisfies its gate. Where it says to read `.opencode/skills/sk-code/SKILL.md`, do so, and also read `.opencode/skills/sk-design/sk-design-chart/SKILL.md`, because the chart corpus is the surface you edit.

--- BEGIN AGENT CONTRACT (.claude/agents/code.md) ---
---
name: code
...
>
> **A direct operator invocation satisfies this gate.** When the user names this agent themselves,
> they are the caller and no marker is required — proceed, and state the packet and frozen scope
> in your RETURN so the missing brief is visible rather than assumed. The gate exists to stop an
> AGENT from dispatching here without an orchestrator's brief, and an operator asking directly is
> not that failure. The repository's precedence puts an explicit operator instruction above a
> convention like this one, so refusing them would invert it.
>
...
Before returning `DONE` or any successful RETURN, answer these 7 questions. **All must be YES.**
...
7. Did I cite sk-code's detected stack + selected intents + loaded resource paths in my RETURN Summary, proving the smart router actually fired (not just the SKILL.md read)? If sk-code returned UNKNOWN, did I escalate `UNKNOWN_STACK` instead of implementing?
...
- Challenge wrong-abstraction patches: is the file I'm editing the right ownership boundary, or am I patching downstream of the actual cause? If the natural home for this fix is an out-of-scope file (e.g. role policy lives in `auth/policy.go` but I'm special-casing in `handlers/profile.go`), escalate `SCOPE_CONFLICT` with the right-home file path — even when an in-scope workaround is technically possible. "Acceptance criteria pass in-scope" does NOT justify burying the fix in the wrong layer.
...
# TASK: shadcn visual upgrade of the chart corpus

Read first, in this order, and quote nothing back:
1. `specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/015-shadcn-visual-upgrade/spec.md` (the eight visual moves in section 3 are the scope; sections 4 and 5 are the bar), `goal.md` (decisions D1 to D5 are frozen), `plan.md`, `tasks.md`, `acceptance-criteria.md`.
2. `.opencode/skills/sk-design/sk-design-chart/references/template-contract.md` and `color-system.md`; `scripts/check-corpus.cjs` (all of it; the families `type-scale`, `radius`, `card-parts`, `interaction-hygiene`, `gallery`, `number-format` and the render checks matter most); `scripts/build-gallery.cjs`.
3. The frozen shadcn copy: `specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/013-shadcn-reference-research/scratch/shadcn/chart.tsx` and the examples under `charts/`. Every visual value you apply is measured there and cited by file and line in `scratch/measurements.md`. Nothing is fetched from the network.
4. Two current captures so you know the starting point: `screenshots/templates/daily-line.png` and `screenshots/gallery.png` (its light column renders dark; that is a bug you fix in the gallery builder, not in the templates).

What to build, in this order (tasks T001 to T014 in `tasks.md`; tick each as you finish it, with one line of evidence):

A. Baseline. Run `node .opencode/skills/sk-design/sk-design-chart/scripts/check-corpus.cjs` and, if Chrome cooperates, `--render`; write the per-family assertion counts into `scratch/baseline.md`. Those counts are a floor for the whole task: no family may end lower.

B. Measurements and inventory (`scratch/measurements.md`, `scratch/inventory.md`). The value table needs at least: card title size and weight, description size and colour role, footer structure, tick margin, tick text size and colour role, axis and tick line absence, grid orientation and dash, bar radius single and stacked, line width, dot policy, area gradient stops and stacked flat opacity, tooltip card padding, radius, border, shadow, text size, indicator size and radius, legend chip size, radius, gap and label style. The inventory is the 26 forms by part present (axis, bars, path, area, tooltip, more than one series).

C. Assertions first (goal decision D5). Add a `legend` family (every multi-series form carries a legend keyed to its declared series, one chip per key, labels matching the series names) and a `tooltip-card` family (every tooltip-bearing form renders its tooltip as a positioned HTML element carrying a per-series indicator, label and value read from `READOUT`, and no longer paints tooltip text as SVG text). Prove each on a mutated copy under `scratch/` before the corpus is edited; record the exact failure line in `scratch/mutations.md`. Delete the mutated copies afterwards; they weigh megabytes and the recorded lines are the evidence.

D. Retune `type-scale`, `radius`, `card-parts` and `interaction-hygiene` to the measured system. Retune means the same checks hold new values and cover the same or more files. If a check encodes a value that the new system replaces, change the value; never delete the check. Assertion counts per family must not fall.

E. Templates. Move one form (`daily-line.html`) to the full system first and get it through the gate, then carry the same vocabulary to the other 25 forms one at a time, running the static checker after each. The eight moves: card anatomy with a footer finding line and a muted description (the source line stays in the footer); bare axes with 12px muted ticks and an 8px margin; horizontal dashed grid only; bar radius 4px on free corners (8px when one bar fills the band, outer ends only on stacks); 2px lines with no per-point dots except the contract's highlighted point; vertical gradient area fill 0.8 to 0.1 with stacked areas at 0.4 flat; the tooltip card; legend chips on multi-series forms; single-series cartesian forms painted with the first categorical token, the emphasis token kept for the highlighted mark. Unit grids, rings and other non-cartesian forms keep their colour logic. Keep every `CHART_DATA`, palette, geometry, `READOUT` and `CURVE` block contract exactly as phase 14 left it. Palette values do not change; if a form fails a contrast gate, fix the form.

F. Deliveries under `assets/examples/` follow their templates. Fix `build-gallery.cjs` so each frame renders its own scheme in a way the templates actually honour (the templates key their palette off `prefers-color-scheme`; pinning `color-scheme` on the frame does not flip that; find a mechanism that does, for example a `data-scheme` attribute the templates' palette selectors respect, applied consistently to all 26 forms). Regenerate `assets/gallery.html`. Keep the `gallery` family green with its count intact.

G. References and version. Write the visual system into `references/template-contract.md` in the file's own voice with the shadcn cites, and the single-series colour rule into `references/color-system.md`. Add `changelog/v1.3.0.0.md` in the shape of `changelog/v1.2.0.0.md`, and bump the version wherever `SKILL.md` and `README.md` carry it.

H. Screenshots. Regenerate `screenshots/` for every form and delivery in both schemes plus `gallery.png` with the existing mechanism (`build-gallery.cjs` or the checker's render pass, whichever produces them today; read the scripts README). If Chrome aborts in this sandbox, say so in the handback and leave the old captures in place; the conductor regenerates them.

I. Packet close-out. Fill `implementation-summary.md` from the scaffold's own anchors; set every acceptance row's Verification cell to observed evidence (file and line, or a command output) and its Status to Met or Unmet; tick `tasks.md`; update `goal.md` progress and criteria; set `spec.md` and the acceptance metadata Status. Then regenerate the packet metadata and validate:
    node .opencode/skills/system-spec-kit/runtime/cli/dist/spec-folder/generate-description.js specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/015-shadcn-visual-upgrade "$PWD"
    node .opencode/skills/system-spec-kit/runtime/cli/dist/graph/backfill-graph-metadata.js specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/015-shadcn-visual-upgrade
    NODE_PRESERVE_SYMLINKS=1 bash "$(realpath .opencode)/skills/system-spec-kit/runtime/cli/spec/validate.sh" specs/sk-design/018-sk-design-parent-v2/001-sk-create-chart/015-shadcn-visual-upgrade --strict --no-recursive
  Require the literal line `RESULT: PASSED`. Acceptance table cells must not contain a bare pipe inside inline code.

Rules that bind throughout:
- Write only under the two roots named above.
- Every value cited to the frozen copy; every claim in the packet docs backed by a command you ran.
- Determinism: no `Math.random`, no time-based drawing; two opens paint the same picture.
- The tooltip card must stay inside the card's box at the narrow viewport; pointer reach and card-readout render checks must still pass.
- Do not touch `assets/color/`, the palette JSON, or any numeric gate.

# VERIFICATION (run all, paste the summary lines in the handback)
    ...
    rg -c "data-chart-legend" .opencode/skills/sk-design/sk-design-chart/assets/templates | sort
    git diff --stat -- .opencode/skills/sk-design/sk-design-chart/assets/color
    validate.sh as above

# HANDBACK (your final message, in this shape)

SUMMARY: three to six sentences on what landed.
FILES CHANGED: one line per file group, with what changed.
ASSERTIONS ADDED AND RETUNED: name, what it errors on, the mutation's exact error line; per-family counts before and after.
VERIFICATION: the check-corpus summary lines (static and render), the validate.sh RESULT line, the grep counts, screenshot status.
UNKNOWNS AND DEVIATIONS: anything you could not verify or had to do differently, and why. Never claim completion without the command outputs above.
