...
Invariant 1 verification incomplete: the read-only sandbox rejected the live sk-doc adapter batch because the shell attempted to create a heredoc temporary file. The hard 13-tool-call limit prevented a safe retry, so no findings or clean-artifact claims are asserted.
