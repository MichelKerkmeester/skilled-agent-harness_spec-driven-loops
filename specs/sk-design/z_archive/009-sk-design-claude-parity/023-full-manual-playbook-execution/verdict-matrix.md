...
## Fresh audit (post-024) — findings, closed out in phase 026
...
- **`MR-004` and `HM-004`'s originally-claimed root causes were REFUTED** by adversarial re-verification — the underlying facts cited were accurate, but neither claim's causal mechanism reproduced against the actual dispatch evidence.
- **`MR-004`'s live-daemon-path logic-sync conflict — fixed in phase 026** (see `../026-mr004-fix-and-doc-symmetry/`): added `graph-metadata.json` `intent_signals` (`"design slop"`, `"anti-slop UI audit"`, `"contrast and keyboard focus"`). Standalone probe (the exact scenario prompt, no addendum, native daemon confirmed live) now decisively wins `sk-design` 0.9078 vs `sk-code` 0.8761, stable across 3 repeats — was `sk-code` 0.8719 top-1 before. The live dispatch's own internal advisor tool call in one verification round echoed dispatch-recipe addendum text into its own query (pulling in a `sk-doc` confound); downstream mode/packet resolution still landed correctly (`sk-design`/`audit`/`design-audit`, all 5 expected mode resources cited, findings-first report matching the PASS shape). Treated as PASS on the core fix (proven on the literal, unpolluted prompt) with the dispatch-recipe confound documented, not silently smoothed over.
- **New, small finding from that same verification**: the `audit` mode's live dispatch made 2 non-mutating `Bash` calls (WCAG contrast-ratio `node -e` computations) despite `mode-registry.json` listing `forbidden: [Write, Edit, Bash]` / `bashAllowlist: []` for `audit`. Non-mutating, doesn't trip the scenario's literal FAIL clause, but a genuine tool-surface deviation — not yet fixed, flagged for a future session.
- **`AI-004`'s pre-existing `skill_advisor.py` bug — fixed in phase 026**: removed the bare `design-review` keyword from `sk-design/SKILL.md`'s Keywords comment (redundant with the more specific `design-quality-audit`/`audit-the-design`/`review-the-ui` keywords already present). Verified: `AI-004`'s negation-clause prompt now correctly resolves `sk-code` top-1 (0.8814, was `sk-design` 0.95 via the local-fallback path); a legitimate "do a design review of this landing page" prompt still correctly resolves `sk-design` top-1 (0.9096) — no regression.
- 4 confirmed, fixed defects independent of any scenario (family-mislabel front-matter on 3 mode packets, a nonexistent-command doc bug in `command-metadata.json`, `FR-001`'s index-vs-feature-file prompt mismatch, and the PB-002 fix) — see phase 025's `implementation-summary.md`.
- 2 more fix-now improvement findings closed in phase 026: parent version-number alignment (`SKILL.md`/`description.json`/`mode-registry.json`/`hub-router.json` all synced to 1.4.3.0, plus a changelog entry covering the net change since v1.2.0.0) and the transform-verb-precedence doc-symmetry gap (`excludedAliases.audit`'s `harden`/`polish` exception now documented in `SKILL.md`'s Mode Vocabulary Guardrails, mirroring the `foundations` exception already there). The `taskProjections`-vs-`excludedAliases` layering note was also added to `mode-registry.json`'s `transformVerbRouting.note`.
...
**All items from the fresh audit are now closed** (fixed, or explicitly accepted-as-is with documented reasoning). Only the newly-surfaced `audit`-mode `Bash`-usage deviation remains open.
