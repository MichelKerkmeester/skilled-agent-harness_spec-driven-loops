...
description: "Retired design-motion (39 files, 4,175 lines) as a standalone mode and /interface:motion as a command, folding its content into design-interface while preserving the restraint-gate-runs-first ordering guarantee that made motion procedural rather than declarative. Shipped as commit c52071edb6."
trigger_phrases:
  - "motion merge"
...
_memory:
  continuity:
    ...
    recent_action: "Reconciled packet docs against the shipped motion merge commit c52071edb6"
...
| **Level** | 2 |
| **Priority** | P0 |
| **Status** | Complete — shipped as commit `c52071edb6`; one P0 verification item (CHK-060) still open |
| **Created** | 2026-07-27 |
...
Shipped as commit `c52071edb6` ("refactor(sk-design): merge the motion mode into interface"), 92 files, `+759/-2390`. `design-motion/` no longer exists; `mode-registry.json` declares no `motion` mode; the motion references live at `design-interface/references/motion/`. The restraint gate was preserved **three redundant ways** rather than one, and six `MOTION_*` intents carry the temporal layer inside the interface mode. Full record in `implementation-summary.md`.
<!-- /ANCHOR:problem -->
...
- Updating `preferSiblingWhen` (`:916`), `typicallyBefore`/`handoff.nextOptions` (`:983`/`:1249`), and `hub-router.json`'s `tieBreak` (`:7`, must equal declared modes in registry order — a mismatch here is the exact failure that produced commit `f5eeddf42c8`).
...
|-----------|-------------|-------------|
Delivered in commit `c52071edb6` (92 files, `+759/-2390`). Where the outcome differs from the plan, the Description column says so.
...
| `commands/interface/design.md` | Modify (`6 +`) | Six `motion-*` lane rows added; `next` repointed. **Post-merge residue:** its sibling-discriminator block still said "Prefer `/interface:motion`" and was corrected later, in `2e240ac45f` |
...
| REQ-005 | `hub-router.json:7`'s `tieBreak` equals the declared modes in registry order post-merge | Manual diff confirms order; this exact mismatch previously produced commit `f5eeddf42c8` | **Met** — `tieBreak` reads `["interface", "md-generator", "design-mcp-open-design"]`, matching the checker's reported `workflowModes` set |
...
| Risk | `tieBreak` / registry order mismatch shipped silently | Compiled hub serves legacy routing — this exact failure produced commit `f5eeddf42c8` | Manual diff of `hub-router.json:7` against the post-merge mode registry before commit |
...
| Dependency | Commit `562074ad7c5` (the foundations-merge sequence) | This packet reuses its proven sequence rather than inventing a fresh approach | Re-read that commit's diff before starting Phase 1 |
<!-- /ANCHOR:risks -->
...
- **Predecessor**: `../009-aesthetics-retirement/`
- **Successor**: `../011-retirement-residue/`
- **Precedent commit**: `562074ad7c5` (foundations-merge sequence, reused here)
