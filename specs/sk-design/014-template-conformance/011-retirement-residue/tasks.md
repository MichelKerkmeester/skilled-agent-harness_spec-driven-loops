...
- [x] T010 Read `008-manual-testing-playbook/spec.md` in full; inspect `design-interface/manual-testing-playbook/` on disk (`008-manual-testing-playbook/spec.md`) [20m] — confirmed root cause via `git show --stat 562074ad7c5`; **disproved** the `foundations-*` residue hypothesis (legitimate, documented naming convention); found + fixed a stale scenario count and one dead cross-reference; found (did not fix) an 18-file 9-column-format gap
