...
- [x] T004 `git mv` the four skin skeletons into `assets/diagrams/`, renaming `template.html`→`starter-light.html`, `template-dark.html`→`starter-dark.html`, `template-terminal.html`→`starter-terminal.html`, `template-full.html`→`starter-full.html` (REQ-001; D13) (assets/templates/*.html → assets/diagrams/starter-*.html) — DONE in `9f03950aba`: `git show --stat` lists each as a `{...}` rename with 0 changed lines; `git log --follow -- assets/diagrams/starter-light.html` reaches `277efcb1c37` (the pre-move origin commit).
- [x] T005 `git mv` the 34 forms into `assets/diagrams/`, dropping the `example-` prefix from each filename (REQ-001; D13) (assets/examples/*.html → assets/diagrams/*.html) — DONE in `9f03950aba`: 34 rename entries, 0 changed lines each; `git log --follow -- assets/diagrams/architecture.html` reaches `77898f77767`.
