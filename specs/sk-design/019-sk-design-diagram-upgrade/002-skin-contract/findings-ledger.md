...
| F1.4 | no-external: 38/38 files depend on `fonts.googleapis.com`, the only remote host | Confirmed | CONFIRMED | **002** / 005 | T006 — 002 signs the self-contained bar, one whitelisted stylesheet host; 005 writes the `no-external` family that enforces it, net-new work with no exception precedent to port |
| F1.5 | 4px grid fails at `example-flowchart.html:90-112`; `SKILL.md:403` contradicts `:337` | Confirmed | CONFIRMED | **002** / 005 | T002 — 002 adjudicates the 4px contradiction and signs the exemption list; 005 writes the grid family that holds it, exemptions included |
...
| F1.7 | Marker trio (`arrow`, `arrow-accent`, `arrow-link`) defined together in "11/34" files; `dots` id unprefixed in 26/34 | Corrected — 10/34, not 11/34 | CORRECTED | **002** / 005 | T003 — 002 signs the marker vocabulary and the per-file id-uniqueness scope; 005 writes the marker and unique-id families at that scope |
| F1.8 | Budget lacks node/coral markup conventions; accent has two spellings; occurrence is not element; `example-high-level.html` has "29" raw `<rect>`s | Corrected — 36 raw rects, not 29 (the 7-rect gap is arrow-label mask rects, not nodes) | CORRECTED | **002** / 005 | T004 — 002 signs `data-diagram-node` as the node markup convention and the coral two-spellings; 005 writes the budget family that counts tagged nodes rather than raw rects |
...
| F1.10 | Not statically holdable: pairwise geometry, budget semantics, page-level uniqueness, semantic rules, the `:403`/`:337` adjudication | Confirmed | CONFIRMED | **002** / 005 / 006 | 002 also resolves part of it — the `SKILL.md:403`/`:337` adjudication is one of the items F1.10 listed as not statically holdable, and signing it removes it from that list; 005 registers what stays judged as the checker's boundary — the enforceable half; 006 judges the remainder — connector overlap, fan, visible gap, behind-box, focal balance |
...
| F4.1 | The 34 examples = 27 canonical (1:1 with the 27 type references) + 5 variant proofs + 2 import proofs + 0 decoration; sketchy is unproven; each non-light template is proven exactly once | Confirmed | CONFIRMED | 004 / 005 | Sketchy proof-or-descope executed at repaint time — enacts parent D9's second clause, with the reason (zero decoration examples on disk) recorded here; 004 holds the 27 + 5 + 2 + 0 taxonomy and descopes sketchy, 005 writes the family that keeps `examples − references = ∅` true in both directions |
...
|------|--------------------------|
| 001 (settled) | None |
| **002 (this node)** | F1.4 (partial), F1.5 (partial), F1.7 (partial), F1.8 (partial), F1.9, F1.10 (partial), F2.2, F2.4, F2.5, F3.1 (partial), F3.2, F3.5, F3.6, F3.7, F4.3 (partial), F4.5 (decision slice), F4.6, S2.1, S2.3, S3.1, S3.2, S4.3 (partial), S5.1, S5.2 |
| 003 | F2.1 (partial), F2.3, F3.4 |
| 004 | F2.1 (partial), F2.6, F4.1 (partial), F4.2, F4.4 (partial), F4.5 (execution slice), S4.1, S4.3 (partial) |
| 005 | F1.1, F1.2, F1.3, F1.4 (partial), F1.5 (partial), F1.6 (partial), F1.7 (partial), F1.8 (partial), F1.10 (partial), F3.1 (partial), F4.1 (partial), F4.3 (partial), F4.4 (partial), S1.1, IT5-MUT, IT5-CI |
