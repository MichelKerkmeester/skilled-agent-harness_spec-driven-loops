...
- [x] T001 Confirm 009's merged form library state on disk before assuming `assets/diagrams/` exists; if 009 has not landed, build against `assets/templates/` + `assets/examples/` instead and record which path was actually used (D13) (scripts/apply-design-md.cjs) — executor: DeepSeek V4.1 Flash max via cli-pi — evidence: `assets/diagrams/` exists with 38 forms + `README.md` (39 entries, `ls assets/diagrams | wc -l`); `FORM_DIR` in `apply-design-md.cjs` points there directly (line 24). 009 had landed (commit `9f03950aba` predates this phase's `78ab2b220b`); the fallback path was never needed
...
- [ ] T022 Wire `SKILL.md`: an activation-trigger bullet in WHEN TO USE, an "Applying a Style Reference" paragraph in HOW IT WORKS pointing at the new script and reference doc, two REFERENCES rows, and a minor version bump per the Frontmatter Versioning Standard (REQ-016) (SKILL.md) — executor: DeepSeek V4.1 Flash max via cli-pi — **partially done**: HOW IT WORKS carries one sentence (line 214, "To repaint a copy in another visual language, see `references/design-md-theming.md`") but not a dedicated paragraph; there is no WHEN TO USE activation-trigger bullet or Keyword-trigger naming Style Reference/theming/DESIGN.md; there are no REFERENCES-section rows for `apply-design-md.cjs` or `design-md-theming.md`; `version:` is `1.1.0.0` in every one of this phase's ten commits (`git show <sha>:SKILL.md | grep ^version:`), so no bump landed. Confirmed outside this closeout's write authority to build, only to record
<!-- /ANCHOR:phase-2 -->
...
- [ ] T023 Run `node --test scripts/tests/` locally and read the output line by line: the two new cases pass for their stated reason, the existing stock-drift case still passes unchanged, and the completeness triple reports clean (scripts/tests/corpus-mutations.test.cjs) — executor: human review — **partially done**: reran during this closeout pass — 16/16 tests pass, the existing stock-drift `derivation-gates` case passes unchanged, and the completeness triple (`every family... has a case`, `nothing here names a family... does not register`, `no exemption outlives the family`) all pass. The "two new cases" this task names were never written (T020), so that half is unmet
...
- [ ] All tasks marked `[x]` — not true: T019, T020, T022 unticked (not built); T023 ticked as partial only. See each task's evidence line and `goal.md`'s LOG
- [x] No `[B]` blocked tasks remaining — none of T001-T026 carry a `[B]` marker
- [ ] Manual verification passed — the applicator itself is solid (identity, second-reference theming, refusals, gate-then-write all reran clean during this closeout pass); the `derivation-gates.cjs` extension and the `SKILL.md` routing this phase's own requirements call for were not built, so verification did not pass end to end
<!-- /ANCHOR:completion -->
...
- [ ] CHK-020 [P0] All acceptance criteria met — AC-001 through the final row in acceptance-criteria.md — not true: several rows remain `Unmet` (AC-015 and its `derivation-gates` extension, AC-018's SKILL.md rows). See `acceptance-criteria.md`
...
- [x] CHK-042 [P2] README updated (if applicable) — not applicable this phase; `scripts/README.md` documents the Python extractors and its last commit (`f56d3efecf`) predates this phase's work
<!-- /ANCHOR:docs -->
...
- [x] CHK-050 [P1] Temp files in scratch/ only — this authoring/closeout pass created no temp files under this packet; all verification output went to `/tmp`, outside the repository. Note: the fixture this checklist originally expected under `scripts/tests/fixtures/` was never built — see T020
...
|----------|-------|----------|
| P0 Items | 8 | 7/8 (CHK-020 unmet: not every acceptance-criteria row is `Met`) |
| P1 Items | 10 | 10/10 |
| P2 Items | 1 | 1/1 |

**Verification Date**: 2026-09-11 (closeout pass)
<!-- /ANCHOR:summary -->
