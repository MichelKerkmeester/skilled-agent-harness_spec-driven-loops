...
| `link` | light | A chromatic row whose Role text names link, anchor, interactive, or hyperlink text; absent that tag, the second most-saturated chromatic row after the one chosen for `accent`, provided it clears `markOnPaper` and sits >=30 degrees of hue from `accent` | Stays the stock hard-coded value, unthemed, and the run says so by name - a documented, common case, since a generic v3 Colors table rarely tags a dedicated link role |
| `link` | dark | Same rule as light, run against dark `paper` and the already-chosen dark `accent`, picked independently | Same fallback as light: stays the stock hard-coded value, unthemed, and the run says so by name |
