...
- [ ] `node --test .opencode/skills/sk-design/sk-design-diagram/scripts/tests/` passes, including two new `derivation-gates` cases that prove the `system=design-md` extension fires for its own stated reason, and the completeness triple reports clean — **not met**: `scripts/families/derivation-gates.cjs` carries no `system=` handling at all (`grep -c "system=" derivation-gates.cjs` = 0), `scripts/tests/mutation-cases.cjs` carries one `derivation-gates` case, the pre-existing stock-drift one, and `scripts/tests/fixtures/` does not exist. The 16-test suite itself passes (`node --test scripts/tests/` → 16/16), including that one case and the completeness triple, but the two design-md-specific cases this criterion asks for were never written. See LOG.
...
| `derivation-gates.cjs` extension (`system=design-md`) | **Not built** | `grep -n "system=" scripts/families/derivation-gates.cjs` finds nothing; the sentinel regex the family reads has no optional `system=` group. REQ-014, REQ-015, T019, T020 unmet |
| Mutation cases + fixture for the extension | **Not built** | `scripts/tests/fixtures/` does not exist; `scripts/tests/mutation-cases.cjs` carries exactly one `derivation-gates` case, the pre-existing stock-drift one |
| `SKILL.md` routing | **Partially built** | HOW IT WORKS carries one sentence pointing at `references/design-md-theming.md` (line 214: "To repaint a copy in another visual language, see..."); there is no activation-trigger bullet in WHEN TO USE / Keyword triggers naming Style Reference / theming / DESIGN.md, no REFERENCES-section row for `apply-design-md.cjs` or `design-md-theming.md`, and `version:` has stayed `1.1.0.0` across every commit this phase's brief lists (`git show <sha>:SKILL.md \| grep ^version:` for all ten shas returns `1.1.0.0`). REQ-016, T022 unmet as specified |
