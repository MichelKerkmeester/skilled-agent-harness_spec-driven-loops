...
| `.opencode/specs/sk-code/001-sk-code-parent/graph-metadata.json` | Update | Add child ids 010-019, set `last_active_child_id`, and parent status |
...
- **SC-003 — MET**: Checks 5-9 in `parent-skill-check.cjs` promoted from WARN to FAIL only after SC-002 held. [EVIDENCE: commit `3c06bda2eb` — `STRICT_HUB_CANON` default flipped to FAIL with a `PARENT_HUB_CHECK_STRICT=0` WIP opt-out]
