...
- **[P1] `verify_stack_folders.py` is permanently broken AND a near-orphan AND breaks a sibling's test** — parses nonexistent `STACK_FOLDERS` → always `exit 1` (iter-2); referenced only from the stale README + external `manual_testing_playbook/design-restraint/stack-folders-validator.md` (DR-004, which expects a clean `exit 0` that can **never pass**) + parent `changelog/v3.4.0.0.md:47` (stale path). Cross-packet structural contradiction.
