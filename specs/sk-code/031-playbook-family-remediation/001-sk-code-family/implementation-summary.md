...
6. **The two already-clean roots held.** `sk-code-mobile-cli` at `scenarios=26 categories=7` and `sk-code-obsidian` at `scenarios=27 categories=7`, both `violations=0 warnings=0`. Their coverage expansion is `028-surface-playbook-expansion`, not this phase.
...
| Treat the two already-clean roots as out of scope rather than as free wins | They were at zero before this phase started. Their work was coverage breadth, which belongs to `028-surface-playbook-expansion`; claiming it here would inflate this phase's result with someone else's. |
