...
- `032-authoring-hardening` phase `002-validator-false-positives`: the durable repair, which stops the link scan from reading fenced code.
...
Remediate the six CLI orchestration roots and the eight MCP tooling roots by violation class. Classify the `mcp-notion` link violations as a grader defect and route the durable fix to `032-authoring-hardening` rather than rewriting the call syntax in 34 files.
...
- `032-authoring-hardening` phase `002-validator-false-positives` for the grader fix this phase depends on.
...
Documentation-only within this phase and fully reversible: reverting the fourteen roots' `manual-testing-playbook/` trees restores their prior contents and counts. The grader change this phase depends on is owned and reversible in `032-authoring-hardening`; reverting it there would restore the false positive in `mcp-notion` without changing any document in this phase.
