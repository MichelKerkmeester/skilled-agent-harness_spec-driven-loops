...
Out of scope: the twenty routing-gold-excluded scenarios inside `system-spec-kit` and the six inside the `system-deep-loop` parent, all of which stay excluded by registration; the grader and template defects owned by `032-authoring-hardening`; and the fail-closed graduation of these roots, which is `004-fail-closed-graduation`.
