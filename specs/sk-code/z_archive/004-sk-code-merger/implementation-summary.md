...
_memory:
  continuity:
    packet_pointer: "sk-code/z_archive/004-sk-code-merger"
...
| Field | Value |
|-------|-------|
| **Spec Folder** | 004-sk-code-merger |
...
`.opencode/skills/sk-code/` (68 files). The advisor lists it with full description; live web smoke test pending.
...
3. **`skill-graph.json` is stale**. The auto-generated file at `.opencode/skills/system-spec-kit/mcp_server/skill_advisor/scripts/skill-graph.json` still lists `sk-code-web` and `sk-code-full-stack` in its families/adjacency/signals sections. Will be regenerated by `doctor:skill-advisor :confirm`.
