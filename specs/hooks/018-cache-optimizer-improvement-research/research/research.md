...
description: "Eight iterations across two models on where the Pi cache extension is weakest, what survived adversarial verification, and the ordered backlog that came out of it."
trigger_phrases:
...
`final-recommendations.md` in this directory adjudicates the two legs and is the file to act from.
This file is the loop's record; that one is the verdict.
