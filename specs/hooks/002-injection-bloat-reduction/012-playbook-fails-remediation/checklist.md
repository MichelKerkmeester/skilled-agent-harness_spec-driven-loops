...
_memory:
  continuity:
    ...
    answered_questions: []
---
# Verification Checklist: Manual Testing Playbook FAIL Remediation
...
- [x] CHK-010 [P0] Codex group covers CX-012, CX-013, CX-014, CX-016, CX-017, CX-018, CX-021, CX-022, CX-023, CX-026, CX-028, CX-003, CX-006, and CX-008 with the matrix's exact scenario edits, shared guidance, operator prerequisites, or SKIP branch. Evidence: commits `ea71336c42`/e80bbeb8fd/6518672dbb; re-run 13 PASS + CX-023 SKIP, 0 FAIL (EV-GROUP-CODEX).
- [x] CHK-011 [P0] OpenCode group covers CO-004 and CO-024 with prompt/file ordering and canonical CLEAR pointer validation. Evidence: commit `ea71336c42`; CO-004 PASS (prompt-before-f), CO-024 PASS (canonical CLEAR pointer) (EV-GROUP-OPENCODE).
- [x] CHK-012 [P0] Pi group covers PI-009, PI-011, PI-012, PI-001, and PI-020 with mirror sync, current package semantics, cite-only/SKIP handling, and paired-event identity. Evidence: commit `978ea1fa3c`; PI-009/001/020 PASS + PI-011/012 SKIP (EV-GROUP-PI).
- [x] CHK-013 [P0] Cursor group covers CU-011, CU-024, CU-026, and CU-004 with operator trust handling, derived roster membership, Shell payload support, and supported plan output evidence. Evidence: commits `544e9734dc`/978ea1fa3c; CU-026/024/004 PASS + CU-011 SKIP (EV-GROUP-CURSOR).
- [x] CHK-014 [P0] Devin group covers DV-012, DV-014, DV-015, DV-007, and DV-008 with native skill discovery, current lifecycle behavior, and documented headless-event SKIP. Evidence: commit `978ea1fa3c`; DV-012/014 PASS + DV-007/008/015 SKIP (EV-GROUP-DEVIN).
- [x] CHK-015 [P0] The three TOOL-BUG seams are fixed and checked: PI-009 mirror generation, PI-020 identity/paired handlers, and CU-026 shared-hook payload delivery. Evidence: PI-009 sync-agents-pi --check 13/13; PI-020 pi-extension advisory test; CU-026 7 hook payload tests + negative control (EV-TOOLBUG-3).
- [x] CHK-016 [P1] Shared Codex/OpenCode/Pi/Cursor/Devin guidance is updated at its owning file rather than only in a single scenario. Evidence: cli-<runtime> `SKILL.md`/references updated alongside scenarios within each tranche commit (EV-SHARED-OWNERS).
<!-- /ANCHOR:code-quality -->
...
- [x] CHK-FIX-002 [P0] All 20 IN-REPO FIX rows have a changed scenario file and an owning shared seam when the matrix names one. Evidence: 20 in-repo scenario files changed across `ea71336c42`/544e9734dc/978ea1fa3c/e80bbeb8fd/6518672dbb (EV-REPO-20).
- [x] CHK-FIX-003 [P0] No operator machine file appears in the repository diff, including Codex profile files, Codex hooks, and Cursor trust state. Evidence: git diff carries no `~/.codex` or trust-state file; machine actions applied off-repo (EV-NO-MACHINE-DIFF).
- [x] CHK-FIX-004 [P1] The old Codex flags/profile sections, literal Cursor roster count, unsupported Devin command mirror, old Cursor proxy claim, and duplicate Pi IDs are absent from affected surfaces. Evidence: `--full-auto` removed from codex exec, sk-git proxy language replaced, literal cursor roster count removed, .devin/skills mirror replaced with native discovery, Pi paired IDs deduped (EV-STALE-ORACLE-SWEEP).
- [x] CHK-FIX-005 [P1] The playbook validator and compiler/report path reject duplicate scenario identity before a result can be associated with the wrong file. Evidence: PI-020 paired-handler identity fix; pi-extension re-run asserts unique `tool_call`/`tool_result` binding (EV-ID-GUARD).
- [x] CHK-FIX-006 [P1] Pi and runtime mirror checks are run from the final repository state and report no stale generated mirror. Evidence: sync-runtime-mirrors --check PASS 131/131; sync-agents-pi --check 13/13 from final state (EV-MIRROR-CLEAN).
<!-- /ANCHOR:fix-completeness -->
...
- [x] CHK-030 [P0] No scenario installs the optional Pi MCP host, approves Cursor trust, or migrates Codex profiles implicitly. Evidence: SKIP scenarios record docs verified via `git diff` without installing a host, approving trust, or migrating profiles (EV-NO-IMPLICIT-MUTATION).
- [x] CHK-031 [P0] Codex profile and hook actions have a recoverable local backup and a read-only verification before legacy state is removed. Evidence: backups ~/.codex/config.toml.bak-20260808T183212Z + hooks.json.bak-20260808T183502Z; read-only `codex doctor` before legacy removal (EV-OPERATOR-ROLLBACK).
- [x] CHK-032 [P1] The 011 run destination is not baseline/ and historical report records are retained. Evidence: 011 records written under `reports/` (never `baseline/`); original suite records retained as history (EV-REPORT-SAFETY).
<!-- /ANCHOR:security -->
...
- [x] CHK-042 [P1] Scenario SKIP wording, operator checklist wording, and shared-tool guidance match the final implementation and rerun evidence. Evidence: SKIP wording, operator checklist, and shared guidance match final impl and re-run (`implementation-summary.md` results section) (EV-DOC-SYNC).
<!-- /ANCHOR:docs -->
...
- [x] CHK-051 [P1] The final scoped diff contains no generated benchmark reports, scratch artifacts, or machine-local configuration. Evidence: `git status` confirms scoped commits exclude scratchpad and stale review/ artifacts; benchmark records are the intended deliverable (EV-NO-STRAY-FILES).
- [x] CHK-052 [P2] The parent packet's strict validation and whole workspace gate are rerun after implementation. Evidence: validate.sh --strict on 012 returns 0 errors after this reconcile (EV-PARENT-GATE).
<!-- /ANCHOR:file-org -->
...
| Runtime and shared fixes | 7 | 7 | 0 |
| Operator and SKIP outcomes | 2 | 2 | 0 |
