...
_memory:
  continuity:
    ...
    blockers: []
    key_files:
      - ".opencode/specs/hooks/003-goal-hooks-cross-runtime/spec.md"
...
### Overview
This packet has two halves: (1) name and reference the 6 goal-hook playbook docs that belong in each CLI skill's `manual-testing-playbook/` tree, and (2) actually run the goal hook live against cheap/free models per runtime, grep the raw transcript for a per-run canary token and the `[active_goal]` marker, and record an honest verdict. No goal-hook implementation code changes here — packet 003 already shipped and closed that work.
<!-- /ANCHOR:summary -->
...
### Definition of Ready
- [x] Problem statement clear and scope documented — packet 003 closed with no playbook/live-proof coverage, this packet's `spec.md` §2-3
- [x] Success criteria measurable — `spec.md` SC-001..SC-003
- [x] Dependencies identified — packet 003 confirmed `completion_pct: 100` before this packet started
...
Not applicable — this packet documents and validates the goal-hook code that packet 003 already shipped; no code fix or diff is authored here. No producer/consumer inventory is required.
<!-- /ANCHOR:affected-surfaces -->
...
### Phase 1: Setup
- [x] Confirmed packet 003 (goal-hook implementation) shipped `completion_pct: 100` before authoring playbooks
- [x] Packet `004-goal-hook-playbooks-and-validation` folder plus `evidence/` subdir created
...
- [x] Documentation updated — spec/plan/tasks/checklist/implementation-summary authored together, cross-referencing packet 003 and `evidence/`
<!-- /ANCHOR:phases -->
...
|------------|------|--------|-------------------|
| Packet `003-goal-hooks-cross-runtime` | Internal | Green — shipped, `completion_pct: 100` | Without it there is nothing to validate live |
