...
Once all concern adapters honor isHookEnabled, MK_HOOKS_DISABLED=1 silences the enforcement layer. That is now true for mcp-route-guard, the Phase 3 adapters, the OpenCode dispatch / post-edit-quality / task-dispatch / goal / spec-gate / skill-advisor plugins, and the git-preflight / session-lifecycle / spec-memory families — session-lifecycle proven end-to-end: its compiled dist hooks emit nothing under MK_HOOKS_DISABLED=1. The only surface still ungated is the non-OpenCode skill-advisor / spec-gate runtime adapters.

## 7. STATUS

**In progress — Phases 1-5 shipped; Phase 4 nearly complete.** Shared guard + master switch (`node --test` 7/7). Pilot: all six `mcp-route-guard` adapters gated and proven. Phase 3: every `dispatch`, `post-edit-quality`, `task-dispatch`, `goal` adapter plus the OpenCode plugins honor `isHookEnabled` (the deep-review gap — plugins missing the guard — is closed, including the Cursor dispatch proxy). Phase 4: OpenCode `spec-gate`/`skill-advisor` plugins, Cursor/Pi `completion` adapters, and now `git-preflight`, `session-lifecycle` (16 sources; compiled-dist hooks resolve the guard via `createRequire(import.meta.url)`, proven to no-op under `MK_HOOKS_DISABLED=1`), and `spec-memory` all honor `isHookEnabled`. Only the non-OpenCode `skill-advisor`/`spec-gate` runtime adapters remain ungated. Phase 6 cross-runtime validation pending.
