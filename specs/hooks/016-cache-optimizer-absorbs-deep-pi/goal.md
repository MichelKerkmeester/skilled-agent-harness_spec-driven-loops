...
- [x] Economics, retry guard and verified edits are exercised beyond the DeepSeek-direct pair.
      Token accounting is confirmed live on a non-DeepSeek model: the persisted record shows 67
      requests on `llmgateway/glm-5.3-flash` with 66 hits and 7,729,152 of 8,194,905 input tokens
      served from cache. The cost half is NOT live-proven: no model entry carries a cost block, so
      pricing resolves undefined, `pricedRequests` stays 0, and the cost, baseline and savings
      arithmetic has only ever run under test. `prefixChurnCount: 0` proves nothing either way,
      since zero is also its initial value. The retry guard and verified edits stay suite-covered
