...
| AC-006 | REQ-005 | Given the three `pull_request`-only workflows, When their run history is checked, Then whether their gates are redundancy or sole enforcement is settled on evidence | All three last ran 2026-08-29; two pull requests merged the same day, so the gates are redundancy and were kept as duplication | Met | - |
...
merges here are branch merges. That reasoning was wrong — squash-merged pull requests leave no
merge commit, and all three workflows ran on 2026-08-29. The gates were kept anyway, on the
weaker ground that cheap diff-scoped duplication is worth its lines.
