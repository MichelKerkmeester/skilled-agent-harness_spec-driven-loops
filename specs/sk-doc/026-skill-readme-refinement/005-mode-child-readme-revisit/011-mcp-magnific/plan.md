...
trigger_phrases:
  - "phase 011 plan"
  - "mcp magnific readme plan"
  - "magnific rewrite plan"
...
_memory:
  continuity:
    ...
    recent_action: "Scaffolded phase 011 plan inside 005-mode-child-readme-revisit"
...
# Implementation Plan: Phase 011 mcp-magnific mode skill README rewrite

<!-- ANCHOR:summary -->
## 1. SUMMARY

Rewrite `.opencode/skills/mcp-tooling/mcp-magnific/README.md` as a purpose-first document on the refined template from phase 001, using the mcp-obsidian README as the exemplar. The rewrite keeps every fact, bumps the version field from 0.1.0.0 to 0.1.1.0, adds `changelog/v0.1.1.0.md`, passes `validate_document.py --type readme` with zero issues and the HVR grep with zero violations. No SKILL.md, template or vault file is touched. Rollback is a git revert of the rewrite commit.
<!-- /ANCHOR:summary -->
...
| Version field | Frontmatter version bumped to 0.1.1.0 | `rg -n` |
| Changelog entry | `changelog/v0.1.1.0.md` present | `ls -la` |
...
| Artifact | Change |
|----------|--------|
| `README.md` | Rewrite: one-line pitch, problem-first OVERVIEW, purpose-first sections per the refined template, version bump to 0.1.1.0 |
| `changelog/v0.1.1.0.md` | Add: changelog entry for the rewrite |
...
| Phase | Work |
|-------|------|
| Setup | Read the refined template, read the current README and record baseline, read the mcp-obsidian exemplar |
| Implementation | Rewrite the README purpose-first, fix HVR violations, bump version, add changelog entry, confirm facts by diff |
