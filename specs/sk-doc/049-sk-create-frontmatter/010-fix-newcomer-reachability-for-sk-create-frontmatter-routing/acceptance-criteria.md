...
| AC-008 | REQ-004 | Given the canary red on two tool digests this phase did not move, When each is classified against HEAD, Then only committed drift is re-pinned | Both scripts changed in commit `bedf5691b7` and are clean at HEAD. Re-pinned in the canary, the shared `protected-digests.json` and the four sibling canaries. sk-doc canary `REAL-GREEN`, 23 of 23 rows | Met | - |
