...
State means one of four things. **Fixed** was repaired and verified. **Planned** has a phase in
this packet that owns it and has not closed it. **Decision** is closed by a recorded judgment
rather than by a change. **Superseded, pending 049** means another packet deletes the code the
finding lives in, so the fix would be written into files that are already scheduled to go.
...
| 34 | Five contract questions have the test and the code asserting opposite things | 007 | Superseded, pending 049 | ADR-001 to ADR-004 and ADR-007 all name paths under `.opencode/skills/system-spec-kit/mcp-server/`, which `specs/system-speckit/049-memory-decommission` phase 003 lists as Delete. Source: the 049 mapping table in `007-spec-kit-residue/goal.md` |
