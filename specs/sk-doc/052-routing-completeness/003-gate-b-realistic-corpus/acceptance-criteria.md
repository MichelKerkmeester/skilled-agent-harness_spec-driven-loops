...
| AC-002 | REQ-002 | Given the corpus, When it is measured through the daemon, Then the rate is recorded and reproducible | The second run against the state it was measured in returned 8 of 180 as top pick, matching `research/gate-b-measurement.md`, with cache hits observed on verbatim repeats. A third run on 2026-09-02 at HEAD `a25af73c11` returns 21 top-only and 24 any-position, which reproduces the 21 of 180 recorded in `aa11c2b622` and measures the post-fix state rather than this baseline | Met | |
