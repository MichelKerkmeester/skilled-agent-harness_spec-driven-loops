...
|---|---|---|---|---|---|
| AC-001 | REQ-001 | Given duplicate uncompiled entries under bare executor names, When they are removed, Then the compiled route wins | Re-run 2026-09-02: six bare executor names (codex, opencode, cursor, devin, pi, claude code) each return `cli-external-orchestration` at rank one carrying a compiled route, and no routeless bare-name entry appears at any rank. The before state is recorded in `aa11c2b622` and in `executor-delegation.ts:1` | Met | |
| AC-002 | REQ-002 | Given a vocabulary change, When both hubs are re-measured, Then neither loses a prompt it owned | Re-run 2026-09-02: `capture-scorer-eval-baseline.mjs:1` re-scores the corpus and returns metrics and fixture hashes identical to `scorer-eval-baseline.json:1`, captured at `ca9e3885b8` after the routing edits. The ratchet moves on any lost row, so none moved. A direct read of the 46 sk-doc and sk-code gold rows puts 40 at rank one, which is the same figure the frozen baseline already carries | Met | |
