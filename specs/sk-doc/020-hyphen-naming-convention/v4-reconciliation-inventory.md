...
**Provenance**
- Authoring base (what the 020 specs grounded on): `a62523522e` (the structure scaffold).
- Original-pass v4 tip: `d5948d46f4`. Method: Claude git-delta analysis + GPT-5.6-SOL (xhigh, fast) exhaustive cross-reference, 2026-07-15.
- Delta measured: `git diff --name-status -M a62523522e..d5948d46f4` over the skill/command/agent/runtime surface = **222 paths: 66 R + 110 M + 46 A + 0 D**.
- **Refresh (this pass):** current v4 tip `b7876714d7`; branch HEAD `9f94825543` (= v4 + 020 specs + reconciliation). Re-scanned `d5948d46f4..b7876714d7` (3 concurrent commits). See the Refresh section below.

## Refresh — 2026-07-15 (v4 `d5948d46f4` → `b7876714d7`; reconciliation actions APPLIED)

**The reconciliation actions in this inventory are now APPLIED** — commit `9f94825543` on branch
`sk-doc/0042-017-authoring` (rebased onto v4 `b7876714d7`, ff-ready). Applied to the *020 spec docs*
...
**Re-scan of the 3 concurrent commits since the original endpoint** (`d5948d46f4..b7876714d7`):
- 🟡 **NEW — `sk-doc/create-benchmark/conformance_benchmark` (5 snake files)** added by `b7876714d7`
...
- ⚪ `6df89a198f` "renumber sk-git specs 000-006→007-014" — **naming-neutral**: renames only the
  *number prefixes* of `.opencode/specs/sk-git/**` folders (still kebab); no skill-surface name change.
- ⚪ `d0991ac6f6` + the 066 portion of `b7876714d7` — **naming-neutral**: `system-deep-loop/066-*`
  spec-packet docs on kebab-prefixed phase folders; not a rename surface.

## Bottom line

**The fast-forward merge is safe — there is no file to "override."** Verified: `d5948d46f4` is an
ancestor of HEAD `a316f4490e`; `git diff d5948d46f4..HEAD` = 1,241 paths, **all** under
...
| **sk-doc/create-benchmark (conformance)** | 🟡 NEW-UNCOVERED *(refresh)* | `b7876714d7` added `conformance_benchmark/` (5 snake files) after the census → extend `008/003/003/007-create-benchmark` rename map. |
...
Commit `b7876714d7` added a 5th benchmark taxonomy under `sk-doc/create-benchmark/`, all snake_case:
...
be **re-pinned against the post-v4 surface**, not `a62523522e`. This is the single highest-leverage
...
> re-pin SHA is now the current tip `b7876714d7`, not `d5948d46f4`.

| 020 phase | Action |
|---|---|
| `006-inventory-and-frozen-map` | Re-pin/re-hash against `b7876714d7` (current tip), not `a62523522e`; add `.codex/prompts` surface. |
| `008/003/003/007-create-benchmark` *(refresh)* | Extend the rename map to add the `conformance_benchmark/` family (5 snake files) from `b7876714d7`; re-verify all 21 create-benchmark snake paths against the manifest. |
