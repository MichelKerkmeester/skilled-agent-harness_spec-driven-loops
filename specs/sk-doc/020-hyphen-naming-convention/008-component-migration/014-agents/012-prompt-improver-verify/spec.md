---
title: "Feature Specifi [sk-doc/020-hyphen-naming-convention/008-component-migration/014-agents/012-prompt-improver-verify/spec]"
description: "The agents surface needs an explicit candidate audit for prompt-improver across the three runtime agent directories. This verify-only phase records the actual definition paths and proves whether any in-scope snake_case filesystem name requires a rename."
trigger_phrases:
  - "prompt-improver agent naming"
  - "agents phase 012"
  - "prompt-improver filename verification"
importance_tier: "important"
contextType: "planning"
_memory:
  continuity:
    packet_pointer: "sk-doc/020-hyphen-naming-convention/008-component-migration/014-agents/012-prompt-improver-verify"
    last_updated_at: "2026-07-14T00:00:00Z"
    last_updated_by: "codex"
    recent_action: "Authored prompt-improver phase docs"
...
# Feature Specification: Prompt Improver Agent Naming Verification

> Phase adjacency under the 020 agents component parent (grouping order, not a runtime dependency): predecessor 011-orchestrate-verify; successor 013-review-verify.
...
| Field | Value |
|-------|-------|
| **Packet** | sk-doc/020-hyphen-naming-convention/008-component-migration/014-agents/012-prompt-improver-verify |
| **Level** | 2 |
| **Priority** | P1 |
| **Status** | Planned |
| **Created** | 2026-07-14 |
| **Owner skill** | sk-doc |
| **Origin** | Phase 012 of the 020 agents component migration |
<!-- /ANCHOR:metadata -->

<!-- ANCHOR:problem -->
## 2. PROBLEM & PURPOSE

The agents surface must be checked before any migration batch treats this definition as a rename candidate. The current inventory contains exactly three runtime definitions for prompt-improver: .opencode/agents/prompt-improver.md, .claude/agents/prompt-improver.md, and .codex/agents/prompt-improver.toml. Each basename is already kebab-case, so this phase must record an explicit zero-candidate result rather than silently disappearing from the migration evidence.

The purpose is to produce a reproducible, read-only candidate-set record for prompt-improver, grounded in the filesystem naming policy and suitable for the agents rollup gate.
...
### In Scope
- Inventory the three prompt-improver definition paths across the OpenCode, Claude, and Codex runtime agent directories.
...
|-----------|-------------|-------------|
| .opencode/agents/prompt-improver.md | Inspect | Definition filename is already kebab-case; no rename expected |
| .claude/agents/prompt-improver.md | Inspect | Definition filename is already kebab-case; no rename expected |
| .codex/agents/prompt-improver.toml | Inspect | Codex definition filename is already kebab-case; no rename expected |
<!-- /ANCHOR:scope -->
...
- **SC-001**: All three prompt-improver definition paths are recorded against the pinned baseline.
