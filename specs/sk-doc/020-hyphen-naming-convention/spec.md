...
| Field | Value |
|-------|-------|
| **Packet** | sk-doc/020-hyphen-naming-convention |
...
| **013** | `013-install-guide-canonical-naming` | leaf | **Follow-on (absorbed from former top-level packet 021).** Normalize every skill install-guide filename to the single canonical `INSTALL-GUIDE.md` (uppercase + hyphen), resolving three inconsistent casings; add additive hyphen-stem recognition to the classifier so renamed files still type as `install_guide` (a preserved code-contract id) instead of silently downgrading to `readme`. Does not re-run the migration. |
| **014** | `014-naming-standard-hardening` | parent (×2) | **Follow-on (absorbed from former top-level packet 022).** Harden the kebab canon from documented to enforced: reconcile the shared standard docs + doc-validation tooling to the canon, wire the existing kebab guards into a real gate, and add per-mode naming conformance. Children: `001-fix-shared-standard-and-wire-guards`, `002-per-mode-naming-conformance`. Does not re-run the migration. |
...
**Follow-on note:** Phases 000-011 are the original, self-contained migration program. Phases **012-014** are later follow-on extensions, registered after the 000-011 closeout, each closing naming scope the migration deliberately did not touch — code-directory names inside skills (012), install-guide filename normalization (013, absorbed from former top-level packet 021), and standard-doc + gate + per-mode enforcement hardening (014, absorbed from former top-level packet 022). They are independent of the 000-011 sequence and do not re-run the migration.
<!-- /ANCHOR:phases -->
...
| 013 | 013-install-guide-canonical-naming/ | Follow-on (was top-level 021): INSTALL-GUIDE canonical filename normalization + classifier hyphen-stem recognition | Planned |
| 014 | 014-naming-standard-hardening/ | Follow-on (was top-level 022): kebab canon enforced not just documented — docs + gate + per-mode conformance (parent ×2) | Draft |
