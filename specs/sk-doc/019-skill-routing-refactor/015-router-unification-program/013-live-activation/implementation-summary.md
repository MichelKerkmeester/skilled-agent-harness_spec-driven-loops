...
2. **The P4b literal cutover (T16-T17) is complete.** The runtime resolver that consumes `selectedPolicy` and the `servingAuthority` `legacy → compiled` flip — the only stage that changes runtime routing — were delivered in the sibling `014-runtime-engine` (commits engine `4aa00aa959`, sk-code cutover `ffb752f10b`, remaining-6 cutover `d84dc4bfc2`, pushed on v4). All 7 hubs are flipped, each canary-green via the real scorer with a byte-exact rollback retained; the path is inert by default (`SPECKIT_COMPILED_ROUTING` off) and reversible per-hub or fleet-wide. The advisor-hook machine-enforcement layer that would machine-enforce the wired routing remains in progress.
