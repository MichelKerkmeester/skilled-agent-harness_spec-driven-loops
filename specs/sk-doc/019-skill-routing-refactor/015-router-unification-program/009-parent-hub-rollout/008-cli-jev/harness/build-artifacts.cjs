#!/usr/bin/env node
// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ HARNESS: CLI JEV ARTIFACT BUILDER                   ║
// ╚══════════════════════════════════════════════════════════════════════════╝
'use strict';
...
function sourceInputs() {
  const paths = {
    'cli-jev/SKILL.md': path.join(SKILL_ROOT, 'SKILL.md'),
    'cli-jev/cli-usage/SKILL.md': path.join(SKILL_ROOT, 'cli-usage', 'SKILL.md'),
    'cli-jev/hub-router.json': path.join(SKILL_ROOT, 'hub-router.json'),
    'cli-jev/mode-registry.json': path.join(SKILL_ROOT, 'mode-registry.json'),
  };
...
function loadSnapshot() {
  ...
  const registry = JSON.parse(
    sourceBytes['cli-jev/mode-registry.json'].toString('utf8'),
  );
  const hubRouter = JSON.parse(
    sourceBytes['cli-jev/hub-router.json'].toString('utf8'),
  );
  const snapshot = compileRegistry({
    ...
    packetSkillMarkdown: Object.fromEntries(registry.modes.map((mode) => (
      [
        mode.workflowMode,
        sourceBytes[`cli-jev/${mode.packet}/SKILL.md`].toString('utf8'),
      ]
    ))),
    registry,
    skillMarkdown: sourceBytes['cli-jev/SKILL.md'].toString('utf8'),
    sourceBytes,
  });
...
function scorerScenario(entry) {
  return {
    ...
    scenarioId: entry.id,
    source: { featureFile: 'cli-jev-canary', shape: 'sk-doc' },
  };
}
    ...
    const row = {
      assertions: {
        ...
        authorityWithheldUntilVerify: result.decision.action !== 'route'
          || result.decision.route.authority === 'WithheldUntilVerify',
...
function blastRadius(snapshot) {
  return {
    ...
    archetype: 'external-judgment-transport',
