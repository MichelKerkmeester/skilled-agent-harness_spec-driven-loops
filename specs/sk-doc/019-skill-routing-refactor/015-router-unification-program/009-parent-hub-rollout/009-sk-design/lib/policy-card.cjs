// ╔══════════════════════════════════════════════════════════════════════════╗
// ║ LIBRARY: DOCUMENT-ONLY POLICY CARD (DESIGN HUB)                          ║
// ╚══════════════════════════════════════════════════════════════════════════╝
'use strict';
...
function generatePolicyCard(snapshot) {
  ...
  const frontmatter = {
    admission: ['positive-signal', 'zero-signal-default', 'forbidden-reject'],
    authorityEdges: policy.authorityGraph.map((edge) => (
      `${edge.fromAuthorityRef} ${edge.relation} ${canonicalize(edge.toDestinationId)}`
    )),
    // Every mode on this hub is a mutating workflow actor, so the only bundle
    // grammar this card can attest is the role-unrestricted ordered one.
...
function documentDecision(card, prompt) {
  ...
  // Only the absence of a match defers, and only because this card's own
  // admission frontmatter declares the zero-signal case: the live decision
  // falls to the hub default, so the document projects the same. A card that
  // disagreed with the engine on any request is the one thing a replay must
  // never ship.
  if (matched.length === 0) {
    return parseRouteDecisionShape({
      action: 'defer',
...
/**
 * Replay only policy data embedded in the card and emit an unattested
 * terminal. This replay never acts: it exists so the card's documented
 * decision shape can be compared against the live one without granting the
