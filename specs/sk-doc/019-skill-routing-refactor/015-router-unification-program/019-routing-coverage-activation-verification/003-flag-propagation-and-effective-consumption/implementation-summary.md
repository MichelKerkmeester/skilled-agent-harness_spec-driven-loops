...
description: "Completion record for the flag-propagation and effective-consumption phase. Implemented and committed in c926b25506 behind the still-off SPECKIT_COMPILED_ROUTING flag: the flag added to both CHILD_ENV_ALLOWLIST sets and the compiled decision threaded through the native brief, the CLI subprocess interface, and the hook render. Routing stays byte-identical to legacy; the staged default-on cutover stays operator-gated (P4/011)."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Reconciled the completion record to the implemented+committed state (code landed in c926b25506)"
    next_safe_action: "P4/011 operator-gated cutover remains pending"
    blockers:
      - "None for this child (implemented in c926b25506); the program-level default-on cutover stays operator-gated (P4/011)."
    key_files:
      - "spec.md"
      - "decision-record.md"
      - "plan.md"
    ...
    answered_questions:
      - "Full compiledRoute object or a top-level metadata.compiledRouteSummary? Settled: a top-level metadata.compiledRouteSummary (ADR-001), implemented in c926b25506."
---
...
| Field | Value |
|-------|-------|
| **Status** | Implemented — landed in `c926b25506`, behind the still-off `SPECKIT_COMPILED_ROUTING` flag. The flag was added to both `CHILD_ENV_ALLOWLIST` sets and the compiled decision threaded through the native brief, the CLI `subprocess.ts` interface, and the hook render; routing stays byte-identical to legacy. The staged default-on cutover stays operator-gated (P4/011) and is not done. |
| **Date** | 2026-07-20 |
| **Level** | 3 |
| **Runtime change** | Additive propagation and consumption plumbing landed in `c926b25506`: two exact-key env allowlist additions, a typed `compiledRouteSummary`, brief rendering, and serving-state-aware cache invalidation |
...
| **Verification** | Commit `c926b25506` contains the native/bridge flag tests, four-outcome brief tests, legacy-shape fallback test, and `=0` cache-invalidation test; packet strict validation is rerun after this reconciliation |
<!-- /ANCHOR:metadata -->
...
This work is implemented and committed in `c926b25506`: the flag was added to both allowlists; the compiled decision threaded (as a top-level `metadata.compiledRouteSummary`, ADR-001) through the native brief, the CLI interface, and the hook render; 002's serving-state fingerprint folded into both cache keys (ADR-002) so a flip or `=0` invalidates a stale brief; and the 4-action outcome rendered as an additive brief line (ADR-003). Every change is additive and byte-identical to legacy on routing fields, the three frozen scorer files stay pinned, every step names a byte-scoped or flag-based rollback, and no touched runtime path reads under `.opencode/specs`.
<!-- /ANCHOR:exec-summary -->
...
Confirmed the `002` foundation in `6b81fde29e`, re-anchored the runtime surfaces, preserved the frozen-scorer baseline, and implemented the three selected architecture choices.
...
All delivered runtime changes are contained in `c926b25506`; this reconciliation changes documentation only.
<!-- /ANCHOR:what-built -->
...
Delivery followed the ordered gates in `plan.md`: the `002` foundation landed first, then `c926b25506` added the two allowlist entries, decision-summary threading, CLI preservation, hook rendering, and cache invalidation. The same commit added native-launcher and bridge-path env probes plus bridge/plugin consumption tests. The change stayed additive and left the default off.
<!-- /ANCHOR:how-delivered -->
...
|----------|--------|----------------|
| Thread a top-level `metadata.compiledRouteSummary` (ADR-001) | Implemented in `c926b25506` | One small typed shape crosses the CLI interface and both briefs; auditable, byte-stable |
| Consume serving-state fingerprints for cache invalidation (ADR-002) | Implemented in `c926b25506` | A flag or serving-state change forces a fresh brief; the promoted engine cache keys by manifest fingerprint |
| Render the 4-action outcome as an additive brief line (ADR-003) | Implemented in `c926b25506` | Legacy brief shape remains unchanged when no compiled decision is served |
<!-- /ANCHOR:arch-decisions -->
...
| Check | Result |
|-------|--------|
| Source anchors and delivered diff | Pass — `c926b25506` contains both exact-key allowlist additions, `compiledRouteSummary` threading, CLI preservation, hook rendering, and cache inputs |
...
| Frozen scorer digest comparison | Pass — none of the three frozen scorer paths appears in `c926b25506`; end-of-reconciliation SHA-256 is checked separately |
...
|-----------|--------|-------------------|
| M0 ready | Done | `002` foundation landed first in `6b81fde29e`; frozen scorer baseline preserved |
| M1 flag reachable | Done | Both exact-key allowlist additions landed in `c926b25506` with unset/`0`/`1` tests |
| M2 decision consumable | Done | Native brief summary, CLI field, and injected-context render landed in `c926b25506` |
...
4. **Decision-record status text remains a documentation follow-up.** The implemented choices are unambiguous in `c926b25506`; this summary records the delivered state without claiming a live cutover.
<!-- /ANCHOR:limitations -->
...
- [x] `002` foundation confirmed in `6b81fde29e` before this child landed.
- [x] Both exact-key allowlist additions, decision-summary threading, CLI preservation, hook render, and cache inputs landed in `c926b25506`.
- [x] Native and bridge propagation tests plus four-action/legacy-shape/`=0` consumption tests landed in `c926b25506`.
