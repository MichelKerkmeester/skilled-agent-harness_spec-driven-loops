...
1. It is a key in the compiled-route engine map `HUB_CHILD`
   (`014-runtime-engine/lib/compiled-route.cjs`), **and**
...
The parent hubs satisfy both: all seven live under `009-parent-hub-rollout/00N-<hub>`
with a canary harness and an `acceptance.json`. The non-hub routers satisfy neither.

## The archetype difference

The non-hub routers were rolled out under `012-non-hub-rollout/` using a **different
...
|-----------|------------------|----------------|
| `sk-git` | Real, built child: `012-non-hub-rollout/001-sk-git/` (fenced-manifest archetype, no `acceptance.json`) | Absent from `HUB_CHILD`; no `acceptance.json`; manifest legacy/shadow-only. |
| `system-code-graph` | Real, built child: `012-non-hub-rollout/002-system-code-graph/` (run-drill archetype, no `acceptance.json`) | Absent from `HUB_CHILD`; no `acceptance.json`; manifest legacy/shadow-only. |
| `system-skill-advisor` | Real, built child: `012-non-hub-rollout/003-system-skill-advisor/` (fenced-manifest archetype, no `acceptance.json`) | Absent from `HUB_CHILD`; no `acceptance.json`; manifest legacy/shadow-only. |
| `system-spec-kit` | Real, built child: `012-non-hub-rollout/004-system-spec-kit/` (fenced-manifest archetype, no `acceptance.json`) | Absent from `HUB_CHILD`; no `acceptance.json`; manifest legacy/shadow-only. |
| `mcp-code-mode` | **Un-onboarded candidate.** No rollout child, no activation manifest, no `012-non-hub-rollout/005-mcp-code-mode/`. It appears only as an upstream compiler-shadow output (`004-compiler-n1-shadow/compiled/mcp-code-mode/`) and calibration fixtures (`008-calibration/…/mcp-code-mode.no-slice.json`), never as a rollout/activation target. | Absent from `HUB_CHILD`; has no rollout child or acceptance record at all. |
...
candidates are real, built `012-non-hub-rollout/` children; `mcp-code-mode` is a fifth
candidate that has **not** been onboarded to the non-hub rollout at all. It **must not**
be invented as a `012-non-hub-rollout/005-mcp-code-mode/` folder to "match" the other
...
- `017-create-skill-alignment/checklist.md` CHK-023 — the sibling item this policy
