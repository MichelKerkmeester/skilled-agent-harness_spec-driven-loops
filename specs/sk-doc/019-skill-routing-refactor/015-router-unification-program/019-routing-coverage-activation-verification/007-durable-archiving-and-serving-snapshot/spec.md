...
- Hard-gating every archive against the **active** `013-live-activation/activation/<hub>/manifest.json`; a mid-run change in the referenced manifest's digest aborts the archive rather than silently completing it (`CF-ARC-4`).
...
- Archiving against a `009-parent-hub-rollout` shadow candidate manifest — [why] that manifest was never live; archiving it as if it were the serving decision would misattribute state.
