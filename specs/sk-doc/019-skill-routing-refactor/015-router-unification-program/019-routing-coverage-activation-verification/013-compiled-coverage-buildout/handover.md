...
- **`b6f9637951`** — sk-code routing recipe + manifest `refresh` verb + 013 spec.
- **`b903b8af9a`** — parity-harness correctness (3 bugs) + sk-doc/system-deep-loop unblock.
- **`a381edc1f8`** — sk-design + system-deep-loop + mcp-tooling compiled-serving.
- **`66d179b023`** — sk-doc under-routing fixes (27/5 → 31/1).
- **`7c323e90a9`** — parity non-route/surface-layer refinement (SD-015).
- **`4cd19370da`** — the default-on flip: `DEFAULT_ON_HUBS` populated with all 7 hubs (both resolver copies); 7 `SKILL.md` directives + both create-skill parent templates + 7 catalog leaves rewritten to default-on wording; all 7 manifests re-minted.

**STATUS: COMPLETE. ALL 7 hubs verified `compiled-serving` (drift=0 each; sk-code 23, sk-design 38, sk-doc 32, sk-prompt 5, mcp-tooling 14, system-deep-loop 21, cli-ext 8), routing == legacy. Coverage build-out COMPLETE and the default-on flip SHIPPED in `4cd19370da` — `DEFAULT_ON_HUBS` (both resolver copies, byte-identical) lists all 7 hubs; `SPECKIT_COMPILED_ROUTING=0` remains the fleet-wide kill-switch; per-hub cohort removal restores legacy byte-for-byte. Re-confirmed live during this reconciliation pass (2026-07-21): `compiled-route-status.cjs` reports `compiled-serving`/`fresh: true` for all 7 hubs; the `=0` kill-switch drilled live across all 7 (`flag-off` for every hub); `compiled-route-sync.cjs --verify` reports 0 reads under `.opencode/specs`; the 3 frozen scorer SHA-256 digests are unchanged; the skill-benchmark suite is 258/258 (grown from 247 as lock-in tests were added); `sk-doc` and `sk-prompt` route-gold parity independently re-run and matched exactly (32/0, 5/0). §"NEXT" below is now historical (records what shipped, not what remains). Open follow-ups, none blocking: (1) **RESOLVED 2026-07-21 (follow-up dispatch, same worktree):** the advisor-side `compiledRoute` enrichment cohort in `system-skill-advisor/mcp-server/lib/compiled-routing-flag.ts` — a separate `DEFAULT_ON_HUBS` copy — was populated with the same 7 hubs (+ `dist/` mirror). The "dropped downstream" premise that justified leaving it empty no longer held on this branch: the bridge already threads a `compiledRouteSummary` into `buildNativeBrief` and the OpenCode plugin already renders it into the injected system-context line (`renderCompiledRouteSummaryLine`), both proven by the already-green `tests/compiled-routing-consumption.vitest.ts` (12/12); `SPECKIT_COMPILED_ROUTING` is present in both `CHILD_ENV_ALLOWLIST` sets. The enrichment shells out to `.opencode/bin/compiled-route.cjs`, a thin delegate to the identical `014-runtime-engine/lib/resolve.cjs` this cutover already proved byte-identical to legacy — so the flip is additive metadata only, no routing decision changed. `.opencode/bin/compiled-routing-foundation.vitest.ts` now asserts the advisor cohort equals the resolver cohort exactly (was previously asserting it stayed empty); (2) add an explicit parity vitest for the non-route/surface-layer clause (verified live + 47/47 existing, but no dedicated lock-in test yet); (3) LUNA-HIGH acceptance sweep DONE — full 7-hub two-plane sweep 14/14 PASS, archived + committed (`9b2874f6f2`); (4) `011-activation-cutover-p4`'s own docs narrated the earlier superseded halt at `sk-prompt` — reconciled to record this outcome in the same reconciliation pass as this document.**

## Verified state — FINAL (all 7 compiled-serving; re-confirmed live during this reconciliation pass)
| Hub | verdict | notes |
|---|---|---|
| sk-code | **compiled-serving** (23/0) | routing fixed (recipe below); `b903b8af9a` |
| sk-prompt | **compiled-serving** (5/0) | harness fix alone unblocked it; re-run live 2026-07-21, exact match |
| cli-external-orchestration | **compiled-serving** (8/0) | harness fix alone; `b903b8af9a` |
| sk-design | **compiled-serving** (38/0) | TV-003 over-detection fixed; `a381edc1f8` |
| sk-doc | **compiled-serving** (32/0) | under-routing fixed (`66d179b023`) then SD-015 closed the rest (`7c323e90a9`); re-run live 2026-07-21, exact match |
| system-deep-loop | **compiled-serving** (21/0) | `a381edc1f8` |
| mcp-tooling | **compiled-serving** (14/0) | MT-008 over-detection fixed alongside coverage; `a381edc1f8` |
