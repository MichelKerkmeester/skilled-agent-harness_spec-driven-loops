...
- [x] T-12 Cut `COMMAND_BRIDGES` over to the generated blocks in both files, as a separate, independently revertible commit from T-05–T-10 [evidence: cutover shipped alone as ae8f287fbc — three binding-line swaps plus the guard pin — separate from shadow commit 8abf251585 and singly revertible]
<!-- /ANCHOR:phase-2 -->
...
- [x] T-16 Confirm the two-commit structure (shadow-mode landing / live cutover) is in place so the cutover alone is one-command revertible [evidence: git history shows the two-commit structure (8abf251585 shadow, ae8f287fbc cutover-only), so reverting the cutover alone restores hand-authored routing]
