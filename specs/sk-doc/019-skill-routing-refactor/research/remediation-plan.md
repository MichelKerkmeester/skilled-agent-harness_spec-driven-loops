...
packet, its 16 direct children, and nested descendants (forced-depth, no early convergence):
...
| F-14 | **011** FILE_EXISTS/LEVEL_MATCH (Errors:2) — fails where identical-shaped 012 passes | F01+F02 / CF-01 | P1 | PRE | ⬜ | Wave R1 |
| F-15 | `001-research/006` spec "Research Complete" vs graph `in_progress` | F04 / CF-04 | P1 | PRE | ⬜ | Wave R2 |
| F-16 | `014` + `006` pending-state desync (spec/graph vs summary/checklist) | F05 / CF-03 | P1 | PRE | ⬜ | Wave R2 |
| F-17 | Parent phase-map marks E/F Active while `015`/`016` graphs say `planned` | F14 / CF-21 | P1 | PRE | ⬜ | Wave R2 |
| F-18 | Nested `015/003/009` + `015/003/013` claim Complete with open gates/items | F11 / CF-19+20 | P1 | PRE | ⬜ | Wave R2 |
| F-19 | 001-research/008 spec "Research complete" vs graph `in_progress` | — / CF-23 | P2 | PRE | ⬜ | Wave R2 |
| F-20 | 001-research/008 + 001-research/010 declare Level 2 but omit required files | — / CF-22 | P1 | PRE | ⬜ | Wave R3 |
| F-21 | 001-research/009 all 8 idea children declare Level 2 but omit required files | — / CF-24 | P1 | PRE | ⬜ | Wave R3 |
| F-22 | 001-research/009 children 001-004 claim complete without completion evidence | — / CF-25 | P1 | PRE | ⬜ | Wave R3 |
| F-23 | 001-research/009 parent overstates children 005-007 complete vs graphs | — / CF-26 | P1 | PRE | ⬜ | Wave R3 |
| F-24 | 001-research/009 parent `planned` + null pointer while child 008 unfinished | — / CF-27 | P1 | PRE | ⬜ | Wave R3 |
| F-25 | 015 `context-index.md` "New location" path base ambiguous | — / CF-17 | P2 | PRE | ⬜ | Wave R4 |
...
### Wave R1 — 011 hard errors (F-14) · P1 · ~1 unit
- **Symptom:** 011 (Planned, Level 3) fails `FILE_EXISTS`("missing 1 required file") +
  `LEVEL_MATCH`, but 012 — identical file set, also Planned Level 3 without
...
- **Packets:** `001-research/006`, `014`, `001-research/008`, nested `015/003/009` + `015/003/013`, plus the parent
  phase-map E/F rows.
...
### Wave R3 — 001-research/009 out-of-box subtree (F-20..F-24) · P1 · ~5 units
- **Packets:** 001-research/008, 001-research/010, and 001-research/009's 8 idea-children — all research/OOB packets that
...
- **Recommendation:** treat 001-research/009 as one coordinated sub-task; decide the level policy once,
...
- **F-25** (015 context-index path ambiguity): clarify the "New location" base path. Small doc edit.
...
- **sol CF-14** — claimed `015/spec.md` + `016/spec.md` are "wrong" paths; both exist.
...
2. **Sequence the remainder** R1 → R2 → R3 → R4. R2 is the cheapest win (one mechanical pattern
   ×5). R3 (001-research/009 subtree) needs a **one-time level-policy decision** before bulk edits — the
...
(Errors:3 each: FILE_EXISTS + LEVEL_MATCH + SECTION_COUNTS). Passing research packets (for example, `001-research/004`) carry
...
mirroring the established `001-research/004` pattern.

**R4 — F-25 done, F-26 scoped out.** F-25 (015 context-index "New location" base) clarified. F-26
