...
- **Related Spec (spec_kit Codex compatibility)**: `010-speckit-codex-compatibility` — this folder does not exist in `03--commands-and-skills/`; the three-pronged approach originated in `system-spec-kit` (spec_kit command infrastructure). No local predecessor folder applies.
