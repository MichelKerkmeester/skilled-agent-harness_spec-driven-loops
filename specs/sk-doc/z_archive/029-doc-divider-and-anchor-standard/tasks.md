...
- [x] T013 DONE: flipped `SKDOC_ENFORCE_STRUCTURE` to default-on (opt out with `=0`), documented for repo users in `core-standards.md` §6 + `validation-and-enforcement.md` §3. Verified: negative control fails by default / passes with `=0`; fleet 0-blocking default-on; sk-doc suite 101 passed. Not wired into any git hook or CI, so it surfaces via manual / quality-control runs (no auto-block).
<!-- /ANCHOR:phase-3 -->
