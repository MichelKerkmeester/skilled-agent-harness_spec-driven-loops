...
**Verification (v4 `c28c54c33b`):** fleet class gate `checked=11 passed=11 failed=0`; contract unit test passes; `parent-skill-check.cjs` exits 0 for all hubs; leaf-manifest freshness 11/11. **Out-of-scope pre-existing failure surfaced, not fixed here:** `command-binding-existence.vitest.ts`'s namespace sanity check expects a `design` command namespace that no longer exists (sk-design's commands live under `interface/` after that rename) — a stale test assertion independent of this change.
<!-- /ANCHOR:amendment-presence-optional -->
