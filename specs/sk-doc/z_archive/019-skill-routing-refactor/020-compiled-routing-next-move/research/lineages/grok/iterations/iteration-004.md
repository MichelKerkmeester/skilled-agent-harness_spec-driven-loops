...
## Findings
1. **Historical hazard is real and confirmed:** the P0 foundation build did `fs.rmSync(RUNTIME_ROOT, { recursive: true, force: true })` then copied into the live root — a crash mid-copy leaves serving half-deleted. [SOURCE: git show 6b81fde29e .opencode/bin/compiled-route-sync.cjs (rmSync at historical +169)]
...
4. **Case AGAINST full complexity:**
   ...
   - Packet is oversized (sync tool ~1086 lines; tests ~1372); speculative three-way external-manifest reconciliation was already trimmed in `4c0336cb65`. [SOURCE: wc -l] [SOURCE: git show 4c0336cb65 commit message / diff notes]
...
## Ruled Out
- Direct live-root `rmSync` + copy publication: historically present; unsafe. [SOURCE: 6b81fde29e rmSync]
...
- git history 6b81fde29e, 4c0336cb65
