...
Both questions this phase opened are now answered and closed. The program **caused** the regression rather than inheriting it: the pin was captured directly at the baseline sha and recorded a healthy 53/72 and 10/11, and the in-range rename commit `33294851882` is what moved the number (REQ-004). The disposition is **fix, not accept**: the attribution (REQ-003) is an unambiguous stale hardcoded path in the delegation scorer, not a deliberate trade-off, so a one-line correction restores every metric to its pin.
