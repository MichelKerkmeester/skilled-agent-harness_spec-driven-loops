...
- [x] T015 [P1] Operator-requested follow-up, 2026-09-14: trim the Skill Routing Reference and Gate 4 in `AGENTS.md`
  ...
  - DONE, fifth pass, a fresh Fable 5.1 high-effort review of the cut at `scratch/fable-review-of-root-doc-cut.md`: verdict safe after eight repairs, all applied. The Opus audit's claim that `command-spec-kit` was retired was false, the bridge is live in the scorer's projection, so Gate 4 names it again. Restored: the retry-count sentence, the memory-save writer pointer, the two artifact-trigger clauses, the branch-primitives wording, the full retrieval path. Repointed four rule files and the code agent that cited deleted headings. Three further cuts the reviewer proposed applied, the confidence table's override row kept. 274 lines, 3451 words.
  - DONE, sixth pass on the operator's instruction that root-only logic stay intact: a line-by-line diff of the original against the current file, every removed line judged for an owner. Restored the seven pieces no skill, command or rule carries: the Iron Law by name, the hard-violation sentence under the plan lock, the four working principles, the stakes read on all non-trivial work, and acknowledging a set goal without asking. 283 lines, 3611 words.
