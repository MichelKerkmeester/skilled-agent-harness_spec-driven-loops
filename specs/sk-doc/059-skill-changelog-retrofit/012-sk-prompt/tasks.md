...
- [ ] T002 Confirm the sk-prompt files are clean in git before the run (`../scratch/lists/sk-prompt.txt`)
<!-- /ANCHOR:phase-1 -->
...
- [ ] T003 Run the driver over the list (`../scratch/rewrite-driver.cjs --list ../scratch/lists/sk-prompt.txt --state ../scratch/state.jsonl`)
...
- [ ] T007 Commit the kept sk-prompt files only, then run `compiled-route-guard.cjs` and each mirror generator's `--check`
