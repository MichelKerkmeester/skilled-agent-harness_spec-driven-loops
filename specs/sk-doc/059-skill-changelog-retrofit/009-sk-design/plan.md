...
# Implementation Plan: Phase 9: sk-design changelogs
...
- [ ] The sk-design commit is pushed, with routing and mirrors fresh
<!-- /ANCHOR:quality-gates -->
...
### Key Components
- **`../scratch/rewrite-driver.cjs`**: run as `--list ../scratch/lists/sk-design.txt --state ../scratch/state.jsonl`
...
Before the commit, restore a file from `../scratch/orig/` or with `git checkout -- <file>`. After it, revert the sk-design commit.
<!-- /ANCHOR:rollback -->
