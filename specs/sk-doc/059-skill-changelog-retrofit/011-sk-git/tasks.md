...
- [ ] T002 Confirm the sk-git files are clean in git before the run (`../scratch/lists/sk-git.txt`)
<!-- /ANCHOR:phase-1 -->
...
- [ ] T003 Run the driver over the list (`../scratch/rewrite-driver.cjs --list ../scratch/lists/sk-git.txt --state ../scratch/state.jsonl`)
...
- [ ] T007 Commit the kept sk-git files only, then run `compiled-route-guard.cjs` and each mirror generator's `--check`
