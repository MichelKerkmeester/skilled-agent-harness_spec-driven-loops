---
title: "Implementation Plan: Phase 2: cli-external-orchestration changelogs"
description: "Runs the phase 001 driver over the 112 listed cli-external-orchestration changelogs on two lanes, keeps each rewrite only when all three gates pass, then commits the skill once."
trigger_phrases:
  - "cli-external-orchestration changelog rewrite plan"
importance_tier: "normal"
contextType: "implementation"
---
<!-- SPECKIT_TEMPLATE_SOURCE: plan-core | v2.2 -->
# Implementation Plan: Phase 2: cli-external-orchestration changelogs
...
### Overview
The driver takes `../scratch/lists/cli-external-orchestration.txt` and the shared state file, so files the pilot already kept are skipped. 109 files run through GPT-6 Luna at xhigh on cli-pi and cli-codex, each gated by the checker, the HVR scan and a fact check. The skill is committed once, after a final gate run over every kept file.
<!-- /ANCHOR:summary -->
...
- [x] The target list exists at `../scratch/lists/cli-external-orchestration.txt`
...
- [ ] The cli-external-orchestration commit is pushed, with routing and mirrors fresh
<!-- /ANCHOR:quality-gates -->
...
### Key Components
- **`../scratch/rewrite-driver.cjs`**: run as `--list ../scratch/lists/cli-external-orchestration.txt --state ../scratch/state.jsonl`
...
Before the commit, restore a file from `../scratch/orig/` or with `git checkout -- <file>`. After it, revert the cli-external-orchestration commit.
<!-- /ANCHOR:rollback -->
