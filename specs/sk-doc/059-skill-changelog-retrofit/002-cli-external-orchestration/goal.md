---
title: "Goal: Phase 2: rewrite the cli-external-orchestration changelogs"
...
_memory:
  continuity:
    ...
    answered_questions: []
---
# Goal: rewrite the cli-external-orchestration changelogs
...
**Objective:** The 112 listed cli-external-orchestration changelogs read in the current format, and each still records exactly what shipped in its version.
...
| D2 | The target set is `../scratch/lists/cli-external-orchestration.txt`. A changelog outside it is not touched. |
...
- [ ] Every file in `../scratch/lists/cli-external-orchestration.txt` has a pass or a failure with its reason in `../scratch/state.jsonl`
...
- [ ] One commit holds the cli-external-orchestration rewrites, and the compiled-route guard and every mirror `--check` pass after it
