---
title: "Goal: Phase 16: rewrite the system-spec-kit changelogs"
...
_memory:
  continuity:
    ...
    answered_questions: []
---
# Goal: rewrite the system-spec-kit changelogs
...
**Objective:** The 108 listed system-spec-kit changelogs read in the current format, and each still records exactly what shipped in its version.
...
| D2 | The target set is `../scratch/lists/system-spec-kit.txt`. A changelog outside it is not touched. |
...
- [ ] Every file in `../scratch/lists/system-spec-kit.txt` has a pass or a failure with its reason in `../scratch/state.jsonl`
...
- [ ] One commit holds the system-spec-kit rewrites, and the compiled-route guard and every mirror `--check` pass after it
...
| Item | State | Evidence |
|------|-------|----------|
| Pilot: v3+/v3.7.0.0 | Pending | Restored in pilot run 2, retry in the wave |
