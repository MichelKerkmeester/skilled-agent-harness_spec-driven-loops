...
### Problem Statement
The root README rework produced formatting patterns the skill never documented: decorative section emoji on a marketing front page versus semantic-only emoji elsewhere, headings that name the feature and its benefit, inventories where every item carries a description and a per-feature H2 structure for large documents. Without those rules written down, the next README falls back to the old shapes.
...
### In Scope
- `SKILL.md`: two-register emoji rule, heading-names-the-feature rule, per-item inventory description rule, and a 2-4 sentence paragraph bound.
- `references/readme/writing-patterns.md`: `Emoji Use` rule, `Paragraphs and Bullets` rule and the `Itemized Inventory` pattern.
- `references/readme/types-and-voice.md`: front-page marketing register versus technical register for all other README types, refined so a skill README may borrow the front page's engaging voice while keeping technical formatting.
...
- `sk-create-skill` README templates: value-first openings (problem narrative optional) and the semantic-only emoji rule, per ADR-002.
...
|-----------|-------------|-------------|
| `.skilled/skills/sk-doc/sk-create-readme/SKILL.md` | Modify | New format rules, version 1.2.0.0 |
...
| `.skilled/skills/sk-doc/sk-create-readme/references/readme/types-and-voice.md` | Modify | Register distinction, version 1.2.1.0 |
...
| `.skilled/skills/sk-doc/sk-create-skill/assets/skill/skill-readme-template.md` | Modify | Value-first opening, emoji rule, stale root refs, version 1.3.0.0 |
| `.skilled/skills/sk-doc/sk-create-skill/assets/parent-skill/parent-skill-readme-template.md` | Modify | Same emoji rule and value-first shift, version 1.3.0.0 |
| `.skilled/skills/sk-doc/sk-create-skill/changelog/v1.3.0.0.md` | Create | Release entry for the template alignment |
<!-- /ANCHOR:scope -->
...
| REQ-004 | ~~An optional `Problem` section is documented for front-page READMEs~~ Superseded by ADR-001: the operator removed the Problem section from the root README, so the pattern lost its flagship example |
| REQ-005 | New rules are opt-in or correctly scoped so existing conformant READMEs stay conformant |
| REQ-011 | `sk-create-skill` README templates teach value-first openings modeled on the current root README, with the problem narrative optional (ADR-002) |
| REQ-012 | `sk-create-skill` README templates carry the semantic-only emoji rule: decorative section emoji never |
