...
| **Phase** | 1 of 7 |
| **Predecessor** | None |
| **Successor** | 002-inventory-and-skill-contract |
| **Handoff Criteria** | The rule set ships and validates, so phase 2 has a working reference implementation to distil a skill contract from |
| **Note** | Moved here from `specs/agents/007-repo-rules-router` when the work became the reference implementation for a create mode. Artifacts under `003-.../research/` retain the original path, because they record where that run actually executed |
<!-- /ANCHOR:metadata -->
