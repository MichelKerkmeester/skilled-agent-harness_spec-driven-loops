...
3. **One defect fixed in the grader, not the documents.** In `mcp-notion`, the real Code Mode call syntax `notion["notion_tool"]({...})` was read as a markdown link across 34 files because the link scan did not exclude fenced code. The durable repair shipped in `038-authoring-hardening` phase `002-validator-false-positives`. The call syntax in the playbook is unchanged, because it was correct.
