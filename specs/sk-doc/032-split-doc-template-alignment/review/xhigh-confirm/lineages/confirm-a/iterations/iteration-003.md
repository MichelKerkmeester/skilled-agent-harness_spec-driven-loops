...
- Read-only target: `.opencode/specs/sk-doc/032-split-doc-template-alignment` plus the three declared sk-code resource/asset surfaces.
...
2. **P2-003: Task and checklist evidence contracts overstate the evidence stored in their rows** -- `.opencode/specs/sk-doc/032-split-doc-template-alignment/tasks.md:23-25`; `.opencode/specs/sk-doc/032-split-doc-template-alignment/checklist.md:23-25,30-82` -- The task file says every checked row carries evidence and the checklist says every item carries a command plus result, but the checked rows contain completion statements without commands, output references, or replay pointers. The summary itself acknowledges checklist rows without inline evidence markers [SOURCE: `.opencode/specs/sk-doc/032-split-doc-template-alignment/implementation-summary.md:83-86`]. Current-state structure and corpus claims remain corroborated, but historical R4 preservation, commit sequencing, push state, and pre-commit gate claims cannot be replayed from these rows.
...
- YAML-owned route, scope, write boundary, lineage, executor, and strict-v2 requirement: `.opencode/specs/sk-doc/032-split-doc-template-alignment/review/xhigh-confirm/lineages/confirm-a/prompts/iteration-003.md:1-50`.
...
- The `code-quality/references` root yielded no files, while `code-quality/assets` contains the documented three files; this is consistent with the packet’s “code-quality (assets)” distribution [SOURCE: `.opencode/specs/sk-doc/032-split-doc-template-alignment/implementation-summary.md:41-46`].
