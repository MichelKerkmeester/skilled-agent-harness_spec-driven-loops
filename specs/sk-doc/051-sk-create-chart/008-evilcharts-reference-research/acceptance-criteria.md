...
| AC-003 | REQ-002 | Given convergence is disabled, When the fan-out runs, Then each lineage completes five iterations | The DeepSeek lineage holds five iteration files. The GLM lineage holds four: it stopped before its fifth, which was the registry question plus a final sweep. The DeepSeek lineage answered that same question, so the pair covers it, but this criterion asked for five each and got nine of ten |  Unmet | Unmet | - |
