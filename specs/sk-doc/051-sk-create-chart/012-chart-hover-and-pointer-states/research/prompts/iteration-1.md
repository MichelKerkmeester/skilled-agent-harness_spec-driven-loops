...
Iteration: 1 of 3
Focus Area: Inventory the twenty forms in assets/examples and assets/templates. For each, decide whether a reader can already read every value off the static figure. Classify each form as needs-pointer or correctly-inert, and say which of the thirteen currently-inert forms are which. Then start on technique, reading the vendored evilcharts source for how it binds a pointer to a mark.
Remaining Key Questions: - Which of the thirteen forms with no pointer response should gain one, and which are correctly inert?
- What hover technique transfers to a single self-contained file, from evilcharts and other named libraries inspected rather than recalled?
- Which of crosshair, nearest-point snapping, axis readout, focus ring, cursor affordance and hover transition earn their place here?
- Does a hover-only affordance break the corpus accessibility posture, and what is the keyboard and screen-reader equivalent?
- What do assets/templates and the corpus checker need so pointer behaviour is enforced rather than hand-applied?
...
- Write iteration narrative to: specs/sk-doc/051-sk-create-chart/012-chart-hover-and-pointer-states/research/iterations/iteration-001.md
- Write per-iteration delta file to: specs/sk-doc/051-sk-create-chart/012-chart-hover-and-pointer-states/research/deltas/iteration-001.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `specs/sk-doc/051-sk-create-chart/012-chart-hover-and-pointer-states/research/iterations/iteration-001.md`, this iteration's narrative markdown
  - `specs/sk-doc/051-sk-create-chart/012-chart-hover-and-pointer-states/research/deltas/iteration-001.jsonl`, this iteration's delta JSONL
...
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the research. NEVER execute the out-of-scope mutation. The research packet (`specs/sk-doc/051-sk-create-chart/012-chart-hover-and-pointer-states/research/iterations/iteration-001.md` directory and parents) is the only zone for your writes; the researched target/topic surface is off-limits.
...
1. **Iteration narrative markdown** at `specs/sk-doc/051-sk-create-chart/012-chart-hover-and-pointer-states/research/iterations/iteration-001.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
`--event-json` must name the SINGLE-record file (the gateway `JSON.parse`s it whole), never the multi-line `specs/sk-doc/051-sk-create-chart/012-chart-hover-and-pointer-states/research/deltas/iteration-001.jsonl`. Exit `0` = the record is durable in the ledger and the projection is refreshed; exit `2` = refused → STOP and name the failed check. Never fall back to a direct write.

For non-native CLI executors, the workflow owns executor provenance. It writes a pre-dispatch sentinel, then patches the first canonical `"type":"iteration"` record with the `executor` block before `post_dispatch_validate` runs. Do NOT append your own `dispatch_failure` event or a partial fallback record when the executor itself crashes or times out; the workflow emits the typed failure event on that path.

3. **Per-iteration delta file** at `specs/sk-doc/051-sk-create-chart/012-chart-hover-and-pointer-states/research/deltas/iteration-001.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
