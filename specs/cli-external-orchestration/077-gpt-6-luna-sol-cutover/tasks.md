...
- [x] T065 The two named Codex profiles, approved by the operator after the close on 2026-09-23: `~/.codex/luna-impl.config.toml` to `gpt-6-luna` and `~/.codex/sol-verify.config.toml` to `gpt-6-sol`, each backed up first. Evidence: no `codex exec` running; one changed line in each; no `5.6` left in either; `codex -p <profile> debug prompt-input` exits 0 for both; the two backups deleted afterwards at the operator's direction
