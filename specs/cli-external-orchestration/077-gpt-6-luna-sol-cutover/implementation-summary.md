...
_memory:
  continuity:
    ...
    recent_action: "Gateway Luna default kept; xiaomi block dropped"
...
| One-turn live smokes | PASS: Pi `openai-codex/gpt-6-luna`, `openai-codex/gpt-6-sol`, `llmgateway/gpt-6-luna` ($0.0009) and Hermes `gpt-6-luna` replied `OK`. Not run: Hermes `gpt-6-sol`, Codex, OpenCode. cli-claude-code refused inside Claude Code |
...
1. **Some routes have no live round-trip.** Hermes `gpt-6-sol`, both Codex ids and both OpenCode ids are catalog-listed only; the operator stopped the smoke run before them. cli-claude-code refuses to dispatch from inside Claude Code, so `claude-opus-5-5` needs a smoke from another shell.
2. **The `-pro` slugs are unconfirmed.** `opencode models openai` listed no `-pro` slug for either generation on 2026-09-23, so the renamed `openai/gpt-6-sol-pro` and `-luna-pro` rows, and the cli-opencode fallback that recommends `openai/gpt-6-sol-pro`, may name ids this account cannot select.
...
4. **Pi's default changed under a running Pi session.** `.pi/settings.json`, which `~/.pi/agent/settings.json` links to, was rewritten at 08:22 local while a Pi session was open. Pi saving its current model is the likely cause, not confirmed. A model switch in any open Pi session can change the repo default again.
