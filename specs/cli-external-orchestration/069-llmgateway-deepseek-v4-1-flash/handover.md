...
| **cli-pi self-dispatch is allowed by name; ambiguous compound commands are denied** | Any `pi` dispatch from a pi session | Load-bearing | A named cli-pi dispatch is allowed, judged by the same named-executor rule as any sibling; separate guards still refuse a dispatch from inside a fan-out lineage and one whose kind already appears in the dispatch stack. Compound commands (vars, `&`, `&&`) are classed `ambiguous` and denied. `pi --help` is not denied: the command inspector classifies it as not a dispatch, so it is never evaluated |
