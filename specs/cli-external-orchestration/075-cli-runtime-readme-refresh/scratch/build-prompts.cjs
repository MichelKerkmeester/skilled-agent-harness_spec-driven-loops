#!/usr/bin/env node
// ───────────────────────────────────────────────────────────────
// Emit one self-contained dispatch prompt per cli runtime README.
//
// The eight prompts share a single edit contract, so they are generated from one
// copy of that contract rather than hand-written eight times: eight hand-written
// copies drift, and the drift shows up as eight subtly different READMEs. The
// folder inventory is computed from disk here and baked into the artifact, so the
// child receives a fixed snapshot and can report a disagreement rather than
// silently anchoring on whatever it happened to read.
//
// Output: scratch/prompts/<runtime>.md (one file per runtime).
// ───────────────────────────────────────────────────────────────
'use strict';
...
const SKILLS = path.join(ROOT, '.skilled/skills/cli-external-orchestration');
const OUT = path.join(HERE, 'prompts');

const PACKET_REL = 'specs/cli-external-orchestration/075-cli-runtime-readme-refresh';
const RUNTIMES = ['claude-code', 'codex', 'cursor', 'devin', 'hermes', 'jev', 'opencode', 'pi'];

// A runtime whose README already carries the section needs it extended in place
// instead of inserted, and jev additionally owes the validator an OVERVIEW heading.
const PER_RUNTIME = {
  hermes: {
    mode: 'extend',
    extra: [
      'Your README ALREADY HAS a `## 5. STRUCTURE` section. Do NOT add a second one and do NOT renumber anything.',
      'Its fenced tree omits `feature-catalog/`, which exists on disk. Add `feature-catalog/` and its entries to',
      'the existing tree, and add the matching `File | Role` rows. Leave the rest of that section as it stands.',
    ],
  },
  jev: {
    mode: 'retitle',
    extra: [
      'Two changes, both specific to this file:',
      '1. Retitle `## 1. WHAT THIS PACKET IS` to `## 1. OVERVIEW`, keeping its existing body and adding one lead',
      '   sentence. The shared validator requires a section named OVERVIEW; this file is the only one that fails.',
      '2. Your README already has a `## 5. LAYOUT` section holding a fenced tree. EXTEND that tree so it names every',
      '   `references/` and `assets/` file below, and extend its role table to match. Do not add a second tree and do',
      '   not renumber any heading.',
    ],
  },
};

function inventory(runtime) {
  const dir = path.join(SKILLS, `cli-${runtime}`);
  const dirs = fs
  ...
  for (const sub of ['references', 'assets']) {
    ...
    files[sub] = fs.existsSync(subDir)
      ? fs.readdirSync(subDir).filter((f) => fs.statSync(path.join(subDir, f)).isFile()).sort()
      : [];
  }
  return { dirs, files };
}

function inventoryBlock(runtime) {
  const { dirs, files } = inventory(runtime);
  const lines = [];
  lines.push(`cli-${runtime}/`);
  lines.push('  SKILL.md              # the routing contract (read it: every claim you write must already live in it)');
  lines.push('  README.md               # THIS FILE - your one write target');
  for (const d of dirs) lines.push(`  ${d}/`);
  for (const sub of ['references', 'assets']) {
    for (const f of files[sub]) lines.push(`    ${sub}/${f}`);
  }
  return lines.join('\n');
}

function prompt(runtime) {
  const spec = PER_RUNTIME[runtime] || { mode: 'insert', extra: [] };
  const { dirs, files } = inventory(runtime);

  const requiredDirs = dirs.map((d) => `\`${d}/\``).join(', ');
  const requiredFiles = [
    ...files.references.map((f) => `\`references/${f}\``),
    ...files.assets.map((f) => `\`assets/${f}\``),
  ].join(', ');

  return `GATE 3 IS PRE-RESOLVED. DO NOT ASK THE DOCUMENTATION-SCOPE QUESTION.
...
BINDING: target=${SKILLS.replace(ROOT + '/', '')}/cli-${runtime}/README.md
BINDING: output=${SKILLS.replace(ROOT + '/', '')}/cli-${runtime}/README.md
BINDING: template=.skilled/skills/sk-doc/sk-create-readme/assets/readme-template.md
BINDING: mode=AUTONOMOUS
BINDING: specFolder=${PACKET_REL}
\`\`\`
...
\`.skilled/skills/cli-external-orchestration/cli-${runtime}/README.md\`
...
3. NAME EVERY FILE. These must all appear, spelled exactly:
   immediate subdirectories: ${requiredDirs}
   files: ${requiredFiles}

${spec.extra.length ? spec.extra.map((l) => `   ${l}`).join('\n') : ''}
...
python3 .skilled/skills/sk-doc/shared/scripts/validate_document.py .skilled/skills/cli-external-orchestration/cli-${runtime}/README.md

# 2. section numbering must be sequential with no gaps
grep -n '^## ' .skilled/skills/cli-external-orchestration/cli-${runtime}/README.md

# 3. your diff must be additive
git diff --stat -- .skilled/skills/cli-external-orchestration/cli-${runtime}/README.md
git diff -- .skilled/skills/cli-external-orchestration/cli-${runtime}/README.md
\`\`\`
...
for (const runtime of RUNTIMES) {
  const body = prompt(runtime);
  fs.writeFileSync(path.join(OUT, `${runtime}.md`), body);
  written++;
}
console.log(`build-prompts: wrote ${written} prompt(s) to ${path.relative(ROOT, OUT)}`);
for (const runtime of RUNTIMES) {
  const f = path.join(OUT, `${runtime}.md`);
  console.log(`  cli-${runtime.padEnd(12)} ${fs.statSync(f).size} bytes`);
}
