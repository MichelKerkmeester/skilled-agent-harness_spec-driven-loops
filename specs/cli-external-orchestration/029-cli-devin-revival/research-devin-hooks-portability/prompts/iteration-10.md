...
Questions: 0/6 answered | Last focus: Iteration 9 — ADR-001 evidence package assembly
Last 2 ratios: 0.92 -> 0.31
Resource map: resource-map.md not present; skipping coverage gate.
Memory context refresh: phase 001 implementation-summary.md is on disk in spec folder
Next focus: Iteration 10 — final convergence report and remaining gaps. Produce research/convergence-report.md (or append to research/research.md's appendix): total iterations run, total findings, registry key count, ratios sparkline, key-question cove
...
Iteration: 10 of 10
Focus Area: Iteration 10 — final convergence report and remaining gaps. Produce research/convergence-report.md (or append to research/research.md's appendix): total iterations run, total findings, registry key count, ratios sparkline, key-question coverage matrix (Q1-Q6 answered/not with per-row evidence), remaining open questions (live or deferred to phase 004 implementation), recommended next-step for phase 004-devin-hook-adapter-layer (the ADR-001 evidence package will live at research/evidence/). This iteration MUST be the convergence-report-only iteration — no new file:line claims beyond consolidating prior iterations.
...
<!-- ANCHOR:next-focus -->
Last 3 Iterations Summary: run 7: Iteration 7 — PreCompact → PostCompaction substitution (0.9) | run 8: Iteration 8 — OpenCode plugins that cannot port + alternativ (0.92) | run 9: Iteration 9 — ADR-001 evidence package assembly (0.31)
...
- Write iteration narrative to: .opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-010.md
- Write per-iteration delta file to: .opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-010.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-010.md`, this iteration's narrative markdown
  ...
  - `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/deltas/iter-010.jsonl`, this iteration's delta JSONL
- **BANNED OPERATIONS (NEVER execute against any path)**: `rm`, `rm -rf`, `git rm`, `mv`, `sed -i` (including `sed -i ''`), `rmdir`, `find ... -delete`, shell output-redirect truncate `>` against any file not in the allowed-write list, and any tool call whose effect is to delete, rename, or replace a file outside the allowed-write list. Reading is unrestricted; **writing, renaming, and deleting are scoped**.
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the research. NEVER execute the out-of-scope mutation. The research packet (`.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-010.md` directory and parents) is the only zone for your writes; the researched target/topic surface is off-limits.
...
1. **Iteration narrative markdown** at `.opencode/specs/cli-external-orchestration/029-cli-devin-revival/research/iterations/iteration-010.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
