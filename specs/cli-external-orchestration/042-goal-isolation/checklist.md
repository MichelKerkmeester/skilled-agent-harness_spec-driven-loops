...
- [x] CHK-018 [P1] Same-session concurrency has a tested serialization or revision contract. [Evidence: `.opencode/hooks/goal/bin/goal.test.cjs` concurrent-writer test]
...
- [x] CHK-031 [P0] Empty, delimiter-heavy, Unicode-control, traversal-shaped, oversized, and cross-workspace identity cases are tested. [Evidence: `.opencode/hooks/goal/lib/goal-core.test.cjs` scope-validation table]
...
- [x] CHK-073 [P1] Final scoped diff contains no task-created residue or unrelated task changes.
  - Evidence: `git diff --check` and temp-residue checks passed; unrelated dirty-worktree changes were preserved.
<!-- /ANCHOR:deploy-ready -->
...
**Current verdict**: implementation and required verification complete; Pi goal extension re-enabled with a verified rollback path.
<!-- /ANCHOR:summary -->
