...
_memory:
  continuity:
    ...
    recent_action: "Composite session scope accepted and verified"
...
**How it works**: one resolver validates the three identity fields and derives an opaque collision-resistant scope key. Scoped JSON uses `.goal-state/<runtime>-<scope-hash>.json`; archives use `.goal-state/.archive/<scope-key>/`. Runtime adapters supply identity from their native context; the management surface obtains that same identity without asking users to type it.
...
| Same-session cross-process writes race | Medium | Preserve atomic rename and add lock/revision handling if a dedicated negative test reproduces lost updates. |
<!-- /ANCHOR:adr-001-consequences -->
...
- Tests cover two simultaneous sessions, namespace collisions, missing identity, legacy state, and lifecycle mutations.
