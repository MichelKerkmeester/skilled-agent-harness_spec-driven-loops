...
description: "Draft implementation and release gates for Phase 008; implementation evidence remains pending."
trigger_phrases:
...
_memory:
  continuity:
    ...
    recent_action: "Aligned the Phase 008 checklist with package hardening and terminal release evidence."
    next_safe_action: "Obtain project-owner approval, then collect evidence without marking release gates early."
    blockers:
...
- [x] CHK-003 [P1] Dependencies and handoff are identified. [evidence: `spec.md` and `plan.md` name 007-evaluation-and-observability and the parent packet release decision]
<!-- /ANCHOR:pre-impl -->
...
- [ ] CHK-023 [P1] Edge cases pass: unsupported runtime major after upgrade; stale privacy facts with a healthy endpoint; expired OpenCode Go evidence; presentation-tier mismatch; partial installation; rollback during active generation or after provider failure.
...
- [ ] CHK-051 [P1] Parent map, signed evidence manifest, and terminal parent release decision match final status.
...
- [ ] CHK-110 [P1] The provisional five-second local-doctor target is measured under the frozen Phase 002 benchmark profile, with network probes timed separately.
...
- [ ] CHK-122 [P1] Operator runbooks and the terminal parent release handoff are current.
<!-- /ANCHOR:deploy-ready -->
