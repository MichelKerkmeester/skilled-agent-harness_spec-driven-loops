...
| Plugin suite | PASS: `node --test .opencode/plugins/tests/mk-communication-projection.test.cjs` passes 19/19 tests |
...
The Phase 029 design touch list names `docs/enablement.md` and `docs/configuration.md` as a later-build user-facing docs touch point. This phase scoped `docs/` out, and this follow-up delivers that touch point by documenting the `localProvider` block in both files, so the record is consistent.
<!-- /ANCHOR:deviations -->
