...
| **Parent Spec** | `../spec.md` |
| **Predecessor** | `../012-codex-claude-hooks-discovery-mirrors/spec.md` |
| **Successor** | `../014-cursor-agents-skills-rules-parity/spec.md` |
<!-- /ANCHOR:metadata -->
...
| REQ-008 | The enforce consumer resolves `workspace_roots[0]` the same way the prebind producer does. | A whitespace-only or missing root falls back to `process.cwd()` on both sides, so state is read from the same directory it was written to. |
| REQ-009 | The prebind's `process.cwd()` fallback path is covered by a discriminating test. | A payload omitting `workspace_roots` writes state under the spawn cwd and the enforce consumer denies a Write through that same cwd. |
<!-- /ANCHOR:requirements -->
...
- **Post-closeout hardening — RESOLVED.** An independent review found a producer-consumer disagreement: the prebind trims and falls back to `process.cwd()` for whitespace-only `workspace_roots[0]`, but the enforce consumer used a raw `||` that treated `"   "` as a truthy root and read state from a different directory. The same review found the prebind's `process.cwd()` fallback path was untested. Both are fixed: the enforce consumer now matches the prebind's trim-and-fallback, and two discriminating tests (missing-roots and whitespace-root) were added to the prebind suite.
<!-- /ANCHOR:questions -->
