...
_memory:
  continuity:
    ...
    recent_action: "Recorded local, official-source, provider, and runtime research."
    next_safe_action: "Authorize and execute the isolated-worktree deep-research fanout."
    blockers:
      - "External cli-opencode executors cannot run in the dirty current worktree under their safety contract."
    key_files:
      - "plan.md"
      - "spec.md"
      - "checklist.md"
    ...
    completion_pct: 55
    open_questions:
      - "Authorization for a temporary isolated executor worktree remains outstanding."
    answered_questions:
...
- [ ] An isolated executor worktree is authorized for dangerous-permission cli-opencode dispatch.

### Definition of Done

- [ ] All P0 requirements and checklist items have evidence.
- [ ] Native research and both deep-loop lineages have been triangulated.
- [ ] Exactly 7 DeepSeek and 3 GPT iterations are proven by canonical state and iteration files.
- [ ] The final synthesis recommends downstream phases and inherited gates.
- [ ] Parent and child pass recursive strict spec validation from final state.
<!-- /ANCHOR:quality-gates -->
...
- [ ] Preserve the final dated matrices in deep-research synthesis or a dedicated research artifact.

### Phase 3: Capability matrices

- [x] Establish initial `confirmed`, `inferred`, `unsupported`, and `unknown` labels.
- [ ] Reconcile runtime/version caveats and provider claims with deep-loop output.
- [ ] Identify which claims require read-only active probes before implementation.
...
- [ ] Compare every load-bearing deep-loop claim with primary sources or explicit probe evidence.
- [ ] Record disagreements between native agents, DeepSeek, and GPT rather than averaging them away.
- [ ] Distinguish confirmed facts, inferences, dated policy, unknowns, and implementation hypotheses.
- [ ] Convert provider/runtime uncertainty into capability probes and release gates.

### Phase 6: Synthesize downstream work

- [ ] Freeze the normalized architecture and provider policy only after triangulation.
- [ ] Produce the evaluation corpus design, deterministic gates, blind rubric, and operational metrics.
- [ ] Recommend child phases with scope, dependencies, handoffs, rollback, and authoritative checks.
<!-- /ANCHOR:phases -->
...
- **Procedure**: Stop the workflow, preserve logs for diagnosis, copy nothing from the isolated executor worktree, and leave the current packet/reference unchanged. Remove the temporary worktree only after confirming it contains no unique accepted artifact.
<!-- /ANCHOR:rollback -->
...
- [ ] Temporary worktree path and branch are explicitly authorized.
- [ ] Current branch commit and dirty-state recovery information are recorded.
- [ ] Executor prompt contains literal banned operations and allowed write paths.
- [ ] Model IDs, reasoning effort, iteration counts, stop policy, timeout, and live-web policy pass preflight.
- [ ] Copy-back allowlist is restricted to validated research artifacts for this phase.
