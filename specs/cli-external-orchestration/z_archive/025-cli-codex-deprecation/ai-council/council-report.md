...
### Recommended Plan
Use the existing packet `.opencode/specs/cli-external-orchestration/025-cli-codex-deprecation`. No new top-level spec is needed. Because the packet currently records completion for exact `cli-codex` retirement, the implementation should add a follow-up task/evidence update in the same packet rather than split context into a new phase folder.
...
bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh .opencode/specs/cli-external-orchestration/025-cli-codex-deprecation --strict
```
