...
| **Level** | 1 |
| **Priority** | P1 |
| **Status** | Retired — not started (upstream model withdrawn 2026-09-18; successor declined by the operator) |
| **Created** | 2026-09-17 |
...
- **Answered 2026-09-18 by live probe, and it closes the packet: the successor is reachable on Cline, and metered.** Cline's live API carries `unbiased/pareto` among 446 ids, so the model exists and the route reaches it — `pi --provider cline-pass --model cline-pass/unbiased/pareto` returned `402 {"code":"insufficient_credits","message":"Insufficient balance. Your Cline Credits balance is $-0.04"}`, a billing refusal rather than the `404` the withdrawn id gives. **The control separates the two:** `cline-pass/z-ai/glm-5.3-flash` on the same account returned its token at exit 0, so the account and the Pass subscription serve normally and Pareto specifically bills against Cline Credits rather than the Pass. The other half of the route does not exist: `opencode models opencode-go` carries neither `union` nor `pareto`, so no opencode-go row is available at any price. pi also warns `Model "unbiased/pareto" not found for provider "cline-pass". Using custom model id`, so a first-class row would additionally need an `enabledModels` entry. **Operator decision 2026-09-18: skip it.** The packet existed to make a free model dispatchable; the successor is metered on the only surviving route, and the subsidised DeepSeek and GLM routes already carry this work. Both rosters stay unchanged and this packet is retired rather than repointed.
- **Closed 2026-09-18: the four roster-row sentences.** They no longer need a carrier — `--provider opencode-go --model <id>`, the two `Reachable as` and `Select flash with` selections, and the `select this route by hand with --provider opencode-go` clause were each rewritten to the provider-qualified `--model` form, and the rule's own example dropped the withdrawn `openrouter/stealth/union-alpha` id. The one remaining split-form string in that file is a dated dispatch record, kept as written.
