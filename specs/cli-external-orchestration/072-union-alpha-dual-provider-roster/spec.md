...
- **Answered 2026-09-17: does the OpenRouter credential still authorize a completion, or only a catalog read?** It authorizes completions. Both OpenRouter dispatches returned the expected token, so the account packet 068 described as being retired is serving, and no row needs the listing-only hedge.
- **Does the opencode-go route have a thinking tier at all?** Its catalog column says yes and the one behavioral test says no, and this packet does not resolve it. Settling it needs a discriminator this packet did not build: a prompt whose answer differs measurably with and without reasoning, run against a control on the same provider. Until then the row states both readings, and no dispatch guidance depends on which wins.
- Should `union-alpha` enter the hub's advisor vocabulary so a prompt naming the model routes to these skills? Out of scope here by operator decision, and worth raising now that all four routes are verified.
