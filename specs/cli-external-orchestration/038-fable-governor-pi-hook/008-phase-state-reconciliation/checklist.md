...
_memory:
  continuity:
    ...
    key_files:
      ...
      - "../graph-metadata.json"
    session_dedup:
      fingerprint: "sha256:e6c9b0fb7a5999d735708871e5fbb980eb0348dbed2ef599831c507786c9471e"
...
- [x] CHK-002 [P0] Pre-change metadata copies and diff baseline are captured for rollback. [TESTED: baseline validator and status snapshot]
  - [EVIDENCE: baseline `bash .opencode/skills/system-spec-kit/scripts/spec/validate.sh .opencode/specs/cli-external-orchestration/038-fable-governor-pi-hook --strict` reported metadata-integrity failures before repair; the pre-change status snapshot and packet file hashes were captured outside the repository.]
...
- [x] CHK-023 [P1] Resume pointer (`last_active_child_id`) identifies the most recently reconciled child while matching the Complete parent rollup. [TESTED: final JSON read]
  - [EVIDENCE: final parent graph metadata points to `cli-external-orchestration/038-fable-governor-pi-hook/009-injection-contract-directive-sync`; parent and all nine child statuses are Complete.]
<!-- /ANCHOR:testing -->
...
- [x] CHK-032 [P1] Changes are confined to spec artifacts and generated metadata under `.opencode/specs`. [TESTED: scope sweep]
  - [EVIDENCE: final status comparison against the pre-worker snapshot shows no new path outside `.opencode/specs/cli-external-orchestration/038-fable-governor-pi-hook`; production source and tests remain untouched by this worker.]
<!-- /ANCHOR:security -->
