...
| Item | Note |
|------|------|
| Hermes bounds `post_tool_call` and `transform_tool_result` on worker threads | A post hook cannot stage state for the transform hook; an advisory that must reach the result is computed inside the transform hook itself. |
