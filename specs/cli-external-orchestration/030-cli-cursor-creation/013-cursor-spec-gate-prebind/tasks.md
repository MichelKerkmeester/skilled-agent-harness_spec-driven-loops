...
- [x] T014 Fix the enforce consumer's workspace-root resolution to match the prebind producer (trim-and-fallback instead of raw `||`). (`spec-gate-enforce.mjs`) [EVIDENCE: enforce consumer uses trim-and-fallback; whitespace-root regression test denies.]
- [x] T015 Add `process.cwd()` fallback and whitespace-root discriminating tests to the prebind suite. (`spec-gate-prebind.test.mjs`) [EVIDENCE: prebind suite reports 11/11, including the missing-roots and whitespace-root rows.]
<!-- /ANCHOR:phase-4 -->
