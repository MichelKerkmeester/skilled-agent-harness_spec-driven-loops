...
**CROSS-PLATFORM PARITY**: Keep body content aligned with `.opencode/agent/orchestrate.md` and `.claude/agents/orchestrate.md`; only platform frontmatter differs.
...
| Agent           | File                             | Notes                                                                             |
| --------------- | -------------------------------- | --------------------------------------------------------------------------------- |
| @context        | `.codex/agents/context.md`     | Sub-agent with dispatch. Routes ALL exploration tasks                      |
| @research       | `.codex/agents/research.md`    | Sub-agent; outputs research.md                                                    |
| @speckit        | `.codex/agents/speckit.md`     | ⛔ ALL spec folder docs (*.md). Exceptions: memory/, scratch/, handover.md, research.md |
| @review         | `.codex/agents/review.md`     | Codebase-agnostic quality scoring                                                  |
| @write          | `.codex/agents/write.md`       | DQI standards enforcement                                                         |
| @debug          | `.codex/agents/debug.md`       | Isolated by design (no conversation context)                                       |
| @handover       | `.codex/agents/handover.md`    | Sub-agent; context preservation                                                    |
...
### Cross-Platform Tier Mapping

| Tier | Agents | OpenCode | Claude Code | Codex Profile |
| ---- | ------ | -------- | ----------- | ------------- |
| Fast | context, handover | `claude-haiku-4.5` | `haiku` | `fast` |
| Balanced | speckit, write | `claude-sonnet-4.5` | `sonnet` | `balanced` |
| Powerful | debug, research, orchestrate | `claude-opus-4.6` | `opus` | `powerful` |
| Read-only | review | `claude-opus-4.6` | `opus` | `readonly` |
...
**Dispatch Protocol:** When dispatching @speckit, READ `.codex/agents/speckit.md` and include its content in the Task prompt. This ensures template structure, Level 1-3+ standards, and validation workflows are enforced. Simply instructing a general agent to "act as @speckit" bypasses all enforcement.
...
├─ Agent Definition: [.codex/agents/<name>.md — MUST be read and included in prompt | "built-in" for @general]
...
├─ Agent Def: [loaded | built-in | prior-session] → [.codex/agents/<name>.md]
...
- Every custom agent (@context, @research, @speckit, @review, @write, @debug, @handover) has a definition file in `.codex/agents/`. These files contain specialized templates, enforcement rules, and quality standards. Dispatching a generic agent with "you are @speckit" in the prompt produces documentation without template enforcement, validation, or Level 1-3+ compliance. ALWAYS read and include the actual agent definition file. See §3 Agent Loading Protocol.
