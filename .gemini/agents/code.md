...
>
> "SCOPE_CONFLICT: @code does not author skill/agent/command components. The orchestrator should dispatch this work to @markdown (see `.gemini/agents/markdown.md`) using the orchestrator's agent dispatch. @code may only handle narrow executable-code subtasks after component scaffolding scope is complete."
>
...
- Author skill, agent, or command component scaffolds/docs under `.opencode/skills/**`, `.opencode/agents/**`, or `.opencode/commands/**`; return `SCOPE_CONFLICT` and tell the orchestrator to dispatch `@markdown` (see `.gemini/agents/markdown.md`) unless the task is a narrow executable-code subtask with exact files and verification
