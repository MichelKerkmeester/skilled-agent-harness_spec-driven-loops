...
>
> **YOUR FIRST ACTION:** Identify the requested agent from the user input and read its `.md` identity file from the Gemini runtime directory first: `.gemini/agents/`. If runtime guidance is insufficient and you need authoring-family context, then consult `.opencode/agents/` as a supporting source.
...
2. LOCATE and READ the agent's identity file:
   ...
   - If additional authoring context is needed, check `.opencode/agents/<agent_name>.md`
   - If additional authoring context is needed, check `.opencode/agents/<agent_name>.md`
