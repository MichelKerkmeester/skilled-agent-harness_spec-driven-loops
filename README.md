...
- **`cli-hermes`**: Hermes Agent CLI orchestrator. Use it for **quiet oneshot headless dispatch, cross-AI validation and LLM Gateway model routing**, dispatched through `hermes chat -Q --oneshot` (Nous Research's Python agent CLI, closed two-id roster: `deepseek-v4.1-flash` and `glm-5.3-flash` through the operator's `llmgateway` provider). **Availability-gated / fails closed:** checks `command -v hermes` and reads `hermes status` for a configured provider before dispatching, and refuses the route when either fails. It has no agents directory, so the resolved persona is inlined into the prompt, and its repo surface is `.hermes/` (per-skill symlinks, generated prompt templates, one `repo-guards` plugin)
