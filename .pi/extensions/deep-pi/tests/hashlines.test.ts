...
// This mock writes external content between the validation read and atomic replacement.
// The stale-snapshot race is therefore reproduced deterministically.
...
it('rejects unsupported models without touching the file', async () => {
  ...
  const result = await tool.execute(
    'call-1',
    {
      ...
      edits: [{
        from: 1,
        from_hash: lineHash('alpha'),
        to: 1,
        to_hash: lineHash('alpha'),
        new_text: 'A',
      }],
    },
...
it('does not replace a symlink while leaving its target stale', async () => {
  // Replacing through a symlink would swap a temporary inode while leaving the target stale.
  // The atomic path must therefore reject it before writing.
  const dir = await mkdtemp(join(tmpdir(), 'deeppi-hashlines-'));
  created.push(dir);
...
it('does not give known distinct line content the same hash', () => {
  // This known pair guards distinct line content against a hash collision regression.
