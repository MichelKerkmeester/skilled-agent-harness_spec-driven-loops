...
This package is a fork of `jiangge/pi-cache-optimizer` v2.8.0. The fork is published on GitHub as `MichelKerkmeester/pi-cache-optimizer`. The copy vendored at `.pi/extensions/pi-cache-optimizer/` is the actual runtime source used by Pi; the GitHub fork records provenance and is not the live install source. The fork briefly ceded two DeepSeek-direct models (`deepseek/deepseek-v4-flash`, `deepseek/deepseek-v4-pro`) to a sibling extension through an ownership predicate and a duplicated allowlist; that split ended on 2026-09-08, when the sibling was retired and the predicate and its machinery were removed. The fork now runs for every model Pi can reach. The MIT license is unchanged. See [CHANGES-FROM-UPSTREAM.md](./CHANGES-FROM-UPSTREAM.md) for the complete patch and verification record.
