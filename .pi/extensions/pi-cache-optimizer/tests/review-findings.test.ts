// ───────────────────────────────────────────────────────────────────
// MODULE: Pi Cache Optimizer Review Findings Tests
// ───────────────────────────────────────────────────────────────────
...
describe('footer stats modes', () => {
  // Keep session and total buckets distinct so scope-selection tests catch cross-scope fallback.
  ...
  test('config command overrides the environment mode', async () => {
    ...
    try {
      // Reload the extension against the isolated agent directory so persisted mode is test-local.
      process.env.PI_CODING_AGENT_DIR = tempAgentDir;
  ...
  test('interactive menu exposes and applies footer mode', async () => {
    ...
    try {
      // Keep the menu's persisted choice in an isolated temporary directory.
      process.env.PI_CODING_AGENT_DIR = tempAgentDir;
...
describe('explicit compat precedence', () => {
  ...
  test('before_provider_request reads modelOverrides from the active agent directory', async () => {
    ...
    try {
      // Reload after changing the agent directory so configuration is read from this fixture.
      process.env.PI_CODING_AGENT_DIR = tempAgentDir;
      const jiti = createJiti(join(process.cwd(), 'tests', 'review-findings.test.ts'), {
        interopDefault: false,
        moduleCache: false,
...
describe('/cache-optimizer fix command', () => {
  ...
  "providers": {
    ...
    try {
      // Keep backup and replacement assertions away from user configuration.
      process.env.PI_CODING_AGENT_DIR = tempAgentDir;
      ...
      const assertApplied = async (): Promise<string> => {
        ...
        assert.equal(
          parsed.providers.proxy.modelOverrides['deepseek-v4'].compat.overrideOnly,
          'preserved',
        );
