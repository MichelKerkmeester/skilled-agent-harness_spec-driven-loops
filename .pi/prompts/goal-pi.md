...
The `goal-context.ts` extension registers the authoritative `/goal-pi` command, whose actions on the current runtime are `bind <packet-path>`, `unbind`, `resent`, `log <item> | <state> | <evidence>`, `packet <packet-path>`, `packet-log <packet-path> <item> | <state> | <evidence>`, `set <objective>`, `show`, `clear`, `complete`, `pause`, `resume`, `history`, `doctor`, `health`, and the three `legacy-*` migration actions. `bind` makes a packet's `goal.md` the directive; `packet` and `packet-log` need a workspace but no session. An unrecognized action is not rejected: it falls through to `set`, so a mistyped action becomes the objective text. Pi executes registered extension commands before prompt templates, so a healthy goal extension handles the command natively with `ctx.sessionManager.getSessionId()` and this file is never expanded.
