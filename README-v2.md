...
Multi-agent AI development framework with cognitive memory, structured documentation, 12 agents, 18 skills, 22 commands, 42 MCP tools -- built for [OpenCode][opencode-url], Codex CLI, Claude Code and Gemini CLI.
...
<span>&nbsp;&nbsp;&bull;&nbsp;&nbsp;</span>
<a href="https://github.com/MichelKerkmeester/opencode-spec-kit-framework/releases">Releases</a>
<span>&nbsp;&nbsp;&bull;&nbsp;&nbsp;</span>
<a href="https://github.com/MichelKerkmeester/opencode-spec-kit-framework/issues">Issues</a>
<span>&nbsp;&nbsp;&bull;&nbsp;&nbsp;</span>
<a href="https://buymeacoffee.com/michelkerkmeester">Buy Me a Coffee</a>
...
OpenCode is a multi-agent AI development framework built on top of the [OpenCode][opencode-url] platform. Think of it like giving your AI assistant a filing cabinet, a long-term memory and a team of specialists -- instead of one forgetful generalist that starts from scratch every time you open a new chat window.
...
<th width="180">Manual Approach</th>
<th width="180">Basic AI Chat</th>
<th width="340">OpenCode Framework</th>
</tr>
<tr>
<td><strong>Documentation</strong></td>
<td>Ad hoc, inconsistent</td>
<td>None</td>
<td>Templated spec folders at 4 levels with 20-rule validation</td>
</tr>
<tr>
<td><strong>Context across sessions</strong></td>
<td>Copy-paste from notes</td>
<td>Stateless</td>
<td>Persistent semantic memory with session recovery</td>
</tr>
<tr>
<td><strong>Search</strong></td>
<td>Ctrl+F in files</td>
<td>Single-pass vector</td>
<td>5-channel hybrid fused with RRF<a href="#user-content-fn-rrf"><sup>[1]</sup></a></td>
</tr>
<tr>
<td><strong>"Why" queries</strong></td>
<td>Grep through commits</td>
<td>Not possible</td>
<td>Causal graph with 6 relationship types</td>
</tr>
<tr>
<td><strong>Save quality</strong></td>
<td>Accept everything</td>
<td>Accept everything</td>
<td>3-layer gate: structure, semantic, duplicate</td>
</tr>
<tr>
<td><strong>Forgetting curve</strong></td>
<td>Everything treated equally</td>
<td>None</td>
<td>FSRS<a href="#user-content-fn-fsrs"><sup>[2]</sup></a> power-law decay by content type</td>
</tr>
<tr>
<td><strong>Query understanding</strong></td>
<td>Keyword match</td>
<td>Keyword match</td>
<td>Intent classification (7 types), complexity routing</td>
</tr>
<tr>
<td><strong>Agent coordination</strong></td>
<td>One generalist</td>
<td>One generalist</td>
<td>12 specialists with gate-driven routing</td>
</tr>
<tr>
<td><strong>Access control</strong></td>
<td>Filesystem perms</td>
<td>None</td>
<td>Shared memory spaces, deny-by-default</td>
</tr>
<tr>
<td><strong>Evaluation</strong></td>
<td>Manual testing</td>
<td>None</td>
<td>Ablation studies, 12-metric computation (MRR, NDCG)</td>
</tr>
</table>
...
flowchart TD
    ...
    subgraph B["GATE SYSTEM -- 3 mandatory gates"]
        B1["Gate 1: Context\nSurface relevant prior memory"]
        B2["Gate 2: Skills\nAuto-load domain expertise"]
        B3["Gate 3: Spec Folder (HARD BLOCK)\nEvery file change needs documentation"]
    end

    B --> C["AGENT NETWORK\n12 specialized agents\nwith routing logic"]
    B --> D["SKILLS LIBRARY\n18 domain skills\nauto-loaded by task keywords"]
    C <--> D

    C --> E
    D --> E

    subgraph E["MEMORY ENGINE -- 42 MCP tools"]
        E1["5-channel hybrid: Vector, BM25, FTS5, Causal Graph, Degree"]
        E2["FSRS decay -- RRF fusion -- Query intel -- PE gating"]
        E3["Voyage | OpenAI | HuggingFace Local"]
    end

    E --> F["SPEC KIT -- documentation framework\nspecs/###-feature/ -- memory/ -- scratch/\n4 levels -- 81 templates -- 20 rules"]
```
...
- <kbd>P0</kbd> Hard blocker. Cannot ship without this. Cannot defer.
- <kbd>P1</kbd> Required. Must complete or get explicit user approval to defer.
- <kbd>P2</kbd> Optional. Nice to have. Can defer without approval.
...
#### CORE + ADDENDUM Template Architecture <sup><sub>v2.2</sub></sup>
...
<details>
<summary><strong>Core Templates (4)</strong></summary>
...
<details>
<summary><strong>Addendum Layers (4)</strong></summary>
...
<details>
<summary><strong>Special Templates (4)</strong></summary>
...
For the full spec folder workflow, template architecture (81 templates) and validation rules, see the [Spec Kit README][spec-kit-readme].
...
The memory engine uses a 222-feature pipeline developed across a 19-phase refinement program. The full 33-tool API reference is in the [MCP Server README][mcp-server-readme].
...
- **Co-activation boost** <kbd>+0.25</kbd> -- Memories co-occurring with matched results get a lift. Fan-effect `1/sqrt(neighbors)` prevents hub bias.
- **FSRS decay** <kbd>multiplicative</kbd> -- Adjusts score by memory retrievability. Recently accessed memories score higher.
- **Interference penalty** <kbd>-0.08/neighbor</kbd> -- Suppresses near-identical memory clusters (>0.75 Jaccard similarity).
- **Cold-start boost** <kbd>+0.15 max</kbd> -- Fresh memories (<48h) get `0.15 * exp(-elapsed/12)`, 12h half-life, capped at 0.95.
- **Session recency** <kbd>cap 0.20</kbd> -- Memories accessed in the current session get a recency bump.
- **Causal 2-hop** <kbd>variable</kbd> -- Memories 1-2 hops from causal neighbors get a contextual boost.
- **Intent weights** <kbd>variable</kbd> -- Each of 7 task intents has its own channel weight profile.
- **Channel min-rep** <kbd>0.005</kbd> -- Floor ensures each active channel has at least one result.
...
- <kbd>quick</kbd> Top answer only, minimal context. For fast lookups and single-topic questions.
- <kbd>focused</kbd> Targeted results for one topic. For standard development queries.
- <kbd>deep</kbd> Full results with evidence trails. For research and architecture decisions.
- <kbd>resume</kbd> State summary + next-steps. For session recovery after crash or compaction.


#### 3.2.4 MEMORY LIFECYCLE AND SCORING

Memories fade using FSRS[^fsrs] (Free Spaced Repetition Scheduler), a model validated on 100M+ Anki flashcard users. The formula `R(t, S) = (1 + (19/81) x t/S)^(-0.5)` calculates a retrievability score where `t` is time since last access and `S` is a stability parameter.
...
<kbd>HOT</kbd> just used &raquo; <kbd>WARM</kbd> recently relevant &raquo; <kbd>COLD</kbd> not accessed lately &raquo; <kbd>DORMANT</kbd> inactive &raquo; <kbd>ARCHIVED</kbd> stored but rarely surfaced
...
- **`caused`** <kbd>1.0</kbd> -- A led directly to B
- **`enabled`** <kbd>0.75</kbd> -- A made B possible
- **`supersedes`** -- B replaces A
- **`contradicts`** -- A and B conflict
- **`derived_from`** -- B is based on A
- **`supports`** <kbd>0.5</kbd> -- A provides evidence for B
...
**Prediction Error gating**[^pe] compares new content against existing memories and picks one of four outcomes:

- <kbd>CREATE</kbd> No similar memory exists. Stored as new knowledge.
- <kbd>REINFORCE</kbd> Similar exists, new one adds value. Both kept, old one gets a confidence boost.
- <kbd>UPDATE</kbd> Similar exists, new one is better. Old version replaced in place.
- <kbd>SUPERSEDE</kbd> New knowledge contradicts the old. New version active, old one demoted to deprecated.
...
For the full shared memory guide, see [SHARED_MEMORY_DATABASE.md][shared-memory-guide].
...
For the complete 33-tool API reference (7 layers, 7,600 total token budget) and configuration, see the [MCP Server README][mcp-server-readme].
...
> [!IMPORTANT]
> Gate 3 is a **HARD BLOCK** -- no file changes without a spec folder answer. Use option D to skip for read-only exploration.
...
```mermaid
flowchart TD
    A["User message arrives"] --> G1

    G1["Gate 1 (SOFT)\nSurface prior memory, classify intent\nconfidence >= 0.70, uncertainty <= 0.35"] --> G2

    G2["Gate 2 (REQUIRED)\nskill_advisor.py recommends skill\nconfidence >= 0.8 → MUST load skill"] --> G3

    G3["Gate 3 (HARD BLOCK)\nOnly if file modification detected\nWhich spec folder? A / B / C / D / E"] --> EX
...
<dl>
  ...
  <dd>Is this the simplest solution? Are abstractions earned?</dd>
  <dt><strong>SYSTEMS</strong></dt>
  <dd>What does this touch? What are the side effects?</dd>
  <dt><strong>BIAS</strong></dt>
  <dd>Is the user solving a symptom? Is the framing correct?</dd>
  <dt><strong>SUSTAINABILITY</strong></dt>
  <dd>Will future developers understand this?</dd>
  <dt><strong>VALUE</strong></dt>
  <dd>Does this change behavior or just refactor?</dd>
  <dt><strong>SCOPE</strong></dt>
  <dd>Does solution complexity match problem size?</dd>
</dl>
...
For the complete flag reference with per-flag defaults, see [MCP Server README Section 5][mcp-server-readme].
...
- **New feature end-to-end** -- <kbd>/spec_kit:complete</kbd> Starting any feature from scratch.
- **Planning without building** -- <kbd>/spec_kit:plan</kbd> When you want to review the plan first.
- **Pick up previous work** -- <kbd>/spec_kit:resume</kbd> Returning to an in-progress spec.
- **Investigate a codebase** -- <kbd>@context</kbd> Exploration and research tasks.
- **Generate a README** -- <kbd>/create:folder_readme</kbd> Documenting a directory or component.
- **Improve a prompt** -- <kbd>/create:prompt</kbd> Prompt engineering with structured frameworks.
...
<details>
<summary><strong>Do I need all 18 skills installed to use the framework?</strong></summary>

No. Skills are loaded on demand by Gate 2. You only need the ones relevant to your work. The two core skills -- `system-spec-kit` and `sk-doc` -- cover most documentation workflows. The MCP and cross-AI CLI skills require additional API keys or tools.

</details>

<details>
<summary><strong>Is this only for OpenCode, or does it work with other runtimes?</strong></summary>

It works with OpenCode, Codex CLI, Claude Code and Gemini CLI. Agent definitions are mirrored across all four runtime directories. Each runtime has its own adapter files that translate the source-of-truth definitions into the format that runtime expects.

</details>

<details>
<summary><strong>What happens if I do not use a spec folder?</strong></summary>

Gate 3 blocks file modifications until a spec folder answer is provided. You can skip it with option D, but skipped sessions are undocumented and will not be recoverable via memory search. For trivial changes under 5 characters in a single file, Gate 3 does not trigger.

</details>

<details>
<summary><strong>How does the memory system know what is relevant to my current task?</strong></summary>

Every memory file has YAML frontmatter with tags, context type and trigger phrases. When you start a session, `memory_match_triggers()` runs a 5-channel hybrid search across all indexed memory files and returns the top matches. The system classifies your intent (7 types), routes by complexity and applies Reciprocal Rank Fusion to combine results from all channels.

</details>

<details>
<summary><strong>Can I use this framework without the cognitive memory features?</strong></summary>

Yes. The Spec Kit documentation workflow (Gate 3, spec folders, templates) works independently of the memory MCP server. You will not have cross-session memory retrieval, but you will still get structured documentation, agent routing and skill loading.

</details>

<details>
<summary><strong>How do I add a new skill to the framework?</strong></summary>

Use `/create:sk-skill` to scaffold the skill structure. The command creates the `SKILL.md`, references and assets directories following the `sk-doc` template. Then register the skill in `.opencode/skill/README.md`.

</details>

<details>
<summary><strong>What does "local-first" mean for the memory system?</strong></summary>

The memory database is a SQLite file on your local machine. No session data, code or context is sent to any external service unless you configure a cloud embedding provider (Voyage AI or OpenAI). HuggingFace Local embeddings run entirely on-device.

</details>

<details>
<summary><strong>How do I contribute a new agent definition?</strong></summary>

Define the agent in `.opencode/agent/` (the source of truth), then copy the adapter to `.claude/agents/`, `.codex/agents/` and `.gemini/agents/`. Use `/create:agent` to scaffold the file from the agent template.

</details>

<details>
<summary><strong>How many MCP tools are there and where are they defined?</strong></summary>

42 total across 4 native MCP servers: 33 memory tools (spec_kit_memory), 7 code mode tools, 1 semantic code search tool (cocoindex_code) and 1 sequential thinking tool. All server bindings are defined in `opencode.json`. The 33 memory tool definitions live in `.opencode/skill/system-spec-kit/mcp_server/tool-schemas.ts`.

</details>

<details>
<summary><strong>What is the feature catalog?</strong></summary>

The feature catalog is a 222-entry reference across 21 categories documenting every capability of the memory system. It comes in two versions: a technical reference and a simple-terms companion with plain-language explanations. Both are in `.opencode/skill/system-spec-kit/feature_catalog/`.
...
- **[Spec Kit README][spec-kit-readme]** -- Full spec folder workflow, template architecture (81 templates), validation rules and memory pipeline
- **[MCP Server README][mcp-server-readme]** -- Complete memory API reference (33 tools across 7 layers), retrieval architecture and configuration
- **[INSTALL_GUIDE.md][install-guide]** -- Step-by-step MCP server installation with embedding providers and environment setup
- **[SHARED_MEMORY_DATABASE.md][shared-memory-guide]** -- Shared memory guide with spaces, roles, membership and kill switch
- **[ARCHITECTURE.md][architecture]** -- API boundary contract between scripts/ and mcp_server/
- **[sk-doc SKILL.md][sk-doc]** -- Documentation standards, DQI scoring, templates and HVR writing rules
- **[Skills README][skills-readme]** -- Index of all 18 skills with descriptions and invocation patterns
- **[FEATURE_CATALOG.md][feature-catalog]** -- 222-entry technical reference across 21 categories
- **[FEATURE_CATALOG_IN_SIMPLE_TERMS.md][feature-catalog-simple]** -- Plain-language companion to the technical catalog
...
- **[OpenCode][opencode-url]** -- The underlying AI coding assistant platform this framework extends
...
[^rrf]: **Reciprocal Rank Fusion (RRF)** combines ranked results from multiple retrieval channels by assigning scores based on rank position rather than raw similarity scores. Formula: `1/(K + rank)` where K is a tuning constant.

[^fsrs]: **Free Spaced Repetition Scheduler (FSRS)** is a memory decay model validated on 100M+ Anki users that estimates when stored context becomes stale based on review history and content importance. Formula: `R(t,S) = (1 + (19/81) x t/S)^(-0.5)`.

[^pe]: **Prediction Error (PE) gating** is a neuroscience-inspired arbitration process that compares new content against existing memories to determine whether to create, reinforce, update, or supersede.

<!-- Reference-style links (centralized URL management) -->
[stars-shield]: https://img.shields.io/github/stars/MichelKerkmeester/opencode-spec-kit-framework?style=for-the-badge&logo=github&color=fce566&labelColor=222222
[stars-url]: https://github.com/MichelKerkmeester/opencode-spec-kit-framework/stargazers
[license-shield]: https://img.shields.io/github/license/MichelKerkmeester/opencode-spec-kit-framework?style=for-the-badge&color=7bd88f&labelColor=222222
[license-url]: LICENSE
[release-shield]: https://img.shields.io/github/v/release/MichelKerkmeester/opencode-spec-kit-framework?style=for-the-badge&color=5ad4e6&labelColor=222222
[release-url]: https://github.com/MichelKerkmeester/opencode-spec-kit-framework/releases
[opencode-url]: https://github.com/sst/opencode
[spec-kit-readme]: .opencode/skill/system-spec-kit/README.md
[mcp-server-readme]: .opencode/skill/system-spec-kit/mcp_server/README.md
[install-guide]: .opencode/skill/system-spec-kit/mcp_server/INSTALL_GUIDE.md
[shared-memory-guide]: .opencode/skill/system-spec-kit/SHARED_MEMORY_DATABASE.md
[architecture]: .opencode/skill/system-spec-kit/ARCHITECTURE.md
[sk-doc]: .opencode/skill/sk-doc/SKILL.md
[skills-readme]: .opencode/skill/README.md
[feature-catalog]: .opencode/skill/system-spec-kit/feature_catalog/FEATURE_CATALOG.md
[feature-catalog-simple]: .opencode/skill/system-spec-kit/feature_catalog/FEATURE_CATALOG_IN_SIMPLE_TERMS.md
