...
- ❌ Intent-based code discovery (use mcp-code-graph skill - **NATIVE MCP**, call `agentic_context()` directly)
...
- **Sequential Thinking**: `process_thought()`, `generate_summary()`
...
**Pairs with mcp-code-graph**:
- Use **mcp-code-graph** for intent-based code discovery (NATIVE MCP - call directly)
...
**Workflow**:
```typescript
// 1. Find relevant code using mcp-code-graph (NATIVE MCP - call directly)
agentic_context("authentication implementation");  // Direct call, NOT through Code Mode
...
- **[mcp-code-graph](../mcp-code-graph/SKILL.md)** - Intent-based code discovery (**NATIVE MCP** - call `agentic_context()` directly, NOT through Code Mode)
