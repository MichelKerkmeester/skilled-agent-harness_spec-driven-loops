...
```
.opencode/skills/system-memory/database/memory-index.sqlite
```
...
```bash
cd .opencode/skills/system-memory/mcp_server
npm install
```
...
node .opencode/skills/system-memory/mcp_server/semantic-memory.js
...
```json
{
  "mcpServers": {
    "semantic_memory": {
      "command": "node",
      "args": [
        "${workspaceFolder}/.opencode/skills/system-memory/mcp_server/semantic-memory.js"
      ],
      "env": {},
      "disabled": false
    }
...
`/Users/yourname/projects/my-project/.opencode/skills/system-memory/mcp_server/semantic-memory.js`
...
```json
{
  "mcp": {
    "semantic_memory": {
      ...
      "environment": {
        "_NOTE_DATABASE": "Stores vectors in: ${workspaceFolder}/.opencode/skills/system-memory/database/memory-index.sqlite",
...
1. **Portable**: Copy `.opencode/skills/system-memory/` to any project and it works
...
1. Copy the entire `.opencode/skills/system-memory/` folder
...
The default database path is `.opencode/skills/system-memory/database/memory-index.sqlite`. This can be overridden via environment variable:
...
```bash
sqlite3 .opencode/skills/system-memory/database/memory-index.sqlite "SELECT 'OK: ' || COUNT(*) || ' memories' FROM memory_index" 2>/dev/null || echo "Database not created yet (will be created on first save)"
```
...
ls -la .opencode/skills/system-memory/mcp_server/
# Should show: semantic-memory.js, lib/, package.json

ls -la .opencode/skills/system-memory/mcp_server/lib/
...
ls -la .opencode/skills/system-memory/mcp_server/node_modules/better-sqlite3
...
sqlite3 .opencode/skills/system-memory/database/memory-index.sqlite ".tables"
# Expected: memory_index vec_memories

# Count indexed memories
sqlite3 .opencode/skills/system-memory/database/memory-index.sqlite "SELECT COUNT(*) FROM memory_index"
```
...
| `importanceTier`   | string | No       | -       | Set importance tier (see 7.15) |
...
### 7.15 Six-Tier Importance System
...
```bash
# Check node_modules exists
ls -la .opencode/skills/system-memory/mcp_server/node_modules
...
ls .opencode/skills/system-memory/mcp_server/node_modules/sqlite-vec-darwin-arm64/  # macOS ARM
ls .opencode/skills/system-memory/mcp_server/node_modules/sqlite-vec-darwin-x64/    # macOS Intel
ls .opencode/skills/system-memory/mcp_server/node_modules/sqlite-vec-linux-x64/     # Linux x64
```
...
ls .opencode/skills/system-memory/database/memory-index.sqlite

# Verify embeddings exist
sqlite3 .opencode/skills/system-memory/database/memory-index.sqlite "SELECT COUNT(*) FROM vec_memories"
```
...
cd .opencode/skills/system-memory/mcp_server/scripts
...
sqlite3 .opencode/skills/system-memory/database/memory-index.sqlite "PRAGMA journal_mode"
# Should return: wal
```
...
node .opencode/skills/system-memory/mcp_server/semantic-memory.js --version 2>&1 | head -1
...
| ------------ | -------------------------------------------- | ----------------------- |
| Skill README | `.opencode/skills/system-memory/README.md`   | Skill overview          |
| SKILL.md     | `.opencode/skills/system-memory/SKILL.md`    | AI agent instructions   |
| References   | `.opencode/skills/system-memory/references/` | Technical documentation |
...
```json
{
  "mcpServers": {
    "semantic_memory": {
      "command": "node",
      "args": ["${workspaceFolder}/.opencode/skills/system-memory/mcp_server/semantic-memory.js"],
      "env": {},
      "disabled": false
    }
  }
...
```json
{
  "mcp": {
    "semantic_memory": {
      "type": "local",
      "command": ["node", "${workspaceFolder}/.opencode/skills/system-memory/mcp_server/semantic-memory.js"],
      "environment": {},
      "enabled": true
    }
  }
...
**Need help?** See [Troubleshooting](#8--troubleshooting) or check your AI client terminal for server logs.
