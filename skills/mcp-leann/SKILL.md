...
- ❌ Dependency analysis (use `codegraph` MCP - graph-aware relationships)
...
def route_leann_resources(task):
    ...
    if task.needs_installation or task.setup_required:
        return load("../../install_guides/MCP - LEANN.md")  # Full installation workflow
    ...
    # ../../install_guides/MCP - LEANN.md → Installation and setup guide
```
...
- Use **codegraph** for dependency analysis and architecture understanding (NATIVE MCP)
- Use **LEANN** for semantic code search by intent (NATIVE MCP)
- Example: LEANN finds auth code → codegraph shows what depends on it
...
# 3. Understand dependencies using codegraph
agentic_impact("src/auth/login.ts")  # What depends on this?
```
...
**Installation guide**: See `../../install_guides/MCP - LEANN.md`
...
- **[mcp-code-graph](../mcp-code-graph/SKILL.md)** - GraphRAG for dependency and architecture analysis (NATIVE MCP)
