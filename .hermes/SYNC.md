...
|---|---|---|
| `pre_llm_call` | `bin/skill-advisor.cjs`; `spec-gate/devin/spec-gate-classify.mjs` | Appends the skill-advisor brief (every turn, an orchestrated leaf included, because a leaf still chooses how to do its work) and classifies the prompt for the Gate 3 spec-folder gate. The classification is skipped for an orchestrated leaf, whose write authority is already bound to a lineage directory. The question itself never rides this channel: it reaches the model once, at the first mutation |
| `pre_tool_call` (every tool) | self-dispatch refusal; read-only refusal; `spec-gate/devin/spec-gate-enforce.mjs` (write tools under `SYSTEM_SPEC_GATE_ENFORCE=1`); `dispatch/devin/dispatch-preflight-lint.mjs`; `git-preflight/shared/git-preflight-advisory.mjs`; `mcp-route-guard/devin/mcp-route-guard.cjs`; `task-dispatch/devin/task-dispatch-guard.cjs`; `sk-vision/devin/sk-vision.mjs` | Blocks `hermes chat` from inside Hermes; blocks `write_file`, `patch`, `terminal` and code execution when `SPECKIT_HERMES_READ_ONLY=1`; blocks a `write_file` or `patch` the enforced spec gate has not been bound for yet, with the gate's own reason; blocks a `cli-*` dispatch that violates a blocking hard rule; blocks a `delegate_task` the task-dispatch guard rejects (every entry of its `tasks` array); stages the sk-git, MCP-route and vision advisories for the result |
| `transform_tool_result` | the Gate 3 notice for an un-enforced write; the staged advisories; `post-edit-quality/devin/post-edit-quality.cjs` | Appends the Gate 3 notice to the first write of a session whose gate is still open (the enforce adapter records its own delivery, so the next write stays silent), the staged sk-git, MCP-route and vision advisories, and the post-edit quality warning for `write_file` and `patch` (run here, not in `post_tool_call`, because both hooks run on bounded worker threads and a post-hook stage would race the transform) |
