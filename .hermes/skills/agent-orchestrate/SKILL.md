...
> Agent persona `orchestrate` from `.hermes/agents/orchestrate.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-orchestrate` and adopt it for the session.
