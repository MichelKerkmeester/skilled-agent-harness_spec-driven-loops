...
> Agent persona `deep-research` from `.hermes/agents/deep-research.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-deep-research` and adopt it for the session.
