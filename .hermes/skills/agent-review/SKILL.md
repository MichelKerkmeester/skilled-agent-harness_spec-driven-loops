...
> Agent persona `review` from `.hermes/agents/review.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-review` and adopt it for the session.
