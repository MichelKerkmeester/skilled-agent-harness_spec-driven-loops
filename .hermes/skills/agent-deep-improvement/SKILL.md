...
> Agent persona `deep-improvement` from `.hermes/agents/deep-improvement.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-deep-improvement` and adopt it for the session.
