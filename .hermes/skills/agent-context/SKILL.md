...
> Agent persona `context` from `.hermes/agents/context.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-context` and adopt it for the session.
