...
> Agent persona `design` from `.hermes/agents/design.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-design` and adopt it for the session.
