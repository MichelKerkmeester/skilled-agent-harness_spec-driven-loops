...
> Agent persona `code` from `.hermes/agents/code.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-code` and adopt it for the session.
