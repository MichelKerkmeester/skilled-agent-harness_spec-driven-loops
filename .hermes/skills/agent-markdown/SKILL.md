...
> Agent persona `markdown` from `.hermes/agents/markdown.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-markdown` and adopt it for the session.
