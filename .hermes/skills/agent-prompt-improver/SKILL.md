...
> Agent persona `prompt-improver` from `.hermes/agents/prompt-improver.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-prompt-improver` and adopt it for the session.
