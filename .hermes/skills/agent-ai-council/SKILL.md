...
> Agent persona `ai-council` from `.hermes/agents/ai-council.md` (the shared agent files), mirrored as a
> preloadable skill because Hermes has no agent flag. Preload with `-s agent-ai-council` and adopt it for the session.
