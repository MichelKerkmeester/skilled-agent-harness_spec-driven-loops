# Barter AI Assistant Framework

> **Behavior framework** for Barter's multi-stack ecosystem (Go, Angular, React Native, Python) — enforcing guardrails, code standards, and decision protocols across all repositories.

---

### Multi-Repository Architecture

**Universal Framework:** This AGENTS.md is shared across ALL Barter repositories. The same rules, gates, and workflows apply everywhere — `sk-code` detects the active Barter repo and loads its patterns, and `sk-git` enforces Barter's read-only Git Operations Policy.

**Supported Repositories:**

| Repository                 | Stack             | Detection Marker  | Key Patterns                                    |
| -------------------------- | ----------------- | ----------------- | ----------------------------------------------- |
| **backend-system**         | Go                | `go.mod`          | Domain layers, repositories, table-driven tests |
| **fe-partners-app**        | Angular           | `angular.json`    | Standalone components, signals, RxJS            |
| **barter-expo**            | React Native/Expo | `app.json` + expo | Hooks, navigation, native modules               |
| **gaia-services**          | Python/Flask      | `pyproject.toml`  | Blueprints, SQLAlchemy, pytest                  |
| **fe-backoffice-app**      | Angular           | `angular.json`    | Standalone components, signals, RxJS            |
| **bartender**              | Go                | `go.mod`          | API gateway, middleware, routing                |
| **postgres-backup-system** | DevOps            | `Makefile`        | Backup scripts, deployment                      |

**How It Works:**
1. `sk-code` detects stack via marker files at session start
2. Stack-specific patterns load from `.opencode/skills/sk-code/references/{repo}/`
3. Verification commands auto-adjust per stack (see Quick Reference below)
4. `sk-git` enforces the read-only Git Operations Policy on every git request
...
- **Git (sk-git) — READ-ONLY.** Never modify version control state.
  - **Allowed:** `git status`, `git log`, `git diff`, `git show`, `git branch` (list), `git blame`, `git reflog`, `git tag --list`, `git remote -v`
  - **Forbidden:** `git add`, `git commit`, `git push`, `git merge`, `git rebase`, `git checkout`, `git branch -c/-d/-m`, `git reset`, `git pull`
  - **If user requests git modifications:** REFUSE → Explain why → Provide exact commands for user to run manually
...
| **Application code**      | sk-code skill → smart router (auto-detects active Barter repo from marker files; loads `references/{repo}/` patterns for Go / Angular / React Native / Python / DevOps); Phase 1-3 (Implement → Quality Gate → Debug → Verify) |
| **Git analysis**          | sk-git skill → Read-only operations only → Enforces Git Operations Policy                                                          |
...
| DevOps        | Manual testing per `Makefile` targets | — | —       |
...
**If the user specifies the executor CLI** (e.g. "use cli-codex gpt-5.5 high"), that is the HOW — it still runs INSIDE the skill's workflow. Never let the executor name override the skill-owned route.
...
For ALL code implementation across Barter repositories, `sk-code` is the primary orchestrator skill.
...
**Auto-Detection Flow:**
```
Session Start → Check marker files → Detect stack → Load patterns
```

**Stack Patterns Location:** `.opencode/skills/sk-code/references/{repository}/`
- `backend-system/` → Go standards, domain layers, GORM patterns
- `fe-partners-app/` → Angular standards, component architecture, RxJS
- `fe-backoffice-app/` → Angular standards, standalone components, signals
- `bartender/` → Go standards, API gateway, middleware
- `barter-expo/` → React Native standards, hooks, navigation
- `gaia-services/` → Python standards, Flask patterns
- `postgres-backup-system/` → Deployment, architecture

**Invocation:** Automatic via Gate 2 routing when code tasks detected.

---

### Git Analysis Skill: sk-git

For ALL git repository analysis, `sk-git` enforces the strict read-only policy defined in §1 Git Operations Policy.

**Key Features:**
- **Read-only enforcement**: Only allows safe git commands (status, log, diff, show, blame, etc.)
- **Automatic blocking**: Prevents intrusive operations (add, commit, push, checkout, branch creation)
- **Policy alignment**: Directly enforces the Git Operations Policy from Section 1

**Trigger Keywords:** git history, git log, branch analysis, commit history, git diff, repository analysis, git blame, git status

**When to Use:**
- Analyzing commit history or branch structure
- Investigating changes between commits/branches
- Understanding repository state
- Generating reports from git data
...
**Invocation:** Automatic via Gate 2 routing when git analysis tasks detected, or manually via `Read(".opencode/skills/sk-git/SKILL.md")`.