...
export const EMBEDDING_MODEL_CATALOG = {
  ...
  // Ollama entries carry both names on purpose. `model` is the catalog slug,
  // which must stay equal to the reference suffix because an index manifest
  // stores `provider` and `model` and rebuilds the reference from them. `tag`
  // is the name the Ollama server serves, which may not be a legal slug.
  //
  // `maxInputTokens` is the context length the Ollama build pins for the tag,
  // not the upstream model card figure. The server silently truncates longer
  // input without reporting it, so a larger limit would lose content.
  "ollama/nomic-embed-text-v1.5": {
    backend: "ollama",
    reference: "ollama/nomic-embed-text-v1.5",
    provider: "ollama",
    model: "nomic-embed-text-v1.5",
  ...
  "ollama/mxbai-embed-large": {
    backend: "ollama",
    reference: "ollama/mxbai-embed-large",
    provider: "ollama",
  ...
  // Batch size stays lower here because batch size times context length is the
  // server's working set for one request, and this model has both a long
  // context and the most parameters of the three.
  "ollama/bge-m3": {
    backend: "ollama",
    reference: "ollama/bge-m3",
    provider: "ollama",
    model: "bge-m3",
    tag: "bge-m3:latest",
    dimension: 1024,
...
export type OllamaEmbeddingCatalogEntry = Extract<
  EmbeddingCatalogEntry,
  { backend: "ollama" }
>;
