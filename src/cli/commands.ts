...
async function runServer(parsed: ParsedArgs): Promise<void> {
  ...
  if (parsed.options.serverStdio) {
    const { resolveStdioServerMode, runDirectStdioServer } =
      await import("../mcp/stdio-direct.js");
    if (resolveStdioServerMode(parsed.options.mode) === "direct") {
      if (parsed.options.listen !== undefined) {
        throw new Error(
          "zg server --stdio --mode direct does not start a listener; drop --listen",
        );
      }
      await runDirectStdioServer({
...
async function runDirectQuery(
  ...
  try {
    ...
    // Index status is computed by walking the whole workspace, which costs far
    // more than the search itself on a large tree. Its only use here is the
    // stale-index hint below, so only pay for it when the caller asked to wait
    // for a refresh -- that path walks the tree anyway. `zg status` remains the
    // way to inspect staleness on demand.
    const includeStatus = resolveDirectSearchPolicy(commandOptions).autoUpdate;
    const info = await directQueryInfo(zvecGrep, includeStatus);
