...
type InstallAgentOptions = {
  ...
  /** Written into stdio launch arguments; `direct` runs MCP without a daemon. */
...
export async function runInstall(parsed: ParsedArgs): Promise<void> {
  ...
  // Direct mode only reaches the launch arguments through stdio; an HTTP entry
  // always talks to the daemon.
  const mode = transport === "stdio" ? parsed.options.mode : undefined;
  ...
  if (mode === "direct") {
    ...
    console.log("    stdio in direct mode; no server is started or required");
  } else {
...
async function installCursorIntegration(
  ...
  await installJsonMcpServer({
    ...
    server:
      options.transport === "stdio"
        ? { command: "zg", args: stdioArgs(options) }
        : {
            url: resolveServerUrl(),
            ...(options.mcpTokenEnv
              ? {
...
function qoderIdeMcpServer(
  ...
  const timeout = options.mcpToolTimeoutSeconds * 1_000;
  if (options.transport === "stdio") {
    const launch = stableQoderIdeStdioLaunch(options);
    return {
...
function isManagedJsonMcpServer(value: unknown): boolean {
  ...
  return (
    ...
    value.command[0] === "zg" &&
    isStdioArgs(value.command.slice(1))
  );
}
...
function isStdioArgs(value: unknown): boolean {
  return Array.isArray(value) && isStdioArgList(value);
}
...
function isStableStdioArgs(value: unknown): boolean {
  return (
    ...
    isStdioArgList(value.slice(1))
  );
}

/**
 * Recognizes `server --stdio` plus the optional managed flags in any order, so
 * an entry written with a newer flag is still detected as zg-managed by an
 * older or newer install run.
 */
function isStdioArgList(value: readonly unknown[]): boolean {
  if (value[0] !== "server" || value[1] !== "--stdio") return false;
  const flags = value.slice(2);
  if (flags.length % 2 !== 0) return false;
  for (let index = 0; index < flags.length; index += 2) {
    const name = flags[index];
    const flagValue = flags[index + 1];
    if (name === "--mcp-toolset") {
      if (flagValue !== "agent" && flagValue !== "full") return false;
      continue;
    }
    if (name === "--mode") {
      if (
...
type StdioLaunchOptions = {
  mcpToolset?: McpToolset;
  mode?: ZvecGrepClientMode;
};

function stdioArgs(options: StdioLaunchOptions = {}): string[] {
  return [
    "server",
    "--stdio",
    ...(options.mcpToolset ? ["--mcp-toolset", options.mcpToolset] : []),
    ...(options.mode ? ["--mode", options.mode] : []),
  ];
}

function stdioCommand(options: StdioLaunchOptions = {}): string[] {
  return ["zg", ...stdioArgs(options)];
}

function stableQoderIdeStdioLaunch(options: StdioLaunchOptions = {}): {
  command: string;
  args: string[];
} {
  const cliPath = process.argv[1];
  if (!cliPath) {
    return { command: "zg", args: stdioArgs(options) };
  }
  return {
    command: process.execPath,
    args: [resolve(cliPath), ...stdioArgs(options)],
  };
}
...
${
  options.transport === "stdio"
    ? `command = "zg"
args = ${tomlStringArray(stdioArgs(options))}`
    : `url = "${resolveServerUrl()}"`
}
${
  options.mcpTokenEnv
