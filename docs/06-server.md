...
- an Agent connects through MCP and can keep a daemon running;
...
An Agent that must not run a long-lived process can serve MCP from its own
child process instead:

```bash
zg server --stdio --mode direct
```

`zg server --stdio` on its own keeps the existing behavior: it starts or reuses
the shared daemon and proxies MCP over stdin/stdout. Adding `--mode direct`
starts nothing, proxies nothing, and answers every request against an
in-process engine. `ZVEC_GREP_MODE=direct` selects the same behavior; any other
mode keeps the daemon bootstrap, and an explicit `--mode` wins over the
environment.

The tool contract is unchanged. Direct mode reuses the same schemas, input
normalization, and result formatting as the daemon-backed server, so the same
query against the same index returns the same tool output. Only the execution
backend differs.

| Concern | Daemon-backed stdio | Direct stdio |
| --- | --- | --- |
| Process | Shared daemon, outlives the client | One child process, exits with the client |
| Index freshness | File watcher plus background reconciliation | The caller's job; nothing refreshes in the background |
| `freshness` in a response | Tracked runtime state, with `background_refresh` when an update is active | Status scan per search; never reports `background_refresh` |
| `wait_for_fresh` | Waits for the daemon's update | Updates the index inside the call |
| `autoUpdate` | May schedule a background update | Ignored; no background worker exists |
| Embedding model | Pooled and shared across clients | Loaded lazily on the first vector search, then kept for the life of the process |
| Concurrent clients | Share one runtime and one model | Each client pays its own model load |
| `full` toolset | Six tools | Five tools; `zvec_grep_server_status` has no daemon to report |

Keep the index current from outside the Agent, for example by running
`zg index` after a substantial change, or ask for `freshness: "wait_for_fresh"`
when the latest file state must be reflected. A stale index still answers; it
reports `freshness: possibly_stale` so the caller can decide.

Direct mode writes to the same Workspace index as everything else. While a
daemon holds a Workspace lease, direct indexing of that Workspace is refused
rather than allowed to corrupt it; searches remain available.
...
A Direct search reports no staleness of its own. Detecting drift requires a
workspace-wide scan, which Direct mode runs only under `--refresh wait`, so
`--refresh off` and the Direct default print no `possibly_stale` hint. Run
`zg status` when a Direct caller needs to know whether the index is current.
