...
In Direct mode a query does not scan the workspace unless `--refresh wait` is
given, so it prints no `status: possibly_stale` hint. Skipping that scan is what
keeps a direct query proportional to the search rather than to the size of the
workspace. Use [`zg status`](#zg-status) to check whether the index is stale, or
`--refresh wait` to update it as part of the query.
...
| `ZVEC_GREP_OLLAMA_URL` | Ollama server base URL, default `http://127.0.0.1:11434` |
| `OLLAMA_BASE_URL` | Ollama base-URL fallback after `ZVEC_GREP_OLLAMA_URL` |
