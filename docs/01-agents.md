...
For an agent that must not leave a background process running, add `--mode
direct` to a stdio install:

```bash
zg install --target claude --mcp-transport stdio --mode direct --yes
```

The managed entry then launches `zg server --stdio --mode direct`, which serves
the same tools from the agent's own child process and starts no daemon. The
flag applies to stdio only, so an HTTP entry ignores it, and omitting it keeps
the default daemon-backed launch. Index refresh becomes the caller's job in
direct mode; see
[Direct-mode stdio](./06-server.md#direct-mode-stdio) for the trade-offs.
...
A direct-mode stdio install has no server to check. Confirm the workspace index
instead, and keep it current yourself, because direct mode has no watcher and
