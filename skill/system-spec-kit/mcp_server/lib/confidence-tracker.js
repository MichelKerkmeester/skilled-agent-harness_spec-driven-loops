 ...
 * Enables automatic promotion to critical tier for highly validated memories.
 *
 * Confidence Formula:
 * - Base confidence: 0.5
...
function recordValidation(db, memoryId, wasUseful) {
  ...
  if (promotionEligible) {
    wasPromoted = promoteToCritical(db, memoryId);
  }
 ...
 */
function promoteToCritical(db, memoryId) {
  // Verify eligibility
