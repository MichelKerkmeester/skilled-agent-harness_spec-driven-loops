...
| **Open Design dispatch** | UI or design work through `design-mcp-open-design` (a nested transport packet of `sk-design`) MUST co-load `sk-design`'s own workflow modes first (the transport never decides taste). Pure transport is exempt. `mcp-figma` is the external sibling Figma transport. |
...
| **Owner-first branch grammar** | New worktree branches use `{owner}/{NNNN}-{slug}` (e.g. `sk-git/0041-fix-thing`) with the matching directory `.worktrees/{NNNN}-{owner}-{slug}`. Legacy `wt/{NNNN}-{slug}` branches are permitted-but-non-conformant — migrate by renaming, never by rewriting history. `work/{runtime}/{slug}` is the separate, unnumbered, auto-managed launch-wrapper lane and is not owner-scoped. |
| **Allocate, never count** | The `{NNNN}` counter is a clone-wide value held by `.opencode/skills/sk-git/scripts/worktree-naming.sh` under its own lock (`create <owner> <slug> [base]` or `allocate`). Never hand-count existing worktrees or branches to guess the next number. |
...
| **UI / design work**              | `sk-design` (design judgment, required) → `design-mcp-open-design` (nested) or `mcp-figma` (external) as transport → hand the build to `sk-code`                                                              |                                                                                                                                                                          |
