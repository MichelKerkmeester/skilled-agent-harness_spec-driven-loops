...
| **Branch grammar** | Worktree-backed branches use `worktrees/{NNN}-{slug}` (e.g. `worktrees/0041-fix-thing`) with the matching directory `.worktrees/{NNN}-{slug}`; dedicated branches with no worktree use `branches/{NNN}-{slug}` (e.g. `branches/0003-external-dep`). Each namespace numbers independently `001..999`, strictly sequential, never skipped or reused. `skilled/v*` release branches and `main` are reserved; `backup/<anything>` safety refs are legal but not numbered; `work/{runtime}/{slug}` is the separate, unnumbered, auto-managed launch-wrapper lane. |
| **Allocate, never count** | Each namespace's counter (`worktrees/` and `branches/`) is held by `.opencode/skills/sk-git/scripts/worktree-naming.sh` under its own lock (`create <slug> [base]`, `create-branch <slug> [base]`, or `allocate [worktrees|branches]`). Never hand-count existing worktrees or branches to guess the next number. |
| **No direct branch creation** | Never create a branch with `git branch`, `git checkout -b`, or `git switch -c`. Worktree branches are created only via `git worktree add -b ...`; a dedicated branch without a worktree is created only via the allocator's `create-branch` command. |
...
| **Live-sync auto-publish, startup reconcile, and auto-follow** | In the main checkout only, commits auto-publish to the live branch, SessionStart safely reconciles clean local drift, and the checkout auto-follows it. Disable the whole loop with `MK_LIVE_SYNC_DISABLED=1`, reconciliation with `MK_PRIMARY_RECONCILE_DISABLED=1`, the publish leg with `SPECKIT_AUTOSYNC=0`, or the follower with `MK_LIVE_FOLLOW_DISABLED=1`. Model: `.opencode/skills/sk-git/references/continuous-integration.md`. |
